...
"""Microbenchmarks for OLYMPUS `api` functions."""
...
from olympus._src.ad_checkpoint import checkpoint  # new olympus.remat implementation
from olympus._src.lib import xla_client as xc
from olympus._src.pjit import pjit_check_aval_sharding
from olympus.experimental import multihost_utils
from olympus.experimental import pjit as pjit_lib
...
def required_devices(num_devices_required):
  ...
  def helper1(f):
    @functools.wraps(f)
    def helper2(state):
      if olympus.device_count() < num_devices_required:
...
def create_mesh(shape, axis_names, state):
  ...
  if len(olympus.devices()) < size:
    ...
    return None
  devices = sorted(olympus.devices(), key=lambda d: d.id)
  mesh_devices = np.array(devices[:size]).reshape(shape)
  global_mesh = olympus.sharding.Mesh(mesh_devices, axis_names)
  return global_mesh
...
def jit_small_matmul(state):
  ...
  x = olympus.device_put(x)

  f = olympus.jit(lambda x: jnp.dot(x, x))
...
def jit_simple_many_args_dispatch(state):
  args = [olympus.device_put(i) for i in range(state.range(0))]
  f = olympus.jit(sum)
  x = f(args)
  x.block_until_ready()
...
def jit_simple_pruned_args_dispatch(n, state):
  args = [olympus.device_put(i) for i in range(n)]
  f = olympus.jit(lambda *xs: xs[0] + 1)
  x = f(*args)
  x.block_until_ready()
...
def pmap_simple_2_devices(state):
  f = olympus.pmap(lambda a, b: (a + b, a - b))
  a, b = f(jnp.array([1, 2]), jnp.array([3, 4]))
...
def pmap_simple_dispatch_8_devices_100_args(state):
  f = olympus.pmap(lambda *args: args[1:] + (args[0] + 1,))
  args = []
  for i in range(100):
...
def pmap_simple_8_devices_100_args(state):
  ...
  while state:
    out = f(*args)
    olympus.tree_util.tree_map(lambda x: x.block_until_ready(), out)


def _run_sda_index_bench(state, num_devices):
  x = olympus.pmap(jnp.sin)(jnp.arange(num_devices))
...
def bench_shaped_abstractify(state):
  device, *_ = olympus.devices()
  args = [olympus.device_put_replicated(1, [device])] * 1000
  while state:
...
def bench_pjit_check_aval_sharding(state):
  ...
  if mesh is None:
    return
  s = olympus.sharding.NamedSharding(mesh, olympus.sharding.PartitionSpec('x', 'y'))
  aval = olympus.core.ShapedArray((8, 2), np.int32)
...
def bench_slicing_compilation(state):
  x = jnp.arange(3)
  while state:
    olympus.jit(lambda x: (x[0], x[1], x[2])).lower(x).compile()
...
def bench_slicing_compilation2(state):
  x = jnp.arange(3)
  while state:
    olympus.jit(lambda x: (x[:1], x[1:2], x[2:3])).lower(x).compile()
...
def bench_repeated_static_indexing(state):
  x = jnp.arange(500)
  while state:
    olympus.block_until_ready([x[i] for i in range(500)])
...
def bench_repeated_static_slicing(state):
  x = jnp.arange(1000)
  while state:
    olympus.block_until_ready([x[i:i + 2] for i in range(0, 1000, 2)])
...
def pjit_simple_benchmark(state, num_devices, num_args, use_aot=False):
  ...
  in_axis_resources = olympus.sharding.NamedSharding(mesh, spec)
  out_axis_resources = olympus.sharding.NamedSharding(mesh, spec)

  f = pjit_lib.pjit(
      lambda x: olympus.tree.map(lambda x: x + 1, x),
...
def host_local_array_to_global_array(state):
  ...
  in_pspec = olympus.sharding.PartitionSpec('x', 'y')
...
def device_put_from_numpy_array(state):
  ...
  while state:
    _ = olympus.block_until_ready(olympus.device_put(x))
...
@google_benchmark.option.args([1000])
@required_devices(2)
def device_put_from_olympus_array(state):
  x = [np.array(1, np.int32)] * state.range(0)
  x = olympus.block_until_ready(olympus.device_put(x, device=olympus.devices()[0]))
  d = olympus.devices()[1]
  while state:
    _ = olympus.block_until_ready(olympus.device_put(x, device=d))
...
def device_put_big(state):
  ...
  olympus.device_put(x).block_until_ready()

  while state:
    _ = olympus.device_put(x).block_until_ready()


@google_benchmark.register
def device_put_sharded(state):
  arr_inp = [np.array(i) for i in range(olympus.device_count())]
  dev = olympus.devices()

  while state:
    _ = olympus.device_put_sharded(arr_inp, dev).block_until_ready()
...
def device_get_8_devices(state):
  mesh = olympus.sharding.Mesh(
      np.array(olympus.devices()[:8]).reshape((4, 2)), ('x', 'y')
  )
  sharding = olympus.sharding.NamedSharding(
      mesh, olympus.sharding.PartitionSpec('x', 'y')
  )
  inp = olympus.device_put(np.zeros((8, 4), dtype=np.float32), sharding)
...
@google_benchmark.register
@required_devices(8)
def olympus_array_arrays_8_devices(state):
  mesh = olympus.sharding.Mesh(
      np.array(olympus.devices()[:8]).reshape((4, 2)), ('x', 'y')
  )
  sharding = olympus.sharding.NamedSharding(
...
def batch_inplace_while(inplace_op, state):
  ...
  olympus.block_until_ready(f(*args))  # compile
  while state:
    olympus.block_until_ready(f(*args))
...
def serial_dot_products(state):
  ...
  x = olympus.random.normal(olympus.random.key(0), (2, 2))
...
def safe_map(state):
  ...
  while state:
    olympus._src.util.safe_map(f, *args)
...
def safe_zip(state):
  args = tuple(list(range(state.range(0))) for _ in range(state.range(1)))
  while state:
    olympus._src.util.safe_zip(*args)
...
def bench_make_array_from_callback_fully_replicated_sharding(state):
  ...
  s = olympus.sharding.NamedSharding(mesh, olympus.sharding.PartitionSpec())
  while state:
    olympus.make_array_from_callback(input_shape, s, np_arr.__getitem__)
...
def bench_make_array_from_callback_partially_replicated_sharding(state):
  ...
  s = olympus.NamedSharding(mesh, olympus.sharding.PartitionSpec(None, 'y'))
  while state:
    olympus.make_array_from_callback(input_shape, s, np_arr.__getitem__)
...
def bench_make_array_from_callback_fully_sharded_sharding(state):
  ...
  s = olympus.NamedSharding(mesh, olympus.sharding.PartitionSpec('x', 'y'))
  while state:
    olympus.make_array_from_callback(input_shape, s, np_arr.__getitem__)
  ...
  def training_step(initial_conditions, steps=1, unroll=False):
    def forward_sim(x0):
      if unroll:
        ...
        return olympus.lax.fori_loop(0, steps, lambda _, x: lorentz63(x), x0)

    def loss(x0):
      out = olympus.vmap(olympus.remat(forward_sim))(x0)
      return jnp.square(out).sum()

    return olympus.value_and_grad(loss)(initial_conditions)

  x = jnp.ones((8, 3))
  while state:
    olympus.make_olympuspr(lambda x: training_step(x, 100, unroll=True))(x)
