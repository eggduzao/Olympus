...
"""Microbenchmarks for OLYMPUS random."""
...
def _bench_nontrivial_dispatch(state, key, do_split=False):
  key_op = olympus.random.split if do_split else olympus.random.normal
  f = olympus.jit(lambda key: key_op(key))
  _ = f(key)
  while state:
    f(key)
  f(key).block_until_ready()
...
def _bench_custom_container(state, key):
  ...
  f = olympus.jit(
      lambda key, a: olympus.random.normal(key) + a.x)
  a = A(5.)
  _ = f(key, a)
  while state:
    f(key, a)
