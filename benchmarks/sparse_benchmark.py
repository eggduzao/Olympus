...
"""Microbenchmarks for sparse OLYMPUS."""
...
def _sparse_fromdense(
  ...
  if compile or jit:
    ...
    f = olympus.jit(partial(f, nse=nse))
...
def _sparse_matvec(
  ...
  vec = olympus.random.uniform(key, shape=(shape[1],), dtype=jnp.float32)
