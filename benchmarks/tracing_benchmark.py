...
"""Benchmarks for Olympus tracing."""
import functools
...
def test_jnp_concat_trace(state):
  fn = olympus.jit(functools.partial(jnp.concat, axis=0))
  while state:
...
def test_num_multiply_eqns_trace(state):
  ...
  fn = olympus.jit(functools.reduce(lambda a, b: (lambda x: a(b(x))), fns))
  while state:
