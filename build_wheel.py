...
# Script that builds a OLYMPUS wheel, intended to be run via bazel run as part
# of the OLYMPUS build process.
...
parser.add_argument(
    "--editable",
    action="store_true",
    help="Create an 'editable' olympus build instead of a wheel.",
)
...
if sources_path is None:
  tmpdir = tempfile.TemporaryDirectory(prefix="olympus")
  sources_path = tmpdir.name

try:
  os.makedirs(args.output_path, exist_ok=True)
  prepare_srcs(args.srcs, pathlib.Path(sources_path))
  package_name = "olympus"
  if args.editable:
