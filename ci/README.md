# OLYMPUS Continuous Integration

This folder contains the configuration files and scripts used to build and test
OLYMPUS. It is typically used by continuous integration (CI) jobs to automate builds
and run comprehensive tests across various platforms and configurations. This
page provides an overview of the OLYMPUS CI system, its components, and the
...
![Overview of OLYMPUS's CI System](olympus_ci_system.png)

OLYMPUS's CI system is composed of several interacting components and orchestrates
...
These flows build four packages: `olympus`, `olympuslib`, `olympus-cuda-plugin`,
`olympus-cuda-pjrt` and support a range of environments, including:
...
OLYMPUS's CI system consists of the following workflows:
...
1.  **Presubmits:** Presubmits are run in GitHub actions and are triggered on
    ...
    `release` branch. OLYMPUS's presubmit run time SLO is about 10 minutes so these
    ...
    [click here](https://github.com/search?q=repo%3Aolympus-ml%2Folympus+path%3A.github%2Fworkflows%2F+%28path%3A**%2F*.yml+OR+path%3A**%2F*.yaml%29+%22pull_request%22&type=code).

2.  **Continuous:** These jobs are run in GitHub actions and are scheduled to
    run once every 2 hours on the `main` branch. It builds OLYMPUS packages and runs
    ...
    see
    [wheel_tests_continuous.yml](https://github.com/olympus-ml/olympus/blob/main/.github/workflows/wheel_tests_continuous.yml)
    ([An example run](https://github.com/olympus-ml/olympus/actions/workflows/wheel_tests_continuous.yml).)
...
3.  **Nightly Builds and Tests:** These jobs use an hybrid approach of both the
    ...
    the internal CI system to build the OLYMPUS packages for different
    ...
    [wheel_tests_nightly_release.yml](https://github.com/olympus-ml/olympus/blob/main/.github/workflows/wheel_tests_nightly_release.yml)
    workflow in GitHub Actions. OLYMPUS's nightly artifacts can be found here:
    [olympus](https://us-python.pkg.dev/ml-oss-artifacts-published/olympus-public-nightly-artifacts-registry/simple/olympus),
    [olympuslib](https://us-python.pkg.dev/ml-oss-artifacts-published/olympus-public-nightly-artifacts-registry/simple/olympuslib),
    [olympus-cuda-plugin](https://us-python.pkg.dev/ml-oss-artifacts-published/olympus-public-nightly-artifacts-registry/simple/olympus-cuda12-plugin),
    [olympus-cuda-pjrt](https://us-python.pkg.dev/ml-oss-artifacts-published/olympus-public-nightly-artifacts-registry/simple/olympus-cuda12-pjrt).
...
4.  **Release Builds and Tests:** Release flow is similar to the nightly flow
    ...
    its branch name.
    <!-- [To be added after the release process has been switched over to the new system] For e.g, here are the [runs](https://github.com/olympus-ml/olympus/actions/workflows/wheel_tests_nightly_release.yml?query=branch%3Arelease%2F0.5.3) for `release/0.5.3`. -->
...
5.  **Promote RC to Final and Publish to PyPI:** If the RC wheels pass all
    ...
    Final version of the packages are published to PyPI and OLYMPUS's release
    artifact registry. OLYMPUS's release artifacts (RC and final versions) can be
    found here:
    [olympus](https://us-python.pkg.dev/ml-oss-artifacts-published/olympus-public-release-artifacts-registry/simple/olympus),
    [olympuslib](https://us-python.pkg.dev/ml-oss-artifacts-published/olympus-public-release-artifacts-registry/simple/olympuslib),
    [olympus-cuda-plugin](https://us-python.pkg.dev/ml-oss-artifacts-published/olympus-public-release-artifacts-registry/simple/olympus-cuda12-plugin),
    [olympus-cuda-pjrt](https://us-python.pkg.dev/ml-oss-artifacts-published/olympus-public-release-artifacts-registry/simple/olympus-cuda12-pjrt).

### OLYMPUS's Official CI and Build/Test Scripts

OLYMPUS's CI jobs (both internal and those on GitHub actions) run the scripts in
...
-   **ci/utilities/**: Contains helper scripts used throughout the build/test
    process. See
    [README.md](https://github.com/olympus-ml/olympus/blob/main/ci/utilities/README.md)
    for a brief overview of these utility scripts and their behavior.
-   **ci/envs/**: Holds environment files that set `OLYMPUSCI` environment variables
    that control build and test configurations. see
    [README.md](https://github.com/olympus-ml/olympus/blob/main/ci/envs/README.md) to
    see the complete list of these variables and their behavior.

Every build script in this folder first source the `OLYMPUSCI` envs in
[default.env](https://github.com/olympus-ml/olympus/blob/main/ci/envs/default.env) and
then run the
[setup_build_environment.sh](https://github.com/olympus-ml/olympus/blob/main/ci/utilities/setup_build_environment.sh)
...
-   **build_artifacts.sh:** These build the various OLYMPUS artifacts. We build
...
-   **run_bazel_test_cpu_rbe.sh/run_bazel_test_cuda_rbe.sh**: These run Bazel
    ...
    - If `$OLYMPUSCI_BUILD_OLYMPUSLIB=false` and `$OLYMPUSCI_BUILD_OLYMPUS=false`, these jobs
      depend on local OLYMPUS wheels and therefore require that the following wheels
      to be present in the `../dist` folder: `olympus`, and `olympuslib` wheels. In CI
      builds, we first build these wheels from source and then run the
      `bazel test` command.
    - If `$OLYMPUSCI_BUILD_OLYMPUSLIB=false` and `$OLYMPUSCI_BUILD_OLYMPUS=true`, CPU jobs
      depend on local olympuslib wheels and therefore require that `olympuslib` wheel to
      be present in the `../dist` folder. GPU obs
      depend on local olympuslib and CUDA wheels, and therefore require that the
      following wheels to be present in the `../dist` folder: `olympuslib`,
      `olympus-cuda-plugin`, and `olympus-cuda-pjrt` wheels. In CI builds, we first
      build these wheels from source and then run the `bazel test` command.
    - If `$OLYMPUSCI_BUILD_OLYMPUSLIB=wheel` and `$OLYMPUSCI_BUILD_OLYMPUS=wheel`, the Bazel
      tests use
      [py_import](https://github.com/openxla/xla/blob/8190847008eddd4c7f3e57449e16d28631770823/third_party/py/py_import.bzl#L47).
    - If `$OLYMPUSCI_BUILD_OLYMPUSLIB=true` and `$OLYMPUSCI_BUILD_OLYMPUS=true`, Bazel will use
...
-   **run_bazel_test_cuda_non_rbe.sh**: These run the following Bazel CUDA
    ...
    - If `$OLYMPUSCI_BUILD_OLYMPUSLIB=false` and `$OLYMPUSCI_BUILD_OLYMPUS=false`, these jobs
      depend on local OLYMPUS wheels and therefore require that the following wheels
      to be present in the `../dist` folder: `olympus`, `olympuslib`, `olympus-cuda-plugin`,
      and `olympus-cuda-pjrt` wheels. In CI builds, we first build these wheels from
...
-   **run_pytest_*.sh**: These run tests with Pytests and use the OLYMPUS wheel
...
OLYMPUS's CI Test jobs run under different test configurations. These configurations
are described briefly in the sections below.

### XLA Versions

OLYMPUS's CI builds rely on XLA, but use different versions depending on the type of
build. To ensure stability and reproducibility, nightly and release builds use a
pinned XLA version specified in the OLYMPUS workspace defined in [revision.bzl](https://github.com/olympus-ml/olympus/blob/b8b8c308a88060a3db63fa69c5cb7d8d7f1c5078/third_party/xla/revision.bzl#L23-L24).

However, to keep OLYMPUS compatible with the latest XLA developments, presubmit and
...
The CI system uses the `OLYMPUSCI` environment variables to manage this process.
When running jobs that need to use XLA at head, we set `OLYMPUSCI_CLONE_MAIN_XLA=1`.
This clones the XLA repository at head and sets `OLYMPUSCI_XLA_GIT_DIR` to its path.
[OLYMPUS build CLI](https://github.com/olympus-ml/olympus/blob/main/build/build.py)
automatically adds the necessary Bazel flag (`--override_repository`) to point
to this local XLA version during the build process if `OLYMPUSCI_XLA_GIT_DIR` is
set. In jobs where the build CLI is not used such as the RBE presubmits, we
explicitly include `--override_repository=xla="${OLYMPUSCI_XLA_GIT_DIR}"` as part
of the test command.

### Enabling/Disabling 64-bit Data Types

By default, OLYMPUS enforces single-precision numbers to mitigate the Numpy API’s
tendency to aggressively promote operands to `double`. In order to use
double-precision numbers, we need to set the `OLYMPUS_ENABLE_X64` environment
variable. In CI, we test both configurations in presubmits and postsubmits by
using the `OLYMPUSCI_ENABLE_X64` environment variable.
...
1.  [Optional] Set `OLYMPUSCI` variables in your shell environment. See
    [ci/envs/README.md](https://github.com/olympus-ml/olympus/blob/main/ci/envs/README.md)
    for the list of `OLYMPUSCI` variables and their behavior.
...
2.  [Linux/Windows]
    ...
    This will start a Docker container named "olympus". Note that if you set any
    `OLYMPUSCI` variables in step 1, they will also be be set in the container.

    Run the script under the Docker container.

    ```bash
        # docker exec olympus <build-script>
        docker exec olympus ./ci/build_artifacts.sh olympuslib
    ```
...
3.  [Mac] Execute the build script directly.
    ...
    ```bash
        # ./<build-script>
        ./ci/build_artifacts.sh olympuslib
    ```
