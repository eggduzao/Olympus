...
BZLMOD_CONFIG=""
if [[ "${OLYMPUSCI_ENABLE_BZLMOD:-0}" == "1" ]]; then
...
if [[ "$OLYMPUSCI_BAZEL_CPU_RBE_MODE" == 'build' ]]; then
...
if [[ $os == "darwin" ]] || ( [[ $os == "linux" ]] && [[ $arch == "aarch64" ]] ); then
    ...
    if [[ "$OLYMPUSCI_BAZEL_CPU_RBE_MODE" == 'test' ]]; then
...
bazel $bazel_output_base $OLYMPUSCI_BAZEL_CPU_RBE_MODE \
    $BZLMOD_CONFIG \
    ...
    $test_strategy \
    --test_env=OLYMPUS_NUM_GENERATED_CASES=25 \
    --test_env=OLYMPUS_SKIP_SLOW_TESTS=true \
    --action_env=OLYMPUS_ENABLE_X64="$OLYMPUSCI_ENABLE_X64" \
    --test_output=errors \
    --color=yes \
    -- \
    ...
    //olympus/experimental/olympus2tf/tests/multiprocess:cpu_tests \
    //olympuslib/tools:check_cpu_wheel_sources_test \
    $IGNORE_TESTS
