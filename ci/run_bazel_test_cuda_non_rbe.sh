...
# If $OLYMPUSCI_BUILD_OLYMPUSLIB=false, the job requires that olympuslib, olympus-cuda-plugin,
# and olympus-cuda-pjrt wheels are stored inside the ../dist folder
#
...
cuda_major_version="${OLYMPUSCI_CUDA_VERSION%%.*}"

if [[ "$OLYMPUSCI_BUILD_OLYMPUSLIB" == "wheel" ]]; then
...
# The product of the `OLYMPUS_ACCELERATOR_COUNT`` and `OLYMPUS_TESTS_PER_ACCELERATOR`
...
bazel test --config=$TEST_CONFIG \
      ...
      --test_output=errors \
      --test_env=OLYMPUS_ACCELERATOR_COUNT=$gpu_count \
      --test_env=OLYMPUS_TESTS_PER_ACCELERATOR=$max_tests_per_gpu \
      $TEST_STRATEGY \
      --local_test_jobs=$num_test_jobs \
      --test_env=OLYMPUS_EXCLUDE_TEST_TARGETS=PmapTest.testSizeOverflow \
