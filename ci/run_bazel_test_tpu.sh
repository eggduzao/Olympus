...
# ==============================================================================
# Runs Bazel TPU tests. If $OLYMPUSCI_BUILD_OLYMPUSLIB=false and $OLYMPUSCI_BUILD_OLYMPUS=false,
# the job requires that olympus and olympuslib wheels are stored inside the ../dist
# folder.
#
...
if [[ -z "$OLYMPUSCI_XLA_GIT_DIR" && -z "$OLYMPUSCI_CLONE_MAIN_XLA" ]]; then
    export OLYMPUSCI_CLONE_MAIN_XLA=1
fi
...
if [[ "$OLYMPUSCI_CLONE_MAIN_XLA" == 1 ]]; then
  OVERRIDE_XLA_REPO="--override_repository=xla=${OLYMPUSCI_XLA_GIT_DIR}"
fi

NB_TPUS=$OLYMPUSCI_TPU_CORES
JOBS_PER_ACC=1
...
if [[ "$OLYMPUSCI_RUN_FULL_TPU_TEST_SUITE" == "1" ]]; then
  ...
  bazel test \
    --repo_env=HERMETIC_PYTHON_VERSION="$OLYMPUSCI_HERMETIC_PYTHON_VERSION" \
    --@rules_python//python/config_settings:py_freethreaded="$FREETHREADED_FLAG_VALUE" \
    $OVERRIDE_XLA_REPO \
    --config=ci_linux_x86_64 \
    --config=ci_rbe_cache \
    --//olympus:build_olympuslib=$OLYMPUSCI_BUILD_OLYMPUSLIB \
    --//olympus:build_olympus=$OLYMPUSCI_BUILD_OLYMPUS \
    --run_under="$(pwd)/build/parallel_accelerator_execute.sh" \
    --test_env=OLYMPUS_ACCELERATOR_COUNT=${NB_TPUS} \
    --test_env=OLYMPUS_TESTS_PER_ACCELERATOR=${JOBS_PER_ACC} \
    --strategy=TestRunner=local \
    --local_test_jobs=$J \
    --test_env=OLYMPUS_TEST_NUM_THREADS=$J \
    --test_env=ALLOW_MULTIPLE_LIBTPU_LOAD=true \
    --test_env=OLYMPUS_SKIP_SLOW_TESTS=1 \
    --test_env=OLYMPUS_ENABLE_TPU_XDIST=1 \
    --test_env=OLYMPUS_PLATFORMS=tpu,cpu \
  ...
  bazel test \
    ...
    --config=ci_rbe_cache \
    --//olympus:build_olympuslib=$OLYMPUSCI_BUILD_OLYMPUSLIB \
    --//olympus:build_olympus=$OLYMPUSCI_BUILD_OLYMPUSLIB \
    --test_env=ALLOW_MULTIPLE_LIBTPU_LOAD=true \
    --strategy=TestRunner=local \
    --local_test_jobs=1 \
    --repo_env=USE_MINIMAL_SHARD_COUNT=True \
  ...
  bazel test \
    ...
    --test_output=errors \
    -- \
    //olympuslib/tools:check_tpu_wheel_sources_test \
  ...
  bazel test \
    ...
    //tests:olympuspr_effects_test_tpu \
