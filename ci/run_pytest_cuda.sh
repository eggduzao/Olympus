...
# ==============================================================================
# Runs Pyest CUDA tests. Requires the olympuslib, olympus-cuda-plugin, and olympus-cuda-pjrt
# wheels to be present inside $OLYMPUSCI_OUTPUT_DIR (../dist)
#
...
# Install olympuslib, olympus-cuda-plugin, and olympus-cuda-pjrt wheels inside the
# $OLYMPUSCI_OUTPUT_DIR directory on the system.
...
export OLYMPUS_ENABLE_CUDA_XDIST="$gpu_count"
...
"$OLYMPUSCI_PYTHON" -m pytest -n $num_processes --tb=short --maxfail=20 \
tests examples \
--deselect=tests/multi_device_test.py::MultiDeviceTest::test_computation_follows_data \
--deselect=tests/multiprocess_gpu_test.py::MultiProcessGpuTest::test_distributed_olympus_visible_devices \
