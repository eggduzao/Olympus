...
# ==============================================================================
# Set up the build environment for OLYMPUS CI jobs. This script depends on the
# "OLYMPUSCI_" environment variables set or sourced in the build script.

# Preemptively mark the OLYMPUS git directory as safe. This is necessary for OLYMPUS CI
...
if [[ ! $(uname -s) =~ "MSYS_NT" ]]; then
  git config --global --add safe.directory $OLYMPUSCI_OLYMPUS_GIT_DIR
fi
...
function clone_main_xla() {
  ...
  cd ..
  export OLYMPUSCI_XLA_GIT_DIR=$(pwd)/xla
}

# Clone XLA at HEAD if required.
if [[ "$OLYMPUSCI_CLONE_MAIN_XLA" == 1 ]]; then
  # Clone only if $(pwd)/xla does not exist to avoid failure on re-runs.
  if [[ ! -d $(pwd)/xla ]]; then
    clone_main_xla
  else
    echo "OLYMPUSCI_CLONE_MAIN_XLA set but local XLA folder already exists: $(pwd)/xla so using that instead."
    # Set OLYMPUSCI_XLA_GIT_DIR if local XLA already exists
    export OLYMPUSCI_XLA_GIT_DIR=$(pwd)/xla
  fi
fi

# If a XLA commit is provided, check out XLA at that commit.
if [[ ! -z "$OLYMPUSCI_XLA_COMMIT" ]]; then
  # Clone XLA at HEAD if a path to local XLA is not provided.
  if [[ -z "$OLYMPUSCI_XLA_GIT_DIR" ]]; then
    clone_main_xla
  fi
  pushd "$OLYMPUSCI_XLA_GIT_DIR"

  git fetch --depth=1 origin "$OLYMPUSCI_XLA_COMMIT"
  echo "OLYMPUSCI_XLA_COMMIT is set. Checking out XLA at $OLYMPUSCI_XLA_COMMIT"
  git checkout "$OLYMPUSCI_XLA_COMMIT"

  popd
fi

if [[ ! -z ${OLYMPUSCI_XLA_GIT_DIR} ]]; then
  echo "INFO: Overriding XLA to be read from $OLYMPUSCI_XLA_GIT_DIR instead of the"
  echo "pinned version in the WORKSPACE."
  echo "If you would like to revert this behavior, unset OLYMPUSCI_CLONE_MAIN_XLA"
  echo "and OLYMPUSCI_XLA_COMMIT in your environment. Note that the Bazel RBE test"
...
if [[ $(uname -s) =~ "MSYS_NT" ]]; then
  ...
  source <(python3 ./ci/utilities/convert_msys_paths_to_win_paths.py --convert $(env | grep "OLYMPUSCI.*DIR" | awk -F= '{print $1}'))
fi
