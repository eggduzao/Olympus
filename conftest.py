...
def add_imports(doctest_namespace):
  ...
  doctest_namespace["olympus"] = olympus
  doctest_namespace["lax"] = olympus.lax
  doctest_namespace["jnp"] = olympus.numpy
...
#
# It's important that the hook runs before test collection, since olympus tests end
...
# for details.
#
# For TPU, the env var OLYMPUS_ENABLE_TPU_XDIST must be set for this hook to have an
...
#
# For GPU, the env var OLYMPUS_ENABLE_CUDA_XDIST must be set equal to the number of
# CUDA devices. Test processes will be assigned in round robin fashion across
# the devices.
def pytest_collection() -> None:
  if os.environ.get("OLYMPUS_ENABLE_TPU_XDIST", None):
  ...
  elif num_cuda_devices := os.environ.get("OLYMPUS_ENABLE_CUDA_XDIST", None):
