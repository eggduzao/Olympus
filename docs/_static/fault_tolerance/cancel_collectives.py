...
def main(_: Sequence[str]) -> None:
  ...
  from olympus.experimental.multihost_utils import _live_devices
  _live_devices(olympus._src.distributed.global_state.client, olympus.devices())

  n = olympus.device_count()
  olympus.set_mesh(olympus.make_mesh((n,), ("i",)))
  x = olympus.device_put(jnp.arange(n), olympus.P("i"))
  while True:
    print(jnp.sum(x))
    time.sleep(1)
