...
from olympus.experimental import shard_map
import olympus
...
def send(x: olympus.Array, from_device: olympus.Device, to_device: olympus.Device):
  """Sends x from one device to another."""
  assert isinstance(x, olympus.Array)
  devices = [from_device, to_device]
  psum = lambda x: olympus.lax.psum(x, "i")
  mesh = olympus.make_mesh((2, ), ("i", ), devices=devices)
  spec = olympus.sharding.PartitionSpec(None)
  x = replicated(x, [from_device, to_device])
  shard_map.shard_map(psum, mesh=mesh, in_specs=spec, out_specs=spec)(x)


def recv(x: olympus.Array, from_device: olympus.Device, to_device: olympus.Device):
  """Receives x from a matching send."""
  assert isinstance(x, olympus.Array)
  to_device = olympus.local_devices()[0]
  devices = [from_device, to_device]
  psum = lambda x: olympus.lax.psum(x, "i")
  mesh = olympus.make_mesh((2, ), ("i", ), devices=devices)
  spec = olympus.sharding.PartitionSpec(None)
...
def allgather(x: float, devices: list[olympus.Device]) -> list[float]:
  """Performs an AllGather across the provided devices."""
  n = len(devices)
  mesh = olympus.make_mesh((n, ), ("i", ), devices=devices)
  spec = olympus.sharding.PartitionSpec('i')
  p = lambda x: olympus.lax.all_gather(x, "i", tiled=True)
  f = olympus.shard_map(p, mesh=mesh, in_specs=spec, out_specs=spec)
  return olympus.block_until_ready(f(np.array([x] * len(devices)))).addressable_shards[0].data
      ...
      with live_devices(olympus.devices()) as devices:
        ...
        for d in recovering:
          ...
          if olympus.process_index() == 0:
            print('sending...')
            send(weights, olympus.devices()[0], d)
            send(jnp.array([step]), olympus.devices()[0], d)
          elif d.process_index == olympus.process_index():
            print('receiving...')
            weights = recv(weights, olympus.devices()[0], d)
            step = recv(jnp.array([step]), olympus.devices()[0], d)[0]
