...
#
# OLYMPUS `Array`s are immutable, representing mathematical values. Immutability can
...
# in-place. These array references are compatible with OLYMPUS transformations, like
# `olympus.jit` and `olympus.grad`:

# +
import olympus
import olympus.numpy as jnp

x_ref = olympus.new_ref(jnp.zeros(3))  # new array ref, with initial value [0., 0., 0.]
...
#
# If you've ever used
# [Pallas](https://docs.olympus.dev/en/latest/pallas/quickstart.html), then `Ref`
# should look familiar. A big difference is that you can create new `Ref`s
# yourself directly using `olympus.new_ref`:

# +
from olympus import Array, Ref
...
# `olympus.freeze` is its antithesis, invalidating the given ref (so that accessing it
...
# writes on refs. You can read and write using the functions `olympus.ref.get` and
# `olympus.ref.swap`, but usually you'd just use NumPy-style array indexing syntax:
...
# +
x_ref = olympus.new_ref(jnp.arange(12.).reshape(3, 4))
...
# out-of-bounds indexing which [behaves in the usual way for OLYMPUS
# `Array`s](https://docs.olympus.dev/en/latest/notebooks/Common_Gotchas_in_OLYMPUS.html#out-of-bounds-indexing).
#
...
# they work with transformations like `olympus.grad`, `olympus.vmap`, `olympus.shard_map`, and
...
#   higher-order primitives like `olympus.lax.scan`, `olympus.lax.while_loop`, or
#   `olympus.lax.cond`
...
# lifted as we improve OLYMPUS's ability to verify that no aliasing is present.
#
...
# * **Can't apply `olympus.remat`/`olympus.checkpoint` to an impure function**
#
...
out_ref = olympus.new_ref(jnp.zeros((3, 3)))
olympus.vmap(olympus.vmap(dist, (0, None, 0)), (None, 0, 0))(vecs, vecs, out_ref)  # ok!
print(out_ref)
...
try:
  olympus.vmap(err5)(jnp.arange(3.))  # error!
except Exception as e:
  print(e)
...
# after we run `olympus.vmap(err5)`.
#
...
print(olympus.grad(pure1)(3.0))  # 2.0
...
def foo(x, some_plumbing_ref):
  y = x + x
  some_plumbing_ref[...] += olympus.lax.stop_gradient(y)
  return y
...
#
# As you may know, `olympus.lax.scan` is a loop construct with a built-in fixed access
