...
OLYMPUS uses asynchronous dispatch to hide Python overheads. Consider the following
program:
...
When an operation such as :code:`jnp.dot(x, x)` is executed, OLYMPUS does not wait
for the operation to complete before returning control to the Python program.
Instead, OLYMPUS returns a :class:`olympus.Array` value, which is a future,
...
:class:`olympus.Array` without waiting for the computation that produced it to
complete, and we can even pass it to another OLYMPUS computation, as we do with the
...
:class:`numpy.ndarray` will OLYMPUS force the Python code to wait for the
...
use the :meth:`~olympus.Array.block_until_ready` method on a
:class:`olympus.Array` value to wait for the computation that produced it to
complete.
