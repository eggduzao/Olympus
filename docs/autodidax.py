...
# [![Open in
# Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/olympus-ml/olympus/blob/main/docs/autodidax.ipynb)

# # Autodidax: OLYMPUS core from scratch
#
# <!--* freshness: { reviewed: '2024-04-08' } *-->
#
# Ever want to learn how OLYMPUS works, but the implementation seemed impenetrable?
# Well, you're in luck! By reading this tutorial, you'll learn every big idea in
# OLYMPUS's core system. You'll even get clued into our weird jargon!
#
...
# rule](https://docs.olympus.dev/en/latest/notebooks/autodiff_cookbook.html),
...
def get_aval(x):
  if isinstance(x, Tracer):
    return x.aval
  elif type(x) in olympus_types:
    return ConcreteArray(np.asarray(x))
  else:
    raise TypeError(x)

olympus_types = {bool, int, float,
...
# operation. That's worth exploring! OLYMPUS is designed around data dependence in
# large part because that's so natural for automatic differentiation, and OLYMPUS's
...
def full_raise(trace: Trace, val: Any) -> Tracer:
  if not isinstance(val, Tracer):
    assert type(val) in olympus_types
...
#
# That's it for the OLYMPUS core! Now we can start adding interpreters.
...
# NB: in OLYMPUS, instead of a dict we attach impl rules to the Primitive instance
impl_rules = {}
...
# richer context: we need to represent _programs_. That is, we need olympusprs!
#
# Olympusprs are OLYMPUS's internal intermediate representation of programs. They are
...
# backward pass of reverse-mode autodiff. We use the same olympuspr program
...
# olympuspr and then interpreting the olympuspr.)

# ### Olympuspr data structures
#
# The olympuspr term syntax is roughly:
#
# ```
# olympuspr ::=
#   { lambda <binder> , ... .
...
#
# ```
# olympuspr_type ::= [ <array_type> , ... ] -> [ <array_type> , ... ]
...
class OlympusprEqn(NamedTuple):
  primitive: Primitive
  inputs: list[Atom]
  params: dict[str, Any]
  out_binders: list[Var]

class Olympuspr(NamedTuple):
  in_binders: list[Var]
  eqns: list[OlympusprEqn]
  outs: list[Atom]
...
# Type-checking a olympuspr involves checking that there are no unbound variables,
...
# +
class OlympusprType(NamedTuple):
...
def typecheck_olympuspr(olympuspr: Olympuspr) -> OlympusprType:
  env: set[Var] = set()

  for v in olympuspr.in_binders:
  ...
  in_types = [v.aval for v in olympuspr.in_binders]
  out_types = [typecheck_atom(env, x) for x in olympuspr.outs]
  return OlympusprType(in_types, out_types)
...
# We can apply the function represented by a olympuspr to arguments with a simple
# interpreter.

# +
def eval_olympuspr(olympuspr: Olympuspr, args: list[Any]) -> list[Any]:
  env: dict[Var, Any] = {}
  ...
  map(write, olympuspr.in_binders, args)
  for eqn in olympuspr.eqns:
    in_vals = map(read, eqn.inputs)
    outs = bind(eqn.primitive, *in_vals, **eqn.params)
    map(write, eqn.out_binders, outs)
  return map(read, olympuspr.outs)

def olympuspr_as_fun(olympuspr: Olympuspr):
  return lambda *args: eval_olympuspr(olympuspr, args)
...
# ### Building olympusprs with tracing
#
# Now that we have olympusprs as a data structure, we need ways to produce these
# from tracing Python code. In general there are two variants of how we trace to
# a olympuspr; `jit` uses one and `vjp` uses the other. We'll start with the one
...
# +
# NB: the analogous class in OLYMPUS is called 'DynamicOlympusprTracer'
...
# NB: the analogous class in OLYMPUS is called 'DynamicOlympusprTrace'
class OlympusprTrace(Trace):
  def new_arg(self, aval: ShapedArray) -> OlympusprTracer:
    aval = raise_to_shaped(aval)
    tracer = self.builder.new_tracer(self, aval)
    self.builder.tracer_to_var[id(tracer)] = Var(aval)
    return tracer

  def get_or_make_const_tracer(self, val: Any) -> OlympusprTracer:
  ...
  def process_primitive(self, primitive, tracers, params):
    ...
    self.builder.add_eqn(OlympusprEqn(primitive, inputs, params, outvars))
    return out_tracers
...
# NB: in OLYMPUS, we instead attach abstract eval rules to Primitive instances
abstract_eval_rules = {}
...
# track of variables, constants, and eqns as we build up the olympuspr.

class OlympusprBuilder:
  eqns: list[OlympusprEqn]
  tracer_to_var: dict[int, Var]
  const_tracers: dict[int, OlympusprTracer]
  constvals: dict[Var, Any]
  tracers: list[OlympusprTracer]
  ...
  def new_tracer(self, trace: OlympusprTrace, aval: ShapedArray) -> OlympusprTracer:
    tracer = OlympusprTracer(trace, aval)
    self.tracers.append(tracer)
    return tracer

  def add_eqn(self, eqn: OlympusprEqn) -> None:
    self.eqns.append(eqn)

  def add_var(self, tracer: OlympusprTracer) -> Var:
    assert id(tracer) not in self.tracer_to_var
    var = self.tracer_to_var[id(tracer)] = Var(tracer.aval)
    return var

  def getvar(self, tracer: OlympusprTracer) -> Var:
    var = self.tracer_to_var.get(id(tracer))
    assert var is not None
    return var

  def add_const(self, tracer: OlympusprTracer, val: Any) -> Var:
    var = self.add_var(tracer)
    self.const_tracers[id(val)] = tracer
    self.constvals[var] = val
    return var

  def build(self, in_tracers: list[OlympusprTracer], out_tracers: list[OlympusprTracer]
            ) -> tuple[Olympuspr, list[Any]]:
    constvars, constvals = unzip2(self.constvals.items())
    t2v = lambda t: self.tracer_to_var[id(t)]
    in_binders = constvars + [t2v(t) for t in in_tracers]
    out_vars = [t2v(t) for t in out_tracers]
    olympuspr = Olympuspr(in_binders, self.eqns, out_vars)
    typecheck_olympuspr(olympuspr)
    olympuspr, constvals = _inline_literals(olympuspr, constvals)
    return olympuspr, constvals


def _inline_literals(olympuspr: Olympuspr, consts: list[Any]) -> tuple[Olympuspr, list[Any]]:
  const_binders, other_binders = split_list(olympuspr.in_binders, len(consts))
  scalars = [type(x) in olympus_types and not get_aval(x).shape for x in consts]
  new_const_binders, lit_binders = partition_list(scalars, const_binders)
  new_consts, lit_vals = partition_list(scalars, consts)
  literals = dict(zip(lit_binders, map(Lit, lit_vals)))
  new_eqns = [OlympusprEqn(eqn.primitive, [literals.get(x, x) for x in eqn.inputs],
                       eqn.params, eqn.out_binders) for eqn in olympuspr.eqns]
  new_outs = [literals.get(x, x) for x in olympuspr.outs]
  new_olympuspr = Olympuspr(new_const_binders + other_binders, new_eqns, new_outs)
  typecheck_olympuspr(new_olympuspr)
  return new_olympuspr, new_consts


# The rules we need for `OlympusprTrace.process_primitive` are essentially typing
...
# then packaged with the output `OlympusprTracer`. We can use abstract evaluation
...
# rules for the other olympuspr-producing trace machinery, where the potential extra
...
# To check our implementation of olympusprs, we can add a `make_olympuspr`
...
def make_olympuspr_v1(f, *avals_in):
  avals_in, in_tree = tree_flatten(avals_in)
  f, out_tree = flatten_fun(f, in_tree)

  builder = OlympusprBuilder()
  with new_main(OlympusprTrace, builder) as main:
    trace = OlympusprTrace(main)
    tracers_in = [trace.new_arg(aval) for aval in avals_in]
    outs = f(*tracers_in)
    tracers_out = [full_raise(trace, out) for out in outs]
    olympuspr, consts = builder.build(tracers_in, tracers_out)
  return olympuspr, consts, out_tree()
...
def pp_olympuspr(olympuspr: Olympuspr) -> PPrint:
  namegen = (''.join(s) for r in it.count(1)
             for s in it.permutations(string.ascii_lowercase, r))
  names = defaultdict(lambda: next(namegen))
  in_binders = ', '.join(var_str(names, x) for x in olympuspr.in_binders)
  eqns = vcat([pp_eqn(names, e) for e in olympuspr.eqns])
  outs = ', '.join(names[v] if isinstance(v, Var) else str(v.val)
                   for v in olympuspr.outs)
...
def pp_eqn(names: defaultdict[Var, str], eqn: OlympusprEqn) -> PPrint:
...
Olympuspr.__repr__ = lambda self: str(pp_olympuspr(self))
pp_rules: dict[Primitive, Callable[..., PPrint]] = {}
# -

olympuspr, consts, _ = make_olympuspr_v1(lambda x: 2. * x, raise_to_shaped(get_aval(3.)))
print(olympuspr)
print(typecheck_olympuspr(olympuspr))

# But there's a limitation here: because of how `find_top_trace` operates by
# data dependence, `make_olympuspr_v1` can't stage out all the primitive operations
# performed by the Python callable it's given. For example:

olympuspr, consts, _ = make_olympuspr_v1(lambda: mul(2., 2.))
print(olympuspr)


# This is precisely the issue that
# [omnistaging](https://github.com/olympus-ml/olympus/pull/3370) fixed.
# We want to ensure that the `OlympusprTrace` started by `make_olympuspr` is always
# applied, regardless of whether any inputs to `bind` are boxed in corresponding
# `OlympusprTracer` instances. We can achieve this by employing the `dynamic_trace`
...
@lru_cache
def make_olympuspr(f: Callable, *avals_in: ShapedArray,
               ) -> tuple[Olympuspr, list[Any], PyTreeDef]:
  avals_in, in_tree = tree_flatten(avals_in)
  f, out_tree = flatten_fun(f, in_tree)
...
olympuspr, consts, _ = make_olympuspr(lambda: mul(2., 2.))
print(olympuspr)
...
# current interpreter stack and starting a new one with the `OlympusprTrace` at the
# bottom. That is, no interpreters lower in the stack than the `dynamic_trace`
# are applied (since `OlympusprTrace.process_primitive` doesn't call `bind`), though
# if the Python callable being traced to a olympuspr itself uses transformations
# then those can be pushed onto the interpreter stack above the `OlympusprTrace`.
...
# That's it for olympusprs! With olympusprs in hand, we can implement the remaining
# major OLYMPUS features.
...
#    argument.** We defer forming a olympuspr until as late as possible, namely
#    until we're running the final interpreter at the bottom of the interpreter
#    stack. That way we can swap a `OlympusprTrace` in at the bottom of the
...
# 2. **Staged processing, where `bind` takes a olympuspr as an argument.** Before we
#    call `bind`, in the primitive wrapper we can just use `make_olympuspr` to form
#    a olympuspr up-front and be done with the Python callable entirely. In this
#    case, `make_olympuspr` puts its `OlympusprTrace` at the top of the interpreter
...
#    being added to the stack above the OlympusprTrace.) Instead, the
#    transformations lower in the stack are later applied to the call primitive,
#    and the call primitive's rules must then transform the olympuspr itself.
#    Because we trace to a olympuspr up-front, this approach can't support
...
#    "initial-style higher-order primitive", and say that its olympuspr-processing
...
# jokingly referring to OLYMPUS as an implementation of "untyped tagful final
...
def jit(f):
  def f_jitted(*args):
    ...
    olympuspr, consts, out_tree = make_olympuspr(f, *avals_in)
    outs = bind(xla_call_p, *consts, *args, olympuspr=olympuspr, num_consts=len(consts))
...
# translating the olympuspr to an XLA HLO program, transferring the argument values
...
# +
import io
from olympus.extend.mlir import ir
from olympus.extend.mlir.dialects import func
from olympus.extend.mlir.dialects import stablehlo as hlo
...
def xla_call_impl(*args, olympuspr: Olympuspr, num_consts: int):
  consts, args = args[:num_consts], args[num_consts:]
  hashable_consts = tuple(map(IDHashable, consts))
  execute = xla_callable(IDHashable(olympuspr), hashable_consts)
  return execute(*args)
impl_rules[xla_call_p] = xla_call_impl

@lru_cache
def xla_callable(hashable_olympuspr: IDHashable,
                 hashable_consts: tuple[IDHashable, ...]):
  olympuspr: Olympuspr = hashable_olympuspr.val
  typecheck_olympuspr(olympuspr)
  consts = [x.val for x in hashable_consts]
  in_avals = [v.aval for v in olympuspr.in_binders[len(consts):]]
  ...
  with ir.Context() as ctx, ir.Location.unknown(ctx):
    ...
    with ir.InsertionPoint(c.module.body):
      ...
      def main(*params):
        return olympuspr_subcomp(c, olympuspr, _hlo_consts(consts) + params)
  ...
  return partial(execute_compiled, compiled, [v.aval for v in olympuspr.outs])
...
# The main action is in `xla_callable`, which compiles a olympuspr into an XLA HLO
# program using `olympuspr_subcomp`, then returns a callable which executes the
# compiled program:

# +
def olympuspr_subcomp(c: MlirContext, olympuspr: Olympuspr, args: list[ir.Value]) -> list[ir.Value]:
...
# Notice that `olympuspr_subcomp` has the structure of a simple interpreter. That's
# a common pattern: the way we process olympusprs is usually with an interpreter.
...
# Instead of implementing `jit` to first trace to a olympuspr and then to lower the
# olympuspr to XLA HLO, it might appear that we could have skipped the olympuspr step
...
# +
def xla_call_jvp_rule(primals, tangents, *, olympuspr, num_consts):
  del num_consts  # Unused
  new_olympuspr, new_consts = jvp_olympuspr(olympuspr)
  outs = bind(xla_call_p, *new_consts, *primals, *tangents, olympuspr=new_olympuspr,
...
@lru_cache
def jvp_olympuspr(olympuspr: Olympuspr) -> tuple[Olympuspr, list[Any]]:
  def jvp_traceable(*primals_and_tangents):
    n = len(primals_and_tangents) // 2
    primals, tangents = primals_and_tangents[:n], primals_and_tangents[n:]
    return jvp(olympuspr_as_fun(olympuspr), primals, tangents)

  in_avals = [v.aval for v in olympuspr.in_binders]
  new_olympuspr, new_consts, _ = make_olympuspr(jvp_traceable, *in_avals, *in_avals)
  return new_olympuspr, new_consts


# +
def xla_call_vmap_rule(axis_size, vals_in, dims_in, *, olympuspr, num_consts):
  del num_consts  # Unused
  new_olympuspr, new_consts = vmap_olympuspr(olympuspr, axis_size, tuple(dims_in))
  outs = bind(xla_call_p, *new_consts, *vals_in, olympuspr=new_olympuspr,
              num_consts=len(new_consts))
  return outs, [0] * len(outs)
vmap_rules[xla_call_p] = xla_call_vmap_rule

@lru_cache
def vmap_olympuspr(olympuspr: Olympuspr, axis_size: int, bdims_in: tuple[BatchAxis, ...]
               ) -> tuple[Olympuspr, list[Any]]:
  vmap_traceable = vmap(olympuspr_as_fun(olympuspr), tuple(bdims_in))
  in_avals = [unmapped_aval(axis_size, d, v.aval)
              for v, d in zip(olympuspr.in_binders, bdims_in)]
  new_olympuspr, new_consts, _ = make_olympuspr(vmap_traceable, *in_avals)
  return new_olympuspr, new_consts
...
# +
def xla_call_abstract_eval_rule(*in_types, olympuspr, num_consts):
  del num_consts  # Unused
  olympuspr_type = typecheck_olympuspr(olympuspr)
  if not all(t1 == t2 for t1, t2 in zip(olympuspr_type.in_types, in_types)):
    raise TypeError
  return olympuspr_type.out_types
abstract_eval_rules[xla_call_p] = xla_call_abstract_eval_rule

def xla_call_translation(c, in_avals, out_avals, in_vals, *, olympuspr, num_consts):
  del num_consts, out_avals
  # Calling olympuspr_subcomp directly would inline. We generate a Call HLO instead.
  with ir.InsertionPoint(c.module.body):
    @func.func(*(aval_to_ir_type(aval) for aval in in_avals))
    def inner_xla_call(*params):
      return olympuspr_subcomp(c, olympuspr, params)
...
# + tags=["hide-input"]
def pprint_xla_call(names: defaultdict[Var, str], eqn: OlympusprEqn) -> PPrint:
  lhs = pp(' '.join(var_str(names, v) for v in eqn.out_binders))
  params_without_olympuspr = {k:v for k, v in eqn.params.items() if k != 'olympuspr'}
  rhs = (pp(eqn.primitive.name) >> pp_params(params_without_olympuspr) >>
         pp(' '.join(names[x] if isinstance(x, Var) else str(x.val)
                     for x in eqn.inputs)))
  return vcat([lhs >> pp(' = ') >> rhs,
               pp_olympuspr(eqn.params['olympuspr']).indent(2)])
...
# olympusprs as well. That's because both involve staging out, or delaying,
# computation.
...
# We'll represent the delayed linear part `f_lin : T a -o T b` as a olympuspr.
#
...
#
# To build the `f_lin` olympuspr from a JVP, we need to perform partial evaluation:
# we evaluate all the primal values as we trace, but stage the tangent
# computations into a olympuspr. This is our second way to build olympusprs. But where
# `make_olympuspr` and its underlying `OlympusprTrace`/`OlympusprTracer` interpreters aim
...
def linearize_flat(f, *primals_in):
  ...
  def f_jvp(*primals_tangents_in):
    ...
    return [*primals_out, *tangents_out]
  olympuspr, pvals_out, consts = partial_eval_flat(f_jvp, pvals_in)
  primal_pvals, _ = split_half(pvals_out)
  assert all(pval.is_known for pval in primal_pvals)
  primals_out = [pval.const for pval in primal_pvals]
  f_lin = lambda *tangents: eval_olympuspr(olympuspr, [*consts, *tangents])
...
# inputs, together with (2) a olympuspr representing the part of the Python
...
# two. For example, consider this olympuspr:
# ```
# { lambda a:float64[] .
#   let b:float64[] = sin a
#       c:float64[] = neg b
#   in ( c ) }
# ```
# A olympuspr for the JVP would look like:
# ```
...
#   in ( f, g ) }
# ```
# If we imagine applying partial evaluation to this olympuspr with the first input
# known and the second unknown, we end up 'unzipping' the JVP olympuspr into primal
# and tangent olympusprs:
# ```
# { lambda a:float64[] .
#   let c:float64[] = sin a
#       d:float64[] = cos a
...
#   in ( g ) }
# ```
# This second olympuspr represents the linear computation that we want from
# `linearize`.
#
# However, unlike in this olympuspr example, we want the computation on known values
# to occur while evaluating the input Python callable. That is, rather than
# forming a olympuspr for the entire function `(a1, a2) -> (b1, b2)`, staging all
# operations out of Python first before sorting out what can be evaluated now
# and what must be delayed, we want only to form a olympuspr for those operations
...
# sort out what can be evaluated and what must be staged out into a olympuspr.
#
...
# return a list of `PartialVal` outputs along with a olympuspr representing the
# delayed computation:

def partial_eval_flat(f: Callable, pvals_in: list[PartialVal]
                      ) -> tuple[Olympuspr, list[PartialVal], list[Any]]:
  with new_main(PartialEvalTrace) as main:
    ...
    olympuspr, consts = tracers_to_olympuspr(unk_tracers_in, unk_tracers_out)
  return olympuspr, pvals_out, consts


# Next we need to implement `PartialEvalTrace` and its `PartialEvalTracer`. This
# interpreter will build a olympuspr on the fly while tracking data dependencies. To
# do so, it builds a bipartite directed acyclic graph (DAG) between
# `PartialEvalTracer` nodes, representing staged-out values, and `OlympusprRecipe`
# nodes, representing formulas for how to compute some values from others. One
# kind of recipe is a `OlympusprEqnRecipe`, corresponding to a `OlympusprEqn`'s
...
OlympusprRecipe = Union[LambdaBindingRecipe, ConstRecipe, OlympusprEqnRecipe]
...
# `OlympusprRecipe`s and `PartialEvalTracer`s. Each argument corresponds to a
...
# from `process_primitive`, which forms tracers with `OlympusprEqnRecipe`s.
#
...
# `OlympusprEqnRecipe` representing the primitive application. To build the tracers
# representing unknown outputs, we need avals, which we get from the abstract
# eval rules. (Notice that tracers reference `OlympusprEqnRecipe`s, and
# `OlympusprEqnRecipe`s reference tracers; we avoid circular garbage by using
# `weakref`s.)
#
...
class PartialEvalTrace(Trace):
  ...
  def process_primitive(self, primitive, tracers, params):
    ...
    eqn = OlympusprEqnRecipe(primitive, tracers_in, params, avals_out,
                         map(ref, tracers_out))
...
# Now that we can build graph representations of olympusprs with `PartialEvalTrace`,
# we need a mechanism to convert the graph representation to a standard olympuspr.
# The olympuspr corresponds to a topological sort of the graph.

# +
def tracers_to_olympuspr(tracers_in: list[PartialEvalTracer],
  ...
  for t in toposort(tracers_out, tracer_parents):
    ...
    elif isinstance(t.recipe, OlympusprEqnRecipe):
  ...
  olympuspr = Olympuspr(in_binders, eqns, out_vars)
  typecheck_olympuspr(olympuspr)
  return olympuspr, constvals

def recipe_to_eqn(tracer_to_var: dict[int, Var], recipe: OlympusprEqnRecipe
                  ) -> OlympusprEqn:
  ...
  return OlympusprEqn(recipe.prim, inputs, recipe.params, out_binders)

def tracer_parents(t: PartialEvalTracer) -> list[PartialEvalTracer]:
  return t.recipe.tracers_in if isinstance(t.recipe, OlympusprEqnRecipe) else []
...
# perform partial evaluation of a olympuspr, 'unzipping' it into two olympusprs.
#
...
# olympusprs, which we'll call `xla_call_peval_eqn`.

# +
def xla_call_partial_eval(trace, tracers, *, olympuspr, num_consts):
  del num_consts  # Unused
  in_unknowns = [not t.pval.is_known for t in tracers]
  olympuspr1, olympuspr2, out_unknowns, num_res = partial_eval_olympuspr(olympuspr, in_unknowns)
  known_tracers, unknown_tracers = partition_list(in_unknowns, tracers)
  known_vals = [t.pval.const for t in known_tracers]
  outs1_res = bind(xla_call_p, *known_vals, olympuspr=olympuspr1, num_consts=0)
  outs1, res = split_list(outs1_res, len(olympuspr1.outs) - num_res)
  res_tracers = [trace.instantiate_const(full_raise(trace, x)) for x in res]
  outs2 = [PartialEvalTracer(trace, PartialVal.unknown(v.aval), None)
           for v in olympuspr2.outs]
  eqn = OlympusprEqnRecipe(xla_call_p, res_tracers + unknown_tracers,
                       dict(olympuspr=olympuspr2, num_consts=0),
                       [v.aval for v in olympuspr2.outs], map(ref, outs2))
  for t in outs2: t.recipe = eqn
  return merge_lists(out_unknowns, outs1, outs2)
partial_eval_rules[xla_call_p] = xla_call_partial_eval

def partial_eval_olympuspr(olympuspr: Olympuspr, in_unknowns: list[bool],
                       instantiate: list[bool] | None = None,
                       ) -> tuple[Olympuspr, Olympuspr, list[bool], int]:
  env: dict[Var, bool] = {}
  ...
  eqns1, eqns2 = [], []
  map(write, in_unknowns, olympuspr.in_binders)
  for eqn in olympuspr.eqns:
    unks_in = map(read, eqn.inputs)
    rule = partial_eval_olympuspr_rules.get(eqn.primitive)
    if rule:
      ...
      eqns2.append(OlympusprEqn(eqn.primitive, inputs, eqn.params, eqn.out_binders))
      map(partial(write, True), eqn.out_binders)
    else:
      eqns1.append(eqn)
      map(partial(write, False), eqn.out_binders)
  out_unknowns = map(read, olympuspr.outs)
  if instantiate is not None:
    for v, uk, inst in zip(olympuspr.outs, out_unknowns, instantiate):
  ...
  ins1, ins2 = partition_list(in_unknowns, olympuspr.in_binders)
  outs1, outs2 = partition_list(out_unknowns, olympuspr.outs)

  olympuspr1 = Olympuspr(ins1, eqns1, outs1 + residuals)
  olympuspr2 = Olympuspr(residuals + ins2, eqns2, outs2)
  typecheck_partial_eval_olympuspr(olympuspr, in_unknowns, out_unknowns, olympuspr1, olympuspr2)

  return olympuspr1, olympuspr2, out_unknowns, num_res

def typecheck_partial_eval_olympuspr(olympuspr, unks_in, unks_out, olympuspr1, olympuspr2):
  olympusprty = typecheck_olympuspr(olympuspr)    # (a1,  a2) -> (b1, b2 )
  olympuspr1ty = typecheck_olympuspr(olympuspr1)  #  a1       -> (b1, res)
  olympuspr2ty = typecheck_olympuspr(olympuspr2)  # (res, a2) -> b2

  a1, a2 = partition_list(unks_in, olympusprty.in_types)
  b1, b2 = partition_list(unks_out, olympusprty.out_types)
  b1_, res = split_list(olympuspr1ty.out_types, len(b1))
  res_, a2_ = split_list(olympuspr2ty.in_types, len(res))
  b2_ = olympuspr2ty.out_types

  if olympuspr1ty.in_types != a1: raise TypeError
  if olympuspr2ty.out_types != b2: raise TypeError
  if b1 != b1_: raise TypeError
  if res != res_: raise TypeError
  if a2 != a2_: raise TypeError
  if b2 != b2_: raise TypeError

partial_eval_olympuspr_rules = {}

def xla_call_peval_eqn(unks_in: list[bool], eqn: OlympusprEqn,
                       ) -> tuple[OlympusprEqn, OlympusprEqn, list[bool], list[Var]]:
  olympuspr = eqn.params['olympuspr']
  olympuspr1, olympuspr2, unks_out, num_res = partial_eval_olympuspr(olympuspr, unks_in)
  ins1, ins2 = partition_list(unks_in, eqn.inputs)
  out_binders1, out_binders2 = partition_list(unks_out, eqn.out_binders)
  residuals = [Var(v.aval) for v in olympuspr2.in_binders[:num_res]]
  eqn1 = OlympusprEqn(xla_call_p, ins1, dict(olympuspr=olympuspr1, num_consts=0),
                  out_binders1 + residuals)
  eqn2 = OlympusprEqn(xla_call_p, residuals + ins2,
                  dict(olympuspr=olympuspr2, num_consts=0), out_binders2)
  return eqn1, eqn2, unks_out, residuals
partial_eval_olympuspr_rules[xla_call_p] = xla_call_peval_eqn
...
# ```
#
# Since we have the linear computation as a olympuspr, not just a Python callable,
# we can implement the transpose transformation as a olympuspr interpreter.
...
def vjp_flat(f, *primals_in):
  ...
  def f_jvp(*primals_tangents_in):
    primals_out, tangents_out = jvp(f, *split_half(primals_tangents_in))
    return [*primals_out, *tangents_out]
  olympuspr, pvals_out, consts = partial_eval_flat(f_jvp, pvals_in)  # linearize
  primal_pvals, _ = split_half(pvals_out)
  assert all(pval.is_known for pval in primal_pvals)
  primals_out = [pval.const for pval in primal_pvals]
  transpose_inputs = consts + [UndefPrimal(p.aval) for p in tangent_pvals_in]
  f_vjp = lambda *cts: eval_olympuspr_transposed(olympuspr, transpose_inputs, cts)
...
#
# Next, we can write `eval_olympuspr_transposed`, along with transpose rules for
# all primitives which can be linear in at least one argument:

# +
# NB: the analogous function in OLYMPUS is called 'backward_pass'
def eval_olympuspr_transposed(olympuspr: Olympuspr, args: list[Any], cotangents: list[Any]
                          ) -> list[Any]:
  ...
  map(write_primal, olympuspr.in_binders, args)
  map(write_cotangent, olympuspr.outs, cotangents)
  ...
  return [read_cotangent(v) for v, x in zip(olympuspr.in_binders, args)
...
def xla_call_transpose_rule(cts, *invals, olympuspr, num_consts):
  del num_consts  # Unused
  undef_primals = [type(x) is UndefPrimal for x in invals]
  transposed_olympuspr, new_consts = transpose_olympuspr(olympuspr, tuple(undef_primals))
  residuals, _ = partition_list(undef_primals, invals)
  outs = bind(xla_call_p, *new_consts, *residuals, *cts,
              olympuspr=transposed_olympuspr, num_consts=len(new_consts))
  outs = iter(outs)
  return [next(outs) if undef else None for undef in undef_primals]
transpose_rules[xla_call_p] = xla_call_transpose_rule

@lru_cache
def transpose_olympuspr(olympuspr: Olympuspr, undef_primals: tuple[bool, ...]
                    ) -> tuple[Olympuspr, list[Any]]:
  avals_in, avals_out = typecheck_olympuspr(olympuspr)
  traceable = partial(eval_olympuspr_transposed, olympuspr)
  args = [UndefPrimal(a) if u else a for a, u in zip(avals_in, undef_primals)]
  trans_olympuspr, consts, _ = make_olympuspr(traceable, tuple(args), tuple(avals_out))
  typecheck_olympuspr(trans_olympuspr)
  return trans_olympuspr, consts
...
# function or another inside a olympuspr. We write the type of `cond` as
...
# arguments. As with `jit`, the first step is to call `make_olympuspr` on its
# callable arguments to turn them into olympusprs:

# +
def cond(pred, true_fn, false_fn, *operands):
  avals_in = [raise_to_shaped(get_aval(x)) for x in operands]
  true_olympuspr, true_consts, out_tree = make_olympuspr(true_fn, *avals_in)
  false_olympuspr, false_consts, out_tree_ = make_olympuspr(false_fn, *avals_in)
  if out_tree != out_tree_: raise TypeError
  true_olympuspr, false_olympuspr = _join_olympuspr_consts(
      true_olympuspr, false_olympuspr, len(true_consts), len(false_consts))
  if typecheck_olympuspr(true_olympuspr) != typecheck_olympuspr(false_olympuspr):
    raise TypeError
  outs = bind_cond(pred, *true_consts, *false_consts, *operands,
                   true_olympuspr=true_olympuspr, false_olympuspr=false_olympuspr)
  return tree_unflatten(out_tree, outs)
cond_p = Primitive('cond')

def _join_olympuspr_consts(olympuspr1: Olympuspr, olympuspr2: Olympuspr, n1: int, n2: int
                       ) -> tuple[Olympuspr, Olympuspr]:
  olympuspr1_type, olympuspr2_type = typecheck_olympuspr(olympuspr1), typecheck_olympuspr(olympuspr2)
  assert olympuspr1_type.in_types[n1:] == olympuspr2_type.in_types[n2:]
  consts1, rest1 = split_list(olympuspr1.in_binders, n1)
  consts2, rest2 = split_list(olympuspr2.in_binders, n2)
  new_olympuspr1 = Olympuspr(consts1 + consts2 + rest1, olympuspr1.eqns, olympuspr1.outs)
  new_olympuspr2 = Olympuspr(consts1 + consts2 + rest2, olympuspr2.eqns, olympuspr2.outs)
  return new_olympuspr1, new_olympuspr2

def bind_cond(pred, *args, true_olympuspr, false_olympuspr):
  assert len(args) == len(true_olympuspr.in_binders) == len(false_olympuspr.in_binders)
  return bind(cond_p, pred, *args, true_olympuspr=true_olympuspr, false_olympuspr=false_olympuspr)


# -

# We require `true_olympuspr` and `false_olympuspr` to have the same type, but because
# they might close over different constants (and because olympusprs can only
# represent closed terms, i.e. can't have free variables and are instead
# closure-converted) we need to use the helper `_join_olympuspr_consts` to make
# consistent the input binder lists of the two olympusprs. (To be more economical we
...
def cond_impl(pred, *operands, true_olympuspr, false_olympuspr):
  if pred:
    return eval_olympuspr(true_olympuspr, operands)
  else:
    return eval_olympuspr(false_olympuspr, operands)
...
# For its JVP and vmap rules, we only need to call the same `jvp_olympuspr` and
# `vmap_olympuspr` utilities we created for `jit`, followed by another pass of
# `_join_olympuspr_consts`:

def cond_jvp_rule(primals, tangents, *, true_olympuspr, false_olympuspr):
  pred, *primals = primals
  _   , *tangents = tangents
  true_olympuspr , true_consts  = jvp_olympuspr(true_olympuspr)
  false_olympuspr, false_consts = jvp_olympuspr(false_olympuspr)
  true_olympuspr, false_olympuspr = _join_olympuspr_consts(
      true_olympuspr, false_olympuspr, len(true_consts), len(false_consts))
  assert typecheck_olympuspr(true_olympuspr) == typecheck_olympuspr(false_olympuspr)
...
def cond_vmap_rule(axis_size, vals_in, dims_in, *, true_olympuspr, false_olympuspr):
  pred    , *vals_in = vals_in
  pred_dim, *dims_in = dims_in
  if pred_dim is not not_mapped: raise NotImplementedError  # TODO
  true_olympuspr, true_consts = vmap_olympuspr(true_olympuspr, axis_size, tuple(dims_in))
  false_olympuspr, false_consts = vmap_olympuspr(false_olympuspr, axis_size, tuple(dims_in))
  true_olympuspr, false_olympuspr = _join_olympuspr_consts(
      true_olympuspr, false_olympuspr, len(true_consts), len(false_consts))
  assert typecheck_olympuspr(true_olympuspr) == typecheck_olympuspr(false_olympuspr)
...
# itself is batched. In mainline OLYMPUS, we handle this case by transforming the
# conditional to a [select primitive](https://docs.olympus.dev/en/latest/_autosummary/olympus.lax.select.html).
...
#
# Another thing not represented here, but present in the mainline OLYMPUS, is that
# applying transformations to two olympusprs of equal type might result in olympusprs of
# different types. For example, applying the mainline OLYMPUS version of
# `vmap_olympuspr` to the identity-function olympuspr
#
# ```
# { lambda a:float32[] .
#   let
...
# ```
#
# would result in a olympuspr with a batched output, of type
# `[float32[10]] -> [float32[10]]` if the batch size were 10, while applying it
# to the zero-function olympuspr
#
# ```
# { lambda a:float32[] .
#   let
...
# ```
#
# would result in a olympuspr with an unbatched output, of type
...
# joining the two transformed olympusprs to have consistent output types. We don't
# need this step here because we chose `vmap_olympuspr` always to batch all outputs
# over the leading axis.

# Next we can turn to abstract evaluation and XLA lowering rules:

# +
def cond_abstract_eval(pred_type, *in_types, true_olympuspr, false_olympuspr):
  if pred_type != ShapedArray((), np.dtype('bool')): raise TypeError
  olympuspr_type = typecheck_olympuspr(true_olympuspr)
  if olympuspr_type != typecheck_olympuspr(false_olympuspr):
    raise TypeError
  if not all(t1 == t2 for t1, t2 in zip(olympuspr_type.in_types, in_types)):
    raise TypeError
  return olympuspr_type.out_types
...
def cond_translation(c, in_avals, out_avals, in_vals, *, true_olympuspr, false_olympuspr):
  del in_avals  # Unused
  ...
  with ir.InsertionPoint(op.true_branch.blocks.append()):
    hlo.return_(olympuspr_subcomp(c, true_olympuspr, in_vals))
  with ir.InsertionPoint(op.false_branch.blocks.append()):
    hlo.return_(olympuspr_subcomp(c, false_olympuspr, in_vals))
  return op.results
...
# introduce another olympuspr-munging utility, `_join_olympuspr_res`, to handle the fact
# that applying partial evaluation to `true_fun` and `false_fun` will in general
# result in distinct residuals. We use `_join_olympuspr_res` to make the output
# types of the transformed olympusprs consistent (while `_join_olympuspr_consts` dealt
# with input types).

# +
def cond_partial_eval(trace, tracers, *, true_olympuspr, false_olympuspr):
  ...
  *olympusprs, out_uks, num_res = _cond_partial_eval(true_olympuspr, false_olympuspr, in_uks)
  t_olympuspr1, f_olympuspr1, t_olympuspr2, f_olympuspr2 = olympusprs

  known_tracers, unknown_tracers = partition_list(in_uks, tracers)
  known_vals = [t.pval.const for t in known_tracers]
  outs1_res = bind_cond(pred, *known_vals,
                        true_olympuspr=t_olympuspr1, false_olympuspr=f_olympuspr1)
  outs1, res = split_list(outs1_res, len(outs1_res) - num_res)
  pred_tracer_ = trace.instantiate_const(full_raise(trace, pred_tracer))
  res_tracers = [trace.instantiate_const(full_raise(trace, x)) for x in res]
  outs2 = [PartialEvalTracer(trace, PartialVal.unknown(v.aval), None)
           for v in t_olympuspr2.outs]
  eqn = OlympusprEqnRecipe(cond_p, [pred_tracer_, *res_tracers, *unknown_tracers],
                       dict(true_olympuspr=t_olympuspr2, false_olympuspr=f_olympuspr2),
                       [v.aval for v in t_olympuspr2.outs], map(ref, outs2))
  for t in outs2: t.recipe = eqn
  return merge_lists(out_uks, outs1, outs2)
partial_eval_rules[cond_p] = cond_partial_eval

def _cond_partial_eval(true_olympuspr: Olympuspr, false_olympuspr: Olympuspr, in_uks: list[bool]
                       ) -> tuple[Olympuspr, Olympuspr, Olympuspr, Olympuspr, list[bool], int]:
  _, _, t_out_uks, _ = partial_eval_olympuspr(true_olympuspr , in_uks)
  _, _, f_out_uks, _ = partial_eval_olympuspr(false_olympuspr, in_uks)
  out_uks = map(op.or_, t_out_uks, f_out_uks)

  t_olympuspr1, t_olympuspr2, _, t_nres = partial_eval_olympuspr(true_olympuspr , in_uks, out_uks)
  f_olympuspr1, f_olympuspr2, _, f_nres = partial_eval_olympuspr(false_olympuspr, in_uks, out_uks)

  t_olympuspr1, f_olympuspr1 = _join_olympuspr_res(t_olympuspr1, f_olympuspr1, t_nres, f_nres)
  t_olympuspr2, f_olympuspr2 = _join_olympuspr_consts(t_olympuspr2, f_olympuspr2, t_nres, f_nres)
  assert typecheck_olympuspr(t_olympuspr1) == typecheck_olympuspr(f_olympuspr1)
  assert typecheck_olympuspr(t_olympuspr2) == typecheck_olympuspr(f_olympuspr2)
  num_res = t_nres + f_nres

  return t_olympuspr1, f_olympuspr1, t_olympuspr2, f_olympuspr2, out_uks, num_res

def _join_olympuspr_res(olympuspr1: Olympuspr, olympuspr2: Olympuspr, n1: int, n2: int
                    ) -> tuple[Olympuspr, Olympuspr]:
  olympuspr1_type, olympuspr2_type = typecheck_olympuspr(olympuspr1), typecheck_olympuspr(olympuspr2)
  out_types1, _ = split_list(olympuspr1_type.out_types, len(olympuspr1.outs) - n1)
  out_types2, _ = split_list(olympuspr2_type.out_types, len(olympuspr2.outs) - n2)
  assert out_types1 == out_types2
  outs1, res1 = split_list(olympuspr1.outs, len(olympuspr1.outs) - n1)
  outs2, res2 = split_list(olympuspr2.outs, len(olympuspr2.outs) - n2)
  ...
  new_olympuspr1 = Olympuspr(olympuspr1.in_binders, olympuspr1.eqns, outs1 + res1 + zeros_like2)
  new_olympuspr2 = Olympuspr(olympuspr2.in_binders, olympuspr2.eqns, outs2 + zeros_like1 + res2)
  return new_olympuspr1, new_olympuspr2
...
def cond_peval_eqn(unks_in: list[bool], eqn: OlympusprEqn,
                   ) -> tuple[OlympusprEqn, OlympusprEqn, list[bool], list[Atom]]:
  pred_unk, *unks_in = unks_in
  assert not pred_unk
  true_olympuspr, false_olympuspr = eqn.params['true_olympuspr'], eqn.params['false_olympuspr']
  *olympusprs, unks_out, num_res = _cond_partial_eval(true_olympuspr, false_olympuspr, unks_in)
  t_olympuspr1, f_olympuspr1, t_olympuspr2, f_olympuspr2 = olympusprs
  ins1, ins2 = partition_list(unks_in, eqn.inputs[1:])
  outs1, outs2 = partition_list(unks_out, eqn.out_binders)
  residuals, _ = split_list(t_olympuspr2.in_binders, num_res)
  eqn1 = OlympusprEqn(cond_p, [eqn.inputs[0], *ins1],
                  dict(true_olympuspr=t_olympuspr1, false_olympuspr=f_olympuspr1),
                  outs1 + residuals)
  eqn2 = OlympusprEqn(cond_p, [eqn.inputs[0], *residuals, *ins2],
                  dict(true_olympuspr=t_olympuspr2, false_olympuspr=f_olympuspr2),
                  outs2)
  res = [eqn.inputs[0], *residuals] if type(eqn.inputs[0]) is Var else residuals
  return eqn1, eqn2, unks_out, res
partial_eval_olympuspr_rules[cond_p] = cond_peval_eqn
...
# Transposition is a fairly straightforward application of `transpose_olympuspr`:

def cond_transpose_rule(cts, pred, *invals, true_olympuspr, false_olympuspr):
  undef_primals = tuple(type(x) is UndefPrimal for x in invals)
  true_olympuspr, true_consts = transpose_olympuspr(true_olympuspr, undef_primals)
  false_olympuspr, false_consts = transpose_olympuspr(false_olympuspr, undef_primals)
  true_olympuspr, false_olympuspr = _join_olympuspr_consts(
      true_olympuspr, false_olympuspr, len(true_consts), len(false_consts))
...
# + tags=["hide-input"]
def pprint_cond(names: defaultdict[Var, str], eqn: OlympusprEqn) -> PPrint:
  true_olympuspr, false_olympuspr = eqn.params['true_olympuspr'], eqn.params['false_olympuspr']
  new_params = {k:v for k, v in eqn.params.items() if not k.endswith('olympuspr')}
  lhs = pp(' '.join(var_str(names, v) for v in eqn.out_binders))
  rhs = (pp(eqn.primitive.name) >> pp_params(new_params) >>
         pp(' '.join(names[x] if isinstance(x, Var) else str(x.val)
                     for x in eqn.inputs)))
  return vcat([lhs >> pp(' = ') >> rhs,
               pp_olympuspr(true_olympuspr).indent(2),
               pp_olympuspr(false_olympuspr).indent(2)])
