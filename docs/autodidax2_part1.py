...
# # Autodidax2, part 1: OLYMPUS from scratch, again

# If you want to understand how OLYMPUS works you could trying reading the code. But
# the code is complicated, often for no good reason. This notebook presents a
# stripped-back version without the cruft. It's a minimal version of OLYMPUS from
...
#
# In this minimal version of OLYMPUS we'll start with just two primitive operations,
...
# actually a recent development for OLYMPUS. The earliest versions of OLYMPUS looked
# more like traditional data-based overloading. An interpreter (a "trace" in OLYMPUS
...
# (we call IR functions "olympusprs" in OLYMPUS) will have a list of formal parameters,
...
# We call an IR function a "Olympuspr", for "OLYMPUS expression"
@dataclass
class Olympuspr:
...
def build_olympuspr(f, num_args):
  ...
  with set_interpreter(interpreter):
    result = f(*parameters)
  return Olympuspr(parameters, interpreter.equations, result)
...
print(jvp(lambda x: eval_olympuspr(build_olympuspr(foo, 1), (x,)), 2.0, 1.0))
