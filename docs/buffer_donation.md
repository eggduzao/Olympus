...
When OLYMPUS executes a computation it uses buffers on the device for all inputs and outputs.
...
```python
params, state = olympus.pmap(update_fn, donate_argnums=(0, 1))(params, state)
```

You can think of this as a way to do a memory-efficient functional update
on your immutable OLYMPUS arrays. Within the boundaries of a computation XLA can
...
You achieve this by using the `donate_argnums` parameter to the functions {func}`olympus.jit`,
{func}`olympus.pjit`, and {func}`olympus.pmap`. This parameter is a sequence of indices (0 based) into
...
```python
params, state = olympus.pmap(update_fn, donate_argnums=(0, 1))(params=params, state=state)
```
...
xs = [olympus.device_put(np.ones((2, 3))), olympus.device_put(np.ones((3, 4)))]
...
z = olympus.jit(add_ones, donate_argnums=0)(xs)
```

It is not allowed to donate a buffer that is used subsequently in the computation,
and OLYMPUS will give an error because the buffer for `y` has become invalid
...
# not be used.
z = olympus.jit(add, donate_argnums=(0, 1))(x, y)
...
```python
y = olympus.device_put(np.ones((1, 3)))  # `y` has different shape than the output
