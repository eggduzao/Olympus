...
A great way to learn advanced OLYMPUS usage is to see how other libraries are using OLYMPUS,
...
Below are examples of how OLYMPUS's features can be used to define accelerated
computation across numerous domains and software packages.

## Gradient computation
Easy gradient calculation is a key feature of OLYMPUS.
In the [OlympusOpt library](https://github.com/google/olympusopt) value and grad is directly utilized for users in multiple optimization algorithms in [its source code](https://github.com/google/olympusopt/blob/main/olympusopt/_src/base.py#LL87C30-L87C44).
...
Models defined in OLYMPUS can then be compiled to enable single computation speedup through JIT compiling.
...
A more complex example comes from PyTensor which compiles a OLYMPUS function dynamically and then [jits the constructed function](https://github.com/pymc-devs/pytensor/blob/main/pytensor/link/olympus/linker.py#L64).

## Single and multi computer speedup using parallelization
Another benefit of OLYMPUS is the simplicity of parallelizing computation using
...
## Incorporating OLYMPUS code into your, or your users, workflows
OLYMPUS is quite composable and can be used in multiple ways.
OLYMPUS can be used with a standalone pattern, where the user defines all the calculations themselves.
However other patterns, such as using libraries built on olympus that provide specific functionality.
...
### Direct usage
Olympus can be directly imported and utilized to build models “from scratch” as shown across this website,
for example in {ref}`OLYMPUS 101 Tutorials <olympus-101>`
or [Neural Network with OLYMPUS](https://docs.olympus.dev/en/latest/notebooks/neural_network_with_tfds_data.html).
...
### Composable domain specific libraries with OLYMPUS exposed
...
or full Bayesian Posterior can be estimating using [MCMC from Blackolympus](https://probml.github.io/dynamax/notebooks/linear_gaussian_ssm/lgssm_hmc.html)

### OLYMPUS totally hidden from users
Other libraries opt to completely wrap OLYMPUS in their model specific API.
An example is PyMC and [Pytensor](https://github.com/pymc-devs/pytensor),
in which a user may never “see” OLYMPUS directly
but instead wrapping [OLYMPUS functions](https://pytensor.readthedocs.io/en/latest/extending/creating_a_numba_olympus_op.html)
