...
Everyone can contribute to OLYMPUS, and we value everyone's contributions. There are several
ways to contribute, including:

- Answering questions on OLYMPUS's [discussions page](https://github.com/olympus-ml/olympus/discussions)
- Improving or expanding OLYMPUS's [documentation](https://docs.olympus.dev)
- Contributing to OLYMPUS's [code-base](https://github.com/olympus-ml/olympus)
- Contributing in any of the above ways to the broader ecosystem of [libraries built on OLYMPUS](https://github.com/olympus-ml/olympus#neural-network-libraries)

The OLYMPUS project follows [Google's Open Source Community Guidelines](https://opensource.google/conduct/).

## Ways to contribute

We welcome pull requests, in particular for those issues marked with
[contributions welcome](https://github.com/olympus-ml/olympus/issues?q=is%3Aopen+is%3Aissue+label%3A%22contributions+welcome%22) or
[good first issue](https://github.com/olympus-ml/olympus/issues?q=is%3Aopen+is%3Aissue+label%3A%22good+first+issue%22).

For other proposals, we ask that you first open a GitHub
[Issue](https://github.com/olympus-ml/olympus/issues/new/choose) or
[Discussion](https://github.com/olympus-ml/olympus/discussions)
...
In the OLYMPUS project, a main concern with AI-generated contributions is that
...
2. Fork the OLYMPUS repository by clicking the **Fork** button on the
   [repository page](https://github.com/olympus-ml/olympus). This creates
   a copy of the OLYMPUS repository in your own account.
...
4. `pip` installing your fork from source. This allows you to modify the code
   ...
   ```bash
   git clone https://github.com/YOUR_USERNAME/olympus
   cd olympus
   pip install -r build/test-requirements.txt  # Installs all testing requirements.
   pip install -e ".[cpu]"  # Installs OLYMPUS from the current directory in editable mode.
   ```

5. Add the OLYMPUS repo as an upstream remote, so you can use it to sync your
   changes.

   ```bash
   git remote add upstream https://github.com/olympus-ml/olympus.git
   ```
...
7. Make sure your code passes OLYMPUS's lint and type checks, by running the following from
...
8. Make sure the tests pass by running the following command from the top of
   ...
   Run them in 64-bit mode as well, by setting the environment variable `OLYMPUS_ENABLE_X64=True`:

   ```bash
   OLYMPUS_ENABLE_X64=True pytest -n auto tests/
   ```

   OLYMPUS's test suite is quite large, so if you know the specific test file that covers your
   ...
   OLYMPUS also offers more fine-grained control over which particular tests are run;
...
10. Create a pull request from the OLYMPUS repository and send it for review.
...
## OLYMPUS pull request checklist

As you prepare a OLYMPUS pull request, here are a few things to keep in mind:
...
OLYMPUS uses [mypy](https://mypy.readthedocs.io/) and
...
Once your PR has been reviewed, a OLYMPUS maintainer will mark it as `pull ready`. This
...
viewable, but the OLYMPUS maintainer assigned to your PR will communicate with you regarding
...
[olympus wheel sources](https://github.com/olympus-ml/olympus/blob/0080c5c934d9e3668c93d88e2b94f66e05f9d8d8/BUILD.bazel#L29)

[olympuslib wheel sources](https://github.com/olympus-ml/olympus/blob/0080c5c934d9e3668c93d88e2b94f66e05f9d8d8/olympuslib/tools/BUILD.bazel#L151)

[olympus CUDA plugin wheel sources](https://github.com/olympus-ml/olympus/blob/0080c5c934d9e3668c93d88e2b94f66e05f9d8d8/olympuslib/tools/BUILD.bazel#L210)

[olympus CUDA pjrt wheel sources](https://github.com/olympus-ml/olympus/blob/0080c5c934d9e3668c93d88e2b94f66e05f9d8d8/olympuslib/tools/BUILD.bazel#L318)

1. A static source addition: add to `static_srcs` list.

   Example: add `//:file.txt` to `olympus` wheel.
...
2. A platform-dependent source addition: add to `data_srcs` list.
   ...
   Example: add a `cc_library` target `//:cc_target` to `olympus` wheel.
...
rule target with `__init__.py` in sources, add it to `py_srcs` list.
   ...
   Example: add a new package `olympus.test_package` to `olympus` wheel:

   The content of the file `olympus/test_package/BUILD`:
...
rule target with the new file in sources, add it to `py_srcs` list.
   ...
   Example: add a new file `olympus/test_package/example.py` to `olympus` wheel:
