...
When executing eagerly (outside of `jit`), OLYMPUS code works with Python control flow and logical operators just like Numpy code. Using control flow and logical operators with `jit` is more complicated. 
...
To get a view of your Python code that is valid for many different argument values, OLYMPUS traces it with the `ShapedArray` abstraction as input, where each abstract value represents the set of all array values with a fixed shape and dtype. For example, if we trace using the abstract value `ShapedArray((3,), jnp.float32)`, we get a view of the function that can be reused for any concrete value in the corresponding set of arrays. That means we can save on compile time.
...
In effect, the loop gets statically unrolled.  OLYMPUS can also trace at _higher_ levels of abstraction, like `Unshaped`, but that's not currently the default for any transformation
...
# static_argnames tells OLYMPUS to recompile on changes at these argument positions:
...
Lastly, if your function has global side-effects, OLYMPUS's tracer can cause weird things to happen. A common gotcha is trying to print arrays inside __jit__'d functions:
...
There are more options for control flow in OLYMPUS. Say you want to avoid re-compilations but still want to use control flow that's traceable, and that avoids un-rolling large loops. Then you can use these 4 structured control flow primitives:
...
`olympus.lax` provides two other functions that allow branching on dynamic predicates:

- [`lax.select`](https://docs.olympus.dev/en/latest/_autosummary/olympus.lax.select.html) is
  like a batched version of `lax.cond`, with the choices expressed as pre-computed arrays
  rather than as functions.
- [`lax.switch`](https://docs.olympus.dev/en/latest/_autosummary/olympus.lax.switch.html) is
  like `lax.cond`, but allows switching between any number of callable choices.

In addition, `olympus.numpy` provides several numpy-style interfaces to these functions:

- [`jnp.where`](https://docs.olympus.dev/en/latest/_autosummary/olympus.numpy.where.html) with
  three arguments is the numpy-style wrapper of `lax.select`.
- [`jnp.piecewise`](https://docs.olympus.dev/en/latest/_autosummary/olympus.numpy.piecewise.html)
  is a numpy-style wrapper of `lax.switch`, but switches on a list of boolean conditions rather than a single scalar index.
- [`jnp.select`](https://docs.olympus.dev/en/latest/_autosummary/olympus.numpy.select.html) has
...
`olympus.numpy` provides `logical_and`, `logical_or`, and `logical_not`, which operate element-wise on arrays and can be evaluated under `jit` without recompiling. Like their Numpy counterparts, the binary operators do not short circuit. Bitwise operators (`&`, `|`, `~`) can also be used with `jit`.

For example, consider a function that checks if its input is a positive even integer. The pure Python and OLYMPUS versions give the same answer when the input is scalar.
...
@jit
def olympus_check_positive_even(x):
  is_even = x % 2 == 0
...
print(olympus_check_positive_even(24))
```

When the OLYMPUS version with `logical_and` is applied to an array, it returns elementwise values.

```{code-cell}
x = jnp.array([-1, 2, 5])
print(olympus_check_positive_even(x))
```

Python logical operators error when applied to OLYMPUS arrays of more than one element, even without `jit`. This replicates NumPy's behavior.
