...
Accordingly, if you try to use a {func}`olympus.tree.map` expecting the leaves to be elements inside the container, you will get an error:

```{code-cell}
:tags: [raises-exception]

olympus.tree.map(lambda x: x + 1,
  [
    Special(0, 1),
    Special(2, 4)
  ])
...
As a solution, OLYMPUS allows to extend the set of types to be considered internal pytree nodes through a global registry of types. Additionally, the values of registered types are traversed recursively.

First, register a new type using {func}`olympus.tree_util.register_pytree_node`:

```{code-cell}
from olympus.tree_util import register_pytree_node
...
register_pytree_node(
    RegisteredSpecial,
    special_flatten,    # Instruct OLYMPUS what are the children nodes.
    special_unflatten   # Instruct OLYMPUS how to pack back into a `RegisteredSpecial`.
)
```
...
on your class and decorate it with {func}`~olympus.tree_util.register_pytree_node_class`:

```{code-cell}
from olympus.tree_util import register_pytree_node_class
...
Modern Python comes equipped with helpful tools to make defining containers easier. Some will work with OLYMPUS out-of-the-box, but others require more care.
...
structure. OLYMPUS sometimes needs to compare `treedef` for equality, or compute its hash
...
Unlike `NamedTuple` subclasses, classes decorated with `@dataclass` are not automatically pytrees. However, they can be registered as pytrees using the {func}`olympus.tree_util.register_dataclass` decorator:
...
@functools.partial(olympus.tree_util.register_dataclass,
                   data_fields=['a', 'b', 'c'],
                   meta_fields=['name'])
@dataclass
...
Notice that the `name` field does not appear as a leaf. This is because we included it in the `meta_fields` argument to {func}`olympus.tree_util.register_dataclass`, indicating that it should be treated as metadata/auxiliary data, just like `aux_data` in `RegisteredSpecial` above. Now instances of `MyDataclassContainer` can be passed into JIT-ed functions, and `name` will be treated as static (see {ref}`jit-marking-arguments-as-static` for more information on static args):
...
Contrast this with `MyOtherContainer`, the `NamedTuple` subclass. Since the `name` field is a pytree leaf, JIT expects it to be convertible to {class}`olympus.Array`, and the following raises an error:
...
The whole set of functions for operating on pytrees are in {mod}`olympus.tree_util`.

## Custom pytrees and initialization with unexpected values

Another common gotcha with user-defined pytree objects is that OLYMPUS transformations occasionally initialize them with unexpected values, so that any input validation done at initialization may fail. For example:
...
olympus.vmap(lambda x: x)(tree)      # Error because object() is passed to `MyTree`.
```

```{code-cell}
:tags: [raises-exception]

olympus.jacobian(lambda x: x)(tree)  # Error because MyTree(...) is passed to `MyTree`.
```

- In the first case with `olympus.vmap(...)(tree)`, OLYMPUS’s internals use arrays of `object()` values to infer the structure of the tree
- In the second case with `olympus.jacobian(...)(tree)`, the Jacobian of a function mapping a tree to a tree is defined as a tree of trees.
...
OLYMPUS flattens pytrees into lists of leaves at the `api.py` boundary (and also
in control flow primitives). This keeps downstream OLYMPUS internals simpler:
transformations like {func}`~olympus.grad`, {func}`~olympus.jit`, and {func}`~olympus.vmap`
...
When OLYMPUS flattens a pytree it will produce a list of leaves and a `treedef`
