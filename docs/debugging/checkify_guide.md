...
err, z = olympus.jit(jittable_f)(jnp.ones((5,)), -2)
print(err.get())
...
```python
olympus.jit(f)(jnp.ones((5,)), -1)  # checkify transformation not used
...
```python
err, z = olympus.pmap(checked_f)(jnp.ones((3, 5)), jnp.array([-1, 2, 100]))
err.throw()
"""
ValueError:
...
## Why does OLYMPUS need checkify?

Under some OLYMPUS transformations you can express runtime error checks with ordinary Python assertions, for example when only using `olympus.grad` and `olympus.numpy`:
...
OLYMPUS transformation semantics rely on functional purity, especially when composing multiple transformations, so how can we provide an error mechanism without disrupting all that?
Beyond needing a new API, the situation is trickier still:
XLA HLO doesn't support assertions or throwing errors, so even if we had a OLYMPUS API which was able to stage out assertions, how would we lower these assertions to XLA?
...
err, y = olympus.jit(f_checked)(0.)
if err:
  raise ValueError("must be positive!")
# ValueError: "must be positive!"
```

The error is a regular value computed by the function, and the error is raised outside of `f_checked`. `f_checked` is functionally pure, so we know by construction that it'll already work with `jit`, pmap, pjit, scan, and all of OLYMPUS's transformations. The only problem is that this plumbing can be a pain!
...
err, x = olympus.jit(f_checked)(-1.)
err.throw()
# ValueError: -1. must be positive! (check failed at <...>:2 (f))
```

We call this functionalizing or discharging the effect introduced by calling check. (In the "manual" example above the error value is just a boolean. checkify's error values are conceptually similar but also track error messages and expose throw and get methods; see {mod}`olympus.experimental.checkify`). `checkify.check` also allows you to add run-time values to your error message by providing them as format arguments to the error message.
...
The API for selecting which automatic checks to enable is based on Sets. See {mod}`olympus.experimental.checkify` for more details.

## `checkify` under OLYMPUS transformations.

As demonstrated in the examples above, a checkified function can be happily
jitted. Here's a few more examples of `checkify` with other  OLYMPUS
transformations. Note that checkified functions are functionally pure, and
should trivially compose with all OLYMPUS transformations!

### `jit`

You can safely add `olympus.jit` to a checkified function, or `checkify` a jitted
...
checkify_of_jit = checkify.checkify(olympus.jit(f))
jit_of_checkify = olympus.jit(checkify.checkify(f))
...
errs, out = olympus.vmap(checked_f)(jnp.ones((3, 5)), jnp.array([-1, 2, 100]))
errs.throw()
"""
ValueError:
...
with olympus.sharding.Mesh(mesh.devices, mesh.axis_names):
...
olympus.grad(assert_gradient_negative)(-1.)
...
## Strengths and limitations of `olympus.experimental.checkify`
### Strengths
...
* Throwing an error value will materialize that error value on the host, meaning
  it's a blocking operation which defeats OLYMPUS's async run-ahead.
