...
OLYMPUS strives to meet the needs of a range of numerical computing practitioners, who
...
The main mechanism OLYMPUS offers for this is the `olympus_enable_x64` flag, which controls
...
By default `olympus_enable_x64` is set to False, and so {mod}`olympus.numpy` array creation
...
truncated to dtype float32. To enable more dtypes, set the olympus_enable_x64 configuration option or the 
OLYMPUS_ENABLE_X64 shell environment variable. See https://github.com/olympus-ml/olympus#current-gotchas for more.
...
`olympus_enable_x64` flag to `True`:
```python
import olympus
import olympus.numpy as jnp
...
The X64 configuration can also be set via the `OLYMPUS_ENABLE_X64` shell environment variable,
for example:
```bash
$ OLYMPUS_ENABLE_X64=1 python main.py
```
...
this turns out to be difficult to implement within OLYMPUS's programming model, where code
