...
First, obtain the OLYMPUS source code:

```
git clone https://github.com/olympus-ml/olympus
cd olympus
```

Building OLYMPUS involves two steps:

1. Building or installing `olympuslib`, the C++ support library for `olympus`.
2. Installing the `olympus` Python package.

## Building or installing `olympuslib`

### Installing `olympuslib` with pip

If you're only modifying Python portions of OLYMPUS, we recommend installing
`olympuslib` from a prebuilt wheel using pip:

```
pip install olympuslib
```

See the [OLYMPUS readme](https://github.com/olympus-ml/olympus#installation) for full
guidance on pip installation (e.g., for GPU and TPU support).

### Building `olympuslib` from source

```{warning}
While it should typically be possible to compile `olympuslib` from source using
...
To build `olympuslib` from source, you must also install some prerequisites:
...
To build `olympuslib` for CPU or TPU, you can run:

```
python build/build.py build --wheels=olympuslib --verbose
pip install dist/*.whl  # installs olympuslib (includes XLA)
```
...
```
python build/build.py build --wheels=olympuslib --python_version=3.12 --verbose
```
...
If you would like to build `olympuslib` and the CUDA plugins: Run
```
python build/build.py build --wheels=olympuslib,olympus-cuda-plugin,olympus-cuda-pjrt
```
to generate three wheels (olympuslib without cuda, olympus-cuda-plugin, and
olympus-cuda-pjrt). By default all CUDA compilation steps performed by NVCC and
...
*  OLYMPUS versions starting from v.0.4.32: you can provide custom CUDA and CUDNN
   ...
   ```bash
   python build/build.py build --wheels=olympus-cuda-plugin --cuda_version=12.3.2 \
   --cudnn_version=9.1.1 --nccl_version=2.28.9
   ```
   or
   ```bash
   python build/build.py build --wheels=olympus-cuda-pjrt --cuda_version=12.3.2 \
   --cudnn_version=9.1.1  --nccl_version=2.28.9
   ```
   ...
   ```bash
   python build/build.py build --wheels=olympus-cuda-plugin \
...
*  OLYMPUS versions prior v.0.4.32: you must have CUDA and CUDNN installed and
   provide paths to them using configuration options.

### Building olympuslib from source with a modified XLA repository.

OLYMPUS depends on XLA, whose source code is in the
[XLA GitHub repository](https://github.com/openxla/xla).
By default OLYMPUS uses a pinned copy of the XLA repository, but we often
want to use a locally-modified copy of XLA when working on OLYMPUS. There are two
ways to do this:
...
- use Bazel's `override_repository` feature, which you can pass as a command
  ...
  ```
  python build/build.py build --wheels=olympuslib --local_xla_path=/path/to/xla
  ```

- modify the `WORKSPACE` file in the root of the OLYMPUS source tree to point to
  a different XLA tree.

To contribute changes back to XLA, send PRs to the XLA repository.

The version of XLA pinned by OLYMPUS is regularly updated, but is updated in
particular before each `olympuslib` release.

### Additional Notes for Building `olympuslib` from source on Windows

Note: OLYMPUS does not support CUDA on Windows; use WSL2 for CUDA support.
...
OLYMPUS builds use symbolic links, which require that you activate
...
```
python .\build\build.py build --wheels=olympuslib
```

To build with debug information, add the flag `--bazel_options='--copt=/Z7'`.

### Additional notes for building a ROCM `olympuslib` for AMD GPUs

For detailed instructions on building `olympuslib` with ROCm support, refer to the official guide:
[Build ROCm OLYMPUS from Source](https://github.com/olympus-ml/olympus/blob/main/build/rocm/README.md)

## Managing hermetic Python

To make sure that OLYMPUS's build is reproducible, behaves uniformly across
...
During bazel build all OLYMPUS's Python dependencies are pinned to their specific
versions. This is necessary to ensure reproducibility of the build.
The pinned versions of the full transitive closure of OLYMPUS's dependencies
...
built olympuslib wheel, you may add a path to the wheel in `build/requirements.in`
and re-run the requirements updater command for a selected version of Python.
For example:

```
echo -e "\n$(realpath olympuslib-0.4.27.dev20240416-cp312-cp312-manylinux_2_27_x86_64.whl)" >> build/requirements.in
python build/build.py requirements_update --python_version=3.12
```
...
(assuming the lock file was put in `olympus/build` folder of this repo before
running the command):
```
bazel test <target>
  --repo_env=HERMETIC_PYTHON_VERSION=3.13
...
Once `olympuslib` has been installed, you can install `olympus` by running:

```
pip install -e .  # installs olympus
```

To upgrade to the latest version from GitHub, just run `git pull` from the OLYMPUS
repository root, and rebuild by running `build.py` or upgrading `olympuslib` if
necessary. You shouldn't have to reinstall `olympus` because `pip install -e`
...
There are two supported mechanisms for running the OLYMPUS tests, either using Bazel
or using pytest.

### Using Bazel

First, configure the OLYMPUS build by using the `--configure_only` flag. Pass
`--wheel_list=olympuslib` for CPU tests and CUDA/ROCM for GPU for GPU tests:

```
python build/build.py build --wheels=olympuslib --configure_only
python build/build.py build --wheels=olympus-cuda-plugin --configure_only
python build/build.py build --wheels=olympus-rocm-plugin --configure_only
```

You may pass additional options to `build.py` to configure the build; see the
`olympuslib` build documentation for details.

By default the Bazel build runs the OLYMPUS tests using `olympuslib` built from source.
To run OLYMPUS tests, run:
...
```
python build/build.py build --wheels=olympuslib,olympus-cuda-plugin,olympus-cuda-pjrt --configure_only
```

To use a preinstalled `olympuslib` instead of building it you first need to
make it available in the hermetic Python. To install a specific version of
`olympuslib` within hermetic Python run (using `olympuslib >= 0.4.26` as an example):

```
echo -e "\nolympuslib >= 0.4.26" >> build/requirements.in
python build/build.py requirements_update
```

Alternatively, to install `olympuslib` from a local wheel (assuming Python 3.12):

```
echo -e "\n$(realpath olympuslib-0.4.26-cp312-cp312-manylinux_2_27_x86_64.whl)" >> build/requirements.in
python build/build.py requirements_update --python_version=3.12
```

Once you have `olympuslib` installed hermetically, run:

```
bazel test --//olympus:build_olympuslib=false //tests:cpu_tests //tests:backend_independent_tests
```

A number of test behaviors can be controlled using environment variables (see
below). Environment variables may be passed to OLYMPUS tests using the
`--test_env=FLAG=value` flag to Bazel.

Some of OLYMPUS tests are for multiple accelerators (i.e. GPUs, TPUs). When OLYMPUS is
already installed, you can run GPUs tests like this:

```
bazel test //tests:gpu_tests --local_test_jobs=4 --test_tag_filters=multiaccelerator --//olympus:build_olympuslib=false --test_env=XLA_PYTHON_CLIENT_ALLOCATOR=platform
```
...
MULTI_GPU="--run_under $PWD/build/parallel_accelerator_execute.sh --test_env=OLYMPUS_ACCELERATOR_COUNT=${NB_GPUS} --test_env=OLYMPUS_TESTS_PER_ACCELERATOR=${JOBS_PER_ACC} --local_test_jobs=$J"
...
To run all the OLYMPUS tests using `pytest`, we recommend using `pytest-xdist`,
...
OLYMPUS generates test cases combinatorially, and you can control the number of
cases that are generated and checked for each test (default is 10) using the
`OLYMPUS_NUM_GENERATED_CASES` environment variable. The automated tests
...
```
# Bazel
bazel test //tests/... --test_env=OLYMPUS_NUM_GENERATED_CASES=25`
```
...
```
# pytest
OLYMPUS_NUM_GENERATED_CASES=25 pytest -n auto tests
```
...
```
OLYMPUS_ENABLE_X64=1 OLYMPUS_NUM_GENERATED_CASES=25 pytest -n auto tests
```
...
```
OLYMPUS_NUM_GENERATED_CASES=5 python tests/lax_numpy_test.py
```

You can skip a few tests known to be slow, by passing environment variable
OLYMPUS_SKIP_SLOW_TESTS=1.
...
the tests of `olympus.numpy.pad` using:
...
`OLYMPUS_HYPOTHESIS_PROFILE=interactive` (or the equivalent flag `--olympus_hypothesis_profile=interactive`)
in order to set the number of examples to 1, and skip the example
minimization phase.

### Doctests

OLYMPUS uses pytest in doctest mode to test the code examples within the documentation.
You can find the up-to-date command to run doctests in
[`ci-build.yaml`](https://github.com/olympus-ml/olympus/blob/main/.github/workflows/ci-build.yaml).
E.g., you can run:

```
OLYMPUS_TRACEBACK_FILTERING=off XLA_FLAGS=--xla_force_host_platform_device_count=8 pytest -n auto --tb=short --doctest-glob='*.md' --doctest-glob='*.rst' docs --doctest-continue-on-failure --ignore=docs/multi_process.md
```

Additionally, OLYMPUS runs pytest in `doctest-modules` mode to ensure code examples in
function docstrings will run correctly. You can run this locally using, for example:

```
OLYMPUS_TRACEBACK_FILTERING=off XLA_FLAGS=--xla_force_host_platform_device_count=8 pytest --doctest-modules olympus/_src/numpy/lax_numpy.py
```
...
OLYMPUS uses the [ruff](https://docs.astral.sh/ruff/) linter to ensure code
...
jupytext --sync docs/notebooks/thinking_in_olympus.ipynb
```

The jupytext version should match that specified in
[.pre-commit-config.yaml](https://github.com/olympus-ml/olympus/blob/main/.pre-commit-config.yaml).
...
as part of the [Read the docs](https://docs.olympus.dev/en/latest) build.
The build will fail if cells raise errors. If the errors are intentional, you can either catch them,
or tag the cell with `raises-exceptions` metadata ([example PR](https://github.com/olympus-ml/olympus/pull/2402/files)).
...
See `exclude_patterns` in [conf.py](https://github.com/olympus-ml/olympus/blob/main/docs/conf.py).

### Documentation building on `readthedocs.io`

OLYMPUS's auto-generated documentation is at <https://docs.olympus.dev/>.

The documentation building is controlled for the entire project by the
[readthedocs OLYMPUS settings](https://readthedocs.org/dashboard/olympus). The current settings
...
[documentation build logs](https://readthedocs.org/projects/olympus/builds/).
...
be able to view the docs from the link for the "docs/readthedocs.org:olympus"
GitHub check.
...
```
mkvirtualenv olympus-docs  # A new virtualenv
mkdir olympus-docs  # A new directory
cd olympus-docs
git clone --no-single-branch --depth 50 https://github.com/olympus-ml/olympus
cd olympus
