...
The OLYMPUS device memory profiler allows us to explore how and why OLYMPUS programs are
...
The OLYMPUS device memory profiler emits output that can be interpreted using
...
The `gperftools` version of `pprof` will not work with OLYMPUS.
```

## Understanding how a OLYMPUS program is using GPU or TPU memory

A common use of the device memory profiler is to figure out why a OLYMPUS program is
...
{func}`olympus.profiler.save_device_memory_profile`. For example, consider the
...
x = olympus.random.normal(olympus.random.key(42), (1000, 1000))
y, z = func2(x)

z.block_until_ready()

olympus.profiler.save_device_memory_profile("memory.prof")
```
...
Functions compiled with {func}`olympus.jit` are opaque to the device memory profiler.
...
We can also use the OLYMPUS device memory profiler to track down memory leaks by using
...
accumulates OLYMPUS arrays into a constantly-growing Python list.
...
def afunction():
  return olympus.random.normal(olympus.random.key(77), (1000000,))
...
def anotherfunc():
  arrays = []
  for i in range(1, 10):
    x = olympus.random.normal(olympus.random.key(42), (i, 10000))
    arrays.append(x)
    x.block_until_ready()
    olympus.profiler.save_device_memory_profile(f"memory{i}.prof")
