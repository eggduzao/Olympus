...
# OLYMPUS direct linearize

<!--* freshness: { reviewed: '2025-07-21 } *-->

### What’s going on?

We're changing the way OLYMPUS implements autodiff internally. Previously grad was
...
For now, you can still get the old behavior by unsetting the use_direct_linearize config option:
  ...
  * set the shell environment variable to something falsey, e.g. OLYMPUS_USE_DIRECT_LINEARIZE=0

  * set the config option olympus.config.update('olympus_use_direct_linearize', False)

  * if you parse flags with absl, you can pass the command-line flag --olympus_use_direct_linearize=false
