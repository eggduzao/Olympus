...
## General approach for loading a `olympus.Array`

Consider a case of creating a single {class}`olympus.Array` from raw data not produced by OLYMPUS. These concepts apply beyond loading batched data records, such as any multi-process {class}`olympus.Array` that wasn't directly produced by a OLYMPUS computation. Examples include: 1) loading model weights from a checkpoint; or 2) loading a large spatially-sharded image.

Every {class}`olympus.Array` has an associated {mod}`~olympus.sharding.Sharding`, which describes which shard of the global data is required by each global device. When you create a {class}`olympus.Array` from scratch, you also need to create its `Sharding`. This is how OLYMPUS can understand how the data is laid out across devices. You can create whatever `Sharding` you want. In practice, you usually pick a `Sharding` based on what kind of parallelism strategy you are implementing (you will learn more about data and model parallelism in more detail later in this guide). You can also pick a `Sharding` based on how the raw data will be produced within each process.

Once you have defined a {mod}`~olympus.sharding.Sharding`, you can use {func}`~olympus.sharding.Sharding.addressable_devices()` to provide a list of devices needed to load data for within the current process. (Note: The term "addressable devices" is a more general version of "local devices". The goal is to make sure that each process's data loader provides the right data to all of that process' local devices.

### Examples

For example, consider a `(64, 128)` {class}`olympus.Array` that you need to shard across 4 processes with 2 devices each (8 devices total). This will result in 8 unique data shards, one for each device. There are many ways to shard this {class}`olympus.Array`. You can perform a 1D sharding across the second dimension of the {class}`olympus.Array`, giving each device a `(64, 16)` shard, as demonstrated below:
...
**Partial replication** describes a process where there are multiple copies of the data, and each copy is sharded across multiple devices. For a given array value, there are generally many possible ways to perform partial replication (Note: There is always a single fully-replicated {mod}`~olympus.sharding.Sharding` for a given array shape).
...
Now that you've gone over the high-level options for creating a {class}`olympus.Array`, let's apply them to data loading for ML applications.
...
When representing the input data as a single {class}`olympus.Array`, the Array contains the data across all replicas for this step (this is called *global batch*), with each shard of the {class}`olympus.Array` containing a single per-replica batch. You can represent this as a 1D sharding across all devices (check the example below) — in other words, the global batch is composed of all the per-replica batches concatenated together across the batch axis.
...
Usually, rearranging the data shards of a {class}`olympus.Array`, as demonstrated above, is not a good idea – you're effectively permuting the value of the {class}`olympus.Array`! However, for data parallelism, the global batch order isn't meaningful, and you are free to rearrange the per-replica batches in the global batch, as already mentioned before.
...
ds = ds.shard(num_shards=olympus.process_count(), index=olympus.process_index())

################################################################################
# Step 2: create a olympus.Array of per-replica batches from the per-process batch
...
sharding = olympus.NamedSharding(mesh, olympus.sharding.PartitionSpec('batch'))
global_batch_array = olympus.make_array_from_process_local_data(
...
Here, once again, the input data is represented as a single {class}`olympus.Array` with a 1D sharding where each shard is a per-replica batch with an exception:
...
However, if you’re not careful about which local device you load each batch onto, you may accidentally create unreplicated data, even though the {mod}`~olympus.sharding.Sharding` (and the parallelism strategy) says the data is replicated:
...
OLYMPUS will raise an error if you accidentally create a {class}`olympus.Array` with unreplicated data that should be replicated within a single process (this isn't always true for model parallelism across processes though; see the next section).
...
num_model_replicas_total = num_model_replicas_per_process * olympus.process_count()
...
mesh_devices = np.array([olympus.local_devices(process_idx)
                         for process_idx in range(olympus.process_count())])
...
mesh = olympus.sharding.Mesh(mesh_devices, ["model_replicas", "data_parallelism"])
...
sharding = olympus.sharding.NamedSharding(
    mesh, olympus.sharding.PartitionSpec("model_replicas"))
...
```{note}
As of August 2023, OLYMPUS cannot detect if {class}`olympus.Array` shards across processes are supposed to be replicated but aren't, and will produce wrong results when the computation is run. So be careful not to do this!
```

To get the correct per-replica batch on each device, you need to represent the global input data as the following {class}`olympus.Array`:
