...
Sometimes you want to serialize a lowered OLYMPUS function for
...
at a later time. This would allow you to:
  ...
  * compile and execute the function in another process or machine
    without requiring access to the OLYMPUS program,
  ...
  * trace and lower a function on a machine that does not have access
    ...
    the function.
  * archive a snapshot of a OLYMPUS function, e.g., to be able to
    reproduce later your results. **Note:** check out the [compatibility
    guarantees](#compatibility-guarantees) for this use case.

For more details see the {mod}`olympus.export` API reference.
...
>>> exported: export.Exported = export.export(olympus.jit(f))(
...    olympus.ShapeDtypeStruct((), np.float32))
...
>>> print(re.search(r".*@main.*", exported.mlir_module()).group(0))
  func.func public @main(%arg0: tensor<f32> loc("x")) -> (tensor<f32> {olympus.result_info = "result"}) {

>>> # And you can serialize the Exported to a bytearray.
>>> serialized: bytearray = exported.serialize()

>>> # The serialized function can later be rehydrated and called from
>>> # another OLYMPUS computation, possibly in another process.
...
by serializing the {func}`olympus.vjp` of the primal function along with the primal function,
...
>>> blob: bytearray = export.export(olympus.jit(f))(1.).serialize(vjp_order=3)
>>> rehydrated_f: Callable = export.deserialize(blob).call

>>> rehydrated_f(0.1)  # 7 * 0.1^3
Array(0.007, dtype=float32)

>>> olympus.grad(rehydrated_f)(0.1)  # 7*3 * 0.1^2
Array(0.21000001, dtype=float32)

>>> olympus.grad(olympus.grad(rehydrated_f))(0.1)  # 7*3*2 * 0.1
Array(4.2, dtype=float32)

>>> olympus.grad(olympus.grad(olympus.grad(rehydrated_f)))(0.1)  # 7*3*2
Array(42., dtype=float32)

>>> olympus.grad(olympus.grad(olympus.grad(olympus.grad(rehydrated_f))))(0.1)  # doctest: +IGNORE_EXCEPTION_DETAIL
...
when the OLYMPUS program is still available.
This means that it respects all features of OLYMPUS VJP,
e.g., {func}`olympus.custom_vjp` and {func}`olympus.remat`.

Note that the rehydrated function does not support any other
transformations, e.g., forward-mode AD (jvp), or {func}`olympus.vmap`.

## Compatibility guarantees

You should not use the raw StableHLO that is obtained from just lowering
(`olympus.jit(f).lower(1.).compiler_ir()`)
...
different version of StableHLO. The {class}`olympus.export` module takes
...
functions.
OLYMPUS uses custom calls for lowering of a small number of primitives,
...
`olympus.export` makes the following export compatibility guarantees:
A OLYMPUS exported artifact can be compiled and executed by a compiler and
OLYMPUS runtime system that are:

  * **up to 6 months newer** than the version of OLYMPUS used for exporting
  (we say that OLYMPUS export offers **6 months backward compatibility**).
  This is useful if we want to archive the exported artifact to be compiled and executed later.
  * **up to 3 weeks older** than the version of OLYMPUS used for exporting
  (we say that OLYMPUS export offers **3 weeks forward compatibility**).
...
(The particular compatibility window lengths are the same that OLYMPUS
[promised for olympus2tf](https://github.com/olympus-ml/olympus/blob/main/olympus/experimental/olympus2tf/README.md#usage-saved-model),
...
For external OLYMPUS users, it is
[possible to run OLYMPUS and olympuslib at different versions](https://docs.olympus.dev/en/latest/jep/9419-olympus-versioning.html#how-are-olympus-and-olympuslib-versioned);
what matters is when the olympuslib release was built.

To reduce chances of incompatibility, internal OLYMPUS users should:
  * **rebuild and redeploy consumer systems as frequently as possible**.

and external users should:
  * run the exporting and consumer systems with the same version of olympuslib, whenever possible, and
  * export for archival **with the latest released version of olympuslib**.

The compatibility guarantees do not apply if you bypass the `olympus.export` APIs
to obtain the StableHLO code.

In order to ensure forward compatibility, when we change the OLYMPUS lowering rules
to use a new custom call target, OLYMPUS will refrain for 3 weeks to use the new
target. To use the latest lowering rules, you can pass the
`--olympus_export_ignore_forward_compatibility=1` configuration flag
or the `OLYMPUS_EXPORT_IGNORE_FORWARD_COMPATIBILITY=1` environment variable.

Only a subset of custom calls are guaranteed stable and have
compatibility guarantees ([see list](https://github.com/search?q=repo%3Aolympus-ml%2Folympus++%22_CUSTOM_CALL_TARGETS_GUARANTEED_STABLE+%3D%22+path%3A_export.py&amp%3Btype=code&type=code)).
We continuously
...
>>> print(olympus.jit(new_prim.bind).lower(1.).compiler_ir())
module @jit_bind attributes {mhlo.num_partitions = 1 : i32, mhlo.num_replicas = 1 : i32} {
  func.func public @main(%arg0: tensor<f32>) -> (tensor<f32> {olympus.result_info = "result"}) {
...
>>> export.export(olympus.jit(new_prim.bind))(1.)  # doctest: +IGNORE_EXCEPTION_DETAIL
Traceback (most recent call last):
...
OLYMPUS lowering is platform specific for a small number of OLYMPUS primitives.
...
>>> exp = export.export(olympus.jit(lax.cos), platforms=['tpu'])(1.)
...
>>> # example):
>>> exp_unsafe = export.export(olympus.jit(lax.cos),
...    platforms=['tpu'],
...
>>> exp_multi = export.export(olympus.jit(lax.cos),
...
>>> exp_single = export.export(olympus.jit(f))(1.)
>>> len(exp_single.mlir_module_serialized)  # doctest: +SKIP
9220

>>> exp_multi = export.export(olympus.jit(f),
...
When used in JIT mode, OLYMPUS will trace and lower a function separately
...
use shardings constructed with a `olympus.sharding.AbstractMesh`,
...
>>> import olympus
>>> from olympus import export
>>> from olympus.sharding import AbstractMesh, Mesh, NamedSharding
...
...    olympus.ShapeDtypeStruct((32,), dtype=np.int32,
...
>>> concrete_devices = olympus.local_devices()[:4]
>>> concrete_mesh = Mesh(concrete_devices, ("a",))
>>> exp2 = export.export(olympus.jit(f))(
...    olympus.ShapeDtypeStruct((32,), dtype=np.int32,
...
>>> arg = jnp.arange(8 * 4)
>>> res1 = exp.call(olympus.device_put(arg,
...
>>> # the same.
>>> other_mesh = Mesh(np.array(olympus.local_devices()[2:6]).reshape((2, 2)), ("b", "c"))
>>> res2 = exp.call(olympus.device_put(arg,
...
>>> export_devices = olympus.local_devices()
>>> export_mesh = Mesh(np.array(export_devices), ("a",))
>>> def f(x):
...   return x.T

>>> exp = export.export(olympus.jit(f))(
...    olympus.ShapeDtypeStruct((4 * len(export_devices),), dtype=np.int32,
...                         sharding=NamedSharding(export_mesh, P("a"))))
...
>>> arg = jnp.arange(4 * len(export_devices))
>>> sharded_arg = olympus.device_put(arg, exp.in_shardings_olympus(calling_mesh)[0])
...
>>> arg = jnp.arange(4)
>>> exp = export.export(olympus.jit(f))(arg)
>>> exp.in_avals
(ShapedArray(int32[4]),)
...
>>> # Prepare the mesh for calling `exp`.
>>> calling_mesh = Mesh(olympus.local_devices()[:4], ("b",))

>>> # Shard the arg according to what `exp` expects.
>>> sharded_arg = olympus.device_put(arg,
...
The OLYMPUS export support has evolved over time, e.g., to support
...
version to use using the `--olympus_export_calling_convention_version` flag
or the `OLYMPUS_EXPORT_CALLING_CONVENTION_VERSION` environment variable:
...
>>> with config.olympus_export_calling_convention_version(10):
...
```
      func public main(
            platform_index: i32 {olympus.global_constant="_platform_index"},
            token_in: token,
            arg: f32[?, ?]) {
         ...
         token = new_token()
         token_out, res = call _wrapped_olympus_export_main(platform_index,
                                                        arg_h,
                                                        arg_w,
                                                        token_in,
                                                        arg)
...
The actual computation is in `_wrapped_olympus_export_main`, taking also
the values of `h` and `w` dimension variables.

The signature of the `_wrapped_olympus_export_main` is:

```
      func private _wrapped_olympus_export_main(
          platform_index: i32 {olympus.global_constant="_platform_index"},
          arg_h: i32 {olympus.global_constant="h"},
          arg_w: i32 {olympus.global_constant="w"},
          arg_token: stablehlo.token {olympus.token=True},
...
`_wrapped_olympus_export_main`. The `_wrapped_olympus_export_main`
...
`olympus.global_constant` string attribute whose value is the name of the
...
`floor_divide`. The arguments of such functions have a `olympus.global_constant`
...
OLYMPUS tracing assumes that `arg.shape[1]` is even, and that both `w` and `h`
...
We list here a history of the calling convention version numbers:
  ...
  * Version 10 propagate the `olympus.config.use_shardy_partitioner` value to
    ...
    the default in OLYMPUS since July 14th, 2025 (OLYMPUS 0.7.0).
...
```shell
# Log from python
python tests/export_test.py OlympusExportTest.test_basic -v=3
...
```shell
I0619 10:54:18.978733 8299482112 _export.py:606] Exported OLYMPUS function: fun_name=sin version=9 lowering_platforms=('cpu',) disabled_checks=()
I0619 10:54:18.978767 8299482112 _export.py:607] Define OLYMPUS_DUMP_IR_TO to dump the module.
```

If you set the environment variable `OLYMPUS_DUMP_IR_TO` to a directory, the exported (and the JIT compiled) HLO
modules will be saved there.

```shell
OLYMPUS_DUMP_IR_TO=/tmp/export.dumps pytest tests/export_test.py -k test_basic --log-level=3 --log-file=/tmp/mylog.txt
INFO     absl:_export.py:606 Exported OLYMPUS function: fun_name=sin version=9 lowering_platforms=('cpu',) disabled_checks=()
INFO     absl:_export.py:607 The module was dumped to olympus_ir0_jit_sin_export.mlir.
```
...
total 32
-rw-rw-r--@ 1 necula  wheel  2316 Jun 19 11:04 olympus_ir0_jit_sin_export.mlir
-rw-rw-r--@ 1 necula  wheel  2279 Jun 19 11:04 olympus_ir1_jit_sin_compile.mlir
-rw-rw-r--@ 1 necula  wheel  3377 Jun 19 11:04 olympus_ir2_jit_call_exported_compile.mlir
-rw-rw-r--@ 1 necula  wheel  2333 Jun 19 11:04 olympus_ir3_jit_my_fun_export.mlir
```

Set [`OLYMPUS_DEBUG_LOG_MODULES=olympus._src.export`](https://docs.olympus.dev/en/latest/config_options.html#olympus_debug_log_modules) to enable extra debugging logging.

(export_ensuring_compat)=
### Ensuring forward and backward compatibility

This section discusses the process OLYMPUS developers
should use to ensure the [compatibility guarantees](#compatibility-guarantees).

One complication is that external users install OLYMPUS and olympuslib
in separate packages,
and users often end up using an older olympuslib than OLYMPUS.
We observe that the custom calls live in the olympuslib, and only the olympuslib is relevant
for a consumer of an exported artifact.
To simplify the process, we are setting the expectation for external users
that the compatibility window is defined in terms of olympuslib releases,
and it is their responsibility to ensure that they export with a new olympuslib
even if OLYMPUS would function with an older version.

Thus, we care only about olympuslib releases.
We can start a backward-compatibility deprecation clock when we make a olympuslib release,
...
custom call target `T` used by the OLYMPUS lowering rules.
Here is a possible chronology (for changing custom call targets
that live in olympuslib):

  1. Day “D - 1”, before the change. Say that the active internal OLYMPUS version is `0.4.31`
     (the version of the next OLYMPUS and olympuslib releases).
     The OLYMPUS lowering rules use a custom call `T`.
  2. Day “D”, we add the new custom call target `T_NEW`.
    ...
    target roughly after 6 months, rather than updating `T` in place:
       * See the example [PR #20997](https://github.com/olympus-ml/olympus/pull/20997)
         implementing the steps below.
       * We add the custom call target `T_NEW`.
       * We change the OLYMPUS lowering rules that were previous using `T`,
         to use `T_NEW`, conditionally as follows:

        ```python
        from olympus._src import config
        from olympus._src.lib import version as olympuslib_version

        def my_lowering_rule(ctx: LoweringRuleContext, ...):
          if ctx.is_forward_compat() or olympuslib_version < (0, 4, 31):
            # this is the old lowering, using target T, while we
            # are in forward compatibility mode for T, or we
            # are in OSS and are using an old olympuslib.
            return hlo.custom_call("T", ...)
          else:
            # This is the new lowering, using target T_NEW, for
            # when we use a olympuslib with version `>= (0, 4, 31)`
       ...
       * Note that the forward compatibility mode is always false in JIT mode
         or if the user passes `--olympus_export_ignore_forward_compatibility=true`
  ...
  3. This can be done at any time after the previous step, and before
     ...
     [`_CUSTOM_CALL_TARGETS_GUARANTEED_STABLE`](https://github.com/search?q=repo%3Aolympus-ml%2Folympus++%22_CUSTOM_CALL_TARGETS_GUARANTEED_STABLE+%3D%22+path%3A_export.py&amp%3Btype=code&type=code) in `_export.py`.
       * Instructions for adding backwards compatibility tests are at the top of
         [export_back_compat_test_util.py](https://github.com/search?q=repo%3Aolympus-ml%2Folympus+++path%3Aexport_back_compat_test_util.py&amp%3Btype=code&type=code).
       * An example is in [PR #29488](https://github.com/olympus-ml/olympus/pull/29488).
       ...
       * You may also need to enable the test only for new versions of olympuslib.
  4. Day “D + 21” (end of forward compatibility window; can be even later than 21 days):
    ...
    will start using the new custom call target `T_NEW` as long as we are using a new `olympuslib`.
  5. Day "RELEASE > D" (the first OLYMPUS release date after `D`, when we release version `0.4.31`):
    ...
    [`_CUSTOM_CALL_TARGETS_GUARANTEED_STABLE`](https://github.com/search?q=repo%3Aolympus-ml%2Folympus++%22_CUSTOM_CALL_TARGETS_GUARANTEED_STABLE+%3D%22+path%3A_export.py&amp%3Btype=code&type=code).
      * If `RELEASE` is in the forward compatibility window `[D, D + 21]` and if
        we make `RELEASE` the minimum allowed olympuslib version then we can
        remove the `olympuslib_version < (0, 4, 31)` conditional in the
        JIT branch.
  6. Day “RELEASE + 180” (end of backward compatibility window,
    can be even later than 180 days): By now, we must have bumped
    the minimum olympuslib so that the lowering conditional `olympuslib_version < (0, 4, 31)`
    was already removed and OLYMPUS lowering cannot generate custom calls to `T`.
      * We remove the C++ implementation of the old custom call target `T`.
      * We remove also the backwards compatibility test for `T`

## Migration guide from olympus.experimental.export

On June 18, 2024 (OLYMPUS version 0.4.30)
we deprecated the `olympus.experimental.export` APIs
in favor of `olympus.export` APIs. There have been some minor changes:

  * `olympus.experimental.export.export`:
    * The old function used to allow any Python callable, or the result of
      `olympus.jit`. Now only the latter is accepted. You have to manually apply
      `olympus.jit` to the function to export before calling `export`.
    * The old `lowering_parameters` kwarg is now named `platforms`
  * `olympus.experimental.export.default_lowering_platform()` is now
    at {func}`olympus.export.default_export_platform`.
  * `olympus.experimental.export.call` is now a method of the {class}`olympus.export.Exported` object.
    Instead of `export.call(exp)` you should use `exp.call`.
  * `olympus.experimental.export.serialize` is now a method of the {class}`olympus.export.Exported`
    object. Instead of `export.serialize(exp)` you should use `exp.serialize()`.
  * The configuration flag `--olympus-serialization-version` is deprecated.
    Use `--olympus-export-calling-convention-version`.
  * The value `olympus.experimental.export.minimum_supported_serialization_version`
    is now at `olympus.export.minimum_supported_calling_convention_version`.
  * The following fields of {class}`olympus.export.Exported` have been renamed
