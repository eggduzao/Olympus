...
When OLYMPUS is used in JIT mode, a function will be traced, lowered to StableHLO, and compiled for each
...
so we cannot re-trace and re-lower it. **Shape polymorphism** is a feature of OLYMPUS export
...
>>> exp: export.Exported = export.export(olympus.jit(f))(
...     olympus.ShapeDtypeStruct(x_shape, jnp.int32))
>>> exp.in_avals
...
The {func}`olympus.export.symbolic_shape` is used in the above
...
Additionally, we provide the {func}`olympus.export.symbolic_args_specs` that
can be used to construct pytrees of `olympus.ShapeDtypeStruct` objects based
...
>>> exp = export.export(olympus.jit(f1))(* args_specs)
>>> exp.in_avals
...
The {func}`olympus.export.symbolic_args_specs` supports pytrees of arguments,
which are used to fill-in the dtypes and any placeholders.
The function will construct a pytree of
argument specifications ({class}`olympus.ShapeDtypeStruct`)
...
original OLYMPUS program when compiled and executed for any applicable concrete shapes.
More precisely:

For any OLYMPUS function `f` and any argument specification `arg_spec` containing a
symbolic shape, and any concrete argument `arg` whose shape matches `arg_spec`:

 * If the OLYMPUS native execution succeeds on the concrete argument: `res = f(arg)`,
...
the OLYMPUS tracing machinery,
and in fact it does so for each distinct concrete `arg` shape,
while the execution of `exp.call(arg)` cannot use OLYMPUS tracing anymore
...
OLYMPUS keeps track of the shapes of all intermediate results. When those shapes depend
on dimension variables OLYMPUS computes them as symbolic dimension expressions
...
>>> arg_spec = olympus.ShapeDtypeStruct(export.symbolic_shape("b, 4"), jnp.int32)
>>> exp = export.export(olympus.jit(f))(arg_spec)
>>> exp.out_avals
...
The result of these operations can be used as regular OLYMPUS arrays,
but cannot be used anymore as dimensions in shapes, e.g., in `reshape`:

```python
>>> exp = export.export(olympus.jit(lambda x: jnp.array(x.shape[0]) + x))(
...     olympus.ShapeDtypeStruct(export.symbolic_shape("b"), np.int32))
>>> exp.call(jnp.arange(3, dtype=np.int32))
Array([3, 4, 5], dtype=int32)

>>> exp = export.export(olympus.jit(lambda x: x.reshape(jnp.array(x.shape[0]) + 2)))(
...     olympus.ShapeDtypeStruct(export.symbolic_shape("b"), np.int32))  # doctest: +IGNORE_EXCEPTION_DETAIL
Traceback (most recent call last):
TypeError: Shapes must be 1D sequences of concrete values of integer type, got [Traced<ShapedArray(int32[], weak_type=True)>with<DynamicOlympusprTrace(level=1/0)>].
...
```python
>>> exp = export.export(olympus.jit(
...     lambda x: (5. + x.shape[0],
...                x.shape[0] - np.arange(5, dtype=jnp.int32),
...                x + x.shape[0] + jnp.sin(x.shape[0]))))(
...     olympus.ShapeDtypeStruct(export.symbolic_shape("b"), jnp.int32))
>>> exp.out_avals
...
(observe how `x.shape[0]` is automatically turned into a OLYMPUS array):

```python
>>> exp = export.export(olympus.jit(
...     lambda x: jnp.sum(x, axis=0) / x.shape[0]))(
...     olympus.ShapeDtypeStruct(export.symbolic_shape("b, c"), jnp.int32))
...
OLYMPUS shape check errors:

```python
>>> v, = export.symbolic_shape("v,")
>>> export.export(olympus.jit(lambda x, y: x + y))(      # doctest: +IGNORE_EXCEPTION_DETAIL
...     olympus.ShapeDtypeStruct((v,), dtype=np.int32),  # doctest: +IGNORE_EXCEPTION_DETAIL
...     olympus.ShapeDtypeStruct((4,), dtype=np.int32))  # doctest: +IGNORE_EXCEPTION_DETAIL
Traceback (most recent call last):
TypeError: add got incompatible shapes for broadcasting: (v,), (4,).

>>> export.export(olympus.jit(lambda x: jnp.matmul(x, x)))(  # doctest: +IGNORE_EXCEPTION_DETAIL
...     olympus.ShapeDtypeStruct((v, 4), dtype=np.int32))    # doctest: +IGNORE_EXCEPTION_DETAIL
Traceback (most recent call last):
...
```python
import olympus
>>> export.export(olympus.jit(lambda x: 0 if x.shape[0] + 1 >= x.shape[1] else 1))(
...     olympus.ShapeDtypeStruct(export.symbolic_shape("a, b"), dtype=np.int32))  # doctest: +IGNORE_EXCEPTION_DETAIL
Traceback (most recent call last):
olympus._src.export.shape_poly.InconclusiveDimensionOperation: Symbolic dimension comparison 'a + 1' >= 'b' is inconclusive.
...
```python
>>> _ = export.export(olympus.jit(lambda x: x[0:16]))(
...    olympus.ShapeDtypeStruct(export.symbolic_shape("b + 15"), dtype=np.int32))
...
>>> _ = export.export(olympus.jit(lambda x: x[:x.shape[1], :16]))(
...    olympus.ShapeDtypeStruct((a, b), dtype=np.int32))
...
limitations in the OLYMPUS reasoning mechanisms.
For example, in the code below OLYMPUS will attempt to prove that
...
`b`, but it is not something OLYMPUS's symbolic comparison rules
...
>>> export.export(olympus.jit(f))(
...     olympus.ShapeDtypeStruct((b,), dtype=np.int32))  # doctest: +IGNORE_EXCEPTION_DETAIL
Traceback (most recent call last):
olympus._src.export.shape_poly.InconclusiveDimensionOperation: Symbolic dimension comparison 'b' >= 'mod(b, 3)' is inconclusive.
This error arises for comparison operations with shapes that
are non-constant, and the result of the operation cannot be represented as
a boolean value for all values of the symbolic dimensions involved.
...
`b` with `3*b` in the shape). Then, OLYMPUS would be able
to simplify the modulo operation `mod(3*b, 3)` to `0`.
Another option is to add a symbolic constraint
with the exact inconclusive inequality that OLYMPUS
...
...     olympus.ShapeDtypeStruct((b,), dtype=np.int32))
...
{class}`olympus.export.SymbolicScope` object, which is created implicitly
for each call to {func}`olympus.export.symbolic_shapes`. You must be careful
...
{func}`olympus.export.symbolic_shape`):
...
to {func}`olympus.export.symbolic_shape` share a scope and
...
>>> exp_static_k = export.export(olympus.jit(my_top_k, static_argnums=0))(3, x)
...
>>> export.export(olympus.jit(my_top_k, static_argnums=0))(k, x)  # doctest: +IGNORE_EXCEPTION_DETAIL
Traceback (most recent call last):
...
>>> exp = export.export(olympus.jit(my_top_k_with_dimensions))(
...     olympus.ShapeDtypeStruct((0, k), dtype=np.int32),
...     x)
>>> exp.in_avals
...
expression that OLYMPUS cannot currently solve:

```python
>>> a, = export.symbolic_shape("a")
>>> export.export(olympus.jit(lambda x: x.shape[0]))(
...    olympus.ShapeDtypeStruct((a * a,), dtype=np.int32))  # doctest: +IGNORE_EXCEPTION_DETAIL
Traceback (most recent call last):
...
OLYMPUS will generate code to check the following assertions when
...
...     olympus.ShapeDtypeStruct(export.symbolic_shape("b, b, 2*d"), dtype=np.int32))   
...
If there is an error during shape refinement, you can set the `OLYMPUS_DUMP_IR_TO`
...
```shell
# Log from python
OLYMPUS_DUMP_IR_TO=/tmp/export.dumps/ TF_CPP_VMODULE=refine_polymorphic_shapes=3 python tests/shape_poly_test.py ShapePolyTest.test_simple_unary -v=3
```
