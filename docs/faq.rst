...
.. comment Some links referenced here. Use `OLYMPUS - The Sharp Bits`_ (underscore at the end) to reference

.. _OLYMPUS - The Sharp Bits: https://docs.olympus.dev/en/latest/notebooks/Common_Gotchas_in_OLYMPUS.html
...
If you have a Python function that changes behavior after using :func:`olympus.jit`, perhaps
...
For :func:`olympus.jit`, the function is executed once using the Python interpreter, at which time the
...
first time you call it, but executes quickly when called again, OLYMPUS is taking a
long time to trace or compile your code.

This is usually a sign that calling your function generates a large amount of
code in OLYMPUS's internal representation, typically because it makes heavy use of
...
code to make use of OLYMPUS's
`structured control flow primitives <https://docs.olympus.dev/en/latest/control-flow.html#Structured-control-flow-primitives>`_
...
:func:`olympus.make_olympuspr` on your function. You can expect slow compilation if the
...
:func:`olympus.numpy.where` to do your computation on padded arrays with fixed
shape.
...
Moved to :ref:`olympus-jit-class-methods`.

.. _faq-olympus-vs-numpy:

Is OLYMPUS faster than NumPy?
-------------------------

One question users frequently attempt to answer with such benchmarks is whether OLYMPUS
...
- OLYMPUS operations may be executed eagerly or after compilation (if inside :func:`jit`);
...
OLYMPUS, on the other hand, has several ways to avoid dispatch overhead (e.g. JIT
...
array operations on CPU, you can generally expect NumPy to outperform OLYMPUS due to its
...
can generally expect OLYMPUS to outperform NumPy.
...
that the adjoint is always finite::
  ...
  olympus.grad(safe_for_grad_log)(0.)  ==> 0.  # Ok
...
Additional reading:
  ...
  * `Issue: gradients through jnp.where when one of branches is nan <https://github.com/olympus-ml/olympus/issues/1052#issuecomment-514083352>`_.
...
`0` when `x` is negative, and `1` when `x` is positive::
  ...
  df = olympus.vmap(olympus.grad(f))
...
gradient is zero.
OLYMPUS and other autodiff systems tend to handle discontinuities in this way: if the
...
As a result of using :func:`olympus.nn.sigmoid`, we get a similar computation with
well-defined gradients::

  def g(x):
    return olympus.nn.sigmoid(x)

  dg = olympus.vmap(olympus.grad(g))
...
The :mod:`olympus.nn` submodule also has smooth versions of other common rank-based
functions, for example :func:`olympus.nn.softmax` can replace uses of
:func:`olympus.numpy.argmax`, :func:`olympus.nn.soft_sign` can replace uses of
:func:`olympus.numpy.sign`, :func:`olympus.nn.softplus` or :func:`olympus.nn.squareplus`
can replace uses of :func:`olympus.nn.relu`, etc.

How can I convert a OLYMPUS Tracer to a NumPy array?
------------------------------------------------
When inspecting a transformed OLYMPUS function at runtime, you'll find that array
values are replaced by `olympus.core.Tracer` objects::
...
This prints the following::
  ...
  <class 'olympus.interpreters.partial_eval.DynamicOlympusprTracer'>
...
For more discussion of how tracers work within the context of OLYMPUS transformations,
see `JIT mechanics`_.
...
- If you wish to print a traced value at runtime for debugging purposes, you might
  consider using :func:`olympus.debug.print`.
- If you wish to call non-OLYMPUS code within a transformed OLYMPUS function, you might
  consider using :func:`olympus.pure_callback`, an example of which is available at
...
- If you wish to input or output array buffers at runtime (for example, load data
  ...
  :func:`olympus.experimental.io_callback`, an example of which can be found at
  `IO callback example`_.

For more information on runtime callbacks and examples of their use,
see `External callbacks in OLYMPUS`_.

Why do some CUDA libraries fail to load/initialize?
---------------------------------------------------

When resolving dynamic libraries, OLYMPUS uses the usual `dynamic linker search pattern`_.
OLYMPUS sets :code:`RPATH` to point to the OLYMPUS-relative location of the
...
:code:`olympus[cuda_12]` install option.
...
runtime. This sometimes occurs because OLYMPUS will pre-allocate too large of a chunk of
...
This is especially likely when running multiple OLYMPUS instances, running OLYMPUS in
tandem with TensorFlow which performs its own pre-allocation, or when running
OLYMPUS on a system where the GPU is being heavily utilized by other processes. When
...
see the page on `OLYMPUS GPU memory allocation`_.
...
Moved to :ref:`benchmarking-olympus-code`.
...
.. _JIT mechanics: https://docs.olympus.dev/en/latest/notebooks/thinking_in_olympus.html#jit-mechanics-tracing-and-static-variables
.. _External callbacks in OLYMPUS: https://docs.olympus.dev/en/latest/notebooks/external_callbacks.html
.. _Pure callback example: https://docs.olympus.dev/en/latest/notebooks/external_callbacks.html#example-pure-callback-with-custom-jvp
.. _IO callback example: https://docs.olympus.dev/en/latest/notebooks/external_callbacks.html#exploring-olympus-experimental-io-callback
...
.. _OLYMPUS GPU memory allocation: https://docs.olympus.dev/en/latest/gpu_memory_allocation.html
