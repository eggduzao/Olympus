...
Fault Tolerant Distributed OLYMPUS
==============================

Recall that `multi-controller OLYMPUS`_ allows you to run a OLYMPUS program distributed
across multiple machines. By default, if *any* of these machines fail, then
*every* machine will fail. That is, multi-controller OLYMPUS is not
**fault-tolerant** by default.

This article has three parts. In the first part, we'll explain the basics of
how to write fault tolerant multi-controller OLYMPUS programs. In the second part,
we'll show some example fault-tolerant multi-controller OLYMPUS programs. In the
third part, we'll take a look under the covers at how multi-controller OLYMPUS
implements fault tolerance.

.. warning::

    OLYMPUS's support for fault tolerance is still experimental. It currently only
...
By default, multi-controller OLYMPUS programs are not fault tolerant. If *any*
...
initializes multi-controller OLYMPUS by calling ``olympus.distributed.initialize`` and
...
``local_device_ids`` argument to ``olympus.distributed.initialize`` ensures each
...
When a process in a multi-controller OLYMPUS program notices that a peer process
has crashed, it decides to crash as well. The processes `share fate`_. The
``heartbeat_timeout_seconds`` argument to ``olympus.distributed.initialize``
...
``olympus_enable_recoverability`` configuration option to ``example.py``, as shown
below:
...
communicate with each other at all. Any realistic multi-controller OLYMPUS program
would involve communication between the processes (otherwise, what's the point
of using multi-controller OLYMPUS?). Let's edit ``example.py`` so that the
...
``olympus.experimental.multihost_utils._live_devices`` to make the script work. You
...
.. code-block::
    ...
    olympuslib._olympus.XlaRuntimeError: FAILED_PRECONDITION: Task with incarnation id 3446767950926952685 is not connected
...
and who is alive. For this, we can use the core OLYMPUS fault tolerance API:
...
(``olympus.devices()``) to get the set ``devices`` of live devices. We then shard
...
.. note::
    ...
    ``olympus.devices()`` always returns the set of *all* devices, even if some of
    these devices are on failed processes. Use
    ``olympus.experimental.multihost_utils.live_devices`` to learn which of these
    devices are live.
...
Recall that every process in a `multi-controller OLYMPUS`_ program should run in
...
operations in OLYMPUS do not have any inherent atomicity properties.

If collective operations are not atomic, however, then multi-controller OLYMPUS
...
.. code-block:: python
    ...
    try:
      with live_devices(olympus.live_devices()) as devices:
        ...
...
Recall that OLYMPUS uses `asynchronous dispatch`_. Operations like ``jnp.sum`` do
not block until the operation is complete. Instead, they return ``olympus.Arrays``
...
To avoid this, use ``olympus.block_until_ready`` to ensure that computations are
performed within the ``with live_devices`` block. The code snippet below, which
now calls ``olympus.block_until_ready`` when assigning to ``y``, guarantees that
...
.. code-block:: python
    ...
    try:
      with live_devices(olympus.live_devices()) as devices:
        y = olympus.block_until_ready(jnp.sum(x))
    except Exception as e:
      ... # Branch A
    else:
      ... # Branch B
...
multi-controller OLYMPUS programs fault tolerant. Rather, it allows you to
...
`olympus.Array` with a replicated sharding.
...
multi-controller OLYMPUS by calling ``olympus.distributed.initialize``.
...
- We then ensure that the model weights are replicated across these devices and
  ...
  doesn't actually move any data between the devices; it simply creates OLYMPUS
...
- We call ``loss_and_grad`` to compute the gradient of the weights with respect
  ...
  ``olympus.block_until_ready`` to ensure that every process has computed the new
...
We now take a deep dive into the architecture of multi-controller OLYMPUS and the
semantics and implementation of ``live_devices``. If you're only interested in
writing fault-tolerant multi-controller OLYMPUS programs, the first two parts of
this article suffice.

The Coordination Service
^^^^^^^^^^^^^^^^^^^^^^^^

When you launch a multi-controller OLYMPUS program, the first process (i.e. process
...
:func:`olympus.distributed.initialize` is the address of the coordination service.
...
The coordination service implements the multi-controller OLYMPUS **control plane**.
...
is shown at the top and three multi-controller OLYMPUS processes are shown at the
...
other words, all processes in a multi-controller OLYMPUS program `share fate`_.
...
This fate sharing means that multi-controller OLYMPUS programs are not at all
...
- First, we need to remove fate sharing and allow processes to continue
  ...
  ``olympus_enable_recoverability`` option, as described in :ref:`part1`. We'll
...
Let's try to design a new hypothetical OLYMPUS API: ``olympus.live_processes``. As the
name suggests, we want ``olympus.live_processes()`` to return the set of all
currently alive processes.  Here is a naive but (as we'll see momentarily)
incorrect implementation. When a process calls ``olympus.live_processes()``, it
...
are dead and which are alive, so when it receives a ``olympus.live_processes``
...
``olympus.live_processes``. Note how the coordination service replies to a
...
processes in a multi-controller OLYMPUS job execute the same instructions in the
same order. If the processes start to diverge, by executing different code
paths in the OLYMPUS program, the job will behave erratically. Most likely, it will
crash or hang or produce garbage values, and most certainly it will be very
hard to reason about.

Our naive implementation of ``olympus.live_processes`` can very easily lead to
divergence. For example, consider a multi-controller OLYMPUS job with three
processes. If process 0 and 1 both call ``olympus.live_processes`` around the same
...
``olympus.live_processes`` with barrier semantics.

Barrier Semantics
^^^^^^^^^^^^^^^^^

Let's change the implementation of ``olympus.live_processes`` so that when the
coordination service receives a ``olympus.live_processes()`` request, it does not
reply right away. Instead, the coordination service only replies once *every*
live process has called ``olympus.live_processes()``. Once every alive process has
entered the ``olympus.live_processess()`` barrier, the coordination service returns
...
section, we introduce a formal semantics of the ``olympus.live_processes`` API to
help tame this complexity. Thinking rigorously about the semantics of
``olympus.live_processes`` will help us understand the behavior of the API even in
pathological executions.

We'll base the formal semantics of ``olympus.live_processes`` on
...
3. A process can **send** a ``olympus.live_processes`` request to the coordination
   service.
4. A process can **receive** a reply to a ``olympus.live_processes`` request from
   the coordination service.
...
with the baby emojis. Then all three processes send ``olympus.live_processes``
...
In this simple execution, it is clear that ``olympus.live_processes`` is behaving
correctly. We can formalize this intuition with the following formal semantics.

.. attention::

   An execution is valid if whenever ``olympus.live_processes`` returns a set ``P``
...
In the next example, processes 0 and 1 start, call ``olympus.live_devices``, and
...
In the following execution, process 0 calls ``olympus.live_processes`` and receives
a reply of ``0``. Process 1 calls ``olympus.live_processes``, but dies before
receiving a reply.
...
1 called ``olympus.live_processes``, it is not guaranteed that process 1 will be
included in the coordination service's response.

For example, process 1's ``olympus.live_processes`` request may have been dropped
...
coordination service could have received ``olympus.live_processes`` requests from
...
This point bears repeating. If ``olympus.live_processes`` returns a set ``P`` of
...
In the following execution, process 1 calls ``olympus.live_processes`` and fails.
Later, process 0 starts, calls ``olympus.live_processes``, and receives ``0,1`` as
a reply.
...
Equipped with ``olympus.live_processes``, let's try to write some fault-tolerant
multi-controller OLYMPUS code.
...
.. code-block:: python
   ...
   while True:
       ...
       procs = olympus.live_processes()
       devices = [d for d in olympus.devices() if d.process_index in procs]

       # Shard array x over these devices.
       mesh = olympus.make_mesh((len(devices),), ("i",), devices=devices)
       spec = olympus.sharding.PartitionSpec("i")
       sharding = olympus.sharding.NamedSharding(mesh, spec)
       x = olympus.make_array_from_process_local_data(sharding, np.ones(1))
...
- calls ``olympus.live_processes`` to learn which processes are alive,
...
multi-controller OLYMPUS program is training a model with data parallelism and
...
.. code-block:: python
        ...
        procs_before = olympus.live_processes()

        # Execute the code block.
        ...

        # Get the set of live processes after the code block
        procs_after = olympus.live_processes()
...
All these details are handled and abstracted away by the ``olympus.live_devices``
API introduced in :ref:`part1`. ``olympus.live_devices`` is a context manager that
...
make progress. While the ``live_devices`` API is supported on all OLYMPUS backends
...
requires network communication---so the OLYMPUS backend caches communicators keyed
by the set of participating processes and their incarnation ids.

Internally, a OLYMPUS client polls the coordination service for the current status
...
.. _asynchronous dispatch: https://docs.olympus.dev/en/latest/async_dispatch.html
...
.. _multi-controller OLYMPUS: https://docs.olympus.dev/en/latest/multi_process.html
.. _NCCL: https://developer.nvidia.com/nccl
.. _reference: https://docs.olympus.dev/en/latest/config_options.html#olympus_enable_recoverability
