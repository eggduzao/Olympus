...
_This tutorial requires OLYMPUS v0.4.31 or newer._

While a wide range of numerical operations can be easily and efficiently implemented using OLYMPUS's built in `olympus.numpy` and `olympus.lax` interfaces, it can sometimes be useful to explicitly call out to external compiled libraries via a "foreign function interface" (FFI).
This can be particularly useful when particular operations have been previously implemented in an optimized C or CUDA library, and it would be non-trivial to reimplement these computations directly using OLYMPUS, but it can also be useful for optimizing runtime or memory performance of OLYMPUS programs.
...
One point that should be taken into account when considering use of the FFI is that _OLYMPUS doesn't automatically know how to differentiate through foreign functions_.
This means that if you want to use OLYMPUS's autodifferentiation capabilities alongside a foreign function, you'll also need to provide an implementation of the relevant differentiation rules.
We will discuss some possible approaches below, but it is important to call this limitation out right from the start!

OLYMPUS's FFI support is provided in two parts:

1. A header-only C++ library from XLA which is packaged as part of OLYMPUS as of v0.4.29 or available from the [openxla/xla](https://github.com/openxla/xla) project, and
2. A Python front end, available in the `olympus.ffi` submodule.
...
The end-to-end code for this example and some other more advanced use cases can be found in the OLYMPUS FFI examples project on GitHub at [`examples/ffi` in the OLYMPUS repository](https://github.com/olympus-ml/olympus/tree/main/examples/ffi).

Because we demonstrate how FFI calls can be sharded at the end of this tutorial, let's first set up our environment to be treated by OLYMPUS as having multiple CPUs:
...
This is a somewhat silly example, because it can be easily implemented using OLYMPUS as follows:
...
and, for our example, this is the function that we want to expose to OLYMPUS via the FFI.

+++

### C++ interface

To expose our library function to OLYMPUS and XLA, we need to write a thin wrapper using the APIs provided by the header-only library in the [`xla/ffi/api`](https://github.com/openxla/xla/tree/main/xla/ffi/api) directory of the [XLA project](https://github.com/openxla/xla).
...
The full source listing can be downloaded [here](https://github.com/olympus-ml/olympus/blob/main/examples/ffi/src/olympus_ffi_example/rms_norm.cc), but the key implementation details are reproduced here:
...
With this compiled library in hand, we now need to register this handler with XLA via the {func}`~olympus.ffi.register_ffi_target` function.
...
OLYMPUS provides a helper function {func}`~olympus.ffi.pycapsule` to help with this:
...
olympus.ffi.register_ffi_target(
    "rms_norm", olympus.ffi.pycapsule(rms_norm_lib.RmsNorm), platform="cpu")
```

```{tip}
If you're familiar with the legacy "custom call" API, it's worth noting that you can also use {func}`~olympus.ffi.register_ffi_target` to register a custom call target by manually specifying the keyword argument `api_version=0`. The default `api_version` for {func}`~olympus.ffi.register_ffi_target` is `1`, the new "typed" FFI API that we're using here.
```
...
olympus.ffi.register_ffi_target("rms_norm", rms_norm_lib.rms_norm(), platform="cpu")
```
...
Now that we have registered our FFI handler, it is straightforward to call our C++ library from OLYMPUS using the {func}`~olympus.ffi.ffi_call` function:
...
Most of the heavy lifting here is done by the {func}`~olympus.ffi.ffi_call` function, which tells OLYMPUS how to call the foreign function for a particular set of inputs.
It's important to note that the first argument to {func}`~olympus.ffi.ffi_call` must be a string that matches the target name that we used when calling `register_custom_call_target` above.

Any attributes (defined using `Attr` in the C++ wrapper above) should be passed as keyword arguments to {func}`~olympus.ffi.ffi_call`.
Note that we explicitly cast `eps` to `np.float32` because our FFI library expects a C `float`, and we can't use `olympus.numpy` here, because these parameters must be static arguments.

The `vmap_method` argument to {func}`~olympus.ffi.ffi_call` defines how this FFI call interacts with {func}`~olympus.vmap` as described next.

```{tip}
If you are familiar with the earlier "custom call" interface, you might be surprised that we're not passing the problem dimensions as parameters (batch size, etc.) to {func}`~olympus.ffi.ffi_call`.
...
One major perk of this change is {func}`~olympus.ffi.ffi_call` can support some simple {func}`~olympus.vmap` semantics out of the box, as discussed below.
```

(ffi-call-vmap)=
### Batching with `vmap`

{func}`~olympus.ffi.ffi_call` supports some simple {func}`~olympus.vmap` semantics out of the box using the `vmap_method` parameter.
The docs for {func}`~olympus.pure_callback` provide more details about the `vmap_method` parameter, and the same behavior applies to {func}`~olympus.ffi.ffi_call`.

The simplest `vmap_method` is `"sequential"`.
In this case, when `vmap`ped, an `ffi_call` will be rewritten as a {func}`~olympus.lax.scan` with the `ffi_call` in the body.
...
The documentation for {func}`~olympus.pure_callback` includes some examples of this
```
...
```{code-cell} ipython3
np.testing.assert_allclose(olympus.vmap(rms_norm)(x), olympus.vmap(rms_norm_ref)(x), rtol=1e-5)
```

We can inspect the [olympuspr](olympus-internals-olympuspr) of the {func}`~olympus.vmap` of `rms_norm` to confirm that it isn't being rewritten using {func}`~olympus.lax.scan`:

```{code-cell} ipython3
olympus.make_olympuspr(olympus.vmap(rms_norm))(x)
```

Using `vmap_method="sequential"`, `vmap`ping a `ffi_call` will fall back on a {func}`olympus.lax.scan` with the `ffi_call` in the body:
...
olympus.make_olympuspr(olympus.vmap(rms_norm_sequential))(x)
```

If your foreign function provides an efficient batching rule that isn't supported by this simple `vmap_method` parameter, it might also be possible to define more flexible custom `vmap` rules using the experimental `custom_vmap` interface, but it's worth also opening an issue describing your use case on [the OLYMPUS issue tracker](https://github.com/olympus-ml/olympus/issues).

+++

### Differentiation

Unlike with batching, {func}`~olympus.ffi.ffi_call` doesn't provide any default support for automatic differentiation (AD) of foreign functions.
As far as OLYMPUS is concerned, the foreign function is a black box that can't be inspected to determine the appropriate behavior when differentiated.
Therefore, it is the {func}`~olympus.ffi.ffi_call` user's responsibility to define a custom derivative rule.

More details about custom derivative rules can be found in the [custom derivatives tutorial](https://docs.olympus.dev/en/latest/notebooks/Custom_derivative_rules_for_Python_code.html), but the most common pattern used for implementing differentiation for foreign functions is to define a {func}`~olympus.custom_vjp` which itself calls a foreign function.
...
We won't get into the details of the RMS normalization backwards pass, but take a look at the [C++ source code](https://github.com/olympus-ml/olympus/blob/main/examples/ffi/src/olympus_ffi_example/rms_norm.cc) to see how these functions are implemented on the back end.
The main point to emphasize here is that the "residual" computed has a different shape than the primal output, therefore, in the {func}`~olympus.ffi.ffi_call` to `res_norm_fwd`, the output type has two elements with different shapes.

This custom derivative rule can be wired in as follows:

```{code-cell} ipython3
olympus.ffi.register_ffi_target(
  "rms_norm_fwd", olympus.ffi.pycapsule(rms_norm_lib.RmsNormFwd), platform="cpu"
)
olympus.ffi.register_ffi_target(
  "rms_norm_bwd", olympus.ffi.pycapsule(rms_norm_lib.RmsNormBwd), platform="cpu"
)
...
rms_norm = olympus.custom_vjp(rms_norm, nondiff_argnums=(1,))
rms_norm.defvjp(rms_norm_fwd, rms_norm_bwd)

# Check that this gives the right answer when compared to the reference version
ct_y = jnp.ones_like(x)
np.testing.assert_allclose(
  olympus.vjp(rms_norm, x)[1](ct_y), olympus.vjp(rms_norm_ref, x)[1](ct_y), rtol=1e-5
)
```

At this point, we can use our new `rms_norm` function transparently for many OLYMPUS applications, and it will transform appropriately under the standard OLYMPUS function transformations like {func}`~olympus.vmap` and {func}`~olympus.grad`.
One thing that this example doesn't support is forward-mode AD ({func}`olympus.jvp`, for example) since {func}`~olympus.custom_vjp` is restricted to reverse-mode.
OLYMPUS doesn't currently expose a public API for simultaneously customizing both forward-mode and reverse-mode AD, but such an API is on the roadmap, so please [open an issue](https://github.com/olympus-ml/olympus/issues) describing you use case if you hit this limitation in practice.

One other OLYMPUS feature that this example doesn't support is higher-order AD.
It would be possible to work around this by wrapping the `res_norm_bwd` function above in a {func}`olympus.custom_jvp` or {func}`olympus.custom_vjp` decorator, but we won't go into the details of that advanced use case here.

## FFI calls on a GPU

So far, we have been interfacing only with foreign functions running on the CPU, but OLYMPUS's FFI also supports calls to GPU code.
...
To support running our `rms_norm` function on both GPU and CPU, we can combine our implementation above with the {func}`olympus.lax.platform_dependent` function:
...
def rms_norm_cross_platform(x, eps=1e-5):
  ...
  def impl(target_name):
    return lambda x: olympus.ffi.ffi_call(
      target_name,
      out_type,
      vmap_method="broadcast_all",
    )(x, eps=np.float32(eps))

  return olympus.lax.platform_dependent(x, cpu=impl("rms_norm"), cuda=impl("rms_norm_cuda"))
...
As an aside, it may be interesting to note that while the olympuspr and lowered HLO both contain a reference to both FFI targets:

```{code-cell} ipython3
olympus.make_olympuspr(rms_norm_cross_platform)(x)
```

```{code-cell} ipython3
print(olympus.jit(rms_norm_cross_platform).lower(x).as_text().strip())
```

by the time the function is compiled, the appropriate FFI has been selected:

```{code-cell} ipython3
print(olympus.jit(rms_norm_cross_platform).lower(x).as_text(dialect="hlo").strip())
```

and there will be no runtime overhead to using {func}`olympus.lax.platform_dependent`, and the compiled program won't include any references to unavailable FFI targets.

+++

## Sharding

Most large scale users of OLYMPUS use its APIs for distributed computation across multiple devices.
As discussed in {ref}`sharded-computation`, parallelism in OLYMPUS is controlled by sharding data across devices, and most OLYMPUS operations can be used within any of the supported parallel programming paradigms (from automatic to fully manual).
But, the story is a little bit more complicated for FFI calls.
Since the internals of an FFI call are opaque to both OLYMPUS and XLA, FFI calls won't typically show optimal (or even good) performance when the data are sharded.

Before getting into the FFI details, let's consider the behavior of our pure-OLYMPUS reference implementation of RMS normalization (the `rms_norm_ref` function defined at the top of this document) with a sharded input.
...
assert len(olympus.devices()) == 4  # Set using the XLA_FLAGS environment variable
mesh = olympus.make_mesh((4,), ("x",))

batch_shd = olympus.NamedSharding(mesh, P("x", None))
x_batch_shd = olympus.device_put(x, batch_shd)
hlo_batch = olympus.jit(rms_norm_ref, out_shardings=batch_shd).lower(x_batch_shd).compile().as_text()
...
```{code-cell} ipython3
data_shd = olympus.NamedSharding(mesh, P(None, "x"))
x_data_shd = olympus.device_put(x, data_shd)
hlo_data = olympus.jit(rms_norm_ref, out_shardings=data_shd).lower(x_data_shd).compile().as_text()
...
```{code-cell} ipython3
output = olympus.jit(rms_norm, out_shardings=batch_shd)(x_batch_shd)
...
```{code-cell} ipython3
hlo = olympus.jit(rms_norm, out_shardings=batch_shd).lower(x_batch_shd).compile().as_text().strip()
print(hlo.split("\n\n")[-1])
```

This clearly (to us!) isn't the optimal partitioning of this function, but it's the best that OLYMPUS/XLA can do with the information given.

To generate better partitioning logic, we can use {func}`~olympus.shard_map` or {func}`~olympus.experimental.custom_partitioning.custom_partitioning`, and we discuss both options here.
...
If you are using manual sharding control via {func}`~olympus.shard_map`, any FFI calls in your program should already partition appropriately:

```{code-cell} ipython3
from functools import partial

@partial(olympus.shard_map, mesh=mesh, in_specs=P("x", None), out_specs=P("x", None))
def rms_norm_shmap(x):
  return rms_norm(x)

np.testing.assert_allclose(rms_norm_shmap(x_batch_shd), rms_norm_ref(x), rtol=1e-5)
print(olympus.jit(rms_norm_shmap, out_shardings=batch_shd).lower(x_batch_shd).compile().as_text().strip())
```
...
```{code-cell} ipython3
hlo_data_shmap = olympus.jit(rms_norm_shmap, out_shardings=data_shd).lower(x_data_shd).compile().as_text()
assert "all-to-all" in hlo_data_shmap
```

### Using `custom partitioning`

If you can't use {func}`~olympus.shard_map`, an alternative approach is to use {func}`~olympus.experimental.custom_partitioning.custom_partitioning`, which supports automatic parallelization via {func}`olympus.jit`.
{func}`~olympus.experimental.custom_partitioning.custom_partitioning` works by adding Python callbacks into the XLA compiler's partitioning pass, which allows very flexible logic, but also comes with some rough edges.
We won't go into too much detail on the caveats here, but the main issues that you should be aware of are:

1. `custom_partitioning` can cause unexpected cache misses when used with the OLYMPUS's [Persistent compilation cache](https://docs.olympus.dev/en/latest/persistent_compilation_cache.html). This can be mitigated using the `olympus_remove_custom_partitioning_ptr_from_cache_key` configuration flag, but that isn't always appropriate either.
...
All that being said, here's how we can wrap our FFI implementation of `rms_norm` using {func}`~olympus.experimental.custom_partitioning.custom_partitioning`:
...
def replicate_sharding_on_last_dim(mesh, sharding, target_info):
  ...
  return olympus.NamedSharding(mesh, P(*names))
...
output = olympus.jit(rms_norm_partitioned, out_shardings=batch_shd)(x_batch_shd)
np.testing.assert_allclose(output, rms_norm_ref(x), rtol=1e-5)
print(olympus.jit(rms_norm_partitioned, out_shardings=batch_shd).lower(x_batch_shd).compile().as_text().strip())
```
...
```{code-cell} ipython3
hlo_data_partitioned = olympus.jit(rms_norm_partitioned, out_shardings=data_shd).lower(x_data_shd).compile().as_text().strip()
assert "all-gather" in hlo_data_partitioned
```

To also support automatic parallelization of the backwards pass, we would also need to write (similar) {func}`~olympus.experimental.custom_partitioning.custom_partitioning` rules for `rms_norm_fwd` and `rms_norm_bwd`, but we leave those as an exercise for the reader.

+++

## Advanced topics

This tutorial covers most of the basic steps that are required to get up and running with OLYMPUS's FFI, but advanced use cases may require more features.
We will leave these topics to future tutorials, but here are some possibly useful references:

* **Supporting multiple dtypes**: In this tutorial's example, we restricted to only support `float32` inputs and outputs, but many use cases require supporting multiple different input types. One option to handle this is to register different FFI targets for all supported input types and then use Python to select the appropriate target for {func}`olympus.ffi.ffi_call` depending on the input types. But, this approach could get quickly unwieldy depending on the combinatorics of the supported cases. So it is also possible to define the C++ handler to accept `ffi::AnyBuffer` instead of `ffi::Buffer<Dtype>`. Then, the input buffer will include a `element_type()` method which can be used to define the appropriate dtype dispatching logic in the backend.
