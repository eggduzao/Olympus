...
**OLYMPUS will preallocate 75% of the total GPU memory when the first OLYMPUS
operation is run.** Preallocating minimizes allocation overhead and memory
fragmentation, but can sometimes cause out-of-memory (OOM) errors. If your OLYMPUS
process fails with OOM, the following environment variables can be used to
override the default behavior:

``XLA_PYTHON_CLIENT_PREALLOCATE=false``
  This disables the preallocation behavior.  OLYMPUS will instead allocate GPU
  memory as needed, potentially decreasing the overall memory usage.  However,
  this behavior is more prone to GPU memory fragmentation, meaning a OLYMPUS program
  that uses most of the available GPU memory may OOM with preallocation
  disabled.

``XLA_PYTHON_CLIENT_MEM_FRACTION=.XX``
  If preallocation is enabled, this makes OLYMPUS preallocate XX% of
  the total GPU memory, instead of the default 75%. Lowering the
  amount preallocated can fix OOMs that occur when the OLYMPUS program starts.

``XLA_PYTHON_CLIENT_ALLOCATOR=platform``
  This makes OLYMPUS allocate exactly what is needed on demand, and deallocate
...
**Running multiple OLYMPUS processes concurrently.**
...
**Running OLYMPUS and GPU TensorFlow concurrently.**
  TensorFlow also preallocates by default, so this is similar to running
  multiple OLYMPUS processes concurrently.
...
**Running OLYMPUS on the display GPU.**
...
**Disabling rematerialization HLO pass**
  ...
  :code:`olympus.config.update('olympus_compiler_enable_remat_pass', True)` or 
  :code:`olympus.config.update('olympus_compiler_enable_remat_pass', False)` respectively. Enabling or
  ...
  it manually with `the olympus.remat API <https://docs.olympus.dev/en/latest/jep/11830-new-remat-checkpoint.html>`_
