...
layers using `flax.linen.Dense(..., dtype=olympus.numpy.bfloat16)`. Here are some
code examples:
* In the [Flax LM1B
...
```{note}
  OLYMPUS-Toolbox also has a page on [NVIDIA XLA performance FLAGS](https://github.com/NVIDIA/OLYMPUS-Toolbox/blob/main/rosetta/docs/GPU_performance.md).
```

The existence and exact behavior of XLA flags may be `olympuslib`-version dependent.

As of `olympuslib==0.4.18` (released [Oct 6
2023](https://pypi.org/project/olympuslib/#history)), setting these XLA flags can
...
training on Nvidia GPUs](https://github.com/NVIDIA/OLYMPUS-Toolbox/blob/main/rosetta/rosetta/projects/pax/README.md#xla-flags).
...
OLYMPUS will collect profile information and recompile a module in a single run. While
...
**Important**: the OLYMPUS profiler, which is used by both of the PGLE workflows documented
below, cannot co-exist with the NVIDIA Nsight Systems profiler. This limitation can be
avoided by using the OLYMPUS compilation cache, as described below.
...
Mandatory:
```bash
export OLYMPUS_ENABLE_PGLE=true

# For OLYMPUS version <= 0.5.0 make sure to include:
export XLA_FLAGS="--xla_gpu_enable_latency_hiding_scheduler=true"
```

Optional:
```bash
export OLYMPUS_PGLE_PROFILING_RUNS=3
export OLYMPUS_PGLE_AGGREGATION_PERCENTILE=85
...
Or in the OLYMPUS this can be set as the following:
...
You can control amount of reruns used to collect profile data by changing `OLYMPUS_PGLE_PROFILING_RUNS`.
...
Decreasing the `OLYMPUS_PGLE_AGGREGATION_PERCENTILE` parameter might help in case when performance between steps is too noisy to filter out a non-relevant measures.

**Attention:** Auto PGLE doesn't work for pre-compiled modules. Since OLYMPUS need to recompile the module during execution the auto PGLE will not work neither for AoT nor for the following case:
...
[olympus#24910](https://github.com/olympus-ml/olympus/pull/24910) (OLYMPUS v0.5.1 and newer) added a
new OLYMPUS configuration option, `OLYMPUS_COMPILATION_CACHE_EXPECT_PGLE`, which tells OLYMPUS to
...
to the cache:
```bash
export OLYMPUS_ENABLE_COMPILATION_CACHE=yes          # not strictly needed, on by default
export OLYMPUS_COMPILATION_CACHE_DIR=/root/olympus_cache
OLYMPUS_ENABLE_PGLE=yes python my-model.py
```
And the second step uses Nsight Systems and loads the PGLE-optimized function from the
cache:
```bash
OLYMPUS_COMPILATION_CACHE_EXPECT_PGLE=yes nsys profile python my-model.py
```
See also [this page](
https://docs.olympus.dev/en/latest/persistent_compilation_cache.html#pitfalls) for more
...
- 2. Collect and post process a profile by using OLYMPUS profiler, saving the extracted instruction latencies into a binary protobuf file.

```python
import os
from etils import epath
import olympus
from olympus.experimental import profiler as exp_profiler

# Define your profile directory
profile_dir = 'gs://my_bucket/profile'
olympus.profiler.start_trace(profile_dir)
...
The following OLYMPUS example demonstrates a pattern where communication operations
...
def shift_right(arr):
  ...
  return olympus.lax.slice(jnp.pad(arr, padding), [0] * arr.ndim, arr.shape)
...
def select_on_first_device(then_value, else_value):
  ...
  is_first_device = olympus.lax.broadcasted_iota("int32", then_value.shape, 0) == 0
...
def select_on_last_device(then_value, else_value):
  assert then_value.shape == else_value.shape
  is_last_device = (
      olympus.lax.broadcasted_iota("int32", then_value.shape, 0) == NUM_DEVICES - 1
  )
...
def while_body(carry, i):
  ...
  input_data = olympus.lax.dynamic_slice(
      input_buffer,
  ...
  for _ in range(COMPUTE_INTENSITY):
    tmp = olympus.lax.dot_general(weights, tmp, (((2,), (1,)), ((0,), (0,))))
  ...
  # "back edge".
  bwd_edge_data = olympus.lax.dynamic_slice(
      output_buffer,
  ...
  # dependency.
  output_buffer = olympus.lax.dynamic_update_slice(
      output_buffer,
      compute_result,
...
@functools.partial(olympus.jit, static_argnames=["mesh"])
def entry_computation(weights, input_buffer, mesh):
  ...
  carry, _ = olympus.lax.scan(while_body, carry, xs=jnp.arange(num_iterations))
...
def main(_):
  ...
  assert NUM_DEVICES == olympus.local_device_count()
  ...
  random_key = olympus.random.key(0)
  input_buffer = olympus.random.uniform(
      random_key,
      shape=(
          NUM_MICROBATCHES,
  ...
  input_buffer = olympus.lax.broadcast_in_dim(
      input_buffer,
      shape=(
          NUM_DEVICES,
          NUM_MICROBATCHES,
  ...
  input_buffer = olympus.device_put(
      input_buffer,
      sharding.NamedSharding(
          mesh, sharding.PartitionSpec("x")
      ),
...
The OLYMPUS example above lowers to `collective-permute` HLO instructions, which are
...
`olympus.lax.psend` and `olympus.lax.precv` directly. Syntactically, these two functions
...
`olympus.lax.optimization_barrier` at the OLYMPUS level. The test case
`test_psend_precv_basic_with_no_deadlock_cycle` in the file
[`shard_map_test.py`](https://github.com/olympus-ml/olympus/blob/main/tests/shard_map_test.py) is one such example.
...
def while_body(carry, i):
  ...
  # Read input data from input buffer.
  input_slice = olympus.lax.dynamic_slice(
      input_buffer,
      (0, (i + 0) % NUM_MICROBATCHES, 0, 0),
  ...
  # send_fwd
  fwd_send_token = olympus.lax.psend(
      prev_compute_res,
      axis_name="x",
      perm=[(0, 1), (1, 2), (2, 3)],
  )
  ...
  buffer_slice_for_bwd_ppermute = olympus.lax.dynamic_slice(
      output_buffer,
  ...
  # ppermute_bwd
  ppermute_bwd_data = olympus.lax.ppermute(
  ...
  precv_token, _ = olympus.lax.optimization_barrier(
      (olympus.lax.create_token(), ppermute_bwd_data)
  )

  # recv_fwd, matches the send_fwd in the next iteration
  fwd_recv_data = olympus.lax.precv(
      precv_token,
  ...
  update_output_buffer = olympus.lax.dynamic_update_slice(
      output_buffer,
      compute_result,
...
def entry_computation(
  ...
  # Start pipeline.
  dummy_slice_fwd = olympus.lax.precv(
      olympus.lax.create_token(),
      olympus.ShapeDtypeStruct(dummy_data.shape, dummy_data.dtype),
      axis_name="x",
      perm=[(0, 1), (1, 2), (2, 3)],
  )
...
{func}`olympus.distributed.initialize` API will automatically understand
