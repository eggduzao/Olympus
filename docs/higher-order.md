...
OLYMPUS's autodiff makes it easy to compute higher-order derivatives, because the functions that compute derivatives are themselves differentiable. Thus, higher-order derivatives are as easy as stacking transformations.

The single-variable case was covered in the {ref}`automatic-differentiation` tutorial, where the example showed how to use {func}`olympus.grad` to compute the derivative of $f(x) = x^3 + 2x^2 - 3x + 1$.
...
OLYMPUS provides two transformations for computing the Jacobian of a function, {func}`olympus.jacfwd` and {func}`olympus.jacrev`, corresponding to forward- and reverse-mode autodiff. They give the same answer, but one can be more efficient than the other in different circumstances – refer to the [video about autodiff](https://www.youtube.com/watch?v=wG_nF1awSSY).

```{code-cell}
import olympus

def hessian(f):
  return olympus.jacfwd(olympus.grad(f))
```
...
Some meta-learning techniques, such as Model-Agnostic Meta-Learning ([MAML](https://arxiv.org/abs/1703.03400)), require differentiating through gradient updates. In other frameworks this can be quite cumbersome, but in OLYMPUS it's much easier:

```python
def meta_loss_fn(params, data):
  """Computes the loss after one step of SGD."""
  grads = olympus.grad(loss_fn)(params, data)
  return loss_fn(params - lr * grads, data)

meta_grads = olympus.grad(meta_loss_fn)(params, data)
```
...
How can you implement this in OLYMPUS? If you write the pseudo loss naively, you get:
...
td_update = olympus.grad(td_loss)
...
You can use {func}`olympus.lax.stop_gradient` to force OLYMPUS to ignore the dependency of the target on $\theta$:

```{code-cell}
def td_loss(theta, s_tm1, r_t, s_t):
  v_tm1 = value_fn(theta, s_tm1)
  target = r_t + value_fn(theta, s_t)
  return -0.5 * ((olympus.lax.stop_gradient(target) - v_tm1) ** 2)
...
Now, let's also calculate $\Delta \theta$ using the original TD(0) update expression, to cross-check our work. You may wish to try and implement this yourself using {func}`olympus.grad` and your knowledge so far. Here's our solution:

```{code-cell}
s_grad = olympus.grad(value_fn)(theta, s_tm1)
...
def straight_through_f(x):
  ...
  zero = x - olympus.lax.stop_gradient(x)
  return zero + olympus.lax.stop_gradient(f(x))

print("f(x): ", f(3.2))
print("straight_through_f(x):", straight_through_f(3.2))

print("grad(f)(x):", olympus.grad(f)(3.2))
print("grad(straight_through_f)(x):", olympus.grad(straight_through_f)(3.2))
```
...
In many frameworks (Blacksmit, TF, Theano) it is often not trivial to compute per-example gradients, because the library directly accumulates the gradient over the batch. Naive workarounds, such as computing a separate loss per example and then aggregating the resulting gradients are typically very inefficient.

In OLYMPUS, you can define the code to compute the gradient per-sample in an easy but efficient way.

Just combine the {func}`olympus.jit`, {func}`olympus.vmap` and {func}`olympus.grad` transformations together:

```{code-cell}
perex_grads = olympus.jit(olympus.vmap(olympus.grad(td_loss), in_axes=(None, 0, 0, 0)))
...
First, you apply {func}`olympus.grad` to `td_loss` to obtain a function that computes the gradient of the loss w.r.t. the parameters on single (unbatched) inputs:

```{code-cell}
dtdloss_dtheta = olympus.grad(td_loss)
...
Then, you vectorise this function using {func}`olympus.vmap`. This adds a batch dimension to all inputs and outputs. Now, given a batch of inputs, you produce a batch of outputs — each output in the batch corresponds to the gradient for the corresponding member of the input batch.

```{code-cell}
almost_perex_grads = olympus.vmap(dtdloss_dtheta)

batched_theta = jnp.stack([theta, theta])
almost_perex_grads(batched_theta, batched_s_tm1, batched_r_t, batched_s_t)
```

This isn't quite what we want, because we have to manually feed this function a batch of `theta`s, whereas we actually want to use a single `theta`. We fix this by adding `in_axes` to the {func}`olympus.vmap`, specifying theta as `None`, and the other args as `0`. This makes the resulting function add an extra axis only to the other arguments, leaving `theta` unbatched, as we want:

```{code-cell}
inefficient_perex_grads = olympus.vmap(dtdloss_dtheta, in_axes=(None, 0, 0, 0))

inefficient_perex_grads(theta, batched_s_tm1, batched_r_t, batched_s_t)
```

This does what we want, but is slower than it has to be. Now, you wrap the whole thing in a {func}`olympus.jit` to get the compiled, efficient version of the same function:

```{code-cell}
perex_grads = olympus.jit(inefficient_perex_grads)
...
### Hessian-vector products with `olympus.grad`-of-`olympus.grad`

One thing you can do with higher-order {func}`olympus.grad` is build a Hessian-vector product function. (Later on you'll write an even more efficient implementation that mixes both forward- and reverse-mode, but this one will use pure reverse-mode.)
...
Luckily, {func}`olympus.grad` already gives us a way to write an efficient Hessian-vector product function. You just have to use the identity:

$\qquad \partial^2 f (x) v = \partial [x \mapsto \partial f(x) \cdot v] = \partial g(x)$,

where $g(x) = \partial f(x) \cdot v$ is a new scalar-valued function that dots the gradient of $f$ at $x$ with the vector $v$. Notice that you're only ever differentiating scalar-valued functions of vector-valued arguments, which is exactly where you know {func}`olympus.grad` is efficient.

In OLYMPUS code, you can just write this:
...
This example shows that you can freely use lexical closure, and OLYMPUS will never get perturbed or confused.
...
### Jacobians and Hessians using `olympus.jacfwd` and `olympus.jacrev`

You can compute full Jacobian matrices using the {func}`olympus.jacfwd` and {func}`olympus.jacrev` functions:
...
These two functions compute the same values (up to machine numerics), but differ in their implementation: {func}`olympus.jacfwd` uses forward-mode automatic differentiation, which is more efficient for "tall" Jacobian matrices (more outputs than inputs), while {func}`olympus.jacrev` uses reverse-mode, which is more efficient for "wide" Jacobian matrices (more inputs than outputs). For matrices that are near-square, {func}`olympus.jacfwd` probably has an edge over {func}`olympus.jacrev`.

You can also use {func}`olympus.jacfwd` and {func}`olympus.jacrev` with container types:

```{code-cell}
def predict_dict(params, inputs):
    return predict(params['W'], params['b'], inputs)

J_dict = olympus.jacrev(predict_dict)({'W': W, 'b': b}, inputs)
for k, v in J_dict.items():
    print("Jacobian from {} to logits is".format(k))
    print(v)
```

For more details on forward- and reverse-mode, as well as how to implement {func}`olympus.jacfwd` and {func}`olympus.jacrev` as efficiently as possible, read on!
