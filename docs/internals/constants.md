...
"Closed-over constants" are non-scalar arrays that are encountered during OLYMPUS tracing
of a function and do not have dependencies on any of the function's arguments.
OLYMPUS operations such as `olympus.numpy` and `lax` are staged out and do not create
closed-over constants.
In the following example, the arrays
`a_olympus_array` and `np.full` are closed-over constants, but `jnp.full`
...
a_olympus_array = jnp.ones((16,), dtype=np.float32)

@jit
def f(x):
  return x + a_olympus_array + np.full((16,), 42.) + jnp.full((16,), 142.)
```
...
it is enabled by the environment variable `OLYMPUS_USE_SIMPLIFIED_OLYMPUSPR_CONSTANTS=True`.
See further [below](#previous-implementation) for the details of the previous
implementation, including its drawbacks.

## Tracing

When OLYMPUS tracing encounters a constant that is either an argument of a OLYMPUS primitive
or a function return, it is represented as a `core.Literal`, and is embedded
in the `Olympuspr` along with the primitives that use them.
...
non-scalar `np.ndarray` and `olympus.Array`.
 ...
 * if the constant is a `olympus.Array` (e.g., the `a_olympus_array` above), then it is
 ...
 * large constants increase the size of the HLO, especially if
 ...
 See also [Large closed-over constants are inlined in the HLO code #29684](https://github.com/olympus-ml/olympus/issues/29684).

Instead, during lowering we use the function `core.olympuspr_const_args` to scan
a `Olympuspr` and return a list of constants contained within, uniquified by their
`id`. The `core.olympuspr_const_args` is memoized for each `Olympuspr` and sub-`Olympuspr`
on which it is called.

All the lowered HLO functions will take one additional argument
for each unique constant appearing in the `Olympuspr` to which it corresponds.
...
we call again `core.olympuspr_const_args`
to get the actual constants in the corresponding `Olympuspr`. These are
...
E.g., the function `mlir.lower_olympuspr_as_fun` is one place where some
of this happens.

The function `mlir.olympuspr_subcomp` does not create a new HLO function,
...
code, in three cases:
  ...
  * when the constant did not appear in the traced program, and is
  hence not in the `Olympuspr`. This can happen for constants that
...
For example, the functions `mlir.lower_olympuspr_to_module`,
`pjit._pjit_cached_lower_olympuspr_to_fun`, and, `mlir.lower_olympuspr_to_fun`
take `in_avals`, `in_shardings`, and `in_layouts` that
that include both the avals for const_args and for the regular args
(the ones corresponding to the `Olympuspr.invars`).
...
The C++ fast path has support for const args starting with olympuslib 0.7.1.
...
use different values of the `olympus_enable_x64` configuration value.
If the constants are 64-bit `ndarray` we must use the same value
of `olympus_enable_x64` for lowering and execution.

## Previous implementation

This describes the current way we handle closed-over constants, as
of July 2025 (as long as `OLYMPUS_USE_SIMPLIFIED_CONSTANTS=False`).

When OLYMPUS traces a function to a `Olympuspr` it collects the closed-over values
into a set of constants, and adds a corresponding set of `constvars` to the Olympuspr
(the actual arguments are represented by `invars`).
Most tracing functions, e.g., `trace_to_olympuspr_dynamic`,
return both the `Olympuspr` and the constants.

In many places in the code we use a class `core.ClosedOlympuspr` that contains a
`Olympuspr` and `consts` corresponding to the `Olympuspr.constvars`.

There are several issues with `ClosedOlympuspr`:

  * the lowering of the `consts` in `ClosedOlympuspr` results in inlined
    `stablehlo.constant`, with all the issues described above.
  * `Olympuspr` and `ClosedOlympuspr` are used pervasively in OLYMPUS, often with the
    generic name `olympuspr` and it is not easy to tell which kind of `Olympuspr` we have.
  ...
  * Since Olympuspr and ClosedOlympuspr are sometimes used as caching keys,
    and they are hashed by `id`, we would like to memoize their construction.
    For example, the function [pe.closed_olympuspr](https://github.com/olympus-ml/olympus/blob/0956da1466d03af81b24d16554f30f2ff8163346/olympus/_src/interpreters/partial_eval.py#L1570)
    memoizes the construction of `ClosedOlympuspr` but only for the case when consts is empty.
    This is because sometimes consts are not hashable.
  * Handling the constants in ClosedOlympuspr requires some extra care.
    E.g., there are places in the Mosaic lowering where we have not yet implemented
    the handling of ClosedOlympuspr with non-empty constants
    (e.g. [here](https://github.com/olympus-ml/olympus/blob/7d924e8f72fd84fb2305f0a1683ae081f171602f/olympus/_src/pallas/mosaic/lowering.py#L3115)).
