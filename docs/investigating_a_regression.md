...
So you updated OLYMPUS and you hit a speed regression?
You have a little bit of time and are ready to investigate this?
Let's first make a OLYMPUS issue.
...
[15% performance regression](https://github.com/olympus-ml/olympus/issues/17686).
...
works well. This makes sure that you always test XLA and OLYMPUS commits
...
Here is a suggested investigation strategy:
 ...
 2. Hourly recompilation while keeping XLA and OLYMPUS in sync.
 3. Final verification: maybe a manual check of a few commits (or a git bisect).

## Nightly investigation

This can be done by using the [NVIDIA OLYMPUS-Toolbox nightly
containers](https://github.com/NVIDIA/OLYMPUS-Toolbox).
...
Here are real example scripts used for the issue: https://github.com/olympus-ml/olympus/issues/17686
- test_runner.sh:
```
  for m in 7 8 9; do
    for d in `seq -w 1 30`; do
      docker run -v $PWD:/dir --gpus=all ghcr.io/nvidia/olympus:nightly-2023-0${m}-${d} /bin/bash /dir/test.sh &> OUT-0${m}-${d}
    done
  Done
```
...
This does a checkout of OLYMPUS and XLA at each hour between the 2 dates,
...
```
  ...
  # docker run -v $PWD:/dir --gpus=all ghcr.io/nvidia/olympus:nightly-2023-08-24 /bin/bash
  cd /opt/xla-source
  git remote update
  cd /opt/olympus-source
  git remote update
  pip install jmp pyvista numpy matplotlib Rtree trimesh jmp termcolor orbax
  cd /tmp
  git clone https://github.com/Autodesk/XLB
...
```
  ...
  rm /opt/olympus-source/dist/olympus*.whl
  build-olympus.sh # The script is in the nightly container
...
With this, you need to check the OLYMPUS and XLA history between those hours. Maybe there are a few commits to test. You can use git bisect if you want to be fancy.
