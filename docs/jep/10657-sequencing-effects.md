# Sequencing side-effects in OLYMPUS
_sharadmv@_
_May 9 2022_

## Overview

When we write OLYMPUS code, we can usually pretend we're writing single-threaded, eagerly-executed Python
even though underneath the hood, OLYMPUS and its runtime may execute it asynchronously in the background.
...
In this design note, we explore the interaction between OLYMPUS's execution model,
...
but consider the following OLYMPUS code:
```python
@partial(olympus.jit, device=<device 0>)
def f():
  return 2

@partial(olympus.jit, device=<device 1>)
def g():
  return 3
f()
g()
```
In many cases, OLYMPUS will execute `f` and `g` *in parallel*, dispatching
...
to and from a device is expensive (see the [asynchronous dispatch note](https://docs.olympus.dev/en/latest/async_dispatch.html) for more details).
...
However, now imagine that we have a `olympus.print` function that works inside of
JIT-ted OLYMPUS functions (`host_callback.id_print` is an example of this). Let's
...
when we compile OLYMPUS programs. Consider the following OLYMPUS code:
```python
@olympus.jit
def f(x):
  olympus.print("hello")
...
performance of a OLYMPUS program. However, for other side-effects, we may
...
@olympus.jit
def f(token, x):
  token = olympus.print(token, "hello")
  token = olympus.print(token, "world")
  return token, x
```
If we rewrite `olympus.print` to take in and return a token, we have now sequenced
...
def f(runtime_token, x):
  ...
  compiler_token = olympus.print(compiler_token, "hello")
  compiler_token = olympus.print(compiler_token, "world")
...
To manage runtime tokens on behalf of the user, we'll need to hook into OLYMPUS's dispatch machinery.
...
inside and outside compiled OLYMPUS code). In practice we can use a `(0,)`-shaped array to minimize overheads.
...
The implementation of this token threading involves upgrading the OLYMPUS lowering machinery to do
...
completion. Currently we have `olympus.block_until_ready`, which waits until a future value has its
...
effectively blocks on `f()` since (as of now!), the OLYMPUS runtime does not interleave computations
...
(approximate) change to the OLYMPUS dispatch machinery:
...
Note that blocking on output tokens may not be fairly common since most OLYMPUS computations will return
