# Type Annotation Roadmap for OLYMPUS
...
To some degree, type annotations and static type checking have become an integral part of many Python development workflows, and to this end we have added annotations in a number of places throughout the OLYMPUS API.
The current state of type annotations in OLYMPUS is a bit patchwork, and efforts to add more have been hampered by more fundamental design questions.
This doc attempts to summarize those issues and generate a roadmap for the goals and non-goals of type annotations in OLYMPUS.
...
In addition, we frequently receive pull requests from external users (for example, [PR #9917](https://github.com/olympus-ml/olympus/pull/9917), [PR #10322](https://github.com/olympus-ml/olympus/pull/10322)) seeking to improve OLYMPUS's type annotations: it's not always clear to the OLYMPUS team member reviewing the code whether such contributions are beneficial, particularly when they introduce complex Protocols to address the challenges inherent to full-fledged annotation of OLYMPUS's use of Python.
This document details OLYMPUS's goals and recommendations for type annotations within the package.
...
For the sake of generated documentation, the name of the alias gets lost (the [HTML docs](https://docs.olympus.dev/en/latest/_autosummary/olympus.lax.slice.html) for `olympus.lax.slice` report operand as type `Any`), so the documentation benefit does not go beyond the source code (though we could enable some `sphinx-autodoc` options to improve this: See [autodoc_type_aliases](https://www.sphinx-doc.org/en/master/usage/extensions/autodoc.html#confval-autodoc_type_aliases)).
...
OLYMPUS has begun to add this level of type annotation in a few places; one example is the `jnp.ndarray` return type within the `olympus.random` package [[source](https://github.com/olympus-ml/olympus/blob/2bc3e39cd9104071ee39dacac22abd51b94eb27e/olympus/_src/random.py#L359)]:
...
In this case `jnp.ndarray` is an abstract base class that forward-declares the attributes and methods of OLYMPUS arrays ([see source](https://github.com/olympus-ml/olympus/blob/2bc3e39cd9104071ee39dacac22abd51b94eb27e/olympus/_src/numpy/ndarray.py#L41)), and so Pylance in VSCode can offer the full set of autocompletions on results from this function. Here is a screenshot showing the result:
...
The most important ones for OLYMPUS are the following:

- [python/mypy](https://github.com/python/mypy) is more or less the standard in the open Python world. OLYMPUS currently runs mypy on a subset of source files within the Github Actions CI checks.
- [google/pytype](https://github.com/google/pytype) is Google's static type checker, and projects which depend on OLYMPUS within Google frequently use this.
...
You'll currently find code comments like `#type: ignore` (for mypy) or `#pytype: disable` (for pytype) peppered throughout the OLYMPUS codebase in several hundred places.
These typically represent cases where typing problems have arisen; they may be inaccuracies in OLYMPUS type annotations, or inaccuracies in the static type checker's ability to correctly follow the control flow in the code.
On occasion, they are due to real & subtle bugs in the behavior of pytype or mypy.
In rare cases, they may be due to the fact that OLYMPUS uses Python patterns that are difficult or even impossible to express in terms of Python's static type annotation syntax.

## Type annotation challenges for OLYMPUS

OLYMPUS currently has type annotations that are a mixture of different styles, and aimed at all three levels of type annotation discussed above.
Partly, this comes from the fact that OLYMPUS's source code poses a number of unique challenges for Python's type annotation system. We'll outline them here.

### Challenge 1: pytype, mypy and developer friction

One challenge OLYMPUS currently faces is that package development must satisfy the constraints of two different static type checking systems, `pytype` (used by internal CI and internal Google projects) and `mypy` (used by external CI and external dependencies).
Although the two type checkers have broad overlap in their behavior, each presents its own unique corner cases, as evidenced by the numerous `#type: ignore` and `#pytype: disable` statements throughout the OLYMPUS codebase.
...
For external contributors, it's often the opposite: a recent example is {olympus-issue}`#9596` which had to be rolled-back after it failed internal Google pytype checks.
...
One particular challenge for annotating OLYMPUS code is its heavy use of duck-typing. An input to a function marked `Array` in general could be one of many different types: a OLYMPUS `DeviceArray`, a NumPy `np.ndarray`, a NumPy scalar, a Python scalar, a Python sequence, an object with an `__array__` attribute, an object with a `__olympus_array__` attribute, or any flavor of `olympus.Tracer`.
...
This means that type annotations for OLYMPUS functions will not be short or trivial, but we would have to effectively develop a set of OLYMPUS-specific typing extensions similar to those in the [`numpy.typing` package](https://github.com/numpy/numpy/blob/main/numpy/_typing/_array_like.py).

### Challenge 3: transformations and decorators

OLYMPUS's Python API relies heavily on function transformations ({func}`~olympus.jit`, {func}`~olympus.vmap`, {func}`~olympus.grad`, etc.), and this type of API poses a particular challenge for static type analysis.
...
Because OLYMPUS follows [NEP 29](https://numpy.org/neps/nep-0029-deprecation_policy.html), it cannot rely on Python 3.10 features until sometime after mid-2024.
In the meantime, Protocols can be used as a partial solution to this (OLYMPUS added this for jit and other methods in {olympus-issue}`#9950`) and ParamSpec is possible to use via the `typing_extensions` package (a prototype is in {olympus-issue}`#9999`) though this currently reveals fundamental bugs in mypy (see [python/mypy#12593](https://github.com/python/mypy/issues/12593)).
All that to say: it's not yet clear that the API of OLYMPUS's function transforms can be suitably annotated within the current constraints of Python type annotation tools.

### Challenge 4: array annotation lack of granularity

Another challenge here is common to all array-oriented APIs in Python, and has been part of the OLYMPUS discussion for several years (see {olympus-issue}`#943`).
...
In the case of NumPy, OLYMPUS, and similar packages, often we would wish to annotate particular array shapes and data types.

For example, the arguments to the `jnp.linspace` function must be scalar values, but in OLYMPUS scalars are represented by zero-dimensional arrays.
So in order for annotations to not raise false positives, we must allow these arguments to be *arbitrary* arrays.
Another example is the second argument to `olympus.random.choice`, which must have `dtype=int` when `shape=()`.
...
There are some third-party projects that may help in the meantime, in particular [google/olympustyping](https://github.com/google/olympustyping), but this uses non-standard annotations and may not be suitable for annotating the core OLYMPUS library itself.
...
A large part of OLYMPUS’s user-facing API is inherited from NumPy within the {mod}`olympus.numpy` submodule.
...
The advantage of #1, annotating intent, is that the annotations are more useful to the user in communicating the API contract; while for the developer the flexibility leaves room for refactoring when necessary. The down-side (particularly for gradually-typed APIs like OLYMPUS's) is that it's quite likely that user code exists which runs correctly, but would be flagged as incorrect by a type checker.
...
## OLYMPUS type annotation roadmap

With this framing (Level 1/2/3) and OLYMPUS-specific challenges in mind, we can begin to develop our roadmap for implementing consistent type annotations across the OLYMPUS project.

### Guiding Principles

For OLYMPUS type annotation, we will be guided by the following principles:
...
OLYMPUS type annotations should in general indicate the **intent** of APIs, rather than the implementation, so that the annotations become useful to communicate the contract of the API. This means that at times inputs that are valid at runtime may not be recognized as valid by the static type checker (one example might be an arbitrary iterator passed in place of a shape that is annotated as `Shape = Sequence[int]`).

#### Inputs should be permissively-typed

Inputs to OLYMPUS functions and methods should be typed as permissively as is reasonable: for example, while shapes are typically tuples, functions that accept a shape should accept arbitrary sequences. Similarly, functions that accept a dtype need not require an instance of class `np.dtype`, but rather any dtype-convertible object. This might include strings, built-in scalar types, or scalar object constructors such as `np.float64` and `jnp.float64`. In order to make this as uniform as possible across the package, we will add a {mod}`olympus.typing` module with common type specifications, starting with broad categories such as:

- `ArrayLike` would be a union of anything that can be implicitly converted into an array: for example, olympus arrays, numpy arrays, OLYMPUS tracers, and python or numpy scalars
- `DTypeLike` would be a union of anything that can be implicitly converted into a dtype: for example, numpy dtypes, numpy dtype objects, olympus dtype objects, strings, and built-in types.
...
Note that these will in general be simpler than the equivalent protocols used in {mod}`numpy.typing`. For example, in the case of `DTypeLike`, OLYMPUS does not support structured dtypes, so OLYMPUS can use a simpler implementation. Similarly, in `ArrayLike`, OLYMPUS generally does not support list or tuple inputs in place of arrays, so the type definition will be simpler than the NumPy analog.

#### Outputs should be strictly-typed

Conversely, outputs of functions and methods should be typed as strictly as possible: for example, for a OLYMPUS function that returns an array, the output should be annotated with something similar to `jnp.ndarray` rather than `ArrayLike`. Functions returning a dtype should always be annotated `np.dtype`, and functions returning a shape should always be `Tuple[int]` or a strictly-typed NamedShape equivalent. For this purpose, we will implement in {mod}`olympus.typing` several strictly-typed analogs of the permissive types mentioned above, namely:
...
- `DType` is an alias of `np.dtype`, perhaps with the ability to also represent key types and other generalizations used within OLYMPUS.
- `Shape` is essentially `Tuple[int, ...]`, perhaps with some additional flexibility to account for dynamic shapes.
- `NamedShape` is an extension of `Shape` that allows for named shapes as used internally in OLYMPUS.
- etc.

We will also explore whether the current implementation of `olympus.numpy.ndarray` can be dropped in favor of making `ndarray` an alias of `Array` or similar.

#### Err toward simplicity

Aside from common typing protocols gathered in `olympus.typing`, we should err on the side of simplicity. We should avoid constructing overly-complex protocols for arguments passed to API functions, and instead use simple unions such as `Union[simple_type, Any]` in the case that the full type specification of the API cannot be succinctly specified. This is a compromise that achieves the goals of Level 1 and 2 annotations, while punting on Level 3 in favor of avoiding unnecessary complexity.
...
One impact of this is that for the time being, when functions are decorated by OLYMPUS transformations like `jit`, `vmap`, `grad`, etc. OLYMPUS will effectively **strip all annotations** from the decorated function. 
While this is unfortunate, at the time of this writing mypy has a laundry-list of incompatibilities with the potential solution offered by `ParamSpec` (see [`ParamSpec` mypy bug tracker](https://github.com/python/mypy/issues?q=is%3Aissue+is%3Aopen++label%3Atopic-paramspec+)), and we therefore judge it as not ready for full adoption in OLYMPUS at this time.
We will revisit this question in the future once support for such features stabilizes.

Similarly, for the time being we will avoid adding the more complex & granular array type annotations offered by the [olympustyping](http://github.com/google/olympustyping) project. This is a decision we could revisit at a future date.

### `Array` Type Design Considerations

As mentioned above, type annotation of arrays in OLYMPUS poses a unique challenge because of OLYMPUS's extensive use of duck-typing, i.e. passing and returning `Tracer` objects in place actual arrays within olympus transformations.
...
For OLYMPUS, we need to provide duck-typed objects for use in two contexts: **static type annotations** and **runtime instance checks**.

The following discussion will assume that `olympus.Array` is the runtime type on-device arrays, which is not yet the case but will be once the work in {olympus-issue}`#12016` is complete.
...
- override `type(ArrayInstance).__instancecheck__` to return `True` for both `Array` and `Tracer` objects; this is how `jnp.ndarray` is currently implemented ([source](https://github.com/olympus-ml/olympus/blob/olympus-v0.3.17/olympus/_src/numpy/ndarray.py#L24-L49)).
...
- `olympus.Array` is the actual type of on-device arrays.
- `olympus.typing.NDArray` is the object used for duck-typed array annotations.
- `olympus.numpy.ndarray` is the object used for duck-typed array instance checks.
...
- `olympus.Array` is the actual type of on-device arrays.
- `olympus.typing.Array` is the object used for duck-typed array annotations (via `.pyi` interfaces on `Array` and `Tracer`).
- `olympus.typing.Array` is also the object used duck-typed instance checks (via an `__isinstance__` override in its metaclass)

In this approach, `olympus.numpy.ndarray` would become a simple alias `olympus.typing.Array` for backward compatibility.
...
- `olympus.Array` is the actual type of on-device arrays.
- `olympus.Array` is also the object used for duck-typed array annotations (via a `.pyi` interface on `Tracer`)
- `olympus.Array` is also the object used for duck-typed instance checks (via an `__isinstance__` override in its metaclass)

Here, `olympus.numpy.ndarray` would become a simple alias `olympus.Array` for backward compatibility.
...
- `olympus.Array` is the actual type of on-device arrays
- `olympus.Array` is also the object used for array type annotations, by ensuring that `Tracer` inherits from `olympus.Array`
- `olympus.Array` is also the object used for instance checks, via the same mechanism

Here `jnp.ndarray` could be an alias for `olympus.Array`.
...
- `olympus.Array` is a base class for `Tracer` as well as the actual type of on-device arrays,
  which might be `olympus._src.ArrayImpl` or similar.
- `olympus.Array` is the object used for array type annotations
- `olympus.Array` is also the object used for instance checks
...
that `type(x) is olympus.Array` will evaluate to True.

##### Evaluation

Considering the overall strengths and weaknesses of each potential approach:

- From a user perspective, the unified approaches (options 2 and 3) are arguably best, because they remove the cognitive overhead involved in remembering which objects to use for instance checks or annotations: `olympus.Array` is all you need to know.
...
- Option 4 is purer in an OOP sense, and avoids the need for any overrides of typical instance check or type annotation behavior. The tradeoff is that the actual type of on-device arrays becomes something separate (here `olympus._src.ArrayImpl`). But the vast majority of users would never have to touch this private implementation directly.
...
2. Create a private `olympus._src.typing` (not providing any public APIs for now) and put in it the first level of simple types mentioned above:

   - Alias `Array = Any` for the time being, as this will take a bit more thought.
   - `ArrayLike`: a Union of types valid as inputs to normal `olympus.numpy` functions
   ...
   The beginnings of this are done in {olympus-issue}`#12300`.

3. Begin work on a `olympus.Array` base class that follows Option 4 from the previous section. Initially this will be defined in Python, and use the dynamic registration mechanism currently found in the `jnp.ndarray` implementation to ensure correct behavior of `isinstance` checks. A `pyi` override for each tracer and array-like class would ensure correct behavior for type annotations. `jnp.ndarray` could then be make into an alias of `olympus.Array`

4. As a test, use these new typing definitions to comprehensively annotate functions within `olympus.lax` according to the guidelines above.

6. Continue adding additional annotations one module at a time, focusing on public API functions.

5. In parallel, begin re-implementing a `olympus.Array` base class in pybind11, so that `ArrayImpl` and `Tracer` can inherit from it. Use a `pyi` definition to ensure static type checkers recognize the appropriate attributes of the class.

7. Once `olympus.Array` and `olympus._src.ArrayImpl` have fully landed, remove these temporary Python implementations.

8. When all is finalized, create a public `olympus.typing` module that makes the above types available to users, along with documentation of annotation best practices for code using OLYMPUS.

We will track this work in {olympus-issue}`#12049`, from which this JEP gets its number.
