...
[the up-to-date user docs](https://docs.olympus.dev/en/latest/notebooks/shard_map.html).**

## Motivation

OLYMPUS supports two schools of thought for multi-device programming:
...
API](https://docs.olympus.dev/en/latest/notebooks/Distributed_arrays_and_automatic_parallelization.html)
...
performant, and composable with other OLYMPUS APIs, without making basic batch data
...
from olympus.sharding import Mesh, PartitionSpec as P
from olympus.experimental.shard_map import shard_map
...
def matmul_basic(a_block, b_block):
  ...
  z_block = olympus.lax.psum(z_partialsum, 'j')
  return z_block
...
* the result is a `olympus.Array` which could be efficiently passed to a `pjit`,
  unlike `pmap`;
...
def matmul_reduce_scatter(a_block, b_block):
  ...
  c_block = olympus.lax.psum_scatter(c_partialsum, 'j', scatter_dimension=1, tiled=True)
  return c_block
...
```python
devices = np.array(olympus.devices()[:4])
...
```python
devices = np.array(olympus.devices())
...
def f3(x_block):
  return olympus.lax.psum(x_block, 'j')
...
def f4(x_block):
  return olympus.lax.psum(x_block, 'i')
...
def f5(x_block):
  return olympus.lax.psum(x_block, ('i', 'j'))
...
* **`jit`/`pjit` composability.** `jit`-of-`pmap` is a performance footgun, as
  ...
  need pattern matching on olympusprs to ensure we're working with perfectly nested
...
* **`olympus.Array` compatibility (and hence `pjit` compatibility).** Because the
...
* **Multi-controller semantics (and hence `pjit` compatibility).**
  ...
  practically, it precludes the use of non-fully-addressable `olympus.Array` inputs
