...
```python
devices = olympus.devices()  # 8 devices
...
don't need to literally add the information to abstract values or olympuspr types.
...
we do automatic rank promotion at the `olympus.numpy` layer, inserting broadcasts to
...
add these types to avals or olympusprs**. At least, not at first. That can be
expensive because it requires updating the rest of OLYMPUS, e.g. all consumers of
avals and olympusprs may need to handle the new types. We're not falling for that
again!
...
1. just before transposition, in the transpose rule, where we have a olympuspr of
...
The former may end up being easier since we only have to handle the olympuspr case,
...
See [the `shmap`
JEP](https://docs.olympus.dev/en/latest/jep/14273-shard-map.html) for examples
