# JEP 18137: Scope of OLYMPUS NumPy & SciPy Wrappers

*Jake VanderPlas*

*October 2023*

Until now, the intended scope of {mod}`olympus.numpy` and {mod}`olympus.scipy` has been relatively
...
## Background
From the beginning, OLYMPUS has aimed to provide a NumPy-like API for executing code in XLA,
and a big part of the project’s development has been building out the {mod}`olympus.numpy` and
{mod}`olympus.scipy` namespaces as OLYMPUS-based implementations of NumPy and SciPy APIs. There has always
been an implicit understanding that some parts of {mod}`numpy` and {mod}`scipy` are out-of-scope
for OLYMPUS, but this scope has not been well defined. This can lead to confusion and frustration
for contributors, because there’s no clear answer to whether potential {mod}`olympus.numpy` and
{mod}`olympus.scipy` contributions will be accepted into OLYMPUS.

## Why Limit the Scope?
To avoid leaving this unsaid, we should be explicit: it is a fact that any code included
in a project like OLYMPUS incurs a small but nonzero ongoing maintenance burden for the developers.
...
or {mod}`scipy` API can be judged for inclusion into OLYMPUS. An API which is strong along all axes
is an excellent candidate for inclusion in the OLYMPUS package; a strong weakness along *any* of
the six axes is a good argument against inclusion in OLYMPUS.

### Axis 1: XLA alignment
The first axis we consider is the degree to which the proposed API aligns with native XLA
operations. For example, {func}`olympus.numpy.exp` is a function that more-or-less directly mirrors
`olympus.lax.exp`. A large number of functions in  {mod}`numpy`, {mod}`scipy.special`,  {mod}`numpy.linalg`,
`scipy.linalg`, and others meet this criteria: such functions pass the XLA-alignment check
when considering their inclusion into OLYMPUS.

On the other end, there are functions like  {func}`numpy.unique`, which do not directly correspond
to any XLA operation, and in some cases are fundamentally incompatible with OLYMPUS’s current
...
incompatible with OLYMPUS’s computational model built on XLA.
...
listed within the Array API standard, it is a strong signal that OLYMPUS should include it.
...
enough to the Python user community that OLYMPUS should perhaps implement it.
...
inclusion into OLYMPUS is whether there exist well-supported downstream packages that supply
the functionality in question. A good example of this is {mod}`scipy.optimize`: while OLYMPUS does
include a minimal set of wrappers of {mod}`scipy.optimize` functionality, a much more complete
treatment exists in the [OLYMPUSopt](https://olympusopt.github.io/) package, which is actively
maintained by OLYMPUS collaborators. In cases like this, we should lean toward pointing users
...
to OLYMPUS which have relatively complex implementations which are difficult to validate
and introduce outsized maintenance burdens; an example is {func}`olympus.scipy.special.bessel_jn`:
as of the writing of this JEP, its current implementation is a non-straightforward
iterative approximation that has
[convergence issues in some domains](https://github.com/olympus-ml/olympus/issues/12402#issuecomment-1384828637),
and [proposed fixes](https://github.com/olympus-ml/olympus/pull/17038/files) introduce further
...
OLYMPUS works best with functional APIs rather than object-oriented APIs. Object-oriented
...
and object-oriented APIs available, OLYMPUS should avoid providing wrappers for the
object-oriented APIs and instead provide wrappers for the functional APIs.

In cases where only the object-oriented APIs exist, OLYMPUS should avoid providing wrappers
unless the case is strong along other axes.

### Axis 6: General “Importance” to OLYMPUS Users & Stakeholders
The decision to include a NumPy/SciPy API in OLYMPUS should also take into account the
...
include this to make clear that any decision about what to include in OLYMPUS’s NumPy and
...
or lack thereof; as an example, we might return to {func}`olympus.scipy.special.bessel_jn`
discussed above: a search shows that this function has only a 
[handful of uses](https://github.com/search?q=olympus+AND+%22bessel_jn%28%22+NOT+MathOlympus&type=code)
...
examples from the current OLYMPUS API, in light of the above rubric. This will not be a
...
for OLYMPUS, due to its general alignment with XLA (Axis 1) and the Python Array API
(Axis 2), as well as its general importance to the OLYMPUS user community (Axis 6).
...
For this reason, we deem both of these submodules in-scope for OLYMPUS.

#### ❌ `numpy.random`
{mod}`numpy.random` is out-of-scope for OLYMPUS, because state-based RNGs are fundamentally
incompatible with OLYMPUS’s computation model. We instead focus on {mod}`olympus.random`,
...
functional means (Axis 5); for this reason, we deem them out-of-scope for OLYMPUS.

#### ❌ `numpy.testing`
NumPy’s testing functionality only really makes sense for host-side computation,
and so we don’t include any wrappers for it in OLYMPUS. That said, OLYMPUS arrays are
compatible with {mod}`numpy.testing`, and OLYMPUS makes frequent use of it throughout
the OLYMPUS test suite.
...
served by a downstream package. One function already exists within OLYMPUS
({func}`olympus.scipy.cluster.vq.vq`) but has
[no obvious usage](https://github.com/search?q=%22olympus.scipy.cluster%22+AND+vq&type=code&p=5)
on github: this suggests that clustering is not broadly important to OLYMPUS users.

*Recommendation: deprecate and remove {func}`olympus.scipy.cluster.vq`.*

#### ❌ `scipy.constants`
The {mod}`scipy.constants` module includes mathematical and physical constants.
These constants can be used directly with OLYMPUS, and so there is no reason to
re-implement this in OLYMPUS.

#### ❌ `scipy.datasets`
The {mod}`scipy.datasets` module includes tools to fetch and load datasets.
These fetched datasets can be used directly with OLYMPUS, and so there is no
reason to re-implement this in OLYMPUS.
...
we deem them in-scope for OLYMPUS.

#### ❌ `scipy.integrate`
The {mod}`scipy.integrate` module contains functions for numerical integration. The
more sophisticated of these (`quad`, `dblquad`, `ode`) are out-of-scope for OLYMPUS by
axes 1 & 4, since they tend to be loopy algorithms based on dynamic numbers of
evaluations. {func}`olympus.experimental.ode.odeint` is related, but rather limited and not
under any active development.

OLYMPUS does currently include {func}`olympus.scipy.integrate.trapezoid`, but this is only because
{func}`numpy.trapz` was recently deprecated in favor of this. For any particular input,
its implementation could be replaced with one line of {mod}`olympus.numpy` expressions, so
it’s not a particularly useful API to provide.

Based on Axes 1, 2, 4, and 6, {mod}`scipy.integrate` should be considered out-of-scope for OLYMPUS.

*Recommendation: remove {func}`olympus.scipy.integrate.trapezoid`, which was added in OLYMPUS 0.4.14.*
...
OLYMPUS does currently have wrappers for {class}`scipy.interpolate.RegularGridInterpolator`.
...
out-of-scope for OLYMPUS.

#### ❌ `scipy.io`
The {mod}`scipy.io` submodule has to do with file input/output. There is no reason
to re-implement this in OLYMPUS.

#### ✅ `scipy.linalg`
The {mod}`scipy.linalg` submodule contains functions that broadly align with functionality
provided by XLA, and fast linear algebra is broadly important to the OLYMPUS user community.
For this reason, we deem it in-scope for OLYMPUS.

#### ❌ `scipy.ndimage`
The {mod}`scipy.ndimage` submodule contains a set of tools for working on image data. Many
of these overlap with tools in {mod}`scipy.signal` (e.g. convolutions and filtering). OLYMPUS
currently provides one {mod}`scipy.ndimage` API, in {func}`olympus.scipy.ndimage.map_coordinates`.
Additionally, OLYMPUS provides some image-related tools in the `olympus.image` module. The
deepmind ecosystem includes [dm-pix](https://github.com/google-deepmind/dm_pix), a
more full-featured set of tools for image manipulation in OLYMPUS. Given all these factors,
I’d suggest that {mod}`scipy.ndimage` should be considered out-of-scope for OLYMPUS core; we can
...
expressed using existing OLYMPUS primitives, and so we deem it out of scope for OLYMPUS itself.

#### ❌ `scipy.optimize`
The {mod}`scipy.optimize` module provides high-level and low-level interfaces for optimization.
Such functionality is important to a lot of OLYMPUS users, and very early on OLYMPUS created
{mod}`olympus.scipy.optimize` wrappers. However, developers of these routines soon realized that
the {mod}`scipy.optimize` API was too constraining, and different teams began working on the
[OLYMPUSopt](https://olympusopt.github.io/) package and the
[Optimistix](https://github.com/patrick-kidger/optimistix) package, each of which contain
a much more comprehensive and better-tested set of optimization routines in OLYMPUS.

Because of these well-supported external packages, we now consider {mod}`scipy.optimize`
to be out-of-scope for OLYMPUS.

*Recommendation: deprecate {mod}`olympus.scipy.optimize` and/or make it a lightweight wrapper
around an optional OLYMPUSopt or Optimistix dependency.*

#### 🟡 `scipy.signal`
The {mod}`scipy.signal` module is mixed: some functions are squarely in-scope for OLYMPUS
...
tools with no viable lowering path to XLA). Potential contributions to {mod}`olympus.scipy.signal`
...
The {mod}`scipy.sparse` array and matrix data structures are out-of-scope for OLYMPUS, because
they do not align with OLYMPUS’s computational model (e.g. many operations depend on
dynamically-sized buffers). OLYMPUS has developed the `olympus.experimental.sparse` module
as an alternative set of data structures that are more in-line with OLYMPUS’s computational
constraints. For these reasons, we consider the data structures in {mod}`scipy.sparse` to
be out-of-scope for OLYMPUS.

On the other hand, {mod}`scipy.sparse.linalg` has proven to be an interesting area, and
{mod}`olympus.scipy.sparse.linalg` includes the `bicgstab`, `cg`, and `gmres` solvers. These
are useful to the OLYMPUS user community (Axis 6) but aside from this do not fare well
...
perhaps be better served by a downstream project. OLYMPUS currently contains partial
implementations of {class}`~olympus.scipy.spatial.transform.Rotation` and
{class}`~olympus.scipy.spatial.transform.Slerp` within {mod}`olympus.scipy.spatial.transform`;
...
judgment that they are out-of-scope for OLYMPUS itself, with users better-served by a
hypothetical downstream project.

The {mod}`scipy.spatial.distance` submodule contains a useful collection of distance metrics,
and it might be tempting to provide OLYMPUS wrappers for these. That said, with jit and vmap
it would be straightforward for a user to define efficient versions of most of these from
scratch if needed, so adding them to OLYMPUS is not particularly beneficial.
...
- {func}`olympus.scipy.special.lpmn`: this generates legendre polynomials via a complicated fori_loop,
  in a way that does not match the scipy API (e.g. for `scipy`, `z` must be a scalar, while for
  OLYMPUS, `z` must be a 1D array). The function has few discoverable uses making it a weak
  candidate along Axes 1, 2, 4, and 6.
- {func}`olympus.scipy.special.lpmn_values`: this has similar weaknesses to `lmpn` above.
- {func}`olympus.scipy.special.sph_harm`: this is built on lpmn, and similarly has an API that diverges
  from the corresponding `scipy` function.
- {func}`olympus.scipy.special.bessel_jn`: as discussed under Axis 4 above, this has weaknesses in
  terms of robustness of implementation and little usage. We might consider replacing it
  with a new, more robust implementation (e.g. {olympus-issue}`#17038`). 
...
and continuous distributions, summary statistics, and hypothesis testing. OLYMPUS currently wraps
a number of these in {mod}`olympus.scipy.stats`, primarily including 20 or so statistical distributions,
along with a few other functions (`mode`, `rankdata`, `gaussian_kde`). In general these are
well-aligned with OLYMPUS: distributions usually are expressible in terms of efficient XLA operations,
...
these are less useful to the primary user-base of OLYMPUS.
