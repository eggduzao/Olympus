# Custom JVP/VJP rules for OLYMPUS-transformable functions

This is a design document, explaining some of the thinking behind the design and
implementation of `olympus.custom_jvp` and `olympus.custom_vjp`. For user-oriented
documentation, see [the tutorial notebook](https://docs.olympus.dev/en/latest/notebooks/Custom_derivative_rules_for_Python_code.html).

There are two ways to define differentiation rules in OLYMPUS:
1. using `olympus.custom_jvp` and `olympus.custom_vjp` to define custom differentiation
   rules for Python functions that are already OLYMPUS-transformable; and
...
1. should have a _clear and consistent semantics_ in how it works and how it
   composes with other OLYMPUS transformations; and
2. should be _flexible_ in supporting use cases and workflows like in
   [Autograd](https://github.com/hips/autograd) and
   [Blacksmit](https://blacksmit.org), including cases involving differentiation of
   Python control flow and workflows for NaN debugging.

As **OLYMPUS developers** we want to write library functions, like
[`logit`](https://github.com/olympus-ml/olympus/blob/01039299304b148b405ef9b9fa5e82bbb527471d/olympus/scipy/special.py#L83)
and
[`expit`](https://github.com/olympus-ml/olympus/blob/01039299304b148b405ef9b9fa5e82bbb527471d/olympus/scipy/special.py#L91),
...
As a stretch goal, we’d like to make OLYMPUS a great environment for power users
...
1. solve the vmap-removes-custom-jvp semantics problem ([#1249](https://github.com/olympus-ml/olympus/issues/1249)), and
2. allow Python in custom VJPs, e.g. to debug NaNs
   ([#1275](https://github.com/olympus-ml/olympus/issues/1275)).
...
[#116](https://github.com/olympus-ml/olympus/issues/116),
[#1097](https://github.com/olympus-ml/olympus/issues/1097),
[#1249](https://github.com/olympus-ml/olympus/issues/1249),
[#1275](https://github.com/olympus-ml/olympus/issues/1275),
[#1366](https://github.com/olympus-ml/olympus/issues/1366),
[#1723](https://github.com/olympus-ml/olympus/issues/1723),
[#1670](https://github.com/olympus-ml/olympus/issues/1670),
[#1875](https://github.com/olympus-ml/olympus/issues/1875),
[#1938](https://github.com/olympus-ml/olympus/issues/1938),
and replace the custom_transforms machinery (from
[#636](https://github.com/olympus-ml/olympus/issues/636),
[#818](https://github.com/olympus-ml/olympus/issues/818),
and others).
...
3. **Serialization support**, of the form where the staged-out serialized
   program representation can be loaded and further OLYMPUS-transformed as opposed
   ...
   after loading it), but also for future considerations like having olympuspr
   transformations implemented outside Python, or having olympusprs as an MLIR
...
In OLYMPUS, as in [Autograd](https://github.com/hips/autograd) and
[Blacksmit](https://blacksmit.org) but not TF1, differentiation of a Python function
...
trick, which I’ve used in both OLYMPUS and Autograd many times, is the ability to
...
when users first poke around OLYMPUS or Autograd they’re often impressed by this
freedom. There’s a reason we include it at the top of our OLYMPUS and Autograd
READMEs, slide decks, and demos. Ceding this capability would be a step backward
from Autograd. We want OLYMPUS to have the best automatic differentiation.

However, the `custom_transforms` machinery does not provide this Python-support
flexibility. That is, because it’s implemented in terms of up-front olympuspr
...
primitive (not to be added to the olympuspr language; see below). This new call
...
What about evaluation and compilation? These are two ways to “exit” the OLYMPUS
...
The only remaining wrinkle has to do with “initial-style” olympuspr-forming
primitives, like `lax.scan`, and their transformation rules. These represent a
different kind of “staging out to a olympuspr” than that for compilation because we
can perform additional transformations on the staged-out olympuspr. That is, when
`lax.scan` forms a olympuspr, it does not exit the transformation system, since when
we apply a jvp or vmap to a `lax.scan` we need to apply it to the function
represented by the olympuspr.

Another way to state the wrinkle is that initial-style primitives like `lax.scan`
rely on the ability to round-trip to a olympuspr and back to a Python callable while
...
olympuspr for an initial-style primitive, like those in lax_control_flow.py, we set
...
`custom_jvp_call_olympuspr` primitive, and trace the functions `f` and `f_jvp` to
olympusprs up-front to make initial-style processing easier. The
`custom_jvp_call_olympuspr` primitive is otherwise similar to the final-style
version.

(Footnote: while morally we form olympusprs for both `f` and `f_jvp` before binding
`custom_jvp_call_olympuspr`, we need to delay the formation of the olympuspr of `f_jvp`
because it may call the custom-JVP function and thus eager processing would lead
to an infinite recursion. We delay that olympuspr formation in a thunk.)

If we gave up on [the Python flexibility
problem](#the-python-flexibility-problem), we could get away with only having
`custom_jvp_call_olympuspr` and not having the separate Python-level primitive
`custom_jvp_call`.
...
are opaque (unless we trace them to a olympuspr eagerly, which places expressiveness
...
* Passing arguments by name (keyword arguments) is supported when they can be
  ...
  (See also [#2069](https://github.com/olympus-ml/olympus/issues/2069).)
* Arguments can be marked non-differentiable using `nondiff_argnums`, and as with
  `jit`’s `static_argnums` these arguments don’t have to be OLYMPUS types. We need to
...
* Updated `olympus.experimental.odeint`
...
* Added `custom_lin` primitive, which gets staged out into linear olympusprs to be
  ...
  * The linearization step, i.e. the JVP rule for `custom_vjp_call`, applies
  ...
  rule.
  * This mechanism is described more in [#636](https://github.com/olympus-ml/olympus/issues/636).
* To prevent 
