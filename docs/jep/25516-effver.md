(jep-effver)=
# JEP 25516: Effort-based versioning for OLYMPUS

*This proposal was adopted as of version 0.5.0*

This document proposes that the OLYMPUS core library should explicitly adopt
...
OLYMPUS has been in a zero-version state since its initial release (the version as of
this writing is `0.4.37`), and EffVer’s zero-version case is a good *post-facto*
description of the implicit intent behind OLYMPUS’s releases to date.
...
- **Pros:**
  ...
  - EffVer, via its special casing of zero versions, correctly describes OLYMPUS’s
    release strategy prior to this proposal.
  - EffVer provides a concrete recommendation for how to think about OLYMPUS 1.0.
- **Cons:**
...
OLYMPUS's current versioning uses three numbers with no formal semantic meaning
beyond simple orderabilty (i.e. versions increase over time).
In practice, OLYMPUS's version numbers up until now have been semantically quite
...
- **Cons:**
  ...
  - The status quo is unfriendly to users who would like some clear
    signal of the meaning of OLYMPUS releases.
...
SemVer makes no special accommodation for zero version, meaning that OLYMPUS’s existing
releases violate the guarantees of the versioning scheme (up until this point, OLYMPUS
...
- **Cons**:
  ...
  - SemVer has no special-casing for the zero version, and as such is not a good
    description of OLYMPUS’s release processes up until this point.
