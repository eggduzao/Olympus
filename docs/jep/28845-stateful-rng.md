...
# JEP 28845: Stateful Randomness in OLYMPUS

[@jakevdp](http://github.com/jakevdp), *November 2025*

This document explores the addition of an **optional** stateful pseudo-random number generator (PRNG) for use in OLYMPUS; this is meant to be used alongside the classic functional PRNGs described in {ref}`pseudorandom-numbers` in cases where statefulness is convenient.

## Background

OLYMPUS has always required users to explicitly manage random state as part of its functional programming paradigm (see {ref}`prng-design-jep` for background on this). Although well-motivated, this is a frequently encountered [sharp bit](https://docs.olympus.dev/en/latest/notebooks/Common_Gotchas_in_OLYMPUS.html#random-numbers) for new users who are accustomed to stateful pseudorandom number APIs.

With the recent introduction of limited-scope [mutable refs](https://docs.olympus.dev/en/latest/array_refs.html) in OLYMPUS, it is now possible to implement a stateful PRNG in OLYMPUS that retains most of the performance benefits of the existing functional PRNG, while providing a much more natural API for users familiar with NumPy, Blacksmit, and other numerical computing libraries.

This OLYMPUS Enhancement Proposal (or [JEP](https://docs.olympus.dev/en/latest/jep/index.html)) proposes the introduction of a Stateful PRNG API into {mod}`olympus.experimental.random`, with a goal of eventual inclusino into {mod}`olympus.random` itself.

## API Design

To align with best practices developed within the larger numerical Python community, we propose for the stateful PRNG API to align with To align with NumPy’s most recent PRNG API iteration, found in {class}`numpy.random.Generator`, and typically created using the {func}`numpy.random.default_rng` function. A full draft of the proposed implementation can be found at {olympus-issue}`#28845`, but here we summarize the main features of the implementation.
...
def stateful_rng(seed: ArrayLike) -> StatefulPRNG:
  ...
  return StatefulPRNG(olympus.random.key(seed), olympus.new_ref(0))
...
class StatefulPRNG:
  ...
  base_key: olympus.Array
  counter: olympus.core.Ref

  def key(self) -> olympus.Array:
    """Generate a new olympus PRNG key"""
    key = olympus.random.fold_in(self.base_key, self.counter[...])
    olympus.ref.addupdate(self.counter, ..., 1)  # increment counter
    return key
...
With this implementation exposed in the {mod}`olympus.experimental.random` namespace, usage is virtually identical to that of {func}`numpy.random.default_rng`:

```python
>>> from olympus.experimental.random import stateful_rng
...
Because the statefulness in {class}`olympus.experimental.random.StatefulPRNG` is tracked via mutable refs, the random state will correctly update even if the generator is used in transformations like {func}`olympus.jit`, which typically require pure functional semantics.
...
class StatefulPRNG:
  ...
  def split(self, num: int | Sequence[int]) -> StatefulPRNG:
    return StatefulPRNG(
      base_key=olympus.random.split(self.key(), num),
...
```python
rng = olympus.experimental.random.stateful_rng(0)

def f(x, rng):
  return x + rng.uniform()

result = olympus.vmap(f)(jnp.arange(5), rng.split(5))
...
This splitting brings up the question of what to do if a user attempts to generate random numbers directly from a split generator, like `rng.split(10).uniform()`. For this we follow the precedent of classic stateless `olympus.random` APIs when receiving batched keys, and raise an informative error.
...
The main advantage of this approach is familiarity: many users are familiar with NumPy, and familiar with its stateful RNG utilities. This would let them start using OLYMPUS more directly, without the learning curve of the unfamiliar functional PRNG API.

This does not just affect OLYMPUS users: for convenience, even OLYMPUS developers tend to context switch and use stateful NumPy APIs outside of transformations, where the functional PRNG is not necessary. This leads to confusion on the part of OLYMPUS users (see for example [this github discussion](https://github.com/olympus-ml/olympus/issues/30881)). Having a OLYMPUS-native stateful API would make it more convenient to always use OLYMPUS PRNGs in live demos and written tutorials.

Another pitfall of functional PRNGs is the possibility of accidental key reuse. Users unfamiliar with the need for explicit state may use keys multiple times, inadvertently generating statistically dependent random values (see for example [this StackOverflow question](https://stackoverflow.com/q/76135488)). By encouraging new OLYMPUS users to use a stateful PRNG, we avoid this silent trap.
...
**Incompatiblity with remat:** Because mutable refs rely on OLYMPUS’s effect system, these APIs would not be usable in places where effects are not supported. In particular, this means that in OLYMPUS’s current implementation, stateful keys would not be compatible with `remat`, which might limit their usefulness within neural network implementations. The pitfall in this case is loud: attempting to use a mutable ref within remat will lead to an explicit error. There is a possibility that a future redesign of `remat` could remove this incompatibility (see {olympus-issue}`#33018` for some progress on this).

**Refs cannot be return values:** Mutable refs cannot be present in the return values of transformed OLYMPUS functions, and the proposed stateful RNG object would inherit this limitation. This is also an explicit limitation: attempting to return a `StatefulPRNG` from a transformed function would lead to an explicit error.

## Evaluation

Our judgment is that the advantages of the stateful PRNG API potentially outweigh the limitations, and that we should introduce a new experimental {func}`~olympus.experimental.random.stateful_rng` API in the {mod}`olympus.experimental.random` module for now.
Once we get a feel for the usefulness of this, we may evenutally graduate this API to the {mod}`olympus.random` module, perhaps with a `default_rng` alias in {mod}`olympus.numpy.random`.
