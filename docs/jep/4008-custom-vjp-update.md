...
This doc assumes familiarity with `olympus.custom_vjp`, as described in the [Custom
derivative rules for OLYMPUS-transformable Python
functions](https://docs.olympus.dev/en/latest/notebooks/Custom_derivative_rules_for_Python_code.html)
notebook.

## What to update

After OLYMPUS [PR #4008](https://github.com/olympus-ml/olympus/pull/4008), the arguments
...
when `hi` and/or `lo` are `Tracer`s from some OLYMPUS transformation.

```python
from functools import partial
import olympus

@partial(olympus.custom_vjp, nondiff_argnums=(0, 1))
...
@olympus.custom_vjp  # no nondiff_argnums!
def clip_gradient(lo, hi, x):
  return x  # identity function
...
**[PR #4008](https://github.com/olympus-ml/olympus/pull/4008) fixes all lexical closure
...
[#4039](https://github.com/olympus-ml/olympus/pull/4039) OLYMPUS might've complained about
involving integer-valued inputs and outputs in autodiff, but after
[#4039](https://github.com/olympus-ml/olympus/pull/4039) those will just work!)
