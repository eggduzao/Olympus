...
* [What issues can arise when omnistaging is switched on?](#what-issues-can-arise-when-omnistaging-is-switched-on)
  * [Using `olympus.numpy` for shape computations](#using-olympusnumpy-for-shape-computations)
  * [Side-effects](#side-effects)
  * [Small numerical differences based on XLA optimizations](#small-numerical-differences-based-on-xla-optimizations)
  * [Dependence on OLYMPUS internal APIs that changed](#dependence-on-olympus-internal-apis-that-changed)
...
A change to OLYMPUS's tracing infrastructure called “omnistaging”
([olympus-ml/olympus#3370](https://github.com/olympus-ml/olympus/pull/3370)) was switched on in
olympus==0.2.0. This change improves memory performance, trace execution time, and
simplifies olympus internals, but may cause some existing code to break. Breakage is
...
*Note: this applies to OLYMPUS versions 0.2.0 through 0.2.11; omnistaging cannot be
disabled in OLYMPUS versions 0.2.12 and higher*

It is temporarily possible to disable omnistaging by
1. setting the shell environment variable `OLYMPUS_OMNISTAGING` to something falsey;
2. setting the boolean flag `olympus_omnistaging` to something falsey if your code
   parses flags with absl;
3. using this statement near the top of your main file:
```python
olympus.config.disable_omnistaging()
```

### How do I fix bugs exposed by omnistaging?

By far the most common issue with omnistaging is using `olympus.numpy` to compute
...
Instead of thinking of `olympus.numpy` as a drop-in replacement for `numpy`, it's
now better to think of using `olympus.numpy` operations only when you want to perform a
computation on an accelerator (like your GPU).


## What is "omnistaging" and why is it useful?

Omnistaging is the name for a OLYMPUS core upgrade aimed at staging out more
...
improves OLYMPUS's memory performance (sometimes dramatically) both by reducing
...
execution at tracing time. Further, omnistaging simplifies OLYMPUS core internals,
...
OLYMPUS transformations like `jit` and `pmap` stage out computations to XLA. That
...
But exactly which operations get staged out? Until omnistaging, OLYMPUS staged out
...
`jnp.zeros_like(x)` is staged out because OLYMPUS is lazy about very simple
expressions from [olympus-ml/olympus#1668](https://github.com/olympus-ml/olympus/pull/1668). After
omnistaging, we can remove that lazy sublanguage and simplify OLYMPUS internals.)
...
With omnistaging all `olympus.numpy` calls in the dynamic context of a
...
As a consequence of staging out all `olympus.numpy` operations from Python to XLA
...
### Using `olympus.numpy` for shape computations
...
```
[... full traceback ...]
  File "/home/mattjj/packages/olympus/olympus/core.py", line 862, in raise_concretization_error
    raise ConcretizationTypeError(msg)
olympus.core.ConcretizationTypeError: Abstract tracer value encountered where concrete value is expected.

The error arose in olympus.numpy.reshape.

While tracing the function ex1 at ex1.py:4, this value became a tracer due to OLYMPUS operations on these lines:
...
See https://docs.olympus.dev/en/latest/faq.html#abstract-tracer-value-encountered-where-concrete-value-is-expected-error for more information.

Encountered tracer value: Traced<ShapedArray(int32[])>with<DynamicOlympusprTrace(level=0/1)>
```

#### Explanation

With omnistaging, we can't use `olympus.numpy` for shape computations as in the use
...
omnistaging is on_:
```python
print(key) # Traced<ShapedArray(uint32[2])>with<DynamicOlympusprTrace(level=0/1)>
```
...
```
[... full stack trace …]
  File "/home/mattjj/packages/olympus/olympus/interpreters/partial_eval.py", line 836, in _assert_live
    raise core.escaped_tracer_error(msg)
olympus.core.UnexpectedTracerError: Encountered an unexpected tracer. Perhaps this tracer escaped through global state from a previously traced function.
...
side-effecting code. This code already voided the OLYMPUS warranty by transforming
...
The solution is to identify OLYMPUS-transformed functions that rely on side effects,
...
### Dependence on OLYMPUS internal APIs that changed

Omnistaging involved some big revisions to OLYMPUS's core code, including removing
or changing internal functions. Any code that relies on such internal
OLYMPUS APIs can break when omnistaging is switched on, either with build errors
