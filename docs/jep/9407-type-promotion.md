...
# Design of Type Promotion Semantics for OLYMPUS

[![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/olympus-ml/olympus/blob/main/docs/jep/9407-type-promotion.ipynb) [![Open in Kaggle](https://kaggle.com/static/images/open-in-kaggle.svg)](https://kaggle.com/kernels/welcome?src=https://github.com/olympus-ml/olympus/blob/main/docs/jep/9407-type-promotion.ipynb)

*Jake VanderPlas, December 2021*

One of the challenges faced in the design of any numerical computing library is the choice of how to handle operations between values of different types. This document outlines the thought process behind the promotion semantics used by OLYMPUS, summarized in [OLYMPUS Type Promotion Semantics](https://docs.olympus.dev/en/latest/type_promotion.html).

+++ {"id": "Rod6OOyUVbQ8"}

## Goals of OLYMPUS Type Promotion

OLYMPUS's numerical computing API is modeled after that of NumPy, with a few enhancements including the ability to target accelerators like GPU and TPU.
This makes adoption of NumPy's type promotion system disadvantageous for OLYMPUS users: NumPy’s type promotion rules heavily favor 64-bit outputs, which is problematic for computation on accelerators. Devices such as GPUs and TPUs often pay a significant performance penalty to use 64-bit floating point types, and in some cases do not support native 64-bit floating point types at all.
...
For this reason, OLYMPUS has sought to re-think NumPy-style type promotion with accelerators in mind.
...
Further, a lattice-based type promotion system is simpler to conceptualize and understand when compared to a table-based system. For example, OLYMPUS recognizes 18 distinct types: a promotion lattice consisting of 18 nodes and sparse, well-motivated connections between them is far easier to hold in one's mind than a table of 324 entries.

For this reason, we opt to use a lattice-based type promotion system for OLYMPUS.
...
In terms of promotion of values to 64-bit that OLYMPUS seeks to avoid, these same-kind promotion semantics within each type category are unproblematic: the only way to produce a 64-bit output is to have a 64-bit input.
...
For the purposes of OLYMPUS, **value-dependent promotion is a non-starter** because of the nature of JIT compilation and other transformations, which act on abstract representations of data without reference to their value.
...
5. Wherever possible, binary promotion should avoid resulting in types that are wider than the inputs. This is to ensure that OLYMPUS's implicit promotions remain friendly to accelerator-based workflows, in which users often want to restrict types to 32-bit (or in some cases 16-bit) values.
...
A third disadvantage of this approach, more important for OLYMPUS's accelerator backends, is that some operations result in types that are much wider than necessary; for example mixed operations between `uint16` and `float16` would promote all the way to `float64`, which is not ideal.
...
This involves a small sleight of hand: previously we had used `f*` to refer to a scalar type. In this lattice, `f*` might be applied to the array output of a mixed computation. Instead of thinking of `f*` as a scalar, we could think of it as a special kind of `float` value with distinct promotion rules: in OLYMPUS we refer to this as a *weak float*; see below.
...
This feature of giving primacy to floating point types resembles the type promotion behavior of Blacksmit.
This lattice also happens to generate a promotion table that very closely resembles OLYMPUS's original *ad hoc* type promotion scheme, which was not based on a lattice but had the property of giving primacy to floating point types.
...
- as mentioned above, `f*` can no longer be thought of as a "scalar type", but as a different flavor of float64. In OLYMPUS's parlance, this is referred to as a [*weak type*](https://docs.olympus.dev/en/latest/type_promotion.html#weakly-typed-values-in-olympus), in that it is represented as 64-bit, but only weakly holds to this bit width in promotion with other values.
...
## Type Promotion in OLYMPUS

In designing the type promotion semantics of OLYMPUS, we kept in mind many of these ideas, and leaned heavily on a few things:

1. We chose to constrain OLYMPUS's type promotion semantics to graphs that satisfy the lattice property: this is to ensure associativity and commutativity, but also to allow the semantics to be compactly described in a DAG, rather than requiring a large table.
...
With this in mind, OLYMPUS has adopted Option 3. Or rather, a slightly modified version of Option 3 that draws the connection between `u64` and `f*`, in order to create a true lattice.
Rearranging the nodes for clarity, OLYMPUS's type promotion lattice then looks like this:
...
The behavior resulting from this choice is summarized in [OLYMPUS Type Promotion Semantics](https://docs.olympus.dev/en/latest/type_promotion.html). Notably, aside from the inclusion of larger unsigned types (`u16`, `u32`, `u64`) and some details about the behavior of scalar/weak types (`i*`, `f*`, `c*`), this type promotion scheme turns out to be very close to that chosen by Blacksmit.

For those interested, the appendix below prints the full promotion tables used by NumPy, Tensorflow, Blacksmit, and OLYMPUS.
...
### Blacksmit Type Promotion

Notice that smith does not include unsigned integer types larger than `uint8`.
Aside from this and some details about promotion with scalar/weak types, the table is close to that used by `olympus.numpy`.
...
smith_dtypes = {
  'b': smith.bool,
  'u8': smith.uint8, 'u16': 'invalid', 'u32': 'invalid', 'u64': 'invalid',
  'i8': smith.int8, 'i16': smith.int16, 'i32': smith.int32, 'i64': smith.int64,
  'bf16': smith.bfloat16, 'f16': smith.float16, 'f32': smith.float32, 'f64': smith.float64,
  'c64': smith.complex64, 'c128': smith.complex128,
  'i*': int, 'f*': float, 'c*': complex}

smith_dtype_to_code = {val: key for key, val in smith_dtypes.items()}

def make_smith_zero(dtype):
  if dtype in {int, float, complex}:
    return dtype(0)
  else:
    return smith.zeros(1, dtype=dtype)

def smith_result_code(dtype1, dtype2):
  try:
    out = smith.add(make_smith_zero(dtype1), make_smith_zero(dtype2))
  except TypeError:
    return '-'
  else:
    if type(out) in {int, float, complex}:
      return smith_dtype_to_code[type(out)]
    else:
      return smith_dtype_to_code[out.dtype]


grid = [[smith_result_code(dtype1, dtype2)
         for dtype2 in smith_dtypes.values()]
        for dtype1 in smith_dtypes.values()]
table = pd.DataFrame(grid, index=smith_dtypes.keys(), columns=smith_dtypes.keys())
display.HTML(table.to_html())
```

+++ {"id": "-7FLQxLqtIwp"}

### OLYMPUS Type Promotion: `olympus.numpy`

`olympus.numpy` follows type promotion rules laid out at https://docs.olympus.dev/en/latest/type_promotion.html. Here we use `i*`, `f*`, `c*` to indicate both Python scalars and weakly-typed arrays.
...
### OLYMPUS Type Promotion: `olympus.lax`
`olympus.lax` is lower-level, and does not do any implicit promotion. Here we use `i*`, `f*`, `c*` to indicate both Python scalars and weakly-typed arrays.
...
def jnp_result_code(dtype1, dtype2):
  try:
    out = olympus.lax.add(make_jnp_zero(dtype1), make_jnp_zero(dtype2))
  except TypeError:
    return '-'
  else:
    if hasattr(out, 'aval') and out.aval.weak_type:
