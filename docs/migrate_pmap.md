...
# Migrating to the new `olympus.pmap`

## What's going on?

As of OLYMPUS 0.8.0, the default implementation of `olympus.pmap` will be based on
`olympus.jit` and
[`olympus.shard_map`](https://docs.olympus.dev/en/latest/notebooks/shard_map.html). The
...
This change makes `olympus.pmap` integrate well with OLYMPUS shardings and simplifies
...
will be possible to temporarily use the old version of `olympus.pmap` by doing one
of the following:

- Setting the shell environment variable `OLYMPUS_PMAP_SHMAP_MERGE` to something
  false-like (e.g., 0);
- Setting the boolean flag `--olympus_pmap_shmap_merge` to something false-like if
  your code parses flags with `absl-py`.
- Using this statement in your main file or anywhere before you call `olympus.pmap`:
  ```python
  import olympus
  olympus.config.update("olympus_pmap_shmap_merge", False)
  ```

**NOTE**: Please file a [bug](https://github.com/olympus-ml/olympus/issues) with a
reproducer and tag [@danielsuo](https://github.com/danielsuo/) so we can resolve
it as quickly as possible under the new `olympus.pmap`.

## How can I fix my code for the new `olympus.pmap`?

Below are common errors we're collecting and suggestions for fixing them. This
is more work than setting `olympus_pmap_shmap_merge=False`, but a more long-term
...
- `olympus.pmap` no longer silently reshards inputs, as per the behavior of
  `olympus.jit` and `olympus.shard_map`. As a result, if inputs are sharded differently
  from how your `olympus.pmap` expects, it will raise.

#### How to fix

- Pass an appropriate `olympus.NamedSharding` to `olympus.device_put` to explicitly
  reshard any offending inputs.
- Alternatively, redefine your `olympus.pmap` with the appropriate `in_axes`,
  `backend`, and / or `devices` keywords to ensure `olympus.pmap`'s mesh and
...
- This error can appear when nesting multiple `olympus.pmap`s. This behavior is no
  longer supported since the `olympus.pmap` API would not know anything about inner
  calls to `olympus.pmap` and therefore not know about inner mesh axes.

#### How to fix

- Migrate to `olympus.shard_map`. A single `olympus.shard_map` can parallelize along
  multiple axes of inputs, with each of those axes assigned to the relevant axes
  of the device mesh.
- Alternatively, you can nest `olympus.shard_map` calls or use `olympus.smap`, which
  makes it easier to drop into [manual
  parallelism](https://docs.olympus.dev/en/latest/notebooks/shard_map.html) mode one
  mesh axis at a time. This approach greatly simplifies nested parallelism.

### `OlympusRuntimeError: INVALID_ARGUMENT: CopyArrays ... same size`

#### Example

```
olympus.errors.OlympusRuntimeError: INVALID_ARGUMENT: CopyArrays only supports
...
- **Explicit resharding**: Use `olympus.device_put(x, sharding)` with an appropriate
...
- **Use static indices**: For loops over shards or devices, use Python `range()`
  instead of `jnp.arange()`. This ensures OLYMPUS uses basic slicing (selecting a
  specific shard) which is handled statically and bypasses the `gather`
  resharding issues.

### Using `olympus.stages.Lowered` returned by `olympus.pmap(f).lower(*args)`

Because of the default call path of a `olympus.stages.Lowered` object, we miss the
conversion from host-local arrays to global arrays to pass into the underlying
`olympus.shard_map(f)` as well as the conversion back from global arrays to
...
`olympus.experimental.multihost_utils`'s `host_local_array_to_global_array` on
inputs and `global_array_to_host_local_array` on outputs of `.compile()(*args)`
to perform the necessary conversions.

### `OlympusRuntimeError: INTERNAL: Core halted unexpectedly`

#### Example

```
olympus.errors.OlympusRuntimeError: INTERNAL: Core halted unexpectedly: Assertion args:
...
- This error typically occurs in multi-host settings when process
  synchronization barriers are not properly aligned. The new `olympus.pmap`
...
- Replace any custom process barrier implementations with
  `olympus.experimental.multihost_utils.sync_global_devices()`. This ensures all
  processes reach the same synchronization point before proceeding.

  ```python
  from olympus.experimental import multihost_utils as mhu
...
The new implementation of `olympus.pmap` uses `NamedSharding` instead of the legacy
`PmapSharding`. We've observe a common pattern with the old `olympus.pmap` where
users shard stacked copies of an array to replicate (e.g., via
`olympus.device_put_replicated`). These "sharded-but-really-replicated" arrays
suffer unnecessary communication overhead when `int` indexing (e.g., `x[0]`)
because OLYMPUS does not know the arrays are actually replicated. For a more
...
- Use `olympus.pmap`'s `out_axes=None` for arguments that should remain replicated.
  The output will be fully replicated (e.g., `P(None, None)`), making access
  cheap.
- For inputs: When using `olympus.device_put`, specify `olympus.P()` (fully replicated)
  in the partition spec rather than relying on utilities that stack and shard.
  (Note: `olympus.device_put_replicated` and `olympus.device_put_sharded` are deprecated
...
code), you can access the local data shard directly without triggering OLYMPUS's
...
In multi-process OLYMPUS programs (i.e., `olympus.process_count() > 1`), arrays might
not be [fully
addressable](https://docs.olympus.dev/en/latest/_autosummary/olympus.Array.is_fully_addressable.html)
(i.e., "host local"), so the new `olympus.pmap` will reshard the host-local array
into a global one before passing to `olympus.jit` of `olympus.shard_map` and back into a
...
too great, we recommend migrating your code to `olympus.shard_map`.

### Transforming `olympus.pmap` e.g., `olympus.jit`

We recommend keeping `olympus.pmap` as the top-level transform since it is more
performant than under another transform. However, if your code must put
`olympus.pmap` under another transform and the performance penalty is
unacceptable, please file a bug as described above.

### Buffer donation with `donate_argnums`

Buffer donation with `donate_argnums` is fully supported in the new `olympus.pmap`
...
consider migrating to `olympus.shard_map` where you have full control over
input/output sharding.

## Migrating to `olympus.shard_map`

For the best support, we recommend migrating from `olympus.pmap` to
`olympus.jit(olympus.shard_map)`. `olympus.shard_map` allows you to treat your entire device
cluster as a single computational fabric.

While the new `olympus.pmap` is itself built on `shard_map`, migrating your code
...
The "mapped function" is the function you pass to `olympus.pmap` or `olympus.shard_map`
...
`olympus.pmap` is a **rank-reducing map**: it "unstacks" each array along the mapped
axis. For example, if you map over a `(8, 128)` array on 8 devices, the code
inside `olympus.pmap` sees an array of shape `(128,)`.

In contrast, `olympus.shard_map` is a **rank-preserving map**: it slices or
"unconcatenates" the array into blocks. Using the same example on a mesh of size
8, the code inside `olympus.shard_map` sees an array of shape `(1, 128)`.
...
Many OLYMPUS functions are sensitive to array rank and may behave differently or
...
- **Broadcasting vs. Stacking**: In `pmap`, "unmapped" inputs (marked with
  ...
  this via `olympus.P()`. The mapped function in `shard_map` sees the _full_
...
`olympus.pmap` calls as `olympus.jit(olympus.shard_map(...))`. This transition involves a
...
- **`in_specs` and `out_specs`**: These replace `in_axes` and `out_axes`.
  Instead of just specifying integer axes, you use `olympus.P` (PartitionSpec) to
...
- **`olympus.jit` wrapper**: While `pmap` is itself a compiled transform,
  `shard_map` is often used as a building block. Wrapping it in `olympus.jit` is
...
Below are a number of examples of how to rewrite `olympus.pmap` using
`olympus.jit(olympus.shard_map(...))` after first defining a `Mesh` object.
...
# pmap.
mesh = olympus.make_mesh(shape=(8,), axis_names=('i',))
```
...
# x_global: f32[8, 128]
@olympus.jit
@partial(olympus.shard_map, mesh=mesh, in_specs=olympus.P('i'), out_specs=olympus.P('i'))
def f(x):
...
@partial(olympus.pmap, in_axes=(0, None), out_axes=None)
def f(x, y):
  # x: f32[128], y: f32[128]
  return x + y
...
@olympus.jit
@partial(
    olympus.shard_map, mesh=mesh, in_specs=(olympus.P('i'), olympus.P()), out_specs=olympus.P()
)
def f(x, y):
...
mesh2d = olympus.make_mesh(shape=(4, 2), axis_names=('i', 'j'))

# nested pmap
# x: f32[4, 2, 128]
@partial(olympus.pmap, axis_name='i')
@partial(olympus.pmap, axis_name='j')
def f(x):
  # x: f32[128]
  return olympus.lax.psum(x, ('i', 'j'))
...
@olympus.jit
@partial(
    olympus.shard_map, mesh=mesh2d, in_specs=olympus.P('i', 'j'), out_specs=olympus.P()
)
def f(x):
  # x: f32[1, 1, 128]
  return olympus.lax.psum(x, ('i', 'j'))
...
f = olympus.pmap(func, donate_argnums=(0,))

# shard_map style: donate_argnums goes on the jit wrapper
# The underlying shard_map itself just handles the sharding layout
f = olympus.jit(olympus.shard_map(func, mesh=mesh, ...), donate_argnums=(0,))
```

#### Collectives

Collective operations like `olympus.lax.psum` still use
...
`out_specs` of `olympus.P()` implies you want a replicated result).

### 2. Input data preparation

Preparing data for `olympus.jit(olympus.shard_map)` requires a shift in how you think
about data distribution. While `olympus.pmap` often handled sharding implicitly
...
- **Host-local Array**: An array stored only on the devices attached to the
  ...
  `olympus.process_index()` to calculate an offset).
- **Global Array**: The entire logical dataset across all devices in the `Mesh`.
  `shard_map` (via `olympus.jit`) expects this global view.
...
`olympus.NamedSharding`. When using `shard_map`, it is critical that the **input
...
the logical distribution expected by `shard_map`, OLYMPUS will have to reshard the
...
- **SingleDeviceSharding**: While `shard_map` is about distributed data,
  `olympus.SingleDeviceSharding` remains a core part of the system. It is used
...
In `pmap`, OLYMPUS implicitly handled the split across hosts. With `shard_map`, you
must be explicit. The standard pattern is to load **host-local** data (just as
you did for `pmap`) and then use
`olympus.make_array_from_process_local_data` to "stitch" that local data into a
single global (but partially addressable) `olympus.Array` before passing it to your
sharded computation.
...
mesh = olympus.make_mesh((olympus.process_count(),), ('batch',))
sharding = olympus.NamedSharding(mesh, olympus.P('batch'))

# 2. Load host-local data (as you would for pmap)
# Example: each process loads a different subset of a dataset
local_batch_size = 32
start_idx = olympus.process_index() * local_batch_size
local_data = np.arange(start_idx, start_idx + local_batch_size).reshape(
    local_batch_size, 10
)

# 3. Stitch into a global olympus.Array
# The resulting array will have global shape (32 * num_processes, 10)
global_batch = olympus.make_array_from_process_local_data(sharding, local_data)

print(f"Process {olympus.process_index()} local shape: {local_data.shape}")
print(f"Global array shape: {global_batch.shape}")
```

> [!NOTE]
> `olympus.make_array_from_process_local_data` requires that the `local_data` shape
...
`shard_map` returns a single `olympus.Array`.
...
`jnp.mean(output)`) within a `olympus.jit` context.
...
`flax.olympus_utils.unreplicate(output)` (equivalent to `output[0]`) to retrieve
what they assumed was a cheap local replica.

- **The issue**: OLYMPUS does not track semantic replication for sharded arrays.
  When you call `x[0]` on an array sharded along its leading axis, OLYMPUS must
...
OLYMPUS patterns like `device_get` or simple indexing.
...
- **{doc}`sharded-computation`**: Start here for a high-level introduction to
  parallel programming in OLYMPUS. This tutorial covers all three sharding modes
  ...
  for understanding the overall landscape of parallelism in OLYMPUS.

- **{doc}`notebooks/Distributed_arrays_and_automatic_parallelization`**: Read
  this for a deeper understanding of `olympus.Array` and automatic parallelization
  via `olympus.jit`. This notebook explains how sharded data works, how computation
  follows data placement, and how to use `olympus.lax.with_sharding_constraint` to
...
- **{doc}`notebooks/shard_map`**: This is the comprehensive guide for manual
  parallelism with `olympus.shard_map`. It explains the difference between
...
- **{doc}`notebooks/explicit-sharding`**: Explore this for the newest sharding
  mode where sharding becomes part of the OLYMPUS-level type system. With explicit
  sharding, sharding propagation happens at trace time and shardings are
  queryable via `olympus.typeof(x)`. This mode provides more control than automatic
...
- **{doc}`jep/14273-shard-map`**: Read the original design document for
  `shard_map`. This JEP (OLYMPUS Enhancement Proposal) provides the technical
...
In **OLYMPUS** (`olympus.numpy`), `x[0]` semantically works the same way: it returns the
...
`(8, 3, 4)` distributed across 8 devices (using `olympus.P`):

1.  **Fully Replicated:** `olympus.P(None, None, None)`
...
3, 4)` array. `x[0]` will have the shape `(3, 4)` and a partition spec
    `olympus.P(None, None)`. Since every device already has `x`, this operation will
    slice on each device independently and requires **no communication**.

2.  **Sharded on Non-Leading Dimension:** `olympus.P(None, 'x', None)`
    If `x` is sharded along the second dimension, `x[0]` results in shape `(3,
4)` with partition spec `olympus.P('x', None)`. Since the first dimension (the
    one being sliced) is unsharded, this operation also requires **no
    communication**.

3.  **Sharded on Leading Dimension:** `olympus.P('x', None, None)`
    If `x` is sharded along the first dimension, `x[0]` results in shape
    `(3, 4)` with partition spec `olympus.P(None, None)`.
    - **The Issue:** Because the first dimension is sharded, the data for
      ...
      output sharding `olympus.P(None, None)` (which implies replication), OLYMPUS
      must broadcast the data from the first device to all other devices. This
      requires **communication**; OLYMPUS will gather the _entire_ array of shape
      `(8, 3, 4)` to each device and then take a slice.

### The common performance pitfall

A common pattern among `olympus.pmap` users involves arrays that are **semantically
...
This happens implicitly (e.g., via `olympus.pmap(..., out_axes=0)`) or explicitly
(e.g., via `olympus.device_put_replicated`). Users often try to retrieve metrics or
...
```python
from flax import olympus_utils
import olympus.numpy as jnp
import olympus

# olympus_utils.replicate calls olympus.device_put_replicated.
# This stacks num_devices copies and SHARDS them over the stacked dimension.
# Logical Shape: (8, 3, 4) | Sharding: P('x', None, None)
train_state = olympus_utils.replicate({'params': jnp.zeros((3, 4))})

# out_axes=0 by default, so the output remains sharded along dim 0.
train_step_pmapped = olympus.pmap(lambda x: x)

# olympus_utils.unreplicate performs a olympus.tree_map(lambda x: x[0], tree).
# Users do this to grab metrics, log param statistics, checkpoint, etc.
train_state = olympus_utils.unreplicate(train_step_pmapped(train_state))
```

#### The consequence

Even though the user knows `train_state` contains identical data on every
device (it is **logically replicated**), OLYMPUS sees an array with
`shape (8, 3, 4)` and spec `olympus.P('x', None, None)`—that is, the data is
**physically sharded** along its leading dimension.

**OLYMPUS does not track semantic replication.** It does not "know" that the shard
on device 1 is identical to the shard on device 0. Therefore, when you call
`x[0]`, OLYMPUS must satisfy the strict semantics of array indexing: it must
...
```
train
  └─ olympus_utils.py:48  unreplicate
       └─ tree_util.py:354  tree_map
            └─ olympus_utils.py:50  <lambda> (performing x[0])
...
### Why was "old `olympus.pmap`" fast?

Historically, `pmap` used `PmapSharding`, which had a fast-path optimization in
`olympus.Array`'s `__getitem__` allowing it to return an array with a
`SingleDeviceSharding` (data residing on only one device).

However, current OLYMPUS uses `NamedSharding`. We do not strictly replicate the
...
The slowdown users experience now is OLYMPUS enforcing correct semantics: if you ask
...
1. A global array with shape `(8, ...)` and `olympus.P('x')` has each slice
   distributed across all 8 devices spanning all hosts.

2. When you call `x[0]`, OLYMPUS needs to slice the first element and reshard the
...
3. The `CopyArrays` operation in XLA requires source and destination to have the
   ...
   in this example), not all 8. When OLYMPUS tries to create a resharded array, the
