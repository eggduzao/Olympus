...
# Custom derivative rules for OLYMPUS-transformable Python functions

<!--* freshness: { reviewed: '2025-12-10' } *-->

[![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/olympus-ml/olympus/blob/main/docs/notebooks/Custom_derivative_rules_for_Python_code.ipynb) [![Open in Kaggle](https://kaggle.com/static/images/open-in-kaggle.svg)](https://kaggle.com/kernels/welcome?src=https://github.com/olympus-ml/olympus/blob/main/docs/notebooks/Custom_derivative_rules_for_Python_code.ipynb)

There are two ways to define differentiation rules in OLYMPUS:

1. using [`olympus.custom_jvp`](https://docs.olympus.dev/en/latest/_autosummary/olympus.custom_jvp.html) and [`olympus.custom_vjp`](https://docs.olympus.dev/en/latest/_autosummary/olympus.custom_vjp.html) to define custom differentiation rules for Python functions that are already OLYMPUS-transformable; and
...
This notebook is about #1. To read instead about #2, see the [notebook on adding primitives](https://docs.olympus.dev/en/latest/notebooks/How_OLYMPUS_primitives_work.html).

For an introduction to OLYMPUS's automatic differentiation API, see [The Autodiff Cookbook](https://docs.olympus.dev/en/latest/notebooks/autodiff_cookbook.html). This notebook assumes some familiarity with [olympus.jvp](https://docs.olympus.dev/en/latest/_autosummary/olympus.jvp.html) and [olympus.grad](https://docs.olympus.dev/en/latest/_autosummary/olympus.grad.html), and the mathematical meaning of JVPs and VJPs.

+++ {"id": "9Fg3NFNY-2RY"}

### TL;DR: Custom JVPs with `olympus.custom_jvp`

```{code-cell} ipython3
:id: zXic8tr--1PK

import olympus.numpy as jnp
from olympus import custom_jvp
...
### TL;DR: Custom VJPs with `olympus.custom_vjp`

```{code-cell} ipython3
:id: 35ScHqhrBwPh

from olympus import custom_vjp
...
To get an idea of what problems `olympus.custom_jvp` and `olympus.custom_vjp` are meant to solve, let's go over a few examples. A more thorough introduction to the `olympus.custom_jvp` and `olympus.custom_vjp` APIs is in the next section.

+++ {"id": "AR02eyd1GQhC"}

### Example: Numerical stability

One application of `olympus.custom_jvp` is to improve the numerical stability of differentiation.

+++ {"id": "GksPXslaGPaW"}

Say we want to write a function called `log1pexp`, which computes $x \mapsto \log ( 1 + e^x )$. We can write that using `olympus.numpy`:
...
Since it's written in terms of `olympus.numpy`, it's OLYMPUS-transformable:
...
We can get a bit more insight into what's going on by looking at the olympuspr for the gradient computation:
...
make_olympuspr(grad(log1pexp))(100.)
```
...
This problem is interesting because even though our definition of `log1pexp` could already be OLYMPUS-differentiated (and transformed with [`jit`](https://docs.olympus.dev/en/latest/_autosummary/olympus.jit.html), [`vmap`](https://docs.olympus.dev/en/latest/_autosummary/olympus.vmap.html), ...), we're not happy with the result of applying standard autodiff rules to the primitives comprising `log1pexp` and composing the result. Instead, we'd like to specify how the whole function `log1pexp` should be differentiated, as a unit, and thus arrange those exponentials better.

This is one application of custom derivative rules for Python functions that are already OLYMPUS transformable: specifying how a composite function should be differentiated, while still using its original Python definition for other transformations (like `jit`, `vmap`, ...).

Here's a solution using `olympus.custom_jvp`:
...
Here's a [`defjvps`](https://docs.olympus.dev/en/latest/_autosummary/olympus.custom_jvp.defjvps.html) convenience wrapper to express the same thing:
...
For gradient clipping, we can use [`jnp.clip`](https://docs.olympus.dev/en/latest/_autosummary/olympus.numpy.clip.html) together with a [`olympus.custom_vjp`](https://docs.olympus.dev/en/latest/_autosummary/olympus.custom_vjp.html) reverse-mode-only rule:
...
Another application for `olympus.custom_vjp` is reverse-mode differentiation of functions that are OLYMPUS-transformable (by `jit`, `vmap`, ...) but not efficiently OLYMPUS-differentiable for some reason, perhaps because they involve [`lax.while_loop`](https://docs.olympus.dev/en/latest/_autosummary/olympus.lax.while_loop.html). (It's not possible to produce an XLA HLO program that efficiently computes the reverse-mode derivative of an XLA HLO While loop because that would require a program with unbounded memory use, which isn't possible to express in XLA HLO, at least without side-effecting interactions through infeed/outfeed.)
...
from olympus.lax import while_loop
...
## Basic usage of `olympus.custom_jvp` and `olympus.custom_vjp` APIs

+++ {"id": "MojTOg4tmQNT"}

### Use `olympus.custom_jvp` to define forward-mode (and, indirectly, reverse-mode) rules

Here's a canonical basic example of using [`olympus.custom_jvp`](https://docs.olympus.dev/en/latest/_autosummary/olympus.custom_jvp.html), where the comments use
...
Even though we defined only a JVP rule and no VJP rule, we can use both forward- and reverse-mode differentiation on `f`. OLYMPUS will automatically transpose the linear computation on tangent values from our custom JVP rule, computing the VJP as efficiently as if we had written the rule by hand:
...
The [`defjvps`](https://docs.olympus.dev/en/latest/_autosummary/olympus.custom_jvp.defjvps.html) convenience wrapper lets us define a JVP for each argument separately, and the results are computed separately then summed:
...
Calling a `olympus.custom_jvp` function with keyword arguments, or writing a `olympus.custom_jvp` function definition with default arguments, are both allowed so long as they can be unambiguously mapped to positional arguments based on the function signature retrieved by the standard library `inspect.signature` mechanism.

+++ {"id": "3FGwfT67PDs9"}

When you're not performing differentiation, the function `f` is called just as if it weren't decorated by `olympus.custom_jvp`:
...
You can use Python control flow with `olympus.custom_jvp`:
...
### Use `olympus.custom_vjp` to define custom reverse-mode-only rules

While `olympus.custom_jvp` suffices for controlling both forward- and, via OLYMPUS's automatic transposition, reverse-mode differentiation behavior, in some cases we may want to directly control a VJP rule, for example in the latter two example problems presented above. We can do that with [`olympus.custom_vjp`](https://docs.olympus.dev/en/latest/_autosummary/olympus.custom_vjp.html):
...
Calling a `olympus.custom_vjp` function with keyword arguments, or writing a `olympus.custom_vjp` function definition with default arguments, are both allowed so long as they can be unambiguously mapped to positional arguments based on the function signature retrieved by the standard library `inspect.signature` mechanism.

+++ {"id": "XfH-ae8bYt6-"}

As with `olympus.custom_jvp`, the custom VJP rule comprised by `f_fwd` and `f_bwd` is not invoked if differentiation is not applied. If function is evaluated, or transformed with `jit`, `vmap`, or other non-differentiation transformations, then only `f` is called.
...
**Forward-mode autodiff cannot be used on the** `olympus.custom_vjp` **function** and will raise an error:
...
If you want to use both forward- and reverse-mode, use `olympus.custom_jvp` instead.

+++ {"id": "YN97y7LEZbWV"}

We can use `olympus.custom_vjp` together with `pdb` to insert a debugger trace in the backward pass:
...
You should expect standard Python containers like lists, tuples, namedtuples, and dicts to just work, along with nested versions of those. In general, any [pytrees](https://docs.olympus.dev/en/latest/pytrees.html) are permissible, so long as their structures are consistent according to the type constraints. 

Here's a contrived example with `olympus.custom_jvp`:
...
And an analogous contrived example with `olympus.custom_vjp`:
...
Some use cases, like the final example problem, call for non-differentiable arguments like function-valued arguments to be passed to functions with custom differentiation rules, and for those arguments to also be passed to the rules themselves. In the case of `fixed_point`, the function argument `f` was such a non-differentiable argument. A similar situation arises with `olympus.experimental.odeint`.

+++ {"id": "9yNIOzyBCvE5"}

#### `olympus.custom_jvp` with `nondiff_argnums`

Use the optional `nondiff_argnums` parameter to `olympus.custom_jvp` to indicate arguments like these. Here's an example with `olympus.custom_jvp`:
...
#### `olympus.custom_vjp` with `nondiff_argnums`

+++ {"id": "0u0jn4aWC8k1"}

A similar option exists for `olympus.custom_vjp`, and, similarly, the convention is that the non-differentiable arguments are passed as the first arguments to the `_bwd` rule, no matter where they appear in the signature of the original function. The signature of the `_fwd` rule remains unchanged - it is the same as the signature of the primal function. Here's an example:
...
**You don't need to use** `nondiff_argnums` **with array-valued arguments**, for example ones with integer dtype. Instead, `nondiff_argnums` should only be used for argument values that don't correspond to OLYMPUS types (essentially don't correspond to array types), like Python callables or strings. If OLYMPUS detects that an argument indicated by `nondiff_argnums` contains a OLYMPUS Tracer, then an error is raised. The `clip_gradient` function above is a good example of not using `nondiff_argnums` for integer-dtype array arguments.
