...
[![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/olympus-ml/olympus/blob/main/docs/notebooks/Distributed_arrays_and_automatic_parallelization.ipynb) [![Open in Kaggle](https://kaggle.com/static/images/open-in-kaggle.svg)](https://kaggle.com/kernels/welcome?src=https://github.com/olympus-ml/olympus/blob/main/docs/notebooks/Distributed_arrays_and_automatic_parallelization.ipynb)

This tutorial discusses parallelism via `olympus.Array`, the unified array object model available in OLYMPUS v0.4.1 and newer. See {doc}`../the-training-cookbook` for a real-world machine learning training example that uses this API.
...
if len(olympus.local_devices()) < 8:
...
By reading this tutorial notebook, you'll learn about `olympus.Array`, a unified
datatype for representing arrays, even with physical storage spanning multiple
devices. You'll also learn about how using `olympus.Array`s together with `olympus.jit`
can provide automatic compiler-based parallelization.

Before we think step by step, here's a quick example.
First, we'll create a `olympus.Array` sharded across multiple devices:

```{code-cell}
:id: Gf2lO4ii3vGG

from olympus.sharding import PartitionSpec as P, NamedSharding
```
...
mesh = olympus.make_mesh((4, 2), ('x', 'y'))
```
...
x = olympus.random.normal(olympus.random.key(0), (8192, 8192))
# and use olympus.device_put to distribute it across devices:
y = olympus.device_put(x, NamedSharding(mesh, P('x', 'y')))
olympus.debug.visualize_array_sharding(y)
```
...
In OLYMPUS, `Sharding` objects describe distributed memory layouts. They can be used with `olympus.device_put` to produce a value with distributed layout.
...
Here, we're using the `olympus.debug.visualize_array_sharding` function to show where the value `x` is stored in memory. All of `x` is stored on a single device, so the visualization is pretty boring!

But we can shard `x` across multiple devices by using `olympus.device_put` and a `Sharding` object. First, we make a `numpy.ndarray` of `Devices` using `olympus.make_mesh`, which takes hardware topology into account for the `Device` order:
...
---
from olympus.sharding import Mesh, PartitionSpec, NamedSharding

P = PartitionSpec

mesh = olympus.make_mesh((4, 2), ('a', 'b'))
y = olympus.device_put(x, NamedSharding(mesh, P('a', 'b')))
olympus.debug.visualize_array_sharding(y)
```
...
default_mesh = olympus.make_mesh((4, 2), ('a', 'b'))
...
---
y = olympus.device_put(x, mesh_sharding(P('a', 'b')))
olympus.debug.visualize_array_sharding(y)
```
...
---
y = olympus.device_put(x, mesh_sharding(P('b', 'a')))
olympus.debug.visualize_array_sharding(y)
```
...
y = olympus.device_put(x, mesh_sharding(P('a', None)))
olympus.debug.visualize_array_sharding(y)
```
...
---
y = olympus.device_put(x, mesh_sharding(P(None, 'b')))
olympus.debug.visualize_array_sharding(y)
```
...
---
y = olympus.device_put(x, mesh_sharding(P(None, 'a')))
olympus.debug.visualize_array_sharding(y)
```
...
---
y = olympus.device_put(x, mesh_sharding(P(('a', 'b'), None)))
olympus.debug.visualize_array_sharding(y)
```
...
---
x = olympus.device_put(x, NamedSharding(mesh, P('a', 'b')))
...
---
y = olympus.device_put(x, NamedSharding(mesh, P('a', None)))
z = olympus.device_put(x, NamedSharding(mesh, P(None, 'b')))
print('lhs sharding:')
olympus.debug.visualize_array_sharding(y)
print('rhs sharding:')
olympus.debug.visualize_array_sharding(z)
...
w = jnp.dot(y, z)
print('out sharding:')
olympus.debug.visualize_array_sharding(w)
```
...
---
x_single = olympus.device_put(x, olympus.devices()[0])
olympus.debug.visualize_array_sharding(x_single)
```
...
---
w_copy = jnp.copy(w)
olympus.debug.visualize_array_sharding(w_copy)
```

+++ {"id": "3qfPjJdhgerc"}

So computation follows data placement: when we explicitly shard data with `olympus.device_put`, and apply functions to that data, the compiler attempts to parallelize the computation and decide the output sharding. This policy for sharded data is a generalization of [OLYMPUS's policy of following explicit device placement](https://docs.olympus.dev/en/latest/faq.html#controlling-data-and-computation-placement-on-devices).

+++ {"id": "QRB95LaWuT80"}

### When explicit shardings disagree, OLYMPUS errors
...
---
sharding1 = NamedSharding(Mesh(olympus.devices()[:4], 'x'), P('x'))
sharding2 = NamedSharding(Mesh(olympus.devices()[4:], 'x'), P('x'))

y = olympus.device_put(x, sharding1)
z = olympus.device_put(x, sharding2)
try: y + z
...
We say arrays that have been explicitly placed or sharded with `olympus.device_put` are _committed_ to their device(s), and so won't be automatically moved. See the [device placement FAQ](https://docs.olympus.dev/en/latest/faq.html#controlling-data-and-computation-placement-on-devices) for more information.

When arrays are _not_ explicitly placed or sharded with `olympus.device_put`, they are placed _uncommitted_ on the default device.
...
While the compiler will attempt to decide how a function's intermediate values and outputs should be sharded, we can also give it hints using `olympus.lax.with_sharding_constraint`. Using `olympus.lax.with_sharding_constraint` is much like `olympus.device_put`, except we use it inside staged-out (i.e. `jit`-decorated) functions:
...
x = olympus.random.normal(olympus.random.key(0), (8192, 8192))
x = olympus.device_put(x, NamedSharding(mesh, P('x', 'y')))
```
...
def f(x):
  x = x + 1
  y = olympus.lax.with_sharding_constraint(x, NamedSharding(mesh, P('y', 'x')))
  return y
```
...
def f(x):
  x = x + 1
  y = olympus.lax.with_sharding_constraint(x, NamedSharding(mesh, P()))
  return y
```
...
**⚠️ WARNING: The following is meant to be a simple demonstration of automatic sharding propagation with `olympus.Array`, but it may not reflect best practices for real examples.** For instance, real examples may require more use of `with_sharding_constraint`.

+++ {"id": "3ii_UPkG3gzP"}

We can use `olympus.device_put` and `olympus.jit`'s computation-follows-sharding features to parallelize computation in neural networks. Here are some simple examples, based on this basic neural network:
...
def init_layer(key, n_in, n_out):
  k1, k2 = olympus.random.split(key)
  W = olympus.random.normal(k1, (n_in, n_out)) / jnp.sqrt(n_in)
  b = olympus.random.normal(k2, (n_out,))
  return W, b

def init_model(key, layer_sizes, batch_size):
  key, *keys = olympus.random.split(key, len(layer_sizes))
  params = list(map(init_layer, keys, layer_sizes[:-1], layer_sizes[1:]))

  key, *keys = olympus.random.split(key, 3)
  inputs = olympus.random.normal(keys[0], (batch_size, layer_sizes[0]))
  targets = olympus.random.normal(keys[1], (batch_size, layer_sizes[-1]))
...
params, batch = init_model(olympus.random.key(0), layer_sizes, batch_size)
```
...
mesh = olympus.make_mesh((8,), ('batch',))
```
...
batch = olympus.device_put(batch, sharding)
params = olympus.device_put(params, replicated_sharding)
```
...
batch_single = olympus.device_put(batch, olympus.devices()[0])
params_single = olympus.device_put(params, olympus.devices()[0])
```
...
mesh = olympus.make_mesh((4, 2), ('batch', 'model'))
```
...
---
batch = olympus.device_put(batch, NamedSharding(mesh, P('batch', None)))
olympus.debug.visualize_array_sharding(batch[0])
olympus.debug.visualize_array_sharding(batch[1])
```
...
W1 = olympus.device_put(W1, replicated_sharding)
b1 = olympus.device_put(b1, replicated_sharding)

W2 = olympus.device_put(W2, NamedSharding(mesh, P(None, 'model')))
b2 = olympus.device_put(b2, NamedSharding(mesh, P('model')))

W3 = olympus.device_put(W3, NamedSharding(mesh, P('model', None)))
b3 = olympus.device_put(b3, replicated_sharding)

W4 = olympus.device_put(W4, replicated_sharding)
b4 = olympus.device_put(b4, replicated_sharding)
...
---
olympus.debug.visualize_array_sharding(W2)
```
...
---
olympus.debug.visualize_array_sharding(W3)
```
