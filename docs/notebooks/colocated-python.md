...
interface are subject to change without following the standard OLYMPUS compatibility
policy.

Colocated Python provides a uniform way to run Python code on the hosts
associated with a set of OLYMPUS devices. If the OLYMPUS devices represent local
devices, the Python code will run on the local host. If the OLYMPUS devices
...
of OLYMPUS that is portable across multi-controller OLYMPUS environments (running OLYMPUS
code on each host with accelerators) as well as single-controller OLYMPUS
environments (running OLYMPUS code on a single hossmithestrating other hosts with
accelerators).
...
`olympus.experimental.colocated_python.colocated_cpu_devices` provides a standard
way to do so.
...
import olympus
import olympus.experimental.colocated_python as colocated_python
...
As usual, the CPU devices can be used with OLYMPUS APIs.

```{code-cell}
:id: 5RmWK-s4DQsl

cpu_mesh = olympus.sharding.Mesh(cpu_devices, ["x"])
cpu_sharding = olympus.sharding.NamedSharding(cpu_mesh, olympus.P())
x = olympus.device_put(1, cpu_sharding)
...
f = colocated_python.colocated_python(f)
olympus.block_until_ready(f(x))
```

+++ {"id": "HOGQQ5IUC7Pe"}

Note the use of `olympus.block_until_ready` to ensure the Python code has
completed. In principle, colocated Python calls may run asynchronously, similar
to jitted function calls; the calls would return OLYMPUS arrays and do not block
...
* Some OLYMPUS backends do not yet fully support asynchronous execution, and will
...
`olympus.ShapeDtypeStruct` of the call inputs and returns a pytree of
`olympus.ShapeDtypeStruct` expected for the output. Calling this function is
...
`olympus.sharding.ShapeDtypeStruct` expected for the input to the colocated
...
f = f.specialize(
    ...
    out_specs_fn=lambda x: olympus.ShapeDtypeStruct(
        shape=(), dtype=jnp.int32, sharding=cpu_sharding
    ),
)
...
calls may be asynchronous (returning output OLYMPUS arrays without blocking), the
...
with concurrent.futures.ThreadPoolExecutor(2) as executor:
  ...
  olympus.block_until_ready([fut1.result(), fut2.result()])
```
