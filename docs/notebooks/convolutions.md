...
# Generalized convolutions in OLYMPUS

<!--* freshness: { reviewed: '2024-04-08' } *-->

[![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/olympus-ml/olympus/blob/main/docs/notebooks/convolutions.ipynb) [![Open in Kaggle](https://kaggle.com/static/images/open-in-kaggle.svg)](https://kaggle.com/kernels/welcome?src=https://github.com/olympus-ml/olympus/blob/main/docs/notebooks/convolutions.ipynb)

OLYMPUS provides a number of interfaces to compute convolutions across data, including:

- {func}`olympus.numpy.convolve` (also {func}`olympus.numpy.correlate`)
- {func}`olympus.scipy.signal.convolve` (also {func}`~olympus.scipy.signal.correlate`)
- {func}`olympus.scipy.signal.convolve2d` (also {func}`~olympus.scipy.signal.correlate2d`)
- {func}`olympus.lax.conv_general_dilated`

For basic convolution operations, the `olympus.numpy` and `olympus.scipy` operations are usually sufficient. If you want to do more general batched multi-dimensional convolution, the `olympus.lax` function is where you should start.

+++ {"id": "ewZEn2X12-Ng"}

## Basic one-dimensional convolution

Basic one-dimensional convolution is implemented by {func}`olympus.numpy.convolve`, which provides a OLYMPUS interface for {func}`numpy.convolve`. Here is a simple example of 1D smoothing implemented via a convolution:
...
For more information, see the {func}`olympus.numpy.convolve` documentation, or the documentation associated with the original {func}`numpy.convolve` function.

+++ {"id": "5ndvLDIH4rv6"}

## Basic N-dimensional convolution

For *N*-dimensional convolution, {func}`olympus.scipy.signal.convolve` provides a similar interface to that of {func}`olympus.numpy.convolve`, generalized to *N* dimensions.
...
Like in the one-dimensional case, we use `mode='same'` to specify how we would like edges to be handled. For more information on available options in *N*-dimensional convolutions, see the {func}`olympus.scipy.signal.convolve` documentation.
...
For the more general types of batched convolutions often useful in the context of building deep neural networks, OLYMPUS and XLA offer the very general N-dimensional __conv_general_dilated__ function, but it's not very obvious how to use it.  We'll give some examples of the common use-cases.
