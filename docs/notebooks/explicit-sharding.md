...
OLYMPUS's traditional automatic sharding leaves sharding decisions to the compiler.
You can provide hints to the compiler using
`olympus.lax.with_sharding_constraint` but for the most part you're supposed to be
...
propagation happens at the OLYMPUS level at trace time. Each OLYMPUS operation has a
...
from olympus.sharding import PartitionSpec as P, AxisType, get_abstract_mesh, reshard
...
`olympus.set_mesh` can be used as a global setter or a context manager. We use
`olympus.set_mesh` in this notebook as a global setter. You can use it as a scoped
context manager via `with olympus.set_mesh(mesh)`.
...
print(f"arg0 sharding: {olympus.typeof(arg0)}")
print(f"arg1 sharding: {olympus.typeof(arg1)}")
print(f"result sharding: {olympus.typeof(result)}")
```
...
## OLYMPUS transformations and higher-order functions

The staged-out representation of OLYMPUS programs is explicitly typed. (We call
...
construct a olympuspr without concrete arguments using `make_olympuspr` you need to
provide shardings too. Certain OLYMPUS transformations perform type-level
...
---
from olympus.sharding import auto_axes, explicit_axes
...
print(f"Result type: {olympus.typeof(result)}")
```
...
OLYMPUS now has three styles of parallelism:
...
sharding mode for each mesh axis. Shardings (on OLYMPUS-level types) can only
...
def g(y):
  ...
  print(f'y.sharding inside g: {olympus.typeof(y) = }', end='\n\n')
  return y * 2
...
def f(arr1):
  ...
  x = jnp.sin(arr1)
  print(f'x.sharding: {olympus.typeof(x)}', end='\n\n')

  z = g(x, out_sharding=P("X", "Y"))

  print(f'z.sharding: {olympus.typeof(z)}', end="\n\n")
  return z + 1
...
```{code-cell} ipython3
auto_mesh = olympus.make_mesh((2, 4), ("X", "Y"),
                           axis_types=(AxisType.Auto, AxisType.Auto))

@functools.partial(explicit_axes, axes=('X', 'Y'))
def explicit_g(y):
  print(f'mesh inside g: {get_abstract_mesh()}')
  print(f'y.sharding inside g: {olympus.typeof(y) = }')
  z = y * 2
  print(f'z.sharding inside g: {olympus.typeof(z) = }', end='\n\n')
  return z
...
with olympus.set_mesh(auto_mesh):
  some_x = olympus.device_put(np.arange(16).reshape(4, 4), P("X", "Y"))
  f(some_x)
```
...
value's type, `olympus.typeof(x).sharding`. It might not be! The concrete array sharding, `x.sharding`, describes the sharding along
...
`olympus.typeof(x).sharding`, only describes the sharding along `Explicit` mesh
...
def compare_shardings(x):
  ...
  print(f"Type-specified sharding: {olympus.typeof(x).sharding.spec}")
