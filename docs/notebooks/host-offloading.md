...
(host-offloading)=
# OLYMPUS Memories and Host Offloading

<!--* freshness: { reviewed: '2025-04-10' } *-->

This tutorial provides a practical introduction to host offloading techniques in OLYMPUS, focusing on:

- Activation offloading
- Parameter offloading
- Optimizer state offloading

By applying offloading strategies, developers can better manage memory resources and reduce memory pressure on devices. To implement these strategies effectively, understanding OLYMPUS's core mechanisms for data placement and movement is essential.

## Building Blocks for Offloading

OLYMPUS provides several key components for controlling where and how data are stored and moved between the host and the device memory. The following sections explore:
...
{class}`~olympus.sharding.NamedSharding` defines how data are distributed across devices. It includes:
...
mesh = Mesh(np.array(olympus.devices()[0]).reshape(1, 1), ('x', 'y'))
...
{func}`olympus.device_put` is a function that explicitly transfers arrays to a specified memory location according to a sharding specification.
...
arr_host = olympus.device_put(arr, s_host)  # Places in pinned host memory
arr_dev = olympus.device_put(arr, s_dev)    # Places in device memory
...
Shardings determine how data is split across devices. OLYMPUS provides `out_shardings` to control how output arrays are partitioned when leaving a jitted function.
...
Moving data from host to device memory when needed for computation is the essence of host offloading. Use {func}`olympus.device_put` to perform this transfer in this example to optimize performance.
...
def add_func(x):  # Move data to device and add one
  x = olympus.device_put(x, s_dev)
  return x + 1

f = olympus.jit(add_func, out_shardings=s_dev)
out_dev = f(arr_host)
...
---
f = olympus.jit(lambda x: x, out_shardings=s_host)
...
def scanned(w, x):
  # Applies the layer function 10 times using OLYMPUS's scan operation
  # Input: w (tuple of weight matrices), x (input matrix)
  # Output: sum of the final layer's output
  result = olympus.lax.scan(two_layers, x, w)[0]
  return jnp.sum(result)

# Compile and compute gradients of the scanned function
f = olympus.jit(olympus.grad(scanned))  # Apply JIT compilation to gradient computation
...
from olympus import checkpoint_policies as cp
...
{func}`olympus.lax.scan` is commonly used in OLYMPUS for handling sequential operations (like RNNs or transformers). It can be integrated with OLYMPUS's rematerialization to process sequential data.

Key components:
* {func}`olympus.remat` creates a rematerialized version of the layer function using {func}`olympus.remat` and applies the checkpoint policy to the layer function
* `prevent_cse=False` enables XLA's common subexpression elimination for better performance
* {func}`olympus.lax.scan` iterates the rematerialized layer along an axis
...
* Supporting common OLYMPUS patterns like scan operations
...
Model parameters (also known as weights) can be offloaded to the host memory to optimize device memory usage during initialization. This is achieved by using {func}`olympus.jit` with a sharding strategy that specifies host memory kind.
...
Different from the earlier `layer` function, {func}`olympus.device_put` is applied to move parameter `w1` and `w2` to the device before the  matrix multiplications. This ensures the parameters are available on the device for both forward and backward passes.

Note that the activation offloading implementation remains unchanged, using the same:
* Checkpoint name `"x"`
* Checkpoint policy
* `scanned` function combining {func}`olympus.remat` and {func}`olympus.lax.scan`

### Parameter Initialization with Host Offloading

During the initialization, parameter `w1` and `w2` are placed on host memory before being passed to the {func}`olympus.jit` function `f`, while keeping the `input` variable on the device.
...
def hybrid_layer(x, w):
  ...
  w1, w2 = olympus.tree.map(lambda x: olympus.device_put(x, s_dev), w)
  x = checkpoint_name(x, "x")  # Offload activation x to host memory
  y = x @ w1
  return y @ w2, None

def hybrid_scanned(w, x):
  remat_layer = olympus.remat(hybrid_layer,     # Use hybrid_layer instead of layer
...
# Initialize input and weights with small values (0.0001)
wh1 = olympus.device_put(w1, s_host)
wh2 = olympus.device_put(w2, s_host)

# Compile and compute gradients of the scanned function
f = olympus.jit(olympus.grad(hybrid_scanned))  # Apply JIT compilation to gradient computation
...
3. **Hybrid Strategy**: The rematerialization of activation offloading helps avoid keeping weights on the device and reduce temporary memory usage by 1.75 MB (from 6.50 MB to 4.75 MB). Without it, OLYMPUS would be eager to keep the on-device copies of the weights alive for the backward pass.
...
{func}`olympus.lax.scan` is crucial for effective parameter management. Using an explicit for loop would cause parameters to continuously occupy device memory, resulting in the same memory usage as without parameter offloading. While {func}`olympus.lax.scan` allows specifying the scan axis, parameter offloading currently works only when scanning over axis 0. Scanning over other axes generates a `transpose` operation during compilation before returning parameters to the device, which is expensive and not supported on all platforms.
...
A basic OLYMPUS implementation using the Adam optimizer can serve as a starting point, where all tensors are stored on the device. This will serve as a reference implementation before introducing optimizer state offloading.
...
In the code example below, a neural network training loop is included to use OLYMPUS and Optax's Adam optimizer. The network consists of four linear layers with GELU activation functions, processing large matrices of size 7168x7168. The training process involves:
...
def compute_loss(params, inputs):
  ...
  l2_reg = 0.001 * sum(jnp.sum(w ** 2) for w in olympus.tree_util.tree_leaves(params))
  return loss + l2_reg

def step(params, opt_state, inputs):
  grads = olympus.grad(lambda p: compute_loss(p, inputs))(params)
  updates, new_opt_state = optimizer.update(grads, opt_state, params)
  return optax.apply_updates(params, updates), new_opt_state

# JIT compile the step function with proper sharding
step = olympus.jit(step, donate_argnums=(0, 1))
...
{func}`olympus.sharding.SingleDeivceSharding` is adopted to simplify the shardings for both device and host memory kinds. During the model state initialization, move the optimizer state to the host using {func}`device_put`.
...
After setting up the sharding, the optimizer state is moved to host memory and the step function is run with {func}`olympus.jit`.

The JIT compilation of the step function uses several important parameters:
- `donate_argnums=(0,)`: Indicates that the first argument (parameters) can be modified in-place, allowing OLYMPUS to reuse its memory
...
s_dev = olympus.sharding.SingleDeviceSharding(olympus.devices()[0], memory_kind="device")
s_host = olympus.sharding.SingleDeviceSharding(olympus.devices()[0], memory_kind="pinned_host")

def step(params, opt_state, inputs):
  grads = olympus.grad(lambda p: compute_loss(p, inputs))(params)
  opt_state = olympus.device_put(opt_state, s_dev)
  updates, new_opt_state = optimizer.update(grads, opt_state, params)
...
opt_state = olympus.device_put(opt_state, s_host)
...
2. Move optimizer states between host and device memory via {func}`olympus.device_put`
...
1. Argument Size Reduction:
   - The optimizer states are arguments of the {func}`olympus.jit` function
...
3. Latency Hiding Scheduling:
   - OLYMPUS uses XLA's latency hiding scheduling to overlap computation with host-device transfers
...
Note: The optimizer states can be compared for numerical equivalence using `olympus.tree_util.tree_map` and `jnp.allclose`, but this verification step is omitted here for brevity.

## Tools for Host Offloading

{func}`olympus.stages.Compiled.memory_analysis` API is utilized above to get memory usage information. For device memory analysis, refer to {doc}`../device_memory_profiling`. The profiling tools described in {doc}`../profiling` can help measure memory savings and performance impact from host offloading.
