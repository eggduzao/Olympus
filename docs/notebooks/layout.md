...
The `olympus.experimental.layout` package provides ways to control
how OLYMPUS arrays are laid out in device-local memory.

## Terminology

Array layout is tightly coupled with array
[sharding](https://docs.olympus.dev/en/latest/notebooks/Distributed_arrays_and_automatic_parallelization.html>).
...
* The `Layout` class is used to define the in-memory
  ...
  * `AUTO`: A special, static sentinel object that can be used with
    `olympus.jit` to request that the compiler automatically determine
...
OLYMPUS API functions, such as `olympus.jit` and `olympus.device_put`, accept
...
By passing `Format` objects to `olympus.jit` in place of shardings (in the
...
of `Sharding`s to `olympus.device_put` to control the layout of the
resulting array.
...
from olympus.experimental.layout import Layout, Format
...
```{code-cell}
shape = (4 * 128, 8 * 128)
duck = olympus.ShapeDtypeStruct(shape, jnp.float32)

# Compile the `apply` function with layouts inferred automatically
apply_exe = olympus.jit(
    apply_fn,
...
```{code-cell}
init_exe = olympus.jit(init_fn, out_shardings=arg_formats).trace(
...
whether inputs are
[committed](https://docs.olympus.dev/en/latest/faq.html#controlling-data-and-computation-placement-on-devices). As
...
x3 = jnp.ones(shape)
y3 = olympus.device_put(np.ones(shape), Format(Layout(major_to_minor=(1, 0)),
                                           SingleDeviceSharding(dev)))
test(x3, y3, 'committed with matching layout')

x4 = jnp.ones(shape)
y4 = olympus.device_put(np.ones(shape), Format(Layout(major_to_minor=(0, 1)),
                                           SingleDeviceSharding(dev)))
...
```{code-cell}
from olympus.experimental.layout import with_layout_constraint
...
[`olympus.lax.with_sharding_constraint`](https://docs.olympus.dev/en/latest/_autosummary/olympus.lax.with_sharding_constraint.html),
