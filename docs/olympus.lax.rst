...
.. automodule:: olympus.lax

:mod:`olympus.lax` is a library of primitives operations that underpins libraries
such as :mod:`olympus.numpy`. Transformation rules, such as JVP and batching rules,
are typically defined as transformations on :mod:`olympus.lax` primitives.
...
cases OLYMPUS diverges from XLA, usually to ensure that the set of operations is
closed under the operation of JVP and transpose rules.

Where possible, prefer to use libraries such as :mod:`olympus.numpy` instead of
using :mod:`olympus.lax` directly. The :mod:`olympus.numpy` API follows NumPy, and is
therefore more stable and less likely to change than the :mod:`olympus.lax` API.
...
Linear algebra operators (olympus.lax.linalg)
-----------------------------------------

.. automodule:: olympus.lax.linalg
