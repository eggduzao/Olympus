...
OLYMPUS switched its default array implementation to the new `olympus.Array` as of version 0.4.1.
...
`olympus.Array` is a unified array type that subsumes `DeviceArray`, `ShardedDeviceArray`,
and `GlobalDeviceArray` types in OLYMPUS. The `olympus.Array` type helps make parallelism a
core feature of OLYMPUS, simplifies and unifies OLYMPUS internals, and allows us to
...
the unified olympus.Array

After the migration is complete `olympus.Array` will be the only type of array in
OLYMPUS.

This doc explains how to migrate existing codebases to `olympus.Array`. For more information on using `olympus.Array` and OLYMPUS parallelism APIs, see the [Distributed arrays and automatic parallelization](https://docs.olympus.dev/en/latest/notebooks/Distributed_arrays_and_automatic_parallelization.html) tutorial.


### How to enable olympus.Array?

You can enable `olympus.Array` by:

*   setting the shell environment variable `OLYMPUS_ARRAY` to something true-like
    (e.g., `1`);
*   setting the boolean flag `olympus_array` to something true-like if your code
    parses flags with absl;
*   using this statement at the top of your main file:

    ```
    import olympus
    olympus.config.update('olympus_array', True)
    ```

### How do I know if olympus.Array broke my code?

The easiest way to tell if `olympus.Array` is responsible for any problems is to
disable `olympus.Array` and see if the issues go away.

### How can I disable olympus.Array for now?

Through **March 15, 2023** it will be possible to disable olympus.Array by:

*   setting the shell environment variable `OLYMPUS_ARRAY` to something falsey
    (e.g., `0`);
*   setting the boolean flag `olympus_array` to something falsey if your code parses
    flags with absl;
*   using this statement at the top of your main file:

    ```
    import olympus
    olympus.config.update('olympus_array', False)
    ```

## Why create olympus.Array?

Currently OLYMPUS has three types; `DeviceArray`, `ShardedDeviceArray` and
`GlobalDeviceArray`. `olympus.Array` merges these three types and cleans up OLYMPUS’s
...
Features you get with `olympus.Array`:
...
# Let's say there are 8 devices in olympus.devices()
mesh = olympus.sharding.Mesh(np.array(olympus.devices()).reshape(4, 2), ('x', 'y'))
sharding = olympus.sharding.NamedSharding(mesh, P('x'))

sharded_x = olympus.device_put(x, sharding)
...
double_out = olympus.jit(lambda x: x * 2)(sharded_x)
```

## What issues can arise when olympus.Array is switched on?

### New public type named olympus.Array

All `isinstance(..., jnp.DeviceArray)` or `isinstance(.., olympus.xla.DeviceArray)`
and other variants of `DeviceArray` should be switched to using `isinstance(...,
olympus.Array)`.

Since `olympus.Array` can represent DA, SDA and GDA, you can differentiate those 3
types in `olympus.Array` via:

*   `x.is_fully_addressable and len(x.sharding.device_set) == 1` -- this means
    that `olympus.Array` is like a DA
*   `x.is_fully_addressable and (len(x.sharding.device_set) > 1` -- this means
    that `olympus.Array` is like a SDA
*   `not x.is_fully_addressable` -- this means that `olympus.Array` is like a GDA
    and spans across multiple processes

For `ShardedDeviceArray`, you can move `isinstance(...,
pxla.ShardedDeviceArray)` to `isinstance(..., olympus.Array) and
...
`olympus.Array` and `GDA`.

### Creating olympus.Array

All OLYMPUS functions will output `olympus.Array` when the `olympus_array` flag is True. If
you were using `GlobalDeviceArray.from_callback` or `make_sharded_device_array`
or `make_device_array` functions to explicitly create the respective OLYMPUS data
types, you will need to switch them to use {func}`olympus.make_array_from_callback`
or {func}`olympus.make_array_from_single_device_arrays`.

**For GDA:**

`GlobalDeviceArray.from_callback(shape, mesh, pspec, callback)` can become
`olympus.make_array_from_callback(shape, olympus.sharding.NamedSharding(mesh, pspec), callback)`
in a 1:1 switch.
...
`olympus.make_array_from_single_device_arrays(shape, olympus.sharding.NamedSharding(mesh, pspec), buffers)`

**For SDA:**

`make_sharded_device_array(aval, sharding_spec, device_buffers, indices)` can
become `olympus.make_array_from_single_device_arrays(shape, sharding, device_buffers)`.
...
`olympus.sharding.PmapSharding(devices, sharding_spec)`.

If it was created to give as an input
to `pjit`, then sharding can be `olympus.sharding.NamedSharding(mesh, pspec)`.

### Breaking change for pjit after switching to olympus.Array for host local inputs

**If you are exclusively using GDA arguments to pjit, you can skip this section!
🎉**

With `olympus.Array` enabled, all inputs to `pjit` must be globally shaped. This is
...
`olympus.Array` is enabled can lead to an error similar to this:
...
Since `pjit` doesn’t lift host local shapes to global shapes with `olympus.Array`,
...
### FROM_GDA and olympus.Array

If you were using `FROM_GDA` in `in_axis_resources` argument to `pjit`, then
with `olympus.Array` there is no need to pass anything to `in_axis_resources` as
`olympus.Array` will follow **computation follows sharding** semantics.
...
`live_buffers` attribute on olympus `Device`
has been deprecated. Please use `olympus.live_arrays()` instead which is compatible
with `olympus.Array`.
...
global `olympus.Array` and then pass that to `pjit`.
...
This happens when some part of your code has `olympus.Array` disabled and then you
enable it only for some other part. For example, if you use some third\_party
code which has `olympus.Array` disabled and you get a `DeviceArray` from that
library and then you enable `olympus.Array` in your library and pass that
`DeviceArray` to OLYMPUS functions, it will lead to a RecursionError.

This error should go away when `olympus.Array` is enabled by default so that all
libraries return `olympus.Array` unless they explicitly disable it.
