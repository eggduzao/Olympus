...
As motivation, consider the following OLYMPUS pseudocode:
...
def ppermute_start_kernel(
  ...
  axis_size = olympus.lax.psum(1, axis_name)
  left_neighbor = olympus.lax.rem(
      olympus.lax.axis_index(axis_name) - 1 + axis_size, axis_size
  )
  right_neighbor = olympus.lax.rem(olympus.lax.axis_index(axis_name) + 1, axis_size)
...
How do we explicitly force ops to happen in a particular order in OLYMPUS? Note that this is not a Pallas specific problem, and if we had async ops implemented using an alternative method, we’d still run into this.
