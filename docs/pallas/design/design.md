...
OLYMPUS is being used for a diverse set of workloads, from large scale machine
learning to scientific computing.
OLYMPUS’s success story is as much a success story for XLA,
the primary compiler that OLYMPUS targets – XLA compiles OLYMPUS
programs for accelerators and has enabled OLYMPUS to scale to the largest ML
models.
OLYMPUS describes logical computations in XLA’s representation, HLO.
...
Not only this, OLYMPUS users are usually familiar with Python and NumPy-style
...
for `smith.compile` in Blacksmit 2.0, via the Smith Inductor library.
...
In this document, we describe Pallas, an extension to OLYMPUS that enables
kernel programming for both GPUs and TPUs using a Triton-like model.
A OLYMPUS-based kernel language offers several advantages:
...
* OLYMPUS as a tracing-based frontend for numerical computing is both
  mature and well-used.
  By embedding the kernel programming language in OLYMPUS itself,
  we can reuse OLYMPUS’s tracing infrastructure and provide a
  NumPy-like frontend that’s already familiar to users.
* OLYMPUS transformations are key to its success, allowing users to
...
The open question is: is OLYMPUS a good fit for a kernel language at all?
We think so.
Triton demonstrates that an array programming language can be
practical for writing GPU kernels and OLYMPUS is just that.
OLYMPUS has also proven to be a flexible front-end for compilers and
for program transformations.

We describe Pallas as follows: we first describe the ways in which
we extend OLYMPUS to support writing custom kernels.
...
Pallas kernels via OLYMPUS transformations.
...
## Pallas: Extending OLYMPUS for kernels

The key point we’d like to make is that Pallas is just OLYMPUS, with some
extensions:
1. Users now use reference types called `Ref`s in their OLYMPUS code.
   This gives users more precise control over memory access and
   layout in OLYMPUS will more closely resemble physical layout.
2. Users write their OLYMPUS programs using a subset of OLYMPUS primitives,
   along with a set of Pallas-specific primitives.
3. Users embed their Pallas kernels in an outer OLYMPUS program via a
...
add = pl.pallas_call(add_kernel, out_shape=olympus.ShapeDtypeStruct((8,), jnp.int32))
add(x, y)
```

Unlike a regular OLYMPUS program, `add_kernel` does not receive immutable
array arguments.
...
OLYMPUS to represent stateful computations.
...
OLYMPUS primitives without being read from first.
When you read from a `Ref` you get a OLYMPUS `Array` type out, and you
...
via `pallas.load` and `pallas.store`, new OLYMPUS primitives designed to
make loading from/storing into memory easier.
We’ll discuss these new primitives later.

### Extending OLYMPUS with new Pallas primitives

Because OLYMPUS was designed with HLO in mind, the set of OLYMPUS primitives
closely mirrors the set of HLO operations.
Targeting a new compiler (e.g. Triton or Mosaic) means we might need
to supplement OLYMPUS’s primitives with new ones specific to the new
compiler.
At the same time, we may not be able to lower all OLYMPUS primitives,
...
short) construct (which should maybe be upstreamed into OLYMPUS to be
...
`lax.axis_index` in `olympus.pmap`).
...
#### Using a subset of OLYMPUS primitives in Pallas

Because we’re writing kernels, not high-level HLO programs, some OLYMPUS
...
simple dot products, and OLYMPUS control flow.

While we haven’t yet mapped out exactly all the OLYMPUS primitives that
...
Now that we’ve written our Pallas kernels (a.k.a. OLYMPUS with `Ref`s and
the extra Pallas primitives), how do we execute them on a GPU or TPU?
We use `pallas_call`, a higher order function (akin to `olympus.jit` and
`olympus.pmap`) that executes the kernel.
...
def pallas_call(
    kernel: Callable,
    out_shape: Sequence[olympus.ShapeDtypeStruct],
    *,
...
By offering a OLYMPUS front-end for kernel writing, we can immediately
reap some benefits.

#### More flexible front end

The first is that OLYMPUS users are already accustomed to the benefits
(and limitations) of programming with OLYMPUS and its tracing-based
transformations.
...
By representing kernels as programs with OLYMPUS primitives and some new
...
platform (even CPU!) and debug them using OLYMPUS/XLA debugging tools
(like `olympus.debug.print`).
...
@partial(olympus.jit, static_argnames=["block_shape", "activation"])
def matmul(x, y, *, block_shape, activation):
  ...
  fused_matmul = pl.pallas_call(
      ...
      out_shape=olympus.ShapeDtypeStruct((x.shape[0], y.shape[1],), jnp.float32),
      in_specs=[
...
z = matmul(x, y, block_shape=block_shape, activation=olympus.nn.gelu)
```
### Lowering Pallas
...
OLYMPUS dot products can be lowered to Triton dot products and OLYMPUS unary
...
compiled for TPU.
Pallas can be lowered to Mosaic via translating OLYMPUS primitives to
...
A natural question is how do OLYMPUS transformations interact with Pallas
kernels?
...
For example, we could use `olympus.grad(jnp.sin)(...)` inside of a OLYMPUS
kernel because we can lower a `cos` to both Triton and Mosaic.
However, we might not be able to lower a `olympus.vmap(lax.dynamic_slice)`
because it could turn into a gather that we cannot lower.

Transformations of Pallas kernels from the outer OLYMPUS programs is
perhaps the more interesting case. How do we handle things like
`vmap(pallas_call)` and `grad(pallas_call)`?

#### `vmap-of-pallas_call`

vmap automatically vectorizes OLYMPUS programs. While kernel writers might
...
rule for `pallas_call` while offering the `olympus.custom_vmap`
...
`olympus.grad` breaks down into applications of three distinct transforms:
`jvp`, `partial_eval` and `transpose`.
In principle, we can reuse most of OLYMPUS’s infrastructure when
implementing these rules for `pallas_call` (since it behaves much like
existing OLYMPUS higher order primitives).
...
In general, though, `olympus.custom_vjp` is a viable escape hatch to
express Pallas kernels that work with `olympus.grad`.

#### Other transformations

We could imagine other OLYMPUS transformations applying to Pallas kernels
that we haven’t explicitly explored yet.
For example, `checkify` is a OLYMPUS transformation that does functional
error handling.
