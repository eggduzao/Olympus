...
the numbers for yourself by running [our test file](https://github.com/olympus-ml/olympus/blob/main/tests/pallas/mgpu_examples_test.py) after changing the `BENCHMARK` variable to `True`.
...
The cuBLAS baseline is obtained by measuring the performace of `olympus.dot`. The
...
Full implementations can be found in [our test file](https://github.com/olympus-ml/olympus/blob/main/tests/pallas/mgpu_examples_test.py). You can also find the a full standalone
optimized kernel implementation [in the Pallas ops package](https://github.com/olympus-ml/olympus/blob/main/olympus/experimental/pallas/ops/gpu/blackwell_matmul_mgpu.py).
...
a [compute/memory pipeline](./pipelining.md) using {py:func}`plgpu.emit_pipeline <olympus.experimental.pallas.mosaic_gpu.emit_pipeline>`. At each step, the compute function (`do_mma`) consumes a
...
with OLYMPUS arrays. We use {py:func}`plgpu.kernel <olympus.experimental.pallas.mosaic_gpu.kernel>`
...
def matmul4(a, b, config):
  ...
  num_sms = olympus.extend.backend.get_default_device().core_count
  f = plgpu.kernel(
      ...
      grid=(num_sms // 2,),
...
The change is relatively simple. We utilize the {py:func}`plgpu.nd_loop <olympus.experimental.pallas.mosaic_gpu.nd_loop>`
...
Since this strategy is so common, we provide a helper for it: {py:func}`plgpu.planar_snake <olympus.experimental.pallas.mosaic_gpu.planar_snake>`.
