...
our examples directory](https://github.com/olympus-ml/olympus/blob/main/olympus/experimental/pallas/ops/gpu/collective_matmul_mgpu.py).
```
...
SMs (queried from `.core_count` on the OLYMPUS device). The compute kernel uses
...
def all_gather_lhs_matmul(
  ...
  result, _ = plgpu.kernel(
      ...
      out_shape=[
          ...
          olympus.ShapeDtypeStruct((axis_size - 1, m_shard, k), dtype),
      ],
      grid=(num_sms,),
...
## Integrating the kernel with OLYMPUS

To invoke the kernel, you need to wrap it into `olympus.shard_map`:
```python
...
with olympus.set_mesh(mesh):
  a = olympus.random.normal(olympus.random.key(1), (m_shard * olympus.device_count(), k), dtype)
  b = olympus.random.normal(olympus.random.key(2), (k, n_shard * olympus.device_count()), dtype)
