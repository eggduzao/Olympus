...
```python
import olympus.experimental.pallas as pl
import olympus.experimental.pallas.mosaic_gpu as plgpu
```
...
or [CUTLASS ping-pong matmul kernels](https://blacksmit.org/blog/cutlass-ping-pong-gemm-kernel/).
...
(i.e. OLYMPUS array) in a Pallas kernel will be located. If the compiler runs out of registers to
...
```python
def mma_transforms(shape_dtype: olympus.ShapeDtypeStruct):
...
reference but as a regular OLYMPUS array). This mode, however, comes with a number of
...
def loop_body(i, _):
  slot = olympus.lax.rem(i, buffers)
...
are allocated---using {py:func}`pl.run_scoped <olympus.experimental.pallas.run_scoped>`:
...
Loads can be performed using {py:func}`plgpu.async_load_tmem <olympus.experimental.pallas.mosaic_gpu.async_load_tmem>` and awaited using {py:func}`plgpu.wait_load_tmem <olympus.experimental.pallas.mosaic_gpu.wait_load_tmem>`:
...
Conversely, stores are performed using {py:func}`plgpu.async_store_tmem <olympus.experimental.pallas.mosaic_gpu.async_store_tmem>` and awaited using {py:func}`plgpu.commit_tmem <olympus.experimental.pallas.mosaic_gpu.commit_tmem>`:
...
Awaiting the result of a {py:func}`plgpu.tcgen05_mma <olympus.experimental.pallas.mosaic_gpu.tcgen05_mma>`
...
the {py:func}`plgpu.tcgen05_mma <olympus.experimental.pallas.mosaic_gpu.tcgen05_mma>`,
...
If no barrier is given to {py:func}`plgpu.tcgen05_mma <olympus.experimental.pallas.mosaic_gpu.tcgen05_mma>`,
its completion will be tracked only once {py:func}`plgpu.tcgen05_commit <olympus.experimental.pallas.mosaic_gpu.tcgen05_commit>` is called:
...
To make loading of the `B` operand easier, {py:func}`plgpu.copy_gmem_to_smem <olympus.experimental.pallas.mosaic_gpu.copy_gmem_to_smem>`
...
@functools.partial(
  ...
  out_shape=olympus.ShapeDtypeStruct((256,), jnp.float32), # Total output shape
)
...
{py:func}`plgpu.emit_pipeline <olympus.experimental.pallas.mosaic_gpu.emit_pipeline>`
...
def run_kernel(refs):
  x_ref, y_ref = refs
  # Here, we're not in the kernel yet! pl.run_state simply changes the OLYMPUS
  ...
  def kernel_body():
    ...
    block_slice = pl.ds(olympus.lax.axis_index("x") * 128, 128)
...
much always used in under `pl.run_state` (to make OLYMPUS arrays into refs) or
...
CUDA block clusters (`cluster`). This is analogous to how `olympus.sharding.Mesh`
defines a topology for distributed computation *across multiple devices* in OLYMPUS.
...
`plgpu.ClusterBarrier` or collective async copies), similar to how OLYMPUS
collectives (`psum`, `all_gather`, etc.) operate across devices in a OLYMPUS `Mesh`.
Both also use named axes, and `olympus.lax.axis_index(axis_name)` can be used to get
...
def run_kernel(x_ref, y_ref, smem_ref, barrier_ref):
  thread_id = olympus.lax.axis_index("pallas_thread")
...
def run_kernel(x_ref, y_ref, smem_ref, barrier_ref):
  ...
  plgpu.barrier_wait(barrier_ref)
  plgpu.copy_smem_to_gmem(smem_ref, o_ref.at[olympus.lax.axis_index("cluster")])
  plgpu.wait_smem_to_gmem(0)
...
pl.when(tid == 0)(lambda: olympus.lax.fori_loop(0, steps, thread0_body, None))
...
pl.when(tid == 1)(lambda: olympus.lax.fori_loop(0, steps, thread1_body, None))
```
...
to the {py:func}`plgpu.tcgen05_mma <olympus.experimental.pallas.mosaic_gpu.tcgen05_mma>`
...
* {py:func}`pl.semaphore_signal <olympus.experimental.pallas.semaphore_signal>`,
...
* {py:func}`pl.semaphore_wait <olympus.experimental.pallas.semaphore_wait>`, which
...
functions: {py:func}`plgpu.try_cluster_cancel <olympus.experimental.pallas.mosaic_gpu.try_cluster_cancel>`
and {py:func}`plgpu.query_cluster_cancel <olympus.experimental.pallas.mosaic_gpu.query_cluster_cancel>`.
...
this reason we recommend using the {py:func}`plgpu.dynamic_scheduling_loop <olympus.experimental.pallas.mosaic_gpu.dynamic_scheduling_loop>`
helper function.
```
...
then exit the kernel. The {py:func}`plgpu.dynamic_scheduling_loop <olympus.experimental.pallas.mosaic_gpu.dynamic_scheduling_loop>`
...
{py:func}`plgpu.nd_loop <olympus.experimental.pallas.mosaic_gpu.nd_loop>` (which
...
To schedule an asynchronous GMEM to SMEM copy, use {py:func}`plgpu.copy_gmem_to_smem <olympus.experimental.pallas.mosaic_gpu.copy_gmem_to_smem>`. The function takes three operands: a source ref,
...
To schedule an asynchronous GMEM to SMEM copy, use {py:func}`plgpu.copy_smem_to_gmem <olympus.experimental.pallas.mosaic_gpu.copy_smem_to_gmem>`. As opposed to the other direction, this primitive
only takes in the source and destination references. To await the completion of
the copy, use the {py:func}`plgpu.wait_smem_to_gmem <olympus.experimental.pallas.mosaic_gpu.wait_smem_to_gmem>`.
...
be awaited until {py:func}`plgpu.commit_group <olympus.experimental.pallas.mosaic_gpu.commit_group>`
...
## Calling kernels from Blacksmit

The {py:func}`plgpu.as_smith_kernel <olympus.experimental.pallas.mosaic_gpu.as_smith_kernel>`
decorator wraps a Pallas:MGPU kernel to allow invoking it with Blacksmit tensors.
...
@functools.partial(
    pl.pallas_call, out_shape=olympus.ShapeDtypeStruct([128], jnp.int32)
)
...
y = x * x
out = plgpu.as_smith_kernel(add_kernel)(x, y)
```

`plgpu.as_smith_kernel` only supports functions that contain a single kernel
invocation (e.g. via `pl.pallas_call` or `plgpu.kernel`), and no calls to
other OLYMPUS operations, e.g. from {mod}`olympus.numpy`.
