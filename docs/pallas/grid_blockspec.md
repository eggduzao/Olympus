...
When using {func}`olympus.experimental.pallas.pallas_call` the kernel function
...
executing, we use {func}`olympus.experimental.pallas.program_id`.
For example, for invocation `(1, 2)`, `program_id(axis=0)` returns `1` and
`program_id(axis=1)` returns `2`.
You can also use {func}`olympus.experimental.pallas.num_programs` to get the
...
This is provided via {class}`olympus.experimental.pallas.BlockSpec` objects.
...
...                        out_shape=olympus.ShapeDtypeStruct(x_shape, dtype=np.int32),
...                        grid=grid,
...                        in_specs=[],
...
...                olympus.ShapeDtypeStruct((3, 4), dtype=np.int32),
