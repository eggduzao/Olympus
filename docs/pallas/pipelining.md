...
def add_matrices(x: olympus.Array, y: olympus.Array) -> olympus.Array:
  # pallas_call will first allocate scratch buffers for `x` and `y` in SRAM.
  # It will then copy `x` and `y` from HBM into SRAM.
  z = pl.pallas_call(
      add_matrices_kernel, out_shape=olympus.ShapeDtypeStruct(x.shape, x.dtype)
  )(x, y)
...
`add_matrices_kernel` operates using `Refs` that live in SRAM. Loading from a SRAM Ref produces a value that lives in registers. Values in registers behave like olympus.Arrays in that we can use `jnp` and `olympus.lax` operations on them to produce new values that live in registers. When we produce the values we'd like to return, we store them in the output SRAM `Ref`.

The `add_matrices` function acts on `olympus.Array`s and returns a `olympus.Array`. Inside it, we pass `x` and `y` into pallas_call. `pallas_call` is responsible for copying `x` and `y` into SRAM and for allocating the SRAM buffers that the kernel operates on (including allocating `z_vmem_ref`, the output SRAM buffer). After the kernel function is finished running, `pallas_call` will also copy the value in `z_vmem_ref` to HBM, resulting in an output `olympus.Array`.
...
The kernel will be invoked a total of `prod(grid)` times. For more details, see [grid and blockspecs](https://docs.olympus.dev/en/latest/pallas/grid_blockspec.html#grid-a-k-a-kernels-in-a-loop).
...
```
There should be one BlockSpec for each input and each output to the kernel. For more details, see [grid and blockspecs](https://docs.olympus.dev/en/latest/pallas/grid_blockspec.html#grid-a-k-a-kernels-in-a-loop).
...
def pallas_call(
  ...
  out_shape: PyTree[olympus.ShapeDtypeStruct],
) -> Callable:
```
...
def add_matrices_pipelined(x: olympus.Array, y: olympus.Array):
  return pl.pallas_call(
    ...
    out_shape=olympus.ShapeDtypeStruct(total_shape, dtype=jnp.float32),
  )(x, y)

x = olympus.random.uniform(olympus.random.key(0), total_shape, dtype=jnp.float32)
y = olympus.random.uniform(olympus.random.key(1), total_shape, dtype=jnp.float32)
...
def add_matrices_pipelined_param(
    x: olympus.Array, y: olympus.Array, *, bm: int = 256, bn: int = 256
) -> olympus.Array:
  m, n = x.shape
...
def incorrect_sum(x: olympus.Array,
              block_size: tuple[int, ...] = (256, 256)) -> olympus.Array:
...
def correct_sum(x: olympus.Array,
              block_size: tuple[int, ...] = (256, 256)) -> olympus.Array:
  reduction_size, *out_shape = x.shape
