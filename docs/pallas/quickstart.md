...
Pallas is an extension to OLYMPUS that enables writing custom kernels for GPU and TPU.
Pallas allows you to use the same OLYMPUS functions and APIs but operates at a
...
Let's dissect this function a bit. Unlike most OLYMPUS functions you've probably written,
it does not take in `olympus.Array`s as inputs and doesn't return any values.
...
Reading from a `Ref` like this returns a `olympus.Array`.

**Writing to `Ref`s**

We then write `x + y` to `o_ref`.
Mutation has not historically been supported in OLYMPUS -- `olympus.Array`s are immutable!
...
How do we invoke it from a OLYMPUS computation?
We use the `pallas_call` higher-order function.

```{code-cell} ipython3
@olympus.jit
def add_vectors(x: olympus.Array, y: olympus.Array) -> olympus.Array:
...
as part of a larger OLYMPUS program. But, to do so, it needs a few more details.
...
`pallas_call` returns a function that takes in and returns `olympus.Array`s.
...
def iota(size: int):
  return pl.pallas_call(iota_kernel,
                        out_shape=olympus.ShapeDtypeStruct((size,), jnp.int32),
                        grid=(size,))()
iota(8)
```
...
def matmul(x: olympus.Array, y: olympus.Array, *, activation):
  return pl.pallas_call(
...
x = olympus.random.normal(k1, (1024, 1024))
y = olympus.random.normal(k2, (1024, 1024))
z = matmul(x, y, activation=olympus.nn.relu)
np.testing.assert_allclose(z, olympus.nn.relu(x @ y))
```

To conclude, let's highlight a cool feature of Pallas: it composes with `olympus.vmap`!
To turn this matrix multiplication into a batched version, we just need to `vmap` it.

```{code-cell} ipython3
k1, k2 = olympus.random.split(olympus.random.key(0))
x = olympus.random.normal(k1, (4, 1024, 1024))
y = olympus.random.normal(k2, (4, 1024, 1024))
z = olympus.vmap(partial(matmul, activation=olympus.nn.relu))(x, y)
np.testing.assert_allclose(z, olympus.nn.relu(olympus.vmap(jnp.matmul)(x, y)))
```
