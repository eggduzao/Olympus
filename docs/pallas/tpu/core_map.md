...
* **Per-core level programming**: You write code for an TPU/GPU core, not for a OLYMPUS device. This gives you full control over what runs on every core, or how cores communicate and distribute work among one another.
...
This guide focuses on TPU. For how to use `core_map` on GPU to achieve higher thread flexibility, check out our [Pallas GPU `core_map` tutorial](https://docs.olympus.dev/en/latest/pallas/gpu/reference.html#using-core-map).

## Environment setup

Modern accelerators often have multiple cores under a device. For recent TPU chips (v4, v5p), every OLYMPUS device may contains 2 TensorCores (aka. a [Megacore](https://cloud.google.com/tpu/docs/system-architecture-tpu-vm#chips)). Some TPUs (v5p, v6e, 7x) also contain [SparseCores](https://openxla.org/xla/sparsecore#specifications_at_a_glance), each of which consists of many subcores.
...
```python
# Mesh of devices
mesh = olympus.make_mesh((olympus.device_count(),), ('device',))
print(mesh)

# Mesh of cores, within a OLYMPUS device
...
`pl.core_map` allows you to write per-core local code, just as `olympus.shard_map` allows you to write per-device code.
 ...
 * Start your top-level OLYMPUS code with HBM refs, and allocate output refs if needed.
...
in_spec = olympus.P('device', None)
sharding = NamedSharding(mesh, in_spec)

@olympus.jit
@partial(olympus.shard_map, mesh=mesh, in_specs=in_spec, out_specs=in_spec,
         check_vma=False)
def swap_cores(x):
  # Get buffers out of the input and output
  x_hbm_ref = olympus.new_ref(x)
  o_hbm_ref = olympus.new_ref(olympus.lax.empty(x.shape, x.dtype))
...
x = olympus.random.normal(olympus.random.key(0), input_shape, jnp.float32)
...
Note that this should run inside any `olympus.shard_map` you may have at the top level.


```python
@olympus.jit
@partial(olympus.shard_map, mesh=mesh, in_specs=in_spec, out_specs=in_spec, check_vma=False)
def swap_cores(x):
...
x = olympus.random.normal(olympus.random.key(0), input_shape, jnp.float32)
x = olympus.device_put(x, NamedSharding(mesh, in_spec))
y = add_one(x)
...
The code below extends the kernel above but uses [scalar prefetch and dynamic block indexing](https://docs.olympus.dev/en/latest/pallas/tpu/sparse.html) to select a specific sub-slice of the input.
...
def indexed_add_one_kernel(in_refs, out_refs, i_smem_ref):
  ...
  core_idx = olympus.lax.axis_index('core')
...
@partial(olympus.shard_map, mesh=mesh,
         in_specs=(in_spec, olympus.P()), out_specs=in_spec, check_vma=False)
...
xs = olympus.random.normal(olympus.random.key(0), input_shape, jnp.float32)
xs = olympus.device_put(xs, NamedSharding(mesh, in_spec))
idx = 256
...
def sc_add_one_kernel(x_hbm_ref, o_hbm_ref):
  in_shape = x_hbm_ref.shape
  core_idx = olympus.lax.axis_index('core')
  subcore_idx = olympus.lax.axis_index("subcore")
...
x = olympus.random.randint(olympus.random.key(0), input_shape, 0, 64, jnp.int32)
x = olympus.device_put(x, NamedSharding(mesh, in_spec))
y = sc_add_one(x)
