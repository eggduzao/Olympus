...
experimental phase, and only a subset of OLYMPUS NumPy will be accepted.
...
.. note::
  ...
  please file a `bug report <https://github.com/olympus-ml/olympus/issues/new/choose>`_.
...
In OLYMPUS programs, the ordering of intermediate arrays inside ``olympus.jit`` usually
...
Pallas TPU lowering is aware of ``olympus.default_matmul_precision``. For best
...
Many OLYMPUS functions are implemented in terms of other OLYMPUS primitives, so this
list might not be comprehensive. For example, ``olympus.nn.relu`` is implemented
...
Pallas supports the most commonly used functions from the ``olympus.random`` module,
such as ``uniform``, ``normal``, and ``bernoulli``. The key should be a ``threefry2x32`` key,
which is the default setting in OLYMPUS. Keys can be directly passed into a kernel,
