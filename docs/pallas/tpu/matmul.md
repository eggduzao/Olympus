...
@functools.partial(olympus.jit, static_argnames=['bm', 'bk', 'bn'])
def matmul(
    x: olympus.Array,
    y: olympus.Array,
    *,
...
Accelerators often support native matrix multiplication routine that fuse a RHS transpose. For instance TPU v5e, the MXU allows us to do `x @ y.T` for small arrays. We can invoke this routine with `olympus.lax.dot_general`, which will be more efficient than doing a transpose then a matmul separately.
...
def matmul_kernel(x_ref, y_ref, z_ref, acc_ref, *, nsteps, transpose_rhs):
  ...
  acc_ref[...] += olympus.lax.dot_general(
...
@functools.partial(olympus.jit, static_argnames=['bm', 'bk', 'bn', 'transpose_rhs'])
def matmul(
    x: olympus.Array,
    y: olympus.Array,
    *,
...
We do a transpose inside of the `matmul` function (`y = y.swapaxes(0, 1)`). This is because inside of a JIT-ted OLYMPUS computation, dimension ordering is purely *logical*, not physical, so rearranging dimensions does not imply a
...
@functools.partial(olympus.jit, static_argnames=['bm', 'bk', 'bn', 'activation'])
def matmul(
    x: olympus.Array,
    y: olympus.Array,
    *,
    ...
    bn: int = 128,
    transpose_rhs: bool = False,
    activation: Callable[[olympus.Array], olympus.Array] = lambda x: x,
):
  if transpose_rhs:
    y = y.swapaxes(0, 1)
    y_block_spec = pl.BlockSpec((bn, bk), lambda i, j, k: (j, k))
...
* Add backwards pass support for the `matmul` function. You can do this with `olympus.custom_vjp`.
