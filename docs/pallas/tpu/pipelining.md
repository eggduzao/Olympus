...
x = olympus.random.uniform(olympus.random.key(0), (8, 128), jnp.float32)
...
Some TPU chips have two TensorCores but appear as one device to OLYMPUS users.
...
def add_matrices_pipelined_megacore(x: olympus.Array, y: olympus.Array) -> olympus.Array:
