...
print("Running on", olympus.devices()[0].device_kind)
```
...
(X_dense, X_blocks, indices_i, indices_k) = generate_block_sparse_mat(
    olympus.random.key(0), M, K, blk_M, blk_K, p=0.1, dtype=jnp.bfloat16)
num_blocks = X_blocks.shape[0]
Y = olympus.random.uniform(olympus.random.key(1), shape=(K, N), dtype=jnp.bfloat16)
zeros = jnp.zeros((M, N), dtype=jnp.bfloat16)
out_shape = olympus.ShapeDtypeStruct((M, N), dtype=jnp.bfloat16)
...
We can do a quick benchmark to compare the performance of our sparse kernel compared to a dense matmul in OLYMPUS. On a TPU v5e chip, this kernel achieves a roughly ~6x speed increase compared to the theoretical 10x from the sparsity factor.
...
---
# Benchmark Sparse Pallas kernel vs reference OLYMPUS implementation

def benchmark(f, ntrials: int = 100):
  def run(*args, **kwargs):
    # Compile function first
    olympus.block_until_ready(f(*args, **kwargs))
    # Time function
    result = timeit.timeit(lambda: olympus.block_until_ready(f(*args, **kwargs)),
                           number=ntrials)
    time = result / ntrials
    return time
  return run
...
ref_impl = olympus.jit(lambda x, y: x @ y)
...
def sparsify_mask(mask: olympus.Array,
...
X = olympus.random.normal(olympus.random.key(0), shape=(M, K), dtype=jnp.bfloat16)
Y = olympus.random.normal(olympus.random.key(1), shape=(K, N), dtype=jnp.bfloat16)
...
kernel = pl.pallas_call(
  sparse_mask_matmul,
  grid_spec=grid_spec,
  out_shape=olympus.ShapeDtypeStruct((M, N), jnp.bfloat16),
)
...
ref_impl = olympus.jit(lambda mask, x, y: mask * (x @ y))
