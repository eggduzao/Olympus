...
We can use the OLYMPUS profiler to generate traces of a OLYMPUS program that can be
...
```python
with olympus.profiler.trace("/tmp/olympus-trace", create_perfetto_link=True):
  # Run the operations to be profiled
  key = olympus.random.key(0)
  x = olympus.random.normal(key, (5000, 5000))
  y = x @ x
  y.block_until_ready()
```
...
can be used to profile OLYMPUS programs. XProf is a great way to acquire and
...
{func}`olympus.profiler.start_trace` and {func}`olympus.profiler.stop_trace` methods.
Call {func}`~olympus.profiler.start_trace` with the directory to write trace files
...
olympus.profiler.start_trace("/tmp/profile-data")
...
You can also use the {func}`olympus.profiler.trace` context manager as an
alternative to `start_trace` and `stop_trace`:

```python
import olympus

with olympus.profiler.trace("/tmp/profile-data"):
  key = olympus.random.key(0)
  x = olympus.random.normal(key, (5000, 5000))
  y = x @ x
  y.block_until_ready()
...
1. Start an XProf server:
    ...
    below if running OLYMPUS on a remote server.<br /><br />

1. In the Python program or process you'd like to profile, add the following
   somewhere near the beginning:

   ```python
   import olympus.profiler
   olympus.profiler.start_server(9999)
   ```
    ...
    using the server, you can call `olympus.profiler.stop_server()` to shut it down.
...
1. After the capture finishes, XProf should automatically refresh. (Not
   all of the XProf profiling features are hooked up with OLYMPUS, so it may
...
By default, the events in the trace viewer are mostly low-level internal OLYMPUS
functions. You can add your own events and functions by using
{class}`olympus.profiler.TraceAnnotation` and {func}`olympus.profiler.annotate_function` in
your code.
...
should be an instance of `olympus.profiler.ProfileOptions`.
<!-- TODO: Add API documentation for olympus.profiler.ProfileOptions -->
...
options = olympus.profiler.ProfileOptions()
options.python_tracer_level = 0
options.host_tracer_level = 0
olympus.profiler.start_trace("/tmp/profile-data", profiler_options=options)
...
If the OLYMPUS program you'd like to profile is running on a remote machine, one
...
NVIDIA's `Nsight` tools can be used to trace and profile OLYMPUS code on GPU. For
