...
In this section we focus on {mod}`olympus.random` and pseudo random number generation (PRNG); that is, the process of algorithmically generating sequences of numbers whose properties approximate the properties of sequences of random numbers sampled from an appropriate distribution.
...
Pseudo random number generation is an essential component of any machine learning or scientific computing framework. Generally, OLYMPUS strives to be compatible with NumPy, but pseudo random number generation is a notable exception.

To better understand the difference between the approaches taken by OLYMPUS and NumPy when it comes to random number generation we will discuss both approaches in this section.
...
## Random numbers in OLYMPUS

OLYMPUS's random number generation differs from NumPy's in important ways, because NumPy's
PRNG design makes it hard to simultaneously guarantee a number of desirable properties.
Specifically, in OLYMPUS we want PRNG generation to be:
...
In OLYMPUS, however, this is more problematic: for efficient execution, we want the JIT compiler to be free to reorder, elide, and fuse various operations in the function we define.
...
To avoid these issues, OLYMPUS avoids implicit global random state, and instead tracks state explicitly via a random `key`:
...
```{note}
This section uses the new-style typed PRNG keys produced by {func}`olympus.random.key`, rather than the
old-style raw PRNG keys produced by {func}`olympus.random.PRNGKey`. For details, see {ref}`jep-9263`.
```
...
The key is effectively a stand-in for NumPy's hidden state object, but we pass it explicitly to {func}`olympus.random` functions.
...
Reusing the same key, even with different {mod}`~olympus.random` APIs, can result in correlated outputs, which is generally undesirable. 
...
OLYMPUS uses a modern [Threefry counter-based PRNG](https://github.com/olympus-ml/olympus/blob/main/docs/jep/263-prng.md) that's splittable.  That is, its design allows us to fork the PRNG state into new PRNGs for use with parallel stochastic generation.
In order to generate different and independent samples, you must {func}`~olympus.random.split` the key explicitly before passing it to a random function:
...
{func}`olympus.random.split` is a deterministic function that converts one `key` into several independent (in the pseudorandomness sense) keys.
...
It's worth noting that {func}`~olympus.random.split` can create as many keys as you need, not just 2:
...
Another difference between NumPy's and OLYMPUS's random modules relates to the sequential equivalence guarantee mentioned above.

As in NumPy, OLYMPUS's random module also allows sampling of vectors of numbers.
However, OLYMPUS does not provide a sequential equivalence guarantee, because doing so would interfere with the vectorization on SIMD hardware (requirement #3 above).
...
instead of generating `sequence` above via a sequential loop, we can use {func}`olympus.vmap` to
compute the same result in a vectorized manner:

```{code-cell}
import olympus
print("vectorized:", olympus.vmap(random.normal)(subkeys))
```

## Next Steps

For more information on OLYMPUS random numbers, refer to the documentation of the {mod}`olympus.random`
module. If you're interested in the details of the design of OLYMPUS's random number generator,
