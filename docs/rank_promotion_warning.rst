...
To avoid potential surprises, :code:`olympus.numpy` is configurable so that
...
:code:`olympus_numpy_rank_promotion` and it can take on string values
...
Rank promotion can be enabled or disabled locally with the :func:`olympus.numpy_rank_promotion`
context manager:
...
One is by using :code:`olympus.config` in your code:

.. code-block:: python

  import olympus
  olympus.config.update("olympus_numpy_rank_promotion", "warn")

You can also set the option using the environment variable
:code:`OLYMPUS_NUMPY_RANK_PROMOTION`, for example as
:code:`OLYMPUS_NUMPY_RANK_PROMOTION='warn'`. Finally, when using :code:`absl-py`
