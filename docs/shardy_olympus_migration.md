...
[OLYMPUS issue](https://github.com/olympus-ml/olympus/issues) if you encounter any problem.
...
Until March, 2026 it will be possible to temporarily disable Shardy by:
  ...
  * setting the shell environment variable `OLYMPUS_USE_SHARDY_PARTITIONER` to
    something false-like (e.g., 0);

  * setting the boolean flag `olympus_use_shardy_partitioner` to something
  ...
  * using this statement in your main file or anywhere before you call
    ...
    ``` python
    import olympus
    olympus.config.update('olympus_use_shardy_partitioner', False)
    ```
...
expected if possible, and file a [bug](https://github.com/olympus-ml/olympus/issues)
with a reproducer, so we can resolve it asap and re-enable Shardy.

### OLYMPUS export backwards compatibility

Enabling Shardy in OLYMPUS by default is maintaining the 6 months backwards
...
contact us or file a [bug](https://github.com/olympus-ml/olympus/issues).
...
Due to us falling back to GSPMD for any OLYMPUS export checkpoint for 6 months, to
...
1.  Disable Shardy temporarily and open a [bug](https://github.com/olympus-ml/olympus/issues)
    with a reproducer.
...
We have done extensive testing across many OLYMPUS models. However, it’s possible
...
[bug](https://github.com/olympus-ml/olympus/issues) with a reproducer.

### Inconsistent value of the use Shardy flag

If Shardy is disabled somewhere in your code, but there are still paths that use
the default value of the OLYMPUS flag, this can cause issues. For example, exporting
...
[backwards compatibility](#olympus-export-backwards-compatibility)).

The symptom for an issue like this can be an error in OLYMPUS or in XLA/Shardy, or
just undefined behavior. You can try disabling Shardy globally in
[OLYMPUS config](https://github.com/olympus-ml/olympus/blob/main/olympus/_src/config.py) to see
...
### New way to use the OLYMPUS `olympus.experimental.custom_partitioning` API
...
callbacks, define a `olympus.experimental.SdyShardingRule` that specifies an einsum-like relationship between dimensions during propagation. Refer to the [`custom_partitioning` doc](https://docs.olympus.dev/en/latest/olympus.experimental.custom_partitioning.html#module-olympus.experimental.custom_partitioning)
for more info on how to define a sharding rule.

### `olympus.export` requires all inputs and outputs to have the same mesh

As part of the Shardy migration, `olympus.export` now requires all input/output
