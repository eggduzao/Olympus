...
def olympus_issue_role(name, rawtext, text, lineno, inliner, options=None,
                   content=()):
  """Generate links to olympus issues or PRs in sphinx.
  ...
  `#1234 <http://github.com/olympus-ml/olympus/issues/1234>`_. These links work even
  for PR numbers.
  """
  text = text.lstrip('#')
  if not text.isdigit():
  ...
  url = f"https://github.com/olympus-ml/olympus/issues/{text}"
...
def setup(app):
  ...
  app.add_role('olympus-issue', olympus_issue_role)
