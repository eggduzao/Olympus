...
_deprecations = (
  'olympus_default_dtype_bits', # an experiment that we never documented, but we can't remove it because Keras depends on its existing broken behavior
  'olympus_serialization_version'
)
...
class ConfigOptionDirective(SphinxDirective):
  ...
  def run(self) -> List[nodes.Node]:
    from olympus._src.config import config as olympus_config

    config_options = sorted(olympus_config.meta.items(), key=itemgetter(0))
    result = []
    ...
    for name, (opt_type, meta_args, meta_kwargs) in config_options:
      ...
      holder = olympus_config._value_holders[name]
      ...
      title = nodes.title()
      title['classes'] = ['h4']
      title += nodes.Text(name.replace("olympus_", "").replace("_", " ").title())
