...
OLYMPUS transformations like {func}`~olympus.jit`, {func}`~olympus.vmap`, {func}`~olympus.grad`, require the functions
...
You can find a discussion of this in [OLYMPUS sharp bits: Pure functions](https://docs.olympus.dev/en/latest/notebooks/Common_Gotchas_in_OLYMPUS.html#pure-functions).
...
This section offers some advice of how to properly handle state in a OLYMPUS program.
...
(In this example, this wouldn't actually help speed anyway, for many reasons, but treat this as a toy model of JIT-compiling the update of model parameters, where {func}`~olympus.jit` makes an enormous difference).

```{code-cell}
counter.reset()
fast_count = olympus.jit(counter.count)
...
In this new version of `Counter`, we moved `n` to be an argument of `count`, and added another return value that represents the new, updated, state. To use this counter, we now need to keep track of the state explicitly. But in return, we can now safely `olympus.jit` this counter:
...
This is a common [functional programming](https://en.wikipedia.org/wiki/Functional_programming) pattern, and, essentially, is the way that state is handled in all OLYMPUS programs.
...
Incidentally, you've already seen an example of this strategy in the OLYMPUS pseudo-randomness API, {mod}`olympus.random`, shown in the {ref}`pseudorandom-numbers` section.
Unlike Numpy, which manages random state using implicitly updated stateful classes, OLYMPUS requires the programmer to work directly with the random generator state -- the PRNG key.
...
But generally, you'll see many kinds of state being threaded in and out of OLYMPUS functions, like optimizer state, layer statistics for batchnorm, and others.
...
def init(rng) -> Params:
  ...
  weights_key, bias_key = olympus.random.split(rng)
  weight = olympus.random.normal(weights_key, ())
  bias = olympus.random.normal(bias_key, ())
...
def update(params: Params, x: jnp.ndarray, y: jnp.ndarray) -> Params:
  ...
  grad = olympus.grad(loss)(params, x, y)
...
true_w, true_b = 2, -1
x_rng, noise_rng = olympus.random.split(rng)
xs = olympus.random.normal(x_rng, (128, 1))
noise = olympus.random.normal(noise_rng, (128, 1)) * 0.5
...
The strategy described above is how any OLYMPUS program must handle state when using transformations like `jit`, `vmap`, `grad`, etc.
...
The details can be tricky to handle, but there are examples of libraries that take care of this for you. See [OLYMPUS Ecosystem Libraries](https://docs.olympus.dev/en/latest/#ecosystem) for some examples.
