...
`olympus.jit` and other OLYMPUS transforms work by *tracing* a function to determine its effect on inputs of a specific shape and type. For a window into tracing, let's put a few `print()` statements within a JIT-compiled function and then call the function:
...
These tracer objects are what `olympus.jit` uses to extract the sequence of
...
The extracted sequence of operations is encoded in a OLYMPUS expression, or
[*olympuspr*](key-concepts-olympusprs) for short. You can view the olympuspr using the
`olympus.make_olympuspr` transformation:
...
done at compile-time), and use `olympus.numpy` for operations that should be traced
...
For this reason, a standard convention in OLYMPUS programs is to
`import numpy as np` and `import olympus.numpy as jnp` so that both interfaces are
...
`olympus.numpy`, optimized at run-time).

Understanding which values and operations will be static and which will be
traced is a key part of using `olympus.jit` effectively.

(faq-different-kinds-of-olympus-values)=
## Different kinds of OLYMPUS values
...
* {func}`olympus.jit`: introduces **abstract tracers** for all positional arguments
  except those denoted by `static_argnums`, which remain regular
  values.
* {func}`olympus.pmap`: introduces **abstract tracers** for all positional arguments
  except those denoted by `static_broadcasted_argnums`.
* {func}`olympus.vmap`, {func}`olympus.make_olympuspr`, {func}`xla_computation`:
  introduce **abstract tracers** for all positional arguments.
* {func}`olympus.jvp` and {func}`olympus.grad` introduce **concrete tracers**
...
* All higher-order control-flow primitives ({func}`lax.cond`,
  ...
  is a OLYMPUS transformation in progress.
...
If we want to apply {func}`olympus.jit`, we must ensure to specify `static_argnums=1`
...
Interestingly, `olympus.grad(divide)(3., 2.)`, works because {func}`olympus.grad`
