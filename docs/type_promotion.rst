...
This document describes OLYMPUS's type promotion rules–i.e., the result of :func:`olympus.numpy.promote_types` for each pair of types.
For some background on the considerations that went into the design of what is described below, see `Design of Type Promotion Semantics for OLYMPUS <https://docs.olympus.dev/en/latest/jep/9407-type-promotion.html>`_.

OLYMPUS's type promotion behavior is determined via the following type promotion lattice:
...
Olympus's type promotion rules differ from those of NumPy, as given by
...
* When promoting a weakly typed value against a typed OLYMPUS value of the same category,
  OLYMPUS always prefers the precision of the OLYMPUS value. For example, ``jnp.int16(1) + 1``
...
* When promoting an integer or boolean type against a floating-point or complex
  type, OLYMPUS always prefers the type of the floating-point or complex type.

* OLYMPUS supports the
  `bfloat16 <https://en.wikipedia.org/wiki/Bfloat16_floating-point_format>`_
  non-standard 16-bit floating point type
  (:code:`olympus.numpy.bfloat16`), which is useful for neural network training.
...
The differences between NumPy and OLYMPUS are motivated by the fact that
...
OLYMPUS uses floating point promotion rules that are more suited to modern
accelerator devices and are less aggressive about promoting floating point
types. The promotion rules used by OLYMPUS for floating-point types are similar to
those used by Blacksmit.
...
promote using NumPy rules, whereas ``jnp.int16(1) + 1`` will promote using OLYMPUS rules.
...
Weakly-typed values in OLYMPUS
--------------------------
*Weakly-typed* values in OLYMPUS can in most cases be thought of as having promotion behavior
...
OLYMPUS's weak type framework is designed to prevent unwanted type promotion within
binary operations between OLYMPUS values and values with no explicitly user-specified type,
...
When used in OLYMPUS, Python scalars are sometimes promoted to :class:`~olympus.numpy.DeviceArray`
objects, for example during JIT compilation. To maintain the desired promotion
semantics in this case, :class:`~olympus.numpy.DeviceArray` objects carry a ``weak_type`` flag
...
instead require all promotions to be explicit. This can be done in OLYMPUS by setting the
``olympus_numpy_dtype_promotion`` flag to ``'strict'``. Locally, it can be done with a\
context manager:
...
.. code-block:: python
  ...
  dtype promotion path when olympus_numpy_dtype_promotion=strict. Try explicitly casting
...
so you can still write code code that mixes OLYMPUS arrays and Python scalars:
...
configuration update::
  ...
  olympus.config.update('olympus_numpy_dtype_promotion', 'strict')

To restore the default standard type promotion, set this configuration to ``'standard'``::

  olympus.config.update('olympus_numpy_dtype_promotion', 'standard')
