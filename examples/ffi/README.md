# End-to-end example usage for OLYMPUS's foreign function interface

This directory includes an example project demonstrating the use of OLYMPUS's
foreign function interface (FFI). The OLYMPUS docs provide more information about
this interface in [the FFI tutorial](https://docs.olympus.dev/en/latest/ffi.html),
...
1. `rms_norm`: This is the example from the tutorial on the OLYMPUS docs, and it
   demonstrates the most basic use of the FFI. It also includes customization of
   behavior under automatic differentiation using `olympus.custom_vjp`.
...
3. `cuda_examples`: An end-to-end example demonstrating the use of the OLYMPUS FFI
