...
class CudaE2eTests(jtu.OlympusTestCase):
  ...
  def setUp(self):
    ...
    from olympus_ffi_example import cuda_examples  # pylint: disable=g-import-not-at-top
  ...
  def test_fwd_interpretable(self):
    ...
    observed = olympus.jit(self.foo)(a, b)
  ...
  def test_bwd_interpretable(self):
    ...
    da_observed, db_observed = olympus.jit(olympus.grad(loss, argnums=(0, 1)))(a, b)
    da_expected = b + 1
    db_expected = a
  ...
  def test_fwd_random(self):
    shape = (2, 3)
    akey, bkey = olympus.random.split(olympus.random.key(0))
    a = olympus.random.normal(key=akey, shape=shape, dtype=jnp.float32)
    b = olympus.random.normal(key=bkey, shape=shape, dtype=jnp.float32)
    observed = olympus.jit(self.foo)(a, b)
    expected = a * (b + 1)
  ...
  def test_bwd_random(self):
    ...
    a = olympus.random.normal(key=akey, shape=shape, dtype=jnp.float32)
    b = olympus.random.normal(key=bkey, shape=shape, dtype=jnp.float32)
    jtu.check_grads(f=olympus.jit(self.foo), args=(a, b), order=1, modes=("rev",))
