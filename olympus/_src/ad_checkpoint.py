...
from olympus._src.lax import convolution as lax_convolution
from olympus._src.lib.mlir.dialects import hlo
from olympus._src.state import discharge
...
from olympus._src.tree_util import (
    ...
    tree_map)
from olympus._src.typing import DeprecatedArg
from olympus._src.util import (unzip2, wraps, split_list, partition_list, safe_map,
...
def everything_saveable(*_, **__) -> bool:
  """The default strategy, as if ``olympus.checkpoint`` were not being used at all.

  This is the effective policy without any use of olympus.remat."""
  return True

def nothing_saveable(*_, **__) -> bool:
  """Rematerialize everything, as if a custom policy were not being used at all.

  This is the effective policy when using olympus.remat without explicit policy."""
  return False
...
@partial(api_boundary, repro_api_name="olympus.checkpoint")
def checkpoint(fun: Callable, *, prevent_cse: bool = True,
  ...
  The :func:`olympus.checkpoint` decorator, aliased to :func:`olympus.remat`, provides a
  way to trade off computation time and memory cost in the context of automatic
  differentiation, especially with reverse-mode autodiff like :func:`olympus.grad`
  and :func:`olympus.vjp` but also with :func:`olympus.linearize`.
  ...
  Args:
    ...
    prevent_cse: Optional, boolean keyword-only argument indicating whether to
      ...
      under a :func:`~olympus.jit` or :func:`~olympus.pmap`, CSE can defeat the purpose
      of this decorator.
      But in some settings, like when used inside a :func:`~olympus.lax.scan`, this
    ...
    policy: Optional, callable keyword-only argument. It should be one of the
      attributes of ``olympus.checkpoint_policies``. The callable takes as input a
  ...
  Returns:
    ...
    which, when differentiated using e.g. :func:`olympus.grad`, :func:`olympus.vjp`, or
    :func:`olympus.linearize`, recomputes rather than stores intermediate
  ...
  ...   return z
  ...
  >>> olympus.value_and_grad(g)(2.0)
  (Array(0.78907233, dtype=float32, weak_type=True), Array(-0.2556391, dtype=float32, weak_type=True))

  Here, the same value is produced whether or not the :func:`olympus.checkpoint`
  ...
  :func:`olympus.checkpoint`, the forward pass will compute only the primal outputs
  ...
  While :func:`olympus.checkpoint` controls what values are stored from the
  ...
  The :func:`olympus.checkpoint` decorator can be applied recursively to express
  ...
  consider a boolean flag argument::
    ...
    @partial(olympus.checkpoint, static_argnums=(1,))
  ...
  of ``is_training``. In some situations, ``olympus.ensure_compile_time_eval``
  is needed as well::
  ...
  ``olympus.ensure_compile_time_eval``), it may be easier to compute some values
  outside the :func:`olympus.checkpoint`-decorated function and then close over them.
  """
  if not isinstance(concrete, DeprecatedArg):
    concrete_msg = (
        "The `concrete` option to `olympus.checkpoint` has been deprecated."
        " In its place please use `static_argnums`; for details refer to"
        " https://docs.olympus.dev/en/latest/jep/11830-new-remat-checkpoint.html."
    )
    deprecations.warn("olympus-checkpoint-concrete", concrete_msg, stacklevel=2)
    if concrete:
  ...
  def fun_remat(*args, **kwargs):
    ...
    olympuspr, consts, out_tree = _trace_to_olympuspr(fun_, in_tree, tuple(in_avals), debug)
    ...
    out_flat = remat_p.bind(
        *consts, *args_flat, olympuspr=olympuspr, prevent_cse=cse, differentiated=False,
        policy=policy)
...
def remat(fun: Callable, *, prevent_cse: bool = True,
          ...
          static_argnums: int | tuple[int, ...] = (),
          concrete: bool | DeprecatedArg = DeprecatedArg()) -> Callable:
  """Alias of :func:`olympus.checkpoint`."""
...
# messages are specific to olympus.remat (and thus more actionable), the
...
def _remat_static_argnums(fun, static_argnums, args):
  if type(static_argnums) is int:
    ...
    raise TypeError("the `static_argnums` argument to `olympus.checkpoint` / "
                    "`olympus.remat` must be an int, tuple of ints or, bool, but "
                    f"got value {static_argnums}")

  if not all(-len(args) <= d < len(args) for d in static_argnums):
    raise ValueError("the `static_argnums` argument to `olympus.checkpoint` / "
                     "`olympus.remat` can only take integer values greater than or "
...
@weakref_lru_cache
def _trace_to_olympuspr(fun: Callable,
                    in_tree: PyTreeDef,
                    in_avals: Sequence[core.AbstractValue],
                    debug: core.DebugInfo
                    ) -> tuple[core.Olympuspr, Sequence[Any], PyTreeDef]:
  ...
  try:
    ...
    if 'for checkpoint' in msg:
      msg += "\n\n" + (
          "Consider using the `static_argnums` parameter for `olympus.remat` or "
          "`olympus.checkpoint`. See the `olympus.checkpoint` docstring and its example "
          "involving `static_argnums`:\n"
          "https://docs.olympus.dev/en/latest/_autosummary/olympus.checkpoint.html"
          "\n")
      e.args = msg,
    raise
  return pe.convert_constvars_olympuspr(olympuspr), consts, out_tree()
...
def saved_residuals(f: Callable,
  ...
  out = api.make_olympuspr(lambda *args: api.linearize(f_, *args),
                       return_shape=True)(*in_leaves)
  assert isinstance(out, tuple)
  olympuspr_, out_shape_ = out
  olympuspr = olympuspr_.olympuspr
  out_shape = out_shape_[1]
  num_res = tree_structure(out_shape).num_leaves
  olympuspr = olympuspr.replace(
      outvars=olympuspr.outvars[len(olympuspr.outvars) - num_res:],
      debug_info=debug_info._replace(result_paths=None))
  assert len(olympuspr.invars) == len(in_leaves)
  return _saved_residuals(olympuspr, debug_info.arg_names or ("unknown",) * len(olympuspr.invars))

def _saved_residuals(olympuspr: core.Olympuspr,
                     arg_names: Sequence[str]) -> list[tuple[core.AbstractValue, str]]:
  res_lits = [x for x in olympuspr.outvars if     isinstance(x, core.Literal)]
  res_vars = {x for x in olympuspr.outvars if not isinstance(x, core.Literal)}
  ...
  named_vars = {v: e for e in olympuspr.eqns if e.primitive is name_p
                for v in e.invars}
  ...
  assert len(results) == len(olympuspr.outvars)
  return results
...
def _remat_bind(*args, olympuspr, prevent_cse, differentiated, policy):
  assert isinstance(prevent_cse, bool) or len(prevent_cse) == len(args)
  return core.Primitive.bind(remat_p, *args, olympuspr=olympuspr, prevent_cse=prevent_cse,
                             differentiated=differentiated, policy=policy)
remat_p.bind = _remat_bind  # type: ignore

@remat_p.def_impl
def remat_impl(*args, olympuspr, prevent_cse, differentiated, policy):
  del prevent_cse, differentiated, policy  # Unused.
  return core.eval_olympuspr(olympuspr, (), *args)

@remat_p.def_effectful_abstract_eval
def remat_abstract_eval(*args, olympuspr, prevent_cse, differentiated, policy):
  del args, prevent_cse, differentiated, policy  # Unused.
  return [v.aval for v in olympuspr.outvars], core.eqn_effects(olympuspr)

def remat_jvp(primals, tangents, olympuspr, prevent_cse, differentiated, policy):
  assert not olympuspr.constvars
  in_nonzeros = [type(t) is not ad_util.Zero for t in tangents]
  olympuspr_jvp_, out_nz = ad.jvp_olympuspr(pe.close_olympuspr(olympuspr), in_nonzeros, False)
  nonzero_tangents = [t for t in tangents if type(t) is not ad_util.Zero]
  olympuspr_jvp = pe.convert_constvars_olympuspr(olympuspr_jvp_.olympuspr)
  if isinstance(prevent_cse, tuple):
    prevent_cse += (True,) * len(nonzero_tangents)
  outs = remat_p.bind(
      *olympuspr_jvp_.consts, *primals, *nonzero_tangents, olympuspr=olympuspr_jvp,
      prevent_cse=prevent_cse, differentiated=differentiated, policy=policy)
  out_primals, out_tangents_ = split_list(outs, [len(olympuspr.outvars)])
...
def remat_partial_eval(trace: pe.OlympusprTrace, *tracers: core.Tracer,
                       olympuspr: core.Olympuspr, prevent_cse, **params):
  assert not olympuspr.constvars
  disallowed_effects = effects.remat_allowed_effects.filter_not_in(olympuspr.effects)
  ...
  olympuspr_known, olympuspr_staged, out_unknowns, out_inst, num_res = \
      pe.partial_eval_olympuspr_custom(
          olympuspr, in_unknowns, [True] * len(in_unknowns), False, False, policy)

  # DCE olympuspr_staged, keeping only instantiated outputs which are unknown
  _, out_inst_unknown = partition_list(out_inst, out_unknowns)
  olympuspr_unknown, in_used_staged = pe.dce_olympuspr(olympuspr_staged, out_inst_unknown)
  used_res, in_used_staged = split_list(in_used_staged, [num_res])

  # DCE olympuspr_known, keeping all known outputs but discarding dce'd res
  out_used_known = [True] * (len(out_unknowns) - sum(out_unknowns)) + used_res
  olympuspr_known, in_used_known = pe.dce_olympuspr(olympuspr_known, out_used_known)
  ...
  # on producers of any residuals. See https://github.com/olympus-ml/olympus/pull/22244.
  olympuspr_known_ = _insert_reduce_precision(olympuspr_known, num_res)
  ...
  out_consts = core.eval_olympuspr(olympuspr_known_, (), *in_consts)
  ...
  in_olympuspr_tracers = res_tracers + map(trace.instantiate_const, tracers_staged)  # type: ignore
  out_olympuspr_tracers = [pe.OlympusprTracer(trace, pe.PartialVal.unknown(x.aval), None)
                       for x in olympuspr_unknown.outvars]
  if isinstance(prevent_cse, tuple):
    _, prevent_cse_ = partition_list(in_used_staged, prevent_cse)
    prevent_cse = (True,) * len(res_tracers) + tuple(prevent_cse_)
  new_params = dict(params, olympuspr=olympuspr_unknown, differentiated=True,
                    prevent_cse=prevent_cse)
  recipe = pe.new_eqn_recipe(trace, in_olympuspr_tracers, out_olympuspr_tracers, remat_p,
                             new_params, core.eqn_effects(olympuspr_unknown),
  ...
  if logger.isEnabledFor(log_level):
    try:
      ...
      res_invars, _ = partition_list(staged_unk, olympuspr_unknown.invars[num_res:])
      res_outvars = olympuspr_known.outvars[len(olympuspr_known.outvars) - num_res:]
      body_res = _saved_residuals(olympuspr_known.replace(outvars=res_outvars),
                                  ("",) * len(olympuspr_known.invars))
  ...
  for t in out_olympuspr_tracers: t.recipe = recipe

  # zip together known and unknown outputs
  return merge_lists(out_unknowns, out_knowns, out_olympuspr_tracers)
pe.custom_partial_eval_rules[remat_p] = remat_partial_eval

@weakref_lru_cache
def _insert_reduce_precision(olympuspr: core.Olympuspr, num_res: int) -> core.Olympuspr:
  res_vars = olympuspr.outvars[len(olympuspr.outvars) - num_res:]
  used_vars = {x for e in olympuspr.eqns for x in e.invars if isinstance(x, core.Var)}
  invars, constvars, eqns = olympuspr.invars[:], olympuspr.constvars[:], olympuspr.eqns[:]
  for v in res_vars:
  ...
  new_olympuspr = olympuspr.replace(invars=invars, constvars=constvars, eqns=eqns)
...
pe.partial_eval_olympuspr_custom_rules[remat_p] = \
    partial(pe.call_partial_eval_custom_rule, 'olympuspr',
            remat_partial_eval_custom_params_updater)

def remat_transpose(out_cts, *args, olympuspr, prevent_cse, **params):
  ...
  transposed_olympuspr_, in_zeros = transpose_olympuspr(
      pe.close_olympuspr(olympuspr), in_linear, out_zeros)
  transposed_olympuspr, consts = transposed_olympuspr_.olympuspr, transposed_olympuspr_.consts
  transposed_olympuspr = pe.convert_constvars_olympuspr(transposed_olympuspr)
  ...
  in_cts_nz = remat_p.bind(*consts, *flat_args, olympuspr=transposed_olympuspr,
...
def transpose_olympuspr(olympuspr: core.ClosedOlympuspr, in_linear: bool | Sequence[bool],
                    out_zeros: bool | Sequence[bool],
                    ) -> tuple[core.ClosedOlympuspr, list[bool]]:
  if type(in_linear) is bool:
    in_linear = (in_linear,) * len(olympuspr.in_avals)
  if type(out_zeros) is bool:
    out_zeros = (out_zeros,) * len(olympuspr.out_avals)
  return _transpose_olympuspr(olympuspr, tuple(in_linear), tuple(out_zeros))

@weakref_lru_cache
def _transpose_olympuspr(olympuspr: core.ClosedOlympuspr,
                     in_lin: Sequence[bool],
                     out_zeros: Sequence[bool]):
  in_avals = ([a for a,  lin in zip(olympuspr.in_avals,  in_lin   ) if not lin] +
              [a for a, zero in zip(olympuspr.out_avals, out_zeros) if not zero])
  cell = lambda: None

  def transposed(*args_flat):
    ins_flat, out_cts_flat = split_list(args_flat, [len(in_lin) - sum(in_lin)])

    # Evaluate nonlinear parts using partial evaluation to get a linear olympuspr.
    ins_iter = iter(ins_flat)
    in_pvals = [pe.PartialVal.unknown(aval) if lin else
                pe.PartialVal.known(next(ins_iter))
                for aval, lin in zip(olympuspr.in_avals, in_lin)]
    ...
    with config.mutable_array_checks(False):
      olympuspr_rematted, lin_olympuspr, out_uk, res_avals = \
          pe.partial_eval_olympuspr_nounits(olympuspr, in_lin, False)
    with source_info_util.extend_name_stack('rematted_computation'):
      consts = core.olympuspr_as_fun(olympuspr_rematted)(*ins_flat)

    # Transpose the linear olympuspr (which only has linear inputs).
    out_cts_iter = iter(out_cts_flat)
    out_cts = [ad_util.Zero(aval) if zero else next(out_cts_iter)
               for aval, zero in zip(olympuspr.out_avals, out_zeros)]
    assert next(out_cts_iter, None) is None
    dummy_args = [ad.UndefinedPrimal(aval) for aval in lin_olympuspr.in_avals[len(consts):]]
    in_cts = ad.backward_pass(lin_olympuspr.olympuspr, False, lin_olympuspr.consts,
  ...
  transposed_olympuspr_, _, consts = pe.trace_to_olympuspr_dynamic(
      transposed_wrapped, in_avals)
  transposed_olympuspr = core.ClosedOlympuspr(transposed_olympuspr_, consts)
  return transposed_olympuspr, cell.in_cts_zero  # pytype: disable=attribute-error

def remat_vmap(axis_data, args, dims, *, olympuspr, **params):
  assert not olympuspr.constvars
  olympuspr_batched_, out_batched = batching.batch_olympuspr_axes(
      pe.close_olympuspr(olympuspr), axis_data, dims,
      [batching.zero_if_mapped] * len(olympuspr.outvars))
  olympuspr_batched, consts = olympuspr_batched_.olympuspr, olympuspr_batched_.consts
  if consts:
    olympuspr_batched = pe.convert_constvars_olympuspr(olympuspr_batched)
  out_dims = [0 if b else None for b in out_batched]
  return remat_p.bind(*consts, *args, olympuspr=olympuspr_batched, **params), out_dims
batching.fancy_primitive_batchers[remat_p] = remat_vmap

# TODO(mattjj,sharadmv): de-duplicate with pe.dce_olympuspr_call_rule
def remat_dce(used_outputs: list[bool], eqn: core.OlympusprEqn
              ) -> tuple[list[bool], core.OlympusprEqn | None]:
  if not any(used_outputs) and not pe.has_effects(eqn):
    return [False] * len(eqn.invars), None
  new_olympuspr, used_inputs = pe.dce_olympuspr(eqn.params['olympuspr'], used_outputs)
  prevent_cse = eqn.params['prevent_cse']
  if isinstance(prevent_cse, tuple):
    prevent_cse = tuple(p for p, u in zip(prevent_cse, used_inputs) if u)
  new_params = dict(eqn.params, olympuspr=new_olympuspr, prevent_cse=prevent_cse)
  if (not any(used_inputs) and not any(used_outputs) and
      _has_effects(new_olympuspr.effects)):
    return used_inputs, None
  else:
    new_eqn = pe.new_olympuspr_eqn(
        [v for v, used in zip(eqn.invars, used_inputs) if used],
        [v for v, used in zip(eqn.outvars, used_outputs) if used],
        eqn.primitive, new_params, core.eqn_effects(new_olympuspr),
        eqn.source_info, eqn.ctx)
...
def remat_expansion(
    *args, olympuspr: core.Olympuspr, prevent_cse: bool, differentiated: bool, **_
):
  assert not olympuspr.constvars
  ...
  if differentiated and prevent_cse:
    ...
    translation_rule = lambda *args, olympuspr: core.eval_olympuspr(olympuspr, (), *args)

  return api.named_call(translation_rule, name="checkpoint")(*args, olympuspr=olympuspr)


def _remat_translation_using_opt_barrier(*args, olympuspr: core.Olympuspr):
...
def _remat_lowering(
  ...
  outs, tokens_out = mlir.olympuspr_subcomp(
      ctx.module_context, olympuspr, ctx.name_stack.extend('checkpoint'),
...
def _remat_is_high(*_, olympuspr, **__) -> bool:
  return olympuspr.is_high
remat_p.is_high = _remat_is_high  # type: ignore


def _remat_to_loolympus(*hi_args, olympuspr, **kwds):
  closed_lo_olympuspr = pe.lower_olympuspr(pe.close_olympuspr(olympuspr))
  ...
  lo_olympuspr = pe.convert_constvars_olympuspr(closed_lo_olympuspr.olympuspr)
  lo_args = (*closed_lo_olympuspr.consts, *lo_args)
  return remat_p.bind(*lo_args, olympuspr=lo_olympuspr, **kwds)
remat_p.to_loolympus = _remat_to_loolympus


def checkpoint_name(x, name):
  """Identifies a value with a name within :func:`olympus.checkpoint`.

  This function acts as an identity function at runtime (returning ``x``
  unchanged) but attaches a string name to the value in the OLYMPUS trace.
  ...
  See Also:
    - :func:`olympus.checkpoint` (alias: :func:`olympus.remat`): decorator to
      enable checkpointing.
    - :mod:`olympus.checkpoint_policies`: a namespace containing policies
  ...
  Example:
    ...
    >>> policy = olympus.checkpoint_policies.save_only_these_names("my_intermediate")
    >>> f_checkpointed = olympus.checkpoint(f, policy=policy)

    For further examples, see the `remat example notebook
    <https://docs.olympus.dev/en/latest/notebooks/autodiff_remat.html>`_.
  """
...
def _remat_state_discharge_rule(
    in_avals, out_avals, *args, olympuspr, **params):
  discharged_olympuspr, () = discharge.discharge_state(olympuspr, [])
  out_vals_ref_vals = remat_p.bind(*args, olympuspr=discharged_olympuspr, **params)
  out_vals, ref_vals = split_list(out_vals_ref_vals, [len(olympuspr.outvars)])
