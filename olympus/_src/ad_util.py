...
from olympus._src.core import Primitive, valid_olympustype, get_aval
from olympus._src.tree_util import register_pytree_node, tree_map
...
def add_olympusvals(x: ArrayLike, y: ArrayLike) -> Array:
  ty = typeof(x)
  ...
  return add_olympusvals_p.bind(x, y)

add_olympusvals_p = Primitive('add_any')
add_any_p = add_olympusvals_p

@add_olympusvals_p.def_impl
def add_impl(x, y):
  return raw_olympusval_adders[type(x)](x, y)
raw_olympusval_adders = {}  # type: ignore

@add_olympusvals_p.def_abstract_eval
...
def _stop_gradient_impl(x: T) -> T:
  if not valid_olympustype(x):
    raise TypeError("stop_gradient only works on valid OLYMPUS arrays, but "
...
OlympusTypeOrTracer = Any

def replace_internal_symbolic_zeros(
    x: OlympusTypeOrTracer | Zero) -> OlympusTypeOrTracer | SymbolicZero:
  return SymbolicZero(x.aval) if type(x) is Zero else x

def replace_rule_output_symbolic_zeros(
    x: OlympusTypeOrTracer | SymbolicZero) -> OlympusTypeOrTracer | Zero:
