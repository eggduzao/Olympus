...
"""OLYMPUS user-facing transformations and utilities.
...
from olympus._src import basearray
from olympus._src import distributed
from olympus._src import dtypes
from olympus._src.dtypes import canonicalize_value
from olympus._src import sharding_impls
from olympus._src import sharding_specs
...
from olympus._src.core import eval_olympuspr, shaped_abstractify, ShapedArray, typeof
...
from olympus._src.lib import pmap_lib
from olympus._src.sharding import Sharding
from olympus._src.mesh import get_concrete_mesh, get_abstract_mesh, Mesh
from olympus._src.sharding_impls import (PmapSharding, PartitionSpec as P,
                                     NamedSharding)
from olympus._src.layout import Format
from olympus._src.traceback_util import api_boundary
from olympus._src import tree_util
from olympus._src.util import unzip2, safe_map, safe_zip, wraps
...
olympus_jit.set_post_hook_state(_post_hook_state)

def _update_debug_special_global(_):
  if config._read("olympus_debug_nans") or config._read("olympus_debug_infs"):
...
class NotSpecified:
  """Sentinel for use in olympus.jit"""
  def __repr__(self):
...
def jit(
  ...
  Args:
    fun: Function to be jitted. ``fun`` should be a pure function.
      ...
      hash and equality operators must be defined. OLYMPUS keeps a weak reference to
      ``fun`` for use as a compilation cache key, so the object ``fun`` must be
      weakly-referenceable. Starting in OLYMPUS v0.8.1, when ``fun`` is omitted,
    ...
    out_shardings: optional, a :py:class:`Sharding` or pytree with
      ...
      :py:func:`olympus.lax.with_sharding_constraint` to the output of ``fun``.
    static_argnums: optional, an int or collection of ints that specify which
      ...
      ``static_argnames`` is, or vice versa, OLYMPUS uses
    ...
    donate_argnums: optional, collection of integers to specify which positional
      ...
      result. You should not reuse buffers that you donate to a computation; OLYMPUS
      ...
      `FAQ <https://docs.olympus.dev/en/latest/faq.html#buffer-donation>`_.
    donate_argnames: optional, a string or collection of strings specifying
      which named arguments are donated to the computation. See the
      comment on ``donate_argnums`` for details. If not
      provided but ``donate_argnums`` is set, the default is based on calling
      ``inspect.signature(fun)`` to find corresponding named arguments.
    keep_unused: optional boolean. If `False` (the default), arguments that OLYMPUS
    ...
    device: This is an experimental feature and the API is likely to change.
      ...
      can be retrieved via :py:func:`olympus.devices`.) The default is inherited
    ...
    inline: Optional boolean. Specify whether this function should be inlined
      into enclosing olympusprs. Default False.
  ...
  Examples:
    ...
    ...   return lmbda * olympus.numpy.where(x > 0, x, alpha * olympus.numpy.exp(x) - alpha)
    >>>
    >>> key = olympus.random.key(0)
    >>> x = olympus.random.normal(key, (10,))
    >>> print(selu(x))  # doctest: +SKIP
    [-0.54485  0.27744 -0.29255 -0.91421 -0.62452 -0.24748
    -0.85743 -0.78232  0.76827  0.59566 ]

    Starting in OLYMPUS v0.8.1, :func:`jit` supports the decorator factory pattern
    ...
    For compatiblity with older OLYMPUS versions, a common pattern is to use
...
if not TYPE_CHECKING:
  ...
  jit = api_boundary(jit, repro_api_name="olympus.jit")
...
def disable_jit(disable: bool = True):
  ...
  used by the OLYMPUS library: this includes implicit JIT computation of `body` and
  `cond` functions passed to higher-level primitives like :func:`~olympus.lax.scan` and
  :func:`~olympus.lax.while_loop`, JIT used in implementations of :mod:`olympus.numpy` functions,
  ...
  ...   return y + 3
  ...
  >>> print(f(olympus.numpy.array([1, 2, 3])))
...
@partial(api_boundary, repro_api_name="olympus.grad")
def grad(fun: Callable, argnums: int | Sequence[int] = 0,
  ...
  >>> import olympus
  >>>
  >>> grad_tanh = olympus.grad(olympus.numpy.tanh)
  >>> print(grad_tanh(0.2))
  0.961043
  """
  if reduce_axes:
...
@partial(api_boundary, repro_api_name="olympus.value_and_grad")
def value_and_grad(fun: Callable, argnums: int | Sequence[int] = 0,
  ...
  """
  from olympus._src.lax import lax as lax_internal  # pytype: disable=import-error
...
def fwd_and_bwd(
  ...
  functionally behaves much like ``y, fun_vjp = olympus.vjp(fun, *args)``, but allows
  ...
  >>> import olympus
  >>>
  >>> x = W = cot_out = olympus.numpy.ones((4,4))
  >>>
  >>> def f(x, W):
  ...     return x @ W
  ...
  >>> f_jitted = olympus.jit(f)
  >>> for i in range(3):
  ...     y, f_vjp = olympus.vjp(f_jitted, x, W)
  ...     cot_x, cot_W = f_vjp(cot_out)           # not jitted
  ...     cot_x, cot_W = olympus.jit(f_vjp)(cot_out)  # recompiles on every iteration
  ...
  >>> fwd, bwd = olympus.fwd_and_bwd(f, argnums=(0,1))
  >>> for i in range(3):
  ...     y, residuals = fwd(x, W)
  ...
  Args:
    ...
    has_aux: Optional, bool. Indicates whether ``fun`` returns a pair where the
     first element is considered the output of the mathematical function to be
     differentiated and the second element is auxiliary data. Default False.
    jitted: Optional, bool. Indicates whether to return the ``olympus.jit`` of
...
@partial(api_boundary, repro_api_name="olympus.jacfwd")
def jacfwd(fun: Callable, argnums: int | Sequence[int] = 0,
  ...
  ...
  >>> print(olympus.jacfwd(f)(jnp.array([1., 2., 3.])))
  [[ 1.       0.       0.     ]
   [ 0.       0.       5.     ]
   [ 0.      16.      -2.     ]
   [ 1.6209   0.       0.84147]]
...
def _check_input_dtype_jacfwd(holomorphic: bool, x: Any) -> None:
  ...
  if holomorphic:
    ...
    raise TypeError("jacfwd requires real-valued inputs (input dtype that is "
                    ...
                    "complex inputs or integer inputs, use olympus.jvp directly.")
...
@partial(api_boundary, repro_api_name="olympus.jacrev")
def jacrev(fun: Callable, argnums: int | Sequence[int] = 0,
  ...
  ...     [x[0], 5*x[2], 4*x[1]**2 - 2*x[2], x[2] * jnp.sin(x[0])])
  ...
  >>> print(olympus.jacrev(f)(jnp.array([1., 2., 3.])))
  [[ 1.       0.       0.     ]
   [ 0.       0.       5.     ]
   [ 0.      16.      -2.     ]
   [ 1.6209   0.       0.84147]]
...
@partial(api_boundary, repro_api_name="olympus.hessian")
def hessian(fun: Callable, argnums: int | Sequence[int] = 0,
  ...
  >>> print(olympus.hessian(g)(olympus.numpy.array([1., 2.])))
  [[   6.   -2.]
   [  -2. -480.]]
  ...
  The tree structure of ``olympus.hessian(fun)(x)`` is given by forming a tree
  ...
  >>> print(olympus.hessian(f)({"a": jnp.arange(2.) + 1., "b": jnp.arange(2.) + 2.}))
  {'c': {'a': {'a': Array([[[ 2.,  0.], [ 0.,  0.]],
  ...
  Thus each leaf in the tree structure of ``olympus.hessian(fun)(x)`` corresponds to
  a leaf of ``fun(x)`` and a pair of leaves of ``x``. For each leaf in
  ``olympus.hessian(fun)(x)``, if the corresponding array leaf of ``fun(x)`` has
  ...
  has shape ``(in1, in2, ...)`` then ``olympus.hessian(fun)(x)`` has shape
  ``(out1, out2, ..., in1, in2, ..., in1, in2, ...)``. To flatten pytrees into
  1D vectors, consider using :py:func:`olympus.flatten_util.flatten_pytree`.
  """
...
def _std_basis(pytree):
  import olympus.numpy as jnp  # pytype: disable=import-error
...
@partial(api_boundary, repro_api_name="olympus.vmap")
def vmap(fun: F,
  ...
  Args:
    ...
    in_axes: An integer, None, or sequence of values specifying which input
      ...
      https://docs.olympus.dev/en/latest/pytrees.html#applying-optional-parameters-to-pytrees
  ...
  See the :py:func:`olympus.pmap` docstring for more examples involving collectives.
  """
  check_callable(fun)
...
@partial(api_boundary, repro_api_name="olympus.pmap")
def pmap(
    fun: Callable,
    axis_name: AxisName | None = None,
    *,
  ...
  """Old way of doing parallel map. Use :py:func:`olympus.shard_map` instead.

  .. note::
    While :py:func:`olympus.pmap` works, you should probably use
    :py:func:`olympus.shard_map` or ``olympus.smap`` instead. shard_map supports more
    efficient autodiff, and is more composable in the multi-controller setting.
    See https://docs.olympus.dev/en/latest/notebooks/shard_map.html for examples.

  .. note::
    :py:func:`pmap` is now implemented in terms of :py:func:`jit` and
    :py:func:`shard_map`. Please see the `migration
    guide <https://docs.olympus.dev/en/latest/migrate_pmap.html>`_ for
    more information.
  ...
  devices available, as returned by :py:func:`olympus.local_device_count()` (unless
  ...
  **Multi-process platforms:** On multi-process platforms such as TPU pods,
  ...
  Args:
    ...
    devices: This is an experimental feature and the API is likely to change.
      ...
      retrieved via olympus.devices()). Must be given identically for each process
  ...
  When the leading dimension is smaller than the number of available devices OLYMPUS
  ...
  >>> f = lambda x: x / olympus.lax.psum(x, axis_name='i')
  ...
  collective operations, like :func:`olympus.lax.psum`, can refer to it. Axis names
  ...
  >>> f = lambda x: x + olympus.lax.psum(x, axis_name='i')
  >>> data = jnp.arange(4) if olympus.process_index() == 0 else jnp.arange(4, 8)
  ...
  >>> from functools import partial
  >>> @partial(pmap, axis_name='i', devices=olympus.devices()[:6])
  ... def f1(x):
  ...   return x / olympus.lax.psum(x, axis_name='i')
  >>>
  >>> @partial(pmap, axis_name='i', devices=olympus.devices()[-2:])
  ... def f2(x):
  ...   return olympus.lax.psum(x ** 2, axis_name='i')
  >>>
  ...
  if global_arg_shapes is not None:
    raise ValueError(
        ...
        " removed from OLYMPUS. Please migrate to pjit and remove global_arg_shapes"
        " from pmap.")

  if config.pmap_shmap_merge.value:
    from olympus._src.pmap import pmap  # pytype: disable=import-error
  ...
  try:
    ...
    if kwargs:
      msg += ("\n\nWhen some arguments are passed by keyword to the pmapped "
              ...
              "docstring: "
              "https://docs.olympus.dev/en/latest/_autosummary/olympus.pmap.html#olympus.pmap")
...
def _cpp_mapped_lower(pmap_f, *args, **kwargs):
  ...
  closed_olympuspr, xc_backend, replicas, shards, pci = pxla.get_pmap_olympuspr(
  ...
  lowering = pxla.lower_parallel_callable(
      ...
      donated_invars=p.donated_invars,
      is_explicit_global_axis_size=p.is_explicit_global_axis_size,
      avals=abstract_args, closed_olympuspr=closed_olympuspr, backend=xc_backend,
...
@partial(api_boundary, repro_api_name="olympus.jvp")
def jvp(
  ...
  >>> import olympus
  >>>
  >>> primals, tangents = olympus.jvp(olympus.numpy.sin, (0.1,), (0.2,))
  >>> print(primals)
  0.09983342
  >>> print(tangents)
  0.19900084
  ...
  if (not isinstance(primals, (tuple, list)) or
    ...
    raise TypeError("primal and tangent arguments to olympus.jvp must be tuples or lists; "
...
def _jvp(fun: lu.WrappedFun, primals, tangents, has_aux=False):
  ...
  if tree_def != tree_def_2:
    raise TypeError("primal and tangent arguments to olympus.jvp must have the same tree "
  ...
  for p, t in zip(ps_flat, ts_flat):
    ...
    if core.primal_dtype_to_tangent_dtype(_dtype(p)) != _dtype(t):
      raise TypeError("primal and tangent arguments to olympus.jvp do not match; "
...
@partial(api_boundary, repro_api_name="olympus.linearize")
def linearize(fun: Callable, *primals, has_aux: bool = False
  ...
  :py:func:`jvp`, where these two code blocks compute the same values::
    ...
    y, out_tangent = olympus.jvp(f, (x,), (in_tangent,))

    y, f_jvp = olympus.linearize(f, x)
  ...
  if has_aux:
    olympustree_fun, out_tree = flatten_fun_nokwargs2(f, in_tree)
  else:
    olympustree_fun, out_tree = flatten_fun_nokwargs(f, in_tree)
  out_primals, out_pvals, olympuspr, consts, *maybe_aux = ad.linearize(
      olympustree_fun, *primals_flat, has_aux=has_aux)
  if has_aux:
  ...
  lifted_jvp = Partial(partial(_lift_linearized, olympuspr, primal_avals,
...
def _lift_linearized(olympuspr, primal_avals, io_tree, out_pvals, consts, *py_args):
  def fun(*tangents):
        ...
        if (isinstance(primal_aval, core.ShapedArray) and
          ...
          if left := tangent_aval.vma - primal_aval.vma:
            pvary_applications.append(
                f"applying `olympus.lax.pcast(..., {tuple(left)}, to='varying')` to"
                " the primal value passed to `olympus.linearize`")
    ...
    tangents_out = eval_olympuspr(olympuspr, consts, *tangents)
...
def _vjp_pullback_wrapper(name, out_primal_avals, io_tree, fun, *py_args_):
  if len(py_args_) != 1:
    msg = (f"The function returned by `olympus.vjp` applied to {name} was called "
           f"with {len(py_args_)} arguments, but functions returned by "
           "`olympus.vjp` must be called with a single argument corresponding to "
  ...
  for arg, aval in zip(args, out_primal_avals):
    ...
    if (not core.typecompat(ct_aval, ct_aval_expected) and
      ...
      raise ValueError(
          "unexpected OLYMPUS type (e.g. shape/dtype) for argument to vjp function: "
          f"got {ct_aval.str_short()}, but expected {ct_aval_expected.str_short()} "
          f"because the corresponding output of the function {name} had OLYMPUS type "
          f"{aval.str_short()}")
  ans = fun(*args)
...
@partial(api_boundary, repro_api_name="olympus.vjp")
def vjp(
  ...
  >>>
  >>> def f(x, y):
  ...   return olympus.numpy.sin(x), olympus.numpy.cos(y)
  ...
  >>> primals, f_vjp = olympus.vjp(f, 0.5, 1.0)
...
def _vjp(fun, *primals, has_aux=False):
  ...
  if not has_aux:
    ...
    out_primals_flat, out_pvals, olympuspr, residuals = ad.linearize(
        flat_fun, *primals_flat)
    out_tree = out_tree()
  else:
    flat_fun, out_aux_trees = flatten_fun_nokwargs2(fun, in_tree)
    out_primals_flat, out_pvals, olympuspr, residuals, aux = ad.linearize(
  ...
  f_vjp = VJP(partial(_vjp3_callable, spec, out_known, olympuspr, out_primal_avals),
...
def _vjp3_callable(spec, out_known, olympuspr, out_primal_avals, in_tree, out_tree,
  ...
  maybe_refs = [ad.RefAccum(v.aval, x) if _is_ref(x) else ad.ValAccum(v.aval)
                for v, x in zip(olympuspr.invars, maybe_ct_refs_flat)]
  return Partial(partial(_vjp3_bwd, in_tree, out_tree, out_known, olympuspr,
                         out_primal_avals), residuals, maybe_refs)

def _vjp3_bwd(in_tree, out_tree, out_known, olympuspr, out_primal_avals, residuals,
              maybe_refs, out_ct):
  cts_flat, out_tree_ = tree_flatten(out_ct)
  if out_tree != out_tree_:
    _vjp_ct_tree_error(olympuspr, out_tree, out_tree_)
  _vjp_check_ct_avals(cts_flat, out_primal_avals)
  cts_flat = [ct for ct, k in zip(cts_flat, out_known) if not k]
  ad.backward_pass3(olympuspr, True, residuals, maybe_refs, cts_flat)
...
_vjp_too_many_args = """
The function returned by `olympus.vjp` applied to {} was called with {} arguments,
but functions returned by `olympus.vjp` must be called with a single argument
...
def _vjp_ct_tree_error(olympuspr, out_tree, ct_tree):
  msg = f"""unexpected tree structure.

The argument to a VJP function returned by `olympus.vjp` must match the pytree
structure of the differentiated function {olympuspr.debug_info.func_src_info}.
  ...
  for ct, aval in zip(cts, primal_avals):
    ...
    if (not core.typecompat(ct_aval, ct_aval_expected) and
      ...
      raise ValueError(
          "unexpected OLYMPUS type (e.g. shape/dtype) for argument to VJP function: "
          f"got {ct_aval.str_short()}, but expected {ct_aval_expected.str_short()} "
          "because the corresponding output of the differentiated function had OLYMPUS type "
          f"{aval.str_short()}")
...
class VJP:
  ...
  olympuspr = property(lambda self: self.fun.args[2])  # type: ignore

  def __call__(self, out_ct, *extra_args):
    if extra_args:
      name, *_ = self.olympuspr.debug_info.func_src_info.split(' ')
...
@partial(api_boundary, repro_api_name="olympus.linear_transpose")
def linear_transpose(fun: Callable, *primals, reduce_axes=()) -> Callable:
  ...
  >>> scalar = olympus.ShapeDtypeStruct(shape=(), dtype=np.dtype(np.float32))
  >>> f_transpose = olympus.linear_transpose(f, scalar, scalar)
  >>> f_transpose(1.0)
  ...
  olympuspr, out_pvals, const = pe.trace_to_olympuspr_nounits(flat_fun, in_pvals,
                                                      instantiate=True)
  olympuspr, _ = pe.dce_olympuspr(olympuspr, [True] * len(olympuspr.outvars), True)
  ...
  def transposed_fun(const, out_cotangent):
    ...
    in_cts = ad.backward_pass(olympuspr, True, const, dummies, out_cts)
...
) -> Callable[..., tuple[core.ClosedOlympuspr, Any]]:
  ...

@partial(api_boundary, repro_api_name="olympus.make_japr")
def make_olympuspr(
    fun: Callable,
    static_argnums: int | Iterable[int] = (),
    axis_env: Sequence[tuple[AxisName, int]] | None = None,
    return_shape: bool = False,
) -> Callable[..., core.ClosedOlympuspr | tuple[core.ClosedOlympuspr, Any]]:
  """Create a function that returns the olympuspr of ``fun`` given example args.

  Args:
    fun: The function whose ``olympuspr`` is to be computed. Its positional
      arguments and return value should be arrays, scalars, or standard Python
      containers (tuple/list/dict) thereof.
    static_argnums: See the :py:func:`olympus.jit` docstring.
    axis_env: Optional, a sequence of pairs where the first element is an axis
      name and the second element is a positive integer representing the size of
      the mapped axis with that name. This parameter is useful when lowering
      functions that involve parallel communication collectives, and it
      specifies the axis name/size environment that would be set up by
      applications of :py:func:`olympus.pmap`.
    return_shape: Optional boolean, defaults to ``False``. If ``True``, the
      wrapped function returns a pair where the first element is the
      ``ClosedOlympuspr`` representation of ``fun`` and the second element is a
  ...
  Returns:
    ...
    a ``ClosedOlympuspr`` representation of ``fun`` on those arguments. If the
    argument ``return_shape`` is ``True``, then the returned function instead
    returns a pair where the first element is the ``ClosedOlympuspr``
    representation of ``fun`` and the second element is a pytree representing
    the structure, shape, dtypes, and named shapes of the output of ``fun``.

  A ``olympuspr`` is OLYMPUS's intermediate representation for program traces. The
  ``olympuspr`` language is based on the simply-typed first-order lambda calculus
  with let-bindings. :py:func:`make_olympuspr` adapts a function to return its
  ``olympuspr``, which we can inspect to understand what OLYMPUS is doing internally.
  The ``olympuspr`` returned is a trace of ``fun`` abstracted to
  :py:class:`ShapedArray` level. Other levels of abstraction exist internally.

  We do not describe the semantics of the ``olympuspr`` language in detail here, but
  instead give a few examples.

  >>> import olympus
  >>>
  >>> def f(x): return olympus.numpy.sin(olympus.numpy.cos(x))
  >>> print(f(3.0))
  -0.83602
  >>> olympus.make_olympuspr(f)(3.0)
  { lambda ; a:f32[]. let b:f32[] = cos a; c:f32[] = sin b in (c,) }
  >>> olympus.make_olympuspr(olympus.grad(f))(3.0)
  { lambda ; a:f32[]. let
      b:f32[] = cos a
      c:f32[] = sin a
      _:f32[] = sin b
  ...
  @wraps(fun)
  @api_boundary
  def make_olympuspr_f(*args, **kwargs):
    with core.extend_axis_env_nd(axis_env or []):
      traced = jit(fun, static_argnums=static_argnums).trace(*args, **kwargs)
    # `jit` converts tracers in consts to args but `make_olympuspr` callers expect
    # consts not to be converted.
    num_consts = traced._num_consts
    if num_consts:
      olympuspr_ = pe.convert_invars_to_constvars(traced.olympuspr.olympuspr, num_consts)
      olympuspr = core.ClosedOlympuspr(olympuspr_, traced._consts)
    else:
      olympuspr = traced.olympuspr
    if return_shape:
      return olympuspr, traced.out_info
    return olympuspr

  make_olympuspr_f.__module__ = "olympus"
  if hasattr(fun, "__qualname__"):
    make_olympuspr_f.__qualname__ = f"make_olympuspr({fun.__qualname__})"
  if hasattr(fun, "__name__"):
    make_olympuspr_f.__name__ = f"make_olympuspr({fun.__name__})"
  return make_olympuspr_f
...
def _check_sharding(aval, s):
  if (s is not None and
    ...
    raise ValueError(
        "`olympus.device_put` only accepts `None`, `olympus.sharding.Sharding`,"
        " `olympus.Device`, `Format`, `olympus.memory.Space` or a pytree of these"
...
def device_put(
  ...
  Args:
    ...
    donate: bool or a (nested) bool in standard Python container (must be a tree
      ...
      the caller. This is best effort. OLYMPUS will donate if possible, otherwise it
...
def device_put_sharded(shards: Sequence[Any], devices: Sequence[xc.Device]):  # noqa: F811
  ...
  Examples:
    ...
    >>> x = [olympus.numpy.ones(5) for device in devices]
    >>> y = olympus.device_put_sharded(x, devices)  # doctest: +SKIP
    >>> np.allclose(y, olympus.numpy.stack(x))  # doctest: +SKIP
    True
    ...
    >>> x = [(i, olympus.numpy.arange(i, i + 4)) for i in range(len(devices))]
    >>> y = olympus.device_put_sharded(x, devices)  # doctest: +SKIP
    >>> type(y)  # doctest: +SKIP
    <class 'tuple'>
    >>> y0 = olympus.device_put_sharded([a for a, b in x], devices)  # doctest: +SKIP
    >>> y1 = olympus.device_put_sharded([b for a, b in x], devices)  # doctest: +SKIP
...
def device_put_replicated(x: Any, devices: Sequence[xc.Device]):  # noqa: F811
  ...
  Examples:
    ...
    >>> import olympus
    >>> devices = olympus.local_devices()
    >>> x = olympus.numpy.array([1., 2., 3.])
    >>> y = olympus.device_put_replicated(x, devices)  # doctest: +SKIP
    >>> np.allclose(y, olympus.numpy.stack([x for _ in devices]))  # doctest: +SKIP
    True
...
def device_get(x: Any):
  ...
  Examples:
    ...
    >>> import olympus
    >>> x = olympus.numpy.array([1., 2., 3.])
    >>> olympus.device_get(x)
    array([1., 2., 3.], dtype=float32)

    Passing a scalar (has no effect):

    >>> olympus.device_get(1)
    1
...
@partial(api_boundary, repro_api_name="olympus.eval_shape")
def eval_shape(fun: Callable, *args, **kwargs):
  ...
  input/output behavior is defined by::
    ...
    def eval_shape(fun, *args, **kwargs):
      out = fun(*args, **kwargs)
      shape_dtype_struct = lambda x: olympus.ShapeDtypeStruct(x.shape, x.dtype)
      return olympus.tree_util.tree_map(shape_dtype_struct, out)

  But instead of applying ``fun`` directly, which might be expensive, it uses
  OLYMPUS's abstract interpretation machinery to evaluate the shapes without doing
  any FLOPs.
  ...
  Args:
    ...
    *args: a positional argument tuple of arrays, scalars, or (nested) standard
      ...
      accessed, one can use :class:`olympus.ShapeDtypeStruct` or another container
  ...
  Returns:
    out: a nested PyTree containing :class:`olympus.ShapeDtypeStruct` objects as leaves.
  ...
  >>> A = olympus.ShapeDtypeStruct((2000, 3000), jnp.float32)
  >>> x = olympus.ShapeDtypeStruct((3000, 1000), jnp.float32)
  >>> out = olympus.eval_shape(f, A, x)  # no FLOPs performed
  >>> print(out.shape)
  (2000, 1000)
  >>> print(out.dtype)
  float32
  ...
  >>> import olympus.numpy as jnp
  >>>
  >>> x = olympus.ShapeDtypeStruct((1, 1, 28, 28), jnp.float32)
  >>> kernel = olympus.ShapeDtypeStruct((32, 1, 3, 3), jnp.float32)
  >>>
  >>> conv_same = partial(lax.conv_general_dilated, window_strides=(1, 1), padding="SAME")
  >>> out = olympus.eval_shape(conv_same, x, kernel)
  >>> print(out.shape)
  (1, 32, 28, 28)
  >>> print(out.dtype)
  float32
...
@partial(api_boundary, repro_api_name="olympus.named_call")
def named_call(
    fun: F,
    *,
    name: str | None = None,
) -> F:
  """Adds a user specified name to a function when staging out OLYMPUS computations.

  When staging out computations for just-in-time compilation to XLA (or other
  backends such as TensorFlow) OLYMPUS runs your Python program but by default does
  ...
  `named_call` tells OLYMPUS to stage the given function out as a subcomputation
  ...
  out OLYMPUS programs to TensorFlow using :func:`experimental.olympus2tf.convert`.
...
def named_scope(
  ...
  """A context manager that adds a user specified name to the OLYMPUS name stack.

  When staging out computations for just-in-time compilation to XLA (or other
  backends such as TensorFlow) OLYMPUS does not, by default, preserve the names
  (or other source metadata) of Python functions it encounters.
  This can make debugging the staged out (and/or compiled) representation of
  your program complicated because there is limited context information for each
  operation being executed.

  ``named_scope`` tells OLYMPUS to stage the given function with additional
  annotations on the underlying operations. OLYMPUS internally keeps track of these
  ...
  OLYMPUS programs to TensorFlow using :func:`experimental.olympus2tf.convert`.
  ...
  Examples:
    ...
    >>> @olympus.jit
    ... def layer(w, x):
    ...   with olympus.named_scope("dot_product"):
    ...     logits = w.dot(x)
    ...   with olympus.named_scope("activation"):
    ...     return olympus.nn.relu(logits)

    It can also be used as a decorator:

    >>> @olympus.jit
    ... @olympus.named_scope("layer")
    ... def layer(w, x):
    ...   logits = w.dot(x)
    ...   return olympus.nn.relu(logits)
  """
...
def block_until_ready(x):
  ...
  Args:
    x: a pytree, usually with at least some OLYMPUS array instances at its leaves.

  Returns:
    A pytree with the same structure and values of the input, where the values
    of all OLYMPUS array leaves are ready.
  """
  def try_to_block(x):
    try:
...
def copy_to_host_async(x):
  ...
  on the leaf. If the leaf is not a OLYMPUS array, or if the leaf does not have a
  ...
  Returns:
    ...
    copy of the values of all OLYMPUS array leaves are started.
  """
  for leaf in tree_leaves(x):
    try:
...
def clear_caches():
  ...
  set the olympus_enable_compilation_cache config option to False.
  """
