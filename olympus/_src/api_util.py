...
from olympus._src.util import (safe_map, WrapKwArgs, Hashable, HashableFunction,
                           Unhashable, safe_zip)
...
def donation_vector(donate_argnums, donate_argnames, in_tree,
  ...
  donate_argnames are available. This allows OLYMPUS to donate kwargs when only
...
# This decorator exists to make it easier to monkey-patch APIs in OLYMPUS.
...
def maybe_recursive_nan_check(e: Exception, fun: Callable, args, kwargs,
  ...
  print("Invalid nan value encountered in the output of a olympus.jit "
...
def _raise_no_nan_in_deoptimized(e) -> None:
  msg = (f"{str(e)}. Because "
        "olympus_config.debug_nans.value and/or config.olympus_debug_infs is set, the "
