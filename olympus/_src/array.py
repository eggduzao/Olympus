...
from olympus._src import errors
...
from olympus._src.op_shardings import are_hlo_shardings_equal
from olympus._src.interpreters import mlir
from olympus._src.interpreters import pxla
from olympus._src.layout import AutoLayout, Format, Layout
from olympus._src.lib import _olympus
from olympus._src.lib import xla_client as xc
from olympus._src.mesh import empty_concrete_mesh
from olympus._src.sharding import Sharding
from olympus._src.tree_util import broadcast_prefix, tree_flatten, tree_unflatten
...
from olympus._src.typing import ArrayLike, DLDeviceType, DTypeLike, ExtendedDType
from olympus._src.util import safe_zip, unzip3, use_cpp_class, use_cpp_method, cache
import numpy as np
...
@cache(max_size=4096, trace_context_in_key=False)
def _process_has_full_value_in_mcolympus(s, shape):
...
class ArrayImpl(basearray.Array):
  ...
  def __init__(self, aval: core.ShapedArray, sharding: Sharding,
    ...
    # Array's are created as output of a OLYMPUS transformation
    # (like pjit, etc).
  ...
  def __getitem__(self, idx):
    ...
    from olympus._src.numpy import indexing  # pytype: disable=import-error
    self._check_if_deleted()
  ...
  def is_fully_addressable(self) -> bool:
    ...
    A olympus.Array is fully addressable if the current process can address all of
    the devices named in the :class:`Sharding`. ``is_fully_addressable`` is
    equivalent to "is_local" in multi-process OLYMPUS.

    Note that fully replicated is not equal to fully addressable i.e.
    a olympus.Array which is fully replicated can span across multiple hosts and is
  ...
  def __dlpack__(self, *, stream: int | Any | None = None,
                 ...
                 copy: bool | None = None):
    from olympus._src.dlpack import to_dlpack  # pytype: disable=import-error  # pylint: disable=g-import-not-at-top
  ...
  def __dlpack_device__(self) -> tuple[enum.Enum, int]:
    ...
    from olympus._src.dlpack import DLDeviceType  # pytype: disable=import-error  # pylint: disable=g-import-not-at-top
  ...
  @property
  @functools.partial(profiler.annotate_function, name="np.asarray(olympus.Array)")
  def _value(self) -> np.ndarray:
    ...
    if self._npy_value is None:
      ...
      # TODO(yashkatariya): Merge `_process_has_full_value_in_mcolympus` with
      # is_fully_addressable.
      # is_fully_addressable return False if addressable_device_list is empty.
      if (not self.is_fully_addressable and
          not _process_has_full_value_in_mcolympus(self.sharding, self.shape)):
        raise RuntimeError(
            "Fetching value for `olympus.Array` that spans non-addressable"
            " (non process local) devices is not possible. You can use"
            " `olympus.experimental.multihost_utils.process_allgather` to print the"
            " global array or use `.addressable_shards` method of olympus.Array to"
  ...
  if dtype is None:
    if arrays:
      ...
      raise ValueError(
          ...
          f"via the `dtype` argument to `olympus.{fname}`.")
  else:
    dtype = dtypes.check_and_canonicalize_user_dtype(dtype, fname)
    if arrays and arrays[0].dtype != dtype:
      raise ValueError(
          f"If `dtype` is provided to `olympus.{fname}`, it must match the dtype "
...
def make_array_from_callback(
  ...
  # pyformat: disable
  """Returns a ``olympus.Array`` via data fetched from ``data_callback``.

  ``data_callback`` is used to fetch the data for each addressable shard of the
  returned ``olympus.Array``. This function must return concrete arrays, meaning that
  ``make_array_from_callback`` has limited compatibility with OLYMPUS transformations
  like :func:`jit` or :func:`vmap`.

  Args:
    shape : Shape of the ``olympus.Array``.
    sharding: A ``Sharding`` instance which describes how the ``olympus.Array`` is
    ...
    dtype: The dtype of the output ``olympus.Array``. If not provided, the dtype of
      the data for the first addressable shard is used. If there are no
      addressable shards, the ``dtype`` argument must be provided.

  Returns:
    A ``olympus.Array`` via data fetched from ``data_callback``.
  ...
  Examples:
    ...
    >>> global_mesh = Mesh(np.array(olympus.devices()).reshape(2, 4), ('x', 'y'))
    >>> inp_sharding = olympus.sharding.NamedSharding(global_mesh, P('x', 'y'))
    ...
    >>> def cb(index):
    ...  return global_input_data[index]
    ...
    >>> arr = olympus.make_array_from_callback(input_shape, inp_sharding, cb)
  ...
  if isinstance(dll, AutoLayout):
    raise TypeError(
        ...
        f" layout when calling `olympus.make_array_from_callback`. Got {sharding}")
  sharding = sharding.sharding if isinstance(sharding, Format) else sharding
  if not isinstance(sharding, Sharding):
    raise TypeError(
        f"sharding should be an instance of `olympus.sharding`. Got {sharding} of"
  ...
  def get_data(
    ...
    if isinstance(r, core.Tracer):
      raise errors.UnexpectedTracerError(
          "olympus.make_array_from_callback cannot be called within a traced"
          " context."
      )
...
def make_array_from_process_local_data(
    sharding,  # PyTree[olympus.sharding.Sharding]
  ...
  `per_device_shape[i] * olympus.local_device_count()`.  Each device will be mapped
  ...
  Examples:
    >>> from olympus.sharding import PartitionSpec as P
    >>> mesh_rows = 2
    >>> mesh_cols =  olympus.device_count() // 2
    ...
    >>> mesh = olympus.sharding.Mesh(np.array(olympus.devices()).reshape(mesh_rows, mesh_cols), ('x', 'y'))
    ...
    >>> output_global_array = olympus.make_array_from_process_local_data(sharding, per_host_data, global_shape)
    ...
...
def make_array_from_single_device_arrays(
  ...
  r"""Returns a ``olympus.Array`` from a sequence of ``olympus.Array``\s each on a single device.
      Every device in input ``sharding``\'s mesh must have an array in ``arrays``\s.

  Args:
    shape : Shape of the output ``olympus.Array``. This conveys information already included with
      ``sharding`` and ``arrays`` and serves as a double check.
    sharding: Sharding: A global Sharding instance which describes how the output olympus.Array is laid out across devices.
    arrays: `list` or `tuple` of ``olympus.Array``\s that are each single device addressable. ``len(arrays)``
      ...
      These arrays are commonly created via ``olympus.device_put``.
    dtype: The dtype of the output ``olympus.Array``. If not provided, the dtype of the first array in
      ``arrays`` is used. If ``arrays`` is empty, the ``dtype`` argument must be provided.

  Returns:
    A global ``olympus.Array``, sharded as ``sharding``, with shape equal to ``shape``, and with per-device
  ...
  Examples:
    ...
    ...
    >>> global_shape = (8, 8)
    >>> mesh = Mesh(np.array(olympus.devices()).reshape(mesh_rows, mesh_cols), ('x', 'y'))
    >>> sharding = olympus.sharding.NamedSharding(mesh, P('x', 'y'))
    ...
    ...
    >>> arr = olympus.make_array_from_single_device_arrays(global_shape, sharding, arrays)
    >>> assert arr.shape == (8,8) # arr.shape is (8,8) regardless of olympus.device_count()

  For cases where you have a local array and want to convert it to a global
  olympus.Array, use ``olympus.make_array_from_process_local_data``.
  """
  ...
  try:
    ...
    if not isinstance(arrays, list):
      raise TypeError("olympus.make_array_from_single_device_arrays `arrays` "
                      "argument must be a list or tuple, but got "
                      f"{type(arrays)}.")
    if any(isinstance(arr, core.Tracer) for arr in arrays):
      raise ValueError(
          "olympus.make_array_from_single_device_arrays requires a list of concrete"
...
def _array_mlir_constant_handler(val, aval):
  try:
    ...
    if 'Fetching value for `olympus.Array` that spans non-addressable' in str(e):
      raise RuntimeError(
          "Closing over olympus.Array that spans non-addressable (non process"
          " local) devices is not allowed. Please pass such arrays as arguments"
          f" to the function. Got olympus.Array: {val.aval.str_short()}") from e
    raise
