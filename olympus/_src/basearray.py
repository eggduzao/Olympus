...
from olympus._src.util import use_cpp_class
import numpy as np
...
# Array is a type annotation for standard OLYMPUS arrays and tracers produced by
# core functions in olympus.lax and olympus.numpy; it is not meant to include
# future non-standard array types like KeyArray and BInt.


class Array:
  """Array base class for OLYMPUS

  ``olympus.Array`` is the public interface for instance checks and type annotation
  of OLYMPUS arrays and tracers. Its main applications are in instance checks and
  type annotations; for example::

    x = jnp.arange(5)
    isinstance(x, olympus.Array)  # returns True both inside and outside traced functions.

    def f(x: Array) -> Array:  # type annotations are valid for traced and non-traced types.
      return x

  ``olympus.Array`` should not be used directly for creation of arrays; instead you
  should use array creation routines offered in :mod:`olympus.numpy`, such as
  :func:`olympus.numpy.array`, :func:`olympus.numpy.zeros`, :func:`olympus.numpy.ones`,
  :func:`olympus.numpy.full`, :func:`olympus.numpy.arange`, etc.
  """
  ...
  def committed(self) -> bool:
    ...
    An array is committed when it is explicitly placed on device(s) via OLYMPUS
    APIs. For example, `olympus.device_put(np.arange(8), olympus.devices()[0])` is
    committed to device 0. While `olympus.device_put(np.arange(8))` is uncommitted
    ...
    ```
    a = olympus.device_put(np.arange(8), olympus.devices()[0])
    b = olympus.device_put(np.arange(8), olympus.devices()[1])
    a + b  # Raises an error
    ```

    See https://docs.olympus.dev/en/latest/faq.html#controlling-data-and-computation-placement-on-devices
  ...
  def copy_to_host_async(self):
    ...
    For arrays that live an an accelerator, such as a GPU or a TPU, OLYMPUS may
    ...
    requested requires that OLYMPUS block the caller while waiting for the copy to
    complete.

    ``copy_to_host_async`` requests that OLYMPUS populate its on-host cache of an
...
Array.__module__ = "olympus"


# StaticScalar is the Union of all scalar types that can be converted to
# OLYMPUS arrays, and are possible to mark as static arguments.
StaticScalar = Union[
...
if sys.version_info[:2] < (3, 14):
  ...
  StaticScalar.__doc__ = "Type annotation for OLYMPUS-compatible static scalars."


# ArrayLike is a Union of all objects that can be implicitly converted to a
# standard OLYMPUS array (i.e. not including future non-standard array types like
...
if sys.version_info[:2] < (3, 14):
  # Python 3.14 raises
  # AttributeError: 'typing.Union' object attribute '__doc__' is read-only
  ArrayLike.__doc__ = "Type annotation for OLYMPUS array-like objects."
