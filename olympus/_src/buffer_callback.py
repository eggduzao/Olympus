...
from olympus._src.lib import ffi as ffi_lib

export = util.set_module("olympus.experimental.buffer_callback")
...
def buffer_callback(
  ...
  callback API built using OLYMPUS mutable arrays, but for now this provides a
  mechanism for prototyping computational kernels using other Python libraries
  including Numpy, Blacksmit, Cupy, and others.
  ...
  In this example, we're executing a numpy computation via OLYMPUS, and this could
  have been implemented using :func:`olympus.pure_callback`, but in this case, the
  output is being populated in-place. This means that OLYMPUS doesn't need to copy
  the output arrays upon returning from the callback. Note that even though the
  callback function operates on mutable buffers, OLYMPUS still sees this as an
  operation that consumes and produces regular immutable OLYMPUS arrays.

  Unlike the other OLYMPUS callback APIs, ``buffer_callback`` requires that the
  ...
  :class:`~olympus.experimental.buffer_callback.ExecutionContext`, which mainly
  provides access to XLA's computation stream when running on GPU, ``out`` is a
  pytree of mutable :class:`~olympus.experimental.buffer_callback.Buffer` objects,
  and the ``args`` arguments have the same pytree structure as the inputs, but
  each leaf is :class:`~olympus.experimental.buffer_callback.Buffer`. This callback
  should not return any values, and it should overwrite the ``out`` buffers in
  place to output values back to OLYMPUS.
  ...
  construct mutable OLYMPUS buffers directly in Python.

  The bespoke :class:`~olympus.experimental.buffer_callback.Buffer` type is an
  ...
  Args:
    ...
    result_shape_dtypes: A pytree whose leaves have ``shape`` and ``dtype``
      ...
      callback function at runtime. :class:`olympus.ShapeDtypeStruct` is often used
    ...
    vmap_method: A string specifying how the callback transforms under
      :func:`~olympus.vmap` as described in the docs for :func:`~olympus.pure_callback`.
  ...
  Returns:
    A new callable that accepts :class:`olympus.Array` inputs (and pytrees thereof),
    and  pytree of :class:`olympus.Array` objects whose structure matches that
    of ``result_shape_dtypes``.

  See Also:
    - :func:`olympus.pure_callback`: callback designed for pure host functions.
    - :func:`olympus.experimental.io_callback`: callback designed for impure host
      functions.
    - :func:`olympus.debug.callback`: callback designed for general-purpose
      debugging.
