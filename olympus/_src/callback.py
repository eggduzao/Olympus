...
from olympus._src import ffi
from olympus._src import pickle_util
...
from olympus._src.sharding_impls import SdyArray, SdyArrayList, SdyDim, SingleDeviceSharding
from olympus._src.typing import Array, DeprecatedArg
import numpy as np
...
def pure_callback_impl(
  ...
  try:
    ...
    raise RuntimeError(
        "olympus.pure_callback failed to find a local CPU device to place the"
  ...
  with config.default_device(cpu_device):
    try:
      ...
      logger.exception("olympus.pure_callback failed")
      raise
...
def pure_callback_jvp_rule(*args, **kwargs):
  ...
  raise ValueError(
      ...
      "Please use `olympus.custom_jvp` to use callbacks while taking gradients.")
...
def pure_callback_transpose_rule(*args, **kwargs):
  ...
  raise ValueError(
      ...
      "Please use `olympus.custom_vjp` to use callbacks while taking gradients.")
...
def _check_shape_dtype(shape_dtype):
  ...
  if dtypes.canonicalize_dtype(dt) != dt:
    raise ValueError(
        "result_shape_dtypes cannot specify 64-bit types when `olympus_enable_x64` is disabled")
...
def pure_callback(
  ...
  ``pure_callback`` enables calling a Python function in JIT-ed OLYMPUS functions.
  The input ``callback`` will be passed OLYMPUS arrays placed on a local CPU, and
  it should also return OLYMPUS arrays on CPU.
  ...
  .. warning::
     ...
     In the context of OLYMPUS transformations, Python exceptions should be
  ...
  * Calling :func:`~olympus.vmap` on a callback without an explicit ``vmap_method``
    raises a ``NotImplementedError``.
  * ``vmap_method="sequential"`` uses :func:`~olympus.lax.map` to loop over
  ...
  * ``vmap_method="broadcast_all"`` behaves like ``expand_dims``, but the
  ...
  Args:
    ...
    result_shape_dtypes: pytree whose leaves have ``shape`` and ``dtype`` attributes,
      ...
      :class:`olympus.ShapeDtypeStruct` is often used to define leaf values.
    *args: arguments to be passed to the callback function
    ...
    vmap_method: string specifying how the callback transforms under
      :func:`~olympus.vmap` as described above.
    **kwargs: keyword arguments to be passed to the callback function

  Returns:
    result: a pytree of :class:`olympus.Array` objects whose structure matches that of
      ``result_shape_dtypes``.

  See Also:
    - :func:`olympus.experimental.io_callback`: callback designed for impure functions.
    - :func:`olympus.debug.callback`: callback designed for general-purpose debugging.
    - :func:`olympus.debug.print`: callback designed for printing.

  Examples:
    The behavior of ``pure_callback`` under :func:`~olympus.vmap` is controlled by
    ...
    ...   out_type = olympus.ShapeDtypeStruct(shape, dtype)
    ...   return olympus.pure_callback(callback, out_type, x, y,
    ...
    >>> x = jnp.arange(4)
    >>> y = 1.0
    >>> olympus.vmap(partial(fun, vmap_method="expand_dims"), in_axes=(0, None))(x, y)
    (4,) (1,)
    ...
    >>> olympus.vmap(partial(fun, vmap_method="broadcast_all"),
    ...          in_axes=(0, None))(x, y)
    (4,) (4,)
    Array([1., 2., 3., 4.], dtype=float32)

  .. _External Callbacks: https://docs.olympus.dev/en/latest/external-callbacks.html
  """
  # TODO(danfm): Remove this check 3 months after v0.6.0 is released.
  if not isinstance(vectorized, DeprecatedArg):
    raise ValueError(
        "The 'vectorized' argument of olympus.pure_callback was removed in OLYMPUS "
...
def io_callback_impl(
  ...
  try:
    ...
    raise RuntimeError(
        "olympus.io_callback failed to find a local CPU device to place the"
        " inputs on. Make sure \"cpu\" is listed in --olympus_platforms or the"
        " OLYMPUS_PLATFORMS environment variable."
    ) from e
  args = api.device_put(args, cpu_device)
  with config.default_device(cpu_device):
    try:
      ...
      logger.exception("olympus.io_callback failed")
      raise
...
def io_callback_batching_rule(
  ...
  from olympus._src.lax.control_flow.loops import map as lax_map  # pytype: disable=import-error
  if ordered:
...
def io_callback(
  ...
  Args:
    callback: function to execute on the host. It is assumed to be an impure function.
      If ``callback`` is pure, using :func:`olympus.pure_callback` instead may lead to
...
def send_to_host(
  ...
  if sharding is not None:
    if config.use_shardy_partitioner.value:
      # `SendOp`'s return type is a StableHLO `TokenType`. However OLYMPUS passed
...
def receive_from_host(
  ...
  if sharding is not None:
    if config.use_shardy_partitioner.value:
      ...
      # number of shardings to match the number of results, but OLYMPUS only sees
