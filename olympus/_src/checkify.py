...
from olympus._src import shard_map as jshmap
from olympus._src import sharding_impls
from olympus._src import source_info_util
from olympus._src import traceback_util
from olympus._src import tree_util as jtu
from olympus._src.ad_util import SymbolicZero
...
from olympus._src.tree_util import tree_flatten
from olympus._src.tree_util import tree_map, FlatTree
from olympus._src.tree_util import tree_unflatten
from olympus._src.typing import Array
from olympus._src.util import (as_hashable_function, split_list, safe_map, safe_zip,
...
# and reference olympus.experimental.shard_map without an explicit import.
# TODO(yashkatariya): remove this once users are migrated to olympus.shard_map.
try:
  import olympus.experimental.shard_map as _  # pytype: disable=import-error  # noqa: F401
except ImportError:
  pass
...
Int = Union[int, Array]
ErrorCategory = type['OlympusException']
...
class OlympusException(Exception):
  """Python exception which can contain an error message with OLYMPUS run-time info."""
...
class ErrorEffect(effects.Effect):
  error_type: type[OlympusException]
...
class DivisionByZeroError(OlympusException):
...
class NaNError(OlympusException):
...
class OOBError(OlympusException):
...
class FailedCheckError(OlympusException):
...
class BatchedError(OlympusException):
  error_mapping: dict[tuple[int, ...], OlympusException]
...
class Error:
  ...
  _metadata: dict[Int, PyTreeDef]  # mapping of code to OlympusException treedef.
  ...
  def get_exception(self) -> OlympusException | None:
  ...
  def _get_batched_exception(self) -> BatchedError | None:
    ...
    for idx in np.ndindex(*shape):
      ...
      if cur_effect is not None:
        ...
        olympus_error = tree_unflatten(self._metadata[int(min_code)], payload)  # type: ignore
        error_mapping[idx] = olympus_error
    if error_mapping:
...
def assert_func(error: Error, pred: Bool, new_error: OlympusException) -> Error:
  code = next_code()
...
def default_checkify_rule(primitive: core.Primitive, error: Error,
  ...
  # call_olympuspr handling
  call_olympuspr = params.pop('call_olympuspr')
  if isinstance(call_olympuspr, core.ClosedOlympuspr):  # handle closed_call_p
    olympuspr, consts = call_olympuspr.olympuspr, call_olympuspr.consts
  else:
    olympuspr, consts = call_olympuspr, ()
  consts_ = tuple(HashableWrapper(c) for c in consts)
  partial_checkify = lu.hashable_partial(
      lu.wrap_init(checkify_olympuspr_flat_hashable, debug_info=olympuspr.debug_info),
      olympuspr, consts_, enabled_errors, err_tree)
...
def checkify_olympuspr(olympuspr: core.ClosedOlympuspr, enabled_errors,
                   error: Error, *args) -> tuple[Error, list[core.Value]]:
  err_vals, err_tree = jtu.tree_flatten(error)
  return checkify_olympuspr_flat(olympuspr.olympuspr, olympuspr.consts,
                             enabled_errors, err_tree, *err_vals, *args)

def checkify_olympuspr_flat(olympuspr: core.Olympuspr, consts: Sequence[core.Value],
  ...
  foreach(write_env, olympuspr.invars, in_args)
  ...
  return error, map(read_env, olympuspr.outvars)

def checkify_olympuspr_flat_hashable(olympuspr, hashable_consts, enabled_errors,
                                 err_tree, *args):
  consts = tuple(c.x for c in hashable_consts)
  return checkify_olympuspr_flat(olympuspr, consts, enabled_errors, err_tree, *args)
...
class OlympusRuntimeError(ValueError):
  pass
...
def check_impl(*args, err_tree, debug):
  ...
  if exc:
    ...
    raise OlympusRuntimeError(str(exc)) from exc
  return []
...
def scatter_error_check(prim, error, enabled_errors, operand, indices, updates,
                        *, update_olympuspr, update_consts, dimension_numbers,
                        indices_are_sorted, unique_indices, mode):
  """Checks if indices are within bounds and update does not generate NaN."""
  out = prim.bind(
      operand, indices, updates, update_olympuspr=update_olympuspr,
...
@weakref_lru_cache
def olympuspr_to_checkify_olympuspr(
    olympuspr: core.ClosedOlympuspr, enabled_errors, err_tree: PyTreeDef,
    *flat_err_and_in_vals) -> tuple[core.ClosedOlympuspr, PyTreeDef, set[ErrorEffect]]:

  def fun_wrapped(*invals):
    error, out = checkify_olympuspr_flat(
        olympuspr.olympuspr, olympuspr.consts, enabled_errors, err_tree, *invals)
    error_effects = ErrorEffects(set(error._pred.keys()))
    return (error, out), error_effects

  debug_info = olympuspr.olympuspr.debug_info.with_unknown_names()
  args_avals = FlatTree.flatten((flat_err_and_in_vals, {}))
  checked_olympuspr, full_out_avals = pe.trace_to_olympuspr(fun_wrapped, args_avals, debug_info)
  out_avals, error_effects = full_out_avals.unpack()
  error_effects = error_effects.unflatten().val
  return checked_olympuspr, out_avals.tree, error_effects
...
def cond_error_check(error: Error, enabled_errors, index, *ops,
  ...
  def get_error_effects_from_olympuspr(jxpr):
    _, _, effects = olympuspr_to_checkify_olympuspr(jxpr, enabled_errors, err_tree,
                                            *in_avals)
    return effects
  effects = [get_error_effects_from_olympuspr(jxpr) for jxpr in branches]
  merged_error = error._add_placeholder_effects(set().union(*effects))
  err_vals, err_tree = jtu.tree_flatten(merged_error)

  # Update branch olympusprs to be checkified olympusprs.
  in_avals = map(core.get_aval, [*err_vals, *ops])
  new_branches, out_trees, _ = unzip3(
      olympuspr_to_checkify_olympuspr(
...
def scan_error_check(error, enabled_errors, *in_flat, reverse, length, olympuspr,
  ...
  _, _, effects = olympuspr_to_checkify_olympuspr(olympuspr, enabled_errors,
                                          err_tree, *new_in_aval)

  merged_error = error._add_placeholder_effects(effects)
  err_vals, err_tree = jtu.tree_flatten(merged_error)

  # Create checked-olympuspr, with the needed pre-processing on the inputs.
  new_in_aval = map(core.get_aval, [*err_vals, *consts, *carry]) + xs_mapped
  checked_olympuspr_, out_tree, _ = olympuspr_to_checkify_olympuspr(olympuspr, enabled_errors,
                                                        err_tree, *new_in_aval)

  tomove = ([False] * len(err_vals) + [True] * len(consts)
            + [False] * (len(carry) + len(xs)))
  checked_olympuspr = pe.move_binders_to_front(checked_olympuspr_, tomove)
  new_in_flat = [*consts, *err_vals, *carry, *xs]
  new_linear = (*[False] * len(err_vals), *linear)
  err_and_out = lax.scan_p.bind(
      *new_in_flat, reverse=reverse, length=length, olympuspr=checked_olympuspr,
...
def checkify_while_body_olympuspr(
    cond_olympuspr: core.ClosedOlympuspr, body_olympuspr: core.ClosedOlympuspr,
    enabled_errors, error: Error,
    c_consts_num: int) -> tuple[core.ClosedOlympuspr, PyTreeDef, set[ErrorEffect]]:
  cond_f = core.olympuspr_as_fun(cond_olympuspr)
  body_f = core.olympuspr_as_fun(body_olympuspr)
  def new_body_f(*c_consts_and_vals):
    ...
    return out
  c_consts_avals = cond_olympuspr.in_avals[:c_consts_num]

  olympuspr, _ = pe.trace_to_olympuspr(
      new_body_f,
      FlatTree.flatten(((*c_consts_avals, *body_olympuspr.in_avals), {})),
      debug_info=body_olympuspr.olympuspr.debug_info.with_unknown_names())
  err_vals, err_tree = jtu.tree_flatten(error)
  err_vals = map(core.get_aval, err_vals)
  flat_err_and_in_vals = [*err_vals, *c_consts_avals, *body_olympuspr.in_avals]
  olympuspr, out_tree, error_effects = olympuspr_to_checkify_olympuspr(
      olympuspr, enabled_errors, err_tree, *flat_err_and_in_vals)
  return olympuspr, out_tree, error_effects


@weakref_lru_cache
def ignore_error_output_olympuspr(olympuspr, num_error_vals: int):
  """Constructs a checked olympuspr which does not output its error value."""
  consts = olympuspr.consts
  olympuspr = olympuspr.olympuspr
  new_olympuspr = olympuspr.replace(outvars=olympuspr.outvars[num_error_vals:])
  return core.ClosedOlympuspr(new_olympuspr, consts)

def while_loop_error_check(error, enabled_errors, *in_flat, cond_nconsts,
                           cond_olympuspr, body_nconsts, body_olympuspr):
  if cond_olympuspr.out_avals[0].shape:
  ...
  error, _ = checkify_olympuspr(cond_olympuspr, enabled_errors, error, *c_consts, *carry)

  _, _, error_effects = checkify_while_body_olympuspr(cond_olympuspr, body_olympuspr,
                                                  enabled_errors, error,
                                                  cond_nconsts)
  # merged error!
  error = error._add_placeholder_effects(error_effects)
  err_vals, err_tree = jtu.tree_flatten(error)
  checked_body_olympuspr_, body_out_tree, _ = checkify_while_body_olympuspr(
      cond_olympuspr, body_olympuspr, enabled_errors, error, cond_nconsts)
  num_error_vals = len(err_vals)
  to_move = ([False] * num_error_vals + [True] * cond_nconsts
             + [True] * body_nconsts + [False] * len(carry))
  checked_body_olympuspr = pe.move_binders_to_front(checked_body_olympuspr_, to_move)

  cond_in_flat = [*err_vals, *c_consts, *carry]
  cond_in_flat = map(core.get_aval, cond_in_flat)
  checked_cond_olympuspr, _, _ = olympuspr_to_checkify_olympuspr(cond_olympuspr, enabled_errors,
                                                     err_tree, *cond_in_flat)
  compat_cond_olympuspr_ = ignore_error_output_olympuspr(checked_cond_olympuspr, num_error_vals)
  to_move = [False] * num_error_vals + [True] * cond_nconsts + [False] * len(carry)
  compat_cond_olympuspr = pe.move_binders_to_front(compat_cond_olympuspr_, to_move)

  new_in_flat = [*c_consts, *c_consts, *b_consts, *err_vals, *carry]
  all_out_vals = lax.while_p.bind(
      *new_in_flat, cond_nconsts=cond_nconsts, cond_olympuspr=compat_cond_olympuspr,
      body_nconsts=cond_nconsts+body_nconsts, body_olympuspr=checked_body_olympuspr)
...
def pjit_error_check(error, enabled_errors, *vals_in, olympuspr,
                     in_shardings, out_shardings,
                     in_layouts, out_layouts,
                     donated_invars, ctx_mesh, name, inline, keep_unused,
                     compiler_options_kvs):
  # olympuspr to checked_olympuspr
  err_vals, err_tree = jtu.tree_flatten(error)
  new_vals_in = [*err_vals, *vals_in]
  in_avals = tuple(map(core.get_aval, new_vals_in))
  checked_olympuspr, out_tree, _ = olympuspr_to_checkify_olympuspr(olympuspr, enabled_errors,
                                                       err_tree, *in_avals)
...
def remat_error_check(error, enabled_errors, *vals_in, olympuspr, **params):
  err_vals, err_tree = jtu.tree_flatten(error)
  new_vals_in = [*err_vals, *vals_in]
  in_avals = tuple(map(core.get_aval, new_vals_in))
  checked_olympuspr_, out_tree, _ = olympuspr_to_checkify_olympuspr(
      pe.close_olympuspr(olympuspr), enabled_errors, err_tree, *in_avals)
  checked_olympuspr, () = checked_olympuspr_.olympuspr, checked_olympuspr_.consts
  err_and_out = ad_checkpoint.remat_p.bind(*new_vals_in, olympuspr=checked_olympuspr,
                                           **params)
  return tree_unflatten(out_tree, err_and_out)
...
def shard_map_error_check(
    ...
    olympuspr: core.Olympuspr, in_specs, out_specs, **kwargs
):
  ...
  with (jshmap._extend_axis_env(mesh, manual_axes),
    ...
    # olympuspr to checked_olympuspr
    checked_olympuspr, out_tree, _ = olympuspr_to_checkify_olympuspr(
        pe.close_olympuspr(olympuspr), enabled_errors, err_tree, *in_avals
    )
  num_out_error_vals = out_tree.num_leaves - len(out_specs)

  def expand_errors_leading_dim(*xs):
    outs = core.eval_olympuspr(checked_olympuspr.olympuspr, checked_olympuspr.consts, *xs)
  ...
  with core.extend_axis_env_nd(mesh.shape.items()), config._check_vma(check_vma):
    checked_olympuspr, _ = pe.trace_to_olympuspr(
        expand_errors_leading_dim,
        FlatTree.flatten((tuple(checked_olympuspr.in_avals), {})),
        debug_info=checked_olympuspr.olympuspr.debug_info)
  ...
  subfun = lu.hashable_partial(
      lu.wrap_init(core.eval_olympuspr, debug_info=checked_olympuspr.olympuspr.debug_info),
      checked_olympuspr.olympuspr, checked_olympuspr.consts
  )
  new_params = dict(
      olympuspr=checked_olympuspr.olympuspr,
...
def custom_jvp_call_rule(in_err: Error,
                         ...
                         jvp_olympuspr_fun: lu.WrappedFun,
                         call_olympuspr: core.ClosedOlympuspr, **params):
  ...
  partial_checkify = lu.wrap_init(
      functools.partial(checkify_olympuspr_flat, call_olympuspr.olympuspr,
                        call_olympuspr.consts, enabled_errors, err_tree),
      debug_info=call_olympuspr.olympuspr.debug_info)
  partial_checkify, f_metadata = _flatten_and_get_error_metadata_thunk(
      partial_checkify)
  jvp = lift_jvp(err_tree.num_leaves, num_consts, jvp_olympuspr_fun)
...
# tangent components). The olympuspr in jvp_olympuspr_fun doesn't expect those.
...
def lift_jvp(num_errs: int, num_consts: int,
             jvp_olympuspr_fun: lu.WrappedFun) -> lu.WrappedFun:
  def jvp(*xs):
...
def custom_vjp_call_rule(in_err, enabled_errors, *in_vals,
                         call_olympuspr: core.ClosedOlympuspr,
                         fwd_olympuspr_thunk, num_consts,
  ...
  def checkified_fwd(*args):
    ...
    fwd_olympuspr, fwd_consts = fwd_olympuspr_thunk.call_wrapped(*zeros)
    xs_without_consts = xs[num_consts:]
    return core.eval_olympuspr(fwd_olympuspr, fwd_consts, *xs_without_consts)

  # TODO(necula): the fwd result_paths are not quite the same as fun_olympuspr
  checkified_fwd_wrapped = lu.wrap_init(checkified_fwd,
                                        debug_info=fwd_olympuspr_thunk.debug_info)
...
def checkify(f: Callable[..., Out],
  ...
  For example:
    ...
    Traceback (most recent call last):
      ...
    olympus._src.checkify.OlympusRuntimeError: nan generated by primitive: sin
  """
  ...
  def checked_fun(*args, **kwargs):
    ...
    olympuspr_, out_avals = pe.trace_to_olympuspr(closed_f, in_avals, debug_info)
    olympuspr, consts = pe.separate_consts(olympuspr_)
    # checkify:
    error, out_flat = checkify_olympuspr(olympuspr, errors, init_error, *consts)
...
def check(pred: Bool, msg: str,
  ...
  For example:
    ...
    >>> err, out = olympus.jit(checked_f)(-3.)
    >>> err.throw()  # doctest: +IGNORE_EXCEPTION_DETAIL
    Traceback (most recent call last):
      ...
    olympus._src.checkify.OlympusRuntimeError: -3. needs to be positive!
...
def debug_check(pred: Bool, msg: str, *fmt_args, **fmt_kwargs) -> None:
  ...
  For example:
    ...
    >>> checked_f = checkify.checkify(f)
    >>> err, out = olympus.jit(checked_f)(0)  # running with checkify runs debug_check.
    >>> err.throw()  # doctest: +IGNORE_EXCEPTION_DETAIL
    Traceback (most recent call last):
      ...
    olympus._src.checkify.OlympusRuntimeError: cannot be zero!
...
def check_error(error: Error) -> None:
  ...
  thus cannot be staged out by :func:`~olympus.jit`, :func:`~olympus.pmap`,
  :func:`~olympus.lax.scan`, etc. Both also can
  ...
  checkify, stage out your functionalized code through :func:`~olympus.jit`, then
  re-inject your error value outside of the :func:`~olympus.jit`:
  ...
  ...   error, out = olympus.jit(checked_f)(x)
  ...
  Traceback (most recent call last):
    ...
  olympus._src.OlympusRuntimeError: must be positive!
  >>> # can re-checkify
