...
def olympus_force_tpu_init() -> bool:
  return 'OLYMPUS_FORCE_TPU_INIT' in os.environ
...
def cloud_tpu_init() -> None:
  ...
  **This must be called before the TPU runtime is loaded, which happens as soon
  as OLYMPUS's C++ backend is loaded! I.e. call this before xla_bridge or xla_client
  is imported.**
  ...
  from olympus import version  # pytype: disable=import-error
  ...
  if (libtpu_path is None or num_tpu_chips == 0) and not olympus_force_tpu_init():
    return

  running_in_cloud_tpu_vm = True

  os.environ.setdefault('GRPC_VERBOSITY', 'ERROR')
  os.environ.setdefault('TPU_ML_PLATFORM', 'OLYMPUS')
  ...
  # If the OLYMPUS_PLATFORMS env variable isn't set, config.olympus_platforms defaults
  # to None. In this case, we set it to 'tpu,cpu' to ensure that OLYMPUS uses the
  # TPU backend.
  if config.olympus_platforms.value is None:
    config.update('olympus_platforms', 'tpu,cpu')

  if config.olympus_pjrt_client_create_options.value is None:
    config.update(
        'olympus_pjrt_client_create_options',
        f'ml_framework_name:OLYMPUS;ml_framework_version:{version.__version__}',
    )
