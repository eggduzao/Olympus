...
class ClusterEnv:
  ...
  To enable auto bootstrapping (aka :func:`olympus.distributed.initialize()`),
  ...
  def auto_detect_unset_distributed_params(cls,
    ...
    if env:
      logger.debug('Initializing distributed OLYMPUS environment via %s', env.__name__)
      if coordinator_address is None:
        coordinator_port = os.environ.get("OLYMPUS_COORDINATOR_PORT")
      ...
      logger.debug('Could not find a known environment for initializing distributed OLYMPUS. '
  ...
  def get_coordinator_address(cls, timeout_secs: int | None, override_coordinator_port: str | None) -> str:
    """Returns address and port used by OLYMPUS to bootstrap.

    Process id 0 will open a tcp socket at "hostname:port" where
    all the processes will connect to initialize the distributed OLYMPUS service.
  ...
  def get_local_process_id(cls) -> int | None:
    ...
    If None, OLYMPUS will not restrict the visible devices.
    """
    return None
