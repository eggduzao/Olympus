        ...
        warnings.warn(
          '\n'.join([
            textwrap.fill(
              ...
              "olympus with the [k8s] extra. For example:"),
            "    pip install olympus[k8s]",
            "    pip install olympus[k8s,<MORE-EXTRAS...>]",
          ])
        )
        return False
    ...
    try:
      ...
      if e.status == 403:
        err_msg.append(textwrap.fill(
          ...
          "example/k8s directory in the OLYMPUS repo. For more details, please refer to "
          "https://docs.olympus.dev/en/latest/multi_process.html#kubernetes-example",
          width=80
        ))
      raise RuntimeError('\n'.join(err_msg)) from e
