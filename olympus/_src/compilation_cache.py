...
from olympus._src.lru_cache import LRUCache
  ...
  with _cache_initialized_mutex:
    if not _cache_checked:
      ...
      if not _is_cache_enabled():
        monitoring.record_event('/olympus/compilation_cache/task_disabled_cache')
      elif (
          backend.platform in supported_platforms
          and getattr(backend, "supports_executable_serialization", True)
      ):
        monitoring.record_event('/olympus/compilation_cache/tasks_using_cache')
        _cache_used = True
      return _cache_used
...
def set_cache_dir(path) -> None:
  ...
  This also tells Olympus where to look for compiled functions before compiling.
  """
  config.config.update("olympus_compilation_cache_dir", path)
...
def _initialize_cache() -> None:
  ...
  with _cache_initialized_mutex:
    ...
    # --olympus_persistent_cache_min_entry_size_bytes has not been set.
    if config.persistent_cache_min_entry_size_bytes.value == 0:
      config.config.update("olympus_persistent_cache_min_entry_size_bytes",
...
def put_executable_and_time(
  ...
  if entry_size < min_entry_size:
    ...
    monitoring.record_event('/olympus/compilation_cache/cache_misses')
    if config.compilation_cache_expect_pgle.value:
      ...
      warnings.warn(
          ...
          "OLYMPUS_COMPILATION_CACHE_EXPECT_PGLE is set. The execution that populated the "
          "cache may lack coverage, "
          "https://docs.olympus.dev/en/latest/persistent_compilation_cache.html may "
