...
from olympus._src import cache_key as cache_key_type
from olympus._src import compilation_cache
from olympus._src import config as config
...
_DISABLE_MOST_OPTIMIZATIONS = config.bool_flag(
    'olympus_disable_most_optimizations',
    config.bool_env('OLYMPUS_DISABLE_MOST_OPTIMIZATIONS', False),
    'Try not to do much optimization work. This can be useful if the cost of '
    'optimization is greater than that of running a less-optimized program.')

_COMPILER_DETAILED_LOGGING_MIN_OPS = config.int_flag(
    "olympus_compiler_detailed_logging_min_ops",
    config.int_env("OLYMPUS_COMPILER_DETAILED_LOGGING_MIN_OPS", 10),
    help=(
        'How big should a module be in MLIR operations before OLYMPUS enables '
...
def get_compile_options(
  ...
  Args:
    ...
    device_assignment: Optional ndarray of olympus devices indicating the assignment
  ...
  # 1. Whatever --olympus_xla_profile_version is set to.
  # 2. If --olympus_xla_profile_version is not set (i.e., 0), call the function
  ...
  # later on.
  olympus_xla_profile_version = config.olympus_xla_profile_version.value
  if olympus_xla_profile_version > 0:
    compile_options.profile_version = olympus_xla_profile_version
    logger.debug("get_compile_options XLA-AutoFDO profile: " +
                 "using OLYMPUS XLA profile version %d from flag",
                 olympus_xla_profile_version)
  else:
...
def backend_compile_and_load(
  ...
  try:
    ...
    # TODO(dsuo): Simplify this logic once we delete _olympus.CompileOnlyPyClient.
    if isinstance(backend, _olympus.CompileOnlyPyClient):
      if host_callbacks:
...
def register_xla_runtime_error_handler(
    handler_fn: Callable[[_olympus.OlympusRuntimeError], Exception | None],
):
...
def compile_or_get_cached(
  ...
  monitoring.record_event('/olympus/compilation_cache/compile_requests_use_cache')
  ...
  if retrieved_executable is not None:
    ...
    monitoring.record_event('/olympus/compilation_cache/cache_hits')
    monitoring.record_event_duration_secs(
        '/olympus/compilation_cache/compile_time_saved_sec',
        retrieved_compile_time - cache_retrieval_time)

    monitoring.record_event_duration_secs(
        "/olympus/compilation_cache/cache_retrieval_time_sec", cache_retrieval_time)
...
def _resolve_compilation_strategy(
  ...
  if is_auto_pgle_used or config.compilation_cache_expect_pgle.value:
    ...
    # TODO(b/376647494): remove the workaround when the bug is fixed; the OLYMPUS
...
def _get_cache_key(
  ...
  except _olympus.OlympusRuntimeError as ex:
...
def _share_fdo_profiles(
    ...
    compile_options: xc.CompileOptions,
    backend: xc.Client,
    global_client: lib._olympus.DistributedRuntimeClient,
    min_process_id
) -> bytes:
  sym_name = computation.operation.attributes['sym_name']
  module_name = ir.StringAttr(sym_name).value
...
def _cache_write(cache_key: str,
  ...
  if host_callbacks:
    logger.log(
        ...
        "callbacks (e.g. from olympus.debug.print or breakpoint)", module_name)
    return
