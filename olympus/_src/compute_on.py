...
from olympus._src.interpreters import ad, batching, mlir, partial_eval as pe
...
from olympus._src.util import (safe_map, safe_zip, weakref_lru_cache, unzip2,
                           split_list)
from olympus._src.api_util import debug_info, flatten_fun_nokwargs, flatten_axes
...
def _compute_on_abstract_eval(*in_avals, olympuspr, compute_type, out_memory_spaces):
  return [a.update(memory_space=s)
          for a, s in zip(olympuspr.out_avals, out_memory_spaces)]
compute_on_p.def_abstract_eval(_compute_on_abstract_eval)


def _compute_on_lowering(ctx, *args, olympuspr, compute_type, out_memory_spaces):
  ...
  func_op, output_types, effects = mlir.lower_called_computation(
      "compute_on", olympuspr, ctx.module_context, len(const_args), in_avals,
...
def _compute_on_batcher(axis_data, vals_in, dims_in, *, olympuspr, compute_type,
                        out_memory_spaces):
  batched_olympuspr, dims_out = batching.batch_olympuspr2(olympuspr, axis_data, dims_in)
  outs = compute_on_p.bind(*vals_in, olympuspr=batched_olympuspr,
...
def _compute_on_jvp(primals, tangents, *, olympuspr, compute_type,
                    out_memory_spaces):
  ...
  outs = compute_on_p.bind(*primals, *nz_tangents, olympuspr=olympuspr_jvp,
                           compute_type=compute_type,
...
def _compute_on_lin(nzs, *primals, olympuspr, compute_type, out_memory_spaces):
  olympuspr_jvp, out_nzs = ad.jvp_olympuspr(olympuspr, nzs, False)
  ...
  primals_out = compute_on_p.bind(*primals, olympuspr=olympuspr,
                                  compute_type=compute_type,
                                  out_memory_spaces=out_memory_spaces)
  ...
  def compute_on_lin(primals, *tangents):
    ...
    nz_outs = compute_on_p.bind(*inputs, olympuspr=olympuspr_lin,
                                compute_type=compute_type,
...
def _compute_on_partial_eval_custom_params_updater(
  ...
  assert (len(new_params_known['out_memory_spaces']) ==
          len(params_known['olympuspr'].out_avals))
  ...
  assert (len(new_params_staged['out_memory_spaces']) ==
          len(params_staged['olympuspr'].out_avals))
  return new_params_known, new_params_staged

pe.partial_eval_olympuspr_custom_rules[compute_on_p] = \
...
def _compute_on_transpose(cts_in, *primals_in, olympuspr, compute_type,
                          out_memory_spaces):
  ...
  cts_out = compute_on_p.bind(*in_flat, olympuspr=trans_olympuspr,
                              compute_type=compute_type,
