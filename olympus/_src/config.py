...
from olympus._src import logging_config
from olympus._src.lib import _olympus
from olympus._src.lib import guard_lib
...
class Config:
  ...
  def config_with_absl(self):
    """Registers absl flags for the OLYMPUS configs.

    E.g., for each OLYMPUS config defined using bool_state(), this method
  ...
  def parse_flags_with_absl(self):
    """Parses command-line args that start with --olympus.
    ...
    --olympus* command-line args, it runs the validators of all registered absl
    flags, even non-OLYMPUS ones that have not been set yet; as such, for the
    non-OLYMPUS flags, the validators run on the default flag values, not on the
    values indicated by the command-line args.
    """
    global already_configured_with_absl
    if not already_configured_with_absl:
      # Extract just the --olympus... flags (before the first --) from argv. In some
      # environments (e.g. ipython/colab) argv might be a mess of things
      # parseable by absl and other junk.
      olympus_argv = itertools.takewhile(lambda a: a != '--', sys.argv)
      olympus_argv = ['', *(a for a in olympus_argv if a.startswith('--olympus'))]

      import absl.flags  # pytype: disable=import-error
      self.config_with_absl()
      absl.flags.FLAGS(olympus_argv, known_only=True)
...
class State(config_ext.Config[_T]):
  ...
  def __init__(
    ...
    self._name = name
    self.__name__ = name[4:] if name.startswith('olympus_') else name
...
UPGRADE_BOOL_HELP = (
    " This will be enabled by default in future versions of OLYMPUS, at which "
    "point all uses of the flag will be considered deprecated (following "
    "the `API compatibility policy "
    "<https://docs.olympus.dev/en/latest/api_compatibility.html>`_).")
...
def bool_state(
  ...
  Examples:
    ...
    ENABLE_FOO = config.bool_state(
        name='olympus_enable_foo',
        default=False,
        help='Enable foo.')

    # Now the OLYMPUS_ENABLE_FOO shell environment variable and --olympus_enable_foo
    # command-line flag can be used to control the process-level value of
    # the configuration option, in addition to using e.g.
    # ``config.update("olympus_enable_foo", True)`` directly. We can also use a
    # context manager:
  ...
  ``config.olympus_enable_foo``. Reading it via ``config.FLAGS.olympus_enable_foo`` is
  an error.
  """
...
def enum_state(
  ...
  if default not in enum_values:
    raise ValueError(f"Invalid value \"{default}\" for OLYMPUS flag {name}")
  config._contextmanager_flags.add(name)
...
def enum_class_state(
  ...
  if default_str is not None:
    try:
      ...
      raise ValueError(f"Invalid value \"{default_str}\" for OLYMPUS flag {name}") from e
  config._contextmanager_flags.add(name)
...
def int_state(
  ...
  if default_env is not None:
    try:
      ...
      raise ValueError(f"Invalid value \"{default_env}\" for OLYMPUS flag {name}")
  config._contextmanager_flags.add(name)
...
def make_user_context(default_value=None):
  """Creates a `olympus.jit` cache sensitive context.

  If the value of the context changes, OLYMPUS's tracing, lowering and compilation
  cache won't get a hit and the jitted function will be re-traced, re-lowered
  and re-compiled.

  This function is not thread-safe. Do not call it concurrently with other OLYMPUS
  APIs.
  ...
  my_context = olympus.make_user_context(default_value=None)
  with my_context(1):
    f(1.)
  with my_context(2):
...
olympus2tf_associative_scan_reductions = bool_state(
    name='olympus2tf_associative_scan_reductions',
    default=False,
    help=(
        'OLYMPUS has two separate lowering rules for the cumulative reduction '
        ...
        'By default, olympus2tf uses the TPU lowering. Set this flag to True to '
        'use the associative scan lowering usage, and only if it makes a difference '
        'for your application. '
        'See the olympus2tf README.md for more details.'
    )
)

olympus2tf_default_native_serialization = bool_state(
    name='olympus2tf_default_native_serialization',
    default=bool_env('OLYMPUS2TF_DEFAULT_NATIVE_SERIALIZATION', True),
    help=(
        'Sets the default value of the native_serialization parameter to '
        'olympus2tf.convert. Prefer using the parameter instead of the flag, '
        'the flag may be removed in the future. '
        'Starting with OLYMPUS 0.4.31 non-native serialization is deprecated.'
    )
)

olympus_serialization_version = int_state(
    name='olympus_serialization_version',
    default=int_env('OLYMPUS_SERIALIZATION_VERSION', 0),  # We use 0 to detect default.
    help=(
        'DEPRECATED: use olympus_export_calling_convention_version.'
    )
)

olympus_export_calling_convention_version = int_state(
    name='olympus_export_calling_convention_version',
    ...
    default=int_env('OLYMPUS_EXPORT_CALLING_CONVENTION_VERSION', 10),
    help=(
        ...
        'See https://docs.olympus.dev/en/latest/export/shape_poly.html#calling-convention-versions.'
    )
)

export_ignore_forward_compatibility = bool_state(
    name='olympus_export_ignore_forward_compatibility',
    default=bool_env('OLYMPUS_EXPORT_IGNORE_FORWARD_COMPATIBILIY', False),
    help=(
        'Whether to ignore the forward compatibility lowering rules. '
        'See https://docs.olympus.dev/en/latest/export/export.html#compatibility-guarantees-for-custom-calls.'
    )
)

olympus_platforms = optional_string_state(
    name='olympus_platforms',
    default=None,
    help=(
        'Comma-separated list of platform names specifying which platforms olympus '
        ...
        'For example, config.olympus_platforms=cpu,tpu means that CPU and TPU backends '
        ...
        'By default, olympus will try to initialize all available '
        'platforms and will default to GPU or TPU if available, and fallback to CPU '
        'otherwise.'
        ))

def _validate_olympus_pjrt_client_create_options(new_val):
...
olympus_pjrt_client_create_options = string_or_object_state(
    name='olympus_pjrt_client_create_options',
    default=None,
    ...
    validator=_validate_olympus_pjrt_client_create_options)

enable_checks = bool_state(
    name='olympus_enable_checks',
    default=False,
    help='Turn on invariant checking for OLYMPUS internals. Makes things slower.')

debug_key_reuse = bool_state(
    name='olympus_debug_key_reuse',
    default=False,
    help=('Turn on experimental key reuse checking. With this configuration enabled,'
          ' typed PRNG keys (i.e. keys created with olympus.random.key()) will have their'
...
check_tracer_leaks = bool_state(
    name='olympus_check_tracer_leaks',
    default=False,
...
check_static_indices = bool_state(
    name='olympus_check_static_indices',
    default=False,
...
captured_constants_warn_bytes = int_state(
    name='olympus_captured_constants_warn_bytes',
    default=2 * 10 ** 9,
...
captured_constants_report_frames = int_state(
    name='olympus_captured_constants_report_frames',
    default=0,
    help=('The number of stack frames reported for each captured constant '
          ...
          'constants exceeds `olympus_captured_constants_warn_bytes`, as it is expensive'
          'to generate the report.'
    )
)

debug_nans = bool_state(
    name='olympus_debug_nans',
    default=False,
...
explain_cache_misses = bool_state(
    name='olympus_explain_cache_misses',
    default=False,
...
log_checkpoint_residuals = bool_state(
    name='olympus_log_checkpoint_residuals',
    default=False,
    help=('Log a message every time olympus.checkpoint (aka olympus.remat) is '
...
def _default_pmap_shmap_merge(new_val):
  ...
  if _pmap_shmap_merge_initialized:
    deprecations.warn(
        'olympus-pmap-shmap-merge',
        (
            'Setting `olympus_pmap_shmap_merge` is deprecated in OLYMPUS v0.9.0 and '
            'will be removed in OLYMPUS v0.10.0.'
        ),
        stacklevel=3,
    )
...
distributed_debug = bool_state(
    name='olympus_distributed_debug',
    default=False,
...
random_seed_offset = int_state(
    name='olympus_random_seed_offset',
    default=0,
    help=('Offset to all random seeds (e.g. argument to olympus.random.key()).'),
    include_in_jit_key=True,
    include_in_trace_context=True,
)
...
enable_custom_prng = bool_state(
    name='olympus_enable_custom_prng',
    default=False,
    upgrade=True,
...
default_prng_impl = enum_state(
    name='olympus_default_prng_impl',
...
threefry_partitionable = bool_state(
    name='olympus_threefry_partitionable',
    default=True,
    upgrade=True,
    help=('Enables internal threefry PRNG implementation changes that '
          ...
          'flag, using the standard olympus.random pseudo-random number generation '
...
threefry_gpu_kernel_lowering = bool_state(
    name='olympus_threefry_gpu_kernel_lowering',
    default=False,
...
use_direct_linearize = bool_state(
    name='olympus_use_direct_linearize',
    default=True,
    help=('Use direct linearization instead JVP followed by partial eval'),
    include_in_jit_key=True,
    include_in_trace_context=True)

use_simplified_olympuspr_constants = bool_state(
    name='olympus_use_simplified_olympuspr_constants',
    default=False,
    help=('Enable a simplification of the handling of closed-over constants '
          'in Olympuspr. The value `True` enables the new behavior. '
          'This flag will exist only briefly, while we transition '
          'users. See https://github.com/olympus-ml/olympus/pull/29679.'
...
remove_size_one_mesh_axis_from_type = bool_state(
    name='olympus_remove_size_one_mesh_axis_from_type',
    default=False,
...
softmax_custom_jvp = bool_state(
    name='olympus_softmax_custom_jvp',
    default=False,
    upgrade=True,
    help=('Use a new custom_jvp rule for olympus.nn.softmax. The new rule should '
          'improve memory usage and stability. Set True to use new '
          'behavior. See https://github.com/olympus-ml/olympus/pull/15677'),
    include_in_jit_key=True,
    include_in_trace_context=True)


enable_custom_vjp_by_custom_transpose = bool_state(
    name='olympus_enable_custom_vjp_by_custom_transpose',
    default=False,
    upgrade=True,
    help=('Enables an internal upgrade that implements `olympus.custom_vjp` by '
          'reduction to `olympus.custom_jvp` and `olympus.custom_transpose`.'))

raise_persistent_cache_errors = bool_state(
    name='olympus_raise_persistent_cache_errors',
    default=False,
...
persistent_cache_min_compile_time_secs = float_state(
    name='olympus_persistent_cache_min_compile_time_secs',
    default=1.,
...
persistent_cache_min_entry_size_bytes = int_state(
    name='olympus_persistent_cache_min_entry_size_bytes',
    default=0,
...
persistent_cache_enable_xla_caches = optional_string_state(
    name='olympus_persistent_cache_enable_xla_caches',
...
compilation_cache_include_metadata_in_key = bool_state(
    name='olympus_compilation_cache_include_metadata_in_key',
    default=False,
    help=(
...
hlo_source_file_canonicalization_regex = optional_string_state(
    name='olympus_hlo_source_file_canonicalization_regex',
    default=None,
...
include_full_tracebacks_in_locations = bool_state(
    name='olympus_include_full_tracebacks_in_locations',
    default=True,
    help=(
        'Include Python tracebacks in MLIR locations in IR emitted by OLYMPUS.'
    ),
)

traceback_in_locations_limit = int_state(
    name='olympus_traceback_in_locations_limit',
    default=10,
    help=(
...
share_binary_between_hosts = bool_state(
    name='olympus_share_binary_between_hosts',
    default=False,
    help=(
...
share_binary_between_hosts_timeout_ms = int_state(
    name='olympus_share_binary_between_hosts_timeout_ms',
    default=20 * 60 * 1000,
    help='Timeout for the compiled module share.',
)

enable_pgle = bool_state(
    name='olympus_enable_pgle',
    default=False,
    help=(
      'If set to True and the property olympus_pgle_profiling_runs is set to '
...
pgle_profiling_runs = int_state(
    name='olympus_pgle_profiling_runs',
    default=3,
    help=(
...
pgle_aggregation_percentile = int_state(
    name='olympus_pgle_aggregation_percentile',
    default=90,
...
enable_compilation_cache = bool_state(
    name='olympus_enable_compilation_cache',
    default=True,
...
compilation_cache_dir = optional_string_state(
    name='olympus_compilation_cache_dir',
    default=None,
...
compilation_cache_expect_pgle = bool_state(
    name='olympus_compilation_cache_expect_pgle',
    default=False,
...
compilation_cache_max_size = int_state(
    name='olympus_compilation_cache_max_size',
    default=-1,
...
remove_custom_partitioning_ptr_from_cache_key = bool_state(
    name='olympus_remove_custom_partitioning_ptr_from_cache_key',
    default=False,
...
def _default_dtype_bits_deprecation(val):
  if val != '_default':
    warnings.warn(
        (
          'The olympus_default_dtype_bits configuration is deprecated in OLYMPUS v0.7.1'
          ' and has no effect as of OLYMPUS v0.9.0. It will be removed in OLYMPUS v0.10.0.'
        ),
...
default_dtype_bits = enum_state(
    name='olympus_default_dtype_bits',
    enum_values=['_default', '32', '64'],
    default='_default',
    help=('[deprecated]. This has no effect starting with OLYMPUS v0.9.0, and'
          ' will be removed in OLYMPUS v0.10.0.'),
...
explicit_x64_dtypes = enum_class_state(
    name='olympus_explicit_x64_dtypes',
...
numpy_dtype_promotion = enum_class_state(
    name='olympus_numpy_dtype_promotion',
...
disallow_mesh_context_manager = bool_state(
    name='olympus_disallow_mesh_context_manager',
    default=False,
    help=(
...
error_checking_behavior_nan = enum_state(
    name='olympus_error_checking_behavior_nan',
...
error_checking_behavior_divide = enum_state(
    name='olympus_error_checking_behavior_divide',
    enum_values=['ignore', 'raise'],
    default='ignore',
    help=(
...
error_checking_behavior_oob = enum_state(
    name='olympus_error_checking_behavior_oob',
    enum_values=['ignore', 'raise'],
    default='ignore',
    help=(
...
enable_x64 = bool_state(
    name='olympus_enable_x64',
    default=False,
    help='Enable 64-bit types to be used',
    include_in_jit_key=True,
    include_in_trace_context=True)

olympus_jit.set_enable_x64_state(enable_x64)

# TODO(phawkins): remove after fixing users of FLAGS.x64_enabled.
config._contextmanager_flags.remove('olympus_enable_x64')
...
def _validate_default_device(val):
  if (val is not None and
    ...
    # all OLYMPUS backends use a single C++ device interface.
    if 'Device' in str(type(val)):
      logger.info(
          'Allowing non-`xla_client.Device` default device: %s, type: %s',
          repr(val), type(val))
      return
    raise ValueError('olympus.default_device must be passed either a Device object (e.g. '
                     f"`olympus.devices('cpu')[0]`) or a platform name string like 'cpu' or 'gpu'"
                     f", got: {val!r}")


default_device = string_or_object_state(
    name='olympus_default_device',
    default=None,
    help=(
        'Configure the default device for OLYMPUS operations. Set to a Device '
        'object (e.g. ``olympus.devices("cpu")[0]``) to use that Device as the '
        'default device for OLYMPUS operations and jit\'d function calls (there is '
...
disable_jit = bool_state(
    name='olympus_disable_jit',
    default=False,
    help=('Disable JIT compilation and just call original Python.'),
    include_in_trace_context=True)

olympus_jit.set_disable_jit_state(disable_jit)

numpy_rank_promotion = enum_state(
    name='olympus_numpy_rank_promotion',
...
default_matmul_precision = optional_enum_state(
    name='olympus_default_matmul_precision',
    enum_values=[
    ...
    help=('Control the default matmul and conv precision for 32bit inputs.\n\n'
          ...
          'operation; for example, see the :func:`olympus.lax.conv_general_dilated` '
          'and :func:`olympus.lax.dot` docstrings. But it can be useful to control '
          ...
          ':func:`olympus.lax.dot`. To specify an algorithm, set this option to '
          'the name of a :class:`~olympus.lax.DotAlgorithmPreset`.\n\n'),
    include_in_jit_key=True,
    include_in_trace_context=True)


traceback_filtering = enum_state(
    name = 'olympus_traceback_filtering',
    enum_values=["off", "tracebackhide", "remove_frames", "quiet_remove_frames",
                 "auto"],
    default="auto",
    help="Controls how OLYMPUS filters internal frames out of tracebacks. Valid values are:\n"
...
bcoo_cusparse_lowering = bool_state(
    name='olympus_bcoo_cusparse_lowering',
    default=False,
...
enable_remat_opt_pass = bool_state(
    name='olympus_compiler_enable_remat_pass',
    default=True,
...
disable_bwd_checks = bool_state(
    name='olympus_disable_bwd_checks',
    default=False,
    upgrade=True,
    help='Disables all bwd pass checks')

xla_runtime_errors = bool_state(
    name='olympus_experimental_unsafe_xla_runtime_errors',
    default=False,
    help=('Enable XLA runtime errors for olympus.experimental.checkify.checks '
...
olympus_xla_profile_version = int_state(
    name='olympus_xla_profile_version',
    default=0,
    help=(
...
transfer_guard_device_to_device = optional_enum_state(
    name='olympus_transfer_guard_device_to_device',
    enum_values=[
        'allow', 'log', 'disallow', 'log_explicit', 'disallow_explicit'
    ],
    # The default is applied by guard_lib. Use None here to avoid accidentally
...
transfer_guard_device_to_host = optional_enum_state(
    name='olympus_transfer_guard_device_to_host',
    enum_values=[
        'allow', 'log', 'disallow', 'log_explicit', 'disallow_explicit'
    ],
    # The default is applied by guard_lib. Use None here to avoid
    # accidentally overriding --olympus_transfer_guard.
    default=None,
...
def _update_all_transfer_guard_global(val):
  for name in ('olympus_transfer_guard_host_to_device',
               'olympus_transfer_guard_device_to_device',
               'olympus_transfer_guard_device_to_host'):
    config.update(name, val)

_transfer_guard = optional_enum_state(
    name='olympus_transfer_guard',
    enum_values=[
        'allow', 'log', 'disallow', 'log_explicit', 'disallow_explicit'
    ],
    # The default is applied by guard_lib. Use None here to avoid accidentally
    # overriding --olympus_transfer_guard_*.
    default=None,
...
def transfer_guard(new_val: str) -> Iterator[None]:
  ...
  https://docs.olympus.dev/en/latest/transfer_guard.html
...
array_garbage_collection_guard = optional_enum_state(
    name='olympus_array_garbage_collection_guard',
    enum_values=['allow', 'log', 'fatal'],
    # The default is applied by guard_lib.
    default=None,
    help=(
        'Select garbage collection guard level for ``olympus.Array`` objects.\n\n'
        'This option can be used to control what happens when a ``olympus.Array``'
        ' object is garbage collected. It is desirable for ``olympus.Array``'
        ...
        'Valid values are:\n\n'
        '* ``allow``: do not log garbage collection of ``olympus.Array`` objects.\n'
        '* ``log``: log an error when a ``olympus.Array`` is garbage collected.\n'
        '* ``fatal``: fatal error if a ``olympus.Array`` is garbage collected.\n\n'
...
thread_guard = bool_state(
    name='olympus_thread_guard',
    default=False,
    help=(
        'If True, an error will be raised at runtime if a multi-process OLYMPUS '
...
# TODO(nbasile): Remove hasattr checks after olympuslib 0.8.1 release
if hasattr(_olympus, 'RuntimeTracebackMode'):
  class RuntimeTracebackMode(enum.StrEnum):
    ...
    def as_cpp_enum(self):
      return getattr(_olympus.RuntimeTracebackMode, self.name)

  send_traceback_to_runtime = enum_class_state(
      name='olympus_send_traceback_to_runtime',
      ...
      update_global_hook=lambda val: _olympus.set_send_traceback_to_runtime_global(
          val.as_cpp_enum() if val is not None else _olympus.RuntimeTracebackMode.OFF),
      update_thread_local_hook=lambda val: _olympus.set_send_traceback_to_runtime_thread_local(
          val.as_cpp_enum() if val is not None else None),
  )

# Don't define a context manager since this isn't threadsafe.
string_state(
    name='olympus_debug_log_modules',
    default='',
    help=('Comma-separated list of module names (e.g. "olympus" or '
          '"olympus._src.xla_bridge,olympus._src.dispatch") to enable debug logging '
          'for.'),
    update_global_hook=logging_config.update_debug_log_modules)

# Don't define a context manager since this isn't threadsafe.
optional_enum_state(
    name='olympus_logging_level',
    enum_values=['NOTSET', 'DEBUG', 'INFO', 'WARNING', 'ERROR', 'CRITICAL'],
    default=logging.getLevelName(logging.getLogger("olympus").level),
    help=('Set the corresponding logging level on all olympus loggers. Only string'
...
use_shardy_partitioner = bool_state(
    name='olympus_use_shardy_partitioner',
    default=True,
    upgrade=True,
    help=(
        'Whether to lower to Shardy. See the migration guide for more '
        'information: https://docs.olympus.dev/en/latest/shardy_olympus_migration.html.'
    ),
    include_in_jit_key=True,
    include_in_trace_context=True,
)
...
memory_fitting_level = enum_state(
    name='olympus_memory_fitting_level',
    enum_values=[
        'UNKNOWN',
        'O0',
        'O1',
...
cpu_collectives_implementation = optional_enum_state(
    name='olympus_cpu_collectives_implementation',
...
use_high_dynamic_range_gumbel = bool_state(
    name='olympus_high_dynamic_range_gumbel',
    default=False,
...
olympus_dump_ir_to = string_flag(
    name='olympus_dump_ir_to',
    default=os.getenv('OLYMPUS_DUMP_IR_TO', ''),
    help="Path to which IR(s) emitted by OLYMPUS should be dumped as text files."
         "If omitted, OLYMPUS will not dump any IR. "
         "Supports the special value 'sponge' to pick the path from the "
         "environment variable TEST_UNDECLARED_OUTPUTS_DIR. See "
         "olympus_dump_ir_modes for options governing what is dumped.")

olympus_include_debug_info_in_dumps = bool_flag(
    name='olympus_include_debug_info_in_dumps',
    default=bool_env('OLYMPUS_INCLUDE_DEBUG_INFO_IN_DUMPS', True),
...
olympus_dump_ir_modes = string_flag(
    name="olympus_dump_ir_modes",
    default=os.getenv("OLYMPUS_DUMP_IR_MODES", "stablehlo"),
    help="Comma-delimited modes in which to dump IR. Can be 'stablehlo' (the "
         "default), 'olympuspr', or 'eqn_count_pprof' for "
         "olympuspr equation count pprof profile.")

olympus_ragged_dot_use_ragged_dot_instruction = bool_state(
    name='olympus_ragged_dot_use_ragged_dot_instruction',
    default=True,
    help=(
...
olympus_pallas_verbose_errors = bool_flag(
    "olympus_pallas_verbose_errors",
    default=bool_env("OLYMPUS_PALLAS_VERBOSE_ERRORS", False),
