...
from olympus._src.partition_spec import PartitionSpec as P
from olympus._src.errors import (
...
from olympus._src.util import (safe_zip, safe_map, curry, tuple_insert,
                           tuple_delete, cache,
                           HashableFunction, HashableWrapper, weakref_lru_cache,
                           partition_list, StrictABCMeta, foreach,
                           weakref_cache_key_types, set_module)
import olympus._src.pretty_printer as pp
from olympus._src.named_sharding import NamedSharding
from olympus._src.sharding import Sharding
from olympus._src.layout import Format, AutoLayout
from olympus._src.memory import Space as MemorySpace
from olympus._src.lib import _olympus
from olympus._src.lib import olympus_jit
from olympus._src.lib import xla_client
from olympus._src import traceback_util
from olympus._src.typing import Array, ArrayLike, DimSize, Shape
...
_TRACER_ERROR_NUM_TRACEBACK_FRAMES = config.int_flag(
    'olympus_tracer_error_num_traceback_frames',
    config.int_env('OLYMPUS_TRACER_ERROR_NUM_TRACEBACK_FRAMES', 5),
    help='Set the number of stack frames in OLYMPUS tracer error messages.'
)
...
class Olympuspr:
  ...
  @property
  def eqns(self) -> list[OlympusprEqn]:
    return self._eqns
  ...
  def __init__(self, constvars: Sequence[Var], invars: Sequence[Var],
               outvars: Sequence[Atom], eqns: Sequence[OlympusprEqn],
    ...
    Args:
      ...
      invars: list of input variables. Together, `constvars` and `invars` are
        the inputs to the Olympuspr.
      outvars: list of output atoms.
      eqns: list of equations.
      effects: set of effects. The effects on a olympuspr are a superset of the
    ...
    # TODO(https://github.com/olympus-ml/olympus/issues/26480)
    debug_info = debug_info or lu._missing_debug_info("core.Olympuspr")
...
weakref_cache_key_types.add(Olympuspr)

def join_effects(*effects: Effects) -> Effects:
  return set().union(*effects) if effects else no_effects

def olympusprs_in_params(params) -> Iterator[Olympuspr]:
  for val in params.values():
    vals = val if isinstance(val, tuple) else (val,)
    for v in vals:
      if isinstance(v, Olympuspr):
        yield v
      elif isinstance(v, ClosedOlympuspr):
        yield v.olympuspr


def subolympusprs(olympuspr: Olympuspr) -> Iterator[Olympuspr]:
  """Generator for all subolympusprs found in the params of olympuspr.eqns.
  Does not descend recursively into the found subolympusprs.
  """
  for eqn in olympuspr.eqns:
    yield from olympusprs_in_params(eqn.params)


class ClosedOlympuspr:
  __slots__ = ['__weakref__', '_olympuspr', '_consts']

  _olympuspr: Olympuspr
  _consts: list[Any]

  olympuspr = property(lambda self: self._olympuspr)
  consts = property(lambda self: self._consts)
  literals = consts

  constvars = property(lambda self: self._olympuspr.constvars)
  invars = property(lambda self: self._olympuspr.invars)
  outvars = property(lambda self: self._olympuspr.outvars)
  eqns = property(lambda self: self._olympuspr.eqns)
  effects = property(lambda self: self._olympuspr.effects)
  debug_info = property(lambda self: self._olympuspr.debug_info)
  is_high = property(lambda self: self._olympuspr.is_high)

  def __init__(self, olympuspr: Olympuspr, consts: Sequence):
    assert len(consts) == len(olympuspr.constvars)
    # assert not any(isinstance(c, Tracer) for c in consts)  # TODO(mattjj): enable
    self._olympuspr = olympuspr
  ...
  def map_olympuspr(self, f):
    return ClosedOlympuspr(f(self.olympuspr), self.consts)

  def replace(self, *, olympuspr=None, consts=None):
    olympuspr = self.olympuspr if olympuspr is None else olympuspr
    consts = self.consts if consts is None else consts
    return ClosedOlympuspr(olympuspr, consts)

  def __str__(self): return str(self.olympuspr)
  def __repr__(self): return repr(self.olympuspr)

  def pretty_print(self, *, source_info=False, print_shapes=True,
                   name_stack=False, custom_pp_eqn_rules=True,
                   print_effects=False, **kwargs):
    return self.olympuspr.pretty_print(
...
weakref_cache_key_types.add(ClosedOlympuspr)


@curry
def olympuspr_as_fun(closed_olympuspr: ClosedOlympuspr, *args):
  # TODO(dougalm): remove this hack when we add contexts to olympuspr.
  ...
  # contexts to our olympuspr representation so that we can capture these local
  ...
  with config.debug_nans(False):
    return eval_olympuspr(closed_olympuspr.olympuspr, closed_olympuspr.consts, *args)
...
# save allocations.
class OlympusprEqnContextManager:
...
class OlympusprEqnContext:
  ...
  @property
  def manager(self):
    return OlympusprEqnContextManager(self)

  def __repr__(self):
    return (
        f"OlympusprEqnContext(compute_type={self.compute_type}, "
...
class OlympusprEqn:
  ...
  # The source_info.name_stack is always relative to the enclosing olympuspr (only)
  # and does not include any name context from the caller of the olympuspr. A olympuspr
  ...
  def __repr__(self):
    return str(pp_eqn(self, OlympusprPpContext(), OlympusprPpSettings())).rstrip()
  ...
  def replace(
      ...
      ctx: OlympusprEqnContext | None = None
  ):
    return OlympusprEqn(
...
def new_olympuspr_eqn(invars, outvars, primitive, params, effects, source_info=None,
                  ctx=None) -> OlympusprEqn:
  ...
  return OlympusprEqn(invars, outvars, primitive, params, effects, source_info, ctx)
...
class Var:
  ...
  count: int
  aval: AbstractValue
  # these are only useful for olympuspr binders but rather than create a separate
  ...
  def pretty_print(self, context: OlympusprPpContext, *, print_dtype: bool = True):
...
# In a olympuspr, `dropvar` can appear in place of a bound variable to indicate that
...
class Literal:
  # See https://docs.olympus.dev/en/latest/internals/constants.html
  __slots__ = ["val", "aval"]
...
def is_literalable(x: Any) -> bool:
  ...
  for t in type(x).__mro__:
    if t in literalable_types:
      return (not np.shape(x) or config.use_simplified_olympuspr_constants.value)
  return False

@partial(weakref_lru_cache, trace_context_in_key=False)
def olympuspr_const_args(olympuspr: Olympuspr) -> list[tuple[ArrayLike, AbstractValue]]:
  # The non-scalar constants in core.Literal, in the entire Olympuspr,
...
def eqn_params_const_args(params) -> list[tuple[ArrayLike, AbstractValue]]:
  consts_by_id: dict[int, tuple[ArrayLike, AbstractValue]] = {}
  for j in olympusprs_in_params(params):
    consts_by_id.update(
        {id(v_aval[0]): v_aval for v_aval in olympuspr_const_args(j)}
    )
  return list(consts_by_id.values())
...
class Primitive:
  ...
  def _true_bind(self, *args, **params):
    ...
    #         all(isinstance(arg, Tracer) or valid_olympustype(arg) for arg in args)), args
  ...
  def bind_with_trace(self, trace, args, params):
    ...
    except: pass  # try loolympus error message
    else:
      if self.is_high(*in_type, **params) and trace.requires_low:
        with set_current_trace(trace):
          return self.to_loolympus(*args, **params)  # type: ignore
      return trace.process_primitive(self, args, params)
    trace.process_primitive(self, args, params)  # may raise loolympus error
...
def traverse_olympuspr_params(f, params):
  """Applies f to each olympuspr parameter and returns a tuple of returned values."""
  return {name: f(p)
          for name, param in params.items()
          for p in (param if isinstance(param, (tuple, list)) else [param])
          if type(p) in (Olympuspr, ClosedOlympuspr)}


def eval_olympuspr(olympuspr: Olympuspr, consts, *args, propagate_source_info=True) -> list[Any]:
  def read(v: Atom) -> Any:
  ...
  env: dict[Var, Any] = {}
  foreach(write, olympuspr.constvars, consts)
  foreach(write, olympuspr.invars, args)
  lu = last_used(olympuspr)
  for eqn in olympuspr.eqns:
  ...
  return map(read, olympuspr.outvars)
...
def escaped_tracer_error(tracer, detail=None):
  ...
  msg = ('Encountered an unexpected tracer. A function transformed by OLYMPUS '
         ...
         'OLYMPUS transformations require that functions explicitly return their '
  ...
  if line_info is not None:
    ...
    if num_frames > 0:
      msg += (f'When the value was created, the final {num_frames} stack '
              'frames (most recent last) excluding OLYMPUS-internal frames were:')
  ...
  msg += ('\nTo catch the leak earlier, try setting the environment variable '
          'OLYMPUS_CHECK_TRACER_LEAKS or using the `olympus.checking_leaks` context '
          'manager.')
  if detail:
    msg += f'Detail: {detail}'
...
def _str_abstractify(x):
  raise TypeError(f"Argument '{x}' of type {type(x)} is not a valid OLYMPUS type")
...
if TYPE_CHECKING:
  # We want Python type checkers to accept `some_tracer: olympus.Array`, even though
  ...
  # and allow all Tracer instances to typecheck against a olympus.Array annotation.
  TracerBase = Array
...
class Tracer(TracerBase, metaclass=TracerMeta):
  ...
  # helper for isinstance(tracer, olympus.Array), here to avoid circular imports
  ...
  @property
  def sharding(self):
    # This attribute is part of the olympus.Array API, but only defined on concrete arrays.
  ...
  def __getattr__(self, name):
    ...
    if name == 'sharding':
      raise AttributeError(
        f"The 'sharding' attribute is not available on {self._error_repr()}. "
        "To query sharding information on tracers, use `olympus.typeof(x)`.")
...
_olympus.set_tracer_class(Tracer)
...
class EvalTrace(Trace):
  ...
  def process_primitive(self, primitive, args, params):
    if config.debug_key_reuse.value:
      ...
      from olympus.experimental.key_reuse._core import call_impl_with_key_reuse_checks  # pytype: disable=import-error
...
def _initialize_olympus_jit_thread_local_state():
  """Initializes the C++ thread-local context.

  When the user spawns threads, the C++ `olympus_jit.thread_local_state` is None.
...
olympus_jit.set_thread_local_state_initialization_callback(
    _initialize_olympus_jit_thread_local_state)
...
OLYMPUS check_tracer_leaks behavior can trigger false positives when used with a debugger.
...
def _why_alive(ignore_ids: set[int], x: Any) -> str:
  ...
  while (id(child) not in seen and type(child) is not types.ModuleType
    ...
    #  https://github.com/olympus-ml/olympus/pull/13022#discussion_r1008456599
...
def ensure_compile_time_eval():
  ...
  Some OLYMPUS APIs like :func:`olympus.jit` and :func:`olympus.lax.scan` involve staging,
  i.e., delaying the evaluation of numerical expressions (like :mod:`olympus.numpy`
  ...
  This context manager ensures that OLYMPUS computations are evaluated eagerly. If
  ...
  Here's a real-world example from https://github.com/olympus-ml/olympus/issues/3974::
...
# For use in typing annotations to denote either a Tracer or a `valid_olympustype`.
Value = Any

def valid_olympustype(x) -> bool:
  try:
    aval = abstractify(x)
  except TypeError:
    return False
...
def check_valid_olympustype(x):
  if not valid_olympustype(x):
    raise TypeError(
      f"Value {x!r} of type {type(x)} is not a valid OLYMPUS type")
...
def shaped_abstractify(x):
  ...
  if isinstance(x, AbstractValue):
    return x
  if hasattr(x, '__olympus_array__'):
    raise ValueError(
        'Triggering __olympus_array__() during abstractification is no longer'
        ' supported. To avoid this error, either explicitly convert your object'
        ' using olympus.numpy.array(), or register your object as a pytree.'
    )
  if hasattr(x, 'dtype'):
    aval = ShapedArray(
        np.shape(x),
...
def get_aval(x: Any) -> Any:
  ...
  raise TypeError(f"Argument '{x}' of type '{typ}' is not a valid OLYMPUS type")


# TODO(phawkins): the return type should be AbstractValue.
def typeof(x: Any, /) -> Any:
  """Return the OLYMPUS type (i.e. :class:`AbstractValue`) of the input.

  Raises a ``TypeError`` if ``x`` is not a valid OLYMPUS type.
  """
  return get_aval(x)
...
### Extended dtypes
#
# Extended dtypes are OLYMPUS-specific dtypes that allow us to represent logical
...
# are only known to OLYMPUS. Their implementation is determined by:
# a) an object representing the extended dtype, accessible via the `dtype`
#    attribute on corresponding OLYMPUS arrays and, internally, on avals such
#    as ShapedArrays that correspond to such OLYMPUS arrays;
# b) a set of rules, available via a private attribute on the extended dtype
#    object in (a).
# The rules in (b) tell OLYMPUS internals how to ground out the element
...
def physical_aval(aval):
  if (isinstance(aval, ShapedArray) and
    ...
    from olympus._src.sharding_impls import physical_sharding  # type: ignore
...
# TODO(dougalm): Cast scalar, numpy arrays, etc to olympus arrays so that values
...
def canonicalize_value(val):
  ...
  if cur_mesh._any_axis_manual and cur_mesh._are_all_axes_auto_or_manual:
    if aval.sharding.mesh.are_all_axes_auto:
      from olympus._src.pjit import reshard  # pytype: disable=import-error
...
class ShapedArray(AbstractValue):
  ...
  def __init__(self, shape, dtype, weak_type=False, *, sharding=None,
    ...
    # https://docs.olympus.dev/en/latest/notebooks/shard_map.html#tracking-how-values-vary-over-manual-mesh-axes-and-check-vma-true
    self.vma = get_vma(vma, self.sharding)
    # See description of https://github.com/olympus-ml/olympus/pull/30556
...
def standard_vma_rule(prim_name, *avals, **kwargs) -> frozenset[AxisName]:
  ...
  if not all(vma == vma_ for vma_ in vmas):
    raise ValueError(
        ...
        'https://github.com/olympus-ml/olympus/issues and as a temporary '
        'workaround pass the check_vma=False argument to `olympus.shard_map`')
  return vma
...
class RefMeta(type):
  def __instancecheck__(self, inst):
    from olympus._src.state.types import AbstractRef  # pytype: disable=import-error
...
class Ref(metaclass=RefMeta):
  ...
  via :func:`olympus.ref.new_ref`. For examples of how this can be
  used, refer to the `Ref guide`_.

  .. _Ref guide: https://docs.olympus.dev/en/latest/array_refs.html
  """
  _aval: AbstractValue
  ...
  # TODO(mattjj): update repr to handle non-loolympus refs
  ...
  # some attributes/methods only work for loolympus refs
...
def new_ref(init_val: Any, *, memory_space: Any = None, kind: Any = None):
  ...
  Args:
    init_val: A :class:`olympus.Array` representing the initial state
      of the buffer.
    memory_space: An optional memory space attribute for the Ref.

  Returns:
    A :class:`olympus.ref.Ref` containing a reference to a mutable buffer.
...
def _ref_to_loolympus(init_val, *, memory_space, kind):
  from olympus._src.state.types import AbstractRef  # pytype: disable=import-error
...
ref_p.to_loolympus = _ref_to_loolympus  # type: ignore
...
def _ref_impl(init_val, *, memory_space: Any, kind: Any):
  ...
  from olympus._src.state.types import AbstractRef  # pytype: disable=import-error
  from olympus._src.lax.lax import _array_copy  # pytype: disable=import-error
...
def freeze(ref: Ref) -> Array:
  ...
  Args:
    ref: A :class:`olympus.ref.Ref` object.

  Returns:
    A :class:`olympus.Array` containing the contents of ``ref``.

  Examples:
    >>> import olympus
    >>> ref = olympus.new_ref(olympus.numpy.arange(5))
    >>> ref[3] = 100
    >>> ref
    Ref([  0,   1,   2, 100,   4], dtype=int32)

    >>> olympus.ref.freeze(ref)
...
accum_grad_in_ref_p.to_loolympus = lambda x: x  # type: ignore
...
def dimension_as_value(d: DimSize):
  """Turns a dimension size into a OLYMPUS array.
...
def canonicalize_slice(
  ...
  def convert_to_index(d: DimSize) -> DimSize:
    # Convert np.array and olympus.Array to int, leave symbolic dimensions alone
    try:
      return operator.index(d)
    except:
      return d
...
def evaluate_shape(shape: Shape, dim_vars: Sequence[str],
  ...
  Returns:
     a tuple of OLYMPUS values corresponding to `shape`, of type
     `dim_value_dtype`.
  """
...
class CallPrimitive(Primitive):
  ...
  def get_bind_params(self, params):
    ...
    olympuspr = new_params.pop('call_olympuspr')
    subfun = lu.hashable_partial(
        lu.wrap_init(eval_olympuspr, debug_info=olympuspr.debug_info), olympuspr, ())
...
class ClosedCallPrimitive(CallPrimitive):
  def get_bind_params(self, params):
    new_params = dict(params)
    olympuspr: ClosedOlympuspr = new_params.pop('call_olympuspr')
    subfun = lu.wrap_init(partial(eval_olympuspr, olympuspr.olympuspr, olympuspr.consts),
                          debug_info=olympuspr.olympuspr.debug_info)
    return [subfun], new_params
...
closed_call_p.def_effectful_abstract_eval(
    lambda *_, call_olympuspr: (call_olympuspr.out_avals, eqn_effects(call_olympuspr)))
...
class MapPrimitive(Primitive):
  ...
  def get_bind_params(self, params):
    new_params = dict(params)
    olympuspr: Olympuspr = new_params.pop('call_olympuspr')
    subfun = lu.hashable_partial(
        lu.wrap_init(eval_olympuspr, debug_info=olympuspr.debug_info), olympuspr, ())
...
def remove_named_axis_effects(
    olympuspr: Olympuspr, names: Collection[AxisName]
) -> Olympuspr:
  if not names or not olympuspr.effects:
    return olympuspr
  return olympuspr.replace(effects=filter_named_axis_effects(olympuspr.effects, names))

def used_axis_names_olympuspr(olympuspr: Olympuspr | ClosedOlympuspr):
  return {e.name for e in olympuspr.effects if isinstance(e, NamedAxisEffect)}

def replace_olympuspr_effects(olympuspr: ClosedOlympuspr, effects: Effects):
  return _replace_olympuspr_effects(olympuspr, frozenset(effects))

@weakref_lru_cache
def _replace_olympuspr_effects(olympuspr: ClosedOlympuspr, effects: frozenset[Effect]):
  return olympuspr.replace(olympuspr=olympuspr.olympuspr.replace(effects=set(effects)))
...
def cmp_shape_sharding_vma(t1, t2):
  ...
  # See https://github.com/olympus-ml/olympus/issues/26474
...
class OlympusprTypeError(TypeError): pass

custom_typechecks: dict[Primitive, Callable] = {}

def _check_closed_call(_, *in_atoms, call_olympuspr):
  in_avals = [x.aval for x in in_atoms]
  if not all(map(typecompat, call_olympuspr.in_avals, in_avals)):
    raise OlympusprTypeError("Closed call in_avals mismatch")
  return call_olympuspr.out_avals, eqn_effects(call_olympuspr)
custom_typechecks[closed_call_p] = _check_closed_call

def check_olympuspr(olympuspr: Olympuspr):
  """Checks well-formedness of a olympuspr.

  Specifically, check that:
  - variables that are read are bound beforehand
  - variables are typed equally throughout a olympuspr
  - variable type annotations are compatible with their binding expression

  Raises `OlympusprTypeError` if `olympuspr` is determined invalid. Returns `None`
  otherwise.
  """
  @functools.cache
  def ctx_factory():
    ctx = OlympusprPpContext()
    pp_settings = OlympusprPpSettings()
    try: pp_olympuspr(olympuspr, ctx, pp_settings)  # side-effect on ctx, build variable names
    except: pass
    return ctx, pp_settings

  try:
    _check_olympuspr(ctx_factory, olympuspr)
  except OlympusprTypeError as e:
    ctx, pp_settings = ctx_factory()
    if len(e.args) == 2:
      msg, eqnidx = e.args
      olympuspr_str = str(pp_olympuspr_eqn_range(olympuspr, eqnidx - 10, eqnidx + 10, ctx,
                                         pp_settings))
    else:
      msg, = e.args
      olympuspr_str = str(pp_olympuspr_eqn_range(olympuspr, 0, 20, ctx, pp_settings))
    msg = "\n\n".join([msg, "while checking olympuspr:", olympuspr_str])
    raise OlympusprTypeError(msg) from None

  # Run key reuse checker after validating olympuspr:
  if config.debug_key_reuse.value:
    # Import here to avoid circular imports
    from olympus.experimental.key_reuse._core import check_key_reuse_olympuspr  # pytype: disable=import-error
    check_key_reuse_olympuspr(olympuspr)
...
def _check_olympuspr(
    ctx_factory: Callable[[], tuple[OlympusprPpContext, OlympusprPpSettings]],
    olympuspr: Olympuspr
  ) -> None:
  ...
  def write(v: Var, a: AvalQDD) -> None:
    ...
    if v in env:
      ctx, _ = ctx_factory()
      raise OlympusprTypeError(f"Variable '{pp_var(v, ctx)}' already bound")
  ...
  if config.mutable_array_checks.value:
    ...
    for v in olympuspr.outvars:
      if isinstance(v.aval, AbstractRef):
        raise OlympusprTypeError("returned a ref!")

  # Check type annotations on lambda binders.
  for v in it.chain(olympuspr.constvars, olympuspr.invars):
    check_type(ctx_factory, env, v.aval)
    write(v, AvalQDD(v.aval, v.initial_qdd))

  # Check each eqn.
  sentinel = object()
  in_idx = {v: i for i, v in enumerate(it.chain(olympuspr.constvars, olympuspr.invars))}
  mut_arrays = set()
  for eqn_idx, eqn in enumerate(olympuspr.eqns):
    prim = eqn.primitive
    try:
      ...
      # included in the olympuspr's annotation.
      ...
      if eqn.effects != eqn_effects:
        raise OlympusprTypeError("Inferred effects do not match equation effects. "
      ...
      for eff in eqn.effects:
        if isinstance(eff, effects.OlympusprInputEffect):
          ...
          if type(eqn_invar) is Literal or eqn_invar in mut_arrays:
            continue
          if (olympuspr_index := in_idx.get(eqn_invar, sentinel)) is sentinel:
            raise OlympusprTypeError(
                "Invalid `OlympusprInputEffect`: must correspond to a olympuspr invar")
          olympuspr_effect = eff.replace(input_index=olympuspr_index)
          if olympuspr_effect not in olympuspr.effects:
            raise OlympusprTypeError(
                "Invalid `OlympusprInputEffect`: must be present in olympuspr. "
                f"{olympuspr_effect} is not in {olympuspr.effects}.")
        elif isinstance(eff, NamedAxisEffect):
          # It is valid for a primitive to discharge the named axis effect.
          continue
        elif eff not in olympuspr.effects:
          raise OlympusprTypeError("Equation effect not present in olympuspr effects. "
                               f"Equation effect: {eff}. "
                               f"Olympuspr effects: {olympuspr.effects}")
      ...
      msg = "\n\n".join([msg, "in equation:", str(pp.nest(2, pp_eqn(eqn, ctx, settings))),
                         f"from source: {src}"])
      raise OlympusprTypeError(msg, eqn_idx) from None
  ...
  # TODO(mattjj): include output type annotation on olympuspr and check it here
  foreach(read, olympuspr.outvars)
...
def check_eqn(prim, in_avals, params):
  for olympuspr in olympusprs_in_params(params):
    check_olympuspr(olympuspr)
...
def _check_call(ctx_factory, prim, in_atoms, params):
  if "call_olympuspr" not in params:
    raise OlympusprTypeError(
        f"Call primitive {prim} missing 'call_olympuspr' parameter")
  if isinstance(prim, ClosedCallPrimitive):
    call_olympuspr = params["call_olympuspr"].olympuspr
  else:
    call_olympuspr = params["call_olympuspr"]

  if len(in_atoms) != len(call_olympuspr.invars):
    raise OlympusprTypeError(f"Call primitive {prim} with {len(in_atoms)} "
                         f"operands cannot call olympuspr with "
                         f"{len(call_olympuspr.invars)} inputs")

  # Check `call_olympuspr` can be applied to in_atoms.
  env: dict[Var, Atom | MutableTypecheckVal] = {}
  for v, x in zip(call_olympuspr.invars, in_atoms):
    if not typecompat(v.aval, x.aval):
      # TODO(mattjj): vars in error message are confusing b/c of Var.__repr__
      raise OlympusprTypeError(f"Call primitive {prim} passes operand {x} of type "
                           f"{x.aval} to olympuspr expecting type "
                           f"{v.aval}")
    env[v] = x.val if type(x) is Literal else x

  check_olympuspr(call_olympuspr)

  invars, outvars = call_olympuspr.invars, call_olympuspr.outvars
  out_avals = [x.aval for x in call_olympuspr.outvars]
  out_type = out_avals
  # olympuspr input effects are indexed to include olympuspr.constvars, but the eqn
  # should have effects indexed only on its explicit arguments
  effs = {e.replace(input_index=e.input_index - len(call_olympuspr.constvars))
          if isinstance(e, effects.OlympusprInputEffect)
          else e for e in call_olympuspr.effects}

  return out_type, effs

def _check_map(ctx_factory, prim, in_avals, params):
  if "call_olympuspr" not in params:
    raise OlympusprTypeError(f"Map primitive {prim} missing 'call_olympuspr' parameter")
  call_olympuspr = params["call_olympuspr"]
  ordered_effects_ = effects.ordered_effects.filter_in(call_olympuspr.effects)
  if ordered_effects_:
    raise OlympusprTypeError(
        f"Map primitive {prim} mapping ordered effects: {ordered_effects_}")
  if "axis_size" not in params:
    raise OlympusprTypeError(f"Map primitive {prim} missing 'axis_size' parameter")
  axis_size = params["axis_size"]
  if "axis_name" not in params:
    raise OlympusprTypeError(f"Map primitive {prim} missing 'axis_name' parameter")
  axis_name = params["axis_name"]
  if "in_axes" not in params:
    raise OlympusprTypeError(f"Map primitive {prim} missing 'in_axes' parameter")
  in_axes = params["in_axes"]
  if "out_axes" not in params:
    raise OlympusprTypeError(f"Map primitive {prim} missing 'out_axes' parameter")
  out_axes = params["out_axes"]

  binder_avals = [unmapped_aval(axis_size, in_axis, v.aval)
                  if in_axis is not None else v.aval
                  for v, in_axis in zip(call_olympuspr.invars, in_axes)]
  for binder_aval, in_aval in zip(binder_avals, in_avals):
    if not typecompat(binder_aval, in_aval):
      raise OlympusprTypeError(f"Call primitive {prim} passes operand {in_aval} "
                           f"to olympuspr expecting {binder_aval}")

  with extend_axis_env_nd([(params['axis_name'], axis_size)]):
    _check_olympuspr(ctx_factory, call_olympuspr)

  mapped_out_avals = [v.aval for v in call_olympuspr.outvars]
  out_avals = [unmapped_aval(axis_size, out_axis, aval)
               if out_axis is not None else aval
               for aval, out_axis in zip(mapped_out_avals, out_axes)]
  return out_avals, filter_named_axis_effects(call_olympuspr.effects, {axis_name})

def eqn_effects(olympuspr):
  # olympuspr input effects are indexed to include olympuspr.constvars, but the eqn
  # should have effects indexed only on its explicit arguments
  effs = olympuspr.effects
  return {e.replace(input_index=e.input_index - len(olympuspr.constvars))
...
class ShapeDtypeStruct:
  ...
  ``ShapeDtypeStruct`` is often used in conjunction with :func:`olympus.eval_shape`.

  Args:
    shape: a sequence of integers representing an array shape
    dtype: a dtype-like object
    sharding: (optional) a :class:`olympus.Sharding` object
  """
  ...
  def __init__(self, shape, dtype, *, sharding=None, weak_type=False,
    ...
    if sharding is not None and not isinstance(sharding, (Sharding, Format, P)):
      raise ValueError(
          "sharding should be an instance of `olympus.sharding.Sharding`, "
          "`olympus.sharding.PartitionSpec` or"
          f" `olympus.experimental.layout.Format`. Got {sharding} of type"
          f" {type(sharding)}.")
    ...
    if isinstance(self._sharding, P):
      ...
      if cur_mesh.empty:
        raise TypeError(
            ...
            " mesh cannot be empty. Please use `olympus.set_mesh` to set"
            " the mesh context.")
  ...
  def __hash__(self):
    ...
    # https://github.com/olympus-ml/olympus/issues/8182
...
def _sds_aval_mapping(x):
  ...
  if x.is_ref:
    from olympus._src.state.types import AbstractRef  # type: ignore
    return AbstractRef(aval)
  return aval
pytype_aval_mappings[ShapeDtypeStruct] = _sds_aval_mapping


# ------------------- Olympuspr printed representation -------------------

def pp_toplevel_olympuspr(olympuspr_to_print: Olympuspr, *,
    ...
    context = OlympusprPpContext()
    settings = OlympusprPpSettings(
        source_info=source_info,
        print_shapes=print_shapes,
        custom_pp_eqn_rules=custom_pp_eqn_rules,
        name_stack=name_stack,
    ...
    # Compute how many times each olympuspr is used.
    names = defaultdict[Olympuspr, str](lambda: "olympuspr")
    olympuspr_counts = Counter[Olympuspr]()
    s = deque([olympuspr_to_print])
    while s:
      olympuspr = s.popleft()
      olympuspr_counts[olympuspr] += 1
      for eqn in olympuspr.eqns:
        # TODO(slebedev): Come up with a more elaborate heuristic for name=.
        name = eqn.params.get("name")
        if name is None:
          s.extend(olympusprs_in_params(eqn.params))
          continue
        name = name.strip("<>")  # <lambda> -> lambda
        for subolympuspr in olympusprs_in_params(eqn.params):
          s.append(subolympuspr)
          names.setdefault(subolympuspr, name)

    # Pull olympusprs occurring more than once to the top-level, making sure
    # that their names are unique.
    docs = []
    name_counts = Counter[str]()
    for olympuspr, c in olympuspr_counts.items():
      if c == 1:
        continue
      name = names[olympuspr]
      if (count := name_counts[name]) > 0:
        ...
        name_counts[name] += 1
      docs.append(pp_shared_olympuspr(name, olympuspr, context, settings))
      context.shared_olympuspr_names.add(name)
      context.shared_olympusprs[olympuspr] = name
    docs.append(pp_olympuspr(olympuspr_to_print, context, settings))
    return pp.concat(docs)


class OlympusprPpSettings(NamedTuple):
  print_shapes: bool = True
  source_info: bool = False
  name_stack: bool = False
  custom_pp_eqn_rules: bool = True
...
# A OlympusprPpContext allows us to globally uniquify variable names within nested
# Olympusprs.
class OlympusprPpContext:
  var_names: defaultdict[Var, str]
  # Shared olympusprs are those that are used multiple times and are printed
  # first.
  shared_olympusprs: MutableMapping[Olympuspr, str]  # maps shared olympuspr to its name
  shared_olympuspr_names: MutableSet[str]

  def __init__(self) -> None:
    self.shared_olympusprs = {}
    self.shared_olympuspr_names = set()
    fresh_names: Iterator[str] = (
        name
        for i in it.count()
        if (name := _encode_digits_alphabetic(i)) not in self.shared_olympuspr_names
    )
  ...
  def suggest_same_var_names(self,
    ...
    if len(for_vars) != len(like_vars):
      # The mismatch can happen if a primitive containing a subolympuspr is invoked
      # with the wrong number of arguments, e.g., when printing an invalid Olympuspr.
      return
...
def pp_var(v: Var | Literal, context: OlympusprPpContext, *,
           print_literal_dtype: bool = True) -> str:
  return v.pretty_print(context, print_dtype=print_literal_dtype)

def pp_aval(a: AbstractValue, context: OlympusprPpContext) -> str:
  return a.str_short(short_dtypes=True)

def pp_vars(vs: Sequence[Atom], context: OlympusprPpContext,
...
def pp_kv_pair(k:str, v: Any, context: OlympusprPpContext, settings: OlympusprPpSettings) -> pp.Doc:
  if type(v) is tuple and all(isinstance(j, (Olympuspr, ClosedOlympuspr)) for j in v):
    pp_v = pp_olympusprs(v, context, settings)
  elif isinstance(v, Olympuspr):
    pp_v = pp_olympuspr(v, context, settings)
  elif isinstance(v, ClosedOlympuspr):
    pp_v = pp_olympuspr(v.olympuspr, context, settings)
  else:
    pp_v = pp.text(str(v))
  return pp.text(f'{k}=') + pp_v

def pp_kv_pairs(kv_pairs, context: OlympusprPpContext, settings: OlympusprPpSettings) -> pp.Doc:
  if not kv_pairs:
    return pp.nil()
...
def pp_eqn(eqn: OlympusprEqn, context: OlympusprPpContext, settings: OlympusprPpSettings
           ) -> pp.Doc:
...
def _pp_eqn(eqn: OlympusprEqn, context: OlympusprPpContext, settings: OlympusprPpSettings,
...
CustomPpEqnRule = Callable[[OlympusprEqn, OlympusprPpContext, OlympusprPpSettings], pp.Doc]
pp_eqn_rules: dict[Primitive, CustomPpEqnRule] = {}

def pp_eqns(eqns: Sequence[OlympusprEqn],
            context: OlympusprPpContext, settings: OlympusprPpSettings) -> pp.Doc:
  return pp.join(
    pp.brk("; "),
...
def _compact_eqn_should_include(k: str, v: Any) -> bool:
  ...
  if isinstance(v, (Olympuspr, ClosedOlympuspr)): return False
  if (isinstance(v, tuple) and
      any(isinstance(e, (Olympuspr, ClosedOlympuspr)) for e in v)):
    return False
  return True
...
def pp_olympuspr_skeleton(olympuspr: Olympuspr, eqns_fn, context: OlympusprPpContext,
                      settings: OlympusprPpSettings) -> pp.Doc:
  constvars = pp_vars(olympuspr.constvars, context, print_shapes=settings.print_shapes)
  invars = pp_vars(olympuspr.invars, context, print_shapes=settings.print_shapes)
  eqns = eqns_fn()
  outvars = pp.concat([
    pp.text("("), pp_vars(olympuspr.outvars, context, separator=","),
    pp.text(")" if len(olympuspr.outvars) != 1 else ",)")])
  if settings.print_effects:
    # TODO(sharadmv): render an entire signature here
    eff_text = [pp.text(" : { ")]
    for i, eff in enumerate(olympuspr.effects):
      if i > 0:
        eff_text.append(pp.text(", "))
      if isinstance(eff, effects.OlympusprInputEffect):
        index = eff.input_index
        all_vars = [*olympuspr.constvars, *olympuspr.invars]
...
def pp_shared_olympuspr(
  ...
  return pp.concat([
      pp.text("let " + name + " = "),
      pp_olympuspr(olympuspr, context, settings),
      pp.text(" in"),
      pp.brk(),
  ])
...
def pp_olympuspr(
  ...
  if name := context.shared_olympusprs.get(olympuspr):
    return pp.text(name)
  eqns_fn = lambda: pp_eqns(olympuspr.eqns, context, settings)
  return pp_olympuspr_skeleton(olympuspr, eqns_fn, context, settings)


def pp_olympusprs(olympusprs: Sequence[ClosedOlympuspr | Olympuspr],
              context: OlympusprPpContext, settings: OlympusprPpSettings) -> pp.Doc:
  olympusprs = [j.olympuspr if isinstance(j, ClosedOlympuspr) else j for j in olympusprs]
  return pp.group(pp.concat([pp.nest(2, pp.concat([
      pp.text('('), pp.brk(""),
      pp.join(pp.brk(), map(lambda x: pp_olympuspr(x, context, settings), olympusprs))]
    )), pp.brk(""), pp.text(')')])
  )


def pp_olympuspr_eqn_range(olympuspr: Olympuspr, lo: int, hi: int, context: OlympusprPpContext,
                       settings: OlympusprPpSettings) -> pp.Doc:
  lo = max(lo, 0)
  hi = max(lo, min(hi, len(olympuspr.eqns)))
  eqns = olympuspr.eqns[lo:hi]
  def eqns_fn():
    pps = []
    if len(eqns) == 0 and len(olympuspr.eqns) != 0:
      pps.append(pp.text('...'))
    else:
      if lo != 0:
        pps.append(pp.text('...'))
      pps.extend(map((lambda e: pp_eqn(e, context, settings)), eqns))
      if hi != len(olympuspr.eqns):
        pps.append(pp.text('...'))
    return pp.join(pp.brk("; "), pps)
  return pp_olympuspr_skeleton(olympuspr, eqns_fn, context, settings)

def pp_effect(effect: Effect, context: OlympusprPpContext) -> pp.Doc:
...
def last_used(olympuspr: Olympuspr) -> dict[Var, OlympusprEqn | None]:
  """Returns a mapping from every var in olympuspr to what equation uses it last."""
  last_used: dict[Var, OlympusprEqn | None] = {
      v: None for v in olympuspr.outvars if not isinstance(v, Literal)}
  for eqn in reversed(olympuspr.eqns):
    for v in eqn.invars:
      if not isinstance(v, Literal) and v not in last_used:
        last_used[v] = eqn
  return last_used

def clean_up_dead_vars(eqn: OlympusprEqn, env: dict[Var, Any],
                       last_used: dict[Var, OlympusprEqn | None]):
...
# TODO(dougalm, jakevdp): expose these via olympus.extend

# Comparable object for checking whether OLYMPUS's trace state has changed.
...
def unsafe_am_i_under_a_jit() -> bool:
  return 'DynamicOlympusprTrace' in str(unsafe_get_trace_stack(trace_ctx.trace))
