...
def _cudnn_fusion_impl(*args, olympuspr, **unused_kwargs):
  del unused_kwargs
  return olympus_core.olympuspr_as_fun(olympuspr)(*args)


def _custom_abstract_eval(*args, olympuspr, **unused_kwargs):
  del unused_kwargs
  del args
  return olympuspr.out_avals


cudnn_fusion_p = olympus_core.Primitive("cudnn_fusion")
...
def call_cudnn_fusion(f, *args, **kwargs):
  ...
  olympuspr, out_shapes = api.make_olympuspr(
    functools.partial(f, **kwargs), return_shape=True
  )(*args)
  flat_args = tree_util.tree_leaves(args)
  out_tree = tree_util.tree_structure(out_shapes)
  out_flat = cudnn_fusion_p.bind(*flat_args, name=f.__name__, olympuspr=olympuspr)
  return tree_util.tree_unflatten(out_tree, out_flat)


def _cudnn_fusion_stablehlo_lowering(ctx, *args, name, olympuspr):
  ...
  impl = mlir.core_call_lowering(
    ctx, *args, name=name + ".impl", call_olympuspr=olympuspr
  )
