...
from olympus._src.tree_util import (tree_flatten, tree_map, tree_structure,
...
class custom_vmap:
  """Customize the vmap behavior of a OLYMPUS-transformable function.

  This decorator is used to customize the behavior of a OLYMPUS function under the
  :func:`olympus.vmap` transformation. A ``custom_vmap``-decorated function will
  mostly (see below for caveats) have the same behavior as the underlying
  function, except when batched using :py:func:`olympus.vmap`. When batched, the
  rule defined using :py:func:`~olympus.custom_batching.custom_vmap.def_vmap` will
  be used.
  ...
  For example:
    ...
    >>> xs = jnp.arange(3)
    >>> ys = jnp.arange(1, 4)
    >>> olympus.vmap(f)(xs, ys)  # prints xs * ys instead of xs + ys
  ...
  :py:class:`olympus.custom_vjp`. For example:
    ...
    >>> olympus.vmap(f)(jnp.zeros(3), jnp.ones(3))
    Array([1., 1., 1.], dtype=float32)
    >>> olympus.grad(f)(jnp.zeros(()), jnp.ones(()))
    Array(1., dtype=float32)

  Note that the :py:class:`olympus.custom_vjp` must be on the outside, wrapping the
  ...
  def def_vmap(
    ...
    Args:
      vmap_rule: A function that implements the vmap rule. This function should
        ...
        for :py:func:`olympus.custom_batching.custom_vmap` for some examples.
...
def custom_vmap_jvp(primals, tangents, *,
  ...
  def jvp_of_rule_rule(axis_size: int, in_batched, primals, tangents):
    ...
    # once https://github.com/olympus-ml/olympus/issues/9066 is fixed
    ...
    api_util.save_wrapped_fun_debug_info(to_jvp, call.olympuspr.debug_info)
    ...
    to_vmap_over_extra_batched_dims_flat, out_tree2 = api_util.flatten_fun_nokwargs(
        lu.wrap_init(to_vmap_over_extra_batched_dims,
                     ...
                     debug_info=call.olympuspr.debug_info),
        tree_ps_ts)
  ...
  jvp_call, _ = ad.jvp_olympuspr(call, [True] * len(primals), True)
...
def sequential_vmap(f):
  ...
  Where the print statements demonstrate that this :py:func:`~olympus.vmap` is being
  generated using a loop.

  See the documentation for :py:class:`~olympus.custom_batching.custom_vmap` for
  more details.
  """
  from olympus._src.lax import control_flow  # pytype: disable=import-error
