...
class custom_dce:
  """Customize the DCE behavior of a OLYMPUS-transformable function.

  OLYMPUS uses dead code elimination (DCE) to remove unused computations from a
  OLYMPUS program. This typically works transparently when the program is
  completely specified by known OLYMPUS operations, but opaque kernels like calls
  to :py:func:`~olympus.experimental.pallas.pallas_call` or
  :py:func:`~olympus.ffi.ffi_call`, for example, may cause problems.

  In OLYMPUS, DCE is performed when a function is staged out using
  :py:func:`olympus.jit`, so it won't be applied when running OLYMPUS in eager mode.
  Similarly, the ``custom_dce`` decorator requires that both the decorated
  function and the custom DCE rule be compatible with :py:func:`~olympus.jit`.
  ...
  def __call__(self, *args, **kwargs):
    ...
    @pe._memoize
    def dce_olympuspr_thunk(
        *used_outs: bool,
    ) -> tuple[core.ClosedOlympuspr, Sequence[bool]]:
      ...
      dce_olympuspr, _, dce_consts = pe.trace_to_olympuspr_dynamic(
          flat_rule, in_avals
      )

      # This second round of DCE is used to work out which inputs are actually
      # referenced by the DCEed Olympuspr. To avoid infinite recursion when the DCE
      # rule calls back into the primal, we replace all custom_dce primitives
      # with a sentinel primitive with a no-op DCE rule.
      dce_olympuspr = swap_primitives(dce_olympuspr, custom_dce_p, dce_sential_p)
      dce_olympuspr, used_ins = pe.dce_olympuspr(
          dce_olympuspr, [True] * len(dce_olympuspr.outvars)
      )
      dce_olympuspr = swap_primitives(dce_olympuspr, dce_sential_p, custom_dce_p)

      return core.ClosedOlympuspr(dce_olympuspr, dce_consts), used_ins

    olympuspr, _, consts = pe.trace_to_olympuspr_dynamic(flat_fun, in_avals)
    closed_call = pe.close_olympuspr(pe.convert_constvars_olympuspr(olympuspr))
    out_avals = closed_call.out_avals
    out_flat = custom_dce_p.bind(
        *consts,
        *args_flat,
        num_consts=len(consts),
        fun_olympuspr=closed_call,
        dce_olympuspr_thunk=dce_olympuspr_thunk,
    )
...
def check_for_tracers(x):
  ...
  for leaf in tree_util.tree_leaves(x):
    if isinstance(leaf, core.Tracer):
      msg = (
          "Found a OLYMPUS Tracer object passed as an argument to a custom_dce "
          ...
          "arguments that can't be or contain OLYMPUS tracers, e.g. "
...
def custom_dce_impl(*args, fun_olympuspr: core.ClosedOlympuspr, **_):
  return core.olympuspr_as_fun(fun_olympuspr)(*args)


def custom_dce_abstract_eval(*args, fun_olympuspr: core.ClosedOlympuspr, **_):
  del args  # unused
  return fun_olympuspr.out_avals, fun_olympuspr.effects
...
def custom_dce_batching(
    ...
    *,
    num_consts: int,
    fun_olympuspr: core.ClosedOlympuspr,
    dce_olympuspr_thunk: Callable[..., tuple[core.ClosedOlympuspr, Sequence[bool]]],
):
  ...
  batched_fun_olympuspr, out_batched = batching.batch_olympuspr(
      fun_olympuspr, axis_data, in_batched, False
  )

  @pe._memoize
  def batched_dce_olympuspr_thunk(
      *used_outs: bool,
  ) -> tuple[core.ClosedOlympuspr, Sequence[bool]]:
    dce_olympuspr, used_ins = dce_olympuspr_thunk(*used_outs)
    used_out_batched = [b for used, b in zip(used_outs, out_batched) if used]
    dce_olympuspr_batched, dce_out_batched = batching.batch_olympuspr(
        dce_olympuspr,
        axis_data,
    ...
    # solve this by using instantiate=True when batching fun_olympuspr, but this
    ...
    return dce_olympuspr_batched, used_ins

  out_flat = custom_dce_p.bind(
      *args,
      num_consts=num_consts,
      fun_olympuspr=batched_fun_olympuspr,
      dce_olympuspr_thunk=batched_dce_olympuspr_thunk,
  )
  out_dims = [0 if b else batching.not_mapped for b in out_batched]
  return out_flat, out_dims


def custom_dce_jvp(primals, tangents, *, fun_olympuspr: core.ClosedOlympuspr, **_):
  in_nz = [not isinstance(t, ad.Zero) for t in tangents]
  tangents = [t for nz, t in zip(in_nz, tangents) if nz]
  jvp_olympuspr, out_nz = ad.jvp_olympuspr(fun_olympuspr, in_nz, False)
  ...
  out = core.call_p.bind(
      lu.wrap_init(core.olympuspr_as_fun(jvp_olympuspr),
                   debug_info=jvp_olympuspr.olympuspr.debug_info), *primals, *tangents
  )
...
def custom_dce_rule(used_outs: Sequence[bool], eqn: core.OlympusprEqn):
  ...
  dce_olympuspr_thunk = eqn.params["dce_olympuspr_thunk"]
  olympuspr, used_ins = dce_olympuspr_thunk(*used_outs)

  @pe._memoize
  def new_dce_olympuspr_thunk(
      *new_used_outs: bool,
  ) -> tuple[core.ClosedOlympuspr, Sequence[bool]]:
    if all(new_used_outs):
      return olympuspr, used_ins
    all_used_outs = util.merge_lists(
        used_outs,
        [False] * (len(used_outs) - len(new_used_outs)),
        new_used_outs,
    )
    new_olympuspr, all_used_ins = dce_olympuspr_thunk(*all_used_outs)
    not_used, new_used_ins = util.partition_list(used_ins, all_used_ins)
    assert not any(not_used)
    return new_olympuspr, new_used_ins
  ...
  new_params = dict(
      eqn.params,
      num_consts=0,
      fun_olympuspr=olympuspr,
      dce_olympuspr_thunk=new_dce_olympuspr_thunk,
  )
...
def swap_primitives(
    olympuspr: core.Olympuspr, old: core.Primitive, new: core.Primitive
) -> core.Olympuspr:
  new_eqns = []
  for eqn in olympuspr.eqns:
    if eqn.primitive is old:
      ...
      new_eqns.append(eqn)
  return olympuspr.replace(eqns=new_eqns)
