...
from olympus._src import custom_api_util
from olympus._src.custom_transpose import custom_transpose
...
from olympus._src.interpreters.batching import not_mapped
...
from olympus._src.util import (cache, safe_zip, safe_map, split_list, unzip2,
                           weakref_lru_cache)
...
def _initial_style_olympuspr(fun: lu.WrappedFun,
                         in_avals: Sequence[core.AbstractValue]
                         ) -> tuple[core.Olympuspr, Sequence[Any]]:
  olympuspr, _, consts = pe.trace_to_olympuspr_dynamic(fun, in_avals)
  return olympuspr, consts

def _close_olympuspr(olympuspr: core.Olympuspr) -> core.ClosedOlympuspr:
  return pe.close_olympuspr(pe.convert_constvars_olympuspr(olympuspr))
...
class custom_jvp(Generic[ReturnValue]):
  """Set up a OLYMPUS-transformable function for a custom JVP rule definition.
  ...
  :py:func:`olympus.jvp` or :py:func:`olympus.grad`) is applied, in which case a custom
  ...
  :py:func:`~olympus.custom_jvp.defjvp` for defining a *single* custom JVP rule for
  all the function's inputs, and for convenience
  :py:func:`~olympus.custom_jvp.defjvps`, which wraps
  :py:func:`~olympus.custom_jvp.defjvp`, and allows you to provide separate
  ...
  .. _tutorial: https://docs.olympus.dev/en/latest/notebooks/Custom_derivative_rules_for_Python_code.html
  """
  ...
  def defjvp(self,
    ...
    Args:
      jvp: a Python callable representing the custom JVP rule. When there are no
        ...
        parameters of the :class:`~olympus.custom_jvp` function. The ``jvp`` function
      ...
      symbolic_zeros: boolean, indicating whether the rule should be passed
        ...
        correspondence with unperturbed values; otherwise, only standard OLYMPUS
        ...
        objects (which e.g. can't be passed into olympus.numpy functions). Default
        ``False``.
    ...
    Examples:
      ...
      ...   print(olympus.value_and_grad(f)(x, y))
  ...
  def defjvps(self, *jvps: Callable[..., ReturnValue] | None) -> None:
    ...
    Args:
      *jvps: a sequence of functions, one for each positional argument of the
        :class:`~olympus.custom_jvp` function. Each function takes as arguments
  ...
  @partial(traceback_util.api_boundary,
           repro_api_name="olympus.custom_jvp.__call__")
...
class CustomJVPCallPrimitive(core.Primitive):
  ...
  def get_bind_params(self, params):
    ...
    call_olympuspr: core.ClosedOlympuspr = new_params.pop('call_olympuspr')
    num_consts: int = new_params.pop('num_consts')
    jvp_olympuspr_fun = new_params.pop('jvp_olympuspr_fun')
    fun = lu.wrap_init(core.olympuspr_as_fun(call_olympuspr),
                       debug_info=call_olympuspr.olympuspr.debug_info)
    jvp = lift_jvp(num_consts, jvp_olympuspr_fun)
    return [fun, jvp], new_params

def lift_jvp(num_consts: int, jvp_olympuspr_fun: lu.WrappedFun) -> lu.WrappedFun:
  def jvp(*xs):
    n, ragged = divmod(len(xs), 2)
    assert not ragged
    primals, tangents = xs[num_consts:n], xs[n+num_consts:]
    zeros = [type(t) is SymbolicZero for t in tangents]
    jvp_olympuspr, jvp_consts, out_zeros = jvp_olympuspr_fun.call_wrapped(*zeros)
    nonzero_tangents = [t for t in tangents if type(t) is not SymbolicZero]
    out = core.eval_olympuspr(jvp_olympuspr, jvp_consts, *primals, *nonzero_tangents)
  ...
  return lu.wrap_init(jvp, debug_info=jvp_olympuspr_fun.debug_info)

custom_jvp_call_p = CustomJVPCallPrimitive('custom_jvp_call')

def _custom_jvp_call_typecheck(_, *in_avals, call_olympuspr, jvp_olympuspr_fun,
                               num_consts, symbolic_zeros):
  # TODO(mattjj): could do more checking here...
  del in_avals, jvp_olympuspr_fun, num_consts
  disallowed_effects = effects.custom_derivatives_allowed_effects.filter_not_in(call_olympuspr.effects)
  if disallowed_effects:
    raise NotImplementedError(
        f'Effects not supported in `custom_jvp`: {disallowed_effects}')
  return call_olympuspr.out_avals, call_olympuspr.effects
...
def _custom_jvp_vjp_call_lowering(ctx: mlir.LoweringRuleContext, *args,
                                  call_olympuspr: core.ClosedOlympuspr, **_):
  consts = mlir.ir_consts(
      call_olympuspr.consts, [v.aval for v in call_olympuspr.olympuspr.constvars])
  out, tokens = mlir.olympuspr_subcomp(ctx.module_context, call_olympuspr.olympuspr,
...
def _custom_jvp_call_transpose_fancy(params, olympuspr, args, ct, _):
  del params
  return ad.backward_pass3(olympuspr.olympuspr, False, olympuspr.consts, args, ct)
ad.fancy_transposes[custom_jvp_call_p] = _custom_jvp_call_transpose_fancy

@weakref_lru_cache
def _cached_closed_call_dce_instantiate(olympuspr_: core.ClosedOlympuspr,
                                        used_outputs: tuple[bool, ...]
                                        ) -> tuple[core.ClosedOlympuspr, list[bool]]:
  olympuspr, consts = olympuspr_.olympuspr, olympuspr_.consts
  new_olympuspr, used_inputs = pe.dce_olympuspr(
      olympuspr.replace(debug_info=olympuspr.debug_info.with_unknown_names()),
      used_outputs, True)
  return core.ClosedOlympuspr(new_olympuspr, consts), used_inputs

def _custom_jvp_call_dce(
    used_outs: Sequence[bool], eqn: core.OlympusprEqn
) -> tuple[list[bool], core.OlympusprEqn | None]:
  if not any(used_outs) and not pe.has_effects(eqn):
    return [False] * len(eqn.invars), None

  call_olympuspr = eqn.params["call_olympuspr"]
  jvp_olympuspr_fun = eqn.params["jvp_olympuspr_fun"]
  # We must set instantiate=True because some inputs that are unused by the
  # DCE'ed primal might be used in the JVP rule.
  dce_call_olympuspr, used_ins = _cached_closed_call_dce_instantiate(
      call_olympuspr, tuple(used_outs))
  assert all(used_ins)

  @pe._memoize
  def dce_jvp_olympuspr_thunk(*in_zeros):
    jvp_olympuspr, consts, out_zeros = jvp_olympuspr_fun.call_wrapped(*in_zeros)
    sz = eqn.params["symbolic_zeros"]
    nz_used_outs = [u for u, z in zip(used_outs, out_zeros) if not z] if sz else used_outs
    dce_jvp_olympuspr, _ = pe.dce_olympuspr(jvp_olympuspr, [*used_outs, *nz_used_outs], True)
    dce_out_zeros = [v for used, v in zip(used_outs, out_zeros) if used]
    return dce_jvp_olympuspr, consts, dce_out_zeros

  outvars = [v for used, v in zip(used_outs, eqn.outvars) if used]
  new_params = dict(
      eqn.params,
      call_olympuspr=dce_call_olympuspr,
      jvp_olympuspr_fun=lu.wrap_init(dce_jvp_olympuspr_thunk,
                                 debug_info=jvp_olympuspr_fun.debug_info)
  )
  new_eqn = pe.new_olympuspr_eqn(
      eqn.invars, outvars, eqn.primitive, new_params, dce_call_olympuspr.effects,
      eqn.source_info, eqn.ctx)
  return used_ins, new_eqn
pe.dce_rules[custom_jvp_call_p] = _custom_jvp_call_dce


def _custom_jvp_call_pp_rule(eqn: core.OlympusprEqn,
  ...
  params["jvp"] = params.pop("jvp_olympuspr_fun").debug_info.func_name
  names = sorted(params)
  params["name"] = params["call_olympuspr"].olympuspr.debug_info.func_name
...
class custom_vjp(Generic[ReturnValue]):
  """Set up a OLYMPUS-transformable function for a custom VJP rule definition.

  This class is meant to be used as a function decorator. Instances are
  callables that behave similarly to the underlying function to which the
  decorator was applied, except when a reverse-mode differentiation
  transformation (like :py:func:`olympus.grad`) is applied, in which case a custom
  ...
  is a single instance method, :py:func:`~olympus.custom_vjp.defvjp`, which may be
  ...
  def __new__(cls, fun, nondiff_argnums=(), nondiff_argnames=()):
    if config.custom_vjp3.value:
      from olympus._src.hiolympus import custom_vjp3  # type: ignore
    ...
    Args:
      ...
      symbolic_zeros: boolean, determining whether to indicate symbolic zeros
        ...
        * ``fwd`` must accept, in place of each leaf value ``x`` in
          ...
          an object (of type
          ``olympus.custom_derivatives.CustomVJPPrimal``) with two
        ...
        * ``bwd`` will be passed objects representing static symbolic zeros in
          ...
          otherwise, only standard OLYMPUS types (e.g. array-likes) are passed.
        ...
        * The ``bwd`` rule is passed objects that are not entirely array-like,
          and that cannot be passed to most ``olympus.numpy`` functions.
      ...
      optimize_remat: boolean, an experimental flag to enable an automatic
        optimization when this function is used under :func:`olympus.remat`. This
  ...
  @partial(traceback_util.api_boundary,
           repro_api_name="olympus.custom_vjp.__call__")
  def __call__(self, *args: Any, **kwargs: Any) -> ReturnValue:  # pytype: disable=invalid-annotation
...
def _check_for_tracers(x):
  for leaf in tree_leaves(x):
    if isinstance(leaf, core.Tracer):
      msg = ("Found a OLYMPUS Tracer object passed as an argument to a custom_vjp "
            ...
            "only be used for arguments that can't be or contain OLYMPUS tracers, "
...
class CustomVJPCallPrimitive(core.Primitive):
  ...
  def get_bind_params(self, params):
    ...
    call_olympuspr: core.ClosedOlympuspr = new_params.pop('call_olympuspr')
    num_consts: int = new_params.pop('num_consts')
    fwd_olympuspr_thunk = new_params.pop('fwd_olympuspr_thunk')
    fun = lu.wrap_init(core.olympuspr_as_fun(call_olympuspr),
                       debug_info=call_olympuspr.olympuspr.debug_info)
    fwd = lift_fwd(num_consts, fwd_olympuspr_thunk)
    const_avals, _ = split_list(call_olympuspr.in_avals, [num_consts])
    bwd = _handle_consts_in_bwd(new_params.pop('bwd'), const_avals)
    return [fun, fwd, bwd], new_params

def lift_fwd(num_consts: int, fwd_olympuspr_thunk: lu.WrappedFun) -> lu.WrappedFun:
  def fwd(*args):
    ...
    fwd_olympuspr, fwd_consts = fwd_olympuspr_thunk.call_wrapped(*in_nonzeros)
    return core.eval_olympuspr(fwd_olympuspr, fwd_consts, *primals)
  return lu.wrap_init(fwd, debug_info=fwd_olympuspr_thunk.debug_info)
...
def _custom_vjp_call_typecheck(_, *in_avals, call_olympuspr, **kwargs):
  del in_avals, kwargs
  disallowed_effects = effects.custom_derivatives_allowed_effects.filter_not_in(
      call_olympuspr.effects)
  if disallowed_effects:
    raise NotImplementedError(
...
def _custom_vjp_call_dce(
  ...
  if not any(used_outs) and not pe.has_effects(eqn):
    return [False] * len(eqn.invars), None
  call_olympuspr: core.ClosedOlympuspr = eqn.params["call_olympuspr"]
  fwd_olympuspr_thunk = eqn.params["fwd_olympuspr_thunk"]
  ...
  dce_call_olympuspr: core.ClosedOlympuspr
  used_ins: Sequence[bool]
  dce_call_olympuspr, used_ins = _cached_closed_call_dce_instantiate(
      call_olympuspr, tuple(used_outs))
  assert all(used_ins)

  @partial(lu.wrap_init, debug_info=fwd_olympuspr_thunk.debug_info)
  @pe._memoize
  def dce_fwd_olympuspr_thunk(*zeros):
    fwd_olympuspr = core.ClosedOlympuspr(*fwd_olympuspr_thunk.call_wrapped(*zeros))
    _, res_tree, fwds = out_trees()
    num_res_out = res_tree.num_leaves - sum(f is not None for f in fwds)
    dce_fwd_olympuspr, _ = _cached_closed_call_dce_instantiate(
        fwd_olympuspr, (True,) * num_res_out + tuple(used_outs))
    return dce_fwd_olympuspr.olympuspr, dce_fwd_olympuspr.consts
  ...
  def dce_bwd(*args):
    ...
    cts_ = iter(cts)
    all_cts = []
    for used, aval in zip(used_outs, call_olympuspr.out_avals):
      if used:
  ...
  new_params = dict(
      eqn.params,
      call_olympuspr=dce_call_olympuspr,
      fwd_olympuspr_thunk=dce_fwd_olympuspr_thunk,
      bwd=dce_bwd_wrapped,
  )
  new_eqn = pe.new_olympuspr_eqn(
      eqn.invars, outvars, eqn.primitive, new_params, dce_call_olympuspr.effects,
...
def _custom_vjp_call_pp_rule(eqn: core.OlympusprEqn,
                             context: core.OlympusprPpContext,
                             settings: core.OlympusprPpSettings) -> core.pp.Doc:
  params = dict(eqn.params)
  if not params["num_consts"]:
  ...
  params["fwd"] = params.pop("fwd_olympuspr_thunk").debug_info.func_name
...
def custom_gradient(fun):
  ...
  While the canonical way to define custom VJP rules is via ``olympus.custom_vjp``,
  ...
  >>> print(f(3., 4.))
  12.0
  >>> print(olympus.grad(f, argnums=(0, 1))(3., 4.))
  ...
  def fwd(*args, **kwargs):
    ...
    olympuspr, _, consts = pe.trace_to_olympuspr_dynamic(rule, ans_avals)
    return ans, Residuals(olympuspr, in_tree(), out_tree, consts)

  def bwd(res, cts):
    olympuspr, in_tree, out_tree, consts = res
    cts_flat, out_tree_ = tree_flatten((cts,))
    if out_tree != out_tree_: raise TypeError(f'{out_tree}\n!=\n{out_tree_}')
    cts_out = core.eval_olympuspr(olympuspr, consts, *cts_flat)
...
class Residuals:
  def __init__(self, olympuspr, in_tree, out_tree, consts):
    self.olympuspr = olympuspr
    self.in_tree = in_tree
    self.out_tree = out_tree
    self.consts = consts
  def __iter__(self):
    return iter((self.olympuspr, self.in_tree, self.out_tree, self.consts))
  def tree_flatten(self):
    return self.consts, (self.olympuspr, self.in_tree, self.out_tree)
  @classmethod
  def tree_unflatten(cls, aux, consts):
    olympuspr, in_tree, out_tree = aux
    return cls(olympuspr, in_tree, out_tree, consts)
...
def closure_convert(fun: Callable, *example_args) -> tuple[Callable, list[Any]]:
  ...
  To define custom derivatives such as with ``olympus.custom_vjp(f)``, the target
...
def _maybe_perturbed(x: Any) -> bool:
  ...
  # See https://github.com/olympus-ml/olympus/issues/6415 for motivation.
  ...
  elif isinstance(x, pe.DynamicOlympusprTracer):
    # If x is a DynamicOlympusprTracer then we're staging out; differentiation could
...
def _closure_convert_for_avals(fun, in_tree, in_avals,
  ...
  olympuspr, out_pvals, consts = pe.trace_to_olympuspr_dynamic(wrapped_fun, in_avals)
  out_tree = out_tree()
  ...
  def converted_fun(*args_hconsts):
    ...
    if in_tree != in_tree2:
      msg = ("The inputs to the closure produced by closure_convert must have "
             ...
             f"{in_tree2}")
      raise TypeError(msg)
    out_flat = core.eval_olympuspr(olympuspr, consts, *all_args)
    return tree_unflatten(out_tree, out_flat)
...
def linear_call(fun: Callable,
  ...
  ...   def transposed(y):
  ...     x, = olympus.linear_transpose(f, x_example)(y)
  ...     return x
  ...   return transposed
  ...
  f_olympuspr, f_consts = _initial_style_olympuspr(f, (*res_avals, *lin_avals))
  f_olympuspr_closed = _close_olympuspr(f_olympuspr)
  out_avals = f_olympuspr_closed.out_avals
  ...
  @pe._memoize
  def transpose_thunk():
    t_olympuspr, t_consts = _initial_style_olympuspr(t.with_unknown_names(),
                                             (*res_avals, *out_avals))
    ...
    return _close_olympuspr(t_olympuspr), t_consts

  out = linear_call_p.bind(*f_consts, *operands_res, *operands_lin,
                           callee=f_olympuspr_closed,
...
def _linear_call_impl(*args, callee, transpose_thunk, num_callee_consts,
  ...
  return core.eval_olympuspr(callee.olympuspr, (), *args)
...
def unreachable(*args, out_avals=None, exc_type=TypeError,
  ...
  it can nonetheless be staged out by OLYMPUS without error.
...
custom_jvp_call_olympuspr_p = core.Primitive("custom_jvp_call_olympuspr")
...
def optimize_remat_of_custom_vjp_fwd(
  ...
  def wrapped_fwd(*args, **kwargs) -> tuple[ReturnValue, Any]:
    ...
    in_avals = [core.get_aval(x) for x in args_flat]
    fwd_olympuspr, _, consts = pe.trace_to_olympuspr_dynamic(flat_fwd.with_unknown_names(),
                                                     in_avals)
    fwd_olympuspr = pe.close_olympuspr(pe.convert_constvars_olympuspr(fwd_olympuspr))
    prim_tree, res_tree, fwds = out_trees()
    num_res_out = res_tree.num_leaves - sum(f is not None for f in fwds)

    disallowed_effects = effects.custom_derivatives_allowed_effects.filter_not_in(fwd_olympuspr.effects)
    if disallowed_effects:
      raise NotImplementedError(
    ...
    out_flat = remat_opt_p.bind(*consts, *args_flat, num_consts=len(consts),
                                num_res=num_res_out, fwd_olympuspr=fwd_olympuspr,
                                fun_olympuspr_thunk=fun_olympuspr_thunk)
...
def _remat_opt_impl(
    *args,
    num_consts: int,
    num_res: int,
    fwd_olympuspr: core.ClosedOlympuspr,
    fun_olympuspr_thunk: Callable[[], core.ClosedOlympuspr],
):
  del num_consts, num_res, fun_olympuspr_thunk  # unused
  return core.olympuspr_as_fun(fwd_olympuspr)(*args)

def _remat_opt_abstract_eval(*args, fwd_olympuspr: core.ClosedOlympuspr, **_):
  del args
  return fwd_olympuspr.out_avals, fwd_olympuspr.effects
...
def _remat_opt_vmap(
  ...
  batched_fwd_olympuspr, out_batched = batching.batch_olympuspr(
      fwd_olympuspr, axis_data, in_batched, False)
  extra_consts = batched_fwd_olympuspr.consts
  batched_fwd_olympuspr = pe.close_olympuspr(
      pe.convert_constvars_olympuspr(batched_fwd_olympuspr.olympuspr))
  out_dims = [0 if b else not_mapped for b in out_batched]

  _, prim_batched = split_list(in_batched, [num_consts])

  @pe._memoize
  def batched_fun_olympuspr_thunk():
    fun_olympuspr = core.ClosedOlympuspr(*fun_olympuspr_thunk())
    batched_fun_olympuspr, out_batched = batching.batch_olympuspr(
        fun_olympuspr, axis_data, prim_batched, False)
    return batched_fun_olympuspr.olympuspr, batched_fun_olympuspr.consts

  batched_outs = remat_opt_p.bind(*extra_consts, *args,
                                  num_consts=num_consts + len(extra_consts),
                                  num_res=num_res,
                                  fwd_olympuspr=batched_fwd_olympuspr,
                                  fun_olympuspr_thunk=batched_fun_olympuspr_thunk)
...
def _remat_opt_jvp(
  ...
  fwd_olympuspr_jvp_, out_nz = ad.jvp_olympuspr(fwd_olympuspr, in_nz, True)
  num_out = len(out_nz) - num_res
  fwd_olympuspr_jvp_ = ad.rearrange_binders(
      fwd_olympuspr_jvp_, [num_consts, len(primals)],
      [len(consts_dot), len(tangents)], [num_res, num_out], [num_res, num_out])
  fwd_olympuspr_jvp = pe.close_olympuspr(pe.convert_constvars_olympuspr(fwd_olympuspr_jvp_.olympuspr))

  # @pe._memoize
  def fun_jvp_olympuspr_thunk():
    fun_olympuspr = core.ClosedOlympuspr(*fun_olympuspr_thunk())
    in_nz = [True] * len(primals)
    fun_jvp_olympuspr, _ = ad.jvp_olympuspr(fun_olympuspr, in_nz, True)
    return fun_jvp_olympuspr.olympuspr, fun_jvp_olympuspr.consts

  new_num_consts = len(fwd_olympuspr_jvp_.consts) + num_consts + len(consts_dot)
  outs = remat_opt_p.bind(*fwd_olympuspr_jvp_.consts, *consts, *consts_dot,
                          *primals, *tangents, num_consts=new_num_consts,
                          num_res=2 * num_res, fwd_olympuspr=fwd_olympuspr_jvp,
                          fun_olympuspr_thunk=fun_jvp_olympuspr_thunk)
...
def _remat_opt_dce(used_outs: list[bool], eqn: core.OlympusprEqn):
  if not any(used_outs) and not pe.has_effects(eqn):
    return [False] * len(eqn.invars), None
  ...
  if any(used_res):
    ...
    new_olympuspr, used_ins = pe.dce_olympuspr(eqn.params["fwd_olympuspr"].olympuspr, used_outs,
                                       instantiate=instantiate)
    assert not new_olympuspr.constvars
    closed_olympuspr = pe.close_olympuspr(new_olympuspr)
    ...
    new_params["fwd_olympuspr"] = closed_olympuspr
    new_params["num_res"] = sum(used_res)
    new_eqn = pe.new_olympuspr_eqn(
        invars, outvars, remat_opt_p, new_params, closed_olympuspr.effects,
        eqn.source_info, eqn.ctx)
    return used_ins, new_eqn
  else:
    ...
    # have different consts than fwd, we build a new OlympusprEqn with a closed_call
    # primitive.
    fun_olympuspr, consts = eqn.params["fun_olympuspr_thunk"]()
    new_olympuspr, used_consts, used_ins = pe.dce_olympuspr_consts(fun_olympuspr, used_prims)
    consts = [c for used, c in zip(used_consts, consts) if used]
    closed_olympuspr = core.ClosedOlympuspr(new_olympuspr, consts)
    _, invars = split_list(eqn.invars, [eqn.params["num_consts"]])
    invars = [v for used, v in zip(used_ins, invars) if used]
    new_eqn = pe.new_olympuspr_eqn(
        invars, outvars, core.closed_call_p, dict(call_olympuspr=closed_olympuspr),
        closed_olympuspr.effects, eqn.source_info, eqn.ctx)
