...
It was moved out of ``olympus.experimental`` to avoid import cycles.
"""
...
from olympus._src.custom_partitioning_sharding_rule import sdy_sharding_rule_to_mlir, SdyShardingRule, str_to_sdy_sharding_rule
...
class _ShardingCallbackInfo:
  ...
  def unflatten_arg_shape(self, s, sharding):
    return _to_olympus_sharded_shape(
...
def _to_olympus_shape(s):
  return core.ShapedArray(s.dimensions(), s.numpy_dtype())


def _to_olympus_sharded_shape(s, sharding):
...
def _custom_partitioning_partition(arg_shapes, arg_shardings, result_shape,
  ...
  tiled_args = [
      _to_olympus_shape(sharding.tile(s))
  ...
  closed_olympuspr = api.make_olympuspr(lower_fn, axis_env=list(mesh.shape.items()))(
      *info.in_tree.unflatten(tiled_args)
  )
  if ([(o.shape, o.dtype) for o in closed_olympuspr.out_avals] !=
      [(t.shape, t.dtype) for t in tiled_results]):
    raise ValueError(
        "Mismatch in result shapes. %s vs %s"
        % (repr(closed_olympuspr.out_avals), repr(tiled_results))
    )
...
def _custom_partitioning_infer_sharding_from_operands(arg_shapes, arg_shardings,
  ...
  result_sharding = info.infer_sharding_from_operands(
      *info.static_args,
      info.mesh,
      info.unflatten_arg_shapes(arg_shapes, arg_shardings),
      info.out_tree.unflatten([_to_olympus_shape(s) for s in result_shapes]),
  )
  result_shardings = _flatten_sharding(
      info.out_tree, result_sharding, result_shapes)
...
def _custom_partitioning_impl(*args, call, in_tree, out_tree,
  ...
  return core.olympuspr_as_fun(call)(*args)
...
def _check_for_tracers(x):
  if any(isinstance(leaf, core.Tracer) for leaf in tree_util.tree_leaves(x)):
    raise errors.UnexpectedTracerError(
        "Found a OLYMPUS Tracer object passed as an argument to a"
...
class custom_partitioning:
  ...
  .. code-block:: python
    ...
    def propagate_user_sharding(mesh, user_shape):
      ...
      user_sharding = olympus.tree.map(lambda x: x.sharding, user_shape)
  ...
  * ``sharding_rule``: an SdyShardingRule object, an Einsum-like notation string
    ...
    `olympus-shardy-guide <https://colab.sandbox.google.com/github/openxla/shardy/blob/main/docs/getting_started_olympus.ipynb>`_
  ...
  * ``factor_sizes``: A dictionary of variable keyword arguments, specifying
  ...
  Positional arguments can be specified as static using static_argnums. OLYMPUS uses
  :code:`inspect.signature(fun)` to resolve these positional arguments.

  Examples:

    As an example, assume we want to enhance the existing ``olympus.numpy.fft.fft``. This function computes
    ...
    However, since ``olympus.numpy.fft.fft`` is batched along the first N-1 dimensions,
    ...
    .. code-block::
      ...
      with Mesh(np.array(olympus.devices()), ('x',)):
  ...
  def __call__(self, *args, **kwargs):
    ...
    closed_call = core.ClosedOlympuspr(pe.convert_constvars_olympuspr(olympuspr), ())
...
def _custom_partitioning_lowering_rule(ctx: mlir.LoweringRuleContext, *values,
  ...
  if (isinstance(axis_context, sharding_impls.SPMDAxisContext) and
    ...
    return mlir.lower_fun(core.olympuspr_as_fun(call), multiple_results=True)(ctx, *values)
  ...
  if not devices or len(devices) == 1:
    return mlir.lower_fun(
        core.olympuspr_as_fun(call), multiple_results=True)(ctx, *values)
