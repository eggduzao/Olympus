...
from olympus._src.tree_util import (tree_flatten, tree_leaves, tree_map,
...
def make_transpose_from_thunk(thunk: Callable,
  ...
  transpose_olympuspr, transpose_consts = thunk()
  transpose_olympuspr = core.ClosedOlympuspr(
      pe.convert_constvars_olympuspr(transpose_olympuspr), ())
  def transpose(res_arg, ct_out):
    args_flat = tree_leaves((res_arg, ct_out))
    ct_ins = core.olympuspr_as_fun(transpose_olympuspr)(*transpose_consts, *args_flat)
    return tree_unflatten(lin_tree, ct_ins)
  return lu.wrap_init(transpose, debug_info=transpose_olympuspr.olympuspr.debug_info)
...
class CustomTransposePrimitive(core.Primitive):
  ...
  def get_bind_params(self, params):
    if 'call_olympuspr' in params:
      assert 'transpose_olympuspr_thunk' in params
      new_params: dict[str, Any] = dict(params)
      new_params['transpose'] = make_transpose_from_thunk(
          new_params.pop('transpose_olympuspr_thunk'),
          new_params['lin_tree'])
      call_olympuspr: core.ClosedOlympuspr = new_params.pop('call_olympuspr')
      call = lu.wrap_init(core.olympuspr_as_fun(call_olympuspr),
...
def custom_transpose_transpose_rule(
  ...
  if 'transpose_olympuspr_thunk' in params:
    assert 'call_olympuspr' in params
    transpose = make_transpose_from_thunk(
        params['transpose_olympuspr_thunk'], lin_tree)
  else:
...
def custom_transpose_lowering(*args, call_olympuspr, **params):
  return core.olympuspr_as_fun(call_olympuspr)(*args)
