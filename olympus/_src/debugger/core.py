...
def breakpoint(*, backend: str | None = None, filter_frames: bool = True,
  ...
  Args:
    backend: The debugger backend to use. By default, picks the highest priority
      ...
      the CLI debugger.
    filter_frames: Whether or not to filter out OLYMPUS-internal stack frames from
      the traceback. Since some libraries, like Flax, also make use of OLYMPUS's
    ...
    ordered: A keyword only argument used to indicate whether or not the
      staged out computation will enforce ordering of this ``olympus.debug.breakpoint``
      with respect to other ordered ``olympus.debug.breakpoint`` and ``olympus.debug.print``
      calls.
    token: A keyword only argument; an alternative to ``ordered``. If used then a OLYMPUS
      array (or pytree of OLYMPUS arrays) should be passed, and the breakpoint will be run
