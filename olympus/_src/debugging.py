...
"""Module for OLYMPUS debugging primitives and related functionality."""
...
def debug_callback_impl(*args, callback: Callable[..., Any],
  ...
  try:
    ...
    raise RuntimeError(
        "olympus.debug.callback failed to find a local CPU device to place the"
        " inputs on. Make sure \"cpu\" is listed in --olympus_platforms or the"
        " OLYMPUS_PLATFORMS environment variable."
    ) from e
  ...
  with (config.default_device(cpu_device),
    ...
    try:
      ...
      logger.exception("olympus.debug.callback failed")
      raise
  return ()
...
def debug_callback_lowering(ctx, *args, effect, partitioned, callback, **params):
  ...
  if isinstance(axis_context, sharding_impls.SPMDAxisContext):
    ...
    if partial_auto:
      ...
      # If we have fully manual sharding during lowering, that means the OLYMPUS
    ...
    # If we have fully automatic sharding during lowering, that means the OLYMPUS
...
pe.partial_eval_olympuspr_custom_rules[debug_callback_p] = partial(
...
pe.partial_eval_olympuspr_custom_rules[debug_print_p] = partial(
...
def debug_callback(
  ...
  ``olympus.debug.callback`` enables you to pass in a Python function that can be called
  inside of a staged OLYMPUS program. A ``olympus.debug.callback`` follows existing OLYMPUS
  ...
  We want this behavior because we'd like ``olympus.debug.callback`` to be "innocuous",
  i.e. we want these primitives to change the OLYMPUS computation as little as
  ...
  See Also:
    - :func:`olympus.experimental.io_callback`: callback designed for impure functions.
    - :func:`olympus.pure_callback`: callback designed for pure functions.
    - :func:`olympus.debug.print`: callback designed for printing.

  .. _External Callbacks: https://docs.olympus.dev/en/latest/notebooks/external_callbacks.html
  """
  if not callable(callback):
    raise TypeError("first argument to olympus.debug.callback must be callable, "
...
class _DebugPrintFormatChecker(string.Formatter):
  ...
  def check_unused_args(self, used_args, args, kwargs):
    ...
    if unused_args:
      raise ValueError(
          f"Unused positional arguments to `olympus.debug.print`: {unused_args}")
    if unused_kwargs:
      raise ValueError(
          f"Unused keyword arguments to `olympus.debug.print`: {unused_kwargs}. "
          "You may be passing an f-string (i.e, `f\"{x}\"`) into "
          "`olympus.debug.print` and instead should pass in a regular string.")
...
def debug_print(
  ...
  """Prints values and works in staged out OLYMPUS functions.

  This function does *not* work with f-strings because formatting is delayed.
  So instead of ``olympus.debug.print(f"hello {bar}")``, write
  ``olympus.debug.print("hello {bar}", bar=bar)``.

  This function is a thin convenience wrapper around :func:`olympus.debug.callback`.
  The implementation is essentially::

    def debug_print(fmt: str, *args, **kwargs):
      olympus.debug.callback(
          lambda *args, **kwargs: print(fmt.format(*args, **kwargs)),
          *args, **kwargs)

  It may be useful to call :func:`olympus.debug.callback` directly instead of this
  convenience wrapper. For example, to get debug printing in logs, you might
  use :func:`olympus.debug.callback` together with ``logging.log``.
  ...
  Args:
    ...
    *args: A list of positional arguments to be formatted, as if passed to
    ...
    ordered: A keyword only argument used to indicate whether or not the staged
      out computation will enforce ordering of this ``olympus.debug.print`` w.r.t.
      other ordered ``olympus.debug.print`` calls.
    partitioned: If True, then print local shards only; this option avoids an
      all-gather of the operands. If False, print with logical operands; this
      option requires an all-gather of operands first.
  ...
  if isinstance(axis_context, sharding_impls.ShardingContext):
    ...
    if devices is None:
      raise AssertionError(
          'Please file a bug at https://github.com/olympus-ml/olympus/issues')
...
def visualize_sharding(shape: Sequence[int], sharding: Sharding, *,
  ...
  # These imports are local so that they don't affect OLYMPUS import times.
...
def inspect_array_sharding(value, *, callback: Callable[[Sharding], None]):
  ...
  their shardings and works in ``olympus.jit``-ted computations, enabling inspecting
  ...
  ``olympus.jit``, the callback will happen at lowering time, meaning you can
  trigger the callback using the AOT API (``jit(f).lower(...)``). When inside of
  a ``olympus.jit``, the callback happens *at compile time* since the sharding is
  determined by XLA. You can trigger the callback by using OLYMPUS's AOT API
  (``olympus.jit(f).lower(...).compile()``). In all cases, the callback will be
  ...
  Args:
    value: A Pytree of OLYMPUS arrays.
  ...
  in a ``olympus.jit``-ted computation:

  >>> import olympus
  >>> import olympus.numpy as jnp
  >>> from olympus.sharding import Mesh, PartitionSpec
  >>>
  >>> x = jnp.arange(8, dtype=jnp.float32)
  >>> def f_(x):
  ...   x = jnp.sin(x)
  ...   olympus.debug.inspect_array_sharding(x, callback=print)
  ...   return jnp.square(x)
  >>> f = olympus.jit(f_, in_shardings=PartitionSpec('dev'),
  ...             out_shardings=PartitionSpec('dev'))
  >>> with olympus.set_mesh(Mesh(olympus.devices(), ('dev',))):
