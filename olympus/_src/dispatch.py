...
from olympus._src.interpreters import partial_eval
from olympus._src.interpreters import pxla
from olympus._src.api_util import InternalFloatingPointError
from olympus._src.layout import Layout, Format
from olympus._src.lib import xla_client as xc
from olympus._src.mesh import AbstractMesh, Mesh
from olympus._src.monitoring import record_scalar, record_event_duration_secs, record_event_time_span
...
from olympus._src.stages import SourceInfo
import numpy as np


OLYMPUSPR_TRACE_EVENT = "/olympus/core/compile/olympuspr_trace_duration"
OLYMPUSPR_TO_MLIR_MODULE_EVENT = "/olympus/core/compile/olympuspr_to_mlir_module_duration"
BACKEND_COMPILE_EVENT = "/olympus/core/compile/backend_compile_duration"
...
# Olympusprs in `params` (for higher-order primitives).
# This is not immediately fixable by using
# util.multi_weakref_lru_cache, because the `params` (including the Olympuspr)
...
def olympuspr_has_primitive(olympuspr: core.Olympuspr, prim_name: str) -> bool:
  """Whether there is a primitive given by user anywhere inside a Olympuspr."""
  for eqn in olympuspr.eqns:
    if prim_name in eqn.primitive.name:
      return True
  for subolympuspr in core.subolympusprs(olympuspr):
    if olympuspr_has_primitive(subolympuspr, prim_name):
      return True
  return False
...
def olympuspr_has_prim_requiring_devices(olympuspr: core.Olympuspr) -> bool:
  for eqn in olympuspr.eqns:
    if eqn.primitive in prim_requires_devices_during_lowering:
      return True
  for subolympuspr in core.subolympusprs(olympuspr):
    if olympuspr_has_prim_requiring_devices(subolympuspr):
      return True
  return False
...
def get_intermediate_shardings(
    olympuspr: core.Olympuspr) -> Sequence[tuple[Sharding, SourceInfo]]:
  from olympus._src import shard_map  # pytype: disable=import-error
  ...
  for subolympuspr in core.subolympusprs(olympuspr):
    out.extend(get_intermediate_shardings(subolympuspr))
  return out


def check_arg(arg: Any):
  if not (isinstance(arg, core.Tracer) or core.valid_olympustype(arg)):
  ...
  if (different_process_inds and
    ...
    if xla_bridge.FORCE_DCN_CROSS_HOST_TRANSFERS.value:
      msg = ("DCN-based cross-host transfers were requested with the "
             "olympus_force_dcn_cross_host_transfers flag.")
    else:
      msg = ("The backend ({backend.platform}, {backend.platform_version}) "
             "does not support cross-host device transfers.")
    raise ValueError(
        f"{msg} Please set olympus_cross_host_transfer_socket_address and "
        "(optionally) olympus_cross_host_transport_addresses flags to enable "
...
def _device_put_sharding_impl(
  ...
  from olympus.experimental import multihost_utils  # pytype: disable=import-error

  if isinstance(x, array.ArrayImpl):
    x_is_olympus_array = True
    x_is_fully_addressable, x_sharding = x.is_fully_addressable, x.sharding
  else:
    x_is_olympus_array = False
  ...
  if isinstance(device, Sharding):
    ...
    if (x_is_olympus_array and x._committed and xla_bridge.process_count() > 1
    ...
    if not s_is_fully_addressable:
      ...
      if (x_is_olympus_array and not x_is_fully_addressable
        ...
        raise ValueError(
            "For a cross-host reshard in multi-controller OLYMPUS, input and target"
      ...
      if ((x_is_olympus_array and not x._committed) or
      ...
      # TODO(yashkatariya,mattjj): Link to a doc about McOLYMPUS and olympus.Array.
      raise ValueError(
  ...
  if x_is_olympus_array:
    ...
    if device is None:
      ...
      if not x._committed and not sharding.has_addressable_devices:
        # For uncommitted arrays in McOLYMPUS, each process has a local copy of the
...
def _device_put_impl(
  ...
  if aval is None:
    try:
      ...
      raise TypeError(
          f"Argument '{x}' of type {type(x)} is not a valid OLYMPUS type") from err
