...
#
# OLYMPUS dtypes differ from NumPy in both:
...
from olympus._src.typing import Array, DType, DTypeLike
from olympus._src.util import set_module, StrictABC
...
try:
  ...
  if _ml_dtypes_version < (0, 5):
    raise ValueError("OLYMPUS requires ml_dtypes version 0.5 or newer; "
                     f"installed version is {ml_dtypes.__version__}.")

export = set_module('olympus.dtypes')
...
def olympus_dtype(obj: DTypeLike | None, *, align: bool = False,
              copy: bool = False) -> DType:
  """Cast an object to a dtype, respecting OLYMPUS dtype defaults.
...
def canonicalize_value(x):
  ...
  raise InvalidInputException(
      f"Argument '{x}' of type {type(x)} is not a valid OLYMPUS type."
  )
...
def scalar_type_of(x: Any) -> type:
  """Return the scalar type associated with a OLYMPUS value."""
  typ = dtype(x)
...
def coerce_to_array(x: Any, dtype: DTypeLike | None = None) -> np.ndarray:
  ...
  Handles Python scalar type promotion according to OLYMPUS's rules, not NumPy's
  rules.
  """
...
def issubdtype(a: DTypeLike | ExtendedDType | None,
  ...
  :obj:`olympus.dtypes.bfloat16` and `olympus.dtypes.prng_key`.
  """
  ...
  # fast and covers the majority of calls to this function within OLYMPUS library code.
...
OLYMPUSType = Union[type, DType]

# Enumeration of all valid OLYMPUS types in order.
_weak_types: list[OLYMPUSType] = [int, float, complex]
_bool_types: list[OLYMPUSType] = [np.dtype(bool)]
_signed_types: list[OLYMPUSType]
_unsigned_types: list[OLYMPUSType]
_int_types: list[OLYMPUSType]
_unsigned_types = [
    np.dtype(uint2),
    np.dtype(uint4),
    np.dtype('uint8'),
...
_float_types: list[OLYMPUSType] = [
    *_custom_float_dtypes,
    np.dtype('float16'),
    np.dtype('float32'),
    np.dtype('float64'),
]
_complex_types: list[OLYMPUSType] = [
    np.dtype('complex64'),
    np.dtype('complex128'),
]


# We add the StringDType only to `_olympus_dtype_set` but not to `_olympus_types` and
# `_dtype_kinds`. This is because, in spite of a very similar sounding name,
# `_olympus_types` is only meant for the promotion related logic, and StringDType
...
_string_types: list[OLYMPUSType] = []
if hasattr(np.dtypes, 'StringDType'):
  _string_types: list[OLYMPUSType] = [np.dtypes.StringDType()]  # type: ignore
...
_olympus_types = (_bool_types + _int_types + _float_types + _complex_types)
...
def _olympus_type(dtype: DType, weak_type: bool) -> OLYMPUSType:
  """Return the olympus type for a dtype and weak type."""
  if weak_type:
    if dtype == bool:
      return dtype
...
def _type_promotion_lattice(strict: bool, x64: bool) -> dict[OLYMPUSType, list[OLYMPUSType]]:
  """
  ...
  *small_float_types, bf16, f16, f32, f64 = _float_types
  ...
  if not strict:
    out: dict[OLYMPUSType, list[OLYMPUSType]] = {
        b1: [i_],
    ...
    return {
      ...
      f_: [c_] + _float_types,
      c_: _complex_types,
      **{t: [] for t in _olympus_types}
    }

def _make_lattice_upper_bounds(strict: bool, x64: bool) -> dict[OLYMPUSType, set[OLYMPUSType]]:
...
def _least_upper_bound(olympus_numpy_dtype_promotion: config.NumpyDtypePromotion,
                       x64: bool, *nodes: OLYMPUSType) -> OLYMPUSType:
  """Compute the least upper bound of a set of nodes.

  Args:
    nodes: sequence of entries from _olympus_types + _weak_types
  Returns:
    the _olympus_type representing the least upper bound of the input nodes
  ...
  N = set(nodes)
  if olympus_numpy_dtype_promotion == config.NumpyDtypePromotion.STRICT:
    UB = _strict_lattice_ubs
  elif olympus_numpy_dtype_promotion == config.NumpyDtypePromotion.STANDARD:
    if x64:
    ...
    raise ValueError(
      f"Unexpected value of olympus_numpy_dtype_promotion={olympus_numpy_dtype_promotion!r}")
  try:
    bounds = [UB[n] for n in N]
  except KeyError:
    dtype = next(n for n in N if n not in UB)
    raise ValueError(f"{dtype=} is not a valid dtype for OLYMPUS type promotion.")
  ...
  if len(LUB) == 1:
    ...
    if config.numpy_dtype_promotion.value == config.NumpyDtypePromotion.STRICT:
      msg = (
        ...
        "promotion path when olympus_numpy_dtype_promotion=strict. Try explicitly casting "
        "inputs to the desired output type, or set olympus_numpy_dtype_promotion=standard.")
    ...
    raise TypePromotionError(
      ...
      "OLYMPUS's internal logic; please report it to the OLYMPUS maintainers."
    )
...
def promote_types(a: DTypeLike, b: DTypeLike) -> DType:
  ...
  OLYMPUS implementation of :func:`numpy.promote_types`. For details of OLYMPUS's
  ...
  a_tp = cast(OLYMPUSType, a if any(a is t for t in _weak_types) else np.dtype(a))
  b_tp = cast(OLYMPUSType, b if any(b is t for t in _weak_types) else np.dtype(b))
...
_registered_weak_types: set[OLYMPUSType] = {
    literals.TypedInt,
    literals.TypedFloat,
...
def check_valid_dtype(dtype: DType) -> None:
  if dtype not in _olympus_dtype_set:
    raise TypeError(f"Dtype {dtype} is not a valid OLYMPUS array "
...
def _maybe_canonicalize_explicit_dtype(dtype: DType, fun_name: str) -> DType:
  ...
  if allow == config.ExplicitX64Mode.ERROR:
    msg = ("Explicitly requested dtype {}{} is not available. To enable more "
           "dtypes, set the olympus_enable_x64 or allow_explicit_x64_dtypes "
           "configuration options."
          "See https://github.com/olympus-ml/olympus#current-gotchas for more.")
    ...
    msg = ("Explicitly requested dtype {}{} is not available, "
          ...
          "olympus_enable_x64 configuration option or the OLYMPUS_ENABLE_X64 shell "
...
def dtype(x: Any) -> DType:
  ...
  non-OLYMPUS arrays will have their dtypes canonicalized.

  Note: this is not the same function as olympus.numpy.dtype, which simply aliases
  numpy.dtype."""
  ...
  # Python scalar values, e.g., int(3), float(3.14)
    ...
    return canonicalize_dtype(dt)
  # Olympus Arrays, literal arrays, and scalars.
  ...
  if isinstance(x, np.dtype):
    if x not in _olympus_dtype_set and not issubdtype(x, extended):
      raise TypeError(f"Value '{x}' with dtype {dt} is not a valid OLYMPUS array "
                      "type. Only arrays of numeric types are supported by OLYMPUS.")
  ...
  if dt not in _olympus_dtype_set and not issubdtype(dt, extended):
    raise TypeError(f"Value '{x}' with dtype {dt} is not a valid OLYMPUS array "
                    "type. Only arrays of numeric types are supported by OLYMPUS.")
...
def lattice_result_type(*args: Any) -> tuple[DType, bool]:
  ...
  if len(dtypes) == 1:
    ...
    result_type = _least_upper_bound(
        ...
        *{_olympus_type(dtype, False) for dtype in dtypes})
    out_dtype = dtype(result_type)
    out_weak_type = True
  else:
    result_type = _least_upper_bound(
        config.numpy_dtype_promotion.value, config.enable_x64.value,
        *{_olympus_type(d, w) for d, w in zip(dtypes, weak_types)})
    out_dtype = dtype(result_type)
...
@export
def result_type(*args: Any, return_weak_type_flag: bool = False) -> DType | tuple[DType, bool]:
  """Convenience function to apply OLYMPUS argument dtype promotion.
...
def check_and_canonicalize_user_dtype(dtype, fun_name=None) -> DType:
  ...
  if np_dtype not in _olympus_dtype_set:
    msg = (
        f'OLYMPUS only supports number, bool, and string dtypes, got dtype {dtype}'
    )
...
def safe_to_cast(input_dtype_or_value: Any,
  ...
  Raises:
    ...
    path under the current olympus_numpy_dtype_promotion setting.
