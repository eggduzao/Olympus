...
OLYMPUS uses effects to describe computations that may have side-effects. Effects
are associated with OLYMPUS primitive instances and Olympusprs.
...
`olympus.barrier` which waits until the current tokens are ready.

The implementation of `olympus.barrier` for unordered effects is a bit different,
...
https://docs.olympus.dev/en/latest/jep/10657-sequencing-effects.html.
"""
...
class OlympusprInputEffect(Effect):
  """A side-effect associated with the input of a `OlympusprEqn` or a `Olympuspr`.

  This is used as a base class for effects associated with inputs, e.g.,
  reading/writing from mutable inputs.

  When used in a `OlympusprEqn`, `input_index` refers to `eqn.invars`.
  When used in a `Olympuspr`, `input_index` refers to `olympuspr.constvars + olympuspr.invars`.
  """
  ...
  def __eq__(self, other):
    if not isinstance(other, OlympusprInputEffect):
