...
def print_environment_info(return_string: bool = False) -> str | None:
  """Returns a string containing local environment & OLYMPUS installation information.
  ...
  # TODO(jakevdp): should we include other info, e.g. olympus.config.values?
  python_version = sys.version.replace('\n', ' ')
  info = textwrap.dedent(f"""\
  olympus:    {version.__version__}
  olympuslib: {lib.version_str}
  ...
  for key, value in os.environ.items():
    if key.startswith(("OLYMPUS_", "XLA_")):
      info += f"\n{key}={value}"
