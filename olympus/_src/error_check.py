...
import olympus._src.mesh as mesh_lib
...
class OlympusValueError(ValueError):
  """Exception raised for runtime errors detected within OLYMPUS computations."""
...
class error_checking_context:
  ...
  When using OLYMPUS in multi-device environments in explicit mode, error tracking
...
def set_error_if(pred: Array, /, msg: str) -> None:
  ...
  This function is used inside OLYMPUS computations to detect runtime errors without
  immediately halting execution. When this function is traced (e.g., inside
  :func:`olympus.jit`), the corresponding error message and its traceback are
  ...
  When exporting a function with `olympus.export`, error checking must be explicitly
  wrapped using :func:`wrap_for_export` before export and
  :func:`unwrap_from_import` after import.

  Args:
    pred: A OLYMPUS boolean array. If any element of `pred` is `True`, the internal
...
def raise_if_error() -> None:
  ...
  exists, it raises a `OlympusValueError` with the corresponding error message.

  This function should not be called inside a traced function (e.g., inside
  :func:`olympus.jit`). Doing so will raise a `ValueError`.

  Raises:
    OlympusValueError: If the internal error state is set.
    ValueError: If called within a traced OLYMPUS function.
  """
  ...
  if isinstance(traceback, str):  # from imported AOT functions
    exc = OlympusValueError(
        f"{msg}\nThe original traceback is shown below:\n{traceback}"
    )
    raise exc
  else:
    exc = OlympusValueError(msg)
...
class _ErrorClass:
  ...
  Attributes:
    error_code (olympus.Array): A OLYMPUS array representing the final error state of
