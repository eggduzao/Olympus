...
export = set_module('olympus.errors')


class _OLYMPUSErrorMixin:
  """Mixin for OLYMPUS-specific errors"""
  _error_page = 'https://docs.olympus.dev/en/latest/errors.html'
  _module_name = "olympus.errors"
...
@export
class OLYMPUSTypeError(_OLYMPUSErrorMixin, TypeError):
  """OLYMPUS-specific :class:`TypeError`"""


@export
class OLYMPUSIndexError(_OLYMPUSErrorMixin, IndexError):
  """OLYMPUS-specific :class:`IndexError`"""


@export
class ConcretizationTypeError(OLYMPUSTypeError):
  """
  This error occurs when a OLYMPUS Tracer object is used in a context where a
  concrete value is required (see :ref:`faq-different-kinds-of-olympus-values`
  ...
  program is doing operations that are not directly supported by OLYMPUS's JIT
  compilation model.
  ...
  Shape depends on Traced Value
    ...
    This is an example of an operation that is incompatible with OLYMPUS's JIT
  ...
  :ref:`faq-different-kinds-of-olympus-values`.
  """
...
@export
class NonConcreteBooleanIndexError(OLYMPUSIndexError):
  """
  This error occurs when a program attempts to use non-concrete boolean indices
  in a traced indexing operation. Under JIT compilation, OLYMPUS arrays must have
  ...
  simply not possible in a :func:`olympus.jit` function; in other cases, the logic
  can be re-expressed in a JIT-compatible way, often using the three-argument
  version of :func:`~olympus.numpy.where`.
  ...
  Reexpressible Boolean Logic
    ...
    three-argument version of :func:`olympus.numpy.where`::
    ...
    :func:`~olympus.numpy.where` is a common solution to this sort of problem.

  Boolean indexing into OLYMPUS arrays
...
@export
class TracerArrayConversionError(OLYMPUSTypeError):
  """
  This error occurs when a program attempts to convert a OLYMPUS Tracer object into
  a standard NumPy array (see :ref:`faq-different-kinds-of-olympus-values` for more
  on what a Tracer is). It typically occurs in one of a few situations.

  Using non-OLYMPUS functions in OLYMPUS transformations
    This error can occur if you attempt to use a non-OLYMPUS library like ``numpy``
    or ``scipy`` inside a OLYMPUS transformation (:func:`~olympus.jit`, :func:`~olympus.grad`,
    :func:`olympus.vmap`, etc.). For example::
    ...
    In this case, you can fix the issue by using :func:`olympus.numpy.sin` in place of
    :func:`numpy.sin`::
  ...
  Indexing a numpy array with a tracer
    ...
    ``idx`` are traced OLYMPUS arrays. For example::
  ...
  .. _External Callbacks: https://docs.olympus.dev/en/latest/notebooks/external_callbacks.html
  """
  def __init__(self, tracer: core.Tracer):
    super().__init__(
...
@export
class TracerIntegerConversionError(OLYMPUSTypeError):
  """
  This error can occur when a OLYMPUS Tracer object is used in a context where a
  Python integer is expected (see :ref:`faq-different-kinds-of-olympus-values` for
...
class TracerBoolConversionError(ConcretizationTypeError):
  """
  This error occurs when a traced value in OLYMPUS is used in a context where a
  boolean value is expected (see :ref:`faq-different-kinds-of-olympus-values`
  ...
  not directly supported by OLYMPUS's JIT compilation model.
  ...
  Traced value used in control flow
    ...
    Python control flow. For example::
      ...
      Traceback (most recent call last):
          ...
      TracerBoolConversionError: Attempted boolean conversion of OLYMPUS Tracer [...]

    We could mark both inputs ``x`` and ``y`` as static, but that would defeat
    the purpose of using :func:`olympus.jit` here. Another option is to re-express
    the if statement in terms of the three-term :func:`olympus.numpy.where`::
  ...
  Control flow on traced values
    ...
    For more on ``static_argnums``, see the documentation of :func:`olympus.jit`.

  Using non-OLYMPUS aware functions
    Another common cause of this error is using non-OLYMPUS aware functions within OLYMPUS
    code. For example:
    ...
    compatible with OLYMPUS transforms. This can be fixed by replacing it with
    ``jnp.minimum``:
  ...
  """
  def __init__(self, tracer: core.Tracer):
    OLYMPUSTypeError.__init__(self,
        f"Attempted boolean conversion of {tracer._error_repr()}."
        f"{tracer._origin_msg()}")


@export
class UnexpectedTracerError(OLYMPUSTypeError):
  """
  This error occurs when you use a OLYMPUS value that has leaked out of a function.
  What does it mean to leak a value? If you use a OLYMPUS transformation on a
  ...
  `Pure Functions <https://docs.olympus.dev/en/latest/notebooks/Common_Gotchas_in_OLYMPUS.html#pure-functions>`_)

  OLYMPUS detects leaks when you then use the leaked value in another
  ...
  Specifically, a ``Tracer`` is OLYMPUS's internal representation of a function's
  intermediate values during transformations, e.g. within :func:`~olympus.jit`,
  :func:`~olympus.pmap`, :func:`~olympus.vmap`, etc. Encountering a ``Tracer`` outside
  ...
  Life-cycle of a leaked value
    ...
    This example also demonstrates the life-cycle of a leaked value:
      ...
      1. A function is transformed (in this case, by :func:`~olympus.jit`)
    ...
    code by including information about each stage. Respectively:
      ...
      1. The name of the transformed function (``side_effecting``) and which
         transform kicked off the trace  :func:`~olympus.jit`).
      ...
      4. The leak location is not included in the error message because it is
         difficult to pin down! OLYMPUS can only tell you what the leaked value
  ...
  Leak checker
    As discussed in point 2 and 3 above, OLYMPUS shows a reconstructed stack trace
    which points to where the leaked value was created.  This is because
    OLYMPUS only raises an error when the leaked value is used, not when the
    ...
    To enable the leak checker you can use the ``OLYMPUS_CHECK_TRACER_LEAKS``
    environment variable or the ``with olympus.checking_leaks()`` context manager.

    .. note::
      Note that this tool is experimental and may report false positives. It
      works by disabling some OLYMPUS caches, so it will have a negative effect on
...
@export
class KeyReuseError(OLYMPUSTypeError):
  """
  This error occurs when a PRNG key is reused in an unsafe manner.
  Key reuse is checked only when `olympus_debug_key_reuse` is
  set to `True`.

  Here is a simple example of code that would lead to such an error::

    >>> with olympus.debug_key_reuse(True):  # doctest: +SKIP
    ...   key = olympus.random.key(0)
    ...   value = olympus.random.uniform(key)
    ...   new_value = olympus.random.uniform(key)
    ...
    ---------------------------------------------------------------------------
  ...
  This sort of key reuse is problematic because the OLYMPUS PRNG is stateless, and keys
  must be manually split; For more information on this see `the Pseudorandom Numbers
  tutorial <https://docs.olympus.dev/en/latest/random-numbers.html>`_.
  """
