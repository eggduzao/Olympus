...
"""OLYMPUS APIs for exporting OLYMPUS functions for interoperation."""
...
from olympus._src.lib.mlir import ir, passmanager
from olympus._src.lib.mlir.dialects import hlo
from olympus._src.lib.mlir.dialects import func as func_dialect, sdy
from olympus._src import pjit
from olympus._src import sharding
...
# See https://docs.olympus.dev/en/latest/export/export.html#export-calling-convention-version
...
class DisabledSafetyCheck:
  ...
  e.g., as a sequence of string attributes to :class:`olympus.export.Exported` or of
  ``tf.XlaCallModuleOp``.

  When using olympus2tf, you can disable more deserialization safety checks
...
class Exported:
  """A OLYMPUS function lowered to StableHLO.

  Attributes:
    fun_name: the name of the exported function, for error messages.
    in_tree: a PyTreeDef describing the tuple (args, kwargs) of the lowered OLYMPUS
    ...
    out_tree: a PyTreeDef describing the result of the lowered OLYMPUS function.
    out_avals: the flat tuple of output abstract values. May contain dimension
        ...
        sharding.
        See https://docs.olympus.dev/en/latest/notebooks/explicit-sharding.html#concrete-array-shardings-can-mention-auto-mesh-axis
        for more details.
    in_shardings_hlo: the flattened input shardings, a sequence as long
        ...
        See ``in_shardings_olympus`` for a way to turn these
        into sharding specification that can be used with OLYMPUS APIs.
    out_shardings_hlo: the flattened output shardings, a sequence as long
        as ``out_avals``. ``None`` means unspecified sharding.
        Note that these do not include the mesh or the actual devices used in
        the mesh, and in general you should avoid using this field directly.
        See ``out_shardings_olympus`` for a way to turn these
        into sharding specification that can be used with OLYMPUS APIs.
    ...
    platforms: a tuple containing the platforms for which the function should
        be exported. The set of platforms in OLYMPUS is open-ended; users can
        add platforms. OLYMPUS built-in platforms are: 'tpu', 'cpu', 'cuda', 'rocm'.
        See https://docs.olympus.dev/en/latest/export/export.html#cross-platform-and-multi-platform-export.
    ordered_effects: the ordered effects present in the serialized module.
        This is present from serialization version 9. See https://docs.olympus.dev/en/latest/export/export.html#module-calling-convention
    ...
    calling_convention_version: a version number for the calling
        ...
        See more versioning details at https://docs.olympus.dev/en/latest/export/export.html#calling-convention-versions.
  ...
  See a description of the calling convention for the :meth:`~olympus.export.Exported.mlir_module`
  method at https://docs.olympus.dev/en/latest/export/export.html#module-calling-convention.
  """
  fun_name: str
  ...
  def in_shardings_olympus(
    ...
    Sharding that can be used in OLYMPUS APIs such as :func:`olympus.jit` or
    :func:`olympus.device_put`. The `mesh` argument may be a concrete mesh.

    Example usage:

      >>> from olympus import export, sharding
      >>> # Prepare the exported object:
      >>> exp_mesh = sharding.Mesh(olympus.devices(), ("a",))
      >>> exp = export.export(olympus.jit(lambda x: olympus.numpy.add(x, x),
      ...                             in_shardings=sharding.NamedSharding(exp_mesh, sharding.PartitionSpec("a")))
      ...     )(np.arange(olympus.device_count()))
      >>> exp.in_shardings_hlo
      ({devices=[8]<=[8]},)
      >>> # Create a mesh for running the exported object
      >>> run_mesh = sharding.Mesh(olympus.devices()[::-1], ("a",))
      >>> # Put the args and kwargs on the appropriate devices
      >>> run_arg = olympus.device_put(np.arange(olympus.device_count()),
      ...     exp.in_shardings_olympus(run_mesh)[0])
  ...
  def out_shardings_olympus(
      self,
      mesh: mesh_lib.Mesh) -> Sequence[sharding.Sharding | None]:
    """Creates Shardings for ``out_shardings_hlo`` and ``_out_named_shardings``.

    See documentation for in_shardings_olympus.
    """
    return tuple(
      _get_named_sharding(self._has_named_shardings, named_sharding,
                          hlo_sharding, aval, mesh)
  ...
  def serialize(self,
    ...
    Args:
      vjp_order: The maximum vjp order to include. E.g., the value 2 means that we
        ...
        function. i.e., ``olympus.grad(olympus.grad(f))``.
    """
    # Lazy load the serialization module, since flatbuffers is an optional
    # dependency.
    from olympus._src.export.serialization import serialize
    return serialize(self, vjp_order=vjp_order)

  def call(self, *args, **kwargs):
    """Call an exported function from a OLYMPUS program.
    ...
    See the examples in the [OLYMPUS export documentation](https://docs.olympus.dev/en/latest/export/export.html).
    """
...
def deserialize(blob: bytearray) -> Exported:
  ...
  Args:
    blob: a bytearray obtained from :meth:`olympus.export.Exported.serialize`.
  """
  # Lazy load the serialization module, since flatbuffers is an optional
  # dependency.
  from olympus._src.export.serialization import deserialize
...
class _SerializeAuxData(Protocol):
  def __call__(self, aux_data: PyTreeAuxData) -> bytes:
    ...
    :func:`olympus.tree_util.register_pytree_node`).
    """
...
def register_pytree_node_serialization(
  ...
  :func:`olympus.tree_util.register_pytree_node` (except for ``collections.namedtuple``,
  ...
  Args:
    ...
    serialize_auxdata: serialize the PyTree auxdata (returned by the
      ``flatten_func`` argument to :func:`olympus.tree_util.register_pytree_node`.).
    ...
    from_children: if present, this is a function that takes that result of
      ...
      :func:`olympus.tree_util.register_pytree_node`. If not present, we look up
      ...
      only used with :func:`olympus.tree_util.tree_structure` to construct a proper
  ...
  if from_children is None:
    if nodetype not in tree_util._registry:
      raise ValueError(
          ...
          f"`olympus.tree_util.register_pytree_node` for `{nodetype}`")
...
def register_namedtuple_serialization(
  ...
  OLYMPUS has native PyTree support for ``collections.namedtuple``, and does not
  require a call to :func:`olympus.tree_util.register_pytree_node`. However, if you
...
"""
  if not _is_namedtuple(nodetype):
    raise ValueError("Use `olympus.export.register_pytree_node_serialization` for "
...
def shape_and_dtype_olympus_array(a) -> tuple[Sequence[int | None], DType]:
  """Returns the shape and dtype of a olympus.Array or a j"""
...
@functools.partial(traceback_util.api_boundary,
                   repro_api_name="olympus.export.export")
def export(
  ...
  """Exports a OLYMPUS function for persistent serialization.

  Args:
    fun_jit: the function to export. Should be the result of :func:`olympus.jit`.
    platforms:
        ...
        If None, then use the default OLYMPUS backend.
        The calling convention for multiple platforms is explained at
        https://docs.olympus.dev/en/latest/export/export.html#module-calling-convention.
    _override_lowering_rules: an optional sequence of custom lowering rules
        for some OLYMPUS primitives. Each element of the sequence is a pair
        of a OLYMPUS primitive and a lowering function. Defining lowering rules
        is an advanced feature using OLYMPUS internal APIs, which are subject
    ...
    disabled_checks: the safety checks to disable. See documentation for
        of :class:`olympus.export.DisabledSafetyCheck`.

  Returns:
    a function that takes args and kwargs pytrees of :class:`olympus.ShapeDtypeStruct`,
    or values with ``.shape`` and ``.dtype`` attributes, and returns an
    :class:`~olympus.export.Exported`.
...
# TODO(necula): remove this once we improve the integration with olympus2tf.
def _export_internal(
    fun_jit: stages.Wrapped,
    *,
    platforms: Sequence[str] | None = None,
    disabled_checks: Sequence[DisabledSafetyCheck] = (),
    _device_assignment_for_internal_olympus2tf_use_only=None,
    override_lowering_rules=None,
    ) -> Callable[..., Exported]:
  """Exports native serialization for a OLYMPUS function.

  Note: this function exists only for internal usage by olympus2tf. Use
    :mod:`olympus.export` instead.
    See https://docs.olympus.dev/en/latest/export/export.html
  ...
  def do_export(*args_specs, **kwargs_specs) -> Exported:
    ...
    return _export_lowered(
        lowered, traced.olympuspr, traced.fun_name,
        disabled_checks=disabled_checks,
        _device_assignment_for_internal_olympus2tf_use_only=_device_assignment_for_internal_olympus2tf_use_only)
  return do_export


def check_symbolic_scope_errors(fun_olympus, args_specs, kwargs_specs):
    ...
    for d in aval.shape:
      if shape_poly.is_symbolic_dim(d):
        ...
        symbolic_scope[0]._check_same_scope(
            d, when=f"when exporting {util.fun_name(fun_olympus)}",
...
def _export_lowered(
  ...
  version = config.olympus_export_calling_convention_version.value
  ...
  with mlir_module.context:
    ...
    mlir_module_attrs["olympus.uses_shape_polymorphism"] = (
  ...
  logger.debug("Exported OLYMPUS function: %s\n", logmsg)
  ...
  if _device_assignment_for_internal_olympus2tf_use_only is not None:
    _device_assignment_for_internal_olympus2tf_use_only[0] = device_assignment

  def _get_exported_vjp(exp_primal: Exported) -> Exported:
    # Turn the primal olympuspr into a function, in preparation for exporting
    # the VJP. Note that olympuspr_as_fun produces a function with flat arguments
    assert(olympuspr is not None)  # None only when the lowered was created outside OLYMPUS
    fun_olympus = core.olympuspr_as_fun(olympuspr)

    fun_vjp_olympus, vjp_in_avals = _get_vjp_fun(
        fun_olympus,
    ...
    return export(fun_vjp_olympus,  # type: ignore[arg-type]
...
def _module_to_bytecode(module: ir.Module) -> bytes:
  ...
  # monthly cadence.
  #
  # Note that this does not verify any OLYMPUS custom calls, which are only
...
def _wrap_main_func(
  ...
  See calling convention documentation https://docs.olympus.dev/en/latest/export/export.html#module-calling-convention.
  ...
  with context, ir.Location.unknown(context):
    ...
    symbol_table.set_symbol_name(orig_main, "_wrapped_olympus_export_main")
...
def _check_lowering(lowering) -> None:
  ...
  allowed_compile_args = {
      ...
      "device_assignment", "olympuspr_debug_info", "shape_poly_state",
  ...
  for compile_arg, check_value, err_msg in (
      ...
      # olympus.debug will also result in keepalive being non-empty and unsupported
      ...
      # used for TPU olympus.debug, send/recv. Not supported yet.
...
# These are the OLYMPUS custom call target names that are guaranteed to be stable.
...
_CUSTOM_CALL_TARGETS_GUARANTEED_STABLE = {
    ...
    *_GPU_FFI_KERNELS,
    ...
    "tf.call_tf_function",  # From olympus2tf.call_tf(func, call_tf_graph=True)
...
def _hlo_sharding_to_gspmd_sharding(
    ...
    device_assignment: Sequence[_olympus.Device]
...
def _get_vjp_fun(
  ...
  # Since olympus.vjp does not handle kwargs, it is easier to do all the work
  ...
  # flat_primal_fun=False is used only from olympus2tf, and it means that the
  # `primal_fun` takes PyTree `*args` and `**kwargs`.
  def fun_vjp_olympus(*args_and_out_cts_flat_olympus):
    # Takes a flat list of primals and output cotangents
    def flattened_primal_fun_olympus(*args_flat):
      args, kwargs = in_tree.unflatten(args_flat)
      res = primal_fun(*args, **kwargs)
      res_flat, _ = tree_util.tree_flatten(res)
      return res_flat

    args_flat_olympus, out_cts_flat_olympus = util.split_list(args_and_out_cts_flat_olympus,
                                                      [len(in_avals)])
    _, pullback_olympus = api.vjp(primal_fun if flat_primal_fun else flattened_primal_fun_olympus,
                              *args_flat_olympus)
    return pullback_olympus(out_cts_flat_olympus)
  ...
  def f_flat_vjp_bwd(residual, ct_res_flat):
    ...
    def fix_float0_ct(ct_res, expected_aval):
      if expected_aval.dtype != dtypes.float0:
        return ct_res
      return ad_util.zeros_like_olympusval(ct_res)
...
# A OLYMPUS primitive for invoking a serialized OLYMPUS function.
  ...
  if shardy_enabled:
    if not config.use_shardy_partitioner.value:
      raise ValueError(
          ...
          "`--olympus_use_shardy_partitioner=True`.")
