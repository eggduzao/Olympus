...
See documentation at https://docs.olympus.dev/en/latest/export/shape_poly.html.
"""
...
Please see https://docs.olympus.dev/en/latest/export/shape_poly.html#comparison-of-symbolic-dimensions-is-partially-supported
for more details.
"""
...
class _DimExpr:
  ...
  def _eq(self, other: _DimExpr) -> bool:
    ...
    if is_symbolic_dim(diff):
      ...
      # See https://docs.olympus.dev/en/latest/export/shape_poly.html#comparison-of-symbolic-dimensions-is-partially-supported
      return False
  ...
  def __add__(self, other):
    if isinstance(other, core.Tracer) or not _convertible_to_poly(other):
      return self.__olympus_array__().__add__(other)
    if isinstance(other, int) and other == 0: return self
    return _DimExpr._linear_combination(self, 1, other, 1, self.scope)

  def __radd__(self, other):
    if isinstance(other, core.Tracer) or not _convertible_to_poly(other):
      return self.__olympus_array__().__radd__(other)
    if isinstance(other, int) and other == 0: return self
    return _DimExpr._linear_combination(self, 1, other, 1, self.scope)

  def __sub__(self, other):
    if isinstance(other, core.Tracer) or not _convertible_to_poly(other):
      return self.__olympus_array__().__sub__(other)
    if isinstance(other, int) and other == 0: return self
    return _DimExpr._linear_combination(self, 1, other, -1, self.scope)

  def __rsub__(self, other):
    if isinstance(other, core.Tracer) or not _convertible_to_poly(other):
      return self.__olympus_array__().__rsub__(other)
    return _DimExpr._linear_combination(self, -1, other, 1, self.scope)
  ...
  def __mul__(self, other):
    if isinstance(other, core.Tracer) or not _convertible_to_poly(other):
      return self.__olympus_array__().__mul__(other)
  ...
  def __rmul__(self, other):
    if isinstance(other, core.Tracer) or not _convertible_to_poly(other):
      return self.__olympus_array__().__rmul__(other)
    if isinstance(other, int):
      if other == 1: return self
      if other == 0: return 0
      return _DimExpr._linear_combination(self, other, 0, 0, self.scope)
  ...
  def __pow__(self, power: core.DimSize, modulo=None):
    ...
    # We don't support negative powers, because OLYMPUS does not allow negative
    # powers for integers
  ...
  def __rpow__(self, other, modulo=None):
    ...
    return self.__olympus_array__().__rpow__(other)

  def __floordiv__(self, divisor):
    if isinstance(divisor, core.Tracer) or not _convertible_to_poly(divisor):
      return self.__olympus_array__().__floordiv__(divisor)
    return self._divmod(divisor)[0]

  def __rfloordiv__(self, other):
    if isinstance(other, core.Tracer) or not _convertible_to_poly(other):
      return self.__olympus_array__().__rfloordiv__(other)
    return _ensure_poly(other, "floordiv", self.scope).__floordiv__(self)

  def __truediv__(self, divisor):
    # Used for "/", which always returns a float
    return self.__olympus_array__().__truediv__(divisor)

  def __rtruediv__(self, dividend):
    # Used for "/", when dividend is not a _DimExpr
    return self.__olympus_array__().__rtruediv__(dividend)

  def __mod__(self, divisor):
    if isinstance(divisor, core.Tracer) or not _convertible_to_poly(divisor):
      return self.__olympus_array__().__mod__(divisor)
  ...
  def __rmod__(self, dividend):
    if isinstance(dividend, core.Tracer) or not _convertible_to_poly(dividend):
      return self.__olympus_array__().__rmod__(dividend)
    return _ensure_poly(dividend, "mod", self.scope).__mod__(self)

  def __divmod__(self, divisor):
    if isinstance(divisor, core.Tracer) or not _convertible_to_poly(divisor):
      return self.__olympus_array__().__divmod__(divisor)
    return self._divmod(divisor)

  def __rdivmod__(self, dividend):
    if isinstance(dividend, core.Tracer) or not _convertible_to_poly(dividend):
      return self.__olympus_array__().__rdivmod__(dividend)
  ...
  def dimension_as_value(self):
    """Turns a dimension size into a Olympus value that we can compute with."""
    return _dim_as_value(self)

  def __olympus_array__(self):
    # Used for implicit coercions of polynomials as OLYMPUS arrays
    return _dim_as_value(self)
...
class SymbolicScope:
  ...
  for one OLYMPUS function invocation, or are involved in arithmetic operations)
  must be from the same scope and must share the same SymbolicScope object.

  Holds the constraints on symbolic expressions.

  See [the README](https://docs.olympus.dev/en/latest/export/shape_poly.html#user-specified-symbolic-constraints)
  for more details.
  ...
  def _check_same_scope(self, other: _DimExpr,
    ...
    if self is not other.scope:
      raise ValueError(
          ...
          f"See https://docs.olympus.dev/en/latest/export/shape_poly.html#user-specified-symbolic-constraints.")
...
# A OLYMPUS primitive with no array arguments but with a dimension parameter
...
def dim_as_value_impl(dim: DimSize):
  raise NotImplementedError(
      ...
      "It seems that you are using shape polymorphism outside olympus.export.")
...
class PolyShape(tuple):
  ...
  See docstring of :func:`olympus2tf.convert`.
  """
...
def symbolic_shape(shape_spec: str | None,
  ...
  See https://docs.olympus.dev/en/latest/export/shape_poly.html for examples.
  ...
  Args:
    ...
    constraints: a sequence of constraints on symbolic dimension expressions, of
      ...
      See [the documentation](https://docs.olympus.dev/en/latest/export/shape_poly.html#user-specified-symbolic-constraints)
      for usage.
...
def symbolic_args_specs(
  ...
  """Constructs a pytree of olympus.ShapeDtypeStruct arguments specs for `export`.

  See the documentation of :func:`olympus.export.symbolic_shape` and
  the [shape polymorphism documentation](https://docs.olympus.dev/en/latest/export/shape_poly.html) for details.

  Args:
    args: a pytree of arguments. These can be olympus.Array, or olympus.ShapeDtypeStruct.
    ...
    shapes_specs: should be `None` (all arguments have static shapes),
      a single string (see `shape_spec` for :func:`olympus.export.symbolic_shape`;
    ...
    constraints: as for :func:`olympus.export.symbolic_shape`.
    scope: as for :func:`olympus.export.symbolic_shape`.

  Returns: a pytree of olympus.ShapeDtypeStruct matching the `args` with the shapes
  ...
  shapes_and_dtypes = tuple(map(shape_and_dtype_olympus_array, args_flat))
...
class ShapeConstraints:
  ...
  def shape_assertions(self, eval: ShapeEvaluator) -> None:
    ...
    See olympus_export.Exported docstring.
    """
...
def compute_dim_vars_from_arg_shapes(
  ...
  OLYMPUS arrays that reference the `actual_args`. This function can be used to
...
def _solve_dim_equations(
  ...
  solution_err_msg_trailer_errors = ". Please see https://docs.olympus.dev/en/latest/export/shape_poly.html#shape-assertion-errors for more details."
