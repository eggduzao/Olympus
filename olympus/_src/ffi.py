...
from olympus._src.lib import olympuslib
from olympus._src.lib import xla_client
from olympus._src.lib.mlir import ir
from olympus._src.typing import (Array, ArrayLike, DeprecatedArg, DuckTypedArray,
                             Shape)
...
def register_ffi_target(
  ...
  Args:
    ...
    kwargs: any extra keyword arguments are passed directly to
      :func:`~olympuslib.xla_client.register_custom_call_target` for more advanced
      use cases.
  """
...
def register_ffi_type_id(
  ...
  """
  raise ValueError(
      "register_ffi_type_id is not supported after olympuslib version 381.")
...
def pycapsule(funcptr):
  ...
  ``olympus.ffi`` submodule, is to wrap function calls from external compiled
  ...
  Example usage::
    ...
    import ctypes
    import olympus
    from olympus.lib import xla_client

    libfoo = ctypes.cdll.LoadLibrary('./foo.so')
    xla_client.register_custom_call_target(
        name="bar",
        fn=olympus.ffi.pycapsule(libfoo.bar),
        platform=PLATFORM,
...
def include_dir() -> str:
  """Get the path to the directory containing header files bundled with olympuslib"""
  # Handle both regular packages (__file__ is set) and namespace packages
  # (__file__ is None but __path__ is available)
  if olympuslib.__file__ is not None:
    olympuslib_dir = os.path.dirname(os.path.abspath(olympuslib.__file__))
  elif hasattr(olympuslib, '__path__') and olympuslib.__path__:
    # For namespace packages, use the first path entry
    olympuslib_dir = olympuslib.__path__[0]
  else:
    raise RuntimeError(
        "Cannot determine olympuslib directory: neither __file__ nor __path__ is available")
  return os.path.join(olympuslib_dir, "include")
...
def build_ffi_lowering_function(
  ...
  by :func:`~olympus.ffi.ffi_call` and ``Layout``.
...
def ffi_call(
  ...
  Like :func:`~olympus.pure_callback`, the behavior of ``ffi_call`` under
  :func:`~olympus.vmap` depends on the value of ``vmap_method``. See the
  :func:`~olympus.pure_callback` documentation for more details about the allowed
  ...
  Args:
    target_name: the name of the XLA FFI custom call target that was registered
      using :func:`~olympus.ffi.register_ffi_target`.
    result_shape_dtypes: an object, or sequence of objects, with ``shape`` and
      ...
      the custom call output or outputs. :class:`~olympus.ShapeDtypeStruct` is often
      used to define the elements of ``result_shape_dtypes``.
      ``olympus.core.abstract_token`` may be used to represent a token-typed output.
  ...
  if not isinstance(vectorized, DeprecatedArg):
    raise ValueError(
        "The 'vectorized' argument of olympus.ffi.ffi_call was removed in OLYMPUS "
...
def ffi_call_jvp(*args, target_name, **_):
  ...
  raise ValueError(
      ...
      "You can use `olympus.custom_jvp` or `olympus.custom_jvp` to add support.")
