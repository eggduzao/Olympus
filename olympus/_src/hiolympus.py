...
from olympus._src.api_util import resolve_kwargs, infer_argnums_and_argnames
...
from olympus._src.custom_derivatives import CustomVJPPrimal
...
from olympus._src.util import safe_zip, safe_map, split_list, unzip2
...
class HiPrimitive(core.Primitive):
  ...
  # lowering implements the primitive in terms of loolympus inputs/outputs/ops
  def to_loolympus(self, *lotypes_wrapped_in_hitypes, **params):
...
class HiType(core.AbstractValue):
  ...
  # lowering from hiolympus type to loolympus types
  def lo_ty(self) -> list[core.AbstractValue]:
    assert False, "must override"

  # define lowering from hiolympus value to loolympus values and back (like pytrees)
...
class MutableHiType(core.AbstractValue):
  ...
  # define lowering from (mutable) hiolympus type to (immutable) loolympus types
  def lo_ty_qdd(self, state: QDD) -> list[core.AbstractValue]:
    assert False, "must override"
  def lo_ty(self):
    assert False, "mutable hitypes should use lo_ty_qdd instead"

  # define lowering from hiolympus value to loolympus values and back, depending on qdd
  def new_from_loval(self, state: QDD, *vals: LoVal) -> HiVal:
    assert False, "must override"
  def read_loval(self, state: QDD, val: HiVal) -> list[LoVal]:
    assert False, "must override"

  # define how to mutate/set the mutable hiolympus value given immutable loolympus vals
...
class BoxSet(HiPrimitive):
  ...
  def to_loolympus(_, box, *leaves, treedef):
  ...
  def jvp(_, primals, tangents, *, treedef):
    ...
    if type(box_dot) is ad_util.Zero:
      raise Exception("can't differentiate Box.set operation, "
                      "did you forget olympus.lax.stop_gradient?")
...
# === new-style hiolympus primitive implementation ===
...
class VJPHiPrimitive:
  ...
  # Operation implementation in terms of loolympus primitives
...
def _call_hi_primitive_to_loolympus(*args_flat, prim):
  args = tree_unflatten(prim.in_tree, args_flat)
  ans = prim.expand(*args)
  return tree_leaves_checked(prim.out_tree, ans)
call_hi_primitive_p.to_loolympus = _call_hi_primitive_to_loolympus
...
call_hi_primitive_linearized_p.to_loolympus = ad.raise_custom_vjp_error_on_jvp
...
class CustomVJPTraced(VJPHiPrimitive):
  ...
  def batch_dim_rule(self, axis_data, in_dims):
    ...
    _, out_dims = batching.batch_olympuspr2(self.traced.olympuspr, axis_data, tuple(in_dims_flat))  # type: ignore
...
class OptRemat(VJPHiPrimitive):
  ...
  def dce(self, used_outs, eqn):
    num_primals_in = len(self.traced_primal.olympuspr.in_avals)
    num_primals_out = len(self.traced_primal.olympuspr.out_avals)
    ...
    if any(used_res):
      ...
      primal_eqn = pe.new_olympuspr_eqn(
          eqn.invars, outvars, core.closed_call_p, dict(call_olympuspr=self.traced_primal.olympuspr),
          self.traced_primal.olympuspr.effects, eqn.source_info, eqn.ctx)
