...
data_2024_05_31["c128"] = dict(
    ...
    mlir_module_text=r"""
#loc3 = loc("third_party/py/olympus/tests/export_back_compat_test.py":384:11)
#loc12 = loc("jit(<lambda>)/jit(main)/while/body/closed_call"(#loc3))
#loc21 = loc("jit(<lambda>)/jit(main)/while/body/scatter[update_olympuspr=None update_consts=() dimension_numbers=ScatterDimensionNumbers(update_window_dims=(), inserted_window_dims=(0,), scatter_dims_to_operand_dims=(0,)) indices_are_sorted=True unique_indices=True mode=GatherScatterMode.FILL_OR_DROP]"(#loc3))
module @jit__lambda_ attributes {olympus.uses_shape_polymorphism = false, mhlo.num_partitions = 1 : i32, mhlo.num_replicas = 1 : i32} {
  func.func public @main() -> (tensor<3x3xcomplex<f64>> {olympus.result_info = "[0]", mhlo.layout_mode = "default"}, tensor<3xi32> {olympus.result_info = "[1]", mhlo.layout_mode = "default"}, tensor<3xi32> {olympus.result_info = "[2]", mhlo.layout_mode = "default"}) {
  ...
  func.func private @None(%arg0: tensor<i64> loc("jit(<lambda>)/jit(main)/while/body/closed_call"(#loc3)), %arg1: tensor<3xi32> loc("jit(<lambda>)/jit(main)/while/body/closed_call"(#loc3)), %arg2: tensor<3xi32> loc("jit(<lambda>)/jit(main)/while/body/closed_call"(#loc3))) -> (tensor<i64>, tensor<3xi32>, tensor<3xi32>) {
    ...
    ^bb0(%arg3: tensor<i32> loc("jit(<lambda>)/jit(main)/while/body/scatter[update_olympuspr=None update_consts=() dimension_numbers=ScatterDimensionNumbers(update_window_dims=(), inserted_window_dims=(0,), scatter_dims_to_operand_dims=(0,)) indices_are_sorted=True unique_indices=True mode=GatherScatterMode.FILL_OR_DROP]"(#loc3)), %arg4: tensor<i32> loc("jit(<lambda>)/jit(main)/while/body/scatter[update_olympuspr=None update_consts=() dimension_numbers=ScatterDimensionNumbers(update_window_dims=(), inserted_window_dims=(0,), scatter_dims_to_operand_dims=(0,)) indices_are_sorted=True unique_indices=True mode=GatherScatterMode.FILL_OR_DROP]"(#loc3))):
...
#loc = loc(unknown)
#loc1 = loc("third_party/py/olympus/tests/export_back_compat_test.py":383:26)
#loc2 = loc("third_party/py/olympus/tests/export_back_compat_test.py":383:14)
...
#loc21 = loc("jit(<lambda>)/jit(main)/while/body/scatter[update_olympuspr=None update_consts=() dimension_numbers=ScatterDimensionNumbers(update_window_dims=(), inserted_window_dims=(0,), scatter_dims_to_operand_dims=(0,)) indices_are_sorted=True unique_indices=True mode=GatherScatterMode.FILL_OR_DROP]"(#loc3))
module @jit__lambda_ attributes {olympus.uses_shape_polymorphism = false, mhlo.num_partitions = 1 : i32, mhlo.num_replicas = 1 : i32} {
  func.func public @main() -> (tensor<3x3xcomplex<f32>> {olympus.result_info = "[0]", mhlo.layout_mode = "default"}, tensor<3xi32> {olympus.result_info = "[1]", mhlo.layout_mode = "default"}, tensor<3xi32> {olympus.result_info = "[2]", mhlo.layout_mode = "default"}) {
...
#loc21 = loc("jit(<lambda>)/jit(main)/while/body/scatter[update_olympuspr=None update_consts=() dimension_numbers=ScatterDimensionNumbers(update_window_dims=(), inserted_window_dims=(0,), scatter_dims_to_operand_dims=(0,)) indices_are_sorted=True unique_indices=True mode=GatherScatterMode.FILL_OR_DROP]"(#loc3))
module @jit__lambda_ attributes {olympus.uses_shape_polymorphism = false, mhlo.num_partitions = 1 : i32, mhlo.num_replicas = 1 : i32} {
  func.func public @main() -> (tensor<3x3xf32> {olympus.result_info = "[0]", mhlo.layout_mode = "default"}, tensor<3xi32> {olympus.result_info = "[1]", mhlo.layout_mode = "default"}, tensor<3xi32> {olympus.result_info = "[2]", mhlo.layout_mode = "default"}) {
...
#loc21 = loc("jit(<lambda>)/jit(main)/while/body/scatter[update_olympuspr=None update_consts=() dimension_numbers=ScatterDimensionNumbers(update_window_dims=(), inserted_window_dims=(0,), scatter_dims_to_operand_dims=(0,)) indices_are_sorted=True unique_indices=True mode=GatherScatterMode.FILL_OR_DROP]"(#loc3))
module @jit__lambda_ attributes {olympus.uses_shape_polymorphism = false, mhlo.num_partitions = 1 : i32, mhlo.num_replicas = 1 : i32} {
  func.func public @main() -> (tensor<3x3xf64> {olympus.result_info = "[0]", mhlo.layout_mode = "default"}, tensor<3xi32> {olympus.result_info = "[1]", mhlo.layout_mode = "default"}, tensor<3xi32> {olympus.result_info = "[2]", mhlo.layout_mode = "default"}) {
