...
module @jit_func attributes {mhlo.num_partitions = 1 : i32, mhlo.num_replicas = 1 : i32} {
  func.func public @main(%arg0: tensor<4xui32> {olympus.arg_info = "key", mhlo.sharding = "{replicated}"} loc(unknown), %arg1: tensor<?x?xf32> {olympus.arg_info = "a", mhlo.sharding = "{replicated}"} loc(unknown)) -> (tensor<?x4xui32> {olympus.result_info = ""}) {
    ...
    %7 = call @_wrapped_olympus_export_main(%1, %3, %5, %6) : (tensor<i64>, tensor<i64>, tensor<4xui32>, tensor<?x?xf32>) -> tensor<?x4xui32> loc(#loc)
    return %7 : tensor<?x4xui32> loc(#loc)
  } loc(#loc)
  func.func private @_wrapped_olympus_export_main(%arg0: tensor<i64> loc(unknown), %arg1: tensor<i64> loc(unknown), %arg2: tensor<4xui32> {olympus.arg_info = "key", mhlo.sharding = "{replicated}"} loc(unknown), %arg3: tensor<?x?xf32> {olympus.arg_info = "a", mhlo.sharding = "{replicated}"} loc(unknown)) -> (tensor<?x4xui32> {olympus.result_info = ""}) {
...
#loc1 = loc("third_party/py/olympus/experimental/olympus2tf/tests/back_compat_test.py":260:0)
#loc2 = loc("third_party/py/olympus/experimental/olympus2tf/tests/back_compat_test.py":697:0)
