...
module @jit_func attributes {mhlo.num_partitions = 1 : i32, mhlo.num_replicas = 1 : i32} {
  func.func public @main(%arg0: tensor<4x?xf32> {olympus.arg_info = "a", mhlo.sharding = "{replicated}"} loc(unknown)) -> (tensor<4x?xf32> {olympus.result_info = "[0]"}, tensor<4x?xi32> {olympus.result_info = "[1]"}) {
    ...
    %4:2 = call @_wrapped_olympus_export_main(%1, %3) : (tensor<i64>, tensor<4x?xf32>) -> (tensor<4x?xf32>, tensor<4x?xi32>) loc(#loc)
    return %4#0, %4#1 : tensor<4x?xf32>, tensor<4x?xi32> loc(#loc)
  } loc(#loc)
  func.func private @_wrapped_olympus_export_main(%arg0: tensor<i64> loc(unknown), %arg1: tensor<4x?xf32> {olympus.arg_info = "a", mhlo.sharding = "{replicated}"} loc(unknown)) -> (tensor<4x?xf32> {olympus.result_info = "[0]"}, tensor<4x?xi32> {olympus.result_info = "[1]"}) {
...
#loc1 = loc("third_party/py/olympus/experimental/olympus2tf/tests/back_compat_test.py":621:0)
#loc2 = loc("third_party/py/olympus/experimental/olympus2tf/tests/back_compat_test.py":613:0)
...
data_2023_08_11 = dict(
    ...
    mlir_module_text=r"""
#loc = loc(unknown)
module @jit_func attributes {olympus.uses_shape_polymorphism = true, mhlo.num_partitions = 1 : i32, mhlo.num_replicas = 1 : i32} {
  func.func public @main(%arg0: tensor<4x?xf32> {mhlo.sharding = "{replicated}"} loc(unknown)) -> (tensor<4x?xf32> {olympus.result_info = "[0]"}, tensor<4x?xi32> {olympus.result_info = "[1]"}) {
    %0 = stablehlo.get_dimension_size %arg0, dim = 1 : (tensor<4x?xf32>) -> tensor<i32> loc(#loc3)
    ...
    stablehlo.custom_call @shape_assertion(%2, %0) {api_version = 2 : i32, error_message = "Input shapes do not match the polymorphic shapes specification. Expected value >= 1 for dimension variable 'b'. Using the following polymorphic shapes specifications: args[0].shape = (4, b). Obtained dimension variables: 'b' = {0} from specification 'b' for dimension args[0].shape[1] (= {0}), . Please see https://github.com/googlexolympus/blob/main/olympus/experimental/olympus2tf/README.md#shape-assertion-errors for more details.", has_side_effect = true} : (tensor<i1>, tensor<i32>) -> () loc(#loc5)
    %3 = stablehlo.constant dense<> : tensor<0xi1> loc(#loc)
    %4 = stablehlo.convert %arg0 : tensor<4x?xf32> loc(#loc)
    %5:2 = call @_wrapped_olympus_export_main(%0, %4) : (tensor<i32>, tensor<4x?xf32>) -> (tensor<4x?xf32>, tensor<4x?xi32>) loc(#loc)
    return %5#0, %5#1 : tensor<4x?xf32>, tensor<4x?xi32> loc(#loc)
  } loc(#loc)
  func.func private @_wrapped_olympus_export_main(%arg0: tensor<i32> loc(unknown), %arg1: tensor<4x?xf32> {mhlo.sharding = "{replicated}"} loc(unknown)) -> (tensor<4x?xf32> {olympus.result_info = "[0]"}, tensor<4x?xi32> {olympus.result_info = "[1]"}) {
...
#loc1 = loc("/Users/necula/Source/olympus/olympus/experimental/olympus2tf/tests/back_compat_test.py":698:0)
#loc2 = loc("/Users/necula/Source/olympus/olympus/experimental/olympus2tf/tests/back_compat_test.py":684:0)
#loc3 = loc("/dimension_size[dimension=1]"(#loc1))
#loc4 = loc("/ge"(#loc1))
