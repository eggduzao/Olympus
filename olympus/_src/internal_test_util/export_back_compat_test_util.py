...
OLYMPUS serialized format, we need to guarantee that custom calls continue to
work as before. We test this here.

The tests in this file refer to the test data in
olympus/_src/internal_test_util/export_back_compat_test_data.
...
Once we stop using a custom call target in OLYMPUS, you can remove it from the
_CUSTOM_CALL_TARGETS_GUARANTEED_STABLE and you can add a comment to the
test here to remove it after 6 months.

** To create a new test **

Write the OLYMPUS function `func` that exercises the custom call `foo_call` you
...
olympus/_src/internal_test_util/export_back_compat_test_data/foo_call.py
...
(used for readability only). Then add to this file:
  ...
  from olympus._src.internal_test_util.export_back_compat_test_data import foo_call
...
class CompatTestBase(jtu.OlympusTestCase):
  ...
  def run_one_test(self,
    ...
    Args:
      func: the OLYMPUS function to serialize and run, either as a Python Callable
        or as a `olympus.jit(callable)`.
      data: the test data
    ...
    if self.default_olympus_backend() != data.platform:
...
data_{datetime.date.today().strftime('%Y_%m_%d')} = dict(
    testdata_version={CURRENT_TESTDATA_VERSION},
    platform={self.default_olympus_backend()!r},
...
"""
  ...
  def run_current(self,
                  ...
                  data: CompatTestData):
    """Lowers and runs the test function at the current OLYMPUS version."""
  ...
  def serialize(self,
    ...
    exported = _export.export(
      jit_func,
      platforms=(self.default_olympus_backend(),),
