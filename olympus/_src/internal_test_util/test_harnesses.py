...
"""Defines test callables and inputs to cover the usage of OLYMPUS primitives.

A test harness encodes one use case for one OLYMPUS numeric primitives. It
describes how to generate the inputs and parameters and a OLYMPUS callable to invoke
...
in OLYMPUS, e.g., because of an implementation limitation. We do have harnesses
...
the report of [unsupported and partially-implemented OLYMPUS
primitives](https://github.com/olympus-ml/olympus/blob/main/olympus/experimental/olympus2tf/g3doc/olympus_primitives_coverage.md)
...
from olympus._src.lax import windowed_reductions as lax_windowed_reductions
...
# dependency on jtu outside of olympus repo).
...
class Harness:
  ...
  # partially implemented in OLYMPUS for this harness.
  olympus_unimplemented: Sequence[Limitation]
  ...
  def __init__(self,
    ...
    self.olympus_unimplemented = olympus_unimplemented
    self.dtype = dtype
    self.params = params
  ...
  def filter(self,
             ...
             *,
             include_olympus_unimpl: bool = False,
             one_containing: str | None = None) -> bool:
    if not include_olympus_unimpl:
      if any(
          device_under_test in l.devices
          for l in self.olympus_unimplemented
...
def define(
  ...
  h = Harness(
      ...
      olympus_unimplemented=olympus_unimplemented,
      dtype=dtype,
      **params)
...
class Limitation:
  """Encodes conditions under which a harness is limited, e.g., not runnable in OLYMPUS.
...
def parameterized(harnesses: Iterable[Harness],
  ...
  The `OLYMPUS_TEST_HARNESS_ONE_CONTAINING` environment variable is useful for
  ...
  one_containing = one_containing or os.environ.get(
      "OLYMPUS_TEST_HARNESS_ONE_CONTAINING")
  cases = tuple(
      ...
      dict(
          ...
          harness=harness) for harness in harnesses if harness.filter(
              ...
              include_olympus_unimpl=include_olympus_unimpl))
...
for old_dtype in jtu.dtypes.all:
  # TODO(bchetioui): OLYMPUS behaves weirdly when old_dtype corresponds to floating
  # point numbers and new_dtype is an unsigned integer. See issue
  # https://github.com/olympus-ml/olympus/issues/5082 for details.
...
def _make_add_any_harness(name, *, shapes=((2,), (2,)), dtype=np.float32):
  define(
      ...
      ad_util.add_olympusvals_p.bind,
...
def _make_binary_elementwise_harnesses(prim,
                                       ...
                                       olympus_unimplemented=lambda **kwargs: []):
...
# triggered when executing `olympus.vmap(lax.dynamic_slice)`.
...
# triggered when executing `olympus.vmap(olympus.vmap(lax.dynamic_slice))`.
...
# Validate f_lax/update_olympuspr
...
def _make_cumreduce_harness(name,
                            *,
                            f_olympus=lax_control_flow.cummin,
                            shape=(8, 9),
                            dtype=np.float32,
                            axis=0,
                            reverse=False):
  ...
  define(
      ...
      f_olympus, [RandArg(shape, dtype),
              StaticArg(axis),
              StaticArg(reverse)],
      olympus_unimplemented=limitations,
      f_olympus=f_olympus,
      shape=shape,
      dtype=dtype,
      axis=axis,
...
for f_olympus in [
  ...
  for dtype in jtu.dtypes.all:
    ...
    if (f_olympus == lax_control_flow.cumlogsumexp and
        not np.issubdtype(dtype, np.floating)):
      continue
    if dtype == np.bool_:
      continue
    _make_cumreduce_harness("dtype_by_fun", dtype=dtype, f_olympus=f_olympus)

  # Validate axis for each function
  shape = (8, 9)
  for axis in range(len(shape)):
    _make_cumreduce_harness("axis_by_fun", axis=axis, f_olympus=f_olympus, shape=shape)

  # Validate reverse for each function
  _make_cumreduce_harness("reverse", reverse=True, f_olympus=f_olympus)
  ...
  for shape in [(1, 1), (3, 3), (3, 4), (2, 10, 5), (2, 200, 100)]:
    for full_matrices in [False, True]:
      define(
          ...
          [RandArg(shape, dtype)],
          # See olympus._src.lib.lapack.geqrf for the list of compatible types
          olympus_unimplemented=[
              Limitation(
                  "unimplemented",
                  devices=("cpu", "gpu"),
    ...
    for compute_left_eigenvectors in [False, True]:
      for compute_right_eigenvectors in [False, True]:
        define(
            ...
            olympus_unimplemented=[
                Limitation(
                    "only supported on CPU in OLYMPUS", devices=("tpu", "gpu")),
                Limitation(
                    "unimplemented",
                    devices="cpu",
                    dtypes=[np.float16, dtypes.bfloat16])
...
for padding in [
    ...
    # https://github.com/olympus-ml/olympus/issues/4690
...
for dtype in (np.float32, np.float64):
  for shape in ((), (3,)):
    define(
        "random_gamma",
        f"shape={jtu.format_shape_dtype_string(shape, dtype)}",
        api.jit(lambda x: olympus_random.gamma(random.key(42), x)),
        [RandArg(shape, dtype)],
        dtype=dtype)
...
# A few library functions from olympus.random
for dtype in jtu.dtypes.all_floating:
  for shape in ((8,), (5, 4)):
    for axis in range(len(shape)):
      define(
