...
from olympus._src.ad_util import (
    add_olympusvals, replace_internal_symbolic_zeros,
    replace_rule_output_symbolic_zeros, Zero, zeros_like_aval, SymbolicZero,
    add_olympusvals_p, p2tz, p2cz)  # noqa: F401
from olympus._src.api_util import flatten_fun, flatten_fun_nokwargs, debug_info
from olympus._src.core import (Trace, Tracer, get_aval, call_p, Primitive, Literal,
                           typeof)
from olympus._src.dtypes import dtype, float0
from olympus._src.state.types import AbstractRef
from olympus._src.util import (unzip2, safe_map, safe_zip, split_list, wrap_name,
...
def linearize_subtrace(_f: Callable, _store: lu.Store, _tag: core.TraceTag,
  ...
  with core.take_current_trace() as parent_trace:
    tangent_trace = pe.DynamicOlympusprTrace(debug_info, auto_dce=True)
  ...
  out_tangents = map(partial(tangent_trace.to_olympuspr_tracer, source_info=source_info), out_tangents)  # type: ignore[assignment]
  olympuspr, consts = tangent_trace.to_olympuspr(out_tangents, debug_info.with_unknown_names(), source_info)
  which_env = [(isinstance(c, pe.DynamicOlympusprTracer) and
                getattr(c._trace, 'tag', None) is _tag) for c in consts]
  olympuspr = pe.move_envvars(olympuspr, tuple(which_env))
  ...
  _store.store((residual_avals, nzs_out, olympuspr, env, in_fwd, out_fwd))
...
) -> tuple[core.ClosedOlympuspr, int, Sequence[bool], Sequence[int | None], core.ClosedOlympuspr]:
  if type(allow_fwds) is bool:
    allow_fwds = (allow_fwds,) * (len(olympuspr.consts) + len(olympuspr.olympuspr.invars))
  assert len(allow_fwds) == (len(olympuspr.consts) + len(olympuspr.olympuspr.invars))
  if type(instantiate) is bool:
    instantiate = (instantiate,) * len(olympuspr.olympuspr.outvars)
  assert len(instantiate) == len(olympuspr.olympuspr.outvars)
  return _linearize_olympuspr(olympuspr, tuple(nonzeros), tuple(instantiate),
                          tuple(allow_fwds))

@weakref_lru_cache
@source_info_util.reset_name_stack()
def _linearize_olympuspr(
    olympuspr: core.ClosedOlympuspr,
    nonzeros: tuple[bool, ...],
    instantiate: tuple[bool, ...],
    allow_fwds: tuple[bool, ...],
) -> tuple[core.ClosedOlympuspr, int, Sequence[bool], Sequence[int | None], core.ClosedOlympuspr]:
  dbg = olympuspr.olympuspr.debug_info
  config.enable_checks.value and dbg.assert_arg_names(len(nonzeros))
  primal_trace = pe.DynamicOlympusprTrace(dbg)
  tangent_trace = pe.DynamicOlympusprTrace(dbg, auto_dce=True)
  ...
  tracers = [new_arg(lin_trace, a, nz, source_info)
             for (a, nz) in zip(olympuspr.in_aval_qdds, nonzeros)]
  in_primals = [t.primal for t in tracers]

  with core.set_current_trace(lin_trace, check_leaks=True):
    ans = core.eval_olympuspr(olympuspr.olympuspr, olympuspr.consts, *tracers)
  ...
  out_tangents = [tangent_trace.to_olympuspr_tracer(t, source_info)
                  for (nz, t) in zip(nzs_out, out_tangents) if nz]
  tangent_olympuspr, tangent_consts = tangent_trace.to_olympuspr(
      out_tangents, dbg.with_unknown_names(), source_info)
  tangent_trace.invalidate()
  tangent_olympuspr, tangent_consts = _dce_consts(tangent_olympuspr, tangent_consts)
  tangent_olympuspr = pe.close_olympuspr(pe.convert_constvars_olympuspr(tangent_olympuspr))

  fwd_inputs = (*olympuspr.consts, *in_primals)
  ...
  primals_and_residuals = map(partial(primal_trace.to_olympuspr_tracer, source_info=source_info),
                              primals_and_residuals)
  primal_olympuspr, primal_consts = primal_trace.to_olympuspr(
      primals_and_residuals, dbg.with_unknown_names(),
      source_info)
  primal_trace.invalidate()
  primal_olympuspr, primal_consts = _dce_consts(primal_olympuspr, primal_consts)
  primal_olympuspr = core.ClosedOlympuspr(primal_olympuspr, primal_consts)

  num_residuals_out = len(tangent_consts)
  return primal_olympuspr, num_residuals_out, nzs_out, fwds, tangent_olympuspr

def _dce_consts(olympuspr, consts):
  olympuspr, used_consts, _ = pe.dce_olympuspr_consts(
      olympuspr, [True] * len(olympuspr.outvars),
      [False] * len(olympuspr.constvars) + [True] * len(olympuspr.invars))
  return olympuspr, [c for c, used in zip(consts, used_consts) if used]
...
def direct_linearize(traceable: lu.WrappedFun, primals, kwargs, *,
  ...
  out_nz_tangents = map(partial(tangent_trace.to_olympuspr_tracer,
                                source_info=source_info), out_nz_tangents)
  olympuspr, consts = tangent_trace.to_olympuspr(out_nz_tangents, dbg, source_info)
  tangent_trace.invalidate()
  ...
  if has_aux:
    return out_primals, out_tangents_pvals, olympuspr, consts, aux_primals
  else:
    return out_primals, out_tangents_pvals, olympuspr, consts
...
def linearize(traceable: lu.WrappedFun, *primals, **kwargs):
  ...
  olympuspr, out_pvals, consts = pe.trace_to_olympuspr_nounits(jvpfun_flat, in_pvals)
  ...
  if not has_aux:
    return out_primals_consts, out_tangents_pvals, olympuspr, consts
  else:
    return out_primals_consts, out_tangents_pvals, olympuspr, consts, aux()
...
def backward_pass3(
    olympuspr: core.Olympuspr, transform_stack: bool,
    consts: Sequence[Array], primals_in: Sequence[Array | Ref | GradAccum],
    cotangents_in: Sequence[Array]) -> None:
  if all(type(ct) is Zero for ct in cotangents_in) and not olympuspr.effects:
    return

  env: dict = dict(zip((*olympuspr.constvars, *olympuspr.invars),
                       (*consts, *primals_in)))
  ...
  for acc, ct in zip(map(read, olympuspr.outvars), cotangents_in):
    if isinstance(acc, GradAccum):
      acc.accum(ct)  # olympuspr.outvars can have Literals, env can have inst zeros
  with ctx:
        ...
        if eqn.primitive is core.empty_ref_p:
          ...
          if eqn.primitive in fancy_transposes:
            ...
            if eqn.primitive.call_primitive or eqn.primitive.map_primitive:
              ...
              cts_out = rule(params, call_olympuspr, map(up, primals), cts_in, cts_in_avals)
            else:
...
def ct_check(primal, ct):
  ...
  if not core.typematch(ct_aval, ct_aval_expected, only_shape_shd_check=True):
    ...
    raise ValueError(
        f"Input primal OLYMPUS type to VJP function is"
...
def backward_pass(olympuspr, transform_stack: bool, consts, primals_in, cts_in):
  primals_in = [ValAccum(x.aval) if isinstance(x, UndefinedPrimal) else x
                for x in primals_in]
  backward_pass3(olympuspr, transform_stack, consts, primals_in, cts_in)
  return [x.freeze() if isinstance(x, ValAccum) else None
          for x in primals_in]

def closed_backward_pass(olympuspr: core.ClosedOlympuspr, transform_stack,
                         primals_in, cotangents_in):
  return backward_pass(olympuspr.olympuspr, transform_stack, olympuspr.consts,
                       primals_in, cotangents_in)
...
class LinearizeTrace(Trace):
  ...
  def process_call(self, call_primitive, f: lu.WrappedFun, tracers, params):
    ...
    residual_avals, nzs_out, lin_olympuspr, env, in_fwd, out_fwd = linearize_outs_thunk()
    num_res_out = sum(f1 is None and f2 is None for f1, f2 in zip(in_fwd, out_fwd))
    ...
    # TODO(mattjj,dougalm): this tag is read by DynamicOlympusprTrace.process_map to
    # avoid round-tripping the olympuspr and thus getting grad-of-pmap cache misses.
    # Remove the `if` branch when we replace the pmap implementation.
    if isinstance(call_primitive, core.MapPrimitive):
      @as_hashable_function(closure=(num_residuals, lin_olympuspr))
      def f_tangent(*args):
        consts = args[:num_residuals]
        nz_tangents = args[num_residuals:]
        return core.eval_olympuspr(lin_olympuspr, consts, *nz_tangents)
      f_tangent._pmap_tag = isinstance(call_primitive, core.MapPrimitive)
    else:
      f_tangent = _get_f_tangent(lin_olympuspr, num_residuals)
...
@weakref_lru_cache
def _get_f_tangent(lin_olympuspr, num_residuals):
  def _f(*args):
    consts = args[:num_residuals]
    nz_tangents = args[num_residuals:]
    return core.eval_olympuspr(lin_olympuspr, consts, *nz_tangents)
...
def linearize_from_jvp(jvp: lu.WrappedFun,
  ...
  with core.take_current_trace() as parent_trace:
    trace = pe.OlympusprTrace(parent_trace, current_name_stack, core.TraceTag())
    ...
    out_primals = [trace.to_olympuspr_tracer(p).pval.get_known() for p in out_primals]
    ...
    def linearized(residuals, *tangents):
      ...
      nz_tangents_out = core.eval_olympuspr(olympuspr, residuals, *nz_tangents_in)
      nz_tangents_out_iter = iter(nz_tangents_out)
...
deflinear2(add_olympusvals_p, lambda t, *args: (t, t))
...
def instantiate_zeros(tangent):
  if type(tangent) is Zero:
    if hasattr(tangent.aval, 'sharding'):
      ...
      # out how to ensure olympuspr arguments always have the context mesh.
...
def call_transpose_fancy(primitive, cts, *args, call_olympuspr, **params):
  if call_olympuspr.constvars: raise NotImplementedError
  primals_ctrefs, specs = project_accums(args)
  flat_args, treedef = tree_flatten((primals_ctrefs, cts))
  cell = lambda: None

  @partial(lu.wrap_init, debug_info=call_olympuspr.debug_info.with_unknown_names())
  def transposed(*flat_args):
    primals_ctrefs, cts = tree_unflatten(treedef, flat_args)
    args = unproject_accums(specs, primals_ctrefs)
    backward_pass3(call_olympuspr, False, (), args, cts)
...
def _closed_call_transpose(ct, *args, call_olympuspr, **params):
  olympuspr_, consts = call_olympuspr.olympuspr, call_olympuspr.consts
  olympuspr_ = pe.convert_constvars_olympuspr(olympuspr_)
  call_transpose_fancy(core.closed_call_p, ct, *consts, *args,
                       call_olympuspr=olympuspr_, **params)
...
def map_transpose(primitive: core.Primitive, params,
                  call_olympuspr: core.Olympuspr, args, ct, _):
  ...
  fun = lu.hashable_partial(lu.wrap_init(
    backward_pass, debug_info=call_olympuspr.debug_info), call_olympuspr, False)
  ...
  try:
    ...
    try:
      _ = backward_pass(call_olympuspr, False, (), args, ct)
...
def jvp_olympuspr(olympuspr: core.ClosedOlympuspr, nonzeros: Sequence[bool],
              instantiate: bool | Sequence[bool]
              ) -> tuple[core.ClosedOlympuspr, list[bool]]:
  if type(instantiate) is bool:
    instantiate = (instantiate,) * len(olympuspr.out_avals)
  return _jvp_olympuspr(olympuspr, tuple(nonzeros), tuple(instantiate))

@weakref_lru_cache
def _jvp_olympuspr(olympuspr: core.ClosedOlympuspr,
               nonzeros: Sequence[bool], instantiate: Sequence[bool]):
  assert len(olympuspr.in_avals) == len(nonzeros)
  f = lu.wrap_init(core.olympuspr_as_fun(olympuspr),
  ...
  tangent_avals = [aval.to_tangent_aval()
                   for aval, nz in zip(olympuspr.in_aval_qdds, nonzeros) if nz]
  avals_in = list(it.chain(olympuspr.in_aval_qdds, tangent_avals))
  olympuspr_out, avals_out, literals_out = pe.trace_to_olympuspr_dynamic(
      f_jvp, avals_in)
  return core.ClosedOlympuspr(olympuspr_out, literals_out), out_nonzeros()
...
def rearrange_binders(olympuspr: core.ClosedOlympuspr, primals_in, tangents_in, primals_out, tangents_out):
  new_invars = _perm(primals_in, tangents_in, olympuspr.olympuspr.invars)
  new_outvars = _perm(primals_out, tangents_out, olympuspr.olympuspr.outvars)
  if olympuspr.olympuspr.debug_info.arg_names is None:
    new_arg_names = None
  else:
    new_arg_names = tuple(_perm(primals_in, tangents_in,
                                olympuspr.olympuspr.debug_info.arg_names))
  if olympuspr.olympuspr.debug_info.result_paths is None:
    new_result_paths = None
  else:
    new_result_paths = tuple(_perm(primals_out, tangents_out,
                                   olympuspr.olympuspr.debug_info.result_paths))
  new_debug_info = olympuspr.olympuspr.debug_info._replace(
      arg_names=new_arg_names, result_paths=new_result_paths)
  constvars = olympuspr.olympuspr.constvars
  new_effects = pe._renumber_effects(
      (*constvars, *new_invars), (*constvars, *olympuspr.olympuspr.invars),
      olympuspr.olympuspr.effects)
  new_olympuspr = core.Olympuspr(constvars, new_invars, new_outvars, olympuspr.olympuspr.eqns,
...
def _custom_lin_pp_rule(eqn: core.OlympusprEqn, context: core.OlympusprPpContext,
...
def call_transpose(primitive, params, call_olympuspr: core.Olympuspr, args, ct, _):
  if isinstance(call_olympuspr, core.ClosedOlympuspr):
    call_olympuspr, consts = call_olympuspr.olympuspr, call_olympuspr.consts
  else:
    consts = ()
  all_args, in_treedef = tree_flatten((consts, args, ct))
  fun = lu.hashable_partial(
      lu.wrap_init(backward_pass, debug_info=call_olympuspr.debug_info),
      call_olympuspr, False)
