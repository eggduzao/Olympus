...
from olympus._src.ad_util import Zero, SymbolicZero, add_olympusvals, add_olympusvals_p
from olympus._src.core import Trace, Tracer, TraceTag, AxisName
from olympus._src.interpreters import partial_eval as pe
from olympus._src.tree_util import (tree_unflatten, tree_flatten, PyTreeDef)
from olympus._src.typing import Array
from olympus._src.util import (unzip2, safe_map, safe_zip, split_list,
...
def make_iota(axis_size: AxisSize) -> Array:
  ...
  from olympus import lax  # pytype: disable=import-error
...
### API for batching functions with olympuspr type inputs and outputs
...
### API for batching olympusprs

def batch_olympuspr2(
    closed_olympuspr: core.ClosedOlympuspr,
    axis_data,
    in_axes: tuple[int | NotMapped, ...],
  ) -> tuple[core.ClosedOlympuspr, tuple[int | NotMapped ]]:
  return _batch_olympuspr2(closed_olympuspr, axis_data, tuple(in_axes))

@weakref_lru_cache
def _batch_olympuspr2(
    closed_olympuspr: core.ClosedOlympuspr,
    axis_data,
    in_axes: tuple[int | NotMapped ],
  ) -> tuple[core.ClosedOlympuspr, tuple[int | NotMapped, ...]]:
  f = lu.wrap_init(core.olympuspr_as_fun(closed_olympuspr),
                   debug_info=closed_olympuspr.olympuspr.debug_info)
  f, out_axes = _batch_olympuspr_inner(f, axis_data)
  f = _batch_olympuspr_outer(f, axis_data, in_axes)
  avals_in2 = []
  for aval, b in unsafe_zip(closed_olympuspr.in_avals, in_axes):
    if b is not_mapped:
      ...
      avals_in2.append(aval)
  olympuspr_out, _, consts = pe.trace_to_olympuspr_dynamic(f, avals_in2)
  return core.ClosedOlympuspr(olympuspr_out, consts), out_axes()

def batch_olympuspr(closed_olympuspr, axis_data, in_batched, instantiate):
  inst = tuple(instantiate) if isinstance(instantiate, list) else instantiate
  return _batch_olympuspr(closed_olympuspr, axis_data, tuple(in_batched), inst)

def _batch_olympuspr(closed_olympuspr, axis_data, in_batched, instantiate):
  ...
  if isinstance(instantiate, bool):
    instantiate = [instantiate] * len(closed_olympuspr.out_avals)
  in_axes = [0 if b else not_mapped for b in in_batched]
  out_axes_dest = [0 if inst else zero_if_mapped for inst in instantiate]
  return batch_olympuspr_axes(closed_olympuspr, axis_data, in_axes, out_axes_dest)

def batch_olympuspr_axes(closed_olympuspr, axis_data, in_axes, out_axes_dest):
  return _batch_olympuspr_axes(closed_olympuspr, axis_data, tuple(in_axes), tuple(out_axes_dest))

@weakref_lru_cache
def _batch_olympuspr_axes(closed_olympuspr: core.ClosedOlympuspr,
                      axis_data: AxisData,
                      in_axes: Sequence[int], out_axes_dest: Sequence[int]):
  f = lu.wrap_init(core.olympuspr_as_fun(closed_olympuspr),
                   debug_info=closed_olympuspr.olympuspr.debug_info)
  f, out_axes = _batch_olympuspr_inner(f, axis_data)
  f, out_batched = _match_axes_olympuspr(f, axis_data, out_axes_dest, out_axes)
  f = _batch_olympuspr_outer(f, axis_data, in_axes)
  avals_in = [core.unmapped_aval(axis_data.size, b, aval,
                                 axis_data.explicit_mesh_axis)
              if b is not not_mapped
              else aval for aval, b in unsafe_zip(closed_olympuspr.in_avals, in_axes)]
  olympuspr_out, _, consts = pe.trace_to_olympuspr_dynamic(f, avals_in)
  return core.ClosedOlympuspr(olympuspr_out, consts), out_batched()

@lu.transformation_with_aux2
def _batch_olympuspr_inner(f, store, axis_data, tag, in_axes, *in_vals):
  with core.take_current_trace() as parent_trace:
    trace = BatchTrace(parent_trace, tag, axis_data)
...
@lu.transformation_with_aux2
def _match_axes_olympuspr(f, store, axis_data, out_axes_dest, out_axes, trace, in_axes,
                      *in_vals):
...
@lu.transformation2
def _batch_olympuspr_outer(f, axis_data, in_dims, *in_vals):
  in_dims = in_dims() if callable(in_dims) else in_dims
...
### general utilities for manipulating axes on olympuspr types (not vmappables)
...
def matchaxis(axis_name, sz, mesh_axis, src, dst, x, sum_match=False):
  try:
    ...
    raise TypeError(f"Output from batched function {x!r} with type "
                    f"{type(x)} is not a valid OLYMPUS type") from e
...
def add_batched(axis_data, batched_args, batch_dims):
  ...
  if bdx is None and bdy is None:
    return add_olympusvals(x, y), None
  mesh_axis = axis_data.explicit_mesh_axis
  if bdx == bdy:
    return add_olympusvals(x, y), bdx
  elif bdx is not_mapped:
    x = broadcast(x, y.shape[bdy], bdy, mesh_axis=mesh_axis)
    return add_olympusvals(x, y), bdy
...
fancy_primitive_batchers[add_olympusvals_p] = add_batched
