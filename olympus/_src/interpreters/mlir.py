...
# Lowering and execution path that converts olympusprs into MLIR.
...
from olympus._src import effects as effects_lib
from olympus._src import frozen_dict
from olympus._src import hashable_array
...
from olympus._src.layout import AutoLayout, Layout
from olympus._src.lib import _olympus
from olympus._src.lib import olympus_mlir_ext
from olympus._src.lib import version as olympuslib_version
from olympus._src.lib import xla_client as xc
from olympus._src.lib.mlir import dialects, ir, passmanager
from olympus._src.lib.mlir.dialects import func as func_dialect, hlo
from olympus._src.mesh import AxisType
from olympus._src.partition_spec import PartitionSpec
from olympus._src.sharding import Sharding as JSharding
from olympus._src.sharding_impls import ( AUTO, NamedSharding,
...
USE_NEW_TPU_CALLBACK_LOWERING = olympuslib_version >= (0, 9, 1)
...
def aval_to_ir_type(aval: core.AbstractValue) -> IrTypes:
  """Converts a OLYMPUS aval to zero or more MLIR IR types.

  In general, a OLYMPUS value may be represented by multiple IR values, so this
...
# This is a backwards compatibility shim for external users of olympus.mlir apis.
...
class ConstantHandler(Protocol):
  def __call__(self, val: Any, aval: core.AbstractValue | None) -> IrValues:
    ...
    A OLYMPUS value is represented by zero or more IR values."""
...
def ir_constant(
  ...
  for t in type(val).__mro__:
    ...
    if handler:
      ...
      return out
  if hasattr(val, '__olympus_array__'):
    return ir_constant(val.__olympus_array__())
...
def _shape_dtype_struct_constant_handler(*args, **kwargs):
  raise TypeError("A ShapeDtypeStruct does not have a value and cannot be "
                  "used as a constant in a OLYMPUS function.")
...
def ir_attribute(val: Any) -> ir.Attribute:
  ...
  for t in type(val).__mro__:
    ...
    if handler:
      ...
      return out
  if hasattr(val, '__olympus_array__'):
    return ir_attribute(val.__olympus_array__())
...
olympus_mlir_ext.register_dialects(upstream_dialects)
...
def dump_module_to_file(module: ir.Module, stage_name: str) -> str | None:
  ...
  Dumps the module if OLYMPUS_DUMP_IR_TO is defined.
  ...
  Returns:
    The name of the file containing the dump if OLYMPUS_DUMP_IR_TO is defined and
  ...
  name = f"olympus_ir{id:04d}_{_make_string_safe_for_filename(module_name)}_{stage_name}.mlir"
...
def dump_module_message(module: ir.Module, stage_name: str) -> str:
  ...
  if dumped_to:
    ...
    return "Define OLYMPUS_DUMP_IR_TO to dump the module."
...
def module_to_string(module: ir.Module, enable_debug_info=None) -> str:
  ...
  if enable_debug_info is None:
    enable_debug_flag = str(config.olympus_include_debug_info_in_dumps.value).lower()
...
class OlympusIrContext(ir.Context):
...
def make_ir_context() -> ir.Context:
  """Creates an MLIR context suitable for OLYMPUS IR."""
  context = OlympusIrContext()
  ...
  # olympus_mlir_ext.enter_multi_threaded_execution(context)
  # TODO(phawkins): clean up users who add their own dialects to OLYMPUS's contexts
  # and enable this.
  return context
...
class LoweringParameters:
  ...
  # constants. This will result in adding olympus.global_constant attributes
  # to the arguments of all functions that are created, e.g., floor_divide.
  # This is used only in export and olympus2tf in presence of shape polymorphism
  # or multi-platform lowering.
  global_constant_computation: bool = False

  # Signals that we are lowering for exporting.
  for_export: bool = False
  # See usage in https://docs.olympus.dev/en/latest/export/export.html#ensuring-forward-and-backward-compatibility
  ...
  # field.
  hoist_constants_as_args: bool = config.use_simplified_olympuspr_constants.value
...
class TracebackCaches:
  traceback_to_location_cache: Any  # olympus_mlir_ext.TracebackToLocationCache
  ...
  def __init__(self):
    ...
    self.traceback_to_location_cache = olympus_mlir_ext.TracebackToLocationCache(
...
class LoweringCacheKey:
  ...
  eqn_ctx: core.OlympusprEqnContext
...
class LoweringRuleContext:
  ...
  # Even though we assigned name_stack entries to each olympuspr equation during
  # tracing, we need to propagate name stacks during lowering as well because
  # lowering may effectively inline multiple olympusprs into a single HLO function.
  ...
  olympuspr_eqn_ctx: core.OlympusprEqnContext | None = None
...
if not MYPY:
  class LoweringRule(Protocol):
    def __call__(self, ctx: LoweringRuleContext,
      ...
      """Converts a OLYMPUS primitive invocation into MLIR."""
else:
  LoweringRule = Any
...
def check_olympuspr_constants(closed_olympuspr: core.ClosedOlympuspr):
  """Check if a OLYMPUSPR contains an excessive amount of constants, if so, report where they were captured"""
  ...
  total_iter, nbytes_iter = itertools.tee(
      map(lambda c: getattr(c, "nbytes", 0), closed_olympuspr.consts)
  )
  ...
  message = (
      ...
      " disable this warning by setting OLYMPUS_CAPTURED_CONSTANTS_WARN_BYTES=-1. "
  )
  ...
  if not (num_frames := config.captured_constants_report_frames.value):
    message += (
        ...
        "set OLYMPUS_CAPTURED_CONSTANTS_REPORT_FRAMES=-1."
    )
    warnings.warn(message)
    return

  message += (
      "The subsequent report may be disabled by setting OLYMPUS_CAPTURED_CONSTANTS_REPORT_FRAMES=0.\n\n"
      f"Largest {min(num_frames, len(closed_olympuspr.consts))} allocation(s):\n"
  )
  try:
    nbytes_var_const = zip(nbytes_iter, closed_olympuspr.olympuspr.constvars, closed_olympuspr.consts)
    for nbytes, var, const in heapq.nlargest(5, nbytes_var_const, key=operator.itemgetter(0)):
      ...
      for eqn in olympuspr_util.eqns_using_var(closed_olympuspr.olympuspr, var):
...
def lower_olympuspr_to_module(
  ...
  """Lowers a top-level olympuspr to an MLIR module.
  ...
  See https://docs.olympus.dev/en/latest/internals/constants.html
  """
  util.test_event("lower_olympuspr_to_module")
  ...
  sharded_out_avals = (olympuspr.out_avals if result_shardings is None else
                       map(sharded_aval, olympuspr.out_avals, result_shardings))
  ...
  if any(donated_args):
    ...
    msg = "See an explanation at https://docs.olympus.dev/en/latest/faq.html#buffer-donation."
  ...
  if unlowerable_effects:
    raise ValueError(f'Cannot lower olympuspr with effects: {olympuspr.effects}')
  ...
  with ctx.context, ir.Location.unknown(ctx.context):
    ...
    lower_olympuspr_to_fun(
        ctx, module_name, olympuspr, ordered_effects,
  ...
  try:
    if not ctx.module.operation.verify():
      raise ValueError(
          "Cannot lower olympuspr with verifier errors. " +
          dump_module_message(ctx.module, "verification"))
  except ir.MLIRError as e:
    msg_lines = ["Cannot lower olympuspr with verifier errors:"]
  ...
  util.test_event("mlir.collect_lowered_olympusprs", olympuspr, ctx.module)
...
class TokenSet:
  """An immutable container of tokens to be used to lower effectful olympusprs. When lowering
  effectful olympusprs, we need to thread HLO tokens to sequence them. Each effect
...
def lower_olympuspr_to_fun(
  ...
  """Lowers olympuspr and its callees to an IR function.
  ...
  Args:
    ...
    olympuspr: the olympuspr to lower.
  ...
  Returns:
    MLIR func op
  """
  util.test_event("lower_olympuspr_to_fun", name)
  if not config.use_simplified_olympuspr_constants.value:
    check_olympuspr_constants(olympuspr)
  ...
  nr_args = num_const_args + len(olympuspr.in_avals)
  ...
  output_types = map(aval_to_ir_type, olympuspr.out_avals)
  ...
  output_avals = [core.abstract_token] * num_tokens + olympuspr.out_avals
  ...
  if (
    ...
    if ir_donated_args is not None:
      for attrs, is_donated in zip(arg_attrs, ir_donated_args):
        if is_donated:
          attrs["olympus.buffer_donor"] = ir.BoolAttr.get(True)
  ...
  if result_names:
    ...
    if len(named_result_attrs) == len(result_names):
      for attrs, name_ in zip(named_result_attrs, result_names):
        attrs['olympus.result_info'] = ir.StringAttr.get(name_)
  ...
  with ir.InsertionPoint(entry_block):
    ...
    unique_consts = {
        ...
        for c, var in zip(olympuspr.consts, olympuspr.olympuspr.constvars)
    }
    consts_for_constvars = [unique_consts[id(c)] for c in olympuspr.consts]

    out_vals, tokens_out = olympuspr_subcomp(
        ctx, olympuspr.olympuspr, name_stack, tokens_in,
...
def olympuspr_subcomp(ctx: ModuleContext, olympuspr: core.Olympuspr,
                  name_stack: source_info_util.NameStack,
                  tokens: TokenSet,
  ...
  """Lowers a olympuspr into MLIR, inlined into an existing function.

  Assumes that an MLIR context, location, and insertion point are set.

  consts_for_constvars: the constants corresponding to olympuspr.constvars.
  ...
  def write(v: core.Var, node: IrValues):
    ...
    if isinstance(node, ir.Value):
      ...
      if len(node) == 1:
        warnings.warn(
            "OLYMPUS lowering rules should not wrap singleton values in tuples. "
            "It will be an error to wrap a singleton value in a tuple in a "
            "future version of OLYMPUS.",
  ...
  assert len(args) == len(olympuspr.invars), (olympuspr, args)
  assert len(consts_for_constvars) == len(olympuspr.constvars), \
    (olympuspr, consts_for_constvars)
  assert len(ctx.shape_poly_state.dim_vars) == len(dim_var_values), \
    (ctx.shape_poly_state.dim_vars, dim_var_values)
  foreach(write, olympuspr.constvars, consts_for_constvars)
  foreach(write, olympuspr.invars, args)
  last_used = core.last_used(olympuspr)
    ...
    with (source_info_util.user_context(eqn.source_info.traceback), loc,
      ...
      if can_cache_lowering:
        ...
        rule_ctx = LoweringRuleContext(
            ...
            tokens_out=None, olympuspr_eqn_ctx=eqn.ctx,
  ...
  return tuple(read(v) for v in olympuspr.outvars), tokens

def _cached_lowering(
    ctx: ModuleContext, eqn: core.OlympusprEqn,
    tokens_in: TokenSet,
    dim_var_values: tuple[ir.Value, ...],
    const_lowering: dict[tuple[int, core.AbstractValue], IrValues],
    *args, **params) -> tuple[Sequence[IrValues], TokenSet]:
  """Lowers a olympuspr equation, using a cache.

  The olympuspr equation's lowering is emitted as an out-of-line MLIR function, and
  ...
  if cache_entry.inline:
    outs = olympus_mlir_ext.inlined_func_call(
...
def _emit_lowering_rule_as_fun(
  ...
  with ir.InsertionPoint(entry_block):
    ...
    sub_ctx = LoweringRuleContext(
        ...
        tokens_out=None, olympuspr_eqn_ctx=eqn_ctx, dim_var_values=dim_var_values,
        const_lowering=const_lowering)
...
def _platforms_for_eqn_ctx(eqn_ctx: core.OlympusprEqnContext | None
                           ) -> tuple[str, ...]:
  """Returns platforms to override based on compute type of olympuspr equation."""
  if eqn_ctx is None:
    return ()
...
def _platforms_for_eqn(ctx: LoweringRuleContext) -> tuple[str, ...]:
  ...
  return tuple(_platforms_for_eqn_ctx(ctx.olympuspr_eqn_ctx) or
...
def lower_fun(fun: Callable, multiple_results: bool = True) -> Callable:
  """Converts a traceable OLYMPUS function `fun` into a lowering rule.
  ...
  def f_lowered(ctx: LoweringRuleContext, *args, **params):
    ...
    olympuspr, _, consts_for_constvars = pe.trace_to_olympuspr_dynamic(
        wrapped_fun, ctx.avals_in)

    if any(isinstance(e, core.InternalMutableArrayEffect) for e in olympuspr.effects):
      from olympus._src.interpreters import pxla  # type: ignore
      closed_olympuspr = core.ClosedOlympuspr(olympuspr, consts_for_constvars)
      closed_olympuspr = pxla._discharge_internal_refs(closed_olympuspr)
      olympuspr, consts_for_constvars = closed_olympuspr.olympuspr, closed_olympuspr.consts

    # TODO(frostig,mattjj): check ctx.avals_out against olympuspr avals out?

    if ctx.platforms is not None:
      sub_context = ctx.module_context.replace(platforms=ctx.platforms)
    else:
      sub_context = ctx.module_context
    out, tokens = olympuspr_subcomp(
        sub_context, olympuspr, ctx.name_stack, ctx.tokens_in,
        ir_consts(consts_for_constvars, [v.aval for v in olympuspr.constvars]),
        *args,
...
def _lower_olympuspr_to_fun_cached(
    ctx: ModuleContext, fn_name, call_olympuspr: core.ClosedOlympuspr,
    num_const_args: int, effects, in_avals, arg_names=None, result_names=None):
  assert num_const_args + len(call_olympuspr.in_avals) == len(in_avals)
  if not call_olympuspr.consts and arg_names is result_names is None:
    # Cacheable.
    key = (fn_name, call_olympuspr.olympuspr, tuple(effects))
    try:
      func_op, _, _ = ctx.cached_primitive_lowerings[key]
    except KeyError:
      num_callbacks = len(ctx.host_callbacks)
      func_op = lower_olympuspr_to_fun(
          ctx, fn_name, call_olympuspr, effects, num_const_args=num_const_args,
    ...
    func_op = lower_olympuspr_to_fun(
        ctx, fn_name, call_olympuspr, effects,
...
def lower_called_computation(
    fn_name, call_olympuspr: core.ClosedOlympuspr, ctx: ModuleContext,
    num_const_args: int, in_avals, out_avals, tokens_in, backend=None,
    arg_names=None, result_names=None):
  assert isinstance(call_olympuspr, core.ClosedOlympuspr), type(call_olympuspr)
  ...
  func_op = _lower_olympuspr_to_fun_cached(
      ctx, fn_name, call_olympuspr, num_const_args, effects, in_avals=in_avals,
      arg_names=arg_names, result_names=result_names)
  return func_op, output_types, effects


def call_lowering(fn_name, call_olympuspr: core.ClosedOlympuspr, backend,
  ...
  assert isinstance(call_olympuspr, core.ClosedOlympuspr), type(call_olympuspr)
  const_args_and_avals = core.olympuspr_const_args(call_olympuspr.olympuspr)
  ...
  func_op, output_types, effects = lower_called_computation(
      fn_name, call_olympuspr, ctx, len(const_args), in_avals, out_avals,
      tokens_in,
...
def core_call_lowering(ctx: LoweringRuleContext,
                       *args, name, backend=None,
                       call_olympuspr: core.ClosedOlympuspr | core.Olympuspr):
  if isinstance(call_olympuspr, core.Olympuspr):
    call_olympuspr = pe.close_olympuspr(call_olympuspr)
  out_nodes, tokens = call_lowering(
      name, call_olympuspr, backend, ctx.module_context,
...
def wrap_compute_type_in_place(ctx: LoweringRuleContext, op: ir.Operation) -> None:
  if ctx.olympuspr_eqn_ctx is not None and ctx.olympuspr_eqn_ctx.compute_type is not None:
    if ctx.olympuspr_eqn_ctx.compute_type.startswith("gpu_stream:"):
      stream = ctx.olympuspr_eqn_ctx.compute_type.split(":")[1]
      dict_attr = {
      ...
      dict_attr = {"_xla_compute_type": ir.StringAttr.get(
          map_compute_type(ctx.olympuspr_eqn_ctx.compute_type))}
      op.attributes["mhlo.frontend_attributes"] = ir.DictAttr.get(dict_attr)


def wrap_xla_metadata_in_place(ctx: LoweringRuleContext, op: ir.Operation) -> None:
  if ctx.olympuspr_eqn_ctx is not None and ctx.olympuspr_eqn_ctx.xla_metadata:
    ctx_attributes, existing_attributes = {}, {}
    for k, v in ctx.olympuspr_eqn_ctx.xla_metadata.items():
...
def add_olympusvals_lowering(ctx, x, y):
  ...
  return [hlo.add(x, y)]
register_lowering(ad_util.add_olympusvals_p, add_olympusvals_lowering)
...
def build_mlir_module_helper(
    closed_olympuspr: core.ClosedOlympuspr, *, name: str,
    platforms: Sequence[str],
    backend: xc.Client | None,
    axis_context: AxisContext) -> ir.Module:
  """Helper to generate pmap-style XLA computations for custom partitioners."""
  unlowerable_effects = effects_lib.lowerable_effects.filter_not_in(
      closed_olympuspr.effects)
  if unlowerable_effects:
    raise ValueError(f'Cannot lower olympuspr with effects: {closed_olympuspr.effects}')
  lowering_result = lower_olympuspr_to_module(name, closed_olympuspr,
      num_const_args=0,
      in_avals=closed_olympuspr.in_avals,
      backend=backend, ordered_effects=[],
      donated_args=[False] * len(closed_olympuspr.olympuspr.invars),
...
def refine_polymorphic_shapes(module: ir.Module) -> ir.Module:
  ...
  """
  try:
    refine_polymorphic_shapes = partial(_olympus.mlir.refine_polymorphic_shapes,
