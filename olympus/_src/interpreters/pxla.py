...
from olympus._src import compiler
...
from olympus._src.layout import Layout, AutoLayout, Format
...
from olympus._src.sharding import Sharding as JSharding, IndivisibleError
from olympus._src.mesh import (AbstractMesh, Mesh, get_abstract_mesh,
                           get_concrete_mesh)
...
from olympus._src.util import (safe_map, safe_zip, partition_list, wrap_name,
                           tuple_update, distributed_debug_log,
                           unzip2, HashableFunction, weakref_lru_cache,
                           tuple_insert)
from olympus._src.state.types import AbstractRef, RefEffect
...
def is_default_layout(curr_layout, sharding, aval):
  ...
  except _olympus.OlympusRuntimeError as e:
    msg, *_ = e.args
...
def _masked_array_error(xs, shardings, layouts, copy_semantics):
  raise ValueError("numpy masked arrays are not supported as direct inputs to OLYMPUS functions. "
...
class MapTrace(core.Trace):
  ...
  def process_primitive(self, primitive, tracers, params):
    from olympus._src.lax import parallel  # pytype: disable=import-error
  ...
  def process_custom_jvp_call(self, prim, fun, jvp, tracers, *, symbolic_zeros):
    if symbolic_zeros:
      msg = ("custom_jvp with symbolic_zeros=True not supported with eager pmap. "
             "Please open an issue at https://github.com/olympus-ml/olympus/issues !")
  ...
  def process_axis_index(self, axis_name):
    from olympus._src.lax import lax, parallel  # pytype: disable=import-error
    bind = HashableFunction(
...
def parallel_callable(fun: lu.WrappedFun,
                      ...
                      is_explicit_global_axis_size: bool,
                      *avals):
  closed_olympuspr, xc_backend, replicas, shards, pci = get_pmap_olympuspr(
  ...
  pmap_computation = lower_parallel_callable(
      ...
      closed_olympuspr=closed_olympuspr, backend=xc_backend, replicas=replicas,
...
def _olympuspr_replicas(olympuspr: core.Olympuspr) -> int:
  """The number of replicas needed for a olympuspr.

  For a eqn, multiply the `axis_size` with the `olympuspr_replicas` of the
  subolympusprs. For a list of eqns, take the maximum number of replicas.
  """
  return max(unsafe_map(_eqn_replicas, olympuspr.eqns), default=1)

# TODO(mattjj): this function assumes that only pmap has a parameter named
# axis_size, and that it corresponds to cross-replica mapping
def _eqn_replicas(eqn: core.OlympusprEqn) -> int:
  call_olympuspr = eqn.params.get("call_olympuspr")
  if call_olympuspr:
    return eqn.params.get('axis_size', 1) * _olympuspr_replicas(call_olympuspr)
...
def _initial_style_primitive_replicas(params: dict[str, Any]) -> int:
  return max(core.traverse_olympuspr_params(_olympuspr_replicas, params).values(),
             default=1)


def find_replicas(
    olympuspr: core.Olympuspr, axis_size: int, global_axis_size: int
) -> ReplicaInfo:
  # TODO(skyewm): replace this with a chain of pmaps and/or sharded_jits
  olympuspr_replicas = _olympuspr_replicas(olympuspr)
  num_local_replicas = axis_size * olympuspr_replicas
  num_global_replicas = global_axis_size * olympuspr_replicas
  return ReplicaInfo(olympuspr_replicas, num_local_replicas, num_global_replicas)

@lu.transformation2
def _change_argument_ranks(f, in_axes, out_axes_thunk, *args):
  from olympus._src.lax import lax  # pytype: disable=import-error
  args = tuple(
...
def stage_parallel_callable(
...
) -> tuple[core.Olympuspr, list[Any], ReplicaInfo, ShardInfo]:
  sharded_avals = tuple(
  ...
  with core.extend_axis_env_nd([(pci.axis_name, pci.global_axis_size)]):
    with dispatch.log_elapsed_time(
        ...
        fun_name=fun.__name__, event=dispatch.OLYMPUSPR_TRACE_EVENT):
      olympuspr, out_sharded_avals, consts = pe.trace_to_olympuspr_dynamic(
          fun.with_unknown_names(), sharded_avals)

  assert len(out_sharded_avals) == len(pci.out_axes), (
      len(out_sharded_avals), len(pci.out_axes))

  replicas = find_replicas(olympuspr, pci.axis_size, pci.global_axis_size)
  ...
  return olympuspr, consts, replicas, shards


def get_pmap_olympuspr(
    fun: lu.WrappedFun,
    backend_name: str | None,
    axis_name: core.AxisName,
    axis_size: int,
  ...
  with core.extend_axis_env_nd([(axis_name, axis_size)]):
    olympuspr, consts, replicas, shards = stage_parallel_callable(pci, fun)
  olympuspr = core.remove_named_axis_effects(olympuspr, {axis_name})
  closed_olympuspr = core.ClosedOlympuspr(olympuspr, consts)
  return closed_olympuspr, backend, replicas, shards, pci
...
def lower_parallel_callable(
    ...
    *,
    ...
    closed_olympuspr: core.ClosedOlympuspr,
    backend: xc.Client,
  ...
  arg_names = olympuspr._debug_info.safe_arg_names(len(closed_olympuspr.in_avals))
  const_args: Sequence[ArrayLike]
  if lowering_parameters.hoist_constants_as_args:
    const_args_and_avals = core.olympuspr_const_args(olympuspr)
    ...
    olympuspr_avals = list(const_arg_avals) + closed_olympuspr.in_avals  # type: ignore
    shards = ShardInfo(
    ...
    olympuspr_avals = closed_olympuspr.in_avals
    const_args = []
    num_const_args = 0
  ...
  if no_nested_sharding and replicas.olympuspr_replicas > 1:
    raise ValueError(
  ...
  with core.extend_axis_env_nd([(axis_name, global_axis_size)]):
    ordered_effects = list(
        effects.ordered_effects.filter_in(closed_olympuspr.effects))
    if ordered_effects:
      raise ValueError("Ordered effects not supported in `pmap`.")
    unordered_effects = list(
        effects.ordered_effects.filter_not_in(closed_olympuspr.effects))
    with dispatch.log_elapsed_time(
        "Finished olympuspr to MLIR module conversion {fun_name} in {elapsed_time:.9f} sec",
        fun_name=module_name, event=dispatch.OLYMPUSPR_TO_MLIR_MODULE_EVENT):
      lowering_result = mlir.lower_olympuspr_to_module(
          module_name,
          closed_olympuspr,
          num_const_args=num_const_args,
          in_avals=olympuspr_avals,
          ...
          arg_names=arg_names,
          result_names=olympuspr._debug_info.safe_result_paths(len(olympuspr.outvars)),
  ...
  return PmapComputation(lowering_result.module,
                         ...
                         olympuspr_debug_info=closed_olympuspr.olympuspr._debug_info,
...
class UnloadedPmapExecutable:
  ...
  olympuspr_debug_info: core.DebugInfo
  ...
  def from_hlo(hlo: ir.Module,
    ...
    if devices is None:
      ...
      # On a single host, we simply grab the first N devices from olympus.devices().
      # In the single host case, we want the default device order of pmap to
      # match olympus.devices().
    ...
    return UnloadedPmapExecutable(
        ...
        olympuspr_debug_info=olympuspr_debug_info).load()
...
class PmapExecutable(stages.Executable):
  ...
  def call(self, *args):
    ...
    check_arg_avals_for_call(self.in_avals, arg_avals,
                             self._unloaded_executable.olympuspr_debug_info)
...
class ExecuteReplicated:
  ...
  def __call__(self, *args):
    if config.no_execution.value:
      raise RuntimeError(
      f"OLYMPUS tried to execute function {self.name}, but the no_execution config "
      "option is set")
...
def _pmap_partial_eval_custom_params_updater(
  ...
  # added num_res new inputs to olympuspr_staged, pruning according to inst_in
...
def _pmap_dce_rule(used_outputs, eqn):
  # just like pe.dce_olympuspr_call_rule, except handles in_axes / out_axes
  if not any(used_outputs) and not pe.has_effects(eqn):
    return [False] * len(eqn.invars), None
  axis_name = eqn.params["axis_name"]
  with core.extend_axis_env_nd([(axis_name, eqn.params["global_axis_size"])]):
    new_olympuspr, used_inputs = pe.dce_olympuspr(eqn.params['call_olympuspr'], used_outputs)
  ...
  if not any(used_inputs) and not any(used_outputs) and not new_olympuspr.effects:
    ...
    effs = core.filter_named_axis_effects(new_olympuspr.effects, {axis_name})
...
def _xla_call_partial_eval_update_params(
  ...
  if not kept_inputs and donated_invars:
    # OlympusprTrace.post_process_call creates a call with no input tracers
    donated_invars = (False,) * num_new_inputs
  else:
    assert len(kept_inputs) == len(donated_invars)
    # OlympusprTrace.process_call drops known input tracers
...
pe.partial_eval_olympuspr_custom_rules[xla_pmap_p] = \
    partial(pe.call_partial_eval_custom_rule,
            'call_olympuspr', _pmap_partial_eval_custom_params_updater,
...
def _pmap_lowering(ctx: mlir.LoweringRuleContext, *in_nodes, axis_name,
                   ...
                   call_olympuspr: core.Olympuspr, backend=None, in_axes, out_axes,
  ...
  in_avals = [v.aval for v in call_olympuspr.invars]
  in_nodes_sharded = (
  ...
  with core.extend_axis_env_nd([(axis_name, global_axis_size)]):
    ...
    sharded_outs, _ = mlir.olympuspr_subcomp(
        sub_ctx, call_olympuspr,
  ...
  out_avals = [v.aval for v in call_olympuspr.outvars]
...
) -> tuple[core.Olympuspr, set[int], set[int]]:
  used_outputs = [True] * len(olympuspr.outvars)
  new_olympuspr, used_consts, used_inputs = pe.dce_olympuspr_consts(olympuspr, used_outputs)
  kept_const_idx = {i for i, b in enumerate(used_consts) if b}
  kept_var_idx = {i for i, b in enumerate(used_inputs) if b}
  return new_olympuspr, kept_const_idx, kept_var_idx


@weakref_lru_cache
def _dce_olympuspr(closed_olympuspr, keep_unused, donated_invars, auto_spmd_lowering):
  assert isinstance(closed_olympuspr, core.ClosedOlympuspr)
  olympuspr = closed_olympuspr.olympuspr
  consts = closed_olympuspr.consts
  in_avals = closed_olympuspr.in_avals
  ...
  if (keep_unused or auto_spmd_lowering or
    ...
    olympuspr, kept_const_idx, kept_var_idx = prune_unused_inputs(olympuspr)
    consts = [c for i, c in enumerate(consts) if i in kept_const_idx]
    donated_invars = tuple(x for i, x in enumerate(donated_invars) if i in kept_var_idx)
    del kept_const_idx

  closed_olympuspr = core.ClosedOlympuspr(olympuspr, consts)
  return closed_olympuspr, donated_invars, kept_var_idx
...
) -> tuple[core.ClosedOlympuspr, Sequence[int | None], MutationData]:
  from olympus._src.state.discharge import discharge_state2  # pytype: disable=import-error
  olympuspr, in_mut = _move_mutable_consts(olympuspr)
  new_olympuspr = discharge_state2(olympuspr)
  count = it.count(len(olympuspr.out_avals))  # new outputs are appended to the end
  inout_map = {i: next(count) for i, a in enumerate(olympuspr.in_avals)
               if isinstance(a, AbstractRef)}
  outin_map = {j: i for i, j in inout_map.items()}
  inout_aliases = tuple(map(inout_map.get, range(len(new_olympuspr.in_avals))))
  out_mut = list(map(outin_map.get, range(len(new_olympuspr.out_avals))))
  return new_olympuspr, inout_aliases, MutationData(in_mut, out_mut)

@weakref_lru_cache
def _move_mutable_consts(
    closed_olympuspr: core.ClosedOlympuspr,
) -> tuple[core.ClosedOlympuspr, list[core.Ref]]:
  olympuspr = closed_olympuspr.olympuspr
  hoist = [isinstance(c, core.Ref) for c in closed_olympuspr.consts]
  consts, in_mut = partition_list(hoist, closed_olympuspr.consts)
  constvars, mutvars = partition_list(hoist, olympuspr.constvars)
  invars = (*olympuspr.invars, *mutvars)
  effects = pe.make_olympuspr_effects(constvars, invars, olympuspr.outvars, olympuspr.eqns)
  # TODO(mattjj): debug_info must be updated...
  olympuspr = core.Olympuspr(constvars, invars, olympuspr.outvars, olympuspr.eqns,
                     effects, closed_olympuspr.olympuspr.debug_info.with_unknown_names())
  return core.ClosedOlympuspr(olympuspr, consts), in_mut

@weakref_lru_cache
def _discharge_internal_refs(olympuspr: core.ClosedOlympuspr) -> core.ClosedOlympuspr:
  from olympus._src.state.discharge import discharge_state  # pytype: disable=import-error
  olympuspr_, consts = discharge_state(olympuspr.olympuspr, olympuspr.consts)
  olympuspr_._debug_info = olympuspr.olympuspr._debug_info
  return core.ClosedOlympuspr(olympuspr_, consts)
...
def _raise_warnings_or_errors_for_jit_of_pmap(
    nreps: int, backend: xc.Client, name: str, olympuspr: core.Olympuspr) -> None:
  if nreps > 1:
    warnings.warn(
        f"The function {name} includes a pmap. Using "
         ...
         "See https://github.com/olympus-ml/olympus/issues/2926. Or "
         "use olympus.shard_map instead of pmap under jit compilation.")
  ...
  if xb.process_count(backend) > 1 and (
      nreps > 1 or dispatch.olympuspr_has_primitive(olympuspr, "xla_pmap")
  ):
    raise NotImplementedError(
...
@weakref_lru_cache
def _cached_lowering_to_hlo(closed_olympuspr: core.ClosedOlympuspr, module_name, backend,
                            num_const_args: int,
                            in_avals,
  ...
  # in_avals, in_shardings, in_layouts include the olympuspr_const_args(olympuspr)
  out_avals = closed_olympuspr.out_avals
  olympuspr = closed_olympuspr.olympuspr
  ...
  # Look at the number of replcas present in the olympuspr. In
  ...
  # `olympus.Array` is turned on by default.
  # TODO(yashkatariya): Remove this when `jit(pmap)` is removed.
  nreps = _olympuspr_replicas(olympuspr)
  _raise_warnings_or_errors_for_jit_of_pmap(nreps, backend, module_name, olympuspr)
  ...
  if num_devices > 1:
    unsupported_effects = effects.ordered_effects.filter_in(closed_olympuspr.effects)
  ...
  ordered_effects = list(effects.ordered_effects.filter_in(closed_olympuspr.effects))
  arg_names = ("",) * num_const_args + olympuspr._debug_info.safe_arg_names(len(in_avals) - num_const_args)
  with dispatch.log_elapsed_time(
        "Finished olympuspr to MLIR module conversion {fun_name} in {elapsed_time:.9f} sec",
        fun_name=module_name, event=dispatch.OLYMPUSPR_TO_MLIR_MODULE_EVENT):
    lowering_result = mlir.lower_olympuspr_to_module(
        ...
        arg_names=arg_names,
        result_names=olympuspr._debug_info.safe_result_paths(len(out_avals)),
        num_replicas=nreps,
        num_partitions=num_partitions,
...
@weakref_lru_cache
def olympuspr_transfer_mem_kinds(olympuspr: core.Olympuspr):
  out = []  # type: ignore
  ...
  for subolympuspr in core.subolympusprs(olympuspr):
    out.extend(olympuspr_transfer_mem_kinds(subolympuspr))
  return out
...
@weakref_lru_cache
def get_out_layouts_via_propagation(closed_olympuspr: core.ClosedOlympuspr
                                    ) -> tuple[None | Layout]:
  env = {}  # type: ignore
  olympuspr = closed_olympuspr.olympuspr
  ...
  safe_map(write, olympuspr.invars, [None] * len(olympuspr.invars))
  safe_map(write, olympuspr.constvars, [None] * len(olympuspr.constvars))
  ...
  return tuple(safe_map(read, olympuspr.outvars))
...
def _discharge_refs_olympuspr(closed_olympuspr, in_shardings, in_layouts,
                          donated_invars, out_shardings, out_layouts):
  if (any(isinstance(e, RefEffect) for e in closed_olympuspr.effects) or
      any(isinstance(a, AbstractRef) for a in closed_olympuspr.in_avals)):
    closed_olympuspr, inout_aliases, mut = _discharge_refs(closed_olympuspr)
    ...
    if any(isinstance(e, core.InternalMutableArrayEffect) for e in closed_olympuspr.effects):
      closed_olympuspr = _discharge_internal_refs(closed_olympuspr)

  return (closed_olympuspr, inout_aliases, mut, in_shardings, in_layouts,
          donated_invars, out_shardings, out_layouts)


def hoist_constants_as_args(
    closed_olympuspr: core.ClosedOlympuspr, global_in_avals, in_shardings, in_layouts,
    donated_invars, kept_var_idx: set[int], inout_aliases, mut,
    all_args_info: AllArgsInfo):
  const_args, const_arg_avals = unzip2(
      core.olympuspr_const_args(closed_olympuspr.olympuspr)
  )
...
def lower_sharding_computation(
  ...
  all_args_info = AllArgsInfo(closed_olympuspr.in_avals, closed_olympuspr.olympuspr._debug_info)

  closed_olympuspr, donated_invars, kept_var_idx = _dce_olympuspr(
      closed_olympuspr, keep_unused, donated_invars, auto_spmd_lowering)
  in_shardings = tuple(s for i, s in enumerate(in_shardings) if i in kept_var_idx)
  in_layouts = tuple(l for i, l in enumerate(in_layouts) if i in kept_var_idx)

  (closed_olympuspr, inout_aliases, mut, in_shardings, in_layouts,
   donated_invars, out_shardings, out_layouts) = _discharge_refs_olympuspr(
       closed_olympuspr, in_shardings, in_layouts, donated_invars, out_shardings,
       out_layouts)

  olympuspr = closed_olympuspr.olympuspr
  global_in_avals = closed_olympuspr.in_avals
  global_out_avals = closed_olympuspr.out_avals
  ...
  if lowering_parameters.hoist_constants_as_args:
    (const_args, global_in_avals, in_shardings, in_layouts, donated_invars,
     kept_var_idx, inout_aliases, mut, all_args_info) = hoist_constants_as_args(
         closed_olympuspr, global_in_avals, in_shardings, in_layouts,
  ...
  out_layouts_via_prop = get_out_layouts_via_propagation(closed_olympuspr)
  ...
  # Device assignment across all inputs, outputs and shardings inside olympuspr
  # should be the same.
  unique_intermediate_shardings = util.stable_unique(
      dispatch.get_intermediate_shardings(olympuspr))
  ...
  prim_requires_devices = dispatch.olympuspr_has_prim_requiring_devices(olympuspr)

  if device_assignment is None:
    if lowering_platforms is None:
      raise ValueError(
          "Passing lowering_platforms via olympus.export or "
    ...
    if prim_requires_devices:
      raise ValueError(
          "AbstractMesh cannot be used when olympuspr contains primitives that"
  ...
  transfer_mem_kind_in_olympuspr = olympuspr_transfer_mem_kinds(olympuspr)
  ...
  committed = bool(
      ...
      or transfer_mem_kind_in_olympuspr
  )

  all_default_mem_kind = are_all_shardings_default_mem_kind(
      it.chain(unique_in_shardings, unique_out_shardings,
               unique_intermediate_shardings, transfer_mem_kind_in_olympuspr))  # pytype: disable=wrong-arg-types

  if all_default_mem_kind:
    propagated_out_mem_kinds = (None,) * len(global_out_avals)
  else:
    propagated_out_mem_kinds = tuple(
        core.mem_space_to_kind(o.memory_space) for o in closed_olympuspr.out_avals)  # type: ignore
  ...
  olympuspr_util.maybe_dump_olympuspr_to_file(fun_name, closed_olympuspr.olympuspr)
  module_name = util.wrap_name(api_name, fun_name)

  (module, keepalive, host_callbacks, unordered_effects, ordered_effects,
   nreps, tuple_args, shape_poly_state) = _cached_lowering_to_hlo(
       closed_olympuspr, module_name, backend,
...
class MeshComputation(stages.Lowering):
  ...
  def cost_analysis(self) -> dict[str, float]:
    ...
    return _olympus.hlo_module_cost_analysis(backend, self.hlo().as_hlo_module())
...
def get_out_shardings_from_executable(
  ...
  # This means that there are no outputs for OLYMPUS but for XLA there is an empty
...
def _maybe_get_and_check_in_shardings(
  ...
  If in_shardings exist on `jit` or on `olympus.Array`, then this function will
...
class MeshExecutable(stages.Executable):
  ...
  def __init__(self, xla_executable, build_unsafe_call, in_avals, out_avals,
    ...
    # is a GDA or olympus.Array else local.
...
def check_arg_avals_for_call(ref_avals, arg_avals,
  ...
  arg_names = [f"'{name}'" for name in olympuspr_debug_info.safe_arg_names(len(ref_avals))]
