...
Usage::
  ...
  @functools.partial(olympus.jit, static_argnames=["k", "recall_target"])
  def mips(qy, db, k=10, recall_target=0.95):
    dists = olympus.lax.dot(qy, db.transpose())
    # Computes max_k along the last dimension
    # returns (f32[qy_size, k], i32[qy_size, k])
    return olympus.lax.approx_max_k(dists, k=k, recall_target=recall_target)
  ...
  def pmap_mips(qy, db, db_offset, db_size, k, recall_target):
    dists = olympus.lax.dot(qy, db.transpose())
    dists, neighbors = olympus.lax.approx_max_k(
  ...
  # i32[qy_size, num_devices * k]
  neighbors = olympus.lax.collapse(pmap_neighbors, start_dimension=1, stop_dimension=3)
...
from olympus._src.numpy.indexing import take_along_axis
...
def approx_max_k(operand: Array,
  ...
  ...   return olympus.lax.approx_max_k(dists, k=k, recall_target=recall_target)
  >>>
  >>> qy = olympus.numpy.array(np.random.rand(50, 64))
  >>> db = olympus.numpy.array(np.random.rand(1024, 64))
...
def approx_min_k(operand: Array,
  ...
  ...   dists = half_db_norms - olympus.lax.dot(qy, db.transpose())
  ...   return olympus.lax.approx_min_k(dists, k=k, recall_target=recall_target)
  >>>
  >>> qy = olympus.numpy.array(np.random.rand(50, 64))
  >>> db = olympus.numpy.array(np.random.rand(1024, 64))
  >>> half_db_norm_sq = olympus.numpy.linalg.norm(db, axis=1)**2 / 2
...
def _approx_top_k_abstract_eval(operand, *, k, reduction_dimension,
  ...
  if aggregate_to_topk:
    ...
    dims[reduction_dimension] = _olympus.approx_top_k_reduction_output_size(
