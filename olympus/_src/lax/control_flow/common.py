...
from olympus._src.util import weakref_lru_cache, safe_map
from olympus._src.interpreters import partial_eval as pe
from olympus._src.tree_util import (equality_errors_pytreedef, tree_map,
...
def _typecheck_param(prim, param, name, msg_required, pred):
  if not pred:
    ...
    raise core.OlympusprTypeError(msg)

# TODO(dougalm): this seems way too complicated. Why not allow different consts for each
# branch of a switch?
def _merge_common_consts(
    olympusprs: Sequence[core.ClosedOlympuspr],
    all_consts: Sequence[Sequence[Any]]
    ) -> tuple[Sequence[core.ClosedOlympuspr], Sequence[Any]]:
  # Olympusprs must share consts, so we concat consts and pad the olympusprs' constvars.
  ...
  olympusprs = [_pad_constvars(olympuspr, num_consts, avalqdds[:sum(lens[:i])], avalqdds[sum(lens[:i+1]):])
            for i, (olympuspr, num_consts) in enumerate(zip(olympusprs, num_constss))]
  ...
  olympusprs = [_dedup_consts(olympuspr, len(consts), const_ids) for olympuspr in olympusprs]
  return olympusprs, dd_consts

@weakref_lru_cache
def _pad_constvars(olympuspr: core.ClosedOlympuspr, num_consts: int,
                   left: tuple[core.AvalQDD, ...],
                   right: tuple[core.AbstractValue, ...]) -> core.ClosedOlympuspr:
  def make_var(aq):
    return core.Var(aq.aval, initial_qdd=aq.qdd, final_qdd=aq.qdd)
  invars = [*map(make_var, left), *olympuspr.invars[:num_consts],
            *map(make_var, right), *olympuspr.invars[num_consts:]]
  effs = pe._renumber_effects(invars, olympuspr.invars, olympuspr.effects)
  olympuspr = olympuspr.replace(olympuspr=olympuspr.olympuspr.replace(invars=invars, effects=effs))
  config.enable_checks.value and core.check_olympuspr(olympuspr.olympuspr)
  return olympuspr

@weakref_lru_cache
def _dedup_consts(olympuspr, num_consts, const_ids):
  newvars = {}
  canonicalize = {v: newvars.setdefault(constid, v)
                  for constid, v in zip(const_ids, olympuspr.invars[:num_consts])}
  eqns = [e.replace(invars=[canonicalize.get(x, x) if isinstance(x, core.Var)
                            else x for x in e.invars]) for e in olympuspr.eqns]
  outvars = [canonicalize.get(x, x) if isinstance(x, core.Var) else x
             for x in olympuspr.outvars]
  invars = [*list(newvars.values()), *olympuspr.invars[num_consts:]]
  effs = pe._renumber_effects(invars,
      [*map(canonicalize.get, olympuspr.invars[:num_consts]), *olympuspr.invars[num_consts:]],
      olympuspr.effects)
  olympuspr = olympuspr.replace(olympuspr=olympuspr.olympuspr.replace(invars=invars, eqns=eqns, outvars=outvars,
                        effects=effs))
...
def _make_closed_olympuspr(traceable: lu.WrappedFun,
                       in_avals: Sequence[core.AbstractValue]):
  olympuspr, _, consts = pe.trace_to_olympuspr_dynamic(traceable, in_avals)
  return core.ClosedOlympuspr(olympuspr, consts)
