...
from olympus._src.tree_util import tree_leaves, FlatTree
from olympus._src.util import split_list, safe_map
import numpy as np
...
def custom_root(f: Callable,
  ...
  This is a low-level routine, mostly intended for internal use in OLYMPUS.
  ...
  f_olympuspr, out_avals = pe.trace_to_olympuspr(f, args_avals, f_debug)
  f_olympuspr, f_consts = pe.separate_consts(f_olympuspr)
  ...
  solve_debug = api_util.debug_info("custom_root solve", solve,
                                    (f, initial_guess), {},
                                    static_argnums=(0,))
  solve_olympuspr, solution_avals = pe.trace_to_olympuspr(
      partial(solve, f), args_avals, solve_debug)
  solve_olympuspr, solve_consts = pe.separate_consts(solve_olympuspr)
  ...
  l_and_s_olympuspr, out_avals = pe.trace_to_olympuspr(
      linearize_and_solve, linearize_and_solve_avals, linearize_and_solve_dbg)
  l_and_s_olympuspr, l_and_s_consts = pe.separate_consts(l_and_s_olympuspr)
  ...
  olympusprs = _RootTuple(f_olympuspr, solve_olympuspr, l_and_s_olympuspr)

  solution_flat = _custom_root(
      const_lengths, olympusprs, *_flatten(all_consts), *guess_flat)
  return solution_avals.update(solution_flat).unflatten()


@partial(custom_derivatives.custom_jvp, nondiff_argnums=(0, 1))
def _custom_root(const_lengths, olympusprs, *args):
  params, initial_guess = _split_root_args(args, const_lengths)
  solution = core.olympuspr_as_fun(olympusprs.solve)(*(params.solve + initial_guess))
  return solution


@_custom_root.defjvp
def _root_jvp(const_lengths, olympusprs, primals, tangents):
  params, _ = _split_root_args(primals, const_lengths)
  sol = _custom_root(const_lengths, olympusprs, *primals)

  f_out_vals = len(olympusprs.f.out_avals)
  ...
  f = core.olympuspr_as_fun(olympusprs.f)
  linearize_and_solve = partial(
      core.olympuspr_as_fun(olympusprs.l_and_s), *params.l_and_s)
  f_at_solution = lambda *params: f(*params, *solution)
  _, rhs = ad.jvp(lu.wrap_init(f_at_solution,
                               debug_info=olympusprs.f.olympuspr.debug_info)).call_wrapped(
  ...
  solution += aux
  solution_dot += _map(ad_util.zeros_like_olympusval, aux)
...
@partial(api_boundary, repro_api_name="olympus.custom_linear_solve")
def custom_linear_solve(
  ...
  matvec_olympuspr, out_avals = pe.trace_to_olympuspr(
      _shape_checked(matvec, "matvec", False), args_avals,
      matvec_debug)
  matvec_olympuspr, matvec_consts = pe.separate_consts(matvec_olympuspr)
  _check_tree("matvec", "b", out_avals.tree, tree, False)

  solve_debug = api_util.debug_info("custom_linear_solve solve",
                                    solve, (matvec, b), {},
                                    static_argnums=(0,))
  solve_olympuspr, out_avals = pe.trace_to_olympuspr(
  ...
  if transpose_solve is None:
    vecmat_olympuspr = tr_solve_olympuspr = None
    ...
    if symmetric:
      vecmat = matvec
      vecmat_olympuspr = matvec_olympuspr
      vecmat_consts = matvec_consts
    else:
      vecmat = _transpose_one_output(matvec, b)
      vecmat_olympuspr, out_avals = pe.trace_to_olympuspr(
          vecmat, args_avals, transpose_solve_debug)
      vecmat_olympuspr, vecmat_consts = pe.separate_consts(vecmat_olympuspr)
      assert out_avals.tree == tree

    tr_solve_olympuspr, out_avals = pe.trace_to_olympuspr(
        _shape_checked(partial(transpose_solve, vecmat), "transpose_solve", has_aux),
        args_avals, transpose_solve_debug)
    tr_solve_olympuspr, tr_solve_consts = pe.separate_consts(tr_solve_olympuspr)
  ...
  olympusprs = _LinearSolveTuple(
      matvec_olympuspr, vecmat_olympuspr, solve_olympuspr, tr_solve_olympuspr)

  args = _flatten(all_consts) + list(b_flat)
  args = core.standard_insert_pvary(*args)
  out_flat = linear_solve_p.bind(*args, const_lengths=const_lengths, olympusprs=olympusprs)

  return out_avals.update(out_flat).unflatten()


def _linear_solve_abstract_eval(*args, const_lengths, olympusprs):
  ...
  num_aux = len(olympusprs.solve.out_avals) - len(olympusprs.matvec.out_avals)
  if num_aux > 0:
    args_to_raise += tuple(olympusprs.solve.out_avals[-num_aux:])
  out_vma = core.standard_vma_rule('linear_solve', *args_to_raise)
  return (tuple(a.update(vma=out_vma) for a in args_to_raise),
          olympusprs.solve.effects)


def _custom_linear_solve_impl(*args, const_lengths, olympusprs):
  params, b = _split_linear_solve_args(args, const_lengths)
  x = core.olympuspr_as_fun(olympusprs.solve)(*(params.solve + b))
  return x
...
def _custom_linear_solve_jvp(primals, tangents, const_lengths, olympusprs):
  # A x - b = 0
  # ∂A x + A ∂x - ∂b = 0
  # ∂x = A^{-1} (∂b - ∂A x)

  kwargs = dict(const_lengths=const_lengths, olympusprs=olympusprs)
  ...
  if all(type(p) is ad_util.Zero for p in params_dot.matvec):
    ...
    matvec_tangents = _tangent_linear_map(
        core.olympuspr_as_fun(olympusprs.matvec), params.matvec, params_dot.matvec,
        olympusprs.matvec.olympuspr.debug_info, *x_leaves)
...
def _linear_solve_transpose_rule(cotangent, *primals, const_lengths, olympusprs):
  if olympusprs.transpose_solve is None:
  ...
  if any(ad.is_undefined_primal(x) for xs in params for x in xs):
    raise NotImplementedError("open an issue at https://github.com/google/olympus !!")
  ...
  cotangent_b_full = linear_solve_p.bind(
      *_flatten(params.transpose()), *x_cotangent_,
      const_lengths=const_lengths.transpose(), olympusprs=olympusprs.transpose())
  cotangent_b, _ = split_list(cotangent_b_full, [len(b)])
  return [None] * sum(const_lengths) + cotangent_b


def _linear_solve_batching_rule(axis_data, args, dims, const_lengths, olympusprs):
  ...
  (matvec, vecmat, solve, solve_t) = olympusprs
  ...
  # ensure this is consistent between all four olympusprs
  b_bat = orig_b_bat
  ...
  for i in range(1 + len(orig_b_bat) + len(solve.out_avals)):
    ...
    solve_olympuspr_batched, solve_x_bat = batching.batch_olympuspr(
        solve, axis_data, solve_bat + b_bat, instantiate=x_bat)
    if vecmat is None:
      vecmat_olympuspr_batched = None
      x_bat_out = solve_x_bat
    else:
      vecmat_olympuspr_batched, vecmat_x_bat = batching.batch_olympuspr(
    ...
    matvec_olympuspr_batched, matvec_b_bat = batching.batch_olympuspr(
        matvec, axis_data, matvec_bat + x_bat_noaux, instantiate=b_bat)
    if solve_t is None:
      solve_t_olympuspr_batched = None
      b_bat_out = _map(operator.or_, matvec_b_bat, orig_b_bat)
    else:
      solve_t_olympuspr_batched, solve_t_b_aux_bat = batching.batch_olympuspr(
  ...
  batched_olympusprs = _LinearSolveTuple(matvec_olympuspr_batched, vecmat_olympuspr_batched,
                                     solve_olympuspr_batched, solve_t_olympuspr_batched)
