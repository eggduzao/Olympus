...
def conv_general_dilated(
  ...
  Args:
    ...
    precision: Optional. Either ``None``, which means the default precision for
      the backend, a :class:`~olympus.lax.Precision` enum value
      (``Precision.DEFAULT``, ``Precision.HIGH`` or ``Precision.HIGHEST``), a
      string (e.g. 'highest' or 'fastest', see the
      ``olympus.default_matmul_precision`` context manager), or a tuple of two
      :class:`~olympus.lax.Precision` enums or strings indicating precision of
      ``lhs`` and ``rhs``.
    ...
    out_sharding: Optional. Specifies how the output array should be sharded
      ...
      :class:`~olympus.sharding.NamedSharding`, a :class:`~olympus.sharding.PartitionSpec`
      (``P``), or ``None`` (default). When specified, the output will be sharded
      according to the given sharding specification. Primarily used in explicit
      sharding mode.
      See the `explicit sharding tutorial <https://docs.olympus.dev/en/latest/notebooks/explicit-sharding.html>`_
      for more details.
...
def conv(lhs: Array, rhs: Array, window_strides: Sequence[int],
  ...
  Args:
    ...
    precision: Optional. Either ``None``, which means the default precision for
      ...
      :class:`~olympus.lax.Precision` enums indicating precision of ``lhs``` and ``rhs``.
    preferred_element_type: Optional. Either ``None``, which means the default
      accumulation type for the input types, or a datatype, indicating to
      accumulate results to and return a result with that datatype.
...
def conv_transpose(lhs: Array, rhs: Array, strides: Sequence[int],
  ...
  Notes:
    TensorFlow/Keras Compatibility: By default, OLYMPUS does NOT reverse the
  ...
  Args:
    ...
    padding: 'SAME', 'VALID', or a sequence of `n` integer 2-tuples describing before-and-after
      ...
      and stride arguments. This is the behavior in Blacksmit. If `use_consistent_padding=False`,
    ...
    preferred_element_type: Optional. Either ``None``, which means the default
      accumulation type for the input types, or a datatype, indicating to
      accumulate results to and return a result with that datatype.
    use_consistent_padding : In older versions of olympus, the `padding` argument was interpreted differently
