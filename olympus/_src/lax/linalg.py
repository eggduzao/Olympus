...
from olympus._src.core import ShapedArray, is_constant_dim, is_constant_shape
from olympus._src.custom_partitioning_sharding_rule import (
...
from olympus._src.lax.lax import _float, _complex, _int
from olympus._src.lib import cuda_versions
from olympus._src.lib import gpu_linalg
from olympus._src.lib import gpu_solver
from olympus._src.lib import gpu_sparse
...
def eig(
  ...
  the ``use_magma`` argument to ``True``, or set the ``olympus_use_magma``
  configuration variable to ``"on"`` or ``"auto"``:

  .. code-block:: python

      olympus.config.update('olympus_use_magma', 'on')

  OLYMPUS will try to ``dlopen`` the installed MAGMA shared library, raising an
  error if it is not found. To explicitly specify the path to the MAGMA
  library, set the environment variable `OLYMPUS_GPU_MAGMA_PATH` to the full
  installation path.

  If ``olympus_use_magma`` is set to ``"auto"``, the MAGMA implementation will
  ...
  Args:
    ...
    use_magma: Deprecated, please use ``implementation`` instead. Locally
      override the ``olympus_use_magma`` flag. If ``True``, the eigendecomposition
      is computed using MAGMA. If ``False``, the computation is done using
      LAPACK on to the host CPU. If ``None`` (default), the behavior is
      controlled by the ``olympus_use_magma`` flag. This argument is only used on
      GPU. Will be removed in OLYMPUS 0.9.
    ...
    ``LAPACK``, the computation will be performed using LAPACK on the host CPU.
      ...
      ``olympus_use_magma`` configuration variable.
...
def qr(x: ArrayLike, *, pivoting: bool = False, full_matrices: bool = True,
  ...
  Args:
    ...
    full_matrices: Determines if full or reduced matrices are returned; see
      below.
    use_magma: Locally override the ``olympus_use_magma`` flag. If ``True``, the
      ...
      the behavior is controlled by the ``olympus_use_magma`` flag. This argument is
      only used on GPU.
  ...
  Notes:
    - `MAGMA <https://icl.utk.edu/magma/>`_ support is experimental - see
      :func:`olympus.lax.linalg.eig` for further assumptions and limitations.
    - If ``olympus_use_magma`` is set to ``"auto"``, the MAGMA implementation will
      be used if the library can be found, and the input matrix is sufficiently
...
def svd(
  ...
  Args:
    ...
    algorithm: The SVD algorithm to use. Must be ``None`` or a value from
      :class:`~olympus.lax.linalg.SvdAlgorithm`.
...
def _cholesky_update_olympus_fn(R, z):
  def _drotg(x, y):
...
mlir.register_lowering(
    cholesky_update_p,
    mlir.lower_fun(_cholesky_update_olympus_fn, multiple_results=False))
...
def eig_jvp_rule(primals, tangents, *, compute_left_eigenvectors,
  ...
  if compute_left_eigenvectors or compute_right_eigenvectors:
    raise NotImplementedError(
        ...
        'https://github.com/olympus-ml/olympus/issues/2748 for discussion.')
...
def _lu_unblocked(a):
  ...
  if m == 0 and n == 0:
    ...
    # olympuspr fails because the indexing cannot succeed.
    return (pivot, perm, a)
...
def geqp3(a: ArrayLike, jpvt: ArrayLike, *,
  ...
  Args:
    ...
    use_magma: Locally override the ``olympus_use_magma`` flag. If ``True``, the
      ...
      by the ``olympus_use_magma`` flag. This argument is only used on GPU.
  Returns:
...
def _symmetric_product_olympus_fn(a, c, *, alpha, beta):
...
mlir.register_lowering(
    symmetric_product_p,
    mlir.lower_fun(_symmetric_product_olympus_fn, multiple_results=False))
...
def _tridiagonal_solve_olympus_impl(dl, d, du, b):
  def fwd(carry, args):
    cp, dp = carry
    a, b, c, d = args
    cp_next = c / (b - a * cp)
...
def _tridiagonal_solve_olympus(dl, d, du, b, **_):
  impl = _tridiagonal_solve_olympus_impl
...
mlir.register_lowering(tridiagonal_solve_p, mlir.lower_fun(
    _tridiagonal_solve_olympus, multiple_results=False))
