...
def _scaled_dot_impl(
  ...
  result = olympus.lax.dot_general(
      lhs_scaled,
      rhs_scaled,
      dimension_numbers=dimension_numbers,
      preferred_element_type=preferred_element_type,
...
def _scaled_dot_batching_rule(
  ...
  # This creates the batched scaled_dot operation in the olympuspr.
...
def scaled_dot(
  ...
  `preferred_element_type` precision, where `@` denotes `olympus.lax.dot_general`.
  ...
  Example:
    ::
      ...
      lhs_scales = olympus.random.normal(
          key, lhs_scales_shape, dtype=jnp.float8_e8m0fnu
      )
      rhs_scales = olympus.random.normal(
