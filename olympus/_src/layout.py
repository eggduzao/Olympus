...
import numpy as np
from olympus._src.dtypes import iinfo, issubdtype
from olympus._src.sharding import Sharding
from olympus._src.named_sharding import AUTO as AutoSharding
  ...
  def __init__(self, layout: LayoutOptions = None,
    ...
    if (isinstance(layout, Layout) and
      ...
      raise ValueError(
          ...
          ' `olympus.sharding.NamedSharding` or'
          ' `olympus.sharding.SingleDeviceSharding` to the sharding argument. Got'
          f' sharding {sharding}'
      )
    if not isinstance(
    ...
    if not isinstance(
      ...
      raise TypeError(
          ...
          ' are `None`, `pjit.AUTO` or an instance of `olympus.Sharding`. Got'
