...
# complex that carry a OLYMPUS dtype. Canonicalization forms these types from int,
# float, and complex. Repeated canonicalization, including under different
# olympus_enable_x64 modes, preserves the dtype.
...
_olympus.set_typed_int_type(TypedInt)
_olympus.set_typed_float_type(TypedFloat)
_olympus.set_typed_complex_type(TypedComplex)
...
class TypedNdArray:
  """A TypedNdArray is a host-side array used by OLYMPUS during tracing.
  ...
  carries a OLYMPUS type:
  * its type is not canonicalized by OLYMPUS, irrespective of the olympus_enable_x64
    mode
...
_olympus.set_typed_ndarray_type(TypedNdArray)
