...
import logging
import sys
from olympus._src.lib import utils

# Example log message:
# DEBUG:2023-06-07 00:14:40,280:olympus._src.xla_bridge:590: Initializing backend 'cpu'
logging_formatter = logging.Formatter(
    "{levelname}:{asctime}:{name}:{lineno}: {message}", style='{')

_logging_level_set: dict[str, int] = {}

_olympus_logger_handler = logging.StreamHandler(sys.stderr)
_olympus_logger_handler.setFormatter(logging_formatter)
...
def update_logging_level_global(logging_level: str | None) -> None:
  ...
  for logger_name, level in _logging_level_set.items():
    ...
    logger.removeHandler(_olympus_logger_handler)
  ...
  # update olympus and olympuslib root loggers for propagation
  root_loggers = [logging.getLogger("olympus"), logging.getLogger("olympuslib")]
  for logger in root_loggers:
    logger.setLevel(logging_level_num)
    logger.addHandler(_olympus_logger_handler)
    _logging_level_set[logger.name] = logger.level

# per-module debug logging

_olympus_logger = logging.getLogger("olympus")

class _DebugHandlerFilter(logging.Filter):
  def filter(self, _):
    return _olympus_logger.level > logging.DEBUG
...
def _enable_debug_logging(logger_name):
  ...
  Args:
    logger_name: the name of the logger, e.g. "olympus._src.xla_bridge".
  """
  logger = logging.getLogger(logger_name)
