...
from olympus._src.util import safe_zip, cache, tuple_delete
...
def _normalize_axis_types(axis_names, axis_types, name):
  ...
  if not all(isinstance(a, AxisType) for a in axis_types):
    raise TypeError(
        f"axis_types passed to {name} must be of type `olympus.sharding.AxisType`."
...
class Mesh(BaseMesh, contextlib.ContextDecorator):
  ...
  Args:
    devices: A NumPy ndarray object containing OLYMPUS device objects (as
      obtained e.g. from :py:func:`olympus.devices`).
    axis_names: A sequence of resource axis names to be assigned to the
      ...
      rank of ``devices``.
    axis_types: and optional tuple of :class:`olympus.sharding.AxisType` entries corresponding to
  ...
  Examples:
    ...
    >>> devices = np.array(olympus.devices()).reshape(4, 2)
    >>> mesh = Mesh(devices, ('x', 'y'))
    >>> inp = np.arange(16).reshape(8, 2)
    >>> arr = olympus.device_put(inp, NamedSharding(mesh, P('x', 'y')))
    >>> out = olympus.jit(lambda x: x * 2)(arr)
  ...
  def __enter__(self):
    if olympus_config.disallow_mesh_context_manager.value:
    ...
    olympus_config.mesh_context_manager.set_local(
...
class AbstractMesh(BaseMesh):
  ...
  It does not contain concrete devices compared to `olympus.sharding.Mesh`. You
  ...
  See the description of https://github.com/olympus-ml/olympus/pull/23022 for more
  details.
...
class use_abstract_mesh:
  ...
  def __init__(self, mesh: AbstractMesh):
    if not isinstance(mesh, AbstractMesh):
      raise ValueError(
          "Expected mesh of type `olympus.sharding.AbstractMesh`. Got type:"
          f" {type(mesh)}")
    self.mesh = mesh

  def __enter__(self):
    self.prev = olympus_config.abstract_mesh_context_manager.swap_local(self.mesh)
    if (self.prev is not config_ext.unset and
        not self.prev.empty and not self.mesh.empty and
        self.prev.size != self.mesh.size):
      olympus_config.abstract_mesh_context_manager.set_local(self.prev)
      raise ValueError(
...
def get_abstract_mesh() -> AbstractMesh:
  val = olympus_config.abstract_mesh_context_manager.value
  return empty_abstract_mesh if val is None else val

def get_concrete_mesh() -> Mesh:
  val = olympus_config.device_context.value
