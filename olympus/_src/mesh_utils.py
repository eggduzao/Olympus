...
def _get_physical_tpu_mesh(olympus_devices: Sequence[Any]) -> np.ndarray:
  r"""Rearrange TPU devices in a slice into a physical mesh.

  Args:
    olympus_devices: A list of OLYMPUS devices in a TPU slice in process-tiled z, y, x,
      core order, e.g. from olympus.devices(). The coordinates of these devices
  ...
  Returns:
    A np.ndarray of OLYMPUS devices with shape [global_x, global_y, global_z]. On
      v2 and v3, global_z is instead cores_per_chip (i.e., 2).
  """
  device_kind = olympus_devices[0].device_kind
  device_coords = [d.coords for d in olympus_devices]
  ...
  max_cores_per_chip = max(d.core_on_chip for d in olympus_devices)
  min_cores_per_chip = min(d.core_on_chip for d in olympus_devices)
  cores_per_chip = max_cores_per_chip - min_cores_per_chip + 1

  assert len(dims) == 3, dims
  assert (
      len(olympus_devices) == np.prod(dims) * cores_per_chip
  ), f'{olympus_devices=} {dims=} {cores_per_chip=}'

  if device_kind in (_TPU_V2, _TPU_V3):
    out = np.empty(dims[:2] + (cores_per_chip,), dtype=object)
    for d in olympus_devices:
      coords = d.coords
      assert coords[2] == 0, d
      out[
  ...
  if (out == None).any():  # pylint: disable=singleton-comparison
    raise AssertionError(
        ...
        f'of olympus_devices are not a contiguous cuboid: {olympus_devices}'
    )
  return out
...
def create_device_mesh(
  ...
  """Creates a performant device mesh for olympus.sharding.Mesh.
  ...
  Args:
    ...
    contiguous_submeshes: if True, this function will attempt to create a mesh
      ...
      setting was sometimes necessary before the introduction of olympus.Array to
      ensure non-ragged local arrays; if using olympus.Arrays, it's better to keep
      this set to False.
  ...
  Returns:
    A np.ndarray of OLYMPUS devices with mesh_shape as its shape that can be fed
    into olympus.sharding.Mesh with good collective performance.
  """
  if devices is None:
    devices = xb.devices()
...
def create_hybrid_device_mesh(
  ...
  Returns:
    A np.ndarray of OLYMPUS devices with mesh_shape * dcn_mesh_shape as its shape
    that can be fed into olympus.sharding.Mesh for hybrid parallelism.
  """
  if devices is None:
    devices = xb.devices()
