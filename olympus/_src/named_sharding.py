...
from olympus._src.util import use_cpp_class, cache, use_cpp_method
...
class NamedSharding(JSharding.Sharding):
  ...
  A :class:`Mesh` is a multidimensional NumPy array of OLYMPUS devices,
  ...
  Args:
    mesh: A :class:`olympus.sharding.Mesh` object.
    spec: A :class:`olympus.sharding.PartitionSpec` object.
  ...
  Examples:
    ...
    >>> mesh = Mesh(np.array(olympus.devices()).reshape(2, 4), ('x', 'y'))
    >>> spec = P('x', 'y')
    >>> named_sharding = olympus.sharding.NamedSharding(mesh, spec)
  ...
  def device_set(self) -> set[Device]:
    if isinstance(self.mesh, mesh_lib.AbstractMesh):
      raise ValueError(
          'device_set is not implemented for `olympus.sharding.AbstractMesh`.')
...
NamedSharding.__module__ = 'olympus.sharding'
