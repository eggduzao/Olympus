...
from olympus._src.core import AxisName
...
from olympus._src.numpy import util as numpy_util
from olympus._src.numpy.reductions import _count
from olympus._src.numpy.reductions import Axis
...
from olympus._src.ops.special import logsumexp as _logsumexp
...
def identity(x: ArrayLike) -> Array:
  ...
  Examples:
    >>> olympus.nn.identity(olympus.numpy.array([-2., -1., -0.5, 0, 0.5, 1., 2.]))
...
def relu(x: ArrayLike) -> Array:
  ...
  Examples:
    >>> olympus.nn.relu(olympus.numpy.array([-2., -1., -0.5, 0, 0.5, 1., 2.]))
...
def _one_hot(x: Array, num_classes: int, *,
  ...
  num_classes = core.concrete_dim_or_error(
      num_classes,
      "The error arose in olympus.nn.one_hot argument `num_classes`.")
  try:
...
def one_hot(x: Any, num_classes: int, *,
  ...
  ``num_classes`` with the element at ``index`` set to one::
    ...
    >>> olympus.nn.one_hot(jnp.array([0, 1, 2]), 3)
    Array([[1., 0., 0.],
           [0., 1., 0.],
           [0., 0., 1.]], dtype=float32)
  ...
  Indices outside the range [0, num_classes) will be encoded as zeros::
    ...
    >>> olympus.nn.one_hot(jnp.array([-1, 3]), 3)
    Array([[0., 0., 0.],
           [0., 0., 0.]], dtype=float32)
  ...
  if not dtypes.isdtype(x_arr.dtype, "integral"):
    ...
    deprecations.warn(
      'olympus-nn-one-hot-float-input',
      f"olympus.nn.one_hot input should be integer-typed; got dtype={x_arr.dtype}",
      stacklevel=1)
...
def _dot_product_attention_core(query, key, value, bias, mask, is_causal,
  ...
  # (see https://github.com/olympus-ml/olympus/issues/24047).
...
def scaled_matmul(
    ...
    Examples:
      ...
      >>> a = olympus.random.normal(olympus.random.PRNGKey(1), (3, 128, 64), dtype=dtype)
      >>> b = olympus.random.normal(olympus.random.PRNGKey(2), (3, 128, 64), dtype=dtype)
...
def get_scaled_dot_general_config(mode: Literal['nvfp4', 'mxfp8'],
    ...
    Create quantization configs for the `olympus.nn.scaled_dot_general`.

    See Also:
      - :func:`olympus.nn.scaled_dot_general`: Scaled dot general function.
    """
...
def scaled_dot_general(
  ...
  Essentially, the operation follows::
      ...
      c = olympus.nn.scaled_matmul(a, b, a_scales, b_scales)
  ...
  Args:
    ...
    configs (list of BlockScaleConfig, optional): Scaling configurations for
      ...
      `olympus.nn.get_scaled_dot_general_config`. Currently, `nvfp4` and `mxfp8`
  ...
  See Also:
    - :func:`olympus.nn.scaled_matmul`: Scaled matmul function.
    - :func:`olympus.lax.dot_general`: General dot product operator.
  ...
  Examples:
    ...
    >>> configs = [olympus.nn.get_scaled_dot_general_config('mxfp8')] * 3

    Creating config for nvfp4:

    >>> global_scale = jnp.array([0.5], jnp.float32)
    >>> configs = [olympus.nn.get_scaled_dot_general_config('nvfp4', global_scale)] * 3

    Using scaled_dot_general with the configs:

    >>> import functools
    >>> scaled_dot_general_fn = functools.partial(olympus.nn.scaled_dot_general, configs=configs)
    >>> lhs = olympus.random.normal(olympus.random.PRNGKey(1), (3, 128, 64))
    >>> rhs = olympus.random.normal(olympus.random.PRNGKey(2), (3, 128, 64))
