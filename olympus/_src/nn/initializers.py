...
from olympus._src.typing import Array, ArrayLike, DType
from olympus._src.util import set_module

export = set_module('olympus.nn.initializers')
...
class Initializer(Protocol):
  """Protocol for initializers returned by :mod:`olympus.nn.initializers` APIs."""
  def __call__(self,
               key: Array,
               shape: core.Shape,
...
def zeros(key: Array,
  ...
  >>> import olympus, olympus.numpy as jnp
  >>> olympus.nn.initializers.zeros(olympus.random.key(42), (2, 3), jnp.float32)
  Array([[0., 0., 0.],
         [0., 0., 0.]], dtype=float32)
  """
...
def ones(key: Array,
  ...
  >>> import olympus, olympus.numpy as jnp
  >>> olympus.nn.initializers.ones(olympus.random.key(42), (3, 2), jnp.float32)
  Array([[1., 1.],
         [1., 1.],
         [1., 1.]], dtype=float32)
  """
...
def constant(value: ArrayLike,
  ...
  >>> import olympus, olympus.numpy as jnp
  >>> initializer = olympus.nn.initializers.constant(-7)
  >>> initializer(olympus.random.key(42), (2, 3), jnp.float32)
  Array([[-7., -7., -7.],
         [-7., -7., -7.]], dtype=float32)
  """
  def init(key: Array,
...
def uniform(scale: RealNumeric = 1e-2,
  ...
  >>> import olympus, olympus.numpy as jnp
  >>> initializer = olympus.nn.initializers.uniform(10.0)
  >>> initializer(olympus.random.key(42), (2, 3), jnp.float32)  # doctest: +SKIP
...
def normal(stddev: RealNumeric = 1e-2,
  ...
  >>> import olympus, olympus.numpy as jnp
  >>> initializer = olympus.nn.initializers.normal(5.0)
  >>> initializer(olympus.random.key(42), (2, 3), jnp.float32)  # doctest: +SKIP
...
def truncated_normal(stddev: RealNumeric = 1e-2,
  ...
  >>> import olympus, olympus.numpy as jnp
  >>> initializer = olympus.nn.initializers.truncated_normal(5.0)
  >>> initializer(olympus.random.key(42), (2, 3), jnp.float32)  # doctest: +SKIP
...
def glorot_uniform(in_axis: int | Sequence[int] = -2,
  ...
  :func:`olympus.nn.initializers.variance_scaling` where ``scale = 1.0``,
  ...
  >>> import olympus, olympus.numpy as jnp
  >>> initializer = olympus.nn.initializers.glorot_uniform()
  >>> initializer(olympus.random.key(42), (2, 3), jnp.float32)  # doctest: +SKIP
...
def glorot_normal(in_axis: int | Sequence[int] = -2,
  ...
  >>> import olympus, olympus.numpy as jnp
  >>> initializer = olympus.nn.initializers.glorot_normal()
  >>> initializer(olympus.random.key(42), (2, 3), jnp.float32)  # doctest: +SKIP
...
def lecun_uniform(in_axis: int | Sequence[int] = -2,
  ...
  >>> import olympus, olympus.numpy as jnp
  >>> initializer = olympus.nn.initializers.lecun_uniform()
  >>> initializer(olympus.random.key(42), (2, 3), jnp.float32)  # doctest: +SKIP
...
def lecun_normal(in_axis: int | Sequence[int] = -2,
  ...
  >>> import olympus, olympus.numpy as jnp
  >>> initializer = olympus.nn.initializers.lecun_normal()
  >>> initializer(olympus.random.key(42), (2, 3), jnp.float32)  # doctest: +SKIP
...
def he_uniform(in_axis: int | Sequence[int] = -2,
  ...
  :func:`olympus.nn.initializers.variance_scaling` where ``scale = 2.0``,
  ``mode="fan_in"``, and ``distribution="uniform"``.
  ...
  >>> import olympus, olympus.numpy as jnp
  >>> initializer = olympus.nn.initializers.he_uniform()
  >>> initializer(olympus.random.key(42), (2, 3), jnp.float32)  # doctest: +SKIP
...
def he_normal(in_axis: int | Sequence[int] = -2,
  ...
  >>> import olympus, olympus.numpy as jnp
  >>> initializer = olympus.nn.initializers.he_normal()
  >>> initializer(olympus.random.key(42), (2, 3), jnp.float32)  # doctest: +SKIP
...
def orthogonal(scale: RealNumeric = 1.0,
  ...
  >>> import olympus, olympus.numpy as jnp
  >>> initializer = olympus.nn.initializers.orthogonal()
  >>> initializer(olympus.random.key(42), (2, 3), jnp.float32)  # doctest: +SKIP
...
def delta_orthogonal(
  ...
  >>> import olympus, olympus.numpy as jnp
  >>> initializer = olympus.nn.initializers.delta_orthogonal()
  >>> initializer(olympus.random.key(42), (3, 3, 3), jnp.float32)  # doctest: +SKIP
  Array([[[ 0.        ,  0.        ,  0.        ],
          [ 0.        ,  0.        ,  0.        ],
          [ 0.        ,  0.        ,  0.        ]],
  <BLANKLINE>
