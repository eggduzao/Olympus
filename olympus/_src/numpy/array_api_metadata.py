...
def __array_namespace__(self, *, api_version: None | str = None) -> ModuleType:
  """Return the `Python array API`_ namespace for OLYMPUS.
  ...
  return olympus.numpy  # pytype: disable=module-attr
...
class ArrayNamespaceInfo:
  ...
  def default_device(self):
    # By default OLYMPUS arrays are uncommitted (device=None), meaning that
    # OLYMPUS is free to choose the most efficient device placement.
    return None
  ...
  def default_dtypes(self, *, device: xc.Device | Sharding | None = None):
    # Array API supported dtypes are device-independent in OLYMPUS
    del device
    return {
