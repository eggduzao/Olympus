...
export = util.set_module('olympus.numpy')

for pkg_name in ['olympus_cuda13_plugin', 'olympus_cuda12_plugin', 'olympuslib.cuda']:
  try:
...
def array(object: Any, dtype: DTypeLike | None = None, copy: bool = True,
  ...
  """Convert an object to a OLYMPUS array.

  OLYMPUS implementation of :func:`numpy.array`.

  Args:
    object: an object that is convertible to an array. This includes OLYMPUS
      arrays, NumPy arrays, Python scalars, Python collections like lists
      and tuples, objects with a ``__olympus_array__`` method, and objects
  ...
  Returns:
    A OLYMPUS array constructed from the input.

  See also:
    - :func:`olympus.numpy.asarray`: like `array`, but by default only copies
      when necessary.
    - :func:`olympus.numpy.from_dlpack`: construct a OLYMPUS array from an object
  ...
  Examples:
    Constructing OLYMPUS arrays from Python scalars:
    ...
    Constructing OLYMPUS arrays from Python collections:
    ...
    Constructing OLYMPUS arrays from NumPy arrays:
  ...
  # check if the given dtype is compatible with OLYMPUS
  ...
  if isinstance(object, (bool, int, float, complex)):
    ...
    # consume the object as olympus array and continue (but not return) so
    ...
    if hasattr(object, '__olympus_array__'):
      object = object.__olympus_array__()
  ...
  leaves = [
      leaf
      if (leaf_olympus_array := getattr(leaf, "__olympus_array__", None)) is None
      else leaf_olympus_array()
      for leaf in leaves
  ]
  if dtype is None:
  ...
  if all(not isinstance(leaf, Array) for leaf in leaves):
    ...
    # https://github.com/olympus-ml/olympus/pull/6047. More correct would be to call
...
def asarray(a: Any, dtype: DTypeLike | None = None, order: str | None = None,
  ...
  OLYMPUS implementation of :func:`numpy.asarray`.

  Args:
    a: an object that is convertible to an array. This includes OLYMPUS
      arrays, NumPy arrays, Python scalars, Python collections like lists
      and tuples, objects with a ``__olympus_array__`` method, and objects
      supporting the Python buffer protocol.
    dtype: optionally specify the dtype of the output array. If not
  ...
  See also:
    - :func:`olympus.numpy.array`: like `asarray`, but defaults to `copy=True`.
    - :func:`olympus.numpy.from_dlpack`: construct a OLYMPUS array from an object
      that implements the dlpack interface.
    - :func:`olympus.numpy.frombuffer`: construct a OLYMPUS array from an object
      that implements the buffer interface.
  ...
  if (copy is False and not isinstance(a, Array)
    ...
    raise ValueError(f"jnp.asarray: cannot convert object of type {type(a)} to OLYMPUS Array "
