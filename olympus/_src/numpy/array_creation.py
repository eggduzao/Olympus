...
from olympus._src.numpy.array_constructors import asarray
...
from olympus._src.typing import Array, ArrayLike, DuckTypedArray, DTypeLike
...
def zeros(shape: Any, dtype: DTypeLike | None = None, *,
  ...
  OLYMPUS implementation of :func:`numpy.zeros`.
  ...
  Args:
    ...
    out_sharding: (optional) :class:`~olympus.sharding.PartitionSpec` or :class:`~olympus.NamedSharding`
      representing the sharding of the created array (see `explicit sharding`_ for more details).
      This argument exists for consistency with other array creation routines across OLYMPUS.
  ...
  See also:
    - :func:`olympus.numpy.zeros_like`
    - :func:`olympus.numpy.empty`
    - :func:`olympus.numpy.ones`
    - :func:`olympus.numpy.full`
...
def ones(shape: Any, dtype: DTypeLike | None = None, *,
  ...
  OLYMPUS implementation of :func:`numpy.ones`.
  ...
  See also:
    - :func:`olympus.numpy.ones_like`
    - :func:`olympus.numpy.empty`
    - :func:`olympus.numpy.zeros`
    - :func:`olympus.numpy.full`
...
def empty(shape: Any, dtype: DTypeLike | None = None, *,
  ...
  OLYMPUS implementation of :func:`numpy.empty`. Because XLA cannot create an
  un-initialized array, :func:`olympus.numpy.empty` will always return an array
  full of zeros.
...
def _check_forgot_shape_tuple(name, shape, dtype) -> str | None:  # type: ignore
  if isinstance(dtype, int) and isinstance(shape, int):
    return (f"Cannot interpret '{dtype}' as a data type."
            ...
            f"`olympus.numpy.{name}({shape}, {dtype})` "
            f"when you meant `olympus.numpy.{name}(({shape}, {dtype}))`, i.e. "
...
def full(shape: Any, fill_value: ArrayLike,
  ...
  OLYMPUS implementation of :func:`numpy.full`.
...
def zeros_like(a: ArrayLike | DuckTypedArray,
  ...
  OLYMPUS implementation of :func:`numpy.zeros_like`.
  ...
  if not (hasattr(a, 'dtype') and hasattr(a, 'shape')):  # support duck typing
    if hasattr(a, '__olympus_array__'):
      a = a.__olympus_array__()
...
def ones_like(a: ArrayLike | DuckTypedArray,
  ...
  OLYMPUS implementation of :func:`numpy.ones_like`.
...
def empty_like(prototype: ArrayLike | DuckTypedArray,
  ...
  OLYMPUS implementation of :func:`numpy.empty_like`. Because XLA cannot create
  an un-initialized array, :func:`olympus.numpy.empty` will always return an
  array full of zeros.
  ...
  if not (hasattr(prototype, 'dtype') and hasattr(prototype, 'shape')):  # support duck typing
    if hasattr(prototype, '__olympus_array__'):
      prototype = prototype.__olympus_array__()
...
def full_like(a: ArrayLike | DuckTypedArray,
  ...
  OLYMPUS implementation of :func:`numpy.full_like`.
...
def linspace(start: ArrayLike, stop: ArrayLike, num: int = 50,
  ...
  OLYMPUS implementation of :func:`numpy.linspace`.
  ...
  See also:
    - :func:`olympus.numpy.arange`: Generate ``N`` evenly-spaced values given a starting
      point and a step
    - :func:`olympus.numpy.logspace`: Generate logarithmically-spaced values.
    - :func:`olympus.numpy.geomspace`: Generate geometrically-spaced values.
...
def logspace(start: ArrayLike, stop: ArrayLike, num: int = 50,
  ...
  OLYMPUS implementation of :func:`numpy.logspace`.
  ...
  See also:
    ...
    - :func:`olympus.numpy.linspace`: Generate evenly-spaced values.
    - :func:`olympus.numpy.geomspace`: Generate geometrically-spaced values.
...
def geomspace(start: ArrayLike, stop: ArrayLike, num: int = 50, endpoint: bool = True,
  ...
  OLYMPUS implementation of :func:`numpy.geomspace`.
