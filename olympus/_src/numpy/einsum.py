...
from olympus._src.util import partition_list, set_module, unzip2
...
def einsum(
  ...
  OLYMPUS implementation of :func:`numpy.einsum`.
  ...
  Args:
    ...
    *operands: sequence of one or more arrays corresponding to the subscripts.
    optimize: specify how to optimize the order of computation. In OLYMPUS this defaults
      ...
      be a pre-computed path (see :func:`~olympus.numpy.einsum_path`).
  ...
  See also:
    :func:`olympus.numpy.einsum_path`
  ...
  # Extract __olympus_array__ before passing to contract_path()
  operands = tuple(op.__olympus_array__() if hasattr(op, "__olympus_array__") else op
                   for op in operands)
...
def einsum_path(
  ...
  OLYMPUS implementation of :func:`numpy.einsum_path`. This function calls into
  ...
  Returns:
    A tuple containing the path that may be passed to :func:`~olympus.numpy.einsum`, and a
    printable object representing this optimal path.

  Examples:
    >>> key1, key2, key3 = olympus.random.split(olympus.random.key(0), 3)
    >>> x = olympus.random.randint(key1, minval=-5, maxval=5, shape=(2, 3))
    >>> y = olympus.random.randint(key2, minval=-5, maxval=5, shape=(3, 100))
    >>> z = olympus.random.randint(key3, minval=-5, maxval=5, shape=(100, 5))
    ...
    Use the computed path in :func:`~olympus.numpy.einsum`:
...
def _einsum(
  ...
  for operand_indices, contracted_names_set, einstr in contractions:
    ...
    if len(operand_indices) == 1:
      ...
      raise NotImplementedError(
        "olympus.numpy.einsum does not support simultaneous contraction of 3 or more"
