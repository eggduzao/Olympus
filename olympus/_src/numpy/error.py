...
from olympus._src import error_check as error_check_lib
...
def _set_error_if_with_category(
  ...
  This function is intended for use in OLYMPUS internal APIs (e.g., `olympus.numpy`)
...
def _set_error_if_nan(pred: Array, /):
  ...
  This function is disabled if the `olympus_error_checking_behavior_nan` flag is
  set to "ignore".
  """
...
def _set_error_if_divide_by_zero(pred: Array, /):
  ...
  This function is disabled if the `olympus_error_checking_behavior_divide` flag is
  set to "ignore".
  """
