...
from olympus._src.numpy.array_constructors import array
from olympus._src.numpy.util import promote_dtypes
from olympus._src.numpy.array_creation import linspace
...
class _Mgrid:
  ...
  functionality provided by :func:`olympus.numpy.meshgrid` with ``sparse=False``.

  See Also:
    jnp.ogrid: open/sparse version of jnp.mgrid

  Examples:
    Pass ``[start:stop:step]`` to generate values similar to :func:`olympus.numpy.arange`:

    >>> jnp.mgrid[0:4:1]
    Array([0, 1, 2, 3], dtype=int32)

    Passing an imaginary step generates values similar to :func:`olympus.numpy.linspace`:
...
class _Ogrid:
  ...
  functionality provided by :func:`olympus.numpy.meshgrid` with ``sparse=True``.
