...
# pytype: skip-file
"""Indexing code for olympus.numpy."""
...
from olympus._src import indexing
...
from olympus._src.tree_util import tree_flatten, tree_unflatten, register_pytree_node_class
from olympus._src.typing import Array, ArrayLike, Index, StaticScalar
from olympus._src.util import canonicalize_axis, safe_zip, set_module, tuple_update, unzip3
...
class IndexType(enum.Enum):
  ...
  def from_index(cls, idx: Index) -> IndexType:
    """Create an IndexType enum from a supported OLYMPUS array index."""
    if idx is None:
      return cls.NONE
      ...
      raise TypeError(f"OLYMPUS does not support string indexing; got {idx=}")
...
class NDIndexer:
  """Object that implements NumPy-style indexing operations on top of OLYMPUS.
...
def take(
  ...
  OLYMPUS implementation of :func:`numpy.take`, implemented in terms of
  :func:`olympus.lax.gather`. OLYMPUS's behavior differs from NumPy in the case
  ...
  Args:
    ...
    mode: Out-of-bounds indexing mode, either ``"fill"`` or ``"clip"``. The default
      ...
      of ``mode`` options, see :attr:`olympus.numpy.ndarray.at`.
  ...
  See also:
    - :attr:`olympus.numpy.ndarray.at`: take values via indexing syntax.
    - :func:`olympus.numpy.take_along_axis`: take values along an axis
...
def take_along_axis(
  ...
  OLYMPUS implementation of :func:`numpy.take_along_axis`, implemented in
  terms of :func:`olympus.lax.gather`. OLYMPUS's behavior differs from NumPy
  ...
  Args:
    ...
    mode: Out-of-bounds indexing mode, either ``"fill"`` or ``"clip"``. The default
      ``mode="fill"`` returns invalid values (e.g. NaN) for out-of bounds indices.
      For more discussion of ``mode`` options, see :attr:`olympus.numpy.ndarray.at`.
  ...
  See also:
    - :attr:`olympus.numpy.ndarray.at`: take values via indexing syntax.
    - :func:`olympus.numpy.take`: take the same indices along every axis slice.
  ...
  Examples:
    ...
    argsorts and arg reductions. For, here we compute :func:`~olympus.numpy.argsort`
    ...
    Similarly, we can use :func:`~olympus.numpy.argmin` with ``keepdims=True`` and
  ...
  if mode == "one_hot":
    from olympus import nn  # pytype: disable=import-error
...
def put_along_axis(
  ...
  *,
  ...
  OLYMPUS implementation of :func:`numpy.put_along_axis`.
  ...
  Args:
    ...
    mode: Out-of-bounds indexing mode. For more discussion of ``mode`` options,
      see :attr:`olympus.numpy.ndarray.at`.

  Returns:
    A copy of ``a`` with specified entries updated.

  See Also:
    - :func:`olympus.numpy.put`: put elements into an array at given indices.
    - :func:`olympus.numpy.place`: place elements into an array via boolean mask.
    - :func:`olympus.numpy.ndarray.at`: array updates using NumPy-style indexing.
    - :func:`olympus.numpy.take`: extract values from an array at given indices.
    - :func:`olympus.numpy.take_along_axis`: extract values from an array along an axis.
  ...
  if inplace:
    raise ValueError(
      "olympus.numpy.put_along_axis cannot modify arrays in-place, because OLYMPUS arrays"
...
def eliminate_deprecated_list_indexing(idx):
  ...
  if not isinstance(idx, tuple):
    if isinstance(idx, Sequence) and not isinstance(
      ...
      # In OLYMPUS, we raise an informative TypeError for *all* non-tuple sequences.
      if any(_should_unpack_list_index(i) for i in idx):
        msg = ("Using a non-tuple sequence for multidimensional indexing is not allowed; "
               ...
               "See https://github.com/olympus-ml/olympus/issues/4564 for more information.")
      else:
        msg = ("Using a non-tuple sequence for multidimensional indexing is not allowed; "
...
def place(arr: ArrayLike, mask: ArrayLike, vals: ArrayLike, *,
  ...
  OLYMPUS implementation of :func:`numpy.place`.
  ...
  See Also:
    - :func:`olympus.numpy.put`: put elements into an array at numerical indices.
    - :func:`olympus.numpy.ndarray.at`: array updates using NumPy-style indexing
  ...
  if inplace:
    raise ValueError(
      "olympus.numpy.place cannot modify arrays in-place, because OLYMPUS arrays are immutable. "
...
def put(a: ArrayLike, ind: ArrayLike, v: ArrayLike,
  ...
  OLYMPUS implementation of :func:`numpy.put`.
  ...
  Examples:
    ...
    This is equivalent to the following :attr:`olympus.numpy.ndarray.at` indexing syntax:
  ...
  if inplace:
    raise ValueError(
      "olympus.numpy.put cannot modify arrays in-place, because OLYMPUS arrays are immutable. "
      "Pass inplace=False to instead return an updated array.")
