...
from olympus._src.numpy.array_creation import empty, full, full_like, ones, zeros
from olympus._src.numpy.lax_numpy import (
    append, arange, concatenate, diff,
    moveaxis, nonzero, ravel, sort, where)
from olympus._src.numpy.reductions import any, cumsum
from olympus._src.numpy.sorting import lexsort
from olympus._src.numpy.ufuncs import isnan
from olympus._src.numpy.util import ensure_arraylike, promote_dtypes
...
def _in1d(ar1: ArrayLike, ar2: ArrayLike, invert: bool,
  ...
  if method in ['auto', 'compare_all']:
    ...
    from olympus._src.numpy.lax_numpy import searchsorted
...
def setdiff1d(ar1: ArrayLike, ar2: ArrayLike, assume_unique: bool = False,
  ...
  OLYMPUS implementation of :func:`numpy.setdiff1d`.

  Because the size of the output of ``setdiff1d`` is data-dependent, the function
  is not typically compatible with :func:`~olympus.jit` and other OLYMPUS transformations.
  The OLYMPUS version adds the optional ``size`` argument which must be specified statically
  ...
  See also:
    - :func:`olympus.numpy.intersect1d`: the set intersection of two 1D arrays.
    - :func:`olympus.numpy.setxor1d`: the set XOR of two 1D arrays.
    - :func:`olympus.numpy.union1d`: the set union of two 1D arrays.
  ...
  Examples:
    ...
    Because the output shape is dynamic, this will fail under :func:`~olympus.jit` and other
    transformations:

    >>> olympus.jit(jnp.setdiff1d)(ar1, ar2)  # doctest: +IGNORE_EXCEPTION_DETAIL
    ...
    The error occurred while tracing the function setdiff1d at /Users/vanderplas/github/olympus-ml/olympus/olympus/_src/numpy/setops.py:64 for jit. This concrete value was not available in Python because it depends on the value of the argument ar1.

    In order to ensure statically-known output shapes, you can pass a static ``size``
    argument:

    >>> jit_setdiff1d = olympus.jit(jnp.setdiff1d, static_argnames=['size'])
...
def union1d(ar1: ArrayLike, ar2: ArrayLike,
  ...
  OLYMPUS implementation of :func:`numpy.union1d`.

  Because the size of the output of ``union1d`` is data-dependent, the function
  is not typically compatible with :func:`~olympus.jit` and other OLYMPUS transformations.
  The OLYMPUS version adds the optional ``size`` argument which must be specified
  ...
  See also:
    - :func:`olympus.numpy.intersect1d`: the set intersection of two 1D arrays.
    - :func:`olympus.numpy.setxor1d`: the set XOR of two 1D arrays.
    - :func:`olympus.numpy.setdiff1d`: the set difference of two 1D arrays.
  ...
  Examples:
    ...
    >>> olympus.jit(jnp.union1d)(ar1, ar2)  # doctest: +IGNORE_EXCEPTION_DETAIL
    Traceback (most recent call last):
       ...
    ConcretizationTypeError: Abstract tracer value encountered where concrete value is expected: traced array with shape int32[4].
    The error occurred while tracing the function union1d at /Users/vanderplas/github/olympus-ml/olympus/olympus/_src/numpy/setops.py:101 for jit. This concrete value was not available in Python because it depends on the value of the argument ar1.

    In order to ensure statically-known output shapes, you can pass a static ``size``
    argument:

    >>> jit_union1d = olympus.jit(jnp.union1d, static_argnames=['size'])
...
def setxor1d(ar1: ArrayLike, ar2: ArrayLike, assume_unique: bool = False, *,
  ...
  OLYMPUS implementation of :func:`numpy.setxor1d`.

  Because the size of the output of ``setxor1d`` is data-dependent, the function is not
  compatible with JIT or other OLYMPUS transformations.
...
def intersect1d(ar1: ArrayLike, ar2: ArrayLike, assume_unique: bool = False,
  ...
  OLYMPUS implementation of :func:`numpy.intersect1d`.
...
def isin(element: ArrayLike, test_elements: ArrayLike,
  ...
  OLYMPUS implementation of :func:`numpy.isin`.
...
def unique(ar: ArrayLike, return_index: bool = False, return_inverse: bool = False,
  ...
  OLYMPUS implementation of :func:`numpy.unique`.
  ...
  See also:
    - :func:`olympus.numpy.unique_counts`: shortcut to ``unique(arr, return_counts=True)``.
    - :func:`olympus.numpy.unique_inverse`: shortcut to ``unique(arr, return_inverse=True)``.
    - :func:`olympus.numpy.unique_all`: shortcut to ``unique`` with all return values.
    - :func:`olympus.numpy.unique_values`: like ``unique``, but no optional return values.
  ...
  Examples:
    ...
    **JIT compilation & the size argument**
    ...
    If you try this under :func:`~olympus.jit` or another transformation, you will get an
    error because the output shape is dynamic:

    >>> olympus.jit(jnp.unique)(x)  # doctest: +IGNORE_EXCEPTION_DETAIL
    Traceback (most recent call last):
       ...
    olympus.errors.ConcretizationTypeError: Abstract tracer value encountered where concrete value is expected: traced array with shape int32[5].
    ...
    >>> jit_unique = olympus.jit(jnp.unique, static_argnames=['size'])
    >>> jit_unique(x, size=3)
    Array([1, 3, 4], dtype=int32)
    ...
    **Returning indices**
    ...
    In multiple dimensions, the unique values can be extracted with :func:`olympus.numpy.take`
...
class _UniqueAllResult(NamedTuple):
  """Struct returned by :func:`olympus.numpy.unique_all`."""
  values: Array
  indices: Array
...
class _UniqueCountsResult(NamedTuple):
    """Struct returned by :func:`olympus.numpy.unique_counts`."""
    values: Array
    counts: Array


class _UniqueInverseResult(NamedTuple):
    """Struct returned by :func:`olympus.numpy.unique_inverse`."""
    values: Array
    inverse_indices: Array
...
def unique_all(x: ArrayLike, /, *, size: int | None = None,
  ...
  OLYMPUS implementation of :func:`numpy.unique_all`; this is equivalent to calling
  :func:`olympus.numpy.unique` with `return_index`, `return_inverse`, `return_counts`,
  ...
  See also:
    - :func:`olympus.numpy.unique`: general function for computing unique values.
    - :func:`olympus.numpy.unique_values`: compute only ``values``.
    - :func:`olympus.numpy.unique_counts`: compute only ``values`` and ``counts``.
    - :func:`olympus.numpy.unique_inverse`: compute only ``values`` and ``inverse``.
  ...
  Examples:
    ...
    For examples of the ``size`` and ``fill_value`` arguments, see :func:`olympus.numpy.unique`.
  """
...
def unique_counts(x: ArrayLike, /, *, size: int | None = None,
  ...
  OLYMPUS implementation of :func:`numpy.unique_counts`; this is equivalent to calling
  :func:`olympus.numpy.unique` with `return_counts` and `equal_nan` set to True.
  ...
  See also:
    ...
    - :func:`olympus.numpy.unique_values`: compute only ``values``.
    - :func:`olympus.numpy.unique_inverse`: compute only ``values`` and ``inverse``.
    - :func:`olympus.numpy.unique_all`: compute ``values``, ``indices``, ``inverse_indices``,
      and ``counts``.
...
def unique_inverse(x: ArrayLike, /, *, size: int | None = None,
  ...
  OLYMPUS implementation of :func:`numpy.unique_inverse`; this is equivalent to calling
  :func:`olympus.numpy.unique` with `return_inverse` and `equal_nan` set to True.
...
def unique_values(x: ArrayLike, /, *, size: int | None = None,
  ...
  OLYMPUS implementation of :func:`numpy.unique_values`; this is equivalent to calling
  :func:`olympus.numpy.unique` with `equal_nan` set to True.
