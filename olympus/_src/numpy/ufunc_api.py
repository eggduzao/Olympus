...
from olympus._src.numpy.reductions import _moveaxis
from olympus._src.numpy.util import check_arraylike, _broadcast_to, _where
...
_AT_INPLACE_WARNING = """\
Because OLYMPUS arrays are immutable, jnp.ufunc.at() cannot operate inplace like
...
class ufunc:
  ...
  OLYMPUS implementation of :class:`numpy.ufunc`.

  This is a class for OLYMPUS-backed implementations of NumPy's ufunc APIs.
  Most users will never need to instantiate :class:`ufunc`, but rather
  will use the pre-defined ufuncs in :mod:`olympus.numpy`.

  For constructing your own ufuncs, see :func:`olympus.numpy.frompyfunc`.
  ...
  Examples:
    ...
    As an example, consider the function :obj:`olympus.numpy.add`. The object
    ...
    Binary ufuncs like :obj:`olympus.numpy.add` include  number of methods to
    ...
    to :func:`olympus.numpy.cumulative_sum`:

    >>> jnp.add.accumulate(x)
    Array([ 1,  3,  6, 10, 15], dtype=int32)

    The :meth:`ufunc.at` method applies the function at particular indices in the
    array; for ``jnp.add`` the computation is similar to :func:`olympus.lax.scatter_add`:
    ...
    operation is similar to :func:`olympus.ops.segment_sum`:
  ...
  def reduce(self, a: ArrayLike, axis: int | None = 0,
    ...
    OLYMPUS implementation of :meth:`numpy.ufunc.reduce`.
    ...
    Examples:
      ...
      :meth:`olympus.numpy.add.reduce` is equivalent to :func:`olympus.numpy.sum`
      along ``axis=0``:
      ...
      Similarly, :meth:`olympus.numpy.logical_and.reduce` is equivalent to
      :func:`olympus.numpy.all`:
      ...
      for example here is the reduction of :func:`olympus.numpy.bitwise_or` along
      the first axis of ``x``:
  ...
  def accumulate(self, a: ArrayLike, axis: int = 0, dtype: DTypeLike | None = None,
    ...
    OLYMPUS implementation of :func:`numpy.ufunc.accumulate`.
    ...
    Examples:
      ...
      :meth:`olympus.numpy.add.accumulate` is equivalent to
      :func:`olympus.numpy.cumsum` along the specified axis:
      ...
      Similarly, :meth:`olympus.numpy.multiply.accumulate` is equivalent to
      :func:`olympus.numpy.cumprod` along the specified axis:
      ...
      via standard APIs. For example, :meth:`olympus.numpy.bitwise_or.accumulate`
  ...
  def at(self, a: ArrayLike, indices: Any, b: ArrayLike | None = None, /, *,
    ...
    OLYMPUS implementation of :func:`numpy.ufunc.at`.

    Note:
      :meth:`numpy.ufunc.at` mutates arrays in-place. OLYMPUS arrays are immutable,
      so :meth:`olympus.numpy.ufunc.at` cannot replicate these semantics. Instead, OLYMPUS
    ...
    Examples:
      ...
      This is roughly equivalent to OLYMPUS's :meth:`olympus.numpy.ndarray.at` method
      called this way:
  ...
  def reduceat(self, a: ArrayLike, indices: Any, axis: int = 0,
    ...
    OLYMPUS implementation of :meth:`numpy.ufunc.reduceat`
    ...
    Examples:
      ...
      For some binary ufuncs, OLYMPUS provides similar APIs within :mod:`olympus.ops`.
      For example, :meth:`olympus.add.reduceat` is similar to :func:`olympus.ops.segment_sum`,
      although in this case the segments are defined via an array of segment ids:

      >>> segments = jnp.array([0, 0, 1, 1, 1, 2, 2, 2])
      >>> olympus.ops.segment_sum(x, segments)
      Array([ 3, 12, 21], dtype=int32)
    """
    if self.nin != 2:
  ...
  def outer(self, A: ArrayLike, B: ArrayLike, /) -> Array:
    ...
    OLYMPUS implementation of :meth:`numpy.ufunc.outer`.
    ...
    Examples:
      ...
      :meth:`olympus.numpy.multiply.outer`:
...
def frompyfunc(func: Callable[..., Any], /, nin: int, nout: int,
  ...
  """Create a OLYMPUS ufunc from an arbitrary OLYMPUS-compatible scalar function.
  ...
  Returns:
    wrapped : olympus.numpy.ufunc wrapper of func.

  Examples:
    Here is an example of creating a ufunc similar to :obj:`olympus.numpy.add`:

    >>> import operator
    >>> add = frompyfunc(operator.add, nin=2, nout=1, identity=0)

    Now all the standard :class:`olympus.numpy.ufunc` methods are available:
