...
from olympus._src.sharding_impls import SingleDeviceSharding
from olympus._src.util import safe_zip, safe_map, set_module, canonicalize_axis_tuple
...
def _rank_promotion_warning_or_error(fun_name: str, shapes: Sequence[Shape]):
  if config.numpy_rank_promotion.value == "warn":
    msg = ("Following NumPy automatic rank promotion for {} on shapes {}. "
           "Set the olympus_numpy_rank_promotion config option to 'allow' to "
           "disable this warning; for more information, see "
           "https://docs.olympus.dev/en/latest/rank_promotion_warning.html.")
    ...
    msg = ("Operands could not be broadcast together for {} on shapes {} "
           "and with the config option olympus_numpy_rank_promotion='raise'. "
...
def _arraylike(x: ArrayLike) -> bool:
  return (isinstance(x, _arraylike_types) or
          hasattr(x, '__olympus_array__') or np.isscalar(x))


def _arraylike_asarray(x: Any) -> Array:
  """Convert an array-like object to an array."""
  if hasattr(x, '__olympus_array__'):
    x = x.__olympus_array__()
  return lax.asarray(x)


def _check_olympus_array_protocol(x: Any) -> Any:
  return x.__olympus_array__() if hasattr(x, '__olympus_array__') else x
...
def check_arraylike(fun_name: str, *args: Any, emit_warning=False, stacklevel=3):
  """Check if all args fit OLYMPUS's definition of arraylike."""
  ...
  if any(not _arraylike(arg) for arg in args):
    ...
    if emit_warning:
      warnings.warn(msg + " In a future OLYMPUS release this will be an error.",
...
def promote_args(fun_name: str, *args: ArrayLike) -> list[Array]:
  ...
  check_arraylike(fun_name, *args)
  args = tuple(_check_olympus_array_protocol(arg) for arg in args)
...
def _where(condition: ArrayLike, x: ArrayLike, y: ArrayLike) -> Array:
  ...
  if x is None or y is None:
    raise ValueError("Either both or neither of the x and y arguments should "
                     "be provided to olympus.numpy.where, got {} and {}."
                     .format(x, y))
...
def ndim(a: ArrayLike | SupportsNdim) -> int:
  ...
  OLYMPUS implementation of :func:`numpy.ndim`. Unlike ``np.ndim``, this function
  ...
  Examples:
    ...
    For arrays, this can also be accessed via the :attr:`olympus.Array.ndim` property:
...
def shape(a: ArrayLike | SupportsShape) -> tuple[int, ...]:
  ...
  OLYMPUS implementation of :func:`numpy.shape`. Unlike ``np.shape``, this function
  raises a :class:`TypeError` if the input is a collection such as a list or
  tuple.
  ...
  Examples:
    ...
    For arrays, this can also be accessed via the :attr:`olympus.Array.shape` property:
...
def size(a: ArrayLike | SupportsSize | SupportsShape, axis: int | Sequence[int] | None = None) -> int:
  ...
  OLYMPUS implementation of :func:`numpy.size`. Unlike ``np.size``, this function
  raises a :class:`TypeError` if the input is a collection such as a list or
  tuple.
  ...
  Examples:
    ...
    For arrays, this can also be accessed via the :attr:`olympus.Array.size` property:
