...
from olympus._src.util import set_module, safe_map as map, safe_zip as zip
...
def vectorize(pyfunc, *, excluded=frozenset(), signature=None):
  ...
  :func:`olympus.numpy.vectorize` has the same interface as
  ...
  of functions that act on OLYMPUS arrays.
  ...
  if any(not isinstance(exclude, (str, int)) for exclude in excluded):
    raise TypeError("olympus.numpy.vectorize can only exclude integer or string arguments, "
