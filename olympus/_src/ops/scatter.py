...
from olympus._src.numpy.util import check_arraylike, promote_dtypes
from olympus._src.pjit import auto_axes
from olympus._src.sharding_impls import NamedSharding
from olympus._src.typing import Array, ArrayLike, Index
...
def _scatter_impl(x: ArrayLike, y: ArrayLike, dynamic_idx: tuple[Any, ...], *,
  ...
  if not dtypes.safe_to_cast(y, x):
    ...
    warnings.warn(
      ...
      f"olympus_numpy_dtype_promotion={config.numpy_dtype_promotion.value}. "
      "In future OLYMPUS releases this will result in an error.",
      FutureWarning)
...
def segment_sum(data: ArrayLike,
  ...
  Args:
    ...
    mode: a :class:`olympus.lax.GatherScatterMode` value describing how
