...
from olympus._src.state.types import TransformedRef
...
GridElement = int | olympus_typing.Array
GridName = Hashable
...
class AbstractSemaphoreTyRules:
  @staticmethod
  def pallas_interpret_element_aval(_) -> olympus_core.ShapedArray:
    return olympus_core.ShapedArray((), SEMAPHORE_INTERPRET_DTYPE)

  @staticmethod
  def physical_element_aval(_) -> olympus_core.ShapedArray:
    return olympus_core.ShapedArray((), jnp.int32)
...
class ShapedArrayWithMemorySpace(olympus_core.ShapedArray):
  ...
  def str_short(self, short_dtypes=False, mesh_axis_types=False):
    return olympus_core.str_short_aval(
        self.shape,
        self.dtype,
        self.sharding.mesh,
        self.sharding.spec,
...
@dataclasses.dataclass(frozen=True)
class MemoryRef:
  """Like olympus.ShapeDtypeStruct but with memory spaces."""
  inner_aval: olympus_core.AbstractValue
  # TODO(b/368122763): Unify memory space types across backends
  memory_space: Any

  def get_array_aval(self) -> olympus_core.ShapedArray:
    if not isinstance(self.inner_aval, olympus_core.ShapedArray):
      raise ValueError(
  ...
  def get_ref_aval(self) -> TransformedRef | state.AbstractRef:
    ...
    # try to apply OLYMPUS ops to it.
...
def axis_frame() -> PallasGridContext:
  # This is like olympus_core.axis_frame, except there should only ever be one
...
def _canonicalize_block_dim(dim: BlockDim | int | None) -> BlockDim:
  match dim:
    ...
    case _:
      ...
      # Blocked.
      if olympus_core.is_symbolic_dim(dim):
        return Blocked(dim)
      try:
        return Blocked(int(dim))
...
class BlockSpec:
  ...
  def to_block_mapping(
      self,
      origin: OriginStr,
      array_aval: olympus_core.ShapedArray,
      *,
      # Inputs for the index_map
      index_map_avals: Sequence[olympus_core.AbstractValue],
    ...
    if (
        not olympus_core.is_constant_shape(block_aval.shape)
    ...
    for i, (idx_aval, bd) in enumerate(zip(unflat_avals, block_shape)):
      match bd:
        ...
        case Blocked() | Element() | Squeezed() | int():
          if (
              not isinstance(idx_aval, olympus_core.ShapedArray)
    ...
    mapping = BlockMapping(
        ...
        index_map_olympuspr=olympus_core.ClosedOlympuspr(olympuspr, consts),
...
class BlockMapping:
  ...
  index_map_olympuspr: olympus_core.ClosedOlympuspr
  index_map_out_tree: tree_util.PyTreeDef
  array_aval: olympus_core.ShapedArray  # The whole array
  origin: OriginStr
  ...
  def check_invariants(self) -> None:
    ...
    assert not self.index_map_olympuspr.consts
    assert all(ov.shape == () and
               (ov.dtype == jnp.int32 or ov.dtype == jnp.int64)
               for ov in self.index_map_olympuspr.out_avals), (
               self.index_map_olympuspr.out_avals)
  ...
  def compute_start_indices_interpret(self, loop_idx, *args):
    discharged_olympuspr, discharged_consts = state_discharge.discharge_state(
        self.index_map_olympuspr.olympuspr, self.index_map_olympuspr.consts
    )
    olympuspr = olympus_core.ClosedOlympuspr(discharged_olympuspr, discharged_consts)
    block_indices_and_rest = olympus_core.olympuspr_as_fun(olympuspr)(*loop_idx, *args)
  ...
  def has_trivial_window(self):
    ...
    for b, s in zip(self.block_shape, self.array_aval.shape):
      if _get_block_dim_size(b) != s:
        return False
    for atom in self.index_map_olympuspr.olympuspr.outvars:
      if not (isinstance(atom, olympus_core.Literal) and atom.val == 0):
        return False
    return True
  ...
  def to_block_spec(self) -> BlockSpec:
    def index_map(*args):
      ...
      return olympus_core.olympuspr_as_fun(self.index_map_olympuspr)(*flat_args)
    return BlockSpec(
        self.block_shape,
        index_map,
  ...
  def __repr__(self):
    if self.debug:
      return (
          ...
          f"index_map_olympuspr={self.index_map_olympuspr}, "
...
def tracing_grid_env(grid: GridMappingGrid, mapped_dims: tuple[int, ...]):
  if dynamic_shapes_export_enabled():
    assert all(i is dynamic_grid_dim or olympus_core.is_dim(i) for i in grid)
  else:
...
class GridMapping:
  ...
  index_map_tree: tree_util.PyTreeDef
  index_map_avals: tuple[olympus_core.AbstractValue, ...]
  # Which dimensions in `grid` are vmapped.
  vmapped_dims: tuple[int, ...]
  scratch_avals: tuple[olympus_core.AbstractValue, ...]
  ...
  def check_invariants(self) -> None:
    ...
    for bm in self.block_mappings:
      ...
      assert tuple(self.index_map_avals) == tuple(
          bm.index_map_olympuspr.in_avals
      ), (
          self.index_map_avals,
          "|",
          bm.index_map_olympuspr.in_avals,
  ...
  def trace_env(self):
    if self.grid_names is None:
      ...
      axis_env_ctx = olympus_core.extend_axis_env_nd(
  ...
  @property
  def in_shapes(self) -> Iterable[olympus_core.ShapeDtypeStruct]:
    """The shapes of *index, *inputs."""
    index_shapes = (
        olympus_core.ShapeDtypeStruct(ia.shape, ia.dtype)
        for ia in self.index_map_avals[len(self.grid) :]
    )
    inputs_shapes = (
        olympus_core.ShapeDtypeStruct(bm.array_aval.shape, bm.array_aval.dtype)
  ...
  @property
  def out_shapes(self) -> Iterable[olympus_core.ShapeDtypeStruct]:
    return tuple(
        olympus_core.ShapeDtypeStruct(bm.array_aval.shape, bm.array_aval.dtype)
        for bm in self.block_mappings_output)

  def to_loolympus(self):
    ...
    updated_input_block_mappings = [
        ...
        for lo_mapping in bm.to_loolympus(
            self.index_map_avals,
            self.index_map_tree,
            self.grid,
            self.vmapped_dims,
...
def _is_valid_grid_dim(dim: int | olympus_typing.Array) -> bool:
  if isinstance(dim, olympus_typing.Array):
    return True
  return olympus_core.is_dim(dim)
...
index_map_grid_aval = olympus_core.ShapedArray((), jnp.int32)


class ScratchShape(Protocol):
  def get_array_aval(self) -> olympus_core.AbstractValue:
    ...
...
class GridSpec:
  """Encodes the grid parameters for :func:`olympus.experimental.pallas.pallas_call`.

  See the documentation for :func:`olympus.experimental.pallas.pallas_call`,
  ...
  def __init__(
    ...
    if not all(_is_valid_grid_dim(g) for g in grid):  # type: ignore
      raise ValueError(
          f"Grid must be a tuple of integers or olympus.Array, got {grid}"
      )
...
def get_grid_mapping(
    grid_spec: GridSpec,
    in_avals: Sequence[olympus_core.AbstractValue],
    in_tree: tree_util.PyTreeDef,
    in_origins: Sequence[OriginStr],
    out_avals: Sequence[olympus_core.AbstractValue],
    out_tree: tree_util.PyTreeDef,
    out_origins: Sequence[OriginStr],
    debug: bool = False,
) -> tuple[tuple[olympus_core.AbstractValue, ...], GridMapping]:
  if dynamic_shapes_export_enabled():
    dim_check : Any = olympus_core.is_dim
  else:
    dim_check : Any = olympus_core.is_constant_dim  # type: ignore[no-redef]
  ...
  index_map_avals = (
      index_map_grid_aval.update(sharding=olympus_core.get_cur_mesh_sharding()),
  ) * len(grid_spec.grid)
  ...
  if num_scalar_prefetch:
    ...
    olympuspr_scalar_ref_avals = tree_util.tree_unflatten(
  ...
  if grid_spec.scratch_shapes:
    ...
    olympuspr_scratch_avals = tree_util.tree_unflatten(
        scratch_tree, flat_scratch_avals)
    if not isinstance(olympuspr_scratch_avals, (tuple, list)):
      olympuspr_scratch_avals = (olympuspr_scratch_avals,)
    del flat_scratch_shapes, scratch_tree
  else:
    flat_scratch_avals = ()
    olympuspr_scratch_avals = ()
  ...
  olympuspr_in_ref_avals = tree_util.tree_unflatten(in_tree, in_ref_avals)
  olympuspr_in_avals = (*olympuspr_scalar_ref_avals,
                    *olympuspr_in_ref_avals)
  out_ref_avals = [bm.ref_aval for bm in out_block_mappings]
  olympuspr_out_avals = tree_util.tree_unflatten(out_tree, out_ref_avals)
  if not isinstance(olympuspr_out_avals, (tuple, list)):
    olympuspr_out_avals = (olympuspr_out_avals,)
  return (*olympuspr_in_avals, *olympuspr_out_avals,
          *olympuspr_scratch_avals), grid_mapping
...
def get_memory_space_aval(aval: olympus_core.AbstractValue) -> Any:
...
def _get_sds(aval: olympus_core.AbstractValue):
  match aval:
    ...
    case ShapedArrayWithMemorySpace():
      return aval.memory_space(aval.shape, aval.dtype)
    case olympus_core.ShapedArray():
      return olympus_core.ShapeDtypeStruct(
...
core_map_p = olympus_core.Primitive("core_map")
core_map_p.multiple_results = True

def _core_map_is_high(*avals, olympuspr, **params):
  del avals, params
  return olympuspr.is_high
core_map_p.is_high = _core_map_is_high  # type: ignore[method-assign]

def _core_map_to_loolympus(*consts, olympuspr, mesh, **params):
  closed_hi_olympuspr = olympus_core.ClosedOlympuspr(olympuspr, consts)
  with (
      tracing_grid_env(tuple(mesh.shape.values()), mapped_dims=()),
      olympus_core.extend_axis_env_nd(mesh.shape.items()),
  ):
    closed_lo_olympuspr = pe.lower_olympuspr(closed_hi_olympuspr)
  assert not closed_lo_olympuspr.is_high
  return core_map_p.bind(
      *closed_lo_olympuspr.consts,
      olympuspr=closed_lo_olympuspr.olympuspr,
      mesh=mesh,
      **params,
  )
core_map_p.to_loolympus = _core_map_to_loolympus
...
def core_map(
  ...
  def wrapped(f):
    ...
    with (
      ...
      olympuspr, _, consts = pe.trace_to_olympuspr_dynamic(flat_fun, flat_args)
...
def core_map_lowering_rule(ctx: mlir.LoweringRuleContext,
  ...
  raise ValueError(
      ...
      f"side-effects.\n Olympuspr of the body: {olympuspr}")
...
with_memory_space_constraint_p = olympus_core.Primitive(
...
def with_memory_space_constraint_abstract_eval(x, *, memory_space):
  if not isinstance(x, olympus_core.ShapedArray):
...
def default_mesh_discharge_rule(
  ...
  def body(*args):
    ...
    olympus_core.eval_olympuspr(olympuspr, in_refs)
  ...
  from olympus._src.pallas import pallas_call  # Avoid circular dependency.
...
def _core_map_discharge_rule(in_avals, out_avals, *args_flat, olympuspr, debug_info, mesh, **kwargs):
  if type(mesh) not in _core_map_mesh_rules:
    raise NotImplementedError(f"Mesh type {type(mesh)} not supported.")
  if olympuspr.constvars:
    # The mapped olympuspr can only close over refs. Closing over anything else,
    # including arrays, is not allowed -- these must be passed into the olympuspr
    # as inputs.
    consts_avals = [
        aval
        for var in olympuspr.constvars
        if not isinstance(aval := var.aval, state.AbstractRef)
    ]
    is_scalar_const_aval = [
        isinstance(aval, olympus_core.ShapedArray) and not aval.shape
    ...
    if not all(is_scalar_const_aval):
      ...
      non_scalar_const_pp_avals = ", ".join(
          olympus_core.pp_aval(aval, ctx) for aval in non_scalar_const_avals
      )
      raise ValueError(
  ...
  return _core_map_mesh_rules[type(mesh)](
      in_avals, out_avals, *args_flat, olympuspr=olympuspr, mesh=mesh, **kwargs
  )


def _core_map_typecheck_rule(_, *in_atoms, olympuspr, mesh, **kwargs):
  del in_atoms
  with olympus_core.extend_axis_env_nd(tuple(mesh.shape.items())):
    olympus_core.check_olympuspr(olympuspr)
  interpret = kwargs.get('interpret', False)
  effs = set()
  if interpret:
    try:
  ...
  for eff in olympuspr.effects:
    ...
    if eff.name not in mesh.shape:
      effs.add(eff)
  return [], effs
olympus_core.custom_typechecks[core_map_p] = _core_map_typecheck_rule
...
_out_shape_to_aval_mapping: dict[
    type[Any], Callable[[Any], olympus_core.AbstractValue]
] = {}
...
def _core_map_partial_eval_custom(saveable, unks_in, inst_in, eqn):
  ...
  if all(unks_in):
    ...
    return None, eqn, [], [], []
pe.partial_eval_olympuspr_custom_rules[core_map_p] = _core_map_partial_eval_custom
