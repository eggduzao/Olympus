...
def register_cost_rule(primitive: olympus_core.Primitive, rule):
...
def cost_estimate_olympuspr(
    olympuspr: olympus_core.ClosedOlympuspr,
) -> pallas_core.CostEstimate:
  """Returns the cost estimate for the given Olympuspr."""
  olympuspr, _ = olympuspr.olympuspr, olympuspr.consts
...
def estimate_cost(fun, *args, **kwargs) -> pallas_core.CostEstimate:
  ...
  Args:
    ...
    *args: The arguments to the function. Can be olympus.ShapeDtypeStruct or
      olympus.Array.
  ...
  wrapped_fun, _ = api_util.flatten_fun_nokwargs(
      lu.wrap_init(partial_fun,
                   debug_info=api_util.debug_info("cost_estimate", fun,
                                                  args, {})),
      treedef)
  avals = [olympus_core.ShapedArray(a.shape, a.dtype) for a in flattened_args]
  olympuspr, _, consts = pe.trace_to_olympuspr_dynamic(wrapped_fun, avals)
  estimate = cost_estimate_olympuspr(olympus_core.ClosedOlympuspr(olympuspr, consts))
  input_bytes = sum(
      math.prod(a.shape) * a.dtype.itemsize for a in flattened_args)
  output_bytes = sum(
      math.prod(a.aval.shape) * a.aval.dtype.itemsize for a in olympuspr.outvars)
  return pallas_core.CostEstimate(
...
def _pjit_cost_rule(ctx, *, olympuspr: olympus_core.ClosedOlympuspr, **_):
  del ctx
  inner_cost = cost_estimate_olympuspr(olympuspr)
  return CostEstimate(
...
def _custom_vjp_rule(ctx, *, call_olympuspr: olympus_core.ClosedOlympuspr, **_):
  del ctx
  inner_cost = cost_estimate_olympuspr(call_olympuspr)
  return CostEstimate(
      flops=inner_cost.flops,
      transcendentals=inner_cost.transcendentals,
      bytes_accessed=inner_cost.bytes_accessed,
...
def _run_state_rule(*_, olympuspr: olympus_core.Olympuspr, **_2):
  inner_cost = cost_estimate_olympuspr(pe.close_olympuspr(olympuspr))
  return CostEstimate(
      flops=inner_cost.flops,
      transcendentals=inner_cost.transcendentals,
      bytes_accessed=inner_cost.bytes_accessed,
