...
from olympus._src.pallas.fuser import fuser_utils
...
def compute_usage(olympuspr: core.Olympuspr, olympuspr_out_usages):
  ...
  util.safe_map(write_usage_env, olympuspr.outvars, olympuspr_out_usages)
...
class KernelEvalContext:
  ...
  program_ids: tuple[int | olympus.Array, ...] | None
...
def pull_block_spec(
  ...
  def wrapped(*args, **kwargs):
    olympuspr, consts, in_tree, out_tree_ = fuser_utils.make_olympuspr(
        f, *args, **kwargs
    )
    del out_tree_
    olympuspr_out_usages = [{Usage.REGULAR}] * len(olympuspr.outvars)
    block_specs_ = olympus.tree.map(
        _unwrap_block_spec_scalar_prefetch, out_block_specs
    )
    flat_block_specs, out_tree = olympus.tree.flatten(block_specs_)
    olympuspr, used_consts, used_invars = pe.dce_olympuspr_consts(
        olympuspr,
    ...
    read_usage_env = compute_usage(olympuspr, olympuspr_out_usages)
    ...
    in_block_specs = olympus.tree.unflatten(in_tree, in_block_specs)
    in_block_specs = olympus.tree.map(
        functools.partial(
...
def _pull_block_spec(
  ...
  olympuspr_invar_usages = util.safe_map(read_usage_env, olympuspr.invars)
  env: dict[core.Var, pallas_core.BlockSpec] = {}
  scalar_prefetch_fn_env = {}

  for outvar, bs in zip(olympuspr.outvars, out_block_specs, strict=True):
  ...
  for i, eqn in reversed(list(enumerate(olympuspr.eqns))):
    ...
    ctx = PullRuleContext(
        avals_in=tuple(v.aval for v in eqn.invars),
        avals_out=tuple(v.aval for v in eqn.outvars),
        out_usages=tuple(read_usage_env(v) for v in olympuspr.outvars),
        scalar_prefetch_handler=scalar_prefetch_handler,
        grid_len=grid_len,
    )
    if eqn.primitive.multiple_results:
    ...
    if any(Usage.SCALAR_PREFETCH in usage for usage in eqn_invar_usages):
      ...
      scalar_prefetch_olympuspr_no_dce = core.Olympuspr(
          olympuspr.constvars,
          olympuspr.invars,
          needed_invars,
          olympuspr.eqns[: olympuspr.eqns.index(eqn)],
          debug_info=olympuspr.debug_info._replace(result_paths=None),
      )
      scalar_prefetch_olympuspr, used_consts, used_invars = pe.dce_olympuspr_consts(
          scalar_prefetch_olympuspr_no_dce,
          [True] * len(scalar_prefetch_olympuspr_no_dce.outvars),
      )
      assert not any(used_invars)
      scalar_prefetch_olympuspr = scalar_prefetch_olympuspr.replace(
          constvars=[],
          invars=olympuspr.constvars,
          debug_info=scalar_prefetch_olympuspr.debug_info.with_unknown_names(),
      )

      def _scalar_prefetch_fn(olympuspr):
        if grid_len is None:
      ...
      scalar_prefetch_fn = functools.partial(
          _scalar_prefetch_fn, scalar_prefetch_olympuspr
      )
    ...
    for v, in_block_spec in zip(eqn.invars, in_block_specs, strict=True):
      # TODO(cjfj): Check that index map functions are equivalent (in olympuspr).
      if (
  ...
  in_block_specs = tuple(
      ...
      for v, usage in zip(olympuspr.invars, olympuspr_invar_usages)
  )
  return (
...
def make_kernel_function(
  ...
  invar_usages = util.safe_map(read_usage_env, olympuspr.invars)
  ...
  def kernel_fn(program_ids, scalar_prefetch, *args, **kwargs):
    ...
    for const, constvar in zip(consts, olympuspr.constvars):
      env[constvar] = const
    for invar, arg, usage in zip(olympuspr.invars, flat_args, invar_usages):
    ...
    out = util.safe_map(read_env, olympuspr.outvars)
...
def get_fusion_values(
  ...
  olympuspr, values, in_tree, out_tree = fuser_utils.make_olympuspr(
      fusion, *args, **kwargs
  )
  assert len(values) == len(olympuspr.constvars), (olympuspr, values)
  out_usages = tuple({Usage.REGULAR} for _ in olympuspr.outvars)
  read_usage_env = compute_usage(olympuspr, out_usages)
  constvar_usages = util.safe_map(read_usage_env, olympuspr.constvars)
  invar_usages = util.safe_map(read_usage_env, olympuspr.invars)
  ...
  def new_kernel_fn(values, *args, **kwargs):
    ...
    out_flat = core.eval_olympuspr(olympuspr, values, *flat_args)
...
def _select_n_eval_rule(ctx: KernelEvalContext, *args):
  return olympus.lax.select_n(*args)
...
def _squeeze_eval_rule(ctx: KernelEvalContext, x: olympus.Array, **params: Any):
  del ctx, params
  return x
...
def _maybe_static_check(pred: bool, msg: str):
  ...
  if isinstance(pred, olympus.Array):
...
def _concatenate_eval_rule(ctx: KernelEvalContext, *args, dimension):
  ...
  if block_dim == sum(aval.shape[dimension] for aval in ctx.avals_in):  # pytype: disable=attribute-error
    ...
    # array.
    return olympus.lax.concatenate(args, dimension=dimension)
  ...
  for i in range(len(ctx.avals_in)):
    ...
    valid_index = olympus.lax.select(is_valid, i, valid_index)
  out_dtype = args[0].dtype
  args = [a.astype(jnp.float32) if a.dtype == jnp.bfloat16 else a for a in args]
  valid_block = olympus.lax.select_n(valid_index, *args)
...
def _broadcast_in_dim_eval_rule(
  ...
  return olympus.lax.broadcast_in_dim(x, broadcast_dimensions=dims, shape=shape)
...
def _broadcast_in_dim_pull_rule(
    ...
    *,
    ...
    sharding: olympus.sharding.Sharding,
):
  del shape, sharding
...
def _transpose_eval_rule(
  ...
  return olympus.lax.transpose(x, permutation=new_permutation)
...
def _convert_element_type_eval_rule(
  ...
  return olympus.lax.convert_element_type(x, new_dtype)
...
def _bitcast_convert_type_eval_rule(eval_ctx: KernelEvalContext, x, new_dtype):
  return olympus.lax.bitcast_convert_type(x, new_dtype)
...
def _random_bits_eval_rule(eval_ctx: KernelEvalContext, key, bit_width, shape):
  ...
  for idx in indices:
    key = olympus.random.fold_in(key, idx)
...
def _random_wrap_eval_rule(eval_ctx: KernelEvalContext, arr, *, impl):
  del eval_ctx
  return olympus.random.wrap_key_data(arr, impl=impl)
...
def _iota_eval_rule(
  ...
  local_iota = olympus.lax.broadcasted_iota(dtype, iota_shape, dim_)
...
def _reduce_sum_eval_rule(
  ...
  return olympus.lax.reduce_sum(x, axes=axes)
...
def _jit_usage_rule(
    ctx, used_out: list[set[Usage]], *, olympuspr: core.ClosedOlympuspr, **_
):
  read_usage_env = compute_usage(olympuspr.olympuspr, used_out)
  in_usages = util.safe_map(read_usage_env, olympuspr.olympuspr.invars)
  return in_usages


@register_eval_rule(pjit.jit_p)
def _jit_eval_rule(ctx: KernelEvalContext, *args, olympuspr, **kwargs):
  olympuspr, consts = olympuspr.olympuspr, olympuspr.consts
  if consts:
    raise NotImplementedError('pjit with consts not supported yet')
  out_tree = tree_util.tree_structure(tuple(olympuspr.outvars))
  in_tree = tree_util.tree_structure((tuple(olympuspr.invars), {}))
...
def _jit_pull_block_spec_rule(
    ctx: PullRuleContext, out_block_specs, *, olympuspr, **kwargs
):
  olympuspr, consts = olympuspr.olympuspr, olympuspr.consts
  if consts:
    raise NotImplementedError('pjit with consts not supported yet')
...
def _custom_jvp_call_usage_rule(
    ctx, used_out: list[set[Usage]], *, call_olympuspr: core.ClosedOlympuspr, **_
):
  del ctx
  read_usage_env = compute_usage(call_olympuspr.olympuspr, used_out)
  in_usages = util.safe_map(read_usage_env, call_olympuspr.olympuspr.invars)
  return in_usages


@register_eval_rule(custom_derivatives.custom_jvp_call_p)
def _custom_jvp_call_eval_rule(
    ctx: KernelEvalContext, *args, call_olympuspr: core.ClosedOlympuspr, **kwargs
):
...
def _custom_jvp_call_pull_block_spec_rule(
    ctx: PullRuleContext, out_block_specs, *, call_olympuspr, **kwargs
):
  olympuspr, consts = call_olympuspr.olympuspr, call_olympuspr.consts
  if consts:
    raise NotImplementedError('custom_jvp_call with consts not supported yet')
...
@register_pull_block_spec_rule(hiolympus.call_hi_primitive_p)
...
@register_eval_rule(hiolympus.call_hi_primitive_p)
def _custom_call_hi_primitive_eval_rule(
    ctx: KernelEvalContext, *args, prim
):
  return olympus.tree.leaves(prim.block_eval_rule(ctx, *args))
...
def push_block_spec(
  ...
  def wrapper(*args, **kwargs):
    ...
    olympuspr, _, in_tree, out_tree = fuser_utils.make_olympuspr(f, *args, **kwargs)
    if in_tree != in_tree_:
      raise ValueError(f'Expected {in_tree} PyTree, got {in_tree_}')
    out_bs = _push_block_spec_olympuspr(olympuspr, *flat_block_specs)
    return tree_util.tree_unflatten(out_tree, out_bs)

  return wrapper


def _push_block_spec_olympuspr(
    olympuspr: core.Olympuspr,
    *flat_block_specs,
) -> tuple[pallas_core.BlockSpec, ...]:
  num_inputs = len(olympuspr.invars)
  ...
  for invar, bs in zip(olympuspr.invars, flat_block_specs, strict=True):
    env[invar] = bs
  for constvar in olympuspr.constvars:
  ...
  out_block_specs = tuple(util.safe_map(_read_block_spec, olympuspr.outvars))
  valid_block_spec = [
...
def _custom_jvp_call_push_rule(
    ctx, *block_specs, call_olympuspr: core.ClosedOlympuspr, **_
):
  assert not call_olympuspr.consts
  return _push_block_spec_olympuspr(call_olympuspr.olympuspr, *block_specs)
...
def _custom_vjp_call_push_rule(
  ...
  del ctx, num_consts, fwd_olympuspr_thunk, bwd, out_trees, symbolic_zeros
  return _push_block_spec_olympuspr(call_olympuspr.olympuspr, *block_specs)

@register_push_block_spec_rule(hiolympus.call_hi_primitive_p)
...
def _pjit_push_rule(ctx, *block_specs, olympuspr: core.ClosedOlympuspr, **_):
  assert not olympuspr.consts
  return _push_block_spec_olympuspr(olympuspr.olympuspr, *block_specs)
