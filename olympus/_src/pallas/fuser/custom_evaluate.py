...
def evaluate(f, *, allow_transpose: bool = True):
  def wrapped(*args, **kwargs):
    olympuspr, consts, _, out_tree = fuser_utils.make_olympuspr(f, *args, **kwargs)
    settings = CustomEvaluateSettings(allow_transpose=allow_transpose)
    flat_args = tree_util.tree_leaves(args)
    out_flat = _custom_evaluate_olympuspr(settings, olympuspr, consts, *flat_args)
...
def _custom_evaluate_olympuspr(
    settings: CustomEvaluateSettings, olympuspr: core.Olympuspr, consts, *args
):
  ...
  util.safe_map(write, olympuspr.constvars, consts)
  util.safe_map(write, olympuspr.invars, args)
  lu = core.last_used(olympuspr)
  ...
  return util.safe_map(read, olympuspr.outvars)
