...
from olympus._src.pallas.mosaic import lowering as mosaic_lowering
...
class custom_fusion:
  ...
  @functools.partial(api_boundary,
                     repro_api_name="olympus.pallas.custom_fusion.__call__")
  def __call__(self, *args, **kwargs):
    ...
    # if a Pallas implementation was provided, get its olympuspr
    if self.pallas_impl is not None:
      ...
      del pallas_out_tree
      pallas_olympuspr, _, pallas_consts = (
          pe.trace_to_olympuspr_dynamic(flat_pallas_impl, in_avals))
    else:
      pallas_olympuspr = None
      pallas_consts = []
    ...
    out_flat = custom_fusion_p.bind(
        ...
        *args_flat,
        ...
        pallas_olympuspr=pallas_olympuspr,
...
def _custom_fusion_impl(
  ...
  return core.eval_olympuspr(olympuspr, consts, *args)
...
def _custom_fusion_effectful_abstract_eval(
    *args,
    olympuspr: core.Olympuspr,
    pallas_olympuspr: core.Olympuspr | None,
    **_):
  del args
  # TODO(jburnim): Error if pallas_olympuspr has different number of outputs, or
  # different shapes and types of outputs?
  if olympuspr.effects:
    raise NotImplementedError(
        "custom_fusion-decorated function {olympuspr.debug_info.func_src_info} "
        "has effects, which is not yet supported: {olympuspr.effects}")
  if pallas_olympuspr is not None and pallas_olympuspr.effects:
    raise NotImplementedError(
        "custom_fusion-decorated function {olympuspr.debug_info.func_src_info} "
        "has a pallas_impl with effects, which is not yet supported: "
        f"{pallas_olympuspr.effects}")
...
def _custom_fusion_mosaic_lowering_rule(
  ...
  if pallas_olympuspr is None:
    pallas_olympuspr = olympuspr
    pallas_consts = consts
  lowering_context = ctx.lowering_context.replace(block_shapes=ctx.block_shapes)
  return mosaic_lowering.olympuspr_subcomp(
      lowering_context, pallas_olympuspr, *pallas_consts, *args)
...
def _custom_fusion_usage_rule(
  ...
  del ctx
  # TODO(jburnim): Error if olympuspr.olympuspr gives different usage than pallas_olympuspr?
  read_usage_env = block_spec_lib.compute_usage(olympuspr, used_out)
  return util.safe_map(read_usage_env, olympuspr.invars)
