...
from olympus._src.pallas import utils as pl_utils
...
def empty_like(x: object):
  ...
  See Also:
    :func:`olympus.lax.empty`
  """
  return tree_util.tree_map(lambda leaf: empty(leaf.shape, leaf.dtype), x)


def empty_ref_like(x: object) -> olympus_typing.Array:
  ...
  return olympus_core.new_ref(empty_like(x), memory_space=memory_space)


def when(
    condition: bool | olympus_typing.ArrayLike, /
  ...
  Args:
    condition: If a boolean, this is equivalent to ``if condition: f()``. If an
      array, ``when`` produces a :func:`olympus.lax.cond` with the decorated
...
def loop(
    lower: olympus_typing.ArrayLike,
    upper: olympus_typing.ArrayLike,
    *,
    step: olympus_typing.ArrayLike = 1,
    unroll: int | bool | None = None,
) -> Callable[[Callable[[olympus_typing.Array], None]], None]:
  """Returns a decorator that calls the decorated function in a loop."""
  zero: olympus_typing.ArrayLike
  if not all(map(olympus_core.is_concrete, (lower, upper, step))):
...
_ENABLE_DEBUG_CHECKS = config.bool_state(
    "olympus_pallas_enable_debug_checks",
    default=False,
    help=(
...
def debug_check(condition, message):
  ...
  :func:`~olympus.experimental.pallas.enable_debug_checks` is set, otherwise
  do nothing.
  """
...
def _make_kernel(body,
  ...
  def wrapper(*operands):
    arg_refs = tree_util.tree_map(olympus_core.new_ref, operands)
    out_refs = tree_util.tree_map(
        lambda out: olympus_core.new_ref(
            lax.empty(out.shape, out.dtype),
            memory_space=(
                ms
                if hasattr(out, "memory_space")
                and not isinstance(
                    ms := out.memory_space, olympus_core.MemorySpace
                )
                else None
            ),
        ),
...
def kernel(body: Callable | api.NotSpecified = api.NotSpecified(),  # pylint: disable=g-bare-generic
  ...
  Args:
    ...
    out_shape: The shape of the output. Should be a PyTree of
      ``olympus.ShapeDtypeStruct`` or ``olympus.Array`` s.
