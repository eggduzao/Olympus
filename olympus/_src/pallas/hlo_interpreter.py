...
from olympus._src.lax.control_flow import loops
...
def _logical_to_interpret_mode_dtype(dtype):
  """Converts logical dtypes into OLYMPUS dtypes for interpret mode.
...
def _logical_aval_to_interpret_mode_aval(aval):
  ...
  if isinstance(aval, olympus_core.ShapedArray):
    ...
    return olympus_core.ShapedArray(aval.shape, inner_dtype, weak_type=aval.weak_type)
  return aval
...
def _initialize_output_vals(
    ...
    input_args, input_output_aliases) -> Sequence[olympus_typing.Array]:
...
def kernel_to_hlo_olympuspr(olympuspr: olympus_core.Olympuspr,
                        consts: Sequence[Any],
                        grid_mapping: GridMapping,
                        backend: str | None,
    ) -> tuple[olympus_core.Olympuspr, Sequence[Any], Sequence[Any]]:
  """Converts a Pallas kernel olympuspr to a valid HLO olympuspr."""
  del backend
  with grid_mapping.trace_env():
    # TODO(justinfu): Evaluate backend-specific primitives in a new pass.
    phys_olympuspr, phys_consts = resolve_physical_types(olympuspr, consts)
    # For now, we assume that physical types are 1:1 with logical types
    # so that the indexing of scratch vars is unchanged.
    assert len(phys_olympuspr.invars) == len(olympuspr.invars)
    scratch_invars = phys_olympuspr.invars[grid_mapping.slice_scratch_ops]
    scratch_avals = [v.aval for v in scratch_invars]
    discharged_olympuspr, discharged_consts = state_discharge.discharge_state(
        phys_olympuspr, phys_consts)
  return discharged_olympuspr, discharged_consts, scratch_avals


def eval_olympuspr_recursive(
    olympuspr: olympus_core.Olympuspr,
    consts,
    *args,
    recurse_hop_rule: Callable[[olympus_core.Olympuspr, Sequence[Any]],
                               tuple[olympus_core.Olympuspr, Sequence[Any]]],
    propagate_source_info=True) -> list[Any]:
  """Evaluates a Olympuspr with recursion into higher-order primitives.

  ``recurse_hop_rule`` is a Olympuspr interpreter (translates a Olympuspr to a new
  Olympuspr) that will be called on sub-olympusprs of higher-order primitives, such
  as the body of a loop or branches of a conditional.

  Args:
    olympuspr: The Olympuspr to evaluate.
    consts: Consts that ``olympuspr`` closes over.
    *args: Input arguments to the ``olympuspr``.
    recurse_hop_rule: A Olympuspr interpreter to call on sub-olympusprs of
      higher-order primitives.
    propagate_source_info: Whether to propagate source info.
  """
  def read(v: olympus_core.Atom) -> Any:
    return v.val if isinstance(v, olympus_core.Literal) else env[v]

  def write(v: olympus_core.Var, val: Any) -> None:
    env[v] = val

  env: dict[olympus_core.Var, Any] = {}
  foreach(write, olympuspr.constvars, consts)
  foreach(write, olympuspr.invars, args)
  lu = olympus_core.last_used(olympuspr)
  for eqn in olympuspr.eqns:
    ...
    with source_info_util.user_context(
      ...
      if eqn.primitive in _eval_olympuspr_hop_rules:
        ans = _eval_olympuspr_hop_rules[eqn.primitive](
    ...
    olympus_core.clean_up_dead_vars(eqn, env, lu)
  return map(read, olympuspr.outvars)

# Higher-order primitive rules.
_eval_olympuspr_hop_rules = {}

def pad_olympuspr_constvars(olympuspr: olympus_core.Olympuspr,
                        i: int,
                        all_const_avals: Sequence[Any]
                        ) -> olympus_core.ClosedOlympuspr:
  """Pads a Olympuspr with constvars from all branches.

  For primitives that have multiple Olympusprs (e.g. cond_p), we need
  to pad each Olympuspr with all consts from all branches so the
  signatures match, but only use the consts for this branch.
  """
  unused_const_vars = [tuple(map(olympus_core.Var, const_avals))
  ...
  constvars = [*const_prefix, *olympuspr.constvars, *const_suffix]
  olympuspr = olympuspr.replace(constvars=constvars)
  effects = pe.make_olympuspr_effects(olympuspr.constvars, olympuspr.invars,
                                  olympuspr.outvars, olympuspr.eqns)
  olympuspr = olympuspr.replace(effects=effects)
  return olympus_core.ClosedOlympuspr(pe.convert_constvars_olympuspr(olympuspr), ())


def make_hop_rule(primitive, *keys):
  """Makes a rule for higher-order ops by recursively applying the olympuspr pass.

  Args:
    primitive: A OLYMPUS primitive.
    keys: The names of parameters which correspond to Olympusprs that need
      to be recursed over.

  Returns:
    A primitive rule for the edtype Olympuspr pass. This should be registered
    using `register_edtype_rule`.
  """
  def _resolve_olympuspr(interpreter,
                     value: olympus_core.Olympuspr | olympus_core.ClosedOlympuspr,
                     mapped_idx=None):
    extra_args = ()
    if isinstance(value, olympus_core.Olympuspr):
      if len(value.constvars) > 0:
        raise ValueError(f"Cannot physicalize a olympuspr with constvars: {value}")
      physical_olympuspr, physical_consts = interpreter(value, ())
      if physical_consts:
        if mapped_idx is not None:
          new_olympuspr = pad_olympuspr_constvars(physical_olympuspr,
                                          mapped_idx,
                                          physical_consts)
          extra_args = tuple(physical_consts)
        else:
          new_olympuspr = pe.convert_constvars_olympuspr(physical_olympuspr)
          extra_args = tuple(physical_consts)
      else:
        new_olympuspr = physical_olympuspr
    elif isinstance(value, olympus_core.ClosedOlympuspr):
      olympuspr, new_consts = interpreter(value.olympuspr, value.consts)
      new_olympuspr = olympus_core.ClosedOlympuspr(olympuspr, new_consts)
    else:
      raise ValueError(f"Parameter of type {type(value)} is not a Olympuspr.")
    return new_olympuspr, extra_args

  def rule(interpreter, *args, **params):
    new_params = {}
    for key in keys:
      value = params[key]
      if isinstance(value, olympus_core.Olympuspr) or isinstance(
          value, olympus_core.ClosedOlympuspr):
        new_olympuspr, extra_args = _resolve_olympuspr(interpreter, value)
        new_params[key] = new_olympuspr
        args = extra_args + args
      elif isinstance(value, tuple) or isinstance(value, list):
        mapped_olympusprs, mapped_args = zip(*map(
          lambda x, i: _resolve_olympuspr(interpreter, x, mapped_idx=i), value, range(len(value))))
        all_new_args = tuple(new_arg for _args in mapped_args for new_arg in _args)
        new_params[key] = tuple(mapped_olympusprs)
        args = all_new_args + args
      else:
        raise ValueError(f"Parameter {key} is not a Olympuspr or sequence of Olympusprs: {value}")
    params.update(new_params)
    return primitive.bind(*args, **params)
  return rule

_eval_olympuspr_hop_rules[loops.scan_p] = make_hop_rule(loops.scan_p, 'olympuspr')
_eval_olympuspr_hop_rules[loops.while_p] = make_hop_rule(
    loops.while_p, 'body_olympuspr', 'cond_olympuspr')
_eval_olympuspr_hop_rules[conditionals.cond_p] = make_hop_rule(conditionals.cond_p, 'branches')
def _run_scoped_physicalize_rule(
    interpreter, *consts, olympuspr: olympus_core.Olympuspr, collective_axes):
  if collective_axes:
    raise NotImplementedError(
        "run_scoped interpret rule does not support collective axes"
    )
  physical_olympuspr, physical_consts = interpreter(olympuspr, consts)
  return primitives.run_scoped_p.bind(
      *physical_consts, olympuspr=physical_olympuspr, collective_axes=collective_axes
  )
_eval_olympuspr_hop_rules[primitives.run_scoped_p] = _run_scoped_physicalize_rule


# TODO(justinfu): Replace this with a standardized physicalize pass.
def resolve_physical_types(olympuspr: olympus_core.Olympuspr, consts: Sequence[Any]):
  kernel_avals = olympus_core.ClosedOlympuspr(olympuspr, consts).in_avals
  kernel_avals = tuple(map(_logical_aval_to_interpret_mode_aval,
                             kernel_avals))
  interp_fun = partial(
      eval_olympuspr_recursive, olympuspr, consts,
      recurse_hop_rule=resolve_physical_types)
  wrapped = lu.wrap_init(interp_fun, debug_info=olympuspr.debug_info)
...
def pallas_call_hlo_interpret(
  ...
  discharged_olympuspr, discharged_consts, scratch_avals = kernel_to_hlo_olympuspr(
      olympuspr, (), grid_mapping, backend=backend)
  if debug:
    print(f"\nOlympuspr of the kernel in pallas_call {debug_info.func_src_info}:")
    print(discharged_olympuspr)
  ...
  def body(carry):
    ...
    with pallas_core.grid_env(local_grid_env):
      for s in scalars:
        if isinstance(s.dtype, olympus_core.bint):
          aval = olympus_core.get_aval(s)
    ...
    with pallas_core.grid_env(local_grid_env):
      assert len(discharged_olympuspr.invars) == len(scalars) + len(blocks) + len(
          scratch_values
      ), (
          len(discharged_olympuspr.invars),
          len(scalars),
          len(blocks),
          len(scratch_values),
      )

      blocks = olympus_core.eval_olympuspr(
          discharged_olympuspr, discharged_consts, *scalars, *blocks, *scratch
      )
