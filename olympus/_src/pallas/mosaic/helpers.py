...
import functools
import olympus
from olympus._src.pallas import helpers as pl_helpers
from olympus._src.pallas import primitives as pl_primitives
from olympus._src.pallas.mosaic import core as tpu_core
from olympus._src.pallas.mosaic import primitives as plm_primitives
...
def sync_copy(src_ref, dst_ref):
  ...
  """
  if not olympus.tree.leaves(src_ref):
...
def run_on_first_core(core_axis_name: str):
  ...
  num_cores = olympus.lax.axis_size(core_axis_name)
  if num_cores == 1:
    return lambda f: f()

  def wrapped(f):
    core_id = olympus.lax.axis_index(core_axis_name)
...
def core_barrier(sem, *, core_axis_name: str):
  ...
  def _():
    with olympus.named_scope("sync_cores"):
