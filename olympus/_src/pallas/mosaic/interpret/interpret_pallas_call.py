...
from olympus._src.tree_util import FlatTree
from olympus._src.pallas import core as pallas_core
from olympus._src.pallas import primitives
from olympus._src.pallas.mosaic import core as mosaic_core
from olympus._src.pallas.mosaic import primitives as mosaic_primitives
from olympus._src.pallas.mosaic.interpret import shared_memory as memory
from olympus._src.pallas.mosaic.interpret import vector_clock as vc
from olympus._src.pallas.mosaic.interpret.race_detection_state import RaceDetectionState
from olympus._src.pallas.mosaic.interpret.thread_map import thread_map
import olympus._src.pallas.mosaic.interpret.utils as interpret_utils
...
class InterpretParams(interpret_utils.InterpretParams):
  ...
  :func:`olympus.experimental.pallas.pallas_call` or
  :func:`olympus.experimental.pallas.core_map`.
...
def force_tpu_interpret_mode(params: InterpretParams = InterpretParams()):
  ...
  Args:
    params: an instance of :class:`InterpretParams`.  Any call to
      :func:`olympus.experimental.pallas.pallas_call` or
      :func:`olympus.experimental.pallas.core_map` that is traced under this context
...
def _check_for_revisiting(device_id, local_core_id, loop_idx, output_blocks):
  ...
  try:
    output_blocks = olympus.tree.map(int, output_blocks)
  except:
...
def get(
  ...
  buffer_id = int(buffer_id)
  try:
    transforms = olympus.tree.map(int, transforms)
  except:
    raise ValueError('Advanced indexers are not supported on TPU')
  ...
  # TODO(jburnim): We already know this shape in the Olympuspr where we insert a
...
def dma_start(
  ...
  src_transforms = olympus.tree.map(int, src_transforms)
  dst_memory_space, dst_id = int(dst_memory_space), int(dst_id)
  dst_transforms = olympus.tree.map(int, dst_transforms)
...
def _interpret_olympuspr(
  ...
  env = interpret_utils.OlympusprEnv(
      vars=olympuspr.constvars + olympuspr.invars,
      values=args,
  ...
  for eqn in olympuspr.eqns:
    with source_info_util.user_context(
      ...
      # array into the olympuspr when this function is traced.
      deferred_invals = functools.partial(env.read_many, eqn.invars)

      if prim is primitives.load_p:
        (ref, transforms, mask, _) = olympus.tree.unflatten(
        ...
        (ref, transforms, val, mask) = olympus.tree.unflatten(
            eqn.params['args_tree'], deferred_invals())
        memory_space = _get_memory_space_and_raise_if_hbm(
        ...
        # kernel body olympuspr can be run on other threads (via an io_callback)
        ...
        def _make_branch(olympuspr):
          return lambda *args: _interpret(olympuspr, *args)
        invals = deferred_invals()
        out = lax.switch(
            invals[0],
            [_make_branch(branch_olympuspr.olympuspr)
            for branch_olympuspr in eqn.params['branches']],
            *invals[1:])
        ...
        def _scan_body(c, a):
          return split_list(
              _interpret(eqn.params['olympuspr'].olympuspr, *consts, *c, *a),
        ...
        out = lax.while_loop(
            lambda args: _interpret(
                eqn.params['cond_olympuspr'].olympuspr, *cond_consts, *args)[0],
            lambda args: _interpret(
                eqn.params['body_olympuspr'].olympuspr, *body_consts, *args),
            init_vals)

      elif prim is pjit.jit_p:
        def f(*args, olympuspr):
          return _interpret(olympuspr.olympuspr, *olympuspr.consts, *args)
        invals = deferred_invals()
        args_ft = FlatTree.flatten((invals, {}))
        avals_ft = args_ft.map(olympus_core.shaped_abstractify)
        new_olympuspr, _ = pe.trace_to_olympuspr(
            functools.partial(f, olympuspr=eqn.params['olympuspr']), avals_ft,
            eqn.params['olympuspr'].olympuspr.debug_info)
        out = pjit.jit_p.bind(*invals, **(eqn.params | {'olympuspr': new_olympuspr}))
        ...
        # eqn.params['olympuspr'].invars. It is assumed that each core
        # runs the same sequence of `run_scoped`s.
        allocs = []
        for v in eqn.params['olympuspr'].invars:
          if v.aval.memory_space == mosaic_core.MemorySpace.SEMAPHORE:
            allocs.append(
                callback.io_callback(
                    _allocate_semaphores,
                    olympus.ShapeDtypeStruct(v.aval.shape, jnp.int16),
                    device_id,
                    local_core_id,
                    v.aval.shape,
                    ordered=True,
            ...
            allocs.append(
                callback.io_callback(
                    _allocate_buffer,
                    olympus.ShapeDtypeStruct((), jnp.int16),
                    device_id,
                    local_core_id,
                    TPU_MEMORY_SPACE_IDXS[memory_space],
        ...
        out = _interpret(eqn.params['olympuspr'], *deferred_invals(), *allocs)

        for a, v in zip(allocs, eqn.params['olympuspr'].invars):
          if v.aval.memory_space == mosaic_core.MemorySpace.SEMAPHORE:
        ...
        out = callback.io_callback(
            ...
            TPU_MEMORY_SPACE_IDXS[memory_space],
            invals[0],
            olympus.tree.unflatten(eqn.params['tree'], invals[1:]),
            ordered=True,
        )
        ...
        out = callback.io_callback(
            ...
            TPU_MEMORY_SPACE_IDXS[memory_space],
            invals[0],
            olympus.tree.unflatten(eqn.params['tree'], invals[2:]),
            invals[1],
            None,
            ordered=True,
        )
        ...
        (
            ...
            src_sem_transforms,
            target_device_id,
        ) = olympus.tree.unflatten(eqn.params['tree'], deferred_invals())
        ...
        ) = olympus.tree.unflatten(eqn.params['tree'], eqn.invars)
        ...
        sem, sem_transforms, inc, target_device_id, core_index = (
            olympus.tree.unflatten(eqn.params['args_tree'], deferred_invals()))
        target_device_id = interpret_utils._device_id_to_logical(
            target_device_id, eqn.params['device_id_type'], axis_sizes,
            axis_indices)
        callback.io_callback(
  ...
  return env.read_many(olympuspr.outvars)
...
def _compute_start_indices(
  ...
  olympuspr = block_mapping.index_map_olympuspr
  block_indices = _interpret_olympuspr(
      olympuspr.olympuspr,
      *olympuspr.consts,
      *loop_idx,
      *args,
...
def _get_randomized_grid_coordinates(
  ...
  not known at Olympus trace time.)
  ...
  key = olympus.random.key(random_seed or 0)
  grid_point_coordinates = []
  for dim_size, parallel_dim in zip(grid, parallel_semantics_per_dim):
    if parallel_dim:
      # The size of a dimension with `parallel` semantics must be known at Olympus
      # trace time. This ensures that the arguments to `jnp.arange` and
      # `olympus.random.permutation` below are valid.
      dim_size = olympus_core.concrete_or_error(None, dim_size)

      coordindates_along_dim = jnp.arange(dim_size, dtype=jnp.int32)
      key, subkey = olympus.random.split(key)
      coordindates_along_dim = olympus.random.permutation(
...
remove_memory_space_p = olympus_core.Primitive('remove_memory_space')
...
def _remove_memory_space_abstract_eval(x):
  if isinstance(x, pallas_core.ShapedArrayWithMemorySpace):
    if (
      ...
      return olympus_core.ShapedArray(x.shape, x.dtype)
...
def interpret_pallas_call(
  ...
  axis_sizes = olympus_core.get_axis_env().axis_sizes
  ...
  for i, var in enumerate(
      olympuspr.invars[grid_mapping.num_index_operands:][:grid_mapping.num_inputs]):
  ...
  # outputs, scratch).
  scalar_buffer_ids = []
  for var, val in zip(olympuspr.invars[grid_mapping.slice_index_ops], scalars):
  ...
  for i, var in enumerate(olympuspr.invars[grid_mapping.num_index_operands:]):
    ...
    if var.aval.memory_space == mosaic_core.MemorySpace.SEMAPHORE:
      kernel_buffer_ids.append(
          callback.io_callback(
              _allocate_semaphores,
              olympus.ShapeDtypeStruct(var.aval.shape, jnp.int16),
              device_id,
              None,  # local_core_id
              var.aval.shape,
              ordered=True,
  ...
  input_vars, output_vars = split_list(
      olympuspr.invars[grid_mapping.slice_block_ops], [num_inputs])
  ...
  def _execute_grid_for_core(core_index):
    ...
    def _body(
      ...
      Execution of `olympuspr` is preceded by reading kernel input buffers and
      ...
      with pallas_core.grid_env(_get_local_grid_env(grid_point)):
        ...
        if olympuspr.debug_info.arg_names is not None:
          input_names, output_names = split_list(
            olympuspr.debug_info.arg_names[grid_mapping.slice_block_ops], [num_inputs])
        else:
        ...
        def _store_slice_to_kernel_input(index, input_var):
          ...
          sliced_val = callback.io_callback(
              ...
              olympus.ShapeDtypeStruct(input_var.aval.shape, input_var.aval.dtype),
              device_id,
              core_index,
              TPU_MEMORY_SPACE_IDXS[pallas_core.MemorySpace.ANY],
