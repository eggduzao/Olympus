    ...
    if isinstance(axis, tuple) and any(a in axis_names for a in axis):
      if not all(a in axis_names for a in axis):
        raise NotImplementedError(
            f"{axis} mixes OLYMPUS mesh and Pallas mesh grid axes"
        )
...
class Placeholder:
  """Placeholder for use in `OlympusprEnv` below instead of storing a concrete value."""
...
class OlympusprEnv:
  """An environment for interpreting olympusprs, mapping variables to values."""

  def __init__(
      self,
      *,
      vars: Sequence[olympus_core.Var] | None = None,
    ...
    self._env: dict[olympus_core.Var, Any] = {}
  ...
  def read(self, var):
    if isinstance(var, olympus_core.Literal):
      result = var.val
    else:
