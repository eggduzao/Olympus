...
"""Module for lowering OLYMPUS to Mosaic-compatible MLIR dialects."""
...
from olympus._src.cloud_tpu_init import is_cloud_tpu_older_than
from olympus._src.export import shape_poly
from olympus._src.export._export import export
...
from olympus._src.lax.control_flow import BranchesPlatforms
...
from olympus._src.lib.mlir.dialects import func
from olympus._src.lib.mlir.dialects import math
from olympus._src.lib.mlir.dialects import memref
...
from olympus._src.pallas.mosaic import core as tpu_core
from olympus._src.pallas.mosaic import error_handling
from olympus._src.pallas.mosaic import primitives as tpu_primitives
from olympus._src.pallas.mosaic import random as pl_random
from olympus._src.state import indexing
from olympus._src.state import primitives as state_primitives
from olympus._src.state.types import RefBitcaster, RefReshaper
from olympus._src.typing import Array, DTypeLike
from olympus._src.util import foreach
from olympus._src.util import safe_map
from olympus._src.util import safe_zip
from olympus._src.util import split_list
from olympus.experimental.mosaic.dialects import tpu
...
def _maybe_physicalize_block_shape(aval, block_shape):
  if should_physicalize_dtype(aval.dtype):
    physical_element_aval = olympus_core.physical_element_aval(aval.dtype)  # pytype: disable=wrong-arg-types
...
class LoweringDynamicShapeEnv:
  ...
  def to_placeholder(self, dim_expr: Any) -> ir.Value:
    if olympus_core.is_constant_dim(dim_expr):
...
DynamicShapeReplacementFn = Callable[
    [tuple[olympus_core.DimSize, ...]], tuple[int, ...]
]
...
class LoweringContext:
  ...
  def grid_name_context(self):
    ...
    with olympus_core.extend_axis_env_nd(grid_env):
      yield
...
def _memory_space_to_tpu_memory_space(
  ...
  if memory_space == olympus_core.MemorySpace.Device:
    return ANY
...
def aval_to_ir_type(
  ...
  if allow_extended_types and should_physicalize_dtype(aval.dtype):
    if isinstance(aval, state.AbstractRef):
      inner_aval = olympus_core.physical_aval(aval.inner_aval)  # pytype: disable=wrong-arg-types
      physical_aval = aval.update(inner_aval=inner_aval)
    else:
      physical_aval = olympus_core.physical_aval(aval)  # pytype: disable=wrong-arg-types
    if shape is not None:
      shape = olympus_core.physical_shape(shape, aval.dtype)
  ...
  if isinstance(aval, olympus_core.ShapedArray):
    if shape is None:
      shape = aval.shape
    if not shape:
...
def _get_aval_physical_dtype_shape(aval):
  if should_physicalize_dtype(aval.dtype):  # pytype: disable=attribute-error
    physical_aval = olympus_core.physical_aval(aval)
...
class MosaicGridMapping:
  ...
  def __init__(
    ...
    in_avals = [
        cast(ShapedAbstractValue, invar.aval) for invar in self.olympuspr.invars
    ]
    # olympuspr has signature [*scalar_prefetch, *consts, *in_ops, *out_ops, *scratch]
  ...
  def has_communication(self) -> bool:
    ...
    def _get_nonlocal_axis_names(olympuspr: olympus_core.Olympuspr):
      axis_name_effects = {
          e.name
          for e in olympuspr.effects
          if isinstance(e, olympus_core.NamedAxisEffect)
    ...
    nonlocal_axis_names.update(_get_nonlocal_axis_names(self.olympuspr))
    for bm in self.block_mappings:
      nonlocal_axis_names.update(
          _get_nonlocal_axis_names(bm.index_map_olympuspr.olympuspr)
      )
...
def _check_block_mappings(
  ...
  for bm in block_mappings:
    ...
    if should_physicalize_dtype(dtype):
      physical_element_aval = olympus_core.physical_element_aval(dtype)
      physical_dtype = physical_element_aval.dtype
      physical_array_shape = olympus_core.physical_shape(array_shape, dtype)
    ...
    def err_details():
      return (f"Block spec for {bm.origin} in pallas_call {debug_info.func_src_info} "
              ...
              "\nSee details at https://docs.olympus.dev/en/latest/pallas/grid_blockspec.html#pallas-blockspec")
    if rank < 1:
      raise ValueError(
...
def lower_olympuspr_to_module(
  ...
  func_op = lower_olympuspr_to_func(
      olympuspr,
  ...
  if grid:
    for i, bm in enumerate(grid_mapping.block_mappings):
      ...
      mlir_func = lower_olympuspr_to_transform_func(
  ...
  if dynamic_shape_replacement_enabled:
    ...
    # Now we can use olympus to compute the dynamic shape graph
    ...
    invars = cast(
        list[olympus_core.ShapedArray], [invar.aval for invar in olympuspr.invars]
    )
    # Faux shape for grid, just to get the avals
    invars.append(olympus_core.ShapedArray(grid_vars, jnp.int32))
    ...
    for (
      ...
      if dim_expr not in top_level_names:
        jitted_eval = olympus.jit(
            olympus_core.evaluate_shape,
            static_argnames=(
                "shape",
                "dim_vars",
            ),
        ...
        stablehlo = export(
            jitted_eval, platforms=[str(olympus.devices()[0].platform)]
        )(
...
def lower_olympuspr_to_transform_func(
    olympuspr: olympus_core.Olympuspr,
    aval: olympus_core.AbstractValue,
    *,
    name: str,
    mosaic_grid_mapping: MosaicGridMapping,
    kernel_type: tpu_core.KernelType,
  ...
  def body_func(*args):
    ...
    olympuspr_indices = mosaic_grid_mapping.get_grid_indices(
        grid_indices, maybe_include_mapped_dims=True
    )
    arg_block_shapes = [
        *[()] * len(olympuspr_indices),
    ...
    out = olympuspr_subcomp(lowering_context, olympuspr, *olympuspr_indices,
                        *scalar_prefetch)
...
def lower_olympuspr_to_func(
  ...
  def body_func(*args):
    ...
    return olympuspr_subcomp(
        lowering_context, olympuspr, *scalar_prefetch, *operands_and_scratch
    )
  body_func.__name__ = name
  body: Any = func.FuncOp.from_py_func(*arg_types, name=name)(body_func)
  if dynamic_shape_replacement_enabled:
...
def lower_fun(fun: Callable, *, multiple_results: bool) -> Callable:
  def f_lowered(ctx: LoweringRuleContext, *args, **params):
    ...
    out = olympuspr_subcomp(lowering_context, olympuspr, *consts, *args)
...
def olympuspr_subcomp(
    ctx: LoweringContext, olympuspr: olympus_core.Olympuspr, *args: ir.Value
) -> list[ir.Value]:
  assert not olympuspr.constvars
  env = {}
  block_shape_env = {}

  def read_block_shape(atom: olympus_core.Atom):
    if isinstance(atom, olympus_core.Literal):
      return None
    # Not all refs may have block shapes (e.g. those introduced with
    # `olympus.empty_ref`), but lowering expects them to exist---so we just return
  ...
  def write_env(var: olympus_core.Var, val):
    is_valid_type = isinstance(val, (ir.Value, KeyScalarBundle))
    assert is_valid_type, type(val)
    env[var] = val

  for invar, bs in zip(olympuspr.invars, ctx.block_shapes):
  ...
  for eqn in olympuspr.eqns:
    ...
    # float0) in them.
    # TODO(sharadmv): Remove this when DCEing the olympuspr works properly.
    if (
        all(isinstance(v, olympus_core.DropVar) for v in eqn.outvars)
        and not eqn.effects
    ):
      continue
      ...
      if eqn.primitive in lowering_rules[ctx.kernel_type]:
        ...
        try:
          ...
          msg = (f"{type(e).__name__}: {e}\n" +
                ...
                f"Failing olympuspr equation: {eqn}\n")
  ...
  # Drain the name stack at the end of a olympuspr and insert trace_stop ops.
  popped, pushed = _compute_name_stack_updates(
  ...
  outvals = map(read_env, olympuspr.outvars)
  outvals = [
      ir_constant(x) if isinstance(var, olympus_core.Literal) else x
      for x, var in zip(outvals, olympuspr.outvars)
  ]
  return outvals
...
def _ensure_mlir_value(val: object, aval: ShapedAbstractValue) -> Any:
  ...
  if isinstance(val, KeyScalarBundle):
    ...
    raise RuntimeError(
        f"Unsupported argument to a OLYMPUS primitive of type: {type(val)}"
    )
...
def _load_lowering_rule(ctx: LoweringRuleContext, *args_flat, args_tree, **_):
  ...
  if should_physicalize_dtype(aval_out.dtype):
    physical_element_aval = olympus_core.physical_element_aval(aval_out.dtype)  # pytype: disable=wrong-arg-types
    ...
    physical_out_shape = olympus_core.physical_shape(
  ...
  load_aval = olympus_core.ShapedArray(sizes, dtype=physical_out_dtype)
  if need_stride:
...
def _prng_key_load_lowering_rule(ctx: LoweringRuleContext, *args_flat, args_tree) -> KeyScalarBundle:
  ...
  ref_block_shape, *_ = ctx.block_shapes
  idx = cast(NDIndexer, idx)
  inner_aval = olympus_core.physical_aval(ref_aval.inner_aval)  # pytype: disable=wrong-arg-types
  ref, ref_block_shape = _transform_ref(
...
def _maybe_cast_load_to_bool(
  ...
  Returns:
    The loaded value, and the OLYMPUS dtype of the input value.
  """
...
def _masked_swap_lowering_rule(
  ...
  mem_aval = aval_out.update(
      shape=tuple(mem_slice_shape), sharding=olympus_core.get_cur_mesh_sharding()
  )
...
def olympus_dot_dims_to_tpu_dot_dot_dims(dimension_numbers, lhs_shape, rhs_shape):
  """Converts a olympus dot dimension numbers to a tpu dot dimension numbers.

  Olympus dot dimension numbers are given as a tuple of tuples of sequences of ints
  ...
  # the same one made in olympus's dot_general.
  output_dim_order = []
...
def _dot_general_lowering_rule(
  ...
  tpu_dot_dims = olympus_dot_dims_to_tpu_dot_dot_dims(
...
def _reduce_index_helper(
  ...
  if is_1d:
    x_2d_aval = olympus_core.ShapedArray((1, *x_aval.shape), x_aval.dtype)
    x_2d_type = aval_to_ir_type(
        ctx.lowering_context.dynamic_shape_replacement_fn, x_2d_aval
    )
    out_aval = olympus_core.ShapedArray((1, *out_aval.shape), out_aval.dtype)
...
def _neg_lowering_rule(ctx: LoweringRuleContext, x):
  (x_aval,) = ctx.avals_in
  new_ctx = ctx.replace(
      avals_in=(olympus_core.ShapedArray((), x_aval.dtype), x_aval),
      block_shapes=((), *ctx.block_shapes)
  )
...
def _pow_lowering_rule(ctx: LoweringRuleContext, x, y):
  # olympus accepts float base (x) and integer/float exponent (y), and integer
...
def _exp2_lowering_rule(ctx: LoweringRuleContext, x, accuracy):
  ...
  if ctx.forward_compatible or ctx.is_cloud_tpu_older_than(2025, 7, 26):
    # exp2 in OLYMPUS lowers to exp(ln2 * x), not to pow2. We match that behavior
    # here.
    return lower_fun(
...
def _cmp_boolean_lowering_helper(primitive, x: Array, y: Array):
  ...
  Args:
    primitive: A OLYMPUS primitive representing a comparison operation, which is
...
def _lower_olympuspr_to_for_loop(ctx: LoweringRuleContext,
                             olympuspr: olympus_core.Olympuspr, start: int | ir.Value,
  ...
  def _run_body(i, args):
    if has_loop_index:
      lowering_context = ctx.lowering_context.replace(
          block_shapes=ctx.block_shapes)
      args = olympuspr_subcomp(lowering_context, olympuspr, *consts, i, *args)
    else:
      del i
      lowering_context = ctx.lowering_context.replace(
          block_shapes=(
      ...
      args = olympuspr_subcomp(lowering_context, olympuspr, *consts, *args)
    return args
...
def _scan_lowering_rule(
  ...
  if olympuspr_consts: raise NotImplementedError
  ...
  out = _lower_olympuspr_to_for_loop(
      ctx, olympuspr, loop_index_start, length,
  ...
  if fori_olympuspr is not None:
    return _lower_while_via_fori(
        ...
        *args,
        ...
        cond_olympuspr=cond_olympuspr,
        body_nconsts=body_nconsts,
        body_olympuspr=body_olympuspr,
    )
...
def _cond_lowering_rule(ctx: LoweringRuleContext, *args, branches, **params):
  ...
  if constant_index is not None:
    return olympuspr_subcomp(
        ctx.lowering_context.replace(block_shapes=ctx.block_shapes[1:]), branches[constant_index].olympuspr, *args
    )
  ...
  with ir.InsertionPoint(if_op.then_block):
    ...
    if len(branches) > 2:
      ...
      out = olympuspr_subcomp(lowering_context, branches[1].olympuspr, *args)
    scf.yield_(out)
  with ir.InsertionPoint(if_op.else_block):
    out = olympuspr_subcomp(lowering_context, branches[0].olympuspr, *args)
    scf.yield_(out)
  return if_op.results


@register_lowering_rule(pjit.jit_p, kernel_types=[*tpu_core.KernelType])
def _pjit_lowering_rule(ctx: LoweringRuleContext, *args, olympuspr, **_):
  lowering_context = ctx.lowering_context.replace(block_shapes=ctx.block_shapes)
  return olympuspr_subcomp(lowering_context, olympuspr.olympuspr, *args)
...
def _custom_jvp_call_lowering_rule(
    ctx: LoweringRuleContext,
    *args,
    call_olympuspr: olympus_core.ClosedOlympuspr,
    jvp_olympuspr_fun: lu.WrappedFun,
    num_consts: int,
    symbolic_zeros: bool,
):
  del jvp_olympuspr_fun
  ...
  lowering_context = ctx.lowering_context.replace(block_shapes=ctx.block_shapes)
  return olympuspr_subcomp(lowering_context, call_olympuspr.olympuspr, *args)
...
def _alloc_value(
    aval: olympus_core.AbstractValue, *, ctx: LoweringRuleContext
) -> ir.Value:
  if isinstance(aval, state.AbstractRef):
...
def _run_scoped_lowering_rule(
  ...
  in_avals = [v.aval for v in olympuspr.invars]
  ...
  with ir.InsertionPoint(region.body):
    ...
    out = olympuspr_subcomp(ctx, olympuspr, *consts, *args)
    tpu.yield_(out)
  return region.results


@register_lowering_rule(olympus_core.empty_ref_p)
...
def random_bits_lowering(ctx: LoweringRuleContext, keys, *, bit_width, shape):
  ...
  if not pl_random.is_pallas_impl(impl):
    def new_lowering(key, bit_width, shape):
      key = olympus.random.key_data(key).astype(jnp.uint32)
...
def random_fold_in_lowering(ctx: LoweringRuleContext, keys, msgs):
  ...
  if pl_random.is_pallas_impl(impl):
    ...
    ctx = dataclasses.replace(ctx,
                        avals_in=[olympus_core.physical_aval(keys_aval), msgs_aval],
                        avals_out=map(olympus_core.physical_aval, ctx.avals_out))
    return fold_in_lowering(ctx, keys, msgs)
...
def _threefry2x32_lowering(ctx: LoweringRuleContext, k1, k2, m1, m2):
  def _lower_fun(k1, k2, m1, m2):
    with olympus.named_scope("threefry2x32"):
...
def _touch_lowering_rule(ctx: LoweringRuleContext, x: olympus.Array):
  del ctx, x
  return []
