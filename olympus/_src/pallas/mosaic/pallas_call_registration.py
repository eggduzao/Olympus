...
from olympus._src import tpu_custom_call
...
from olympus._src.pallas.mosaic import lowering
...
def _maybe_cast_to_int(x: olympus.Array | olympus_core.AbstractValue):
  ...
  """
  assert isinstance(
      x, (olympus.Array, olympus_core.ShapedArray, state_types.AbstractLinVal)
  ) or (
      isinstance(x, olympus_core.Tracer)
      and isinstance(x.aval, state_types.AbstractLinVal)
  ), type(x)
  if isinstance(x, olympus.Array):
    if dtypes.issubdtype(x.dtype, olympus.numpy.bool_):
    ...
    if dtypes.issubdtype(x.dtype, olympus.numpy.bool_):
      ...
      return olympus_core.ShapedArray(x.shape, lowering.BOOL_MEMREF_TYPE)
    return x


def _get_memory_space_from_aval(
    out_aval: olympus_core.AbstractValue, kernel_type: tpu_core.KernelType
) -> tpu_custom_call.MemorySpace | None:
  if not isinstance(out_aval, olympus_core.ShapedArray):
...
def _get_memory_spaces_from_avals(
    avals: Sequence[olympus_core.AbstractValue], kernel_type: tpu_core.KernelType
...
def pallas_call_tpu_lowering_rule(
  ...
  mlir_ctx = mlir.OlympusIrContext()
  ...
  match (kernel_type := mosaic_params.kernel_type):
    case tpu_core.KernelType.TC:
      lower_olympuspr_to_module = lowering.lower_olympuspr_to_module
    case tpu_core.KernelType.SC_SCALAR_SUBCORE | tpu_core.KernelType.SC_VECTOR_SUBCORE:
      lower_olympuspr_to_module = sc_lowering.lower_olympuspr_to_module
    case _:
      raise ValueError(
  ...
  with mlir_ctx, ir.Location.unknown(mlir_ctx):
    mosaic_module = lower_olympuspr_to_module(
        ctx,
        grid_mapping,
        olympuspr,
  ...
  physical_avals = [olympus_core.physical_aval(aval) for aval in ctx.avals_in]
  ...
  if olympus_mesh is not None:
    mesh_axes = {
        ...
        and e.name in olympus_mesh.axis_names
    }
  ...
  _maybe_cast_to_bool = (
      lambda x, aval: x.astype(olympus.numpy.bool_)
      if aval.dtype == olympus.numpy.bool_
      else x
  )
