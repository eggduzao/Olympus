...
import olympus
from olympus import core as olympus_core
from olympus import lax
from olympus import tree_util
from olympus._src import util as olympus_util
from olympus._src.pallas import core as pallas_core
from olympus._src.pallas import primitives as primitives
from olympus._src.pallas.mosaic import core as tpu_core
from olympus._src.pallas.mosaic import helpers as tpu_helpers
from olympus._src.pallas.mosaic import primitives as tpu_primitives
from olympus._src.pallas.mosaic import tpu_info
...
SemaphoreTuple = olympus.Array
ArrayRef = Union[REF, olympus.Array]

GridIndices = tuple[olympus.Array, ...]
CondVal = Union[olympus.Array, bool]
...
def _round_up_to_nearest_multiple(
    s: int | olympus.Array, multiple: int
) -> int | olympus.Array:
...
def _make_block_ds(
    idx: olympus.Array | int, size: olympus.Array | int
) -> pl.Slice:
...
def _create_bounded_slice(slice_start: olympus.Array | int,
                          slice_size: olympus.Array | int,
                          block_size: int,
                          dim_size: int,
                          tiling: int | None):
...
def _make_block_slice(
    block_index: olympus.Array, block_size: pl.BlockDim | int | None, size: int,
    tiling: int | None
) -> pl.Slice | slice | int | olympus.Array:
...
def _tuples_differ(xs, ys):
  ...
  differences = olympus.tree.leaves(olympus.tree.map(lambda x, y: x != y, xs, ys))
...
def _tuple_all_binop(binop, xs, ys):
  ...
  differences = olympus.tree.leaves(olympus.tree.map(lambda x, y: binop(x, y), xs, ys))
...
class BufferedRefBase:
  ...
  def load_slots(self, predicate: bool | olympus.Array = True) -> BufferedRefBase:
    """Load slot information into registers."""
    raise NotImplementedError()

  def save_slots(self, predicate: bool | olympus.Array = True):
...
def _ref_to_value_aval(ref):
  ...
  return (
      olympus_core.ShapedArray(shape=ref.shape, dtype=ref.dtype)
      if isinstance(ref, state_types.TransformedRef)
      else olympus.typeof(ref).inner_aval
  )
...
class BufferedRef(BufferedRefBase):
  ...
  _copy_in_slot_reg: int | olympus.Array | None
  _wait_in_slot_reg: int | olympus.Array | None
  _copy_out_slot_reg: int | olympus.Array | None
  _wait_out_slot_reg: int | olympus.Array | None
  next_fetch_smem: Sequence[olympus.Array] | None
  next_fetch_sreg: Sequence[olympus.Array] | None
  ...
  def create(
    ...
    ty = (
        dtype_or_type
        if isinstance(dtype_or_type, olympus_core.AbstractValue)
        else olympus_core.ShapedArray((123, 456), dtype_or_type)
    )
  ...
  def with_next_fetch(
    self, next_fetch: Sequence[olympus.Array] | None = None,
  ):
    return dataclasses.replace(self, next_fetch_sreg=next_fetch)

  def with_slot_index(
      self,
      copy_in_slot: int | olympus.Array | None = None,
      copy_out_slot: int | olympus.Array | None = None,
      wait_in_slot: int | olympus.Array | None = None,
      wait_out_slot: int | olympus.Array | None = None,
  ) -> "BufferedRef":
  ...
  def advance_copy_in_slot(self, predicate: bool | olympus.Array = True) -> "BufferedRef":
    ...
    assert isinstance(self.copy_in_slot, olympus.Array)
    self.copy_in_slot[0] = new_current_slot
    return self

  def advance_wait_in_slot(self, predicate: bool | olympus.Array = True) -> "BufferedRef":
    ...
    assert isinstance(self.wait_in_slot, olympus.Array)
    self.wait_in_slot[0] = new_current_slot
    return self

  def advance_copy_out_slot(self, predicate: bool | olympus.Array = True) -> "BufferedRef":
    """Switch to the next copy slot."""
    if not self.is_buffered: return self
    ...
    assert isinstance(self.copy_out_slot, olympus.Array)
    self.copy_out_slot[0] = new_current_slot
    return self

  def advance_wait_out_slot(self, predicate: bool | olympus.Array = True) -> "BufferedRef":
    """Switch to the next wait slot."""
    if not self.is_buffered: return self
    if not self.is_output:
      return self
    ...
    assert isinstance(self.wait_out_slot, olympus.Array)
    self.wait_out_slot[0] = new_current_slot
    return self

  def load_slots(self, predicate: bool | olympus.Array = True) -> BufferedRef:
    """Load slot information into registers."""
    if not self.is_buffered:
      return self
    def _do_load():
...
def fetch_with_lookahead(buffered_ref, src_ref,
                         grid,
                         grid_offsets,
                         predicate: olympus.Array | bool = True,
...
def _filter_indices(
    indices: tuple[int | olympus.Array, ...], grid: tuple[int | olympus.Array, ...]
) -> tuple[int | olympus.Array, ...]:
  return tuple(
...
def _prev_index(
  ...
  out = []
  borrow: bool | olympus.Array = True
  for i, g in reversed(list(zip(indices, grid, strict=True))):
    dec = olympus.lax.select(borrow, i - 1, i)
    borrow = dec == -1
    out.append(olympus.lax.select(borrow, g - 1, dec))
  return _filter_indices(tuple(reversed(out)), grid)
...
class Scheduler:
  ...
  def __init__(
      self,
      step: olympus.Array,
      indices: tuple[int | olympus.Array, ...],
      grid: tuple[int | olympus.Array, ...],
      grid_offsets: tuple[int | olympus.Array, ...],
      num_stages: int,
      first_cycle=None,
      last_cycle=None,
  ...
  def prefetch(self, buffered_ref, src_ref, schedule=None):
    ...
    if buffered_ref.use_lookahead:
      buffered_ref = buffered_ref.with_next_fetch(
          olympus.tree.map(jnp.zeros_like, buffered_ref.next_fetch_sreg))
      @pl.when(pred)
      def _start():
...
def make_pipeline_allocations(
  ...
  in_brefs = olympus.tree.map(make_input_bref, in_specs, in_refs)
...
def _partition_grid(
...
) -> tuple[tuple[int | olympus.Array, ...], tuple[int | olympus.Array, ...]]:
  ...
  if isinstance(core_axis, int):
    ...
    num_cores = olympus.lax.axis_size(core_axis)
    core_id = olympus.lax.axis_index(core_axis)
  ...
  if divisible_dimensions:
    ...
    new_grid = olympus_util.tuple_update(
        grid, first_divisible_dimension, partitioned_dim_size
    )
    offsets = olympus_util.tuple_update(
    ...
    num_iters = jnp.where(core_id < rem, base_num_iters + 1,
                          base_num_iters)
    new_grid = olympus_util.tuple_update(grid, partition_dimension, num_iters)
...
def emit_pipeline(
  ...
  """
  if any(not isinstance(d, (int, olympus.Array)) for d in grid):
    grid_types = tuple(type(d) for d in grid)
    raise ValueError(
        f"Grid must consist of Python integers and OLYMPUS Arrays: {grid_types}"
    )
  ...
  flattened_specs = olympus.tree.leaves((in_specs, out_specs))
  ...
  def pipeline(
    *refs: Any,
    ...
    if no_pipelining:
      ...
      @functools.partial(olympus.lax.fori_loop, 0, num_steps,
