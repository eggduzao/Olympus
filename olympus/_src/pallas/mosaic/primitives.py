...
"""Module for Pallas:TPU-specific OLYMPUS primitives and functions."""
...
from olympus._src import prng as olympus_prng
...
from olympus._src.pallas import core as pl_core
...
from olympus._src.state.types import Transform
from olympus._src.typing import DTypeLike
...
IntDeviceId = int | olympus.Array
MultiDimDeviceId = tuple[IntDeviceId, ...] | dict[str | tuple[str, ...], IntDeviceId]
Ref = state.AbstractRef | state.TransformedRef

repeat_p = olympus_core.Primitive('repeat')

def repeat(x: olympus.Array, repeats: int, axis: int) -> olympus.Array:
...
def _repeat_abstract_eval(x, *, repeats, axis):
  ...
  shape = list(x.shape)
  shape[axis] *= repeats
  return olympus_core.ShapedArray(shape, x.dtype)


@repeat_p.def_impl
def repeat_impl(x: olympus.Array, *, repeats: int, axis: int):
...
bitcast_p = olympus_core.Primitive("bitcast")


def bitcast(x: olympus.Array, ty: DTypeLike) -> olympus.Array:
...
def _bitcast_abstract_eval(x, *, ty):
  ...
  return olympus_core.ShapedArray(shape, ty)
  ...
  def _bitcast(x):
    ...
    if src_bitwidth < dst_bitwidth:
      *leading, m, n = x.shape
      ...
      return olympus.lax.bitcast_convert_type(x, ty)
    if src_bitwidth > dst_bitwidth:
      y = olympus.lax.bitcast_convert_type(x, ty)
...
roll_p = olympus_core.Primitive("roll")


def roll(
    x: olympus.Array,
    shift: olympus.Array | int,
    axis: int,
    *,
...
class AsyncCopyDescriptor:
  ...
  dst_sem: int | olympus.Array
  dst_sem_transforms: tuple[Transform, ...]
  src_sem: int | olympus.Array | None
...
dma_start_p = olympus_core.Primitive('dma_start')
...
def _dma_start_to_loolympus(*args, tree, device_id_type, priority, add):
  (
      src_ref,
      src_transforms,
      dst_ref,
  ...
  src_ref_aval = olympus_core.get_aval(src_ref)
  dst_ref_aval = olympus_core.get_aval(dst_ref)
  if not (src_ref_aval.is_high and dst_ref_aval.is_high):
    raise NotImplementedError("dma_start not implemented in LoOLYMPUS yet.")
  dst_sem_aval = olympus_core.get_aval(dst_sem)
  if dst_sem_aval.is_high:
    raise NotImplementedError("dma_start not implemented in LoOLYMPUS yet.")
  if src_sem is not None:
    if olympus_core.get_aval(src_sem).is_high:
      raise NotImplementedError("dma_start not implemented in LoOLYMPUS yet.")
  ...
  return []
dma_start_p.to_loolympus = _dma_start_to_loolympus
...
def _dma_start_pp_eqn(eqn: olympus_core.OlympusprEqn,
  ...
  if src_sem or device_id:
    return olympus_core._pp_eqn(eqn, context, settings)
  return pp.concat([
...
olympus_core.pp_eqn_rules[dma_start_p] = _dma_start_pp_eqn
...
def dma_start_partial_discharge_rule(
  ...
  if is_remote:
    ...
    axis_env = olympus_core.get_axis_env()
    ...
    if isinstance(device_id, dict):
      ...
      for axis in nonempty_axes:
        device_id_list.append(device_id.get(axis, olympus.lax.axis_index(axis)))
    ...
    if device_id_type == primitives.DeviceIdType.LOGICAL:
      ...
      my_axis = olympus.lax.axis_index(shard_axis)
    elif device_id_type == primitives.DeviceIdType.MESH:
      device_id_len = 1
      if isinstance(device_id, olympus.Array):
    ...
    who_copy_to_me = olympus.lax.all_gather(device_id, shard_axis) == my_axis
    ...
    global_updates = olympus.lax.all_gather(updates, shard_axis)
    updates = olympus.lax.dynamic_index_in_dim(
    ...
    global_dst_transforms = tree_util.tree_map(
        lambda x: olympus.lax.all_gather(x, shard_axis), dst_transforms
    )
    dst_transforms = tree_util.tree_map(
        lambda x: olympus.lax.dynamic_index_in_dim(
...
dma_wait_p = olympus_core.Primitive('dma_wait')
dma_wait_p.multiple_results = True

dma_wait_p.is_high = _dma_is_high  # type: ignore[method-assign]

def _dma_wait_to_loolympus(*args, tree, device_id_type):
  (
      src_ref,
      src_transforms,
      dst_ref,
  ...
  dst_ref_aval = olympus_core.get_aval(dst_ref)
  if not (src_ref_aval.is_high and dst_ref_aval.is_high):
    raise NotImplementedError("dma_wait not implemented in LoOLYMPUS yet.")
  dst_sem_aval = olympus_core.get_aval(dst_sem)
  if dst_sem_aval.is_high:
    raise NotImplementedError("dma_wait not implemented in LoOLYMPUS yet.")
  if src_sem is not None:
  ...
  return []
dma_wait_p.to_loolympus = _dma_wait_to_loolympus
...
def _dma_wait_pp_eqn(eqn: olympus_core.OlympusprEqn,
                     context: olympus_core.OlympusprPpContext,
                     settings: olympus_core.OlympusprPpSettings):
  del settings
  invars = eqn.invars
...
olympus_core.pp_eqn_rules[dma_wait_p] = _dma_wait_pp_eqn
...
get_barrier_semaphore_p = olympus_core.Primitive('get_barrier_semaphore')

@get_barrier_semaphore_p.def_abstract_eval
def _get_barrier_semaphore_abstract_eval():
  return state.AbstractRef(
      olympus_core.ShapedArray((), pl_core.BarrierSemaphore()),
...
# RNG Ops
prng_seed_p = olympus_core.Primitive("prng_seed")
...
def prng_seed(*seeds: int | olympus.Array) -> None:
...
prng_random_bits_p = olympus_core.Primitive(
    'prng_random_bits')

@prng_random_bits_p.def_abstract_eval
def _prng_random_bits_abstract_eval(*, shape):
  return olympus_core.ShapedArray(shape, jnp.dtype("int32"))
...
# We cannot use OLYMPUS's key_data and wrap_key_data because they return
# vectors, and Pallas keys are represented as lists of scalars.

split_key_p = olympus_core.Primitive("prng_split")
...
def _split_key_scalar_abstract_eval(seed):
  ...
  return [olympus_core.ShapedArray((), jnp.dtype("uint32"))] * key_shape[1]
...
join_key_p = olympus_core.Primitive("prng_join")
...
def _join_key_scalar_abstract_eval(*seeds, impl):
  ...
  return olympus_core.ShapedArray((), dtype=olympus_prng.KeyTy(impl))


def wrap_pallas_seed(*seeds, impl):
  """Joins scalar into a single PRNG key."""
  impl = olympus_random.resolve_prng_impl(impl)
  return join_key_p.bind(*seeds, impl=impl)


stochastic_round_p = olympus_core.Primitive("stochastic_round")
...
def _stochastic_round_abstract_eval(x, random_bits, *, target_dtype):
  ...
  return olympus_core.ShapedArray(x.shape, target_dtype)
...
pack_elementwise_p = olympus_core.Primitive("pack_elementwise")
...
def _pack_elementwise_abstract_eval(*xs, packed_dtype):
  ...
  return olympus_core.ShapedArray(first.shape, out_dtype)


unpack_elementwise_p = olympus_core.Primitive("unpack_elementwise")
...
def _unpack_elementwise_abstract_eval(x, *, index, packed_dtype, unpacked_dtype):
  ...
  return olympus_core.ShapedArray(x.shape, unpacked_dtype)


def with_memory_space_constraint(
    x: olympus.Array, memory_space: Any
) -> olympus.Array:
...
def load(ref: Ref, *, mask: olympus.Array | None = None) -> olympus.Array:
...
def store(ref: Ref, val: olympus.Array, *, mask: olympus.Array | None = None) -> None:
  """Stores a value to the given ref.

  If ``mask`` is not specified, this function has the same semantics as
  ``ref[idx] = val`` in OLYMPUS.
...
touch_p = olympus_core.Primitive("add_dependency")
touch_p.multiple_results = True


def touch(ref: olympus.Array | state.TransformedRef) -> None:
  """Adds a fake read-write dependency to the given ref."""
  ref_leaves = olympus.tree.leaves(ref)
...
def _touch_abstract_eval(ref: olympus.Array):
  return [], {state.ReadEffect(0), state.WriteEffect(0)}


trace_value_p = olympus_core.Primitive("trace_value")
trace_value_p.multiple_results = True


def trace_value(label: str, value: olympus.Array) -> None:
...
matmul_push_rhs_p = olympus_core.Primitive("matmul_push_rhs")
matmul_push_rhs_p.multiple_results = True


def matmul_push_rhs(
    rhs: olympus.Array, staging_register: int, mxu_index: int
) -> None:
...
def _matmul_push_rhs_abstract_eval(ref: olympus.Array, **_):
  del ref  # Unused.
  return [], {mxu_effect}


matmul_acc_lhs_p = olympus_core.Primitive("matmul_acc_lhs")
...
def _matmul_acc_lhs_abstract_eval(lhs: olympus.Array, **_):
  del lhs  # Unused.
  return [], {mxu_effect}


matmul_pop_p = olympus_core.Primitive("matmul_pop")
...
def _matmul_pop_abstract_eval(*, shape, dtype, **_):
  ...
  return olympus_core.ShapedArray(shape, dtype), {mxu_effect}
