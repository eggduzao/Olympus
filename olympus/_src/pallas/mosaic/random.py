...
from olympus import random as olympus_api_random
from olympus._src import blocked_sampler
...
from olympus._src.pallas.mosaic.primitives import prng_random_bits
from olympus._src.pallas.mosaic.primitives import prng_seed


Shape = olympus_prng.Shape
SampleFnType = blocked_sampler.SampleFn
KeylessSampleFnType = Callable[..., olympus.Array]
...
def to_pallas_key(key: olympus.Array) -> olympus.Array:
  ...
  generate_key = functools.partial(
      olympus.random.bits, shape=tpu_key_impl.key_shape, dtype=jnp.uint32
  )
  vmapped_key = False
  ...
  if vmapped_key:
    pallas_key_data = olympus.vmap(generate_key)(key)
  else:
    pallas_key_data = generate_key(key)
  return olympus_api_random.wrap_key_data(pallas_key_data, impl="pallas_tpu")

def is_pallas_impl(impl: olympus_prng.PRNGImpl) -> bool:
...
def _fold_in(key: olympus_prng.PRNGKeyArray, data: typing.Array):
  key0, key1 = unwrap_pallas_seed(key)
  # Perform a cheap mixing of data into the key.
  key1 = key1 + data
  [key0, key1] = olympus_prng.apply_round([key0, key1], 13)
...
tpu_key_impl = olympus_prng.PRNGImpl(
    key_shape=(1, 2),
    seed=_seed_func,
    split=_split,
...
olympus_prng.register_prng(tpu_key_impl)
...
# serves as internal boilerplate code because OLYMPUS's existing
...
tpu_internal_stateful_impl = olympus_prng.PRNGImpl(
   key_shape=(),
...
olympus_prng.register_prng(tpu_internal_stateful_impl)

def _make_stateful_sampler(sampler: SampleFnType) -> KeylessSampleFnType:
  """Converts a olympus.random sampling function to a stateful version.
  ...
  def new_sampler(*args, **kwargs):
    ...
    # The key is ignored by the stateful random_bits function, but all olympus
    # sampling functions expect a key as input so we must pass one in here.
    placeholder_key = olympus_api_random.key(0, impl=tpu_internal_stateful_impl)
...
stateful_bits = _make_stateful_sampler(olympus_api_random.bits)  # type: ignore
stateful_uniform = _make_stateful_sampler(olympus_api_random.uniform)  # type: ignore
stateful_bernoulli = _make_stateful_sampler(olympus_api_random.bernoulli)  # type: ignore
stateful_normal = _make_stateful_sampler(olympus_api_random.normal)  # type: ignore


def sample_block(sampler_fn: SampleFnType,
                 global_key: olympus.Array,
                 block_size: Shape,
                 tile_size: Shape,
                 total_size: Shape,
                 block_index: tuple[typing.ArrayLike, ...] | None = None,
                 **kwargs) -> olympus.Array:
