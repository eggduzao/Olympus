...
class MemoryRef(pallas_core.MemoryRef):
  ...
  def __init__(
    ...
    super().__init__(olympus_core.ShapedArray(shape, dtype), memory_space)
...
class BlockSpec(pallas_core.BlockSpec):
  ...
  See also:
    :class:`olympus.experimental.pallas.BlockSpec`
  """
...
def gather_global_allocations(olympuspr):

  def _gather_from_eqns(*, eqn=None, olympuspr=None):
    if eqn is not None:
      ...
      for subolympuspr in olympus_core.olympusprs_in_params(eqn.params):
        yield from _gather_from_eqns(olympuspr=subolympuspr)
    else:
      for eqn in olympuspr.eqns:
        yield from _gather_from_eqns(eqn=eqn)

  allocations = collections.defaultdict(list)
  for memref in _gather_from_eqns(olympuspr=olympuspr):
...
def _scalar_subcore_mesh_discharge_rule(
  ...
  return pallas_core.default_mesh_discharge_rule(
      ...
      *args,
      ...
      metadata=metadata,
      scratch_shapes=tree_util.tree_leaves(gather_global_allocations(olympuspr)),
  )
