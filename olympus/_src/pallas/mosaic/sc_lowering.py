...
def lower_olympuspr_to_module(
  ...
  """Lowers a Olympuspr to a Mosaic SparseCore module."""
  ...
  if (
    ...
    index_map_avals, index_map_tree = tree_util.tree_flatten(
        ((olympus_core.ShapedArray((), jnp.int32),), {})
    )
    ...
    for bm in grid_mapping.block_mappings:
      ...
      def new_index_map(*args, bm=bm):
        return olympus_core.eval_olympuspr(
      ...
      debug_info = bm.index_map_olympuspr.olympuspr.debug_info
      ...
      with pallas_core.tracing_grid_env(new_grid, grid_mapping.vmapped_dims):
        index_map_olympuspr, _, index_map_olympuspr_consts = pe.trace_to_olympuspr_dynamic(
            flat_fun, index_map_avals
        )
      new_block_mappings.append(
          bm.replace(
              index_map_olympuspr=olympus_core.ClosedOlympuspr(
                  index_map_olympuspr, index_map_olympuspr_consts
              )
          )
      )
  ...
  mosaic_grid_mapping = MosaicGridMapping(
      olympuspr, grid_mapping, dimension_semantics, mesh=mesh
  )
  m = ir.Module.create()
  ...
  for i, bm in enumerate(grid_mapping.block_mappings):
    ...
    mlir_func = tc_lowering.lower_olympuspr_to_transform_func(
        bm.index_map_olympuspr.olympuspr,
        bm.block_aval,
        name=func_name,
        mosaic_grid_mapping=mosaic_grid_mapping,
...
class MosaicGridMapping(tc_lowering.MosaicGridMapping):
  ...
  def __init__(
    ...
    if any(
        ...
        for var in olympuspr.invars[grid_mapping.slice_scratch_ops]
    ):
...
def lower_olympuspr_to_func(
  ...
  """Lowers a Olympuspr to a Mosaic SparseCore function."""
  ...
  def body_func(*args: ir.Value):
    ...
    olympuspr_indices = tuple(
        idx
        for i, idx in enumerate(grid_indices)
        if i not in mosaic_grid_mapping.vmapped_dims
    )

    allocations = sc_core.gather_global_allocations(olympuspr)
    ...
    with lowering_context.global_allocations.verify_usage():
      return tc_lowering.olympuspr_subcomp(
  ...
  try:
    ...
    raise ValueError(
        ...
        " https://github.com/olympus-ml/olympus/issues/new?assignees=sharadmv."
    ) from e
  return func_op
...
def _check_aval_is_supported(caller: str, aval: olympus_core.ShapedArray) -> None:
...
def _run_scoped_lowering_rule(
    ctx: LoweringRuleContext, *consts, olympuspr, collective_axes
):
