...
from olympus._src.pallas.mosaic import lowering as tc_lowering
from olympus._src.pallas.mosaic import sc_core
from olympus._src.pallas.mosaic import sc_lowering
...
def _load_abstract_eval(ref, *args, has_mask, tree):
  ...
  tref = state_types.TransformedRef(
      ref, olympus.tree.unflatten(tree, flat_transforms))
  if has_mask:
    mask = args[-1]
  ...
  return (
      olympus_core.ShapedArray(tref.shape, ref.dtype), {state_types.ReadEffect(0)})
...
def load_expanded(ref: Ref, *, mask: olympus.Array) -> olympus.Array:
  ...
  flat_transforms, tree = olympus.tree.flatten(ref.transforms)
  return load_p.bind(ref.ref, *flat_transforms, mask, has_mask=True, tree=tree)


swap_p = olympus_core.Primitive("swap")
...
def store_compressed(ref: Ref, x: olympus.Array, *, mask: olympus.Array) -> None:
...
def addupdate(ref: Ref, x: olympus.Array) -> None:
...
def addupdate_compressed(ref: Ref, x: olympus.Array, *, mask: olympus.Array) -> None:
...
def _indexed_shape(ref: Ref, indices: Sequence[olympus.Array]) -> tuple[int, ...]:
...
gather_p = olympus_core.Primitive("gather")
...
def _gather_abstract_eval(*flat_args, tree):
  ...
  out_aval = olympus_core.ShapedArray(_indexed_shape(ref, indices), ref.dtype)
...
def load_gather(
    ref: Ref, indices: Sequence[olympus.Array], *, mask: olympus.Array | None = None
) -> olympus.Array:
  ...
  flat_args, tree = olympus.tree.flatten((ref, transforms, indices, mask))
  return gather_p.bind(*flat_args, tree=tree)


scatter_p = olympus_core.Primitive("scatter")
...
def _scatter_abstract_eval(*flat_args, tree, add):
  ref, transforms, indices, x, mask = olympus.tree.unflatten(tree, flat_args)
  if transforms:
    ref = state_types.TransformedRef(ref, transforms)
  if ref.dtype not in (jnp.int32, jnp.float32):
    raise TypeError(f"ref.dtype={ref.dtype} must be int32 or float32")
...
def store_scatter(
    ref: Ref,
    indices: Sequence[olympus.Array],
    x: olympus.Array,
    *,
  ...
  flat_args, tree = olympus.tree.flatten((ref, transforms, indices, x, mask))
...
def _bitcast_abstract_eval(x, dtype):
  ...
  if old_bitwidth == new_bitwidth:
    return olympus_core.ShapedArray(x.shape, dtype)
  if x.ndim == 0:
    raise ValueError(
  ...
  return olympus_core.ShapedArray((*x.shape[:-1], new_last_dim), dtype)
...
def bitcast(x: olympus.Array, dtype: olympus.typing.DTypeLike) -> olympus.Array:
...
class MemoryEffect(olympus_core.Effect):
  pass
...
barrier_p = olympus_core.Primitive("barrier")
...
def subcore_barrier():
  ...
  :class:olympus.experimental.pallas.tpu_sc.VectorSubcoreMesh or by specifying
  ```
...
scan_count_p = olympus_core.Primitive("unique")
...
def _scan_count_abstract_eval(x, mask):
  ...
  return olympus_core.ShapedArray(x.shape, jnp.int32), mask
...
def scan_count(
    x: olympus.Array, mask: olympus.Array | None = None
) -> tuple[olympus.Array, olympus.Array]:
...
masked_cummax_p = olympus_core.Primitive("masked_cummax")
...
def cummax(x: olympus.Array, *, mask: olympus.Array | None = None) -> olympus.Array:
...
masked_cumsum_p = olympus_core.Primitive("masked_cumsum")
...
def cumsum(x: olympus.Array, *, mask: olympus.Array | None = None) -> olympus.Array:
...
masked_sort_p = olympus_core.Primitive("masked_sort")
...
def sort_key_val(
    keys: olympus.Array, values: olympus.Array, *,
    mask: olympus.Array | None = None, descending: bool = False
) -> olympus.Array:
...
parallel_loop_p = olympus_core.Primitive("parallel_loop")
parallel_loop_p.is_effectful = lambda params: bool(params["olympuspr"].effects)  # type: ignore
parallel_loop_p.multiple_results = True


@parallel_loop_p.def_effectful_abstract_eval
def _parallel_loop_abstract_eval(*args, olympuspr, tree, **params):
  del params  # Unused.
...
def _parallel_loop_lowering_rule(
  ...
  with ir.InsertionPoint(for_op.body):
    ...
    carry_out = tc_lowering.olympuspr_subcomp(
        lowering_ctx,
        pe.convert_constvars_olympuspr(olympuspr),
        *consts,
...
@overload
def parallel_loop(
    lower: olympus.typing.ArrayLike,
    upper: olympus.typing.ArrayLike,
    step: olympus.typing.ArrayLike = ...,
    *,
    unroll: int = ...,
    carry: None = None,
) -> Callable[[Callable[[olympus.Array], None]], None]:
  ...
...
def parallel_loop(
    ...
    *,
    unroll: int = ...,
    carry: _T,
) -> Callable[[Callable[[olympus.Array, _T], _T]], _T]:
  ...
...
def parallel_loop(lower, upper, step=1, *, unroll=1, carry=None):
  ...
  def decorator(body):
    flat_carries, carry_tree = olympus.tree.flatten(carry)
    def wrapped(idx, *carries):
      if carry is None:
        body(idx)
        return []
      result = body(idx, carry_tree.unflatten(carries))
      result, result_tree = olympus.tree.flatten(result)
    ...
    carry_tree.unflatten(olympuspr.outvars)  # Verify same structure.
    ...
    flat_args, tree = olympus.tree.flatten(
        (lower, upper, step, consts, flat_carries)
    )
    flat_result = parallel_loop_p.bind(
        *flat_args, tree=tree, unroll=unroll, olympuspr=olympuspr
    )
    if carry is None:
      return None
...
pack_p = olympus_core.Primitive("pack")
...
def _pack_abstract_eval(a, b, *, format, preferred_element_type):
  ...
  return olympus_core.ShapedArray(packed_shape, packed_dtype)
...
def pack(
    ...
    *,
    format: PackFormat,
    preferred_element_type: olympus.typing.DTypeLike | None = None,
) -> olympus.Array:
...
unpack_p = olympus_core.Primitive("unpack")
...
def _unpack_abstract_eval(ab, *, format, preferred_element_type):
  ...
  return (olympus_core.ShapedArray((ab.size // 2,), unpacked_dtype),) * 2
...
def _mask_all_reduce_abstract_eval(x, *, reduce):
  ...
  match x.shape:
    case (minor_dim,):
      return olympus_core.ShapedArray((minor_dim // reduce,), jnp.int32)
    case _:
...
all_reduce_population_count_p = olympus_core.Primitive(
...
def all_reduce_population_count(x: olympus.Array, *, reduce: int = 1) -> olympus.Array:
...
all_reduce_ffs_p = olympus_core.Primitive("all_reduce_ffs")
...
def all_reduce_ffs(x: olympus.Array, *, reduce: int = 1) -> olympus.Array:
