...
from olympus._src import custom_batching
...
class MemorySpace(enum.Enum):
  ...
  def __call__(
    ...
    if self == MemorySpace.TMEM:
      ...
      mgpu_layout = None
    return GPUMemoryRef(olympus_core.ShapedArray(shape, dtype), memory_space=self,
...
class SemaphoreType(enum.Enum):
  ...
  def __call__(self, shape: tuple[int, ...]):
    ...
    return pallas_core.MemoryRef(olympus_core.ShapedArray(shape, dtype),
                                 MemorySpace.GMEM)
...
def kernel(
  ...
  def wrapper(*operands):
    ...
    _, outs = state_discharge.run_state(stateful)((
        operands,
        olympus.tree.map(lambda s: olympus.lax.empty(s.shape, s.dtype), out_shape),
    ))
...
class GPUMemoryRef(pallas_core.MemoryRef):
  ...
  def get_ref_aval(self) -> _Ref:
    aval: Any = olympus_core.ShapedArray(self.shape, self.dtype)
...
def _ref_group_size(refs: _GPUMemoryRefTree) -> int:
  size = 0
  for ref in olympus.tree.leaves(refs):
...
def flatten_ref_union(ref_union: AbstractRefUnion) -> tuple[_Ref, ...]:
  ...
  This is the moral equivalent of `olympus.tree.leaves` for aliased references.
  """
  flat_refs = []
  if ref_union.memory_space == SMEM:
    ...
    for ref_group in ref_union.refs:
      ...
      def unflatten(ref):
        ...
        return result
      flat_refs.append(olympus.tree.map(unflatten, ref_group))
...
class AbstractRefUnion(state.AbstractRef):
  ...
  def collective(self) -> bool:
    ...
    ref_leaves = olympus.tree.leaves(self.refs)
...
class RefUnion(GPUMemoryRef):
  ...
  def get_ref_aval(self) -> AbstractRefUnion:
    inner_aval = olympus.core.ShapedArray(self.shape, self.dtype)
    refs_aval = olympus.tree.map(lambda ref: ref.get_ref_aval(), self.refs)
...
class MemoryRefTransform(pallas_core.MemoryRefTransform, abc.ABC):
  ...
  def __call__(self, aval: olympus_core.ShapedArray) -> olympus_core.ShapedArray:
    return aval.update(
...
class UntileRef(state_types.Transform):
  ...
  def pretty_print(self, context: olympus_core.OlympusprPpContext) -> pp.Doc:
...
def remote_ref(
    ref: _Ref,
    device_id: olympus.typing.ArrayLike,
  ...
  if not isinstance(ref, pallas_core.TransformedRef):
    if not isinstance(olympus_core.get_aval(ref), state_types.AbstractRef):
...
def multicast_ref(
  ...
  Args:
    ...
    collective_axes: The OLYMPUS mesh axes indicating the devices to operate on.
  """
...
class WGMMAAccumulatorRef:
  ...
  def get_ref_aval(self) -> state.AbstractRef:
    ...
    return WGMMAAbstractAccumulatorRef(
        olympus_core.ShapedArray(shape=self.shape, dtype=self.dtype), MemorySpace.REGS
    )
...
def _wgmma_ref_type_mapping(ref: WGMMAAccumulatorRef):
  aval = WGMMAAbstractAccumulatorRef(
      olympus_core.ShapedArray(shape=ref.shape, dtype=ref.dtype), MemorySpace.REGS
  )
...
class WGMMAAbstractAccumulatorRef(state.AbstractRef):
  ...
  def _getitem(self, tracer, idx):
    from olympus._src.pallas.mosaic_gpu.primitives import wgmma_accumulator_load  # pytype: disable=import-error
...
class Mesh:
  ...
  def discharges_effect(self, effect: olympus_core.Effect):
...
def _gpu_mesh_discharge_rule(
  ...
  sa_avals = [a for a in in_avals if isinstance(a, olympus_core.ShapedArray)]
  if sa_avals:
    raise NotImplementedError(
...
layout_cast_p = olympus_core.Primitive("layout_cast")
