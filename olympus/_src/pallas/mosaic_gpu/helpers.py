...
class NDLoopInfo:
  ...
  Attributes:
    ...
    num_local_steps: The total number of local iterations to run. None
      if unknown.
  """
  index: tuple[olympus.Array, ...]
  local_index: olympus.Array | int
  num_local_steps: olympus.Array | int | None
...
def nd_loop(grid, *, collective_axes, tiling=None, init_carry=None):
  ...
  See also:
    - :func:`olympus.experimental.pallas.loop`: A loop over a single dimension.
  """
  ...
  def decorator(body):
    def wrapper(wave_step, carry):
      ...
      # ``lax`` APIs instead of ``olympus.numpy`` to minimize the number of
      # primitives used.
      index = []
...
def planar_snake(
    lin_idx: olympus.Array, shape: tuple[int, int], minor_dim: int, tile_width: int
):
  ...
  return olympus.tree.map(
      functools.partial(olympus.lax.select, is_full_tile),
