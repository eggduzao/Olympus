...
from olympus._src.pallas.mosaic_gpu.interpret import shared_memory as memory
...
# an argument that is a Olympus (device) array, this argument is received as a Numpy
...
class HostAllocationRequest:
  ...
  def __iter__(self):
    # We make `self` iterable to ease conversion into Numpy and Olympus arrays (cf.
    # methods `as_array` and `as_olympus_array` below). Note that for this purpose
  ...
  @classmethod
  def shape_and_dtype(cls) -> olympus.ShapeDtypeStruct:
    num_fields = len(dataclasses.fields(cls))
    return olympus.ShapeDtypeStruct((num_fields,), jnp.int32)
  ...
  @property
  def as_olympus_array(self) -> jnp.ndarray:
...
class HostAllocationKey(HostAllocationRequest):
  ...
  def __iter__(self):
    ...
    # `as_array` and `as_olympus_array` methods of the base class. This is intended.
...
def call_allocate_barriers(
  ...
  result_shape_and_dtype = olympus.ShapeDtypeStruct(
