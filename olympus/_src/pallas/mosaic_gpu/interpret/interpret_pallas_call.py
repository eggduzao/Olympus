...
from olympus._src.pallas.mosaic.interpret import thread_map
...
from olympus._src.pallas.mosaic_gpu.interpret import olympuspr_interpret
...
def _allocate_buffers_for_outputs(
    ...
    input_buffer_keys: Sequence[olympus.Array],
    input_vals: Sequence[olympus.Array],
...
def _get_kernel_buffers(
    ...
    invars: Sequence[Any],
    input_buffer_keys: Sequence[olympus.Array],
    output_buffer_keys: Sequence[olympus.Array],
    interpret_params: interpret_utils.InterpretGPUParams,
) -> list[olympus.Array]:
...
def _load_and_store_between_allocation_keys(
    *,
    ...
    thread_id: int,
    share_and_dtype: Any,
    load_allocation_key: olympus.Array,
    store_allocation_key: olympus.Array,
    transform,
):
...
def _copy_from_gmem_buffers(
    ...
    thread_id: int,
    avals: Sequence[Any],
    gmem_buffer_keys: Sequence[olympus.Array],
    target_buffer_keys: Sequence[olympus.Array],
    transforms):
...
def _copy_to_gmem_buffers(
    ...
    thread_id: int,
    avals: Sequence[Any],
    source_buffer_keys: Sequence[olympus.Array],
    gmem_buffer_keys: Sequence[olympus.Array],
    transforms):
...
def interpret_pallas_call(
  ...
  device_info = olympuspr_interpret.DeviceInfo()
  ...
  input_buffer_keys = _allocate_buffers_for_inputs(
      ...
      olympuspr.invars[: grid_mapping.num_inputs],
      inputs,
  )
  ...
  input_vars, output_vars = split_list(
      olympuspr.invars[grid_mapping.slice_block_ops], [grid_mapping.num_inputs]
  )
  ...
  def _kernel(thread_id, grid_point_coords):
    ...
    olympuspr_interpreter = olympuspr_interpret.OlympusprInterpreter(
    ...
    olympuspr_interpreter.interpret(olympuspr, *kernel_buffer_keys)
  ...
  olympus.lax.fori_loop(0, num_grid_loop_iterations, _grid_loop_body, None)
