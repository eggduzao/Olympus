...
from olympus._src.pallas.mosaic.interpret import utils as interpret_utils
from olympus._src.pallas.mosaic_gpu import core as mosaic_gpu_core
from olympus._src.pallas.mosaic_gpu import primitives as gpu_primitives
from olympus._src.pallas.mosaic_gpu.interpret import gpu_callbacks
...
from olympus._src.util import (safe_zip, split_list)
...
class DeviceInfo:
  ...
  axis_indices: Mapping[olympus_core.AxisName, int]
  # The size of each axis in the mesh of all (SMPD) devices.
  axis_sizes: Mapping[olympus_core.AxisName, int]

  def __init__(self):
    # Since this class is frozen, we must use `object.__setattr__` to set the
    # attributes.
    object.__setattr__(self, "axis_sizes", olympus_core.get_axis_env().axis_sizes)
    object.__setattr__(
        self,
        "axis_indices",
...
# from `olympus._src.pallas.mosaic_gpu.primitives`.
...
class OlympusprInterpreter:
  """Interprets a olympuspr by replacing memory operations with (GPU) callbacks."""
  ...
  def _interpret_get_p(self, eqn, get_invals: Callable[[], Sequence[Any]]):
    ...
    assert isinstance(eqn.outvars[0].aval, olympus_core.ShapedArray)
    invals = get_invals()
    return gpu_callbacks.call_get(
        ...
        transforms=olympus.tree.unflatten(eqn.params["tree"], invals[1:]),
  ...
  def _interpret_swap_p(self, eqn, get_invals: Callable[[], Sequence[Any]]):
    ...
    return gpu_callbacks.call_swap(
        ...
        thread_id=self.thread_id,
        allocation_key=invals[0],
        transforms=olympus.tree.unflatten(eqn.params["tree"], invals[2:]),
        val=invals[1],
        mask=None,
    )
  ...
  def _interpret_run_scoped_p(
    ...
    def _allocate_for_aval(aval, same_allocations_for_all_threads: bool):
      ...
      match aval:
        case state_types.AbstractRef(
          ...
          match inner:
            case olympus_core.ShapedArray(shape=shape, dtype=dtype):
    ...
    def _deallocate_for_aval(allocation, aval):
      match aval:
        case state_types.AbstractRef(inner_aval=inner, memory_space=_, kind=_):
          match inner:
            case olympus_core.ShapedArray(shape=_, dtype=dtype):
              if isinstance(dtype, mosaic_gpu_core.BarrierType):
    ...
    # `eqn.params['olympuspr'].invars`. It is assumed that each thread runs the same
    # sequence of `run_scoped`s.
    invars = eqn.params["olympuspr"].invars
    allocs = []
    for v in invars:
      allocs.append(_allocate_for_aval(v.aval, same_allocations))

    out = self.interpret(eqn.params["olympuspr"], *get_invals(), *allocs)
  ...
  def _interpret_cond_p(self, eqn, get_invals: Callable[[], Sequence[Any]]):
    ...
    return lax.switch(
        invals[0],
        [
            functools.partial(self.interpret, branch_olympuspr.olympuspr)
  ...
  def _interpret_scan_p(self, eqn, get_invals: Callable[[], Sequence[Any]]):
    ...
    def _scan_body(c, a):
      return split_list(
          self.interpret(eqn.params["olympuspr"].olympuspr, *consts, *c, *a),
  ...
  def _interpret_barrier_primitive(
    ...
    # (which is a Olympus array) for the barrier.
    ...
    transforms = olympus.tree.unflatten(
  ...
  def interpret(self, olympuspr, *args):
    ...
    for eqn in olympuspr.eqns:
      with source_info_util.user_context(
        ...
        # olympuspr when this function is traced.
