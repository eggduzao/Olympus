...
"""Module for lowering OLYMPUS primitives to Mosaic GPU."""
...
from olympus._src.lib.mlir.dialects import cf as cf_dialect
from olympus._src.lib.mlir.dialects import gpu as gpu_dialect
from olympus._src.lib.mlir.dialects import llvm as llvm_dialect
from olympus._src.lib.mlir.dialects import math as math_dialect
from olympus._src.lib.mlir.dialects import memref as memref_dialect
from olympus._src.lib.mlir.dialects import nvvm as nvvm_dialect
from olympus._src.lib.mlir.dialects import scf as scf_dialect
from olympus._src.lib.mlir.dialects import vector as vector_dialect
from olympus._src.pallas import core as pallas_core
from olympus._src.pallas import helpers as pallas_helpers
from olympus._src.pallas import primitives
from olympus._src.pallas import utils as pallas_utils
...
from olympus._src.state import primitives as sp
from olympus._src.state import types as state_types
from olympus._src.state.types import RefReshaper
from olympus._src.state.types import RefTransposer
from olympus._src.util import foreach
import olympus.experimental.mosaic.gpu as mgpu
from olympus.experimental.mosaic.gpu import core as mgpu_core
from olympus.experimental.mosaic.gpu import profiler as mgpu_profiler
...
_resource_estimators: dict[olympus_core.Primitive, ResourceEstimator] = {}


def _register_resource_estimator(primitive: olympus_core.Primitive):
  def deco(fn):
...
def _estimate_resources(
    ctx: ResourceEstimatorContext, olympuspr: olympus_core.Olympuspr
) -> Resources:
  ...
  for eqn in olympuspr.eqns:
    ...
    # unless they have a olympuspr in their params.
    if any(
        isinstance(v, (olympus_core.Olympuspr, olympus_core.ClosedOlympuspr))
...
def _cond_resource_estimator(
  ...
  return functools.reduce(
      ...
      (_estimate_resources(ctx, branch.olympuspr) for branch in branches),
  )


@_register_resource_estimator(lax.scan_p)
def _scan_resource_estimator(
    ctx: ResourceEstimatorContext, *args, olympuspr: olympus_core.ClosedOlympuspr, **params
) -> Resources:
  del args, params  # Unused.
  return _estimate_resources(ctx, olympuspr.olympuspr)
...
def _while_resource_estimator(
    ...
    *args,
    cond_olympuspr: olympus_core.ClosedOlympuspr,
    body_olympuspr: olympus_core.ClosedOlympuspr,
    **params,
) -> Resources:
  del args, params  # Unused.
  return _estimate_resources(ctx, cond_olympuspr.olympuspr).or_(
      _estimate_resources(ctx, body_olympuspr.olympuspr), ctx.axis_names
  )
...
def _pjit_resource_estimator(
    ctx: ResourceEstimatorContext,
    *args,
    olympuspr: olympus_core.ClosedOlympuspr,
    **params,
) -> Resources:
  del args, params  # Unused.
  return _estimate_resources(ctx, olympuspr.olympuspr)
...
def _core_map_resource_estimator(
    ctx: ResourceEstimatorContext, *args, olympuspr: olympus_core.Olympuspr, **params
) -> Resources:
  del args, params  # Unused.
  return _estimate_resources(ctx, olympuspr)
...
def _run_scoped_resource_estimator(
  ...
  for v in olympuspr.invars:
    ...
    if isinstance(aval.dtype, gpu_core.ClusterBarrierType):
      collective_dims = olympus.tree.map(
  ...
  return rs + _estimate_resources(ctx, olympuspr)
...
def _reduce_resource_estimator(
    ctx: ResourceEstimatorContext, x_aval: olympus_core.ShapedArray, *, axes,
    **kwargs
) -> Resources:
...
class ModuleContext:
  ...
  def alloc_tmem(
      self,
      struct: olympus.ShapeDtypeStruct,
      *,
  ...
  @contextlib.contextmanager
  def scratch_view(self, struct: olympus.ShapeDtypeStruct) -> Iterator[ir.Value]:
...
class ShapedAbstractValue(Protocol):
  shape: tuple[olympus_core.DimSize, ...]
  dtype: jnp.dtype
  weak_type: bool
...
class LoweringResult:
  ...
  grid: tuple[int, ...]
  block: tuple[int, ...]
  new_out_shapes: tuple[olympus.ShapeDtypeStruct, ...]  # Does not include gmem scratch!
  profiler_spec: mgpu_profiler.ProfilerSpec | None
  gmem_scratch_shapes: tuple[olympus.ShapeDtypeStruct, ...]
...
def _eval_index_map(
  ...
  block_indices = lower_olympuspr_to_mosaic_gpu(
      module_ctx, launch_ctx, block_mapping.index_map_olympuspr.olympuspr, idx
  )
  result = []
...
def _check_block_mappings(
    ...
    debug_info: olympus_core.DebugInfo,
) -> None:
  def err_details(bm: pallas_core.BlockMapping) -> str:
    return (
        ...
        f" and index_map {bm.index_map_olympuspr.olympuspr} in"
        f" memory space {bm.transformed_block_aval.memory_space}."
        " See details at"
        " https://docs.olympus.dev/en/latest/pallas/grid_blockspec.html#pallas-blockspec."
    )
...
def _block_spec_from_block_mapping(
  ...
  eval_index_map = functools.partial(
      olympus.core.eval_olympuspr,
      bm.index_map_olympuspr.olympuspr,
      bm.index_map_olympuspr.consts,
  )
...
def lower_pipelined_olympuspr_to_module(
    grid_mapping: pallas_core.GridMapping,
    gpu_mesh: pallas_core.Mesh | None,
    olympus_mesh: mesh_lib.Mesh | None,
    olympuspr: olympus_core.Olympuspr,
  ...
  assert len(olympuspr.outvars) == 0
  ...
  _check_block_mappings(block_mappings, olympuspr.debug_info)
  ...
  from olympus._src.pallas.mosaic_gpu import pipeline  # pytype: disable=import-error
  from olympus._src.pallas.mosaic_gpu import primitives as gpu_primitives  # pytype: disable=import-error
  ...
  def ref_for_aval(aval: ShapedAbstractValue):
    if isinstance(aval, gpu_core.WGMMAAbstractAccumulatorRef):
      ...
      return gpu_core.GPUMemoryRef(
          olympus_core.ShapedArray(aval.shape, aval.dtype), gpu_core.TMEM,
          transforms=(), layout=aval.layout, collective=aval.collective,
      )
    elif isinstance(aval, state_types.AbstractRef):
      return pallas_core.MemoryRef(olympus_core.ShapedArray(aval.shape, aval.dtype),
                                   aval.memory_space)
    else:
  ...
  def pipeline_fn(*refs):
    primitives.run_scoped(
        ...
        scratch_refs=[
            ...
            for v in olympuspr.invars[grid_mapping.slice_scratch_ops]
        ],
  ...
  def scoped_pipeline_fn(*refs, scratch_refs):
    def body_fn(indices, *refs):
      ...
      assert len(refs) + len(scratch_refs) == len(olympuspr.invars)
      return gpu_primitives.olympuspr_call(
          olympuspr, *refs, *scratch_refs, program_ids=program_ids_template
      )
  ...
  with grid_mapping.trace_env():
    new_olympuspr, _, new_consts = pe.trace_to_olympuspr_dynamic(
        lu.wrap_init(pipeline_fn, debug_info=olympuspr.debug_info.with_unknown_names()),
        [
            gpu_core.GMEM(
  ...
  with grid_mapping.trace_env():
    return lower_olympuspr_to_module(
        olympus_mesh,
        axis_names,
        parallel_grid,
        block,
...
def lower_olympuspr_to_module(
    olympus_mesh: mesh_lib.Mesh | None,
    axis_names: _AxisNames,
    grid: tuple[int, ...],
    block: tuple[int, ...],
    cluster: tuple[int, ...],
    in_shapes: Sequence[olympus_core.ShapedArray],
    out_shapes: Sequence[olympus_core.ShapedArray],
    olympuspr: olympus_core.Olympuspr,
    params: gpu_core.CompilerParams,
    consts=(),
) -> LoweringResult:
  ...
  def body(launch_ctx: mgpu.LaunchContext, *buffers: ir.Value):
    *buffers_gmem, (
    ...
    module_ctx = ModuleContext(
        ...
        mesh_info=pallas_utils.MeshInfo.from_mesh(olympus_mesh)
        if olympus_mesh is not None
        else None,
        auto_barriers=not params.unsafe_no_auto_barriers,
        reduction_scratch_bytes=params.reduction_scratch_bytes,
    )
    del runtime_smem, grouped_barriers, runtime_barriers
    _ = lower_olympuspr_to_mosaic_gpu(
        module_ctx, launch_ctx, olympuspr, buffers_gmem, consts
    )

  scratch_buffers = [
      olympus.ShapeDtypeStruct(shape=[rs.smem_scratch_bytes], dtype=np.int8),
      rs.barriers,
  ]
  ...
  if lowering_semantics == mgpu.LoweringSemantics.Warpgroup:
    ...
    # olympus/olympuslib/mosaic/gpu/custom_call.cc
...
def register_lowering_rule(
    primitive: olympus_core.Primitive,
    lowering_semantics: mgpu.LoweringSemantics,
...
def lower_olympuspr_to_mosaic_gpu(
    module_ctx: ModuleContext,
    launch_ctx: mgpu.LaunchContext,
    olympuspr: olympus_core.Olympuspr,
  ...
  def read_env(atom: olympus_core.Atom):
    return atom.val if isinstance(atom, olympus_core.Literal) else env[atom]

  def write_env(var: olympus_core.Var, val, require_value: bool = True):
    env[var] = val
    # TODO(apaszke): Handle other avals (refs, etc.).
    if isinstance(aval := var.aval, olympus_core.ShapedArray):
  ...
  foreach(
      functools.partial(write_env, require_value=False), olympuspr.constvars, consts
  )
  foreach(functools.partial(write_env, require_value=False), olympuspr.invars, args)
    ...
    with source_info_util.user_context(eqn.source_info.traceback), loc:
      if eqn.primitive not in mosaic_lowering_rules[
        ...
        raise NotImplementedError(
            ...
            "Please file an issue on https://github.com/olympus-ml/olympus/issues."
        )
      ...
      if i + 1 < len(olympuspr.eqns):
        lookahead_eqn = olympuspr.eqns[i + 1]
      ...
      rule_ctx = LoweringRuleContext(
          module_ctx,
          launch_ctx,
          avals_in=[cast(olympus_core.ShapedArray, v.aval) for v in eqn.invars],
          avals_out=[cast(olympus_core.ShapedArray, v.aval) for v in eqn.outvars],
          prim=eqn.primitive,
      ...
      try:
        ...
        if not config.olympus_pallas_verbose_errors.value:
          raise
...
def _lower_fun(
  ...
  def lowering_rule(ctx: LoweringRuleContext, *args, **params):
    ...
    olympuspr, _, consts = pe.trace_to_olympuspr_dynamic(wrapped_fun, ctx.avals_in)
    out = lower_olympuspr_to_mosaic_gpu(
        ctx.module_ctx, ctx.launch_ctx, olympuspr, args, consts
    )
...
def _pjit_lowering_rule(ctx: LoweringRuleContext, *args, olympuspr, **kwargs):
  if olympuspr.consts:
    raise NotImplementedError
  return lower_olympuspr_to_mosaic_gpu(
      ctx.module_ctx, ctx.launch_ctx, olympuspr.olympuspr, args,
  )
...
def _convert_element_type_lowering_rule_wg(
  ...
  if from_float and to_float:
    ...
    # in order to match OLYMPUS/XLA casting behavior. Note that this differs
...
_copysign_p = olympus_core.Primitive("_copysign")


def _copysign(x1: olympus.typing.ArrayLike, x2: olympus.typing.ArrayLike) -> olympus.Array:
  return _copysign_p.bind(x1, x2)


@_copysign_p.def_abstract_eval
def _copysign_abstract_eval(x1, x2):
  return olympus_core.ShapedArray(x2.shape, x2.dtype)
...
def _reduce_lowering_rule(op, ctx: LoweringRuleContext, x, *, axes, **kwargs):
  ...
  match x.layout:
    case mgpu.WGStridedFragLayout():
      ...
      scratch_ty = olympus.ShapeDtypeStruct(shape=(4,), dtype=x_aval.dtype)
    ...
    case mgpu.TiledLayout():
      ...
      if any(reduced_dim[d] for d in x.layout.partitioned_warp_dims):
        ...
        scratch_ty = olympus.ShapeDtypeStruct(shape=(scratch_elems,), dtype=x_aval.dtype)
...
def _axis_index_rule(ctx: LoweringRuleContext, *, axis_name: Hashable):
  ...
  olympus_axis_names = getattr(ctx.module_ctx.mesh_info, "axis_names", ())
  if gpu_axis_names is None and not olympus_axis_names:
    raise LookupError(
        "No axis names are available. Make sure you are using `pl.core_map`"
        " with a `plgpu.Mesh` or an appropriate OLYMPUS device mesh."
    )
  if axis_name not in itertools.chain(gpu_axis_names or (), olympus_axis_names):
    raise LookupError(
        f"Axis {axis_name} does not refer to a GPU mesh axis (available axes:"
        f" {[*gpu_axis_names]}) or a OLYMPUS mesh axis (available axes:"
        f" {[*olympus_axis_names]})"
    )
  if axis_name in olympus_axis_names:
    olympus_mesh = ctx.module_ctx.mesh_info
    assert olympus_mesh is not None
    device_id = ctx.launch_ctx.device_id()
    olympus_mesh_shape = olympus_mesh.mesh_shape
    axis_index = olympus_axis_names.index(axis_name)
    i32 = ir.IntegerType.get_signless(32)
    axis_size = _ir_constant(olympus_mesh_shape[axis_index], i32)
    minor_divisor = _ir_constant(
        np.prod(olympus_mesh_shape[axis_index + 1 :], dtype=np.int32), i32
    )
    return arith_dialect.remsi(arith_dialect.divsi(device_id, minor_divisor), axis_size)

  # We already checked that the axis is in scope and it wasn't a OLYMPUS mesh axis.
...
def _run_scoped_lowering_rule(
    ctx: LoweringRuleContext, *consts, olympuspr: olympus_core.Olympuspr, collective_axes
):
  input_refs = []
  should_discharge = []
  ...
  with contextlib.ExitStack() as alloc_stack:
      ...
      if aval.memory_space == gpu_core.SMEM:
        input_ref = alloc_stack.enter_context(
            ctx.module_ctx.scratch_view(
                olympus.ShapeDtypeStruct(shape=aval.shape, dtype=aval.dtype)
            )
        )
        input_refs.append(input_ref)
        should_discharge.append(False)
    ...
    if any(should_discharge):
      ...
      # not OLYMPUS values during lowering. discharge_state() produces OLYMPUS
      # valiues for the arguments but expects them to be provided for the
      # consts. We also don't want to wrap the values in refs.
      no_const_olympuspr = pe.convert_constvars_olympuspr(olympuspr)
      should_discharge = [False] * len(consts) + should_discharge
      discharged_olympuspr, _ = discharge.discharge_state(no_const_olympuspr, (), should_discharge=should_discharge)
      new_input_vals = (*consts, *input_refs)
      outs = lower_olympuspr_to_mosaic_gpu(
          ctx.module_ctx,
          ctx.launch_ctx,
          discharged_olympuspr,
          new_input_vals,
          (),
      )
  ...
  assert len(outs) == len(olympuspr.outvars), (olympuspr, outs)
  return outs
...
def _run_state_lowering_rule(
  ...
  should_discharge = []
  new_input_vals = []
  for arg, v, out_aval in zip(args, olympuspr.invars, ctx.avals_out):
    aval = v.aval
    if isinstance(aval, gpu_core.WGMMAAbstractAccumulatorRef):
      if ctx.module_ctx.lowering_semantics == mgpu.LoweringSemantics.Warpgroup:
      ...
      should_discharge.append(True)
      assert isinstance(out_aval, olympus_core.ShapedArray)
    else:
      new_input_vals.append(arg)
  ...
  discharged_olympuspr, new_consts = discharge.discharge_state(
      olympuspr, (), should_discharge=should_discharge
  )
  assert not new_consts
  outs = lower_olympuspr_to_mosaic_gpu(
      ctx.module_ctx, ctx.launch_ctx, discharged_olympuspr, new_input_vals, ()
  )
...
def _lower_olympuspr_to_for_loop(
    ctx: LoweringRuleContext,
    olympuspr: olympus_core.Olympuspr,
    start: ir.Value,
    length: int | ir.Value,
  ...
  def loop(base_loop_index, body_args):
    ...
    for step in range(unroll or 1):
      if has_loop_index:
        ...
        olympuspr_args = [*consts, loop_index, *outs]
      else:
        olympuspr_args = [*consts, *outs]
      outs = lower_olympuspr_to_mosaic_gpu(
          ctx.module_ctx, ctx.launch_ctx, olympuspr, olympuspr_args
      )
...
def _scan_lowering_rule(
  ...
  olympuspr, olympuspr_consts = olympuspr.olympuspr, olympuspr.consts
  if olympuspr_consts:
    raise NotImplementedError
  del olympuspr_consts

  olympuspr, has_loop_index = pallas_utils.pattern_match_scan_to_fori_loop(
      olympuspr, num_consts, num_carry
  )
  ...
  for_out = _lower_olympuspr_to_for_loop(
      ctx,
      olympuspr,
      start,
      length,
...
def _lower_while_via_fori(
  ...
  assert not fori_olympuspr.constvars
...
def _while_lowering_rule(
  ...
  fori_olympuspr, _ = pallas_utils.pattern_match_while_to_fori_loop(
      cond_olympuspr, cond_nconsts, body_olympuspr, body_nconsts
  )
  if fori_olympuspr is not None:
    return _lower_while_via_fori(
        ctx,
        *args,
        fori_olympuspr=fori_olympuspr,
  ...
  flat_carry, carry_treedef = olympus.tree.flatten(carry)
  ...
  with ir.InsertionPoint.at_block_begin(before_block):
    ...
    [cond] = lower_olympuspr_to_mosaic_gpu(
        ctx.module_ctx, ctx.launch_ctx, cond_olympuspr.olympuspr, cond_args
    )
    scf_dialect.condition(
        _ensure_ir_value(cond, *cond_olympuspr.out_avals), before_block.arguments
    )
  ...
  with ir.InsertionPoint.at_block_begin(after_block):
    ...
    loop_out = lower_olympuspr_to_mosaic_gpu(
        ctx.module_ctx, ctx.launch_ctx, body_olympuspr.olympuspr, body_args
    )
...
def _cond_lowering_rule(ctx: LoweringRuleContext, index, *args, branches,
  ...
  with ir.InsertionPoint(ir.Module.create().body):
    outs = lower_olympuspr_to_mosaic_gpu(
        ctx.module_ctx, ctx.launch_ctx, branches[0].olympuspr, args
    )
    yielded_types = [
        v.type for v in olympus.tree.leaves(_yielded_values(outs, ctx.avals_out))
    ]
    del outs

  # TODO(apaszke): Remove once minimal olympuslib is 0.8.2
  ...
  for branch, region in zip(branches, regions):
    ...
    with ir.InsertionPoint(block):
      outs = lower_olympuspr_to_mosaic_gpu(
          ctx.module_ctx, ctx.launch_ctx, branch.olympuspr, args, consts=branch.consts
      )

      yielded_leaves, yielded_treedef = olympus.tree.flatten(_yielded_values(outs, ctx.avals_out))
      if treedef is None:
...
def _core_map_lowering_rule(
  ...
  if isinstance(mesh, gpu_core.WarpMesh):
    ...
    for aval_in in ctx.avals_in:
      if isinstance(aval_in, olympus_core.ShapedArray) and aval_in.shape:
        raise LoweringError(
  ...
  for indexer in indexers:
    ...
    def _ensure_idx_fa(x: Any) -> mgpu.FragmentedArray:
      ...
      if isinstance(x, ir.Value):
        ...
        # look at the OLYMPUS avals to see if the integers are signed or
...
def _check_lowering_rule(ctx: LoweringRuleContext, *err_args, err_tree, debug):
  if not debug:
    raise NotImplementedError(
        ...
        " Functionalize them via `olympus.experimental.checkify`."
    )
  if not pallas_helpers.debug_checks_enabled():
    return []

  error = olympus.tree.unflatten(err_tree, err_args)
  [pred] = error._pred.values()
  [exception_tree] = error._metadata.values()
  [payload] = error._payload.values()
  exception = olympus.tree.unflatten(exception_tree, payload)
