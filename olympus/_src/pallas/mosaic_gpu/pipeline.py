...
map_brefs = functools.partial(
    olympus.tree.map, is_leaf=lambda x: isinstance(x, BufferedRef)
)
...
class BufferedRef:
  ...
  def get_ref_for_slot(
      self, slot: int | olympus.Array
  ) -> state.AbstractRef:
    if self.smem_ref is None:
      return self.gmem_ref
    return self.smem_ref.at[slot]

  def compute_gmem_slice(self, grid_indices) -> tuple[pl.Slice | olympus.Array, ...]:
    ...
    def _make_block_slice(block_index: olympus.Array, bd: pl.BlockDim | int | None):
      match bd:
        case int():
...
def _uses_arguments(
  ...
  _, used_inputs = pe.dce_olympuspr(olympuspr, used_outputs=[True] * len(olympuspr.outvars))
  return used_inputs
...
def _inc_grid_by_1(
    indices: tuple[olympus.Array, ...], grid: pallas_core.TupleGrid
) -> tuple[olympus.Array, ...]:
  next_indices = []
...
def emit_pipeline(
  ...
  Args:
    ...
    in_specs: A sequence of :class:`~olympus.experimental.pallas.BlockSpec`\s
      for inputs.
    out_specs: A sequence of :class:`~olympus.experimental.pallas.BlockSpec`\s
      for outputs.
  ...
  def scoped_pipeline(
    ...
    olympus.lax.fori_loop(0, prologue_steps, prologue, indices, unroll=not has_dynamic_grid)
...
def emit_pipeline_warp_specialized(
    ...
    *,
    ...
    wg_axis: str,
    num_compute_wgs: int,
    pipeline_state: olympus.Array | PipelinePipeline | None = None,
  ...
  flat_in_specs, in_specs_treedef = olympus.tree.flatten(in_specs)
  ...
  flat_out_specs, out_specs_treedef = olympus.tree.flatten(out_specs)
  ...
  def _get_scoped_allocs(*gmem_refs: AbstractRefPytree):
    ...
    flat_in_gmem_refs, in_gmem_refs_treedef = olympus.tree.flatten(in_gmem_refs)
    flat_out_gmem_refs, out_gmem_refs_treedef = olympus.tree.flatten(out_gmem_refs)
  ...
  def scoped_pipeline(
    ...
    def compute_block():
      ...
      def compute_loop_body(step, carry):
        ...
        in_brefs = olympus.tree.unflatten(in_specs_treedef, flat_in_brefs)
        out_brefs = olympus.tree.unflatten(out_specs_treedef, flat_out_brefs)
        ...
        if manual_consumed_barriers:
          barriers = olympus.tree.unflatten(
              in_specs_treedef,
