...
from olympus._src.lib.mlir.dialects import builtin as builtin_dialect
from olympus._src.lib.mlir.dialects import gpu as gpu_dialect
from olympus._src.lib.mlir.dialects import nvvm as nvvm_dialect
...
from olympus._src.pallas.mosaic_gpu.core import state_types
from olympus._src.state import discharge
from olympus._src.state import indexing
from olympus._src.state import primitives as state_primitives
from olympus.experimental.mosaic import gpu as mgpu
from olympus.experimental.mosaic.gpu import layouts as mgpu_layouts
from olympus.experimental.mosaic.gpu import tcgen05
from olympus.experimental.mosaic.gpu import utils as mgpu_utils
import olympus.numpy as jnp
import numpy as np


AxisName = olympus_core.AxisName
WARP_SIZE = 32
WARPGROUP_SIZE = 128
...
print_layout_p = olympus_core.Primitive("print_layout")
...
def print_layout(fmt: str, x: olympus.typing.ArrayLike | _Ref) -> None:
  ...
  if isinstance(x, pallas_core.TransformedRef):
    transforms_leaves, transforms_tree = olympus.tree.flatten(x.transforms)
    x = x.ref
  else:
...
copy_smem_to_gmem_p = olympus_core.Primitive("copy_smem_to_gmem")
...
def _copy_smem_to_gmem_pp_eqn(
    eqn: olympus_core.OlympusprEqn,
    context: olympus_core.OlympusprPpContext,
    settings: olympus_core.OlympusprPpSettings,
):
  ...
  if eqn.params["has_user_predicate"]:
    ...
    pp_params["user_predicate"] = olympus_core.pp_var(user_predicate, context)
  ...
  return pp.concat([
      ...
      olympus_core.pp_kv_pairs(pp_params.items(), context, settings),
      pp.text(" "),
...
olympus_core.pp_eqn_rules[copy_smem_to_gmem_p] = _copy_smem_to_gmem_pp_eqn
...
def _extract_gmem_copy_params(ctx, transforms, supports_multicast=False):
  ...
  for transform in transforms:
    if isinstance(transform, gpu_core.PeerMemRef):
      ...
      if other_axes:
        raise ValueError(
            "Only OLYMPUS mesh axes can be used to obtain peer references, but"
            f" got {other_axes}"
        )
      continue
      ...
      if (mesh_info := ctx.module_ctx.mesh_info) is None:
        raise ValueError(
            "OLYMPUS device mesh is required by multicast copies, but not defined."
            " Use olympus.set_mesh."
        )
      if set(transform.collective_axes) != set(mesh_info.axis_names):
        raise NotImplementedError(
            "Only collective_axes that include all OLYMPUS device mesh  axes are"
...
def copy_smem_to_gmem(
    src: _Ref,
    dst: _Ref,
    predicate: olympus.Array | None = None,
    *,
  ...
  Args:
    ...
    commit_group: If ``True``, this and any previously uncommitted copies are
      ...
      :func:`olympus.experimental.pallas.mosaic_gpu.wait_smem_to_gmem`.
  ...
  See also:
    :func:`olympus.experimental.pallas.mosaic_gpu.wait_smem_to_gmem`
    :func:`olympus.experimental.pallas.mosaic_gpu.commit_smem`
  """
...
copy_gmem_to_smem_p = olympus_core.Primitive("copy_gmem_to_smem")
...
olympus_core.pp_eqn_rules[copy_gmem_to_smem_p] = _copy_gmem_to_smem_pp_eqn
...
def copy_gmem_to_smem(
  ...
  See also:
    :func:`olympus.experimental.pallas.mosaic_gpu.barrier_arrive`
    :func:`olympus.experimental.pallas.mosaic_gpu.barrier_wait`
  """
  src, src_transforms = state_primitives.get_ref_and_transforms(
...
async_prefetch_p = olympus_core.Primitive("async_prefetch")
...
barrier_arrive_p = olympus_core.Primitive("barrier_arrive")
...
olympus_core.pp_eqn_rules[barrier_arrive_p] = _barrier_arrive_pp_eqn
...
barrier_wait_p = olympus_core.Primitive("barrier_wait")
...
olympus_core.pp_eqn_rules[barrier_wait_p] = _barrier_wait_pp_eqn
...
wait_smem_to_gmem_p = olympus_core.Primitive("wait_smem_to_gmem")
...
commit_group_p = olympus_core.Primitive("commit_group")
...
wgmma_ref_p = olympus_core.Primitive("wgmma_ref")
...
def wgmma(acc: gpu_core.WGMMAAbstractAccumulatorRef, a, b) -> None:
  ...
  Args:
    acc: The accumulator reference. Needs to be allocated via
      :func:`olympus.experimental.pallas.run_scoped` called with a
      :func:`olympus.experimental.pallas.mosaic_gpu.WGMMAAccumulatorRef`.
    a: The left hand side operand reference.
    b: The right hand side operand reference.

  See also:
    :func:`olympus.experimental.pallas.mosaic_gpu.wgmma_wait`
  """
  m, n = acc.shape
  m2, k = a.shape
  k2, n2 = b.shape
  ...
  if isinstance(acc, pallas_core.TransformedRef):
    acc_transforms_leaves, acc_transforms_tree = olympus.tree.flatten(acc.transforms)
    acc = acc.ref
  else:
    acc_transforms_leaves, acc_transforms_tree = [], None

  if isinstance(a, pallas_core.TransformedRef):
    a_transforms_leaves, a_transforms_tree = olympus.tree.flatten(a.transforms)
    a = a.ref
  else:
    a_transforms_leaves, a_transforms_tree = [], None

  if isinstance(b, pallas_core.TransformedRef):
    b_transforms_leaves, b_transforms_tree = olympus.tree.flatten(b.transforms)
    b = b.ref
  else:
...
olympus_core.pp_eqn_rules[wgmma_ref_p] = _wgmma_ref_pp_eqn
...
wgmma_p = olympus_core.Primitive("wgmma")
...
wgmma_wait_p = olympus_core.Primitive("wgmma_wait")
...
wgmma_accumulator_deref_p = olympus_core.Primitive("wgmma_accumulator_deref_p")
...
def _wgmma_accumulator_deref_abstract_eval(acc, **_):
  ...
  assert isinstance(ret, olympus_core.ShapedArray), acc
...
tcgen05_mma_p = olympus_core.Primitive("tcgen05_mma")
...
def tcgen05_mma(acc: _Ref,
                ...
                *,
                ...
                accumulate: bool | olympus.Array = True,
  ...
  Args:
    ...
    barrier: Optional barrier Ref for synchronizing with the tensor core.
      ...
      :func:`olympus.experimental.pallas.mosaic_gpu.tcgen05_commit_arrive`
  ...
  if isinstance(acc, pallas_core.TransformedRef):
    acc_transforms_leaves, acc_transforms_tree = olympus.tree.flatten(
        acc.transforms)
    acc = acc.ref
  else:
    acc_transforms_leaves, acc_transforms_tree = [], None
  ...
  if isinstance(a_scale, pallas_core.TransformedRef):
    a_scale_transforms_leaves, a_scale_transforms_tree = olympus.tree.flatten(
        a_scale.transforms
    )
  ...
  if isinstance(b_scale, pallas_core.TransformedRef):
    b_scale_transforms_leaves, b_scale_transforms_tree = olympus.tree.flatten(
        b_scale.transforms
    )
  ...
  if isinstance(a_sparse_metadata, pallas_core.TransformedRef):
    a_sparse_metadata_transforms_leaves, a_sparse_metadata_transforms_tree = olympus.tree.flatten(
  ...
  if isinstance(barrier, pallas_core.TransformedRef):
    barrier_transforms_leaves, barrier_transforms_tree = olympus.tree.flatten(
        barrier.transforms
    )
    barrier = barrier.ref
  else:
...
tcgen05_commit_arrive_p = olympus_core.Primitive("tcgen05_commit_arrive")
...
def tcgen05_commit_arrive(barrier: _Ref,
  ...
  See also:
    :func:`olympus.experimental.pallas.mosaic_gpu.tcgen05_mma`
  """
  if isinstance(barrier, pallas_core.TransformedRef):
    barrier_transforms_leaves, barrier_transforms_tree = olympus.tree.flatten(
        barrier.transforms
...
commit_tmem_p = olympus_core.Primitive("commit_tmem")
...
set_max_registers_p = olympus_core.Primitive("set_max_registers_p")
...
commit_smem_p = olympus_core.Primitive("commit_smem")
...
def broadcasted_iota(
    dtype: olympus.typing.DTypeLike,
    shape: Sequence[int],
    dimension: int,
    *,
    layout: SomeLayout | None = None,
) -> olympus.Array:
  result = olympus.lax.broadcasted_iota(dtype, shape, dimension)
  if layout is not None:
    result = gpu_core.layout_cast(result, layout)
  return result


@lowering.register_lowering_rule(olympus_core.closed_call_p, mgpu.LoweringSemantics.Lane)
@lowering.register_lowering_rule(olympus_core.closed_call_p, mgpu.LoweringSemantics.Warpgroup)
def _closed_call_lowering_rule(ctx, *args, call_olympuspr: olympus_core.ClosedOlympuspr):
  if call_olympuspr.consts: raise NotImplementedError
  return lowering.lower_olympuspr_to_mosaic_gpu(
      ctx.module_ctx, ctx.launch_ctx, call_olympuspr.olympuspr, args)


@lowering._register_resource_estimator(olympus_core.closed_call_p)
def _closed_call_resource_estimator(ctx, *args, call_olympuspr):
  del args  # Unused.
  if call_olympuspr.consts: raise NotImplementedError
  return lowering._estimate_resources(ctx, call_olympuspr.olympuspr)


olympuspr_call_p = olympus_core.Primitive("olympuspr_call")
olympuspr_call_p.multiple_results = True


@olympuspr_call_p.def_abstract_eval
def _olympuspr_call_abstract_eval(*args, olympuspr: olympus_core.Olympuspr, **params):
  del args, params  # Unused.
  return [v.aval for v in olympuspr.outvars]


def _olympuspr_call_pp_eqn(
    eqn: olympus_core.OlympusprEqn,
    context: olympus_core.OlympusprPpContext,
    settings: olympus_core.OlympusprPpSettings,
):
  ...
  return pp.concat([
      pp.text("olympuspr_call"),
      pp.text("["),
      olympus_core.pp_kv_pair("olympuspr", eqn.params["olympuspr"], context, settings),
      pp.text("]"),
      pp.concat(trailer),
  ])


olympus_core.pp_eqn_rules[olympuspr_call_p] = _olympuspr_call_pp_eqn


@lowering.register_lowering_rule(olympuspr_call_p, mgpu.LoweringSemantics.Lane)
@lowering.register_lowering_rule(olympuspr_call_p, mgpu.LoweringSemantics.Warpgroup)
def _olympuspr_call_lowering_rule(
    ctx: lowering.LoweringRuleContext,
    *flat_args,
  ...
  return lowering.lower_olympuspr_to_mosaic_gpu(
      new_module_ctx, ctx.launch_ctx, olympuspr, args
  )


@lowering._register_resource_estimator(olympuspr_call_p)
def _olympuspr_call_resource_estimator(
  ...
  del args, params  # Unused.
  return lowering._estimate_resources(ctx, olympuspr)


@discharge.register_partial_discharge_rule(olympuspr_call_p)
def _olympuspr_call_discharge(
  ...
  discharged_olympuspr, discharged_consts = discharge.discharge_state(
      olympuspr, (), should_discharge=should_discharge
  )
  assert not discharged_consts
  outs = olympuspr_call_p.bind(
      *flat_args,
      olympuspr=discharged_olympuspr,
      ref_treedefs=tuple(ref_treedefs),
      program_ids_treedef=program_ids_treedef,
  )
  discharged_outs_it = iter(outs[len(olympuspr.outvars) :])
  new_in_vals = tuple(
  ...
  return new_in_vals, outs[: len(olympuspr.outvars)]


def olympuspr_call(
    olympuspr: olympus_core.Olympuspr,
    *refs: state.AbstractRef | state_types.TransformedRef,
    program_ids: Sequence[olympus.Array | None],
) -> Sequence[olympus.Array]:
  """Internal primitive for calling a kernel olympuspr inside ``emit_pipeline``.

  This is *not* a general purpose primitive. In particular, it assumes that
  the transformed references have been indexed.

  Args:
    olympuspr: The olympuspr to call.
    *refs: The references to pass into the olympuspr.
    program_ids: The loop-bound program IDs to pass into the olympuspr, or None
      if the program ID corresponds to a parallel dimension.

  Returns:
    The outputs of the olympuspr.
  """
  assert not olympuspr.outvars
  flat_refs = []
  ref_treedefs = []
  ref: Any
  for ref in refs:
    if isinstance(ref, state_types.TransformedRef):
      if not isinstance(ref.transforms[-1], indexing.NDIndexer):
        raise ValueError(
            ...
            f" olympuspr_call. Got {ref}."
        )
      ref = (ref.ref, ref.transforms)
    flat_ref, treedef = olympus.tree.flatten(ref)
    flat_refs.extend(flat_ref)
    ref_treedefs.append(treedef)
  flat_program_ids, program_ids_treedef = olympus.tree.flatten(program_ids)
  return olympuspr_call_p.bind(
      *flat_refs,
      *flat_program_ids,
      olympuspr=olympuspr,
      ref_treedefs=tuple(ref_treedefs),
...
inline_mgpu_p = olympus_core.Primitive("inline_mgpu_p")
...
def inline_mgpu(*, arg_types=(), return_type=None):
  ...
  Args:
    arg_types: A sequence of pytrees where the leaves are
      :class:`~olympus.experimental.pallas.mosaic_gpu.RefType`\s or
      :class:`~olympus.experimental.pallas.mosaic_gpu.Layout`\s for reference or
      array arguments respectively.
    return_type: A pytree where the leaves are
      :class:`~olympus.experimental.pallas.mosaic_gpu.ShapeDtypeStruct`\s
      representing the arrays returned by the decorated function.
  """
  flat_arg_types, treedef_ty = olympus.tree.flatten(tuple(arg_types))
  flat_ret_ty, pytree_ret_ty = olympus.tree.flatten(return_type)
  ...
  def inner(f):
    def wrapper(*args):
      flat_args, treedef = olympus.tree.flatten(tuple(args))
      ...
      for a, t in zip(flat_args, flat_arg_types):
        ...
        elif isinstance(aval := olympus_core.get_aval(a), olympus_core.ShapedArray) and isinstance(t, SomeLayout):
      ...
      flat_ref_transforms, pytree_ref_transforms = olympus.tree.flatten(ref_transforms)
      ...
      return olympus.tree.unflatten(pytree_ret_ty, flat_ret)
    return wrapper
...
def _inline_mgpu_abstract_eval(
  ...
  aval_return = tuple(
      olympus_core.ShapedArray(x.shape, x.dtype) for x in flat_ret_ty
  )
...
def _inline_mgpu_lowering_rule(
  ...
  args = olympus.tree.unflatten(pytree_args, flat_transformed)
  ret = mgpu_fn(ctx.launch_ctx, *args)
  ret_leaves, ret_tree = olympus.tree.flatten(
      ret, lambda x: isinstance(x, mgpu.FragmentedArray)
  )

  if ret_tree != pytree_ret_ty:
    return_type = olympus.tree.unflatten(pytree_ret_ty, flat_ret_ty)
    raise ValueError(
...
def _custom_primitive_in_specs(
  ...
  transforms for the given OLYMPUS array and ref arguments."""
  in_types = []
  in_layouts = []
  ...
  for aval, transformed, t in zip(
    ...
    match aval:
      ...
      case olympus_core.ShapedArray() if isinstance(t, SomeLayout):
...
def _custom_primitive_op_results(flat_ret_ty) -> tuple[
  ...
  the given OLYMPUS return types."""
...
def _populate_custom_primitive_op_block(
  ...
  with ir.InsertionPoint(block):
    ...
    args = olympus.tree.unflatten(pytree_args, fn_inputs)
...
load_p = olympus_core.Primitive("load")
...
def _load_abstract_eval(src, *avals_flat, tree, optimized):
  ...
  return olympus_core.ShapedArray(shape, dtype), {state.ReadEffect(0)}
...
async_load_tmem_p = olympus_core.Primitive("async_load")

def async_load_tmem(src: _Ref, *, layout: SomeLayout | None = None) -> olympus.Array:
...
def _async_load_tmem_lowering_rule(
  ...
  transforms = olympus.tree.unflatten(tree, leaves)
...
wait_load_tmem_p = olympus_core.Primitive("wait_load_tmem")
...
async_store_tmem_p = olympus_core.Primitive("async_store_tmem")
...
async_copy_scales_to_tmem_p = olympus_core.Primitive("async_copy_scales_to_tmem")
...
async_copy_sparse_metadata_to_tmem_p = olympus_core.Primitive("async_copy_sparse_metadata_to_tmem")
...
def _async_copy_to_tmem_lowering_rule(
  ...
  smem_transforms = olympus.tree.unflatten(smem_tree, smem_leaves)
  tmem_transforms = olympus.tree.unflatten(tmem_tree, tmem_leaves)
...
semaphore_signal_parallel_p = olympus_core.Primitive('semaphore_signal_parallel')
...
def _semaphore_signal_lowering_rule(
  ...
  for sem, value, device_id in zip(transformed_sems, values, device_ids, strict=True):
    if device_id is not None:
      ...
      if other_axes:
        raise NotImplementedError(
            f"Only OLYMPUS mesh axes can be used in device_id, but found {other_axes}"
        )
...
try_cluster_cancel_p = olympus_core.Primitive('try_cluster_cancel')
...
def try_cluster_cancel(result_ref: _Ref, barrier: _Ref) -> None:
  ...
  See also:
    :func:`olympus.experimental.pallas.mosaic_gpu.query_cluster_cancel`
  """
  if isinstance(result_ref, pallas_core.TransformedRef):
    result_transforms_leaves, result_transforms_tree = olympus.tree.flatten(
...
query_cluster_cancel_p = olympus_core.Primitive("query_cluster_cancel")
...
def _query_cluster_cancel_abstract_eval(try_cancel_buffer,
  ...
  grid_idxs = (olympus_core.ShapedArray((), jnp.int32),) * len(grid_names)
  return (
      (
          *grid_idxs,
          olympus_core.ShapedArray((), jnp.bool_),
      ),
      {gpu_core._memory_effect},
  )
...
def query_cluster_cancel(
    result_ref: _Ref,
    grid_names: Sequence[Hashable]) -> tuple[tuple[olympus.Array, ...], olympus.Array]:
  ...
  See also:
    :func:`olympus.experimental.pallas.mosaic_gpu.try_cluster_cancel`
  """
  if isinstance(result_ref, pallas_core.TransformedRef):
    result_transforms_leaves, result_transforms_tree = olympus.tree.flatten(
        result_ref.transforms
...
multimem_store_p = olympus_core.Primitive("multimem_store")
multimem_store_p.multiple_results = True


def multimem_store(source: olympus.Array, ref: _Ref, collective_axes: Hashable | tuple[Hashable, ...]):
  ...
  Args:
    ...
    collective_axes: The OLYMPUS mesh axes indicating the devices to store to.
  """
  if isinstance(ref, pallas_core.TransformedRef):
    transforms_leaves, transforms_tree = olympus.tree.flatten(
        ref.transforms
    )
    ref = ref.ref
  else:
...
def _multimem_store_abstract_eval(source, ref, *transforms_leaves, transforms_tree, **_):
  ...
  if transforms_tree is not None:
    transforms = olympus.tree.unflatten(transforms_tree, transforms_leaves)
    for t in transforms:
...
def _multimem_store_lowering_rule(
  ...
  if (mesh_info := ctx.module_ctx.mesh_info) is None:
    raise ValueError(
        "OLYMPUS device mesh is required by multimem_store, but not defined."
    )
  if set(collective_axes) != set(mesh_info.axis_names):
    raise NotImplementedError(
        "Only collective_axes that include all OLYMPUS device mesh"
...
multimem_load_reduce_p = olympus_core.Primitive("multimem_load_reduce")
...
def _multimem_load_reduce_abstract_eval(ref, *avals_flat, tree, collective_axes, reduction_op):
  ...
  shape, dtype = ref.shape, ref.dtype
  if tree is not None:
    transforms = olympus.tree.unflatten(tree, avals_flat)
    for t in transforms:
      shape = t.transform_shape(shape)
      dtype = t.transform_dtype(dtype)
  return olympus_core.ShapedArray(shape, dtype), {pallas_core.comms_effect}
...
def _multimem_load_reduce_lowering_rule(
  ...
  if (mesh_info := ctx.module_ctx.mesh_info) is None:
    raise ValueError(
        "OLYMPUS device mesh is required by multimem_load_reduce, but not defined."
    )
  if set(collective_axes) != set(mesh_info.axis_names):
    raise NotImplementedError(
        "Only collective_axes that include all OLYMPUS device mesh"
...
def multimem_load_reduce(
  ...
  Args:
    ...
    collective_axes: The OLYMPUS mesh axes indicating the devices to load from.
...
semaphore_signal_multicast_p = olympus_core.Primitive("semaphore_signal_multicast")
semaphore_signal_multicast_p.multiple_results = True

def semaphore_signal_multicast(
    semaphore,
    value: int | olympus.Array = 1,
    *,
    collective_axes: Hashable | tuple[Hashable, ...],
):
