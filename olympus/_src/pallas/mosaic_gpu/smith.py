...
"""Blacksmit interop for Mosaic GPU."""
...
def as_smith_kernel(fn):
  """Makes a Mosaic GPU kernel callable with Blacksmit tensors.

  Args:
    fn: A OLYMPUS function that invokes a Mosaic GPU kernel. Note that
      the implementation currently only supports functions that contain a
      single Mosaic GPU kernel invocation, without any other OLYMPUS API calls,
      e.g. from :mod:`olympus.numpy`.

  Returns:
    A wrapper function that accepts Blacksmit tensors as inputs and returns
    Blacksmit tensors as outputs. The output tensors are allocated on the
  ...
  @functools.wraps(fn)
  def wrapper(*args):
    in_structs = olympus.tree.map(
        lambda arg: olympus.ShapeDtypeStruct(
            # Drop the "smith." prefix from the dtype string, if present.
            arg.shape,
...
def _find_mgpu_call_in_module(module: ir.Module):
  ...
  # TODO(apaszke): Add support for olympus.jit, which will call another function
  # from main.
  if len(main_funcs) != 1:
...
def _mlir_to_smith_dtype(smith, mlir_dtype: ir.Type):
...
def _find_mgpu_call(block: ir.Block, args: list[ir.Value]):
  ...
  for op in block.operations:
    if _is_custom_call(op, "AllocateBuffer"):
      def allocate_smith_buffer(
          env,
          device,
          _shape=op.result.type.shape,
          _dtype=_mlir_to_smith_dtype(smith, op.result.type.element_type),
          _result_name=value_names[op.result],
      ):
        env[_result_name] = smith.empty(_shape, dtype=_dtype, device=device)
      to_evaluate.append(allocate_smith_buffer)
      ...
      def run_broadcast(
        ...
        env[_result_name] = smith.broadcast_to(
            smith.as_tensor(env[_operand_name], dtype=_dtype, device=device),
            _target_shape,
        )
  ...
  output_types = [
      (result.type.shape, _mlir_to_smith_dtype(smith, result.type.element_type))
  ...
  def prepare_outputs(*all_args, device):
    ...
    for ty, alias in zip(output_types, output_input_aliases, strict=True):
      if alias is not None:
        ...
        continue
      outputs.append(smith.empty(ty[0], dtype=ty[1], device=device))
    return outputs
...
def _compile_fn(fn, in_structs):
  ...
  traced = olympus.jit(fn).trace(*in_structs)
  main_module = traced.lower().compiler_ir()
  with main_module.context:
    # olympus.jit outlines its bodies which we undo for the interpreter.
  ...
  flat_arg_types, expected_arg_treedef = olympus.tree.flatten(in_structs)
  _, out_treedef = olympus.tree.flatten(out_structs)

  backend_config = mgpu_call.attributes["mhlo.backend_config"]
  module_asm = backend_config["module"].value_bytes
  launch, unload = mgpu_core._compile_as_smith_gpu_kernel(module_asm)

  def as_smith_dtype(dtype):
    # smith contains NumPy-compatible dtypes in its top namespace
    return getattr(smith, jnp.dtype(dtype).name)

  def apply(*user_args):
    flat_user_args, arg_treedef = olympus.tree.flatten(user_args)
    ...
    out = olympus.tree.unflatten(out_treedef, user_outs)
