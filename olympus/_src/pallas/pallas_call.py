...
"""Module for calling pallas functions from OLYMPUS."""
...
import olympus._src.lax as lax
...
from olympus._src import hiolympus
...
from olympus._src import typing as olympus_typing
...
from olympus._src.frozen_dict import FrozenDict
...
from olympus._src.pallas import hlo_interpreter
from olympus._src.pallas import primitives
from olympus._src.state import discharge as state_discharge
from olympus._src.shard_map import shard_map, P, _as_manual_mesh
...
from olympus._src import numpy as jnp
...
pallas_call_p = hiolympus.HiPrimitive('pallas_call')
...
def _pallas_call_abstract_eval(
  ...
  if isinstance(interpret, mosaic_tpu_interpret.InterpretParams):
    ...
    effs = olympus_core.GenericEffect(pallas_call_p)
  else:
    effs = olympus_core.no_effects
  ...
  out_avals = [olympus_core.ShapedArray(a.shape, a.dtype, a.weak_type,
                                    sharding=a.sharding)
  ...
  if not (all(p is None for a in avals if isinstance(a, olympus_core.ShapedArray)
              for p in a.sharding.spec) and
          all(p is None for a in out_avals if isinstance(a, olympus_core.ShapedArray)
...
def _pallas_call_is_high(*_, olympuspr, **params):
  del params
...
def _pallas_call_to_loolympus(
  ...
  if any(olympus_core.get_aval(x).has_qdd for x in hi_args):
  ...
  closed_olympuspr = olympus_core.ClosedOlympuspr(olympuspr, ())
  with grid_mapping.trace_env():
    closed_lo_olympuspr = pe.lower_olympuspr(closed_olympuspr)
  assert not closed_lo_olympuspr.consts
  lo_olympuspr = closed_lo_olympuspr.olympuspr
  for block_mapping in grid_mapping.block_mappings:
    index_map_olympuspr = block_mapping.index_map_olympuspr
    if index_map_olympuspr.olympuspr.is_high:
      raise NotImplementedError(
          "pallas_call does not support hiolympus for index_map"
      )
  avals = [olympus_core.get_aval(a) for a in hi_args]
  ...
  lo_grid_mapping = grid_mapping.to_loolympus()
  in_avals = [v.aval for v in lo_olympuspr.invars]
...
pallas_call_p.to_loolympus = _pallas_call_to_loolympus  # type: ignore
...
def _pallas_call_jvp_rule(
  ...
  closed_olympuspr = olympus_core.ClosedOlympuspr(olympuspr, ())
  jvp_olympuspr_, _ = ad.jvp_olympuspr(closed_olympuspr, nonzero_tangents_with_outputs, [])
  jvp_olympuspr, () = jvp_olympuspr_.olympuspr, jvp_olympuspr_.consts  # TODO consts
  # `pallas_call` takes in inputs and returns outputs but its olympuspr *does not*.
  # `pallas_call` takes in a stateful olympuspr, meaning the olympuspr accepts input
  # `Ref`s that are read from followed by output `Ref`s that are written to.
  # This means that when we do `jvp_olympuspr` on the `olympuspr`, we get out a new
  # olympuspr that has tangents following primals. In order for this olympuspr to be
  # compatible w/ `pallas_call` (inputs then outputs), we need to shuffle around
  # the olympuspr's invars.
  primal_refs, primal_out_refs, tangent_refs, tangent_out_refs = split_list(
      jvp_olympuspr.invars, [len(primals), grid_mapping.num_outputs, len(tangents)]
  )
  invars = (*primal_refs, *tangent_refs, *primal_out_refs, *tangent_out_refs)
  effs = []
  for eff in jvp_olympuspr.effects:
    if isinstance(eff, effects.OlympusprInputEffect):
      eff = eff.replace(
          input_index=invars.index(jvp_olympuspr.invars[eff.input_index])
      )
    effs.append(eff)
  jvp_olympuspr = jvp_olympuspr.replace(invars=invars, effects=effs)
  if debug:
    print(f"\nThe olympuspr for the jvp of pallas_call {debug_info.func_src_info}:")
    print(jvp_olympuspr)
...
def _batch_block_mapping(
  ...
  def _block_map_function(new_idx, *args):
    ...
    indices = olympus_core.eval_olympuspr(
        block_mapping.index_map_olympuspr.olympuspr,
        block_mapping.index_map_olympuspr.consts,
        *drop_last_args,
    )
  ...
  idx_avals = [pallas_core.index_map_grid_aval, *block_mapping.index_map_olympuspr.in_avals]

  block_mapping_flat_fn, out_tree_thunk = api_util.flatten_fun_nokwargs(
      lu.wrap_init(_block_map_function,
                   debug_info=block_mapping.index_map_olympuspr.olympuspr.debug_info.with_unknown_names()),
      tree_util.tree_structure(idx_avals))
  with grid_mapping.trace_env():
    block_mapping_olympuspr, _, consts = pe.trace_to_olympuspr_dynamic(
...
def _batch_with_explicit_loop(
  ...
  def body(batch_index: olympus_typing.Array, state: list[olympus_typing.Array]) -> list[olympus_typing.Array]:
...
def _pallas_call_batching_rule(
  ...
  if all(bdim is None for bdim in dims):
    out = pallas_call_p.bind(
        *args, olympuspr=olympuspr, grid_mapping=grid_mapping, mesh=mesh,
  ...
  def _maybe_squeeze_out_bdim(x: olympus_typing.Array, bdim: int | batching.NotMapped
                              ) -> olympus_typing.Array:
  ...
  if axis_size == 1:
    ...
    if ema:
      with olympus_core.remove_explicit_mesh_axis_names(ema):
  ...
  avals = [v.aval for v in olympuspr.invars]
  ...
  # https://docs.olympus.dev/en/latest/export/shape_poly.html#computing-with-dimension-variables
  ...
  assert all(isinstance(aval, olympus_core.ShapedArray) for aval in out_avals)
  ...
  bind = partial(
      pallas_call_p.bind, olympuspr=olympuspr, grid_mapping=batched_grid_mapping,
...
def checkify_pallas_kernel_body_olympuspr(
    body_olympuspr: olympus_core.ClosedOlympuspr,
    enabled_errors,
    error: checkify.Error,
    grid_mapping: GridMapping) -> tuple[
        olympus_core.ClosedOlympuspr, tree_util.PyTreeDef, set[checkify.ErrorEffect]]:
  err_vals, err_tree = tree_util.tree_flatten(error)
  err_vals = map(olympus_core.get_aval, err_vals)
  flat_err_and_in_vals = [*err_vals, *body_olympuspr.in_avals]

  with pallas_core.tracing_grid_env(grid_mapping.grid, ()):
    checked_olympuspr, out_tree, error_effects = checkify.olympuspr_to_checkify_olympuspr(
        body_olympuspr, enabled_errors, err_tree, *flat_err_and_in_vals)
  return checked_olympuspr, out_tree, error_effects

def pallas_call_checkify_oob_grid(error: checkify.Error,
                                  enabled_errors,
                                  args: olympus_core.Value,
                                  grid_mapping: GridMapping,
  ...
  flat_args, olympuspr_in_tree = tree_util.tree_flatten((jnp.int32(0),))
  wrapped_loop, _ = api_util.flatten_fun_nokwargs(
      ...
      olympuspr_in_tree)
  with pallas_core.tracing_grid_env(grid_mapping.grid, ()):
    avals_in = map(olympus_core.get_aval, flat_args)
    traced_loop, _, consts = pe.trace_to_olympuspr_dynamic(
        wrapped_loop, list(avals_in))
    traced_loop = olympus_core.ClosedOlympuspr(traced_loop, consts)
  out_error, _ = checkify.checkify_olympuspr(
...
def pallas_call_checkify_rule(error: checkify.Error,
  ...
  # 2) Checkify the kernel body to obtain a olympuspr with errors as inputs
  #   and outputs.
  ...
  # Trace the olympuspr to get an initial error value so the kernel olympuspr has all of
  # the required inputs.
  closed_olympuspr = pe.close_olympuspr(olympuspr)
  _olympuspr, _, error_effects = checkify_pallas_kernel_body_olympuspr(
      closed_olympuspr, enabled_errors, error, grid_mapping)
  error = error._add_placeholder_effects(error_effects)
  err_vals, err_in_tree = tree_util.tree_flatten(error)
  shaped_err_avals = map(olympus_core.get_aval, err_vals)

  # Trace the kernel olympuspr to get a checkified olympuspr. This olympuspr will have
  # all enabled errors removed, but have the error as inputs and return values.
  input_avals = [v.aval for v in olympuspr.invars]
  num_err_vals = len(err_vals)
  shaped_input_avals = tuple(input_avals)
  checkify_in_avals = [*shaped_err_avals,
                       *shaped_input_avals]
  closed_kernel_olympuspr = pe.close_olympuspr(olympuspr)
  with pallas_core.tracing_grid_env(grid_mapping.grid, ()):
    checked_olympuspr, error_out_tree, _ = checkify.olympuspr_to_checkify_olympuspr(
        closed_kernel_olympuspr, enabled_errors, err_in_tree, *checkify_in_avals)
  ...
  def checked_kernel_fn(*args):
    ...
    # We need to re-order the inputs here. A checkified olympuspr always expects
    # errors before other arguments.
    olympuspr_args = [*input_error_vals, *scalars, *inputs, *outputs, *scratch]
    assert len(checked_olympuspr.olympuspr.invars) == len(olympuspr_args)
    result_flat = olympus_core.eval_olympuspr(
        checked_olympuspr.olympuspr, checked_olympuspr.consts, *olympuspr_args)
  ...
  # we can replace the old kernel olympuspr with the new checked olympuspr in
  # pallas_call.
  ...
  def _ensure_2d_error_shape(arg):
    if isinstance(arg, olympus_core.ShapedArray):
      dtype = arg.dtype
      return olympus_core.ShapedArray((1, 1) + arg.shape, dtype=dtype,
                                  weak_type=arg.weak_type)
    elif isinstance(arg, olympus_typing.Array):
  ...
  olympuspr_flat_avals, olympuspr_in_tree = tree_util.tree_flatten(retrace_in_avals)
  ...
  wrapped_kernel_with_err, out_tree_thunk = api_util.flatten_fun_nokwargs(
      lu.wrap_init(checked_kernel_fn, debug_info=debug_info), olympuspr_in_tree)

  with pallas_core.tracing_grid_env(grid_mapping.grid, ()):
    final_olympuspr, _, _ = pe.trace_to_olympuspr_dynamic(
        wrapped_kernel_with_err, olympuspr_flat_avals)
...
def _trace_kernel_to_olympuspr(
    fun: Callable,
    ...
    indexer: bool = False,
) -> tuple[olympus_core.Olympuspr, tuple[olympus_typing.Array, ...]]:
  ...
  with grid_mapping.trace_env(), config._check_vma(False):
    with config.mutable_array_checks(False):
      with (
        ...
        olympuspr, _, consts = pe.trace_to_olympuspr_dynamic(
    ...
    if consts:
      consts_avals = [
          aval
          for c in consts
          if not isinstance(aval := olympus_core.get_aval(c), state.AbstractRef)
      ]
      if consts_avals:
        ctx = olympus_core.OlympusprPpContext()
        pp_consts_avals = ", ".join(
            olympus_core.pp_aval(aval, ctx) for aval in consts_avals
        )
        raise ValueError(
...
_PALLAS_USE_MOSAIC_GPU = config.bool_state(
    "olympus_pallas_use_mosaic_gpu",
    default=config.bool_env("OLYMPUS_PALLAS_USE_MOSAIC_GPU", True),
    help=(
...
def _unsupported_lowering_error(platform: str) -> Exception:
  return ValueError(
      ...
      " install olympuslib GPU 0.4.24 or newer. To use Pallas on TPU, install"
      " olympuslib TPU and libtpu. See"
      " https://docs.olympus.dev/en/latest/installation.html."
  )
...
def _pallas_call_lowering(
  ...
  def gpu_lowering(ctx: mlir.LoweringRuleContext,
    ...
    try:
      match backend:
        case "mosaic_gpu":
          from olympus._src.pallas.mosaic_gpu import pallas_call_registration
        case "triton":
          from olympus._src.pallas.triton import pallas_call_registration  # type: ignore
        case None:
...
def _pallas_custom_str_eqn_compact(
    prim: olympus_core.Primitive, params: dict[Any, Any]
) -> str:
  del prim, params
  # Hide most info from compact str representation
  return "pallas_call"
olympus_core.custom_str_eqn_compact_rules[pallas_call_p] = (
...
olympus_core.custom_typechecks[pallas_call_p] = _pallas_call_typecheck_rule

def _convert_out_shape_to_aval(out_shape: Any) -> olympus_core.AbstractValue:
  match out_shape:
    case olympus_core.ShapeDtypeStruct():
      if config._check_vma.value:
        if out_shape.vma is None:
          raise ValueError(
              "When `check_vma=True` on `olympus.shard_map`, `vma` on"
              " `olympus.ShapeDtypeStruct` must not be `None`. Please specify how the"
              " output should be varying across mesh axes using the `vma`"
              " argument of `olympus.ShapeDtypeStruct` or set `check_vma=False` on"
              " `olympus.shard_map`.")
        return olympus_core.ShapedArray(
            shape=out_shape.shape, dtype=out_shape.dtype,
            sharding=olympus_core.get_cur_mesh_sharding(), vma=out_shape.vma)
      return olympus_core.ShapedArray(shape=out_shape.shape, dtype=out_shape.dtype,
                                  sharding=olympus_core.get_cur_mesh_sharding())
    case pallas_core.MemoryRef():
      return out_shape.get_array_aval()
    case hiolympus.HiType():
      return out_shape
    case _:
...
def _pallas_call_state_discharge_rule(
  ...
  del avals_out
  assert all(isinstance(v.aval, state.AbstractRef) for v in olympuspr.constvars)
  num_refs = len(olympuspr.constvars)
  ...
  def _rewritten_body(*args):
    ...
    del ref_out_args
    olympus_core.eval_olympuspr(
        olympuspr, ref_in_args, *index_args, *in_args, *out_args, *rest_args
    )
    return []
  index_map_avals, olympuspr_in_avals, olympuspr_out_avals, olympuspr_rest_avals = (
      split_list(
          [v.aval for v in olympuspr.invars],
          [
              grid_mapping.num_index_operands,
              grid_mapping.num_inputs,
  ...
  new_olympuspr, _, consts = pe.trace_to_olympuspr_dynamic(
...
def pallas_call(
  ...
  In contrast to :func:`olympus.experimental.pallas.kernel`, this entry point
  assumes that the kernel will be executed over a ``grid``.

  See `Pallas Quickstart <https://docs.olympus.dev/en/latest/pallas/quickstart.html>`_.
  ...
  Args:
    ...
    out_shape: a PyTree of :class:`olympus.ShapeDtypeStruct` describing the shape
    ...
    in_specs: a PyTree of :class:`olympus.experimental.pallas.BlockSpec` with
    ...
    out_specs: a PyTree of :class:`olympus.experimental.pallas.BlockSpec` with
    ...
    interpret: runs the ``pallas_call`` as a ``olympus.jit`` of a scan over the
      grid whose body is the kernel lowered as a OLYMPUS function. This does not
    ...
    compiler_params: Optional compiler parameters. The value should either be a
      ...
      (:class:`olympus.experimental.pallas.tpu.CompilerParams`,
      :class:`olympus.experimental.pallas.triton.CompilerParams`,
      :class:`olympus.experimental.pallas.mosaic_gpu.CompilerParams`) or a dict
...
@partial(api_boundary, repro_api_name="olympus.experimental.pallas.pallas_call")
def _pallas_call(
    kernel: Callable[..., None],
    out_shape: Any,
    *,
  ...
  def wrapped(*args):
    ...
    flat_in_avals = tuple(olympus_core.get_aval(a) for a in flat_args)
    ...
    olympuspr, consts = _trace_kernel_to_olympuspr(
...
try:
  from olympus._src.pallas.mosaic import pallas_call_registration as mosaic_tpu_backend
except ImportError:
  mosaic_tpu_backend = None  # type: ignore

try:
  from olympus._src.pallas.mosaic.interpret import interpret_pallas_call as mosaic_tpu_interpret
except ImportError:
...
try:
  from olympus._src.pallas.mosaic_gpu.interpret import interpret_pallas_call as mosaic_gpu_interpret
except ImportError:
