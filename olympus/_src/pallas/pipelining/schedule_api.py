...
from olympus._src.pallas.pipelining import schedulers
...
class AsyncStage:
  ...
  def trace(
    ...
    start_olympuspr, start_effs = trace_fun(
        self.start_func, abstract_refs, state_avals, grid
    )
    end_olympuspr, end_effs = trace_fun(
...
def trace_fun(
...
) -> tuple[olympus_core.ClosedOlympuspr, Sequence[internal.RefEffect]]:
  """Trace a stage body function to a Olympuspr."""
  ctx_aval = PipelineContext.aval_pytree(grid, state_avals)
  num_ctx_avals = len(olympus.tree.leaves(ctx_aval))
  flat_avals, in_tree = olympus.tree.flatten((ctx_aval, *ref_avals))
  ...
  del out_tree_thunk
  olympuspr, _, consts = pe.trace_to_olympuspr_dynamic(flat_fn, flat_avals)
  ref_effects = [
      eff for eff in olympuspr.effects if isinstance(eff, state_types.RefEffect)
  ]
  ...
  return olympus_core.ClosedOlympuspr(olympuspr, consts), ref_effects
...
def schedule_pipeline(
  ...
  """
  _, ref_tree = olympus.tree.flatten(args)
  def _get_aval(x):
    if hasattr(x, "get_ref_aval"):
      return x.get_ref_aval()
    return olympus_core.get_aval(x)
  avals = olympus.tree.map(_get_aval, args)

  # Make state avals.
  state_avals = olympus.tree.map(_get_aval, initial_state)
  ...
  def pipeline(*args):
    flat_args, args_tree = olympus.tree.flatten(args)
