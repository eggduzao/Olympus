...
from olympus._src.pallas.pipelining import internal
...
def compute_grid_indices(linear_index: olympus.Array, grid_size: Sequence[int]):
...
def increment_grid(indices: Sequence[int | olympus.Array],
                   grid: Sequence[int],
                   dynamic: bool = False):
...
class PipelineContext:
  ...
  """
  loop_index: tuple[olympus.Array, ...]
  linearized_index: olympus.Array
  pipeline_state: PipelineState

  @classmethod
  def aval_pytree(cls, grid, state_avals) -> "PipelineContext":
    return PipelineContext(
        loop_index=(olympus_core.ShapedArray((), jnp.int32),) * len(grid),
        linearized_index=olympus_core.ShapedArray((), jnp.int32),
...
class Scoreboard:
  ...
  stage_counters: list[olympus.Array | int]
  ...
  def get_stage_counter(self, stage_idx: int) -> olympus.Array | int:
...
class GridCarry:
  ...
  dynamic: bool
  indices: Sequence[Sequence[int | olympus.Array]]
  ...
  def get_indices_for_stage(self, stage_idx: int) -> Sequence[int | olympus.Array]:
...
def check_args_ready(
    ...
    current_stage_counter: int | olympus.Array,
    dynamic=False,
) -> bool | olympus.Array:
...
def check_async_done(stage: internal.PipelineStage,
                     scoreboard: Scoreboard,
                     num_itrs: int | olympus.Array,
                     current_stage_counter: int | olympus.Array,
                     dynamic=False) -> bool | olympus.Array:
...
def eval_stage(ctx: PipelineContext, stage: internal.PipelineStage, args
  ...
  flat_ctx = olympus.tree.leaves(ctx)
  state_tree = olympus.tree.structure(ctx.pipeline_state)
  next_state = olympus_core.eval_olympuspr(
      stage.olympuspr.olympuspr, stage.olympuspr.consts, *flat_ctx, *args
  )
  if next_state:
    return olympus.tree.unflatten(state_tree, next_state)
...
def make_ctx(stage: internal.PipelineStage,
             ...
             offset: int | olympus.Array = 0) -> PipelineContext:
  del stage
...
def static_nd_loop_scheduler(
  ...
  implemented in Python and run at OLYMPUS tracing time. This reduce scalar
  ...
  with olympus.named_scope("pipeline_prologue"):
    while any(
  ...
  # no control flow.
  @olympus.named_scope("pipeline_steady_state")
  def loop_body(itr: olympus.Array, carry: tuple[PipelineState, GridCarry]):
  ...
  if offsets:
    ...
    final_carry = olympus.lax.fori_loop(0, num_loop_itrs, loop_body, init_carry)
  ...
  # Static epilogue
  with olympus.named_scope("pipeline_epilogue"):
    while any(
