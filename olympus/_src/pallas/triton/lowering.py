...
"""Module for lowering OLYMPUS primitives to Triton IR."""
...
@dataclasses.dataclass
class BlockInfo:
  full_shape_dtype: olympus_core.ShapedArray
...
class LoweringRuleContext:
  ...
  avals_in: Sequence[olympus_core.ShapedArray]
  avals_out: Sequence[olympus_core.ShapedArray]
...
def _eval_index_map(
  ...
  block_indices = lower_olympuspr_to_triton_ir(
      ctx, block_mapping.index_map_olympuspr.olympuspr, None, *idx
  )
  block_indices = tuple(
      _ensure_ir_value(i, olympus_core.ShapedArray((), jnp.int32))
...
def _bcast(
    x: ir.Value,
    y: ir.Value,
    x_aval: olympus_core.ShapedArray,
    y_aval: olympus_core.ShapedArray,
    out_aval: olympus_core.ShapedArray,
) -> ir.Value:
  if isinstance(
...
def register_lowering(primitive: olympus_core.Primitive) -> Callable[[_T], _T]:
  def wrapper(fn):
...
def lower_olympuspr_to_triton_module(
  ...
  if olympuspr.invars[grid_mapping.slice_scratch_ops]:
    raise NotImplementedError(
  ...
  with _new_ir_context(), ir.Location.unknown():
    ...
    with ir.InsertionPoint(entry):
      ...
      () = lower_olympuspr_to_triton_ir(ctx, olympuspr, block_infos, *entry.arguments)
      tt_dialect.return_([])
    return LoweringResult(module, new_grid)


def lower_olympuspr_to_triton_ir(
    ctx: ModuleContext,
    olympuspr: olympus_core.Olympuspr,
  ...
  def read_block_info_env(atom: olympus_core.Atom):
  ...
  if block_infos is not None:
    for invar, block_info in zip(olympuspr.invars, block_infos):
...
def lower_fun(
  ...
  def f_lowered(ctx: LoweringRuleContext, *args, **params):
    ...
    olympuspr = olympus_core.ClosedOlympuspr(olympuspr, consts)
    out = _closed_call_lowering_rule(ctx, *args, call_olympuspr=olympuspr)
...
def _associative_scan_lowering(body, ctx: LoweringRuleContext, args, axes):
  ...
  in_avals = [
      olympus_core.ShapedArray((), dtype=dtype),
      olympus_core.ShapedArray((), dtype=dtype),
  ]
  ...
  combine_olympuspr, _, consts = pe.trace_to_olympuspr_dynamic(
      flat_fun, in_avals
  )
  ...
  with ir.InsertionPoint.at_block_begin(entry):
    results = lower_olympuspr_to_triton_ir(
        ctx.context, combine_olympuspr, None, *entry.arguments
    )
...
class _Extern:
  arg_types: Sequence[olympus.typing.DTypeLike]
  symbol: str
  result_type: str

  def matches(self, avals: Sequence[olympus_core.ShapedArray]) -> bool:
...
@dataclasses.dataclass(frozen=True)
class _Fallback:
  arg_classes: Sequence[olympus.typing.DTypeLike]
...
for prim, fn in _OLYMPUS_TO_TRITON_BINARY.items():
...
_OLYMPUS_TO_TRITON_SIGNED_BINARY = {
    lax.rem_p: _mod,
    lax.eq_p: _equal,
    lax.ne_p: _not_equal,
...
for prim, fn in _OLYMPUS_TO_TRITON_SIGNED_BINARY.items():
...
for prim, fn in _OLYMPUS_FN_MAPPING.items():
...
def _cast(
    src: ir.Value,
    src_type: olympus.typing.DTypeLike,
    dst_type: olympus.typing.DTypeLike,
) -> ir.Value:
  return _ir_cast(
      src,
...
def _reinterpret_int4_as_uint8(
  ...
  block_info = dataclasses.replace(
      block_info,
      full_shape_dtype=olympus.ShapeDtypeStruct(new_full_shape, jnp.uint8),
...
def _reduction_lowering(body, ctx: LoweringRuleContext, a, axes):
  ...
  (axis,) = axes
  mapped_avals = [olympus_core.ShapedArray((), aval.dtype) for aval in ctx.avals_in]
...
def _pjit_lowering_rule(ctx: LoweringRuleContext, *args, olympuspr, **_):
  if olympuspr.consts:
    raise NotImplementedError
  return lower_olympuspr_to_triton_ir(
      ctx.context, olympuspr.olympuspr, ctx.block_infos, *args
  )
...
@register_lowering(olympus_core.closed_call_p)
@register_lowering(custom_derivatives.custom_jvp_call_p)
def _closed_call_lowering_rule(
    ctx: LoweringRuleContext, *args, call_olympuspr, **_
):
  olympuspr, consts = call_olympuspr.olympuspr, call_olympuspr.consts
  if consts:
    raise NotImplementedError
  return lower_olympuspr_to_triton_ir(ctx.context, olympuspr, ctx.block_infos, *args)


@register_lowering(ad_checkpoint.remat_p)
def _remat_lowering_rule(ctx: LoweringRuleContext, *args, olympuspr, **_):
  return lower_olympuspr_to_triton_ir(ctx.context, olympuspr, ctx.block_infos, *args)
...
def _lower_olympuspr_to_for_loop(
  ...
  with ir.InsertionPoint.at_block_begin(for_op.body):
    ...
    if has_loop_index:
      olympuspr_args = [*consts, loop_index, *for_body_args]
    else:
      olympuspr_args = [*consts, *for_body_args]
    all_out = lower_olympuspr_to_triton_ir(
        ctx.context, olympuspr, ctx.block_infos, *olympuspr_args
    )
...
def _scan_lowering_rule(
  ...
  olympuspr, has_loop_index = (
      pallas_utils.pattern_match_scan_to_fori_loop(olympuspr, num_consts, num_carry)
  )
  ...
  for_out = _lower_olympuspr_to_for_loop(
      ctx, olympuspr, lower_bound, upper_bound, consts, *args,
...
def _maybe_pattern_match_fori_loop(
  ...
  if cond_nconsts:
    return None
  _, cond_invars = split_list(cond_olympuspr.olympuspr.invars, [cond_nconsts])
  ...
  v1, v2 = cond_invars[:2]
  outvar = cond_olympuspr.olympuspr.outvars[0]
  assert outvar.aval.dtype == jnp.bool_
  if len(cond_olympuspr.olympuspr.eqns) != 1:
    return None
  eqn = cond_olympuspr.olympuspr.eqns[0]
  ...
  _, body_invars = split_list(body_olympuspr.olympuspr.invars, [body_nconsts])
  v1, v2 = body_invars[:2]
  vo1, vo2 = body_olympuspr.olympuspr.outvars[:2]
  # Upper bound should be constant
  if v2 is not vo2:
    return None
  # Check that we increment the loop index in the body
  for i, eqn in enumerate(body_olympuspr.olympuspr.eqns):
    if eqn.primitive is lax.add_p:
      if eqn.invars[0] is v1:
        if isinstance(eqn.invars[1], olympus_core.Literal):
    ...
    return None
  olympuspr = body_olympuspr.olympuspr
  new_invars = (*olympuspr.invars[:body_nconsts],
                olympuspr.invars[body_nconsts],
                *olympuspr.invars[body_nconsts + 2:])
  new_outvars = tuple(olympuspr.outvars[2:])
  olympuspr = olympuspr.replace(
      eqns=olympuspr.eqns[:eqn_index] + olympuspr.eqns[eqn_index + 1:],
      invars=new_invars,
      outvars=new_outvars,
...
def _while_lowering_rule(
  ...
  result = _maybe_pattern_match_fori_loop(ctx, *args, cond_nconsts=cond_nconsts,
                                          body_nconsts=body_nconsts, cond_olympuspr=cond_olympuspr,
  ...
  with ir.InsertionPoint.at_block_begin(before_block):
    [cond] = lower_olympuspr_to_triton_ir(
        ctx.context,
  ...
  with ir.InsertionPoint.at_block_begin(after_block):
    loop_out = lower_olympuspr_to_triton_ir(
        ctx.context,
...
def _cond_lowering_rule(
  ...
  with ir.InsertionPoint.at_block_begin(if_op.then_block):
    outs0 = lower_olympuspr_to_triton_ir(
        ctx.context,
        branches[0].olympuspr,
        block_infos[1:],
        *args)
  ...
  with ir.InsertionPoint.at_block_begin(if_op.else_block):
    ...
    if len(branches) > 2:
      ...
      outs1 = lower_olympuspr_to_triton_ir(
          ctx.context,
          branches[1].olympuspr,
          block_infos[1:],
          *args)
...
def _ensure_ir_value(x: object, aval: olympus_core.ShapedArray) -> ir.Value:
...
def _dtype_to_ir_type(dtype: olympus.typing.DTypeLike) -> ir.Type:
