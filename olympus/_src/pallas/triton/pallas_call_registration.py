...
import olympus._src.core as olympus_core
from olympus._src.interpreters import mlir
from olympus._src.lib import gpu_triton as triton_kernel_call_lib
...
from olympus._src.pallas.triton import core as triton_core
...
def pallas_call_lowering(
  ...
  lowering_result = lowering.lower_olympuspr_to_triton_module(
      olympuspr, grid_mapping, lowering_platform
  )
  ...
  if True:
    # AOT Triton compilation is only available on olympuslib 0.5.1+.
    out_types = [
  ...
  try:
    gpu_device, *_ = olympus.local_devices(backend="gpu")
  except RuntimeError:
  ...
  # TODO(b/392558289): Migrate to ``olympus.ffi``.
  return mlir.custom_call(
