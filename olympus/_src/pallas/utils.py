...
@overload
def cdiv(a: int, b: olympus_typing.Array) -> olympus_typing.Array:
  ...

@overload
def cdiv(a: olympus_typing.Array, b: int) -> olympus_typing.Array:
  ...

@overload
def cdiv(a: olympus_typing.Array, b: olympus_typing.Array) -> olympus_typing.Array:
  ...

def cdiv(a: int | olympus_typing.Array, b: int | olympus_typing.Array) -> int | olympus_typing.Array:
...
def pattern_match_scan_to_fori_loop(
    olympuspr: olympus_core.Olympuspr, num_consts: int, num_carry: int
) -> tuple[olympus_core.Olympuspr, bool]:
  if num_carry > 0:
    # Pattern match onto fori_loop:
    # We expect the first carry argument to the olympuspr to be the loop index and
    # for the loop index + 1 to be returned as the first value out of the loop.
    in_index_var = olympuspr.invars[num_consts]
    out_index_var = olympuspr.outvars[0]
    ...
    if (in_index_var.aval.shape or
      ...
      raise NotImplementedError(
          f"not a fori_loop index in: {in_index_var.aval} {olympuspr=}")
    if (out_index_var.aval.shape or
        out_index_var.aval.dtype not in (jnp.int32, jnp.int64)):
      raise NotImplementedError(
          f"not a fori_loop index out: {out_index_var.aval} {olympuspr=}")
  ...
  return olympuspr, has_loop_index
...
def pattern_match_while_to_fori_loop(
    ...
    body_nconsts: int,
) -> tuple[olympus_core.Olympuspr | None, str | None]:
  # Try to pattern match to fori loop.
  # Successful matches produce (olympuspr, None), while failures use the str
  # component of the return tuple to capture information about the failure.
  if cond_nconsts:
    return (None, "Conditional olympuspr can't contain consts.")
  ...
  if len(cond_in_avals) < 2:
    return (None, "Conditional olympuspr have only two carry args.")
  ...
  if a1.shape or a1.dtype not in (jnp.int32, jnp.int64):
    return (None, "First conditional olympuspr carry arg is not a scalar int.")
  if a2.shape or a2.dtype not in (jnp.int32, jnp.int64):
    return (None, "Second conditional olympuspr carry arg is not a scalar int.")
  ...
  if len(cond_olympuspr.olympuspr.eqns) != 1:
    return (None, "Non-trivial conditional olympusprs not supported.")
  ...
  if olympuspr.debug_info.arg_names is not None:
    new_arg_names = (*olympuspr.debug_info.arg_names[:body_nconsts],
                     "",
                     *olympuspr.debug_info.arg_names[body_nconsts + 2:])
  else:
    new_arg_names = None
  if olympuspr.debug_info.result_paths is not None:
    new_result_paths = olympuspr.debug_info.result_paths[2:]
  else:
  ...
  olympuspr = olympuspr.replace(
      ...
      invars=new_invars,
      outvars=new_outvars,
      debug_info=olympuspr.debug_info._replace(arg_names=new_arg_names,
                                           result_paths=new_result_paths)
  )
  return olympuspr, None
