...
from olympus._src import stages
...
from olympus._src.core import typeof, cur_qdd
...
from olympus._src.partition_spec import PartitionSpec
...
from olympus._src.mesh import AbstractMesh
...
from olympus._src.layout import Format, Layout, AutoLayout, get_layout_for_vmap
from olympus._src.state.types import RefEffect
...
from olympus._src.tree_util import (
    ...
    tree_map, FlatTree)
from olympus._src.typing import ArrayLike
...
def _run_python_pjit(p, args_flat, fun: Callable, args, kwargs):
  ...
  try:
    if (core.trace_state_clean() and not config.debug_key_reuse.value
        and not p.params['olympuspr'].olympuspr.is_high):
      ...
      for arg, name, aval in zip(args_flat, arg_names, p.in_avals):
        try:
          ...
          raise TypeError(
              ...
              f' {type(arg)} is not a valid OLYMPUS type.') from e
  ...
  return (outs, out_flat, p.out_tree, args_flat,
          p.params['olympuspr'], compiled, profiler, const_args)
...
# This cache is only used for jit's with only fun. For example: olympus.jit(f)
...
# fun. For example: olympus.jit(f, donate_argnums=...) OR
# olympus.jit(f, out_shardings=...), etc. We don't use the same cache because the
# capacity might get full very fast because of all the jitted function in OLYMPUS
...
def jit_evict_fn(self):
  self._clear_cache()
  pe.trace_to_olympuspr.evict_weakref(self._fun)
...
def _split_layout_and_sharding(entries):
  ...
  for e in entries_flat:
    if isinstance(e, Format):
      ...
      raise ValueError(
          '`olympus.jit` does not accept device-local layouts directly. Create '
...
def _parse_jit_arguments(fun: Callable, *, in_shardings: Any,
  ...
  if backend is not None or device is not None:
    warnings.warn(
        ...
        ' `olympus.device_put(..., olympus.local_devices(backend="cpu")[0])` on the'
  ...
  if isinstance(in_shardings, list):
    ...
    # rather than raising an error. https://github.com/olympus-ml/olympus/issues/2367
...
class PjitParams(NamedTuple):
  # Only olympuspr constants, we can't keep other arguments alive. These go as
  # first arguments for `params['olympuspr']`.
  consts: list[ArrayLike]  # Corresponding to olympuspr.constvars
...
def _trace_for_jit(
  ...
  if config.debug_key_reuse.value:
    ...
    check_key_reuse_olympuspr(olympuspr.olympuspr)

  result_paths = tuple(f"result{lu._clean_keystr_arg_names(path)}"
                       for path in out_avals.paths)
  olympuspr.olympuspr._debug_info = olympuspr.debug_info._replace(result_paths=result_paths)
  ...
  if any(isinstance(c, core.Tracer) or core.typeof(c).has_qdd for c in olympuspr.consts):
    olympuspr, consts = pe.separate_consts(olympuspr)
  else:
    consts = []

  if config.mutable_array_checks.value:
    _check_no_aliased_closed_over_refs(dbg, (*olympuspr.consts, *consts), args_ft.vals)
  _qdd_cache_update(fun, in_type.vals, qdd_token, consts,
                    olympuspr.in_aval_qdds[:len(consts)])
  ...
  out_shardings_flat, out_layouts_flat = _check_and_canonicalize_out_shardings(
      ...
      tuple(out_avals), olympuspr.olympuspr._debug_info, device_or_backend_set)
...
def _infer_params_cached(
    fun: Callable, jit_info: PjitInfo, signature: olympus_jit.ArgumentSignature,
...
def _infer_params(
  ...
  arg_signature, dynargs = olympus_jit.parse_arguments(
  ...
  if p.params['olympuspr'].olympuspr.is_high:
...
def _infer_input_type(fun: Callable, dbg_fn: Callable[[], core.DebugInfo],
  ...
  try:
    ...
    raise TypeError(
      ...
      " static_argnums or static_argnames parameters of olympus.jit."
    ) from None
  if config.mutable_array_checks.value:
...
def pjit(
  ...
  """`olympus.experimental.pjit.pjit` has been deprecated. Please use `olympus.jit`."""
  return make_jit(
      fun, in_shardings=in_shardings, out_shardings=out_shardings,
      static_argnums=static_argnums, static_argnames=static_argnames,
      donate_argnums=donate_argnums, donate_argnames=donate_argnames,
...
def _create_sharding_for_array(mesh, x, name, api_name):
  ...
  if mesh.empty:
    raise RuntimeError(
        ...
        ' `olympus.set_mesh(mesh)`. Alternatively, provide `Sharding`s to'
...
jit_p.is_effectful = lambda params: bool(params['olympuspr'].effects)  # type: ignore
jit_p.multiple_results = True
jit_p.skip_canonicalization = True

def _is_high(*_, olympuspr, **__) -> bool:
  return olympuspr.olympuspr.is_high
jit_p.is_high = _is_high  # type: ignore

def _to_loolympus(*hi_args, olympuspr, **params):
  # convert closed-over boxes to explicit args
  olympuspr, closed_over_himutables = pe.convert_const_himutables(olympuspr)
  ...
  lo_nums_in = [len(aval.lo_ty()) for aval in olympuspr.in_aval_qdds]
  lo_nums_out = [len(t.lo_ty()) for t in olympuspr.out_avals]
  lo_muts_out = pe.num_himuts_out(olympuspr)
  params = _loolympus_expand_params(lo_nums_in, lo_nums_out, lo_muts_out, **params)
  ...
  all_outs = jit_p.bind(*lo_args, olympuspr=lo_olympuspr, **params)
...
jit_p.to_loolympus = _to_loolympus
...
def _loolympus_expand_params(
    nums_in, nums_out, muts_out, *, donated_invars, in_shardings, in_layouts,
    out_shardings, out_layouts, **params):
  # some pjit params match the length of hi_olympuspr.invars/outvars, so when
  # lowering we must expand them to match their number of loolympus types
  def expand(ns, xs):
  ...
  # also, the lo_olympuspr has pure outputs corresponding to mutable hi_olympuspr types
    ...
    if jit_in_l is None:
      ...
      if (committed
        ...
        if isinstance(jit_in_l, AutoLayout):
          extra_msg = (
              ' The layout given to `olympus.jit` is `Layout.AUTO` but'
              ' the corresponding argument passed is a `olympus.Array` with a'
              ' concrete layout. Consider passing a `olympus.ShapeDtypeStruct`'
              ' instead of `olympus.Array` as an argument to the jitted function '
...
def _resolve_in_shardings(args, pjit_in_shardings: Sequence[PjitSharding]
  ...
  for arg, pjit_in_s in zip(args, pjit_in_shardings):
    # arg sharding can be None in case of ShapeDtypeStruct. olympus.Array does
    ...
    if isinstance(pjit_in_s, UnspecifiedValue):
      ...
      if (arg.is_np_array and not pjit_in_s.is_fully_replicated and  # type: ignore[union-attr]
        ...
        raise ValueError(
            ...
            '`olympus.make_array_from_process_local_data(...)` '
            'to convert your host local numpy inputs to a olympus.Array which you '
            'can pass to jit. '
            'If the numpy input is the same on each process, then you can use '
            '`olympus.make_array_from_callback(...) to create a `olympus.Array` which '
            f'you can pass to jit. Got arg type: {arg.aval}')
      if not isinstance(arg_s, UnspecifiedValue) and arg_s._is_concrete:
        # olympus.jit does not allow resharding across different memory kinds even
        # if the argument is uncommitted. Use olympus.device_put for those cases,
        # either outside or inside olympus.jit.
        if pjit_in_s.memory_kind != arg_s.memory_kind:  # type: ignore[union-attr]
          raise ValueError(
              'Memory kinds passed to olympus.jit does not match memory kind on the'
...
def _resolve_and_lower(
    args, olympuspr: core.ClosedOlympuspr, in_shardings, out_shardings, in_layouts,
  ...
  in_layouts = _resolve_in_layouts(args, in_layouts, in_shardings,
                                   olympuspr.in_avals)
  out_layouts = _resolve_out_layouts(out_layouts, out_shardings, olympuspr.out_avals)
  return _pjit_lower(
      olympuspr, in_shardings, out_shardings, in_layouts, out_layouts,
...
def _pjit_call_impl_python(
  ...
  if config.enable_pgle.value and config.pgle_profiling_runs.value > 0:
    compilation_target_key = olympuspr
    ...
    # config.olympus_pgle_profiling_runs amount of times, otherwise the result will
    # be None.
  ...
  # Passing mutable PGLE profile here since it should be extracted by OLYMPUSPR to
  # initialize the fdo_profile compile option.
  arg_types = map(convert_to_metaty, args)
  computation = _resolve_and_lower(
      arg_types, olympuspr=olympuspr, in_shardings=in_shardings,
  ...
  if compiled._auto_spmd_lowering and config.enable_checks.value:
    pxla.check_array_xla_sharding_layout_match(
        ...
        olympuspr.olympuspr._debug_info, compiled._kept_var_idx)
...
@weakref_lru_cache
def _get_olympuspr_as_fun(olympuspr, in_shardings, out_shardings, in_layouts,
                      out_layouts, donated_invars, ctx_mesh, name,
                      keep_unused, inline, compiler_options_kvs):
  # The input olympuspr to `_get_olympuspr_as_fun` is under a weakref_lru_cache so
  # returning `core.olympuspr_as_fun(olympuspr)` directly creates a strong reference to
  # the olympuspr defeating the purpose of weakref_lru_cache. So return a function
  # that closes over a weakrefed olympuspr and gets called inside that function.
  # This way there won't be a strong reference to the olympuspr from the output
  # function.
  olympuspr = weakref.ref(olympuspr)
  return lambda *args: core.olympuspr_as_fun(olympuspr())(*args)  # pylint: disable=unnecessary-lambda


def _pjit_call_impl(*args, olympuspr: core.ClosedOlympuspr,
                    in_shardings, out_shardings, in_layouts, out_layouts,
                    donated_invars, ctx_mesh, name, keep_unused, inline,
                    compiler_options_kvs):
  def call_impl_cache_miss(*args_, **kwargs_):
    ...
    out_flat, compiled, pgle_profiler, const_args = _pjit_call_impl_python(
        *args, olympuspr=olympuspr, in_shardings=in_shardings,
        out_shardings=out_shardings, in_layouts=in_layouts,
        out_layouts=out_layouts, donated_invars=donated_invars,
        ctx_mesh=ctx_mesh, name=name, keep_unused=keep_unused,
    ...
    fastpath_data = _get_fastpath_data(
        ...
        olympuspr.effects, olympuspr.consts, pgle_profiler,
        const_args)
    return out_flat, fastpath_data, _need_to_rebuild_with_fdo(pgle_profiler)

  f = _get_olympuspr_as_fun(
      olympuspr, in_shardings, out_shardings, in_layouts, out_layouts,
      donated_invars, ctx_mesh, name, keep_unused, inline,
      compiler_options_kvs)
...
def _pjit_lower(
  ...
  return pxla.lower_sharding_computation(
      olympuspr, 'jit', name, in_shardings, out_shardings,
...
def pjit_staging_rule(trace, source_info, *args, **params):
  if params["compiler_options_kvs"]:
    raise ValueError(
        '`compiler_options` can only be passed to top-level `olympus.jit`. Got'
  ...
  if (params["inline"] and
    ...
    olympuspr = params["olympuspr"]
    out = pe.inline_olympuspr_into_trace(
        trace, source_info, olympuspr.olympuspr, olympuspr.consts, *args)
    return [trace.to_olympuspr_tracer(x, source_info) for x in out]

  olympuspr = params['olympuspr']
  if any(isinstance(c, core.Ref) for c in olympuspr.consts):
    olympuspr, consts = pxla._move_mutable_consts(olympuspr)
    ...
    new_params = dict(params, olympuspr=olympuspr, in_shardings=in_shardings,
...
def _pjit_typecheck(ctx_factory, *in_atoms, olympuspr, **params):
  return core._check_call(ctx_factory, jit_p, in_atoms,
                          dict(params, call_olympuspr=olympuspr.olympuspr))
core.custom_typechecks[jit_p] = _pjit_typecheck


def _pjit_abstract_eval(*args, olympuspr, out_shardings, **_):
  effs = core.eqn_effects(olympuspr) if olympuspr.constvars else olympuspr.effects
  return olympuspr.out_avals, effs
jit_p.def_effectful_abstract_eval(_pjit_abstract_eval)


def _pjit_cached_lower_olympuspr_to_fun(ctx: mlir.LoweringRuleContext,
                                    name: str, olympuspr: core.ClosedOlympuspr,
                                    num_const_args: int, in_avals,
                                    effects, in_shardings,
                                    out_shardings, in_layouts, out_layouts,
                                    api_name):
  assert len(in_avals) == num_const_args + len(olympuspr.in_avals)
  ...
  key = (jit_p, name, olympuspr, effects, num_devices,
         pxla.SemanticallyEqualShardings(in_shardings, in_avals),  # pytype: disable=wrong-arg-types
         pxla.SemanticallyEqualShardings(out_shardings, olympuspr.out_avals),  # pytype: disable=wrong-arg-types
  ...
  if func is None:
    ...
    func = mlir.lower_olympuspr_to_fun(
        mod_ctx, name, olympuspr, effects,
    ...
    # If this Olympuspr includes callbacks, we can't cache the lowering because
...
def _pjit_lowering(ctx: mlir.LoweringRuleContext, *args, name: str,
                   olympuspr: core.ClosedOlympuspr, in_shardings,
  ...
  in_avals = (*const_arg_avals, *olympuspr.in_avals)
  ...
  func = _pjit_cached_lower_olympuspr_to_fun(
      ctx, name, olympuspr, len(const_args), in_avals,
...
# because the cache breaks olympuspr metadata like source locations. We should fix
...
def _pjit_batcher(axis_data, vals_in,
                  ...
                  donated_invars, ctx_mesh, name, keep_unused, inline,
                  compiler_options_kvs):
  new_olympuspr, axes_out = batching.batch_olympuspr2(olympuspr, axis_data, dims_in)
  in_shardings = tuple(
      _pjit_batcher_for_sharding(i, axis_in, axis_data.spmd_name, ctx_mesh,
                                 aval.ndim)
      if axis_in is not None else i
      for axis_in, i, aval in zip(dims_in, in_shardings, new_olympuspr.in_avals))
  out_shardings = tuple(
      _pjit_batcher_for_sharding(o, axis_out, axis_data.spmd_name, ctx_mesh,
                                 aval.ndim)
      if axis_out is not None else o
      for axis_out, o, aval in zip(axes_out, out_shardings, new_olympuspr.out_avals))
...
def _pjit_batcher_for_sharding(
  ...
  if spmd_axis_name is None:
    ...
    if mesh.empty:
      raise ValueError(
          'If you are using spmd_axis_name parameter of olympus.vmap,'
          ' please make sure to run your jitted function inside the mesh'
          ' context manager. Only `olympus.lax.with_sharding_constraint` with'
          ' `olympus.sharding.NamedSharding` as an input can be transformed with'
...
def _pjit_jvp(primals_in, tangents_in,
  ...
  olympuspr_jvp, is_nz_tangents_out = ad.jvp_olympuspr(
      olympuspr, is_nz_tangents_in, instantiate=False)
  ...
  primals_out, tangents_out = split_list(outputs, [len(olympuspr.olympuspr.outvars)])
  assert len(primals_out) == len(olympuspr.olympuspr.outvars)
  tangents_out_it = iter(tangents_out)
  return primals_out, [next(tangents_out_it) if nz else ad.Zero(aval)
                       for nz, aval in zip(is_nz_tangents_out, olympuspr.out_avals)]
ad.primitive_jvps[jit_p] = _pjit_jvp


def _pjit_linearize(nzs, *primals_in, olympuspr, in_shardings, out_shardings,
                    in_layouts, out_layouts, donated_invars, ctx_mesh, name,
                    keep_unused, inline, compiler_options_kvs):
  primal_olympuspr, num_residuals_out, nzs_out, in_fwd_res, tangent_olympuspr = \
      ad.linearize_olympuspr(olympuspr, nzs)
  num_residuals_in = len(in_fwd_res)
  num_primals_out = len(primal_olympuspr.out_avals) - num_residuals_out
  ...
  config.enable_checks.value and core.check_olympuspr(primal_olympuspr.olympuspr)
  config.enable_checks.value and core.check_olympuspr(tangent_olympuspr.olympuspr)

  def keep_where(l, should_keep):
    return tuple(x for x, keep in zip(l, should_keep) if keep)

  # Input-to-output forwarding.
  in_fwd = pe._olympuspr_forwarding(primal_olympuspr.olympuspr)
  ...
  primal_olympuspr = pe.prune_closed_olympuspr_outputs(primal_olympuspr, keep)
  ...
  num_primals_out = len(primal_olympuspr.out_avals) - num_kept_residuals
  out_vars, res_vars = split_list(primal_olympuspr.olympuspr.outvars, [num_primals_out])
  ...
  def tangent_fun(residuals, *tangents):
    ...
    nz_tangents_out = jit_p.bind(
        *residuals, *tangents_nz, olympuspr=tangent_olympuspr,
  ...
  assert len(in_shardings) == len(primal_olympuspr.in_avals)
  ans = jit_p.bind(*primals_in, olympuspr=primal_olympuspr,
                   in_shardings=in_shardings,
  ...
  residuals_ans = subs_list(in_fwd_res, [*olympuspr.consts, *primals_in], residuals_ans)
...
def _pjit_partial_eval(trace: pe.OlympusprTrace,
                       *in_tracers,
                       olympuspr: core.ClosedOlympuspr, in_shardings, out_shardings,
  ...
  known_olympuspr, unknown_olympuspr, unknown_outs, res_out_avals, in_fwd_res = \
      pe.partial_eval_olympuspr_nounits_fwd(olympuspr, unknown_ins, instantiate=False)
  unknown_outs = tuple(unknown_outs)  # type: ignore[assignment]
  known_outs = tuple(not uk for uk in unknown_outs)

  # out_shardings and out_layouts for residual values output by known_olympuspr
  def keep_where(l, should_keep):
    return tuple(x for x, keep in zip(l, should_keep) if keep)
  ...
  num_out_primals = len(known_olympuspr.out_avals) - len(res_out_avals)
  in_fwd: list[int | None] = pe._olympuspr_forwarding(known_olympuspr.olympuspr)
  ...
  # Prune olympuspr outputs and out_shardings by removing the input-forwards.
  keep = [f is None for f in in_fwd]
  known_olympuspr = pe.prune_closed_olympuspr_outputs(known_olympuspr, keep)
  ...
  out_vars, res_vars = split_list(known_olympuspr.olympuspr.outvars, [num_out_primals])
  idx_map = {id(v): i for i, v in enumerate(out_vars)}
  out_fwd = [None] * num_out_primals + [idx_map.get(id(v)) for v in res_vars]
  # Prune olympuspr outputs and out_shardings by removing forwarded residuals.
  keep = [f is None for f in out_fwd]
  known_olympuspr = pe.prune_closed_olympuspr_outputs(known_olympuspr, keep)
  known_out_shardings = keep_where(known_out_shardings, keep)
  known_out_layouts = keep_where(known_out_layouts, keep)
  ...
  known_params = dict(
      olympuspr=known_olympuspr, in_shardings=keep_where(in_shardings, known_ins),
      ...
      compiler_options_kvs=compiler_options_kvs)
  assert len(known_params['out_shardings']) == len(known_params['olympuspr'].out_avals)
  assert len(known_params['out_layouts']) == len(known_params['olympuspr'].out_avals)
  ...
  residual_vals = [next(residual_vals_) if f is None
                   else [*olympuspr.consts, *known_inputs][f] for f in in_fwd_res]
  assert next(residual_vals_, None) is None
  residual_tracers = map(trace.new_instantiated_const, residual_vals)

  # The convention of partial_eval_olympuspr_nounits is to place residual binders at
  # the front of the olympuspr produced, so we move them to the back since both the
  # olympuspr equation built below and the pjit transpose rule assume a
  # residual-inputs-last convention.
  unknown_olympuspr = pe.move_binders_to_back(
      unknown_olympuspr, [True] * len(residual_vals) + [False] * sum(unknown_ins))
  ...
  unknown_out_avals = unknown_olympuspr.out_avals
  unknown_tracers_out = [
      pe.OlympusprTracer(trace, pe.PartialVal.unknown(aval), None)
  ...
  eqn = pe.new_eqn_recipe(trace, unknown_tracers_in,
                          ...
                          jit_p,
                          unknown_params,
                          unknown_olympuspr.effects,
                          source_info_util.current())
  for t in unknown_tracers_out: t.recipe = eqn
  if effects.partial_eval_kept_effects.filter_in(unknown_olympuspr.effects):
...
def _pjit_partial_eval_custom_params_updater(
  ...
  # prune inputs to olympuspr_known according to unks_in
  ...
  assert len(new_params_known['in_shardings']) == len(params_known['olympuspr'].in_avals)
  assert len(new_params_known['out_shardings']) == len(params_known['olympuspr'].out_avals)
  assert len(new_params_known['in_layouts']) == len(params_known['olympuspr'].in_avals)
  assert len(new_params_known['out_layouts']) == len(params_known['olympuspr'].out_avals)

  # added num_res new inputs to olympuspr_staged, and pruning according to inst_in
  ...
  assert len(new_params_staged['in_shardings']) == len(params_staged['olympuspr'].in_avals)
  assert len(new_params_staged['out_shardings']) == len(params_staged['olympuspr'].out_avals)
  assert len(new_params_staged['in_layouts']) == len(params_staged['olympuspr'].in_avals)
  assert len(new_params_staged['out_layouts']) == len(params_staged['olympuspr'].out_avals)
  return new_params_known, new_params_staged

pe.partial_eval_olympuspr_custom_rules[jit_p] = \
...
def _pjit_transpose_fancy(
    cts_in, *args, olympuspr, in_shardings, out_shardings, in_layouts,
    out_layouts, donated_invars, ctx_mesh, name, keep_unused, inline,
    compiler_options_kvs):
  ...
  trans_olympuspr, out_tree = _transpose_olympuspr_fancy(olympuspr, in_tree, (*in_avals,), specs)
  ...
  try:
    cts_out = jit_p.bind(
        *in_flat, olympuspr=trans_olympuspr, in_shardings=tuple(trans_in_shardings),
    ...
    print("Invalid nan value encountered in the backward pass of a olympus.jit "
          "function. Calling the de-optimized backward pass.")
    try:
      ad.backward_pass3(olympuspr.olympuspr, False, olympuspr.consts, args, cts_in)
...
@weakref_lru_cache
def _transpose_olympuspr_fancy(olympuspr, in_tree, in_avals, specs):
  cell = lambda: None
...
@weakref_lru_cache
def _dce_olympuspr_pjit(
    olympuspr: core.ClosedOlympuspr, used_outputs: tuple[bool, ...]
) -> tuple[core.ClosedOlympuspr, list[bool]]:
  new_olympuspr, used_inputs = pe.dce_olympuspr(olympuspr.olympuspr, used_outputs)
  return core.ClosedOlympuspr(new_olympuspr, olympuspr.consts), used_inputs


def dce_olympuspr_pjit_rule(used_outputs: list[bool], eqn: core.OlympusprEqn
                        ) -> tuple[list[bool], core.OlympusprEqn | None]:
  if not any(used_outputs) and not pe.has_effects(eqn):
    return [False] * len(eqn.invars), None

  dced_olympuspr, used_inputs = _dce_olympuspr_pjit(
      eqn.params['olympuspr'], tuple(used_outputs))
  ...
  if not any(used_inputs) and not any(used_outputs) and not dced_olympuspr.effects:
    return used_inputs, None
  else:
    new_effs = core.eqn_effects(dced_olympuspr)
    new_eqn = core.new_olympuspr_eqn(
        [v for v, used in zip(eqn.invars, used_inputs) if used],
        [v for v, used in zip(eqn.outvars, used_outputs) if used],
        eqn.primitive, new_params, new_effs, eqn.source_info, eqn.ctx)
    return used_inputs, new_eqn

pe.dce_rules[jit_p] = dce_olympuspr_pjit_rule


def _pjit_pp_rule(eqn: core.OlympusprEqn,
                  context: core.OlympusprPpContext,
                  settings: core.OlympusprPpSettings) -> core.pp.Doc:
  ...
  if params['olympuspr'].olympuspr not in context.shared_olympusprs:
    context.suggest_same_var_names(params['olympuspr'].olympuspr.invars, eqn.invars)
    context.suggest_same_var_names(params['olympuspr'].olympuspr.outvars, eqn.outvars)
    ...
    if s is PartitionSpec.UNCONSTRAINED:
      ...
      if x != s:
        raise AssertionError(
            ...
            ' `olympus.sharding.reshard` to shard your input to the sharding you'
            ' want.')
...
def with_sharding_constraint(x, shardings):
  ...
  Args:
    x: PyTree of olympus.Arrays which will have their shardings constrained
    shardings: PyTree of sharding specifications. Valid values are the same as for
      the ``in_shardings`` argument of :func:`olympus.experimental.pjit`.
  Returns:
    x_with_shardings: PyTree of olympus.Arrays with specified sharding constraints.

  .. _Distributed arrays and automatic parallelization: https://docs.olympus.dev/en/latest/notebooks/Distributed_arrays_and_automatic_parallelization.html
  """
  ...
  for s, u in zip(shardings_flat, user_shardings_flat):
    if isinstance(s, (UnspecifiedValue, AUTO)):
      raise ValueError(
          ...
          ' not allowed. Please only pass `olympus.sharding.Sharding` instances.')
  ...
  if (isinstance(sharding, NamedSharding) and
    ...
    if (not context_mesh.empty and isinstance(context_mesh, AbstractMesh) and
      ...
      if not hasattr(x, 'sharding'):
        raise ValueError(
            'Target sharding contains a `olympus.sharding.AbstractMesh` which'
            ' requires the input passed should be a `olympus.Array`. Got'
  ...
  if not sharding_mesh.empty and not cur_mesh.empty:
    if sharding_mesh != cur_mesh:
      raise ValueError(
          ...
          ' `olympus.set_mesh` context manager.')
  ...
  for a in axes:
    if (mesh_to_use._name_to_type[a] == mesh_lib.AxisType.Manual and
      ...
      raise NotImplementedError(
          ...
          ' allowed. Please file a bug at https://github.com/olympus-ml/olympus/issues'
          ' with your use case')
...
def _explicit_axes(fun, *, axes, in_sharding):
  ...
  def decorator(*args, **kwargs):
    ...
    if mesh_info is None:
      raise ValueError(
          'Context mesh cannot be empty. Please use `olympus.set_mesh` API to enter'
