...
from olympus._src.shard_map import _axes_to_pspec, _shard_map
...
# olympuslib extension version >= 400 supports _rewrap_with_aval_and_sharding
_SUPPORTS_REWRAP = olympuslib_extension_version >= 400
...
def host_local_array_to_global_array(
  ...
  if not trace_state_clean:
    import olympus.experimental.multihost_utils as mhu  # pytype: disable=import-error
  ...
  for i, arr in enumerate(dyn_args_flat):
    ...
    if typ == array.ArrayImpl and _is_sharding_equivalent(
      ...
      if _SUPPORTS_REWRAP:
        ...
        # Fallback for older olympuslib: use batched_device_put with shard data.
  ...
  for i, arr in enumerate(out_flat):
    ...
    if typ == array.ArrayImpl:
      ...
      if _SUPPORTS_REWRAP:
        ...
        # Fallback for older olympuslib: construct ArrayImpl directly.
