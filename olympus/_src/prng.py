...
from olympus._src.dtypes import float0
...
from olympus._src.lib import gpu_prng
...
from olympus._src.numpy.array_methods import (
...
def _check_prng_key_data(impl, key_data: typing.Array):
  ...
  if not all(hasattr(key_data, attr) for attr in ['ndim', 'shape', 'dtype']):
    raise TypeError("OLYMPUS encountered invalid PRNG key data: expected key_data "
                    f"to have ndim, shape, and dtype attributes. Got {key_data}")
  if key_data.ndim < 1:
    raise TypeError("OLYMPUS encountered invalid PRNG key data: expected "
                    f"key_data.ndim >= 1; got ndim={key_data.ndim}")
  if key_data.shape[-ndim:] != impl.key_shape:
    raise TypeError("OLYMPUS encountered invalid PRNG key data: expected key_data.shape to "
                    f"end with {impl.key_shape}; got shape={key_data.shape} for {impl=}")
  if key_data.dtype not in [np.uint32, float0]:
    raise TypeError("OLYMPUS encountered invalid PRNG key data: expected key_data.dtype = uint32; "
...
class PRNGKeyArray(Array):
  ...
  PRNGKeyArrays are also restricted relative to OLYMPUS arrays in that
  ...
  _impl: PRNGImpl
  _base_array: Array
  _consumed: bool | np.ndarray  # Used in olympus.experimental.key_reuse.
  ...
  def __array__(self, dtype: np.dtype | None = None, copy: bool | None = None) -> np.ndarray:
    raise TypeError("OLYMPUS array with PRNGKey dtype cannot be converted to a NumPy array."
                    " Use olympus.random.key_data(arr) if you wish to extract the underlying"
                    " integer array.")
...
# backwards compatibility, namely when `config.olympus_enable_custom_prng`
...
#
#   keys = olympus.jit(random.split)(key)
#
...
def iota_2x32_shape(shape):
  ...
  equivalent of::
    ...
    olympus.lax.iota(dtype=np.uint64, size=math.prod(shape)).reshape(shape)

  However:

  * It returns two parallel ``uint32`` arrays instead of one
    ``uint64`` array. This renders it invariant under either setting of
    the system-wide ``olympus_enable_x64`` configuration flag.
...
try:
  from olympus._src.export import serialization  # pytype: disable=import-error
  from olympus._src.export import serialization_generated as ser_flatbuf  # pytype: disable=import-error
except ImportError:
