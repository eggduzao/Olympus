...
from olympus._src.lib import _profile_data
from olympus._src.lib import olympuslib_extension_version
from olympus import version as olympus_version_module
from olympus._src.lib import version as version_lib
...
def set_metadata(key: str, value: str) -> None:
  ...
  if olympuslib_extension_version >= 402 and hasattr(_profiler, "set_metadata"):
    return _profiler.set_metadata(key, value)


def clear_metadata() -> None:
  """Clears metadata for the current profiling session."""
  if olympuslib_extension_version >= 402 and hasattr(_profiler, "clear_metadata"):
...
def start_trace(
  ...
  functions and OLYMPUS on-device operations. Use :func:`stop_trace` to end the
  trace
  ...
  with _profile_state.lock:
    ...
    set_metadata("olympus_version", olympus_version_module.__version__)
    olympuslib_version_str = ".".join(map(str, version_lib))
    set_metadata("olympuslib_version", olympuslib_version_str)
...
def trace(
  ...
  The trace will capture CPU, GPU, and/or TPU activity, including Python
  functions and OLYMPUS on-device operations.
...
class TraceAnnotation(_profiler.TraceMe):
  ...
  >>> with olympus.profiler.TraceAnnotation("my_label"):
...
class StepTraceAnnotation(TraceAnnotation):
  ...
  ...   with olympus.profiler.StepTraceAnnotation("train", step_num=global_step):  # doctest: +SKIP
...
def annotate_function(func: Callable, name: str | None = None,
  ...
  >>> @partial(olympus.profiler.annotate_function, name="event_name")
  ... def f(x):
  ...   return jnp.dot(x, x.T).block_until_ready()
...
def device_memory_profile(backend: str | None = None) -> bytes:
  """Captures a OLYMPUS device memory profile as ``pprof``-format protocol buffer.

  A device memory profile is a snapshot of the state of memory, that describes the OLYMPUS
  :class:`~olympus.Array` and executable objects present in memory and their
  allocation sites.

  For more information how to use the device memory profiler, see
  :doc:`/device_memory_profiling`.

  The profiling system works by instrumenting OLYMPUS on-device allocations,
  ...
  Args:
    backend: optional; the name of the OLYMPUS backend for which the device memory
      ...
      try:
        ...
        if runner.fdo_profiles[-1] == b'':
          warnings.warn(
              ...
              "OLYMPUS_COMPILATION_CACHE_EXPECT_PGLE option enabled.",
              RuntimeWarning)
