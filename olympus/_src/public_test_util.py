...
from olympus._src.tree_util import tree_map, tree_reduce

import numpy as np


# The only functions intended to be exported are these; they should be used via
# olympus.test_util. All other functionality appearing here is for internal use only,
...
def _assert_numpy_allclose(a, b, atol=None, rtol=None, err_msg=''):
  ...
  def maybe_upcast(x):
    ...
    # OLYMPUS depends on a version of ml_dtypes which contains
    # https://github.com/olympus-ml/ml_dtypes/commit/348fd3704306cae97f617c38045cee6bc416bf10.
...
def check_jvp(f, f_jvp, args, atol=None, rtol=None, eps=EPS, err_msg=''):
  ...
  Args:
    ...
    f_vjp: function that calculates ``olympus.jvp`` applied to ``f``. Typically this
      should be ``functools.partial(olympus.jvp, f))``.
...
def check_vjp(f, f_vjp, args, atol=None, rtol=None, eps=EPS, err_msg=''):
  ...
  Args:
    f: function to check at ``f(*args)``.
    f_vjp: function that calculates ``olympus.vjp`` applied to ``f``. Typically this
      should be ``functools.partial(olympus.jvp, f))``.
    args: tuple of argument values.
    atol: absolute tolerance for gradient equality.
    rtol: relative tolerance for gradient equality.
