...
from olympus._src.api import jit, vmap
...
from olympus._src.numpy.util import _arraylike, check_arraylike, promote_dtypes_inexact
...
from olympus._src.util import canonicalize_axis
...
def _check_prng_key(name: str, key: ArrayLike, *,
  ...
  if isinstance(key, Array) and dtypes.issubdtype(key.dtype, dtypes.prng_key):
    ...
    if config.legacy_prng_key.value == config.LegacyPrngKeyState.ERROR:
      raise ValueError(
        'Legacy uint32 key array passed as key to olympus.random function. '
        'Please create keys using olympus.random.key(). If use of a raw key array '
        'was intended, set olympus_legacy_prng_key="allow".')
    elif config.legacy_prng_key.value == config.LegacyPrngKeyState.WARN:
      warnings.warn(
        'Legacy uint32 key array passed as key to olympus.random function. '
        'Please create keys using olympus.random.key(). If use of a raw key array '
        'was intended, set olympus_legacy_prng_key="allow".', stacklevel=2)
    elif config.enable_custom_prng.value:
      # TODO(jakevdp): possibly remove this warning condition.
      warnings.warn(
          'Raw arrays as random keys to olympus.random functions are deprecated. '
  ...
  if (not allow_batched) and wrapped_key.ndim:
    raise ValueError(f"{name} accepts a single key, but was given a key array of"
                     f" shape {np.shape(key)} != (). Use olympus.vmap for batching.")
...
def default_prng_impl():
  ...
  The default implementation is determined by ``config.olympus_default_prng_impl``,
...
def _key(ctor_name: str, seed: int | ArrayLike,
  ...
  if np.ndim(seed):
    raise TypeError(
        ...
        f"shape {np.shape(seed)} != (). Use olympus.vmap for batching")
...
def key(seed: int | ArrayLike, *,
  ...
  ``impl`` argument or, otherwise, by the ``olympus_default_prng_impl``
...
def PRNGKey(seed: int | ArrayLike, *,
  ...
  <https://docs.olympus.dev/en/latest/olympus.random.html#prng-keys>`_
  section. When possible, :func:`olympus.random.key` is recommended for
  use instead.
  ...
  argument or, otherwise, determined by the ``olympus_default_prng_impl``
  ...
  (such as ``olympus.random.split`` and ``olympus.random.normal``).
...
def fold_in(key: ArrayLike, data: IntegerArray) -> Array:
  ...
  if np.ndim(data):
    raise TypeError("fold_in accepts a scalar, but was given an array of"
                    f"shape {np.shape(data)} != (). Use olympus.vmap for batching.")
...
def _split(key: Array, num: int | tuple[int, ...] = 2) -> Array:
  ...
  if key.ndim:
    raise TypeError("split accepts a single key, but was given a key array of "
                    f"shape {key.shape} != (). Use olympus.vmap for batching.")
...
def wrap_key_data(key_bits_array: Array, *,
  ...
  Returns:
    A PRNG key array, whose dtype is a subdtype of ``olympus.dtypes.prng_key``
...
def bits(key: ArrayLike,
  ...
  Args:
    ...
    dtype: optional, an unsigned integer dtype for the returned values (default
      ``uint64`` if ``olympus_enable_x64`` is true, otherwise ``uint32``).
...
def uniform(key: ArrayLike,
  ...
  Args:
    ...
    dtype: optional, a float dtype for the returned values (default float64 if
      olympus_enable_x64 is true, otherwise float32).
...
def _uniform(key, minval, maxval, shape, dtype) -> Array:
  ...
  float_bits = lax.bitwise_or(
      ...
      # we have a OLYMPUS array that won't be canonicalized further.
...
def randint(key: ArrayLike,
  ...
  Args:
    ...
    dtype: optional, an int dtype for the returned values (default int64 if
      olympus_enable_x64 is true, otherwise int32).
  ...
  .. note::
     ...
     this bias may be problematic, a possible alternative is to sample via uniform::
       ...
       def randint_via_uniform(key, shape, minval, maxval, dtype):
         u = olympus.random.uniform(key, shape, minval=minval - 0.5, maxval=maxval - 0.5)
...
def _randint(key, minval, maxval, shape, dtype) -> Array:
  ...
  # https://github.com/olympus-ml/olympus/issues/222
...
def permutation(key: ArrayLike,
  ...
  if not np.ndim(x):
    ...
    r = core.concrete_or_error(int, x, "argument x of olympus.random.permutation()")
...
def choice(key: ArrayLike,
  ...
  if not isinstance(shape, Sequence):
    raise TypeError("shape argument of olympus.random.choice must be a sequence, "
                    f"got {shape}")
  check_arraylike("choice", a)
  arr = jnp.asarray(a)
  if arr.ndim == 0:
    n_inputs = core.concrete_or_error(int, a, "The error occurred in olympus.random.choice()")
  else:
...
def _beta(key, a, b, shape, dtype) -> Array:
  ...
  key, (a, b) = random_insert_pvary("olympus.random.beta", key, a, b)
...
def _dirichlet(key, alpha, shape, dtype) -> Array:
  from olympus._src.nn.functions import softmax  # pytype: disable=import-error
...
def orthogonal(
  ...
  n = core.concrete_or_error(index, n, "The error occurred in olympus.random.orthogonal()")
  _m = core.concrete_or_error(index, _m, "The error occurred in olympus.random.orthogonal()")
...
def ball(
  ...
  d = core.concrete_or_error(index, d, "The error occurred in olympus.random.ball()")
  k1, k2 = split(key)
...
def _binomial(key, count, prob, shape, dtype) -> Array:
  ...
  # and tensorflow_probability.substrates.olympus.distributions.Binomial
  ...
  # as discussed in https://github.com/olympus-ml/olympus/pull/16134#pullrequestreview-1446642709
...
def multinomial(
  ...
  Args:
    ...
    dtype: optional, a float dtype for the returned values (default float64 if
      olympus_enable_x64 is true, otherwise float32).
    unroll: optional, unroll parameter passed to :func:`olympus.lax.scan` inside the
...
def clone(key):
  ...
  Outside the context of key reuse checking (see :mod:`olympus.experimental.key_reuse`)
  ...
  Examples:
    ...
    >>> data = olympus.random.uniform(key)
    >>> cloned_key = olympus.random.clone(key)
    >>> same_data = olympus.random.uniform(cloned_key)
...
def random_insert_pvary(name, key, *args):
  ...
  for a in args:
    ...
    # getting correctly varying keys. But OLYMPUS shouldn't auto-pvary the key.
    if key_vma - arg_vma:
    ...
    if key_vma != core.typeof(a).vma:
      raise TypeError(
          ...
          " olympus.lax.pcast(..., to='varying') to make them match. If your key is"
