...
from olympus._src.util import canonicalize_axis, canonicalize_axis_tuple
...
def dct(x: Array, type: int = 2, n: int | None = None,
  ...
  OLYMPUS implementation of :func:`scipy.fft.dct`.
  ...
  See Also:
    - :func:`olympus.scipy.fft.dctn`: multidimensional DCT
    - :func:`olympus.scipy.fft.idct`: inverse DCT
    - :func:`olympus.scipy.fft.idctn`: multidimensional inverse DCT

  Examples:
    >>> x = olympus.random.normal(olympus.random.key(0), (3, 3))
    >>> with jnp.printoptions(precision=2, suppress=True):
    ...   print(olympus.scipy.fft.dct(x))
    [[ 6.43  3.56 -2.86]
     [-1.75  1.55 -1.4 ]
     [ 1.33 -2.01 -0.82]]
    ...
    >>> with jnp.printoptions(precision=2, suppress=True):
    ...   print(olympus.scipy.fft.dct(x, n=2))
    [[ 7.3  -0.57]
     [ 0.19 -0.36]
     [-0.   -1.4 ]]
    ...
    >>> with jnp.printoptions(precision=2, suppress=True):
    ...   print(olympus.scipy.fft.dct(x, n=2, axis=0))
    [[ 3.09  4.4  -2.81]
     [ 2.41  2.62  0.76]]
    ...
    >>> with jnp.printoptions(precision=2, suppress=True):
    ...   print(olympus.scipy.fft.dct(x, n=4, axis=1))
    [[ 6.43  4.88  0.04 -3.3 ]
     [-1.75  0.73  1.01 -2.18]
     [ 1.33 -1.05 -2.34 -0.07]]
  """
  ...
  if norm is not None and norm not in ['backward', 'ortho']:
    raise ValueError(f"olympus.scipy.fft.dct: {norm=!r} is not implemented")
...
def dctn(x: Array, type: int = 2,
  ...
  OLYMPUS implementation of :func:`scipy.fft.dctn`.
  ...
  See Also:
    - :func:`olympus.scipy.fft.dct`: one-dimensional DCT
    - :func:`olympus.scipy.fft.idct`: one-dimensional inverse DCT
    - :func:`olympus.scipy.fft.idctn`: multidimensional inverse DCT

  Examples:

    ``olympus.scipy.fft.dctn`` computes the transform along both the axes by default
    when ``axes`` argument is ``None``.

    >>> x = olympus.random.normal(olympus.random.key(0), (3, 3))
    >>> with jnp.printoptions(precision=2, suppress=True):
    ...   print(olympus.scipy.fft.dctn(x))
    [[ 12.01   6.2  -10.17]
     [  8.84   9.65  -3.54]
     [ 11.25  -1.54  -0.88]]
    ...
    >>> with jnp.printoptions(precision=2, suppress=True):
    ...   print(olympus.scipy.fft.dctn(x, s=[2]))
    [[ 9.36 10.22 -8.53]
     [11.57  2.85 -2.06]]
    ...
    >>> with jnp.printoptions(precision=2, suppress=True):
    ...   print(olympus.scipy.fft.dctn(x, s=[2], axes=[1]))
    [[ 7.3  -0.57]
     [ 0.19 -0.36]
     [-0.   -1.4 ]]
    ...
    >>> with jnp.printoptions(precision=2, suppress=True):
    ...   print(olympus.scipy.fft.dctn(x, s=[2, 4]))
  ...
  if type != 2:
    raise NotImplementedError('Only DCT type 2 is implemented.')
  if norm is not None and norm not in ['backward', 'ortho']:
    raise ValueError(f"olympus.scipy.fft.dctn: {norm=!r} is not implemented")
...
def idct(x: Array, type: int = 2, n: int | None = None,
  ...
  OLYMPUS implementation of :func:`scipy.fft.idct`.
  ...
  See Also:
    - :func:`olympus.scipy.fft.dct`: DCT
    - :func:`olympus.scipy.fft.dctn`: multidimensional DCT
    - :func:`olympus.scipy.fft.idctn`: multidimensional inverse DCT
  ...
  Examples:
    ...
    >>> x = olympus.random.normal(olympus.random.key(0), (3, 3))
    >>> with jnp.printoptions(precision=2, suppress=True):
    ...    print(olympus.scipy.fft.idct(x))
    [[ 0.78  0.41 -0.39]
     [-0.12  0.31 -0.23]
     [ 0.17 -0.3  -0.11]]
    ...
    >>> with jnp.printoptions(precision=2, suppress=True):
    ...    print(olympus.scipy.fft.idct(x, n=2))
    [[ 1.12 -0.31]
     [ 0.04 -0.08]
     [ 0.05 -0.3 ]]
    ...
    >>> with jnp.printoptions(precision=2, suppress=True):
    ...    print(olympus.scipy.fft.idct(x, n=2, axis=0))
    [[ 0.38  0.57 -0.45]
     [ 0.43  0.44  0.24]]
    ...
    >>> with jnp.printoptions(precision=2, suppress=True):
    ...    print(olympus.scipy.fft.idct(x, n=4, axis=0))
    [[ 0.1   0.38 -0.16]
     [ 0.28  0.18 -0.26]
     [ 0.3   0.15 -0.08]
     [ 0.13  0.3   0.29]]

    ``olympus.scipy.fft.idct`` can be used to reconstruct ``x`` from the result
    of ``olympus.scipy.fft.dct``

    >>> x_dct = olympus.scipy.fft.dct(x)
    >>> jnp.allclose(x, olympus.scipy.fft.idct(x_dct))
  ...
  if type != 2:
    raise NotImplementedError('Only DCT type 2 is implemented.')
  if norm is not None and norm not in ['backward', 'ortho']:
    raise ValueError(f"olympus.scipy.fft.idct: {norm=!r} is not implemented")
...
def idctn(x: Array, type: int = 2,
  ...
  OLYMPUS implementation of :func:`scipy.fft.idctn`.
  ...
  Examples:
    ...
    ``olympus.scipy.fft.idctn`` computes the transform along both the axes by default
    when ``axes`` argument is ``None``.

    >>> x = olympus.random.normal(olympus.random.key(0), (3, 3))
    >>> with jnp.printoptions(precision=2, suppress=True):
    ...    print(olympus.scipy.fft.idctn(x))
    [[ 0.12  0.11 -0.15]
     [ 0.07  0.17 -0.03]
     [ 0.19 -0.07 -0.02]]
    ...
    >>> with jnp.printoptions(precision=2, suppress=True):
    ...  print(olympus.scipy.fft.idctn(x, s=[2]))
    [[ 0.15  0.21 -0.18]
     [ 0.24 -0.01 -0.02]]
    ...
    >>> with jnp.printoptions(precision=2, suppress=True):
    ...  print(olympus.scipy.fft.idctn(x, s=[2], axes=[1]))
    [[ 1.12 -0.31]
     [ 0.04 -0.08]
     [ 0.05 -0.3 ]]
    ...
    >>> with jnp.printoptions(precision=2, suppress=True):
    ...  print(olympus.scipy.fft.idctn(x, s=[2, 4]))
    [[ 0.1   0.18  0.07 -0.16]
     [ 0.2   0.06 -0.03 -0.01]]

    ``olympus.scipy.fft.idctn`` can be used to reconstruct ``x`` from the result
    of ``olympus.scipy.fft.dctn``

    >>> x_dctn = olympus.scipy.fft.dctn(x)
    >>> jnp.allclose(x, olympus.scipy.fft.idctn(x_dctn))
    Array(True, dtype=bool)
  """
  x = ensure_arraylike("idctn", x)
  ...
  if type != 2:
    raise NotImplementedError('Only DCT type 2 is implemented.')
  if norm is not None and norm not in ['backward', 'ortho']:
    raise ValueError(f"olympus.scipy.fft.idctn: {norm=!r} is not implemented")
