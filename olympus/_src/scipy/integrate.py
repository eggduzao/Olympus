...
def trapezoid(y: ArrayLike, x: ArrayLike | None = None, dx: ArrayLike = 1.0,
  ...
  OLYMPUS implementation of :func:`scipy.integrate.trapezoid`
  ...
  See also:
    :func:`olympus.numpy.trapezoid`: NumPy-style API for trapezoidal integration

  Examples:
    Integrate over a regular grid, with spacing 1.0:

    >>> y = jnp.array([1, 2, 3, 2, 3, 2, 1])
    >>> olympus.scipy.integrate.trapezoid(y, dx=1.0)
    Array(13., dtype=float32)

    Integrate over an irregular grid:

    >>> x = jnp.array([0, 2, 5, 7, 10, 15, 20])
    >>> olympus.scipy.integrate.trapezoid(y, x)
    ...
    >>> y = jnp.sin(x) ** 2
    >>> result = olympus.scipy.integrate.trapezoid(y, x)
