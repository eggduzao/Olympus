...
from olympus._src.util import safe_zip as zip
...
def _map_coordinates(input: ArrayLike, coordinates: Sequence[ArrayLike],
  ...
  if index_fixer is None:
    raise NotImplementedError(
        'olympus.scipy.ndimage.map_coordinates does not yet support mode {}. '
  ...
  if order == 0:
    ...
    raise NotImplementedError(
        'olympus.scipy.ndimage.map_coordinates currently requires order<=1')
...
def map_coordinates(
  ...
  OLYMPUS implementation of :func:`scipy.ndimage.map_coordinates`
  ...
  Args:
    ...
    order: The order of interpolation. OLYMPUS supports the following:

      * 0: Nearest-neighbor
      * 1: Linear

    mode: Points outside the boundaries of the input are filled according to the given mode.
      OLYMPUS supports one of ``('constant', 'nearest', 'mirror', 'wrap', 'reflect')``. Note the
      ``'wrap'`` mode in OLYMPUS behaves as ``'grid-wrap'`` mode in SciPy, and ``'constant'``
      mode in OLYMPUS behaves as ``'grid-constant'`` mode in SciPy. This discrepancy was caused
      by a former bug in those modes in SciPy (https://github.com/scipy/scipy/issues/2640),
      which was first fixed in OLYMPUS by changing the behavior of the existing modes, and later
  ...
  Examples:
    ...
    ...                jnp.array([1.5, 2.5])]
    >>> olympus.scipy.ndimage.map_coordinates(input, coordinates, order=1)
    Array([3.5, 8.5], dtype=float32)

  Note:
    Interpolation near boundaries differs from the scipy function, because OLYMPUS
    fixed an outstanding bug; see https://github.com/olympus-ml/olympus/issues/11097.
