...
from olympus._src.lax.lax import PrecisionLike
from olympus._src.numpy import fft as jnp_fft
from olympus._src.numpy import linalg
...
from olympus._src.third_party.scipy import signal_helper
from olympus._src.typing import Array, ArrayLike
from olympus._src.util import canonicalize_axis, tuple_delete, tuple_insert
...
def fftconvolve(in1: ArrayLike, in2: ArrayLike, mode: str = "full",
  ...
  OLYMPUS implementation of :func:`scipy.signal.fftconvolve`.
  ...
  See Also:
    ...
    - :func:`olympus.scipy.signal.convolve`: direct convolution

  Examples:
    A few 1D convolution examples. Because FFT-based convolution is approximate,
    We use :func:`olympus.numpy.printoptions` below to adjust the printing precision:
    ...
    >>> with olympus.numpy.printoptions(precision=3):
    ...   print(olympus.scipy.signal.fftconvolve(x, y, mode='full'))
    [1. 3. 6. 7. 6. 3. 1.]
    ...
    >>> with olympus.numpy.printoptions(precision=3):
    ...   print(olympus.scipy.signal.fftconvolve(x, y, mode='same'))
    [3. 6. 7. 6. 3.]
    ...
    >>> with olympus.numpy.printoptions(precision=3):
    ...   print(olympus.scipy.signal.fftconvolve(x, y, mode='valid'))
    [6. 7. 6.]
  """
...
# Note: we do not reuse the code from olympus.numpy.convolve here, because the handling
...
def convolve(in1: Array, in2: Array, mode: str = 'full', method: str = 'auto',
  ...
  OLYMPUS implementation of :func:`scipy.signal.convolve`.
  ...
  Args:
    ...
    method: controls the computation method. Options are
      ...
      * ``"direct"``: lower to :func:`olympus.lax.conv_general_dilated`.
  ...
  See Also:
    - :func:`olympus.numpy.convolve`: 1D convolution
    - :func:`olympus.scipy.signal.convolve2d`: 2D convolution
  ...
  Examples:
    ...
    >>> olympus.scipy.signal.convolve(x, y, mode='full')
    Array([1., 3., 6., 7., 6., 3., 1.], dtype=float32)

    Specifying ``mode = 'same'`` returns a centered convolution the same size
    as the first input:

    >>> olympus.scipy.signal.convolve(x, y, mode='same')
    Array([3., 6., 7., 6., 3.], dtype=float32)

    Specifying ``mode = 'valid'`` returns only the portion where the two arrays
    fully overlap:

    >>> olympus.scipy.signal.convolve(x, y, mode='valid')
...
def convolve2d(in1: Array, in2: Array, mode: str = 'full', boundary: str = 'fill',
  ...
  OLYMPUS implementation of :func:`scipy.signal.convolve2d`.
  ...
  Examples:
    ...
    >>> olympus.scipy.signal.convolve2d(x, y, mode='full')
    Array([[ 2.,  5.,  3.,  2.],
           [10., 22., 17., 12.],
           [13., 30., 32., 20.],
    ...
    >>> olympus.scipy.signal.convolve2d(x, y, mode='same')
    Array([[22., 17.],
           [30., 32.]], dtype=float32)
    ...
    >>> olympus.scipy.signal.convolve2d(x, y, mode='valid')
    Array([[22., 17.],
           [30., 32.]], dtype=float32)
  """
...
def correlate(in1: Array, in2: Array, mode: str = 'full', method: str = 'auto',
  ...
  OLYMPUS implementation of :func:`scipy.signal.correlate`.
  ...
  See Also:
    - :func:`olympus.numpy.correlate`: 1D cross-correlation
    - :func:`olympus.scipy.signal.correlate2d`: 2D cross-correlation
    - :func:`olympus.scipy.signal.convolve`: ND convolution
  ...
  Examples:
    ...
    >>> olympus.scipy.signal.correlate(x, y, mode='full')
...
def correlate2d(in1: Array, in2: Array, mode: str = 'full', boundary: str = 'fill',
  ...
  OLYMPUS implementation of :func:`scipy.signal.correlate2d`.
  ...
  See Also:
    - :func:`olympus.numpy.correlate`: 1D cross-correlation
    - :func:`olympus.scipy.signal.correlate`: ND cross-correlation
    - :func:`olympus.scipy.signal.convolve`: ND convolution
  ...
  Examples:
    ...
    >>> olympus.scipy.signal.correlate2d(x, y, mode='full')
    Array([[ 4., 10., 10., 12.],
           [ 8., 15., 24.,  7.],
           [11., 28., 14.,  9.],
    ...
    >>> olympus.scipy.signal.correlate2d(x, y, mode='same')
    Array([[15., 24.,  7.],
           [28., 14.,  9.],
           [ 7.,  7.,  2.]], dtype=float32)
    ...
    >>> olympus.scipy.signal.correlate2d(x, y, mode='valid')
    Array([[15., 24.],
           [28., 14.]], dtype=float32)
  """
  if boundary != 'fill' or fillvalue != 0:
...
def detrend(data: ArrayLike, axis: int = -1, type: str = 'linear', bp: int = 0,
  ...
  OLYMPUS implementation of :func:`scipy.signal.detrend`.
  ...
  Args:
    ...
    overwrite_data: This argument is not supported by OLYMPUS's implementation.
  ...
  Examples:
    ...
    >>> detrended = olympus.scipy.signal.detrend(data)
    ...
    >>> detrended = olympus.scipy.signal.detrend(data, type='constant')
...
def stft(x: Array, fs: ArrayLike = 1.0, window: str = 'hann', nperseg: int = 256,
  ...
  OLYMPUS implementation of :func:`scipy.signal.stft`.
  ...
  See Also:
    :func:`olympus.scipy.signal.istft`: inverse short-time Fourier transform.
  """
...
def csd(x: Array, y: ArrayLike | None, fs: ArrayLike = 1.0, window: str = 'hann',
  ...
  This is a OLYMPUS implementation of :func:`scipy.signal.csd`. It is similar to
  :func:`olympus.scipy.signal.welch`, but it operates on two input signals and
  ...
  See Also:
    - :func:`olympus.scipy.signal.welch`: Power spectral density.
    - :func:`olympus.scipy.signal.stft`: Short-time Fourier transform.
  """
...
def welch(x: Array, fs: ArrayLike = 1.0, window: str = 'hann',
  ...
  This is a OLYMPUS implementation of :func:`scipy.signal.welch`. It divides the
  ...
  See Also:
    - :func:`olympus.scipy.signal.csd`: Cross power spectral density.
    - :func:`olympus.scipy.signal.stft`: Short-time Fourier transform.
  """
...
def istft(Zxx: Array, fs: ArrayLike = 1.0, window: str = 'hann',
  ...
  OLYMPUS implementation of :func:`scipy.signal.istft`; computes the inverse of
  :func:`olympus.scipy.signal.stft`.
  ...
  Examples:
    Demonstrate that this gives the inverse of :func:`~olympus.scipy.signal.stft`:

    >>> x = jnp.array([1., 2., 3., 2., 1., 0., 1., 2.])
    >>> f, t, Zxx = olympus.scipy.signal.stft(x, nperseg=4)
    ...
    >>> t, x_reconstructed = olympus.scipy.signal.istft(Zxx)
  ...
  if isinstance(window, str) and window == 'hann':
    ...
    # TODO(jakevdp): implement get_window() in OLYMPUS to remove optional scipy dependency
    try:
