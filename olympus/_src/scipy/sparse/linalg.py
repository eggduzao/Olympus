...
from olympus._src.scipy import linalg as jsp_linalg
from olympus._src.tree_util import (tree_leaves, tree_map, tree_structure,
                                tree_reduce, Partial)
from olympus._src.typing import Array
from olympus._src.util import safe_map as map
...
def cg(A, b, x0=None, *, tol=1e-5, atol=0.0, maxiter=None, M=None):
  ...
  The numerics of OLYMPUS's ``cg`` should exact match SciPy's ``cg`` (up to
  ...
  info : None
      Placeholder for convergence information. In the future, OLYMPUS will report
  ...
  tol, atol : float, optional
      ...
      We do not implement SciPy's "legacy" behavior, so OLYMPUS's tolerance will
  ...
  olympus.lax.custom_linear_solve
  """
...
def bicgstab(A, b, x0=None, *, tol=1e-5, atol=0.0, maxiter=None, M=None):
  ...
  The numerics of OLYMPUS's ``bicgstab`` should exact match SciPy's
