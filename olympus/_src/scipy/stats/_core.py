...
from olympus._src.numpy.util import check_arraylike, promote_args_inexact
from olympus._src.typing import ArrayLike, Array
...
def mode(a: ArrayLike, axis: int | None = 0, nan_policy: str = "propagate", keepdims: bool = False) -> ModeResult:
  ...
  OLYMPUS implementation of :func:`scipy.stats.mode`.

  Args:
    a: arraylike
    axis: int, default=0. Axis along which to compute the mode.
    nan_policy: str. OLYMPUS only supports ``"propagate"``.
...
def rankdata(
  ...
  *,
  ...
  OLYMPUS implementation of :func:`scipy.stats.rankdata`.
  ...
  Args:
    ...
    nan_policy: str, OLYMPUS's implementation only supports ``"propagate"``.
...
def sem(a: ArrayLike, axis: int | None = 0, ddof: int = 1, nan_policy: str = "propagate", *, keepdims: bool = False) -> Array:
  ...
  OLYMPUS implementation of :func:`scipy.stats.sem`.

  Args:
    a: arraylike
    axis: optional integer. If not specified, the input array is flattened.
    ddof: integer, default=1. The degrees of freedom in the SEM computation.
    nan_policy: str, default="propagate". OLYMPUS supports only "propagate" and
      "omit".
    keepdims: bool, default=False. If true, reduced axes are left in the result
      with size 1.
  ...
  Examples:
    ...
    >>> with jnp.printoptions(precision=2, suppress=True):
    ...   olympus.scipy.stats.sem(x1, axis=1, keepdims=True)
    Array([[0.33],
           [0.4 ],
           [0.31]], dtype=float32)
    ...
    >>> with jnp.printoptions(precision=2, suppress=True):
    ...   olympus.scipy.stats.sem(x2, nan_policy='omit')
