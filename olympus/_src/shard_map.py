...
from olympus._src.core import order_wrt_mesh
from olympus._src.core import pvary, Tracer, typeof, shard_aval, unshard_aval
from olympus._src.mesh import (AbstractMesh, Mesh, BaseMesh, AxisType,
                           use_abstract_mesh, get_abstract_mesh,
                           get_concrete_mesh)
from olympus._src.pjit import reshard
from olympus._src.lax import lax, parallel as lax_parallel
from olympus._src.lib.mlir import ir
from olympus._src.lib.mlir.dialects import hlo, sdy
from olympus._src.sharding_impls import NamedSharding, PartitionSpec
from olympus._src.util import (HashableFunction, HashablePartial, unzip2,
...
# See https://github.com/olympus-ml/olympus/pull/30753 to understand why `in_specs`
# defaults to `Infer`.
def shard_map(f=None, /, *, out_specs: Specs,
  ...
  See the docs at https://docs.olympus.dev/en/latest/notebooks/shard_map.html.
  ...
  Args:
    f: callable to be mapped. Each application of ``f``, or "instance" of ``f``,
      ...
      of the output.
    mesh: (optional, default None) a ``olympus.sharding.Mesh`` representing the
      ...
      from the context which can be set via `olympus.set_mesh` context
      manager.
    in_specs: (optional, default `Infer`) a pytree with
      ``olympus.sharding.PartitionSpec`` instances as leaves, with a tree structure
      that is a tree prefix of the args tuple to be mapped over. Similar to
      ``olympus.sharding.NamedSharding``, each ``PartitionSpec`` represents how the
...
def _smap(f, *, in_axes, out_axes, axis_name: AxisName):
  ...
  if (in_axes is not None and in_axes is not Infer and
    ...
    raise TypeError(
        "smap in_axes must be an int, None, olympus.sharding.Infer, or a tuple of"
  ...
  if (in_axes is not Infer and
    ...
    raise TypeError(
        "smap in_axes must be an int, None, olympus.sharding.Infer, or (nested)"
...
@partial(traceback_util.api_boundary, repro_api_name="olympus.shard_map")
...
def _shmap_checks(mesh, axis_names, in_specs, out_specs, _smap):
  if mesh is None:
    ...
    if mesh.empty:
      raise ValueError(
          ...
          " `olympus.set_mesh(mesh)` to enter into a mesh context")
  else:
  ...
  if not isinstance(mesh, (Mesh, AbstractMesh)):
    raise TypeError("shard_map requires a `olympus.sharding.Mesh` or a "
                    "`olympus.sharding.AbstractMesh` instance for its "
  ...
  if not axis_names.issubset(mesh_axis_names_wo_vmap):
    raise ValueError(
        f"olympus.shard_map requires axis_names={axis_names} to be a subset of "
  ...
  if (in_specs is Infer and
    ...
    if _smap:
      ...
      msg = ("shard_map in_specs argument must be a pytree of"
             " `olympus.sharding.PartitionSpec` instances, but it was `None` when"
  ...
  for s in specs_flat:
    ...
    if not full_manual:
      raise NotImplementedError(
          ...
          " file a bug at https://github.com/olympus-ml/olympus/issues.")
...
def _check_specs(error_type: SpecErrorType, specs: Any, manual_axes) -> None:
  if error_type == SpecErrorType.input and specs is None:
    raise TypeError(
        ...
        "`olympus.sharding.PartitionSpec` instances, but it was None.\n"
        "Instead of `in_specs=None`, did you mean `in_specs=P()`, "
        "where `P = olympus.sharding.PartitionSpec`?")
  ...
  raise TypeError(
      ...
      f"`olympus.sharding.PartitionSpec` instances, but:\n\n"
      + '\n\n'.join(msgs) + '\n\n'
      f"Check the {prefix}_specs values passed to shard_map.")
...
def _spec_rank_error(
  ...
  if any(not aval.ndim for _, (_, aval) in _iter_paths(tree, specs, fails)):
    msg += (f"\n\nFor scalar values (rank 0), consider using an {prefix}_specs "
            "entry of `P()`, where `P = olympus.sharding.PartitionSpec`.")
  return msg
...
def _inout_vma_error(f: Callable, mesh: Mesh | AbstractMesh, tree: PyTreeDef,
  ...
  msg = (f"shard_map applied to the function '{fun_name}' was given "
         ...
         "check_vma=False argument to `olympus.shard_map`.")
  return msg
...
OlympusType = Any
MaybeTracer = Union[OlympusType, Tracer]
...
class ShardMapPrimitive(core.Primitive):
  ...
  def get_bind_params(self, params):
    ...
    olympuspr = new_params.pop('olympuspr')
    assert isinstance(olympuspr, core.Olympuspr)
    subfun = lu.hashable_partial(
        lu.wrap_init(core.eval_olympuspr, debug_info=olympuspr.debug_info), olympuspr, ())
...
def _shard_map_staging(
    trace: pe.DynamicOlympusprTrace, prim: core.Primitive, f: lu.WrappedFun,
    in_tracers: Sequence[Any], *, mesh: Mesh,
    in_specs, out_specs_thunk, check_vma: bool, manual_axes: frozenset,
  ) -> Sequence[pe.DynamicOlympusprTracer]:
  source_info = source_info_util.current()
  to_olympuspr_tracer = partial(trace.to_olympuspr_tracer, source_info=source_info)
  ...
  with (_extend_axis_env(mesh, manual_axes), use_abstract_mesh(inner_mesh),
    ...
    olympuspr, out_avals_, consts = pe.trace_to_olympuspr_dynamic(
        f, in_avals_, lower=trace.requires_low)

  _check_names(out_specs_thunk(), out_avals_)
  if check_vma:
    out_vma = [v.aval.vma for v in olympuspr.outvars]
  ...
  with (_extend_axis_env(mesh, manual_axes), use_abstract_mesh(inner_mesh),
        config._check_vma(check_vma)):
    olympuspr = pe.convert_constvars_olympuspr(olympuspr)
  params = dict(mesh=mesh, in_specs=in_specs_staged,
                out_specs=tuple(out_specs_thunk()), olympuspr=olympuspr,
                check_vma=check_vma, manual_axes=manual_axes)
  effs = core.filter_named_axis_effects(olympuspr.effects, mesh.axis_names)
  const_tracers = map(to_olympuspr_tracer, consts)
  trace.frame.is_high |= olympuspr.is_high
  return trace.emit_eqn([*const_tracers, *in_tracers], out_avals, prim, params,
                         effs, source_info)
pe.DynamicOlympusprTrace.process_shard_map = _shard_map_staging
...
def _shard_map_typecheck(_, *in_atoms, olympuspr, mesh, in_specs, out_specs,
                         check_vma, manual_axes):
  # TODO(mattjj,parkers): check auto
  for v, x, in_spec in zip(olympuspr.invars, in_atoms, in_specs):
    sharded_aval = shard_aval(mesh, manual_axes, check_vma, in_spec, x.aval)
    if not core.typecompat(v.aval, sharded_aval):
      raise core.OlympusprTypeError("shard_map argument avals not compatible with "
                                "olympuspr binder avals and in_specs")
  ...
  out_avals_sharded = [x.aval for x in olympuspr.outvars]
...
def _shard_map_lowering_shardy(
    ...
    olympuspr: core.Olympuspr, mesh, in_specs, out_specs, manual_axes, check_vma):
  axis_ctx = ctx.module_context.axis_context
  in_avals_ = [v.aval for v in olympuspr.invars]
  ...
  if prod([mesh.shape[a] for a in manual_axes]) == 1:
    ...
    with _extend_axis_env(mesh, manual_axes), config._check_vma(check_vma):
      out_nodes, tokens_out = mlir.olympuspr_subcomp(
          sub_ctx, olympuspr, ctx.name_stack,
  ...
  #  consts that can be ndarray or olympus.Array?
  ...
  with (ir.InsertionPoint(block), _extend_axis_env(mesh, manual_axes),
    ...
    out_nodes_, tokens_out = mlir.olympuspr_subcomp(
        sub_ctx, olympuspr, ctx.name_stack,
...
def _shard_map_lowering(ctx: mlir.LoweringRuleContext, *in_nodes,
                        olympuspr: core.Olympuspr, mesh, in_specs, out_specs,
                        check_vma, manual_axes):
  if config.use_shardy_partitioner.value:
    return _shard_map_lowering_shardy(
        ctx, in_nodes, olympuspr, mesh, in_specs, out_specs, manual_axes, check_vma)

  in_avals_ = [v.aval for v in olympuspr.invars]
  out_avals_ = [x.aval for x in olympuspr.outvars]
  ...
  with _extend_axis_env(mesh, manual_axes), config._check_vma(check_vma):
    out_nodes_, tokens_out = mlir.call_lowering(
        "shmap_body", pe.close_olympuspr(olympuspr), None, sub_ctx, in_avals_,
...
def _unmatch_spec2(mesh, prev_manual, spec, x) -> OlympusType:
...
def _match_spec2(mesh, prev_manual, spec, x) -> OlympusType:
  with (core.eval_context(), api.disable_jit(False),
        use_abstract_mesh(mesh.abstract_mesh)):
...
def _match_spec(mesh: Mesh, check_vma, manual_axes, src_pspec: PartitionSpec,
                dst_pspec: PartitionSpec, x: OlympusType) -> OlympusType:
...
class ShardMapTrace(core.Trace):
  ...
  def process_call(self, call_primitive, fun, tracers, params):
    raise NotImplementedError(
        ...
        "yet supported. Put a `olympus.jit` around the `shard_map`-decorated "
  ...
  def process_map(self, map_primitive, fun, tracers, params):
    raise NotImplementedError(
        ...
        "Put a `olympus.jit` around the `shard_map`-decorated function, and open "
        "a feature request at https://github.com/olympus-ml/olympus/issues !")
...
def _shard_map_partial_eval(trace: pe.OlympusprTrace, shard_map_p,
                            f: lu.WrappedFun, tracers, mesh, in_specs,
                            out_specs_thunk, check_vma, manual_axes):
  tracers = map(trace.to_olympuspr_tracer, tracers)
  ...
  f = pe.trace_to_subolympuspr_nounits_fwd2(f, trace.tag, f.debug_info, False)
  ...
  out = shard_map_p.bind_with_trace(trace.parent_trace,
                                    ...
                                    known_params)
  in_fwd, out_fwd, out_knowns, res_avals, olympuspr, env = aux()
  ...
  env_tracers = map(trace.to_olympuspr_tracer, env)
  unk_arg_tracers = [t for t in tracers if not t.is_known()]
  out_avals_sharded = [v.aval for v in olympuspr.outvars]
  unk_params = dict(mesh=mesh, in_specs=unk_in_specs,
                    out_specs=tuple(unk_out_specs),
                    olympuspr=olympuspr.replace(debug_info=olympuspr.debug_info.with_unknown_names()),
                    check_vma=check_vma, manual_axes=manual_axes)
...
pe.OlympusprTrace.process_shard_map = _shard_map_partial_eval
...
def _shard_map_linearize(trace, shard_map_p, f: lu.WrappedFun,
  ...
  res_avals, nzs_out, lin_olympuspr, env, in_fwd, out_fwd = linearize_outs_thunk()
  ...
  with (_extend_axis_env(mesh, manual_axes),
        ...
        config._check_vma(check_vma)):
    lin_olympuspr = _promote_scalar_residuals_olympuspr(lin_olympuspr, args_to_promote)
  out_specs = out_specs_thunk()
  ...
  # TODO(mattjj): avoid round-tripping the olympuspr through eval_olympuspr here
  def f_tangent(*args):
    return core.eval_olympuspr(lin_olympuspr, (), *args)

  nz_tangents_in = [t for (t, nz) in zip(tangents, nzs_in) if nz]
  nz_tangents_out = shard_map_p.bind_with_trace(
      trace.tangent_trace,
      (lu.wrap_init(f_tangent, debug_info=lin_olympuspr.debug_info),
...
def _promote_scalar_residuals(f: Callable, *args, **kwargs):
  olympuspr, (in_fwds, out_fwds, out_pvals, out_consts, env) = f(*args, **kwargs)
  which = [f1 is None and f2 is None and not v.aval.shape
           for f1, f2, v in zip(in_fwds, out_fwds, olympuspr.constvars)]
  olympuspr = _promote_scalar_residuals_olympuspr(olympuspr, which)
  out_consts = [lax.broadcast(x, (1,)) if not getattr(x, 'shape', ()) else x
                for x in out_consts]
  return olympuspr, (in_fwds, out_fwds, out_pvals, out_consts, env)

def _promote_scalar_residuals_olympuspr(olympuspr: core.Olympuspr, which: Sequence[bool]):
  def fun(*res_and_args):
    res, args = split_list(res_and_args, [len(olympuspr.constvars)])
    res = [_rem_singleton(x) if w else x for x, w in zip(res, which)]
    return core.eval_olympuspr(olympuspr, res, *args)
  res_avals = [core.unmapped_aval(1, 0, v.aval) if w else v.aval
               for v, w in zip(olympuspr.constvars, which)]
  in_avals = [*res_avals, *[v.aval for v in olympuspr.invars]]
  olympuspr, _, _ = pe.trace_to_olympuspr_dynamic(
      lu.wrap_init(fun, debug_info=olympuspr.debug_info), in_avals)
  return olympuspr
...
def _shard_map_transpose(out_cts, *args,
  ...
  def fun_trans_callable(out_cts, args):
    ...
    olympuspr_known, olympuspr_unknown, _, _ = pe.partial_eval_olympuspr_nounits(
        pe.close_olympuspr(olympuspr), in_undef, False)
    res_reshaped = core.olympuspr_as_fun(olympuspr_known)(*res)
    in_cts = ad.backward_pass(
        olympuspr_unknown.olympuspr, False, (), (*res_reshaped, *undefs), out_cts
    )[len(res_reshaped):]
  ...
  fun_trans_callable.__name__ = f"transpose({olympuspr.debug_info.func_name})"
  fun_trans = lu.wrap_init(fun_trans_callable, debug_info=olympuspr.debug_info)
...
def _partial_eval_olympuspr_custom_rule(
    saveable: Callable[..., pe.RematCases_], unks_in: Sequence[bool],
    inst_in: Sequence[bool], eqn: core.OlympusprEqn
) -> tuple[core.OlympusprEqn, core.OlympusprEqn, Sequence[bool], Sequence[bool],
           list[core.Var]]:
  olympuspr, mesh = eqn.params['olympuspr'], eqn.params['mesh']
  ...
  with (_extend_axis_env(mesh, manual_axes), config._check_vma(check_vma),
    ...
    olympuspr_known, olympuspr_staged, unks_out, inst_out, num_res = \
        pe.partial_eval_olympuspr_custom(olympuspr, unks_in, inst_in, False, False, saveable)
  num_out_primals = len(olympuspr_known.outvars) - num_res
  in_fwd = pe._olympuspr_forwarding(olympuspr_known)[num_out_primals:]
  out_vars, res_vars = split_list(olympuspr_known.outvars, [num_out_primals])
  ...
  with (_extend_axis_env(mesh, manual_axes),
        use_abstract_mesh(_as_manual_mesh(mesh, manual_axes)),
        config._check_vma(check_vma)):
    olympuspr_known = pe.prune_olympuspr_outputs(olympuspr_known, [True] * num_out_primals + which)
    olympuspr_known, olympuspr_staged = _add_reshapes(which, olympuspr_known, olympuspr_staged)
  olympuspr_known = core.remove_named_axis_effects(olympuspr_known, mesh.axis_names)
  olympuspr_staged = core.remove_named_axis_effects(olympuspr_staged, mesh.axis_names)
  ...
  for var, w in zip(olympuspr_staged.invars[:num_res], which):
    if w:
  ...
  params_known, params_staged = _pe_custom_params(
      ...
      dict(eqn.params, olympuspr=olympuspr_known), dict(eqn.params, olympuspr=olympuspr_staged))
  eqn_known = pe.new_olympuspr_eqn(ins_known, [*out_binders_known, *residuals],
                               eqn.primitive, params_known, olympuspr_known.effects,
                               eqn.source_info, eqn.ctx)
  full_res = subs_list2(in_fwd, out_fwd, ins_known, out_binders_known, residuals)
  eqn_staged = pe.new_olympuspr_eqn([*full_res, *ins_staged], out_binders_staged,
                                eqn.primitive, params_staged,
                                olympuspr_staged.effects, eqn.source_info, eqn.ctx)
...
pe.partial_eval_olympuspr_custom_rules[shard_map_p] = \
    _partial_eval_olympuspr_custom_rule

def _add_reshapes(which: Sequence[bool],
                  olympuspr_known: core.Olympuspr,
                  olympuspr_staged: core.Olympuspr) -> tuple[core.Olympuspr, core.Olympuspr]:
  # add singleton axes to residuals which are from olympuspr_known and are scalars
  which_ = [w and not v.aval.shape  # pytype: disable=attribute-error
            for w, v in zip(which, olympuspr_staged.invars[:len(which)])]
  if not any(which_): return olympuspr_known, olympuspr_staged
  assert not olympuspr_known.constvars and not olympuspr_staged.constvars

  def known(*args):
    out = core.eval_olympuspr(olympuspr_known, (), *args)
  ...
  avals_in = [v.aval for v in olympuspr_known.invars]
  olympuspr_known, _, () = pe.trace_to_olympuspr_dynamic(
      lu.wrap_init(known, debug_info=olympuspr_known.debug_info), avals_in)

  def staged(*args):
    res_, ins = split_list(args, [len(which)])
    res = [_rem_singleton(x) if w else x for x, w in zip(res_, which_)]
    return core.eval_olympuspr(olympuspr_staged, (), *res, *ins)
  res_avals = [core.unmapped_aval(1, 0, v.aval) if w else v.aval
               for w, v in zip(which_, olympuspr_staged.invars[:len(which)])]
  avals_in = [*res_avals, *[v.aval for v in olympuspr_staged.invars[len(which):]]]
  olympuspr_staged, _, () = pe.trace_to_olympuspr_dynamic(
      lu.wrap_init(staged, debug_info=olympuspr_staged.debug_info), avals_in)

  return olympuspr_known, olympuspr_staged
...
def _pe_custom_params(unks_in, inst_in, kept_outs_known, kept_outs_staged,
  ...
  assert len(out_specs_known) == len(params_known['olympuspr'].outvars)
...
# TODO(mattjj): de-duplicate with pe.dce_olympuspr_call_rule, and/or _pmap_dce_rule?
def _shard_map_dce(used_outputs: list[bool], eqn: core.OlympusprEqn
  ...
  with (_extend_axis_env(mesh, manual_axes), config._check_vma(check_vma),
        use_abstract_mesh(_as_manual_mesh(mesh, manual_axes))):
    olympuspr, used_inputs = pe.dce_olympuspr(eqn.params['olympuspr'], used_outputs)
  if not any(used_inputs) and not any(used_outputs) and not olympuspr.effects:
    ...
    new_params = dict(eqn.params, olympuspr=olympuspr, in_specs=tuple(in_specs),
...
def _shard_map_discharge(
    in_avals, out_avals, *args, olympuspr, mesh, in_specs, out_specs, check_vma,
    manual_axes):
  inner_mesh = _as_manual_mesh(mesh, manual_axes)
  with (_extend_axis_env(mesh, manual_axes), use_abstract_mesh(inner_mesh),
        config._check_vma(check_vma)):
    discharged_olympuspr, discharged_consts = discharge.discharge_state(olympuspr, ())
  if discharged_consts: raise NotImplementedError
  del discharged_consts

  ref_specs = [spec for spec, invar in zip(in_specs, olympuspr.invars)
               if isinstance(invar.aval, AbstractRef)]
  params = dict(olympuspr=discharged_olympuspr, out_specs=(*out_specs, *ref_specs))
  ...
  out_vals, ref_vals = split_list(out_and_ref_vals, [len(olympuspr.outvars)])
