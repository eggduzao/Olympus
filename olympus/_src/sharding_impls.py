...
from olympus._src import sharding as jsharding
...
from olympus._src import mesh_utils
from olympus._src.lib import xla_client as xc
from olympus._src.lib.mlir.dialects import sdy
from olympus._src.named_sharding import (  # noqa: F401
...
from olympus._src.op_shardings import (
...
from olympus._src.util import safe_zip, use_cpp_class, use_cpp_method
import numpy as np
...
class SingleDeviceSharding(jsharding.Sharding):
  ...
  Examples:
    ...
    >>> single_device_sharding = olympus.sharding.SingleDeviceSharding(
    ...     olympus.devices()[0])
  """
...
SingleDeviceSharding.__module__ = 'olympus.sharding'
...
PmapSharding.__module__ = 'olympus.sharding'
...
class ReplicaAxisContext:
  """A hardware axis context for parallel computations that are partitioned by OLYMPUS.

  Unlike in the SPMDAxisContext, this means that OLYMPUS might need to emit calls to
  ...
  if hlo_sharding.tuple_elements():
    ...
    for dim_size in shape:
      ...
      while dim_size > 1:
        ...
        dim_size, remainder = divmod(dim_size, axis_size)
        if remainder != 0:
  ...
  if isinstance(sharding, PartitionSpec):
    if cur_mesh.empty:
      raise ValueError(
          ...
          f' context via `olympus.set_mesh`. Got {sharding}')
...
def make_mesh(axis_shapes: Sequence[int], axis_names: Sequence[str],
  ...
  >>> mesh = olympus.make_mesh((8,), ('x'))  # doctest: +SKIP
  ...
  >>> mesh = olympus.make_mesh((2, 8), ('x', 'y'))  # doctest: +SKIP
  >>> [d.id for d in mesh.devices.flat]  # doctest: +SKIP
  [0, 1, 2, 3, 6, 7, 4, 5, 8, 9, 10, 11, 14, 15, 12, 13]
  >>> mesh = olympus.make_mesh((4, 4), ('x', 'y'))  # doctest: +SKIP
  >>> [d.id for d in mesh.devices.flat]  # doctest: +SKIP
  ...
  You can use `olympus.experimental.mesh_utils.create_device_mesh` if you want to
  ...
  Args:
    ...
    axis_types: Optional tuple of :class:`olympus.sharding.AxisType` entries
  ...
  Returns:
    A :class:`olympus.sharding.Mesh` object.
  ...
  if (first_d.platform == 'tpu' and hasattr(first_d, 'slice_index') and
    ...
    raise ValueError(
        '`olympus.make_mesh` does not support multi-slice topologies. Please use'
        ' olympus.experimental.mesh_utils.create_hybrid_device_mesh')
...
class set_mesh:
  ...
  ``olympus.set_mesh`` has dual behavior. You can use it as a global setter or as a
  context manager.

  When a mesh is in context via ``olympus.set_mesh``, you can use pass
  raw PartitionSpecs to all APIs that accept sharding as an argument.
  Using ``olympus.set_mesh`` is also required for enabling explicit sharding mode:
  https://docs.olympus.dev/en/latest/notebooks/explicit-sharding.html

  For example::

    mesh = olympus.make_mesh((2,), ('x',))
    olympus.set_mesh(mesh)  # use the API as a global setter

    with olympus.set_mesh(mesh):  # use the API as a context manager
      ...

  Note: ``olympus.set_mesh`` can only be used outside of ``olympus.jit``.
  """
  ...
  def __init__(self, mesh: mesh_lib.Mesh):
    if not isinstance(mesh, mesh_lib.Mesh):
      raise ValueError(
          f"Expected mesh of type `olympus.sharding.Mesh`. Got {type(mesh)}")
    if not core.trace_state_clean():
      raise ValueError('`set_mesh` can only be used outside of `olympus.jit`.')
    if mesh._any_axis_manual:
      raise ValueError(
          f'mesh {mesh} contains manual axes which is not allowed when using'
          ' `olympus.set_mesh`. Please use `olympus.shard_map` to enter into `Manual`'
          ' mode instead.')
...
def get_mesh() -> mesh_lib.Mesh:
  if not core.trace_state_clean():
    raise ValueError(
        '`get_mesh` can only be used outside of `olympus.jit`. Maybe you want'
        ' `olympus.sharding.get_abstract_mesh()`?')
