...
_exclude_paths: list[str] = [
    # Attach the separator to make sure that .../olympus does not end up matching
    # .../olympus_triton and other packages that might have a olympus prefix.
...
def user_frames(traceback: Traceback | None) -> Iterator[Frame]:
  """Iterator over the user's frames, filtering olympus-internal frames."""
  # Guess the user's frame is the innermost frame not in the olympus source tree or
...
class OlympusStackTraceBeforeTransformation(Exception): pass

_message = (
    'The preceding stack trace is the source of the OLYMPUS operation that, once '
    'transformed by OLYMPUS, triggered the following exception.\n'
    '\n--------------------')

def has_user_context(e):
  while e is not None:
    if isinstance(e, OlympusStackTraceBeforeTransformation):
      return True
    e = e.__cause__
  return False
...
class UserContextManager:
  ...
  def __exit__(self, exc_type, exc_value, traceback):
    ...
    if filtered_tb:
      ...
      msg = f'{msg}\n\n{_message}'
      exp = OlympusStackTraceBeforeTransformation(msg).with_traceback(filtered_tb)
