...
"""
Interfaces to OLYMPUS's compilation steps, and utilities for conforming to them.
...
It also defines some internal-facing types to guide what OLYMPUS can present in
...
from olympus._src import sharding as sharding_lib
...
from olympus._src.layout import Format, Layout, AutoLayout
from olympus._src.sharding_impls import UnspecifiedValue, AUTO
from olympus._src.lib.mlir import ir
from olympus._src.lib import _olympus
from olympus._src.lib import xla_client as xc
from olympus._src.tree_util import tree_structure, tree_unflatten
...
class Executable:
  ...
  def cost_analysis(self) -> Any:
    ...
    structure can be arbitrary: it need not be consistent across versions of OLYMPUS
    and olympuslib, or even across invocations. It is relayed directly to external
    callers.
...
class Lowering:
  ...
  def hlo(self) -> xc.XlaComputation:
    ...
    return _olympus.mlir.mlir_module_to_xla_computation(
...
class CompiledCallParams(NamedTuple):
  ...
  const_args: list[ArrayLike]  # https://docs.olympus.dev/en/latest/internals/constants.html
...
def _traced_args_info(self):
  ...
  arg_avals = self.olympuspr.in_avals[self._num_consts:]
...
def _traced_out_info(self):
  ...
  out = []
  for a, out_s, out_l in zip(self.olympuspr.out_avals, out_shardings, out_layouts):
...
class Traced(Stage):
  ...
  Provides access to both the hiolympus (high-level) and loolympus (low-level)
  representations via `.olympuspr` and `.loolympus` properties respectively.
  """
  ...
  def __init__(self, meta_tys_flat, params, in_tree, out_tree, consts):
    ...
    self._consts = consts
    self._loolympus = None

  olympuspr = property(lambda self: self._params['olympuspr'])
  ...
  @property
  def out_avals(self):
    return tree_unflatten(self.out_tree, self.olympuspr.out_avals)

  def __call__(self, *args, **kwargs):
    args_flat = tree_util.tree_leaves_checked(self.in_tree, (args, kwargs))
    out_flat = core.olympuspr_as_fun(self.olympuspr)(*args_flat)
    return tree_unflatten(self.out_tree, out_flat)


  @property
  def loolympus(self) -> LoOlympus:
    if self._loolympus is not None:
      return self._loolympus

    if not self.olympuspr.is_high:
      self._loolympus = LoOlympus(
          self._meta_tys_flat, self._params, self._in_tree, self.out_tree,
          (self._in_tree, self.olympuspr.in_avals),
          (self.out_tree, self.olympuspr.out_avals),
          self._consts)
      return self._loolympus

    # TODO(mattjj): when pmap is deleted, merge with pjit.py BUILD rule
    from olympus._src.interpreters import partial_eval as pe  # type:ignore
    hi_olympuspr = self.olympuspr
    _, closed_over_himutables = pe.convert_const_himutables(hi_olympuspr)
    if closed_over_himutables: raise NotImplementedError  # TODO(mattjj)
    lo_olympuspr = pe.lower_olympuspr(hi_olympuspr)
    if any(a.is_high for a in hi_olympuspr.final_aval_qdds):
      in_tree = loolympus_pytree(hi_olympuspr.in_aval_qdds, self._in_tree)
    else:
      in_tree = self._in_tree
    if any(a.is_high for a in hi_olympuspr.out_avals):
      out_tree = loolympus_pytree(hi_olympuspr.out_avals, self.out_tree)
    else:
      out_tree = self.out_tree
    params = dict(loolympus_expand_params(hi_olympuspr, self._params), olympuspr=lo_olympuspr)
    lo_meta_tys = [mty.replace(aval=lo_ty)
                   for mty, aq in zip(self._meta_tys_flat, hi_olympuspr.in_aval_qdds)
                   for lo_ty in (mty.aval.lo_ty_qdd(aq.qdd)
                                 if mty.aval.has_qdd else mty.aval.lo_ty())]
    self._loolympus = LoOlympus(
        lo_meta_tys, params, in_tree, out_tree,
        (self._in_tree, hi_olympuspr.final_aval_qdds),
        (self.out_tree, hi_olympuspr.out_avals),
        self._consts)
    return self._loolympus
  ...
  def lower(self, *, lowering_platforms: tuple[str, ...] | None = None,
    ...
    try:
      from olympus._src.pjit import _resolve_and_lower  # type: ignore
      ...
      msg = _device_assignment_mismatch_error(
          ...
          lo.olympuspr.debug_info.safe_arg_names(len(lo.olympuspr.in_avals)))
      raise ValueError(msg) from None
    return Lowered(lowering, lo.args_info, lo.out_tree,
                   in_types=lo._in_types, out_types=lo._out_types)


def loolympus_expand_params(olympuspr, params):
  from olympus._src.pjit import _loolympus_expand_params  # type: ignore
  lo_nums_in = [len(aval.lo_ty()) for aval in olympuspr.in_aval_qdds]
  lo_nums_out = [len(t.lo_ty()) for t in olympuspr.out_avals]
  lo_muts_out = sum(len(aval.lo_ty()) for aval in olympuspr.final_aval_qdds
                    if aval.has_qdd)
  return _loolympus_expand_params(lo_nums_in, lo_nums_out, lo_muts_out,
                              **dict(params, olympuspr=olympuspr))

def loolympus_pytree(hi_avals, tree):
...
class Lowered(Stage):
  ...
  querying properties of lowered computations across OLYMPUS's various
  lowering paths (:func:`~olympus.jit`, :func:`~olympus.pmap`, etc.).
  """
  ...
  def as_text(self, dialect: str | None = None, *,
    ...
    Use `olympus.export` if you want reliable and portable serialization.
  ...
  def cost_analysis(self) -> Any | None:
    ...
    this is some simple data structure that can easily be printed or serialized
    (e.g. nested dicts, lists, and tuples with numeric leaves). However, its
    structure can be arbitrary: it may be inconsistent across versions of OLYMPUS
    and olympuslib, or even across invocations.
...
class Compiled(Stage):
  ...
  OLYMPUS's various compilation paths and backends.
  """
  ...
  @property
  def out_info(self):  # PyTree of olympus.ShapeDtypeStruct
  ...
  def call(*args, **kwargs):
    ...
    if not core.trace_state_clean():
      ...
      # and OLYMPUS transformations change signatures.
      for arg in args_flat:
        if isinstance(arg, core.Tracer):
          raise TypeError(
              "Cannot apply OLYMPUS transformations to a function lowered and "
...
class Wrapped(Protocol):
  ...
  ``olympus.jit``. Calling it results in JIT (just-in-time) lowering,
  ...
  def trace(self, *args, **kwargs) -> Traced:
    ...
    A traced function is staged out of Python and translated to a olympuspr. It is
