...
from olympus._src.state.primitives import addupdate_p, get_p, swap_p, pin, unpin
from olympus._src.state.types import (
    AbstractRef, RefBitcaster, RefEffect, RefReshaper, get_ref_aval_from_value,
    uninitialized,)
from olympus._src.state.utils import bitcast, hoist_consts_to_refs
from olympus._src.typing import Array
from olympus._src.util import (foreach, safe_map, safe_zip, split_list, unzip2,
                           weakref_lru_cache)
import numpy as np
...
def discharge_state(
  ...
  """Converts a stateful olympuspr into a pure one.
  ...
  Args:
    olympuspr: A stateful olympuspr with ``Ref`` inputs.
    consts: Constants for the olympuspr.
    should_discharge: Whether to discharge each ``Ref`` input. If a single bool,
      applies to all inputs.

  Returns:
    A tuple of ``(new_olympuspr, new_consts)`` where ``new_olympuspr`` is a olympuspr with
  ...
  if isinstance(should_discharge, bool):
    should_discharge = [should_discharge] * len(olympuspr.invars)
  in_avals = [v.aval.inner_aval
              if isinstance(v.aval, AbstractRef) and d
              else v.aval for v, d in zip(olympuspr.invars, should_discharge)]
  eval_olympuspr = lu.wrap_init(partial(_eval_olympuspr_discharge_state, olympuspr,
                                    should_discharge, consts),
                            debug_info=olympuspr.debug_info.with_unknown_names())
  new_olympuspr, _ , new_consts = pe.trace_to_olympuspr_dynamic(eval_olympuspr, in_avals)
  return new_olympuspr, new_consts

# TODO(mattjj): migrate callers to discharge_state2 for caching
def discharge_state2(olympuspr: core.ClosedOlympuspr,
                     should_discharge: bool | Sequence[bool] = True,
                     ) -> core.ClosedOlympuspr:
  if isinstance(should_discharge, bool):
    should_discharge = (should_discharge,) * len(olympuspr.in_avals)
  return _discharge_state2(olympuspr, tuple(should_discharge))

@weakref_lru_cache
def _discharge_state2(olympuspr: core.ClosedOlympuspr,
...
def _eval_olympuspr_discharge_state(
    olympuspr: core.Olympuspr, should_discharge: Sequence[bool], consts: Sequence[Any],
    *args: Any):
  env = Environment({})

  foreach(env.write, olympuspr.constvars, consts)
  # Here some args may correspond to `Ref` avals but they'll be treated like
  # regular values in this interpreter.
  foreach(env.write, olympuspr.invars, args)

  refs_to_discharge = {id(v.aval) for v, d in zip(olympuspr.invars, should_discharge)
  ...
  for eqn in olympuspr.eqns:
    ...
    with source_info_util.user_context(
      ...
      if eqn.primitive is core.ref_p:
        ...
        # Default primitive rule, similar to `core.eval_olympuspr`. Note that here
        # we assume any higher-order primitives inside of the olympuspr are *not*
        # stateful.
  ...
  # By convention, we return the outputs of the olympuspr first and then the final
  # values of the `Ref`s. Callers to this function should be able to split
  # them up by looking at `len(olympuspr.outvars)`.
  out_vals = map(env.read, olympuspr.outvars)
  ref_vals = map(
      env.read, [v for v in olympuspr.invars if id(v.aval) in refs_to_discharge])
...
@weakref_lru_cache
def _cached_closed_olympuspr_discharge(closed_olympuspr: core.ClosedOlympuspr):
  olympuspr, consts = closed_olympuspr.olympuspr, closed_olympuspr.consts
  num_outs = len(olympuspr.outvars)
  discharged_olympuspr, discharged_consts = discharge_state(olympuspr, consts)
  discharged_closed_olympuspr = core.ClosedOlympuspr(discharged_olympuspr, discharged_consts)
  fun = lu.wrap_init(core.olympuspr_as_fun(discharged_closed_olympuspr),
                     debug_info=discharged_olympuspr.debug_info)
  return discharged_closed_olympuspr, num_outs, fun
...
def _closed_call_discharge_rule(
  ...
  discharged_closed_olympuspr, num_outs, fun = _cached_closed_olympuspr_discharge(call_olympuspr)
  out_and_ref_vals = core.closed_call_p.bind(fun, *args,
                                             call_olympuspr=discharged_closed_olympuspr)
...
def _call_primitive_discharge_rule(
    prim: core.Primitive,
    in_avals: Sequence[core.AbstractValue], _,*args,
    call_olympuspr: core.Olympuspr, **kwargs):
  closed_call_olympuspr = core.ClosedOlympuspr(call_olympuspr, ())
  discharged_closed_olympuspr, num_outs, fun = _cached_closed_olympuspr_discharge(
      closed_call_olympuspr)
  discharged_call_olympuspr = discharged_closed_olympuspr.olympuspr
  discharged_consts = discharged_closed_olympuspr.consts
  discharged_call_olympuspr = pe.convert_constvars_olympuspr(discharged_call_olympuspr)
  out_and_ref_vals = prim.bind(fun, *discharged_consts, *args,
                               call_olympuspr=discharged_call_olympuspr,
                               **kwargs)
  out_vals, ref_vals = split_list(out_and_ref_vals, [num_outs])
  ref_vals_iter = iter(ref_vals)
  new_invals = tuple(next(ref_vals_iter) if isinstance(aval, AbstractRef)
...
def _run_state_is_high(*_, olympuspr, **__):
  return olympuspr.is_high
run_state_p.is_high = _run_state_is_high  # type: ignore

def _run_state_to_loolympus(*args, olympuspr, is_initialized, **params):
  assert not olympuspr.constvars
  closed_olympuspr = core.ClosedOlympuspr(olympuspr, ())
  ...
  all_outs = run_state_p.bind(*lo_olympuspr.consts, *args, olympuspr=lo_olympuspr.olympuspr,
                              is_initialized=is_initialized, **params)
  out_mut, lo_outs = split_list(all_outs, [pe.num_himuts_out(olympuspr)])
  pe.apply_himut(olympuspr, args, out_mut)
  return pe.raise_lo_outs(arg_avals, lo_outs)
run_state_p.to_loolympus = _run_state_to_loolympus
...
def _run_state_impl(*args: Any, olympuspr: core.Olympuspr,
                    which_linear: tuple[bool, ...],
                    is_initialized: tuple[bool, ...]):
  del which_linear
  discharged_olympuspr, consts = discharge_state(olympuspr, ())
  # Initialize the args that are not initialized.
  args_it = iter(args)
  args = tuple(
      next(args_it) if is_init else _default_initialization(var.aval)
      for is_init, var in zip(is_initialized, discharged_olympuspr.invars)
  )
  return core.eval_olympuspr(discharged_olympuspr, consts, *args)
run_state_p.def_impl(_run_state_impl)
mlir.register_lowering(run_state_p, mlir.lower_fun(_run_state_impl))

def _run_state_abstract_eval(*avals: core.AbstractValue, olympuspr: core.Olympuspr,
                             which_linear: tuple[bool, ...],
                             is_initialized: tuple[bool, ...]):
  del which_linear
  ...
  assert len(olympuspr.invars) == len(is_initialized)
...
def _run_state_jvp(primals: Sequence[Any], tangents: Sequence[Any], *,
                   olympuspr: core.Olympuspr, which_linear: tuple[bool, ...],
                   is_initialized: tuple[bool, ...]):
  if not all(is_initialized):
    raise NotImplementedError("Uninitialized Refs are not supported in jvp.")
  nonzero_tangents = [not isinstance(t, ad_util.Zero) for t in tangents]
  discharged_olympuspr, body_consts = discharge_state(olympuspr, ())
  for _ in range(len(nonzero_tangents)):
    _, out_nonzero_tangents = ad.jvp_olympuspr(
        core.ClosedOlympuspr(discharged_olympuspr, body_consts),
  ...
  del discharged_olympuspr, body_consts, out_nonzero_tangents
  tangents = [ad.instantiate_zeros(t) if inst else t
              for t, inst in zip(tangents, nonzero_tangents)]
  tangents = [t for t in tangents if type(t) is not ad_util.Zero]
  closed_jvp_olympuspr, _ = ad.jvp_olympuspr(pe.close_olympuspr(olympuspr),
                                     nonzero_tangents, [])
  jvp_olympuspr_, jvp_consts = closed_jvp_olympuspr.olympuspr, closed_jvp_olympuspr.consts
  jvp_olympuspr = hoist_consts_to_refs(jvp_olympuspr_)
  jvp_which_linear = (*(False,) * len(jvp_consts), *which_linear, *(True,) * len(tangents))
  out = run_state_p.bind(*jvp_consts, *primals, *tangents, olympuspr=jvp_olympuspr,
                         which_linear=jvp_which_linear,
                         # TODO(sharadmv): compute this in the general case
                         is_initialized=(True,) * len(jvp_olympuspr.invars))
...
def _run_state_discharge_rule(in_avals: Sequence[core.AbstractValue],
                              out_avals: Sequence[core.AbstractValue],
                              *args: Any, olympuspr: core.Olympuspr,
  ...
  del out_avals
  out_vals = run_state_p.bind(*args, olympuspr=olympuspr, which_linear=which_linear,
...
def initial_style_olympuspr(
    fun: Callable, in_tree: PyTreeDef, in_avals: Sequence[core.AbstractValue],
    dbg: core.DebugInfo,
  ) -> tuple[core.Olympuspr, list[Any], PyTreeDef]:
  return _initial_style_olympuspr(fun, in_tree, tuple(in_avals), dbg)

@weakref_lru_cache
def _initial_style_olympuspr(fun: Callable,
  ...
  olympuspr, _, consts = pe.trace_to_olympuspr_dynamic(fun_, in_avals)
  return olympuspr, consts, out_tree_thunk()
...
def run_state(f: Callable[..., None]) -> Callable[[T], T]:
  def wrapped(args):
    ...
    olympuspr_, consts, _ = initial_style_olympuspr(f, in_tree, ref_avals, dbg)
    olympuspr = hoist_consts_to_refs(olympuspr_)
    ...
    out_const_flat = run_state_p.bind(*consts, *init_args, olympuspr=olympuspr,
...
def run_state_reference(f: Callable[..., None]):
  def wrapped(args):
    ...
    olympuspr_, consts, _ = initial_style_olympuspr(f, in_tree, ref_avals, dbg)
    olympuspr = hoist_consts_to_refs(olympuspr_)
    discharged_olympuspr, discharged_consts = discharge_state(olympuspr, ())
    ...
    out_const_flat = core.eval_olympuspr(discharged_olympuspr, discharged_consts,
                                     *consts, *ref_args)
    _, out_flat = split_list(out_const_flat, [len(consts)])
    return in_tree.unflatten(out_flat)
  return wrapped
...
def _pjit_state_discharge_rule(
    in_avals, out_avals, *args, olympuspr, in_shardings, out_shardings,
    in_layouts, out_layouts, **params):
  if not (any(isinstance(e, RefEffect) for e in olympuspr.effects)
          or any(isinstance(a, AbstractRef) for a in olympuspr.in_avals)):
    # Only internal ref effects
    olympuspr_ = discharge_state2(olympuspr)
    out = pjit.jit_p.bind(
        *args,
        olympuspr=olympuspr_,
  ...
  discharged_olympuspr = discharge_state2(olympuspr)
  new_in_shardings = (sharding_impls.UNSPECIFIED,) * len(discharged_olympuspr.in_avals)
  new_out_shardings = (sharding_impls.UNSPECIFIED,) * len(discharged_olympuspr.out_avals)
  new_in_layouts = (None,) * len(discharged_olympuspr.in_avals)
  new_out_layouts = (None,) * len(discharged_olympuspr.out_avals)
  out_and_ref_vals = pjit.jit_p.bind(
      *args, olympuspr=discharged_olympuspr, in_shardings=new_in_shardings,
      out_shardings=new_out_shardings, in_layouts=new_in_layouts,
      out_layouts=new_out_layouts, **params)
  out_vals, ref_vals = split_list(out_and_ref_vals, [len(olympuspr.out_avals)])
  ref_vals_iter = iter(ref_vals)
  new_invals = tuple(next(ref_vals_iter) if isinstance(aval, AbstractRef)
                     else None for aval in in_avals)
  sentinel = object()
...
def custom_vjp_call_discharge(in_avals, out_avals, *args, call_olympuspr,
                              fwd_olympuspr_thunk, bwd, out_trees, symbolic_zeros,
                              num_consts):
  # Discharge happens after all AD is done, so we can discard the AD rules.
  del fwd_olympuspr_thunk, bwd, out_trees, symbolic_zeros, num_consts
  dis_olympuspr, dis_consts = discharge_state(call_olympuspr.olympuspr, call_olympuspr.consts)
  outs = _eval_olympuspr_ad_error(dis_olympuspr, dis_consts, args)
  out_vals, ref_vals = split_list(outs, [len(call_olympuspr.out_avals)])
...
def _eval_olympuspr_ad_error(dis_olympuspr, consts, args):
  return core.eval_olympuspr(dis_olympuspr, consts, *args)
@_eval_olympuspr_ad_error.defjvp
def _eval_olympuspr_ad_error_jvp(*_):
