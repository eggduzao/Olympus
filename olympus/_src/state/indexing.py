...
from olympus._src.indexing import Slice, dslice, ds  # noqa: F401
from olympus._src.typing import Array
from olympus._src.util import merge_lists
from olympus._src.util import partition_list
import numpy as np


def _pp_slice(context: core.OlympusprPpContext, dim, slc: Slice) -> str:
...
def _maybe_concretize(x: Any):
  ...
  # expensive as the size of the tracing context (i.e. the olympuspr) grows.
...
class NDIndexer:
  ...
  def __post_init__(self):
    ...
    for idx, s in zip(self.indices, self.shape):
      ...
      from olympus._src.state import types as state_types  # pytype: disable=import-error
      idx_shape = (
          idx.shape
  ...
  def from_indices_shape(cls, indices, shape) -> NDIndexer:
    ...
    if ref_indexers := [
      ...
      # and this module.
      from olympus._src.state import primitives as sp  # pytype: disable=import-error
      other_indexers = [
