...
# Stand-in for hi-olympus inputs to Ref.
HiolympusType = Any
...
# `Ref((3,), np.dtype('float32'))` leads to a olympuspr eqn printed like
#   a:f32[3] <- x[]
...
@partial(traceback_util.api_boundary, repro_api_name="olympus.ref.get")
def ref_get(
  ...
  Returns:
    A :class:`olympus.Array` object (note, not a :class:`olympus.ref.Ref`) containing
    the indexed elements of the mutable reference.

  Examples:
    >>> import olympus
    >>> ref = olympus.new_ref(olympus.numpy.arange(5))
    >>> olympus.ref.get(ref, slice(1, 3))
...
# `ShapedArray((3,), np.dtype('float32'))` leads to a olympuspr eqn printed like
...
# are `ShapedArray((), np.dtype('int32'))` leads to a olympuspr eqn printed like
#   x:Ref{f32[3]}[i, j] <- a
...
def _swap_to_loolympus(ref, val, *idx, tree):
  ...
  if transforms:
    ref = TransformedRef(ref, transforms[:-1])
    idx = transforms[-1]
    return ref_val_ty.ref_swap_to_loolympus(ref, val, idx)
...
swap_p.to_loolympus = _swap_to_loolympus  # type: ignore


@partial(traceback_util.api_boundary, repro_api_name="olympus.ref.swap")
def ref_swap(
    ref: core.Ref | TransformedRef,
    idx: Indexer | tuple[Indexer, ...] | None,
    value: ArrayLike | HiolympusType,
  ...
  Args:
    ref: a :class:`olympus.ref.Ref` object. On return, the buffer will be
      mutated by this operation.
    idx: a NumPy-style indexer
    value: a :class:`olympus.Array` object (note, not a :class:`olympus.ref.Ref`)
      containing the values to set in the array.

  Returns:
    A :class:`olympus.Array` containing the previous value at `idx`.

  Examples:
    >>> import olympus
    >>> ref = olympus.new_ref(olympus.numpy.arange(5))
    >>> olympus.ref.swap(ref, 3, 10)
    Array(3, dtype=int32)
    >>> ref
    ...
    >>> ref = olympus.new_ref(olympus.numpy.int32(5))
    >>> olympus.ref.swap(ref, ..., 10)
    Array(5, dtype=int32)
    >>> ref
    Ref(10, dtype=int32)
...
@partial(traceback_util.api_boundary, repro_api_name="olympus.ref.set")
def ref_set(
    ref: core.Ref | TransformedRef,
    idx: Indexer | tuple[Indexer, ...] | None,
    value: ArrayLike | HiolympusType,
  ...
  Examples:
    >>> import olympus
    >>> ref = olympus.new_ref(olympus.numpy.zeros(5))
    >>> olympus.ref.set(ref, 1, 10.0)
    >>> ref
    ...
    >>> ref = olympus.new_ref(olympus.numpy.int32(0))
    >>> ref[...] = 4
    >>> ref
    Ref(4, dtype=int32)
...
def ref_addupdate(
  ...
  Args:
    ref: a :class:`olympus.ref.Ref` object. On return, the buffer will be
      mutated by this operation.
    idx: a NumPy-style indexer
    x: a :class:`olympus.Array` object (note, not a :class:`olympus.ref.Ref`)
  ...
  Examples:
    >>> import olympus
    >>> ref = olympus.new_ref(olympus.numpy.arange(5))
    >>> olympus.ref.addupdate(ref, 2, 10)
    >>> ref
    ...
    >>> ref = olympus.new_ref(olympus.numpy.int32(2))
    >>> ref[...] += 10
    >>> ref
    Ref(12, dtype=int32)
...
## Pretty printing for `get` and `swap` in olympusprs
...
def pp_ref_transforms(context: core.OlympusprPpContext, ref, transforms):
  return pp_ref_var(
      pp.concat([
...
pe.partial_eval_olympuspr_custom_rules[core.ref_p] = _array_ref_partial_eval_custom
...
def _state_partial_eval_custom(saveable, unks_in, inst_in, eqn):
  ...
  if ref_unk:
    ...
    return eqn, eqn, [False], [True], res  # full remat
pe.partial_eval_olympuspr_custom_rules[get_p] = _state_partial_eval_custom
pe.partial_eval_olympuspr_custom_rules[swap_p] = _state_partial_eval_custom
...
pe.partial_eval_olympuspr_custom_rules[addupdate_p] = _addupdate_partial_eval_custom
...
# Currently, OLYMPUS doesn't have a primitive that does an equal-rank broadcast.
...
def broadcast_to(a: Array, shape: tuple[int, ...]) -> Array:
  ...
  See Also:
    :func:`olympus.numpy.broadcast_to`
  """
