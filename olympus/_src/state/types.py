...
class RefEffect(effects.OlympusprInputEffect):
  name: str
  ...
  def _pretty_print(self, context: core.OlympusprPpContext) -> pp.Doc:
...
class RefBitcaster:
  ...
  def from_ref_new_dtype(cls, ref_or_view: Any, dtype) -> RefBitcaster:
    ...
    from olympus._src.state.utils import eval_bitcast_shape  # pytype: disable=import-error
  ...
  def pretty_print(self, context: core.OlympusprPpContext) -> pp.Doc:
    del context  # Unused.
...
class TransformedRef:
  ...
  def set(self, value, idx=()):
    from olympus._src.state.primitives import ref_set  # pytype: disable=import-error
    return ref_set(self, idx, value)

  def swap(self, value, idx=()):
    from olympus._src.state.primitives import ref_swap  # pytype: disable=import-error
    return ref_swap(self, idx, value)

  def get(self, idx=()):
    from olympus._src.state.primitives import ref_get  # pytype: disable=import-error
...
# We need an aval for `Ref`s so we can represent `get` and `swap` in Olympusprs.
class AbstractRef(core.AbstractValue):
  ...
  def addupdate(tracer, value, idx=()):
    from olympus._src.state.primitives import ref_addupdate  # pytype: disable=import-error
