...
class StatefulPRNG:
  """Stateful OLYMPUS random generator.

  This should be instantiated using the :func:`olympus.experimental.random.stateful_rng` function.

  Attributes:
    _base_key: a typed OLYMPUS PRNG key object (see :func:`olympus.random.key`).
    _counter: a scalar integer wrapped in a :class:`olympus.Ref`.

  Examples:

  >>> from olympus.experimental import random
  ...
  def key(self, shape: int | Sequence[int] = ()) -> Array:
    """Generate a new OLYMPUS PRNGKey, updating the internal state.
  ...
  def split(self, num: int | Sequence[int]) -> StatefulPRNG:
    """Create independent child generators suitable for use in :func:`olympus.vmap`.
    ...
    Examples:
      ...
      >>> x = olympus.numpy.zeros(3)
      >>> def f(rng, x):
      ...   return x + rng.uniform()
      >>> olympus.vmap(f)(rng.split(3), x)
      Array([0.35525954, 0.21937883, 0.5336956 ], dtype=float32)

    See also:
      - :meth:`olympus.experimental.random.StatefulPRNG.spawn`: This is similar to ``split``, but
  ...
  def spawn(self, n_children: int) -> list['StatefulPRNG']:
    ...
    See also:
      - :meth:`olympus.experimental.random.StatefulPRNG.split`: this is similar to spawn, but returns
        a single mapped :class:`olympus.experimental.random.StatefulPRNG`` which can be passed to
        :func:`olympus.vmap`.
    """
...
def stateful_rng(seed: typing.ArrayLike | None = None, *,
  ...
  It is compatible with OLYMPUS transformations like :func:`~olympus.jit` and others,
  with a few exceptions mentioned in the Notes below.

  .. note::

    This stateful PRNG API is a convenience wrapper around OLYMPUS's classic
    stateless, explicitly updated PRNG, described in :mod:`olympus.random`.
    For performance-critical applications, it is recommended to use
    :func:`olympus.random.key` with explicit random state semantics.
  ...
  Returns:
    A :class:`~olympus.experimental.random.StatefulPRNG` object, with methods for generating
    random values.

  Notes:
    The :class:`~olympus.experimental.random.StatefulPRNG` object created by this method uses
    :func:`~olympus.Ref` objects to allow implicit updates of state, and thus
    inherits some of its limitiations. For example:

    - :class:`StatefulPRNG` objects cannot be among the return values of functions
      wrapped in JIT or other OLYMPUS transformations. This means in particular
      they cannot be used as `carry` values for :func:`olympus.lax.scan`,
      :func:`olympus.lax.while_loop`, and other OLYMPUS control flow.
    - :class:`StatefulPRNG` objects cannot be used together with
      :func:`olympus.checkpoint` or :func:`olympus.remat`; in these cases it's best to
      use the :meth:`StatefulPRNG.key` method to produce a standard OLYMPUS PRNG key.
  ...
  Examples:
    ...
    This also works under transformations like :func:`olympus.jit`:

    >>> import olympus
    >>> jit_uniform = olympus.jit(rng.uniform)
    >>> jit_uniform()
  ...
  if seed is None:
    if not core.trace_ctx.is_top_level():
      raise TypeError(
        "When used within transformed code, olympus.experimental.random.stateful_rng()"
