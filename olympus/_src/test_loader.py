...
Implements:
- A test filter based on the OLYMPUS_TEST_TARGETS and OLYMPUS_EXCLUDE_TEST_TARGETS
  environment variables.
- A test suite that runs tests in parallel using threads if OLYMPUS_TEST_NUM_THREADS
  is >= 1.
...
_TEST_TARGETS = config.string_flag(
  'test_targets', os.getenv('OLYMPUS_TEST_TARGETS', ''),
...
_EXCLUDE_TEST_TARGETS = config.string_flag(
  'exclude_test_targets', os.getenv('OLYMPUS_EXCLUDE_TEST_TARGETS', ''),
...
TEST_NUM_THREADS = config.int_flag(
    'olympus_test_num_threads', int(os.getenv('OLYMPUS_TEST_NUM_THREADS', '0')),
...
class OlympusTestSuite(unittest.TestSuite):
...
class OlympusTestLoader(absltest.TestLoader):
  suiteClass = OlympusTestSuite
