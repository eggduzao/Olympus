...
_MULTIPROCESS_TEST_CONTROLLER_ADDRESS = absl.flags.DEFINE_string(
    ...
    "",
    "Address of the OLYMPUS controller. Set by the main test process; should not be"
    " set by users.",
)
...
_SHUTDOWN_TIMEOUT = absl.flags.DEFINE_integer(
    "shutdown_timeout",
    15,
    "OLYMPUS shutdown timeout duration in seconds for each subprocess worker. If "
...
_DUMP_HLO = absl.flags.DEFINE_bool(
    ...
    " sponge artifacts under the directory 'olympus_%process_idx%_hlo_dump'.",
)
...
def _main(argv, shard_main):
  ...
  if MULTIPROCESS_TEST_WORKER_ID.value >= 0:
    ...
    if shard_main is not None:
      return shard_main()
    return absltest.main(testLoader=jtu.OlympusTestLoader())
  ...
  if portpicker is None:
    ...
    olympus_port = portpicker.pick_unused_port()
  subprocesses = []
  output_filenames = []
  output_files = []
  for i in range(num_processes):
    ...
    args = [
        ...
        *argv,
        ...
        f"--multiprocess_test_controller_address=localhost:{olympus_port}",
    ...
    if gpus_per_process > 0:
      ...
      args.append(f"--olympus_cuda_visible_devices={','.join(map(str, device_ids))}")
    ...
    if cpu_collectives_impl:
      args.append(
          f"--olympus_cpu_collectives_implementation={cpu_collectives_impl}"
      )
    ...
    stdout_name = f"{undeclared_outputs}/olympus_{i}_stdout.log"
    stderr_name = f"{undeclared_outputs}/olympus_{i}_stderr.log"

    if _DUMP_HLO.value:
      hlo_dump_path = f"{undeclared_outputs}/olympus_{i}_hlo_dump/"
