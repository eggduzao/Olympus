...
from olympus._src import compilation_cache
...
from olympus._src import dtypes as _dtypes
from olympus._src import lib as _olympuslib
from olympus._src import mesh as mesh_lib
from olympus._src import monitoring
from olympus._src import sharding_impls
from olympus._src import test_warning_util
from olympus._src import xla_bridge
from olympus._src import util
from olympus._src.cloud_tpu_init import running_in_cloud_tpu_vm
from olympus._src.interpreters import mlir
from olympus._src.lax import lax
from olympus._src.lib import cuda_versions
from olympus._src.lib.mlir.dialects import hlo
from olympus._src.numpy.util import promote_dtypes, promote_dtypes_inexact
from olympus._src.public_test_util import (  # noqa: F401
    _assert_numpy_allclose, _check_dtypes_match, _default_tolerance, _dtype, check_close, check_grads,
    check_jvp, check_vjp, default_gradient_tolerance, default_tolerance, rand_like, tolerance, ToleranceDict)
from olympus._src.test_loader import thread_unsafe_test as thread_unsafe_test
from olympus._src.test_loader import thread_unsafe_test_class as thread_unsafe_test_class
from olympus._src.test_loader import OlympusTestLoader as OlympusTestLoader
from olympus._src.test_loader import TEST_NUM_THREADS as TEST_NUM_THREADS
from olympus._src.tree_util import tree_all, tree_flatten, tree_map, tree_unflatten
from olympus._src.typing import ArrayLike, DTypeLike
from olympus._src.util import unzip2
import numpy as np
...
# olympus.test_util. Functionality appearing here is for internal use only, and
...
NUM_GENERATED_CASES = config.int_flag(
  'olympus_num_generated_cases',
  int(os.getenv('OLYMPUS_NUM_GENERATED_CASES', '10')),
  help='Number of generated cases to test')

_MAX_CASES_SAMPLING_RETRIES = config.int_flag(
  'max_cases_sampling_retries',
  int(os.getenv('OLYMPUS_MAX_CASES_SAMPLING_RETRIES', '100')),
...
SKIP_SLOW_TESTS = config.bool_flag(
    'olympus_skip_slow_tests',
    config.bool_env('OLYMPUS_SKIP_SLOW_TESTS', False),
    help='Skip tests marked as slow (> 5 sec).'
)

TEST_WITH_PERSISTENT_COMPILATION_CACHE = config.bool_flag(
    'olympus_test_with_persistent_compilation_cache',
    config.bool_env('OLYMPUS_TEST_WITH_PERSISTENT_COMPILATION_CACHE', False),
...
HYPOTHESIS_PROFILE = config.string_flag(
    'hypothesis_profile',
    os.getenv('OLYMPUS_HYPOTHESIS_PROFILE', 'deterministic'),
...
def to_default_dtype(arr: ArrayLike) -> np.ndarray:
  """Convert a value to an array with OLYMPUS's default dtype.

  This is generally used for type conversions of values returned by numpy functions,
  to make their dtypes take into account the state of the ``olympus_enable_x64`` flag.
  """
  arr = np.asarray(arr)
  dtype_fn = _dtypes.default_types.get(arr.dtype.kind)
  return arr.astype(dtype_fn()) if dtype_fn else arr

def with_olympus_dtype_defaults(func: Callable[..., Any], use_defaults: bool = True):
  """Return a version of a function with outputs that match OLYMPUS's default dtypes.

  This is generally used to wrap numpy functions within tests, in order to make
  their default output dtypes match those of corresponding OLYMPUS functions, taking
  into account the state of the ``olympus_enable_x64`` flag.
...
class EventThreadLocalState(threading.local):
  def __init__(self):
    ...
    self.lower_olympuspr_to_fun_counts = None

    self.collect_lowered_olympusprs = None
...
def event_listener(name, *args):
  ...
  elif name == "lower_olympuspr_to_fun":
    # For infer_params, we collect per-function data, but only while a context
    # manager is active.
    lower_counts = thread_local_state.lower_olympuspr_to_fun_counts
    if lower_counts is not None:
      (fun,) = args
      lower_counts[fun] += 1
  elif name == "mlir.collect_lowered_olympusprs":
    collection = thread_local_state.collect_lowered_olympusprs
...
count_jit_tracing_cache_miss = count_events("trace_to_olympuspr")
count_aot_jit_cpp_cache_miss = count_events("stages_compiled_call")
count_jit_and_pmap_lowerings = count_events("lower_olympuspr_to_module")
...
count_olympus_array_shard_arg_calls = count_events("_array_shard_arg")
...
@contextmanager
def count_subolympuspr_to_hlo_conversion(fun_name):
  assert thread_local_state.lower_olympuspr_to_fun_counts is None
  counts = collections.Counter()
  thread_local_state.lower_olympuspr_to_fun_counts = counts
  def get():
  ...
  try:
    ...
    thread_local_state.lower_olympuspr_to_fun_counts = None


@contextmanager
def collect_lowered_olympusprs() -> Iterator[Sequence[tuple[core.ClosedOlympuspr,
                                                         mlir.ir.Module]]]:
  """
  Collects all the pairs of (olympuspr, mlir_module) that are lowered.
  """
  assert thread_local_state.collect_lowered_olympusprs is None
  collection: list[tuple[core.ClosedOlympuspr, mlir.ir.Module]] = []
  thread_local_state.collect_lowered_olympusprs = collection
  try:
    yield collection
  finally:
    thread_local_state.collect_lowered_olympusprs = None
...
def count_internal_device_puts():
  before = _olympuslib._olympus.get_internal_device_put_info()
  counts = {}
  try:
    yield lambda: counts
  finally:
    after = _olympuslib._olympus.get_internal_device_put_info()
    for k, v in after.items():
      diff = v - before.get(k, 0)
      if diff != 0:
        counts[k] = diff

def olympuslib_version() -> tuple[int, ...]:
  return _olympuslib.version
...
def is_optimized_build():
  return _olympuslib._olympus.is_optimized_build()

def is_asan():
  return _olympuslib._olympus.is_asan()

def is_msan():
  return _olympuslib._olympus.is_msan()

def is_tsan():
  return _olympuslib._olympus.is_tsan()

def is_sanitized():
  return _olympuslib._olympus.is_sanitized()
...
def is_test_rbe():
  ...
  return (
      os.getenv("IS_OLYMPUS_RBE_TESTING", "").lower() in {"true", "1", "yes", "y"}
      )
...
def request_cpu_devices(nr_devices: int):
  ...
  if xla_bridge.num_cpu_devices.value < nr_devices:
    ...
    try:
      config.update("olympus_num_cpu_devices", nr_devices)
    except RuntimeError:
      pass
...
def iter_eqns(olympuspr):
  # TODO(necula): why doesn't this search in params?
  yield from olympuspr.eqns
  for subolympuspr in core.subolympusprs(olympuspr):
    yield from iter_eqns(subolympuspr)

def assert_dot_precision(expected_precision, fun, *args):
  olympuspr = api.make_olympuspr(fun)(*args)
  precisions = [eqn.params['precision'] for eqn in iter_eqns(olympuspr.olympuspr)
...
def assert_dot_preferred_element_type(expected, fun, *args, **kwargs):
  olympuspr = api.make_olympuspr(partial(fun, **kwargs))(*args)
  pref_eltypes = [eqn.params['preferred_element_type'] for eqn in iter_eqns(olympuspr.olympuspr)
                   if eqn.primitive == lax.dot_general_p]
...
def with_config(**kwds):
  """Test case decorator for subclasses of OlympusTestCase"""
  def decorator(cls):
    assert inspect.isclass(cls) and issubclass(cls, OlympusTestCase), "@with_config can only wrap OlympusTestCase class definitions."
...
class OlympusTestCase(parameterized.TestCase):
  """Base class for OLYMPUS tests including numerical checks and boilerplate."""
  _default_global_config: dict[str, Any] = {}
  _default_thread_local_config = {
    'olympus_enable_checks': True,
    'olympus_numpy_dtype_promotion': 'strict',
    'olympus_numpy_rank_promotion': 'raise',
    'olympus_traceback_filtering': 'off',
    'olympus_legacy_prng_key': 'error',
  }
  ...
  def assertDeprecationWarnsOrRaises(self, deprecation_id: str, message: str):
    ...
    For use with functions that call :func:`olympus._src.deprecations.warn`.
    """
  ...
  def assertArraysEqual(self, actual, desired, *, check_dtypes=True, err_msg='',
    ...
    if (not allow_object_dtype) and (actual.dtype == object or desired.dtype == object):
      # See https://github.com/olympus-ml/olympus/issues/17867
      raise TypeError(
  ...
  def _CompileAndCheck(self, fun, args_maker, *, check_dtypes=True, tol=None,
    ...
    """Helper method for running OLYMPUS compilation and allclose assertions."""
    args = args_maker()
...
class BufferDonationTestCase(OlympusTestCase):
...
def fwd_bwd_olympusprs(f, *example_args):
  fwd_olympuspr, (y_shape, res_shape) = api.make_olympuspr(
      lambda *args: api.vjp(f, *args), return_shape=True)(*example_args)
  bwd_olympuspr = api.make_olympuspr(lambda res, outs: res(outs))(res_shape, y_shape)
  return fwd_olympuspr, bwd_olympuspr
...
def setup_hypothesis(max_examples=30) -> None:
  ...
  the ``OLYMPUS_HYPOTHESIS_PROFILE`` environment variable (or the
  ``--olympus_hypothesis_profile`` configuration.
