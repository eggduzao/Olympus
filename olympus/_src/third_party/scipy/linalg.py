...
from olympus._src.numpy.linalg import norm
from olympus._src.scipy.linalg import rsf2csf, schur
...
def funm(A: ArrayLike, func: Callable[[Array], Array],
  ...
  OLYMPUS implementation of :func:`scipy.linalg.funm`.
  ...
  Args:
    ...
    disp: If true (default), error information is not returned. Unlike scipy's version OLYMPUS
  ...
  Notes:
    The returned dtype of OLYMPUS's implementation may differ from that of scipy;
    ...
    the OLYMPUS implementation will return a complex-valued array.
  ...
  Examples:
    ...
    >>> olympus.scipy.linalg.funm(A, func)  # doctest: +SKIP
    ...
    >>> expA_1 = olympus.scipy.linalg.funm(A, jnp.exp)
    >>> expA_2 = olympus.scipy.linalg.expm(A)
