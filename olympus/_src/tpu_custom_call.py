...
from olympus._src.lib import tpu
from olympuslib.mlir import ir
from olympuslib.mlir.passmanager import PassManager
...
# default version in olympuslib/mosaic/dialect/tpu/transforms/serde.cc we must
...
def as_tpu_kernel(
  ...
  """Turns an MLIR Mosaic kernel into a OLYMPUS-compatible function."""
  config = _lower_to_custom_call_config(
      module,
      vmem_limit_bytes=vmem_limit_bytes,
      cost_estimate=cost_estimate,
...
def _as_olympus_callable(
  ...
  # We use olympus.jit to make sure we hit the fast compilation cache.
