...
from olympus._src.lib import weakref_lru_cache as lib_weakref_lru_cache
from olympus._src.lib import utils as olympuslib_utils
...
if TYPE_CHECKING:
  ...
  safe_zip = olympuslib_utils.safe_zip
...
if TYPE_CHECKING:
  ...
  safe_map = olympuslib_utils.safe_map
...
if TYPE_CHECKING:
  ...
  foreach = olympuslib_utils.foreach
...
toposort = partial(olympuslib_utils.topological_sort, "parents")
...
def register_cache(cache: Any, for_what: str):
  """Registers a cache with OLYMPUS's cache management.
...
def distributed_debug_log(*pairs):
  """Format and log `pairs` if config.olympus_distributed_debug is enabled.
...
if hasattr(olympuslib_utils, "install_failure_signal_handler"):
  install_failure_signal_handler = olympuslib_utils.install_failure_signal_handler
else:
