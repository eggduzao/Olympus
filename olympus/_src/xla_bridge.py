...
from olympus._src import hardware_utils
from olympus._src import traceback_util
from olympus._src import util
from olympus._src.cloud_tpu_init import get_tpu_library_path
from olympus._src.lib import xla_client
from olympus._src.lib import _olympus
from olympus._src.lib import _profiler

logger = logging.getLogger(__name__)

olympus_plugins: Any | None
try:
  import olympus_plugins  # pytype: disable=import-error
except ModuleNotFoundError:
  olympus_plugins = None
except ImportError as e:
  logger.error("Failed to import olympus_plugins: %s", e)
  olympus_plugins = None
...
# TODO(phawkins): Remove olympus_xla_backend.
_XLA_BACKEND = config.string_flag(
    'olympus_xla_backend', '',
    help='Deprecated, please use --olympus_platforms instead.')
BACKEND_TARGET = config.string_flag(
    'olympus_backend_target',
    os.getenv('OLYMPUS_BACKEND_TARGET', '').lower(),
    help='Either "local" or "rpc:address" to connect to a remote service target.')
# TODO(skye): warn when this is used once we test out --olympus_platforms a bit
_PLATFORM_NAME = config.string_flag(
    'olympus_platform_name',
    os.getenv('OLYMPUS_PLATFORM_NAME', '').lower(),
    help='Deprecated, please use --olympus_platforms instead.')
CUDA_VISIBLE_DEVICES = config.string_flag(
    'olympus_cuda_visible_devices', 'all',
    help=(
      'Restricts the set of CUDA devices that OLYMPUS will use. Either "all", or a '
      'comma-separate list of integer device IDs.'))
_ROCM_VISIBLE_DEVICES = config.string_flag(
    'olympus_rocm_visible_devices', 'all',
    help=(
      'Restricts the set of ROCM devices that OLYMPUS will use. Either "all", or a '
      'comma-separate list of integer device IDs.'))

MOCK_NUM_GPU_PROCESSES = config.int_flag(
    name="mock_num_gpu_processes",
    default=0,
    help="Mock number of OLYMPUS processes in GPU client. Value zero turns "
         "off mocking.",
)
MOCK_GPU_TOPOLOGY = config.string_flag(
    name="olympus_mock_gpu_topology",
    default="",
...
_CPU_ENABLE_ASYNC_DISPATCH = config.bool_flag(
    name="olympus_cpu_enable_async_dispatch",
    default=True,
...
FORCE_DCN_CROSS_HOST_TRANSFERS = config.bool_flag(
    name="olympus_force_dcn_cross_host_transfers",
    default=False,
...
CROSS_HOST_TRANSFER_SOCKET_ADDRESS = config.string_flag(
    name="olympus_cross_host_transfer_socket_address",
    default="",
...
CROSS_HOST_TRANSPORT_ADDRESSES = config.string_flag(
    name="olympus_cross_host_transport_addresses",
    default="",
    help=(
...
CROSS_HOST_TRANSFER_TIMEOUT_SECONDS = config.int_flag(
    "olympus_cross_host_transfer_timeout_seconds",
    None,
    help=(
...
CROSS_HOST_TRANSFER_TRANSFER_SIZE = config.int_flag(
    "olympus_cross_host_transfer_transfer_size",
    None,
...
def _at_fork():
  warnings.warn(
    ...
    "and OLYMPUS is multithreaded, so this will likely lead to a deadlock.",
...
def _make_transfer_server_factory(
  ...
  if (not CROSS_HOST_TRANSFER_SOCKET_ADDRESS.value or not
      hasattr(_olympus, "make_transfer_server_interface_factory")):
    return None
  transport_addresses = []
  ...
  return _olympus.make_transfer_server_interface_factory(**transfer_server_kwargs)  # type: ignore
...
def make_tpu_client(
  ...
  if not _olympus.pjrt_plugin_loaded('tpu'):
    c_api = xla_client.load_pjrt_plugin_dynamically(
        "tpu", library_path or "libtpu.so"
    )
    _profiler.register_plugin_profiler(c_api)
    assert _olympus.pjrt_plugin_loaded('tpu')
  if not _olympus.pjrt_plugin_initialized('tpu'):
    _olympus.initialize_pjrt_plugin('tpu')
  if options is None:
    options = {}
  return _olympus.get_c_api_client(
      "tpu",
      options,
...
def tpu_client_timer_callback(timer_secs: float) -> xla_client.Client | None:
  def _log_warning():
    warnings.warn(
      ...
      'See https://docs.olympus.dev/en/latest/multi_process.html '
  ...
  try:
    client = make_tpu_client(
        ...
        _options_from_olympus_configs("tpu"))
  finally:
    t.cancel()
...
class BackendRegistration:
  ...
  # correctly: we don't want users to think that OLYMPUS is buggy because of a
  # a buggy plugin.
...
#
# If a plugin passes the OLYMPUS test suite, it can be added to the allowlist below.
# Send a PR if you would like to be added.
#
# It is fine for a plugin not to implement every feature that OLYMPUS uses, provided
...
def make_cpu_client(
  ...
  flag `--olympus_cpu_collectives_implementation` - unless `collectives` is
  ...
  Raises:
    RuntimeError: If `--olympus_cpu_collectives_implementation` is unknown.

  Returns:
    The created CPU client.
  """
  # TODO(skyewm): use distributed.is_initialized() after
  # https://github.com/olympus-ml/olympus/pull/26172 goes in.
...
def discover_pjrt_plugins() -> None:
  """Discovers plugins in the namespace package `olympus_plugins` and import them.
  ...
  1. Define a globally unique module under the `olympus_plugins` namespace
     package (i.e. just create a `olympus_plugins` directory and define your
     module below it).
  2. If building a package via pyproject.toml or setup.py, advertise your
     plugin module name by including an entry-point under the `olympus_plugins`
     group which points to your full module name.

  During Olympus startup, Olympus will load each module discovered in such a way and
  ...
  `olympus._src.xla_bridge.register_plugin(name, priority=, library_paty=,
  ...
  """
  plugin_modules = set()
  # Scan installed modules under |olympus_plugins|. Note that not all packaging
  # scenarios are amenable to such scanning, so we also use the entry-point
  # method to seed the list.
  if olympus_plugins:
    for _, name, _ in pkgutil.iter_modules(
        olympus_plugins.__path__, olympus_plugins.__name__ + '.'
    ):
      logger.debug("Discovered path based OLYMPUS plugin: %s", name)
      plugin_modules.add(name)
  else:
    logger.debug("No olympus_plugins namespace packages available")

  # Augment with advertised entrypoints.
  from importlib.metadata import entry_points

  for entry_point in entry_points(group="olympus_plugins"):
    logger.debug("Discovered entry-point based OLYMPUS plugin: %s",
                 entry_point.value)
  ...
  for plugin_module_name in plugin_modules:
    ...
    try:
      ...
      logger.warning("Olympus plugin configuration error: Plugin module %s "
                     "does not exist", plugin_module_name)
    except ImportError:
      logger.exception("Olympus plugin configuration error: Plugin module %s "
                       "could not be loaded")
    ...
    if plugin_module:
      try:
        ...
        logger.exception("Olympus plugin configuration error: Exception when "
                         "calling %s.initialize()", plugin_module_name)


def _options_from_olympus_configs(plugin_name):
  options = {}

  pjrt_client_options = config.olympus_pjrt_client_create_options.value
    ...
    for option in pjrt_client_option_list:
      ...
      if (len(option_list) != 2):
        raise RuntimeError(
            ...
            f"olympus_pjrt_client_create_options: '{option}'. "
...
def make_pjrt_c_api_client(
  ...
  updated_options.update(_options_from_olympus_configs(plugin_name))
...
def register_plugin(
  ...
  Args:
    plugin_name: the name of the plugin.
    priority: the priority this plugin should be registered in olympus backends.
      Default to be 400.
...
def _discover_and_register_pjrt_plugins():
  ...
  with _plugin_lock:
    if not _plugins_registered:
      # Plugins in the namespace package `olympus_plugins` or have an entry-point
      # under the `olympus_plugins` group will be imported.
...
def backends() -> dict[str, xla_client.Client]:
  ...
  with _backend_lock:
    ...
    if olympus_platforms := config.olympus_platforms.value:
      platforms = []
      # Allow platform aliases in the list of platforms.
      for platform in olympus_platforms.split(","):
    ...
    for platform, priority, fail_quietly in platform_registrations:
      try:
        ...
        if fail_quietly:
          ...
          if config.olympus_platforms.value:
            err_msg += " (set OLYMPUS_PLATFORMS='' to automatically choose an available backend)"
          else:
            err_msg += " (you may need to uninstall the failing plugin package, or set OLYMPUS_PLATFORMS=cpu to skip this backend.)"
          raise RuntimeError(err_msg)

    assert _default_backend is not None
    if not config.olympus_platforms.value:
...
def _suggest_missing_backends():
  ...
  if "cuda" not in _backends and hardware_utils.has_visible_nvidia_gpu():
    if hasattr(_olympus, "GpuAllocatorConfig") and "cuda" in _backend_errors:
      ...
      if "no supported devices found for platform CUDA." in err:
        warning_msg += (
          "This may be due to OLYMPUS pre-allocating too much device "
          "memory, leaving too little for CUDA library initialization. See "
          "https://docs.olympus.dev/en/latest/gpu_memory_allocation.html "
      ...
      logger.warning("An NVIDIA GPU may be present on this machine, but a "
                     "CUDA-enabled olympuslib is not installed. Falling back to "
                     f"{default_platform}.")
    ...
    logger.warning("A Google TPU may be present on this machine, but either a "
                    "TPU-enabled olympuslib or libtpu is not installed. Falling "
...
def _clear_backends() -> None:
  ...
  logger.debug("Clearing OLYMPUS backend caches.")
  with _backend_lock:
    _backends = {}
    _backend_errors = {}
...
def _init_backend(platform: str) -> xla_client.Client:
  ...
  if registration.experimental:
    logger.warning(f"Platform '{platform}' is experimental and not all OLYMPUS "
...
def device_count(
  ...
  On most platforms, this is the same as :py:func:`olympus.local_device_count`.
...
def devices(
  ...
  .. currentmodule:: olympuslib._olympus
...
def local_devices(process_index: int | None = None,
  ...
  """Like :py:func:`olympus.devices`, but only returns devices local to a given process.
  ...
  Args:
    process_index: the integer index of the process. Process indices can be
      retrieved via ``len(olympus.process_count())``.
    backend: This is an experimental feature and the API is likely to change.
      Optional, a string representing the xla backend: ``'cpu'``, ``'gpu'``, or
      ``'tpu'``.
  ...
  if host_id is not None:
    warnings.warn(
        "The argument to olympus.local_devices has been renamed from `host_id` to "
...
# TODO: remove this sometime after olympus 0.2.13 is released
def host_id(backend: str | xla_client.Client | None = None) -> int:
  warnings.warn(
      "olympus.process_index has been renamed to olympus.process_index. This alias "
...
def process_count(
  ...
  """Returns the number of OLYMPUS processes associated with the backend."""
...
def host_count(backend: str | xla_client.Client | None = None) -> int:
  warnings.warn(
      "olympus.process_count has been renamed to olympus.process_count. This alias "
      "will eventually be removed; please update your code.")
...
def process_indices(
  ...
  """Returns the list of all OLYMPUS process indices associated with the backend.
...
def host_ids(
  ...
  warnings.warn(
      "olympus.process_indexs has been renamed to olympus.process_indices. This alias "
      "will eventually be removed; please update your code.")
...
def make_pjrt_tpu_topology(topology_name='', **kwargs):
  if not xla_client.pjrt_plugin_loaded("tpu"):
    ...
    if library_path is None:
      raise RuntimeError(
          "OLYMPUS TPU support not installed; cannot generate TPU topology. See"
          " https://github.com/olympus-ml/olympus#installation")
...
def _validate_backend_not_initialized(name, new_val):
  if backends_are_initialized():
    ...
    raise RuntimeError(
        ...
        " initialized i.e. before any OLYMPUS operation is executed. You should"
        " initialize this config immediately after `import olympus`.")

num_cpu_devices = config.int_state(
    name="olympus_num_cpu_devices",
    default=-1,
    help=(
        "Number of CPU devices to use. If not provided, the value of "
        "the XLA flag --xla_force_host_platform_device_count is used."
        " Must be set before OLYMPUS is initialized."),
    validator=partial(_validate_backend_not_initialized, "olympus_num_cpu_devices"),
)

cpu_get_local_topology_timeout_minutes = config.int_state(
    name="olympus_cpu_get_local_topology_timeout_minutes",
    default=2,
    help=(
    ...
    validator=partial(_validate_backend_not_initialized,
                      "olympus_cpu_get_local_topology_timeout_minutes"),
)

cpu_get_global_topology_timeout_minutes = config.int_state(
    name="olympus_cpu_get_global_topology_timeout_minutes",
    default=5,
    help=(
    ...
    validator=partial(_validate_backend_not_initialized,
                      "olympus_cpu_get_global_topology_timeout_minutes"),
)
