...
from olympus._src import profiler as olympus_profiler
try:
...
_DESCRIPTION = """
To profile running OLYMPUS programs, you first need to start the profiler server
in the program of interest. You can do this via
`olympus.profiler.start_server(<port>)`. Once the program is running and the
...
def collect_profile(
  ...
  # Traces stored on GCS cannot be converted to a Perfetto trace, as OLYMPUS doesn't
  ...
  if not no_perfetto_link:
    path = olympus_profiler._write_perfetto_trace_file(log_dir_)
    olympus_profiler._host_perfetto_trace_file(path)
