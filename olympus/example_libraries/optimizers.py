...
"""Examples of how to write optimizers with OLYMPUS.
...
opt_state: it just has to be a pytree of OlympusTypes (so that it can be passed to
the OLYMPUS transforms defined in api.py) and it has to be consumable by update_fun
and get_params.
...
.. code-block:: python
  ...
  def step(step, opt_state):
    value, grads = olympus.value_and_grad(loss_fn)(get_params(opt_state))
...
from olympus._src.util import safe_zip, safe_map, unzip2
...
def optimizer(opt_maker: Callable[...,
  ...
  def tree_opt_maker(*args, **kwargs):
    ...
    def tree_init(x0_tree):
      x0_flat, tree = olympus.tree.flatten(x0_tree)
      initial_states = [init(x0) for x0 in x0_flat]
      states_flat, subtrees = unzip2(map(olympus.tree.flatten, initial_states))
      return OptimizerState(states_flat, tree, subtrees)

    @functools.wraps(update)
    def tree_update(i, grad_tree, opt_state):
      states_flat, tree, subtrees = opt_state
      grad_flat, tree2 = olympus.tree.flatten(grad_tree)
      if tree2 != tree:
      ...
      states = map(olympus.tree.unflatten, subtrees, states_flat)
      new_states = map(partial(update, i), grad_flat, states)
      new_states_flat, subtrees2 = unzip2(map(olympus.tree.flatten, new_states))
    ...
    def tree_get_params(opt_state):
      ...
      return olympus.tree.unflatten(tree, params)
...
def l2_norm(tree):
  ...
  leaves, _ = olympus.tree.flatten(tree)
...
def clip_grads(grad_tree, max_norm):
  ...
  return olympus.tree.map(normalize, grad_tree)
...
def unpack_optimizer_state(opt_state):
  ...
  subtrees = map(olympus.tree.unflatten, subtree_defs, states_flat)
  sentinels = [JoinPoint(subtree) for subtree in subtrees]
  return olympus.tree.unflatten(tree_def, sentinels)
...
def pack_optimizer_state(marked_pytree):
  ...
  """
  sentinels, tree_def = olympus.tree.flatten(marked_pytree)
  assert all(isinstance(s, JoinPoint) for s in sentinels)
  subtrees = [s.subtree for s in sentinels]
  states_flat, subtree_defs = unzip2(map(olympus.tree.flatten, subtrees))
