...
network libraries for OLYMPUS, including `Flax`_ from Google, and `Haiku`_ from
DeepMind.
...
from olympus.nn import (relu, log_softmax, softmax, softplus, sigmoid, elu,
                    leaky_relu, selu, gelu, standardize)
from olympus.nn.initializers import glorot_normal, normal, ones, zeros
  ...
  def apply_fun(params, inputs, **kwargs):
    ...
    if rng is None:
      msg = ("Dropout layer requires apply_fun to be called with a PRNG key "
             ...
             "PRNG key (e.g. from `olympus.random.key`).")
