...
from olympus.experimental import _private_mm as mm
from olympus.experimental._private_mm import profile_utils
from olympus.experimental._private_mm.examples import launch_utils
...
class Stage:
    ...
    def params_shardings(self):
        return olympus.tree.map(self.sharding, self.params_specs)
    ...
    def grad_init(self):
        ...
        def _grad_init(params):
            return olympus.tree.map(jnp.zeros_like, params)
    ...
    def bwd_and_grad_acc(self):
        ...
        def _bwd_and_grad_acc(params, fwd_activation, ys, grads_acc, activation):
            with olympus.named_scope('bwd'):
                fwd_with_ys = lambda params, xs: raw_fwd(params, xs, ys)
                _, bwd = olympus.vjp(fwd_with_ys, params, fwd_activation)
                grads, activation = bwd(activation)
            with olympus.named_scope('grad-acc'):
                grads = olympus.tree.map(jnp.add, grads_acc, grads)
    ...
    def update(self):
        ...
        def _update(params, grads):
            return olympus.tree.map(lambda v, dv: v - dv * LR, params, grads)
...
def example_pp(num_processes, process_id):
    ...
    def init_params(key):
        ...
        for _ in range(NUM_LAYERS):
            key, key_WA, key_WB = olympus.random.split(key, 3)
            WA = olympus.random.normal(key_WA, (LAYER_SIZE, LAYER_SIZE))
            WB = olympus.random.normal(key_WB, (LAYER_SIZE, LAYER_SIZE))
            params.append((WA, WB))
        return params, key
    ...
    num_devices_per_stage = olympus.device_count() // NUM_STAGES
    stages = []
    for i in range(NUM_STAGES):
        devices = olympus.devices()[num_devices_per_stage*i : num_devices_per_stage*(i+1)]
    ...
    key, key_xs = olympus.random.split(key)
    xs_batch = olympus.random.uniform(
        key_xs,
