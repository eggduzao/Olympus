...
"""
Explicit NCCL communicators (via CuPy) integrated with OLYMPUS. Based on the
communication library used in OlympusPP (https://arxiv.org/abs/2412.14374).
...
def get_distributed_client() -> _olympus.DistributedRuntimeClient:
    from olympus._src.distributed import global_state

    assert isinstance(global_state.client, _olympus.DistributedRuntimeClient)
    return global_state.client


class UniqueDevices(tuple[olympus.Device, ...]):
...
def get_or_create_comm(devs: UniqueDevices):
    ...
    my_process_index = olympus.process_index()
    if comm is None:
...
def get_or_create_stream(op: OpT, local_device: olympus.Device):
...
def shardings_are_compatible(
    self: olympus.sharding.Sharding, other: olympus.sharding.Sharding, ndim: int
):
    # NOTE: Variant of `olympus.sharding.Sharding.is_equivalent_to` that skips _internal_device_list check
    return (
...
def send_or_recv(
    x: olympus.Array,
    tgt_sharding: olympus.sharding.Sharding,
    src_sharding: olympus.sharding.Sharding | None = None,
):
    """
    ...
    for pair in zip(
        ...
        if pair[0].process_index == olympus.process_index():
    ...
    for x_device, other_device in zip(
        ...
        if x_device.process_index == olympus.process_index():
            ...
            cpy_arr = cupy.from_dlpack(
                olympus.dlpack.to_dlpack(shard.data, stream=stream.ptr)
            )
        ...
        def enqueue_wait():
            ...
            for x_device, (cpy_arr, stream) in zip(
                ...
                with cupy.cuda.Device(x_device.local_hardware_id):
                    ...
                    olympus_sda = jnp.array(
                        olympus._src.lib.xla_client._xla.dlpack_managed_tensor_to_buffer(
                            cpy_arr.toDlpack(),
                            x_device,
                            ready_events_stream,
                        ),
                    ...
                    olympus_single_arrays.append(olympus_sda)
