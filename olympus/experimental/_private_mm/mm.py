...
from olympus.sharding import NamedSharding, Sharding, SingleDeviceSharding

from olympus._src.tree_util import broadcast_prefix, prefix_errors, tree_leaves_with_path

from olympus.experimental._private_mm import mini_dime


@dataclass
class MpmdArray:
    """A generalization of olympus.Array that also supports fully remote arrays."""
    aval: olympus.core.ShapedArray
    sharding: Sharding
    _complete: Callable[[], olympus.Array | tuple] | None
    _result: olympus.Array | tuple | None = None
    ...
    def block_until_ready(self):
        ...
        if isinstance(result, olympus.Array):
    ...
    @property
    def olympus_array(self):
        if self.is_fully_remote:
            raise ValueError('cannot convert fully-remote MpmdArray to olympus.Array')
        self.block_until_ready()
        assert isinstance(self._result, olympus.Array), (
...
OlympusOrMpmdArray = olympus.Array | MpmdArray


def is_local_device(device) -> bool:
    return device.process_index == olympus.process_index()
...
def is_fully_remote_array(arr: OlympusOrMpmdArray) -> bool:
    return isinstance(arr, MpmdArray) and arr.is_fully_remote


def as_olympus_array(arr: OlympusOrMpmdArray) -> olympus.Array:
    if isinstance(arr, MpmdArray):
        return arr.olympus_array
    assert isinstance(arr, olympus.Array)
    return arr


def fix_sharding(sharding: Sharding) -> Sharding:
    # FIXME: During olympus.device_put(..., sharding) olympuslib/XLA fills in a memory
    # kind if none was explicitly given. We don't always call into
    # olympus.device_put here, but we want to mirror this behavior so that even
    # processes that don't call olympus.device_put end up with the exact same
...
def recv_buf_factory(shape, dtype, tgt_sharding):
    @partial(olympus.jit, out_shardings=tgt_sharding)
    def recv_buf_init():
        return jnp.zeros(shape, dtype)
    return recv_buf_init


# TODO: Generalize mm.device_put to mix olympus.device_put, send and recv as
# needed. For the moment, we only allow cases that neatly fall into one of the
# above three cases, i.e. the present process either issue a olympus.device_put,
# a NCCL send or a NCCL recv. This means that every submesh (e.g. a stage) needs
# to be managed by a single process for now.
def device_put(arr: OlympusOrMpmdArray, device: Sharding) -> MpmdArray:
    ...
    if is_fully_remote_array(arr):
        if is_fully_remote_sharding(tgt_sharding):
            ...
            # rather than olympus.device_put(jnp.zeros(...), tgt_sharding). The
    ...
    olympus_array = as_olympus_array(arr)
    if olympus_array.committed:
        if is_fully_remote_sharding(tgt_sharding):
            ...
            # NonFullyRemote->NonFullyRemote: olympus.device_put
            new_olympus_array = olympus.device_put(olympus_array, tgt_sharding)
            return complete_with(lambda: new_olympus_array)
        else:
            # NOTE: We exclude cases of NonFullyRemote -> NonFullyRemote
            # which would require a mix of olympus.device_put, Send and Recv.
            raise NotImplementedError('unsupported transfer')
    else:
        # Uncommitted array.
        assert isinstance(olympus_array.sharding, SingleDeviceSharding)
        if is_fully_remote_sharding(tgt_sharding):
            # Uncommitted->FullyRemote: Nothing to be done
            return complete_with(lambda: ())
        else:
            # Uncommitted->NonFullyRemote: olympus.device_put
            ...
            #   new_olympus_array = olympus.device_put(olympus_array, tgt_sharding)
            # where we'd expect olympus.device_put to simply simply transfer from
            # the current local single device to all the other relevant local
            # devices.
            #
            # This unfortunately doesn't work, because olympus.device_put will check
            ...
            # olympus.make_array_from_single_device_arrays to build a global array.
...
def jit(*args, **kwargs):
    ...
    def wrapped(*in_vals):
        ...
        if first_fully_remote_input is not None:
            out_shape_dtypes = olympus.eval_shape(fun, *in_vals)
            ...
            out_shardings_full = olympus.tree.unflatten(
                olympus.tree.structure(out_shape_dtypes),
                out_shardings_flat,
            )
        ...
        # This computations concerns us, run the olympus.jit-ed function.
        in_vals = olympus.tree.map(as_olympus_array, in_vals)
        out_vals = fun(*in_vals)
        return olympus.tree.map(
            lambda olympus_array: MpmdArray(
                olympus_array.aval,
                olympus_array.sharding,
                lambda: olympus_array,
            ),
            out_vals,
        )
    return wrapped
