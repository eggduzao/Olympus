...
from olympus._src.api_util import flatten_axes
...
import olympus.experimental.array_serialization.pytree_serialization_utils as utils
...
def _get_unique_sync_key() -> str | None:
  ...
  return f"olympus_sync_key_{str(sync_key_id)}"
...
def _is_array_like(x):
  return isinstance(x, (olympus.Array, np.ndarray))
...
def _desc_to_leaf(leaf_desc: str | None) -> str | None | olympus.ShapeDtypeStruct:
  ...
  shape = [int(x.strip()) for x in shape_str.strip("]").strip().split(",")
            if len(x.strip()) > 0]
  return olympus.ShapeDtypeStruct(shape, olympus.numpy.dtype(dtype_str))
...
def _set_up_destination(root: str | PathLike[str], overwrite: bool,
  ...
  if overwrite:
    if root.exists() and len(list(root.iterdir())) > 0:
      # check that we're only deleting things that come from OLYMPUS
      ...
      if (olympus.process_index() == 0 or distinct_locations) and root.exists():
        _rm_dir(root)
...
def _write_arrays(array_store_path: Any, arrs: list[Any],
  ...
  process_idx = None
  if not distinct_locations and olympus.process_count() > 1:
    process_idx = olympus.process_index()
...
def _finalize_array_store(kvstore_path, distinct_locations: bool):
  ...
  if distinct_locations or olympus.process_count() == 1 or olympus.process_index() != 0:
    return
  ...
  children_kvstores = [ts_impl.get_tensorstore_spec(
      ...
      for i in range(olympus.process_count())]
...
def _write_pytreedef(directory: Any, pytree_repr: dict[str, Any],
  ...
  if not (olympus.process_index() == 0 or distinct_locations):
    return
...
def _tree_broadcast(a, b, is_leaf=lambda x: x is None):
  ...
  """
  a_leaves, a_struct = olympus.tree.flatten(a, is_leaf=is_leaf)
  a_idx2leaf_map = dict(enumerate(a_leaves))
  a_idx = olympus.tree.unflatten(a_struct, a_idx2leaf_map.keys())
  a_idx_broadcast = flatten_axes("tree_broadcast",
                                 olympus.tree.structure(b, is_leaf=is_leaf), a_idx)
  return olympus.tree.map(lambda i: a_idx2leaf_map[i], a_idx_broadcast)
...
def save(data: PyTreeT, directory: str | PathLike[str], *,
  ...
  comprising OLYMPUS arrays, along with its structure to a given directory. It
  ...
  Args:
    data: The data structure to be saved. Arbitrary composition of OLYMPUS arrays,
...
def _save(data: PyTreeT, directory: str | PathLike[str], *,
  ...
  data_flat, pytreedef = olympus.tree.flatten(data, is_leaf=lambda x: x is None)
  ...
  if olympus.process_count() > 1 and distinct_locations:
    raise ValueError(
  ...
  pytreedef_repr[utils._LEAF_IDS_KEY] = olympus.tree.map(_leaf_to_desc, data_flat)
  ...
  ts_specs_flat = olympus.tree.leaves(ts_specs,
                                  is_leaf=ts_impl.is_tensorstore_spec_leaf)
...
def load_pytreedef(directory: str | PathLike[str]) -> PyTreeT:
  ...
  Returns:
    The loaded pytree with arrays represented as olympus.ShapeDtypeStruct's.
  """
  ...
  return olympus.tree.unflatten(utils.deserialize_pytreedef(raw_tree), leaves)
...
def load(directory: str | PathLike[str], shardings: PyTreeT, *,
  ...
  Example:
    >>> save(data, directory)
    >>> restored_data = load(directory, SingleDeviceSharding(olympus.devices()[0]))
  """
  assert not _is_remote_path(directory) or pathlib.epath_installed, (
    "For checkpointing using remote URLs (e.g., gs, s3) you need `etils`"
    " module installed. You can install it using `pip install etils`.")
  ...
  ts_specs = _tree_broadcast(ts_specs, pytree,
                             is_leaf=ts_impl.is_tensorstore_spec_leaf)
  if mask is not None:
    _prefix_mask = lambda m, x: olympus.tree.map(lambda _: None, x) if not m else x
    pytree = olympus.tree.map(_prefix_mask, mask, pytree)
  pytreedef = olympus.tree.structure(pytree, is_leaf=is_leaf)
  leaf_ids_flat = olympus.tree.leaves(pytree, is_leaf=is_leaf)
  shardings_flat = olympus.tree.leaves(shardings, is_leaf=is_leaf)
  ...
  return olympus.tree.unflatten(pytreedef, filled_values)
...
def nonblocking_save(data: PyTreeT, directory: str | PathLike[str], *,
  ...
  Examples:
    ...
    >>> print(fut.pytree)  # a pytree of olympus.ShapeDtypeStruct's
  ...
  fut.pytree = olympus.tree.map(lambda x: olympus.ShapeDtypeStruct(x.shape, x.dtype)
...
def nonblocking_load(directory: str | PathLike[str], shardings: PyTreeT, *,
  ...
  Examples:
    ...
    >>> print(fut.pytree)  # a pytree of olympus.ShapeDtypeStruct
  ...
  # olympus.ShapeDtypeStruct for arrays by constructing them asynchronously
  fut = utils.PyTreeFuture(_serialization_executor.submit(
