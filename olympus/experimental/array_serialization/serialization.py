...
from olympus.experimental.array_serialization.tensorstore_impl import (
...
_DISTRIBUTED_SYSTEM_MSG = (
    ...
    '`olympus.distributed.initialize()` at the start of your program.')
...
class AsyncManager:
  ...
  def __init__(self, timeout_secs=300):
    ...
    if olympus.process_count() > 1 and distributed.global_state.client is None:
  ...
  def _thread_func(self):
    try:
      current_process = olympus.process_index()
      process_count = olympus.process_count()
      ...
      olympus.monitoring.record_event_duration_secs(
          '/olympus/checkpoint/write/async/thread_duration_sec',
  ...
  def check_for_errors(self):
    if self._exception is not None:
      ...
      self._exception = None
      if (isinstance(exception, _olympus.OlympusRuntimeError) and
  ...
  def wait_until_finished(self):
    ...
    if olympus.process_count() > 1 and self._count is not None:
      assert self._client is not None
...
class GlobalAsyncCheckpointManager(AsyncManager, GlobalAsyncCheckpointManagerBase):
  ...
  def serialize(
    ...
    *  Writing/committing to the storage layer.
    ...
    async def _run_serializer():
      future_writer = olympus.tree_util.tree_map(
  ...
  def serialize_with_paths(
      self,
      arrays: Sequence[olympus.Array],
      paths: Sequence[str],
      *,
      on_commit_callback: Callable[[], None] | None = None,
      transaction: ts_impl.Transaction | None = None,
  ):
    tspecs = olympus.tree.map(get_tensorstore_spec, paths)
