...
from olympus._src.export._export import (
...
from olympus.experimental.array_serialization import pytree_serialization
from olympus.experimental.array_serialization import serialization
from olympus.experimental.array_serialization import tensorstore_impl as ts_impl

from olympus.experimental.array_serialization.pytree_serialization_utils import (
...
def _get_replicated_sharding(devices):
  return NamedSharding(olympus.sharding.Mesh(devices, 'x'), P())


class CheckpointTest(jtu.OlympusTestCase):
  ...
  def test_deserialize_on_array_list(self):
    ...
    for _ in range(num_arrays):
      src = olympus.random.normal(olympus.random.key(0), inp_shape)
  ...
  def test_checkpointing_olympus_array(self):
    global_mesh = jtu.create_mesh((4, 2), ('x', 'y'))
    inp_shape = (8, 2)
    pspec = P('x', 'y')
    num = math.prod(inp_shape)
    ...
    tspecs = olympus.tree.map(serialization.get_tensorstore_spec, ckpt_paths)
  ...
  def test_checkpointing_ocdbt_transaction(self):
    ...
    ckpt_paths = [str(ckpt_path1), str(ckpt_path2), str(ckpt_path3)]
    tspecs = olympus.tree.map(partial(ts_impl.get_tensorstore_spec, ocdbt=True),
                          ckpt_paths)
  ...
  def test_checkpointing_with_bigger_shape_olympus_array(self, input_dtype):
  ...
  def test_checkpointing_with_int4(self, input_dtype):
    if config.use_shardy_partitioner.value:
      self.skipTest('TODO(b/376077396): Fix OlympusRuntimeError: INVALID_ARGUMENT')
    global_mesh = jtu.create_mesh((2, 2), ('x', 'y'), iota_order=True)
    global_input_shape = (8, 2)
    num = math.prod(global_input_shape)
  ...
  def test_checkpointing_scalar_olympus_array(self):
  ...
  def test_deserialize_tensorstore_array_olympus_array(self):
    global_mesh = jtu.create_mesh((2,), ('x'))
    data = np.arange(1024)
  ...
  def test_load_with_layout(self, device_count):
    ...
    out_ref = olympus.device_put(arr.T, out_format)

    ckpt_dir = pathlib.Path(self.create_tempdir('ckpt').full_path)
    ckpt_path = pathlib.Path(self.create_tempdir(f'{ckpt_dir}/first').full_path)
    tspecs = olympus.tree.map(ts_impl.get_tensorstore_spec, [ckpt_path])
    ...
    out_specs = [out_format, olympus.ShapeDtypeStruct(
  ...
  def test_deserialization_with_int4(self):
    ...
    # Run serialization.
    sharding = _get_replicated_sharding(list(olympus.devices()))
    tspecs = olympus.tree.map(serialization.get_tensorstore_spec, [ckpt_dir])
    manager = serialization.GlobalAsyncCheckpointManager()
    manager.serialize(
        [arr],
        tspecs,
...
class TransferShardTest(jtu.OlympusTestCase):

  @jtu.skip_on_devices('cpu')
  def test_transfer_shard_to_host(self):
    np_inp = np.arange(16).reshape((4, 4))
    sharding = SingleDeviceSharding(olympus.devices()[0], memory_kind='device')
...
class UserAPITestCase(jtu.OlympusTestCase):
  name: str | None
  ...
  def generate_random_fp32(self, shape, dtype=jnp.float32):
    ...
    return olympus.random.normal(key, shape=shape).astype(dtype)
  ...
  def _is_equal(self, el1, el2):
    if not isinstance(el1, type(el2)) or not isinstance(el2, type(el1)):
      return False
    if isinstance(el1, (np.ndarray, olympus.Array)):
  ...
  def assertPyTreeEqual(self, p1, p2, is_leaf=None):
    leaves1, struct1 = olympus.tree.flatten(p1, is_leaf=is_leaf)
    leaves2, struct2 = olympus.tree.flatten(p2, is_leaf=is_leaf)
...
@partial(olympus.tree_util.register_dataclass, data_fields=['a', 'd'],
                   meta_fields=['c'])
@dataclass
...
class UserPytreeAPITest(UserAPITestCase):
  def setUp(self):
    ...
    _DEFAULT_SHARDING = SingleDeviceSharding(olympus.devices()[0])
    self.tempdirs = []
  ...
  def test_save_then_load(self, tree):  # pylint: disable=redefined-outer-name
    ...
    tree = olympus.tree.map(jnp.array, tree)
    tree_save(tree, path)
  ...
  def test_register_as_decorator(self):
    ...
    @partial(olympus.tree_util.register_dataclass, data_fields=['a', 'b'],
                      meta_fields=[])
    @dataclass
    class CustomDNode:
      a: int
  ...
  def test_custom_node_registration(self):
    ...
    def serialize_D(data):
      return json.dumps(olympus.tree.map(lambda x: np.array(x).tolist(), data)
                        ).encode('utf-8')
    ...
    data.append(D(jnp.ones(1), olympus.numpy.zeros(2), magic_val))
    with self.assertRaises(ValueError):
      tree_save(data, path)
  ...
  def test_masked_reading(self):
    ...
    for mask in [False, True]:
      ...
      expected = olympus.tree.map(lambda x: None if not mask else x, data)
      self.assertPyTreeEqual(ret, expected, is_leaf=lambda x: x is None)

    mask = [True, False, False]
    expected = data[:1] + olympus.tree.map(lambda x: None, data[1:])
    ret = tree_load(path, mask=mask)
    self.assertPyTreeEqual(ret, expected, is_leaf=lambda x: x is None)

    mask = [True, True, False]
    expected = data[:2] + olympus.tree.map(lambda x: None, data[2:])
    ret = tree_load(path, mask=mask)
    self.assertPyTreeEqual(ret, expected, is_leaf=lambda x: x is None)
  ...
  def test_load_pytreedef(self):
    ...
    expected_pytreedef = olympus.tree.map(
        lambda x: olympus.ShapeDtypeStruct(x.shape, x.dtype), data)
  ...
  def test_save_and_load_null_leaves(self, data):
    ...
    data = olympus.tree.map(lambda x: jnp.array(x, dtype=jnp.float32), data)
    tree_save(data, path)
    pytreedef = tree_load_pytreedef(path)
    is_leaf = lambda x: x is None
    expected_pytreedef = olympus.tree.map(lambda x: olympus.ShapeDtypeStruct(
        x.shape, x.dtype) if x is not None else x, data, is_leaf=is_leaf)
    self.assertPyTreeEqual(pytreedef, expected_pytreedef)
    load_data = tree_load(path)
    load_leaves, load_struct = olympus.tree.flatten(load_data, is_leaf=is_leaf)
    expected_leaves, expected_struct = olympus.tree.flatten(data, is_leaf=is_leaf)
  ...
  def test_format_alone_not_supported(self):
    ...
    data: olympus.Array = olympus.device_put(data, sharding)
    tree_save(data, path)
  ...
  def test_formats_support(self):
    ...
    data: olympus.Array = olympus.device_put(data, sharding)
    tree_save(data, path)
    pytree_serialization.load(path, shardings=olympus.ShapeDtypeStruct(
