...
def _get_tensorstore_metadata(arr, is_remote: bool = False,
  ...
  if isinstance(arr, olympus.Array):
...
def verify_tensorstore_spec(spec: dict[str, Any], arr: olympus.Array | None,
...
def get_tensorstore_spec(
    ...
    process_idx: int | None = None, arr: olympus.Array | None = None,
    driver: str = _TS_ARRAY_DRIVER) -> dict[str, Any]:
...
async def _create_async_array_from_callback(
    ...
    inp_sharding: olympus.sharding.Sharding,
    data_callback: Callable[[array.Index, olympus.Device], Awaitable[olympus.Array]],
):
...
async def _transfer_shard_to_host(shard: array.Shard) -> np.ndarray:
  ...
  if has_pinned_host:
    ...
    sharding = olympus.sharding.SingleDeviceSharding(shard.device,
        memory_kind="pinned_host")
    data = olympus.device_put(data, sharding)
  else:
    data.copy_to_host_async()
  # Allow other transfers to be scheduled simultaneously
  await asyncio.sleep(0)
  # Ensure that olympus.Array's internal numpy array can be zero-copied. Tensorstore
...
async def async_serialize(
  ...
  """
  if (isinstance(arr_inp, array.ArrayImpl) and olympus.process_count() > 1 and
  ...
  if primary_host is None or olympus.process_index() == primary_host:
    open_future = ts.open(
  ...
  async def _write_array(shard):
    if shard.replica_id == replica_id:
      ...
      write_future = t[shard.index].write(
          ...
          # cache.  If `arr_inp` is a olympus.Array, the result of converting
  ...
  future_write_state = olympus.tree_util.tree_map(_write_array, local_shards)
...
def _run_serialization(arrays, tensorstore_specs):
  ...
  async def _run_serializer():
    future_writer = olympus.tree_util.tree_map(async_serialize, arrays, tensorstore_specs)
...
async def async_deserialize(
    in_type: olympus.sharding.Sharding | Format | olympus.ShapeDtypeStruct,
  ...
  elif isinstance(in_type, olympus.ShapeDtypeStruct):
    dtype = in_type.dtype if dtype is None else dtype
    in_sharding = in_type.sharding
    layout = in_type.format.layout
  else:
    if not isinstance(in_type, olympus.sharding.Sharding):
      raise TypeError(
          'sharding passed to deserialization should be specified, concrete and'
          f' an instance of `olympus.sharding.Sharding`. Got {in_type}')
    in_sharding = in_type
    layout = None
  assert isinstance(in_sharding, olympus.sharding.Sharding)
  t = await ts.open(
      tensorstore_spec,
      open=True,
  ...
  async def cb(index: array.Index, device: olympus.Device):
    ...
    result = olympus.device_put(
        out, Format(layout, olympus.sharding.SingleDeviceSharding(device)))
    if byte_limiter is not None:
  ...
  # for deserialization canonicalize dtype to a dtype representable in olympus
  return await _create_async_array_from_callback(
      tuple(shape), olympus.dtypes.canonicalize_dtype(dtype), in_sharding, cb)


# TODO(rdyro): Remove this function.
def _run_deserialization(shardings: Sequence[olympus.sharding.Sharding | Format],
  ...
  async def _run_deserializer():
    ...
    future_arrays = olympus.tree_util.tree_map(
