...
from olympus.experimental.colocated_python.func import make_callable
from olympus.experimental.colocated_python.obj import wrap_class
import numpy as np


@overload
def colocated_cpu_devices(
    devices_or_mesh: Sequence[olympus.Device],
) -> Sequence[olympus.Device]:
  ...


@overload
def colocated_cpu_devices(
    devices_or_mesh: olympus.sharding.Mesh,
...
def colocated_cpu_devices(devices_or_mesh):
  ...
  """
  if isinstance(devices_or_mesh, olympus.sharding.Mesh):
...
def _colocated_cpu_devices_cached_fallback_to_cpu_backend(
  ...
  if devices[0].device_kind == "cpu":
    ...
    # CPU backend.
    cpu_backend_devices = olympus.devices(backend="cpu")
  device_index_map = {device.id: i for i, device in enumerate(olympus.devices())}
...
@util.cache(max_size=1024, trace_context_in_key=False)
def _colocated_cpu_mesh_cached(mesh: olympus.sharding.Mesh) -> olympus.sharding.Mesh:
...
def colocated_python(fun: Callable[..., Any]):
  ...
  docstrings for details and https://docs.olympus.dev/en/latest/notebooks/colocated-python.html
  for examples.
...
def colocated_python_class(cls: type[object]) -> type[object]:
  ...
  on OLYMPUS. An actual object will be instantiated on the host of the devices of
