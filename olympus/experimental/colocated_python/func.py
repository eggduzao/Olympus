...
from olympus._src.util import wraps
from olympus.experimental.colocated_python import func_backend
from olympus.experimental.colocated_python.serialization import _deserialize, _deserialize_specs, _make_specs_for_serialized_specs, _serialize, _serialize_specs
from olympus.extend.backend import register_backend_cache as olympus_register_backend_cache
...
class Specialization:
  ...
  def update(
      ...
      *,
      ...
      out_specs_treedef: tree_util.PyTreeDef | None = None,
      out_specs_leaves: tuple[api.ShapeDtypeStruct, ...] | None = None,
      devices: Sequence[olympus.Device] | xc.DeviceList | None = None,
  ):
...
def _get_spec(x: Any) -> api.ShapeDtypeStruct:
  """Extracts a spec for a value, which must be a OLYMPUS Array."""
  # TODO(hyeontaek): Allow Python values and automatically apply `shard_arg`
  # with a suitable sharding and layout.
  if not isinstance(x, olympus.Array):
    raise ValueError(
        "colocated_python only supports olympus.Array as input and output, but got"
        f" {type(x)}."
    )
...
def _compile_to_executable(
  ...
  out_sdss = tuple(
      olympus.core.ShapedArray(sds.shape, sds.dtype) for sds in out_specs_leaves
  )
  ...
  try:
    ...
    # TODO(hyeontaek): Implement colocated Python support in McOLYMPUS and remove
    # this fallback path.
...
def _make_output_specs_and_push_result_fun(
  ...
  def lowered_fun(*args, **kwargs) -> olympus.Array:
...
class _SpecializedCollection:
  ...
  class WeakSpec:
    ...
    dtypes: tuple[olympus.numpy.dtype, ...]
    shapes: tuple[tuple[int, ...], ...]
    sharding_ids: tuple[int, ...]
    treedef: tree_util.PyTreeDef

    def __init__(
        self, args_leaves: Sequence[olympus.Array], treedef: tree_util.PyTreeDef
    ):
  ...
  class StrongSpec:
    ...
    def __init__(
        self, args_leaves: Sequence[olympus.Array], pytreedef: tree_util.PyTreeDef
    ):
  ...
  def get(
      self,
      args_leaves: Sequence[olympus.Array],
...
class _OlympusSecondLevelCaches:
  """Manages second-level caches registered as a single cache with OLYMPUS."""

  def __init__(self, name: str):
    self._lock = threading.Lock()
    self._callbacks: dict[int, Callable[..., Any]] = {}
    olympus_register_backend_cache(self, name)

  def cache_clear(self):
    """Meant to be invoked by OLYMPUS internals."""
...
class _CachedColocatedFunctionMaker:
  ...
  OLYMPUS_CACHE = _OlympusSecondLevelCaches("colocated_python_specialized_func_cache")
  ...
  def __init__(self, held_by: int | None):
    ...
    _CachedColocatedFunctionMaker.OLYMPUS_CACHE.register_second_level(
        self.held_by,
        clear_caches,
    )
  ...
  def __del__(self):
    ...
    try:
      _CachedColocatedFunctionMaker.OLYMPUS_CACHE.remove_second_level(self.held_by)
  ...
  def _make_callable(
    ...
    def specialize(
        ...
        out_specs_fn: Callable[..., ShapeDtypeStructTree] | None = None,
        devices: Sequence[olympus.Device] | None = None,
    ):
