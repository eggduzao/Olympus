...
from olympus.experimental.colocated_python import func
from olympus.experimental.colocated_python import obj_backend
...
_USE_WEAKREFS = config.bool_state(
    'olympus_experimental_colocated_python_object_use_weakrefs_at_backend',
    False,
    help=(
...
class _InstanceRegistry:
  ...
  def __init__(self) -> None:
    ...
    self._storage: dict[int, set[olympus.Device]] = {}
  ...
  def update_devices(self, uid: int, device_set: set[olympus.Device]) -> None:
    """Updates the set of devices on which it is live."""
    with self._lock:
      self._storage[uid] |= device_set

  def pop_instance(self, uid: int) -> set[olympus.Device]:
...
@olympus._src.util.cache(max_size=4096)
def _update_instance_devices(
    uid: int, shardings: tuple[olympus.sharding.Sharding, ...]
) -> None:
