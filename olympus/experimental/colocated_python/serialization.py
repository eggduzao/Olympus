...
@olympus._src.util.cache(max_size=None)
def _get_cpu_device_map() -> dict[int, olympus.Device]:
  """Returns a map from a device id to a matching device."""
  cpu_device_map: dict[int, olympus.Device] = {}
...
def _lookup_cpu_device(
    cpu_device_map: dict[int, olympus.Device], device_id: int
) -> olympus.Device:
...
) -> tuple[Callable[..., olympus.sharding.Mesh], Any]:
...
) -> olympus.sharding.Mesh:
  cpu_device_map = _get_cpu_device_map()
  mesh_devices = np.vectorize(
      functools.partial(_lookup_cpu_device, cpu_device_map)
  )(mesh_device_ids)
  return olympus.sharding.Mesh(mesh_devices, axis_names, axis_types)


@_make_reduce_func_with_common_obj
def _reduce_named_sharding(
    sharding: olympus.sharding.NamedSharding,
) -> tuple[Callable[..., olympus.sharding.NamedSharding], Any]:
  assert isinstance(sharding.mesh, olympus.sharding.Mesh), "Only Mesh is supported"
...
def _unreduce_named_sharding(reduced_mesh, spec, memory_kind):
  ...
  return olympus.NamedSharding(mesh, spec, memory_kind=memory_kind)
...
def _reduce_single_device_sharding(
    sharding: olympus.sharding.SingleDeviceSharding,
) -> tuple[Callable[..., olympus.sharding.SingleDeviceSharding], Any]:
...
) -> olympus.sharding.SingleDeviceSharding:
  cpu_device_map = _get_cpu_device_map()
  device = _lookup_cpu_device(cpu_device_map, device_id)
  return olympus.sharding.SingleDeviceSharding(device, memory_kind=memory_kind)
...
def _serialize(obj: Any) -> bytes:
  ...
  class _CustomPickler(cloudpickle.Pickler):
    dispatch_table = collections.ChainMap(
        {olympus.sharding.Mesh: _reduce_mesh},
        {olympus.sharding.NamedSharding: _reduce_named_sharding},
        {DeviceList: _reduce_device_list},
        {olympus.sharding.SingleDeviceSharding: _reduce_single_device_sharding},
...
def _make_specs_for_serialized_specs(
  ...
  mesh = olympus.sharding.Mesh(tuple(devices), ("x",))
  replicated_sharding = olympus.sharding.NamedSharding(
      mesh, olympus.sharding.PartitionSpec()
  )
...
def _serialize_specs(
  ...
  """Serializes the output specs into a olympus.Array of string type.
  ...
  # TODO(jmudigonda): Revisit this when OLYMPUS supports HLO sharding for making
  # olympus.Array via make_array_from_single_device_arrays. We should then use a
  ...
  mesh = olympus.sharding.Mesh(tuple(addressable_devices), ("x",))
  replicated_sharding = olympus.sharding.NamedSharding(
      mesh, olympus.sharding.PartitionSpec()
  )

  out_arrays = [
      olympus.device_put(s_np_array, device) for device in addressable_devices
  ]
...
def _deserialize_specs(
    serialized_specs: olympus.Array,
