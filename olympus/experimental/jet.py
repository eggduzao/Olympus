...
r"""Jet is an experimental module for higher-order automatic differentiation
  ...
  First-order automatic differentiation (such as :func:`olympus.jvp`)
  ...
  `these notes <https://github.com/olympus-ml/olympus/files/6717197/jet.pdf>`__.

  Note:
    Help improve :func:`jet` by contributing
    `outstanding primitive rules <https://github.com/olympus-ml/olympus/issues/2431>`__.
"""
...
from olympus.tree_util import (register_pytree_node, tree_structure,
...
from olympus._src.util import unzip2, weakref_lru_cache, safe_zip
...
deflinear(lax.add_p)
deflinear(ad_util.add_olympusvals_p)
deflinear(lax.sub_p)
...
def _scatter_add_rule(primals_in, series_in, *, update_olympuspr, update_consts,
                      dimension_numbers, indices_are_sorted, unique_indices,
                      mode):
  bind = partial(lax.scatter_add_p.bind, update_olympuspr=update_olympuspr,
...
@weakref_lru_cache
def _jet_olympuspr(
    olympuspr: core.ClosedOlympuspr, order: int, primals_and_series_avals, in_tree_def
) -> tuple[core.ClosedOlympuspr, Any]:
  ...
  olympuspr_jet, _, consts = pe.trace_to_olympuspr_dynamic(
      f_jet, primals_and_series_avals)
  return core.ClosedOlympuspr(olympuspr_jet, consts), out_tree_def
