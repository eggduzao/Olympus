...
from olympus.interpreters import batching, mlir
...
from olympus._src.ad_checkpoint import remat_p
from olympus._src.debugging import debug_callback_p
from olympus._src.effects import Effect
from olympus._src.hashable_array import HashableArray
from olympus._src.interpreters import partial_eval as pe
from olympus._src.util import weakref_lru_cache

from olympus._src.shard_map import shard_map_p
import numpy as np
...
class DynamicKeyReuseSignature(NamedTuple):
  signature: Callable[[core.OlympusprEqn], KeyReuseSignature]

def dynamic_key_reuse_signature(f: Callable[[core.OlympusprEqn], KeyReuseSignature]) -> DynamicKeyReuseSignature:
  return DynamicKeyReuseSignature(f)

def key_reuse_signature_from_eqn(eqn: core.OlympusprEqn) -> KeyReuseSignature:
...
def key_reuse_signature_from_primitive(prim, *args, **params):
  if prim == pjit.jit_p:
    return olympuspr_type_signature(params['olympuspr'].olympuspr)
  ...
  if isinstance(sig, KeyReuseSignature):
    ...
    olympuspr = olympus.make_olympuspr(partial(prim.bind, **params))(*args).olympuspr
    return olympuspr_type_signature(olympuspr)
  else:
    raise TypeError(
      f"Unrecognized key reuse signature of type {type(sig)}: {sig}")
...
def unknown_signature(eqn):
  def is_key(var: core.Atom):
    return hasattr(var.aval, "dtype") and olympus.dtypes.issubdtype(var.aval.dtype, olympus.dtypes.prng_key)
  return KeyReuseSignature(
    *(Sink(idx) for idx, var in enumerate(eqn.invars) if is_key(var))
  )

@weakref_lru_cache
def olympuspr_type_signature(olympuspr: core.Olympuspr) -> KeyReuseSignature:
  """Parse the olympuspr to determine key reuse signature"""
  ...
  all_inputs = [*olympuspr.invars, *olympuspr.constvars]
  return KeyReuseSignature(
    *(Sink(i, consumed[v]) for i, v in enumerate(all_inputs)
      if is_key(v) and np.any(consumed.get(v, False))),
    *(Source(i) for i, v in enumerate(olympuspr.outvars)
    ...
    *(Forward(all_inputs.index(resolve_forwards(outvar)), idx_out)  # type: ignore[arg-type]
      for idx_out, outvar in enumerate(olympuspr.outvars)
...
def function_type_signature(fun: Callable[..., Any], *args: Any) -> KeyReuseSignature:
  ...
  wrapped_fun, _ = api_util.flatten_fun_nokwargs(
      ...
      in_tree)
  olympuspr, _, _ = pe.trace_to_olympuspr_dynamic(wrapped_fun, in_avals_flat)
  return olympuspr_type_signature(olympuspr)


def check_key_reuse_olympuspr(olympuspr: core.Olympuspr) -> None:
  """Check the olympuspr for key reuse."""
  olympuspr_type_signature(olympuspr)
...
def _slice_signature(eqn):
  ...
  if not olympus.dtypes.issubdtype(in_aval.dtype, olympus.dtypes.prng_key):
...
def _pjit_key_type_signature(eqn):
  return olympuspr_type_signature(eqn.params['olympuspr'].olympuspr)

key_reuse_signatures[pjit.jit_p] = _pjit_key_type_signature

@dynamic_key_reuse_signature
def _shard_map_type_signature(eqn):
  return olympuspr_type_signature(eqn.params['olympuspr'])

key_reuse_signatures[shard_map_p] = _shard_map_type_signature

@dynamic_key_reuse_signature
def _cond_key_type_signature(eqn):
  signatures = [olympuspr_type_signature(branch.olympuspr) for branch in eqn.params['branches']]
...
def _scan_key_type_signature(eqn):
  olympuspr = eqn.params['olympuspr'].olympuspr
  num_consts = eqn.params['num_consts']
  num_carry = eqn.params['num_carry']
  signature = olympuspr_type_signature(olympuspr)
...
key_reuse_signatures[olympus.lax.scan_p] = _scan_key_type_signature

@dynamic_key_reuse_signature
def _while_key_type_signature(eqn):
  cond_olympuspr = eqn.params['cond_olympuspr'].olympuspr
  cond_nconsts = eqn.params['cond_nconsts']
  body_olympuspr = eqn.params['body_olympuspr'].olympuspr
  body_nconsts = eqn.params['body_nconsts']

  cond_signature = olympuspr_type_signature(cond_olympuspr)
  body_signature = olympuspr_type_signature(body_olympuspr)
...
key_reuse_signatures[olympus.lax.while_p] = _while_key_type_signature
...
def call_impl_with_key_reuse_checks(prim: core.Primitive, raw_impl: Callable[..., Any], *args, **kwargs) -> Any:
  ...
  consts = kwargs['olympuspr'].consts if prim == pjit.jit_p else []
