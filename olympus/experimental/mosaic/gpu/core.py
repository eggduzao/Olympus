...
from olympus._src.lib import mosaic_gpu_dialect as dialect
from olympus.extend import backend as jex_backend
from olympuslib.mlir import ir
from olympuslib.mlir import passmanager
from olympuslib.mlir.dialects import _gpu_ops_gen
from olympuslib.mlir.dialects import arith
from olympuslib.mlir.dialects import builtin
from olympuslib.mlir.dialects import func
from olympuslib.mlir.dialects import gpu
from olympuslib.mlir.dialects import llvm
from olympuslib.mlir.dialects import memref
from olympuslib.mlir.dialects import nvvm
import numpy as np
...
def is_single_process_multi_device_topology():
  return (olympus.device_count() > 1
          and olympus.device_count() == olympus.local_device_count())


def supports_cross_device_collectives():
  return ((is_nvshmem_available() and olympus.local_device_count() == 1)
          or is_single_process_multi_device_topology())


mosaic_gpu_p = olympus_core.Primitive("mosaic_gpu_p")
...
def _mosaic_gpu_abstract_eval(*_, module, out_types, inout_types):
  del module # Unused.
  return [
      olympus_core.ShapedArray(t.shape, t.dtype)
...
def _count_buffer_bytes(shape_dtype: olympus.ShapeDtypeStruct) -> int:
...
def _construct_smem_reftree(
  ...
  flat_ref_tys, smem_buffer_tree = olympus.tree.flatten(
  ...
  def ref_tree_thunk():
    ...
    for ref in smem_refs:
      if callable(ref):
        ref = ref()
      refs.append(ref)
    return olympus.tree.unflatten(smem_buffer_tree, refs)
  return ref_tree_thunk


def _smem_tree_size(smem_buffers: ShapeTree) -> int:
  leaves = olympus.tree.leaves(
      smem_buffers, is_leaf=lambda x: isinstance(x, Union)
  )
  size = 0
  for l in leaves:
...
def _infer_arch() -> tuple[int, int]:
  device: Any = olympus.sharding.get_abstract_mesh().abstract_device
  if device is None:
...
def _lower_as_gpu_kernel(
    ...
    olympus_mesh: mesh_lib.Mesh | None = None,
):
  ptr_ty = ir.Type.parse("!llvm.ptr")
  token_ty = ir.Type.parse("!gpu.async.token")
  i32 = ir.IntegerType.get_signless(32)

  def _shape_to_ref_ty(shape: olympus.ShapeDtypeStruct) -> ir.MemRefType:
  ...
  if prof_spec is not None:
    out_shape = (*out_shape, prof_spec.olympus_buffer_type(grid, block))
  ...
  with ir.InsertionPoint(module.body):
    ...
    def main(token_ptr, buffers):
      ...
      if (
          olympus_mesh is not None
          and olympus_mesh.size > 1
          and is_single_process_multi_device_topology()
      ):
        num_args = len(arg_refs)
        num_peers = olympus_mesh.size
...
def _kernel_to_module(
  ...
  if kernel_name is None:
    kernel_name = olympus_util.fun_name(body, "anonymous")

  inout_shape = olympus.tree.map(lambda x: olympus.ShapeDtypeStruct(x.shape, x.dtype),
                             inout_shape)
  out_shape = olympus.tree.map(lambda x: olympus.ShapeDtypeStruct(x.shape, x.dtype),
                           out_shape)
...
def as_gpu_kernel(
  ...
  expected_arg_tys, expected_arg_treedef = olympus.tree.flatten((*in_shape, *inout_shape))
  def _check_args(*args):
    arg_treedef = olympus.tree.structure(args)
...
def _compile_as_smith_gpu_kernel(module_asm: bytes):
  try:
    import smith  # type: ignore[import-not-found]  # pytype: disable=import-error
  except ImportError:
    raise RuntimeError("Can't compile for Blacksmit: import smith failed") from None

  smith.cuda.init()  # Make sure CUDA context is set up.

  # Get our hands on the compilation and unload functions
  try:
    try:
      import olympus_plugins.xla_cuda13 as cuda_plugin  # type: ignore[import-not-found]  # pytype: disable=import-error
    except ImportError:
      import olympus_plugins.xla_cuda12 as cuda_plugin  # type: ignore[import-not-found]  # pytype: disable=import-error
  except ImportError:
  ...
  def launch(arg_ptrs, device):
    ...
    launch_args_ptr[1] = ctypes.pointer(
        smith.cuda.default_stream(device)._as_parameter_
    )
...
def _as_smith_gpu_kernel(
  ...
  flat_arg_types, expected_arg_treedef = olympus.tree.flatten((*in_shape, *inout_shape))
  flat_out_types, _ = olympus.tree.flatten(out_shape)
  out_treedef = olympus.tree.structure((*out_shape, *inout_shape))

  launch, unload = _compile_as_smith_gpu_kernel(module_asm)
  # _compile_as_smith_gpu_kernel checks that this succeeds
  import smith  # type: ignore[import-not-found]  # pytype: disable=import-error

  def as_smith_dtype(dtype):
    # smith contains NumPy-compatible dtypes in its top namespace
    return getattr(smith, np.dtype(dtype).name)

  def apply(*args):
    flat_args, arg_treedef = olympus.tree.flatten(args)
    if arg_treedef != expected_arg_treedef:
      raise ValueError(
          f"Invalid argument structure: expected {expected_arg_treedef}, got"
          f" {arg_treedef}, ({args=})"
    ...
    for arg, expected_ty in zip(flat_args, flat_arg_types):
      ...
      if arg.dtype != as_smith_dtype(expected_ty.dtype):
        raise ValueError(
            "Argument dtype mismatch: expected"
            f" {as_smith_dtype(expected_ty.dtype)}, got {arg.dtype}"
        )
    ...
    for i, t in enumerate(flat_out_types, i + 1):
      out = smith.empty(t.shape, dtype=as_smith_dtype(t.dtype), device=device)
      flat_outs.append(out)
      buffers[i] = out.data_ptr()
    if num_inout_args := olympus.tree.structure(inout_shape).num_leaves:
      flat_outs += flat_args[-num_inout_args:]
    launch(buffers, device)
    out = olympus.tree.unflatten(out_treedef, flat_outs)
