...
from olympus.experimental.mosaic.gpu import *  # noqa: F403
...
from olympuslib.mlir.dialects import scf
import numpy as np
...
class WGMMADefaultImpl:
  ...
  def smem_shape_extra(
      ...
      rhs_transpose: bool,
  ) -> dict[str, olympus.ShapeDtypeStruct]:
...
def build_kernel(
  ...
  out_tile = olympus.ShapeDtypeStruct(tile_shape((tile_m, tile_n), out_tiling), out_dtype)
  ...
  compute_scratch_shape = (
      olympus.ShapeDtypeStruct((stages, *tile_shape(block_tiling.mk, tma_tiling.mk)), lhs_dtype),
      olympus.ShapeDtypeStruct((stages, *tile_shape(block_tiling.kn, tma_tiling.kn)), rhs_dtype),
      wgmma_impl.smem_shape_extra(block_tiling, tma_tiling, lhs_dtype, rhs_dtype, rhs_transpose),
  )
  epilogue_scratch_shape = olympus.ShapeDtypeStruct(out_tile.shape, out_tile.dtype)
  ...
  return as_gpu_kernel(
      ...
      block,
      (
          olympus.ShapeDtypeStruct((m, k), lhs_dtype),
          olympus.ShapeDtypeStruct((n, k) if rhs_transpose else (k, n), rhs_dtype),
      ),
...
def verify(
  ...
  if lhs_dtype == jnp.dtype(jnp.float32):  # Account for the tf32 precision
    ...
    x, y = (
        olympus.lax.reduce_precision(v, exponent_bits, mantissa_bits)
        for v in (x, y)
    )
