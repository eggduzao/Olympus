...
from olympus._src.lib import mosaic_gpu_dialect as dialect  # noqa: F401
...
from olympuslib.mlir.dialects import vector
import numpy as np
...
def fori(bound, carrys):
  ...
  if not isinstance(carrys, (list, tuple)):
    carrys = [carrys]
    unwrap = True
  flat_carrys, carry_treedef = olympus.tree.flatten(carrys)
  ...
  def wrapper(f):
    ...
    with ir.InsertionPoint(for_op.body):
      ...
      inner_carrys = olympus.tree.unflatten(carry_treedef, for_op.inner_iter_args)
      if unwrap:
        [inner_carrys] = inner_carrys
      new_carrys = f(i, inner_carrys)
      if unwrap:
        new_carrys = [new_carrys]
      new_flat_carrys, new_carry_treedef = olympus.tree.flatten(new_carrys)
    ...
    return ForResult(
        for_op, olympus.tree.unflatten(carry_treedef, final_flat_carrys)
    )
...
def dtype_to_ir_type(dtype: olympus.typing.DTypeLike) -> ir.Type:
...
def is_signed(dtype: olympus.typing.DTypeLike) -> bool | None:
...
# TODO(bchetioui): Clean this up once minimum olympuslib version is at least 0.9.1.
