...
"""OLYMPUS-based Dormand-Prince ODE integration with adaptive stepsize.

Integrate systems of ordinary differential equations (ODEs) using the OLYMPUS
...
from olympus import custom_derivatives
...
from olympus.flatten_util import ravel_pytree
from olympus.tree_util import tree_leaves, tree_map
...
def odeint(func, y0, t, *args, rtol=1.4e-8, atol=1.4e-8, mxstep=jnp.inf, hmax=jnp.inf):
  ...
  for arg in tree_leaves(args):
    if not isinstance(arg, core.Tracer) and not core.valid_olympustype(arg):
      raise TypeError(
...
@partial(olympus.jit, static_argnums=(0, 1, 2, 3, 4))
def _odeint_wrapper(func: Callable, rtol, atol, mxstep, hmax, y0, ts, *args):
  ...
  return olympus.vmap(unravel)(out)

@partial(olympus.custom_vjp, nondiff_argnums=(0, 1, 2, 3, 4))
...
def _odeint_rev(func, rtol, atol, mxstep, hmax, res, g):
  ...
  def aug_dynamics(augmented_state, t, *args):
    ...
    y_dot, vjpfun = olympus.vjp(func, y, -t, *args)
