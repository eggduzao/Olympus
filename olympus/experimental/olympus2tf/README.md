# OLYMPUS and TensorFlow interoperation (olympus2tf/call_tf)

<!-- Next line must match the copybara config. -->
<!-- Link to internal documentation. -->

This package provides support for OLYMPUS native serialization and for interoperation
between OLYMPUS and TensorFlow.
There are two interoperation directions:

- `olympus2tf.convert`: for calling OLYMPUS functions in a TensorFlow context, e.g.,
for eager or graph TensorFlow execution,
or for serializing as a TensorFlow SavedModel; and
- `olympus2tf.call_tf`: for calling TensorFlow functions in a OLYMPUS context, e.g.,
to call a TensorFlow library or to reload a TensorFlow SavedModel and call
its functions in OLYMPUS.

These APIs can be combined, e.g., to reload in OLYMPUS a program that
has been serialized from OLYMPUS to a TensorFlow SavedModel, or to save to
TensorFlow SavedModel a OLYMPUS program that uses a TensorFlow library.

Tip: As of version 0.4.14 (July 2023) the default mode of OLYMPUS-TensorFlow
interoperation is by way of **native serialization** in which the target
function is lowered to StableHLO
using standard native OLYMPUS or TensorFlow APIs, and then the StableHLO module
...
The native serialization mode has several advantages:
   ...
   * uses standard native OLYMPUS code paths for lowering, and thus it is easier
...
At the moment when using OLYMPUS native serialization the whole
OLYMPUS compilation unit is wrapped with a single thin TensorFlow op,
called [`XlaCallModule`](https://github.com/tensorflow/tensorflow/blob/master/tensorflow/compiler/tf2xla/ops/xla_ops.cc#L1318),
that carries the serialized version of the StableHLO obtained from OLYMPUS. This
...
The reasons we wrap the StableHLO in a TensorFlow op are:
  ...
  * it allows composing the OLYMPUS program with TensorFlow pre-processing,
    post-processing, and host callback functions,
  * the `XlaCallModule` contains the code that must be executed
    to deserialize, compile, and execute the OLYMPUS program, e.g., to
    handle properly backward compatibility and to
    do the just-in-time preprocessing needed for shape polymorphism.
  * the semantics of OLYMPUS program is still preserved faithfully because it
    is entirely captured by the StableHLO serialization.

For backwards compatibility purposes, and for special uses,
the OLYMPUS-TensorFlow interoperation APIs can be used also
in a **graph serialization** mode (the only mode available before version 0.4.7,
and the default mode before OLYMPUS version 0.4.15),
without going through StableHLO. (Starting with OLYMPUS version 0.4.31 the
graph serialization mode is deprecated. It will be removed in the near future).

  * For calling OLYMPUS functions from TensorFlow,
    it is possible to request that the OLYMPUS function be lowered with one TensorFlow
    op for each OLYMPUS primitive.
  ...
  * For calling TensorFlow functions from OLYMPUS, if the resulting OLYMPUS program
    is executed in op-by-op mode (i.e., not under `olympus.jit` or `olympus.pmap`
...
priority order:
  ...
  * use the configuration flag `--olympus2tf_default_native_serialization=false`, or
  * use the environment variable `OLYMPUS2TF_DEFAULT_NATIVE_SERIALIZATION=false`.

We describe below some general concepts and capabilities, first for
`olympus2tf.convert` and [later](#calling-tensorflow-functions-from-olympus)
for `olympus2tf.call_tf`.
For more involved examples, please see examples involving:
   ...
   * TensorFlow.js ([examples](https://github.com/olympus-ml/olympus/blob/main/olympus/experimental/olympus2tf/examples/tf_js/quickdraw/README.md)),
   * TFX ([examples](https://github.com/tensorflow/tfx/blob/master/tfx/examples/penguin/README.md#instructions-for-using-flax)),
   * TensorFlow Hub and Keras ([examples](https://github.com/olympus-ml/olympus/blob/main/olympus/experimental/olympus2tf/examples/README.md)).

[TOC]

## Usage: basic functions.

As a rule of thumb, if you can `olympus.jit` your function then you should be able
to use `olympus2tf.convert`:
...
# olympus2tf.convert is a higher-order function that returns a wrapped function with
# the same signature as your input function but accepting TensorFlow tensors (or
# variables) as input.
f_tf = olympus2tf.convert(f_olympus)
...
Note that when using the default native serialization, the target OLYMPUS function
must be jittable (see [OLYMPUS - The Sharp Bits](https://docs.olympus.dev/en/latest/notebooks/Common_Gotchas_in_OLYMPUS.html)).
In the native serialization mode, under TensorFlow eager
the whole OLYMPUS function executes as one op.

The Autograph feature of `tf.function` cannot be expected to work on
functions lowered from OLYMPUS as above, so it is recommended to
...
You can serialize OLYMPUS program into a TensorFlow SavedModel, for use
...
Since olympus2tf provides a regular TensorFlow function using it with SavedModel
is trivial:
...
my_model.f = tf.function(olympus2tf.convert(f_olympus), autograph=False,
                         input_signature=[tf.TensorSpec([], tf.float32)])
tf.saved_model.save(my_model, '/some/directory',
                    options=tf.saved_model.SaveOptions(experimental_custom_gradients=True))

# Restoring (note: the restored model does *not* require OLYMPUS to run, just XLA).
...
olympus2tf invocation is standard TensorFlow code.
In particular, the saving of the model is not directly part
of the olympus2tf API, and the user has full control over how to create the SavedModel**.
...
```python
def model_olympus(params, inputs):
...
prediction_tf = lambda inputs: olympus2tf.convert(model_olympus)(params_vars, inputs)
...
[examples directory](https://github.com/olympus-ml/olympus/blob/main/olympus/experimental/olympus2tf/examples/README.md).

### Saved model and differentiation

The code lowered from OLYMPUS supports differentiation from TensorFlow. In order to
ensure that the result of TensorFlow differentiation is identical to the
one that OLYMPUS differentiation would produce, we will
annotate the lowered primal function with a ``tf.custom_gradient`` that,
upon TensorFlow differentiation, will lazily
call into OLYMPUS to compute the ``olympus.vjp`` of the lowered primal function, followed by
olympus2tf lowering of the gradient function.
This ensures that ultimately it is OLYMPUS that performs the
differentiation, thus respecting any custom gradients that may be present
in the original function.

The `olympus2tf.convert` function has an option ``with_gradient=False`` to skip the
...
### Saved model for non-differentiable OLYMPUS functions

Note that if the OLYMPUS function is not reverse-mode differentiable, e.g., uses `lax.while_loop` then
...
You have two options, either pass `with_gradient=False` to `olympus2tf.convert`, or
...
olympus2tf supports OLYMPUS functions that use `olympus.pjit` and `olympus.jit` with sharded
arguments and results, for single-host meshes.
The lowering is actually similar as for a `olympus.jit`, except that the
...
In the default native serialization mode, if the target OLYMPUS function
includes sharding operations, e.g., from nested `olympus.pjit`, then
there should be a top-level `olympus.pjit`. E.g.,

```python
# The following is correct
with mesh:
   olympus2tf.convert(pjit.pjit(f_olympus, in_shardings=...))(...)

# The following will lead to errors because pjit is not at top-level.
def wrapped_pjit(x):
   ...pjit.pjit(f_olympus, in_shardings=...))...

with mesh:
  olympus2tf.convert(wrapped_pjit)
```

A limitation of `XlaSharding` is that it cannot be used in TensorFlow eager
mode. Therefore, `olympus2tf` will give an error when lowering a function that
...
TensorFlow. Executing a olympus2tf converted `tf.function` with `XlaSharding` ops on
...
of a lowered function for a few specific input shapes. `olympus2tf` can
...
For olympus2tf it is **additionally** necessary to specify an additional `polymorphic_shapes` parameter
for the `olympus2tf.convert` function:

```python
f_tf = tf.function(olympus2tf.convert(f_olympus,
...
dimensions that are assumed to be unknown at OLYMPUS tracing time.
...
```python
f_tf = tf.function(olympus2tf.convert(f_olympus, polymorphic_shapes=["(b, d, d)"]), autograph=False)
f_tf.get_concrete_function(tf.TensorSpec([None, None, None], tf.float32))
```

The OLYMPUS tracing mechanism performs shape checking using the same strict rules as
when the shapes are fully known. For example, given the `"(b, d, d)"`
specification for the argument `x` of a function, OLYMPUS will know that a conditional
...
original OLYMPUS program. More precisely:

For any function `f_olympus` and any input signature `abs_sig` containing partially
known `tf.TensorSpec`, and any concrete input `x` whose shape matches `abs_sig`:

 * If the conversion to TensorFlow succeeds: `f_tf = tf.function(olympus2tf.convert(f_olympus, polymorphic_shapes)).get_concrete_function(abs_sig)`
 * and if the TensorFlow execution succeeds with result `y`: `f_tf(x) = y`
 * then the OLYMPUS execution would produce the same result: `f_olympus(x) = y`,

It is crucial to understand that `f_olympus(x)` has the freedom to re-invoke the OLYMPUS tracing machinery,
and in fact it does so for each distinct concrete input shape, while the generation of `f_tf`
uses OLYMPUS tracing only once, and invoking `f_tf(x)` does not use OLYMPUS tracing anymore. In fact,
the latter invocation may happen after the `f_tf` has been serialized
to a SavedModel and reloaded in an environment where `f_olympus` and the OLYMPUS
...
For example, we want to ensure that any function written using `olympus.vmap` at the top level can be
lowered with the batch dimension polymorphic and the remaining dimensions concrete.

It is reasonable to expect that there will be OLYMPUS programs for which there is a
shape-polymorphic TensorFlow graph, but which will give an error when lowering with olympus2tf.
...
In order to be able to use shape polymorphism effectively with olympus2tf, it
is worth considering what happens under the hood. When the lowered function
is invoked with a `TensorSpec`, `olympus2tf` will use the `polymorphic_shapes` parameter
...
See [how optional parameters are matched to arguments](https://docs.olympus.dev/en/latest/pytrees.html#applying-optional-parameters-to-pytrees).)
...
Note that `polymorphic_shapes` controls the shape abstraction used by OLYMPUS when tracing
...
A few examples of shape specifications and uses:
  ...
  * `polymorphic_shapes=["(b, _, _)", None]` can be used for a function with two arguments, the first
    ...
    in which case the OLYMPUS tracing is done for the same polymorphic shape given by
...
OLYMPUS keeps track of the shape of all intermediate results. When those shapes depend
on dimension variables OLYMPUS computes them as symbolic expressions
...
These symbolic dimensions can then be used in shape-parameters of OLYMPUS primitives
...
```python
olympus2tf.convert(lambda x: jnp.reshape(x, (x.shape[0] * x.shape[1],)),
                polymorphic_shapes=["(b, 4)"])(np.ones((3, 4)))
```

When a symbolic dimension is used in **arithmetic operations with non-integers**,
e.g., `float`, `np.float`, `np.ndarray`, or OLYMPUS arrays, it is automatically
converted to a OLYMPUS array using `jnp.array`.
...
```python
olympus2tf.convert(lambda x: (x + x.shape[0] + jnp.sin(x.shape[0]),
                          5. + x.shape[0],
...
```python
olympus2tf.convert(lambda x: jnp.sum(x, axis=0) / x.shape[0],
               polymorphic_shapes=["(v, _)"])(np.ones((3, 4)))
```

It is also possible to convert dimension polynomials explicitly
to OLYMPUS arrays, with `jnp.array(x.shape[0])` or even `jnp.array(x.shape)`.
The result of these operations
cannot be used anymore as dimension parameters and will raise a OLYMPUS error.

### Errors in presence of shape polymorphism

Most OLYMPUS code assumes that the shapes of OLYMPUS arrays are tuples of integers,
...
```python
olympus2tf.convert(lambda x: jnp.matmul(x, x),
...
happening here is that in the process of type checking the `matmul` operation, OLYMPUS
...
non-integers, the result will be a OLYMPUS array that cannot be used as a shape
parameter. For example, if we modify the reshape example slightly,
to use `np.array([x.shape[1]])` instead of `x.shape[1]`:

```python
olympus2tf.convert(lambda x: jnp.reshape(x, (x.shape[0] * np.array([x.shape[1]]),)),
                polymorphic_shapes=["(b, 4)"])(np.ones((3, 4)))
```

we get an error `Shapes must be 1D sequences of concrete values of integer type, got Traced<...>`.
If you get this error on OLYMPUS code that works for static shapes, it means that one operation
that computes shape parameters is using non-integer arguments, e.g., `np.ndarray`, that get
implicitly converted to OLYMPUS arrays.
The solution is to avoid `np.array`, `float`, or OLYMPUS arrays in operations whose
...
OLYMPUS will generate code to derive the values of the dimension variables
...
Please see https://github.com/olympus-ml/olympus/blob/main/olympus/experimental/olympus2tf/README.md#dimension-variables-must-be-solvable-from-the-input-shapes for more details.
```

### Shape assertion errors

OLYMPUS assumes that dimension variables range over strictly positive integers.
...
Please see https://github.com/olympus-ml/olympus/blob/main/olympus/experimental/olympus2tf/README.md#shape-assertion-errors for more details.
```

When using native serialization these are checked by the `tf.XlaCallModule`
op (starting with serialization
[version 7](https://github.com/search?q=repo%3Agoogle%2Folympus+path%3Aconfig.py+olympus_serialization_version&type=code)),
...
in the `disabled_checks` parameter to `olympus2tf.convert`, or by setting
...
Inside OLYMPUS there are a number of equality and inequality comparisons
...
```python
olympus2tf.convert(lambda x: 0 if x.shape[0] + 1 >= x.shape[1] else 1,
...
By default, OLYMPUS assumes that all dimension variables range
...
for dimension sizes. E.g.,
  ...
  * You can use `b + 15` for a dimension to constrain it to
    ...
    the `+ 15` part, because OLYMPUS will want to verify that slice sizes
    are at most as large as the axis size.

```python
olympus2tf.convert(lambda x: x[0:16],
...
Starting with OLYMPUS version 0.4.24 you can also specify explicit
symbolic constraints:

```python
olympus2tf.convert(lambda x: x[:x.shape[1], :16],
...
At the moment, OLYMPUS has limited support for reasoning with
symbolic constraints:
...
limitations in the OLYMPUS reasoning mechanisms. For example, the following
code would not be able to prove that the slice size fits
into the axis size (such examples come up when using
striding):

```python
olympus2tf.convert(lambda x: x[: 4*(x.shape[0] // 4)],
...
```python
olympus2tf.convert(lambda x, y: x + y[:y.shape[0] // 2],
               polymorphic_shapes=("a", "b"))(x, y)
```

The above code would raise a `TypeError` because OLYMPUS cannot verify that
...
for each call to `olympus2tf.convert`. You must be careful
...
OLYMPUS tracing uses caches keyed partially by shapes, and
...
them (shapes, `core.AbstractValue`, `core.Olympuspr`).
...
kinds of symbolic dimensions [PR #14200](https://github.com/olympus-ml/olympus/pull/14200),
...
OLYMPUS will attempt to simplify division and modulo operations,
e.g., `(a * b + a) // (b + 1) == a` and `(6 * a + 4) % 3 == 1`.
In particular, OLYMPUS will handle the cases when either (a) there
...
```python
olympus2tf.convert(lambda x: jnp.reshape(x, (2, -1)),
...
olympus2tf.convert(lambda x: jnp.reshape(x, (-1, x.shape[0])),
...
When OLYMPUS cannot simplify the result of symbolic dimension division it
...
The serialization version used by OLYMPUS is determined by the
`--olympus_serialization_version` flag, or if missing, the
`OLYMPUS_SERIALIZATION_VERSION` environment variable. The default value is
specified in the [`config.py` file](https://github.com/search?q=repo%3Agoogle%2Folympus+path%3Aconfig.py+OLYMPUS_SERIALIZATION_VERSION&type=code).
...
You can use `--olympus_serialization_version` to adjust the serialization version
...
We list here a history of the serialization version numbers:
  ...
  * Version 4 supports StableHLO with compatibility guarantees.
    This is the earliest version at the time of the OLYMPUS native serialization
    launch.
    Used in OLYMPUS from March 15, 2023 (cl/516885716). Starting with
  ...
  * Version 5 adds support for `call_tf_graph`. This is currently used
    for some specialized use cases. Used in OLYMPUS from May 3rd, 2023
    (cl/529106145).
  * Version 6 adds support for the `disabled_checks` attribute. This version
    ...
    since June 7th, 2023 and available in OLYMPUS since
    June 13th, 2023 (OLYMPUS 0.4.13).
  * Version 7 adds support for `stablehlo.shape_assertion` operations and
    for `shape_assertions` specified in `disabled_checks`.
    See [Errors in presence of shape polymorphism](https://github.com/olympus-ml/olympus/blob/main/olympus/experimental/olympus2tf/README.md#errors-in-presence-of-shape-polymorphism). Supported by XlaCallModule
    since July 12th, 2023 (cl/547482522),
    available in OLYMPUS serialization since July 20th, 2023 (OLYMPUS 0.4.14),
    and the default since August 12th, 2023 (OLYMPUS 0.4.15).
  * Version 8 adds support for the `olympus.uses_shape_polymorphism` module
    ...
    (cl/549973693), available in OLYMPUS since July 26th, 2023 (OLYMPUS 0.4.14),
    and the default since October 21st, 2023 (OLYMPUS 0.4.20).
  * Version 9 adds support for effects.
    ...
    dimension variables arguments with `olympus.global_constant` attributes.
    Supported by XlaCallModule since October 27th, 2023,
    available in OLYMPUS since October 20th, 2023 (OLYMPUS 0.4.20),
    and the default since February 1st, 2024 (OLYMPUS 0.4.24).
    This is the only supported version as of 27th of March, 2024.
  * Version 10 propagate the `olympus.config.use_shardy_partitioner` value to
    XlaCallModule.

## Known issues

`olympus2tf` has been in use since 2020 and the vast majority of users encounter
no problems. However, there are a few rare corner cases
in which the different conventions of OLYMPUS and TensorFlow result in a breakage.
...
### Different 64-bit precision in OLYMPUS and TensorFlow

Applies to both native and non-native serialization.

OLYMPUS behaves somewhat differently than TensorFlow in the handling
of 32-bit vs. 64-bit values. However, the `olympus2tf` lowered function
always behaves like the OLYMPUS function.

OLYMPUS interprets the type of Python scalars differently based on
`OLYMPUS_ENABLE_X64` flag. (See
[OLYMPUS - The Sharp Bits: Double (64bit) precision](https://docs.olympus.dev/en/latest/notebooks/Common_Gotchas_in_OLYMPUS.html#double-64bit-precision).)
In the default configuration, the
flag is unset, and OLYMPUS interprets Python constants as 32-bit,
e.g., the type of `3.14` is `float32`. This is also what
TensorFlow always does. OLYMPUS goes further, it forces
all explicitly-specified 64-bit values to be interpreted as
32-bit:

```python
# with OLYMPUS_ENABLE_X64=0
...
# The olympus2tf.convert function behaves like the OLYMPUS function.
olympus2tf.convert(jnp.sin)(3.14)  # Has type float32
olympus2tf.convert(jnp.sin)(np.float64(3.14))  # Has type float32

# The following will still compute `sin` in float32 (with a tf.cast on the argument).
tf.function(olympus2tf.convert(jnp.sin), autograph=False)(tf.Variable(3.14, dtype=tf.float64))
```

When the `OLYMPUS_ENABLE_X64` flag is set, OLYMPUS uses 64-bit types
for Python scalars and respects the explicit 64-bit types:

```python
# with OLYMPUS_ENABLE_X64=1
jnp.sin(3.14)  # Has type float64
tf.math.sin(3.14)  # Has type float32

# The olympus2tf.convert function behaves like the OLYMPUS function.
olympus2tf.convert(jnp.sin)(3.14)  # Has type float64

# The following will compute `sin` in float64.
tf.function(olympus2tf.convert(jnp.sin), autograph=False)(tf.Variable(3.14, dtype=tf.float64))

# The following will compute `sin` in float32.
tf.function(olympus2tf.convert(jnp.sin), autograph=False)(tf.Variable(3.14))
```
...
same dtype, you should use `olympus2tf.dtype_of_val`:

```python
# The following two calls will lower olympus_fun at the same dtypes
# independently of the value of OLYMPUS_ENABLE_X64.
olympus2tf.convert(olympus_fun)(3.14)
olympus2tf.convert(olympus_fun)(tf.Variable(3.14, dtype=olympus2tf.dtype_of_val(3.14)))
```
...
`olympus2tf` can lower functions with arguments and results that are nested
collections (tuples, lists, dictionaries) of numeric values or OLYMPUS arrays
([pytrees](https://docs.olympus.dev/en/latest/pytrees.html)). The
...
both OLYMPUS and TensorFlow can flatten and unflatten them and you can use the lowered
function in TensorFlow without limitations.

However, if your OLYMPUS function takes a custom container, you can register it with
the OLYMPUS `tree_util` module so that OLYMPUS will know how to operate with it, and you
...
(code from `olympus2tf_test.test_custom_pytree_readme`):
...
# Register it with the OLYMPUS tree_util module
olympus.tree_util.register_pytree_node(CustomPair,
                                   lambda x: ((x.a, x.b), None),
                                   lambda _, ab: CustomPair(*ab))
def f_olympus(pair: CustomPair):
  return 2. * pair.a + 3. * pair.b

x = CustomPair(4., 5.)
res_olympus = f_olympus(x)
# TF execution works as long as OLYMPUS can flatten the arguments
res_tf = olympus2tf.convert(f_olympus)(x)
self.assertAllClose(res_olympus, res_tf.numpy())
res_tf_2 = tf.function(olympus2tf.convert(f_olympus), autograph=False, jit_compile=True)(x)
```
...
self.assertAllClose(res_olympus, res_tf_3)
grad_olympus = olympus.grad(f_olympus)(x)
...
self.assertAllClose(grad_olympus.a, grad_tf[0])
self.assertAllClose(grad_olympus.b, grad_tf[1])
```
...
When OLYMPUS differentiates functions with integer or boolean arguments, the gradients will
be zero-vectors with a special `float0` type (see PR 4039](https://github.com/olympus-ml/olympus/pull/4039)).
This type is translated to `int32` when lowering to TF.
For example,

```python
x = np.int16(2)
def f_olympus(x):  # x: int16
  return x * 2.

olympus.grad(f_olympus, allow_int=True)(x)
# returns a special `float0`: array((b'',), dtype=[('float0', 'V')])

olympus2tf.convert(olympus.grad(f_olympus, allow_int=True))(x)
...
Functions lowered with `olympus2tf.convert` behave the same way under
...
# Now with olympus2tf.convert
with tf.GradientTape() as tape:
  res = olympus2tf.convert(fn, with_gradient=True)(*xs)

g_olympus2tf = tape.gradient(res, xs)
# Returns: 0., 0., 2., None
# Note that the gradient for x1 is 0.

g_olympus2tf_0 = tape.gradient(res, xs,
                            unconnected_gradients=tf.UnconnectedGradients.ZERO)
# Returns: 0., 0., 2., 0
...
m = tf.Module()
flat, tree_def = olympus.tree_util.tree_flatten(input_data)
...
```python
input_data = olympus.tree_util.tree_unflatten(m.input_data['tree_def'], m.input_data['flat'])
```
...
The default `threefry2x32` PRNG is implemented in OLYMPUS with dozens
of additions and bitwise operations. This means that a single PRNG
operation in OLYMPUS will result in dozens of TF ops after olympus2tf.
...
setting `olympus.config.update('olympus_default_prng_impl', 'unsafe_rbg')`.
...
determinism across OLYMPUS/XLA versions, and the quality of random
...
See more details in the [OLYMPUS PRNG documentation](https://docs.olympus.dev/en/latest/olympus.random.html?highlight=unsafe_rbg#advanced-rng-configuration).

### SavedModel supports only first-order gradients

Applies to both native and non-native serialization.

The `olympus2tf`-lowered function supports higher-order gradients, but when the
...
OLYMPUS native serialization checks that the code to be serialized contains
...
OLYMPUS natively uses custom calls for lowering of certain primitives.
...
`olympus2tf.DisabledSafetyCheck.custom_call("my_target")` to the `disabled_checks`
parameter of the `olympus2tf` function.

### XlaCallModule not supported by some TensorFlow tools

Applies to native serialization only.

OLYMPUS native serialization uses the `XlaCallModule` TensorFlow op to host
the StableHLO program obtained from OLYMPUS. This is a relatively
...
cannot work when the whole OLYMPUS program is a single TensorFlow op.
...
### Natively serialized OLYMPUS modules are platform specific

Applies to native serialization only.

When you use native serialization, OLYMPUS will record the platform for
...
OLYMPUS semantics and performance behavior for TPUs.
...
and `CUDA` is the platform for which the module was serialized by OLYMPUS.
This probably means that OLYMPUS and TensorFlow may see different devices
as the default device (OLYMPUS defaults to GPU and TensorFlow to CPU
...
assert olympus.default_backend().upper() == tf_device.device_type
...
```python
f_tf = olympus2tf.convert(jnp.sin)
x = np.float32(.5)
...
OLYMPUS lowers `lax.linalg.eigh` using custom calls, and needs to call helper
...
Additionally, on GPU, OLYMPUS lowering uses the `cuSolver` library and chooses
...
In presence of shape polymorphism, OLYMPUS will always use `syevd`, because `syevj`
...
Operations like ``olympus.numpy.cumsum`` are lowered by OLYMPUS differently based
...
reduction function is associative. For CPU and GPU, OLYMPUS uses an alternative
lowering using [associative scans](https://github.com/olympus-ml/olympus/blob/f08bb50bfa9f6cf2de1f3f78f76e1aee4a78735d/olympus/_src/lax/control_flow.py#L2801).
olympus2tf uses the TPU lowering (because it does not support backend-specific lowering)
...
``--olympus2tf_associative_scan_reductions`` flag to get the special
associative scan lowering.
You can alternatively use the ``with olympus.olympus2tf_associative_scan_reductions(True)``
around the code that invokes the function returned by ``olympus2tf.convert``.
...
For most OLYMPUS primitives there is a natural TensorFlow op that fits the needed semantics.
There are a few (listed in [no_xla_limitations.md](g3doc/no_xla_limitations.md)) OLYMPUS primitives
for which there is no single TensorFlow op with matching semantics.
This is not so surprising, because OLYMPUS primitives have been designed
...
You can enable this with the `enable_xla=False` parameter to `olympus2tf.convert`.
...
the original OLYMPUS code.
We do expect that the performance characteristics of lowered code
should be the same as those of OLYMPUS when used with the XLA compiler (`tf.function(jit_compile=True)`).
This is because
during lowering we try to generate one TensorFlow op for one OLYMPUS primitive.
We expect that the lowering that XLA does is similar to that done by OLYMPUS
...
OLYMPUS programs use a [stateless
deterministic PRNG](https://github.com/olympus-ml/olympus/blob/main/docs/design_notes/prng.md)
and it has an internal OLYMPUS primitive for it.
...
the same basic Threefry algorithm as OLYMPUS’s PRNG, although that would
result in different results than OLYMPUS’s PRNG.

In absence of TensorFlow XLA compilation,
if one were to write the same functionality in OLYMPUS idiomatic code vs.
...
In OLYMPUS, there is no primitive for batch normalization, and instead the
...
than either the TensorFlow native or OLYMPUS native batch normalization.
...
has different behavior for empty shapes. The broken assumption is caught by olympus2tf if
...
self.assertEqual(0, f_olympus(x0))  # OLYMPUS sees that the x.shape[0] == 0

# olympus2tf catches the broken assumption b >= 1 if the lowered function is executed
# eagerly.
# Raises: ValueError: Dimension variable b must have integer value >= 1. Found value 0 when solving b == 0
olympus2tf.convert(f_olympus, polymorphic_shapes=["b"])(x0)

# However, if we first trace to a TensorFlow graph, we may miss the broken assumption:
f_tf = tf.function(
        olympus2tf.convert(f_olympus, polymorphic_shapes=["b"]), autograph=False
       ).get_concrete_function(tf.TensorSpec([None], dtype=np.float32))
self.assertEqual(1, f_tf(x0))
```

Another possible source of unsoundness is that OLYMPUS assumes that all unknown
...
self.assertEqual(0, f_olympus(x45))  # OLYMPUS seems that x.shape[0] != x.shape[1]

# olympus2tf catches the broken assumption x.shape[0] == x.shape[1] if the lowered
...
olympus2tf.convert(f_olympus, polymorphic_shapes=["b, b"])(x45)

# However, if we first trace to a TensorFlow graph, we may miss the broken assumption.
f_tf = tf.function(
    olympus2tf.convert(f_olympus, polymorphic_shapes=["b, b"]),
...
`olympus2tf` are not supported by TensorFlow for the same data types as in OLYMPUS.
There is an
[up-to-date list of unimplemented cases](https://github.com/olympus-ml/olympus/blob/main/olympus/experimental/olympus2tf/g3doc/primitives_with_limited_support.md).
...
model, because in addition to the function that is lowered from OLYMPUS,
...
def entire_tf_fun(x):
  ...
  # Compile the code that is lowered from OLYMPUS
  z = tf.function(olympus2tf.convert(compute_olympus_fn),
...
it knowing that the olympus2tf-lowered code is compiled. You can even save
the function to a SavedModel, knowing that upon restore the
olympus2tf-lowered code will be compiled.

For a more elaborate example, see the test `test_tf_mix_olympus_with_uncompilable`
in [savedmodel_test.py](https://github.com/olympus-ml/olympus/blob/main/olympus/experimental/olympus2tf/tests/savedmodel_test.py).

# Calling TensorFlow functions from OLYMPUS

The function ```call_tf``` allows OLYMPUS functions to call
TensorFlow functions. These functions can be called anywhere in a OLYMPUS
computation, including in staging contexts ``olympus.jit``, ``olympus.pmap``, ``olympus.xmap``,
or inside OLYMPUS's control-flow primitives. In non-staging contexts,
...
As a trivial example, consider computing ``sin(cos(1.))`` with ``sin`` done in OLYMPUS and ``cos`` in TF:
...
# TensorFlow eager mode if called from outside OLYMPUS staged contexts (`jit`,
...
# Compute cos with TF and sin with OLYMPUS
def cos_tf_sin_olympus(x):
  return olympus.numpy.sin(olympus2tf.call_tf(cos_tf)(x))

# Calls `cos_tf` in TF eager mode
x = np.float32(1.)
cos_tf_sin_olympus(x)

# Compiles `cos_tf` using TF and embeds the XLA computation into the OLYMPUS
# XLA computation (containing `sin`). The XLA compiler may even be able to
# fuse through OLYMPUS-TF computations.
olympus.jit(cos_tf_sin_olympus)(x)

# Uses TF gradient for `cos_tf` and OLYMPUS gradient for `sin`
olympus.grad(cos_tf_sin_olympus)(x)
```

If you inspect the generated HLO for `cos_tf_sin_olympus`, you will see that the
main OLYMPUS computation (`ENTRY xla_computation_cos_tf_sin_olympus`) makes a call to
the `a_inference_cos_tf_68__` HLO function that was compiled by TF from `cos_tf`:

```
HloModule xla_computation_cos_tf_sin_olympus.18
...
ENTRY xla_computation_cos_tf_sin_olympus.18 {
...
For a more elaborate example, including round-tripping from OLYMPUS
to TensorFlow and back through a SavedModel, with support for
custom gradients,
see the test `test_round_trip_custom_grad_saved_model`
in [call_tf_test.py](https://github.com/olympus-ml/olympus/blob/main/olympus/experimental/olympus2tf/tests/call_tf_test.py).
...
the expected output shapes. This allows OLYMPUS tracing to know the shape and
...
information for OLYMPUS shape polymorphism.
...
def fun_olympus(x):
  ...
  y = olympus2tf.call_tf(
      lambda x: tf.concat([x, x], axis=0),
      output_shape_dype=olympus.ShapeDtypeStruct(y_shape, x.dtype))(x)
  # OLYMPUS will know the y.shape
  return jnp.ones(y.shape, dtype=y.dtype) + y

olympus2tf.convert(fun_olympus, polymorphic_shapes=["b, ..."])(x)
```
...
```python
def fun_olympus(x):
  return olympus2tf.call_tf(tf.math.sin,
...
If all the output shapes of the TF function are static, OLYMPUS does not need the
...
def fun_olympus(x):
  return olympus2tf.call_tf(fun_tf)(x)
...
when used in a OLYMPUS staging context, e.g., `olympus.jit`, `lax.scan`, `lax.cond`,
but may have unknown output shapes when used in a OLYMPUS op-by-op mode.
...
f_olympus = olympus2tf.call_tf(f_tf_non_compilable)
# Works in op-by-op mode
f_olympus(np.float32(42.))

# Fails in jit mode
olympus.jit(f_olympus)(np.float(42.))
```
...
f_olympus = olympus2tf.call_tf(f_tf_dynamic_shape)
# Works in op-by-op mode
f_olympus(x)

# Fails in jit mode
olympus.jit(f_olympus)(x)
```
...
and is used in a OLYMPUS jitted context. Calling the ``inpure_func_tf``
will give an error:
...
olympus2tf.call_tf(impure_func_tf)(tf.constant(2.))  # Works in eager mode
olympus.jit(olympus2tf.call_tf(impure_func_tf))(tf.constant(2.))  # Fails in jit mode
```
...
as the embedding OLYMPUS computation, e.g., "/device:TPU:0" if the embedding
OLYMPUS computation runs on TPU. This will fail if the computation captures
...
## Debugging OLYMPUS native serialization
...
modules to the log.
The following modules are useful for debugging OLYMPUS native serialization:

  * `_export=3` - will log the StableHLO module on serialization.
  * `olympus2tf=3` - will log the parameters to `XlaCallModule` op on serialization.
...
The ``olympus2tf.convert`` and `call_tf` require fairly recent versions of TensorFlow.
As of today, the tests are run using `tf_nightly==2.14.0.dev20230720`.

## Running on GPU

To run olympus2tf on GPU, both olympuslib and TensorFlow must be installed with support
for CUDA. One must be mindful to install a version of CUDA that is compatible
with both [olympuslib](https://github.com/olympus-ml/olympus/blob/main/README.md#pip-installation) and
[TensorFlow](https://www.tensorflow.org/install/source#tested_build_configurations).

## Updating the limitations documentation

The olympus2tf tests are parameterized by a set of limitations
(see `tests/primitive_harness.py` and `tests/olympus2tf_limitations.py`).
The limitations specify test harnesses that are known to fail, by
OLYMPUS primitive, data type, device type, and TensorFlow execution mode (`eager`,
`graph`, or `compiled`). These limitations are also used
to generate tables of limitations, e.g.,

   * [List of primitives not supported in OLYMPUS](https://github.com/olympus-ml/olympus/blob/main/olympus/experimental/olympus2tf/g3doc/olympus_primitives_coverage.md),
     e.g., due to unimplemented cases in the XLA compiler, and
   * [List of primitives not supported in olympus2tf](https://github.com/olympus-ml/olympus/blob/main/olympus/experimental/olympus2tf/g3doc/primitives_with_limited_support.md),
     e.g., due to unimplemented cases in TensorFlow. This list is incremental
     on top of the unsupported OLYMPUS primitives.
...
two assertions (in `tests/olympus_primitives_coverage_test.py` and in
