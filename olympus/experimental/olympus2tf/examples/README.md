...
Link: go/olympus2tf-examples.

This directory contains a number of examples of using the
[olympus2tf converter](https://github.com/olympus-ml/olympus/blob/main/olympus/experimental/olympus2tf/README.md) to:

  * save SavedModel from trained MNIST models, using both Flax and pure OLYMPUS.
...
The functions generated by `olympus2tf.convert` are standard TensorFlow functions
and you can save them in a SavedModel using standard TensorFlow code, as shown
in the [olympus2tf documentation](https://github.com/olympus-ml/olympus/blob/main/olympus/experimental/olympus2tf/README.md#usage-saved-model).
This decoupling of olympus2tf from SavedModel is important, because it **allows
...
(see [saved_model_lib.py](https://github.com/olympus-ml/olympus/blob/main/olympus/experimental/olympus2tf/examples/saved_model_lib.py).)
...
You can see in
[mnist_lib.py](https://github.com/olympus-ml/olympus/blob/main/olympus/experimental/olympus2tf/examples/mnist_lib.py)
how this can be done for two
implementations of MNIST, one using pure OLYMPUS (`PureOlympusMNIST`) and a CNN
one using Flax (`FlaxMNIST`). Other Flax models can be arranged similarly,
and the same strategy should work for other neural-network libraries for OLYMPUS.
...
# Preparing the model for olympus2tf

Once you have the model in this form, you can use the
`saved_model_lib.save_model` function from
[saved_model_lib.py](https://github.com/olympus-ml/olympus/blob/main/olympus/experimental/olympus2tf/examples/saved_model_lib.py)
to generate the SavedModel. There is very little in that function that is
specific to olympus2tf. The goal of olympus2tf is to convert OLYMPUS functions
...
following sequence of steps:
   ...
   * train an MNIST model, and obtain a pair of an inference function and the
   parameters.
   * convert the inference function with olympus2tf, for one or more batch sizes.
   * save a SavedModel and dump its contents.
   * reload the SavedModel and run it with TensorFlow to test that the inference
   function produces the same results as the OLYMPUS inference function.
...
The default saving location is `/tmp/olympus2tf/saved_models/`.
...
The executable
[keras_reuse_main.py](https://github.com/olympus-ml/olympus/blob/main/olympus/experimental/olympus2tf/examples/keras_reuse_main.py)
extends
[saved_model_main.py](https://github.com/olympus-ml/olympus/blob/main/olympus/experimental/olympus2tf/examples/saved_model_main.py)
with code
to include a olympus2tf SavedModel into a larger TensorFlow Keras model.
...
It is also possible to use olympus2tf-generated SavedModel with TensorFlow serving.
...
[serving examples](https://github.com/olympus-ml/olympus/blob/main/olympus/experimental/olympus2tf/examples/serving/README.md).

# Using olympus2tf with TensorFlow JavaScript

A olympus2tf-generated SavedModel can also be converted to a format usable with
TensorFlow.js, by using the appropriate converters from SavedModel. 
At the moment, these converters may reject some olympus2tf-generated SavedModels due to
some ops not yet implemented in the converters. As a partial workaround, one
can pass the `enable_xla=False` parameter to `olympus2tf.convert` to direct 
`olympus2tf` to avoid problematic ops. This will increase the coverage, and in fact
most, but not all, Flax examples can be converted this way. 

Check out
[Quickdraw TensorFlow.js example](https://github.com/olympus-ml/olympus/blob/main/olympus/experimental/olympus2tf/examples/tf_js/quickdraw/README.md).
