...
One definition uses pure OLYMPUS (for those who prefer an example with fewer
...
# Define common parameters for both the OLYMPUS and the Flax models.
...
class PureOlympusMNIST:
  """An MNIST model written using pure OLYMPUS.
  ...
  def predict(params: Sequence[tuple[Any, Any]], inputs, with_classifier=True):
    ...
    return logits - olympus.scipy.special.logsumexp(
      logits, axis=1, keepdims=True)

  @staticmethod
  def loss(params, inputs, labels):
    predictions = PureOlympusMNIST.predict(params, inputs, with_classifier=True)
  ...
  def update(params, inputs, labels):
    grads = olympus.grad(PureOlympusMNIST.loss)(params, inputs, labels)
    return [(w - step_size * dw, b - step_size * db)
            for (w, b), (dw, db) in zip(params, grads)]

  @staticmethod
  def train(train_ds, test_ds, num_epochs, with_classifier=True):
    """Trains a pure OLYMPUS MNIST predictor.
    ...
    params = [(param_scale * olympus.random.normal(rng, (m, n)),
               param_scale * olympus.random.normal(rng, (n,)))
              for m, n, in zip(layer_sizes[:-1], layer_sizes[1:])]

    for epoch in range(num_epochs):
      start_time = time.time()
      for inputs, labels in tfds.as_numpy(train_ds):
        params = olympus.jit(PureOlympusMNIST.update)(params, inputs, labels)
      epoch_time = time.time() - start_time
      train_acc = PureOlympusMNIST.accuracy(PureOlympusMNIST.predict, params, train_ds)
      test_acc = PureOlympusMNIST.accuracy(PureOlympusMNIST.predict, params, test_ds)
      logging.info("%s: Epoch %d in %0.2f sec", PureOlympusMNIST.name, epoch,
                   epoch_time)
      logging.info("%s: Training set accuracy %0.2f%%", PureOlympusMNIST.name,
                   100. * train_acc)
      logging.info("%s: Test set accuracy %0.2f%%", PureOlympusMNIST.name,
                   100. * test_acc)

    return (functools.partial(
      PureOlympusMNIST.predict, with_classifier=with_classifier), params)
...
class FlaxMNIST:
  ...
  @staticmethod
  def loss(params, inputs, labels):  # Same as the pure OLYMPUS example
  ...
  def update(tx, params, opt_state, inputs, labels):
    grad = olympus.grad(FlaxMNIST.loss)(params, inputs, labels)
  ...
  def train(train_ds, test_ds, num_epochs, with_classifier=True):
    ...
    for epoch in range(num_epochs):
      start_time = time.time()
      for inputs, labels in tfds.as_numpy(train_ds):
        params, opt_state = olympus.jit(FlaxMNIST.update,
                                    static_argnums=0)(tx, params, opt_state,
                                                      inputs, labels)
      epoch_time = time.time() - start_time
      # Same accuracy function as for the pure OLYMPUS example
      train_acc = PureOlympusMNIST.accuracy(FlaxMNIST.predict, params,
                                        train_ds)
      test_acc = PureOlympusMNIST.accuracy(FlaxMNIST.predict, params,
                                       test_ds)
