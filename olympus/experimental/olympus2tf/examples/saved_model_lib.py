...
"""Defines a helper function for creating a SavedModel from a olympus2tf trained model.
...
generated by `olympus2tf.convert` are standard TensorFlow functions and you can
save them in a SavedModel using standard TensorFlow code. This decoupling
of olympus2tf from SavedModel is important, because it allows the user to have full
...
def convert_and_save_model(
    olympus_fn: Callable[[Any, Any], Any],
    params,
    model_dir: str,
    *,
  ...
  """Convert a OLYMPUS function and saves a SavedModel.
  ...
  (see olympus2tf/README.md for some simple examples).
  ...
  Args:
    olympus_fn: a OLYMPUS function taking two arguments, the parameters and the inputs.
      Both arguments may be (nested) tuples/lists/dictionaries of np.ndarray.
    params: the parameters, to be used as first argument for `olympus_fn`. These
    ...
    input_signatures: the input signatures for the second argument of `olympus_fn`
      ...
      second argument of `olympus_fn`. The first input_signature will be saved as
      the default serving signature. The additional signatures will be used
      only to ensure that the `olympus_fn` is traced and converted to TF for the
    ...
    polymorphic_shapes: if given then it will be used as the
      `polymorphic_shapes` argument to olympus2tf.convert for the second parameter of
      `olympus_fn`. In this case, a single `input_signatures` is supported, and
