...
By default, uses a pure OLYMPUS implementation of MNIST. There are flags to choose
...
from olympus.experimental.olympus2tf.examples import mnist_lib
from olympus.experimental.olympus2tf.examples import saved_model_lib
...
_MODEL = flags.DEFINE_enum(
    "model", "mnist_flax", ["mnist_flax", "mnist_pure_olympus"],
    "Which model to use.")
...
_MODEL_PATH = flags.DEFINE_string("model_path", "/tmp/olympus2tf/saved_models",
...
_TEST_SAVEDMODEL = flags.DEFINE_boolean(
    ...
    "Test TensorFlow inference using the SavedModel w.r.t. the OLYMPUS model.")
...
def pick_model_class():
  """Picks one of PureOlympusMNIST or FlaxMNIST."""
  if _MODEL.value == "mnist_pure_olympus":
    return mnist_lib.PureOlympusMNIST
