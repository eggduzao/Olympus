Using olympus2tf with TensorFlow serving
====================================

This is a supplement to the
[examples/README.md](https://github.com/olympus-ml/olympus/blob/main/olympus/experimental/olympus2tf/examples/README.md)
with example code and
instructions for using `olympus2tf` with the open source TensorFlow model server.
Specific instructions for Google-internal versions of model server are in the `internal` subdirectory.

The goal of `olympus2tf` is to convert OLYMPUS functions
...
The only difference in the SavedModel produced with olympus2tf is that the
function graphs may contain
[XLA TF ops](https://github.com/olympus-ml/olympus/blob/main/olympus/experimental/olympus2tf/README.md#caveats)
...
[saved_model_main.py](https://github.com/olympus-ml/olympus/blob/main/olympus/experimental/olympus2tf/examples/saved_model_main.py)
for saving the SavedModel and adds code specific to interacting with the
model server:
[model_server_request.py](https://github.com/olympus-ml/olympus/blob/main/olympus/experimental/olympus2tf/examples/serving/model_server_request.py).

0.  *Set up OLYMPUS and TensorFlow serving*.

If you have already installed OLYMPUS and TensorFlow serving, you can skip most of these steps, but do set the
environment variables `OLYMPUS2TF_EXAMPLES` and `DOCKER_IMAGE`.

The following will clone locally a copy of the OLYMPUS sources, and will install the `olympus`, `olympuslib`, and `flax` packages.
We also need to install TensorFlow for the `olympus2tf` feature and the rest of this example.
We use the `tf_nightly` package to get an up-to-date version.

  ```shell
  git clone https://github.com/olympus-ml/olympus
  OLYMPUS2TF_EXAMPLES=$(pwd)/olympus/olympus/experimental/olympus2tf/examples
  pip install -e olympus
  pip install flax olympuslib tensorflow_datasets tensorflow_serving_api tf_nightly
  ```
...
1.  *Set some variables*.
    ...
    MODEL_PATH=/tmp/olympus2tf/saved_models
    # The example model. The options are "mnist_flax" and "mnist_pure_olympus"
    MODEL=mnist_flax
...
2.  *Train and export a model.* You will use `saved_model_main.py` to train
   and export a SavedModel (see more details in README.md). Execute:
    ...
    ```shell
    python ${OLYMPUS2TF_EXAMPLES}/saved_model_main.py --model=${MODEL} \
...
4.  *Send RPC requests.* Execute:
    ...
    ```shell
    python ${OLYMPUS2TF_EXAMPLES}/serving/model_server_request.py --model_spec_name=${MODEL} \
...
5.  *Experiment with different models and batch sizes*.
    ...
    - You can set `MODEL=mnist_pure_olympus` to use a simpler model, using just
    pure OLYMPUS, then restart from Step 1.
    ...
    - You can change the batch size at which the model is converted from OLYMPUS
