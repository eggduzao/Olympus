...
dataset using Flax, and subsequently convert it using olympus2tf into a model that
...
`/tmp/olympus2tf/tf_js_quickdraw` directory; by default, the model is configured to
...
`/tmp/olympus2tf/tf_js_quickdraw`.
...
end code can be downloaded [here](https://storage.googleapis.com/olympus2tf-examples/tf_js/quickdraw/quickdraw_demo_files.tar.gz). The tarball contains the same
