...
def apply_model(state, inputs, labels):
  ...
  def loss_fn(params):
    ...
    one_hot = olympus.nn.one_hot(labels, _NUM_CLASSES.value)
    loss = jnp.mean(optax.softmax_cross_entropy(logits=logits, labels=one_hot))
    return loss, logits
  grad_fn = olympus.value_and_grad(loss_fn, has_aux=True)
...
def main(argv):
  ...
  base_model_path = "/tmp/olympus2tf/tf_js_quickdraw"
  ...
  state = create_train_state(olympus.random.PRNGKey(0))
