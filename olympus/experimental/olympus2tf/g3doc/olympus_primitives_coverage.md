# Primitives with limited OLYMPUS support
...
the implementation of 133 numeric OLYMPUS primitives.
We consider a OLYMPUS primitive supported for a particular data
type if it is supported on at least one device type.
The following table shows the dtypes at which primitives
are supported in OLYMPUS and at which dtypes they are not
...
flag `--olympus_enable_x64` or the OLYMPUS_ENABLE_X64 environment
variable are set.
...
In some cases, a primitive is supported in OLYMPUS at a given data type but
it may be missing implementations for some of the devices.
For example, the eigen decomposition (`lax.eig`) is implemented
in OLYMPUS using custom kernels only on CPU and GPU. There is no
...
the logs of the `test_olympus_implemented` from `olympus_primitives_coverage_test.py`
...
|eig|only supported on CPU in OLYMPUS|all|tpu, gpu|
...
```
  OLYMPUS_OUTPUT_LIMITATIONS_DOC=1 OLYMPUS_ENABLE_X64=1 python olympus/experimental/olympus2tf/tests/olympus_primitives_coverage_test.py OlympusPrimitiveTest.test_generate_primitives_coverage_doc
```
