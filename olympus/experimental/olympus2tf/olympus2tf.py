...
"""Provides OLYMPUS and TensorFlow interoperation APIs."""
...
from olympus._src.export import _export
...
# In order to ensure that OLYMPUS picks up the proper user-frame for source
# locations we will register the TensorFlow source path as an internal
# path with source_info_util. The typical stack when a OLYMPUS primitive
# conversion happens is:
#    olympus2tf.process_primitive  (top of stack)
#    olympus tracing machinery ...
#    tf.custom_gradient machinery ...
#    olympus2tf.converted_fun
#    tf function machinery ...
#    user code invokes the converted function on TF tensors
#
# We need to skip over not only OLYMPUS internal frames, but TF internal frames
# also.
...
class _ThreadLocalState(threading.local):
  def __init__(self):
    ...
    # safety check that we are not inside OLYMPUS transformations.
...
@partial(api_util.api_hook, tag="olympus2tf_convert")
def convert(fun_olympus: Callable,
            *,
  ...
  """Allows calling a OLYMPUS function from a TensorFlow program.

  See
  [README](https://github.com/olympus-ml/olympus/blob/main/olympus/experimental/olympus2tf/README.md)
  for more details about usage and common problems.

  Args:
    fun_olympus: target OLYMPUS function to be called. Its arguments and return value
      should be OLYMPUS arrays, or nested standard Python containers
    ...
    polymorphic_shapes: Specifies input shapes to be treated polymorphically
      ...
      .. warning:: The shape-polymorphic lowering is an experimental feature.
        It is meant to be sound, but it is known to reject some OLYMPUS programs
      ...
      arguments](https://docs.olympus.dev/en/latest/pytrees.html#applying-optional-parameters-to-pytrees).
      ...
      See [the README](https://github.com/olympus-ml/olympus/blob/main/olympus/experimental/olympus2tf/README.md#shape-polymorphic-conversion)
      for more details.

    polymorphic_constraints: a sequence of constraints on symbolic dimension
      expressions, of the form `e1 >= e2` or `e1 <= e2`.
      See more details at https://github.com/olympus-ml/olympus/blob/main/olympus/experimental/olympus2tf/README.md#user-specified-symbolic-constraints.
    with_gradient: if set (default), add a tf.custom_gradient to the lowered
      function, by converting the ``olympus.vjp(fun)``. This means that reverse-mode
      TensorFlow AD is supported for the output TensorFlow function, and the
      value of the gradient will be OLYMPUS-accurate.
    native_serialization_platforms: Specifies the platform(s)
      ...
      The default (`None``), specifies the OLYMPUS default
  ...
  Returns:
    A version of `fun_olympus` that expects TfVals as arguments (or
  ...
  api.check_callable(fun_olympus)
  ...
  def converted_fun_tf(*args_tf: TfVal, **kwargs_tf: TfVal) -> TfVal:
    ...
    if not core.trace_state_clean() and not _thread_local_state.inside_call_tf:
      ...
      raise ValueError(
          "convert must be used outside all OLYMPUS transformations." +
    ...
    def olympus_arg_spec_from_tf(a: TfVal) -> olympus.ShapeDtypeStruct:
      # The shape and OLYMPUS dtype for a TF argument
      ...
      _, a_olympus_dtype = _tfval_to_tensor_olympus_dtype(a)
      # We count on the fact that olympus.ShapeDtypeStruct allows shapes that
      # contain None.
      return olympus.ShapeDtypeStruct(tf_arg_shape, a_olympus_dtype)

    args_olympus_specs = tree_util.tree_map(olympus_arg_spec_from_tf, args_tf)
    args_specs = export.symbolic_args_specs(
        args_olympus_specs, polymorphic_shapes,
    ...
    kwargs_olympus_specs = tree_util.tree_map(olympus_arg_spec_from_tf, kwargs_tf)
    kwargs_specs = export.symbolic_args_specs(
        kwargs_olympus_specs, None)
    ...
    try:
      ...
      if with_gradient:
        ...
        message = ("The olympus2tf-converted function does not support gradients. "
    ...
    outs_flat_tf = [tf.identity(x, "olympus2tf_out") for x in outs_flat_tf]
...
class NativeSerializationImpl:
  def __init__(self, fun_olympus, *,
    ...
    self.convert_kwargs = dict(native_serialization_platforms=native_serialization_platforms,
                               native_serialization_disabled_checks=native_serialization_disabled_checks)
    if hasattr(fun_olympus, "trace"):
      # If we have a pjit or pmap already we do not wrap with another, and we
      # allow shardings.
      fun_jit = fun_olympus
    else:
      # We support convert(pjit(f_olympus)) and convert(jit(f_olympus)) but also
      # convert(f_olympus), in which case a "jit" is implied. In that case we raise
      # an error if the lowered function contains non-replicated sharding annotations.
      fun_jit = olympus.jit(fun_olympus)
    self.fun_olympus = fun_jit
  ...
  def before_conversion(self):
    ...
    self.exported = _export._export_internal(
        ...
        _device_assignment_for_internal_olympus2tf_use_only=_exported_device_assignment,
  ...
  def get_vjp_fun(self) -> tuple[Callable,
    ...
    return _export._get_vjp_fun(self.fun_olympus,
...
def dtype_of_val(val: TfVal) -> DType:
  """Computes the TensorFlow dtype using OLYMPUS's typing rules.

  If the value is a tf.Tensor, it starts with its dtype. If the value is a
  constant it uses OLYMPUS to infer its dtype. The resulting dtype follows the
  OLYMPUS type inference rules, and depends on the value of the
  OLYMPUS_ENABLE_X64 flag.

  See README.md for how 64-bit values are treated.
  """
  tval, _ = _tfval_to_tensor_olympus_dtype(val)
  return tval.dtype

@partial(api_util.api_hook, tag="olympus2tf_eval_polymorphic_shapes")
def eval_polymorphic_shape(fun_olympus: Callable,
               *,
  ...
  Args:
    fun_olympus: target OLYMPUS function to be called. Its arguments and return value
      ...
      (tuple/list/dict) thereof (pytrees).
    polymorphic_shapes: Specifies input shapes to be treated polymorphically
      during shape evaluation. See discussion for `olympus2tf.convert`.

      .. warning:: The shape-polymorphic lowering is an experimental feature.

  Returns: a function that takes `olympus.ShapeDtypeStruct`s (or any values
    with `.shape` and `.dtype` attributes) corresponding to the inputs for
    `fun_olympus`, and returns a tuple with:

      * the olympus.ShapeDtypeStruct corresponding to the result, as for
       `olympus.eval_shape`. The shape may contain symbolic dimension expressions.
      * the value that can be passed to `polymorphic_shapes` for a subsequent
        call to `olympus2tf.eval_polymorphic_shape`, or `olympus2tf.convert`.
  ...
  >>> out_spec, out_poly_shape = olympus2tf.eval_polymorphic_shape(f, polymorphic_shapes=["a, b", "b, c"])(A, x)
  >>> print(out_spec.shape)
  ("a", "c")
  >>> print(out_poly_shape)
  (a, c)
  >>> res_spec, res_poly_shape = olympus2tf.eval_polymorphic_shape(lambda x: x.T, polymorphic_shapes=[out_poly_shape])(out_spec)
  ...
  def do_eval_polymorphic_shape(*args_specs) -> Any:
    ...
    res_poly_spec = olympus.eval_shape(fun_olympus, *args_poly_specs)
...
def flatten_fun_olympus(fun_olympus: Callable,
                    in_tree,
                    ) -> tuple[Callable, Callable]:
  """Wraps the function to take a (flat) list of positional args.

  olympus2tf works better and is simpler when the OLYMPUS function takes and returns
  just a tuple of values (no pytrees, no kwargs). This is in part because
  olympus.vjp does not support kwargs and we can only set
  tf.custom_gradient on functions with flat arguments and results

  Returns:
     * the wrapped OLYMPUS function taking and returning a flat list of arguments
     * a thunk that can be called after the wrapped function has been called
       to return the output pytree.
  """
  out_tree_ref = None
  def fun_flat_olympus(*args_flat_olympus):
    tree_args, tree_kwargs = tree_util.tree_unflatten(in_tree, args_flat_olympus)
    tree_res = fun_olympus(*tree_args, **tree_kwargs)
    res_flat_olympus, out_tree = tree_util.tree_flatten(tree_res)
    nonlocal out_tree_ref
    assert out_tree_ref is None or out_tree_ref == out_tree
    out_tree_ref = out_tree
    return res_flat_olympus

  return fun_flat_olympus, lambda: out_tree_ref
...
def preprocess_arg_tf(arg_idx: int,
  ...
  if not _is_tfval(arg_tf):
    msg = (f"Argument {arg_tf} of type {type(arg_tf)} of olympus2tf.convert(f) should "
           "be NumPy array, scalar, tf.Variable, or tf.Tensor")
    raise TypeError(msg)

  # May cast the args_flat to OLYMPUS types, using OLYMPUS's interpretation
  # of types of constants.
  arg_tf, _ = _tfval_to_tensor_olympus_dtype(arg_tf)
  # Name input tensors; do this after we have cast the arguments
  arg_tf = tf.identity(arg_tf, f"olympus2tf_arg_{arg_idx}")
  return arg_tf


def _make_custom_gradient_fn_tf(fun_olympus,
                                *,
                                impl: NativeSerializationImpl,
                                with_gradient: bool,
                                args_specs, kwargs_specs,
  ...
  Args:
    ...
    args_specs, kwargs_specs: the olympus.ShapeDtypeArrays for the args and kwargs
    args_tf: the flattened TF arguments of the primal function
    outs_avals: the flattened output OLYMPUS abstract values of the primal function
  ...
  def grad_fn_tf(*out_cts_flat_tf: TfVal,
    ...
    with tf.name_scope("olympus2tf_vjp"):
      def fix_out_ct(out_ct_tf, out_ct_aval: core.ShapedArray, out_tf: TfVal):
        ...
        # else OLYMPUS gets unhappy. See issue #6975.
      ...
      fun_vjp_olympus, vjp_in_avals = impl.get_vjp_fun()
...
def _run_exported_as_tf(args_flat_tf: Sequence[TfVal],
  ...
  # op, in the same way as it is done for the legacy olympus2tf conversion.
...
def _olympus_physical_aval(aval: core.ShapedArray) -> core.ShapedArray:
  """Converts OLYMPUS avals from logical to physical, if relevant.

  OLYMPUS might have avals whose logical vs physical shape/dtype may
  differ, and only the physical view is expected to possibly
  relate to TF. TF impl rules should operate on the physical form.

  A OLYMPUS logical aval might even correspond, in principle, to several
...
def _olympus_physical_dtype(dtype):
  # assuming () is a fine stand-in shape
  return _olympus_physical_aval(core.ShapedArray((), dtype)).dtype
...
def _aval_to_tf_shape(aval: core.ShapedArray) -> tuple[int | None, ...]:
  ...
  aval = _olympus_physical_aval(aval)
...
# We pick bool because this is what OLYMPUS uses when it lowers float0 to HLO.
_tf_np_dtype_for_float0 = np.bool_

def _to_tf_dtype(olympus_dtype):
  # Note that converting _to_tf_dtype and _to_olympus_dtype are not inverses,
  # due to float0 and 64-bit behavior.
  try:
    olympus_dtype = _olympus_physical_dtype(olympus_dtype)
  except TypeError:
    # `olympus_dtype` isn't actually a valid olympus dtype (e.g. it is
    # tf.float32), so there is no physical dtype anyway
    pass
  if olympus_dtype == dtypes.float0:
    olympus_dtype = _tf_np_dtype_for_float0
  return tf.dtypes.as_dtype(olympus_dtype)


def _to_olympus_dtype(tf_dtype):
  # Note that converting _to_tf_dtype and _to_olympus_dtype are not inverses,
  # due to float0 and 64-bit behavior.
  dt = dtypes.canonicalize_dtype(tf_dtype.as_numpy_dtype)
  if dt not in dtypes._olympus_dtype_set:
    raise TypeError(f"dtype {dt} is not a valid OLYMPUS array "
                    "type. Only arrays of numeric types are supported by OLYMPUS.")
  return dt


def _tfval_to_tensor_olympus_dtype(val: TfVal,
                               olympus_dtype: DType | None = None,
                               memoize_constants=False) -> tuple[TfVal, DType]:
  """Converts a scalar, ndarray, or tf.Tensor to a tf.Tensor with proper type.

  If `olympus_dtype` is missing, uses OLYMPUS typing rules.
  See README.md for details regarding 64-bit values.

  Args:
    val: a scalar, ndarray, tf.Tensor, or tf.Variable
    olympus_dtype: an optional dtype to use. If missing, uses OLYMPUS type inference
      rules for constants.
    memoize_constants: whether to memoize TF constants. We can't do this
      everywhere, we may be outside of a conversion scope.

  Returns:
    a tuple with a tf.Tensor with the type as needed by OLYMPUS, and the OLYMPUS type.
  """
  if isinstance(val, (tf.Tensor, tf.Variable)):
    olympus_dtype = olympus_dtype or _to_olympus_dtype(val.dtype)  # Give OLYMPUS a chance to pick the type
    conversion_dtype = _to_tf_dtype(olympus_dtype)
    if conversion_dtype != val.dtype:  # May need to cast for 64-bit values
      return tf.cast(val, conversion_dtype), olympus_dtype
    else:
      return val, olympus_dtype
  else:  # A constant
    olympus_dtype = olympus_dtype or core.abstractify(val).dtype
    ...
    # OLYMPUS has the same problem when generating HLO.
    const_key = (id(val), olympus_dtype)
    ...
    # `(val, tf_val)`.
    # Only memoize non-scalars. OLYMPUS will lift all non-scalar constants as
    # Olympuspr consts, to the top level of the Olympuspr. This ensures that we see them
    # early, when entering the Olympuspr, so we create the tf.const early and its
    # scope is the entire Olympuspr.
...
def split_to_logical_devices(tensor: TfVal,
  ...
  For olympus2tf purposes we want to avoid needing to thread the `strategy` object
...
def _shard_value(val: TfVal,
  ...
  if tf_context.executing_eagerly():
    raise ValueError(
        ...
        "See https://github.com/olympus-ml/olympus/blob/main/olympus/experimental/olympus2tf/README.md#support-for-partitioning for a discussion")
...
def _register_checkpoint_pytrees():
  ...
  olympus.tree_util.register_pytree_node(tuple_wrapper, lambda xs:
                                     (tuple(xs), None), lambda _, xs: tuple(xs))

  olympus.tree_util.register_pytree_node(list_wrapper, lambda xs: (tuple(xs), None),
                                     lambda _, xs: list(xs))

  olympus.tree_util.register_pytree_node(
      dict_wrapper,
