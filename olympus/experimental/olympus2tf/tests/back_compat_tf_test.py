...
from olympus.experimental.olympus2tf.tests.back_compat_testdata import tf_call_tf_function
...
class CompatTensoflowTest(bctu.CompatTestBase):
  ...
  contains a `olympus2tf.call_tf` and uses `olympus2tf.convert` to generate a
  ...
  def run_current(self, func: Callable, data: bctu.CompatTestData):
    ...
    def tf_func(the_input):  # Use recognizable names for input and result
      res = olympus2tf.convert(func)(the_input)
  ...
  def serialize(
    ...
    for op in tf_graph.get_operations():
      if op.type == "XlaCallModule":
        ...
        module_str = _olympus.mlir.deserialize_portable_artifact(
            serialized_module
        )
