...
from olympus._src import dlpack
...
class CallTfTest(tf_test_util.OlympusToTfTestCase):
  ...
  def test_eval_scalar_arg(self, with_jit=True):
    ...
    x = 3.
    res = _maybe_jit(with_jit, olympus2tf.call_tf(f_tf))(x)
    self.assertAllClose(jnp.sin(x), res)

  @_parameterized_jit
  def test_eval_scalar_res(self, with_jit=True):
    x = 3.
    res = _maybe_jit(with_jit, olympus2tf.call_tf(lambda x: 4.))(x)
    self.assertAllClose(4., res, check_dtypes=False)

  @_parameterized_jit
  def test_eval_numpy_arg(self, with_jit=True):
    x = np.ones((2, 3), dtype=np.float32)
    res = _maybe_jit(with_jit, olympus2tf.call_tf(tf.math.sin))(x)
    self.assertAllClose(jnp.sin(x), res)

  @_parameterized_jit
  def test_eval_numpy_res(self, with_jit=False):
    x = np.ones((2, 3))
    res = _maybe_jit(with_jit, olympus2tf.call_tf(lambda _: x))(x)
  ...
  def test_eval_devicearray_arg(self, with_jit=False):
    ...
    res = olympus2tf.call_tf(lambda x: x)(x)
    self.assertAllClose(x, res)
  ...
  def test_eval_pytree(self, with_jit=True):
    ...
    fun_olympus = _maybe_jit(with_jit, olympus2tf.call_tf(fun_tf))
    res = fun_olympus(x, y)
  ...
  def test_result_tuple(self):
    ...
    fun_olympus = olympus.jit(olympus2tf.call_tf(fun_tf))
    res = fun_olympus()
  ...
  def test_error_non_compilable_strings(self):
    ...
    x = np.float32(0.7)
    self.assertAllClose(f_tf_non_compilable(x).numpy(), f_olympus(x))
    ...
    with self.assertRaisesRegex(ValueError,
                                _call_tf_non_compilable_error):
      lax.cond(True, lambda x: f_olympus(x), lambda x: f_olympus(x), x)
  ...
  def test_error_non_compilable_dynamic_shape(self):
    ...
    self.assertAllClose(f_tf_non_compilable(x), f_olympus(x))  # Works in eager mode
  ...
  def test_error_bad_result_tensorarray(self):
    ...
    with self.assertRaisesRegex(ValueError,
                                "The called TF function returns a result that is not convertible to OLYMPUS"):
      fun_olympus = olympus.jit(olympus2tf.call_tf(fun_tf))
      fun_olympus()
  ...
  def test_control_flow(self, with_jit=True):
    ...
    def fun_olympus(x):
      ...
      def body(_, acc):
        return olympus2tf.call_tf(times_5_tf)(acc)

      return lax.fori_loop(0, 3, body, x)

    x = np.float32(3.)
    res = _maybe_jit(with_jit, fun_olympus)(x)
  ...
  def test_dtypes(self, dtype=np.int32, with_jit=True):
    ...
    def fun_olympus(x):
      return olympus2tf.call_tf(fun_tf)(x) + x
  ...
  def test_bool(self, with_jit=False):
    ...
    res = _maybe_jit(with_jit, olympus2tf.call_tf(fun_tf))(x, y)
    self.assertAllClose(
  ...
  def test_x64_input(self, with_jit=True):
    ...
    res_call_tf = _maybe_jit(with_jit, olympus2tf.call_tf(f_tf))(x)
    res_olympus = jnp.sin(x)
    self.assertAllClose(res_call_tf, res_olympus)
  ...
  def test_x64_output(self, with_jit=True):
    ...
    res_call_tf_jit = olympus.jit(olympus2tf.call_tf(f_tf))(x)
    self.assertAllClose(res_call_tf_jit, res_olympus)
  ...
  def test_with_var_read(self, with_jit=True):
    ...
    res = _maybe_jit(with_jit, olympus2tf.call_tf(fun_tf))(x)
  ...
  def test_with_var_different_shape(self):
    # See https://github.com/olympus-ml/olympus/issues/6050
    ...
    olympus_func = olympus.jit(olympus2tf.call_tf(tf_func))
    olympus_out = olympus_func(x)

    self.assertAllClose(tf_out, olympus_out, check_dtypes=False)
  ...
  def test_with_capture_then_convert_again(self):
    ...
    res = olympus2tf.convert(olympus2tf.call_tf(tf_fn))(x)
    self.assertAllClose(res, 2 * x)

    # The bug appears only when we use non-eager mode on the converted func
    res = tf.function(olympus2tf.convert(olympus2tf.call_tf(tf_fn)),
                      autograph=False)(x)
    self.assertAllClose(res, 2 * x)
  ...
  def test_grad(self, with_jit=False):
    x = np.float32(3.)
    res = _maybe_jit(with_jit, olympus.grad(olympus2tf.call_tf(tf.math.sin)))(x)
  ...
  def test_grad_pytree(self, with_jit=False):
    ...
    grad_x = _maybe_jit(with_jit, olympus.grad(olympus2tf.call_tf(fun_tf)))(x, y)
    self.assertAllClose(
  ...
  def test_grad_nested(self):
    ...
    @olympus.jit  # To recognize it in olympuspr
    def f_olympus(x_dict):
    ...
    g_fun_with_tf = olympus.grad(partial(loss, olympus2tf.call_tf(f_tf)))
    g_fun_with_olympus = olympus.grad(partial(loss, f_olympus))

    g_tf = g_fun_with_tf(x_dict)
    g_olympus = g_fun_with_olympus(x_dict)
    self.assertAllClose(g_olympus, g_tf)

  def test_grad_int_argument(self):
    # Similar to https://github.com/olympus-ml/olympus/issues/6975
    ...
    f_call_tf = olympus2tf.call_tf(f)
    g_call_tf = olympus.grad(lambda *args: jnp.sum(f_call_tf(*args)[0]))(param, state, x)
    g = olympus.grad(lambda *args: jnp.sum(f(*args)[0]))(param, state, x)
  ...
  def test_grad_int_argument_unused(self):
    ...
    def olympus_model(params, rng, inputs):
      return jnp.ones([batch_size, 2], dtype=jnp.float32)

    tf_model = olympus2tf.convert(olympus_model, with_gradient=True)

    def _loss_fn(inference_fn, params, rng, inputs):
      prediction = inference_fn(params, rng, inputs)
      return jnp.mean(prediction)

    olympus_loss_fn = partial(_loss_fn, olympus_model)
    olympus_grad = olympus.grad(olympus_loss_fn)(params, rng, inputs)
    ...
    self.assertAllClose(olympus_grad, tf_grad.numpy())

    call_tf_loss_fn = partial(_loss_fn, olympus2tf.call_tf(tf_model))
    call_tf_grad = olympus.grad(call_tf_loss_fn)(params, rng, inputs)
    self.assertAllClose(olympus_grad, call_tf_grad)

  def test_grad_with_float0_result(self):
    # Gradient over integer-argument functions, with float0 result
    def f_olympus(x, y):  # x is an int, y is a float; res is a (int, float)
      return (2 * x, 2 * x + y * y)
    def f_tf(x, y):
    ...
    grad_g = olympus.grad(partial(wrapper, f_olympus),
                      allow_int=True, argnums=(0, 1))
    grad_g_call_tf = olympus.grad(partial(wrapper, olympus2tf.call_tf(f_tf)),
                              allow_int=True, argnums=(0, 1))

    x = np.int32(2)
    y = np.float32(3.)
    g_olympus = grad_g(x, y)
    g_call_tf = grad_g_call_tf(x, y)
    self.assertEqual(g_olympus[0].dtype, dtypes.float0)
    self.assertEqual(g_call_tf[0].dtype, dtypes.float0)
    self.assertAllClose(g_olympus[1], g_call_tf[1])
  ...
  def test_grad_custom(self, with_jit=False):
    ...
    x = np.float32(4.)
    grad_x = _maybe_jit(with_jit, olympus.grad(olympus2tf.call_tf(func_square_tf)))(x)
  ...
  def test_higher_order_grad(self, degree=2, with_jit=False):
    ...
    def fun_olympus(x):
      return 3. * _maybe_jit(with_jit, olympus2tf.call_tf(fun_tf))(x)

    def fun_olympus_pure(x):
      return 3. * fun_tf(x)

    grad_olympus = fun_olympus
    grad_olympus_pure = fun_olympus_pure
    for _ in range(degree):
      grad_olympus = olympus.grad(grad_olympus)
      grad_olympus_pure = olympus.grad(grad_olympus_pure)

    res_olympus = grad_olympus(np.float32(5.))
    logging.info("Grad of %s degree is %s", degree, res_olympus)
    self.assertAllClose(res_olympus, grad_olympus_pure(np.float32(5.)))

  def test_pmap(self):
    logging.info("Running test_pmap on %s devices", olympus.local_device_count())

    def plus_2_tf(x):
      return tf.math.add(2., x)

    def fun_olympus(x):
      return np.float32(3.) * olympus2tf.call_tf(plus_2_tf)(x)

    x = np.arange(olympus.local_device_count(), dtype=np.float32)
    res = olympus.pmap(fun_olympus)(x)
  ...
  def test_function_compile_time_constant_inputs(self):
    ...
    res1 = olympus2tf.call_tf(fun_tf)(x)
  ...
  def test_repro_193754660(self):
    ...
    # We have to have tf.function(olympus2tf.convert(olympus2tf.call_tf(f_tf))).
    ...
    f_olympus2 = olympus2tf.call_tf(f_tf_rt)
    f_tf2 = olympus2tf.convert(f_olympus2)
    res = tf.function(f_tf2, autograph=False)(x)
    self.assertAllClose(res.numpy(), f_olympus(x))

  def test_effectful(self):
    x = np.ones((3,), dtype=np.float32)
    lower_effect = olympus.jit(olympus2tf.call_tf(tf.math.sin, has_side_effects=True)).lower(x)
    self.assertNotEmpty(lower_effect._lowering.compile_args["unordered_effects"])

    lower_no_effect = olympus.jit(olympus2tf.call_tf(tf.math.sin, has_side_effects=False)).lower(x)
  ...
  def test_module_documentation(self):
    ...
    logging.info(olympus.make_olympuspr(cos_tf_sin_olympus)(x))
  ...
  def test_tf_gather(self):
    ...
    fun_olympus = olympus2tf.call_tf(fun_tf)
    grad_fun_olympus = olympus.grad(fun_olympus)
    grad_res = grad_fun_olympus(operand, indices)
  ...
  def test_output_shape_dtype_none(self):
    ...
    fun_olympus_1 = olympus2tf.call_tf(fun_tf, output_shape_dtype=None)
    fun_olympus_2 = olympus2tf.call_tf(fun_tf)
    self.assertIsNone(fun_olympus_1(x))
    self.assertIsNone(fun_olympus_2(x))
    fun_olympus_3 = olympus2tf.call_tf(
        fun_tf, output_shape_dtype=olympus.ShapeDtypeStruct((10,), jnp.float32)
    )
    with self.assertRaisesRegex(
        ValueError,
  ...
  def test_output_shape_dtype_not_none(self):
    ...
    fun_olympus_1 = olympus2tf.call_tf(
        fun_tf, output_shape_dtype=olympus.ShapeDtypeStruct((10,), jnp.float32)
    )
    fun_olympus_2 = olympus2tf.call_tf(fun_tf)
    self.assertAllClose(fun_olympus_1(x), fun_olympus_2(x))

    fun_olympus_3 = olympus2tf.call_tf(fun_tf, output_shape_dtype=None)
    with self.assertRaisesRegex(
        ValueError,
        "The pytree of the TensorFlow function results does not match the"
        " pytree of the declared output_shape_dtype",
  ...
  def test_multi_platform(self):
    ...
    def f_olympus(x):
      return jnp.cos(olympus2tf.call_tf(tf_fun)(jnp.cos(x)))
    x = np.arange(12, dtype=np.float32).reshape((3, 4))

    # Find platforms that are available for both OLYMPUS and TF
    # Pick one device from each available platform
    olympus_platforms = []
    for backend in ["cpu", "gpu", "tpu"]:
      try:
        devices = olympus.devices(backend)
      except RuntimeError:
        devices = []
      if devices:
        olympus_platforms.append(devices[0].platform)

    olympus_and_tf_platforms = (
      set(olympus_platforms) & {d.device_type.lower()
                            for d in self.tf_devices})

    lowering_platforms = ("tpu", "cpu", "cuda")

    exp = export.export(olympus.jit(f_olympus),
                        platforms=lowering_platforms)(x)
    for olympus_platform in olympus_and_tf_platforms:
      with self.subTest(olympus_platform):
        olympus_device = olympus.devices(olympus_platform)[0]
        x_device = olympus.device_put(x, olympus_device)
        logging.info("Running harness natively on %s", olympus_device)
        native_res = f_olympus(x_device)
        logging.info("Running exported harness on %s", olympus_device)
  ...
  def test_multi_platform_call_tf_graph(self):
    ...
    def f_olympus(x):
      return jnp.cos(olympus2tf.call_tf(tf_fun,
                                    call_tf_graph=True,
                                    ordered=True)(jnp.cos(x)))
    x = np.arange(12, dtype=np.float32).reshape((3, 4))
    # When we use call_tf_graph we can serialize for multiple platforms
    lowering_platforms = ("tpu", "cpu", "cuda")
    # We must use olympus2tf.convert to run a call_tf(call_tf_graph)
    ...
    f_tf = tf.function(olympus2tf.convert(
      f_olympus,
    ...
    for tf_device in self.tf_devices:
      with self.subTest(tf_device.device_type):
        ...
        with tf.device(tf_device):
          res = f_tf(x)
        self.assertAllClose(res, f_olympus(x))
  ...
  def test_avoid_copy_between_gpu_and_cpu(self, type_):
    try:
      gpu_devices = olympus.devices("gpu")
    except RuntimeError:
      gpu_devices = []
    if not gpu_devices:
    ...
    olympus_array_on_gpu = jnp.zeros([1], type_, device=gpu_devices[0])
    ...
    def _transfer_guard(guard_level):
      with contextlib.ExitStack() as stack:
        stack.enter_context(olympus.transfer_guard_device_to_device(guard_level))
        stack.enter_context(olympus.transfer_guard_device_to_host(guard_level))
        if type_ != jnp.int32:
          stack.enter_context(olympus.transfer_guard_host_to_device(guard_level))
        yield

    with _transfer_guard("disallow_explicit"):
      olympus2tf.call_tf(tf_fun)(olympus_array_on_gpu)


@jtu.thread_unsafe_test_class()
class RoundTripToOlympusTest(tf_test_util.OlympusToTfTestCase):
  """Reloading output of olympus2tf into OLYMPUS with call_tf."""
  ...
  def test_simple(self):
    f_olympus = jnp.sin
    f_olympus_rt = olympus2tf.call_tf(olympus2tf.convert(f_olympus))
    x = np.float32(0.7)
    self.assertAllClose(f_olympus(x), f_olympus_rt(x))

  def test_pytree(self):
    def f_olympus(x):  # x: dict(a=f32, b=f32)
  ...
  def test_custom_grad(self):
    ...
    f_rt = olympus2tf.call_tf(olympus2tf.convert(f, with_gradient=True))
    x = np.float32(0.7)
    self.assertAllClose(f(x), f_rt(x))
    self.assertAllClose(olympus.grad(f)(x), olympus.grad(f_rt)(x))
  ...
  def test_saved_model_simple(self):
    ...
    restored_olympus = olympus2tf.call_tf(restored_tf)
    self.assertAllClose(f_olympus(x), restored_olympus(x))

  def test_saved_model_variables(self):
    param = np.array([1., 2.], dtype=np.float32)
    x = np.array([0.7, 0.8], dtype=np.float32)
    def f_olympus(param, x):
    ...
    _, restored_model = tf_test_util.SaveAndLoadFunction(
        ...
        input_args=[x],
        variables=[param_v])
    restored_olympus = olympus2tf.call_tf(restored_model.f)
    self.assertAllClose(f_olympus(param, x), restored_olympus(x))
    self.assertAllClose(f_olympus(param, x), olympus.jit(restored_olympus)(x))
    self.assertAllClose(f_olympus(param, x), olympus2tf.convert(restored_olympus)(x))
    self.assertAllClose(f_olympus(param, x),
                        tf.function(olympus2tf.convert(restored_olympus),
                                    autograph=False)(x))
    self.assertAllClose(f_olympus(param, x),
                        tf.function(olympus2tf.convert(restored_olympus),
                                    autograph=True)(x))
  ...
  def test_saved_model_shape_poly(self):
    ...
    f_tf = olympus2tf.convert(f_olympus, polymorphic_shapes=["(b, ...)"])
    ...
    tracing_count = 0
    f_olympus_rt = olympus2tf.call_tf(restored_f)
    self.assertAllClose(res_olympus, f_olympus_rt(x))
    # Ensure that restored_f works at other batch size as well
    y = np.concatenate([x, x])
    self.assertEqual(0, tracing_count)
    res_olympus_y = f_olympus(y)
    self.assertEqual(1, tracing_count)
    # No more tracing for f_olympus_rt
    self.assertAllClose(res_olympus_y, f_olympus_rt(y))
    self.assertEqual(1, tracing_count)
  ...
  def test_custom_grad_saved_model(self):
    ...
    g_tf, _ = tf_test_util.SaveAndLoadFunction(
        olympus2tf.convert(g, with_gradient=True),
        input_signature=[tf.TensorSpec(shape=(1,), dtype=tf.float32)],
    )
    g_rt = olympus2tf.call_tf(g_tf)
    x = np.array([0.7], dtype=np.float32)
    self.assertAllClose(g(x), g_rt(x))
    self.assertAllClose(olympus.grad(g)(x), olympus.grad(g_rt)(x))
  ...
  def test_without_gradient_saved_model(self):
    ...
    f_tf, _ = tf_test_util.SaveAndLoadFunction(
        olympus2tf.convert(f_olympus, with_gradient=False),
        input_args=[x])
    f_rt = olympus2tf.call_tf(f_tf)

    self.assertAllClose(f_olympus(x), f_rt(x))
    with self.assertRaisesRegex(Exception,
                                "Gradient explicitly disabled.*olympus2tf-converted function does not support gradients. Use `with_gradient` parameter to enable gradients"):
      olympus.grad(f_rt)(x)
  ...
  def test_saved_model_no_gradients(self):
    ...
    x = np.array([0.7, 0.8], dtype=np.float32)
    f_tf, _ = tf_test_util.SaveAndLoadFunction(
        olympus2tf.convert(f_olympus, with_gradient=True), input_args=[x],
    ...
    with self.assertRaises(TypeError):
      _ = olympus.grad(f_rt)(x)

  def test_call_tf_under_function_context(self):
    def fun_olympus(x, y):
      z = olympus2tf.call_tf(tf.math.sin)(x) + jnp.cos(y)
      return z

    x = np.array([-1.0, 0.0, 1.0], dtype=np.float32)
    y = np.array([-0.5, 0.0, 0.5], dtype=np.float32)

    converted_fun = tf.function(olympus2tf.convert(fun_olympus))
  ...
  def test_all_floating_input_gradient(self, dtype):
    ...
    olympus_f = olympus2tf.call_tf(tf_f)
    tf_f_rt = olympus2tf.convert(olympus_f)
    ...
    def assert_all_close_support_bfloat16(baseline, candidate):
      ...
      baseline = olympus.tree_util.tree_map(conversion, baseline)
      candidate = olympus.tree_util.tree_map(conversion, candidate)
      tol = (
          1e-2
    ...
    grad_fun_olympus = olympus.grad(olympus2tf.call_tf(tf_f))
    grad_fun_olympus_rt = olympus2tf.call_tf(olympus2tf.convert(grad_fun_olympus))

    # Eager mode
    assert_all_close_support_bfloat16(grad_fun_olympus(x), grad_fun_olympus_rt(x))

    # Jit mode
    assert_all_close_support_bfloat16(
        olympus.jit(grad_fun_olympus)(x), olympus.jit(grad_fun_olympus_rt)(x)
    )
  ...
  def test_complex_input_gradient(self, dtype):
    ...
    # RoundTrip test for the gradient
    grad_fun_olympus = olympus.grad(olympus2tf.call_tf(tf_f), holomorphic=True)
    grad_fun_olympus_rt = olympus2tf.call_tf(olympus2tf.convert(grad_fun_olympus))

    # Eager mode
    self.assertAllClose(grad_fun_olympus(x), grad_fun_olympus_rt(x))

    # Jit mode
    self.assertAllClose(olympus.jit(grad_fun_olympus)(x), olympus.jit(grad_fun_olympus_rt)(x))
  ...
  def test_grad_pytree_arg_with_none_leaf(self):
    ...
    y = jnp.array(2.0)
    actual = olympus.grad(
        olympus2tf.call_tf(tf_f), argnums=(1,))(x, {"y": y, "other": None})
    self.assertDictEqual(actual[0], {"y": x, "other": None})


@jtu.thread_unsafe_test_class()
class RoundTripToTfTest(tf_test_util.OlympusToTfTestCase):
  """Reloading output of call_tf into TF with olympus2tf."""
  ...
  def test_alternate(self):
    # Alternate sin/cos with sin in TF and cos in OLYMPUS
    f_tf_inner = tf.math.sin
    def f_olympus(x_olympus):
      y_olympus = jnp.cos(x_olympus)
      z_olympus = olympus2tf.call_tf(f_tf_inner)(y_olympus)
      return jnp.cos(z_olympus)
    def f_tf_outer(x_tf):
      y_tf = tf.math.sin(x_tf)
      z_tf = olympus2tf.convert(f_olympus)(y_tf)
  ...
  def test_saved_model(self):
    ...
    fun_tf_rt = olympus2tf.convert(fun_olympus)
    res = fun_tf_rt(x)
  ...
  def test_function_dynamic_shape(self):
    ...
    # olympus2tf.convert does abstract evaluation.
    with self.assertRaisesRegex(ValueError, _call_tf_dynamic_shape_error):
      fun_tf_rt = olympus2tf.convert(olympus2tf.call_tf(fun_tf))
      fun_tf_rt(x)
  ...
  def test_shape_poly_error_no_output_shape_dtype(self, with_jit=True):
    ...
    def fun_olympus(x):
      return olympus2tf.call_tf(tf.math.sin)(x)

    fun_tf_rt = _maybe_tf_jit(with_jit,
        olympus2tf.convert(fun_olympus, polymorphic_shapes=["b, ..."]))
    with self.assertRaisesRegex(ValueError, _call_tf_dynamic_shape_error):
      fun_tf_rt(x)
  ...
  def test_shape_poly_error_mismatch_output_shape_dtype_tree(self, with_jit=False):
    ...
    def fun_olympus(x):
      return olympus2tf.call_tf(tf.math.sin,
          output_shape_dtype=(olympus.ShapeDtypeStruct(x.shape, x.dtype),
                              olympus.ShapeDtypeStruct(x.shape, x.dtype)))(x)
  ...
  def test_shape_poly_error_mismatch_output_shape_dtype(self, with_jit=False, kind="bad_rank"):
    ...
    if kind == "bad_rank":
      def fun_olympus(x):
        return olympus2tf.call_tf(lambda x: x,
                              # Wrong shape rank
                              output_shape_dtype=olympus.ShapeDtypeStruct((), x.dtype))(x)
    elif kind == "bad_dim":
      def fun_olympus(x):
        bad_shape = (5 + x.shape[0],)
        y = olympus2tf.call_tf(lambda x: x,
                           # Wrong dimension
                           output_shape_dtype=olympus.ShapeDtypeStruct(bad_shape, x.dtype))(x)
        # OLYMPUS will believe that the following is Ok, leading to downstream error in TF
        return y + jnp.ones(bad_shape, dtype=x.dtype)
    elif kind == "bad_dtype":
      def fun_olympus(x):
        return olympus2tf.call_tf(lambda x: x,
                              output_shape_dtype=olympus.ShapeDtypeStruct(x.shape, np.int32))(x)
    elif kind == "bad_dtype_x64":
      def fun_olympus(x):
        return olympus2tf.call_tf(lambda x: x * np.float64(3.),
                              output_shape_dtype=olympus.ShapeDtypeStruct(x.shape, np.float64))(x)
    else:
      assert False
    ...
    fun_tf_rt = _maybe_tf_jit(with_jit, olympus2tf.convert(fun_olympus))
  ...
  def test_several_round_trips(self,
    ...
    f2_tf = lambda x: x * olympus2tf.convert(f(1))(x)
    if f2_function:
    ...
    self.assertAllClose(olympus.grad(f(2))(x), g_f2_ft.numpy())

    f3_olympus = lambda x: x * olympus2tf.call_tf(f2_tf)(x)
    self.assertAllClose(f(3)(x), f3_olympus(x))
    self.assertAllClose(f(3)(x), olympus.jit(f3_olympus)(x))
    self.assertAllClose(olympus.grad(f(3))(x), olympus.grad(f3_olympus)(x))

    f4_tf = lambda x: x * olympus2tf.convert(f3_olympus)(x)
    self.assertAllClose(f(4)(x), f4_tf(x).numpy())
    _, (g_f4_ft,) = tf_test_util.ComputeTfValueAndGrad(f4_tf, [x])
    self.assertAllClose(olympus.grad(f(4))(x), g_f4_ft.numpy())
  ...
  def test_call_tf_graph(self):
    ...
    f_olympus = olympus.jit(olympus2tf.call_tf(tf_func_3, call_tf_graph=False))
    stablehlo_module = f_olympus.lower(x, y, z).compiler_ir("stablehlo")
    self.assertNotIn("stablehlo.custom_call", str(stablehlo_module))

    f_olympus = olympus.jit(
        olympus2tf.call_tf(
            tf_func_3,
            call_tf_graph=True,
        )
    ...
    with self.assertRaisesRegex(
        ValueError,
        "call_tf_graph=True only support exporting by olympus2tf.convert currently",
    ):
      stablehlo_module = f_olympus.lower(x, y, z).compiler_ir("stablehlo")
  ...
  @parameterized.named_parameters(
      dict(
          ...
          output_shape_dtype=olympus.ShapeDtypeStruct((10,), jnp.float32),
      ),
      dict(
  ...
  def test_call_tf_graph_non_compilable(self, tf_f, output_shape_dtype):
    ...
    # Eager mode
    self.assertAllClose(tf_f(inputs), olympus_f(inputs))
    ...
    with self.assertRaisesRegex(
      ...
      stablehlo_module = olympus.jit(olympus_f).lower(inputs).compiler_ir("stablehlo")
    if stablehlo_module:
      self.assertIn(
    ...
    # If OLYMPUS calls same tensorflow function by `olympus2tf.call_tf` twice,
    # it should return two different tf concrete functions.
    def olympus_f_2(x):
      res1 = olympus2tf.call_tf(
          tf_f,
          call_tf_graph=True,
          output_shape_dtype=output_shape_dtype,
      )(x)
      res2 = olympus2tf.call_tf(
          tf_f,
          call_tf_graph=True,
          output_shape_dtype=output_shape_dtype,
      )(x)
      return res1, res2
    stablehlo_module = None
    with self.assertRaisesRegex(ValueError, "call_tf_graph=True only support exporting by olympus2tf.convert currently"):
      stablehlo_module = olympus.jit(olympus_f_2).lower(inputs).compiler_ir("stablehlo")
    if stablehlo_module:
      self._walk_stablehlo_operations(stablehlo_module, _extract_info)
    xla_call_module_list.clear()
  ...
  def test_b279454591(self):
    ...
    olympus_f_2 = olympus2tf.call_tf(tf.function(tf_f_2), call_tf_graph=True)
    tf_f_rt_2 = olympus2tf.convert(
        olympus_f_2,
        with_gradient=False,
    )
  ...
  def test_call_tf_graph_ordered(self, *, version: int):
    with config.olympus_export_calling_convention_version(version):
      logging.info(
        "Using OLYMPUS serialization version %s",
        olympus.config.olympus_export_calling_convention_version)

      @tf.function
      def tf_print(x):
        tf.print(x)

      call_tf_print = olympus2tf.call_tf(
          tf_print,
          call_tf_graph=True,
          ordered=True,
      )
      ...
      @olympus.jit
      def f_olympus(x):
        return olympus.lax.fori_loop(0, 4, body, x)
  ...
  def test_call_tf_ordered_dead_inputs(self, *, poly: bool, version: int):
    with config.olympus_export_calling_convention_version(version):
      logging.info(
        "Using OLYMPUS serialization version %s",
        olympus.config.olympus_export_calling_convention_version)
      def f_olympus(x1, x_dead, x3):
        return (x1, olympus2tf.call_tf(lambda x: tf.math.sin(x), ordered=True,
                                  call_tf_graph=True)(x3))
      if poly:
        polymorphic_shapes = ["b", None, None]
      else:
        polymorphic_shapes = None
      f_tf = olympus2tf.convert(f_olympus, polymorphic_shapes=polymorphic_shapes)
      x1 = np.arange(3, dtype=np.float32)
      x_dead = np.arange(4, dtype=np.float32)
      x3 = np.arange(5, dtype=np.float32)
      self.assertAllClose(f_olympus(x1, x_dead, x3),
                          f_tf(x1, x_dead, x3))
  ...
  def test_call_tf_graph_polymorphic(self, ordered: bool, version: int):
    with config.olympus_export_calling_convention_version(version):
      ...
      @tf.function(jit_compile=True, autograph=False)
      @partial(olympus2tf.convert,
        with_gradient=False,
        polymorphic_shapes=["(b)"])
      @olympus.jit
      def tf_f_2(x):
        tf_f = lambda x: print(tf.strings.length(tf.constant("hello, world")))
        olympus2tf.call_tf(tf_f,
                      call_tf_graph=True,
                      ordered=ordered)(x)
        return x
  ...
  @unittest.skip(
      ...
      " depends on OLYMPUS call_tf lowering. The 2nd time tf.SavedModel TF tracing"
      " will not trigger call_tf tracing since it was already cached. The"
      " solution is to create the `CallTFContext` to make TF tracing and OLYMPUS"
  ...
  def test_call_tf_graph_save_and_load(self):
    def olympus_func(x):
      ...
      return jnp.cos(
          olympus2tf.call_tf(func_tf, output_shape_dtype=x, call_tf_graph=True)(x)
      )
    data_inputs = (np.array([0.5, 0.7], dtype=np.float32),)

    def tf_func(the_input):
      res = olympus2tf.convert(olympus_func)(the_input)
    ...
    self.assertAllClose(olympus_func(*data_inputs), res)
