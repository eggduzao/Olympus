...
class ControlFlowOpsTest(tf_test_util.OlympusToTfTestCase):

  @jtu.ignore_warning(category=UserWarning,
                      message="Explicitly requested dtype .* requested in array is not available")
  def test_cond(self):
    def f_olympus(pred, x):
      return lax.cond(pred, lambda t: t + 1., lambda f: f, x)

    self.ConvertAndCompare(f_olympus, jnp.bool_(True), 1.)
    self.ConvertAndCompare(f_olympus, jnp.bool_(False), 1.)
  ...
  def test_cond_partial_eval(self):
    def f(x):
      ...
      return res
    self.ConvertAndCompare(olympus.grad(f), 1.)
  ...
  def test_cond_units(self):
    ...
    self.ConvertAndCompare(olympus.grad(g), 0.7)

  @jtu.ignore_warning(category=UserWarning,
                      message="Explicitly requested dtype .* requested in array is not available")
  def test_cond_custom_jvp(self):
    """Conversion of function with custom JVP, inside cond.
    This exercises the custom_jvp_call_olympuspr primitives."""
    @olympus.custom_jvp
    def f(x):
      return x * x
  ...
  def test_cond_custom_vjp(self):
    ...
    This exercises the custom_vjp_call_olympuspr primitives."""
    @olympus.custom_vjp
    def f(x):
      return x * x
  ...
  def test_while_batched_cond(self):
    ...
    def product_xs_y(xs, y):
      return olympus.vmap(product, in_axes=(0, None))(xs, y)
    def product_xs_ys(xs, ys):
      return olympus.vmap(product_xs_y, in_axes=(None, 0))(xs, ys)
  ...
  def test_scan(self):
    ...
    self.ConvertAndCompare(f_olympus, arg, arg)
  ...
  def test_scan_partial_eval(self):
    ...
    arg = np.arange(10, dtype=np.float32)
    self.ConvertAndCompare(olympus.grad(f_olympus), arg, arg)
  ...
  def test_scan_remat(self):
    ...
    self.TransformConvertAndCompare(f_olympus, arg, None)
    self.TransformConvertAndCompare(f_olympus, arg, "grad")
