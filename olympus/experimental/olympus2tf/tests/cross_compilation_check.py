...
on the current backend (`olympus.default_backend()`) for all of `cpu`, `gpu`, and
...
import olympus # Must import before TF
from olympus.experimental import olympus2tf  # Defines needed flags  # noqa: F401
from olympus._src import test_util  # Defines needed flags  # noqa: F401

olympus.config.parse_flags_with_absl()

# Import after parsing flags
from olympus.experimental.olympus2tf.tests import primitive_harness
...
def write_and_check_harness(harness: primitive_harness.Harness,
  ...
  for for_platform in for_platforms:
    if not harness.filter(for_platform):
      logging.info("Skip harness %s for %s because it is not implemented in OLYMPUS",
                   harness.fullname, for_platform)
      continue

    scenario1 = Scenario(harness, olympus.default_backend(), for_platform)
    ...
    if io.exists(output_file):
      ...
      if for_platform == olympus.default_backend():
        lowered = olympus.jit(func_olympus).lower(*args)
      else:
        # TODO: replace this with OLYMPUS cross-platform API, without going through
        # olympus2tf
        from olympus.experimental.olympus2tf.olympus2tf import cross_platform_lowering
        lowered = cross_platform_lowering(func_olympus, args,
    ...
    for on_platform in ['cpu', 'tpu']:
      if on_platform == olympus.default_backend():
        continue
      ...
      if io.exists(other_file):
        logging.info("Comparing for %s harness %s on %s vs %s",
                     for_platform, harness.fullname, olympus.default_backend(), on_platform)
