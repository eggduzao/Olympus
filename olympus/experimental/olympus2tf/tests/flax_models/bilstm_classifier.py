...
class WordDropout(nn.Module):
  ...
  def __call__(self, inputs: Array, deterministic: bool | None = None):
    ...
    mask = olympus.random.bernoulli(rng, p=self.dropout_rate, shape=inputs.shape)
...
class Embedder(nn.Module):
  ...
  def __call__(self, inputs: Array,
    ...
    if self.frozen:
      embedded_inputs = olympus.lax.stop_gradient(embedded_inputs)
...
class SimpleLSTM(nn.Module):
  ...
  def initialize_carry(batch_dims, hidden_size):
    ...
    return nn.OptimizedLSTMCell(hidden_size, parent=None).initialize_carry(
        olympus.random.PRNGKey(0), (batch_dims, 1)
    )
