...
r"""Tests for converting Flax models using olympus2tf.
...
python3 models_test_main.py \
    ...
    --converters=olympus2tf_noxla_tfjs \
...
from olympus.core import InconclusiveDimensionOperation
from olympus.experimental.olympus2tf.tests.model_harness import ALL_HARNESSES
from olympus.experimental.olympus2tf.tests.converters import ALL_CONVERTERS
...
def _get_random_data(x: olympus.Array) -> np.ndarray:
    ...
    for converter in converters:
      ...
      try:
        ...
        if not converter.compare_numerics:
          ...
          xs = [olympus.tree_util.tree_map(_get_random_data, x) for x in harness.inputs]
          olympus_result = harness.apply_with_vars(*xs)
          try:
            tf_result = apply_tf(*xs)
            olympus.tree.map(np_assert_allclose, olympus_result, tf_result)
