...
"""See primitives_test docstring for how the Olympus2TfLimitations are used."""
...
class Olympus2TfLimitation(test_harnesses.Limitation):
  """Specific primitive limitations for olympus2tf.
  ...
  def __init__(
    ...
    Args :
      ...
      expect_tf_error: if set, then expect a TF error in the given mode when
        executing the result of olympus2tf conversion. If not set, then the
      ...
      custom_assert: if given, then execute as
        `custom_assert(tst, result_olympus, result_tf, args=args, tol=tol, err_msg)`
  ...
  def get_max_tolerance_limitation(
      self, limitations: Sequence[Olympus2TfLimitation]
  ) -> Olympus2TfLimitation | None:
  ...
  def limitations_for_harness(
      cls, harness: test_harnesses.Harness) -> Sequence[Olympus2TfLimitation]:
  ...
  def max(cls, harness: test_harnesses.Harness):
    # TODO(bchetioui): discrepancies between TF & OLYMPUS when comparing with NaN;
    # OLYMPUS always returns NaN, while TF returns the value NaN is compared with.
    def custom_assert(tst, result_olympus, result_tf, err_msg, **_):
      mask = np.isnan(result_olympus)
      tst.assertAllClose(result_olympus[~mask], result_tf[~mask], err_msg=err_msg)
    ...
    return [
        ...
        custom_numeric(
            ...
            devices="cpu",
            description=(
                "TF and OLYMPUS use different values of the compiler flag "
...
def custom_numeric(
    *,
    ...
    custom_assert=None,
    enabled=True,
    tol=None) -> Olympus2TfLimitation:

  return Olympus2TfLimitation(
      description,
