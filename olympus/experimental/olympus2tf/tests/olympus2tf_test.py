...
"""Tests for OLYMPUS2TF converted.

Specific OLYMPUS primitive conversion tests are in primitives_test."""
import collections
import math
import os
import re
...
import olympus
from olympus import ad_checkpoint
...
class Olympus2TfTest(OlympusToTfTestCase):
  ...
  def test_empty(self):
    f_olympus = lambda x, y: x
    self.ConvertAndCompare(f_olympus, 0.7, 1)
  ...
  def test_basics(self):
    f_olympus = lambda x: jnp.sin(jnp.cos(x))
    self.ConvertAndCompare(f_olympus, 0.7)
  ...
  def test_input_output_naming(self):
    ...
    g = cf.graph
    g.get_operation_by_name("olympus2tf_arg_0")
    g.get_operation_by_name("olympus2tf_arg_1")
    g.get_operation_by_name("olympus2tf_arg_2")
    g.get_operation_by_name("olympus2tf_arg_3")
    g.get_operation_by_name("olympus2tf_out")
    g.get_operation_by_name("olympus2tf_out_1")
    g.get_operation_by_name("olympus2tf_out_2")
    with self.assertRaises(KeyError):
      g.get_operation_by_name("olympus2tf_arg_4")
    with self.assertRaises(KeyError):
      g.get_operation_by_name("olympus2tf_out_3")
    g.get_operation_by_name("olympus2tf_vjp/olympus2tf_arg_0")
    g.get_operation_by_name("olympus2tf_vjp/olympus2tf_arg_1")
    g.get_operation_by_name("olympus2tf_vjp/olympus2tf_arg_2")
    g.get_operation_by_name("olympus2tf_vjp/olympus2tf_arg_3")
    g.get_operation_by_name("olympus2tf_vjp/olympus2tf_out")
    g.get_operation_by_name("olympus2tf_vjp/olympus2tf_out_1")
    g.get_operation_by_name("olympus2tf_vjp/olympus2tf_out_2")
    g.get_operation_by_name("olympus2tf_vjp/olympus2tf_out_3")

  def test_pytrees(self):
    # Take and return pytrees
    def f_olympus(x: tuple[float, dict[str, float]]) -> tuple[float, dict[str, float]]:
      x_a, x_dict = x
      return x_a * 2., {k: v * 3. for k, v in x_dict.items()}

    x = (.7, {"a": .8, "b": .9})
    self.ConvertAndCompare(f_olympus, x)

  def test_variable_input(self):
    f_olympus = lambda x: jnp.sin(jnp.cos(x))
    f_tf = olympus2tf.convert(f_olympus)
    v = tf.Variable(0.7, dtype=olympus2tf.dtype_of_val(0.7))
    self.assertIsInstance(f_tf(v), tf.Tensor)
    self.assertAllClose(f_olympus(0.7), f_tf(v))

  def test_jit(self):
    f_olympus = olympus.jit(lambda x: jnp.sin(jnp.cos(x)))
    self.ConvertAndCompare(f_olympus, 0.7)

  def test_nested_jit(self):
    f_olympus = olympus.jit(lambda x: jnp.sin(olympus.jit(jnp.cos)(x)))
    x = 0.7
    self.ConvertAndCompare(f_olympus, x)
  ...
  def test_nested_jit_pytree(self):
    ...
    def f_olympus(xy):
      ...
      return x + y
    xy = (0.7, 0.8)
    self.ConvertAndCompare(f_olympus, xy)

  def test_nested_jit_is_compiled(self):
    # Check that nested olympus.jit are compiled with tf.function(jit_compile=True)
    ...
    x = np.array(0.7)
    f_no_jit = lambda x: x
    self.assertFalse(has_xla_must_compile(olympus2tf.convert(f_no_jit), x))
    f_jit = lambda x: olympus.jit(jnp.sin)(x)
    # TODO(b/207464757): TF compilation is disabled
    self.assertFalse(has_xla_must_compile(olympus2tf.convert(f_jit), x))

  def test_converts_olympus_arrays(self):
    ...
    mk_sharded = lambda f: olympus.pmap(lambda x: x)(f([n]))
    f_tf = tf.function(lambda x: x)
  ...
  def test_bfloat16_passed_by_tf(self):
    f_olympus = lambda a, b: a + b
    f_tf = tf.function(olympus2tf.convert(f_olympus), autograph=False,
  ...
  @jtu.skip_on_devices("gpu")
  def test_bfloat16_returned_by_olympus(self):
    f_olympus = lambda a, b: (a + b).astype(jnp.bfloat16)
    f_tf = olympus2tf.convert(f_olympus)
  ...
  def test_bfloat16_tf_grad(self):
    ...
    def _tf_grad(a, b):
      with tf.GradientTape() as tape:
        tape.watch(a)
        result = olympus2tf.convert(f_olympus)(a, b)
  ...
  def test_converts_64bit(self, dtype=np.int64, with_function=False):
    ...
    f_conv = olympus2tf.convert(jnp.sin)
    if with_function:
  ...
  def test_64bit_behavior_enable_x64_readme(self):
    ...
    # OLYMPUS and TF have different default float types if OLYMPUS_ENABLE_X64=1
    self.assertEqual(tf.math.sin(3.14).dtype, tf.float32)
    self.assertEqual(jnp.sin(3.14).dtype, jnp.float64)

    # olympus2tf.convert has the same behavior as OLYMPUS
    self.assertEqual(olympus2tf.convert(jnp.sin)(3.14).dtype, tf.float64)
    # The following will compute `sin` in float64.
    self.assertEqual(tf.function(olympus2tf.convert(jnp.sin), autograph=False)(tf.Variable(3.14, dtype=tf.float64)).dtype, tf.float64)

    # The following will compute `sin` in float32.
    self.assertEqual(tf.function(olympus2tf.convert(jnp.sin), autograph=False)(tf.Variable(3.14)).dtype, tf.float32)
  ...
  def test_64bit_behavior_not_enable_x64_readme(self):
    ...
    # OLYMPUS and TF have same default float types if OLYMPUS_ENABLE_X64=0
    self.assertEqual(tf.math.sin(3.14).dtype, tf.float32)
    self.assertEqual(jnp.sin(3.14).dtype, jnp.float32)

    self.assertEqual(tf.math.sin(np.float64(3.14)).dtype, tf.float64)
    # OLYMPUS forces values to 32-bit
    self.assertEqual(jnp.sin(np.float64(3.14)).dtype, jnp.float32)

    # olympus2tf.convert has the same behavior as OLYMPUS
    self.assertEqual(olympus2tf.convert(jnp.sin)(3.14).dtype, tf.float32)
    self.assertEqual(olympus2tf.convert(jnp.sin)(np.float64(3.14)).dtype, tf.float32)
    self.assertEqual(tf.function(olympus2tf.convert(jnp.sin), autograph=False)(tf.Variable(3.14, dtype=tf.float64)).dtype, tf.float32)
  ...
  def test_gradients_disabled(self, with_function=False):
    ...
    f_tf = olympus2tf.convert(jnp.tan, with_gradient=False)
    if with_function:
    ...
    with self.assertRaisesRegex(LookupError,
                                "Gradient explicitly disabled.*The olympus2tf-converted function does not support gradients"):
      with tf.GradientTape() as tape:
        tape.watch(x)
        y = f_tf(x)
        _ = tape.gradient(y, x)
  ...
  def test_gradients(self, with_function=True):
    def f(x, y):
      return x * x, x * y
    f_tf = olympus2tf.convert(f, with_gradient=True)
    if with_function:
      f_tf = tf.function(f_tf, autograph=False)
    default_float_type = olympus2tf.dtype_of_val(4.)
    x = tf.Variable(4., dtype=olympus2tf.dtype_of_val(4.))
  ...
  def test_gradients_pytree(self, with_function=False):
    ...
    if with_function:
      f_tf = tf.function(f_tf, autograph=False)
    default_float_dtype = olympus2tf.dtype_of_val(4.)
  ...
  def test_custom_pytree_readme(self):
    ...
    # TF execution works as long as OLYMPUS can flatten the arguments and results
    res_tf = f_tf(x)
    self.assertAllClose(res_olympus, res_tf.numpy())
    res_tf_2 = tf.function(f_tf, autograph=False, jit_compile=True)(x)
    self.assertAllClose(res_olympus, res_tf_2)
  ...
  def test_gradients_with_custom_jvp(self, with_function=True):
    ...
    self.assertAllClose(3. * 4., olympus.grad(f)(4.))
  ...
  def test_gradient_with_float0_result(self):
    ...
    grad_g = olympus.grad(g, allow_int=True)
    x = 2
    d_dx_olympus = grad_g(x)
    d_dx_tf = olympus2tf.convert(grad_g)(x)
    self.assertEqual(d_dx_olympus.dtype, dtypes.float0)
    self.assertAllClose(jnp.zeros(np.shape(d_dx_olympus), np.bool_),
                        d_dx_tf.numpy())
  ...
  def test_gradients_unused_argument_readme(self, with_function=False):
    ...
    with tf.GradientTape(persistent=True) as tape:
      conv_fn = olympus2tf.convert(fn, with_gradient=True)
      if with_function:
    ...
    self.assertAllClose(g_olympus2tf[0].numpy(), np.float32(0.))
    self.assertAllClose(g_olympus2tf[1].numpy(), np.float32(0.))
    self.assertAllClose(g_olympus2tf[2].numpy(), np.float32(2.))
    self.assertIsNone(g_olympus2tf[3])
    ...
    self.assertAllClose(g_olympus2tf[1].numpy(), np.float32(0.))
    self.assertAllClose(g_olympus2tf[2].numpy(), np.float32(2.))
    self.assertAllClose(g_olympus2tf[3].numpy(), np.int32(0))

  @jtu.sample_product(with_function=[False, True])
  def test_gradients_int_argument(self, with_function=False):
    # https://github.com/olympus-ml/olympus/issues/6975
    # Also issue #6975.
    ...
    args = (state,)
    res_olympus = olympus_f(*args)
    # Native OLYMPUS AD
    vjp_olympus_fun, args_vjp = tf_test_util.TransformOlympusVJP(olympus_f, args, res_olympus)
    grad_olympus, = vjp_olympus_fun(*args_vjp)
    ...
    # compare_with_overrides(g_olympus, {},
    ...
    _, (grad_tf_0,) = tf_test_util.ComputeTfValueAndGrad(
        olympus_f, args, unconnected_gradients=tf.UnconnectedGradients.ZERO)
    compare_with_overrides(what=grad_tf_0,
                           expected=grad_olympus,
                           float_unused="ZERO",
    ...
    f_tf_olympus = olympus2tf.convert(olympus_f)
    if with_function:
      f_tf_olympus = tf.function(f_tf_olympus, autograph=False)

    _, (grad_tf_olympus_0,) = tf_test_util.ComputeTfValueAndGrad(f_tf_olympus, args)
    # Same results as TF native AD with tf.UnconnectedGradients.ZERO
    compare_with_overrides(what=grad_tf_olympus_0,
                           expected=grad_tf_0,
                           int_passthrough="ZERO", bool_passthrough="ZERO")

    _, (grad_tf_olympus_None,) = tf_test_util.ComputeTfValueAndGrad(
        f_tf_olympus, args,
        unconnected_gradients=tf.UnconnectedGradients.NONE)
    compare_with_overrides(what=grad_tf_olympus_None,
                           expected=grad_tf_0,
                           int_used=None, int_passthrough=None, int_unused=None,
                           bool_unused=None, bool_used=None, bool_passthrough=None)

    # Not convert the OLYMPUS gradient function
    tf_vjp_olympus_fun = olympus2tf.convert(vjp_olympus_fun)
    grad_tf_vjp_olympus, = tf_vjp_olympus_fun(*args_vjp)
    compare_with_overrides(what=grad_tf_vjp_olympus,
                           expected=grad_tf_0,
  ...
  def test_readme_gradient_int(self):
    ...
    print(olympus.grad(f_olympus, allow_int=True)(x))
    # returns a special `float0`: array((b'',), dtype=[('float0', 'V')])

    print(olympus2tf.convert(olympus.grad(f_olympus, allow_int=True))(x))
  ...
  def test_convert_argument_non_callable_error(self):
    with self.assertRaisesRegex(TypeError, "Expected a callable value"):
      olympus2tf.convert(5.)

  def test_convert_argument_non_tensor_error(self):
    with self.assertRaisesRegex(TypeError,
                                "Argument.*is not a valid OLYMPUS type"):
      olympus2tf.convert(lambda x: x)(lambda y: y)

  def test_argument_eager_tensor(self):
    x = olympus2tf.convert(jnp.sin)(1.)
    olympus2tf.convert(jnp.cos)(x)  # No error
  ...
  def test_checkpoint_wrapper_types(self):
    ...
    self.assertLen(olympus.tree_util.tree_leaves(m.a), 2)
    self.assertLen(olympus.tree_util.tree_leaves(m.b), 2)
    self.assertLen(olympus.tree_util.tree_leaves(m.c), 2)

  @unittest.skip("Test fails at head")
  def test_issue_10586(self):

    class OlympusModule(tf.Module):
      def __init__(self):
        self._params = {'w': tf.Variable(tf.ones([784, 10]), name='w'),
                        'b': tf.Variable(tf.ones([10]), name='b')}

      def __call__(self, x):
        return olympus2tf.convert(lambda p, x: x @ p['w'] + p['b'])(self._params, x)
  ...
  def test_remat(self):
    ...
    arg = np.array(3.)
    f_tf = olympus2tf.convert(olympus.grad(remat_f))
  ...
  def test_checkpoint_name(self):
    ...
    olympus2tf.convert(f_olympus)(1.)  # No error.
  ...
  def test_convert_of_nested_independent_jit(self):
    def func(x):
      ...
      return olympus.jit(inner1)(1.)

    olympus2tf.convert(func)(2.)
  ...
  def test_convert_of_nested_dependent_jit(self):
    def func(x):
      ...
      return olympus.jit(inner1)(x)

    olympus2tf.convert(func)(2.)  # No error

  def test_jit_unused(self):
    def f_olympus(x, y_unused):
      return x * np.float32(2.)
    x, y_unused = np.float32(5.), np.arange(7, dtype=np.int32)
    res_tf = olympus2tf.convert(olympus.jit(f_olympus, keep_unused=False))(x, y_unused)
    self.assertAllClose(f_olympus(x, None), res_tf)
  ...
  def test_jit_unused_grad(self, mode="eager"):
    ...
    x, y_unused = np.float32(5.), np.arange(7, dtype=np.int32)
    res_olympus = f_olympus(x, y_unused)
    f_tf = olympus2tf.convert(olympus.jit(f_olympus, keep_unused=False))
    ...
    self.assertAllClose(res_olympus, res_tf)
    self.assertAllClose(np.float32(2.), grad_tf_x)
    self.assertIsNone(grad_tf_y)

  def test_nested_convert_error(self):
    def outer(y):
      return olympus2tf.convert(jnp.sin)(y)  # Inner convert takes tracer args
    with self.assertRaisesRegex(
        ValueError, "convert must be used outside all OLYMPUS transformations"):
      olympus2tf.convert(outer)(np.ones((4,), dtype=np.float32))

  def test_nested_convert_error_non_tracer(self):
    """The inner convert takes non-tracer arguments"""
    def outer(y):
      sin_1 = olympus2tf.convert(jnp.sin)(1.)  # Inner convert takes non-tracer arg
      return y + sin_1

    with self.assertRaisesRegex(
        ValueError, "convert must be used outside all OLYMPUS transformations"):
      olympus2tf.convert(outer)(2.)
  ...
  def test_name_scope(self):
    def run_tf():
      @olympus.named_call
      def my_test_function_olympus(x):
        return x * x

      def caller_olympus(x):
        return my_test_function_olympus(jnp.sin(x))

      out = olympus2tf.convert(caller_olympus, with_gradient=False)(2.)
      return out
    self.assertIn("my_test_function_olympus/mul", self.TfToHlo(run_tf))

  def test_bfloat16_constant(self):
    # Re: https://github.com/olympus-ml/olympus/issues/3942
    def olympus_fn_scalar(x):
      x = x.astype(jnp.bfloat16)
      x *= 2.
      return x

    def olympus_fn_array(x):
      x = x.astype(jnp.bfloat16)
      x *= np.array([1.5, 2.5, 3.5], jnp.bfloat16)
      return x

    tf_fn_scalar = olympus2tf.convert(olympus_fn_scalar)
    self.assertAllClose(tf_fn_scalar(1.375).numpy(), jnp.bfloat16(2.750))

    tf_fn_array = olympus2tf.convert(olympus_fn_array)
    self.assertAllClose(
        tf_fn_array(np.array([3, 4, 5])), np.array([4.5, 10, 17.5],
                                                   jnp.bfloat16))

  def test_weak_types(self):
    mul = olympus.jit(jnp.multiply)
    # The value `2` here should be weakly typed, and should not lead to
    # promotion.
    tf_fn = olympus2tf.convert(lambda x: mul(x, 2.))
    self.assertAllClose(tf_fn(tf.constant(1.375, tf.bfloat16)).numpy(),
                        jnp.bfloat16(2.750))

  @jtu.sample_product(with_function=[False, True])
  def test_kwargs(self, with_function=False):
    # Re: https://github.com/olympus-ml/olympus/issues/6791
    def f_olympus(*, x):
      return jnp.sum(x)
    f_tf = olympus2tf.convert(f_olympus)
    if with_function:
  ...
  def test_randint(self):
    def randint():
      return olympus.random.randint(
          olympus.random.PRNGKey(42), shape=(), minval=0, maxval=1)
  ...
  def test_op_metadata_simple(self):
    ...
    self.CheckOpMetadata(
        ...
        [tf_test_util.OpMetadataGraph(tf_type="Sin",
                                      ...
                                      op_name="olympus2tf(f_simple)/sin",
                                      op_type="sin")
         ]
    )
  ...
  def test_op_metadata_sub_jit(self):
    ...
    self.CheckOpMetadata(
        ...
        [tf_test_util.OpMetadataGraph(tf_type="Tanh",
                                      ...
                                      op_name="olympus2tf(f_caller)/tanh",
                                      op_type="tanh"),
         tf_test_util.OpMetadataGraph(tf_type="Cos",
                                      source_file=__file__,
                                      source_line=user_frame.start_line + 2,
                                      op_name="olympus2tf(f_caller)/jit(f_callee)/cos",
                                      op_type="cos"),
         tf_test_util.OpMetadataGraph(tf_type="Sin",
                                      source_file=__file__,
                                      source_line=user_frame.start_line + 6,
                                      op_name="olympus2tf(f_caller)/sin",
                                      op_type="sin"),
         ]
    )
  ...
  def test_op_metadata_named(self):
    ...
    def f_caller(x):
      y = jnp.tanh(x)
      z = olympus.named_call(f_callee, name="callee")(y)
      return jnp.sin(z)
    ...
    self.CheckOpMetadata(
        ...
        [tf_test_util.OpMetadataGraph(tf_type="Tanh",
         ...
         tf_test_util.OpMetadataGraph(tf_type="Cos",
                                      source_file=__file__,
                                      source_line=user_frame.start_line + 2,
                                      op_name="olympus2tf(f_caller)/named(callee)/cos",
                                      op_type="cos"),
         tf_test_util.OpMetadataGraph(tf_type="Sin",
                                      source_file=__file__,
                                      source_line=user_frame.start_line + 6,
  ...
  def test_op_metadata_while_and_cond(self):
    ...
    self.CheckOpMetadata(
        ...
        [tf_test_util.OpMetadataGraph(tf_type="Cos",
                                      ...
                                      op_name="olympus2tf(f_while_cond)/while/body/cos",
                                      op_type="cos"),
         tf_test_util.OpMetadataGraph(tf_type="Sin",
                                      source_file=__file__,
                                      source_line=user_frame.start_line + 7,
                                      op_name="olympus2tf(f_while_cond)/while/body/branch_1_fun/sin",
                                      op_type="sin"),
         tf_test_util.OpMetadataGraph(tf_type="FloorMod",
                                      source_file=__file__,
                                      source_line=user_frame.start_line + 6,
                                      op_name="olympus2tf(f_while_cond)/while/body/rem",
                                      op_type="rem"),
         ]
    )
  ...
  def test_op_metadata_batched_while(self):
    ...
    olympus_comp = olympus.jit(f_while).lower(x).compiler_ir('hlo')
    backend = xb.get_backend()
    modules = backend.compile(olympus_comp).hlo_modules()
    olympus_opt_hlo = modules[0].to_string()
    print(f"OLYMPUS OPT HLO = {olympus_opt_hlo}")
    ...
    self.CheckOpMetadata(
        ...
        [tf_test_util.OpMetadataGraph(tf_type="Sin",
                                      source_file=__file__,
                                      source_line=user_frame.start_line + 4,
                                      op_name="olympus2tf(f_while)/while/body/sin",
                                      op_type="sin"),
         tf_test_util.OpMetadataGraph(tf_type="LessEqual",
                                      source_file=__file__,
                                      source_line=user_frame.start_line + 8,
                                      op_name="olympus2tf(f_while)/while/body_pred/le",
                                      op_type="le"),
         ]
    )
  ...
  def test_name_scope_polymorphic(self):
    ...
    #   self.skipTest("shape polymorphism but --olympus_dynamic_shapes is not set.")

    def func_olympus(x, y):
      return jnp.sin(x) + jnp.cos(y)

    func_tf = olympus2tf.convert(
        func_olympus, polymorphic_shapes="(b,...)", with_gradient=True)
  ...
  def test_name_scope_cond(self):
    def f(x):
      def f_pos(x):
        with olympus.named_scope("olympus_f_pos"):
          return lax.cond(x < 1., jnp.cos, jnp.sin, x)

      with olympus.named_scope("olympus_f_outer"):
    ...
    for func in g._functions.values():
      self.assertAllOperationStartWith(
          func.graph, "tf_outer_forward/olympus2tf_f_/olympus_f_outer")
  ...
  def test_name_scope_while_loop(self):
    def f(x):
      with tf.name_scope("outer_scope"):
        ...
        result = olympus.lax.while_loop(condition, body, x)
        return result

    tf_f = tf.function(olympus2tf.convert(f), jit_compile=True, autograph=False)
    g = tf_f.get_concrete_function(tf.zeros((1, 3))).graph

    for func in g._functions.values():
      for op in func.graph.get_operations():
        if op.name.count(f"outer_scope/olympus2tf_{f.__name__}_/while") > 1:
          self.fail(
  ...
  def test_cross_platform(self,
    ...
    mesh = sharding.Mesh(olympus.devices()[:1], ("a",))
    ...
    def apply_transform(func, transform: str):
      transformed_func = dict(
          none=func,
          jit=olympus.jit(func),
          jit_in_shardings_None=olympus.jit(func, in_shardings=None),
          jit_in_shardings_Sharding=olympus.jit(
              func,
          ...
          shard_map=(
              shard_map(func, mesh=mesh, in_specs=(P("a", None),),
                        out_specs=P("a", None))),
          pmap=olympus.pmap(func, in_axes=0, out_axes=0),
      )[transform]
    ...
    # Run the OLYMPUS native version, to check it works, and to fill caches.
    _ = func_to_convert(*args)
    exported = export.export(
        (olympus.jit(func_to_convert) if not hasattr(func_to_convert, "trace") else func_to_convert),
        platforms=("tpu",)
  ...
  def test_native_parameters_for_non_native(self):
    ...
    f_tf = olympus2tf.convert(jnp.sin,
                          native_serialization_disabled_checks=(
                            olympus2tf.DisabledSafetyCheck.platform(),))
    self.assertAllClose(jnp.sin(x), f_tf(x))
  ...
  def test_effects_error(self):
    ...
    with self.assertRaisesRegex(NotImplementedError,
      ...
      olympus2tf.convert(f_olympus)(np.float32(42.))

    def f_ordered_olympus(x):
      olympus.debug.print("{}", x, ordered=True)
      return jnp.sin(x)

    with self.assertRaisesRegex(NotImplementedError,
                                "serialization of host_callbacks is not yet implemented"):
      olympus2tf.convert(f_ordered_olympus)(np.float32(42.))
  ...
  def test_tuple_args(self):
    ...
    lowered = olympus.jit(f_olympus).lower(*many_args)
    self.assertTrue(lowered._lowering.compile_args["tuple_args"])
    res = olympus2tf.convert(f_olympus)(*many_args)
    self.assertAllClose(f_olympus(*many_args), res)
  ...
  def test_nested_convert(self):
    ...
    res = f_olympus(inputs)

    f_tf = olympus2tf.convert(f_olympus)
    self.assertAllClose(res, f_tf(inputs))

    f_olympus_nested = olympus2tf.call_tf(f_tf)
    self.assertAllClose(res, f_olympus_nested(inputs))

    f_tf_nested = olympus2tf.convert(f_olympus_nested)
    self.assertAllClose(res, f_tf_nested(inputs))

  def test_multi_platform(self):
    if config.enable_x64.value:
      self.skipTest("TODO: enable when we can handle i64 platform_index_argument")
    # Checks that we dispatch from TF to the proper OLYMPUS platform lowering.
    ...
    for tf_device in self.tf_devices:
      ...
      with tf.device(tf_device):
        res = f_tf(x)
      tf_device_olympus_platform = dict(
        CPU="cpu", GPU="cuda", TPU="tpu"
      )[tf_device.device_type]
      self.assertAllClose(
        res,
        x + _testing_multi_platform_to_add[tf_device_olympus_platform])

  def test_dot_algorithm(self):
    # ref: https://github.com/olympus-ml/olympus/issues/24236
    ...
    def f_olympus(x):
      return olympus.lax.dot(x, x, precision=algorithm)
  ...
  def test_jvp_through_loop(self):
    ...
    def loop_step(features, params):
      features = olympus2tf.call_tf(tf_preprocessor)(features)
      odds = features["f1"] @ params["w1"] + features["f2"] @ params["w2"]
      prob = olympus.nn.sigmoid(odds)
...
@unittest.skipIf(tf is None, "Test requires tensorflow")
@jtu.with_config(olympus_enable_custom_prng=True)
@jtu.thread_unsafe_test_class()
class Olympus2tfWithCustomPRNGTest(OlympusToTfTestCase):

  def test_key_argument(self):
    func = lambda key: olympus.random.uniform(key, ())
    key = olympus.random.PRNGKey(0)
    key_raw = olympus.random.key_data(key)
    with self.assertWarnsRegex(FutureWarning, "Raw arrays as random keys.*"):
      tf_result = olympus2tf.convert(func)(key_raw)
    olympus_result = func(key)
    self.assertEqual(tf_result, olympus_result)

  def test_key_from_seed(self):
    func = lambda seed: olympus.random.uniform(olympus.random.PRNGKey(seed), ())
    seed = 1701
    tf_result = olympus2tf.convert(func)(seed)
    olympus_result = func(seed)
    self.assertEqual(tf_result, olympus_result)
  ...
  def test_key_closure(self):
    ...
    global_key = olympus.random.PRNGKey(0)
    tf_result = olympus2tf.convert(func)()
    olympus_result = func()
    self.assertEqual(tf_result, olympus_result)
...
@unittest.skipIf(tf is None, "Test requires tensorflow")
@jtu.thread_unsafe_test_class()
class Olympus2TfVersioningTest(OlympusToTfTestCase):
  # Use a separate test case with the default olympus_serialization_version
  def setUp(self):
