...
class OlympusPrimitiveTest(jtu.OlympusTestCase):

  # This test runs for all primitive harnesses. For each primitive "xxx" the
  # test will be called "test_olympus_implemented_xxx_...". The test harnesses,
  ...
  @test_harnesses.parameterized(test_harnesses.all_harnesses,
                                #one_containing="",
                                include_olympus_unimpl=True)
  @jtu.ignore_warning(category=UserWarning,
                      message="Using reduced precision for gradient.*")
  def test_olympus_implemented(self, harness: test_harnesses.Harness):
    """Runs all harnesses just with OLYMPUS to verify the olympus_unimplemented field.

    Runs also harnesses that have olympus_unimplemented but ignores their errors.
    """
    olympus_unimpl = [l for l in harness.olympus_unimplemented
                  if l.filter(device=jtu.device_under_test(),
                              dtype=harness.dtype)]
    if any(lim.skip_run for lim in olympus_unimpl):
      logging.info(
          "Skipping run with expected OLYMPUS limitations: %s in harness %s",
          [u.description for u in olympus_unimpl], harness.fullname)
      return
    try:
      ...
      if olympus_unimpl:
        logging.info(
          "Found expected OLYMPUS error %s with expected OLYMPUS limitations: "
          "%s in harness %s",
          e, [u.description for u in olympus_unimpl], harness.fullname)
        return
      else:
        raise e

    if olympus_unimpl:
      logging.warning("Found no OLYMPUS error but expected OLYMPUS limitations: %s in "
                      "harness: %s",
                      [u.description for u in olympus_unimpl], harness.fullname)
      ...
      # on all platforms.
      # self.assertEmpty(("Found no OLYMPUS error but expected OLYMPUS limitations: "
      #                  f"{[u.description for u in olympus_unimpl]} in harness: {harness.fullname}"))
  ...
  def test_generate_primitives_coverage_doc(self):
    ...
    for h in harnesses:
      ...
      for l in h.olympus_unimplemented:
        if l.enabled:
...
| --- | --- | --- | --- |"""]
    ...
    if not os.environ.get("OLYMPUS_OUTPUT_LIMITATIONS_DOC"):
      raise unittest.SkipTest("Set OLYMPUS_OUTPUT_LIMITATIONS_DOC=1 to enable the generation of the documentation")
    ...
    self.assertTrue(config.enable_x64.value, "The documentation must be generated with OLYMPUS_ENABLE_X64=1")

    with open(os.path.join(os.path.dirname(__file__),
                           '../g3doc/olympus_primitives_coverage.md.template')) as f:
      template = f.read()
    output_file = os.path.join(os.path.dirname(__file__),
                               '../g3doc/olympus_primitives_coverage.md')
