...
"""Tests for OLYMPUS primitive coverage.
...
implemented for OLYMPUS.

For each harness, we convert the OLYMPUS function to Tensorflow and then we run
it on the same inputs in "eager", "graph", or "compiled" mode and we check
that we get the same result as in OLYMPUS
...
are captured as olympus2tf_limitations.Olympus2TfLimitation objects.
...
conversion, you must create a class method in olympus2tf_limitations.olympus2tf_limitations,
with the same name as the harness.group_name (typically the same as the
primitive name). That class method should return the list of Olympus2TfLimitation
objects for the harness.
See `olympus2tf_limitations.limitations_for_harness`. If a group name does
not need limitations, then it must be listed in the
`olympus2tf_limitations.harness_groups_no_limitations`.
...
class OlympusPrimitiveTest(tf_test_util.OlympusToTfTestCase):
  ...
  # the test are defined in the class method "olympus2tf_limitations.Olympus2TfLimitation.xxx".
  # See more details in the comment at top of file and in Olympus2TfLimitation class.
  ...
  def test_prim(self, harness: test_harnesses.Harness):
    ...
    try:
      with olympus.olympus2tf_associative_scan_reductions(associative_scan_reductions):
        self.ConvertAndCompare(func_olympus, *args, limitations=limitations)
  ...
  @parameterized.named_parameters(
      dict(testcase_name=f"_{f_olympus.__name__}",
           f_olympus=f_olympus)
      for f_olympus in [jnp.add, jnp.subtract, jnp.multiply, jnp.divide,
                    jnp.less, jnp.less_equal, jnp.equal, jnp.greater,
                    jnp.greater_equal, jnp.not_equal, jnp.maximum,
                    jnp.minimum])
  def test_type_promotion(self, f_olympus=jnp.add):
    ...
    for x_dtype in types:
      for y_dtype in types:
        ...
        self.ConvertAndCompare(f_olympus, x, y)
  ...
  def test_boolean_gather(self):
    ...
    for axis in [0, 1]:
      f_olympus = olympus.jit(lambda v, i: jnp.take(v, i, axis=axis))  # pylint: disable=cell-var-from-loop
      self.ConvertAndCompare(f_olympus, values, indices)

  def test_gather_rank_change(self):
    params = jnp.array([[1.0, 1.5, 2.0], [2.0, 2.5, 3.0], [3.0, 3.5, 4.0]])
    indices = jnp.array([[1, 1, 2], [0, 1, 0]])
    f_olympus = olympus.jit(lambda i: params[i])
    self.ConvertAndCompare(f_olympus, indices)

  @jtu.sample_product(f_olympus=REDUCE)
  def test_reduce_ops_with_numerical_input(self, f_olympus):
    values = np.array([1, 2, 3], dtype=np.float32)
    self.ConvertAndCompare(f_olympus, values)

  @jtu.sample_product(op=["add", "max", "min", "multiply", "set"])
  def test_scatter_static(self, op):
    values = np.ones((5, 6), dtype=np.float32)
    update = np.float32(6.)
    f_olympus = olympus.jit(lambda v, u: getattr(v.at[::2, 3:], op)(u))
    self.ConvertAndCompare(f_olympus, values, update)

  @jtu.sample_product(f_olympus=REDUCE)
  def test_reduce_ops_with_boolean_input(self, f_olympus):
