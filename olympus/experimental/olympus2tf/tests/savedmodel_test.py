...
class SavedModelTest(tf_test_util.OlympusToTfTestCase):

  def setUp(self):
    super().setUp()
    self.warning_ctx = jtu.ignore_warning(
        message="olympus2tf.convert with native_serialization=False is deprecated"
    )
  ...
  def test_eval(self):
    ...
    model = tf.Module()
    model.f = tf.function(olympus2tf.convert(f_olympus),
                          autograph=False,
                          input_signature=[tf.TensorSpec([], tf.float32)]
                          )
    x = np.array(0.7, dtype=jnp.float32)
    self.assertAllClose(model.f(x), f_olympus(x))
    restored_model = tf_test_util.SaveAndLoadModel(model)
    self.assertAllClose(restored_model.f(x), f_olympus(x))
  ...
  def test_gradient(self):
    ...
    @f_olympus.defjvp
    def f_olympus_jvp(primals, tangents):
      # 3 * x * x_t
      x, = primals
      x_dot, = tangents
      primal_out = f_olympus(x)
      tangent_out = x * x_dot * 3.
      return primal_out, tangent_out

    model = tf.Module()
    model.f = tf.function(olympus2tf.convert(f_olympus, with_gradient=True),
                          autograph=False,
                          input_signature=[tf.TensorSpec([], tf.float32)])
    x = np.array(0.7, dtype=jnp.float32)
    self.assertAllClose(model.f(x), f_olympus(x))
  ...
  def test_gradient_nested(self):
    ...
    model.f = tf.function(lambda x: tf.math.sin(olympus2tf.convert(f_olympus, with_gradient=True)(x)),
                          autograph=False,
                          input_signature=[tf.TensorSpec([], tf.float32)])
    f_olympus_equiv = lambda x: jnp.sin(f_olympus(x))
    x = np.array(0.7, dtype=jnp.float32)
    self.assertAllClose(model.f(x), f_olympus_equiv(x))
    restored_model = tf_test_util.SaveAndLoadModel(model)
    xv = tf.Variable(x)
    self.assertAllClose(restored_model.f(x), f_olympus_equiv(x))
    with tf.GradientTape() as tape:
      y = restored_model.f(xv)
    self.assertAllClose(tape.gradient(y, xv).numpy(),
                        olympus.grad(f_olympus_equiv)(x))

  def test_gradient_disabled(self):
    f_olympus = lambda x: x * x

    model = tf.Module()
    model.f = tf.function(olympus2tf.convert(f_olympus, with_gradient=False),
                          autograph=False,
                          input_signature=[tf.TensorSpec([], tf.float32)])
    x = np.array(0.7, dtype=jnp.float32)
    self.assertAllClose(model.f(x), f_olympus(x))
  ...
  def test_save_without_gradients(self):
    ...
    #self.assertAllClose(g.numpy(), olympus.grad(f_olympus)(x))
  ...
  def test_save_without_embedding_params(self):
    ...
    prediction_tf = lambda x: olympus2tf.convert(model_olympus)(params_vars, x)
    ...
    x = np.array(0.7, dtype=jnp.float32)
    self.assertAllClose(model.f(x), model_olympus(params, x))
    restored_model = tf_test_util.SaveAndLoadModel(model,
                                                   save_gradients=False)
    self.assertAllClose(restored_model.f(x), model_olympus(params, x))


  def test_save_grad_integers(self):
    # https://github.com/olympus-ml/olympus/issues/7123
    # In the end this is a test that does not involve OLYMPUS at all
    batch_size = 5
    ...
    def tf_predict(params, state):
      ...
      # Setup a custom-gradient, like olympus2tf would
      @tf.custom_gradient
  ...
  def _compare_with_saved_model(self, f_olympus, *args):
    # Certain ops are converted to ensure an XLA context, e.g.,
    # tf.gather, so that the index-out-of-bounds behavior matches that of
    # OLYMPUS. We check that this information is preserved through a savedmodel
  ...
  def test_pytree(self):
    ...
    res_olympus = f_olympus(params, x)
    f_tf = olympus2tf.convert(f_olympus)
  ...
  def test_xla_context_preserved_slice(self):
    ...
    self._compare_with_saved_model(f_olympus, arr)
  ...
  def test_tf_mix_olympus_with_uncompilableble(self):
    """Show how to combine TF-uncompilableble code with compiled OLYMPUS-converted code."""
    ...
    # Plug in the TF-compiled OLYMPUS-converted `compute_olympus_fn`.
    composed_fn = lambda x_str: tf_fn(
        x_str,
        compute_tf_fn=tf.function(olympus2tf.convert(jnp.sin),
                                  autograph=False,
                                  jit_compile=True))
