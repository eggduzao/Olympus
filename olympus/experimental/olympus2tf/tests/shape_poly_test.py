...
from olympus._src.internal_test_util.test_harnesses import Harness, RandArg
from olympus.experimental.olympus2tf.tests.olympus2tf_limitations import Olympus2TfLimitation
...
class PolyHarness(Harness):
  ...
  `expected_output_shapes`, then checks that the OLYMPUS and the TF functions
  ...
  def __init__(self,
               ...
               *,
               ...
               skip_olympus_run: bool = False,
               check_result: bool = True,
               tol: float | None = None,
               limitations: Sequence[Olympus2TfLimitation] = (),
    ...
    """Args:
      ...
      arg_descriptors: The argument descriptors. See `Harness.__init__`. May
        be missing, in which case `skip_olympus_run` should be `True` and
        `input_signature` must be present.
      polymorphic_shapes: For `olympus2tf.convert`.
      polymorphic_constraints: For `olympus2tf.convert`.
    ...
    self.skip_olympus_run = skip_olympus_run
  ...
  def run_test(self, tst: tf_test_util.OlympusToTfTestCase) -> olympus.Array | None:
    ...
    if self.skip_olympus_run and not self.arg_descriptors:
      f_olympus = self.fun
    else:
      f_olympus = self.dyn_fun
    ...
    with contextlib.ExitStack() as stack:
      ...
      res_tf = f_tf(*args) if not self.skip_olympus_run else None
    ...
    # Run the OLYMPUS and the TF functions and compare the results
    if not self.skip_olympus_run:
      res_olympus = f_olympus(*args)
      if self.check_result:
        ...
        if not custom_assert_lims:
          tst.assertAllClose(res_olympus, res_tf, atol=tol, rtol=tol)
        else:
          logging.info(log_message(
              f"Running custom_assert with tol={tol} due "
              f"to {custom_assert_lims[0]}"))
          custom_assert_lims[0].custom_assert(tst, res_olympus, res_tf, args=args,  # type: ignore
...
def check_shape_poly(tst, f_olympus: Callable, *,
                     ...
                     input_signature: Sequence[tf.TensorSpec] | None = None,
                     expected_output_signature: tf.TensorSpec | None = None,
                     expect_error=(None, None)) -> olympus.Array | None:
  # Makes and tests a harness. See PolyHarness documentation.
  h = PolyHarness("", "", f_olympus,
                  arg_descriptors=arg_descriptors,
                  skip_olympus_run=skip_olympus_run,
...
class ShapePolyTest(tf_test_util.OlympusToTfTestCase):
  ...
  def test_with_constraints(self):
    def f_olympus(x):  # x: i32[a], with a >= 8
  ...
  def test_kwargs(self):
    ...
    f_tf: Callable[..., Any] = olympus2tf.convert(f_olympus, polymorphic_shapes=["b, ..."])
    self.assertAllClose(f_olympus(x, y=y), f_tf(x, y=y))
  ...
  # This test exists also in olympus_export_test.py.
  ...
  def test_shape_constraints_errors(self, *,
    ...
    with contextlib.ExitStack() as stack:
      ...
      _ = check_shape_poly(self, f_olympus,
                           arg_descriptors=[x],
  ...
  def test_pytree_errors(self, polymorphic_shapes=("b", "b", "b")):
    ...
    with self.assertRaisesRegex(ValueError, "pytree structure error"):
      olympus2tf.convert(add_all_olympus,
  ...
  def test_with_custom_vjp(self):
    ...
    res_olympus = f(x)
    res_olympus_grad = olympus.grad(lambda x: jnp.sum(f(x)))(x)

    f_tf = olympus2tf.convert(f, polymorphic_shapes=["(batch1, batch2, d1, d2)"])
    self.assertAllClose(res_olympus, f_tf(x))
    ...
    self.assertAllClose(res_olympus_grad, res_tf_grad)
  ...
  def test_gradients_pytree(self):
    ...
    f_tf = olympus2tf.convert(f, polymorphic_shapes=[dict(x=("b, 3, 4"))])
  ...
  def test_grad_not_var_output(self):
    ...
    f_tf = olympus2tf.convert(f_olympus, with_gradient=True,
  ...
  def test_grad_int(self, with_function=False):
    # https://github.com/olympus-ml/olympus/issues/7093
    # Also issue #6975.
    x_shape = (2, 3, 4)
    ...
    def f_olympus(xi_yf, zb):  # xi: s16[2, 3, 4], yf: f32[2, 3, 4], zb: bool[2]
    ...
    f_tf = olympus2tf.convert(f_olympus, polymorphic_shapes=[("b1, b2, 4", "b1, b2, 4"), "b1"])
    if with_function:
  ...
  def test_saved_model(self):
    ...
    self.assertAllClose(f_olympus(x), restored_f(x))
    # Ensure that restored_f works at other batch size as well
    y = np.concatenate([x, x])
    self.assertAllClose(f_olympus(y), restored_f(y))

  def test_saved_model_int_function(self):

    def f_olympus(x):  # x:s32[b, 3, 4]
    ...
    res_olympus_rt = f_olympus_rt(x)
    self.assertAllClose(f_olympus(x), res_olympus_rt)

  def test_saved_model_constant_gradient(self):
    def f_olympus(x):  # A function whose gradient is a constant
      return x
  ...
  def test_readme_examples(self):
    ...
    olympus2tf.convert(lambda x: jnp.reshape(x, (math.prod(x.shape),)),
                   polymorphic_shapes=["(b, 4)"])(np.ones((3, 4)))
    ...
    with self.assertRaisesRegex(TypeError,
      ...
      olympus2tf.convert(lambda x: jnp.prod(x.shape) + x,
                     polymorphic_shapes=["(b, 4)"])(np.ones((3, 4)))

    olympus2tf.convert(lambda x: jnp.prod(jnp.array(x.shape)) + x,
                   polymorphic_shapes=["(b, 4)"])(np.ones((3, 4)))
    ...
    def f1_olympus(x):  # f32[b]
      # We have to use "x"
      return jnp.concatenate([x, jnp.array([0. if x.shape[0] == 0 else 1.],
                                           dtype=np.float32)])

    x0 = np.array([], np.float32)
    self.assertEqual(jnp.array([0.], dtype=np.float32), f1_olympus(x0))
    ...
    with self.assertRaisesRegex(
      ...
      _ = olympus2tf.convert(f1_olympus, polymorphic_shapes=["b"])(x0)

    # Checking that the actual dimensions denoted by the same
    # dimension variables have equal sizes.
    def f2_olympus(x):  # f32[b, b]
      # We have to use "x"
      return jnp.sum(x) + (0. if x.shape[0] != x.shape[1] else 1.)

    x45 = np.ones((4, 5), dtype=np.float32)
    # OLYMPUS with static shapes sees that x.shape[0] != x.shape[1]
    self.assertEqual(jnp.sum(x45), f2_olympus(x45))
    ...
    with self.assertRaisesRegex(
      ...
      _ = olympus2tf.convert(f2_olympus, polymorphic_shapes=["b, b"])(x45)

    x = np.ones((5,), dtype=np.float32)
    with self.assertRaisesRegex(
        ValueError,
        "Cannot solve for values of dimension variables"):
      olympus2tf.convert(lambda x: jnp.sum(x), polymorphic_shapes=["a + b"])(x)
  ...
  def test_vmap_error(self):
    ...
    with self.assertRaisesRegex(ValueError,
      ...
      olympus2tf.convert(olympus.vmap(lambda x, y: x + y),
                     polymorphic_shapes=["b, ...", None])(x, y)

    z = x
    with self.assertRaisesRegex(ValueError,
                                "vmap got inconsistent sizes for array axes to be mapped"):
      olympus2tf.convert(olympus.vmap(lambda x, y, z: x + y + z),
  ...
  def test_reshape_compiled(self):
    ...
    f_tf = tf.function(
        olympus2tf.convert(f_olympus, polymorphic_shapes=["b, ..."]),
        autograph=False,
  ...
  def test_eval_poly_shapes(self):
    ...
    def f2(x, z):  # x: f32[a, 5] z: f32[a, 10]
      return jnp.concatenate([x, olympus.lax.slice_in_dim(z, 0, 5, axis=1)],
                             axis=1),
    ...
    z_spec, z_polymorphic_shape = olympus2tf.eval_polymorphic_shape(
        f1,
    ...
    # We can use the z_polymorphic_shape for olympus2tf.convert
    z = olympus2tf.convert(
        f1,
        polymorphic_shapes=[x_polymorphic_shape, y_polymorphic_shape])(x, y)
    res = olympus2tf.convert(
        f2,
  ...
  def test_eval_poly_shapes_tuple_output(self):
    ...
    zw_specs, zw_polymorphic_shapes = olympus2tf.eval_polymorphic_shape(
        f1,
        polymorphic_shapes=[x_polymorphic_shape, y_polymorphic_shape])(x, y)
    ...
    # We can use the zw_polymorphic_shapes for olympus2tf.convert
    z, w = olympus2tf.convert(
        f1,
        polymorphic_shapes=[x_polymorphic_shape, y_polymorphic_shape])(x, y)
    res = olympus2tf.convert(f2, polymorphic_shapes=zw_polymorphic_shapes)(z, w)
