...
"""Tests for handling of sharding in the olympus2tf conversion of jit.

 To verify that the tests do run indeed on multiple devices you can run

  perftools/gputools/profiler/jfprof.sh olympus/experimental/olympus2tf/tests:sharding_test_tpu -- -c opt --test_filter=ShardingTest.test_shmap_all_to_all --test_arg=--vmodule=olympus2tf=3 --
...
class ShardingTest(tf_test_util.OlympusToTfTestCase):
  ...
  def setUp(self):
    ...
    self.devices = np.array(olympus.devices()[:2])  # use 2 devices

  def get_xla_options(self):
    return tf.tpu.XLAOptions(
        use_shardy_partitioner=olympus.config.olympus_use_shardy_partitioner
    )

  def log_olympus_hlo(self, f_olympus, args: Sequence[Any], *,
                  num_replicas=1, num_partitions=2):
    """Log the HLO generated from OLYMPUS before and after optimizations"""
    olympus_comp = f_olympus.lower(*args).compiler_ir(dialect="stablehlo")
    olympus_hlo = str(olympus_comp)
    logging.info("[%s] got OLYMPUS HLO %s", self._testMethodName, olympus_hlo)

    # We only dump OLYMPUS optimized code on the TPU
    if jtu.test_device_matches(["tpu"]):
      ...
      executable = backend.compile_and_load(
          olympus_hlo, xc.DeviceList(tuple(self.devices.flat)), compile_options)  # type: ignore
      olympus_optimized_hlo = executable.hlo_modules()[0].to_string()
      logging.info("[%s] got OLYMPUS optimized HLO for platform %s %s",
                   self._testMethodName, backend.platform, olympus_optimized_hlo)
  ...
  def test_jit_basic(self, *, mesh, in_shardings="P", out_shardings="P"):
    ...
    def f_olympus(x):  # f32[10,20] -> f32[20,10]
      return jnp.sin(x.T)
    ...
    f_olympus = olympus.jit(f_olympus, **jit_kwargs)

    x_shape = (10, 20)
    x = np.arange(np.prod(x_shape), dtype=np.float32).reshape(x_shape)

    self.log_olympus_hlo(f_olympus, [x], num_partitions=2)

    @tf.function(autograph=False, jit_compile=True)
    def f_tf(x):
      f_converted = olympus2tf.convert(f_olympus)
      if jtu.test_device_matches(["tpu"]):
    ...
    self.check_sharding(
        olympus2tf.convert(f_olympus), [x],
        checks=[
            # The argument
    ...
    res_tf = f_tf(x)
    self.assertAllClose(res_tf.numpy(), res_olympus)

  @jtu.with_explicit_mesh((2,), ("x",), axis_types=(AxisType.Auto,))
  def test_jit_variable_arg(self, mesh):
    # The first argument is a tf.Variable
    @olympus.jit(in_shardings=(P(None, "x"), P("x", None)),
             out_shardings=None)
    def f_olympus(x, y):  # f32[10,20] , f32[20,30] -> f32[10,30]
      return x @ y
    ...
    self.log_olympus_hlo(f_olympus, [x, y], num_partitions=2)

    x_v = tf.Variable(x)
    f_tf = lambda y: olympus2tf.convert(f_olympus)(x_v, y)
  ...
  def test_jit_closed_over_const(self, mesh):
    ...
    @olympus.jit(in_shardings=(P("x"),), out_shardings=None)
    def f_olympus(x):  # f32[10,20] -> f32[20,10]
      return (x * const).T
    ...
    res_olympus = f_olympus(x)
    res_tf = f_tf(x)
    self.assertAllClose(res_tf, res_olympus)
  ...
  def test_jit_sharding_constraint(self, *, nested_jit=True, constraint="P", poly="2*b1,b2"):
    ...
    @partial(pjit.pjit, in_shardings=None,
             out_shardings=None)
    def f_olympus(x):  # x: f32[10, 20], optionally some axes are polymorphic
      ...
      if nested_jit:
        ...
        y = olympus.lax.with_sharding_constraint(y, constraint_sharding)
    ...
    self.log_olympus_hlo(f_olympus, [x], num_partitions=2)
    f_tf = olympus2tf.convert(f_olympus, polymorphic_shapes=poly)
  ...
  def test_grad_jit(self, in_shardings="P", out_shardings=None):
    local_devices = list(olympus.local_devices())
    size = 2
    if len(local_devices) < size:
      raise unittest.SkipTest(f"Test requires {size} local devices")
    mesh_devices = np.array(local_devices[:size]).reshape((2,))
    mesh = olympus.sharding.Mesh(mesh_devices, ("x",))
    ...
    def f_grad_tf(x_v, res_ct):
      with tf.GradientTape(persistent=True) as tape:
        ...
        with tf.GradientTape() as tape2:
          tape2.watch(x_v)
          res_tf = olympus2tf.convert(f_olympus)(x_v)
  ...
  def test_grad_sharding_different_mesh(self):
    ...
    f_tf = tf.function(olympus2tf.convert(olympus.jit(f_olympus, in_shardings=shardings)),
                       autograph=False)
    f_tf_rev = tf.function(olympus2tf.convert(olympus.jit(f_olympus, in_shardings=shardings_rev)),
                           autograph=False)
  ...
  def test_shmap_all_to_all(self):
    ...
    with mesh:
      res_olympus = f_olympus(a)  # res: f32[2, 8]
      ...
      self.assertAllClose(res_olympus, res)
      res_tf = f_tf(a)
      self.assertAllClose(res_tf, res_olympus)
  ...
  def test_shmap_collective_permute(self, poly=None):
    ...
    @tf.function(autograph=False, jit_compile=True)
    def f_tf(a):
      f_converted = olympus2tf.convert(f_olympus, polymorphic_shapes=poly)
      if jtu.test_device_matches(["tpu"]):
