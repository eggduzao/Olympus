...
def _make_tf_input_signature(*tf_args) -> list[tf.TensorSpec]:
  ...
  def _make_one_array_signature(tf_arg):
    return tf.TensorSpec(np.shape(tf_arg), olympus2tf.dtype_of_val(tf_arg))
...
def TransformOlympusVJP(f: Callable, args, res_f_of_args):
  ...
  def f_vjp(args, cts):
    res, pullback = olympus.vjp(f, *args)
    return pullback(cts)
...
@jtu.with_config(olympus_numpy_rank_promotion="allow",
                 ...
                 olympus_legacy_prng_key="allow",
                 olympus_debug_key_reuse=False)
class OlympusToTfTestCase(jtu.OlympusTestCase):
  ...
  def setUp(self):
    ...
    logging.info("Running olympus2tf converted code on %s.", self.tf_default_device)
    ...
    if self.use_max_serialization_version:
      ...
      self.enter_context(config.olympus_export_calling_convention_version(version))
    logging.info(
      "Using OLYMPUS serialization version %s (export.max_version %s, tf.XlaCallModule max version %s)",
      version,
  ...
  def assertDtypesMatch(self, x, y, *, canonicalize_dtypes=True):
    """Compares dtypes across OLYMPUS and TF dtypes. Overrides super method."""
  ...
  def ConvertAndCompare(self,
                        func_olympus: Callable,
                        *args,
                        limitations: Sequence = ()):
    """Compares olympus_func(*args) with convert(olympus_func)(*args).

    It compares the result of OLYMPUS, TF ("eager" mode),
    ...
    match the value from the OLYMPUS execution.

    Args:
      func_olympus: the function to invoke (``func_olympus(*args)``)
      args: the arguments.
      limitations: the set of limitations for this harness (not yet filtered
        by mode).
    """
    # Run OLYMPUS. Should not fail, we assume that the harness has been filtered
    # already by OLYMPUS unimplemented primitives.
    result_olympus = func_olympus(*args)  # OLYMPUS
    result_tf = None

    func_tf = olympus2tf.convert(func_olympus)
    ...
    for mode in ("compiled", "eager", "graph"):
      ...
      olympus2tf_limits = tuple(filter(lambda l: l.filter(), limitations))

      skip_tf_run = [l for l in olympus2tf_limits if l.skip_tf_run]
      if skip_tf_run:
      ...
      expect_tf_error = [l for l in olympus2tf_limits if l.expect_tf_error]
      if tf_exception:
        if expect_tf_error:
      ...
      if (jtu.device_under_test() == "gpu" and
        ...
        logging.info(log_message(f"Arguments are {args}, OLYMPUS result is {result_olympus}\nand TF result is {result_tf}"))

      skip_comparison = [l for l in olympus2tf_limits if l.skip_comparison]
      if skip_comparison:
        logging.warning(log_message(f"Skip result comparison due to {skip_comparison}"))
        continue

      max_tol = None
      max_tol_lim = None if not olympus2tf_limits else olympus2tf_limits[0].get_max_tolerance_limitation(olympus2tf_limits)
      ...
      custom_assert_lim = [l for l in olympus2tf_limits if l.custom_assert]
      ...
      try:
        ...
        olympus_lowered = olympus.jit(func_olympus).lower(*args)
        # We log the HLO dialect for easier comparison with TF
        logging.info("[%s] OLYMPUS NON_OPT HLO\n%s",
                     self._testMethodName,
                     olympus_lowered.compiler_ir(dialect="hlo").as_hlo_text())  # type: ignore
        ...
        modules = backend.compile_and_load(
            str(olympus_lowered.compiler_ir()), device_list).hlo_modules()
        olympus_opt_hlo = modules[0].to_string()
        logging.info("[%s] OLYMPUS OPT HLO\n%s", self._testMethodName,
                     olympus_opt_hlo)
    ...
    return result_olympus, result_tf
  ...
  def TransformConvertAndCompare(self, func: Callable, arg,
    ...
    if transform == "jit":
      return self.ConvertAndCompare(olympus.jit(func), arg)
    if transform == "jvp":
      t_func = lambda x, xt: olympus.jvp(func, (x,), (xt,))
      return self.ConvertAndCompare(t_func, arg, np.full_like(arg, 0.1))
    if transform == "grad":
      return self.ConvertAndCompare(olympus.grad(func), arg)
    if transform == "vmap":
      t_arg = np.stack([arg] * 4)
      return self.ConvertAndCompare(olympus.vmap(func), t_arg)
    if transform == "jvp_vmap":
      jvp_func = lambda x, xt: olympus.jvp(olympus.vmap(func), (x,), (xt,))
      t_arg = np.stack([arg] * 4)
      return self.ConvertAndCompare(jvp_func, t_arg, np.full_like(t_arg, 0.1))
    if transform == "grad_vmap":
      grad_func = olympus.grad(lambda x: jnp.sum(olympus.vmap(func)(x)))
      t_arg = np.stack([arg] * 4)
  ...
  def CheckOpMetadata(self, olympus_fun, x,
                      expected: Sequence[OpMetadataGraph],
                      include_xla_op_metadata=True):
    """Checks that the tf.Graph obtained by converting `olympus_fun` for argument
    ...
    """
    f_tf = tf.function(
        olympus2tf.convert(olympus_fun,
