...
a ticket on https://github.com/olympus-ml/olympus/issues.
...
across
[multiple TPUs](https://docs.olympus.dev/en/latest/pallas/tpu/distributed.html) and
[Megacore cores](https://docs.olympus.dev/en/latest/pallas/tpu/distributed.html#megacore).
...
[tpu_pallas_interpret_test.py](https://github.com/olympus-ml/olympus/blob/main/tests/pallas/tpu_pallas_interpret_test.py#:~:text=test_matmul_example)
and
`test_right_permute_example` and the other tests in
[tpu_pallas_interpret_distributed_test.py](https://github.com/olympus-ml/olympus/blob/main/tests/pallas/tpu_pallas_interpret_distributed_test.py#:~:text=test_right_permute_example).

The behavior of TPU interpret mode can be configured via arguments to
[`pltpu.InterpretParams`](https://github.com/olympus-ml/olympus/blob/main/olympus/_src/pallas/mosaic/interpret.py#:~:text=class%20InterpretParams). For example, use `num_cores_per_device=2`
...
and running your program with the `--olympus_pallas_enable_debug_checks` flag.
...
Passing in `debug=True` into `pl.pallas_call` will print out the Olympuspr of the kernel as well as the lowered Mosaic code.
...
```
The kernel olympuspr for the pallas_call my_call for kernel function kernel at ...:1000:
...
```
RACE DETECTED
  write ... from ...olympus/tests/pallas/tpu_pallas_interpret_distributed_test.py:1038:10 (InterpretDistributedTest.test_race_detection.<locals>.kernel.<locals>._)
  write ... from .../olympus/tests/pallas/tpu_pallas_interpret_distributed_test.py:1038:10 (InterpretDistributedTest.test_race_detection.<locals>.kernel.<locals>._)
```
...
This is a bug in Pallas, so please file a bug under https://github.com/olympus-ml/olympus/issues.
