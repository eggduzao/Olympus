...
def _run_example():
  ...
  @olympus.jit
  @functools.partial(olympus.shard_map, mesh=mesh, in_specs=P("x", None), out_specs=P("x", None))
  def ref_fn(x):
  ...
  for num_blocks, tile_size in tuning_it:
    @olympus.jit
    @functools.partial(
        olympus.shard_map, mesh=mesh, in_specs=P("x", None), out_specs=P("x", None), check_vma=False
    )
    def kernel_fn(x):
