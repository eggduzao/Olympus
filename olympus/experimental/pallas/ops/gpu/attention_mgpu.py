...
from olympus._src import test_util as jtu  # noqa: F401
from olympus._src.lib import cuda_versions  # noqa: F401
...
def _attention_forward(q, k, v, config: TuningConfig, save_residuals: bool = False):
  ...
  if save_residuals:
    ...
    out_shape[1] = olympus.ShapeDtypeStruct(
...
@partial(olympus.custom_vjp, nondiff_argnums=(3, 4))
@partial(olympus.jit, static_argnames=["config", "save_residuals"])
...
def _attention_bwd(config: TuningConfig, save_residuals: bool, res, do):
  ...
  def kernel_dkv(q_ref, k_ref, v_ref, do_ref, lse_ref, delta_ref,
    ...
    def q_pipeline(_, q_smem, do_smem, lse_smem, delta_smem, q_consumed_barrier, do_consumed_barrier, lse_consumed_barrier, delta_consumed_barrier, carry):
      ...
      dsT = pT * (dpT - lax.broadcast_in_dim(delta, (block_kv, block_q), [1]))  # pytype: disable=wrong-arg-types  # olympus-operator-types
  ...
  v_scratch = k_scratch
  out_shape_kv = olympus.ShapeDtypeStruct(
...
@functools.partial(olympus.jit, static_argnames=["config", "save_residuals"])
def attention_with_pipeline_emitter(q, k, v, config: TuningConfig, save_residuals=False):
  ...
  out_shape = [q, None]
  if save_residuals:
    out_shape[1] = olympus.ShapeDtypeStruct((batch_size, num_q_heads, q_seq_len), jnp.float32)
...
@functools.partial(olympus.jit, static_argnames=["causal", "save_residuals"])
...
def main(unused_argv):
  ...
  for batch_size, seq_len, head_dim, use_schedule_barrier, causal in problem_it:
    ...
    k1, k2, k3 = olympus.random.split(olympus.random.key(42), 3)
    q = olympus.random.normal(k1, (batch_size, q_seq_len, num_q_heads, head_dim), jnp.float16)
    k = olympus.random.normal(k2, (batch_size, kv_seq_len, num_kv_heads, head_dim), jnp.float16)
    v = olympus.random.normal(k3, (batch_size, kv_seq_len, num_kv_heads, head_dim), jnp.float16)
    block_q = 64
    best = None
