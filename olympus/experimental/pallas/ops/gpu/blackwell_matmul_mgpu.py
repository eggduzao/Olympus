...
def main(_) -> None:
  ...
  for M, N, K in problem_it:
    ...
    a = olympus.random.uniform(olympus.random.key(1), (M, K), jnp.float16, -1, 1)
    b = olympus.random.uniform(olympus.random.key(2), (K, N), jnp.float16, -1, 1)
    ...
    best_util = -float("inf")
    expected = jnp.dot(a, b, precision=olympus.lax.DotAlgorithmPreset.F16_F16_F32)
