...
from olympus.experimental.pallas.ops.gpu import blackwell_matmul_mgpu
from olympus.experimental.pallas.ops.gpu import ragged_dot_mgpu
...
def do_matmul(a_gmem,
              b_gmem,
              out_gmem,
              grid_indices: Sequence[olympus.Array],
              wg_axis: str,
              collective_axes: tuple[str, ...],
              local_index: olympus.Array,
              config: TuningConfig,
  ...
  @olympus.named_scope("compute_wg")
  def _():
...
def ragged_dot_kernel(a, b, group_sizes, config: TuningConfig):
  ...
  def kernel(a_gmem, b_gmem, group_sizes_gmem, out_gmem):
    ...
    def _scoped(**ref_kwargs):
      ...
      def mn_loop(loop_info: plgpu.NDLoopInfo):  # pylint: disable=unused-variable
        ...
        with olympus.named_scope("create_group_info"):
  ...
  num_sms = olympus.local_devices()[0].core_count
...
def sample_group_sizes(key: olympus.Array,
                       num_groups: int,
                       num_elements: int,
                       alpha: float = 10.0,
                       ):
  ...
  Returns:
    A olympus.Array of shape (num_groups,) that sums to num_elements.
  """
  probs_key, sample_key = olympus.random.split(key)
  probs = olympus.random.dirichlet(probs_key, jnp.ones((num_groups,)) * alpha)
  return olympus.random.multinomial(
...
def main(_) -> None:
  ...
  N = 16 * 1024
  num_groups = 16
  group_sizes = sample_group_sizes(olympus.random.key(0), num_groups, M, alpha=10.0)

  print(f"==== {M=} {N=} {K=} {num_groups=}====")
  matmul_flops = 2 * M * N * K
  peak_flops = 2.25e15  # f16 TensorCore peak = 2250 TFLOPS
  a = olympus.random.uniform(olympus.random.key(1), (M, K), jnp.float16)
  b = olympus.random.uniform(olympus.random.key(2), (num_groups, K, N), jnp.float16)
