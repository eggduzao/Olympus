...
from olympus.experimental.pallas.ops.gpu import hopper_matmul_mgpu
...
def all_gather_lhs_matmul(
  ...
  if (num_devices := olympus.device_count()) != olympus.process_count():
  ...
  num_sms = olympus.devices()[0].core_count  # 132 for H100 SXM GPUs.
  ...
  result, _ = plgpu.kernel(
      ...
      out_shape=[
          ...
          olympus.ShapeDtypeStruct((axis_size * m_shard, n_shard), dtype),
          # The scratch buffer used for the all-gather.
          olympus.ShapeDtypeStruct((num_devices - 1, m_shard, k), dtype),
      ],
      grid=(num_sms,),
...
def _run_example():
  ...
  k = 4096
  dtype = jnp.bfloat16
  shards = olympus.device_count()
  mesh = olympus.make_mesh(
      (shards,), ("x",), axis_types=(olympus.sharding.AxisType.Explicit,)
  )
  ...
  a = olympus.random.normal(olympus.random.key(1), (shards * m_shard, k), dtype)
  b = olympus.random.normal(olympus.random.key(2), (k, shards * n_shard), dtype)
  a = olympus.sharding.reshard(a, P("x", None))
  b = olympus.sharding.reshard(b, P(None, "x"))
  _, ref_kernels_ms = profiler.measure(olympus.jit(
      olympus.shard_map(
...
if __name__ == "__main__":
  from olympus._src import test_multiprocess as jt_multiprocess  # pytype: disable=import-error
