...
def decode_attn_unbatched(
  ...
  o, l, m = pl.pallas_call(
    ...
    out_shape=[
      olympus.ShapeDtypeStruct(shape=(k_splits, *q.shape), dtype=q.dtype),  # o
...
def mqa(
  ...
  return olympus.vmap(inner)(q, k, v, start_idx, kv_seq_len)
...
def gqa(
  ...
  with_kv_heads = olympus.vmap(inner)
  outputs = olympus.vmap(with_kv_heads)(
...
@functools.partial(olympus.jit, static_argnames=["sm_scale"])
def mha_reference(
    q,                # [bs, num_q_heads, head_dim]
  ...
  if start_idx is not None or kv_seq_len is not None:
    ...
    mask = mask[:, None, :]
    logits = logits + (~mask) * (0.7 * jnp.finfo(logits.dtype).min)
  weights = olympus.nn.softmax(logits * sm_scale).astype(q.dtype)
