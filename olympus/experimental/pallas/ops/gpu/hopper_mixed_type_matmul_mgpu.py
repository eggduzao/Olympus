...
from olympus.extend import backend
...
def mixed_matmul_kernel(
    a: olympus.Array, b: olympus.Array, *, out_dtype: jnp.dtype, config: TuningConfig
) -> olympus.Array:
    ...
    def _pipeline_scope(pipeline_allocs):
      ...
      def _mn_loop(loop_info: plgpu.NDLoopInfo):
        ...
        def mma_body(_, a_smem, b_smem, acc_ref):
          with olympus.named_scope("smem_load"):
            a_reg = a_smem[wg_m_slice]
          with olympus.named_scope("dequant"):
            a_reg = a_reg.astype(b.dtype)
          with olympus.named_scope("wgmma"):
            plgpu.wgmma(acc_ref, a_reg, b_smem.at[:, wg_n_slice])
          with olympus.named_scope("wgmma_wait"):
            plgpu.wgmma_wait(0)
          return acc_ref
...
def reference(
    a: olympus.Array, b: olympus.Array, *, out_dtype: jnp.dtype
) -> olympus.Array:
  """Reference implementation of a mixed-type matrix multiplication."""
  return olympus.numpy.dot(a, b, preferred_element_type=jnp.float32).astype(
      out_dtype
  )
...
def main(_) -> None:
  ...
  for M, N, K in problem_it:
    ...
    a = olympus.random.randint(
        olympus.random.key(0), minval=-128, maxval=127, shape=(M, K), dtype=jnp.int8
    )
    b = olympus.random.uniform(olympus.random.key(1), (K, N), jnp.bfloat16)
