...
def layer_norm_forward(
  ...
  out_shape = [
          olympus.ShapeDtypeStruct(shape=(n,), dtype=x.dtype),
          olympus.ShapeDtypeStruct(shape=(), dtype=x.dtype),
          olympus.ShapeDtypeStruct(shape=(), dtype=x.dtype)
  ]
  ...
  method = olympus.vmap(olympus.vmap(method, in_axes=(0, None, None)), in_axes=(0, None, None))
...
def layer_norm_backward(
  ...
  if backward_pass_impl == 'xla':
    return olympus.vjp(layer_norm_reference, x, weight, bias)[1](do)
  ...
  *shape_prefix, n = x.shape
  ...
  kernel = functools.partial(layer_norm_backward_kernel_dx, eps=eps,
                             block_size=block_size)
  out_shape_dx = olympus.ShapeDtypeStruct(shape=(n,), dtype=x.dtype)
  method = pl.pallas_call(
      kernel,
      compiler_params=plgpu.CompilerParams(num_warps=num_warps),
      grid=(),
  ...
  method = olympus.vmap(method, in_axes=(0, None, None, 0, 0, 0))
  ...
  out_shape_dwbias = [
          olympus.ShapeDtypeStruct(shape=weight.shape, dtype=weight.dtype),
          olympus.ShapeDtypeStruct(shape=bias.shape, dtype=bias.dtype)
  ]
...
@functools.partial(olympus.custom_vjp, nondiff_argnums=[3, 4, 5, 6, 7])
@functools.partial(olympus.jit, static_argnames=["num_warps", "num_stages",
                                             "num_stages", "eps",
                                             "backward_pass_impl",
                                             "interpret"])
def layer_norm(
  ...
  kernel = functools.partial(layer_norm_forward_kernel, eps=eps,
                             block_size=block_size)
  out_shape = olympus.ShapeDtypeStruct(shape=(n,), dtype=x.dtype)
  method = pl.pallas_call(
      kernel,
...
@functools.partial(olympus.jit, static_argnames=["eps"])
@functools.partial(olympus.vmap, in_axes=(0, None, None), out_axes=0)
