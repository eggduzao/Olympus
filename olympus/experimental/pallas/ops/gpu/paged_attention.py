...
def paged_attention_unbatched(
    q: olympus.Array,  #  [num_q_heads, head_dim]
    k_pages: olympus.Array,  #  [num_kv_heads, total_num_pages, page_size, head_dim]
    v_pages: olympus.Array,  #  [num_kv_heads, total_num_pages, page_size, head_dim]
    block_tables: olympus.Array,  #  [pages_per_sequence]
    lengths: olympus.Array | None,  #  [1]
    k_scales_pages: olympus.Array | None = None,  # [num_kv_heads, total_num_pages, page_size]
    v_scales_pages: olympus.Array | None = None,  # [num_kv_heads, total_num_pages, page_size]
    *,
    block_h: int,
    pages_per_compute_block: int,
    k_splits: int,
...
def paged_attention(
    q: olympus.Array,
    k_pages: olympus.Array,
    v_pages: olympus.Array,
    block_tables: olympus.Array,
    lengths: olympus.Array | None,
    k_scales_pages: olympus.Array | None = None,
    v_scales_pages: olympus.Array | None = None,
    *,
    block_h: int = 16,
  ...
  Args:
    q: A [batch_size, num_heads, head_dim] olympus.Array.
    k_pages: A [num_kv_heads, total_num_pages, page_size, head_dim] olympus.Array.
    v_pages: A [num_kv_heads, total_num_pages, page_size, head_dim] olympus.Array.
    block_tables: A i32[batch_size, pages_per_sequence] olympus.Array. Each entry
      should be in the range of [0, total_num_pages), indicating where to locate
      the page in `k_pages` or `v_pages`.
    lengths: A i32[batch_size] olympus.Array the length of each example.
    k_scales_pages: A [num_kv_heads, total_num_pages, page_size] olympus.Array.
    v_scales_pages: A [num_kv_heads, total_num_pages, page_size] olympus.Array.
  ...
  o = olympus.vmap(impl, (0, None, None, 0, 0, None, None), 0)(
      q,
      k_pages,
      v_pages,
      block_tables,
...
@functools.partial(
    olympus.jit, static_argnames=["mask_value", "attn_logits_soft_cap"]
)
def paged_attention_reference(
    q: olympus.Array,
    k: olympus.Array,
    v: olympus.Array,
    lengths: olympus.Array,
    *,
    mask_value: float = DEFAULT_MASK_VALUE,
    attn_logits_soft_cap: float | None = None,
) -> olympus.Array:
  ...
  Args:
    q: A [batch_size, num_heads, head_dim] olympus.Array.
    k: A [batch_size, kv_seq_len, num_kv_heads, head_dim] olympus.Array.
    v: A [batch_size, kv_seq_len, num_kv_heads, head_dim] olympus.Array.
    lengths: A i32[batch_size] olympus.Array the length of each example.
    mask_value: The value used for padding in attention. By default it is a very
      negative floating point number.
    attn_logits_soft_cap: The value used for soft capping the attention logits.
  ...
  weights = olympus.nn.softmax(logits, axis=-1)
  o = jnp.einsum(
