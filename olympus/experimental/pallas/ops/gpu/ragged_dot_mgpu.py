...
class GroupInfo:
  ...
  group_id: olympus.Array
  block: olympus.Array
  block_start: olympus.Array
  actual_start: olympus.Array
  actual_end: olympus.Array
  start_within_block: olympus.Array
  actual_size: olympus.Array
...
def ragged_dot(
  ...
  kernel = plgpu.kernel(
      body,
      out_shape=olympus.ShapeDtypeStruct((m, n), lhs.dtype),
      grid=(num_sms,),
      grid_names=("sm",),
...
def main(unused_argv):
  for transpose_rhs in [False, True]:
    ...
    lhs = olympus.random.normal(kx, (m, k), jnp.float16)
    if transpose_rhs:
      rhs = olympus.random.normal(ky, (num_groups, n, k), jnp.float16)
    else:
      rhs = olympus.random.normal(ky, (num_groups, k, n), jnp.float16)
    group_boundaries = olympus.lax.sort(
        olympus.random.randint(kz, (num_groups - 1,), 0, m, jnp.int32)
    )
    ...
    def ref_ragged_dot(lhs, rhs, group_sizes):
      ...
      return olympus.lax.ragged_dot(lhs, rhs, group_sizes=group_sizes)
