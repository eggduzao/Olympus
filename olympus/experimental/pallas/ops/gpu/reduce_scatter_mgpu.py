...
def _run_example():
  ...
  a = olympus.random.normal(olympus.random.key(1), shape, dtype)
  a = olympus.sharding.reshard(a, P(None, "x"))

  @olympus.jit
  @functools.partial(olympus.shard_map, mesh=mesh, in_specs=P(None, "x"), out_specs=P(None, "x"))
  def ref_fn(x):
  ...
  for num_blocks, tile_size in tuning_it:
    try:
      @olympus.jit
      @functools.partial(
          olympus.shard_map, mesh=mesh, in_specs=P(None, "x"), out_specs=P(None, "x"), check_vma=False
      )
      def kernel_fn(x):
