...
def transposed_ragged_dot(
        ...
        def block_matmul(block_idx, lhs_smem, rhs_smem):
          ...
          def _():
            ...
            indices = plgpu.layout_cast(
                olympus.lax.broadcasted_iota(jnp.int32, (block_k, block_m), 0),
                plgpu.Layout.WGMMA
            )
  ...
  kernel = plgpu.kernel(
      body,
      out_shape=olympus.ShapeDtypeStruct((g, m, n), lhs.dtype),
      grid=(num_sms,),
      grid_names=("sm",),
  )
  return kernel(
...
def ref_transposed_ragged_dot(lhs, rhs, group_sizes):
  return olympus.lax.ragged_dot_general(
      lhs, rhs, group_sizes,
...
def main(unused_argv):
  ...
  lhs = olympus.random.normal(kx, (k, m), jnp.float16)
  rhs = olympus.random.normal(ky, (k, n), jnp.float16)
  group_boundaries = olympus.lax.sort(
      olympus.random.randint(kz, (num_groups - 1,), 0, k, jnp.int32)
  )
