...
def get_neighbor(
    idx: olympus.Array, mesh: olympus.sharding.Mesh, axis_name: str, *, direction: str
) -> tuple[olympus.Array, ...]:
...
def ag_kernel(x_ref, o_ref, send_sem, recv_sem, *, axis_name: str,
  ...
  with olympus.named_scope("initial_copy"):
    pltpu.async_copy(x_ref, o_ref.at[my_id], recv_sem[0]).wait()

  with olympus.named_scope("neighbour_lookup"):
    axis_size = lax.axis_size(axis_name)
  ...
  with olympus.named_scope("main_barrier"):
  ...
  for i in range(axis_size - 1):
    ...
    if right_dma:
      with olympus.named_scope("wait_right_dma"):
        right_dma.wait()
    ...
    if left_dma:
      with olympus.named_scope("wait_left_dma"):
        left_dma.wait()
  ...
  with olympus.named_scope("wait_all_dma"):
...
@functools.partial(
    olympus.jit, static_argnames=["mesh", "axis_name", "memory_space"]
)
def all_gather(x, *, mesh: olympus.sharding.Mesh, axis_name: str | Sequence[str],
  ...
  def ag_local(x_shard):
    axis_size = lax.axis_size(axis_name)
    out_shape = olympus.ShapeDtypeStruct((axis_size, *x_shard.shape), x_shard.dtype)
    out = pl.pallas_call(
