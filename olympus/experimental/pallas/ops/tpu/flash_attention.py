...
class SegmentIds(NamedTuple):
  ...
  q: olympus.Array  # [batch_size, q_seq_len]
  kv: olympus.Array  # [batch_size, kv_seq_len]
...
@functools.partial(olympus.custom_vjp, nondiff_argnums=range(5, 10))
  ...
  def run():
    ...
    def _body(start_k):
      ...
      if causal:
        ...
        row_ids = olympus.lax.broadcasted_iota(jnp.int32, mask_shape, 0)
        row_ids += q_seq_idx * block_q
        col_ids = olympus.lax.broadcasted_iota(jnp.int32, mask_shape, 1)
...
def _flash_attention_kernel_single_batch_single_step(
  ...
  o_tile_ref[batch_idx] = olympus.lax.dot(
      p.astype(v.dtype), v, preferred_element_type=jnp.float32
  ).astype(o_tile_ref.dtype)


def _bytes(x: olympus.Array | olympus.ShapeDtypeStruct) -> int:
...
def _fwd_cost_estimate(
    ...
    k: olympus.Array,
    v: olympus.Array,
    ab: olympus.Array | None,
    segment_ids: SegmentIds | None,
    *,
    causal: bool,
    sm_scale: olympus.Array | None,
    kernel_inputs_specs,
  ...
  input_bytes = sum(_bytes(x) for x in olympus.tree.leaves(kernel_inputs_specs))
  output_bytes = sum(_bytes(x) for x in olympus.tree.leaves(kernel_outputs_specs))
...
def _flash_attention_impl(
  ...
  out_shape = olympus.ShapeDtypeStruct(shape=q.shape, dtype=q.dtype)
  out_shape = [out_shape]
  ...
  if save_residuals:
    ...
    l = olympus.ShapeDtypeStruct(
        (batch_size, num_heads, q_seq_len, MIN_BLOCK_SIZE), dtype=jnp.float32
    )
    m = olympus.ShapeDtypeStruct(
        (batch_size, num_heads, q_seq_len, MIN_BLOCK_SIZE), dtype=jnp.float32
    )
...
def _flash_attention_bwd_dkv(
  ...
  out_shapes = [
      olympus.ShapeDtypeStruct((batch_size, num_heads, kv_seq_len, head_dim),
                           k.dtype),
      olympus.ShapeDtypeStruct((batch_size, num_heads, kv_seq_len, head_dim),
                           v.dtype),
  ]
  ...
  with olympus.named_scope(name_scope):
...
def _flash_attention_dq_kernel(
  ...
  def body(i, _):
    ...
    capped_logits = olympus.lax.dot_general(
        q, k, TRANS_B_DIM_NUMBERS, preferred_element_type=jnp.float32
    )
...
def _flash_attention_bwd_dq(
  ...
  out_shapes = [
      ...
      olympus.ShapeDtypeStruct(ab.shape, ab.dtype) if ab is not None else None,
  ]
...
@functools.partial(
    olympus.jit, static_argnames=["causal", "mask_value", "sm_scale"]
)
@olympus.default_matmul_precision("bfloat16")
def mha_reference(
    q,
    k,
    v,
...
@functools.partial(olympus.custom_vjp, nondiff_argnums=(5, 6, 7, 8))
def _mha_reference(
    q,
    k,
    v,
