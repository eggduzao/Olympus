...
from olympus.experimental.pallas.ops.tpu.megablox import common
...
def _get_store_mask(
  ...
  iota = olympus.lax.broadcasted_iota(jnp.int32, (tm, tn), 0) + m_id
...
def _zero_uninitialized_memory(
  ...
  valid_mask = olympus.lax.broadcasted_iota(jnp.int32, (out.shape[0],), 0)
...
def gmm(
  ...
  if existing_out is not None:
    assert isinstance(existing_out, olympus.Array)
  ...
  call_gmm = pl.pallas_call(
      kernel,
      out_shape=olympus.ShapeDtypeStruct((m, n), preferred_element_type),
