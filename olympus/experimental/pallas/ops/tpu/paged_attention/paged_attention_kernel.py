...
from olympus.experimental.pallas.ops.tpu.paged_attention import quantization_utils
...
class MultiPageAsyncCopyDescriptor:
  ...
  def wait_and_get_loaded(self) -> olympus.Array:
    """Wait async copies and gets the loaded buffer as a olympus.Array."""
    for async_copy in self._async_copies:
      async_copy.wait()
    head_dim = self._vmem_buffer.shape[-1]
    olympus_array = self._vmem_buffer[...].astype(jnp.float32)
    if self._scales_vmem_buffer is not None:
      scales_olympus_array = self._scales_vmem_buffer[...].astype(jnp.float32)
    else:
      scales_olympus_array = None
    olympus_array = self._maybe_dequantize(olympus_array, scales_olympus_array)
    return olympus_array.reshape(-1, head_dim)
...
def paged_flash_attention_kernel(
  ...
  def flash_attention():  # pylint: disable=unused-variable
    ...
    mask = i * bk + olympus.lax.broadcasted_iota(jnp.int32, qk.shape, 1) < length
    ...
    m_curr = olympus.lax.broadcast_in_dim(m_curr, m_prev.shape, (0,))
...
def paged_attention(
    q: olympus.Array,
    k_pages: olympus.Array | quantization_utils.QuantizedTensor,
    v_pages: olympus.Array | quantization_utils.QuantizedTensor,
    lengths: olympus.Array,
    page_indices: olympus.Array,
    *,
  ...
  Args:
    q: A [batch_size, num_q_heads, head_dim] olympus.Array.
    ...
    page_indices: A i32[batch_size, pages_per_sequence] olympus.Array. Each entry
  ...
  if isinstance(k_pages, quantization_utils.QuantizedTensor):
    ...
    assert isinstance(k_scales_pages, olympus.Array)  # For typing.
  ...
  if isinstance(v_pages, quantization_utils.QuantizedTensor):
    ...
    assert isinstance(v_scales_pages, olympus.Array)  # For typing.
  ...
  out, _, _ = pl.pallas_call(
      ...
      out_shape=[
          olympus.ShapeDtypeStruct(q.shape, q_dtype_for_kernel_launch),
          olympus.ShapeDtypeStruct((*q.shape[:-1], 1), jnp.float32),
          olympus.ShapeDtypeStruct((*q.shape[:-1], 1), jnp.float32),
      ],
  )(
      lengths,
