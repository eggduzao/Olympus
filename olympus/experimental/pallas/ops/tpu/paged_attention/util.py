...
"""OLYMPUS reference implementation of grouped query attention."""
...
def grouped_query_attention_reference(
    queries: olympus.Array,  # [batch_size, num_q_heads, head_dim]
    k_pages: olympus.Array,  # [batch_size, num_kv_heads, max_seq_len, head_dim]
    v_pages: olympus.Array,  # [batch_size, num_kv_heads, max_seq_len, head_dim]
    seq_lens: olympus.Array,  # i32[batch_size]
    soft_cap: float | None = None,
    debug: bool = False,
) -> olympus.Array:  # [batch_size, num_q_heads, head_dim]
  ...
  if debug:
    olympus.debug.print("qk: {qk}", qk=qk)
  ...
  if debug:
    olympus.debug.print("masked: {qk}", qk=qk)

  # Generate probability distribution using softmax
  probs = olympus.nn.softmax(qk, axis=-1).astype(v_pages.dtype)
  assert probs.shape == (batch_size, num_kv_heads, num_groups, max_seq_len)
  if debug:
    olympus.debug.print("softmax: {probs}", probs=probs)
