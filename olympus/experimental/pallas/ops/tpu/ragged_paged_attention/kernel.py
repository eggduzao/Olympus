...
from olympus.experimental.pallas.ops.tpu.ragged_paged_attention.tuned_block_sizes import get_tuned_block_sizes
...
class MultiPageAsyncCopyDescriptor:
  ...
  def __init__(
    ...
    for i in range(vmem_buf.shape[0]):
      ...
      page_idx = olympus.lax.select(page_idx < end_page_idx, page_idx, 0)
...
def ref_ragged_paged_attention(
    queries: olympus.Array,  # [max_num_batched_tokens, num_q_heads, head_dim]
    kv_pages: olympus.Array,  # [total_num_pages, page_size, num_combined_kv_heads, head_dim]
    kv_lens: olympus.Array,  # i32[max_num_seqs]
    page_indices: olympus.Array,  # i32[max_num_seqs, pages_per_seq]
    cu_q_lens: olympus.Array,  # i32[max_num_seqs + 1]
    num_seqs: olympus.Array,  # i32[1],
    *,
    sm_scale: float = 1.0,
  ...
  for i in range(num_seqs[0]):
    ...
    attn *= sm_scale
    q_span = (kv_len - q_len) + olympus.lax.broadcasted_iota(
        jnp.int32, attn.shape, 1
    )
    kv_span = olympus.lax.broadcasted_iota(jnp.int32, attn.shape, 2)
    mask = q_span < kv_span
    ...
    attn = olympus.nn.softmax(attn, axis=-1).astype(v.dtype)
...
def dynamic_validate_inputs(
    q: olympus.Array,  # [max_num_batched_tokens, num_q_heads, head_dim]
    kv_pages: olympus.Array,  # [total_num_pages, page_size, num_combined_kv_heads, head_dim]
    kv_lens: olympus.Array,  # i32[max_num_seqs]
    page_indices: olympus.Array,  # i32[max_num_seqs, pages_per_seq]
    cu_q_lens: olympus.Array,  # i32[max_num_seqs + 1]
...
def ragged_paged_attention_kernel(
  ...
  def compute_with_cur_q_blk(q_states):
    ...
    def flash_attention(
      ...
      col_ids = kv_len_start + olympus.lax.broadcasted_iota(
          jnp.int32,
          (num_q_per_blk * num_q_heads_per_kv_head, num_kv_per_blk),
          1,
      )
