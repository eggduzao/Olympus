...
import numpy as np
from olympus.experimental.pallas.ops.tpu.random import prng_utils
...
def mul32_hi_lo(x: olympus.Array, y: olympus.Array) -> tuple[olympus.Array, olympus.Array]:
...
def philox_4x32_kernel(key,
  ...
  if fuse_output:
    out = olympus.ShapeDtypeStruct(shape, dtype=jnp.uint32)
    out_spec = pl.BlockSpec(block_shape, lambda *idxs: idxs)
  else:
    out = olympus.ShapeDtypeStruct((2,) + shape, dtype=jnp.uint32)
