...
from olympus.experimental.pallas.ops.tpu.splash_attention import splash_attention_mask_info as mask_info_lib
...
class SegmentIds(NamedTuple):
  ...
  q: olympus.Array  # [q_seq_len]
  kv: olympus.Array  # [kv_seq_len]
...
SplashCustomReturnType = Union[
    ...
    olympus.Array,
    # out, residuals:
    tuple[olympus.Array, tuple[olympus.Array,]]
]
...
SplashResidualsType = tuple[
    ...
    Optional[olympus.Array],   # sinks
    olympus.Array,  # out
    olympus.Array,  # logsumexp
    Optional[mask_info_lib.MaskInfo],  # dq_mask_info
    Optional[mask_info_lib.MaskInfo],  # dkv_mask_info
]

MaskFunctionType = Callable[..., olympus.Array]
...
def _attention_reference(
    mask: olympus.Array,  # [q_seq_len, kv_seq_len]
    q: olympus.Array,  # [q_seq_len, head_dim]
    k: olympus.Array,  # [kv_seq_len, head_dim]
    v: olympus.Array,  # [kv_seq_len, head_dim]
    segment_ids: SegmentIds | None,
    sinks: olympus.Array | None,
    mask_value: float,
    save_residuals: bool,
...
def _attention_reference_default(
    ...
    v: olympus.Array,  # [kv_seq_len, head_dim]
    segment_ids: SegmentIds | None,
    sinks: olympus.Array | None,  # [] one scalar per qhead
    mask_value: float,
    save_residuals: bool,
    custom_type: str,
    attn_logits_soft_cap: float | None,
...
def _attention_reference_custom_bwd(
    ...
    res,
    do: olympus.Array,
) -> tuple[None, olympus.Array, olympus.Array, olympus.Array, None, olympus.Array | None]:
  del save_residuals
...
_attention_reference_custom = olympus.custom_vjp(
...
def make_attention_reference(
  ...
  def _wrapped(
    ...
    if is_mqa:
      func = olympus.vmap(func, in_axes=(0, 0, None, None, None, 0))
      is_grouped = False
    else:
      ...
      func = olympus.vmap(func, in_axes=(0, 0, 0, 0, None, 0))
...
def _apply_mask_and_soft_cap(
    ...
    *,
    ...
    k_slice: pl.Slice,
    k_offset: int | olympus.Array,
    bq: int,
    k_in_lanes=True,
    mask_function=None,
) -> olympus.Array | tuple[olympus.Array, olympus.Array, olympus.Array, olympus.Array]:
  ...
  if mask_function is not None:
    ...
    if k_in_lanes:
      ...
      k_sequence = k_offset + olympus.lax.broadcasted_iota(
...
def flash_attention_kernel(
  ...
  def body(kv_compute_index, _):
    ...
    l_curr = olympus.lax.broadcast_in_dim(s_curr.sum(axis=-1), l_prev.shape, (0,))
...
def _splash_attention_forward(
  ...
  if segment_ids is not None:
    ...
    q_segment_ids = olympus.lax.broadcast_in_dim(
        segment_ids.q, (q_seq_len, NUM_LANES), (0,)
    )
    kv_segment_ids = olympus.lax.broadcast_in_dim(
  ...
  if fwd_mask_info.q_sequence is not None:
    q_sequence = olympus.lax.broadcast_in_dim(
  ...
  out_shapes = [
      olympus.ShapeDtypeStruct((bq, NUM_LANES), jnp.float32),  # m_scratch
      olympus.ShapeDtypeStruct((bq, NUM_LANES), jnp.float32),  # l_scratch
      olympus.ShapeDtypeStruct((bq, head_dim_v), jnp.float32),  # o_scratch
      olympus.ShapeDtypeStruct((num_q_heads, q_seq_len, head_dim_v), q.dtype),
  ]
  out_specs = [
  ...
  with olympus.named_scope(kernel_name):
...
@partial(olympus.custom_vjp, nondiff_argnames=(
...
def _splash_attention_bwd_dq(
  ...
  out_shapes = [
      olympus.ShapeDtypeStruct((bq, head_dim_qk), jnp.float32),
      olympus.ShapeDtypeStruct(q.shape, q.dtype),
  ]
  out_specs = [
...
def _splash_attention_bwd_dkv(
  ...
  if use_fused_bwd_kernel:
    ...
    dq_shape = olympus.ShapeDtypeStruct((kv_seq_len // bkv, *q.shape), q.dtype)
    if bkv == bkv_compute:
      dq_scratch_spec = dq_scratch_shape = None
    else:
      dq_scratch_spec = pl.BlockSpec((bq, head_dim_qk), lambda *_: (0, 0))
      dq_scratch_shape = olympus.ShapeDtypeStruct((bq, head_dim_qk), jnp.float32)
  else:
  ...
  out_shapes = [
      dq_scratch_shape,
      olympus.ShapeDtypeStruct((bkv, head_dim_qk), jnp.float32),
      olympus.ShapeDtypeStruct((bkv, head_dim_v), jnp.float32),
      dq_shape,
      olympus.ShapeDtypeStruct(k.shape, k.dtype),
      olympus.ShapeDtypeStruct(v.shape, v.dtype),
  ]
  out_specs = [
      dq_scratch_spec,
...
class SplashAttentionKernel:
  ...
  def manual_sharding_spec(self, sharding: olympus.sharding.NamedSharding):
    ...
    spec = sharding.spec
    assert len(spec) == 2
    replicated = olympus.sharding.PartitionSpec()
    ...
    q_sequence_spec = olympus.sharding.PartitionSpec(spec[1])
...
def _make_splash_attention(
    mask: np.ndarray | olympus.Array | mask_lib.MultiHeadMask,
    *,
  ...
  process_mask_fn = (
      ...
      if isinstance(mask, olympus.Array)
