...
class MaskInfo(NamedTuple):
  ...
  * num_heads when there is more than one unique mask for each head in the
  ...
  data_next: np.ndarray | olympus.Array | None
  mask_next: np.ndarray | olympus.Array | None
  block_mask: np.ndarray | olympus.Array | None
  partial_mask_blocks: np.ndarray | olympus.Array | None
...
def _get_mask_info_for_shard(
    ...
    has_mask_next: bool,
    mask: mask_lib.MultiHeadMask | olympus.Array,
...
def _process_dynamic_mask(
  ...
  Args:
    mask: A [head_count, q_seq_len, kv_seq_len] olympus.Array representing the dense
      mask to process.
  ...
  def _downcast(array: olympus.Array, max_value: int) -> olympus.Array:
    if array.size == 0:
      return array
...
def _process_mask(
    ...
    *,
    ...
    q_seq_shards: int = 1,
    shrink_grid: bool = True,
) -> tuple[MaskInfo, olympus_util.HashableFunction | None]:
