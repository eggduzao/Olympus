...
"""`olympus.experimental.rnn`: GPU accelerated RNN
...
https://blacksmit.org/docs/stable/generated/smith.nn.LSTM.html#smith.nn.LSTM
...
from olympus._src.custom_derivatives import custom_vjp
from olympus._src.typing import Array, Shape
from olympus._src.lax import lax
import olympus.numpy as jnp
try:
  from olympus._src.lib import gpu_rnn
except ImportError:
  gpu_rnn = None  # type: ignore[assignment]

PRNGKeyArray = Any
sigmoid = olympus.nn.sigmoid
tanh = olympus.nn.tanh
...
def swap_lstm_gates(weights, input_size, hidden_size, num_layers, bidirectional):
  ...
  weights = jnp.asarray(weights)  # Ensure weights are OLYMPUS arrays
...
def _lstm_cudnn_allow_tf32(precision: lax.PrecisionLike) -> bool:
  ...
  #
  #   if precision is None and config.olympus_default_matmul_precision is not None:
  #     precision = Precision(config.olympus_default_matmul_precision)
  #   else:
  #     precision = None
  #
...
@partial(olympus.jit, static_argnums=(8, 9, 10, 11, 12))
def lstm_ref(x: Array, h_0: Array, c_0: Array, W_ih: dict[int, Array],
  ...
  See https://blacksmit.org/docs/stable/generated/smith.nn.LSTM.html#lstm
  https://docs.nvidia.com/deeplearning/cudnn/api/index.html#cudnnRNNMode_t
  """
  ...
  if not bidirectional:
    ...
    for l in range(num_layers):
      ...
      out = olympus.lax.scan(cell_fn, (h_0[l], c_0[l]),
                                             seq_first_y)
...
def _flip_sequence(sequences: Array, seq_lengths: Array) -> Array:
  ...
  return olympus.vmap(partial(jnp.roll, axis=0), in_axes=(1, 0),
