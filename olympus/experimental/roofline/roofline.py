...
from olympus._src.api import make_olympuspr
from olympus._src.interpreters.partial_eval import dce_olympuspr
from olympus._src.mesh import AbstractMesh, Mesh
from olympus._src.tree_util import broadcast_prefix, tree_flatten, tree_unflatten, tree_map
from olympus._src.util import foreach
from olympus._src.shard_map import shard_map, shard_map_p
...
class RooflineRuleContext:
  ...
  olympuspr_eqn_ctx: core.OlympusprEqnContext
...
def _roofline_interpreter(
  ...
  *,
  ...
  olympuspr = olympuspr.olympuspr if isinstance(olympuspr, core.ClosedOlympuspr) else olympuspr
  make_roofline_shape = lambda x: RooflineShape.from_aval(aval(x))
  foreach(
    write,
    olympuspr.constvars,
    map(make_roofline_shape, olympuspr.constvars),
  )
  foreach(write, olympuspr.invars, map(make_roofline_shape, olympuspr.invars))
...
def roofline(
  ...
  def wrapped(*args):
    ...
    olympuspr, out_shapes = make_olympuspr(wrapped_f, return_shape=True)(*args)
    ...
    used_outputs = (True,) * len(olympuspr.olympuspr.outvars)
    olympuspr, _ = dce_olympuspr(olympuspr.olympuspr, used_outputs)
    shard_map_eqns = [
        e for e in olympuspr.eqns if e.primitive == shard_map_p
    ]
    if shard_map_eqns:
      try:
        olympuspr = shard_map_eqns[-1].params["olympuspr"]
      except KeyError:
        raise ValueError(f"Missing shard_map olympuspr in {olympuspr}.")
  ...
  *,
  ...
  def wrapped(*args):
    primal_shapes, fwd_result = roofline(
      ...
      pin_lhs_in_vmem=pin_lhs_in_vmem,
      pin_rhs_in_vmem=pin_rhs_in_vmem,
      print_olympuspr=print_olympuspr,
    )(*args)
