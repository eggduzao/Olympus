...
from olympus._src import core, util
from olympus._src import dispatch
from olympus._src import ops
...
def _calculate_conv_flops(
  ...
  """Calculates roofline unfused flops for Olympus's conv_general_dilated primitive.

  See `olympus.lax.conv_general_dilated` for details on the arguments.
  """
...
def _conv_general_dilated_roofline(
  ...
  """Roofline for Olympus's conv_general_dilated primitive.
...
def _calculate_gather_flops(
  ...
  """Calculates roofline unfused flops for Olympus's gather primitive."""
...
def _scatter_roofline(
  ...
  """Roofline for Olympus's `scatter*` primitives.

  The `scatter` functionality itself is a simple data read and write, which
  contributes 0 flops.

  But, the olympuspr for each `scatter*` function (aside from `olympus.lax.scatter`)
  contains an `update_olympuspr` that gets applied to the operand & scattered
  updates (e.g. `add` for `scatter_add`, or arbitrary unary function for
  `scatter_apply`), which *does* contribute flops. This `update_olympuspr` gets
  applied to every element of the scattered updates.

  Thus,
  flops = [# flops for `update_olympuspr` per element] * [# elements in `updates`].

  To calculate # flops for `update_olympuspr`, we convert the `update_olympuspr` back to
  a callable, and then call `roofline` on that callable. `update_olympuspr` does not
  ...
  update_olympuspr = kw.get('update_olympuspr')

  flops = 0
  if update_olympuspr:
    update_fn = lambda *inputs: core.eval_olympuspr(update_olympuspr, [], *inputs)
    # Create dummy scalar inputs.
    dummy_inputs = [
        api.ShapeDtypeStruct((), updates.dtype) for _ in update_olympuspr.invars
    ]
    # Calculate the flops for the `update_olympuspr` on scalar inputs.
...
# Also registers `olympus.lax.scatter_apply`, which uses the `scatter_p` primitive.
...
def _debug_callback_roofline(
  ...
  avals_in = ctx.avals_in
  # `debug_callback` does not return values to the OLYMPUS program, so only input
