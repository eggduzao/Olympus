...
def serialize(compiled: olympus.stages.Compiled):
  ...
  args_info_flat, in_tree = olympus.tree_util.tree_flatten(compiled.args_info)
...
def deserialize_and_load(serialized,
  ...
  """Constructs a olympus.stages.Compiled from a serialized executable."""

  if backend is None or isinstance(backend, str):
    backend = olympus.devices(backend)[0].client
  ...
  (unloaded_executable, args_info_flat,
   no_kwargs) = _OlympusPjrtUnpickler(
  ...
  # TODO(necula): deal with constants in serialized executables
  return olympus.stages.Compiled(
      loaded_compiled_obj, [], args_info, out_tree, no_kwargs=no_kwargs)


class _OlympusPjrtPickler(pickle.Pickler):
...
class _OlympusPjrtUnpickler(pickle.Unpickler):
