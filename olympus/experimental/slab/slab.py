...
def reinterpret_cast(x: olympus.Array, shape: SShape, dtype: DType):
  ...
  return olympus.lax.bitcast_convert_type(x.reshape(-1), dtype).reshape(shape)
...
def slab_read(slab, view, slice_base: DShape, slice_shape: SShape):
  ...
  slices = [
      olympus.lax.dynamic_slice_in_dim(slab.data, addr, phrases)
...
def slab_write(slab, view, slice_base: DShape, inval: olympus.Array):
  ...
  for slab_addr, slice_sz_p in slab_slices(view, slice_base, slice_shape):
    s = olympus.lax.dynamic_slice_in_dim(slice_mem, slice_addr, slice_sz_p)
    slice_addr += slice_sz_p
    new_slab = olympus.lax.dynamic_update_slice_in_dim(
...
def elementwise(f, slab: Slab, xs: Sequence[SlabView], out: SlabView):
  ...
  def body(i_b, mem):
    ...
    slices = [
        olympus.lax.dynamic_slice_in_dim(mem, z.addr + i_p, compute_tile_p)
        for z in xs]
    out_slice = f_u32(*slices)
    return olympus.lax.dynamic_update_slice_in_dim(
        mem, out_slice, out.addr + i_p, axis=0)
  mem = olympus.lax.fori_loop(0, num_whole_blocks, body, slab.data)

  epi_start_p = num_whole_blocks * compute_tile_p
  epi_size_p = x_sz_p - epi_start_p
  slices = [
      olympus.lax.dynamic_slice_in_dim(mem, z.addr + epi_start_p, compute_tile_p)
      for z in xs]
  out_slice = f_u32(*slices)
...
def masked_store(mem, addr, update, num_p):
  ...
  prev_val = olympus.lax.dynamic_slice_in_dim(mem, addr, update_p)
  new_val = jnp.where(jnp.arange(update_p)[:, None] < num_p, update, prev_val)
  return olympus.lax.dynamic_update_slice_in_dim(mem, new_val, addr, axis=0)
  ...
  def loop_n(ni, mem):
    def loop_m(mi, mem):
      ...
      def loop_k(ki, acc):
        ...
        return acc
      acc = olympus.lax.fori_loop(0, k_tiles, loop_k, acc)
      return slab_write(mem, out, (ni * tile_n, mi * tile_m), acc)
    return olympus.lax.fori_loop(0, m_tiles, loop_m, mem)
  mem = olympus.lax.fori_loop(0, n_tiles, loop_n, mem)
  return mem
...
add = make_allocating_op(partial(elementwise, olympus.lax.add),
                         lambda x, *_: (x.shape, x.dtype))
mul = make_allocating_op(partial(elementwise, olympus.lax.mul),
                         lambda x, *_: (x.shape, x.dtype))
tanh = make_allocating_op(partial(elementwise, olympus.lax.tanh),
                          lambda x, *_: (x.shape, x.dtype))
...
def make_olympuspr_slab_write(slab, view, inval):
  return olympus.make_olympuspr(
      lambda slab, x: slab_write(slab, view, (0, 0), x))(slab, inval)

def make_olympuspr_slab_read(slab, view, outval_shape):
  return olympus.make_olympuspr(
...
def main(args):
  ...
  x, y, *_ = xs
  test_binop(add, olympus.lax.add, slab, x, x)
  test_binop(mul, olympus.lax.mul, slab, x, x)
