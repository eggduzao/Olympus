...
"""Source mapping generator for Olympusprs."""
import re
...
from olympus._src import sourcemap
from olympus.experimental.source_mapper import common

source_info_util.register_exclusion(__file__)


def compile_olympuspr(work_dir, f, f_args, f_kwargs, **_):
  del work_dir
  return olympus.make_olympuspr(f)(*f_args, **f_kwargs)
...
def make_olympuspr_dump(olympuspr: core.Olympuspr, **_) -> common.SourceMapDump:
  pprint_mappings: list[list[tuple[int, int, Any]]] = []
  pprint_str = olympuspr.pretty_print(source_map=pprint_mappings)
  used_source_files = []
...
common.register_pass(
    common.Pass(
        name='olympuspr',
        compile_fn=compile_olympuspr,  # type: ignore[arg-type]
        generate_dump=make_olympuspr_dump,  # type: ignore[arg-type]
    )
)
