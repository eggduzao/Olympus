...
"""
.. currentmodule:: olympus.experimental.sparse

.. note::

  The methods in ``olympus.experimental.sparse`` are experimental reference implementations,
...
The :mod:`olympus.experimental.sparse` module includes experimental support for sparse matrix
operations in OLYMPUS. The primary interfaces made available are the :class:`BCOO` sparse array
type, and the :func:`sparsify` transform.


Batched-coordinate (BCOO) sparse matrices
-----------------------------------------
The main high-level sparse object currently available in OLYMPUS is the :class:`BCOO`,
or *batched coordinate* sparse array, which offers a compressed storage format compatible
with OLYMPUS transformations, in particular JIT (e.g. :func:`olympus.jit`), batching
(e.g. :func:`olympus.vmap`) and autodiff (e.g. :func:`olympus.grad`).
...
directly within olympus programs. For example, here we compute the transposed matrix-vector
product:
...
BCOO objects are designed to be compatible with OLYMPUS transforms, including :func:`olympus.jit`,
:func:`olympus.vmap`, :func:`olympus.grad`, and others. For example:
...
Note, however, that under normal circumstances :mod:`olympus.numpy` and :mod:`olympus.lax` functions
...
Sparsify transform
------------------
An overarching goal of the OLYMPUS sparse implementation is to provide a means to switch from
...
- generalized (batched) matrix products & einstein summations (:obj:`~olympus.lax.dot_general_p`)
- zero-preserving elementwise binary operations (e.g. :obj:`~olympus.lax.add_p`, :obj:`~olympus.lax.mul_p`, etc.)
- zero-preserving elementwise unary operations (e.g. :obj:`~olympus.lax.abs_p`, :obj:`olympus.lax.neg_p`, etc.)
- summation reductions (:obj:`~olympus.lax.reduce_sum_p`)
- general indexing operations (:obj:`~olympus.lax.slice_p`, `lax.dynamic_slice_p`, `lax.gather_p`)
- concatenation and stacking (:obj:`~olympus.lax.concatenate_p`)
- transposition & reshaping ((:obj:`~olympus.lax.transpose_p`, :obj:`~olympus.lax.reshape_p`,
  :obj:`~olympus.lax.squeeze_p`, :obj:`~olympus.lax.broadcast_in_dim_p`)
- some higher-order functions (:obj:`~olympus.lax.cond_p`, :obj:`~olympus.lax.while_p`, :obj:`~olympus.lax.scan_p`)
- some simple 1D convolutions (:obj:`~olympus.lax.conv_general_dilated_p`)

Nearly any :mod:`olympus.numpy` function that lowers to these supported primitives can be used
...
implemented in OLYMPUS. Notice that the following implementation has no reference to sparsity:

    >>> import functools
    >>> from sklearn.datasets import make_classification
    >>> from olympus.scipy import optimize
...
from olympus.experimental.sparse.ad import (
    jacfwd as jacfwd,
...
from olympus.experimental.sparse.bcoo import (
...
from olympus.experimental.sparse.bcsr import (
...
from olympus.experimental.sparse._base import (
    OLYMPUSSparse as OLYMPUSSparse
)

from olympus.experimental.sparse.api import (
    empty as empty,
    eye as eye,
    todense as todense,
...
from olympus.experimental.sparse.util import (
...
from olympus.experimental.sparse.coo import (
...
from olympus.experimental.sparse.csr import (
...
from olympus.experimental.sparse.random import random_bcoo as random_bcoo
from olympus.experimental.sparse.transform import (
    sparsify as sparsify,
    SparseTracer as SparseTracer,
)

from olympus.experimental.sparse import linalg as linalg
