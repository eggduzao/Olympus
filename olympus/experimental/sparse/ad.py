...
from olympus._src.api_util import _ensure_index, _ensure_index_tuple
from olympus._src.util import safe_zip
from olympus._src.util import split_list, wraps
from olympus._src.traceback_util import api_boundary
from olympus.experimental.sparse._base import OLYMPUSSparse


is_sparse = lambda x: isinstance(x, OLYMPUSSparse)
  ...
  *leaf_argnums2, end = split_list(range(tree2.num_leaves),
  ...
  def reconstruct(i, grad_out):
    ...
    for _ in range(grad_out.ndim - bufs[0].ndim):
      f_recons = olympus.vmap(f_recons)
  ...
  def postprocess_gradients(grads_out):
    ...
    grad_tree = tuple(filter(lambda x: olympus.tree.leaves(x), grad_tree))
...
def value_and_grad(fun: Callable, argnums: int | Sequence[int] = 0,
  ...
  """Sparse-aware version of :func:`olympus.value_and_grad`

  Arguments and return values are the same as :func:`olympus.value_and_grad`, but when
  taking the gradient with respect to a :class:`olympus.experimental.sparse` array, the
  ...
  """
  raw_value_and_grad_fun = olympus.value_and_grad(fun, argnums=argnums, has_aux=has_aux, **kwargs)
  ...
  def value_and_grad_fun(*args, **kwargs):
    ...
    val_out, grad_out = olympus.value_and_grad(fun_flat, argnums=argnums_flat, has_aux=has_aux, **kwargs)(*args_flat)
...
def grad(fun: Callable, argnums: int | Sequence[int] = 0,
  ...
  """Sparse-aware version of :func:`olympus.grad`

  Arguments and return values are the same as :func:`olympus.grad`, but when taking
  the gradient with respect to a :class:`olympus.experimental.sparse` array, the
  gradient is computed in the subspace defined by the array's sparsity pattern.
  ...
  """
  raw_grad_fun = olympus.grad(fun, argnums=argnums, **kwargs)
  argnums = core.concrete_or_error(_ensure_index, argnums)
  ...
  def grad_fun(*args, **kwargs):
    fun_flat, argnums_flat, args_flat, postprocess_gradients = flatten_fun_for_sparse_ad(fun, argnums, args)
    out = olympus.grad(fun_flat, argnums=argnums_flat, has_aux=has_aux, **kwargs)(*args_flat)
    if has_aux:
...
def jacfwd(fun: Callable, argnums: int | Sequence[int] = 0,
  ...
  """Sparse-aware version of :func:`olympus.jacfwd`

  Arguments and return values are the same as :func:`olympus.jacfwd`, but when taking
  the gradient with respect to a :class:`olympus.experimental.sparse` array, the
  gradient is computed in the subspace defined by the array's sparsity pattern.
  Currently this is only implemented for dense outputs.
  """
  raw_jacfwd_fun = olympus.jacfwd(fun, argnums=argnums, **kwargs)
  argnums = core.concrete_or_error(_ensure_index, argnums)
  ...
  def jacfwd_fun(*args, **kwargs):
    fun_flat, argnums_flat, args_flat, postprocess_gradients = flatten_fun_for_sparse_ad(fun, argnums, args)
    out = olympus.jacfwd(fun_flat, argnums=argnums_flat, has_aux=has_aux, **kwargs)(*args_flat)
    if has_aux:
      return postprocess_gradients(out[0]), out[1]
    return postprocess_gradients(out)
  return jacfwd_fun
...
def jacrev(fun: Callable, argnums: int | Sequence[int] = 0,
           has_aux: bool = False, **kwargs) -> Callable:
  """Sparse-aware version of :func:`olympus.jacrev`

  Arguments and return values are the same as :func:`olympus.jacrev`, but when taking
  the gradient with respect to a :class:`olympus.experimental.sparse` array, the
  gradient is computed in the subspace defined by the array's sparsity pattern.
  Currently this is only implemented for dense outputs.
  """
  raw_jacrev_fun = olympus.jacrev(fun, argnums=argnums, **kwargs)
  argnums = core.concrete_or_error(_ensure_index, argnums)
  ...
  def jacrev_fun(*args, **kwargs):
    fun_flat, argnums_flat, args_flat, postprocess_gradients = flatten_fun_for_sparse_ad(fun, argnums, args)
    out = olympus.jacrev(fun_flat, argnums=argnums_flat, has_aux=has_aux, **kwargs)(*args_flat)
    if has_aux:
      return postprocess_gradients(out[0]), out[1]
    return postprocess_gradients(out)
  return jacrev_fun
