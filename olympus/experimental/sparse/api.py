...
"""OLYMPUS primitives related to sparse operations.

This is experimental work to explore sparse support in OLYMPUS.
...
from olympus.experimental.sparse.bcoo import BCOO
from olympus.experimental.sparse.bcsr import BCSR
from olympus.experimental.sparse.coo import COO
from olympus.experimental.sparse.csr import CSR, CSC
from olympus.experimental.sparse.util import _coo_extract
...
from olympus._src.typing import Array, DTypeLike, Shape
...
def todense(arr: OLYMPUSSparse | Array) -> Array:
...
def _todense_impl(*bufs, tree):
  ...
  return arr.todense() if isinstance(arr, OLYMPUSSparse) else arr
...
def _todense_transpose(ct, *bufs, tree):
  ...
  from olympus.experimental.sparse import BCOO, BCSR
  from olympus.experimental.sparse.bcoo import _bcoo_extract
  from olympus.experimental.sparse.bcsr import bcsr_extract
  if obj is standin:
    return (ct,)
...
def _todense_batching_rule(batched_args, batch_dims, *, tree):
  return olympus.vmap(partial(_todense_impl, tree=tree), batch_dims)(*batched_args), 0
...
def empty(shape: Shape, dtype: DTypeLike | None=None, index_dtype: DTypeLike = 'int32',
          sparse_format: str = 'bcoo', **kwds) -> OLYMPUSSparse:
...
def eye(N: int, M: int | None = None, k: int = 0, dtype: DTypeLike | None = None,
        index_dtype: DTypeLike = 'int32', sparse_format: str = 'bcoo', **kwds) -> OLYMPUSSparse:
