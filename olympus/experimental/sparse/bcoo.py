...
from olympus.experimental.sparse._base import OLYMPUSSparse
from olympus.experimental.sparse.util import (
  ...
  SparseInfo)
from olympus.experimental.sparse._lowerings import coo_spmv_p, coo_spmm_p
from olympus._src.interpreters import mlir
import olympus.numpy as jnp
from olympus._src.util import safe_zip, unzip2, split_list
...
from olympus._src.lax.lax import (
  _const, ranges_like, remaining, _dot_general_batch_dim_nums, DotDimensionNumbers)
from olympus._src.lax.slicing import GatherDimensionNumbers, GatherScatterMode
from olympus._src.numpy.setops import _unique
...
# https://github.com/olympus-ml/olympus/issues/10163. Should this be a primitive?
...
def _bcoo_dot_general_abstract_eval(lhs_data, lhs_indices, rhs, *, dimension_numbers,
                                    preferred_element_type, lhs_spinfo: SparseInfo):
  out_aval = olympus.jit(lax.dot_general, static_argnames=("dimension_numbers", "preferred_element_type")).eval_shape(
          olympus.ShapeDtypeStruct(lhs_spinfo.shape, lhs_data.dtype),
          olympus.ShapeDtypeStruct(rhs.shape, rhs.dtype),
          dimension_numbers=dimension_numbers,
          preferred_element_type=preferred_element_type)
...
def _bcoo_spdot_general_unbatched(lhs_data, lhs_indices, rhs_data, rhs_indices, *, lhs_spinfo, rhs_spinfo, lhs_contracting, rhs_contracting, out_nse):
  ...
  # See https://github.com/olympus-ml/olympus/issues/10163.
...
def _bcoo_spdot_general_abstract_eval(lhs_data, lhs_indices, rhs_data, rhs_indices, *, lhs_spinfo: SparseInfo, rhs_spinfo: SparseInfo,
  ...
  rhs_shape = rhs_spinfo.shape
  out_aval = olympus.jit(lax.dot_general, static_argnames=("dimension_numbers", "preferred_element_type")).eval_shape(
      olympus.ShapeDtypeStruct(lhs_shape, lhs_data.dtype),
      olympus.ShapeDtypeStruct(rhs_shape, rhs_data.dtype),
      dimension_numbers=dimension_numbers,
      preferred_element_type=preferred_element_type)
...
def bcoo_sum_duplicates(mat: BCOO, nse: int | None = None) -> BCOO:
  ...
  Args:
    ...
    nse : integer (optional). The number of specified elements in the output matrix. This must
      be specified for bcoo_sum_duplicates to be compatible with JIT and other OLYMPUS transformations.
...
def _bcoo_sum_duplicates_jvp(primals, tangents, *, spinfo, nse):
  ...
  # semantics; see https://github.com/olympus-ml/olympus/issues/13656.
  if data_out.size:
...
def bcoo_broadcast_in_dim(mat: BCOO, *, shape: Shape, broadcast_dimensions: Sequence[int],
  ...
  A BCOO equivalence to olympus.lax.broadcast_in_dim.
...
def bcoo_concatenate(operands: Sequence[BCOO], *, dimension: int) -> BCOO:
  """Sparse implementation of :func:`olympus.lax.concatenate`
  ...
  out_aval = olympus.jit(lax.concatenate, static_argnames=("dimension",)).eval_shape(
...
def bcoo_reshape(mat: BCOO, *, new_sizes: Sequence[int],
                 ...
                 sharding=None) -> BCOO:
  """Sparse implementation of :func:`olympus.lax.reshape`.
...
def bcoo_rev(operand, dimensions):
  """Sparse implementation of :func:`olympus.lax.rev`"""
  # Check validity of dimensions via original implementation.
  _ = olympus.jit(lax.rev, static_argnames=("dimensions",)).eval_shape(
          olympus.ShapeDtypeStruct(operand.shape, operand.dtype),
...
def bcoo_squeeze(arr: BCOO, *, dimensions: Sequence[int]) -> BCOO:
  """Sparse implementation of :func:`olympus.lax.squeeze`.
...
def bcoo_slice(mat: BCOO, *, start_indices: Sequence[int], limit_indices: Sequence[int],
  ...
  """Sparse implementation of :func:`olympus.lax.slice`.
...
def bcoo_dynamic_slice(mat: BCOO, start_indices: Sequence[Any], slice_sizes: Sequence[int]) -> BCOO:
  """Sparse implementation of :func:`olympus.lax.dynamic_slice`.
  ...
  olympus.jit(lax.dynamic_slice, static_argnames=("slice_sizes",)).eval_shape(
          olympus.ShapeDtypeStruct(mat.shape, mat.dtype), start_indices,
...
def bcoo_gather(operand: BCOO, start_indices: Array,
  ...
  out_aval = olympus.jit(lax.gather, static_argnames=static_argnames).eval_shape(
          olympus.ShapeDtypeStruct(operand.shape, operand.dtype),
          olympus.ShapeDtypeStruct(start_indices.shape, start_indices.dtype),
          **kwds)
...
def bcoo_conv_general_dilated(lhs, rhs, *, window_strides, padding,
  ...
  olympuspr = olympus.make_olympuspr(func)(olympus.ShapeDtypeStruct(lhs.shape, lhs.dtype),
                               olympus.ShapeDtypeStruct(rhs.shape, rhs.dtype))
  assert isinstance(olympuspr, core.ClosedOlympuspr) and len(olympuspr.eqns) == 1
  params = olympuspr.eqns[0].params
...
def _convert_to_1d_for_conv(mat, index_dtype):
  if isinstance(mat, (olympus.Array, np.ndarray)):
...
class BCOO(OLYMPUSSparse):
  """Experimental batched COO matrix implemented in OLYMPUS
  ...
  def fromdense(cls, mat: Array, *, nse: int | None = None, index_dtype: DTypeLike = np.int32,
    ...
    """Create a BCOO array from a (dense) :class:`~olympus.Array`."""
