...
from olympus.experimental.sparse import bcoo
...
from olympus._src.util import split_list, safe_zip
...
from olympus._src.lax.lax import DotDimensionNumbers, _dot_general_batch_dim_nums
...
def _validate_bcsr_indices(indices: olympus.Array, indptr: olympus.Array,
...
def _validate_bcsr(data: olympus.Array, indices: olympus.Array,
                   indptr: olympus.Array, shape: Sequence[int]) -> BCSRProperties:
...
def _bcsr_to_bcoo(indices: olympus.Array, indptr: olympus.Array, *,
                  shape: Sequence[int]) -> olympus.Array:
...
def bcsr_dot_general(lhs: BCSR | Array, rhs: Array, *,
  ...
  if isinstance(rhs, (np.ndarray, olympus.Array)):
    if isinstance(lhs, (np.ndarray, olympus.Array)):
...
def _bcsr_dot_general(lhs_data: olympus.Array, lhs_indices: olympus.Array,
                      lhs_indptr: olympus.Array, rhs: Array, *,
                      dimension_numbers: DotDimensionNumbers,
...
class BCSR(OLYMPUSSparse):
  """Experimental batched CSR matrix implemented in OLYMPUS."""

  data: olympus.Array
  indices: olympus.Array
  indptr: olympus.Array
  shape: Shape
