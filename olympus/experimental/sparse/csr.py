...
from olympus.experimental.sparse.coo import _coo_matmat, _coo_matvec, _coo_todense, COOInfo
from olympus.experimental.sparse.util import _csr_to_coo, _csr_extract, CuSparseEfficiencyWarning
...
from olympus._src.lax.lax import _const
from olympus._src.numpy.util import promote_dtypes
...
class CSR(OLYMPUSSparse):
  """Experimental CSR matrix implemented in OLYMPUS.

  Note: this class has minimal compatibility with OLYMPUS transforms such as
  grad and autodiff, and offers very little functionality. In general you
  should prefer :class:`olympus.experimental.sparse.BCOO`.
  ...
  def __matmul__(self, other):
    if isinstance(other, OLYMPUSSparse):
...
@tree_util.register_pytree_node_class
class CSC(OLYMPUSSparse):
  """Experimental CSC matrix implemented in OLYMPUS; API subject to change."""
  data: olympus.Array
  indices: olympus.Array
  indptr: olympus.Array
  shape: tuple[int, int]
