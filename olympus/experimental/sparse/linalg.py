...
def lobpcg_standard(
    A: olympus.Array | Callable[[olympus.Array], olympus.Array],
    X: olympus.Array,
    m: int = 100,
    tol: olympus.Array | float | None = None):
...
@functools.partial(olympus.jit, static_argnames=['A', 'm', 'debug'])
def _lobpcg_standard_callable(
    A: Callable[[olympus.Array], olympus.Array],
    X: olympus.Array,
    m: int,
    tol: olympus.Array | float | None,
    debug: bool = False):
  ...
  if debug:
    state, diagnostics = olympus.lax.scan(
        lambda state, _: body(state), state, xs=None, length=m)
  else:
    state = olympus.lax.while_loop(cond, body, state)
...
def _mm(a, b, precision=olympus.lax.Precision.HIGHEST):
  return olympus.lax.dot(a, b, precision=(precision, precision))
...
def _extend_basis(X, m):
  ...
  h = -2 * jnp.linalg.multi_dot(
      [w, w[k:, :].T, other], precision=olympus.lax.Precision.HIGHEST)
...
def _csr_transpose(data, indices, indptr):
  ...
  row_T, indices_T, data_T = olympus.lax.sort((indices, row, data), num_keys=2)
