...
from olympus._src.lax.lax import DotDimensionNumbers
...
def is_sparse(x):
  return isinstance(x, sparse.OLYMPUSSparse)
...
class SparseTestCase(jtu.OlympusTestCase):
  ...
  def _CheckAgainstDense(self, dense_op, sparse_op, args_maker, check_jit=True,
    ...
    if check_jit:
      sparse_ans_jit = olympus.jit(sparse_op)(*sparse_args)
  ...
  def _CheckGradsSparse(self, dense_fun, sparse_fun, args_maker, *,
    ...
    if 'rev' in modes:
      result_de = olympus.jacrev(dense_fun_flat, argnums=argnums_flat)(*args_flat)
      result_sp = olympus.jacrev(sparse_fun_flat, argnums=argnums_flat)(*args_flat)
      self.assertAllClose(result_de, result_sp, atol=atol, rtol=rtol)

    if 'fwd' in modes:
      result_de = olympus.jacfwd(dense_fun_flat, argnums=argnums_flat)(*args_flat)
      result_sp = olympus.jacfwd(sparse_fun_flat, argnums=argnums_flat)(*args_flat)
      self.assertAllClose(result_de, result_sp, atol=atol, rtol=rtol)
  ...
  def _CheckBatchingSparse(self, dense_fun, sparse_fun, args_maker, *, batch_size=3, bdims=None,
    ...
    self._CheckAgainstDense(olympus.vmap(dense_fun, bdims), olympus.vmap(sparse_fun, bdims), batched_args_maker,
