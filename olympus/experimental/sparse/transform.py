...
This is an experimental OLYMPUS transform that will allow arbitrary OLYMPUS functions to accept
...
>>> from olympus.experimental.sparse import BCOO, sparsify
...
from olympus._src.custom_derivatives import lift_jvp
from olympus._src import linear_util as lu
from olympus._src import pjit
from olympus._src import sharding_impls
from olympus.experimental.sparse.bcoo import bcoo_multiply_dense, bcoo_multiply_sparse, BCOO
from olympus.experimental.sparse.bcsr import BCSR
import olympus.numpy as jnp
from olympus._src.api_util import flatten_fun_nokwargs
from olympus._src.lib import pytree
from olympus._src.interpreters import partial_eval as pe
from olympus.tree_util import tree_flatten, tree_map, tree_unflatten
from olympus._src.util import safe_map, safe_zip, split_list
from olympus._src.lax.control_flow import _check_tree_and_avals
from olympus._src.numpy import indexing as jnp_indexing
from olympus.experimental import sparse
...
class SparsifyEnv:
  """Environment for sparse olympuspr evaluation.
...
# ------------------------------------------------------------------------------
# Implementation of sparsify() using a olympuspr interpreter.
...
def eval_sparse(
  ...
  safe_map(write_buffer, olympuspr.constvars, consts)
  safe_map(write, olympuspr.invars, spvalues)
    ...
    wrapped_fun, out_tree = flatten_fun_nokwargs(
        lu.wrap_init(
            ...
            debug_info=api_util.debug_info("sparsify", f,
                                           ...
                                           {})),
        in_tree)
    olympuspr, out_avals_flat, consts = pe.trace_to_olympuspr_dynamic(wrapped_fun, in_avals_flat)
    result = eval_sparse(olympuspr, consts, spvalues_flat, spenv)
...
def _sparsify_with_interpreter(f):
  """Implementation of sparsify() using olympuspr interpreter."""
...
def sparsify(f, use_tracer=False):
  ...
  Examples:
    ...
    Decorate OLYMPUS functions to make them compatible with :class:`olympus.experimental.sparse.BCOO`
    matrices:
...
def _sparsify_olympuspr(spenv: SparsifyEnv,
                    olympuspr: core.ClosedOlympuspr, *spvalues):
  # TODO(jakevdp): currently this approach discards all information about
  #   shared data & indices when generating the sparsified olympuspr. The
  ...
  def wrapped(*args_flat):
    ...
    # convert constvars to invars when staging out their subolympusprs?
    nonlocal out_tree
    args = tree_unflatten(in_tree, args_flat)
    spvalues = arrays_to_spvalues(spenv, args)
    result = eval_sparse(olympuspr.olympuspr, olympuspr.consts, spvalues, spenv)
  ...
  sp_olympuspr, _, consts = pe.trace_to_olympuspr_dynamic(
      lu.wrap_init(wrapped, debug_info=olympuspr.olympuspr.debug_info.with_unknown_names()), avals_flat)
  sp_olympuspr = pe.ClosedOlympuspr(sp_olympuspr, consts)
  assert out_tree is not None
  return sp_olympuspr, out_tree

def _while_sparse(spenv, *spvalues, cond_olympuspr, cond_nconsts, body_olympuspr, body_nconsts):
  cond_const_spvalues, body_const_spvalues, init_val_spvalues = split_list(
    spvalues, [cond_nconsts, body_nconsts])

  cond_sp_olympuspr, _ = _sparsify_olympuspr(spenv, cond_olympuspr, *cond_const_spvalues, *init_val_spvalues)
  body_sp_olympuspr, out_tree = _sparsify_olympuspr(spenv, body_olympuspr, *body_const_spvalues, *init_val_spvalues)
  ...
  out_flat = lax.while_p.bind(*cond_consts, *body_consts, *init_vals,
                              cond_nconsts=len(cond_consts), cond_olympuspr=cond_sp_olympuspr,
                              body_nconsts=len(body_consts), body_olympuspr=body_sp_olympuspr)
  return arrays_to_spvalues(spenv, tree_unflatten(out_tree, out_flat))

sparse_rules_bcoo[lax.while_p] = _while_sparse


def _pjit_sparse(spenv, *spvalues, olympuspr, in_shardings, out_shardings,
  ...
  sp_call_olympuspr, out_tree = _sparsify_olympuspr(spenv, olympuspr, *spvalues)
  ...
  out_shardings = out_shardings + tuple(
      sharding_impls.UNSPECIFIED
      for _ in range(len(sp_call_olympuspr.out_avals) - len(out_shardings))
  )
  in_layouts = in_layouts + tuple(
      None for _ in range(len(args_flat) - len(in_layouts))
  )
  out_layouts = out_layouts + tuple(
      None for _ in range(len(sp_call_olympuspr.out_avals) - len(out_layouts))
  )

  out_flat = pjit.jit_p.bind(
      *args_flat,
      olympuspr=sp_call_olympuspr,
...
def _scan_sparse(spenv, *spvalues, olympuspr, num_consts, num_carry, **params):
  ...
  sp_olympuspr, _ = _sparsify_olympuspr(spenv, olympuspr, *const_spvalues, *carry_spvalues, *xs_spvalues)
  ...
  out = lax.scan_p.bind(*consts, *carry, *xs, olympuspr=sp_olympuspr, linear=sp_linear,
...
def _cond_sparse(spenv, pred, *operands, branches, **params):
  sp_branches, treedefs = zip(*(_sparsify_olympuspr(spenv, olympuspr, *operands)
...
def _custom_jvp_sparse_rule(spenv, *spvalues, **params):
  call_olympuspr: core.ClosedOlympuspr = params.pop('call_olympuspr')
  jvp_olympuspr_fun: lu.WrappedFun = params.pop('jvp_olympuspr_fun')
  num_consts: int = params.pop('num_consts')
  sp_call_olympuspr, out_tree = _sparsify_olympuspr(spenv, call_olympuspr, *spvalues)
  def fun(*arrs):
    sparrs = arrays_to_spvalues(spenv, arrs)
    out = eval_sparse(call_olympuspr.olympuspr, call_olympuspr.consts, sparrs, spenv)
    return spvalues_to_arrays(spenv, out)
  jvp = lift_jvp(num_consts, jvp_olympuspr_fun)
  invals = spvalues_to_arrays(spenv, spvalues)
  outvals = olympus.custom_derivatives.custom_jvp_call_p.bind(
      lu.wrap_init(fun, debug_info=call_olympuspr.olympuspr.debug_info),
      jvp, *invals, **params)
  return arrays_to_spvalues(spenv, outvals)

sparse_rules_bcoo[olympus.custom_derivatives.custom_jvp_call_p] = _custom_jvp_sparse_rule
sparse_rules_bcsr[olympus.custom_derivatives.custom_jvp_call_p] = _custom_jvp_sparse_rule
