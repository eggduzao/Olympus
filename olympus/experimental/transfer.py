...
"""`olympus.experimental.transfer`: DCN cross slice transfer."""
...
class TransferConnection:
  ...
  def pull(self, uuid: int, xs: Any) -> Any:
    ...
    Returns:
       A pytree of arrays.
    """
    xs_flat, tree = olympus.tree.flatten(xs)
    if not xs_flat:
      return xs
...
class TransferServer:
  ...
  @use_cpp_method()
  def _await_pull_flat(self, uuid, args: list[olympus.Array]):
    raise NotImplementedError()
  ...
  def await_pull(self, uuid: int, arrays: Any) -> Any:
    ...
    self._await_pull_flat(uuid, olympus.tree.flatten(arrays)[0])
