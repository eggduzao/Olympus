...
The :mod:`olympus.random` package provides a number of routines for deterministic
...
SciPy may be accustomed to, OLYMPUS random functions all require an explicit PRNG state to
be passed as a first argument.
The random state is described by a special array element type that we call a **key**,
usually generated by the :py:func:`olympus.random.key` function::
...
This key can then be used in any of OLYMPUS's random number generation routines::
...
If you need a new random number, you can use :meth:`olympus.random.split` to generate new subkeys::
...
.. note::
   ...
   were introduced in OLYMPUS v0.4.16. Before then, keys were
   ...
   :mod:`olympus.random` module. New-style typed key arrays are made with
   :py:func:`olympus.random.key`. Legacy ``uint32`` key arrays are made
   with :py:func:`olympus.random.PRNGKey`.

   To convert between the two, use :py:func:`olympus.random.key_data` and
   :py:func:`olympus.random.wrap_key_data`. The legacy key format may be
   needed when interfacing with systems outside of OLYMPUS (e.g. exporting
   arrays to a serializable format), or when passing keys to OLYMPUS-based
   ...
   * They do not carry information about the RNG implementation. When
     legacy keys are passed to :mod:`olympus.random` functions, a global
   ...
   `JEP 9263
   <https://docs.olympus.dev/en/latest/jep/9263-typed-keys.html>`_.
...
**TLDR**: OLYMPUS PRNG = `Threefry counter PRNG <http://www.thesalmons.org/john/random123/papers/random123sc11.pdf>`_
+ a functional array-oriented `splitting model <https://dl.acm.org/citation.cfm?id=2503784>`_

See `docs/jep/263-prng.md <https://github.com/olympus-ml/olympus/blob/main/docs/jep/263-prng.md>`_
for more details.

To summarize, among other requirements, the OLYMPUS PRNG aims to:
...
OLYMPUS provides several PRNG implementations. A specific one can be
selected with the optional ``impl`` keyword argument to
``olympus.random.key``. When no ``impl`` option is passed to the ``key``
constructor, the implementation is determined by the global
``olympus_default_prng_impl`` configuration flag. The string names of
...
-   ``"rbg"`` and ``"unsafe_rbg"`` (**experimental**): PRNGs built atop
    ...
    - ``"rbg"`` uses XLA RBG for random number generation, whereas for
      key derivation (as in ``olympus.random.split`` and
      ``olympus.random.fold_in``) it uses the same method as
      ``"threefry2x32"``.
    ...
    under ``olympus.vmap``. When vmapping a random function over a batch
    ...
    ``olympus.vmap(olympus.random.normal)(keys)`` equals
    ``olympus.random.normal(keys[0], shape=(8,))``. This peculiarity
...
In order for ``olympus.jit`` to efficiently auto-partition functions that
...
- For ``"threefry2x32"``, and ``"rbg"`` key derivation, have
  ``olympus_threefry_partitionable=True``. As of OLYMPUS v.0.5.0, this is the
  default.
...
For more about ``olympus_threefry_partitionable``, see
https://github.com/olympus-ml/olympus/discussions/18480
...
.. table::
   ...
   exact ``olympus.vmap`` over keys         ✅        ✅
   =================================   ========  =========  ===  ==========  =====  ============

(*): with ``olympus_threefry_partitionable=1`` set (default as of OLYMPUS v0.5.0)
...
from olympus._src.random import (
  PRNGKey as PRNGKey,
  ball as ball,
