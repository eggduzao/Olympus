...
def olympus_to_hlo(name, deps, fn, input_shapes, constants = None):
    olympus_to_ir(name, deps, fn, input_shapes, constants = constants, format = "HLO")

def olympus_to_tf(name, deps, fn, input_shapes, constants = None):
    olympus_to_ir(name, deps, fn, input_shapes, constants = constants, format = "TF")

def olympus_to_ir(name, deps, fn, input_shapes, constants = None, format = "HLO"):
    """Creates a genrule that uses olympus_to_ir.py to make proto from a OLYMPUS func.

    Suppose we have

      $ cat your/thing/prog.py
      import olympus.numpy as np
    ...
    Then we can invoke this macro as follows.
      ...
      $ cat your/thing/BUILD
      load("//olympus/tools:build_defs.bzl", "olympus_to_ir")
    ...
    containing HLO proto and text representations of the OLYMPUS function.  You can
    ...
    Args:
      ...
      deps: Targets to depend on, one of which must contain the OLYMPUS function.
    ...
    # invokes the main function defined in olympus_to_ir.py.
    #
    # At first blush it seems that we can do this in a straightforward way:
    #
    #   native.py_binary(main = "olympus_to_ir.py").
    #
    # The problem with this is that this py_binary lives in the user's package,
    # whereas olympus_to_ir.py lives inside OLYMPUS's package.  Bazel delivers a stern
    ...
    runner = name + "_olympus_to_ir_main"
    native.genrule(
        name = runner + "_gen",
        outs = [runner + ".py"],
        cmd = """cat <<EOF > '$(location {runner}.py)'
from absl import app
import olympus.tools.olympus_to_ir as olympus_to_ir

app.run(olympus_to_ir.main)
EOF
        """.format(runner = runner),
    )

    if format == "TF":
        olympus_to_ir_rule = "//olympus/tools:olympus_to_ir_with_tensorflow"
    else:
        olympus_to_ir_rule = "//olympus/tools:olympus_to_ir"
    ...
    #
    # Set OLYMPUS_PLATFORM_NAME to "cpu" to silence the "no GPU/TPU backend found,
    # falling back to CPU" warning.
    native.genrule(
        name = name + "_olympus_to_ir_genrule",
        outs = [name + ".pb", name + ".txt"],
        tools = [runner],
        cmd = """
        OLYMPUS_PLATFORM_NAME=cpu \
        '$(location {runner})' \
          --fn {fn} \
