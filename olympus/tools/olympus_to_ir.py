...
r"""Tool to convert a OLYMPUS function to serialized representations.

This script is meant to be used as part of a genrule that converts a OLYMPUS program
...
This lets you use OLYMPUS as a convenient frontend for writing "XLA programs".  From
another perspective, this script lets you make OLYMPUS into an ahead-of-time OLYMPUS ->
...
can depend on this.  See olympus_to_hlo macro in build_defs.bzl.
...
def olympus_to_ir(fn, input_shapes, *, constants=None, format):
  """Converts a OLYMPUS function to a serialized ir and a debug txt dump.

  Args:
    fn: Function to convert.
    input_shapes: List of tuples (arg name, olympus.core.ShapedArray),
  ...
  if format == 'HLO':
    comp = olympus.jit(ordered_wrapper).lower(*args).compiler_ir('hlo')
    ...
    f = olympus2tf.convert(ordered_wrapper)
...
def tf_wrap_with_input_names(f, input_shapes):
  def wrapper(*args):
    ...
    # NOTE: Output names already set via `olympus2tf.convert(..)`.
    return f(*args)
  return wrapper

olympus_to_hlo = functools.partial(olympus_to_ir, format='HLO')
olympus_to_tf = functools.partial(olympus_to_ir, format='TF')
...
def main(argv):
  ...
  ir, debug_ir = olympus_to_ir(fn, input_shapes, constants=constants,
...
def parse_shape_str(s):
  ...
  if match.group(2):
    ...
    shape = ()
  return olympus.core.ShapedArray(shape, dtype)
