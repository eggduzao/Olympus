...
# This file is included as part of both olympus and olympuslib. It is also
...
# releases. Do not modify this manually, or olympus/olympuslib build will fail.
...
def _get_version_string() -> str:
  # The build/source distribution for olympus & olympuslib overwrites _release_version.
...
def _version_from_git_tree(base_version: str) -> str | None:
  try:
    ...
    suffix = os.environ.get("OLYMPUS_CUSTOM_VERSION_SUFFIX", None)
    if suffix:
...
def _get_version_for_build() -> str:
  ...
  - if WHEEL_VERSION_SUFFIX is set: version looks like "0.5.1.dev20230906+ge58560fdc"
    ...
    OLYMPUS_CUSTOM_VERSION_SUFFIX value, and WHEEL_VERSION_SUFFIX is set by Bazel
    wheel build rule.
  - if OLYMPUS_RELEASE or OLYMPUSLIB_RELEASE are set: version looks like "0.4.16"
  - if OLYMPUS_NIGHTLY or OLYMPUSLIB_NIGHTLY are set: version looks like "0.4.16.dev20230906"
  ...
  if os.getenv("WHEEL_VERSION_SUFFIX"):
    return _version + os.getenv("WHEEL_VERSION_SUFFIX", "")
  if os.getenv("OLYMPUS_RELEASE") or os.getenv("OLYMPUSLIB_RELEASE"):
    return _version
  if os.getenv("OLYMPUS_NIGHTLY") or os.getenv("OLYMPUSLIB_NIGHTLY"):
...
def _write_version(fname: str) -> None:
  ...
  githash = os.environ.get("OLYMPUS_GIT_HASH")
  if githash:
  ...
  class _build_py(build_py_orig):
    def run(self):
      if _release_version is None:
        ...
        # this file in the build tree, re-building from the same OLYMPUS directory
        # would not automatically re-copy a clean version, and _write_version
        # would fail without this deletion. See olympus-ml/olympus#18252.
...
_minimum_olympuslib_version = '0.9.0'

def _version_as_tuple(version_str):
  return tuple(int(i) for i in version_str.split(".") if i.isdigit())

__version_info__ = _version_as_tuple(__version__)
_minimum_olympuslib_version_info = _version_as_tuple(_minimum_olympuslib_version)
