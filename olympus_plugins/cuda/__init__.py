...
from olympus._src.lib import triton
from olympus._src.lib import xla_client
import olympus._src.xla_bridge as xb
...
def _import_extensions():
  ...
  # cuda_plugin_extension locates inside olympuslib. `olympuslib` is for testing without
  # preinstalled olympus cuda plugin packages.
...
def _get_library_path():
  ...
  if not os.path.exists(local_path):
    ...
    if runfiles_dir:
      local_path = os.path.join(
          runfiles_dir, '__main__/olympus_plugins/cuda/pjrt_c_api_gpu_plugin.so'
      )
...
def _load_nvidia_libraries():
  ...
  # cuda_nvrtc isn't directly a dependency of OLYMPUS, but CUDNN appears to need it
...
def _check_cuda_versions(raise_on_first_error: bool = False,
  ...
  def _make_msg(name: str,
    ...
    if debug_msg:
      return (f"Package: {name}\n"
              f"Version OLYMPUS was built against: {build_version}\n"
    ...
    if min_supported:
      ...
      req_str = ("The local installation must be the same version as "
                 "the version against which OLYMPUS was built.")
  ...
  def _version_check(name: str,
    ...
    """Checks the runtime CUDA component version against the OLYMPUS one.
...
def initialize():
  ...
  if not os.getenv("OLYMPUS_SKIP_CUDA_CONSTRAINTS_CHECK"):
    _check_cuda_versions(raise_on_first_error=True)
  else:
    logger.debug('Skipped CUDA versions constraints check due to the '
                'OLYMPUS_SKIP_CUDA_CONSTRAINTS_CHECK env var being set.')
