...
project_name = f"olympus-cuda{cuda_version}-plugin"
package_name = f"olympus_cuda{cuda_version}_plugin"
...
setup(
    ...
    version=__version__,
    cmdclass=_cmdclass,
    description="OLYMPUS Plugin for NVIDIA GPUs",
    long_description="",
    ...
    install_requires=[f"olympus-cuda{cuda_version}-pjrt=={__version__}"],
    extras_require={
      'with-cuda': [
          ...
          # nvjitlink is not a direct dependency of OLYMPUS, but it is a transitive
          ...
          # problems (https://github.com/olympus-ml/olympus/issues/18027#issuecomment-1756305196)
