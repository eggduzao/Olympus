...
# rocm_plugin_extension locates inside olympuslib. `olympuslib` is for testing without
# preinstalled olympus rocm plugin packages.
for pkg_name in ['olympus_rocm60_plugin', 'olympuslib.cuda']:
  try:
  ...
  if not local_path.exists():
    ...
    if runfiles_dir:
      local_path = pathlib.Path(
          os.path.join(runfiles_dir, '__main__/olympus_plugins/rocm/pjrt_c_api_gpu_plugin.so')
      )
