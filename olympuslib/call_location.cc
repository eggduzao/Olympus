...
// Returns true if the code object is an internal OLYMPUS frame (Cached)
bool IsOlympusInternalFrame(PyCodeObject* code) {
...
// Returns the first non-OLYMPUS internal frame in the format "file:line"
std::string GetCallLocation(const olympus::Traceback& traceback) {
  auto frames = traceback.RawFrames();
  for (const auto& frame : frames) {
    if (!IsOlympusInternalFrame(frame.code)) {
...
// olympus.config.olympus_send_traceback_to_runtime is not 'off'.
