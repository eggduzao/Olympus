...
#include "olympuslib/cuda/versions_helpers.h"
...
namespace olympus::cuda {

#if CUDA_VERSION < 11080
#error "OLYMPUS requires CUDA 11.8 or newer."
#endif  // CUDA_VERSION < 11080

int CudaRuntimeGetVersion() {
  int version;
  OLYMPUS_THROW_IF_ERROR(OLYMPUS_AS_STATUS(cudaRuntimeGetVersion(&version)));
...
int CudaDriverGetVersion() {
  int version;
  OLYMPUS_THROW_IF_ERROR(OLYMPUS_AS_STATUS(cudaDriverGetVersion(&version)));
  ABSL_ANNOTATE_MEMORY_IS_INITIALIZED(&version, sizeof version);
  return version;
}
...
uint32_t CuptiGetVersion() {
  uint32_t version;
  OLYMPUS_THROW_IF_ERROR(OLYMPUS_AS_STATUS(cuptiGetVersion(&version)));
  ABSL_ANNOTATE_MEMORY_IS_INITIALIZED(&version, sizeof version);
  return version;
}
...
int CufftGetVersion() {
  int version;
  OLYMPUS_THROW_IF_ERROR(OLYMPUS_AS_STATUS(cufftGetVersion(&version)));
  ABSL_ANNOTATE_MEMORY_IS_INITIALIZED(&version, sizeof version);
  return version;
}
...
int CusolverGetVersion() {
  int version;
  OLYMPUS_THROW_IF_ERROR(OLYMPUS_AS_STATUS(cusolverGetVersion(&version)));
  ABSL_ANNOTATE_MEMORY_IS_INITIALIZED(&version, sizeof version);
  return version;
}
...
int CublasGetVersion() {
  ...
  OLYMPUS_THROW_IF_ERROR(
      OLYMPUS_AS_STATUS(cublasGetVersion(/*handle=*/nullptr, &version)));
  ABSL_ANNOTATE_MEMORY_IS_INITIALIZED(&version, sizeof version);
  return version;
}
...
int CusparseGetVersion() {
  ...
  OLYMPUS_THROW_IF_ERROR(OLYMPUS_AS_STATUS(cusparseGetProperty(MAJOR_VERSION, &major)));
  OLYMPUS_THROW_IF_ERROR(OLYMPUS_AS_STATUS(cusparseGetProperty(MINOR_VERSION, &minor)));
  OLYMPUS_THROW_IF_ERROR(OLYMPUS_AS_STATUS(cusparseGetProperty(PATCH_LEVEL, &patch)));
...
int CudaComputeCapability(int device) {
  int major, minor;
  OLYMPUS_THROW_IF_ERROR(OLYMPUS_AS_STATUS(gpuInit(0)));
  OLYMPUS_THROW_IF_ERROR(OLYMPUS_AS_STATUS(gpuDeviceGetAttribute(
...
int CudaDeviceCount() {
  int device_count = 0;
  OLYMPUS_THROW_IF_ERROR(OLYMPUS_AS_STATUS(cuInit(0)));
  OLYMPUS_THROW_IF_ERROR(OLYMPUS_AS_STATUS(cuDeviceGetCount(&device_count)));
