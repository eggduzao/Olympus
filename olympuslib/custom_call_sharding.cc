...
class PyCustomCallPartitionerCallbacks {
 public:
  PyCustomCallPartitionerCallbacks(nb::object prop_user_sharding,
    ...
    callbacks_.dtor = +[](OLYMPUS_CustomCallPartitioner_Callbacks* self) {
      delete GetSelfPtr(self);
    };
    callbacks_.partition = +[](OLYMPUS_CustomCallPartitioner_Callbacks* self,
                               OLYMPUS_CustomCallPartitioner_Partition_Args* args) {
      PopulateResults(GetSelfPtr(self)->CallPartition(args), args);
    };
    callbacks_.infer_sharding =
        +[](OLYMPUS_CustomCallPartitioner_Callbacks* self,
            OLYMPUS_CustomCallPartitioner_InferShardingFromOperands_Args* args) {
          PopulateResults(GetSelfPtr(self)->CallInferShardingFromOperands(args),
                          args);
        };
    callbacks_.propagate_user_sharding =
        +[](OLYMPUS_CustomCallPartitioner_Callbacks* self,
            OLYMPUS_CustomCallPartitioner_PropagateUserSharding_Args* args) {
  ...
  CallPartition(OLYMPUS_CustomCallPartitioner_Partition_Args* args) const {
  ...
  absl::StatusOr<std::optional<xla::HloSharding>> CallInferShardingFromOperands(
      OLYMPUS_CustomCallPartitioner_InferShardingFromOperands_Args* args) const {
    if (args->header.api_version != 0) {
      return absl::InternalError("API version mismatch.");
    }
    TF_ASSIGN_OR_RETURN(auto args_tuple, ReadArgs(args));
  ...
  absl::StatusOr<xla::HloSharding> CallPropagateUserSharding(
      OLYMPUS_CustomCallPartitioner_PropagateUserSharding_Args* args) const {
    if (args->header.api_version != 0) {
      return absl::InternalError("API version mismatch.");
    }
    TF_ASSIGN_OR_RETURN(auto args_tuple, ReadArgs(args));
  ...
  OLYMPUS_CustomCallPartitioner_Callbacks* callbacks() { return &callbacks_; }

 private:
  static PyCustomCallPartitionerCallbacks* GetSelfPtr(
      OLYMPUS_CustomCallPartitioner_Callbacks* callbacks) {
    return reinterpret_cast<PyCustomCallPartitionerCallbacks*>(
        callbacks->private_data);
  }

  OLYMPUS_CustomCallPartitioner_Callbacks callbacks_;
...
void CallInspectSharding(void* obj, OLYMPUS_InspectSharding_Callback_Args* args) {
...
Args:
  ...
  m.def("encode_inspect_sharding_callback",
        [](nb::object handler) -> nb::bytes {
          OLYMPUS_InspectSharding_Callback cb;
          cb.call = &CallInspectSharding;
          cb.data = handler.ptr();
          char bytes[sizeof(OLYMPUS_InspectSharding_Callback)];
          std::memcpy(&bytes, &cb, sizeof(OLYMPUS_InspectSharding_Callback));
          return nb::bytes(bytes, sizeof(OLYMPUS_InspectSharding_Callback));
        });
