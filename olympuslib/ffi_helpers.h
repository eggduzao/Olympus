...
#ifndef OLYMPUSLIB_FFI_HELPERS_H_
#define OLYMPUSLIB_FFI_HELPERS_H_
...
#define FFI_RETURN_IF_ERROR_STATUS(...)     \
  do {                                      \
    ...
    if (ABSL_PREDICT_FALSE(!status.ok())) { \
      return ::olympus::AsFfiError(status);     \
    }                                       \
  } while (0)
...
#define FFI_ASSIGN_OR_RETURN_IMPL_(statusor, lhs, rhs)        \
  ...
  if (ABSL_PREDICT_FALSE(!statusor.ok())) {                   \
    return ::olympus::AsFfiError(statusor.status());              \
  }                                                           \
...
#endif  // OLYMPUSLIB_FFI_HELPERS_H_
