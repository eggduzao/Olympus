...
NB_MODULE(_hybrid, m) {
  ...
  m.def("registrations", []() {
    nb::dict dict;
    dict[OLYMPUS_GPU_PREFIX "hybrid_eig_real"] = EncapsulateFfiHandler(kEigReal);
    dict[OLYMPUS_GPU_PREFIX "hybrid_eig_comp"] = EncapsulateFfiHandler(kEigComp);
    dict[OLYMPUS_GPU_PREFIX "hybrid_geqp3"] = EncapsulateFfiHandler(kGeqp3);
    return dict;
  });
}
