...
#include "olympuslib/gpu/hybrid_kernels.h"
...
class HostBuffer {
 public:
  ...
  absl::Status CopyFromDevice(gpuStream_t stream, const T* buffer) {
    return OLYMPUS_AS_STATUS(gpuMemcpyAsync(data_.get(), buffer, size_ * sizeof(T),
                                        gpuMemcpyDeviceToHost, stream));
  }

  absl::Status CopyToDevice(gpuStream_t stream, T* buffer) {
    return OLYMPUS_AS_STATUS(gpuMemcpyAsync(buffer, data_.get(), size_ * sizeof(T),
...
absl::StatusOr<void*> MagmaLookup::FindMagmaInit() {
  ...
  const char* magma_lib_path = std::getenv("OLYMPUS_GPU_MAGMA_PATH");
  ...
  if (handle_ == nullptr || magma_init == nullptr) {
    return absl::InternalError(
        ...
        "symbol. Use the OLYMPUS_GPU_MAGMA_PATH environment variable to "
  ...
  if (symbol == nullptr) {
    if (lib_path_.has_value()) {
      ...
      return absl::InternalError(absl::StrFormat(
          ...
          "OLYMPUS_GPU_MAGMA_PATH environment variable to specify an explicit "
          "path to the library.",
          name));
    }
  }
...
class PivotingQrFactorizationHost {
 ...
 public:
  ...
  ffi::Error compute(int64_t batch, int64_t rows, int64_t cols,
    ...
    FFI_RETURN_IF_ERROR_STATUS(OLYMPUS_AS_STATUS(gpuStreamSynchronize(stream)));
 ...
 public:
  ...
  ffi::Error compute(int64_t batch, int64_t rows, int64_t cols,
    ...
    if (x_data != x_out_data) {
      FFI_RETURN_IF_ERROR_STATUS(
          OLYMPUS_AS_STATUS(gpuMemcpyAsync(x_out_data, x_data, x.size_bytes(),
