...
#include "olympuslib/gpu/py_client_gpu.h"
...
xla::ffi::Error XlaFfiPythonGpuCallback(gpuStream_t stream,
  ...
  for (size_t i = 0; i < arity; ++i) {
    ...
    auto dims = absl::Span<const int64_t>(arg->dimensions().begin(),
                                          arg->dimensions().size());
    // TODO(b/402422886): Remove this once we form Olympus arrays directly instead
...
XLA_FFI_REGISTER_HANDLER(xla::ffi::GetXlaFfiApi(),
                         ...
                         absl::AsciiStrToUpper(OLYMPUS_GPU_PLUGIN_NAME),
...
XLA_FFI_DEFINE_HANDLER_SYMBOL(
    ...
    (olympus::XlaBufferCallback<kDLCUDA>),
#else
    (olympus::XlaBufferCallback<kDLROCM>),
#endif
    xla::ffi::Ffi::Bind()
        .Ctx<xla::ffi::Context>()
