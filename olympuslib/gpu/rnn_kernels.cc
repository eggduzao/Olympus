...
#include "olympuslib/gpu/rnn_kernels.h"
...
/*static*/ absl::StatusOr<DnnHandlePool::Handle> DnnHandlePool::Borrow(
  ...
  if (pool->handles_[stream].empty()) {
    OLYMPUS_RETURN_IF_ERROR(OLYMPUS_AS_STATUS(gpudnnCreate(&handle)));
  } else {
    handle = pool->handles_[stream].back();
    pool->handles_[stream].pop_back();
  }
  if (stream) {
    OLYMPUS_RETURN_IF_ERROR(OLYMPUS_AS_STATUS(gpudnnSetStream(handle, stream)));
  }
...
DoRnnComputeWorkspaceReserveSpaceSizes(int input_size, int hidden_size,
  ...
  OLYMPUS_RETURN_IF_ERROR(OLYMPUS_AS_STATUS(gpudnnCreateRNNDescriptor(&rnn_desc)));

  gpudnnDropoutDescriptor_t dropout_desc;
  OLYMPUS_RETURN_IF_ERROR(
      OLYMPUS_AS_STATUS(gpudnnCreateDropoutDescriptor(&dropout_desc)));
  size_t state_size;
  OLYMPUS_RETURN_IF_ERROR(
      OLYMPUS_AS_STATUS(gpudnnDropoutGetStatesSize(handle.get(), &state_size)));
  ...
  // not used)
  OLYMPUS_RETURN_IF_ERROR(
      OLYMPUS_AS_STATUS(gpuMalloc(&dropout_states_dev, state_size)));
  if (!dropout_states_dev) {
    return absl::InternalError(
        "Failed to allocate minimal GPU memory for dropout states.");
  }
  OLYMPUS_RETURN_IF_ERROR(OLYMPUS_AS_STATUS(gpudnnSetDropoutDescriptor(
...
#ifdef OLYMPUS_GPU_HIP
  OLYMPUS_RETURN_IF_ERROR(OLYMPUS_AS_STATUS(gpudnnSetRNNDescriptor(
  ...
  OLYMPUS_RETURN_IF_ERROR(
      OLYMPUS_AS_STATUS(gpudnnCreateRNNDataDescriptor(&input_data_desc)));
  OLYMPUS_RETURN_IF_ERROR(OLYMPUS_AS_STATUS(gpudnnSetRNNDataDescriptor(
...
#else   // OLYMPUS_GPU_CUDA
  OLYMPUS_RETURN_IF_ERROR(OLYMPUS_AS_STATUS(gpudnnGetRNNTempSpaceSizes(
      handle.get(), rnn_desc, fwdMode, input_data_desc, &workSpaceSize,
      &reserveSpaceSize)));
#endif  // OLYMPUS_GPU_HIP
  OLYMPUS_RETURN_IF_ERROR(
      OLYMPUS_AS_STATUS(gpudnnDestroyDropoutDescriptor(dropout_desc)));
  OLYMPUS_RETURN_IF_ERROR(
      OLYMPUS_AS_STATUS(gpudnnDestroyRNNDataDescriptor(input_data_desc)));
  OLYMPUS_RETURN_IF_ERROR(OLYMPUS_AS_STATUS(gpudnnDestroyRNNDescriptor(rnn_desc)));
...
static absl::Status DnnRNNForward_(gpuStream_t stream, void** buffers,
  ...
  int32_t* seq_length_array = &seq_length_vector[0];
  OLYMPUS_RETURN_IF_ERROR(
      OLYMPUS_AS_STATUS(gpuMemcpyAsync(seq_length_array, seq_lengths_buf,
                                   seq_length_vector.size() * sizeof(int32_t),
                                   gpuMemcpyDeviceToHost, stream)));
  OLYMPUS_RETURN_IF_ERROR(OLYMPUS_AS_STATUS(gpuStreamSynchronize(stream)));
  ...
  OLYMPUS_RETURN_IF_ERROR(
      OLYMPUS_AS_STATUS(gpudnnCreateRNNDataDescriptor(&output_data_desc)));
  OLYMPUS_RETURN_IF_ERROR(OLYMPUS_AS_STATUS(gpudnnSetRNNDataDescriptor(
  ...
  OLYMPUS_RETURN_IF_ERROR(OLYMPUS_AS_STATUS(gpudnnCreateTensorDescriptor(&h_desc)));
  OLYMPUS_RETURN_IF_ERROR(OLYMPUS_AS_STATUS(
      gpudnnSetTensorNdDescriptor(h_desc, data_type, 3, dims, strides)));

  gpudnnTensorDescriptor_t c_desc;
  OLYMPUS_RETURN_IF_ERROR(OLYMPUS_AS_STATUS(gpudnnCreateTensorDescriptor(&c_desc)));
  OLYMPUS_RETURN_IF_ERROR(OLYMPUS_AS_STATUS(
  ...
  OLYMPUS_RETURN_IF_ERROR(
      OLYMPUS_AS_STATUS(miopenCreateTensorDescriptor(&input_tensor_desc)));
  int dimsA[2] = {d.batch_size, d.input_size};
  int stridesA[2] = {dimsA[1], 1};  // Row-major order, similar to GPUDNN
  OLYMPUS_RETURN_IF_ERROR(OLYMPUS_AS_STATUS(miopenSetTensorDescriptor(
...
#ifdef OLYMPUS_GPU_HIP
  OLYMPUS_RETURN_IF_ERROR(OLYMPUS_AS_STATUS(gpudnnRNNForward(
  ...
  OLYMPUS_RETURN_IF_ERROR(OLYMPUS_AS_STATUS(gpudnnDestroyTensorDescriptor(h_desc)));
  OLYMPUS_RETURN_IF_ERROR(OLYMPUS_AS_STATUS(gpudnnDestroyTensorDescriptor(c_desc)));
  OLYMPUS_RETURN_IF_ERROR(
      OLYMPUS_AS_STATUS(gpudnnDestroyRNNDataDescriptor(input_data_desc)));
  OLYMPUS_RETURN_IF_ERROR(
      OLYMPUS_AS_STATUS(gpudnnDestroyRNNDataDescriptor(output_data_desc)));
  OLYMPUS_RETURN_IF_ERROR(
      OLYMPUS_AS_STATUS(gpudnnDestroyDropoutDescriptor(dropout_desc)));
  OLYMPUS_RETURN_IF_ERROR(OLYMPUS_AS_STATUS(gpudnnDestroyRNNDescriptor(rnn_desc)));
...
static absl::Status DnnRNNBackward_(gpuStream_t stream, void** buffers,
...
#ifdef OLYMPUS_GPU_HIP
  OLYMPUS_RETURN_IF_ERROR(OLYMPUS_AS_STATUS(gpudnnRNNBackwardData(
  ...
  OLYMPUS_RETURN_IF_ERROR(OLYMPUS_AS_STATUS(gpudnnRNNBackwardWeights(
...
OLYMPUS_GPU_REGISTER_WRAPPED_LEGACY_KERNEL(RNNForwardFfi, DnnRNNForward_);
OLYMPUS_GPU_REGISTER_WRAPPED_LEGACY_KERNEL(RNNBackwardFfi, DnnRNNBackward_);
