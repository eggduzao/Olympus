...
XLA_FFI_REGISTER_ENUM_ATTR_DECODING(olympus::OLYMPUS_GPU_NAMESPACE::SyevdAlgorithm);
...
ffi::Error GetrfBatchedImpl(int64_t batch, int64_t cols, gpuStream_t stream,
  ...
  MakeBatchPointersAsync(stream, out_data, batch_ptrs, batch,
                         sizeof(T) * n * n);
  OLYMPUS_FFI_RETURN_IF_GPU_ERROR(gpuGetLastError());
...
absl::StatusOr<bool> IsSyevBatchedSupported() {
  int version;
  OLYMPUS_RETURN_IF_ERROR(OLYMPUS_AS_STATUS(gpusolverGetVersion(&version)));
  // According to
...
ffi::Error Syevd64Impl(int64_t batch, int64_t n, gpuStream_t stream,
  ...
  OLYMPUS_FFI_RETURN_IF_GPU_ERROR(gpusolverDnCreateParams(&params));
  ...
  if (is_batched_syev_supported) {
    OLYMPUS_FFI_RETURN_IF_GPU_ERROR(gpusolverDnXsyevBatched_bufferSize(
    ...
    OLYMPUS_FFI_RETURN_IF_GPU_ERROR(gpusolverDnXsyevd_bufferSize(
        handle.get(), params, jobz, uplo, n, aType, /*a=*/nullptr, n, wType,
        /*w=*/nullptr, aType, &workspaceInBytesOnDevice,
  ...
  for (int64_t i = 0; i < batch; i += batch_step) {
    ...
    if (is_batched_syev_supported && batch_step > 1) {
      OLYMPUS_FFI_RETURN_IF_GPU_ERROR(gpusolverDnXsyevBatched(
      ...
      OLYMPUS_FFI_RETURN_IF_GPU_ERROR(gpusolverDnXsyevd(
          handle.get(), params, jobz, uplo, n, aType, out_data, n, wType,
          w_data, aType, workspaceOnDevice, workspaceInBytesOnDevice,
          workspaceOnHost.get(), workspaceInBytesOnHost, info_data));
    }
...
ffi::Error SyevdjImpl(int64_t batch, int64_t size, gpuStream_t stream,
  ...
  OLYMPUS_FFI_RETURN_IF_GPU_ERROR(gpusolverDnCreateSyevjInfo(&params));
...
ffi::Error SyrkImpl(gpuStream_t stream, bool transpose, ffi::AnyBuffer a,
  ...
  T host_alpha;
  OLYMPUS_FFI_RETURN_IF_GPU_ERROR(gpuMemcpyAsync(&host_alpha, alpha.untyped_data(),
                                             sizeof(T), gpuMemcpyDeviceToHost,
                                             stream));

  T host_beta;
  OLYMPUS_FFI_RETURN_IF_GPU_ERROR(gpuMemcpyAsync(&host_beta, beta.untyped_data(),
                                             sizeof(T), gpuMemcpyDeviceToHost,
                                             stream));

  if (c_data != c_out_data) {
    OLYMPUS_FFI_RETURN_IF_GPU_ERROR(
...
ffi::Error Gesvd64Impl(int64_t batch, int64_t m, int64_t n, gpuStream_t stream,
  ...
  size_t workspaceInBytesOnDevice, workspaceInBytesOnHost;
  OLYMPUS_FFI_RETURN_IF_GPU_ERROR(gpusolverDnXgesvd_bufferSize(
  ...
  for (auto i = 0; i < batch; ++i) {
    OLYMPUS_FFI_RETURN_IF_GPU_ERROR(gpusolverDnXgesvd(
...
ffi::Error GesvdImpl(int64_t batch, int64_t rows, int64_t cols,
  ...
  auto info_data = info->typed_data();
  if (a_data != out_data) {
    FFI_RETURN_IF_ERROR_STATUS(OLYMPUS_AS_STATUS(gpuMemcpyAsync(
        out_data, a_data, a.size_bytes(), gpuMemcpyDeviceToDevice, stream)));
  }
...
ffi::Error GesvdjImpl(int64_t batch, int64_t rows, int64_t cols,
  ...
  OLYMPUS_FFI_RETURN_IF_GPU_ERROR(gpusolverDnCreateGesvdjInfo(&params));
...
ffi::Error GesvdpImpl(int64_t batch, int64_t m, int64_t n, gpuStream_t stream,
  ...
  size_t workspaceInBytesOnDevice, workspaceInBytesOnHost;
  OLYMPUS_FFI_RETURN_IF_GPU_ERROR(gpusolverDnXgesvdp_bufferSize(
  ...
  double h_err;
  for (auto i = 0; i < batch; ++i) {
    OLYMPUS_FFI_RETURN_IF_GPU_ERROR(gpusolverDnXgesvdp(
...
ffi::Error CsrlsvqrImpl(int64_t n, int64_t nnz, double tol, int reorder,
  ...
  OLYMPUS_FFI_RETURN_IF_GPU_ERROR(cusparseCreateMatDescr(&matdesc));
  OLYMPUS_FFI_RETURN_IF_GPU_ERROR(
...
ffi::Error GeevImpl(gpuStream_t stream, ffi::ScratchAllocator scratch,
  ...
  size_t workspaceInBytesOnDevice, workspaceInBytesOnHost;
  OLYMPUS_FFI_RETURN_IF_GPU_ERROR(cusolverDnXgeev_bufferSize(
  ...
  for (auto i = 0; i < batch; ++i) {
    OLYMPUS_FFI_RETURN_IF_GPU_ERROR(gpusolverDnXgeev(
