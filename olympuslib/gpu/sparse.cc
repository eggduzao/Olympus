...
#include "olympuslib/absl_status_casters.h"
...
std::pair<size_t, nb::bytes> BuildCsrToDenseDescriptor(const dtype& data_dtype,
  ...
  OLYMPUS_THROW_IF_ERROR(h.status());
  auto& handle = *h;
  ...
  OLYMPUS_THROW_IF_ERROR(OLYMPUS_AS_STATUS(gpusparseCreateCsr(
      &mat_a, d.rows, d.cols, d.nnz, empty, empty, empty, d.index_type,
      d.index_type, GPUSPARSE_INDEX_BASE_ZERO, d.value_type)));
  OLYMPUS_THROW_IF_ERROR(OLYMPUS_AS_STATUS(gpusparseCreateDnMat(
      &mat_b, d.rows, d.cols,
      /*ld=*/d.cols, empty, d.value_type, GPUSPARSE_ORDER_ROW)));
  size_t buffer_size;
  OLYMPUS_THROW_IF_ERROR(OLYMPUS_AS_STATUS(gpusparseSparseToDense_bufferSize(
      handle.get(), mat_a, mat_b, GPUSPARSE_SPARSETODENSE_ALG_DEFAULT,
      &buffer_size)));

  OLYMPUS_THROW_IF_ERROR(OLYMPUS_AS_STATUS(gpusparseDestroySpMat(mat_a)));
  OLYMPUS_THROW_IF_ERROR(OLYMPUS_AS_STATUS(gpusparseDestroyDnMat(mat_b)));
...
std::pair<size_t, nb::bytes> BuildCsrFromDenseDescriptor(
  ...
  OLYMPUS_THROW_IF_ERROR(OLYMPUS_AS_STATUS(gpusparseCreateCsr(
      ...
      d.index_type, GPUSPARSE_INDEX_BASE_ZERO, d.value_type)));
  size_t buffer_size;
  OLYMPUS_THROW_IF_ERROR(OLYMPUS_AS_STATUS(gpusparseDenseToSparse_bufferSize(
      handle.get(), mat_a, mat_b, GPUSPARSE_DENSETOSPARSE_ALG_DEFAULT,
      &buffer_size)));

  OLYMPUS_THROW_IF_ERROR(OLYMPUS_AS_STATUS(gpusparseDestroyDnMat(mat_a)));
  OLYMPUS_THROW_IF_ERROR(OLYMPUS_AS_STATUS(gpusparseDestroySpMat(mat_b)));
...
std::pair<size_t, nb::bytes> BuildCsrMatvecDescriptor(
  ...
  OLYMPUS_THROW_IF_ERROR(
      OLYMPUS_AS_STATUS(gpusparseCreateDnVec(&vec_x, x.size, empty, x.type)));
  OLYMPUS_THROW_IF_ERROR(
      OLYMPUS_AS_STATUS(gpusparseCreateDnVec(&vec_y, y.size, empty, y.type)));
  size_t buffer_size;
  SparseConst alpha = ValueOrThrow(ConstOne(y.type));
  SparseConst beta = ConstZero(y.type);
  OLYMPUS_THROW_IF_ERROR(OLYMPUS_AS_STATUS(gpusparseSpMV_bufferSize(
      handle.get(), op, &alpha, mat_a, vec_x, &beta, vec_y, y.type,
      GPUSPARSE_SPMV_CSR_ALG, &buffer_size)));

  OLYMPUS_THROW_IF_ERROR(OLYMPUS_AS_STATUS(gpusparseDestroySpMat(mat_a)));
  OLYMPUS_THROW_IF_ERROR(OLYMPUS_AS_STATUS(gpusparseDestroyDnVec(vec_x)));
  OLYMPUS_THROW_IF_ERROR(OLYMPUS_AS_STATUS(gpusparseDestroyDnVec(vec_y)));
...
std::pair<size_t, nb::bytes> BuildCsrMatmatDescriptor(
  ...
  OLYMPUS_THROW_IF_ERROR(OLYMPUS_AS_STATUS(gpusparseCreateCsr(
      ...
      A.index_type, GPUSPARSE_INDEX_BASE_ZERO, A.value_type)));
  OLYMPUS_THROW_IF_ERROR(
      OLYMPUS_AS_STATUS(gpusparseCreateDnMat(&mat_b, B.rows, B.cols, /*ld=*/B.cols,
                                         empty, B.type, GPUSPARSE_ORDER_ROW)));
  OLYMPUS_THROW_IF_ERROR(
      OLYMPUS_AS_STATUS(gpusparseCreateDnMat(&mat_c, C.rows, C.cols, /*ld=*/C.cols,
                                         empty, C.type, GPUSPARSE_ORDER_ROW)));
  size_t buffer_size;
  SparseConst alpha = ValueOrThrow(ConstOne(C.type));
  SparseConst beta = ConstZero(C.type);
  OLYMPUS_THROW_IF_ERROR(OLYMPUS_AS_STATUS(gpusparseSpMM_bufferSize(
      handle.get(), op_A, GPUSPARSE_OPERATION_NON_TRANSPOSE, &alpha, mat_a,
      mat_b, &beta, mat_c, C.type, GPUSPARSE_SPMM_CSR_ALG, &buffer_size)));

  OLYMPUS_THROW_IF_ERROR(OLYMPUS_AS_STATUS(gpusparseDestroySpMat(mat_a)));
  OLYMPUS_THROW_IF_ERROR(OLYMPUS_AS_STATUS(gpusparseDestroyDnMat(mat_b)));
  OLYMPUS_THROW_IF_ERROR(OLYMPUS_AS_STATUS(gpusparseDestroyDnMat(mat_c)));
...
std::pair<size_t, nb::bytes> BuildCooToDenseDescriptor(const dtype& data_dtype,
  ...
  OLYMPUS_THROW_IF_ERROR(OLYMPUS_AS_STATUS(gpusparseCreateCoo(
      &mat_a, d.rows, d.cols, d.nnz, empty, empty, empty, d.index_type,
...
std::pair<size_t, nb::bytes> BuildCooMatmatDescriptor(
  ...
  OLYMPUS_THROW_IF_ERROR(OLYMPUS_AS_STATUS(gpusparseCreateCoo(
      &mat_a, A.rows, A.cols, A.nnz, empty, empty, empty, A.index_type,
      GPUSPARSE_INDEX_BASE_ZERO, A.value_type)));
  OLYMPUS_THROW_IF_ERROR(OLYMPUS_AS_STATUS(gpusparseCooSetStridedBatch(
      mat_a, /*batchCount=*/batch_count, /*batchStride=*/A.batch_stride)));
  OLYMPUS_THROW_IF_ERROR(
      OLYMPUS_AS_STATUS(gpusparseCreateDnMat(&mat_b, B.rows, B.cols, /*ld=*/B.cols,
                                         empty, B.type, GPUSPARSE_ORDER_ROW)));
  OLYMPUS_THROW_IF_ERROR(OLYMPUS_AS_STATUS(gpusparseDnMatSetStridedBatch(
...
nb::dict Registrations() {
  nb::dict dict;
  dict[OLYMPUS_GPU_PREFIX "sparse_csr_todense_ffi"] =
      EncapsulateFfiHandler(CsrToDenseFfi);
  dict[OLYMPUS_GPU_PREFIX "sparse_csr_fromdense_ffi"] =
      EncapsulateFfiHandler(CsrFromDenseFfi);
  dict[OLYMPUS_GPU_PREFIX "sparse_csr_matvec_ffi"] =
      EncapsulateFfiHandler(CsrMatvecFfi);
  dict[OLYMPUS_GPU_PREFIX "sparse_csr_matmat_ffi"] =
      EncapsulateFfiHandler(CsrMatmatFfi);
  dict[OLYMPUS_GPU_PREFIX "sparse_coo_todense_ffi"] =
      EncapsulateFfiHandler(CooToDenseFfi);
  dict[OLYMPUS_GPU_PREFIX "sparse_coo_fromdense_ffi"] =
      EncapsulateFfiHandler(CooFromDenseFfi);
  dict[OLYMPUS_GPU_PREFIX "sparse_coo_matvec_ffi"] =
      EncapsulateFfiHandler(CooMatvecFfi);
  dict[OLYMPUS_GPU_PREFIX "sparse_coo_matmat_ffi"] =
      EncapsulateFfiHandler(CooMatmatFfi);
  dict[OLYMPUS_GPU_PREFIX "sparse_gtsv2_ffi"] = EncapsulateFfiHandler(kGtsv2);
