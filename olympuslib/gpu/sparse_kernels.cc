...
#include "olympuslib/gpu/sparse_kernels.h"
...
#include "olympuslib/gpu/ffi_wrapper.h"
#include "olympuslib/gpu/gpu_kernel_helpers.h"
#include "olympuslib/gpu/handle_pool.h"
#include "olympuslib/gpu/vendor.h"
#include "olympuslib/kernel_helpers.h"
#include "xla/ffi/api/ffi.h"

#define OLYMPUS_FFI_RETURN_IF_GPU_ERROR(...) \
  FFI_RETURN_IF_ERROR_STATUS(OLYMPUS_AS_STATUS(__VA_ARGS__))
...
/*static*/ absl::StatusOr<SparseHandlePool::Handle> SparseHandlePool::Borrow(
  ...
  if (pool->handles_[stream].empty()) {
    OLYMPUS_RETURN_IF_ERROR(OLYMPUS_AS_STATUS(gpusparseCreate(&handle)));
  } else {
    handle = pool->handles_[stream].back();
    pool->handles_[stream].pop_back();
  }
  if (stream) {
    OLYMPUS_RETURN_IF_ERROR(OLYMPUS_AS_STATUS(gpusparseSetStream(handle, stream)));
  }
...
static absl::Status CsrToDense_(gpuStream_t stream, void** buffers,
  ...
  OLYMPUS_RETURN_IF_ERROR(OLYMPUS_AS_STATUS(gpusparseCreateDnMat(
      &mat_b, d.rows, d.cols,
  ...
  OLYMPUS_RETURN_IF_ERROR(OLYMPUS_AS_STATUS(gpusparseDestroySpMat(mat_a)));
  OLYMPUS_RETURN_IF_ERROR(OLYMPUS_AS_STATUS(gpusparseDestroyDnMat(mat_b)));
  return absl::OkStatus();
}

OLYMPUS_GPU_REGISTER_WRAPPED_LEGACY_KERNEL(CsrToDenseFfi, CsrToDense_);
...
static absl::Status CsrFromDense_(gpuStream_t stream, void** buffers,
  ...
  OLYMPUS_RETURN_IF_ERROR(OLYMPUS_AS_STATUS(
      gpusparseCreateCsr(&mat_b, d.rows, d.cols, d.nnz,
                         ...
                         GPUSPARSE_INDEX_BASE_ZERO, d.value_type)));
  OLYMPUS_RETURN_IF_ERROR(OLYMPUS_AS_STATUS(gpusparseDenseToSparse_analysis(
      handle.get(), mat_a, mat_b, GPUSPARSE_DENSETOSPARSE_ALG_DEFAULT,
      buffers[4])));
  OLYMPUS_RETURN_IF_ERROR(OLYMPUS_AS_STATUS(gpusparseDenseToSparse_convert(
      handle.get(), mat_a, mat_b, GPUSPARSE_DENSETOSPARSE_ALG_DEFAULT,
      buffers[4])));
  OLYMPUS_RETURN_IF_ERROR(OLYMPUS_AS_STATUS(gpusparseDestroyDnMat(mat_a)));
  OLYMPUS_RETURN_IF_ERROR(OLYMPUS_AS_STATUS(gpusparseDestroySpMat(mat_b)));
  return absl::OkStatus();
}

OLYMPUS_GPU_REGISTER_WRAPPED_LEGACY_KERNEL(CsrFromDenseFfi, CsrFromDense_);
...
static absl::Status CsrMatvec_(gpuStream_t stream, void** buffers,
  ...
  OLYMPUS_ASSIGN_OR_RETURN(SparseConst alpha, ConstOne(d.y.type));
  ...
  OLYMPUS_RETURN_IF_ERROR(OLYMPUS_AS_STATUS(gpusparseCreateCsr(
  ...
  OLYMPUS_RETURN_IF_ERROR(
      OLYMPUS_AS_STATUS(gpusparseCreateDnVec(&vec_x, d.x.size, xbuf, d.x.type)));
  OLYMPUS_RETURN_IF_ERROR(
      OLYMPUS_AS_STATUS(gpusparseCreateDnVec(&vec_y, d.y.size, ybuf, d.y.type)));
  ...
  OLYMPUS_RETURN_IF_ERROR(OLYMPUS_AS_STATUS(gpusparseDestroySpMat(mat_a)));
  OLYMPUS_RETURN_IF_ERROR(OLYMPUS_AS_STATUS(gpusparseDestroyDnVec(vec_x)));
  OLYMPUS_RETURN_IF_ERROR(OLYMPUS_AS_STATUS(gpusparseDestroyDnVec(vec_y)));
  return absl::OkStatus();
}

OLYMPUS_GPU_REGISTER_WRAPPED_LEGACY_KERNEL(CsrMatvecFfi, CsrMatvec_);
...
static absl::Status CsrMatmat_(gpuStream_t stream, void** buffers,
  ...
  // Note that, contrary to cusparse docs, alpha and beta must be host pointers
  // or else the operation will segfault.
  OLYMPUS_ASSIGN_OR_RETURN(SparseConst alpha, ConstOne(d.C.type));
  ...
  OLYMPUS_RETURN_IF_ERROR(OLYMPUS_AS_STATUS(gpusparseSpMM(
      handle.get(), d.op_A, /*opB=*/GPUSPARSE_OPERATION_NON_TRANSPOSE, &alpha,
      mat_a, mat_b, &beta, mat_c, d.C.type, GPUSPARSE_SPMM_CSR_ALG, buf)));

  OLYMPUS_RETURN_IF_ERROR(OLYMPUS_AS_STATUS(gpusparseDestroySpMat(mat_a)));
  OLYMPUS_RETURN_IF_ERROR(OLYMPUS_AS_STATUS(gpusparseDestroyDnMat(mat_b)));
  OLYMPUS_RETURN_IF_ERROR(OLYMPUS_AS_STATUS(gpusparseDestroyDnMat(mat_c)));
  return absl::OkStatus();
}

OLYMPUS_GPU_REGISTER_WRAPPED_LEGACY_KERNEL(CsrMatmatFfi, CsrMatmat_);
...
OLYMPUS_GPU_REGISTER_WRAPPED_LEGACY_KERNEL(CooToDenseFfi, CooToDense_);
...
OLYMPUS_GPU_REGISTER_WRAPPED_LEGACY_KERNEL(CooFromDenseFfi, CooFromDense_);
...
static absl::Status CooMatvec_(gpuStream_t stream, void** buffers,
  ...
  OLYMPUS_RETURN_IF_ERROR(OLYMPUS_AS_STATUS(gpusparseCreateCoo(
...
OLYMPUS_GPU_REGISTER_WRAPPED_LEGACY_KERNEL(CooMatvecFfi, CooMatvec_);
...
OLYMPUS_GPU_REGISTER_WRAPPED_LEGACY_KERNEL(CooMatmatFfi, CooMatmat_);
...
ffi::Error Gtsv2Impl(BufferSizeF getBufferSize, KernelF kernel, int64_t batch,
  ...
  OLYMPUS_FFI_RETURN_IF_GPU_ERROR(getBufferSize(handle.get(), m, n, nullptr,
  ...
  if (b_data != out_data) {
    OLYMPUS_FFI_RETURN_IF_GPU_ERROR(gpuMemcpyAsync(
        out_data, b_data, b.size_bytes(), gpuMemcpyDeviceToDevice, stream));
  }

  for (int64_t i = 0; i < batch; ++i) {
    OLYMPUS_FFI_RETURN_IF_GPU_ERROR(kernel(handle.get(), m, n, dl_data, d_data,
...
ffi::Error Gtsv2BatchedImpl(BufferSizeF getBufferSize, KernelF kernel,
  ...
  FFI_ASSIGN_OR_RETURN(auto handle, SparseHandlePool::Borrow(stream));
  size_t buffer_size_in_bytes;
  OLYMPUS_FFI_RETURN_IF_GPU_ERROR(getBufferSize(handle.get(), m, nullptr, nullptr,
  ...
  OLYMPUS_FFI_RETURN_IF_GPU_ERROR(kernel(handle.get(), m, dl_data, d_data, du_data,
