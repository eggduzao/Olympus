...
#include "olympuslib/gpu/triton_kernels.h"
...
#include "olympuslib/gpu/triton_utils.h"
...
#ifdef OLYMPUS_GPU_HIP
#include "xla/tsl/platform/env.h"
#include "xla/tsl/platform/errors.h"
#endif  // OLYMPUS_GPU_HIP

#define GPU_RETURN_IF_ERROR(expr) OLYMPUS_RETURN_IF_ERROR(OLYMPUS_AS_STATUS(expr))
...
struct gpuModuleDeleter {
  void operator()(gpuModule_t module) {
    absl::Status status = OLYMPUS_AS_STATUS(gpuModuleUnload(module));
    if (!status.ok()) {
...
absl::StatusOr<ModuleImage*> GetModuleImage(std::string kernel_name,
...
#ifdef OLYMPUS_GPU_HIP  // For HIP/ROCM just read the hsaco file
  ...
  // Parse OLYMPUS_TRITON_PTXAS_EXTRA_FLAGS (space-separated list).
  std::vector<std::string> ptxas_extra_flags;
  const char* extra_flags_env = std::getenv("OLYMPUS_TRITON_PTXAS_EXTRA_FLAGS");
  if (extra_flags_env != nullptr) {
    ptxas_extra_flags = absl::StrSplit(extra_flags_env, ' ', absl::SkipEmpty());
  }

  // Parse OLYMPUS_TRITON_PTXAS_DEVICE_DEBUG (boolean: "1" or "true").
  const char* debug_env = std::getenv("OLYMPUS_TRITON_PTXAS_DEVICE_DEBUG");
  ...
  OLYMPUS_ASSIGN_OR_RETURN(std::vector<uint8_t> module_image,
...
absl::StatusOr<float> Benchmark(gpuStream_t stream, KernelCall& kernel_call,
  ...
  OLYMPUS_RETURN_IF_ERROR(kernel_call.Launch(stream, buffers));  // Warm-up.
  GPU_RETURN_IF_ERROR(gpuEventRecord(start, stream));
  for (int i = 0; i < num_iterations; ++i) {
    OLYMPUS_RETURN_IF_ERROR(kernel_call.Launch(stream, buffers));
  }
...
absl::StatusOr<KernelCall*> GetKernelCall(std::string_view opaque,
...

  {
    ...
    if (ABSL_PREDICT_TRUE(it != kernel_calls.end())) {
      OLYMPUS_RETURN_IF_ERROR(it->second.status());
  ...
  auto get_kernel_call = [&]() -> absl::StatusOr<std::unique_ptr<KernelCall>> {
    ...
    OLYMPUS_ASSIGN_OR_RETURN(std::string serialized, ZlibUncompress(opaque));

    olympus_triton::TritonAnyKernelCall proto;
    if (!proto.ParseFromString(serialized)) {
      return absl::InvalidArgumentError("Failed to parse serialized data.");
    }

    if (proto.has_kernel_call()) {
      OLYMPUS_ASSIGN_OR_RETURN(KernelCall kernel_call_,
 ...
 public:
  ...
  absl::StatusOr<gpuFunction_t> GetFunctionForContext(gpuContext_t context) {
    ...
    absl::Cleanup ctx_restorer = [] {
      absl::Status status = OLYMPUS_AS_STATUS(gpuCtxPopCurrent(nullptr));
      if (!status.ok()) {
...
absl::Status Kernel::Launch(gpuStream_t stream, uint32_t grid[3],
  ...
  if (ABSL_PREDICT_FALSE(module_image_ == nullptr)) {
    OLYMPUS_ASSIGN_OR_RETURN(module_image_,
  ...
  OLYMPUS_ASSIGN_OR_RETURN(gpuFunction_t kernel,
                       module_image_->GetFunctionForContext(context));
  return OLYMPUS_AS_STATUS(gpuLaunchKernel(
...
/*static*/ Kernel Kernel::FromProto(const olympus_triton::TritonKernel& proto) {
...
olympus_triton::TritonKernel Kernel::ToProto() const {
  olympus_triton::TritonKernel proto;
...
KernelCall::Parameter::FromProto(
  ...
  using olympus_triton::TritonKernelCall_Parameter;
  Parameter param;
...
olympus_triton::TritonKernelCall_Parameter KernelCall::Parameter::ToProto() const {
  olympus_triton::TritonKernelCall_Parameter proto;
...
/*static*/ absl::StatusOr<KernelCall> KernelCall::FromProto(
  ...
  for (const olympus_triton::TritonKernelCall_Parameter& parameter :
       proto.parameters()) {
    OLYMPUS_ASSIGN_OR_RETURN(Parameter p, Parameter::FromProto(parameter));
...
olympus_triton::TritonKernelCall KernelCall::ToProto() const {
  olympus_triton::TritonKernelCall proto;
...
/*static*/ absl::StatusOr<AutotunedKernelCall> AutotunedKernelCall::FromProto(
  ...
  for (const olympus_triton::TritonAutotunedKernelCall_Config& config :
       proto.configs()) {
    OLYMPUS_ASSIGN_OR_RETURN(auto kernel_call,
                         KernelCall::FromProto(config.kernel_call()));
    configs.push_back(Config{std::move(kernel_call), config.description()});
  }

  std::vector<std::tuple<size_t, size_t, size_t>> input_output_aliases;
  for (const olympus_triton::TritonAutotunedKernelCall_InputOutputAlias& a :
...
olympus_triton::TritonAutotunedKernelCall AutotunedKernelCall::ToProto() const {
  olympus_triton::TritonAutotunedKernelCall proto;
  proto.set_name(name_);
  for (const Config& config : configs_) {
    olympus_triton::TritonAutotunedKernelCall_Config* c = proto.add_configs();
  ...
  for (const auto& [input_idx, output_idx, size] : input_output_aliases_) {
    olympus_triton::TritonAutotunedKernelCall_InputOutputAlias* a =
...
/*static*/ absl::StatusOr<KernelCall> AutotunedKernelCall::Autotune(
  ...
  OLYMPUS_ASSIGN_OR_RETURN(gpuDevice_t device, GetStreamDevice(stream));
  ...
  for (Config& config : kernel_call.configs_) {
    ...
    OLYMPUS_ASSIGN_OR_RETURN(float t,
