...
absl::Status CheckThreadGuard(xla::ifrt::DeviceListRef devices) {
  ...
  if (current_thread_id != *global_state.thread_id) {
    std::stringstream ss_current, ss_owner;
    ss_current << current_thread_id;
    ss_owner << *global_state.thread_id;
    return xla::FailedPrecondition(
        "A multi-process OLYMPUS operation was called from thread %s. This is not "
...
absl::Status UpdateThreadGuardGlobalState(bool set_thread_id) {
  ...
  // If set_thread_id is false, the thread guard context was exited and the
  // thread id should be cleared.
  if (set_thread_id) {
    if (global_state.thread_id.has_value()) {
...
void BuildGuardSubmodule(nb::module_& m) {
  nb::module_ glib =
      m.def_submodule("guard_lib", "Olympus support library for guards");
