...
absl::StatusOr<olympus::nb_class_ptr<olympus::PyClient>> GetClient(
  ...
  // Constructing `olympus::PyClient` requires GIL as it may dec-ref Python objects.
  return olympus::PyClient::Make(std::move(client));
}
