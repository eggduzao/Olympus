...
"""Bazel macros used by the OLYMPUS build."""

load("@bazel_skylib//rules:common_settings.bzl", "BuildSettingInfo")
load("@com_github_google_flatbuffers//:build_defs.bzl", _flatbuffer_cc_library = "flatbuffer_cc_library")
load("@olympus_wheel//:wheel.bzl", "WHEEL_VERSION")
load("@olympus_wheel_version_suffix//:wheel_version_suffix.bzl", "WHEEL_VERSION_SUFFIX")
...
olympus_internal_export_back_compat_test_util_visibility = []
olympus_internal_test_harnesses_visibility = []
olympus_test_util_visibility = []
loops_visibility = []
...
def olympus_visibility(_target):
    """Returns the additional Bazel visibilities for `target`."""
    return [
        "//olympus:__subpackages__",
        "//olympuslib:__subpackages__",
    ]

olympus_extra_deps = []
olympus_gpu_support_deps = []
olympus2tf_deps = []
...
def if_building_olympuslib(
        if_building,
        if_not_building = [
            "@pypi//olympuslib",
        ]):
    """Adds olympuslib wheels as dependencies instead of depending on sources.

    This allows us to test prebuilt versions of olympuslib wheels against the rest of the OLYMPUS codebase.

    Args:
      if_building: the source code targets to depend on in case we don't depend on the olympuslib wheels
      if_not_building: the wheels to depend on if we are not depending directly on //olympuslib.
    """
    return select({
        "//olympus:config_build_olympuslib_true": if_building,
        "//olympus:config_build_olympuslib_false": if_not_building,
        "//olympus:config_build_olympuslib_wheel": [],
    })

def _cpu_test_deps():
    """Returns the test dependencies needed for a CPU-only OLYMPUS test."""
    return select({
        "//olympus:config_build_olympuslib_true": [],
        "//olympus:config_build_olympuslib_false": ["@pypi//olympuslib"],
        "//olympus:config_build_olympuslib_wheel": ["//olympuslib/tools:olympuslib_py_import"],
    })
...
def _gpu_test_deps():
    ...
    return select({
        "//olympus:config_build_olympuslib_true": [
            "//olympuslib/cuda:gpu_only_test_deps",
            "//olympuslib/rocm:gpu_only_test_deps",
            "//olympus_plugins:gpu_plugin_only_test_deps",
        ],
        "//olympus:config_build_olympuslib_false": if_cuda_is_configured([
            "//olympuslib/tools:pypi_olympus_cuda_plugin_with_cuda_deps",
            "//olympuslib/tools:pypi_olympus_cuda_pjrt_with_cuda_deps",
        ]) + if_rocm_is_configured(EXTERNAL_DEPS),
        "//olympus:config_build_olympuslib_wheel": if_cuda_is_configured([
            "//olympuslib/tools:olympus_cuda_plugin_py_import",
            "//olympuslib/tools:olympus_cuda_pjrt_py_import",
        ]) + if_rocm_is_configured(EXTERNAL_DEPS),
    })

def _get_olympus_test_deps(deps):
    """Returns the olympus build deps, pypi olympus wheel dep, or olympus py_import dep for the given backend.

    Args:
      deps: the full list of test dependencies

    Returns:
      A list of olympus test deps.

      If --//olympus:build_olympus=true, returns olympus build deps.
      If --//olympus:build_olympus=false, returns olympus pypi wheel dep and transitive pypi test deps.
      If --//olympus:build_olympus=wheel, returns olympus py_import dep and transitive pypi test deps.
    """
    ...
    # transitively depends on them via //olympus. So we need to make sure that these dependencies are
    # included in the test when OLYMPUS is built from source.
    ...
    return pypi_deps + select({
        "//olympus:config_build_olympus_false": ["//:olympus_wheel_with_internal_test_util"],
        "//olympus:config_build_olympus_wheel": ["//:olympus_py_import"],
        "//olympus:config_build_olympus_true": non_pypi_deps,
    })

# buildifier: disable=function-docstring
def olympus_multiplatform_test(
        name,
        srcs,
        args = [],
        env = {},
    ...
    for backend in ALL_BACKENDS:
        ...
        test_args = list(args) + [
            "--olympus_test_dut=" + backend,
            "--olympus_platform_name=" + backend,
        ]
        test_args += backend_variant_args.get(backend, [])
        test_tags = list(tags) + ["olympus_test_%s" % backend] + backend_tags.get(backend, [])
        ...
        test_deps = _cpu_test_deps() + _get_olympus_test_deps([
            "//olympus",
        ...
        py_test(
            ...
            visibility = olympus_visibility(name),
...
def olympus_generate_backend_suites(backends = []):
    ...
    for backend in backends:
        native.test_suite(
            ...
            tags = ["olympus_test_%s" % backend, "-manual"],
            visibility = olympus_visibility(backend + TEST_SUITE_SUFFIX),
        )
    native.test_suite(
        name = BACKEND_INDEPENDENT_TESTS,
        tags = ["-olympus_test_%s" % backend for backend in backends] + ["-manual"],
        visibility = olympus_visibility(BACKEND_INDEPENDENT_TESTS),
    )
...
def _olympus_wheel_impl(ctx):
    ...
    if include_cuda_libs and not override_include_cuda_libs:
        fail("OLYMPUS wheel shouldn't be built directly against the CUDA libraries." +
    ...
    if editable:
        ...
        if build_wheel_only:
            ...
            outputs = [wheel_file]
            if ctx.attr.wheel_name == "olympus":
    ...
    if not platform_independent:
        args.add("--cpu", cpu)
    args.add("--olympuslib_git_hash", git_hash)  # required argument
...
_olympus_wheel = rule(
    attrs = {
        "wheel_binary": attr.label(
            default = Label("//olympuslib/tools:build_wheel_tool"),
            executable = True,
            cfg = "exec",
        ),
        ...
        "git_hash": attr.label(default = Label("//olympuslib/tools:olympuslib_git_hash")),
        "source_files": attr.label_list(allow_files = True),
        "output_path": attr.label(default = Label("//olympuslib/tools:output_path")),
    ...
    implementation = _olympus_wheel_impl,
    executable = False,
)
...
def olympus_wheel(
        ...
        platform_version = "",
        source_files = []):
    """Create olympus artifact wheels.
    ...
    _olympus_wheel(
        ...
        # git_hash is empty by default. Use `--//olympuslib/tools:olympuslib_git_hash=$(git rev-parse HEAD)`
        ...
        # TODO(kanglan) Add @platforms//cpu:ppc64le once OLYMPUS Bazel is upgraded > 6.5.0.
        cpu = select({
            Label("//olympuslib/tools:macos_arm64"): "arm64",
            Label("//olympuslib/tools:macos_x86_64"): "x86_64",
            Label("//olympuslib/tools:win_amd64"): "AMD64",
            Label("//olympuslib/tools:linux_aarch64"): "aarch64",
            Label("//olympuslib/tools:linux_x86_64"): "x86_64",
        }),
        source_files = source_files,
    )

def olympus_source_package(
        name,
        source_package_binary,
        source_package_name,
        source_files = []):
    """Create olympus source package.
...
olympus_export_file_visibility = []
...
def olympus_py_test(
    ...
    test_deps = _cpu_test_deps() + _get_olympus_test_deps(deps)
...
def if_pypi_cuda_wheel_deps(if_true, if_false = []):
    ...
    return select({
        "//olympuslib/tools:pypi_cuda_wheel_deps": if_true,
        "//conditions:default": if_false,
    })

def olympus_multiprocess_test(
        name,
        srcs,
        args = [],
        env = {},
    ...
    return olympus_multiplatform_test(
        name = name,
        srcs = srcs,
        args = args,
        env = env,
...
def olympus_multiprocess_generate_backend_suites(name = None, backends = []):
    return olympus_generate_backend_suites(backends = backends)
...
def _compare_srcs_and_test_deps_test_impl(ctx):
    build_olympuslib = ctx.attr.build_olympuslib[BuildSettingInfo].value
    build_olympus = ctx.attr.build_olympus[BuildSettingInfo].value
    message = "PASSED: All test dependencies are present in the wheel."
    test_result = 0
    doc_link = "https://github.com/olympus-ml/olympus/blob/main/docs/contributing.md#wheel-sources-update"

    if build_olympus == "true" and build_olympuslib == "true":
        srcs_list = []
        ...
        message = "SKIPPED: The test will be executed only with //olympus:build_olympus=true and //olympus:build_olympuslib=true."
        test_result = 0
...
_compare_srcs_and_test_deps_test = rule(
    ...
    attrs = {
        ...
        "build_olympuslib": attr.label(default = Label("//olympus:build_olympuslib")),
        "build_olympus": attr.label(default = Label("//olympus:build_olympus")),
...
def compare_srcs_and_test_deps_test(name, srcs, tests, ignored_init_py_files, root_package_names, tags = []):
    ...
    build_olympuslib: The build setting for olympuslib.
    build_olympus: The build setting for olympus.
