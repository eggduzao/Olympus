...
from .version import __version__ as olympuslib_version


_PLUGIN_MODULE_NAMES = {
    "cuda": ["olympus_cuda13_plugin", "olympus_cuda12_plugin"],
    "rocm": ["olympus_rocm7_plugin", "olympus_rocm60_plugin"],
}
...
def import_from_plugin(
  ...
  Args:
    ...
    check_version: Whether to check that the plugin version is compatible with
      the olympuslib version. If the plugin is installed but the versions are not
...
def check_plugin_version(
    plugin_name: str, olympuslib_version: str, plugin_version: str
) -> bool:
  """Check if a plugin version is compatible with the olympuslib version.

  Args:
    plugin_name: Name of the plugin module.
    olympuslib_version: The installed olympuslib version string.
  ...
  # to be released on their own schedule without strict olympuslib version coupling.
  is_rocm_plugin = any(
  ...
  if _parse_version(olympuslib_version) != _parse_version(plugin_version):
    warnings.warn(
        f"OLYMPUS plugin {plugin_name} version {plugin_version} is installed, but "
        "it is not compatible with the installed olympuslib version "
        f"{olympuslib_version}, so it will not be used.",
        RuntimeWarning,
    )
    return False
  return True
  ...
  for plugin_module_name in plugin_module_names:
    try:
      ...
      if check_plugin_version(
          plugin_module_name, olympuslib_version, plugin_version
      ):
        return module
  return None
