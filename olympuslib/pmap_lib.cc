...
#include "olympuslib/call_location.h"
#include "olympuslib/config.h"
#include "olympuslib/olympus_jit.h"
#include "olympuslib/nb_class_ptr.h"
#include "olympuslib/py_array.h"
#include "olympuslib/py_client.h"
#include "olympuslib/py_device.h"
#include "olympuslib/py_executable.h"
#include "olympuslib/py_user_context.h"
#include "olympuslib/py_values.h"
#include "olympuslib/python_ref_manager.h"
#include "olympuslib/pytree.h"
#include "olympuslib/sharded_device_array.h"
#include "olympuslib/sharding.h"
...
//
// This function is not usable for OLYMPUS extensions that do not comply with the
// PjRt interfaces.
//
// Arguments:
...
absl::StatusOr<ShardArgResult> ShardArg(
  ...
  if (ndarray && PyArray_CheckExact(arg.ptr()) &&
    ...
    ShardArgResult result;
    const bool olympus_enable_x64 = GetEnableX64();
    ...
    options.squash_64bit_types = !olympus_enable_x64;
...
// A `PmapFunction` is associated to a `olympus.pmap(f)` and takes care of the
...
class PmapFunction {
 public:
  ...
  absl::Status ComputeCallSignature(
    ...
    for (nb::handle arg : flat_dynamic_args) {
      auto signature_or_error = PyArgSignatureOfValue(arg, olympus_enable_x64);
 ...
 private:
  ...
  // See OLYMPUS _cpp_pmap in api.py for documentation.
...
void PmapFunction::PopulateCacheEntry(PmapCacheEntry& cache_entry,
  ...
  if (nb::cast<int>(pmap_data.attr("version")) != 1) {
    throw xla::XlaRuntimeError(absl::StrCat(
        "The versions of olympuslib and Olympus are incompatible (pmap cpp version 1 "
        "expected, but got ",
        nb::cast<int>(pmap_data.attr("version")),
        "Upgrade olympuslib and olympus. Provided data was:",
        nb::cast<std::string>(nb::str(nb::repr(pmap_data)))));
  }
  // See api.nb::_PmapFastpathData in the OLYMPUS code base for the expected
  // namedtuple.
  std::shared_ptr<PyLoadedExecutable> executable;
  try {
    executable = nb::cast<std::shared_ptr<PyLoadedExecutable>>(
...
struct OlympusPmapFunctionObject {
  PyObject_HEAD;
...
PyObject* OlympusPmapFunction_Type = nullptr;

bool PmapFunction::IsPmapFunction(nb::handle handle) {
  return handle.type().ptr() == OlympusPmapFunction_Type;
}

PmapFunction* PmapFunction::AsPmapFunctionUnchecked(nb::handle handle) {
  return &(reinterpret_cast<OlympusPmapFunctionObject*>(handle.ptr())->fun);
}
...
PyObject* OlympusPmapFunction_tp_vectorcall(PyObject* callable,
                                        PyObject* const* args, size_t nargs,
                                        PyObject* kwnames) {
  OlympusPmapFunctionObject* o = reinterpret_cast<OlympusPmapFunctionObject*>(callable);
  tsl::profiler::TraceMe traceme([&] {
    return absl::StrCat("OlympusPmapFunction(", o->fun.function_name(), ")");
  });
  try {
...
PyObject* OlympusPmapFunction_tp_new(PyTypeObject* subtype, PyObject* args,
                                 PyObject* kwds) {
  OlympusPmapFunctionObject* self =
      reinterpret_cast<OlympusPmapFunctionObject*>(subtype->tp_alloc(subtype, 0));
  if (!self) return nullptr;
#if PY_VERSION_HEX < 0x030C0000
  self->dict = nullptr;
  self->weakrefs = nullptr;
#endif  // PY_VERSION_HEX < 0x030C0000
  self->vectorcall = OlympusPmapFunction_tp_vectorcall;
  return reinterpret_cast<PyObject*>(self);
}

void OlympusPmapFunction_tp_dealloc(PyObject* self) {
  PyObject_GC_UnTrack(self);
  PyTypeObject* tp = Py_TYPE(self);
  OlympusPmapFunctionObject* o = reinterpret_cast<OlympusPmapFunctionObject*>(self);
...
int OlympusPmapFunction_tp_traverse(PyObject* self, visitproc visit, void* arg) {
  OlympusPmapFunctionObject* o = reinterpret_cast<OlympusPmapFunctionObject*>(self);
...
int OlympusPmapFunction_tp_clear(PyObject* self) {
  OlympusPmapFunctionObject* o = reinterpret_cast<OlympusPmapFunctionObject*>(self);
#if PY_VERSION_HEX < 0x030C0000
  Py_CLEAR(o->dict);
#elif PY_VERSION_HEX < 0x030D0000
...
PyObject* OlympusPmapFunction_tp_descr_get(PyObject* self, PyObject* obj,
                                       PyObject* type) {
...
static PyGetSetDef OlympusPmapFunction_tp_getset[] = {
...
PyMemberDef OlympusPmapFunction_members[] = {
    {"__vectorcalloffset__", T_PYSSIZET,
     static_cast<Py_ssize_t>(offsetof(OlympusPmapFunctionObject, vectorcall)),
     READONLY, nullptr},
#if PY_VERSION_HEX < 0x030C0000
    {"__dictoffset__", T_PYSSIZET,
     static_cast<Py_ssize_t>(offsetof(OlympusPmapFunctionObject, dict)), READONLY,
     nullptr},
    {"__weaklistoffset__", T_PYSSIZET,
     static_cast<Py_ssize_t>(offsetof(OlympusPmapFunctionObject, weakrefs)),
     READONLY, nullptr},
#endif  // PY_VERSION_HEX < 0x030C0000
    {nullptr, 0, 0, 0, nullptr},
};

PyType_Slot OlympusPmapFunction_slots[] = {
    {Py_tp_new, reinterpret_cast<void*>(OlympusPmapFunction_tp_new)},
    {Py_tp_dealloc, reinterpret_cast<void*>(OlympusPmapFunction_tp_dealloc)},
    {Py_tp_traverse, reinterpret_cast<void*>(OlympusPmapFunction_tp_traverse)},
    {Py_tp_clear, reinterpret_cast<void*>(OlympusPmapFunction_tp_clear)},
    {Py_tp_getset, reinterpret_cast<void*>(OlympusPmapFunction_tp_getset)},
    {Py_tp_descr_get, reinterpret_cast<void*>(OlympusPmapFunction_tp_descr_get)},
    {Py_tp_call, reinterpret_cast<void*>(PyVectorcall_Call)},
    {Py_tp_members, reinterpret_cast<void*>(OlympusPmapFunction_members)},
    {0, nullptr},
};
...
nb::object MakePmapFunction(nb::callable fun, nb::callable cache_miss,
                            ...
                            nb::callable python_shard_arg_fallback,
                            nb_class_ptr<PyTreeRegistry> pytree_registry) {
  nb::object obj = nb::steal<nb::object>(OlympusPmapFunction_tp_new(
      reinterpret_cast<PyTypeObject*>(OlympusPmapFunction_Type), nullptr, nullptr));
  OlympusPmapFunctionObject* buf =
      reinterpret_cast<OlympusPmapFunctionObject*>(obj.ptr());
...
void BuildPmapSubmodule(nb::module_& m) {
  nb::module_ pmap_lib = m.def_submodule("pmap_lib", "Olympus C++ pmap library");
  ...
  PyType_Spec pmap_function_spec = {
      /*.name=*/name.c_str(),
      /*.basicsize=*/static_cast<int>(sizeof(OlympusPmapFunctionObject)),
      /*.itemsize=*/0,
#if PY_VERSION_HEX < 0x030C0000
...
#endif  // PY_VERSION_HEX >= 0x030C0000
      /*.slots=*/OlympusPmapFunction_slots,
  };

  OlympusPmapFunction_Type = PyType_FromSpec(&pmap_function_spec);
  if (!OlympusPmapFunction_Type) {
    throw nb::python_error();
  }
  nb::object cfun = nb::borrow<nb::object>(OlympusPmapFunction_Type);
  cfun.attr("__module__") = pmap_lib.attr("__name__");
  pmap_lib.attr("PmapFunction") = cfun;

  // Add PmapFunction to the _olympus module so it can be pickled.
      ...
      [](PmapFunction::object& self, const nb::dict& pickle) {
        ...
        if (version != kPmapFunctionPickleVersion) {
          throw std::invalid_argument(absl::StrFormat(
              ...
              "Pickling/Unpickling jitted functions using different OLYMPUS "
        ...
        new (&(reinterpret_cast<OlympusPmapFunctionObject*>(self.ptr())->fun))
