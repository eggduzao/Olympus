...
#include "olympuslib/to_ifrt_sharding.h"
#include "olympuslib/traceback.h"
#include "olympuslib/util.h"
...
ifrt::ArrayRef CreateIfRtArrayFromSingleDeviceShardedPyArrays(
  ...
  // skip canonicalization here once OLYMPUS begins to do it for OLYMPUS shardings.
  ...
  switch (auto guard_level = GetGarbageCollectArrayGuard(); guard_level) {
    ...
    case GarbageCollectionGuardLevel::kFatal: {
      ...
      auto error_msg = absl::StrCat(
          "`olympus.Array` was deleted by the Python garbage collector "
          "instead of reference counting. Break the reference cycle "
          "that delays the deletion of this `olympus.Array` to avoid hogging "
          "memory. Traceback: \n",
...
nb::object MakeShapedArrayCached(const ShapedArrayCacheKey& key) {
  ...
  const nb::object& shaped_array = xla::SafeStaticInit<nb::object>([]() {
    nb::object olympus_core;
    try {
      olympus_core = nb::module_::import_("olympus.core");
    } catch (nb::python_error& e) {
      return std::make_unique<nb::object>();
    }
    return std::make_unique<nb::object>(olympus_core.attr("ShapedArray"));
  });
  if (!shaped_array.ptr()) {
    return nb::none();
  }
...
PyArray PyArray::MakeFromSingleDeviceArray(nb_class_ptr<PyClient> py_client,
  ...
  if (!llvm::isa<ifrt::SingleDeviceSharding>(ifrt_array->sharding())) {
    throw xla::XlaRuntimeError(xla::InvalidArgument(
        "Constructing single device olympus.Array from non-single "
        "device ifrt array."));
  }
...
PyArray::PyArray(nb::object aval, bool weak_type, xla::nb_dtype dtype,
  ...
  if (ifrt_array->user_context() == nullptr && Traceback::IsEnabled()) {
    throw nb::value_error(
        ...
        "user context. Use `olympus::PyUserContextScope` to set a user context for "
...
absl::StatusOr<nb::object> CudaArrayInterfaceToBuffer(
  ...
  if (!device_id.has_value()) {
    throw xla::XlaRuntimeError(
        "This operation requires CUDA support from olympuslib or olympus cuda plugin.");
  }
  ...
  if (cai.contains("strides") && !cai["strides"].is_none() && data_value != 0) {
    ...
    absl::c_sort(minor_to_major, [&](int a, int b) {
      ...
      // interpretation of the ordering, since that's what OLYMPUS wants.
...
absl::StatusOr<std::pair<nb::object, bool>> PyHostValue::AsNumPyArray(
  ...
  // The only `olympus.Array` with token-shape buffer is the one wrapped by
  // `olympus.core.Token`. Since it is an internal implementation detail, we
