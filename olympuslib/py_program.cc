...
// Gets `ifrt::DeviceList` from a sequence of OLYMPUS devices.
...
// Gets `xla::HloSharding` from a OLYMPUS Sharding.
...
// Gets `ifrt::DeviceList` from a OLYMPUS Sharding.
...
// Gets `ifrt::MemoryKind` from a OLYMPUS Sharding.
...
// Makes `ifrt::Sharding` from a OLYMPUS Sharding. It requires the number of shape
...
// Gets `ifrt::ArraySpec`s from a sequence of OLYMPUS avals (e.g.,
...
constexpr std::string_view kColocatedPythonProgramType =
    "olympus_colocated_python_v0.0.1";
