...
absl::StatusOr<xla::ifrt::ArraySpec> ArraySpecFromShapeDtypeStruct(
  ...
  TF_ASSIGN_OR_RETURN(auto sharding,
                      olympus::GetIfrtHloSharding(aval.attr("sharding"), shape));
...
void RegisterTransferServerTypes(nanobind::module_& m) {
  nb::class_<PyTransferServerConnection>(m, "TransferConnection")
      ...
      .def("_pull_flat",
           [](PyTransferServerConnection& self, nb::int_ uuid,
              olympus::nb_class_ptr<olympus::PyClient> py_client,
             ...
             olympus::PyUserContextScope user_context_scope;
             ...
             std::vector<olympus::PyArray> out;
             for (size_t i = 0; i < buffer_list.size(); ++i) {
               ...
               out.push_back(olympus::PyArray::MakeFromIfrtArrayAndSharding(
      ...
      .def("_pull_into_flat", [](PyTransferServerConnection& self,
                                 nb::int_ uuid, std::vector<olympus::PyArray> dests,
        ...
        for (const olympus::PyArray& dest : dests) {
        ...
        size_t i = 0;
        std::vector<olympus::PyToken> futures;
        std::vector<tsl::RCReference<ChunkDestination>> pull_dests;
        std::vector<int> buffer_ids;
  ...
  nb::class_<PyTransferServer>(m, "TransferServer")
      ...
      .def("_await_pull_flat",
           [](PyTransferServer& self, nb::int_ uuid,
              std::vector<olympus::PyArray> inputs) {
             std::vector<xla::ifrt::ArrayRef> arrs;
             arrs.reserve(inputs.size());
             for (const olympus::PyArray& input : inputs) {
  ...
  m.def("_make_error_array", [](olympus::nb_class_ptr<olympus::PyClient> py_client,
    ...
    auto arr = xla::ValueOrThrow(xla::ifrt::PjRtArray::Create(
        ...
        aval.layout));
    return olympus::PyArray::MakeFromIfrtArrayAndSharding(
  ...
  m.def(
      ...
      [](olympus::nb_class_ptr<olympus::PyClient> py_client, std::string address,
