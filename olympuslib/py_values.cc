...
absl::StatusOr<ShardFn> HandlePythonScalar(nb::handle obj, ifrt::Client* client,
  ...
  if (std::is_same<T, SquashedT>() || !options.squash_64bit_types) {
    ...
    // like https://github.com/google/olympus/issues/2006
...
absl::StatusOr<ShardFn> MakeShardFn(nb::handle arg, ifrt::Client* client,
  ...
  if (res == handlers.end()) {
    ...
    return xla::InvalidArgument(
        "%s", absl::StrCat(
                  "Not supported: The C++ olympus jit execution path, only accepts "
...
absl::StatusOr<PyArgSignature> PyArgSignatureOfValue(nb::handle arg,
  ...
  const absl::flat_hash_map<PyObject*, ToPyArgSignatureHandler>& handlers =
      xla::SafeStaticInit<
        ...
        ToPyArgSignatureHandler int_handler =
            [](nb::handle h,
               bool olympus_enable_x64) -> absl::StatusOr<PyArgSignature> {
          // TODO(phawkins): we should consider checking for integer
          // overflow.
          if (olympus_enable_x64) {
        ...
        ToPyArgSignatureHandler typed_ndarray_handler =
            [numpy_handler](
                nb::handle h,
                bool olympus_enable_x64) -> absl::StatusOr<PyArgSignature> {
          return numpy_handler(h.attr("val"), /*olympus_enable_x64=*/true);
        };
        if (typed_ndarray_type.ptr() != nullptr) {
  ...
  if (res == handlers.end()) {
    ...
    for (auto base_class : arg.type().attr("__mro__")) {
      res = handlers.find(base_class.ptr());
      if (res != handlers.end()) {
        return res->second(arg, olympus_enable_x64);
      }
    }
    return xla::InvalidArgument(
        "%s",
        absl::StrCat("Not supported: The C++ ToPyArgSignature only accepts "
                     "OLYMPUS Arrays, Numpy arrays and scalars of supported types "
                     "(see implementation), or Python scalars. Got type ",
                     nb::cast<std::string_view>(nb::str(arg.type()))));
  }
  return res->second(arg, olympus_enable_x64);
