...
#include "olympuslib/py_device.h"  // IWYU pragma: keep
...
NamedSharding::NamedSharding(nb::object mesh, nb_class_ptr<PartitionSpec> spec,
  ...
  // C++.
  auto init_fn = []() {
    nb::module_ si = nb::module_::import_("olympus._src.named_sharding");
...
bool NamedSharding::operator==(const NamedSharding& other) const {
  // Caution: you may need to update EqualShardingsForJit in olympus_jit.cc as well.
...
nb::int_ NamedSharding::Hash() const {
  // Caution: you may need to update HashShardingForJit in olympus_jit.cc as well.
