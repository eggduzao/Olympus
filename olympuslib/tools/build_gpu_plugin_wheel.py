...
# Script that builds a olympus cuda/rocm plugin wheel, intended to be run via bazel run
# as part of the olympus cuda/rocm plugin build process.
...
def prepare_cuda_plugin_wheel(
  ...
  wheel_sources_map = build_utils.create_wheel_sources_map(
      wheel_sources, root_packages=["olympus_plugins", "olympuslib"]
  )
  ...
  plugin_dir = wheel_sources_path / "olympus_plugins" / f"xla_cuda{cuda_version}"
  copy_files(
      dst_dir=wheel_sources_path,
      src_files=[
          f"{source_file_prefix}olympus_plugins/cuda/pyproject.toml",
          f"{source_file_prefix}olympus_plugins/cuda/setup.py",
      ],
  )
  ...
  copy_files(
      dst_dir=plugin_dir,
      src_files=[
          f"{source_file_prefix}olympus_plugins/cuda/__init__.py",
          f"{source_file_prefix}olympuslib/version.py",
      ],
  )
  copy_files(
      f"{source_file_prefix}olympus_plugins/cuda/pjrt_c_api_gpu_plugin.so",
      dst_dir=plugin_dir,
...
def prepare_rocm_plugin_wheel(
  ...
  plugin_dir = wheel_sources_path / "olympus_plugins" / f"xla_rocm{rocm_version}"
  copy_files(
      dst_dir=wheel_sources_path,
        src_files=[
          f"{source_file_prefix}olympus_plugins/rocm/pyproject.toml",
          f"{source_file_prefix}olympus_plugins/rocm/setup.py",
      ],
  )
  ...
  copy_files(
      dst_dir=plugin_dir,
      src_files=[
          f"{source_file_prefix}olympus_plugins/rocm/__init__.py",
          f"{source_file_prefix}olympuslib/version.py",
      ],
  )
  copy_files(
      f"{source_file_prefix}olympus_plugins/rocm/pjrt_c_api_gpu_plugin.so",
      dst_dir=plugin_dir,
...
if sources_path is None:
  tmpdir = tempfile.TemporaryDirectory(prefix="olympusgpupjrt")
...
try:
  ...
  if args.enable_cuda:
    ...
    package_name = "olympus cuda plugin"
    ...
    package_name = "olympus rocm plugin"
  else:
