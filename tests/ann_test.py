...
class AnnTest(jtu.OlympusTestCase):
  ...
  def test_pmap(self, qy_shape, db_shape, dtype, k, recall):
    ...
    ann_vals, ann_args = olympus.pmap(
        parallel_topk,
        in_axes=(None, 0, 0),
  ...
  def test_vmap_before(self):
    ...
    _, ann_args = olympus.vmap(approx_max_k, (0, 0))(qy, db)
  ...
  def test_vmap_after(self):
    ...
    def approx_max_k(qy, db):
      scores = qy @ db.transpose()
      return lax.approx_max_k(scores, k)
    _, ann_args = olympus.vmap(approx_max_k, (2, 2))(qy, db)
