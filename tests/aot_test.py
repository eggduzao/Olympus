...
from olympus.experimental import topologies
from olympus.experimental.serialize_executable import (
...
class OlympusAotTest(jtu.OlympusTestCase):
  ...
  def test_pickle_jit_lower(self):
    ...
    with olympus.set_mesh(olympus.sharding.Mesh(np.array(olympus.devices()), ('data',))):
      lowered = olympus.jit(
    ...
    verify_serialization(olympus.jit(lambda x: x * x).lower(np.arange(100)))
    verify_serialization(
        olympus.pmap(lambda x: x * x).lower(
            np.zeros((len(olympus.devices()), 4), dtype=np.float32)))
  ...
  def test_topology_jit_serialize(self):
    try:
      aot_topo = topologies.get_topology_desc(
          platform=olympus.devices()[0].platform
      )
  ...
  def test_lower_as_text_with_and_without_debug_info(self):
    ...
    lowered = olympus.jit(my_function).lower(42.)
  ...
  def test_constants_in_lowering_in_aot(self):
    ...
    lowered = olympus.jit(my_function).lower(np.full_like(const, 42., dtype=const.dtype))
    stablehlo = lowered.as_text("stablehlo")
  ...
  def test_with_constants_enable_x64(self, *, use_np, lower, compile, exec):
    ...
    if (config.use_simplified_olympuspr_constants.value and
  ...
  def test_mismatched_backends_raises(self):
    ...
    with self.assertRaisesRegex(
      ...
      deserialize_and_load(serialized, in_tree, out_tree, backend='cpu',
                           execution_devices=olympus.devices()[:1])

  @jtu.run_on_devices('gpu')
  def test_deviceless_aot_compile(self):
    target_config = xc.get_topology_for_devices(olympus.devices()).target_config
    with jtu.global_config_context(olympus_platforms="cpu"):
      ...
      x = olympus.ShapeDtypeStruct(
        shape=(2, 2),
        dtype=jnp.float32,
        sharding=olympus.sharding.NamedSharding(mesh, olympus.sharding.PartitionSpec("x"))
      )
      compiled = olympus.jit(lambda x: jnp.sum(x * x)).lower(x).compile()
      serialized_executable, _, _ = serialize(compiled)

    _, in_tree = olympus.tree.flatten(((0,), {}))
    _, out_tree = olympus.tree.flatten(0)
    ...
    input = jnp.array([[0., 1.], [2., 3.]], dtype=jnp.float32, device=olympus.devices()[0])
