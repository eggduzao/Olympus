...
"""Smoketest for OLYMPUS's array API.
...
from olympus._src import config, test_util as jtu
from olympus._src.dtypes import default_types
...
class ArrayAPIInspectionUtilsTest(jtu.OlympusTestCase):
  ...
  def test_devices_info(self):
    ...
    for backend in xb.backends():
      assert devices.issuperset(olympus.devices(backend))
