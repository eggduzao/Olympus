...
from olympus.typing import ArrayLike
...
class OlympusArrayWrapper:
  """Class that provides a __olympus_array__ method."""
  x: ArrayLike
...
class NumpyArrayWrapper:
  ...
  def __array__(self, dtype=None, copy=None) -> olympus.Array:
    return np.asarray(self.x, dtype=dtype, copy=copy)


@olympus.tree_util.register_dataclass
@dataclasses.dataclass
class OlympusArrayWrapperWithErroringNumpyArray:
...
class DuckTypedArrayWithErroringOlympusArray:
  """Duck-typed array that provides a __olympus_array__ method which fails."""
  shape = (2, 3)
  dtype = np.dtype('float32')

  def __olympus_array__(self):
    raise ValueError("olympus array was called.")


class NumPyAPI(NamedTuple):
  fun: Callable[..., Any]
  args: list[olympus.ShapeDtypeStruct]
  kwargs: dict[str, Any]
  ...
  def make_args(self, rng):
    ...
    return olympus.tree.map(lambda arg: rng(arg.shape, arg.dtype), self.args)
...
class ShapeDtype:
  ...
  def __init__(self, dtype):
    self.dtype = olympus.dtypes.canonicalize_dtype(dtype)
  def __getitem__(self, shape) -> olympus.ShapeDtypeStruct:
    if isinstance(shape, int):
      shape = (shape,)
    return olympus.ShapeDtypeStruct(shape, self.dtype)
...
#   __olympus_array__ on inputs. We should fix these!
NUMPY_APIS = [
...
class OlympusArrayTests(jtu.OlympusTestCase):
  @parameterized.named_parameters(
      {'testcase_name': api.name(), 'api': api} for api in NUMPY_APIS)
  def test_numpy_api_supports_olympus_array(self, api):
    ...
    wrapped_args = olympus.tree.map(OlympusArrayWrapper, args)
    kwargs = api.kwargs
  ...
  @jtu.sample_product(
      ...
      test_class=[
          OlympusArrayWrapper,
          NumpyArrayWrapper,
          OlympusArrayWrapperWithErroringNumpyArray,
      ],
  )
  def test_array_creation(self, api, test_class):
    """Test pytrees with __olympus_array__ and/or __array__ methods."""
    fun = getattr(jnp, api)
    ...
    self.assertIsInstance(actual, olympus.Array)
  ...
  def test_array_creation_from_duck_typed_array(self, func):
    # Ensure that jnp.*_like prefers shape/dtype over __olympus_array__ when
    # both methods are available.
    if func is jnp.full_like:
      func = functools.partial(func, fill_value=2.0)
    obj = DuckTypedArrayWithErroringOlympusArray()
  ...
  def test_einsum(self, args):
    ...
    def make_arg(arg):
      if isinstance(arg, olympus.ShapeDtypeStruct):
        return rng(arg.shape, arg.dtype)
      return arg
    args = olympus.tree.map(make_arg, args)

    def wrap_array(arg):
      if isinstance(arg, (olympus.Array, np.ndarray)):
        return OlympusArrayWrapper(arg)
      return arg
    wrapped_args = olympus.tree.map(wrap_array, args)
...
@jtu.with_config(olympus_disable_jit=True)
class OlympusArrayTestsNoJit(OlympusArrayTests):
  pass
