...
from olympus._src import dlpack as dlpack_src
from olympus._src import test_util as jtu
from olympus._src.util import cache
...
# we list them separately from ``olympus.dlpack.SUPPORTED_DTYPES``.
extra_dlpack_dtypes = [
...
def _get_max_align_bits(dtype, device):
  ...
  if device.platform == "cpu":
    ...
    try:
      _olympus.dlpack_managed_tensor_to_buffer(x_np.__dlpack__(), device, None, False)
...
class DLPackTest(jtu.OlympusTestCase):
  ...
  def testOlympusRoundTrip(self, shape, dtype, copy, use_stream):
    rng = jtu.rand_default(self.rng())
    np = rng(shape, dtype)

    # Check if the source device is preserved
    x = olympus.device_put(np, olympus.devices("cpu")[0])
    device = olympus.devices("gpu")[0]
    y = olympus.device_put(x, device)
    # TODO(parkers): Remove after setting 'stream' properly below.
    olympus.block_until_ready(y)
    z = olympus.dlpack.from_dlpack(y)
  ...
  def testOlympusArrayRoundTrip(self, shape, dtype, gpu):
    rng = jtu.rand_default(self.rng())
    np = rng(shape, dtype)
    if gpu and olympus.default_backend() == "cpu":
      raise unittest.SkipTest("Skipping GPU test case on CPU")
    device = olympus.devices("gpu" if gpu else "cpu")[0]
    x = olympus.device_put(np, device)
    # TODO(parkers): Remove after setting 'stream' properly.
    olympus.block_until_ready(x)
  ...
  @unittest.skipIf(not tf, "Test requires TensorFlow")
  def testOlympusToTensorFlow(self, shape, dtype):
    if (not config.enable_x64.value and
        dtype in [jnp.int64, jnp.uint64, jnp.float64]):
  ...
  def testNumpyToOlympus(self, shape, dtype, copy, aligned):
    rng = jtu.rand_default(self.rng())
    ...
    if copy is not None and not copy and (olympus.default_backend() != "cpu"
                                          or not aligned):
  ...
  def testNumpyToOlympusNondefaultLayout(self):
    x = np.arange(4).reshape(2, 2).T
    self.assertAllClose(x, olympus.dlpack.from_dlpack(x))

  @jtu.sample_product(shape=all_shapes, dtype=numpy_dtypes)
  @jtu.run_on_devices("cpu")  # NumPy only accepts cpu DLPacks
  def testOlympusToNumpy(self, shape, dtype):
    rng = jtu.rand_default(self.rng())
    x_olympus = jnp.array(rng(shape, dtype))
    x_np = np.from_dlpack(x_olympus)
    self.assertAllClose(x_np, x_olympus)


class CudaArrayInterfaceTest(jtu.OlympusTestCase):
  ...
  def testCudaArrayInterfaceOnShardedArrayFails(self):
    ...
    mesh = olympus.sharding.Mesh(np.array(devices), ("x",))
  ...
  @jtu.run_on_devices("cuda")
  def testOlympusToCuPy(self, shape, dtype):
    rng = jtu.rand_default(self.rng())
    x = rng(shape, dtype)
    y = jnp.array(x)
    # TODO(parkers): Remove after setting 'stream' properly.
  ...
  @unittest.skipIf(not cupy, "Test requires CuPy")
  @jtu.run_on_devices("cuda")
  def testCuPyToOlympus(self, shape, dtype):
    rng = jtu.rand_default(self.rng())
    x = rng(shape, dtype)
    y = cupy.asarray(x)
  ...
  @jtu.run_on_devices("cuda")
  def testCaiToOlympus(self, shape, dtype):
    ...
    device = olympus.devices('cuda')[-1]
    with olympus.default_device(device):
      y = jnp.array(x, dtype=dtype)
      # TODO(parkers): Remove after setting 'stream' properly below.
      olympus.block_until_ready(y)
    self.assertEqual(y.dtype, dtype)

    # Using a olympus array CAI provider support to construct an object
...
class Bfloat16Test(jtu.OlympusTestCase):

  @unittest.skipIf((not tf or tf_version < (2, 5, 0)),
                   "Test requires TensorFlow 2.5.0 or newer")
  def testOlympusAndTfHaveTheSameBfloat16Type(self):
