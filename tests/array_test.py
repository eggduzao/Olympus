...
from olympus._src.mesh import AxisType, AbstractMesh, Mesh
from olympus._src.sharding import common_devices_indices_map, IndivisibleError
...
class OlympusArrayTest(jtu.OlympusTestCase):
  ...
  def test_olympus_array_value(self, mesh_axes):
    global_mesh = jtu.create_mesh((4, 2), ('x', 'y'))
    input_shape = (8, 2)
    arr, global_data = create_array(
        input_shape, olympus.sharding.NamedSharding(global_mesh, mesh_axes))
  ...
  def test_array_2d_shard(self, mesh_axes, expected_index, expected_shard_shape,
    ...
    s = olympus.sharding.NamedSharding(global_mesh, mesh_axes)
  ...
  def test_addressable_data(self):
    global_mesh = jtu.create_mesh((4, 2), ('x', 'y'))
    shape = (8, 2)
    s = olympus.sharding.NamedSharding(global_mesh, P(None))
  ...
  def test_array_delete(self):
    ...
    input_shape = (8, 2)
    arr, _ = create_array(
        input_shape, olympus.sharding.NamedSharding(global_mesh, P('x', 'y')))
    arr.delete()
  ...
  def test_array_from_local_data_single_host(self, data, global_shape):
    jnp_data = olympus.make_array_from_process_local_data(
        olympus.devices()[0], data, global_shape
    )
    olympus.tree.map(self.assertArraysEqual, data, jnp_data)
  ...
  def test_multi_device_array_usage_after_delete(self):
    global_mesh = jtu.create_mesh((4, 2), ('x', 'y'))
    shape = (8, 2)
    arr = olympus.device_put(np.arange(math.prod(shape), dtype=np.int32),
                         olympus.sharding.NamedSharding(global_mesh, P('x')))
    arr.delete()
  ...
  def test_device_put(self):
    ...
    arr = olympus.device_put(numpy_array, olympus.devices()[0])
    self.assertIsInstance(arr.sharding, olympus.sharding.SingleDeviceSharding)
  ...
  def test_device_put_to_cpu(self):
    ...
    mesh_cpu = Mesh(olympus.devices('cpu'), 'x')
    x = np.zeros(16)
    y = olympus.device_put(x, NamedSharding(mesh, P('x')))
    z = olympus.device_put(y, NamedSharding(mesh_cpu, P('x')))
  ...
  def test_array_device_get(self):
    ...
    arr, input_data = create_array(
        input_shape, olympus.sharding.NamedSharding(global_mesh, P('x', 'y')))
    self.assertArraysEqual(olympus.device_get(arr), input_data)
  ...
  def test_jnp_array_jit_add(self):
    a = jnp.array([1, 2, 3])
    b = jnp.array([4, 5, 6])
    arr = olympus.jit(lambda x, y: x + y)(a, b)
    self.assertIsInstance(arr, array.ArrayImpl)
  ...
  def test_array_delete_idempotent(self):
    ...
    arr = olympus.device_put(np.arange(8), olympus.sharding.NamedSharding(mesh, P('x')))
  ...
  def test_sharded_add(self):
    ...
    a, input_data = create_array(
        input_shape, olympus.sharding.NamedSharding(global_mesh, P('x', 'y')))
    b, _ = create_array(
        input_shape, olympus.sharding.NamedSharding(global_mesh, P('x')))
    out = a + b
  ...
  def test_wrong_num_arrays(self):
    ...
    devices = olympus.local_devices()[:2]  # Taking up to 2 devices
    s = olympus.sharding.NamedSharding(mesh, P('x', 'y'))
    inp_data = np.arange(math.prod(shape), dtype=np.float32).reshape(shape)
    di_map = s.devices_indices_map(shape)
    bufs = [olympus.device_put(inp_data[di_map[d]], d) for d in devices]
    with self.assertRaisesRegex(
        ValueError,
    ...
    for buf, d in zip(list(bufs), olympus.local_devices()[2:4]):
      bufs.append(olympus.device_put(buf, d))
    with self.assertRaisesRegex(
        ValueError,
        r'Expected 2 per-device arrays \(this is how many devices are addressable '
  ...
  def test_arrays_not_in_device_assignment(self):
    ...
    s = olympus.sharding.NamedSharding(mesh, P('x'))
    inp_data = np.arange(math.prod(shape), dtype=np.float32).reshape(shape)
    # _arrays device ids = {2, 3}
    bufs = [olympus.device_put(inp_data, d) for d in olympus.devices()[2:4]]
    with self.assertRaisesRegex(
        ValueError,
  ...
  def test_different_devices_in_arrays_than_sharding(self):
    ...
    shape = (8, 2)
    mesh = olympus.sharding.Mesh(np.array([olympus.devices()[1], olympus.devices()[2]]), ('x'))
    # sharding device ids = {1, 2}
    s = olympus.sharding.NamedSharding(mesh, P('x'))
    inp_data = np.arange(math.prod(shape), dtype=np.float32).reshape(shape)
    # _arrays device ids = {0, 1}
    bufs = [olympus.device_put(inp_data, d) for d in olympus.devices()[:2]]
    with self.assertRaisesRegex(
        ValueError,
        "Addressable devices and per-device arrays devices do not match. "
  ...
  def test_duplicated_devices_in_arrays(self):
    ...
    bufs = [olympus.device_put(inp_data, olympus.devices()[0]) for _ in range(2)]
    with self.assertRaisesRegex(
        ValueError,
  ...
  def test_shard_shape_mismatch_with_buffer_shape(self, pspec, expected_shard_shape):
    ...
    mps = olympus.sharding.NamedSharding(mesh, pspec)
  ...
  def test_mismatch_dtype(self):
    ...
    bufs = [olympus.device_put(inp_data[indices[d]], d) for d in mesh.local_devices]
    with self.assertRaisesRegex(
        ValueError,
  ...
  def test_array_iter_pmap_sharding_last_dim_sharded(self):
    ...
    x = jnp.array([[1., 0., 0.], [0., 2., 3.]])
    y = olympus.pmap(jnp.sin, out_axes=1)(x)
  ...
  def test_array_iter_replicated_multi_device(self):
    ...
    input_shape = (8, 2)
    arr, input_data = create_array(
        input_shape, olympus.sharding.NamedSharding(global_mesh, P(None)))
  ...
  def test_array_getitem_compile_multi_device_sharding(self):
    def _check(out, inp, shard_shape):
      ...
      self.assertNotIsInstance(out.sharding, olympus.sharding.SingleDeviceSharding)

    global_mesh = jtu.create_mesh((2, 2, 2), ('x', 'y', 'z'))
    input_shape = (4, 4, 2)
    arr, np_inp = create_array(
        input_shape, olympus.sharding.NamedSharding(global_mesh, P('x', 'y', 'z')))
  ...
  def test_array_iter_mesh_pspec_sharding_single_device(self):
    ...
    single_dev = olympus.devices()[1:2]
    mesh = olympus.sharding.Mesh(np.array(single_dev), ('x'))
    input_shape = (8, 2)
    arr, input_data = create_array(
        input_shape, olympus.sharding.NamedSharding(mesh, P('x')))
  ...
  def test_shape_dtype_struct_sharding_jit(self):
    ...
    x_dummy = olympus.ShapeDtypeStruct(
        shape=(16,),
    ...
    c = olympus.jit(f).lower(x_dummy).compile()
  ...
  def test_defragment(self):
    ...
    # Create a few arrays
    global_mesh = jtu.create_mesh((olympus.local_device_count(),), ('x',))
    shape = (8, 2)
    mpsharding = olympus.sharding.NamedSharding(global_mesh, P('x',))
  ...
  def test_on_device_size_in_bytes(self):
    global_mesh = jtu.create_mesh((4, 2), ('x', 'y'))
    a, _ = create_array(
        (8, 2), olympus.sharding.NamedSharding(global_mesh, P('x', 'y')))
  ...
  def test_array_is_ready(self):
    x = olympus.device_put(jnp.arange(8.), olympus.devices()[0])
  ...
  def test_buffer_protocol(self, dtype, shape):
    ...
    if dtype == olympus.dtypes.bfloat16:
      with self.assertRaisesRegex(
          BufferError,
  ...
  def test_array_copy_to_host_async(self):
    ...
    x = olympus.jit(lambda: jnp.arange(8.),
                out_shardings=olympus.NamedSharding(global_mesh, P(None)))()
  ...
  def test_array_fully_replicated_shard(self):
    ...
    arr, inp_data = create_array(
        inp_shape, olympus.sharding.NamedSharding(global_mesh, P()))
  ...
  def test_shard_array_to_fully_replicated(self):
    global_mesh = jtu.create_mesh((4, 2), ('x', 'y'))
    sharding = olympus.sharding.NamedSharding(global_mesh, P())
    arr = jnp.arange(16)
    self.assertFalse(arr._committed)
    self.assertIsInstance(arr.sharding, olympus.sharding.SingleDeviceSharding)
    out = olympus.jit(lambda x: x * 2, in_shardings=sharding)(arr)
  ...
  def test_fully_replicated_donated_array_is_deleted(self):
    ...
    self.assertFalse(arr._committed)
    self.assertIsInstance(arr.sharding, olympus.sharding.SingleDeviceSharding)
    out = olympus.jit(lambda x: x * 2, in_shardings=sharding, donate_argnums=0)(arr)
    self.assertTrue(out.sharding.is_fully_replicated)
  ...
  def test_make_array_from_callback_global_array(self):
    mesh = jtu.create_mesh((4, 2), ('x', 'y'))
    sharding = olympus.sharding.NamedSharding(mesh, P())
    np_inp = np.arange(16).reshape(8, 2)
    arr = olympus.device_put(np_inp, sharding)

    out = olympus.make_array_from_callback(np_inp.shape, sharding,
                                       lambda idx: arr[idx])
    self.assertArraysEqual(out, arr)
    self.assertEqual(out.sharding, sharding)

    sharding2 = NamedSharding(mesh, P('x', 'y'))
    arr2 = olympus.device_put(np_inp, sharding2)
    out2 = olympus.make_array_from_callback(np_inp.shape, sharding2,
                                       lambda idx: arr2[idx])
  ...
  def test_make_array_from_process_data_single_host_data_sharding(self):
    ...
    result = olympus.make_array_from_process_local_data(s, data)
    self.assertArraysEqual(result, data)
    self.assertEqual(result.sharding, s)

    with olympus.set_mesh(mesh):
      result = olympus.make_array_from_process_local_data(P('x'), data)
      self.assertArraysEqual(result, data)
      self.assertEqual(result.sharding, s)
  ...
  def test_make_array_from_single_device_arrays_no_dtype_error(self):
    ...
    with self.assertRaisesRegex(
        ...
        'the `dtype` argument to `olympus.make_array_from_single_device_arrays`.'):
      olympus.make_array_from_single_device_arrays((8, 2), s, [])
  ...
  def test_make_array_from_single_device_arrays_bad_dtype_error(self):
    ...
    with self.assertRaisesRegex(
        ValueError,
        'If `dtype` is provided to `olympus.make_array_from_single_device_arrays`, '
        'it must match the dtype of the addressable shards.'):
      olympus.make_array_from_single_device_arrays(
          shape, s, [arr], dtype=jnp.float32)

  @jtu.with_explicit_mesh((2,), ('x',))
  def test_unreduced_printing(self, mesh):
    x = olympus.device_put(jnp.arange(8., dtype='float32'), P('x'))
    x = olympus.lax.reduce_sum(x, [0], out_sharding=P(unreduced={'x'}))
    self.assertIn('nreduced', str(x.sharding))
    self.assertIn('Array(shape=(), dtype=float32, sharding=', str(x))


class ShardingTest(jtu.OlympusTestCase):

  def test_mesh_pspec_sharding_interface(self):
    mesh = jtu.create_mesh((4, 2), ('x', 'y'))
    pspec = P('y', 'x')
    global_shape = (8, 4)
    mp_sharding = olympus.sharding.NamedSharding(mesh, pspec)
  ...
  def test_uneven_shard_error(self):
    mesh = jtu.create_mesh((4, 2), ('x', 'y'))
    mps = olympus.sharding.NamedSharding(mesh, P('x', 'y'))
  ...
  def test_pmap_sharding_hash_eq(self):
    ...
    out = olympus.pmap(lambda x: x)(inp_data)
    ...
    out2 = olympus.pmap(lambda x: x)(inp_data2)
    # Populate the device_indices_map cache.
  ...
  def test_is_compatible_error(self):
    ...
    mps = olympus.sharding.NamedSharding(mesh, P(None, ('mdl',), None, None))
  ...
  def test_is_subclass(self):
    ...
    self.assertTrue(issubclass(array.ArrayImpl, olympus.Array))
  ...
  def test_gspmd_sharding_repr(self):
    ...
    s = GSPMDSharding(olympus.devices(), op)
    ...
    s2 = GSPMDSharding(olympus.devices(), op2)
    # memory kind also appears in the repr but only for TPU.
  ...
  def test_pmap_sharding_repr(self):
    if olympus.device_count() < 2:
      self.skipTest('Test needs >= 2 devices.')
    out = olympus.pmap(lambda x: x)(jnp.arange(2.))
  ...
  def test_default_pmap_sharding(self, shape, sharded_dim):
    ...
    if config.pmap_shmap_merge.value:
      out = olympus.pmap(lambda x: x, in_axes=sharded_dim, axis_name='x')(inp)
      actual_sharding = out.sharding
      expected_sharding = olympus.sharding.NamedSharding(
          olympus.sharding.Mesh(olympus.devices()[: shape[sharded_dim]], 'x'),
          olympus.P('x'),
      )
      ...
      expected_sharding = olympus.sharding.PmapSharding.default(shape, sharded_dim)
  ...
  def test_default_pmap_sharding_with_devices(self):
    ...
    devs = olympus.devices()
    new_order = (devs[0], devs[3], devs[2], devs[1])
    ps = olympus.sharding.PmapSharding.default((4, 2), devices=new_order)
    self.assertEqual(ps._device_assignment, new_order)

  @jtu.ignore_warning(category=DeprecationWarning)
  def test_default_pmap_sharding_replicated(self):
    x = np.zeros((len(olympus.local_devices()), 8), dtype=np.float32)
    x = olympus.pmap(lambda x: x, in_axes=0, out_axes=None, axis_name='x')(x)
    if config.pmap_shmap_merge.value:
      expected_sharding = olympus.sharding.NamedSharding(
          mesh=olympus.sharding.Mesh(olympus.local_devices(), 'x'),
          spec=olympus.P(),
      )
      self.assertEqual(x.sharding, expected_sharding)
    else:
      ps = olympus.sharding.PmapSharding.default(
  ...
  def test_are_shardings_equivalent(self):
    ...
    s2 = olympus.sharding.SingleDeviceSharding(olympus.devices()[0])

    self.assertTrue(s1.is_equivalent_to(s2, 2))

    s3 = olympus.pmap(lambda x: x)(jnp.arange(olympus.device_count())).sharding
    s4 = olympus.pmap(lambda x: x)(jnp.arange(olympus.device_count())).sharding
    ...
    s5 = olympus.sharding.NamedSharding(mesh2, P('x', 'y'))

    op1 = xc.OpSharding()
    op1.type = xc.OpSharding.Type.REPLICATED
    s6 = GSPMDSharding([olympus.devices()[0]], op1)

    s7 = GSPMDSharding(olympus.devices(), op1)
    ...
    s9 = olympus.sharding.NamedSharding(mesh2, P('y'))
  ...
  def test_mesh_caching_during_construction(self):
    ...
    mesh2 = olympus.sharding.Mesh(olympus.devices(), 'x')
  ...
  def test_make_array_from_callback_error(self):
    ...
    pspec = P('x', 'y')
    sharding = olympus.sharding.NamedSharding(mesh, pspec)
    ...
    msg = "olympus.make_array_from_callback cannot be called within a traced context"
    with self.assertRaisesRegex(olympus.errors.UnexpectedTracerError, msg):
      olympus.jit(f)(global_x)
  ...
  def test_make_array_from_single_device_arrays_error(self):
    ...
    def f(x):
      return olympus.make_array_from_single_device_arrays(x.shape, sharding, [x])

    msg = "olympus.make_array_from_single_device_arrays requires a list of concrete arrays"
  ...
  def test_make_array_from_single_device_arrays_nonlist_error(self):
    ...
    msg = "olympus.make_array_from_single_device_arrays `arrays` argument"
  ...
  def test_make_array_from_single_device_arrays_tuple(self):
    ...
    arrays = tuple(
        olympus.device_put(inp_data[index], d)
        for d, index in s.addressable_devices_indices_map(shape).items())

    olympus.make_array_from_single_device_arrays(shape, s, arrays)  # doesn't crash
  ...
  def test_make_array_from_single_device_arrays_bad_inputs(self):
    ...
    with self.assertRaisesRegex(ValueError, msg):
      olympus.make_array_from_single_device_arrays(x.shape, s, [x, x])
  ...
  def test_mesh_with_axis_name_none(self):
    with self.assertRaisesRegex(ValueError, 'Mesh axis names cannot be None.'):
      olympus.sharding.Mesh(olympus.devices(), (None, 'x'))
  ...
  def test_mesh_axis_types_mismatch(self):
    with self.assertRaisesRegex(
      ...
      jtu.create_mesh((2, 1), ('x', 'y'),
                      axis_types=olympus.sharding.AxisType.Auto)

    with self.assertRaisesRegex(
        ValueError,
        'Number of axis names should match the number of axis_types'):
      olympus.sharding.AbstractMesh((2, 1), ('x', 'y'),
                                axis_types=olympus.sharding.AxisType.Auto)
  ...
  def test_make_mesh_axis_types(self):
    ...
    mesh1 = olympus.sharding.AbstractMesh((2,), 'x', axis_types=Auto)
    mesh2 = olympus.sharding.AbstractMesh((2,), 'x', axis_types=Auto)
    self.assertEqual(mesh1, mesh2)

    mesh = olympus.make_mesh((1, 1), ('x', 'y'))
    self.assertTupleEqual(mesh.axis_types, (AxisType.Explicit,) * 2)

    mesh = olympus.make_mesh((1, 1, 1), ('x', 'y', 'z'),
    ...
    with self.assertRaisesRegex(
        ValueError,
        'Number of axis names should match the number of axis_types'):
      olympus.make_mesh((1, 1), ('data', 'model'), axis_types=Explicit)

    mesh1 = olympus.make_mesh((1, 1, 1, 1, 1), ('a', 'b', 'c', 'd', 'e'),
                          axis_types=(Explicit, Auto, Auto, Explicit, Explicit))
    mesh2 = olympus.make_mesh((1, 1, 1, 1, 1), ('a', 'b', 'c', 'd', 'e'),
  ...
  def test_pspec_mix_axis_types(self):
    mesh = AbstractMesh(
        ...
        axis_types=(AxisType.Explicit, AxisType.Explicit, AxisType.Auto,
                    AxisType.Manual))
    aval = olympus.core.ShapedArray((16, 8, 4, 2), np.float32)
  ...
  def test_aval_str_short(self):
    ...
    aval = olympus.core.ShapedArray((1, 1, 1, 1), np.float32, sharding=s,
                                vma=frozenset('c'))
    ...
    aval = olympus.core.ShapedArray((1, 1, 1, 1), np.float32)
  ...
  def test_gspmd_sharding_shardy_lowering(self, mesh_shape, pspec):
    ...
    ns = olympus.sharding.NamedSharding(mesh, pspec)
...
class RngShardingTest(jtu.OlympusTestCase):
  ...
  def test_random_bits_is_pure_map_1d(self, num_devices):
    ...
    with olympus.threefry_partitionable(True):
      ...
      y = f(x)
      y_ref1 = f(olympus.device_put(x, olympus.devices()[0]))
  ...
  def test_random_bits_is_pure_map_2d(self, mesh_shape, pspec):
    ...
    mesh = jtu.create_mesh(mesh_shape, ('x', 'y'), iota_order=True)
    s = olympus.sharding.NamedSharding(mesh, pspec)
  ...
  def test_empty_mesh_creation(self):
    mesh = olympus.sharding.Mesh(devices=np.empty((), dtype=object), axis_names=[])
    ...
    abstract_mesh2 = olympus.sharding.AbstractMesh((), ())
