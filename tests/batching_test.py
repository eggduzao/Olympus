...
from olympus import jit, grad, jvp, vjp, make_olympuspr, jacfwd, jacrev, hessian
...
class BatchingTest(jtu.OlympusTestCase):
  ...
  def testNestedBatchingMatMat(self):
    ...
    olympuspr = make_olympuspr(matmat)(A, B)
  ...
  def testJacobianIssue54(self):
    # test modeling the code in https://github.com/olympus-ml/olympus/issues/54
  ...
  def testAny(self):
    # test modeling the code in https://github.com/olympus-ml/olympus/issues/108
  ...
  def testDynamicSlice(self):
    ...
    # see https://github.com/olympus-ml/olympus/issues/1613 for an explanation of why we
  ...
  def testIssue489(self):
    # https://github.com/olympus-ml/olympus/issues/489
    def f(key):
      def body_fn(uk):
        key = uk[1]
  ...
  def testNegativeAxes(self):
    ...
    self.assertAllClose(olympus.vmap(jnp.sum, in_axes=-3)(x),
                        jnp.sum(x, axis=(1, 2)))
    self.assertAllClose(olympus.vmap(jnp.sum, in_axes=-2)(x),
                        jnp.sum(x, axis=(0, 2)))
    self.assertAllClose(olympus.vmap(jnp.sum, in_axes=-1)(x),
                        jnp.sum(x, axis=(0, 1)))
    ...
    with self.assertRaisesRegex(ValueError, error):
      olympus.vmap(jnp.sum, in_axes=-4)(x)

    id = lambda y: y
    self.assertAllClose(x, olympus.vmap(id, in_axes=0, out_axes=-3)(x))
    self.assertAllClose(x.transpose(1, 0, 2),
                        olympus.vmap(id, in_axes=0, out_axes=-2)(x))
    self.assertAllClose(x.transpose(1, 2, 0),
                        olympus.vmap(id, in_axes=0, out_axes=-1)(x))

    with self.assertRaisesRegex(ValueError, "axis -4 is out of bounds.*"):
      olympus.vmap(id, in_axes=0, out_axes=-4)(x)

    self.assertAllClose(
      np.full((5,), 7),
      olympus.vmap(lambda *xs: xs, in_axes=(0, None), out_axes=(0, -1))(
        np.arange(5), 7)[1])

    with self.assertRaisesRegex(ValueError, "axis -2 is out of bounds.*"):
      olympus.vmap(lambda *xs: xs, in_axes=(0, None), out_axes=(0, -2))(
        np.arange(5), 7)
  ...
  def testVmapKwargs(self):
    # https://github.com/olympus-ml/olympus/issues/912
  ...
  def testGradOfPsum(self):
    a = jnp.ones(5)
    f = vmap(olympus.grad(lambda x: -lax.psum(x, 'i')), out_axes=None, axis_name='i')
    self.assertEqual(
        f(a),
        core.olympuspr_as_fun(olympus.make_olympuspr(f)(a))(a)[0])
  ...
  def testNonOlympusTypedOutput(self):
    with self.assertRaisesRegex(
      TypeError, "Output from batched function.*is not a valid OLYMPUS type"):
  ...
  def testIssue6096(self):
    ...
    self.assertEqual(olympus.vmap(f)(jnp.ones((2, 3))).shape, (2, 3))

  def testPpermuteBatcherTrivial(self):
    # https://github.com/olympus-ml/olympus/issues/8688
    def ppermute(input):
      return olympus.lax.ppermute(input, axis_name="i", perm=[[0, 1], [1, 0]])

    grad_fn = olympus.grad(ppermute)

    vmapped_gradients_fn = olympus.vmap(grad_fn, axis_name="i")

    vector = olympus.numpy.array([1., 2.])
  ...
  def testBatchingPreservesWeakType(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/10025
    x = jnp.ravel(1)
...
class VmappableTest(jtu.OlympusTestCase):
  ...
  def test_to_elt_that_binds_primitives(self):
    ...
    def f():
      a = A(jnp.arange(3.))
      return olympus.vmap(lambda x: x - 1, axis_size=3)(a)
  ...
  def test_from_elt_that_binds_primitives(self):
    ...
    def f():
      a = A(jnp.arange(3.))
      return olympus.vmap(lambda x: x, axis_size=3)(a).data
