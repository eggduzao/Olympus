...
def uniform_kernel(block_index, key, total_size, block_size, tile_size):
  ...
  keys = blocked_fold_in(block_index, key,
                         ...
                         block_size=block_size,
                         tile_size=tile_size)
  return blocked_sampler.sample_block(olympus.random.uniform,
                                         keys,
                                         block_size=block_size,
                                         tile_size=tile_size,
...
class BlockedSamplerTest(jtu.OlympusTestCase):
  ...
  def test_block_shape_invariance(self, total_size, block_size_a,
    ...
    global_key = olympus.random.key(0)
...
class BlockedFoldInTest(jtu.OlympusTestCase):
  @parameterized.named_parameters(
  ...
  def test_blocked_fold_in_shape_invariance(self, total_size, block_size_a,
    ...
    result_b = call_kernel_3d(
        ...
        block_size_b, tile_size)
    np.testing.assert_array_equal(olympus.random.key_data(result_a),
                                  olympus.random.key_data(result_b))
