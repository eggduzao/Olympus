...
from olympus.experimental import buffer_callback

olympus.config.parse_flags_with_absl()


class BufferCallbackTest(jtu.OlympusTestCase):
  ...
  def test_numpy(self, dtype):
    def callback(ctx, out, arg):
      with self.assertRaisesRegex(
          olympus.errors.OlympusRuntimeError, "XLA FFI GPU context is not available"
      ):
        ctx.stream
    ...
    fun = buffer_callback.buffer_callback(
        callback, olympus.ShapeDtypeStruct(data.shape, data.dtype)
    )
  ...
  def test_dlpack(self, dtype):
    ...
    # DLPack tensors.
    olympus.block_until_ready(fun(data))
  ...
  def test_cuda_array_interface(self, dtype, command_buffer_compatible):
    if command_buffer_compatible:
      self.skipTest("Requires olympuslib extension version of at least 337.")
  ...
  def test_batching(self, vmap_method):
    ...
    self.assertArraysEqual(olympus.vmap(fun)(x, y), x + y)
  ...
  def test_buffer_callback_multi_mesh(self):
    ...
    def f(x, y):
      z = x * y
      output_shape = olympus.ShapeDtypeStruct(x.shape, x.dtype)
    ...
    x = olympus.device_put(
        jnp.ones((32, 32)), olympus.NamedSharding(mesh1, olympus.P('a', 'b')))
    y = olympus.device_put(
        jnp.ones((32, 32)), olympus.NamedSharding(mesh2, olympus.P('x', 'y')))
    f(x, y)  # doesn't crash
  ...
  def test_side_effect(self):
    ...
    called = False
    fun = buffer_callback.buffer_callback(
        callback, olympus.ShapeDtypeStruct((), jnp.float32), has_side_effect=True)
    olympus.block_until_ready(fun())
