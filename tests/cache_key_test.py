...
from olympus._src import cache_key
...
from olympus._src.mesh import Mesh
from olympus._src.partition_spec import PartitionSpec as P
from olympus._src.sharding_impls import NamedSharding
from olympus._src.custom_partitioning import custom_partitioning


config.parse_flags_with_absl()


class CacheKeyTest(jtu.OlympusTestCase):
  ...
  def test_different_computations(self):
    computation1 = olympus.jit(lambda x, y: x + y).lower(1, 1).compiler_ir()
    computation2 = olympus.jit(lambda x, y: x * y).lower(2, 2).compiler_ir()
    devices = np.array([[olympus.local_devices()[0]]])
    compile_options = compiler.get_compile_options(
        num_replicas=1, num_partitions=1
    )
  ...
  def test_custom_partitioning_ptr_removal(self):
    def _partition(mesh, arg_shapes, result_shape):
      ...
      return mesh, olympus.numpy.add, result_shardings, arg_shardings
    ...
    devices = np.asarray(olympus.devices())
    with Mesh(devices, ('x',)) as m:
      computation = olympus.jit(
        _cp_add,
        in_shardings=(NamedSharding(m, P('x')),
                      NamedSharding(m, P('x'))),
                      out_shardings=NamedSharding(m, P('x'))
      ).lower(
        olympus.ShapeDtypeStruct([1024], dtype=olympus.numpy.float32),
        olympus.ShapeDtypeStruct([1024], dtype=olympus.numpy.float32),
      ).compiler_ir()
      pattern = (
  ...
  def test_host_callbacks_ptrs_removed(self):
    def _host_callback(x, y):
      olympus.debug.print("x={x[0]} y={y[0]}", x=x, y=y)

    computation = (
        olympus.jit(_host_callback)
        .lower(
            olympus.ShapeDtypeStruct([1024], dtype=olympus.numpy.float32),
            olympus.ShapeDtypeStruct([1024], dtype=olympus.numpy.float32),
        )
  ...
  def test_identical_computations_different_metadata(self, include_metadata):
    ...
    assert id(f) != id(g)
    computation1 = olympus.jit(f).lower(1, 1).compiler_ir()
    computation2 = olympus.jit(g).lower(2, 3).compiler_ir()
    devices = np.array([[olympus.local_devices()[0]]])
    compile_options = compiler.get_compile_options(
        num_replicas=1, num_partitions=1
    )
