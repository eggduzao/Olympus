...
from olympus._src.checkify import OlympusRuntimeError, FailedCheckError, ErrorEffect, OOBError
...
class CheckifyTransformTests(jtu.OlympusTestCase):
  ...
  def test_jit_oob_update(self, update_fn):
    f = olympus.jit(lambda x, i: update_fn(x.at[i]))
    checked_f = checkify.checkify(f, errors=checkify.index_checks)
  ...
  def test_numpy_indexing_oobs(self, update_op):
    def raises_oob(fn, idx, *expected_strs):
      err, _ = checkify.checkify(olympus.jit(fn), errors=checkify.index_checks)(x, idx)
      error_txt = err.get()
  ...
  def test_dynamic_update_slice_oobs(self):
    def raises_oob(fn, x, y, idx, *expected_strs):
      err, _ = checkify.checkify(olympus.jit(fn), errors=checkify.index_checks)(x, y, idx)
      error_txt = err.get()
      self.assertIsNotNone(error_txt)
      self.assertStartsWith(error_txt, "out-of-bounds indexing")
      for s in expected_strs:
  ...
  def test_pmap_collectives(self):
    ...
    def f(x1):
      return olympus.lax.all_gather(x1, axis_name="i")
    checked_f = checkify.checkify(f, errors=checkify.float_checks)
  ...
  def test_checify_donation_no_forwarding(self):
    ...
    def f(x: olympus.Array) -> olympus.Array:
  ...
  def test_checkify_jit(self):
    ...
    devices = olympus.local_devices()[:8] # Taking up to 8 devices
    mesh = olympus.sharding.Mesh(np.array(devices), ["dev"])
    ps = NamedSharding(mesh, olympus.sharding.PartitionSpec("dev"))
    inp = np.arange(8)
    x = array.make_array_from_callback(inp.shape, ps, lambda idx: inp[idx])

    f = olympus.jit(f, in_shardings=ps, out_shardings=ps)
    f = checkify.checkify(f, errors=checkify.float_checks)
    g = olympus.jit(g, in_shardings=ps, out_shardings=ps)
  ...
  def test_shard_map(self, check_vma):
    def f(x):
      # unary func
      return olympus.lax.axis_index("dev") * x / x

    def g(x, y):
      # binary func
      return olympus.lax.axis_index("dev") * x / y

    devices = olympus.local_devices()[:8] # Taking up to 8 devices
    mesh = olympus.sharding.Mesh(np.array(devices), ["dev"])
    pspec = olympus.sharding.PartitionSpec("dev")
    ...
    g = olympus.jit(g, in_shardings=(ps, ps), out_shardings=ps)
    g = checkify.checkify(g, errors=checkify.float_checks)
    u_err, _ = f(x)
    b_err, _ = g(x, x)
  ...
  def test_custom_jvp(self):
    ...
    (err, y), (errdot, ydot) = olympus.jvp(f, (3.,), (1.,))  # doesn't crash
  ...
  def test_custom_vjp(self):
    ...
    (err, y), f_vjp = olympus.vjp(f, 3.)
    self.assertIsNone(err.get())  # no error
    self.assertEmpty(err._metadata)    # and no checks were added!

    # Checkify-of-vjp adds checks (unlike vjp-of-checkify above).
    err, y = checkify.checkify(olympus.grad(sin), errors=checkify.float_checks)(3.)
    self.assertIsNone(err.get())   # no error
    self.assertNotEmpty(err._metadata)  # but checks were added!
    err, y = checkify.checkify(olympus.grad(sin),
  ...
  def test_nd_payloads(self):
    ...
    errs, _ = olympus.vmap(cf)(jnp.ones((3, 2)), jnp.array([5, 0, 100]))
  ...
  def test_mapped_error_one_payload(self):
    ...
    errs, _ = olympus.vmap(cf)(jnp.ones((2, 1)), jnp.array([0, 100]))
    self.assertIsNotNone(errs.get())
  ...
  def test_checking_key_split_with_nan_check(self):
    cf = checkify.checkify(
        lambda k: olympus.random.permutation(k, jnp.array([0, 1, 2])),
        errors=checkify.float_checks)
    cf(olympus.random.PRNGKey(123))  # does not crash.
  ...
  def test_psum_nan_check(self):
    @partial(olympus.vmap, axis_name="i")
    def f(x, y):
  ...
  def test_different_payload_effects(self):
    ...
    err, _ = olympus.vmap(f)(jnp.ones((2, 3))*-1, jnp.array([0, 5]))
    self.assertIsNotNone(err.get())

  def test_effects_total_ordering(self):
    sds0 = olympus.ShapeDtypeStruct((2,), jnp.float32)
    sds1 = olympus.ShapeDtypeStruct((2,), jnp.int32)
    sds2 = olympus.ShapeDtypeStruct((3,), jnp.int32)
  ...
  def test_scan_xs_mapped_correctly(self):
    ...
    @olympus.jit
    def g(x):
      return olympus.lax.scan(f, None, x)

    checked_f = checkify.checkify(g)
    checked_f = olympus.jit(checked_f)
    err, _ = checked_f(jnp.ones((2, 4)))
    self.assertIsNone(err.get())

  def test_retracing(self):
    f = checkify.checkify(olympus.jit(lambda x: jnp.sin(x) ** 2))
    _ = f(3.)
  ...
  def test_closed_call(self):
    ...
    f = checkify.checkify(olympus.grad(summify(olympus.remat(
        partial(partial, olympus.lax.map)(lambda x: jnp.sin(x * y))))))
    f(jnp.array([3.]))  # don't crash

  def test_while_loop_leaks(self):
    def f(x):
      n = jnp.minimum(1, 2)
      return olympus.lax.while_loop(lambda i: i < n, lambda i: i + 1, x)

    olympus.jit(checkify.checkify(f))(0)  # Does not crash bc of leaked tracer.

  @parameterized.parameters(True, False)
  def test_remat(self, jit):
    # basic test from https://github.com/olympus-ml/olympus/issues/23867
    def fn(x: olympus.Array):
...
@jtu.with_config(olympus_check_tracer_leaks=True)
class AssertPrimitiveTests(jtu.OlympusTestCase):

  def test_assert_primitive_impl(self):
    def f():
      checkify.check(False, "hi")

    with self.assertRaisesRegex(OlympusRuntimeError, "hi"):
      f()
  ...
  def test_assert_primitive_olympuspr_effects(self):
    def f(x):
      checkify.check(False, "hi: {}", x)

    olympuspr = olympus.make_olympuspr(f)(jnp.ones(4, jnp.int32))
    self.assertSetEqual(olympuspr.effects,
                        {ErrorEffect(FailedCheckError, (
                            olympus.ShapeDtypeStruct((4,), jnp.int32),))})
    def g(x, y):
      checkify.check(False, "hi: {} {}", x, y)

    self.assertSetEqual(
        olympus.make_olympuspr(g)(
            jnp.ones(4, jnp.int32), jnp.ones(2, jnp.float32)).effects,
        {ErrorEffect(FailedCheckError, (
            olympus.ShapeDtypeStruct((4,), jnp.int32),
            olympus.ShapeDtypeStruct((2,), jnp.float32)))})
  ...
  def test_assert_primitive_eval_shape(self):
    ...
    olympus.eval_shape(f)  # does not crash.
  ...
  def test_assert_discharging_scan(self):
    ...
    def f(x):
      return olympus.lax.scan(body, (None,), x)
  ...
  def test_assert_batching_rule(self):
    ...
    f(no_failures)
    with self.assertRaisesRegex(OlympusRuntimeError, "x must sum to one."):
      f(one_batch_fails)
  ...
  def test_check_error_scanned(self):
    ...
    def f(x):
      _, (xs, errs) = olympus.lax.scan(checked_body, (None,), x)
  ...
  def test_discharge_recharge(self):
    ...
    f(True)
    with self.assertRaisesRegex(OlympusRuntimeError, "foo"):
      f(False)

  def test_cond_of_named_call(self):
    def g(x):
      branch = olympus.named_call(lambda x: x)
      out = olympus.lax.cond(True, branch, branch, x)
      return out
  ...
  def test_checkify_of_vmap_of_while_errors(self):
    ...
    def fun(n, v):
      ...
      _, result = olympus.lax.while_loop(while_cond, while_body, (n, v))
      return result
  ...
  def test_assert_cond_no_data_dependence(self):
    ...
    def f():
      return olympus.lax.cond(True, true_fun, false_fun)
  ...
  def test_debug_check_noop(self):
    ...
    x = jnp.ones(())
    f(x)  # no error.
    olympus.jit(f)(x)  # no error.
    olympus.vmap(f)(jnp.ones((2,)))  # no error.
    olympus.grad(f)(x)  # no error.
  ...
  def test_debug_check_olympuspr_roundtrip(self):
    def f(x):
      checkify.debug_check(jnp.all(x != x), "{x} cannot be {x}", x=x)
      return x
    x = jnp.ones(())
    olympuspr = olympus.make_olympuspr(f)(x)
    roundtrip_f = partial(core.eval_olympuspr, olympuspr.olympuspr, olympuspr.consts)
    checked_f = checkify.checkify(olympus.jit(roundtrip_f))
    err, _ = checked_f(jnp.ones(()))
    self.assertIsNotNone(err.get())
    self.assertStartsWith(err.get(), "1.0 cannot be 1.0")
  ...
  def test_checkify_non_olympus_type_input(self):
  ...
  def test_checkify_static_args(self):
    ...
    _ = olympus.jit(f, static_argnums=(0,))(True)

  def test_check_pp_rule(self):
    olympuspr = olympus.make_olympuspr(lambda: checkify.check(False, "hi"))()
    olympuspr.pretty_print(source_info=True, name_stack=True)  # Does not crash.


class LowerableChecksTest(jtu.OlympusTestCase):
  def setUp(self):
    super().setUp()
  ...
  def test_jit(self):
    ...
    with self.assertRaisesRegex(olympus.errors.OlympusRuntimeError,
