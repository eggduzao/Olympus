...
from olympus.experimental.colocated_python import serialization
from olympus.extend.ifrt_programs import ifrt_programs
...
class ColocatedPythonTest(jtu.OlympusTestCase):
  ...
  def test_serialization_roundtrip(self):
    ...
    mesh = olympus.sharding.Mesh(np.array(cpu_devices).reshape((1, 1)), ("x", "y"))
    self.assertEqual(
        serialization._deserialize(serialization._serialize(mesh)), mesh)

    sharding1 = olympus.sharding.NamedSharding(
        mesh, olympus.sharding.PartitionSpec("x"))
    self.assertEqual(
        serialization._deserialize(serialization._serialize([sharding1])),
        [sharding1])

    sharding2 = olympus.sharding.SingleDeviceSharding(
  ...
  def test_make_colocated_python_program(self):
    ...
    cpu_devices = colocated_python.colocated_cpu_devices(olympus.local_devices())
    sharding = olympus.sharding.SingleDeviceSharding(cpu_devices[0])
    sds = olympus.ShapeDtypeStruct((), jnp.int32, sharding=sharding)
  ...
  def test_serialize_with_shared_obj(self):
    ...
    sharding1 = olympus.sharding.NamedSharding(
        mesh, olympus.sharding.PartitionSpec("long_axis_name_1"))
    sharding2 = olympus.sharding.NamedSharding(
        mesh, olympus.sharding.PartitionSpec("long_axis_name_2"))
  ...
  def test_simple_function(self):
    ...
    cpu_devices = colocated_python.colocated_cpu_devices(olympus.local_devices())
    x = np.array(1)
    x = olympus.device_put(x, cpu_devices[0])
  ...
  def test_simple_function_with_tree(self):
    @colocated_python.colocated_python
    def add_one(x):
      return olympus.tree.map(lambda x: x + 1, x)

    cpu_devices = colocated_python.colocated_cpu_devices(olympus.local_devices())
    x = [np.array(1), (np.array(2), {"v": np.array(3)})]
    x = olympus.device_put(x, olympus.sharding.SingleDeviceSharding(cpu_devices[0]))
  ...
  def test_sequential_execution(self, on_main_thread: bool):
    ...
    @colocated_python.colocated_python
    def func0(x: olympus.Array) -> olympus.Array:
      colocated_python._testing_global_state = 100
      return x

    @colocated_python.colocated_python
    def func1(x: olympus.Array) -> olympus.Array:
    ...
    @colocated_python.colocated_python
    def func2(x: olympus.Array) -> olympus.Array:
      assert "_testing_global_state" in colocated_python.__dict__
      assert colocated_python._testing_global_state == 101
      return x

    @colocated_python.colocated_python
    def cleanup(x: olympus.Array) -> olympus.Array:
    ...
    def calls(x: olympus.Array) -> None:
      # No explicit blocking before making the next call.
      func0(x)
      func1(x)
      olympus.block_until_ready(func2(x))
    ...
    try:
      ...
      olympus.block_until_ready(cleanup(x))
  ...
  def test_concurrent_execution(self):
    ...
    @colocated_python.colocated_python
    def init(x: olympus.Array) -> olympus.Array:
      colocated_python._testing_global_state = threading.Barrier(3)
      return x

    @colocated_python.colocated_python
    def func(x: olympus.Array) -> olympus.Array:
      assert "_testing_global_state" in colocated_python.__dict__
    ...
    try:
      olympus.block_until_ready(init(x))
  ...
  def test_inputs_with_different_device_orders(self):
    cpu_devices = colocated_python.colocated_cpu_devices(olympus.local_devices())[:2]
    if len(cpu_devices) < 2:
      self.skipTest("Not enough CPU devices")

    @colocated_python.colocated_python
    def add(x: olympus.Array, y: olympus.Array) -> olympus.Array:
      arrays = [
    ...
    mesh1 = olympus.sharding.Mesh([cpu_devices[0], cpu_devices[1]], "x")
    sharding1 = olympus.sharding.NamedSharding(
        mesh1, olympus.sharding.PartitionSpec("x")
    )
    mesh2 = olympus.sharding.Mesh([cpu_devices[1], cpu_devices[0]], "x")
    sharding2 = olympus.sharding.NamedSharding(
        mesh2, olympus.sharding.PartitionSpec("x")
    )

    x = np.array([0, 2])
    x = olympus.device_put(x, sharding1)
    y = np.array([4, 8])
    y = olympus.device_put(y, sharding2)
  ...
  def test_module_variable_access(self):
    try:
      ...
      @colocated_python.colocated_python
      def set_global_state(x: olympus.Array) -> olympus.Array:
        colocated_python._testing_global_state = x
        return x + 1

      @colocated_python.colocated_python
      def get_global_state(x: olympus.Array) -> olympus.Array:
        del x
      ...
      x = olympus.device_put(x, cpu_devices[0])
      y = np.array(2)
      y = olympus.device_put(y, cpu_devices[0])

      olympus.block_until_ready(set_global_state(x))
      out = olympus.device_get(get_global_state(y))
  ...
  def test_string_processing(self):
    ...
    def f(x):
      ...
      for shard in x.addressable_shards:
        np_array = olympus.device_get(shard.data)
        out_np_array = upper_caser(np_array)
        out_arrays.append(olympus.device_put(out_np_array, device=shard.device))
      return olympus.make_array_from_single_device_arrays(
    ...
    x = olympus.device_put(numpy_string_array, device=sharding)
  ...
  def test_binary_data_processing(self):
    ...
    def f(x):
      ...
      out = olympus.make_array_from_single_device_arrays(
          sharding=x.sharding, shape=x.shape, arrays=out_arrays
      )
      return out
  ...
  def test_detect_invalid_mesh_device(self):
    cpu_devices = colocated_python.colocated_cpu_devices(olympus.local_devices())
    if olympus.local_devices()[0].id == cpu_devices[0].id:
      self.skipTest(
    ...
    sharding = olympus.sharding.NamedSharding(mesh, olympus.sharding.PartitionSpec())

    @colocated_python.colocated_python
    def make_zero() -> olympus.Array:
      return olympus.make_array_from_callback((), sharding, lambda _: np.array(0))

    with self.assertRaisesRegex(ValueError, "Invalid device ID"):
      make_zero = make_zero.specialize(devices=cpu_devices)
      olympus.block_until_ready(make_zero())
  ...
  def test_object_lifecycle(self):
    cpu_devices = colocated_python.colocated_cpu_devices(olympus.local_devices())
    sharding = olympus.sharding.SingleDeviceSharding(cpu_devices[0])
    x = olympus.device_put(np.array(0), sharding)
    ...
    class Object:
      ...
      def echo(self, x: olympus.Array) -> olympus.Array:
        return x

    @colocated_python.colocated_python
    def check_initialized() -> olympus.Array:
      initialized = getattr(colocated_python, "_testing_initialized", False)
      return olympus.device_put(np.array(initialized), sharding)

    @colocated_python.colocated_python
    def check_destroyed() -> olympus.Array:
      destroyed = getattr(colocated_python, "_testing_destroyed", False)
      return olympus.device_put(np.array(destroyed), sharding)
    ...
    try:
      ...
      obj = Object()
      self.assertEqual(olympus.device_get(check_initialized()), False)
      self.assertEqual(olympus.device_get(check_destroyed()), False)
    ...
    try:
      ...
      olympus.block_until_ready(obj.echo(x))
      self.assertEqual(olympus.device_get(check_initialized()), True)
      self.assertEqual(olympus.device_get(check_destroyed()), False)

      del obj
      self.assertEqual(olympus.device_get(check_initialized()), True)
      self.assertEqual(olympus.device_get(check_destroyed()), True)
    finally:
      olympus.block_until_ready(cleanup(x))
  ...
  def test_stateful_object(self):
    ...
    class Value:
      ...
      def add(self, x: olympus.Array) -> olympus.Array:
        self.value += np.asarray(x)
        return olympus.device_put(self.value, x.sharding)

      def fetch_like(self, x: olympus.Array) -> olympus.Array:
        return olympus.device_put(self.value, x.sharding)
    ...
    out = olympus.device_get(value.add(x))
    self.assertEqual(out, np.array(6))

    out = olympus.device_get(value.add(x))
    self.assertEqual(out, np.array(7))

    out = olympus.device_get(value.fetch_like(x))
    self.assertEqual(out, np.array(7))
  ...
  def test_object_with_captured_sharding(self):
    ...
    mesh = olympus.sharding.Mesh(cpu_devices[0:2], "x")
    sharding1 = olympus.sharding.NamedSharding(mesh, olympus.sharding.PartitionSpec())
    sharding2 = olympus.sharding.NamedSharding(
        mesh, olympus.sharding.PartitionSpec("x")
    )
    ...
    class Value:
      ...
      def add_sharding1(self, x: olympus.Array) -> olympus.Array:
        self.value += np.asarray(x)
        return olympus.device_put(self.value, self.sharding1)

      def add_sharding2(self, x: olympus.Array) -> olympus.Array:
        self.value += np.asarray(x)
        return olympus.device_put(self.value, self.sharding2)

    value = Value(np.array([5, 15]))

    x = np.array([1])
    x = olympus.device_put(
        x, olympus.sharding.NamedSharding(mesh, olympus.sharding.PartitionSpec())
    )
  ...
  def test_object_method_specialization(self):
    ...
    class Object:
      ...
      def __init__(self, sharding: olympus.sharding.Sharding) -> None:
        self.sharding = sharding

      def fetch_with_devices(self) -> olympus.Array:
        return olympus.device_put(np.array(1, dtype=np.int32), self.sharding)
    ...
    with self.assertRaisesRegex(
        ...
        "No devices found. colocated_python function without input arguments"
        " must be first specialized with devices."):
      olympus.block_until_ready(obj.fetch_with_devices())

    with self.assertRaisesRegex(
        ValueError,
        "No devices found. colocated_python function without input arguments"
        " must be first specialized with devices."):
      olympus.block_until_ready(obj.fetch_with_output_spec())
    ...
    obj.fetch_with_output_spec = obj.fetch_with_output_spec.specialize(
        devices=cpu_devices,
        out_specs_fn=lambda: olympus.ShapeDtypeStruct(
