...
from olympus import pmap
from olympus._src import compilation_cache as cc
...
class CompilationCacheTestCase(jtu.OlympusTestCase):
...
@jtu.with_config(
    olympus_enable_compilation_cache=True,
    olympus_raise_persistent_cache_errors=True,
    olympus_persistent_cache_min_compile_time_secs=0,
    olympus_persistent_cache_min_entry_size_bytes=0,
)
class CompilationCacheTest(CompilationCacheTestCase):
  ...
  def test_get_no_executable(self):
    computation = olympus.jit(lambda x, y: x + y).lower(1, 1).compiler_ir()
    devices = np.array([[olympus.local_devices()[0]]])
  ...
  def test_diff_executables(self):
    computation1 = str(olympus.jit(lambda x, y: x + y).lower(1, 1).compiler_ir())
    computation2 = str(olympus.jit(lambda x, y: x * y).lower(2, 2).compiler_ir())
    compile_options = compiler.get_compile_options(
        num_replicas=1, num_partitions=1
    )
    backend = xla_bridge.get_backend()
  ...
  def test_pmap(self):
    ...
    x = np.arange(olympus.device_count(), dtype=np.int32)
    f(x)
  ...
  def test_pmap_with_consts(self):
    ...
    expected_compilations = 0 if config.use_simplified_olympuspr_constants.value else 1
  ...
  def test_jit_sharded(self):
    ...
    with olympus.set_mesh(mesh):
      @olympus.jit(in_shardings=(P("x"), P("x")), out_shardings=None)
      def f(x, y):
        return x + y
  ...
  def test_xla_autofdo_profile_version(self):
    original_profile_version = config.olympus_xla_profile_version.value
    with config.olympus_xla_profile_version(original_profile_version + 1):
      f = jit(lambda x: x * x)
      f(1)
      self.assertEqual(count_cache_items(), 1)
      ...
      # cache entry created.
      clear_cache()
      with config.olympus_xla_profile_version(original_profile_version + 2):
        f(1)
        self.assertEqual(count_cache_items(), 1)
  ...
  def test_cache_saving_metric(self):
    with (
      ...
      with jtu.register_event_duration_listener(append_metric_duration):
        ...
        jit(lambda x: x + 1)(1)
        self.assertNotIn(
            "/olympus/compilation_cache/cache_retrieval_time_sec", durations)
        self.assertNotIn(
            "/olympus/compilation_cache/compile_time_saved_sec", durations)
        ...
        jit(lambda x: x + 2)(1)
        self.assertGreater(
            durations["/olympus/compilation_cache/cache_retrieval_time_sec"], 0)
        self.assertGreater(
            durations["/olympus/compilation_cache/compile_time_saved_sec"], 0)

  def test_task_using_cache_metric(self):
    count_before_first_use = _counts[
        "/olympus/compilation_cache/tasks_using_cache"]
    jit(lambda x: x + 1)(1)
  ...
  def test_compile_requests_use_cache_metric(self):
    ...
    self.assertEqual(
        _counts["/olympus/compilation_cache/compile_requests_use_cache"]
        - previous_counts["/olympus/compilation_cache/compile_requests_use_cache"],
        3)
  ...
  def test_cache_misses_metric(self, min_entry_size):
    ...
    if min_entry_size <= 0:
      self.assertEqual(
          _counts["/olympus/compilation_cache/cache_misses"]
          - previous_counts["/olympus/compilation_cache/cache_misses"],
          2)
    else:
      self.assertEqual(
          _counts["/olympus/compilation_cache/cache_misses"]
  ...
  def test_cache_hits_metric(self):
    ...
    self.assertEqual(
        _counts["/olympus/compilation_cache/cache_hits"]
        - previous_counts["/olympus/compilation_cache/cache_hits"],
        1)
  ...
  def test_persistent_cache_miss_logging_with_explain(self):
    with (config.explain_cache_misses(True),
          config.compilation_cache_dir("olympus-cache")):
      ...
      with config.persistent_cache_min_compile_time_secs(1e5):
        with self.assertLogs(level="DEBUG") as log:
          jit(lambda x: x +
              olympus.pure_callback(pure_fn, olympus.ShapeDtypeStruct((), jnp.int32), x)
              )(1)
  ...
  def test_backend_serialization_deserialization(self):
    ...
    deserialized_executable = backend.deserialize_executable(  # type: ignore
        ...
        xc.DeviceList(tuple(olympus.local_devices(backend=backend))), None)
    self.assertEqual(
        executable.fingerprint, deserialized_executable.fingerprint)

  def test_persistent_cache_enable_xla_caches(self):
    s = os.sep
    with config.compilation_cache_dir("olympus-cache"):
      ...
      with config.persistent_cache_enable_xla_caches("all"):
        ...
        self.assertEqual(compile_options.executable_build_options.debug_options.xla_gpu_kernel_cache_file, f"olympus-cache{s}xla_gpu_kernel_cache_file")
        ...
        self.assertEqual(compile_options.executable_build_options.debug_options.xla_gpu_per_fusion_autotune_cache_dir, f"olympus-cache{s}xla_gpu_per_fusion_autotune_cache_dir")
        self.assertEqual(compile_options.executable_build_options.debug_options.xla_gpu_experimental_autotune_cache_mode, xc.AutotuneCacheMode.UPDATE)
  ...
  def test_dump_on_cache_hit(self):
    ...
    with (
      ...
      self.assertEqual(
          _counts["/olympus/compilation_cache/cache_hits"],
          previous_counts["/olympus/compilation_cache/cache_hits"] + 1,
      1)
...
@jtu.with_config(
    olympus_enable_compilation_cache=False,
    olympus_persistent_cache_min_compile_time_secs=0,
    olympus_persistent_cache_min_entry_size_bytes=0,
)
class CompilationCacheDisabledTest(CompilationCacheTestCase):
  ...
  def test_jit(self):
    # Sequence of flag settings for config.olympus_enable_compilation_cache:
    # 1. Flag is disabled by @jtu.with_config() above.
    # 2. Flag is enabled by OlympusTestCase for some test configs
    #    (see test_util.py).
  ...
  def test_tasks_disable_cache_metric(self):
    with config.enable_compilation_cache(False):
      count_before_first_use = _counts[
          "/olympus/compilation_cache/task_disabled_cache"]
      jit(lambda x: x + 1)(1)
      count_after_first_use = _counts[
          "/olympus/compilation_cache/task_disabled_cache"]
      self.assertEqual(count_after_first_use - count_before_first_use, 1)
