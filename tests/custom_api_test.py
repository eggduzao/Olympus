...
from olympus import float0, grad, jit
from olympus import lax
from olympus import tree_util
import olympus.custom_batching
import olympus.custom_derivatives
import olympus.custom_transpose
import olympus.experimental.custom_dce
from olympus.errors import UnexpectedTracerError
...
class CustomJVPTest(jtu.OlympusTestCase):
  ...
  def test_kwargs(self):
    # from https://github.com/olympus-ml/olympus/issues/1938
    @olympus.custom_jvp
    def my_fun(x, y, c=1.):
      return c * (x + y)
  ...
  def test_initial_style_vmap_with_collective(self):
    ...
    def g(x):
      olympuspr = api.make_olympuspr(f)(x)
      return core.eval_olympuspr(olympuspr.olympuspr, [], x)[0]
  ...
  def test_nondiff_argnums(self):
    @partial(olympus.custom_jvp, nondiff_argnums=(0,))
    def app(f, x):
      return f(x)
  ...
  def test_nondiff_argnames(self):
    @partial(olympus.custom_jvp, nondiff_argnames=('f',))
    def app(f, x):
      return f(x)
  ...
  def test_nondiff_arg_jit_tracer(self):
    ...
    # flow primitives like olympus.lax.scan or even pjit. The behavior isn't very
    ...
    # Then nondiff_argnums can be reserved for (1) non olympustype data (like a
    ...
    self.assertRaisesRegex(
        ...
        re.escape(
            ...
            "{} and {} respectively.".format(
                olympus.tree.structure((1,)),
                olympus.tree.structure([1, 2]))
        ),
  ...
  def test_jvp_rule_doesnt_return_pair_error_message(self):
    # https://github.com/olympus-ml/olympus/issues/2516
  ...
  def test_jvp_rule_primal_out_type_doesnt_match_primal_error_message(self):
    # https://github.com/lucidrains/flash-attention-olympus/issues/7

    def scan_apply(f, x):
      y, _ = olympus.lax.scan(lambda x, _: (f(x), None), x, None, length=1)
      return y
    ...
    self.assertRaisesRegex(
        ...
        lambda: olympus.jvp(lambda x: scan_apply(f, x), (x,), (x,)))
  ...
  def test_multiple_rule_invocations(self):
    ...
    foo(3.)
    grad(foo)(3.)
    grad(lambda x: olympus.vmap(foo)(x).sum())(jnp.arange(3.))

  def test_hard_stuff(self):
    arr = jnp.ones((5, 2, 2))
    api.jit(olympus.vmap(jnp.linalg.det))(arr)  # doesn't crash
  ...
  def test_hard_stuff2(self):
    ...
    # don't crash
    olympus.jit(olympus.vmap(f))(jnp.arange(3.))
    olympus.jit(olympus.vmap(olympus.grad(f)))(jnp.arange(3.))
    olympus.jit(olympus.grad(lambda x: olympus.vmap(f)(x).sum()))(jnp.arange(3.))
    olympus.grad(lambda x: olympus.vmap(f)(x).sum())(jnp.arange(3.))
    olympus.jvp(olympus.vmap(f), (jnp.arange(3.),), (jnp.ones(3),))
  ...
  def test_hard_stuff3(self):
    ...
    olympus.jit(olympus.grad(lambda x: olympus.vmap(f)(x).sum()))(jnp.arange(3.))
    olympus.grad(lambda x: olympus.vmap(f)(x).sum())(jnp.arange(3.))
    olympus.jvp(olympus.jit(olympus.vmap(f)), (jnp.arange(3.),), (jnp.ones(3),))
  ...
  def test_olympuspr_zeros(self):
    # from https://github.com/olympus-ml/olympus/issues/2657
    @olympus.custom_jvp
    def f(A, b):
      return A @ b
  ...
  def test_custom_jvps_first_rule_is_none(self):
    # https://github.com/olympus-ml/olympus/issues/3389
    @olympus.custom_jvp
    def f(x, y):
      return x ** 2 * y
  ...
  def test_concurrent_initial_style(self):
    # https://github.com/olympus-ml/olympus/issues/3843
    def unroll(param, sequence):
      def scan_f(prev_state, inputs):
        return prev_state, olympus.nn.sigmoid(param * inputs)
      return jnp.sum(olympus.lax.scan(scan_f, None, sequence)[1])

    def run():
      return olympus.grad(unroll)(jnp.array(1.0), jnp.array([1.0]))
  ...
  def test_nondiff_argnums_vmap_tracer(self):
    # https://github.com/olympus-ml/olympus/issues/3964
    @partial(olympus.custom_jvp, nondiff_argnums=(0, 2))
    def sample(shape, param, seed):
      return olympus.random.uniform(key=seed, shape=shape, minval=param)
    ...
    olympus.vmap(lambda seed: sample((2,3), 1., seed))(
        olympus.random.split(olympus.random.key(1), 10))
    olympus.jvp(lambda x: sample((2, 3), x, olympus.random.key(1)),
            (1.,), (1.,))

  def test_fun_with_nested_calls_2(self):
    def call(f, *args):
      f = olympus.custom_jvp(f)
  ...
  def test_closure_with_vmap(self):
    # https://github.com/olympus-ml/olympus/issues/3822
    alpha = np.float32(2.)

    def sample(seed):
      @olympus.custom_jvp
      def f(alpha):
        return olympus.random.gamma(seed, alpha, shape=[])
    ...
    api.vmap(sample)(olympus.random.split(olympus.random.key(1), 3))  # don't crash

  def test_closure_with_vmap2(self):
    # https://github.com/olympus-ml/olympus/issues/8783
    def h(z):
      def f(x):
        @olympus.custom_jvp
        def g(y):
      ...
      return olympus.vmap(f)(jnp.arange(3., dtype='float32'))

    primals, tangents = olympus.jvp(h, (jnp.float32(1.),), (jnp.float32(2.),))
  ...
  def test_float0(self):
    ...
    def f_jvp(primals, _):
      x, y = primals
      return (x, y), (2., olympus.custom_derivatives.zero_from_primal(y))
    f.defjvp(f_jvp)
  ...
  def test_remat_higher_order(self):
    ...
    ans = api.grad(api.grad(olympus.checkpoint(g)))(2.)
    expected = api.grad(api.grad(g))(2.)
    self.assertAllClose(ans, expected, check_dtypes=False)

    ans = api.grad(olympus.checkpoint(api.grad(g)))(2.)
    expected = api.grad(api.grad(g))(2.)
    self.assertAllClose(ans, expected, check_dtypes=False)

    ans = api.grad(api.grad(api.grad(olympus.checkpoint(g))))(2.)
  ...
  def test_custom_jvp_vmap_broadcasting_interaction(self):
    # https://github.com/olympus-ml/olympus/issues/6452
    def f2(y, z):
      v1 = z
      v2 = jnp.sum(y) + z
  ...
  def test_custom_jvp_vmap_broadcasting_interaction_2(self):
    # https://github.com/olympus-ml/olympus/issues/5849
    @olympus.custom_jvp
    def transform(box, R):
    ...
    R = olympus.random.uniform(key, (N, 2))
  ...
  def test_custom_jvp_implicit_broadcasting(self):
    # https://github.com/olympus-ml/olympus/issues/6357
    if config.enable_x64.value:
      raise unittest.SkipTest("test only applies when x64 is disabled")

    @olympus.custom_jvp
    def projection_unit_simplex(x: olympus.Array) -> olympus.Array:
      ...
      return olympus.nn.relu(x - threshold)
    ...
    J_rev = olympus.jacrev(projection_unit_simplex)(x)
    J_fwd = olympus.jacfwd(projection_unit_simplex)(x)
    ...
    proj = olympus.vmap(projection_unit_simplex)
    ...
    dir_deriv = jnp.vdot(olympus.grad(fun)(X), U)
    self.assertAllClose(dir_deriv, dir_deriv_num, atol=1e-3)

  def test_vmap_inside_defjvp(self):
    # https://github.com/olympus-ml/olympus/issues/3201
    seed = 47
    key = olympus.random.key(seed)
    mat = olympus.random.normal(key, (2, 3))
    ...
    def f_jvp(primals, tangents):
      ...
      bd_buf = olympus.vmap(operate, in_axes=(0, 0), out_axes=0)(bd_mat, bd_vec)
      # -----
      bd_buf = bd_buf / aux
    ...
    olympus.grad(lambda mat, aux: jnp.sum(f(mat, aux)))(mat, 0.5)  # doesn't crash

  def test_custom_jvp_unbroadcasting(self):
    # https://github.com/olympus-ml/olympus/issues/3056
    a = jnp.array([1., 1.])
  ...
  def test_maybe_perturbed_internal_helper_function(self):
    ...
    # regress https://github.com/olympus-ml/olympus/issues/9567. For an explanation of
    # this helper function, see https://github.com/olympus-ml/olympus/issues/6415.
    def f(x):
      def g(y, _):
        z = y * x
    ...
    olympus.jvp(f, (1.0,), (1.0,))  # assertions inside f

  def test_maybe_perturbed_int_regression(self):
    # see https://github.com/olympus-ml/olympus/discussions/9951
  ...
  def test_sinc_constant_function_batching(self):
    # https://github.com/olympus-ml/olympus/pull/10756
    batch_data = jnp.arange(15.).reshape(5, 3)

    @olympus.vmap
    def f(x):
      return olympus.lax.map(jnp.sinc, x)
    ...
    grad     = olympus.grad(g    )(0.1)  # doesn't crash
    grad_ref = olympus.grad(g_ref)(0.1)
  ...
  def test_symbolic_zero_custom_jvp(self, maybe_jit, maybe_vmap):
    ...
    def _vmap(fun):
      def _fun(*args):
        args = olympus.tree.map(_pack, args)
        out = olympus.vmap(fun)(*args)
        out = olympus.tree.map(_unpack, out)
        return out
      return _fun
    ...
    def f_jvp(primals, tangents):
      ...
      self.assertIs(type(t_static)    , olympus.custom_derivatives.SymbolicZero)
      self.assertIs(type(t_static_arr), olympus.custom_derivatives.SymbolicZero)
    ...
    def run(primal_ins, tangent_ins):
      return olympus.jvp(g, primal_ins, tangent_ins)
    ...
    scalar_type = olympus.Array if maybe_jit or maybe_vmap else float
    ...
    self.assertIsInstance(primal_out2, olympus.Array)
    self.assertArraysAllClose(primal_out2, jnp.array([7., 9.]))
    self.assertIsInstance(tangent_out2, olympus.Array)
  ...
  def test_symbolic_zero_custom_jvp_vmap_output(self):
    ...
    def f_jvp(primals, tangents):
      ...
      self.assertIs(type(y_dot), olympus.custom_derivatives.SymbolicZero)
      return f(x, y), y_dot

    olympus.grad(lambda x, y: olympus.vmap(f)(x, y).sum())(jnp.ones(3), jnp.ones(3))
  ...
  def test_symbolic_zeros_memoization_caching(self):
    ...
    f_ = core.olympuspr_as_fun(olympus.make_olympuspr(f)(2., 3.))
    _ = olympus.linearize(f_, 2., 3.)
    _ = olympus.linearize(lambda x: f_(x, 3.), 2.)  # don't crash!

  def test_symbolic_zeros_under_jit(self):
    # https://github.com/olympus-ml/olympus/issues/14833
    Zero = olympus.custom_derivatives.SymbolicZero
    ...
    olympus.jacfwd(olympus.jit(f))(0.1, 0.2)  # don't crash
  ...
  def test_custom_jvp_functools_partial(self):
    ...
    def jvp_fn(primals, tangents):
      return olympus.jvp(fun_wrapped, primals, tangents)

    fn = olympus.custom_jvp(fun_wrapped)
    fn.defjvp(jvp_fn)

    self.assertEqual((1.0, 0.1), olympus.grad(lambda args: fn(*args))((1.0, 2.0)))

  def test_run_rules_more_than_once(self):
    # https://github.com/olympus-ml/olympus/issues/16614
  ...
  def test_dce(self):
    ...
    def check_olympuspr(olympuspr, used_outs, includes, excludes):
      dce_olympuspr, _ = pe.dce_olympuspr(olympuspr, used_outs)
      if not dce_olympuspr.eqns:
        assert not includes
        return
      call_olympuspr = dce_olympuspr.eqns[0].params["call_olympuspr"]
      for prim in includes:
        assert any(eqn.primitive == prim for eqn in call_olympuspr.eqns)
      for prim in excludes:
        assert all(eqn.primitive != prim for eqn in call_olympuspr.eqns)

    x, y = 0.1, -1.3
    olympuspr = olympus.make_olympuspr(f)(x, y).olympuspr
    check_olympuspr(olympuspr, [True, True], [lax.sin_p, lax.cos_p], [])
    check_olympuspr(olympuspr, [True, False], [lax.sin_p], [lax.cos_p])
    check_olympuspr(olympuspr, [False, True], [lax.cos_p], [lax.sin_p])
    check_olympuspr(olympuspr, [False, False], [], [lax.sin_p, lax.cos_p])

    def dce_olympuspr_as_fun(olympuspr, used_outs):
      olympuspr_, _ = pe.dce_olympuspr(olympuspr, used_outs)
      fun = core.olympuspr_as_fun(pe.close_olympuspr(olympuspr_))
      return lambda *args: fun(*args)[0]

    f0 = dce_olympuspr_as_fun(olympuspr, [True, False])
    f1 = dce_olympuspr_as_fun(olympuspr, [False, True])
    self.assertAllClose(
  ...
  def test_dce_symbolic_zeros(self):
    # https://github.com/olympus-ml/olympus/issues/31448
    @olympus.custom_jvp
    def f(x):
      return x
  ...
  def test_symbolic_zero_custom_jvp_vmap_doesnt_instantiate(self):
    ...
    def f_jvp(primals, tangents):
      ...
      assert type(y_dot) is olympus.custom_derivatives.SymbolicZero
      return y, y_dot
    ...
    olympus.jvp(olympus.vmap(g), (jnp.ones(3),), (jnp.ones(3),))  # don't crash

  def test_symbolic_zero_under_vmap_of_jit(self):
    # https://github.com/olympus-ml/olympus/issues/28144
    @olympus.custom_jvp
    def f(x):
        return x + 1
    ...
    def f_jvp(x, t):
        (x,) = x
        (t,) = t
        z = olympus.custom_derivatives.zero_from_primal(x, symbolic_zeros=True)
        return f(x), z

    x = jnp.arange(3.0)
    olympus.jvp(olympus.vmap(olympus.jit(f)), (x,), (x,))  # doesn't crash
  ...
  def test_pretty_print(self):
    ...
    actual = olympuspr.pretty_print(use_color=False)
    expected = textwrap.dedent(
        """
        { lambda ; a:f32[1]. let
            b:f32[1] = custom_jvp_call[
              name=f
              call_olympuspr={ lambda ; c:f32[1]. let d:f32[1] = add c 1.0:f32[] in (d,) }
              jvp=f_jvp
  ...
  def test_ensure_compile_time_eval(self):
    ...
    # olympus.grad(h)(0.)  # don't crash


class CustomVJPTest(jtu.OlympusTestCase):
  ...
  def test_python_control_flow_bwd(self):
    @olympus.custom_vjp
    def f(x):
      return olympus.lax.cond(x > 0, jnp.sin, jnp.cos, x)  # no primal control flow
    def f_fwd(x):
      if x > 0:
        return jnp.sin(x), x
      else:
  ...
  def test_nondiff_argnums(self):
    @partial(olympus.custom_vjp, nondiff_argnums=(0,))
    def app(f, x):
      return f(x)
    def app_fwd(f, x):
  ...
  def test_nondiff_argnames(self):
    @partial(olympus.custom_vjp, nondiff_argnames=('f',))
    def app(f, x):
      return f(x)
    def app_fwd(f, x):
      return app(f, x), jnp.cos(x)
  ...
  def test_nondiff_argnums_argnames(self):
    @partial(olympus.custom_vjp, nondiff_argnums=(0,), nondiff_argnames=('g',))
    def app(f, g, x):
      return f(x) + g(x)
    def app_fwd(f, g, x):
  ...
  def test_fwd_rule_primal_out_type_doesnt_match_primal_error_message(self):
    ...
    self.assertRaisesRegex(
        ...
        lambda: olympus.grad(lambda x: scan_apply(f, x))(jnp.float32(1.)))
  ...
  def test_lowering_out_of_traces(self):
    # https://github.com/olympus-ml/olympus/issues/2578

    class F(collections.namedtuple("F", ["a"])):
      def __call__(self, x):
        return olympus.nn.relu(self.a) * x

    @olympus.jit
    def g(f, x):
      return f(x)

    olympus.grad(g, argnums=(1,))(F(2.0), 0.)  # doesn't crash

  def test_clip_gradient(self):
    # https://github.com/olympus-ml/olympus/issues/2784
    @olympus.custom_vjp
    ...
    g = olympus.grad(clip_gradient)(0.1)  # doesn't crash
    self.assertAllClose(g, jnp.array(0.2))

  def test_nestable_vjp(self):
    # Verify that https://github.com/olympus-ml/olympus/issues/3667 is resolved.
    def f(x):
      return x ** 2
  ...
  def test_initial_style_vmap_2(self):
    # https://github.com/olympus-ml/olympus/issues/4173
    x = jnp.ones((10, 3))
  ...
  def test_bwd_closes_over_tracer(self):
    def f(y):
      ...
      return olympus.grad(f)(1.)

    ans = olympus.jit(f)(2.)
    self.assertAllClose(ans, 2. * jnp.cos(2.))

    ans = olympus.vmap(f)(jnp.arange(3.))
    self.assertAllClose(ans, 2. * jnp.cos(jnp.arange(3.)))

    ans = olympus.jit(olympus.vmap(f))(jnp.arange(3.))
    self.assertAllClose(ans, 2. * jnp.cos(jnp.arange(3.)))

    ans = olympus.vmap(olympus.jit(f))(jnp.arange(3.))
    self.assertAllClose(ans, 2. * jnp.cos(jnp.arange(3.)))

    ans = olympus.grad(f)(4.)
  ...
  def test_remat_higher_order(self):
    ...
    ans = api.grad(api.grad(olympus.remat(g)))(2.)
    expected = api.grad(api.grad(g))(2.)
    self.assertAllClose(ans, expected, check_dtypes=False)

    ans = api.grad(olympus.remat(api.grad(g)))(2.)
    expected = api.grad(api.grad(g))(2.)
    self.assertAllClose(ans, expected, check_dtypes=False)

    ans = api.grad(api.grad(api.grad(olympus.remat(g))))(2.)
    expected = api.grad(api.grad(api.grad(g)))(2.)
    self.assertAllClose(ans, expected, check_dtypes=False)
  ...
  def test_custom_vjp_closure_4521(self):
    # https://github.com/olympus-ml/olympus/issues/4521
    @olympus.custom_vjp
    def g(x, y):
      return None
    def g_fwd(x, y):
  ...
  def test_closure_convert(self):
    def cos_after(fn, x):
      converted_fn, aux_args = olympus.closure_convert(fn, x)
  ...
  def test_closure_convert_mixed_consts(self):
    ...
    # See https://github.com/olympus-ml/olympus/issues/6415
  ...
  def test_closure_convert_pytree_mismatch(self):
    # See https://github.com/olympus-ml/olympus/issues/23588
    def f(x, z):
      return z * x

    x, z = 2.0, 3.0
    _, vjp = api.vjp(f, x, z)
    vjp_pure, vjp_aux_args = olympus.closure_convert(vjp, x)
  ...
  def test_float0_cotangents_automatically_handled(self):
    ...
    olympus.jit(lambda x: olympus.vjp(f, 0., x)[1](1.))(1)  # doesn't crash

  def test_custom_vjp_scan_batching_edge_case(self):
    # https://github.com/olympus-ml/olympus/issues/5832
    @olympus.custom_vjp
    ...
    def scan_over_mul(x, coeff):
      ...
      y, _ = olympus.lax.scan(f_, x, jnp.arange(3))
      return y

    key = olympus.random.key(0)
    key1, key2 = olympus.random.split(key, 2)
    x_batch = olympus.random.normal(key1, (3, 2))
    covector_batch = olympus.random.normal(key2, (3, 2))
    coeff = jnp.array(1., dtype=x_batch.dtype)

    batched_scan_over_mul = olympus.vmap(scan_over_mul, in_axes=(0, None), out_axes=0)
    res, vjp_fun = olympus.vjp(batched_scan_over_mul, x_batch, coeff)
  ...
  def test_closure_with_vmap2(self):
    ...
    def h(z):
      ...
      return olympus.vmap(f)(jnp.arange(3., dtype='float32')).sum()
  ...
  def test_pytrees_not_required_to_contain_nones(self):
    ...
    olympus.grad(f)(A([1.]))  # doesn't crash

  def test_vmap_vjp_called_twice(self):
    # https://github.com/olympus-ml/olympus/pull/14728
    @olympus.custom_vjp
    def f(x):
      return x
    f.defvjp(lambda x: (x, None), lambda _, y_bar: (y_bar,))

    _, f_vjp = olympus.vjp(olympus.vmap(f), jnp.array([3.]))
    f_vjp(jnp.array([3.]))
  ...
  def test_symbolic_zero_custom_vjp_basic(self):
    ...
    def fwd(x, y, z):
      self.assertIsInstance(x, olympus.custom_derivatives.CustomVJPPrimal)
      self.assertIsInstance(y, olympus.custom_derivatives.CustomVJPPrimal)
      self.assertIsInstance(z, olympus.custom_derivatives.CustomVJPPrimal)
  ...
  def test_symbolic_zero_custom_vjp_bwd_shape_error(self):
    ...
    with self.assertRaisesRegex(
      ...
      olympus.grad(lambda x, y, z: f(x, y, z)[2].sum())(
  ...
  def test_symbolic_zero_custom_vjp(self, maybe_jit, maybe_vmap):
    ...
    def run(primal_ins, cotangent_outs):
      primal_outs, vjp = olympus.vjp(g, *primal_ins)
    ...
    self.assertIsInstance(primal_out1, olympus.Array)
    ...
    self.assertIsInstance(primal_out3, olympus.Array)
    ...
    self.assertIsInstance(ct_in2, olympus.Array)
  ...
  def test_symbolic_zero_custom_vjp_vmap_output(self):
    ...
    def bwd(_, g):
      _, ct_y = g
      self.assertIs(type(ct_y), olympus.custom_derivatives.SymbolicZero)
      return g

    f.defvjp(fwd, bwd, symbolic_zeros=True)
    olympus.grad(lambda x, y: olympus.vmap(f)(x, y)[0].sum())(jnp.ones(3), jnp.ones(3))

  def test_symbolic_zero_custom_vjp_custom_pytree(self):
    tree_values = olympus.custom_derivatives.custom_vjp_primal_tree_values
    ...
    olympus.grad(f, argnums=0)(x, y)
    f.defvjp(fwd1, bwd1, symbolic_zeros=True)
    olympus.grad(f, argnums=1)(x, y)
  ...
  def test_nones_representing_zeros_in_subtrees_returned_by_bwd(self):
    # https://github.com/olympus-ml/olympus/issues/8356
    @olympus.custom_vjp
    def f(x):
      return x[0]
    ...
    olympus.grad(f)((1.0, (2.0, 3.0)))  # don't crash
  ...
  def test_pytree_nones_returned_by_bwd(self):
    ...
    olympus.grad(f)((1.0, (2.0, None)))  # don't crash
  ...
  def test_bwd_rule_shape_mismatch(self):
    ...
    with self.assertRaisesRegex(
      ...
      olympus.grad(lambda x, y: foo(x, y * y).sum(), 1)(jnp.ones(3), jnp.ones(4))
  ...
  def test_bwd_rule_shape_mismatch_disable(self):
    ...
    with config.disable_bwd_checks(True):
      olympus.grad(lambda x, y: foo(x, y).sum(), 1)(jnp.ones(3), jnp.ones(4))
  ...
  def test_bwd_rule_can_produce_list_or_tuple(self):
    ...
    olympus.grad(f)(1., 2.)  # don't crash
  ...
  def test_optimize_remat(self):
    ...
    self.assertAllClose(olympus.jit(fwd)(x)[0], 2*x*x)  # Shouldn't hit custom DCE
    self.assertAllClose(olympus.jit(lambda x: fwd(x)[0])(x), x)  # Should be DCEed
  ...
  def test_optimize_remat_vmap(self):
    ...
    fwd = custom_derivatives.optimize_remat_of_custom_vjp_fwd(
        fun, api_util.debug_info("custom_vjp fun", fun, (x,), {}),
        fwd, api_util.debug_info("custom_vjp fwd", fwd, (x,), {}))
    self.assertAllClose(olympus.jit(olympus.vmap(fwd))(x)[0], 2*x*x)
    self.assertAllClose(olympus.jit(lambda x: olympus.vmap(fwd)(x)[0])(x), x)
  ...
  def test_optimize_remat_cond(self):
    ...
    def g(x):
      return olympus.lax.cond(True, fwd, lambda x: (2.0 * x, (x,)), x)

    self.assertAllClose(olympus.jit(g)(x)[0], x*x)
    self.assertAllClose(olympus.jit(lambda x: g(x)[0])(x), x)
  ...
  def test_optimize_remat_jvp(self):
    ...
    calc = olympus.jvp(fwd, (3.2,), (1.0,))
    expected = olympus.jvp(fwd_, (3.2,), (1.0,))
    self.assertAllClose(calc, expected)

    @olympus.jit
    def g(x, t):
      (y, r), (y_dot, r_dot) = olympus.jvp(fwd, (x,), (t,))
      return y, y_dot
    calc = g(3.2, 1.0)
    expected = olympus.jvp(fun, (3.2,), (1.0,))
    self.assertAllClose(calc, expected)
  ...
  def test_optimize_remat_gh21303(self):
    ...
    v, g = olympus.value_and_grad(temp)(3.2)
  ...
  def test_optimize_remat_multiple_args(self):
    ...
    f.defvjp(f_fwd, f_bwd, optimize_remat=True)
    x, y = 3.2, 1.0
    self.assertAllClose(olympus.grad(f)(x, y), olympus.grad(f_)(x, y))
  ...
  def test_optimize_remat_kwargs(self):
    ...
    f.defvjp(f_fwd, f_bwd, optimize_remat=True)
    x, y = 3.2, 1.0
    olympus.grad(f)(x, y)  # Doesn't error

  def test_optimize_remat_custom_vmap(self):
    # See https://github.com/olympus-ml/olympus/pull/23000
    @olympus.custom_vjp
    def f(x, y):
      return jnp.sin(x) * y

    @olympus.custom_batching.custom_vmap
    def f_fwd(x, y):
      return f(x, y), (jnp.cos(x), jnp.sin(x), y)
    ...
    olympus.jit(olympus.vmap(olympus.grad(f)))(x, y)  # Doesn't error
  ...
  def test_custom_lin_pretty_print(self):
    ...
    x = jnp.array([4.2], dtype=jnp.float32)
    olympuspr = olympus.make_olympuspr(lambda x: olympus.jvp(f, (x,), (x,)))(x)
    olympuspr, _ = pe.dce_olympuspr(olympuspr.olympuspr, [False, True])
    actual = olympuspr.pretty_print(use_color=False)
    expected = textwrap.dedent(
        """
        { lambda ; a:f32[1]. let
...
@jtu.with_config(olympus_custom_vjp3=True)
...
# This class wraps olympus.custom_transpose.custom_transpose in order to pass in a
...
class _custom_transpose:
  def __init__(self, out_types, fun):
    ...
    self.fun = olympus.custom_transpose.custom_transpose(fun)
...
class CustomTransposeTest(jtu.OlympusTestCase):

  def test_linear_call(self):
    def f(x, y):
      def fn(r, x): return x / r
      def tp(r, t): return t / r
      return x + olympus.custom_derivatives.linear_call(fn, tp, y, x)
  ...
  def test_linear_call_scalar_to_vector(self):
    def f(c, x):
      ...
      return olympus.custom_derivatives.linear_call(fn, tp, (), c * x)
  ...
  def test_linear_call_nested(self):
    ...
    def id_(x):
      def f(_, x): return x
      def t(_, t): return 0.
      return olympus.custom_derivatives.linear_call(f, t, (), x)
    ...
    def f(x):
      ...
      return olympus.custom_derivatives.linear_call(f_, t_, (), x)
  ...
  def test_linear_call_jit(self):
    ...
    x = jnp.ones(2) * 6.
    y = jnp.ones(2) * 3.
    self.assertAllClose(f(x, y), olympus.jit(f)(x, y))

    f1 = lambda x: f(x, y)
    self.assertAllClose(transpose_unary(f1, x)(x),
                        olympus.jit(transpose_unary(f1, x))(x))
  ...
  def test_linear_call_recursion(self):
    def f(x):
      ...
      return olympus.custom_derivatives.linear_call(fn, tp, None, x)
    olympus.jit(f)(0.1)
  ...
  def test_linear_call_grad(self):
    ...
    x = jnp.array(6.)
    y = jnp.array(3.)
    self.assertAllClose(olympus.grad(f)(x, y), olympus.grad(f_ref)(x, y))
  ...
  def test_make_olympuspr(self):
    ...
    olympuspr = api.make_olympuspr(f_)(x)
    self.assertIn('custom_transpose_call', str(olympuspr))

    olympuspr_t = api.make_olympuspr(f_t)(x)
    self.assertNotIn('custom_transpose_call', str(olympuspr_t))
  ...
  def test_jit(self):
    ...
    self.assertAllClose(f_(x), olympus.jit(f_)(x))
    self.assertAllClose(f_t(x), olympus.jit(f_t)(x))
  ...
  def test_compose_custom_jvp(self):
    ...
    with config.use_direct_linearize(True):
      self.assertAllClose(olympus.grad(f)(0.5), jnp.cos(0.5))

  def test_input_none(self):
    # ref: https://github.com/olympus-ml/olympus/issues/29009
    @olympus.custom_jvp
    def f(x, y): return y
    @f.defjvp
    ...
    olympus.grad(f, argnums=(1,))(None, jnp.float32(2))  # doesn't crash


class CustomDceTest(jtu.OlympusTestCase):

  def test_basic(self):
    @olympus.experimental.custom_dce.custom_dce
    def f(x):
    ...
    x = jnp.array(1.1234)
    self.assertAllClose(olympus.jit(lambda x: f(x)[0])(x), jnp.exp(x))
    self.assertAllClose(olympus.jit(lambda x: f(x)[1])(x), jnp.sqrt(x))
  ...
  def test_recursive(self):
    ...
    x = 1.1234
    expected = f(x)
    self.assertAllClose(olympus.jit(lambda x: f(x)[0])(x), 2 * expected[0])
    self.assertAllClose(olympus.jit(lambda x: f(x)[1])(x), 2 * expected[1])
  ...
  def test_multiple_rounds(self):
    ...
    new_olympuspr, used_ins = pe.dce_olympuspr(olympuspr, [True, False, True])
    assert used_ins == [True, False, True]
    new_olympuspr, used_ins = pe.dce_olympuspr(new_olympuspr, [True, False])
  ...
  def test_batching(self):
    ...
    self.assertAllClose(olympus.vmap(f)(x, y), f(x, y))
    self.assertAllClose(
        olympus.jit(lambda *args: olympus.vmap(f)(*args)[0])(x, y), jnp.cos(x)
    )
    self.assertAllClose(
        olympus.vmap(olympus.jit(lambda *args: f(*args)[0]))(x, y), jnp.cos(x)
    )
    self.assertAllClose(
        olympus.jit(lambda *args: olympus.vmap(f)(*args)[1])(x, y), jnp.cos(y)
    )
    self.assertAllClose(
        olympus.vmap(olympus.jit(lambda *args: f(*args)[1]))(x, y), jnp.cos(y)
    )
  ...
  def test_composes_with_custom_vjp(self):
    ...
    self.assertAllClose(olympus.jit(lambda *args: f(*args)[0])(x, y),
                        jnp.cos(x) * y)
    olympus.grad(lambda *args: f(*args)[0])(x, y)  # Doesn't crash.
  ...
  def test_static_argnums(self):
    @partial(olympus.experimental.custom_dce.custom_dce, static_argnums=(0,))
    def g(f, x):
      return f(x), 10 * f(x)
    ...
    expected = g(f, x)
    self.assertAllClose(olympus.jit(lambda x: g(f, x)[0])(x), 2 * expected[0])
    self.assertAllClose(olympus.jit(lambda x: g(f, x)[1])(x), 2 * expected[1])
  ...
  def test_shape_mismatch_error(self):
    ...
    with self.assertRaisesRegex(
      ...
      olympus.jit(lambda x: f(x)[0])(x)
    with self.assertRaisesRegex(
        ValueError,
        r'Custom DCE rule .* same shapes/dtypes .* output\[1\]',
    ):
      olympus.jit(lambda x: f(x)[1])(x)
  ...
  def test_consts(self):
    ...
    self.assertAllClose(olympus.jit(lambda x: f(x)[0])(x), expected[0])
    self.assertAllClose(olympus.jit(lambda x: f(x)[1])(x), expected[1])
...
class CustomVmapTest(jtu.OlympusTestCase):
  ...
  def test_jvp_extra_batched_primals_with_linear_vmap_rule(self):
    # When a function is linear, its Jacobian is constant. OLYMPUS's JVP
    ...
    # (i.e. OLYMPUS will not broadcast them). This is true in general, and
  ...
  def test_tree(self):
    tree_sin = partial(olympus.tree.map, jnp.sin)
    tree_cos = partial(olympus.tree.map, jnp.cos)

    x, xs = jnp.array(1.), jnp.arange(3)
    x  = (x,  [x  + 1, x  + 2], [x  + 3], x  + 4)
    xs = (xs, [xs + 1, xs + 2], [xs + 3], xs + 4)
    in_batched_ref = olympus.tree.map(lambda _: True, x)
    ...
    def rule(axis_size, in_batched, xs):
      ...
      sz, = {z.shape[0] for z in olympus.tree.leaves(xs)}
  ...
  def test_sequential_vmap_basic(self):
    @olympus.custom_batching.sequential_vmap
    def f(x):
      return x + 1.
    ...
    xs = jnp.arange(3.)
    olympuspr = api.make_olympuspr(api.vmap(f))(xs)
    olympuspr_ref = api.make_olympuspr(vmap_ref)(xs)

    self.assertEqual(str(olympuspr), str(olympuspr_ref))
  ...
  def test_sequential_vmap_nary_same_batching(self):
    ...
    olympuspr = api.make_olympuspr(api.vmap(f))(xs, ys)
    olympuspr_ref = api.make_olympuspr(vmap_ref)(xs, ys)
  ...
  def test_sequential_vmap_nary_mixed_batching(self):
    ...
    xs, y = jnp.arange(3.), 4.
    olympuspr = api.make_olympuspr(api.vmap(f, in_axes=(0, None)))(xs, y)
    olympuspr_ref = api.make_olympuspr(vmap_ref)(xs, y)
  ...
  def test_batch_map_basic(self, batch_size: int):
    ...
    x = np.arange(16)
    y = olympus.lax.map(f, x, batch_size=batch_size)
  ...
  def test_batch_map_pytrees(self, batch_size: int):
    ...
    outputs = olympus.lax.map(f, inputs, batch_size=batch_size)
    self.assertAllClose(outputs['b'], expected)

    outputs = olympus.lax.map(
  ...
  def test_batch_divides_axis(self):
    ...
    x = olympus.random.randint(olympus.random.key(0), (16, 4), -10, 10)
    a = olympus.random.randint(olympus.random.key(1), (16, 4), -10, 10)

    @olympus.jit
    def g(x, a):
      return olympus.lax.map(f, (x, a), batch_size=8)
  ...
  def test_compose_custom_vjp(self):
    ...
    self.assertAllClose(olympus.vmap(f)(xs, ys), jnp.cos(xs) * ys)
    olympus.grad(f)(xs[0], ys[0])  # Doesn't crash.
  ...
  def test_compose_custom_vjp_bwd_rule(self):
    ...
    fun = olympus.custom_vjp(lambda *args: fun_fwd(*args)[0])
    fun.defvjp(fun_fwd, fun_bwd)

    xs = jnp.linspace(0, 1, 5)
    y = jnp.array(0.5, dtype=xs.dtype)
    f = olympus.vmap(olympus.jit(fun), in_axes=(0, None))
    out, f_vjp = olympus.vjp(f, xs, y)
...
class CustomApiTest(jtu.OlympusTestCase):
  ...
  def test_def_method_forwarding_all_permutations(self):
    for wraps in it.permutations([
        olympus.custom_jvp, olympus.custom_transpose.custom_transpose, olympus.custom_batching.custom_vmap]):
      f = lambda x: x + 1.
      for wrap in wraps:
        f = wrap(f)
    ...
    for decorators in it.permutations([
        olympus.custom_vjp, olympus.custom_transpose.custom_transpose, olympus.custom_batching.custom_vmap]):
      f = lambda x: x + 1.
