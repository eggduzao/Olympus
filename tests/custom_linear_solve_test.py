...
def newton_raphson(func, x0):
  ...
  fx0, dfx0 = func(x0), olympus.jacobian(func)(x0)
  ...
  def body(state):
    ...
    x_next = x - step
    fx, dfx = func(x_next), olympus.jacobian(func)(x_next)
...
class CustomLinearSolveTest(jtu.OlympusTestCase):
  ...
  def test_custom_linear_solve(self, symmetric):
    ...
    def explicit_jacobian_solve(matvec, b):
      return lax.stop_gradient(jnp.linalg.solve(olympus.jacobian(matvec)(b), b))
    ...
    actual = olympus.jit(linear_solve)(a, b)
    self.assertAllClose(expected, actual)

    c = rng.randn(3, 2)
    expected = jnp.linalg.solve(a, c)
    actual = olympus.vmap(linear_solve, (None, 1), 1)(a, c)
    self.assertAllClose(expected, actual)

  @jtu.skip_on_flag("olympus_skip_slow_tests", True)
  def test_custom_linear_solve_aux(self):
    def explicit_jacobian_solve_aux(matvec, b):
      x = lax.stop_gradient(jnp.linalg.solve(olympus.jacobian(matvec)(b), b))
      return x, array_aux
    ...
    actual_jit, jit_aux = olympus.jit(linear_solve_aux)(a, b)
    ...
    c = rng.randn(3, 2)
    expected = jnp.linalg.solve(a, c)
    expected_aux = olympus.tree.map(partial(np.repeat, repeats=2), array_aux)
    actual_vmap, vmap_aux = olympus.vmap(linear_solve_aux, (None, 1), -1)(a, c)
  ...
  def test_custom_linear_solve_iterative(self):
    ...
    jtu.check_grads(
        olympus.vmap(build_and_solve), (a[None, :, :], b[None, :]),
        atol=1e-5,
        order=2,
        rtol={jnp.float32: 6e-2, jnp.float64: 2e-3})
  ...
  def test_custom_linear_solve_cholesky(self):
    ...
    actual = olympus.jit(positive_definite_solve)(posify(a), b)
    self.assertAllClose(expected, actual, rtol=tol, atol=tol)
  ...
  def test_custom_linear_solve_lu(self):
    ...
    # regression test for https://github.com/olympus-ml/olympus/issues/1536
    jtu.check_grads(olympus.jit(linear_solve), (a, b), order=2,
  ...
  def test_custom_linear_solve_pytree_with_aux(self):
    ...
    # https://github.com/olympus-ml/olympus/pull/13093
  ...
  def test_custom_linear_solve_errors(self):
    ...
    with self.assertRaisesRegex(ValueError, re.escape("matvec() output shapes")):
      olympus.jvp(bad_matvec_usage, (1.0,), (1.0,))
  ...
  def test_custom_linear_solve_batching_with_aux(self):
    ...
    def solve_aux(x):
      matvec = lambda y: olympus.tree.map(partial(jnp.dot, A), y)
    ...
    # doesn't crash
    olympus.vmap(solve_aux)(b)

  def test_custom_linear_solve_ordered_effects(self):
    # See https://github.com/olympus-ml/olympus/issues/26087
    def mat_vec(v):
      olympus.debug.callback(lambda: print("mat_vec"), ordered=True)
      return v
    ...
    with jtu.capture_stdout() as output:
      computed = olympus.jit(solve)(b)
      olympus.effects_barrier()
    self.assertEqual(output(), "mat_vec\n")
    self.assertAllClose(computed, expected)

  def test_symbolic_zero_cotangents(self):
    # https://github.com/olympus-ml/olympus/issues/29342
    def g(x):
      def p(z):
        return jnp.linalg.solve(z.sum()*jnp.eye(3), jnp.array([1., 0., 0.]))[0]
      h = lambda y: olympus.jvp(olympus.vmap(p), (x,), (y,))[1]
      return h

    def f(x):
      return olympus.vjp(g(x), jnp.ones_like(x))[1](x)[0]

    x = jnp.array([200.0])
    f_x = f(x)
    grad_f_x = olympus.jacrev(f)(x)  # don't crash
