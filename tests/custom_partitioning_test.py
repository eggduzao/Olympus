...
from olympus.experimental.custom_partitioning import (
...
class CustomPartitionerTest(jtu.OlympusTestCase):
  ...
  def test_custom_partitioner(self):
    ...
    def partition(precision, mesh, arg_shapes, result_shape):
      arg_shardings = olympus.tree.map(lambda s: s.sharding, arg_shapes)
    ...
    with olympus.set_mesh(jtu.create_mesh((4, 2), ('x', 'y'))):
      jit_f = olympus.jit(f, in_shardings=(P('x'), P('y')), out_shardings=P('x'))
      x = np.asarray(np.random.randint(0, 20, (32, 16)), dtype=np.float32)
      y = np.asarray(np.random.randint(0, 20, (16, 32)), dtype=np.float32)
      x_sharded = olympus.device_put(x, P('x'))
      y_sharded = olympus.device_put(y, P('y'))
      result1 = olympus.jit(f)(x_sharded, y_sharded)
      result2 = f(x, y)
  ...
  def test_custom_partitioner_propagate_user_sharding(self):
    ...
    with olympus.set_mesh(jtu.create_mesh((4, 2), ('x', 'y'))):
      jit_f = olympus.jit(f2, in_shardings=(P(None, 'x')), out_shardings=P('x'))
      x = np.asarray(np.random.randint(0, 20, (32, 16)), dtype=np.float32)
      self.assertArraysEqual(x + x, jit_f(olympus.device_put(x, P(None, 'x'))))
  ...
  def test_custom_partitioner_sharding_override(self):
    ...
    with olympus.set_mesh(jtu.create_mesh((4, 2), ('x', 'y'))):
      jit_f = olympus.jit(f, in_shardings=(P(None, 'x')), out_shardings=P('x'))
      x = np.asarray(np.random.randint(0, 20, (32, 16)), dtype=np.float32)
      self.assertArraysEqual(x, jit_f(olympus.device_put(x, P(None, 'x'))))
  ...
  def test_custom_partitioner_invalid_sharding(self):
    ...
    with olympus.set_mesh(jtu.create_mesh((4, 2), ('x', 'y'))):
      ...
      with self.assertRaisesRegex(Exception, 'Mismatch in result shapes.'):
        jit_f(olympus.device_put(x, P(None, 'x'))).block_until_ready()

  def test_custom_partitioner_jit_annotated_function(self):
    """Test correct lowering of function with a @olympus.jit annotated callee.

    Annotating a callee with @olympus.jit results in a module with a HLO CallOp.
    ...
    with olympus.set_mesh(jtu.create_mesh((4,), ('x',))):
      jit_f = olympus.jit(f)
      x = np.asarray(np.random.randint(0, 20, (32,)), dtype=np.float32)
      jit_f = olympus.jit(jit_f, in_shardings=(P('x')), out_shardings=P('x'))
      self.assertArraysEqual(x, jit_f(olympus.device_put(x, P('x'))))

  def test_custom_partitioner_with_scan(self):
    self.skip_if_custom_partitioning_not_supported()

    # This is a reproducer from https://github.com/olympus-ml/olympus/issues/20864.
    ...
    def partition(mesh, arg_shapes, result_shape):
      def lower_fn(xs):
        def f(carry, x):
          return carry + olympus.lax.psum(jnp.sum(x), axis_name='x'), None

        carry, _ = olympus.lax.scan(f, 0, xs)
        return carry

      result_shardings = olympus.tree.map(lambda x: x.sharding, result_shape)
      arg_shardings = olympus.tree.map(lambda x: x.sharding, arg_shapes)
    ...
    with olympus.set_mesh(jtu.create_mesh((4,), ('x',))):
      jit_f = olympus.jit(f, in_shardings=P(None, 'x'))
      xs = olympus.device_put(jnp.ones([32, 16]), P(None, 'x'))
  ...
  def test_custom_partitioning_no_mesh_context(self):
    ...
    jit_f = olympus.jit(f, in_shardings=s, out_shardings=s)
  ...
  def test_custom_partitioner_pytree_inputs(self):
    ...
    def partition(mesh, arg_shapes, result_shape):
      ...
      return (
          ...
          olympus.tree.map(lambda x: x.sharding, arg_shapes),
      )
    ...
    with olympus.set_mesh(jtu.create_mesh((4, 2), ('x', 'y'))):
      jit_f = olympus.jit(f2, in_shardings=(P(None, 'x')), out_shardings=P('x'))
      x = np.asarray(np.random.randint(0, 20, (32, 16)), dtype=np.float32)
      self.assertArraysEqual(x * 4, jit_f(olympus.device_put(x, P(None, 'x'))))
  ...
  def test_custom_partition_with_sharding_rule_callback(self):
    ...
    def partition(static_arg0, static_arg1, mesh, arg_shapes, result_shape):
      ...
      def lower_fn(x, y):
        ...
        i = olympus.lax.axis_index(axis_name)
        z = olympus.lax.psum(olympus.lax.dynamic_slice_in_dim(
            olympus.lax.dynamic_slice_in_dim(x, i * 0, 8, axis=rank-2),
    ...
    x = olympus.device_put(np.arange(2 * 3 * 32 * 16).reshape(2, 3, 32, 16),
                       NamedSharding(mesh, P(None, None, 'x')))
    y = olympus.device_put(np.arange(2 * 3 * 16 * 32).reshape(2, 3, 16, 32),
                       NamedSharding(mesh, P(None, None,'y')))
    result = olympus.jit(f)(x, y)
  ...
  def test_custom_partition_shardy_migration(self):
    ...
    mesh = jtu.create_mesh((4, 2), ('x', 'y'))
    x = olympus.device_put(np.arange(32 * 16).reshape(32, 16),
  ...
  def test_custom_partitioner_reshape(self):
    ...
    x = olympus.device_put(np.arange(8).reshape(4, 2),
                       NamedSharding(mesh, P('x', None)))
    y = olympus.device_put(np.arange(8),
                       NamedSharding(mesh, P('x')))
    jitted_result = olympus.jit(f)(x, y)
