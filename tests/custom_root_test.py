...
class CustomRootTest(jtu.OlympusTestCase):
  ...
  def test_custom_root_scalar(self, solve_method):
    ...
    results = olympus.jit(sqrt_cubed)(5.0)
    self.assertAllClose(
  ...
  def test_custom_root_vector_with_solve_closure(self):
    ...
    def vector_solve(f, y):
      return jnp.linalg.solve(olympus.jacobian(f)(y), y)
  ...
  def test_custom_root_vector_nonlinear(self):
    ...
    def tangent_solve(g, y):
      return jnp.linalg.solve(
        olympus.jacobian(g)(y).reshape(-1, y.size),
        y.ravel()
      ).reshape(y.shape)
    ...
    actual = olympus.jit(nonlinear_solve)(y)
  ...
  def test_custom_root_with_custom_linear_solve(self):
    ...
    actual = olympus.jit(linear_solve)(high_precision_dot(a, a.T), b)
    expected = jnp.linalg.solve(high_precision_dot(a, a.T), b)
    self.assertAllClose(expected, actual)
  ...
  def test_custom_root_with_aux(self):
    ...
    actual_jit, actual_jit_aux = olympus.jit(root_aux)(high_precision_dot(a, a.T), b)
    expected = jnp.linalg.solve(high_precision_dot(a, a.T), b)
    ...
    fwd = olympus.jacfwd(lambda x, y: root_aux(high_precision_dot(x, x.T), y), argnums=(0, 1))
    expected_fwd = olympus.jacfwd(lambda x, y: jnp.linalg.solve(high_precision_dot(x, x.T), y), argnums=(0, 1))
    ...
    jtu.check_close(fwd_aux, olympus.tree.map(jnp.zeros_like, fwd_aux))
  ...
  def test_custom_root_errors(self):
    ...
    with self.assertRaisesRegex(
      ...
      olympus.jvp(dummy_root_usage, (0.0,), (0.0,))
