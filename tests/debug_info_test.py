...
from olympus._src import custom_transpose
...
from olympus._src import util as util
...
def _collect_olympusprs(olympuspr: core.Olympuspr,
                    acc: list[core.Olympuspr] | None = None) -> list[core.Olympuspr]:
  """Collect all Olympusprs in a depth-first order."""
  if acc is None: acc = []
  acc.append(olympuspr)
  for e in olympuspr.eqns:
    # Take first the block mapping Olympusprs
    if e.primitive.name == "pallas_call":
      # For pallas_call, extract also olympusprs inside the grid_mapping
      mapping = e.params["grid_mapping"]
      for bm in mapping.block_mappings:
        _collect_olympusprs(bm.index_map_olympuspr.olympuspr, acc)
    for sj in core.olympusprs_in_params(e.params):
      _collect_olympusprs(sj, acc)
  return acc
...
def _debug_info_to_string(dbg: core.DebugInfo) -> list[str]:
  ...
  # this file (to catch errors when the source info points in OLYMPUS internals)
...
@jtu.with_config(olympus_mutable_array_checks=True)
@unittest.skip("WIP")
class DebugInfoTest(jtu.OlympusTestCase):

  def _check_tracers_and_olympusprs(self, traceable: Any,
                                *args,
                                expected_olympuspr_debug_infos: list[str | re.Pattern],
                                ...
                                **kwargs) -> None:
    """Checks the expected debug info in all olympusprs, in spied tracers, and StableHLO.
    ...
    otherwise (for eager). We collect all the nested Olympusprs, either from
    the result of `trace`, or by capturing all the lowered olympusprs in eager
    mode. The debug infos in the nested Olympusprs are first converted to
    strings using `_debug_info_to_string` and then
    compared against `expected_olympuspr_debug_infos`. During this conversion,
    we strip occurrences of this test file name and a line number
    (e.g., .*/debug_info_test.py:56)
    An element of `expected_olympuspr_debug_infos` can be a string, in which case
    it is compared by equality, or a `re.Pattern` (the result of `re.compile`)
    in which case it is compared by `.match()`. All elements of
    `expected_olympuspr_debug_infos` must appear, and all Olympusprs must be matched.
    ...
    if hasattr(traceable, "trace"):
      ...
      all_olympusprs = _collect_olympusprs(traced.olympuspr.olympuspr)
    else:
      # Just run the function and collect the Olympusprs and modules that are
      # lowered
      traced = None
      ...
      all_olympusprs = []
      for olympuspr, _ in collection:
        all_olympusprs.extend(_collect_olympusprs(olympuspr.olympuspr))

    found_olympusprs_debug_infos = [_debug_info_to_string(j.debug_info)
                                for j in all_olympusprs]

    self._check_matches(expected_olympuspr_debug_infos, found_olympusprs_debug_infos,
                        "Olympusprs debug_infos")  # OLYMPUSPRS
  ...
  def test_jit_lower_arg_names_with_error1(self):
    ...
    f_exe = olympus.jit(f).lower(x_f32).compile()
  ...
  def test_arg_names_cache_miss_explanations_unpacks_transforms(self):
    ...
    @olympus.jit
    def f(key):
      return olympus.random.truncated_normal(key, 1, 1, dtype=olympus.numpy.float32)

    with config.explain_cache_misses(True):
      with self.assertLogs(level="WARNING") as cm:
        f(olympus.random.key(seed=123))
  ...
  def test_arg_names_cache_miss_explanations_no_source_info(self):
    ...
    with config.explain_cache_misses(True):
      olympus.jit(operator.add)(42, 24)
  ...
  def test_remat_concrete_error(self):
    @olympus.remat  # no static_argnums or concrete
    def g(x):
      if x > 0:
        return lax.sin(x)
      else:
    ...
    @functools.partial(olympus.remat, static_argnums=(0,))  # using static_argnums but...
    def g(x):
    ...
    @olympus.remat
    def g(x):
      olympus.jit(lambda: 0 if jnp.add(1, 1) else 0)()
      return lax.sin(x)
  ...
  def test_simple_jit(self):
    ...
    self._check_tracers_and_olympusprs(
        olympus.jit(my_f),
        dict(a=1, b=2), 3,
        tracer_spy=tracer_spy,
        expected_olympuspr_debug_infos=[
  ...
  def test_jit_arg_names(self):
    ...
    self._check_tracers_and_olympusprs(
        ...
        expected_lowering_lines=[
            ...
            re.compile(r".*func.func public @main\(.*\{olympus.result_info = \"result\"\}"),
        ]
    )
  ...
  def test_jit_arg_names_static_argnums(self):
    ...
    self._check_tracers_and_olympusprs(
        olympus.jit(my_f, static_argnums=(0, 5)),
  ...
  def test_jit_arg_names_static_argnames(self):
    ...
    self._check_tracers_and_olympusprs(
        olympus.jit(f, static_argnames=("a",)),
  ...
  def test_jit_arg_names_with_out_of_order_kwargs(self):
    ...
    self._check_tracers_and_olympusprs(
        olympus.jit(my_f, static_argnums=(0,), static_argnames=("y4",)),
    ...
    self._check_tracers_and_olympusprs(
        olympus.jit(my_f, static_argnames=("y4",)),
    ...
    util.clear_all_caches()
    self._check_tracers_and_olympusprs(
        olympus.jit(my_f, static_argnums=(3,)),
  ...
  def test_jit_result_info(self):
    ...
    self._check_tracers_and_olympusprs(
        ...
        expected_lowering_lines=[
            ...
            re.compile(r".*func.func public @main\(.*\{olympus.result_info = \"result\['a'\]\"\}"),
            re.compile(r".*func.func public @main\(.*\{olympus.result_info = \"result\['b'\]\[0\]\[0\]\"\}"),
        ])
  ...
  def test_nested_jit_with_const_and_unused_args(self):
    ...
    if config.use_simplified_olympuspr_constants.value:
      ...
      expected_olympuspr_debug_infos.extend([
          "traced_for=jit, fun=my_g, arg_names=u,v, result_paths=result",
      ])
      expected_lowering_lines = [
          re.compile(r".*func.func public @main\(%arg0: tensor<8xf..> {olympus.const = true} loc\(unknown\)"),
    ...
    self._check_tracers_and_olympusprs(
        olympus.jit(my_f),
        x, y,
        expected_olympuspr_debug_infos=expected_olympuspr_debug_infos,
  ...
  def test_jvp_of_jit(self):
    ...
    def f(x, y, z):
      ...
      return {'a': x, 'b': [y]}
    self._check_tracers_and_olympusprs(
        lambda x, y, z: olympus.jvp(olympus.jit(f), (x, y, z), (x, y, z)),
        ...
        expected_lowering_lines=[
            ...
            re.compile(r".*func.func public @main\(.*-> .*tensor<f..> {olympus.result_info = \"\"}"),
        ])
  ...
  def test_vjp_of_jit(self):
    ...
    def my_f(x, y, z):
      ...
      return {'a': x * y[0], 'b': [y]}
    self._check_tracers_and_olympusprs(
        lambda x, y, z: olympus.vjp(olympus.jit(my_f), x, y, z)[1](dict(a=x, b=[y])),
        jnp.float32(1.), (jnp.float32(2.),), [jnp.float32(3.)],
        expected_olympuspr_debug_infos=[
  ...
  def test_vjp_of_nested_jit(self):
    ...
    self._check_tracers_and_olympusprs(
        olympus.jit(lambda x, y, res_ct: olympus.vjp(my_f, x, y)[1](res_ct)),
        2., 3., 0.3,
        tracer_spy=tracer_spy,
        expected_olympuspr_debug_infos=expected_olympuspr_debug_infos,
        expected_tracer_debug_infos=[
        ...
        expected_lowering_lines=[
            ...
            re.compile(r".*func.func public @main\(.*olympus.result_info = \"result\[0\]\"}"),
            re.compile(r".*func.func public @main\(.*olympus.result_info = \"result\[1\]\"}"),
        ])
  ...
  def test_vjp_remat(self):
    ...
    def apply_fn(inp):
      ...
      def to_remat(x):
        tracer_spy.append(x)
        return olympus.nn.relu(x * x)
      fn = olympus.checkpoint(to_remat)
      return olympus.vjp(fn, inp)

    self._check_tracers_and_olympusprs(
        olympus.jit(apply_fn),
        2.,
        tracer_spy=tracer_spy,
        expected_olympuspr_debug_infos=[
  ...
  def test_custom_jvp(self):
    ...
    self._check_tracers_and_olympusprs(
        olympus.jit(lambda a: olympus.jvp(top_f, (a, a),
  ...
  def test_custom_jvp_nondiff_args(self):
    ...
    def top_f(xy):
      ...
      @functools.partial(olympus.custom_jvp, nondiff_argnums=(0,))
      def my_g(h, xy):
        x, y = xy
        tracer_spy.append(x)
        return h(x)
    ...
    self._check_tracers_and_olympusprs(
        olympus.jit(lambda a, b: olympus.jvp(top_f, ((a, b),),
  ...
  def test_custom_vjp(self):
    ...
    self._check_tracers_and_olympusprs(
        olympus.jit(olympus.grad(to_diff)),
        {"a" : 3.},
        tracer_spy=tracer_spy,
        expected_olympuspr_debug_infos=[
  ...
  def test_custom_vjp_nondiff_args(self):
    tracer_spy = TracerSpy()
    @functools.partial(olympus.custom_vjp, nondiff_argnums=(0,))
    def app(f, xy):
      tracer_spy.append(xy[0])
      return f(xy)
    def app_fwd(f, xy):
    ...
    self._check_tracers_and_olympusprs(
        olympus.jit(olympus.grad(lambda xy: app(lambda x: 2 * x[0], xy))),
        (3., 3.),
        tracer_spy=tracer_spy,
        expected_olympuspr_debug_infos=[
  ...
  def test_custom_transpose(self):
    ...
    x = jnp.ones(2) * 6.
    self._check_tracers_and_olympusprs(
        olympus.jit(lambda x: olympus.linear_transpose(functools.partial(my_f_with_cond, 7.),
                                               x)),
        x,
        tracer_spy=tracer_spy,
        expected_olympuspr_debug_infos=[
  ...
  def test_linear_call(self):
    ...
    def my_f(x, y):
      ...
      def fn_tp(r, t):
        ...
        return dict(c=t["b"] * r)
      return dict(a=x["c"] + olympus.custom_derivatives.linear_call(fn, fn_tp, y, x)["b"])

    f1 = lambda x: my_f(x, jnp.ones(2) * 3.)
    x = jnp.ones(2) * 6.

    self._check_tracers_and_olympusprs(
        olympus.jit(lambda x: olympus.linear_transpose(f1, dict(c=x))(dict(a=x))),
        x,
        tracer_spy=tracer_spy,
        expected_olympuspr_debug_infos=[
  ...
  def test_grad_cond_with_remat(self):
    ...
    def my_f(x, y):
      ...
      x2, y2 = olympus.remat(my_g)(x, y)
      return y2 + lax.sin(x2)

    self._check_tracers_and_olympusprs(
        olympus.jit(olympus.grad(my_f)),
        1., 2.,
        tracer_spy=tracer_spy,
        expected_olympuspr_debug_infos=[
            "traced_for=jit, fun=my_f, arg_names=x,y, result_paths=result",
  ...
  def test_grad_scan(self):
    ...
    def my_f(x, as_):  # x: f32, as_: f32[3, 2]
      ...
      return olympus.remat(to_remat)(c, as_)  # c is closed-over

    def the_grad(c, as_):  # c: f32[], as_: f32[3, 2],
      tracer_spy.append(c)
      _, pullback = olympus.vjp(my_f, c, as_)
    ...
    if config.use_simplified_olympuspr_constants.value:
      expected_lowering_lines = [
          re.compile(r".*func.func public @main\(%arg0: tensor<3xf..> {olympus.const = true} loc\(unknown\)"),
          re.compile(r".*func.func public @main\(.*, %arg1: tensor<f..> loc\(\"c\"\)"),
          re.compile(r".*func.func public @main\(.*, %arg2: tensor<3x2xf..> loc\(\"as_\"\)"),
          re.compile(r".*func.func public @main\(.* -> .*tensor<f..> {olympus.result_info = \"result\[0\]\""),
          re.compile(r".*func.func public @main\(.* -> .*tensor<3x2xf..> {olympus.result_info = \"result\[1\]\""),
      ]
    else:
      expected_lowering_lines = [
  ...
  def test_fori(self):
    # See https://github.com/olympus-ml/olympus/issues/23637
    tracer_spy = TracerSpy()
    def my_body(_, c):
      tracer_spy.append(c)
      return 0.
    ...
    self._check_tracers_and_olympusprs(
        olympus.jit(lambda x: olympus.lax.fori_loop(0, 5, my_body, x)),
        3.,
        tracer_spy=tracer_spy,
        expected_olympuspr_debug_infos=[
    ...
    self._check_tracers_and_olympusprs(
        olympus.jit(lambda ub, x: olympus.lax.fori_loop(0, ub, my_body, x)),
        5, 3.,
        tracer_spy=tracer_spy,
        expected_olympuspr_debug_infos=[
  ...
  def test_eval_shape(self):
    ...
    _ = olympus.eval_shape(my_f, 0)
    self._check_tracers_and_olympusprs(
        lambda: olympus.eval_shape(my_f, 0),
        tracer_spy=tracer_spy,
        expected_olympuspr_debug_infos=[],
        expected_tracer_debug_infos=[
  ...
  def test_pmap(self):
    ...
    if config.pmap_shmap_merge.value:
      expected_olympuspr_debug_infos = [
          ...
          re.compile(r"traced_for=jit, fun=call_wrapped at .*olympus._src.linear_util.py:.*, arg_names=args\[0\], result_paths=result\[0\]"),
          re.compile(r"traced_for=shard_map, fun=call_wrapped at .*olympus._src.linear_util.py:.*, arg_names=args\[0\], result_paths=result\[0\]"),
      ]
      expected_tracer_debug_infos = [
          re.compile(r"traced_for=shard_map, fun=call_wrapped at .*olympus._src.linear_util.py:.*, arg_names=args\[0\], from args\[0\]"),
      ]
    else:
      expected_olympuspr_debug_infos = [
    ...
    self._check_tracers_and_olympusprs(
        olympus.pmap(my_f),
        np.ones((olympus.device_count(),), dtype=np.float32),
        expected_olympuspr_debug_infos=expected_olympuspr_debug_infos,
        tracer_spy=tracer_spy,
  ...
  def test_pmap_with_arg_and_result_names(self):
    ...
    if config.pmap_shmap_merge.value:
      expected_olympuspr_debug_infos = [
          # TODO(necula): We should not include `call_wrapped` in the debug info.
          re.compile(r"traced_for=jit, fun=call_wrapped at .*olympus._src.linear_util.py:.*, arg_names=(args\[\d+\],)+ result_paths=result\[0\],result\[1\]"),
          re.compile(r"traced_for=shard_map, fun=call_wrapped at .*olympus._src.linear_util.py:.*, arg_names=(args\[\d+\],)+ result_paths=result\[0\],result\[1\]"),
      ]
      expected_tracer_debug_infos = [
          re.compile(r"traced_for=shard_map, fun=call_wrapped at .*olympus._src.linear_util.py:.*, arg_names=(args\[\d+\],)+ from args\[\d+\]"),
      ]
      expected_lowering_lines = []
    else:
      expected_olympuspr_debug_infos = [
      ...
      expected_lowering_lines = [
          ...
          re.compile(r".*func.func public @main\(.* -> .*\{olympus.result_info = \"\"\}"),
          re.compile(r".*func.func public @main\(.* -> .*\{olympus.result_info = \"\"\}"),
      ]
    self._check_tracers_and_olympusprs(
        olympus.pmap(my_f, static_broadcasted_argnums=(0,)),
        1.,  # x0
        np.ones((olympus.device_count(), 1), dtype=np.float32),  # y1
        np.ones((olympus.device_count(), 2), dtype=np.float32),  # args[0]
        np.ones((olympus.device_count(), 3), dtype=np.float32),  # args[1]
        b4=np.ones((olympus.device_count(), 4), dtype=np.float32),
        a5=np.ones((olympus.device_count(), 5), dtype=np.float32),
        c6=np.ones((olympus.device_count(), 6), dtype=np.float32),
  ...
  def test_pmap_of_grad(self):
    ...
    self._check_tracers_and_olympusprs(
        olympus.pmap(olympus.grad(my_f)),
        np.ones((olympus.device_count(),), dtype=np.float32),
        expected_olympuspr_debug_infos=expected_olympuspr_debug_infos,
        tracer_spy=tracer_spy,
        expected_tracer_debug_infos=[
  ...
  def test_jvp_pmap_eager(self):
    ...
    x = jnp.ones((olympus.device_count(), 1), dtype=np.float32)
    x_tan = jnp.full_like(x, .1)

    if config.pmap_shmap_merge.value:
      expected_olympuspr_debug_infos = [
          # TODO(necula): We should not include `call_wrapped` in the debug info.
          re.compile(r"traced_for=jit, fun=call_wrapped at .*olympus._src.linear_util.py:.*, arg_names=,,,, result_paths=,,,"),
          re.compile(r"traced_for=shard_map, fun=call_wrapped at .*olympus._src.linear_util.py:.*, arg_names=,,,, result_paths=,,,"),
      ]
      # TODO(dsuo): Need to add tpu_pjrt_c_api and tpu_v3 to this conditional.
      if jtu.test_device_matches(["cpu"]):
        expected_olympuspr_debug_infos.extend([
            re.compile(r"traced_for=jit, fun=dynamic_slice at .*olympus._src.dispatch.py:.*, arg_names=(args\[\d+\],)+ result_paths=result"),
        ])
      expected_tracer_debug_infos = [
          re.compile(r"traced_for=shard_map, fun=call_wrapped at .*olympus._src.linear_util.py:.*, arg_names=(args\[\d+\],)+ from args\[\d+\]"),
      ]
    ...
    self._check_tracers_and_olympusprs(
        lambda x, x_tan: olympus.jvp(olympus.pmap(my_f),
  ...
  def test_jvp_pmap(self):
    ...
    x = np.ones((olympus.device_count(), 1), dtype=np.float32)
    x_tan = np.full_like(x, .1)

    if config.pmap_shmap_merge.value:
      expected_olympuspr_debug_infos = [
          # TODO(necula): We should not include `call_wrapped` in the debug info.
          re.compile(r"traced_for=jit, fun=call_wrapped at .*olympus._src.linear_util.py:.*, arg_names=None, result_paths=None"),
          r"traced_for=jit, fun=<lambda>, arg_names=x,x_tan, result_paths=result[0],result[1]",
          re.compile(r"traced_for=shard_map, fun=call_wrapped at .*olympus._src.linear_util.py:.*, arg_names=None, result_paths=None"),
      ]
      expected_tracer_debug_infos = [
          re.compile(r"traced_for=shard_map, fun=call_wrapped at .*olympus._src.linear_util.py:.*, arg_names=args\[0\],args\[1\], from args\[0\]"),
      ]
    else:
      expected_olympuspr_debug_infos = [
    ...
    self._check_tracers_and_olympusprs(
        olympus.jit(lambda x, x_tan: olympus.jvp(olympus.pmap(my_f), (x, x), (x_tan, x_tan))),
        x, x_tan,
        expected_olympuspr_debug_infos=expected_olympuspr_debug_infos,
        tracer_spy=tracer_spy,
        expected_tracer_debug_infos=expected_tracer_debug_infos,
  ...
  def test_hessian(self):
    ...
    self._check_tracers_and_olympusprs(
        olympus.jit(olympus.hessian(olympus.jit(my_f))),
        x,
        expected_olympuspr_debug_infos=expected_olympuspr_debug_infos,
        tracer_spy=tracer_spy,
        expected_tracer_debug_infos=[
  ...
  def test_remat_shard_map(self):
    ...
    mesh = Mesh(np.array(olympus.devices()[:2]), ('x',))
    ...
    self._check_tracers_and_olympusprs(
        olympus.jit(olympus.grad(lambda x: my_f(x).sum())),
  ...
  def test_remat_saved_residuals(self):
    @functools.partial(olympus.remat,
                       static_argnums=(1,),
  ...
  def test_checkify_pmap_basic(self):
    ...
    if config.pmap_shmap_merge.value:
      expected_olympuspr_debug_infos = [
          ...
          re.compile(r"traced_for=jit, fun=call_wrapped at .*olympus._src.linear_util.py:.*, arg_names=None, result_paths=None"),
          re.compile(r"traced_for=shard_map, fun=call_wrapped at .*olympus._src.linear_util.py:.*, arg_names=None, result_paths=None"),
          re.compile(r"traced_for=jit, fun=checked_fun at .*olympus._src.checkify.py:.*, arg_names=args\[0\], result_paths=(result\[\d+\]\[\d+\]\[ErrorEffect\(error_type=<class.*>, shape_dtypes=.*\)\])*"),
      ]
      expected_tracer_debug_infos = [
          re.compile(r"traced_for=shard_map, fun=call_wrapped at .*olympus._src.linear_util.py:.*, arg_names=args\[0\], from args\[0\]"),
      ]
      ...
      expected_olympuspr_debug_infos = [
          # TODO(necula): this should not be pointing into the OLYMPUS internals
          re.compile(r"traced_for=jit, fun=checked_fun at .*olympus._src.checkify.py:.*, arg_names=args\[0\]"),
    ...
    self._check_tracers_and_olympusprs(
        olympus.jit(checkify.checkify(my_f, errors=checkify.nan_checks)),
        np.arange(len(olympus.devices()), dtype=np.float32),
        tracer_spy=tracer_spy,
        expected_olympuspr_debug_infos=expected_olympuspr_debug_infos,
        expected_tracer_debug_infos=expected_tracer_debug_infos,
  ...
  def test_custom_dce_static_argnums(self):
    tracer_spy = TracerSpy()
    @functools.partial(olympus.experimental.custom_dce.custom_dce,
                       static_argnums=(0,))
    def my_g(f, x):
      tracer_spy.append(x)
      return f(x), 10 * f(x)
    ...
    self._check_tracers_and_olympusprs(
        olympus.jit(lambda x: my_g(my_f, x)[0]),
        0.,
        tracer_spy=tracer_spy,
        expected_olympuspr_debug_infos=[
            "traced_for=jit, fun=<lambda>, arg_names=x, result_paths=result",
  ...
  def test_custom_linear_solve_complex(self):
    ...
    self._check_tracers_and_olympusprs(
        olympus.jit(lambda a, b: olympus.jvp(solve, (a, b), (a, b))),
        a, b,
        tracer_spy=tracer_spy,
        expected_olympuspr_debug_infos=[
            "traced_for=jit, fun=<lambda>, arg_names=a,b, result_paths=result[0],result[1]",
  ...
  def test_custom_root_errors(self):
    ...
    self._check_tracers_and_olympusprs(
        olympus.jit(lambda x: olympus.jvp(dummy_root_usage, (x,), (0.0,))),
        0.,
        tracer_spy=tracer_spy,
        expected_olympuspr_debug_infos=[
  ...
  def test_checkify_pallas_call(self):
    ...
    def my_f(input):
      out_shape = olympus.ShapeDtypeStruct(input.shape, input.dtype)
    ...
    self._check_tracers_and_olympusprs(
        ...
        expected_olympuspr_debug_infos=[
            ...
            # TODO(necula): function source location points in OLYMPUS internals
