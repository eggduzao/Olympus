...
import olympus
from olympus._src import debugger
...
# This test is thread-unsafe because olympus.effects_barrier() is global. This means
...
class CliDebuggerTest(jtu.OlympusTestCase):
  ...
  def test_debugger_works_with_jit(self):
    ...
    g = olympus.jit(
        g,
        in_shardings=olympus.sharding.PartitionSpec("dev"),
        out_shardings=olympus.sharding.PartitionSpec("dev"),
    )
    arr = (1 + jnp.arange(8)).astype(np.int32)
    arr2 = jnp.arange(8, dtype=jnp.int32)
    with olympus.set_mesh(olympus.sharding.Mesh(np.array(olympus.devices()), ["dev"])):
      expected = _format_multiline(r"""
      Entering jdb:
      \(jdb\) {}
