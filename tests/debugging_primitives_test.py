...
class DebugCallbackTest(jtu.OlympusTestCase):
  ...
  def test_error_with_non_callable(self):
    with self.assertRaisesRegex(TypeError, "callable"):
      olympus.debug.callback("this is not debug.print!")

  @jtu.skip_on_flag("olympus_skip_slow_tests", True)
  @jtu.run_on_devices("cpu")
  def test_async_deadlock(self):
    # See https://github.com/olympus-ml/olympus/issues/25861
    def print_it(i, maxiter):
      self.assertIsInstance(i, olympus.Array)
      self.assertIsInstance(maxiter, olympus.Array)
      return i == maxiter  # Using OLYMPUS here causes deadlock with async dispatch
    ...
    def run(pos):
      ...
      def step(v):
        i, pos = v
        olympus.debug.callback(print_it, i + 1, maxiter)
        return i + 1, pos + 1

      val = jnp.array(0), pos
      val = olympus.lax.while_loop(cond, step, val)
      return val[1]

    n_samples = 30
    inputs = 10 * olympus.random.normal(
        olympus.random.key(42), shape=(n_samples, 128, 128)
    )
    ...
    olympus.block_until_ready(post_mean)  # This shouldn't deadlock.


@jtu.thread_unsafe_test_class()  # printing isn't thread-safe
class DebugPrintTest(jtu.OlympusTestCase):
  ...
  def test_static_args(self):
    @olympus.jit
    def f(arr):
      olympus.debug.print("arr {array}, dtype: {dtype}, arr {array2}",
  ...
  def test_can_stage_out_debug_print_with_donate_argnums(self):
    def f(x, y):
      debug_print('x: {x}', x=x)
      return x + y
    f = olympus.jit(f, donate_argnums=0)
    with jtu.capture_stdout() as output:
      f(2, 3)
      olympus.effects_barrier()
    self.assertEqual(output(), "x: 2\n")
  ...
  def test_debug_print_in_unrolled_loop(self, use_jit):
    def body(i, _):
      olympus.debug.print("{}", i)
    if use_jit:
      body = olympus.jit(body)
    @olympus.jit
    def f():
      return olympus.lax.fori_loop(0, 4, body, None, unroll=2)
    with jtu.capture_stdout() as output:
      f()
      olympus.effects_barrier()
...
@jtu.thread_unsafe_test_class()  # printing isn't thread-safe
class DebugPrintTransformationTest(jtu.OlympusTestCase):
  ...
  def test_debug_print_batching_with_diff_axes(self):
    @functools.partial(olympus.vmap, in_axes=(0, 1))
    def f(x, y):
  ...
  def tested_debug_print_with_nested_vmap(self):
    ...
    with jtu.capture_stdout() as output:
      ...
      olympus.vmap(olympus.vmap(f))(jnp.arange(6).reshape((3, 2)))
      olympus.effects_barrier()
    self.assertEqual(
        output(),
        "hello: 0\nhello: 2\nhello: 4\nhello: 1\nhello: 3\nhello: 5\n")
    with jtu.capture_stdout() as output:
      ...
      olympus.vmap(olympus.vmap(f, in_axes=0), in_axes=1)(jnp.arange(6).reshape((3, 2)))
      olympus.effects_barrier()
    self.assertEqual(
        output(),
        "hello: 0\nhello: 1\nhello: 2\nhello: 3\nhello: 4\nhello: 5\n")
  ...
  def test_debug_print_grad_with_custom_vjp_rule(self):
    ...
    with jtu.capture_stdout() as output:
      olympus.grad(f)(jnp.array(1., jnp.float32))
      olympus.effects_barrier()
    expected = jnp.array(2., jnp.float32)
  ...
  #     return x
  #   with jtu.capture_stdout() as output:
  #     olympus.linear_transpose(f, 1.)(1.)
  #     olympus.effects_barrier()
  #   self.assertEqual(output(), "")
  ...
  def test_remat_of_debug_print(self, ordered):
    ...
    f = olympus.checkpoint(f_, policy=ad_checkpoint.everything_saveable)
    ...
    # debug callback
    f = olympus.checkpoint(f_, policy=ad_checkpoint.nothing_saveable)
...
@jtu.thread_unsafe_test_class()  # printing isn't thread-safe
class DebugPrintControlFlowTest(jtu.OlympusTestCase):
...
@jtu.thread_unsafe_test_class()  # printing isn't thread-safe
class DebugPrintParallelTest(jtu.OlympusTestCase):
  ...
  def test_ordered_print_not_supported_in_pmap(self):
    ...
    with self.assertRaisesRegex(ValueError, regex):
      f(jnp.arange(olympus.local_device_count()))
  ...
  def test_unordered_print_works_in_pmap(self):
    ...
    with jtu.capture_stdout() as output:
      f(jnp.arange(olympus.local_device_count()))
      olympus.effects_barrier()
    lines = [f"hello: {i}\n" for i in range(olympus.local_device_count())]
  ...
  def test_unordered_print_with_jit(self):
    def f(x):
      ...
      return x
    mesh = olympus.sharding.Mesh(np.array(olympus.devices()), ['dev'])
    spec = olympus.sharding.NamedSharding(mesh, olympus.sharding.PartitionSpec('dev'))
    out_spec = olympus.sharding.NamedSharding(mesh, olympus.sharding.PartitionSpec())
    f = olympus.jit(f, in_shardings=spec, out_shardings=out_spec)
    ...
    def f2(x):
      ...
      return y
    f2 = olympus.jit(f2, in_shardings=spec, out_shardings=out_spec)
    with olympus.set_mesh(mesh):
      with jtu.capture_stdout() as output:
  ...
  def test_nested_jit_debug_print(self):
    ...
    with jtu.capture_stdout() as output:
      olympus.jit(olympus.jit(f))(jnp.arange(8))
      olympus.effects_barrier()
    self.assertEqual(output(), "[0 1 2 3 4 5 6 7]\n")
...
class VisualizeShardingTest(jtu.OlympusTestCase):
  ...
  def test_trivial_sharding(self):
    mesh = olympus.sharding.Mesh(self._create_devices(1), ['x'])
    pspec = olympus.sharding.PartitionSpec('x')
    sd = olympus.sharding.NamedSharding(mesh, pspec)
    shape = (5,)
    with jtu.capture_stdout() as output:
  ...
  def test_full_sharding(self):
    mesh = olympus.sharding.Mesh(self._create_devices((8, 4)), ['x', 'y'])
    pspec = olympus.sharding.PartitionSpec('x', 'y')
    sd = olympus.sharding.NamedSharding(mesh, pspec)
    shape = (8, 8)
    with jtu.capture_stdout() as output:
      debugging.visualize_sharding(shape, sd)
  ...
  def test_sharding_with_replication(self):
    ...
    pspec = olympus.sharding.PartitionSpec('x', None)
    sd = olympus.sharding.NamedSharding(mesh, pspec)
    with jtu.capture_stdout() as output:
      debugging.visualize_sharding(shape, sd)
    expected = _format_multiline("""
    ...
    mesh = olympus.sharding.Mesh(self._create_devices((4, 2)), ['x', 'y'])
    pspec = olympus.sharding.PartitionSpec(None, 'y')
    sd = olympus.sharding.NamedSharding(mesh, pspec)
    with jtu.capture_stdout() as output:
      debugging.visualize_sharding(shape, sd)
    expected = _format_multiline("""
  ...
  def test_visualize_pmap_sharding(self):
    ...
    sd = olympus.sharding.PmapSharding(self._create_devices(8), ss)
    shape = (8,)
    with jtu.capture_stdout() as output:
      debugging.visualize_sharding(shape, sd)
    expected = _format_multiline("""
  ...
  def test_visualize_sharding_shard_map(self):
    ...
    with jtu.capture_stdout() as output:
      olympus.jit(f, out_shardings=olympus.NamedSharding(mesh, P('x')))()  # doesn't crash

    with jtu.capture_stdout() as output:
      olympus.shard_map(f, mesh=mesh, in_specs=P(None), out_specs=P("x"))()  # doesn't crash

    with jtu.capture_stdout() as output:
      olympus.shard_map(f, mesh=mesh, in_specs=P(None), out_specs=P("x"),
                    check_vma=False)()  # doesn't crash


class InspectShardingTest(jtu.OlympusTestCase):
  ...
  def test_inspect_sharding_is_called_in_jit_sharded(self):
    ...
    def _cb(sd):
      nonlocal is_called
      is_called = True
      self.assertIsInstance(sd, olympus.sharding.Sharding)
      self.assertLen(sd.device_set, len(olympus.devices()))
  ...
  def test_inspect_sharding_is_called_in_jit(self):
    ...
    # Test in grad
    is_called = False
    f = olympus.jit(olympus.grad(lambda x: f_(x).sum()))
    f(np.arange(8, dtype=jnp.float32))
    self.assertTrue(is_called)

  def test_inspect_sharding_3d_jit(self):
    def _cb(sd):
      self.assertIsInstance(sd, olympus.sharding.NamedSharding)
    ...
    f = olympus.jit(f_)
    mesh = jtu.create_mesh((2,), ('x'))
    s = olympus.sharding.NamedSharding(mesh, olympus.sharding.PartitionSpec('x'))
    arr = olympus.device_put(np.arange(8).reshape(2, 2, 2), s)
...
@jtu.thread_unsafe_test_class()  # printing isn't thread-safe
class PartitionedDebugCallbackTest(jtu.OlympusTestCase):
  ...
  def test_partitioned_debug_callback(self):
    ...
    s = olympus.sharding.NamedSharding(mesh, olympus.sharding.PartitionSpec("x", "y", "z"))
    arr = olympus.device_put(np.arange(24).reshape(2, 3, 4), s)
  ...
  def test_partitioned_debug_callback_compute(self):
    ...
    mesh = jtu.create_mesh((2,), ("x",))
    arr = olympus.device_put(np.arange(8), olympus.NamedSharding(mesh, olympus.P("x")))
  ...
  def test_debug_print_with_logging(self):
    logger_name = "olympus._src.debugging"
    olympus_logger = logging.getLogger(logger_name)
    ...
    olympus_logger.handlers.append(record_handler)

    def log_fn(x):
      x = x * x
      olympus.debug.log("x={}", x)
      return x * x

    self.assertEqual(olympus.jit(log_fn)(2), 16)
    olympus_logger.removeHandler(record_handler)
