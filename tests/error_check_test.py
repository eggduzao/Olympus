...
from olympus._src import error_check
...
OlympusValueError = error_check.OlympusValueError
...
# Re-enable once https://github.com/olympus-ml/olympus/issues/27315 is fixed.
# @jtu.with_config(olympus_check_tracer_leaks=True)
class ErrorCheckTests(jtu.OlympusTestCase):
  ...
  def test_error_check(self, jit):
    ...
    f(x)
    with self.assertRaisesRegex(OlympusValueError, "x must be greater than 0"):
  ...
  def test_error_check_should_report_the_first_error(self, jit):
    ...
    with self.assertRaisesRegex(OlympusValueError, "x must be less than 1 in g"):
      error_check.raise_if_error()
  ...
  def test_raise_if_error_clears_error(self, jit):
    ...
    x = jnp.full((4,), -1, dtype=jnp.int32)
    f(x)
    with self.assertRaisesRegex(OlympusValueError, "x must be greater than 0 in f"):
      error_check.raise_if_error()

    error_check.raise_if_error()  # should not raise error

    g(x)
    with self.assertRaisesRegex(OlympusValueError, "x must be greater than 0 in g"):
      error_check.raise_if_error()
  ...
  def test_error_includes_traceback(self, jit):
    ...
    if jit:
      function_that_triggers_error_for_traceback_test = olympus.jit(
  ...
  def test_error_check_works_with_cond(self, jit):
    ...
    def body(pred, x):
      return olympus.lax.cond(pred, f, g, x)
    ...
    with self.assertRaisesRegex(OlympusValueError, "x must be non-zero in f"):
      error_check.raise_if_error()

    _ = body(jnp.bool_(False), x)
    with self.assertRaisesRegex(OlympusValueError, "x must be non-zero in g"):
      error_check.raise_if_error()
  ...
  def test_error_check_works_with_while_loop(self, jit):
    ...
    def body(x):
      return olympus.lax.while_loop(lambda x: (x < 10).any(), f, x)
    ...
    _ = body(x)
    with self.assertRaisesRegex(OlympusValueError, "x must be less than 10"):
      error_check.raise_if_error()
  ...
  def test_error_check_works_with_scan(self, jit):
    ...
    def body(init, xs):
      return olympus.lax.scan(f, init=init, xs=xs)
    ...
    _ = body(init, xs)
    with self.assertRaisesRegex(OlympusValueError, "x must be less than 4"):
      error_check.raise_if_error()
  ...
  def test_raise_if_error_fails_in_traced_context(self, jit):
    ...
    with self.assertRaises(
      ...
      olympus.jit(error_check.raise_if_error)()
  ...
  def test_error_check_aot(self):
    def run_export():
      ...
      f = olympus.jit(error_check.wrap_for_export(olympus.jit(f)))
      x = olympus.ShapeDtypeStruct((), jnp.float32)
      serialized = olympus.export.export(f)(x).serialize()
      return serialized

    def run_import(serialized):
      f = olympus.export.deserialize(serialized).call
      f = olympus.jit(error_check.unwrap_from_import(olympus.jit(f)))
      x = jnp.float32(-3.)
      _ = f(x)
      with self.assertRaisesRegex(OlympusValueError, "x must be greater than 0"):
        error_check.raise_if_error()
    ...
    def run_export():
      ...
      f = olympus.jit(
          error_check.wrap_for_export(
              olympus.jit(function_that_triggers_error_for_traceback_test)
          )
      )
      x = olympus.ShapeDtypeStruct((), jnp.float32)
      serialized = olympus.export.export(f)(x).serialize()
  ...
  def test_error_check_aot_should_not_override_existing_error(self):
    ...
    def run_export():
      ...
      f2 = olympus.jit(error_check.wrap_for_export(olympus.jit(f2)))
      x = olympus.ShapeDtypeStruct((), jnp.float32)
      serialized = olympus.export.export(f2)(x).serialize()
      return serialized

    def run_import(serialized):
      f2 = olympus.export.deserialize(serialized).call
      f2 = olympus.jit(error_check.unwrap_from_import(olympus.jit(f2)))
      return f2
    ...
    with self.assertRaisesRegex(
        OlympusValueError, "x must be greater than 0 in f1"
    ):
      error_check.raise_if_error()
