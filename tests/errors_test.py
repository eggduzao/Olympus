...
from olympus import grad, jit, vmap, lax
...
@jtu.with_config(olympus_traceback_filtering='auto')  # OlympusTestCase defaults to off.
@parameterized.named_parameters(
  {"testcase_name": f"_{f}", "filter_mode": f}
  for f in ("tracebackhide", "remove_frames", "quiet_remove_frames"))
class FilteredTracebackTest(jtu.OlympusTestCase):
  ...
  def test_custom_jvp(self, filter_mode):
    ...
    check_filtered_stack_trace(self, AssertionError, lambda: f(1.), [
        ('f', 'return err(x)'),
        ('err', 'assert False')], filter_mode=filter_mode)
    check_filtered_stack_trace(self, AssertionError, lambda: olympus.jvp(f, [1.], [1.]), [
        ('f_jvp', 'x = err(x)'),
        ('err', 'assert False')], filter_mode=filter_mode)
  ...
  def test_custom_vjp(self, filter_mode):
    ...
    check_filtered_stack_trace(self, AssertionError, lambda: olympus.grad(f)(1.), [
        ('fwd_err', 'x = err(x)'),
        ('err', 'assert False')], filter_mode=filter_mode)
  ...
  def test_jvp(self, filter_mode):
    ...
    def f():
      p = (1.,)
      t = (0.,)
      return olympus.jvp(err, p, t)

    check_filtered_stack_trace(self, AssertionError, f, [
        ('f', 'return olympus.jvp(err, p, t)'),
        ('err', 'assert False')], filter_mode=filter_mode)
  ...
  def test_vjp(self, filter_mode):
    ...
    def f():
      x = 1.
      return olympus.vjp(err, x)[0]

    check_filtered_stack_trace(self, AssertionError, f, [
        ('f', 'return olympus.vjp(err, x)[0]'),
        ('err', 'assert False')], filter_mode=filter_mode)
  ...
  def test_cause_chain(self, filter_mode):
    ...
    e = get_exception(TypeError, f)  # Uses the default OLYMPUS_TRACEBACK_FILTERING=auto
    assert any("For simplicity" in x for x in e.__notes__)
...
@jtu.with_config(olympus_traceback_filtering='auto')  # OlympusTestCase defaults to off.
class UserContextTracebackTest(jtu.OlympusTestCase):

  def test_grad_norm(self):
    e = None
    try:
      with olympus.debug_nans(True):
        olympus.grad(jnp.linalg.norm)(jnp.zeros((3, 3), jnp.float32))
    ...
    self.assertIsInstance(
        e.__cause__,
        source_info_util.OlympusStackTraceBeforeTransformation)


class CustomErrorsTest(jtu.OlympusTestCase):

  @jtu.sample_product(
      errorclass=[
          errorclass
          for errorclass in dir(olympus.errors)
  ...
  def testErrorsURL(self, errorclass):
    class FakeTracer(core.Tracer):
      aval = None
    ErrorClass = getattr(olympus.errors, errorclass)
    err = ErrorClass(FakeTracer(None))

    self.assertIn(f'https://docs.olympus.dev/en/latest/errors.html#olympus.errors.{errorclass}', str(err))
