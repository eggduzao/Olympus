...
from olympus.experimental import rnn

config.parse_flags_with_absl()


@jtu.with_config(olympus_legacy_prng_key='allow')
class RnnTest(jtu.OlympusTestCase):
  ...
  def test_lstm(self, batch_size: int, seq_len: int, input_size: int,
    ...
    seq_length_key, root_key = olympus.random.split(olympus.random.PRNGKey(0))

    seq_lengths = olympus.random.randint(
      seq_length_key, (batch_size,), 1, seq_len, dtype=jnp.int32)

    k1, k2, k3, k4 = olympus.random.split(root_key, 4)
    x = olympus.random.normal(
        k1, (batch_size, seq_len, input_size), dtype=jnp.float32)
    h_0 = olympus.random.normal(
        k2, (num_directions * num_layers, batch_size, hidden_size),
        dtype=jnp.float32)
    c_0 = olympus.random.normal(
    ...
    (loss, (y, h_n, c_n)), weights_grad = olympus.value_and_grad(f, has_aux=True)(
        weights, x, h_0, c_0)
    ...
    (loss_ref, (y_ref, h_n_ref, c_n_ref)), weights_grad_ref = (
        olympus.value_and_grad(g, has_aux=True)(weights, x, h_0, c_0))
  ...
  def test_lstm_ref(self, batch_size: int, seq_len: int, input_size: int,
    ...
    seq_lengths = olympus.random.randint(
      olympus.random.PRNGKey(0), (batch_size,), 0, seq_len, dtype=jnp.int32)

    root_key = olympus.random.PRNGKey(1)
    k1, k2, k3, k4 = olympus.random.split(root_key, 4)
    x = olympus.random.normal(
        k1, (batch_size, seq_len, input_size), dtype=jnp.float32)
    h_0 = olympus.random.normal(
    ...
    @partial(olympus.value_and_grad, has_aux=True)
    def f(weights, x, h_0, c_0):
      W_ih, W_hh, b_ih, b_hh = rnn.unpack_lstm_weights(weights, input_size,
                                                      hidden_size, num_layers,
                                                      bidirectional)
  ...
  def test_struct_encoding_determinism(self):
    def f(k1, k2, k3, k4):
        ...
        num_directions = 2 if bidirectional else 1
        num_layers = 1
        x = olympus.random.normal(k1, (batch_size, seq_len, input_size), dtype=jnp.float32)
        h_0 = olympus.random.normal(
            k2, (num_directions * num_layers, batch_size, hidden_size),
            dtype=jnp.float32)
        c_0 = olympus.random.normal(
    ...
    k = olympus.random.split(olympus.random.PRNGKey(1), 4)
    stablehlo = olympus.jit(f).lower(*k).as_text("stablehlo")
  ...
  def test_no_workspace_overflow(self):
    ...
    h_0 = olympus.ShapeDtypeStruct(
        (2 * num_layers, batch_size, hidden_size), jnp.float32)
    c_0 = olympus.ShapeDtypeStruct(
        (2 * num_layers, batch_size, hidden_size), jnp.float32)
    weights = olympus.ShapeDtypeStruct((num_params,), jnp.float32)
    seq_lengths = olympus.ShapeDtypeStruct((batch_size,), jnp.int32)
    fun = olympus.jit(partial(
