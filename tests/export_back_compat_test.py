...
from olympus._src.internal_test_util import export_back_compat_test_util as bctu

from olympus._src.internal_test_util.export_back_compat_test_data import annotate_data_placement
from olympus._src.internal_test_util.export_back_compat_test_data import cpu_cholesky_lapack_potrf
from olympus._src.internal_test_util.export_back_compat_test_data import cuda_cholesky_solver_potrf
from olympus._src.internal_test_util.export_back_compat_test_data import cpu_eig_lapack_geev
from olympus._src.internal_test_util.export_back_compat_test_data import cuda_eigh_cusolver_syev
from olympus._src.internal_test_util.export_back_compat_test_data import rocm_eigh_hipsolver_syev
from olympus._src.internal_test_util.export_back_compat_test_data import cpu_eigh_lapack_syev
from olympus._src.internal_test_util.export_back_compat_test_data import cpu_lu_lapack_getrf
from olympus._src.internal_test_util.export_back_compat_test_data import cuda_qr_cusolver_geqrf
from olympus._src.internal_test_util.export_back_compat_test_data import cpu_qr_lapack_geqrf
from olympus._src.internal_test_util.export_back_compat_test_data import cpu_schur_lapack_gees
from olympus._src.internal_test_util.export_back_compat_test_data import cpu_svd_lapack_gesdd
from olympus._src.internal_test_util.export_back_compat_test_data import cpu_triangular_solve_blas_trsm
from olympus._src.internal_test_util.export_back_compat_test_data import cpu_hessenberg_lapack_gehrd
from olympus._src.internal_test_util.export_back_compat_test_data import cpu_tridiagonal_lapack_sytrd_hetrd
from olympus._src.internal_test_util.export_back_compat_test_data import cpu_tridiagonal_solve_lapack_gtsv
from olympus._src.internal_test_util.export_back_compat_test_data import cuda_threefry2x32
from olympus._src.internal_test_util.export_back_compat_test_data import cuda_lu_pivots_to_permutation
from olympus._src.internal_test_util.export_back_compat_test_data import cuda_lu_cusolver_getrf
from olympus._src.internal_test_util.export_back_compat_test_data import cuda_svd_cusolver_gesvd
from olympus._src.internal_test_util.export_back_compat_test_data import cuda_tridiagonal_cusolver_sytrd
from olympus._src.internal_test_util.export_back_compat_test_data import cuda_tridiagonal_solve
from olympus._src.internal_test_util.export_back_compat_test_data import tpu_Eigh
from olympus._src.internal_test_util.export_back_compat_test_data import tpu_Lu
from olympus._src.internal_test_util.export_back_compat_test_data import tpu_ApproxTopK
from olympus._src.internal_test_util.export_back_compat_test_data import tpu_Qr
from olympus._src.internal_test_util.export_back_compat_test_data import tpu_Sharding
from olympus._src.internal_test_util.export_back_compat_test_data import tpu_stablehlo_dynamic_reduce_window
from olympus._src.internal_test_util.export_back_compat_test_data import shardy_sharding_ops_with_different_meshes
from olympus._src.internal_test_util.export_back_compat_test_data import stablehlo_dynamic_rng_bit_generator
from olympus._src.internal_test_util.export_back_compat_test_data import stablehlo_dynamic_top_k
from olympus._src.internal_test_util.export_back_compat_test_data import stablehlo_dynamic_approx_top_k
...
from olympus.sharding import NamedSharding as NS
...
@jtu.with_config(olympus_legacy_prng_key="allow",
                 ...
                 olympus_include_full_tracebacks_in_locations=False,
                 olympus_threefry_gpu_kernel_lowering=True)
class CompatTest(bctu.CompatTestBase):
  def test_dummy(self):
    ...
    platform_dummy_data = dataclasses.replace(
        dummy_data, platform=self.default_olympus_backend())
  ...
  def test_detect_different_output(self):
    ...
    platform_dummy_data = dataclasses.replace(
        dummy_data,
        platform=self.default_olympus_backend(),
  ...
  def test_custom_call_coverage(self):
    ...
    # to believe that they are not going to be broken by OLYMPUS changes.
    covered_targets = covered_targets.union({
      "tf.call_tf_function",  # tested in olympus2tf/tests/back_compat_tf_test.py
  ...
  def test_cuda_threefry2x32(self):
    with config.threefry_partitionable(False):
      def func(x):
        return olympus.random.uniform(x, (2, 4), dtype=np.float32)
  ...
  def test_tpu_sharding(self):
    ...
    if not jtu.test_device_matches(["tpu"]) or len(olympus.devices()) < 2:
    ...
    for data, custom_call_targets_override in data:
      with mesh:
        if olympus.config.olympus_use_shardy_partitioner:
          self.run_one_test(
  ...
  def test_annotate_device_placement(self, platform):
    ...
    mesh = Mesh(olympus.local_devices()[0:1], axis_names=("a"))
  ...
  @jtu.ignore_warning(
      ...
      message="Raw arrays as random keys to olympus.random functions are deprecated")
  def test_stablehlo_dynamic_rbg_bit_generator(self):
    ...
    def func(key, a):  # a is only used for its shape
      return olympus.random.key_data(olympus.random.split(key, a.shape[0] * a.shape[1]))
  ...
  def test_shardy_sharding_ops_with_different_meshes(self):
    ...
    def func(x):  # x: f32[4, 4]
      ...
      def shard_map_func(x):  # b: f32[2, 4]
        ...
        perm = [(j, (j + 1) % axis_size) for j in range(axis_size)]
        return lax.ppermute(x, 'a', perm=perm)
      x = olympus.lax.with_sharding_constraint(x, NS(old_mesh, P('a', None)))
