...
"""Tests for backwards compatibility of serialization of OLYMPUS exports.

Whenever we change the serialization format for olympus.export.Exported
(see file olympus.export.serialization), we should first save a serialization
...
To add a new test:
  ...
  * Copy the test data defined in the output file, to the file
    olympus._src.internal_test_util.export_back_compat_test_data.export_{name}.py.
...
from olympus._src.export.serialization import _SERIALIZATION_VERSION
from olympus.sharding import PartitionSpec as P
from olympus._src import test_util as jtu

from olympus._src.internal_test_util.export_back_compat_test_data import export_with_specified_sharding
from olympus._src.internal_test_util.export_back_compat_test_data import export_with_unspecified_sharding
from olympus._src.internal_test_util.export_back_compat_test_data import export_with_memory_space

config.parse_flags_with_absl()
jtu.request_cpu_devices(8)


class CompatTest(jtu.OlympusTestCase):
...
"""
  ...
  def test_with_specified_sharding(self, testdata: dict[str, Any] | None):
    ...
    with olympus.set_mesh(mesh):
      @olympus.jit(in_shardings=(olympus.sharding.NamedSharding(mesh, P("x", None),),),
               out_shardings=olympus.sharding.NamedSharding(mesh, P(None, "x")))
      def f(b):
        return b * 2.

      a = olympus.device_put(a, olympus.sharding.NamedSharding(mesh, P("x", None)))
      if testdata is None:
  ...
  def test_with_memory_space(self, testdata: dict[str, Any] | None):
    ...
    mesh = jtu.create_mesh((2,), "x")
    with olympus.set_mesh(mesh):
      shd = olympus.sharding.NamedSharding(mesh, P("x", None),
