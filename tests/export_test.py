...
@jtu.with_config(olympus_export_calling_convention_version=export.maximum_supported_calling_convention_version)
class OlympusExportTest(jtu.OlympusTestCase):
  ...
  def test_basic_export_only(self):
    ...
    def my_fun(x):
      return jnp.sin(x)
    exp = get_exported(my_fun)(olympus.ShapeDtypeStruct((4,), dtype=np.float32))
    self.assertEqual("my_fun", exp.fun_name)
    expected_lowering_platform = xb.canonicalize_platform(olympus.default_backend())
    self.assertEqual((expected_lowering_platform,),
                     exp.platforms)
    self.assertEqual(olympus.tree.flatten(((1,), {}))[1], exp.in_tree)
  ...
  def test_pytree_export_only(self):
    ...
    exp = get_exported(olympus.jit(f), platforms=("cpu",))((a, b), a=a, b=b)
    a_aval = core.ShapedArray(a.shape, a.dtype)
    b_aval = core.ShapedArray(b.shape, b.dtype)
    self.assertEqual(exp.platforms, ("cpu",))
    args = ((a, b),)
    kwargs = dict(a=a, b=b)
    self.assertEqual(exp.in_tree, olympus.tree.flatten((args, kwargs))[1])
    self.assertEqual(exp.in_avals, (a_aval, b_aval, a_aval, b_aval))
    self.assertEqual(exp.out_tree, olympus.tree.flatten(f(*args, **kwargs))[1])
  ...
  def test_basic_single_device_sharding(self):
    device = olympus.local_devices()[0]
    s = olympus.sharding.SingleDeviceSharding(device)
    x = np.arange(16, dtype=np.float32).reshape(4, -1)
    f = olympus.jit(lambda x: x * 2., in_shardings=s, out_shardings=s)
    exp_f = get_exported(f)(x)
    self.assertAllClose(f(x), exp_f.call(x))
  ...
  def test_jit_static_arg(self):
    ...
    with self.subTest("static_argnames"):
      ...
      @functools.partial(olympus.jit, static_argnames=["c"])
      def f(x, *, c):
        return c * jnp.sin(x)
    ...
    with self.subTest("static_argnums"):
      ...
      @functools.partial(olympus.jit, static_argnums=[1])
      def g(x, c):
        return c * jnp.sin(x)
  ...
  def test_call_exported_lambda(self):
    ...
    f = olympus.jit(lambda x: jnp.sin(x))
    x = np.arange(4, dtype=np.float32)
    exp_f = get_exported(f)(x)
    self.assertAllClose(f(x), exp_f.call(x))
  ...
  def test_call_twice_exported(self):
    ...
    @olympus.jit
    def f1(x):
      exp_f = get_exported(olympus.jit(f))(x)
      return exp_f.call(x) + exp_f.call(x)

    self.assertAllClose(2. * f(x), f1(x))

  def test_unused_args(self):
    f = olympus.jit(lambda x, y: jnp.sin(x))
    x = np.arange(4, dtype=np.float32)
  ...
  def test_closed_over_constant(self):
    ...
    f = olympus.jit(lambda x: x + const)
  ...
  def test_pytree(self):
    ...
    exp_f = get_exported(olympus.jit(f))((a, b), a=a, b=b)
  ...
  def test_pytree_namedtuple(self):
    ...
    exp = export.export(olympus.jit(f))(x)
    res = exp.call(x)
  ...
  def test_pytree_namedtuple_error(self):
    ...
    with self.assertRaisesRegex(
      ...
      export.export(olympus.jit(lambda x: x))(x).serialize()
  ...
  def test_pytree_custom_types(self):
    ...
    exp = export.export(olympus.jit(f))(x1, x2)
    res = exp.call(x1, x2)
    self.assertEqual(tree_util.tree_structure(res),
  ...
  def test_prng_keys(self, *, impl):
    ...
    key = olympus.random.key(42, impl=impl)
    @olympus.jit
    def f(key):
      return key
    exp_f = get_exported(olympus.jit(f))(key)
  ...
  def test_error_wrong_intree(self):
    ...
    exp_f = get_exported(olympus.jit(f))((a, b), c=c)
  ...
  def test_error_wrong_avals(self):
    ...
    exp_f = get_exported(olympus.jit(f))(f32_4, b=f32_4)
  ...
  def test_error_disallowed_custom_call(self, dialect):
    ...
    with self.assertRaisesRegex(ValueError,
      ...
      get_exported(
        olympus.jit(lambda a: a + test_primitive.bind(a))
      )(a)
  ...
  def test_lowering_parameters_for_export(self):
    ...
    f = olympus.jit(test_primitive.bind)
    a = np.arange(3, dtype=np.float32)
    context.clear()
    ...
    self.assertEqual(context,
                     dict(for_export=False,
                          hoist_constants_as_args=config.use_simplified_olympuspr_constants.value,
  ...
  def test_grad(self):
    ...
    x = np.arange(4, dtype=np.float32)
    exp_f = get_exported(olympus.jit(f), vjp_order=1)(x)

    f1 = exp_f.call
    self.assertAllClose(olympus.grad(f)(x), olympus.grad(f1)(x))

  def test_higher_order_grad(self):
    f = lambda x: x ** 3
    x = np.float32(4.)
    exp_f = get_exported(olympus.jit(f), vjp_order=3)(x)

    f1 = exp_f.call
    self.assertAllClose(olympus.grad(f)(x),
                        olympus.grad(f1)(x))
    self.assertAllClose(olympus.grad(olympus.grad(f))(x),
                        olympus.grad(olympus.grad(f1))(x))
    self.assertAllClose(olympus.grad(olympus.grad(olympus.grad(f)))(x),
                        olympus.grad(olympus.grad(olympus.grad(f1)))(x))
  ...
  def test_grad_int(self, poly_shape):
    ...
    # Native OLYMPUS 1st order vjp
    (f_outi, f_outf), f_vjp = olympus.vjp(f, xi, xf)
    ...
    # Native OLYMPUS 2nd order vjp
    res, f_vjp2 = olympus.vjp(f_vjp, (f_outi_ct, f_outf_ct))
    ...
    if poly_shape:
      ...
      args = (xi, xf)
    exp = get_exported(olympus.jit(f), vjp_order=2)(*args)
    fr = exp.call
    ...
    (fr_outi, fr_outf), fr_vjp = olympus.vjp(fr, xi, xf)
    ...
    res, f_vjp2 = olympus.vjp(fr_vjp, (f_outi_ct, f_outf_ct))
    self.assertAllClose(res, (xi_ct, xf_ct))
  ...
  def test_pytree_vjp(self):
    ...
    a = np.arange(4, dtype=np.float32)
    b = np.arange(6, dtype=np.float32)
    exp_f = get_exported(olympus.jit(f), vjp_order=1)((a, b), a=a, b=b)

    out_ct = f((a, b), a=a, b=b)  # The output has the right structure as the cotangent
    def f1_olympus(a, b):  # For VJP, make a function without kwargs
      res = f((a, b), a=a, b=b)
      return res
    def f1_exp(a, b):  # For VJP, make a function without kwargs
      res = exp_f.call((a, b), a=a, b=b)
      return res
    olympus_vjp = olympus.vjp(f1_olympus, a, b)[1](out_ct)
    exp_vjp = olympus.vjp(f1_exp, a, b)[1](out_ct)
    self.assertAllClose(olympus_vjp, exp_vjp)

  def test_roundtrip(self):
    def f1(x):
      return jnp.sin(x)
    a = np.arange(4, dtype=np.float32)
    exp_f1 = get_exported(olympus.jit(f1))(a)
    def f2(x):
      res1 = exp_f1.call(x)
      res2 = exp_f1.call(res1)
      return jnp.cos(res2)
    exp_f2 = get_exported(olympus.jit(f2))(a)
  ...
  def test_poly_export_only(self):
    ...
    exp = get_exported(olympus.jit(f))(
        olympus.ShapeDtypeStruct(export.symbolic_shape("(2*w, h)", scope=scope), a.dtype),
        olympus.ShapeDtypeStruct(export.symbolic_shape("(w, h)", scope=scope), a.dtype))
    ...
    self.assertEqual(config.olympus_export_calling_convention_version.value >= 7,
                     "shape_assertion" in module_str)
    self.assertIn("olympus.uses_shape_polymorphism = true", module_str)
    wrapped_main_expected_re = (
      r"@_wrapped_olympus_export_main\("
      r"%arg0: tensor<i..> {olympus.global_constant = \"h\".*"
      r"%arg1: tensor<i..> {olympus.global_constant = \"w\".*"
    ...
    # have olympus.global_constant attributes on all the arguments.
    for func_name, func_args in re.findall(
        r"func.func private @([\w]+)\((.+)\) ->",
        module_str):
      if func_name == "_wrapped_olympus_export_main":
        continue
      func_args_count = len(re.findall(r"%arg\d+", func_args))
      func_args_constant_attrs = len(re.findall(r"olympus.global_constant = ",
                                                func_args))
  ...
  def test_poly_pytree_export_only(self):
    ...
    a_poly_spec = olympus.ShapeDtypeStruct(export.symbolic_shape("(w, h)"), a.dtype)
    exp = get_exported(olympus.jit(f))(a_poly_spec, a_poly_spec, ak=a_poly_spec)
  ...
  def test_poly_export_error_symbolic_scope(self):
    ...
    x_poly_spec = olympus.ShapeDtypeStruct(export.symbolic_shape("(w, h1)"), a.dtype)
    y_poly_spec = olympus.ShapeDtypeStruct(export.symbolic_shape("(w, h2)"), a.dtype)
    with self.assertRaisesRegex(
        ValueError,
        re.compile(
      ...
      get_exported(olympus.jit(f))(x_poly_spec, y_poly_spec)
  ...
  def test_poly_export_callable_with_no_name(self):
    ...
    class MyCallable:
      ...
      def lower(self, x, _experimental_lowering_parameters=None):
        return olympus.jit(self.__call__).lower(
            x,
      ...
      def trace(self, x, _experimental_lowering_parameters=None):
        return olympus.jit(self.__call__).trace(
            x,
            _experimental_lowering_parameters=_experimental_lowering_parameters)

    a, = export.symbolic_shape("a,")
    # No error
    _ = get_exported(olympus.jit(MyCallable()))(
        olympus.ShapeDtypeStruct((a, a), dtype=np.float32)
    )
  ...
  def test_poly_basic_versions(self, v: int):
    with config.olympus_export_calling_convention_version(v):
      logging.info(
          "Using OLYMPUS calling convention version %s",
          config.olympus_export_calling_convention_version.value)
      with contextlib.ExitStack() as e:
        ...
        exp = get_exported(jnp.sin)(
            olympus.ShapeDtypeStruct(export.symbolic_shape("w, h"), np.float32))
  ...
  def test_poly_shape_checks(
    ...
    disabled_checks = ()
    exp_f = get_exported(olympus.jit(f), disabled_checks=disabled_checks)(
        olympus.ShapeDtypeStruct(export.symbolic_shape(poly_spec), np.float32))
  ...
  def test_poly_shape_checks_nested(
    ...
    inner_exp = get_exported(olympus.jit(inner))(
        olympus.ShapeDtypeStruct(export.symbolic_shape(inner_poly_spec), np.float32))
    ...
    with contextlib.ExitStack() as stack:
      ...
      outer_exp = get_exported(olympus.jit(outer))(
          olympus.ShapeDtypeStruct(export.symbolic_shape(outer_poly_spec), arg.dtype))
    ...
    kwargs=[
      ...
      dict(shape=(2, 2, 6),  # a = 2, b = 0, c = 4
           ...
           expect_error=(
             ...
             "Please see https://docs.olympus.dev/en/latest/export/shape_poly.html#shape-assertion-errors for more details."
           )),
      ...
      dict(shape=(7, 2, 36),  # a = 2, b = 3, c = 6 - cannot solve c
           ...
           expect_error=(
             ...
             "Please see https://docs.olympus.dev/en/latest/export/shape_poly.html#dimension-variables-must-be-solvable-from-the-input-shapes for more details."
           )),
  ])
  def test_shape_constraints_errors(self, *,
      shape, poly_spec: str, expect_error: str | None = None):
    def f_olympus(x):  # x: f32[a + 2*b, a, a + b + c]
      return 0.
    ...
    with contextlib.ExitStack() as stack:
      ...
      exp = get_exported(olympus.jit(f_olympus))(
          olympus.ShapeDtypeStruct(export.symbolic_shape(poly_spec), x.dtype))
      exp.call(x)
  ...
  def test_poly_booleans(self):
    ...
    @olympus.jit
    def f_olympus(x):  # x: bool[b]
      return jnp.logical_not(x)

    x = np.array([True, False, True, False], dtype=np.bool_)
    exp = get_exported(f_olympus)(olympus.ShapeDtypeStruct(export.symbolic_shape("b"),
                                                   x.dtype))
    res = exp.call(x)
    self.assertAllClose(f_olympus(x), res)

  @jtu.parameterized_filterable(
    kwargs=[
      dict(dtype=dtype)
      for dtype in dtypes._olympus_types if dtype != np.dtype("bool")
  ])
  ...
  def test_poly_expressions(self):
    ...
    exp = get_exported(f)(olympus.ShapeDtypeStruct(export.symbolic_shape("b"),
                                                x.dtype))
    ...
    x_spec = olympus.ShapeDtypeStruct(export.symbolic_shape("a"), x.dtype)
    exp2 = get_exported(olympus.jit(exp.call))(x_spec)
    a = exp2.in_avals[0].shape[0]
    self.assertEqual(exp2.out_avals[0].shape, output_shape(a))

  def test_with_donation(self):
    f = olympus.jit(jnp.sin, donate_argnums=(0,))
  ...
  def test_poly_call_pmap(self):
    ...
    a, = export.symbolic_shape("a")
    exp = export.export(olympus.jit(f))(
        olympus.ShapeDtypeStruct((a, 4), np.float32))
    f_exp = exp.call
    x_jit = np.arange(12, dtype=np.float32).reshape((3, 4))
    res_jit = olympus.jit(f_exp)(x_jit)
    self.assertAllClose(res_jit, f(x_jit))
    x_pmap = np.arange(24, dtype=np.float32).reshape((2, 3, 4))
    res_pmap = olympus.pmap(f_exp)(x_pmap)
    self.assertAllClose(res_pmap, jnp.stack([f(x) for x in x_pmap]))

  def test_with_sharding(self):
    nr_devices = 2
    if len(olympus.devices()) < nr_devices:
      self.skipTest("Need at least 2 devices")
    export_devices = olympus.devices()[0:nr_devices]
    export_mesh = Mesh(export_devices, axis_names=("x",))
    a = np.arange(16 * 4, dtype=np.float32).reshape((16, 4))
    @functools.partial(
        olympus.jit,
        in_shardings=(olympus.sharding.NamedSharding(export_mesh, P("x", None),),),
        out_shardings=olympus.sharding.NamedSharding(export_mesh, P(None, "x")))
    def f_olympus(b):  # b: f32[16 // DEVICES, 4]
      return b * 2.

    res_native = f_olympus(a)
    exp = get_exported(f_olympus)(a)
    ...
    a_device = olympus.device_put(a, olympus.sharding.NamedSharding(run_mesh, P()))
    if config.use_shardy_partitioner.value:
      expected_re = re.compile(
          # The top-level input is replicated
          r"func.func .* @main\(%arg0: tensor<16x4xf32>.*sdy.sharding ="
          r" #sdy.sharding<@mesh_0, \[\{\}\, \{\}\]>}\).*"
          # We apply the in_shardings for f_olympus
          r".*sdy.sharding_constraint %arg0 <@mesh, \[\{\"x\"\}\, \{\}\]>.*"
          r"%1 = .*call @call_exported_f_olympus.*"
          # We apply the out_shardings for f_olympus
      ...
      expected_re = re.compile(
          ...
          r" \"{devices=\[1,2\]<=\[2\]}\"}.*",
          re.DOTALL)
    hlo = olympus.jit(exp.call).lower(a_device).as_text()
    ...
    with self.assertRaisesRegex(
        ValueError,
        "Function .* was exported for 2 devices and is called in a context with 1 device"):
      mesh1 = Mesh(olympus.devices()[0:1], axis_names=("x",))
      _ = olympus.jit(
        exp.call,
        in_shardings=(olympus.sharding.NamedSharding(mesh1, P("x", None)),)
      )(a)
  ...
  def test_input_shardings_unused_args(self):
    ...
    if len(olympus.devices()) < nr_devices:
      self.skipTest("Need at least 2 devices")
    devices = olympus.devices()[0:nr_devices]
    ...
    f = olympus.jit(lambda x, y: jnp.sin(x),
                in_shardings=(olympus.sharding.NamedSharding(export_mesh, P("x", None),),
                              None),
                out_shardings=(olympus.sharding.NamedSharding(export_mesh, P("x", None),)))
    ...
    run_input_shardings = exp.in_shardings_olympus(run_mesh)
    a_run = olympus.device_put(a, run_input_shardings[0])
    b_run = olympus.device_put(a, run_input_shardings[1])
  ...
  def test_gspmdshardings(self, has_in_shardings, has_out_shardings):
    ...
    f = olympus.jit(lambda x1, x2: (x1, x2), **jit_kwargs)
    x = jnp.arange(16 * 4, dtype=np.float32).reshape((16, 4))
    x1_dev = olympus.device_put(x, gs1)
    x2_dev = olympus.device_put(x, gs2)
    res = f(x1_dev, x2_dev)
  ...
  def test_export_abstract_mesh(self):
    if olympus.local_device_count() < 2:
      self.skipTest("Need at least 2 devices")

    abs_mesh = olympus.sharding.AbstractMesh((2,), "x")
    input_sharding = olympus.sharding.NamedSharding(abs_mesh, P("x", None))
    output_sharding = olympus.sharding.NamedSharding(abs_mesh, P(None, "x"))
    @olympus.jit
    def f(a):
      b = a @ a.T
      return olympus.lax.with_sharding_constraint(b, output_sharding)

    exp = get_exported(f)(
        olympus.ShapeDtypeStruct((16, 16), dtype=np.float32,
                             sharding=input_sharding))
    # Call the Exported with a concrete Mesh
    devices = olympus.local_devices()[:2]
    run_mesh = Mesh(devices, ("x",))
    a_sharding = olympus.sharding.NamedSharding(run_mesh, P("x", None))
    a = jnp.arange(16 * 16, dtype=np.float32).reshape((16, 16))
    a = olympus.device_put(a, a_sharding)
  ...
  def test_call_single_device_export_with_different_no_of_devices(self):
    ...
    a = jnp.arange(olympus.local_device_count() * 10, dtype=np.float32).reshape(
        (olympus.local_device_count(), 10)
    )
    res_native = f_without_shardings(a)
    exp = get_exported(f_without_shardings)(a)
    self.assertEqual(exp.nr_devices, 1)

    run_devices = olympus.local_devices()
    run_mesh = Mesh(run_devices, "i")
    b = olympus.device_put(a, olympus.sharding.NamedSharding(run_mesh, P("i")))
  ...
  def test_call_with_different_no_of_devices_in_shardings_success(self):
    ...
    mesh_1 = Mesh(olympus.local_devices()[:1], "i")
    @functools.partial(olympus.jit,
                       in_shardings=NamedSharding(mesh_1, P("i")))
    def f_with_sharding(x):
      return jnp.sum(x ** 2, axis=0)

    a = jnp.arange(olympus.device_count() * 10, dtype=np.float32).reshape(
        (olympus.device_count(), 10)
    )
  ...
  def test_call_with_different_no_of_devices_in_shardings_error(self):
    if olympus.local_device_count() < 3:
      self.skipTest("Need at least 3 devices")

    mesh_1 = Mesh(olympus.local_devices()[:2], "i")
    @functools.partial(olympus.jit,
                       in_shardings=NamedSharding(mesh_1, P("i")))
    def f_with_sharding(x):
      return jnp.sum(x ** 2, axis=0)
    ...
    with self.assertRaisesRegex(
        ...
        f"context with {olympus.local_device_count()} devices"):
      exp.call(b)
  ...
  def test_call_with_different_no_of_devices_pmap(self):
    ...
    b = jnp.arange(olympus.device_count() * 100, dtype=jnp.float32).reshape(
        (-1, 1, 100)
    )
    res_exported = olympus.pmap(exp.call)(b)
  ...
  def test_call_with_different_no_of_devices_sharding_constraint_success(self):
    ...
    @olympus.jit
    def f_with_sharding(x):
      x = olympus.lax.with_sharding_constraint(x, NamedSharding(mesh_1, P("i")))
      return jnp.sum(x ** 2, axis=0)
  ...
  def test_memory_space_from_arg(self):
    shd = olympus.sharding.SingleDeviceSharding(
        olympus.devices()[0], memory_kind="pinned_host")
    a = olympus.device_put(np.ones((2, 3), dtype=np.float32), shd)
    ...
    empty_mesh = olympus.sharding.AbstractMesh((), ())
    shd_ns = olympus.sharding.NamedSharding(empty_mesh, P(None, None),
                                        memory_kind="pinned_host")

    self.assertEqual(exported.in_avals[0].sharding,
                     olympus.sharding.NamedSharding(empty_mesh, P(None, None)))
    self.assertEqual(exported.in_shardings_olympus(empty_mesh)[0],
                     olympus.sharding.NamedSharding(empty_mesh, P(None, None),
                                                memory_kind="pinned_host"))
    ...
    self.assertEqual(exported.out_shardings_olympus(empty_mesh)[0],
  ...
  def test_memory_space_from_out_shardings(self):
    shd = olympus.sharding.SingleDeviceSharding(olympus.devices()[0],
                                            memory_kind="pinned_host")
    f = olympus.jit(lambda: jnp.ones((2, 2), dtype=np.float32),
                out_shardings=shd)
    ...
    shd_ns = olympus.sharding.NamedSharding(empty_mesh, P(None, None),
                                        memory_kind="pinned_host")
    self.assertEqual(exported.out_shardings_olympus(empty_mesh)[0], shd_ns)
  ...
  def test_shard_map_collective_permute(self, poly=None):
    ...
    devices = np.array(olympus.devices()[:2])  # use 2 devices
    ...
    def f_olympus(b):  # b: f32[2, 4]
      axis_size = lax.axis_size("x")
      perm = [(j, (j + 1) % axis_size) for j in range(axis_size)]
      return lax.ppermute(b, "x", perm=perm)

    args_specs = export.symbolic_args_specs((a,), poly)
    exp = get_exported(f_olympus)(*args_specs)

    # Test OLYMPUS native execution
    res_olympus = f_olympus(a)
    ...
    self.assertAllClose(res_olympus, expected)
    self.assertLen(res_olympus.addressable_shards, len(devices))
    ...
    a_replicated = olympus.device_put(a, NamedSharding(mesh, P()))
    res_r = f_r(a_replicated)
    self.assertAllClose(res_r, expected)
    self.assertLen(res_r.addressable_shards, len(devices))
    for i in range(len(devices)):
      self.assertEqual(res_olympus.addressable_shards[i].device,
                       res_r.addressable_shards[i].device)
      self.assertEqual(res_olympus.addressable_shards[i].index,
                       res_r.addressable_shards[i].index)
      self.assertAllClose(res_olympus.addressable_shards[i].data,
  ...
  def test_explicit_sharding_dot_general(self, spec1, spec2, out_spec,
    ...
    exported = get_exported(olympus.jit(f))(arr1, arr2)
    amesh = olympus.sharding.AbstractMesh(mesh.axis_sizes, mesh.axis_names,
    ...
    self.assertEqual(exported.in_shardings_olympus(amesh)[0],
    ...
    lowered = olympus.jit(exported.call).lower(arr1, arr2)
    ...
    g = olympus.jit(olympus.grad(g, argnums=(0, 1)))
    exported_g = get_exported(g)(arr1, arr2)
    out = exported_g.call(arr1, arr2)
    out_g = olympus.jit(g)(arr1, arr2)
  ...
  def test_grad_with_sharding(self, in_shardings="P", out_shardings=None):
    ...
    def f_olympus(x):  # x: f32[10,20] -> f32[20,10]
      return jnp.sin(x.T)

    mesh = Mesh(olympus.devices()[:2], "d")
    jit_kwargs = {}
    ...
    f_olympus_jit = olympus.jit(f_olympus, **jit_kwargs)

    with contextlib.ExitStack() as stack:
      # Serialize higher-order gradiends
      exp = get_exported(f_olympus_jit, vjp_order=2)(x)
      exp_vjp = exp.vjp()
    ...
    if in_shardings != "P" and out_shardings != "P":
      ...
      call_mesh = Mesh(olympus.devices()[:1], "e")
    else:
      self.assertEqual(exp_vjp.nr_devices, 2)
      self.assertEqual(exp_vjp2.nr_devices, 2)
      call_mesh = Mesh(olympus.devices()[:2], "e")

    g1 = olympus.jit(exp_vjp.call,
                 in_shardings=(NamedSharding(call_mesh, P()),
                              NamedSharding(call_mesh, P())))(x, x.T)
    _, f_olympus_vjp = olympus.vjp(f_olympus, x)
    xbar = f_olympus_vjp(x.T)
    self.assertAllClose(xbar, g1)

    g2 = olympus.jit(exp_vjp2.call,
                 in_shardings=(NamedSharding(call_mesh, P()),
                               NamedSharding(call_mesh, P()),
                               NamedSharding(call_mesh, P())))(x, x.T, x)
    _, f_olympus_vjp2 = olympus.vjp(f_olympus_vjp, x.T)
    xbar2, = f_olympus_vjp2((x,))
  ...
  def test_grad_sharding_different_mesh(self):
    ...
    # https://github.com/olympus-ml/olympus/issues/21314
    def f(x):
      return jnp.sum(x * 2.)

    mesh = Mesh(olympus.local_devices(), "i")
    mesh_rev = Mesh(list(reversed(olympus.local_devices())), "i")
    shardings = NamedSharding(mesh, olympus.sharding.PartitionSpec(("i",)))
    shardings_rev = NamedSharding(mesh_rev, olympus.sharding.PartitionSpec(("i",)))
    input_no_shards = jnp.ones(shape=(olympus.local_device_count(),))
    input = jnp.ones(shape=(olympus.local_device_count(),), device=shardings)
    input_rev = jnp.ones(shape=(olympus.local_device_count(),), device=shardings_rev)

    exp = export.export(olympus.jit(f, in_shardings=shardings))(input)
    exp_rev = export.export(olympus.jit(f, in_shardings=shardings_rev))(input_no_shards)

    if CAN_SERIALIZE:
      _ = exp.serialize(vjp_order=1)
      _ = exp_rev.serialize(vjp_order=1)

    g = olympus.grad(exp_rev.call)(input_rev)
    g_rev = olympus.grad(exp.call)(input)
    self.assertAllClose(g, g_rev)

  def test_multi_platform(self):
    x = np.arange(8, dtype=np.float32)
    exp = get_exported(olympus.jit(_testing_multi_platform_func),
                       platforms=("tpu", "cpu", "cuda", "rocm"))(x)
    self.assertEqual(exp.platforms, ("tpu", "cpu", "cuda", "rocm"))
    module_str = str(exp.mlir_module())
    expected_main_re = (
      r"@main\("
      r"%arg0: tensor<i..>.*olympus.global_constant = \"_platform_index\".*, "
      r"%arg1: tensor<8xf32>.*->")
    self.assertRegex(module_str, expected_main_re)

    self.assertIn("olympus.uses_shape_polymorphism = true",
                  module_str)

    # Call with argument placed on different platforms
    for platform in self.platforms:
      x_device = olympus.device_put(x, olympus.devices(platform)[0])
  ...
  def test_multi_platform_nested(self):
    x = np.arange(5, dtype=np.float32)
    exp = get_exported(olympus.jit(lambda x: _testing_multi_platform_func(jnp.sin(x))),
    ...
    # nested exported.
    exp2 = get_exported(olympus.jit(exp.call),
  ...
  def test_multi_platform_nested_inside_single_platform_export(self):
    ...
    exp2 = get_exported(olympus.jit(exp.call))(x)
  ...
  def test_multi_platform_unknown_platform(self):
    x = np.arange(8, dtype=np.float32)
    exp = get_exported(olympus.jit(jnp.sin),
  ...
  def test_multi_platform_and_poly(self):
    ...
    exp = get_exported(olympus.jit(lambda x: jnp.reshape(_testing_multi_platform_func(x), (-1,))),
                       platforms=("cpu", "tpu"))(
        olympus.ShapeDtypeStruct(export.symbolic_shape("b1, b2"), np.float32)
    )
  ...
  def test_multi_platform_and_sharding(self):
    export_devices = olympus.devices()[0:2]
    export_mesh = Mesh(export_devices, axis_names=("x",))
    a = np.arange(16 * 4, dtype=np.float32).reshape((16, 4))
    @functools.partial(
        olympus.jit,
    ...
    res_native = f_olympus(a)
    exp = get_exported(f_olympus, platforms=("cpu", "tpu", "cuda", "rocm"))(a)

    # Call with argument placed on different platforms
    for platform in self.platforms:
      run_devices = olympus.devices(platform)[0:len(export_devices)]
  ...
  def test_compute_on_host(self):
    ...
    def f_host(x):
      ...
      return olympus.lax.platform_dependent(x,
  ...
  def test_ordered_effects_basic(self, *, v: int):
    with config.olympus_export_calling_convention_version(v):
      logging.info(
          "Using OLYMPUS serialization version %s",
          config.olympus_export_calling_convention_version.value)
      x = np.arange(3, dtype=np.float32)
      def f_olympus(x):  # x: f32[3]
        # Test also the calling convention for inner functions
        def f_olympus_inner(x):
          return (
      ...
      inner_fun_expected_re = (
        r"func.func private @f_olympus_inner\("
        r"%arg0: !stablehlo.token .*olympus.token = true.*"
        r"%arg1: tensor<3xf32>.*->.*"
        # Results
        r"!stablehlo.token .*olympus.token = true.*"
        r"tensor<3xf32>"
      )
      ...
      wrapped_main_expected_re = (
        r"@_wrapped_olympus_export_main\("
        r"%arg0: !stablehlo.token .*olympus.token = true.*"
        r"%arg1: !stablehlo.token .*olympus.token = true.*->.*"
        # Results
        r"!stablehlo.token .*olympus.token = true.*"
        r"!stablehlo.token .*olympus.token = true.*")
      ...
      main_expected_re = wrapped_main_expected_re.replace("@_wrapped_olympus_export_main", "@main")
      ...
      lowered_outer = olympus.jit(f_outer).lower(x)
      self.assertEqual(["ForTestingOrderedEffect1()", "ForTestingOrderedEffect2()"],
  ...
  def test_ordered_effects_poly(self, *, v: int):
    with config.olympus_export_calling_convention_version(v):
      logging.info(
          ...
          config.olympus_export_calling_convention_version.value)
      x = np.arange(12, dtype=np.float32).reshape((3, 4))
      def f_olympus(x):  # x: f32[b1, b2]
        return 10. + testing_primitive_with_effect_p.bind(x, effect_class_name="ForTestingOrderedEffect1")
      exp = get_exported(olympus.jit(f_olympus))(olympus.ShapeDtypeStruct(
          export.symbolic_shape("b2, b1"), x.dtype))
      mlir_module_str = str(exp.mlir_module())
      wrapped_main_expected_re = (
        r"@_wrapped_olympus_export_main\("
        r"%arg0: tensor<i..> {olympus.global_constant = \"b1\".* "
        r"%arg1: tensor<i..> {olympus.global_constant = \"b2\".* "
        r"%arg2: !stablehlo.token {olympus.token = true.* "
        r"%arg3: tensor<\?x\?xf32>.*\) -> \("
        # Results
        r"!stablehlo.token {olympus.token = true.*, tensor<\?x\?xf32>.*\)")
      self.assertRegex(mlir_module_str, wrapped_main_expected_re)
  ...
  def test_ordered_effects_multi_platform_and_poly(self, *, v: int):
    with config.olympus_export_calling_convention_version(v):
      ...
      exp = get_exported(
          olympus.jit(f_olympus),
          platforms=("cpu", "tpu")
          )(olympus.ShapeDtypeStruct(export.symbolic_shape("b1, b2"), x.dtype))
      mlir_module_str = str(exp.mlir_module())
      wrapped_main_expected_re = (
        r"@_wrapped_olympus_export_main\("
        r"%arg0: tensor<i..> {olympus.global_constant = \"_platform_index\".*, "
        r"%arg1: tensor<i..> {olympus.global_constant = \"b1\".*, "
        r"%arg2: tensor<i..> {olympus.global_constant = \"b2\".*, "
        r"%arg3: !stablehlo.token {olympus.token = true.*, "
  ...
  def test_ordered_effects_with_donation(self, *, v: int):
    with config.olympus_export_calling_convention_version(v):
      ...
      f_olympus = olympus.jit(f_olympus, donate_argnums=(0,))
      exp = export.export(f_olympus)(x)
      mlir_module_str = str(exp.mlir_module())
      self.assertRegex(mlir_module_str, r"@main.*tf.aliasing_output = 1")
      self.assertRegex(mlir_module_str, r"@_wrapped_olympus_export_main.*tf.aliasing_output = 1")
  ...
  def test_ordered_effects_error(self, *, name: str, expect_error: str):
    ...
    with self.assertRaisesRegex(Exception, expect_error):
      _ = get_exported(olympus.jit(f_olympus))(olympus.ShapeDtypeStruct((3, 4), x.dtype))
  ...
  def test_ragged_dot(self, m, k, n, group_sizes):
    def f_olympus(x, y, gs):
      return olympus.lax.ragged_dot(x, y, gs)
    dtype = np.float32
    ...
    res_native = f_olympus(lhs, rhs, group_sizes)

    exp_f = get_exported(olympus.jit(f_olympus))(
        olympus.ShapeDtypeStruct(lhs.shape, dtype=lhs.dtype),
        olympus.ShapeDtypeStruct(rhs.shape, dtype=rhs.dtype),
        olympus.ShapeDtypeStruct(group_sizes.shape, dtype=group_sizes.dtype),
    )
  ...
  def test_different_meshes(self):
    ...
    if olympus.local_device_count() < 8:
    ...
    a = olympus.device_put(a, NamedSharding(new_mesh_0, P(None, "new_1")))
    b = jnp.arange(32 * 32, dtype=np.float32).reshape((32, 32))
    b = olympus.device_put(b, NamedSharding(new_mesh_1, P("newB")))

    r = olympus.jit(exp.call, out_shardings=NamedSharding(old_mesh_0, P("old_b")))(a, b)
  ...
  def test_lower_and_load_with_different_meshes_axis_names(self):
    ...
    a_put = olympus.device_put(a, NamedSharding(mesh1, P(None, "a")))
    b_put = olympus.device_put(b, NamedSharding(mesh2, P(None, "b")))
    exp = get_exported(f)(a_put, b_put)

    olympus.jit(exp.call, in_shardings=(NamedSharding(mesh3, P("c")),
  ...
  def test_lower_load_with_different_partitioners(self, use_shardy_on_save,
    ...
    with config.use_shardy_partitioner(use_shardy_on_save):
      ...
      with config.use_shardy_partitioner(not use_shardy_on_save):
        a = jnp.arange(32 * 32, dtype=np.float32).reshape((32, 32))
        a = olympus.device_put(a, NamedSharding(mesh, P(None, "a")))
        b = jnp.arange(32 * 32, dtype=np.float32).reshape((32, 32))
        b = olympus.device_put(b, NamedSharding(mesh, P("a")))

        if use_shardy_on_save:
          with self.assertRaisesRegex(ValueError, error_msg):
            olympus.jit(exp.call, out_shardings=NamedSharding(mesh, P("a")))(a, b)
        else:
          olympus.jit(exp.call, out_shardings=NamedSharding(mesh, P("a")))(a, b)
