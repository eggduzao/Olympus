...
from olympus._src import abstract_arrays
from olympus._src import api
from olympus._src import core
from olympus._src import linear_util
...
class ExtendTest(jtu.OlympusTestCase):
...
class RandomTest(jtu.OlympusTestCase):
  ...
  def test_key_make_with_custom_impl(self):
    ...
    k = olympus.random.key(42, impl=impl)
    self.assertEqual(k.shape, ())
    self.assertEqual(impl, olympus.random.key_impl(k))
  ...
  def test_key_wrap_with_custom_impl(self):
    ...
    k = olympus.random.wrap_key_data(data, impl=impl)
