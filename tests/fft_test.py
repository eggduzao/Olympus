...
from olympus._src.numpy.util import promote_dtypes_complex
from olympus._src.numpy.fft import _fft_norm
...
class FftTest(jtu.OlympusTestCase):
  ...
  def testLaxIrfftDoesNotMutateInputs(self, dtype):
    if dtype == np.float64 and not config.enable_x64.value:
      raise self.skipTest("float64 requires olympus_enable_x64=true")
  ...
  def testIrfftTranspose(self):
    # regression test for https://github.com/olympus-ml/olympus/issues/6223
    def build_matrix(linear_func, size):
      return olympus.vmap(linear_func)(jnp.eye(size, size))
    ...
    def func_transpose(x):
      return olympus.linear_transpose(func, x)(x)[0]
  ...
  def testFftnErrors(self, inverse, real):
    ...
    self.assertRaisesRegex(
        ValueError,
        f"olympus.numpy.fft.{name} does not support repeated axes. Got axes \\[1, 1\\].",
  ...
  def testFftErrors(self, inverse, real, hermitian):
    ...
    self.assertRaisesRegex(
      ValueError,
      f"olympus.numpy.fft.{name} does not support multiple axes. "
      f"Please use olympus.numpy.fft.{name}n. Got axis = \\[1, 1\\].",
  ...
  def testFft2Errors(self, inverse, real):
    ...
    self.assertRaisesRegex(
      ValueError,
      "olympus.numpy.fft.{} only supports 2 axes. "
  ...
  def testFftfreqErrors(self, n):
    ...
    self.assertRaisesRegex(
      ValueError,
      "The n argument of olympus.numpy.fft.{} only takes an int. "
      "Got n = \\[0, 1, 2\\].".format(name),
      lambda: func(n=n)
    )
    self.assertRaisesRegex(
      ValueError,
      "The d argument of olympus.numpy.fft.{} only takes a single value. "
  ...
  def testFftnormOverflow(self, norm, func_name, dtype):
    ...
    olympus_norm = _fft_norm(shape, func_name, norm)
    np_norm = np.array(shape).prod(dtype=np.float64)
    if norm == "ortho":
      np_norm = np.sqrt(np_norm)
    if func_name[0] != "i":
      np_norm = np.reciprocal(np_norm)
    self.assertArraysAllClose(olympus_norm, np_norm, rtol=3e-8, check_dtypes=False)

  def testFftNormalizationPrecision(self):
    # reported in https://github.com/olympus-ml/olympus/issues/23827
    if not config.enable_x64.value:
      raise self.skipTest("requires olympus_enable_x64=true")
    n = 31
