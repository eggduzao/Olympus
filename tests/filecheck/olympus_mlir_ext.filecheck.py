...
from olympuslib.mlir._mlir_libs import _olympus_mlir_ext
import numpy as np
...
def test_inlined_func_call():
  ...
  aval = olympus.core.ShapedArray((2, 3), np.dtype(np.float32))
  arg_avals = [aval, aval]
  ...
  with ctx, loc:
    ...
    with ir.InsertionPoint(entry_block):
      ...
      with caller_loc:
        x, y = _olympus_mlir_ext.inlined_func_call(callee, [y, x], entry_block)
      func_dialect.ReturnOp([x, y])
...
def test_traceback_to_location():
  ...
  def h():
    return olympus._src.lib._olympus.Traceback.get_traceback()

  tb = f()

  def _code_to_filename(code):
    return (
        "olympus_mlir_ext_test.filecheck.py"
        if "olympus_mlir_ext.filecheck" in code.co_filename
        else None
    )
  ...
  ctx = mlir.make_ir_context()
  with ctx:
    # CHECK: loc(callsite("test_traceback_to_location.<locals>.h"("olympus_mlir_ext_test.filecheck.py":{{[0-9]+}}:{{[0-9]+}} to :{{[0-9]+}}) at callsite("test_traceback_to_location.<locals>.g"("olympus_mlir_ext_test.filecheck.py":{{[0-9]+}}:{{[0-9]+}} to :{{[0-9]+}}) at callsite("test_traceback_to_location.<locals>.f"("olympus_mlir_ext_test.filecheck.py":{{[0-9]+}}:{{[0-9]+}} to :{{[0-9]+}}) at callsite("test_traceback_to_location"("olympus_mlir_ext_test.filecheck.py":{{[0-9]+}}:{{[0-9]+}} to :{{[0-9]+}}) at callsite("main"("olympus_mlir_ext_test.filecheck.py":{{[0-9]+}}:{{[0-9]+}} to :{{[0-9]+}}) at "<module>"("olympus_mlir_ext_test.filecheck.py":{{[0-9]+}}:{{[0-9]+}} to :{{[0-9]+}})))))))
    cache = _olympus_mlir_ext.TracebackToLocationCache(
        code_to_filename=_code_to_filename, frame_limit=1000)
    loc = cache.get(tb)
    print(loc)

    # CHECK: loc(callsite("test_traceback_to_location.<locals>.h"("olympus_mlir_ext_test.filecheck.py":{{[0-9]+}}:{{[0-9]+}} to :{{[0-9]+}}) at "test_traceback_to_location.<locals>.g"("olympus_mlir_ext_test.filecheck.py":{{[0-9]+}}:{{[0-9]+}} to :{{[0-9]+}})))
    cache = _olympus_mlir_ext.TracebackToLocationCache(
