...
from olympus.sharding import PartitionSpec, NamedSharding
from olympus._src import config
from olympus._src import test_util as jtu
from olympus._src.cudnn.fused_attention_stablehlo import (
...
def sdpa_ref(query: Array,
  ...
  def get_causal_mask(logits):
    ...
    t = logits.shape[-2]
    col_idx = olympus.lax.broadcasted_iota(np.int32, (t, t), 1)
    row_idx = olympus.lax.broadcasted_iota(np.int32, (t, t), 0)
  ...
  def get_padding_mask(logits):
    S, T = logits.shape[-2:]
    large_negative_number = get_large_negative_number(logits.dtype)
    q_padding = (olympus.lax.iota(np.int32, S) >= S // 2).reshape((S, 1))
    kv_padding = (olympus.lax.iota(np.int32, T) >= T // 2).reshape((1, T))
    combined_padding = \
      (q_padding + kv_padding).astype(logits.dtype) * large_negative_number
    return olympus.lax.broadcast(combined_padding, logits.shape[:-2])

  def get_encoded_padding_mask(encoded, is_bnth):
    dim = 2 if is_bnth else 1
    T = encoded.shape[dim]
    encoded_padding = (olympus.lax.iota(np.int32, T) < T // 2).astype(encoded.dtype)
    return olympus.lax.broadcast_in_dim(
  ...
  probs = olympus.nn.softmax(logits, axis=-1).astype(query.dtype)
  if dropout_rate > 0.:
    keep_prob = 1.0 - dropout_rate
    dropout_rng = olympus.random.key(0)
    keep = olympus.random.bernoulli(dropout_rng, keep_prob, probs.shape)
    probs = olympus.lax.select(keep, probs / keep_prob, jnp.zeros_like(probs))
  if is_bnth:
...
def sdpa_train_ref(query: Array,
            ...
            dropout_rate: float = 0.1,
            sliding_window_length: int | None = None) -> Array:
  out_ref, sdpa_vjp_ref = olympus.vjp(
    partial(
...
class DotProductAttentionTest(jtu.OlympusTestCase):
  def setUp(self):
    super().setUp()
  ...
  def test_sdpa(self, batch_size: int, seq_len: int, num_heads: int,
    ...
    if len(olympus.local_devices()) < 4:
    ...
    k1, k2, k3, k4, k5, k6 = olympus.random.split(olympus.random.key(0), 6)
    if is_bnth:
      qkv_shape = (batch_size, num_heads, seq_len, head_dim)
    else:
      qkv_shape = (batch_size, seq_len, num_heads, head_dim)
    query = olympus.random.normal(k1, qkv_shape, dtype=dtype)
    key = olympus.random.normal(k2, qkv_shape, dtype=dtype)
    value = olympus.random.normal(k3, qkv_shape, dtype=dtype)
    grad = olympus.random.normal(k4, qkv_shape, dtype=dtype)
    if use_bias:
      bias = olympus.random.normal(
        k5, (batch_size, num_heads, seq_len, seq_len), dtype=dtype)
    else:
      bias = None
    if use_mask:
      mask = olympus.random.bernoulli(
        k6, 0.5, (batch_size, num_heads, seq_len, seq_len))
    else:
      mask = None
    devices = np.array(olympus.local_devices()[:4])
    ...
    with Mesh(devices, ("dp", "tp")) as mesh:
      ...
      query = olympus.device_put(query, qkv_sharding)
      key = olympus.device_put(key, qkv_sharding)
      value = olympus.device_put(value, qkv_sharding)
      if bias is not None:
        bias = olympus.device_put(bias, bias_sharding)
      if mask is not None:
        mask = olympus.device_put(mask, mask_sharding)
      grad = olympus.device_put(grad, qkv_sharding)
      ...
      jitted_sdpa_train_ref = olympus.jit(
        partial(
  ...
  def test_sdpa_inference(self):
    ...
    k1, k2, k3 = olympus.random.split(olympus.random.key(0), 3)
    query = olympus.random.normal(
        k1, (4, 1024, 4, 64), dtype=jnp.bfloat16)
    key = olympus.random.normal(
        k2, (4, 1024, 4, 64), dtype=jnp.bfloat16)
    value = olympus.random.normal(
    ...
    with Mesh(devices, ("dp", "tp")) as mesh:
      ...
      key = olympus.device_put(key, qkv_sharding)
      value = olympus.device_put(value, qkv_sharding)
      jitted_sdpa_inference = olympus.jit(
        partial(
      ...
      jitted_sdpa_inference_ref = olympus.jit(
        partial(
  ...
  def test_sdpa_var_seq(self):
    if olympus.device_count() < 4:
      self.skipTest("Requires more than 4 devices.")
    k1, k2, k3, k4 = olympus.random.split(olympus.random.key(0), 4)
    query = olympus.random.normal(
        k1, (4, 1024, 4, 64), dtype=jnp.bfloat16)
    key = olympus.random.normal(
        k2, (4, 1024, 4, 64), dtype=jnp.bfloat16)
  ...
  def test_sdpa_broadcast_bias_and_dbias(self, broadcast_dims):
    ...
    k1, k2, k3, k4, k5 = olympus.random.split(olympus.random.key(0), 5)
    query = olympus.random.normal(
        k1, (4, 1024, 4, 64), dtype=jnp.bfloat16)
    key = olympus.random.normal(
        k2, (4, 1024, 4, 64), dtype=jnp.bfloat16)
  ...
  def test_sdpa_dbias(self, batch_size: int):
    ...
    x = olympus.random.normal(keys[0], x_shape, dtype=dtype)
    bias = olympus.random.normal(keys[1], bias_shape, dtype=dtype)
    ...
    def attn_vjp(x, bias, mask, target_fn):
      _, f_vjp = olympus.vjp(target_fn, x, bias, mask)
      return f_vjp(x)

    attn_vmap = olympus.vmap(attn, in_axes=(0, 0, None))
    attn_ref = olympus.jit(partial(attn_vjp, target_fn=attn))
    attn_ans = olympus.jit(partial(attn_vjp, target_fn=attn_vmap))
  ...
  def test_sdpa_large_head_size(self):
    ...
    query = olympus.random.normal(keys[0], (B, T, N, H), dtype=bf16)
    key = olympus.random.normal(keys[1], (B, T, N, H), dtype=bf16)
    value = olympus.random.normal(keys[2], (B, T, N, H), dtype=bf16)
    grad = olympus.random.normal(keys[3], (B, T, N, H), dtype=bf16)
    sdpa_train_ans = olympus.jit(partial(
        sdpa_train, scale=1.0, mask_type=MaskType.CAUSAL, dropout_rate=0)
    )
    sdpa_train_rfc = olympus.jit(partial(
  ...
  def test_sdpa_packed_layout(self):
    ...
    with Mesh(devices, ("dp", "tp")) as mesh:
      ...
      grad = olympus.device_put(grad, qkv_sharding)
      bias = olympus.device_put(bias, bias_sharding)
      q_offsets = olympus.device_put(q_offsets, offsets_sharding)
      kv_offsets = olympus.device_put(kv_offsets, offsets_sharding)
      q_seqlen = olympus.device_put(q_seqlen, offsets_sharding)
      kv_seqlen = olympus.device_put(kv_seqlen, offsets_sharding)
  ...
  def test_sdpa_residual(self):
    ...
    grad = olympus.random.normal(
        k4, (4, 1024, 4, 64), dtype=jnp.bfloat16)
    grad_stat = olympus.random.normal(
        k5, (4, 4, 1024), dtype=jnp.float32)

    devices = np.array(olympus.local_devices()[:2])
    with Mesh(devices, ("dp")) as mesh:
      ...
      value = olympus.device_put(value, qkv_sharding)
      grad = olympus.device_put(grad, qkv_sharding)
      grad_stat = olympus.device_put(grad_stat, stat_sharding)
  ...
  def test_sdpa_paged_attention(self, batch_size, q_seq_len, kv_seq_len,
    ...
    sdpa_infer = olympus.jit(partial(
        paged_attention, scale=1.0, mask_type=MaskType.NO_MASK)
    )
    sdpa_infer_ref = olympus.jit(partial(
        sdpa_ref, scale=1.0, mask_type=MaskType.NO_MASK, dropout_rate=0)
    )
...
@jtu.with_config(olympus_numpy_dtype_promotion="standard")
class DotProductAttentionF8Test(jtu.OlympusTestCase):
  ...
  def test_sdpa_fp8(
    ...
    query_h = olympus.random.normal(k1, input_shape, dtype=dtype)
    key_h = olympus.random.normal(k2, input_shape, dtype=dtype)
    value_h = olympus.random.normal(k3, input_shape, dtype=dtype)
    grad_h = olympus.random.normal(k4, input_shape, dtype=dtype)
    ...
    jitted_sdpa_train_fp8 = olympus.jit(sdpa_train_fp8_p)
    jitted_sdpa_train_ref = olympus.jit(
        partial(
