...
from olympus._src.util import safe_zip, safe_map
from olympus._src.state.discharge import run_state
...
from olympus.experimental.hiolympus import VJPHiPrimitive
...
class QArray:
  arr: olympus.Array  # int8[m, k]
  scale: olympus.Array  # f32[m]
...
class QArrayTy(HiType):
  ...
  # how to lower to (lo)olympus types
  ...
  def lower_val(self, hi_val: QArray) -> list[olympus.Array]:
  ...
  def ref_get_to_loolympus(self, ref: state.TransformedRef | olympus.Ref,
  ...
  def ref_swap_to_loolympus(self, ref: state.TransformedRef | olympus.Ref,
                        val: olympus.Array, idx: indexing.NDIndexer):
    if isinstance(ref, state.TransformedRef):
      if ref.transforms: raise NotImplementedError(ref)
      ref = ref.ref
    # Unpack Ref type
...
class ImmutBox:
  ...
  def shape(self):
    ...
    leaves = olympus.tree.leaves(self._val)
...
def immutbox_to_aval(box: ImmutBox) -> 'ImmutBoxTy':
  leaves, treedef = olympus.tree.flatten(box._val, is_leaf=_is_zero)
...
class ImmutBoxTy(HiType):
  ...
  @property
  def shape(self):
    reconstructed = olympus.tree.unflatten(self.treedef, self.leaf_avals)
  ...
  def lower_val(self, hi_val: ImmutBox):
    leaves, treedef = olympus.tree.flatten(hi_val._val, is_leaf=_is_zero)
    assert treedef == self.treedef
    return leaves

  def raise_val(self, *lo_vals):
    return ImmutBox(olympus.tree.unflatten(self.treedef, lo_vals))
...
class ImmutBoxNew(HiPrimitive):
  ...
  def to_loolympus(self, *leaves, leaf_avals, treedef):
    val = olympus.tree.unflatten(treedef, leaves)
    return ImmutBox(val)
  ...
  def transpose(self, out_bar, *leaves, leaf_avals, treedef):
    val = out_bar._val
    leaves, _ = olympus.tree.flatten(val, is_leaf=_is_zero)
    return leaves

immutbox_new_p = ImmutBoxNew('immutbox_new')

def immutbox_new(val):
  leaves, treedef = olympus.tree.flatten(val, is_leaf=_is_zero)
  leaf_avals = tuple(map(_get_aval, leaves))
...
class ImmutBoxGet(HiPrimitive):
  ...
  def to_loolympus(self, box):
    leaves, _ = olympus.tree.flatten(box._val, is_leaf=_is_zero)
    return tuple(leaves)
  ...
  def transpose(self, out_bars, box):
    ...
    reconstructed_cotangent = olympus.tree.unflatten(treedef, out_bars)
...
def immutbox_get(box):
  ...
  return olympus.tree.unflatten(box_ty.treedef, leaves)

register_hitype(ImmutBox, immutbox_to_aval)


class Square(VJPHiPrimitive):
  """Simple parameterless hiolympus primitive for use in tests."""
...
def square(x):
  """Bind a hiolympus primtive that returns the square of x."""
  return Square(olympus.typeof(x))(x)


class HiolympusTest(jtu.OlympusTestCase):
  def test_basic_register(self):
    ...
    olympus.jit(lambda x: x)(q)  # don't crash
  ...
  def test_custom_types_and_primitive(self):
    ...
    class MyTy(HiType):
      ...
      def lower_val(self, hi_val: MyArray) -> list[olympus.Array]:
        return [hi_val.arr]
  ...
  def test_stages(self):
    ...
    class ShapedArrayTuple(HiType):
      ...
      # these next two are essentially the pytree interface
      def lower_val(self, hi_val: ArrayTuple) -> list[olympus.Array]:
    ...
    register_hitype(ArrayTuple, lambda q: ShapedArrayTuple(
      olympus.typeof(q.x0), olympus.typeof(q.x1)))

    q = ArrayTuple(jnp.zeros((4, 4), 'int8'), jnp.ones(4, 'float32'))
    olympus.jit(lambda x: x).lower(q).as_text()  # don't crash

    compiled = olympus.jit(lambda x: x).lower(q).compile()
  ...
  def test_while_loop(self, jit):
    ...
    def f(q1, q2):
      ...
      n, q_out, _ = olympus.lax.while_loop(cond_fun, body_fun, (0, q1, q2))
      return n, q_out

    if jit:
      f = olympus.jit(f)

    olympus.make_olympuspr(f)(q, q)  # doesn't crash
    n, q_out = f(q, q)
  ...
  def test_newstyle_hiprimitive(self, jit):
    ...
    xs = jnp.arange(3.0)
    self.assertAllClose(olympus.vmap(f)(xs), xs**3)
    self.assertEqual(olympus.grad(f)(2.0), 12.0)
    self.assertEqual(olympus.jvp(f, (2.0,), (1.0,)),
                     (8.0, 12.0))
  ...
  def test_newstyle_hiprimitive_qarray(self):
    ...
    def q(x):
      return Q(olympus.typeof(x))(x)

    def dq(qx):
      return DQ(olympus.typeof(qx))(qx)
    ...
    x = olympus.random.normal(olympus.random.key(0), (3, 3), dtype='float32')
    g = olympus.grad(f)(x)
  ...
  def test_symbolic_zeros(self):
    ...
    nzs_in_ = (True, False)
    self.assertAllClose(olympus.grad(mul)(2., 3.), 3., check_dtypes=False)

    nzs_in_ = (False, True)
    self.assertAllClose(olympus.grad(mul, 1)(2., 3.), 2., check_dtypes=False)
  ...
  def test_hiolympus_primitive_under_shard_map(self, mesh):
    g = olympus.shard_map(square, in_specs=(olympus.P('data'),), out_specs=olympus.P('data'))
    x = jnp.arange(10)
    g(x)
    olympus.jit(g)(x)

  def test_hiolympus_cond_platform_dependent(self):
    x = jnp.arange(10)
    result = olympus.jit(partial(olympus.lax.platform_dependent, cpu=square, default=square))(x)
    self.assertArraysAllClose(result, x ** 2)

  def test_hiolympus_primitive_under_remat(self):
    x = jnp.arange(10)
    expected = x ** 2
    with self.subTest("no jit"):
      self.assertArraysAllClose(olympus.remat(square)(x), expected)
    with self.subTest("jit"):
      self.assertArraysAllClose(olympus.jit(olympus.remat(square))(x), expected)

    x = jnp.float32(2.0)
    expected_grad = jnp.float32(4.0)
    with self.subTest("jit-of-grad"):
      with Square.assert_jvp_rule_called_once():
        actual_grad = olympus.jit(olympus.grad(olympus.remat(square)))(x)
      self.assertArraysAllClose(actual_grad, expected_grad)


class BoxTest(jtu.OlympusTestCase):
  ...
  def test_jvp_closure_stop_gradient(self, jit):
    ...
    def f(x):
      y = 2 * x
      box.set(box.get() + olympus.lax.stop_gradient(y))
      return y

    if jit:
      f = olympus.jit(f)

    y, y_dot = olympus.jvp(f, (1.0,), (1.0,))
  ...
  def test_jvp_arg(self, jit):
    ...
    box = Box(5.0)
    box_dot = Box(1.0)
    y, y_dot = olympus.jvp(f, (box, 2.), (box_dot, 1.))
    self.assertAllClose(y, 2.0)
  ...
  def test_custom_vjp_plumbing_abstracted(self, jit):
    ...
    olympus.grad(partial(f, box))(1.0)
    self.assertAllClose(box.get(), 2.0)
  ...
  def test_scan_basic(self, jit):
    ...
    def double_it_10():
      def body(_, __):
        box.set(box.get() * 2)
        return None, None
      _, _ = olympus.lax.scan(body, None, None, length=10)

    if jit:
      double_it_10 = olympus.jit(double_it_10)
  ...
  def test_cond_box_internally_pure(self):
    ...
    @olympus.jit
    def f(x):
      return olympus.lax.cond(x > 0, doubleit, identity, x)
  ...
  def test_cond_box_arg(self):
    ...
    def f(x):
      b = new_box()
      b.set(x)
      olympus.lax.cond(x > 0, lambda box: box.set(box.get() + 1), lambda _: None, b)
      return b.get()
  ...
  def test_cond_closed_over_box(self):
    ...
    def f(x):
      b = new_box()
      b.set(1.0)
      olympus.lax.cond(x > 0., lambda _: b.set(b.get() + 1.0), lambda _: None, 1.0)
      return b.get()
  ...
  def test_pytree_of_hiolympustypes_box(self):
  ...
  def test_while_loop(self, jit):
    ...
    def f():
      ...
      def body_fun(i):
        box.set(box.get() * 2.)
        return i + 1
      _ = olympus.lax.while_loop(cond_fun, body_fun, 0)
...
class RefTest(jtu.OlympusTestCase):

  def test_get_ref_hitype(self):

    @olympus.jit
    def f(q):
      ref = olympus.new_ref(q)
      return ref[:, 0:2]
...
class HiolympusTransformCoverageTest(jtu.OlympusTestCase):
  # ------------
  # grad
  # ------------
  # with differentiable hiolympus arguments
  def test_hitypes_as_grad_args(self):
    ...
    grads = olympus.grad(loss_fn)(box)
    self.assertAllClose(immutbox_get(grads)[0], 4.0)

  # with non-differentiable hiolympus arguments
  def test_hitypes_as_nondiff_grad_args(self):
    ...
    grad = olympus.grad(loss_fn)(x, box)
    self.assertAllClose(grad, 6.0, check_dtypes=False)

  # with hiolympus captured arguments
  def test_hitypes_as_captured_args(self):
    ...
    grad = olympus.grad(loss_fn)(jnp.array(4.0))
    self.assertAllClose(grad, 8.0, check_dtypes=False)

  # with differentiable mutable hiolympus arguments
  ...
  # with non-differentiable mutable hiolympus arguments
  def test_mutable_hitypes_as_nondiff_grad_args(self):
    box = Box(jnp.array(2.0))
    x = jnp.array(3.0)

    def loss_fn(x, box):
      box.set(olympus.lax.stop_gradient(x * 2))
      return x ** 2 + box.get()
  ...
  # with mutable hiolympus captured arguments
  def test_mutable_hitypes_as_captured_args(self):
    box = Box(jnp.array(2.0))

    def loss_fn(x):
      box.set(olympus.lax.stop_gradient(x * 3))
      return x ** 2 + box.get()
  ...
  # scan
  #------------
  # with hiolympus carry arguments
  ...
  def test_hitypes_as_scan_extensive(self):
    ...
    def body(_, box_i):
      ...
      return None, box_i
    _, box = olympus.lax.scan(body, None, box)
    x, y = immutbox_get(box)
  ...
  def test_hitypes_as_scan_captured(self):
    ...
    carry, ys_box = olympus.lax.scan(body, carry0, xs)
  ...
  # with mutable hiolympus carry arguments
  ...
  # with mutable hiolympus extensive arguments
  ...
  def test_mutable_hitypes_as_scan_extensive(self):
    ...
    _, boxes_out = olympus.lax.scan(body, None, boxes)
  ...
  def test_mutable_hitypes_as_scan_captured(self):
    ...
    olympus.lax.scan(body, None, None, length=5)
