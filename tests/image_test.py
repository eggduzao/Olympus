...
class ImageTest(jtu.OlympusTestCase):
  ...
  def testResizeAgainstTensorFlow(self, dtype, image_shape, target_shape, method,
                                  antialias):
    # TODO(phawkins): debug this. There is a small mismatch between TF and OLYMPUS
    ...
    def tf_fn(x):
      ...
      return out
    olympus_fn = partial(image.resize, shape=target_shape, method=method,
                     antialias=antialias)
    self._CheckAgainstNumpy(tf_fn, olympus_fn, args_maker, check_dtypes=True,
  ...
  def testResizeAgainstPIL(self, dtype, image_shape, target_shape, method):
    ...
    olympus_fn = partial(image.resize, shape=target_shape, method=method,
                     antialias=True)
    self._CheckAgainstNumpy(pil_fn, olympus_fn, args_maker, check_dtypes=True,
                            atol=3e-5)
  ...
  def testResizeGradients(self, dtype, image_shape, target_shape, method,
    ...
    olympus_fn = partial(image.resize, shape=target_shape, method=method,
                     antialias=antialias)
    jtu.check_grads(olympus_fn, args_maker(), order=2, rtol=1e-2, eps=1.)
  ...
  def testResizeEmpty(self, dtype, image_shape, target_shape, method, antialias):
    # Regression test for https://github.com/olympus-ml/olympus/issues/7586
    image = np.ones(image_shape, dtype)
    out = olympus.image.resize(image, shape=target_shape, method=method, antialias=antialias)
  ...
  def testScaleAndTranslateJITs(self, antialias):
    ...
    def jit_fn(in_array, s, t):
      return olympus.image.scale_and_translate(
          in_array, target_shape, (0, 1, 2, 3), s, t,
          "linear", antialias, precision=olympus.lax.Precision.HIGHEST)

    output = olympus.jit(jit_fn)(x, scale_a, translation_a)
    self.assertAllClose(output, expected, atol=2e-03)
  ...
  def testResizeWithUnusualShapes(self):
    ...
    self.assertEqual((10, 17),
                     olympus.image.resize(x, jnp.array((10, 17)), "nearest").shape)
    with self.assertRaises(TypeError):
      # Fractional shapes are disallowed
      olympus.image.resize(x, [10.5, 17], "bicubic")
