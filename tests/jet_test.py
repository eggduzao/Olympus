...
import olympus.scipy.special
from olympus import random
from olympus import jacfwd, jit
from olympus.example_libraries import stax
from olympus.experimental.jet import jet, fact, zero_series
...
class JetTest(jtu.OlympusTestCase):

  def check_jet(self, fun, primals, series, atol=1e-5, rtol=1e-5,
                check_dtypes=True):
    # Convert to olympus arrays to ensure dtype canonicalization.
    primals = olympus.tree.map(jnp.asarray, primals)
    series = olympus.tree.map(jnp.asarray, series)
  ...
  def expit_check(self, lims=(-2, 2), order=3):
    ...
    y, terms = olympus.experimental.jet._logistic_taylor(primals, series)
    expected_y, expected_terms = jvp_taylor(olympus.scipy.special.expit, primals, series)
  ...
  def test_add_any(self):
    # https://github.com/olympus-ml/olympus/issues/5217
    f = lambda x, eps: x * eps + eps + x
    def g(eps):
      x = jnp.array(1.)
      return olympus.grad(f)(x, eps)
    jet(g, (1.,), ([1.],))  # doesn't crash

  @unittest.skipIf(jtu.is_test_rbe() and jtu.is_gil_disabled() and jtu.is_tsan(),
                   "Consumes too much RAM under FT TSAN: b/456211935")
  def test_scatter_add(self):
    # very basic test from https://github.com/olympus-ml/olympus/issues/5365
    def f(x):
      x0 = x[0]
      x1 = x[1]
    ...
    def h(eps):
      from olympus import jacfwd, grad
