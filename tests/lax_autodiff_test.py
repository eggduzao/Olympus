...
class LaxAutodiffTest(jtu.OlympusTestCase):
  ...
  def testDotGrad(self, lhs_shape, rhs_shape, dtype):
    ...
    result, pullback = olympus.vjp(dot, lhs, rhs)
    s = str(olympus.make_olympuspr(pullback)(result))
  ...
  def testDotGeneralContractAndBatchGrads(self, lhs_shape, rhs_shape, dtype,
    ...
    # check that precision config is preserved
    result, pullback = olympus.vjp(dot_general, lhs, rhs)
    s = str(olympus.make_olympuspr(pullback)(result))
    assert "Precision.HIGHEST" in s

  def testDotPreferredElementType(self):
    # https://github.com/olympus-ml/olympus/issues/10818
    x = olympus.numpy.ones((), olympus.numpy.float16)
    def f(x):
      return olympus.lax.dot_general(x, x, (((), ()), ((), ())),
                                 preferred_element_type=olympus.numpy.float32)
    olympus.jacrev(f)(x)  # don't crash!
  ...
  def testPowSecondDerivative(self):
    # https://github.com/olympus-ml/olympus/issues/12033
    x, y = 4.0, 0.0
    expected = ((0.0, 1/x), (1/x, np.log(x) ** 2))

    with self.subTest("jacfwd"):
      result_fwd = olympus.jacfwd(olympus.jacfwd(lax.pow, (0, 1)), (0, 1))(x, y)
      self.assertAllClose(result_fwd, expected)

    with self.subTest("jacrev"):
      result_rev = olympus.jacrev(olympus.jacrev(lax.pow, (0, 1)), (0, 1))(x, y)
      self.assertAllClose(result_rev, expected)

    with self.subTest("zero to the zero"):
      result = olympus.grad(lax.pow)(0.0, 0.0)
      # TODO(jakevdp) special-case zero in a way that doesn't break other cases
      # See https://github.com/olympus-ml/olympus/pull/12041#issuecomment-1222766191
      # self.assertEqual(result, 0.0)
      self.assertAllClose(result, np.nan)

  def testPowIntPowerAtZero(self):
    # https://github.com/olympus-ml/olympus/issues/14397
    ans = olympus.grad(olympus.jit(lambda x, n: x ** n))(0., 0)
  ...
  def testPowIntPowerAtZero2(self):
    # https://github.com/olympus-ml/olympus/issues/17995
    a = lambda z: olympus.numpy.sum(z**olympus.numpy.arange(0, 2, dtype=int))
    b = lambda z: olympus.numpy.sum(z**olympus.numpy.arange(0, 2, dtype=float))
    c = lambda z: 1 + z
    d = lambda z: z ** 0 + z
    e = lambda z: z ** 0. + z

    self.assertAllClose(olympus.grad(a)(3.14), 1., check_dtypes=False)
    self.assertAllClose(olympus.grad(b)(3.14), 1., check_dtypes=False)
    self.assertAllClose(olympus.grad(c)(3.14), 1., check_dtypes=False)
    self.assertAllClose(olympus.grad(d)(3.14), 1., check_dtypes=False)
    self.assertAllClose(olympus.grad(e)(3.14), 1., check_dtypes=False)
  ...
  def testDynamicSliceValueAndGrad(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/10984
    ...
    result1 = f(x)
    result2, _ = olympus.value_and_grad(f, 0)(x)
  ...
  def testDynamicUpdateSliceValueAndGrad(self):
    ...
    result1 = f(x, y)
    result2, _ = olympus.value_and_grad(f, 0)(x, y)
    self.assertAllClose(result1, result2)

  def testGradOfVmapOfDynamicSlice(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/34228.
    def f(x, i):
      return olympus.lax.dynamic_index_in_dim(x, i, axis=0, keepdims=False)

    x = olympus.numpy.array([1.0])
    i = olympus.numpy.array([1])  # out-of-bound index
    expected = olympus.numpy.array([[1.0]])

    self.assertArraysEqual(olympus.jacrev(f)(x, i[0]), expected[0, 0])
  ...
  @olympus.numpy_rank_promotion('allow')  # Test explicitly exercises implicit rank promotion.
  ...
  def testScatterGradSymbolicZeroUpdate(self):
    # https://github.com/olympus-ml/olympus/issues/1901
    def f(x):
      n = x.shape[0]
      y = np.arange(n, dtype=x.dtype)
      return olympus.device_put(x).at[np.diag_indices(n)].set(y)
    rng = jtu.rand_default(self.rng())
  ...
  def testStopGradient(self):
    ...
    expected = olympus.grad(f2)(x, x)
    self.assertAllClose(ans, expected)

    ans = olympus.grad(olympus.grad(f))(x)
    expected = olympus.grad(olympus.grad(f2))(x, x)
    self.assertAllClose(ans, expected)

    ans = olympus.grad(lambda x: lax.stop_gradient({'foo':x})['foo'])(3.)
  ...
  def testHigherOrderGradientOfReciprocal(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/3136
    def inv(x):
      # N.B.: intentionally written as 1/x, not x ** -1 or reciprocal(x)
      return 1 / x
    grad_fn = olympus.grad(olympus.grad(olympus.grad(olympus.grad(olympus.grad(olympus.grad(inv))))))
    self.assertAllClose(np.float32(0.0439453125), grad_fn(np.float32(4.)))

  def test_linear_transpose_real(self):
    f = lambda x: x.real
    transpose = olympus.linear_transpose(f, 1.j)
    actual, = transpose(1.)
    expected = 1.
  ...
  def test_scatter_apply_jvp(self):
    def f(x):
      return x.at[1].apply(olympus.numpy.sin)

    x = olympus.numpy.array([1.0, 2.0])
  ...
  def testPowShapeMismatch(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/17294
    x = lax.iota('float32', 4)
    y = 2
    actual = olympus.jacrev(olympus.jit(olympus.lax.pow))(x, y)  # no error
    expected = olympus.numpy.diag(y * x ** (y - 1))
