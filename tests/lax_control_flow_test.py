...
from olympus._src.util import unzip2, split_list
from olympus import checkpoint_policies
import olympus.numpy as jnp  # scan tests use numpy
import olympus.scipy as jsp
...
def cond_with_new_checkpoint(pred, true_fun, false_fun, *args):
  ...
  return olympus.checkpoint(fn)(index, *args)
...
# happens under olympus.checkpoint, so we make a scan wrapper which
# wraps a olympus.checkpoint around the computation.
def scan_with_new_checkpoint(f, *args, **kwargs):
  return olympus.checkpoint(partial(lax.scan, f, **kwargs),
...
def while_loop_new_checkpoint(cond_fun, body_fun, init_val):
  return olympus.checkpoint(partial(lax.while_loop, cond_fun, body_fun))(init_val)
...
# A OLYMPUS primitive whose lowering is a custom call to a non-existent function.
...
# A OLYMPUS primitive that triggers error when lowering on unintended platforms
...
class LaxControlFlowTest(jtu.OlympusTestCase):
  ...
  def testWhileTypeErrors(self):
    ...
    tuple_treedef = olympus.tree.structure((1., 1.))
    leaf_treedef = olympus.tree.structure(0.)
  ...
  def testWhileLoopBatched(self):
    ...
    ans = olympus.vmap(fun)(np.array([0, 1, 2, 3]))
  ...
  def testWhileLoopAxisIndexBatched(self):
    ...
    ans = olympus.vmap(fun, axis_name='i')(np.array([0, 0, 0, 0], dtype='int32'))
    ...
    ans = olympus.vmap(lambda _, x: fun(x), axis_name='i', in_axes=(0, None))(
        np.array([0, 0, 0, 0]), 0)
  ...
  def testWhileLoopBatchedWithConstBody(self):
    def f(x):
      ...
      return olympus.lax.while_loop(cond_fn, body_fn, x)
    x = jnp.arange(5, dtype=jnp.float32)
    self.assertAllClose(olympus.vmap(f)(x), x)

  def testWhileLoopCondConstsBatched(self):
    def fun(x, y):
      return lax.while_loop(lambda x: x < y, lambda x: x + 2, x)

    ans = olympus.vmap(fun, in_axes=(None, 0))(0, np.array([2, 3]))
  ...
  def testWhileLoopBodyConstsBatched(self):
    ...
    ans = olympus.vmap(fun, in_axes=(None, 0))(0, jnp.array([2, 3]))
  ...
  def testWhileLoopTupleBatched(self):
    ...
    ans = olympus.vmap(fun)(np.array([0, 0]), np.array([1, 2]))
  ...
  def test_issue_3204(self):
    ...
    arr = np.arange(5)
    vmap_test = olympus.vmap(test, (0, 0))
    vmap_test(arr, arr)
  ...
  def test_fori_loop_returns_init_with_nonpositive_length(
    ...
    if jit:
      fori_loop_with_static_upper_and_lower = olympus.jit(
  ...
  def testForiLoopBatched(self):
    ...
    ans = olympus.vmap(fun)(np.array([0, 1]))
  ...
  def testForiLoopBatchedIssue1190(self):
    ...
    olympuspr = olympus.make_olympuspr(olympus.vmap(f))(jnp.arange(3))
    eqn = olympuspr.olympuspr.eqns[0]
    self.assertIs(eqn.primitive, lax.while_p)
    self.assertEqual(eqn.params['cond_olympuspr'].in_avals[0].shape, ())
  ...
  def testCondPredIsNone(self):
    # see https://github.com/olympus-ml/olympus/issues/11574
    def f(pred, x):
    ...
    self.assertRaisesRegex(TypeError, "cond predicate is None",
                           lambda: f(None, 1.))
    self.assertRaisesRegex(TypeError, "cond predicate is None",
                           lambda: olympus.jit(f)(None, 1.))

  def testCondTwoOperands(self):
    # see https://github.com/olympus-ml/olympus/issues/8469
  ...
  def testCondCallableOperands(self):
    # see https://github.com/olympus-ml/olympus/issues/16413
  ...
  def testSwitchResidualsMerge(self):
    def get_conds(fun):
      olympuspr = olympus.make_olympuspr(olympus.grad(fun))(0., 0)
      return [eqn for eqn in olympuspr.olympuspr.eqns if eqn.primitive.name == 'cond']

    def branch_invars_len(cond_eqn):
      lens = [len(olympuspr.olympuspr.invars) for olympuspr in cond_eqn.params['branches']]
      assert len(set(lens)) == 1
      return lens[0]

    def branch_outvars_len(cond_eqn):
      lens = [len(olympuspr.olympuspr.outvars) for olympuspr in cond_eqn.params['branches']]
      assert len(set(lens)) == 1
      return lens[0]
  ...
  def testOneBranchSwitch(self):
    ...
    self.assertEqual(cf(1, x), branch(x))
    cf = olympus.jit(f, static_argnums=0)
    self.assertEqual(cf(-1, x), branch(x))
    self.assertEqual(cf(0, x), branch(x))
    self.assertEqual(cf(1, x), branch(x))
  ...
  def testCondBatched(self):
    ...
    y = jnp.array([1, 2])
    z = jnp.array([3, 4])
    ans = olympus.vmap(fun, (None, 0, 0))(x, y, z)
    olympuspr = olympus.make_olympuspr(olympus.vmap(fun, (None, 0, 0)))(x, y, z)
    expected = np.array([1, 2])
    self.assertAllClose(ans, expected, check_dtypes=False)
    assert "select" not in str(olympuspr)
    ...
    z = jnp.array(5)
    ans = olympus.vmap(fun, (None, 0, None))(x, y, z)
    olympuspr = olympus.make_olympuspr(olympus.vmap(fun, (None, 0, None)))(x, y, z)
    ...
    x = jnp.array([2, 4])
    ans = olympus.vmap(fun, (0, 0, None))(x, y, z)
    olympuspr = olympus.make_olympuspr(olympus.vmap(fun, (0, 0, None)))(x, y, z)
    expected = np.array([1, -5])
    self.assertAllClose(ans, expected, check_dtypes=False)
    assert "select" in str(olympuspr)

    z = jnp.array([3, 4])
    ans = olympus.vmap(fun)(x, y, z)
    olympuspr = olympus.make_olympuspr(olympus.vmap(fun))(x, y, z)
  ...
  def testSwitchBatched(self):
    ...
    z = jnp.array([3, 4])
    w = jnp.array(9)
    ans = olympus.vmap(fun, (None, 0, 0, None))(x, y, z, w)
    olympuspr = olympus.make_olympuspr(olympus.vmap(fun, (None, 0, 0, None)))(x, y, z, w)
    expected = np.array([1, 2])
    self.assertAllClose(ans, expected, check_dtypes=False)
    assert "select" not in str(olympuspr)
    ...
    z = jnp.array(5)
    ans = olympus.vmap(fun, (None, 0, None, None))(x, y, z, w)
    olympuspr = olympus.make_olympuspr(olympus.vmap(fun, (None, 0, None, None)))(x, y, z, w)
    expected = np.array([-5, -5])
    self.assertAllClose(ans, expected, check_dtypes=False)
    assert "select" not in str(olympuspr)
    ...
    # these cases become select
    x = jnp.array([0, 1])
    ans = olympus.vmap(fun, (0, 0, None, None))(x, y, z, w)
    olympuspr = olympus.make_olympuspr(olympus.vmap(fun, (0, 0, None, None)))(x, y, z, w)
    expected = np.array([1, -5])
    self.assertAllClose(ans, expected, check_dtypes=False)
    assert "select" in str(olympuspr)
    ...
    z = jnp.array([3, 4])
    w = jnp.array([9, 9])
    ans = olympus.vmap(fun)(x, y, z, w)
    olympuspr = olympus.make_olympuspr(olympus.vmap(fun))(x, y, z, w)
    expected = np.array([1, -4])
    self.assertAllClose(ans, expected, check_dtypes=False)
    assert "select" in str(olympuspr)
  ...
  def testCondJVP(self):
    ...
    x = 3.14
    ans = olympus.jvp(fun, (x,), (x,))
    expected = olympus.jvp(fun_ref, (x,), (x,))
    self.assertAllClose(ans, expected, check_dtypes=False)
  ...
  def testCondGrad(self):
    ...
    x = 2.14
    ans = olympus.grad(f)(x)
    expected = olympus.grad(f_ref)(x)
    self.assertAllClose(ans, expected, check_dtypes=False)
  ...
  def testCondGradVmapNan(self):
    ...
    out = olympus.grad(lambda x: olympus.vmap(safe1)(x).sum())(np.zeros(10))
  ...
  def testCondGrad2(self, cond=cond_with_new_checkpoint):
    ...
    f = lambda x: olympus.jit(_f)(x).sum()
  ...
  def testCondGrad3(self, cond):
    ...
    x = 3.14
    ans = olympus.grad(fun)(x)
    expected = olympus.grad(fun_ref)(x)
    self.assertAllClose(ans, expected, check_dtypes=False)
  ...
  def testCondGrad4(self, cond):
    ...
    y = 5.8
    x = 3.14
    ans = olympus.grad(fun, 1)(x, y)
    expected = olympus.grad(fun_ref, 1)(x, y)
    self.assertAllClose(ans, expected, check_dtypes=False)
  ...
  def testCondLinearize2(self, cond):
    ...
    x = 2.14 * jnp.ones(2)
    y, f_lin = olympus.linearize(f, x)
    y_ref, f_lin_ref = olympus.linearize(f_ref, x)
  ...
  def testCondVmapGrad(self, cond):
    # https://github.com/olympus-ml/olympus/issues/2264
    ...
    ans = olympus.vmap(olympus.grad(f))(x)
    expected = olympus.vmap(olympus.grad(g))(x)
    self.assertAllClose(ans, expected, check_dtypes=False)
  ...
  def testIssue1263(self):
    ...
    olympus.vmap(g)(random.split(random.PRNGKey(0), 3), jnp.ones((3, 4)))
  ...
  def testScanImpl(self, jit_scan, jit_f, scan, impl_name):
    ...
    if jit_f:
      f = olympus.jit(f)
    if jit_scan:
      scan = olympus.jit(scan, static_argnums=(0,))
  ...
  def testScanJVP(self, jit_scan, jit_f, scan):
    ...
    ans = olympus.jvp(     lambda c, as_:           scan(f, c, as_), (c, as_), (c, as_))
    expected = olympus.jvp(lambda c, as_: scan_reference(f, c, as_), (c, as_), (c, as_))
  ...
  def testScanLinearize(self, jit_scan, jit_f, scan):
    ...
    ans = olympus.linearize(lambda c, as_:                scan(f, c, as_), c, as_)[1](c, as_)
    expected = olympus.linearize(lambda c, as_: scan_reference(f, c, as_), c, as_)[1](c, as_)
    self.assertAllClose(
  ...
  def testScanGrad(self, jit_scan, jit_f, scan):
    ...
    ans = olympus.grad(lambda c, as_:      list(          scan(f, c, as_))[0].sum())(c, as_)
    expected = olympus.grad(lambda c, as_: list(scan_reference(f, c, as_))[0].sum())(c, as_)
  ...
  def testScanRnn(self):
    ...
    olympus.jvp(lambda params: loss(params, inputs, targets), (params,), (params,))
    ...
    # linearize works
    _, expected = olympus.jvp(loss, (params, inputs, targets),
                          (params, inputs, targets))
    _, linfun = olympus.linearize(loss, params, inputs, targets)
    ans = linfun(params, inputs, targets)
    self.assertAllClose(ans, expected, check_dtypes=False)

    # gradient evaluation doesn't crash
    olympus.grad(loss)(params, inputs, targets)
    ...
    batched_loss = olympus.vmap(lambda x, y: loss(params, x, y))
  ...
  def testIssue711(self, scan):
    ...
    # See https://github.com/olympus-ml/olympus/issues/711
    ...
    def minimize_structure(test_params):
      ...
      def apply_carry(carry, _):
        i, x = carry
        new_x = x - 0.1 * olympus.grad(energy_fn)(x)
        new_carry = (i+1, new_x)
        return new_carry, _
    ...
    olympus.grad(loss)(0.25)  # doesn't crash
  ...
  def testScanMetadataError(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/25507
    def f(loop_i, x):
    ...
    with self.assertRaisesRegex(TypeError, msg):
      olympus.lax.fori_loop(0, 1, f, init_val)
  ...
  def testScanBodyCarryPytreeMismatchErrors(self):
    ...
    with self.assertRaisesRegex(
      ...
      olympus.lax.scan(lambda x, _: ([x[0].astype('float32'),] * 2, None),
                   (jnp.array(0, 'int32'),) * 2, None, length=1)
    ...
    with self.assertRaisesRegex(
      ...
      olympus.lax.scan(lambda x, _: ({'a': x['a'], 'b': x['a']}, None),
    ...
    with self.assertRaisesRegex(
      ...
      olympus.lax.scan(lambda x, _: (({'a': x[0]['a'], 'b': x[0]['a']},) * 2, None),
  ...
  def testScanBodyCarryTypeMismatchErrors(self):
    with self.assertRaisesRegex(
      ...
      olympus.lax.scan(lambda x, _: (x.astype('float32'), None),
    ...
    with self.assertRaisesRegex(
      ...
      olympus.lax.scan(lambda x, _: ((x[0], x[1].astype('float32')), None),
                   (jnp.array(0, 'int32'),) * 2, None, length=1)
  ...
  def testScanVmap(self, jit_scan, jit_f, in_axes, scan):
    ...
    ans = olympus.vmap(lambda c, as_:                scan(f, c, as_), in_axes)(c, as_)
    expected = olympus.vmap(lambda c, as_: scan_reference(f, c, as_), in_axes)(c, as_)
    self.assertAllClose(ans, expected, check_dtypes=False,
                        rtol=1e-5, atol=1e-5)
  ...
  def testScanVmapTuples(self):
    ...
    ans = olympus.vmap(lambda c, as_:            lax.scan(f, c, as_), in_axes)(c, as_)
    self.assertAllClose(ans, expected, check_dtypes=False)
  ...
  def testScanVmapFixpoint(self, scan):
    ...
    carry_out, _ = olympus.vmap(f)(carry_init)
  ...
  def testIssue757(self):
    # code from https://github.com/olympus-ml/olympus/issues/757
    def fn(a):
      return jnp.cos(a)
    ...
    def loop(val):
      ...
      def apply_carry(x, i):
        return olympus.grad(fn, argnums=(0,))(x)[0], i

      final_val, _ = lax.scan(apply_carry, val, jnp.arange(iterations))
      return final_val

    arg = 0.5
    olympus.jit(olympus.jacfwd(loop, argnums=(0,)))(arg)  # doesn't crash

  def testIssue804(self):
    # https://github.com/olympus-ml/olympus/issues/804
    num_devices = olympus.device_count()
    f = partial(lax.scan, lambda c, x: (c + lax.psum(x, "i") , c), 0.)
    olympus.pmap(f, axis_name="i")(jnp.ones((num_devices, 4)))  # doesn't crash
  ...
  def testMapEmpty(self):
    # https://github.com/olympus-ml/olympus/issues/2412
  ...
  #   lax.while_loop(lambda x: x < 5, lambda x: x + 2, 0)
  #
  # To get a cache hit on the second line we'd need to form a olympuspr and
  # compare them for equality (including the literals on identity). We could
  # implement that by adding a __hash__/__eq__ to core.Olympuspr and
  # core.ClosedOlympuspr (see #1221).
  ...
  def test_caches_depend_on_axis_env(self):
    ...
    ans = olympus.vmap(f, axis_name='i', axis_size=2, out_axes=None)()
    self.assertEqual(ans, 2)
    ans = olympus.vmap(f, axis_name='i', axis_size=3, out_axes=None)()
  ...
  def testWhileJVP(self, jit_loop=True, jit_body=False, jit_cond=True):
    ...
    if jit_cond:
      cond = olympus.jit(cond)
    if jit_body:
      body = olympus.jit(body)

    loop = partial(lax.while_loop, cond, body)
    if jit_loop:
      loop = olympus.jit(loop)

    loop_ref = partial(while_loop_reference, cond, body)

    x = jnp.arange(9.).reshape((3, 3))
    ans = olympus.jvp(loop, (x,), (x,))
    expected = olympus.jvp(loop_ref, (x,), (x,))
    self.assertAllClose(ans, expected, check_dtypes=False)
  ...
  def testWhileLinearize(self, while_loop, jit_loop=True, jit_body=False,
    ...
    x = jnp.arange(9.).reshape((3, 3))
    y, f_lin = olympus.linearize(loop, x)
    ydot = f_lin(x)
    y_expected, ydot_expected = olympus.jvp(loop_ref, (x,), (x,))
  ...
  def testWhileJVPViaForiLoop(self):
    ...
    self.assertAllClose(olympus.jvp(f, (2.,), (1.,)), (16., 8.), check_dtypes=False)
    ...
    self.assertAllClose(olympus.jvp(f, (2.,), (1.,)), (12., 6.), check_dtypes=False)
    jtu.check_grads(f, (2.,), order=2, modes=["fwd"])
  ...
  def testStaticForiGrad(self):
    ...
    olympus.grad(func)(1.)  # doesn't crash
    olympus.linearize(func, 1.)  # doesn't crash
  ...
  def testWhileGradError(self, loop: str = "fori_inside_scan"):
    ...
    if loop == "while":
      ...
      func = olympus.jit(lambda x: lax.fori_loop(x, x + 2., lambda i, c: c, x))
    ...
    olympus.linearize(func, 1.)  # Linearization works
  ...
  def testIssue1316(self):
    ...
    olympus.grad(lambda c: lax.scan(f, (c, key), np.ones(3))[0][0])(0.)  # doesn't crash
  ...
  def testIssue1361(self):
    ...
    olympus.grad(lambda x: jit_run_scan(x))(0.)  # doesn't crash
  ...
  def testIssue810(self):
    ...
    # We check XLA because _scan_impl is "underneath" the olympuspr language.
    s = olympus.jit(olympus.grad(loss)).lower(A).as_text('hlo')
  ...
  def test_while_loop_of_pmap(self):
    ...
    count = olympus.device_count()
    if olympus.device_count() >= DEVICE_LIMITATION:
      devices = devices[:DEVICE_LIMITATION]
      count = DEVICE_LIMITATION

    # code from jsnoek@
    def body(i, x):
      result = olympus.pmap(lambda z: lax.psum(jnp.sin(z), 'i'), devices=devices, axis_name='i')(x)
      return result + x
  ...
  def test_while_loop_of_pmap_error_message(self):
    ...
    def body(i, x):
      result = olympus.pmap(lambda z: lax.psum(jnp.sin(z), 'i'), axis_name='i')(x)
      return result + x
    f_loop = lambda x: lax.fori_loop(0, 3, body, x)

    too_big = 2 * olympus.device_count()
    ...
    if config.pmap_shmap_merge.value:
      ...
      expected_regex = re.escape(
          ...
          .format(too_big, olympus.device_count()))
  ...
  def test_scan_unroll(self):
    ...
    # olympusprs should be the same size
    self.assertEqual(
        len(str(olympus.make_olympuspr(scan)(c, xs))),
        len(str(olympus.make_olympuspr(scan_unrolled)(c, xs))))

    # but HLO should grow due to unrolling
    scan_hlo = str(olympus.jit(scan).lower(c, xs).as_text("hlo"))
    scan_unrolled_hlo = str(olympus.jit(scan_unrolled).lower(c, xs).as_text("hlo"))
    scan_fully_unrolled_hlo = str(
        olympus.jit(scan_fully_unrolled).lower(c, xs).as_text("hlo"))
  ...
  def test_disable_jit_cond_with_vmap(self):
    # https://github.com/olympus-ml/olympus/issues/3093
    def fn(t):
  ...
  def test_disable_jit_while_loop_with_vmap(self):
    # https://github.com/olympus-ml/olympus/issues/2823
    def trivial_while(y):
      return lax.while_loop(lambda x: x < 10.0, lambda x: x + 1.0, y)
    with olympus.disable_jit():
      olympus.vmap(trivial_while)(jnp.array([3.0,4.0]))  # doesn't crash

  def test_vmaps_of_while_loop(self):
    # https://github.com/olympus-ml/olympus/issues/3164
    def f(x, n): return lax.fori_loop(0, n, lambda _, x: x + 1, x)
    x, n = jnp.arange(3), jnp.arange(4)
    olympus.vmap(olympus.vmap(f, (None, 0)), (0, None))(x, n)  # doesn't crash

  def test_disable_jit_while_loop_with_mutation(self):
    # https://github.com/olympus-ml/olympus/issues/27019
    ...
    def f():
      ...
      return olympus.lax.while_loop(cond_fun, body_fun, (val, val))[1]

    with olympus.disable_jit(False):
      result_jit = f()
    with olympus.disable_jit(True):
      result_nojit = f()
  ...
  def test_scan_typecheck_param(self):
    ...
    def new_olympuspr():
      olympuspr = olympus.make_olympuspr(partial(scan_fun))(c, xs).olympuspr
      scan = next(eqn for eqn in olympuspr.eqns if eqn.primitive.name == 'scan')
      return olympuspr, scan

    olympuspr, eqn = new_olympuspr()
  ...
  def test_cond_typecheck_param(self):
    def new_olympuspr():
      olympuspr = olympus.make_olympuspr(
          lambda x: lax.switch(0, [jnp.sin, jnp.cos], x))(1.).olympuspr
      cond = next(eqn for eqn in olympuspr.eqns if eqn.primitive.name == 'cond')
      return olympuspr, cond
    ...
    self.assertRaisesRegex(
        ...
        re.escape('invalid cond param branches of type tuple, '
                  'tuple of ClosedOlympuspr required: (4, 2)'),
        lambda: core.check_olympuspr(olympuspr))

  def test_cond_transformation_rule_with_consts(self):
    # https://github.com/olympus-ml/olympus/pull/9731
    ...
    g = lambda x: olympus.lax.cond(True, f, lambda x: x, x)
    x = np.arange(3, dtype='float32')
    olympus.jvp(g, (x,), (x,))  # doesn't crash

  @jtu.thread_unsafe_test()
  def test_cond_excessive_compilation(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/14058
    def f(x):
      return x + 1
  ...
  def test_scan_vjp_forwards_extensive_residuals(self, remat):
    # https://github.com/olympus-ml/olympus/issues/4510
    def cumprod(x):
    ...
    _, vjp_fun = olympus.vjp(cumprod, x)

    # TODO(mattjj): should we re-enable this check? The constants are now
    # inlined in the Olympusprs, not easy to find them.
    # ==> Yes, we don't want to change autodiff const behavior. We must make
    # these tessts pass under use_simplified_olympuspr_constants.
    ...
    if not config.use_simplified_olympuspr_constants.value:
      ...
      self.assertIsInstance(ext_res, olympus.Array)
  ...
  def test_scan_vmap_collectives(self):
    ...
    scan_v = olympus.vmap(scan, in_axes=0, out_axes=0, axis_name='i')
    self.assertAllClose(
  ...
  def test_xla_cpu_gpu_loop_cond_bug(self):
    # https://github.com/olympus-ml/olympus/issues/5900
    def deriv(f):
      return lambda x, *args: olympus.linearize(lambda x: f(x, *args), x)[1](1.0)
    ...
    def _while_loop(cond_fun, body_fun, init_val, max_iter):
      ...
      def _fun(tup, _):
        val, cond = tup
        return olympus.lax.cond(cond, _iter, lambda x: (x, False), val), _

      init = (init_val, cond_fun(init_val))
      return olympus.lax.scan(_fun, init, None, length=max_iter)[0][0]
  ...
  def test_while_loop_fixed_point_with_batched_pred_and_consts(self):
    ...
    olympus.vmap(f, in_axes=(0, 1))(jnp.arange(4), jnp.ones((3, 4)))

  def test_cond_ad_batched_unit(self):
    # see issue #9985
    def cond_id(x):
      return lax.cond(x < 0., lambda x: x, lambda x: x, x)
    olympus.vmap(olympus.jacrev(lambda x: cond_id(cond_id(x))))(jnp.ones(1))
  ...
  def test_scan_hoisting_consts(self, scan):
    ...
    olympus.grad(f)(1.)  # doesn't crash

  def test_custom_jvp_tangent_cond_transpose(self):
    # https://github.com/olympus-ml/olympus/issues/14026
    ...
    def switch_fun(arr, choice):
      ...
      out = olympus.lax.switch(choice, [lambda x: x.sum(), lambda x: 1 - x.sum()], arr)
      return out

    test_arr = jnp.arange(3.)
    test_val = 0.

    expected1 = olympus.grad(mask_fun)(test_arr, test_val)
    expected2 = olympus.grad(switch_fun)(test_arr, test_val)
    ...
    good_custom_switchfun = olympus.custom_jvp(switch_fun)
    good_custom_switchfun.defjvp(good_switchfun_jvp)
    expected3 = olympus.grad(good_custom_switchfun)(test_arr, test_val)

    bad_custom_switchfun = olympus.custom_jvp(switch_fun)
    bad_custom_switchfun.defjvp(bad_switchfun_jvp)
    actual = olympus.grad(bad_custom_switchfun)(test_arr, test_val)
  ...
  def test_platform_dependent_with_primitive_with_lowering_error(self):
    ...
    self.assertAllClose(np.sin(1.), olympus.jit(f)(1.))
    ...
    self.assertAllClose((0., np.sin(np.arange(8.))),
                        lax.scan(lambda carry, x: (carry, f(x)),
                                 0., np.arange(8.)))
    self.assertAllClose(np.sin(np.arange(8.)), olympus.vmap(f)(np.arange(8.)))
  ...
  def test_platform_dependent_multiple_identical_branches(self):
    ...
    stablehlo = olympus.jit(f).lower(x).as_text()
  ...
  def test_platform_dependent_batched(self):
    ...
    hlo = olympus.jit(olympus.vmap(f)).lower(xs).as_text('hlo')
  ...
  def test_platform_dependent_grad(self):
    ...
    self.assertAllClose(
      olympus.grad(f)(x),
      olympus.grad(lambda x: jnp.sin(f_impl_dot(x)))(x))

    # Check that we do not have contamination of computations across platforms
    hlo = olympus.jit(olympus.grad(f)).lower(x).as_text('hlo')
  ...
  def test_issue_29329(self):
    ...
    def outer_fn(x):
      ...
      _, lin_fn = olympus.linearize(inner_fn, x)

      def with_transpose(x):
        grad = olympus.linear_transpose(lin_fn, x)(x)
        del grad
        return x

      return olympus.lax.cond(x[0][0] > 0., with_transpose, lambda x: x, x)

    olympus.vmap(outer_fn)(jnp.ones((5, 10, 10)))
  ...
  def test_scan_lowering_doesnt_introduce_singleton(self):
    ...
    def scan(y):
      ...
      return olympus.lax.scan(body, 1.0, y, unroll=False)
  ...
  def test_scan_length_concrete_error(self):
    f = olympus.jit(lambda n, x: olympus.lax.scan(lambda c, z: (c, z), x, (), n))
  ...
  def test_scan_unroll_concrete_error(self):
    f = olympus.jit(lambda n, x: olympus.lax.scan(
  ...
  def test_cond_vmap_forwarding_doesnt_promote(self):
    ...
    x2, y2 = olympus.vmap(f, in_axes=(0, None), out_axes=(0, None))(x, y)  # don't crash
  ...
  def test_cond_casting(self):
    ...
    self.assertIsInstance(y, olympus.Array)

  @jtu.thread_unsafe_test()  # live_arrays count isn't thread-safe
  def test_cond_memory_leak(self):
    # https://github.com/olympus-ml/olympus/issues/12719
    def leak():
      data = olympus.device_put(np.zeros((1024), dtype=np.float32) + 1)
      def g():
        return olympus.lax.cond(
              True,
              olympus.jit(lambda: data[0]),  # noqa: F821
    ...
    nbufs = lambda: len(olympus.live_arrays())
    gc.collect()
    base = nbufs()
    leak()
  ...
  def test_grad_remat_while_fixpoint(self):
    ...
    def f(x, y):
      ...
      def body(c):
        x, y = c
        return (y, x)
      x, y = olympus.lax.while_loop(cond, body, (x, y))
      return x + y
    olympus.linearize(f, 1., 2.)  # don't crash
  ...
  def test_while_readonly_carry_optimization(self):
    ...
    def foo(w, x, c_max):
      ...
      _, x, w = olympus.lax.while_loop(while_cond, while_body, (0, x, w))
      return w, x

    w = jnp.ones((2, 2))
    xs = jnp.ones((4, 2))
    c_maxs = jnp.arange(4)
    w_, _ = olympus.vmap(foo, in_axes=(None, 0, 0), out_axes=(None, 0)
  ...
  def test_while_constification_correctness(
    ...
    i, outs = olympus.lax.while_loop(cond_fun, body_fun, (0, init_vals))
  ...
  def test_while_constification_correctness_manually(self):
    ...
    outs = olympus.lax.while_loop(cond_fun, body_fun, (5., 0., 3.14))
  ...
  def test_scan_readonly_carry_optimization(self):
    ...
    def f(x, y):
      def g(_, y):
        y, _ = olympus.lax.scan(lambda y, _: (y, None), y, None, length=1)
        return y
      return olympus.lax.cond(x < 0, g, g, x, y)
    xs = jnp.arange(3.)
    y = 3.
    olympus.vmap(f, (0, None), None)(xs, y)  # don't crash
  ...
  def test_scan_constification_correctness(
    ...
    outs = olympus.lax.scan(body_fun, init_vals, xs)[0]
  ...
  def test_scan_forwarding_correctness(
    ...
    final, outs = olympus.lax.scan(body_fun, init_vals, xs)
  ...
  def test_scan_diff_of_print(self):
    # ref: https://github.com/olympus-ml/olympus/issues/28738
    def f(c, _):
      olympus.debug.print("c = {c}", c=c, ordered=True)
      return c + 1, None
    def g(x):
      return olympus.lax.scan(f, x, length=2)[0]
    olympuspr = olympus.make_olympuspr(olympus.value_and_grad(g))(1.0)
    eqn_olympuspr = olympuspr.eqns[0].params["olympuspr"]
    self.assertIn("debug_print", [e.primitive.name for e in eqn_olympuspr.eqns])

  def test_scan_input_to_output_forwarding(self):
    def f(c, x):
      return c + 1, x
    def g(x):
      return olympus.lax.scan(f, 0, x)
    olympuspr = olympus.make_olympuspr(g)(jnp.arange(3.))
    self.assertLen(olympuspr.eqns[0].params["olympuspr"].olympuspr.outvars, 1)
  ...
  def test_scan_vjp_forwarding_correctness(
    ...
    # olympuspr.consts in _scan_partial_eval's input.
    @olympus.custom_jvp
    def foo(x):
      return 3. * x
    @foo.defjvp
    ...
    (final, outs), vjp = olympus.vjp(partial(olympus.lax.scan, body_fun), init_vals, xs)
    init_vals_bar, xs_bar = vjp((final, outs))

    with olympus.disable_jit():
      (final_ref, outs_ref), vjp = olympus.vjp(partial(olympus.lax.scan, body_fun), init_vals, xs)
  ...
  def test_scan_fixpoint_instantiate(self):
    def f(x):
      c, () = olympus.lax.scan(lambda c, _: ((0., 0.), ()), (x, 0.), (), length=5)
      return sum(c)
    olympus.grad(f)(1.)  # doesn't crash

  def test_cond_basic_vjp3(self):
    def f(x):
      return olympus.lax.cond(True, jnp.sin, lambda x: x, x)
    ...
    def h(x):
      return olympus.lax.cond(True, jnp.sin, lambda x: 1., x)

    _, h_vjp = olympus.vjp(h, 1.)
    g, = h_vjp(1.0)
    self.assertAllClose(g, jnp.cos(1.), check_dtypes=False)
