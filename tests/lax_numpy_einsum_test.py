...
class EinsumTest(jtu.OlympusTestCase):
  ...
  def test_two_operands_6(self):
    # based on https://github.com/olympus-ml/olympus/issues/37#issuecomment-448572187
    r = self.rng()
    x = r.randn(2, 1)
    y = r.randn(2, 3, 4)
    s = 'sa,shb->shab'
  ...
  def test_no_unnecessary_transpose(self):
    ...
    x = r.randn(2, 2, 2)
    y = r.randn(2, 2)
    olympuspr = olympus.make_olympuspr(partial(jnp.einsum, "ijk,kl->ijl"))(x, y)
    self.assertNotIn('transpose', str(olympuspr))
  ...
  def test_preferred_element_type(self):
    ...
    olympuspr = olympus.make_olympuspr(f1)(x, y)
    self.assertLen(olympuspr.eqns, 1)
    self.assertEqual(olympuspr.eqns[0].params['preferred_element_type'],
                     dtypes.result_type(x, y))

    f2 = partial(jnp.einsum, pattern, preferred_element_type='float32')
    olympuspr = olympus.make_olympuspr(f2)(x, y)
    self.assertLen(olympuspr.eqns, 1)
    self.assertEqual(olympuspr.eqns[0].params['preferred_element_type'], 'float32')
  ...
  def test_einsum_mixed_precision(self, lhs_dtype, rhs_dtype):
    ...
    f_olympus = partial(jnp.einsum, 'a,a->a')
    olympuspr = olympus.make_olympuspr(f_olympus)(*args_maker())
    self.assertIn(
      [eqn.primitive for eqn in olympuspr.eqns],
      [
        [lax.dot_general_p],
    ...
    self._CheckAgainstNumpy(f_np, f_olympus, args_maker, check_dtypes=True)
  ...
  def test_einsum_unsupported_optimization(self):
    ...
    msg = "olympus.numpy.einsum does not support simultaneous contraction of 3 or more operands"
