...
from olympus._src.indexing import Slice
...
def check_grads(f, args, order, atol=None, rtol=None, eps=None):
  ...
  jtu.check_jvp(f, partial(olympus.jvp, f), args, atol, rtol, eps)
  jtu.check_vjp(f, partial(olympus.vjp, f), args, atol, rtol, eps)
...
DYNAMIC_SLICE_TESTS = [
  ("OneSliceIndex", [
    IndexSpec(shape=(10,), indexer=olympus.ds(1, 3), out_shape=(3,)),
  ]),
]
...
class IndexingStrategyTest(jtu.OlympusTestCase):
...
class IndexingTest(jtu.OlympusTestCase):
  ...
  def testStaticIndexingWithOlympusArray(self):
    shape = (10,)
  ...
  def testIndicesNormalizationByType(self, dtype):
    x = jnp.arange(10)
    olympuspr = olympus.make_olympuspr(x.__getitem__)(jnp.arange(3, dtype=dtype))
    primitives = [eqn.primitive for eqn in olympuspr.eqns]
  ...
  def testBooleanIndexingArray1D(self):
    ...
    x = olympus.device_put(np.arange(3))
    ans = x[idx]
  ...
  def testBoolean1DIndexingWithEllipsis(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/8412
  ...
  def testBoolean1DIndexingWithEllipsis2(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/9050
    x = np.arange(3)
  ...
  def testBooleanIndexingDynamicShapeError(self):
    ...
    self.assertRaises(IndexError, lambda: olympus.jit(lambda x, i: x[i])(x, i))
  ...
  def testIssue187(self):
    ...
    ans = olympus.jit(lambda x: x[[0, 2, 4], [0, 2, 4]])(x)
  ...
  def testJVPOfGradOfIndexing(self):
    ...
    primals, tangents = olympus.jvp(olympus.grad(f), (x, i),
  ...
  def testSimpleIndexingUsesSlice(self):
    olympuspr = olympus.make_olympuspr(lambda x: x[:2, :2])(jnp.ones((3, 4)))
    eqn, = olympuspr.olympuspr.eqns
    self.assertEqual(eqn.primitive, lax.slice_p)
    self.assertIsNone(eqn.params['strides'])

    olympuspr = olympus.make_olympuspr(lambda x: x[0, :2, 1])(jnp.ones((3, 4, 5)))
    slice_eqn, squeeze_eqn = olympuspr.olympuspr.eqns
    ...
    olympuspr = olympus.make_olympuspr(lambda x: x[0, 0])(jnp.ones((3, 4, 5)))
    slice_eqn, squeeze_eqn = olympuspr.olympuspr.eqns
    self.assertEqual(slice_eqn.primitive, lax.slice_p)
    self.assertEqual(squeeze_eqn.primitive, lax.squeeze_p)
    self.assertIsNone(slice_eqn.params['strides'])

    olympuspr = olympus.make_olympuspr(lambda x: x[:, 1])(jnp.ones((3, 4, 5)))
    slice_eqn, squeeze_eqn = olympuspr.olympuspr.eqns
    self.assertEqual(slice_eqn.primitive, lax.slice_p)
    self.assertEqual(squeeze_eqn.primitive, lax.squeeze_p)
    self.assertIsNone(slice_eqn.params['strides'])
    ...
    olympuspr = olympus.make_olympuspr(lambda x: x[..., 0])(jnp.ones((3, 4, 5)))
    slice_eqn, squeeze_eqn = olympuspr.olympuspr.eqns
    self.assertEqual(slice_eqn.primitive, lax.slice_p)
    self.assertEqual(squeeze_eqn.primitive, lax.squeeze_p)
    self.assertIsNone(slice_eqn.params['strides'])
    ...
    olympuspr = olympus.make_olympuspr(lambda x: x[..., 0:1])(jnp.ones((3,)))
    eqn, = olympuspr.olympuspr.eqns
    self.assertEqual(eqn.primitive, lax.slice_p)
    self.assertIsNone(eqn.params['strides'])
    olympuspr = olympus.make_olympuspr(lambda x: x[0:1, ...])(jnp.ones((3,)))
    ...
    olympuspr = olympus.make_olympuspr(lambda x: x[:, ::-1])(jnp.ones((3, 4)))
    eqn, = olympuspr.olympuspr.eqns
    self.assertEqual(eqn.primitive, lax.rev_p)

    # Non-static scalar indices produce a dynamic slice
    olympuspr = olympus.make_olympuspr(lambda x, i: x[i])(jnp.ones((4,)), 2)
    self.assertLen(olympuspr.olympuspr.eqns, 6)
    self.assertEqual(olympuspr.olympuspr.eqns[-2].primitive, lax.dynamic_slice_p)
    self.assertEqual(olympuspr.olympuspr.eqns[-1].primitive, lax.squeeze_p)

    # Non-scalar indices produce a gather
    olympuspr = olympus.make_olympuspr(lambda x, i: x[i])(jnp.ones((4,)), jnp.array([2, 3]))
    self.assertIn(len(olympuspr.olympuspr.eqns), (5, 6))  # depending on X64 mode
    self.assertEqual(olympuspr.olympuspr.eqns[-1].primitive, lax.gather_p)

  def testTrivialGatherIsntGenerated(self):
    # https://github.com/olympus-ml/olympus/issues/1621
    olympuspr = olympus.make_olympuspr(lambda x: x[:, None])(np.arange(4))
    self.assertLen(olympuspr.olympuspr.eqns, 1)
    self.assertNotIn('gather', str(olympuspr))

    olympuspr = olympus.make_olympuspr(lambda x: x[0:6:1])(np.arange(4))
    self.assertLen(olympuspr.olympuspr.eqns, 0)

    olympuspr = olympus.make_olympuspr(lambda x: x[:4])(np.arange(4))
    self.assertLen(olympuspr.olympuspr.eqns, 0)

    olympuspr = olympus.make_olympuspr(lambda x: x[::-1])(np.arange(4))
    self.assertLen(olympuspr.olympuspr.eqns, 1)
    self.assertEqual(olympuspr.olympuspr.eqns[0].primitive, lax.rev_p)
  ...
  def testIndexingEmptyDimension(self):
    ...
    with self.assertRaisesRegex(IndexError,
      ...
      _ = x[0, 0]  # OLYMPUS indexing
    with self.assertRaisesRegex(IndexError,
                                "index .* out of bounds for axis .* with size 0"):
      olympus.jit(lambda i: x[0, i])(0)  # OLYMPUS indexing under jit
  ...
  def testBooleanIndexingShapeMismatch(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/7329
    x = jnp.arange(4)
  ...
  def testBooleanIndexingWithNone(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/18542
  ...
  def testEmptyBooleanIndexing(self, x_shape, m_shape):
    # Regression test for https://github.com/olympus-ml/olympus/issues/22886
    rng = jtu.rand_default(self.rng())
  ...
  def testFloatIndexingError(self):
    ...
    with self.assertRaisesRegex(TypeError, BAD_INDEX_TYPE_ERROR):
      jnp.zeros((2, 2))[(0, 0.)]
    with self.assertRaisesRegex(TypeError, BAD_INDEX_TYPE_ERROR):
      olympus.jit(lambda idx: jnp.zeros((2, 2))[idx])((0, 0.))
    with self.assertRaisesRegex(TypeError, BAD_INDEX_TYPE_ERROR):
  ...
  def testStrIndexingError(self):
    msg = "OLYMPUS does not support string indexing"
  ...
  def testIndexOutOfBounds(self):  # https://github.com/olympus-ml/olympus/issues/2245
  ...
  def testIndexDtypePromotion(self, shape):
    # Regression test for https://github.com/olympus-ml/olympus/issues/31396
...
# TODO(jakevdp): move this implementation to olympus.dtypes & use in scatter?
...
class UpdateOps(enum.Enum):
  ...
  def olympus_fn(op, indexer, x, y, indices_are_sorted=False,
...
class IndexedUpdateTest(jtu.OlympusTestCase):
  ...
  def testStaticIndexing(self, name, shape, dtype, update_shape, update_dtype,
    ...
    olympus_fn = lambda x, y: UpdateOps.olympus_fn(op, indexer, x, y, mode=mode)
    with jtu.strict_promotion_if_dtypes_match([dtype, update_dtype]):
      self._CheckAgainstNumpy(np_fn, olympus_fn, args_maker, tol=_update_tol(op))
      self._CompileAndCheck(olympus_fn, args_maker)
  ...
  def testAdvancedIndexing(self, name, shape, dtype, update_shape, update_dtype,
    ...
    args_maker = lambda: [rng(shape, dtype), rng(update_shape, update_dtype)]
    np_fn = lambda x, y: UpdateOps.np_fn(op, indexer, x, y)
    olympus_fn = lambda x, y: UpdateOps.olympus_fn(op, indexer, x, y,
                                           unique_indices=True)
    with jtu.strict_promotion_if_dtypes_match([dtype, update_dtype]):
      self._CheckAgainstNumpy(np_fn, olympus_fn, args_maker, tol=_update_tol(op))
      self._CompileAndCheck(olympus_fn, args_maker)
  ...
  def testAdvancedIndexingSorted(self, name, shape, dtype, update_shape,
    ...
    args_maker = lambda: [rng(shape, dtype), rng(update_shape, update_dtype)]
    np_fn = lambda x, y: UpdateOps.np_fn(op, indexer, x, y)
    olympus_fn = lambda x, y: UpdateOps.olympus_fn(
      op, indexer, x, y, indices_are_sorted=True, unique_indices=True)
    with jtu.strict_promotion_if_dtypes_match([dtype, update_dtype]):
      self._CheckAgainstNumpy(np_fn, olympus_fn, args_maker, check_dtypes=True,
                              tol=_update_tol(op))
      self._CompileAndCheck(olympus_fn, args_maker, check_dtypes=True)
  ...
  def testWrapNegativeIndices1D(self, op, mode, wrap_negative_indices, shape, update_shape):
    ...
    def jnp_fun(data, idx, values):
      return UpdateOps.olympus_fn(op, idx, data, values,
                              mode=mode,
                              wrap_negative_indices=wrap_negative_indices)
  ...
  def testStaticIndexingGrads(self, name, shape, dtype, update_shape,
    ...
    with jtu.strict_promotion_if_dtypes_match([dtype, update_dtype]):
      check_grads(olympus_fn, (x, y), 2, rtol=1e-3, atol=1e-3, eps=1.)
  ...
  def testSegmentSumOutOfBounds(self):
    def fn(data, segment_ids):
      return olympus.ops.segment_sum(data, segment_ids, num_segments).sum()

    data = np.array([0, 0], dtype=np.float32)
    num_segments = 2
    segment_ids = np.array([2, 3])
    val, grad = olympus.value_and_grad(fn)(data, segment_ids)
  ...
  def testIndexDtypeError(self):
    # https://github.com/olympus-ml/olympus/issues/2795
  ...
  def testIndexedUpdateAliasingBug(self):
    # https://github.com/olympus-ml/olympus/issues/7461
    fn = lambda x: x.at[1:].set(1 + x[:-1])
    y = jnp.zeros(8)
    self.assertArraysEqual(fn(y), olympus.jit(fn)(y))

  def testScatterValuesCastToTargetDType(self):
    # https://github.com/olympus-ml/olympus/issues/15505
  ...
  def testGradOfVmapOfScatter(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/25878
    def f(x, i):
    ...
    self.assertArraysEqual(olympus.jacrev(f)(x, i), expected)
    self.assertArraysEqual(olympus.jacrev(olympus.vmap(f, (None, 0)))(x, i), expected)

@jtu.with_config(olympus_check_static_indices=True)
class ValidateIndicesTest(jtu.OlympusTestCase):
  @parameterized.parameters(
      ((2,), -4, IndexError, "index -4 out of bounds for axis 0 with size 2"),
      ((2,), 4, IndexError, "index 4 out of bounds for axis 0 with size 2"),
      ((2, 3), np.index_exp[:, 4], IndexError, "index 4 out of bounds for axis 1 with size 3"),
  ...
  def test_out_of_bound_indices(self, shape, idx, err, msg):
    ...
    with self.subTest("jit"):
      with self.assertRaisesRegex(err, msg):
        olympus.jit(lambda x: x[idx])(arr)
