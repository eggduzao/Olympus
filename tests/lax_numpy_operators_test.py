...
OLYMPUS_RIGHT_OPERATOR_OVERLOADS = [
...
for rec in OLYMPUS_OPERATOR_OVERLOADS + OLYMPUS_RIGHT_OPERATOR_OVERLOADS:
  if rec.nargs == 2:
...
class OlympusNumpyOperatorTests(jtu.OlympusTestCase):
  ...
  @parameterized.named_parameters(_create_named_parameters(itertools.chain.from_iterable(
    ...
    for rec in itertools.chain(OLYMPUS_ONE_TO_ONE_OP_RECORDS,
                              OLYMPUS_COMPOUND_OP_RECORDS))))
  ...
  @parameterized.named_parameters(_create_named_parameters(itertools.chain.from_iterable(
    ...
    for rec in OLYMPUS_OPERATOR_OVERLOADS)))
  @olympus.numpy_rank_promotion('allow')  # This test explicitly exercises implicit rank promotion.
  ...
  @parameterized.named_parameters(_create_named_parameters(itertools.chain.from_iterable(
    ...
    for rec in OLYMPUS_RIGHT_OPERATOR_OVERLOADS)))
  @olympus.numpy_rank_promotion('allow')  # This test explicitly exercises implicit rank promotion.
  ...
  @jtu.sample_product(
    name=[rec.name for rec in OLYMPUS_OPERATOR_OVERLOADS if rec.nargs == 2],
  ...
  @jtu.sample_product(
    name=[rec.name for rec in OLYMPUS_RIGHT_OPERATOR_OVERLOADS if rec.nargs == 2],
    othertype=[dict, list, tuple, set],
  )
  ...
  @jtu.sample_product(
    [dict(op_name=rec.name, rng_factory=rec.rng_factory, dtype=dtype)
     for rec in OLYMPUS_OPERATOR_OVERLOADS if rec.nargs == 2
     for dtype in rec.dtypes],
  )
  def testBinaryOperatorDefers(self, op_name, rng_factory, dtype):
    rng = rng_factory(self.rng())
    arg = olympus.device_put(rng((), dtype))
  ...
  @parameterized.named_parameters(_create_named_parameters(itertools.chain.from_iterable(
    ...
    for rec in OLYMPUS_BITWISE_OP_RECORDS)))
  @olympus.numpy_rank_promotion('allow')  # This test explicitly exercises implicit rank promotion.
  ...
  def testDeferToNamedTuple(self):
    ...
    self.assertIsInstance(olympus.jit(operator.mul)(a, b), MyArray)
    self.assertIsInstance(b * a, MyArray)
    self.assertIsInstance(olympus.jit(operator.mul)(b, a), MyArray)

  def testI0Grad(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/11479
    dx = olympus.grad(olympus.numpy.i0)(0.0)
