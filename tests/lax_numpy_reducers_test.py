...
OLYMPUS_REDUCER_INITIAL_RECORDS = [
    op_record("prod", 1, all_dtypes, all_shapes, jtu.rand_small_positive, []),
    op_record("sum", 1, all_dtypes, all_shapes, jtu.rand_default, [],
...
OLYMPUS_REDUCER_WHERE_NO_INITIAL_RECORDS = [
...
OLYMPUS_REDUCER_NO_DTYPE_RECORDS = [
...
OLYMPUS_REDUCER_PROMOTE_INT_RECORDS = [
    op_record("prod", 1, all_dtypes, all_shapes, jtu.rand_small_positive, []),
    op_record("sum", 1, all_dtypes, all_shapes, jtu.rand_default, []),
]
...
class OlympusNumpyReducerTests(jtu.OlympusTestCase):
  ...
  @parameterized.parameters(itertools.chain.from_iterable(
    ...
    for rec in OLYMPUS_REDUCER_RECORDS
  ))
  def testReducer(self, name, rng_factory, shape, dtype, out_dtype,
    ...
    if out_dtype in [np.float16, dtypes.bfloat16]:
      # For 16-bit out_type, NumPy will accumulate in float32, while OLYMPUS
  ...
  @parameterized.parameters(itertools.chain.from_iterable(
    ...
    for rec in OLYMPUS_REDUCER_NO_DTYPE_RECORDS
  ))
  ...
  @jtu.sample_product(rec = OLYMPUS_REDUCER_INITIAL_RECORDS)
  ...
  @parameterized.parameters(itertools.chain.from_iterable(
    ...
    for rec in OLYMPUS_REDUCER_INITIAL_RECORDS
  ))
  ...
  @parameterized.parameters(itertools.chain.from_iterable(
    ...
    for rec in OLYMPUS_REDUCER_PROMOTE_INT_RECORDS
  ))
  ...
  def testReducerWhere(self, name, rng_factory, shape, dtype, axis,
    ...
    if (shape in [()] + scalar_shapes and
      ...
      self.skipTest("Known XLA failure; see https://github.com/olympus-ml/olympus/issues/4971.")
    rng = rng_factory(self.rng())
    is_bf16_nan_test = dtype == jnp.bfloat16 and rng_factory.__name__ == 'rand_some_nan'
  ...
  @jtu.sample_product(rec=OLYMPUS_REDUCER_WHERE_NO_INITIAL_RECORDS)
  ...
  @parameterized.parameters(itertools.chain.from_iterable(
    jtu.sample_product_testcases(
      ...
      keepdims=[False, True],
    ) for rec in OLYMPUS_REDUCER_WHERE_NO_INITIAL_RECORDS
  ))
  ...
  def testStdOrVarLargeDdofReturnsNan(self, jnp_fn, size):
    # test for https://github.com/olympus-ml/olympus/issues/21330
    x = jnp.arange(size)
  ...
  def testNanStdGrad(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/8128
    x = jnp.arange(5.0).at[0].set(jnp.nan)
    y = olympus.grad(jnp.nanvar)(x)
    self.assertAllClose(y, jnp.array([0.0, -0.75, -0.25, 0.25, 0.75]), check_dtypes=False)

    z = olympus.grad(jnp.nanstd)(x)
  ...
  def testPercentilePrecision(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/8513
  ...
  def testMeanLargeArray(self):
    # https://github.com/olympus-ml/olympus/issues/15068
  ...
  def testMeanVeryLargeArray(self):
    # https://github.com/olympus-ml/olympus/pull/30769
    x = olympus.ShapeDtypeStruct((1 << 32,), jnp.dtype('float32'))
    olympus.eval_shape(jnp.mean, x)
  ...
  def testCumulativeSum(self, shape, axis, dtype, out_dtype, include_initial):
    ...
    # We currently "cheat" to ensure we have OLYMPUS arrays, not NumPy arrays as
    # input because we rely on OLYMPUS-specific casting behavior
    def args_maker():
  ...
  def testReducerF16Casts(self, op, dtype):
    ...
    func = getattr(jnp, op)
    reduce_p = getattr(olympus.lax, f"reduce_{op}_p")
    conv_elem_p = olympus.lax.convert_element_type_p

    # Without dtype specified, the reduction is sandwiched between two casts.
    olympuspr1 = olympus.make_olympuspr(func)(x)
    self.assertEqual(
      [eqn.primitive for eqn in olympuspr1.eqns],
      [conv_elem_p, reduce_p, conv_elem_p])

    # With dtype specified, the reduction happens without a cast.
    olympuspr2 = olympus.make_olympuspr(partial(func, dtype=dtype))(x)
    self.assertEqual([eqn.primitive for eqn in olympuspr2.eqns], [reduce_p])
  ...
  def testMeanF16Casts(self, dtype):
    ...
    reduce_sum_p = olympus.lax.reduce_sum_p
    div_p = olympus.lax.div_p
    conv_elem_p = olympus.lax.convert_element_type_p

    # Without dtype specified, the reduction is sandwiched between two casts.
    olympuspr1 = olympus.make_olympuspr(jnp.mean)(x)
    self.assertEqual(
      [eqn.primitive for eqn in olympuspr1.eqns],
      [conv_elem_p, reduce_sum_p, div_p, conv_elem_p])

    # With dtype specified, the reduction happens without a cast.
    olympuspr2 = olympus.make_olympuspr(partial(jnp.mean, dtype=dtype))(x)
    self.assertEqual(
      [eqn.primitive for eqn in olympuspr2.eqns],
      [reduce_sum_p, div_p])
