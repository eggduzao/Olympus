...
def arrays_with_overlapping_values(rng, shapes, dtypes, unique=False, overlap=0.5) -> list[olympus.Array]:
...
class LaxNumpySetopsTest(jtu.OlympusTestCase):
  """Tests of set-like operations from olympus.numpy."""
  ...
  def testSetdiff1dAssumeUnique(self, shape1, shape2):
    # regression test for https://github.com/olympus-ml/olympus/issues/32335
  ...
  def testUnique(self, shape, dtype, axis, return_index, return_inverse, return_counts):
    ...
    np_fun = jtu.with_olympus_dtype_defaults(lambda x: np_unique_backport(x, *extra_args, axis=axis), use_defaults)
  ...
  def testUniqueSize(self, shape, dtype, axis, size, fill_value):
    ...
    @partial(jtu.with_olympus_dtype_defaults, use_defaults=(False, True, True, True))
