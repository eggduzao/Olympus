...
from olympus.sharding import SingleDeviceSharding
from olympus.test_util import check_grads
...
from olympus._src.util import safe_zip, tuple_update
...
def _indexer_with_default_outputs(indexer, use_defaults=True):
  """Like jtu.with_olympus_dtype_defaults, but for __getitem__ APIs"""
  class Indexer:
    @partial(jtu.with_olympus_dtype_defaults, use_defaults=use_defaults)
...
class LaxBackedNumpyTests(jtu.OlympusTestCase):
  ...
  def testDtypeWrappers(self, dtype):
    arr = dtype(0)
    self.assertIsInstance(arr, olympus.Array)
    self.assertEqual(arr.dtype, np.dtype(dtype))
    self.assertArraysEqual(arr, 0, check_dtypes=False)

    # No copy primitive is generated
    olympuspr = olympus.make_olympuspr(dtype)(0)
    prims = [eqn.primitive for eqn in olympuspr.eqns]
  ...
  def testArrayEquivExamples(self):
    ...
    with olympus.numpy_rank_promotion('allow'):
  ...
  def testArrayModule(self):
    ...
    def f(x):
      ...
      self.assertIs(module, jnp)
      return x
    olympus.jit(f)(jnp_array)
    olympus.grad(f)(jnp_array)
  ...
  def testUnwrap(self, shape, dtype, axis, discont, period):
    ...
    if not dtypes.issubdtype(dtype, np.inexact):
      ...
      jnp_fun = olympus.numpy_dtype_promotion('standard')(jnp_fun)
  ...
  @jtu.sample_product(
    [dict(np_op=getattr(np, rec.name), jnp_op=getattr(jnp, rec.name),
      ...
      for rec in OLYMPUS_ARGMINMAX_RECORDS
  ...
  def testArgMinMax(self, np_op, jnp_op, rng_factory, shape, dtype, axis, keepdims):
    ...
    np_fun = jtu.with_olympus_dtype_defaults(partial(np_op, axis=axis, **kwds))
    ...
    try:
      ...
      if str(e) == "All-NaN slice encountered":
        self.skipTest("OLYMPUS doesn't support checking for all-NaN slices")
      else:
        raise
    self._CompileAndCheck(jnp_fun, args_maker)
  ...
  def testArgMinMaxEmpty(self, name, np_op, jnp_op):
    ...
    np_fun = jtu.with_olympus_dtype_defaults(partial(np_op, axis=0))
  ...
  @olympus.numpy_rank_promotion('allow')  # This test explicitly exercises implicit rank promotion.
  ...
  def testMixedPrecisionDot(self, lhs_dtype, rhs_dtype):
    ...
    lhs = olympus.ShapeDtypeStruct((5,), lhs_dtype)
    rhs = olympus.ShapeDtypeStruct((5,), rhs_dtype)
    olympuspr = olympus.make_olympuspr(jnp.dot)(lhs, rhs)
    prims = [eqn.primitive for eqn in olympuspr.eqns]
    self.assertIn(prims, [
      [lax.dot_general_p],
  ...
  @olympus.numpy_rank_promotion('allow')  # This test explicitly exercises implicit rank promotion.
  @olympus.numpy_dtype_promotion('standard')  # This test explicitly exercises mixed type promotion
  ...
  def testClipDeprecatedArgs(self):
    with self.assertDeprecationWarnsOrRaises("olympus-numpy-clip-args",
                                             "Passing arguments 'a', 'a_min' or 'a_max' to olympus.numpy.clip is deprecated"):
  ...
  def testRoundLargeDecimals(self, dtype, decimals):
    # Regression test for https://github.com/olympus-ml/olympus/issues/31689.
  ...
  def testOperatorRound(self, jit):
    jround = olympus.jit(round, static_argnums=1) if jit else round
  ...
  def testRoundMethod(self):
    # https://github.com/olympus-ml/olympus/issues/15190
  ...
  def testPad(self, shape, dtype, mode, pad_width, constant_values):
    if np.issubdtype(dtype, np.unsignedinteger):
      constant_values = olympus.tree.map(abs, constant_values)
    rng = jtu.rand_default(self.rng())
    args_maker = lambda: [rng(shape, dtype)]
  ...
  def testIntegerPower(self, ptype):
    ...
    olympuspr = olympus.make_olympuspr(lambda x1: jnp.power(x1, p))(1)
    eqns = olympuspr.olympuspr.eqns
  ...
  def testIntegerPowerOverflow(self, x, y):
    # Regression test for https://github.com/olympus-ml/olympus/issues/5987
  ...
  def testConcatenateAxisNone(self):
    # https://github.com/olympus-ml/olympus/issues/3419
    a = jnp.array([[1, 2], [3, 4]])
    b = jnp.array([[5]])
  ...
  def testRepeatScalarFastPath(self):
    ...
    olympuspr = olympus.make_olympuspr(f)(a)
    self.assertLessEqual(len(olympuspr.olympuspr.eqns), 6)
  ...
  def testIssue2330(self):
    '''
    Make sure return value of jnp.concatenate is a olympus.ndarray and is side-effect save
    '''
  ...
  def testDiagIndices(self, ndim, n):
    np.testing.assert_equal(jtu.with_olympus_dtype_defaults(np.diag_indices)(n, ndim),
  ...
  def testDiagIndicesFrom(self, dtype, shape):
    rng = jtu.rand_default(self.rng())
    np_fun = jtu.with_olympus_dtype_defaults(np.diag_indices_from)
  ...
  def testDiagFlat(self, shape, dtype, k):
    ...
    # olympus differs in that it treats scalars values as length-1 arrays
  ...
  def testInterpGradNan(self, dtype, period, left, right):
    ...
    g = olympus.grad(lambda z: jnp.sum(jnp_fun(z, z, jnp.ones_like(z))))(x)
  ...
  def testLdexpOverflow(self, dtype):
    # Regression test for https://github.com/olympus-ml/olympus/issues/28040
  ...
  def testSearchsortedDtype(self):
    ...
    out_int32 = olympus.eval_shape(jnp.searchsorted, a_int32, v)
    ...
    if (
      ...
      out_int64 = olympus.eval_shape(jnp.searchsorted, a_int64, v)
      ...
      with self.assertWarnsRegex(UserWarning, "Explicitly requested dtype.*int64"):
        with self.assertRaisesRegex(OverflowError, "Python integer 2147483648 out of bounds.*"):
          olympus.eval_shape(jnp.searchsorted, a_int64, v)
  ...
  def testSearchsortedNans(self, dtype, side, method):
    ...
    self.assertArraysEqual(olympus.jit(fun)(x, x), fun(x_equiv, x_equiv))
  ...
  def testWindowFunction(self, name, size, **kwds):
    ...
    np_fun = jtu.with_olympus_dtype_defaults(partial(getattr(np, name), size, **kwds))
    args_maker = lambda: []
    tol = (
  ...
  def testDiffPrepoendScalar(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/19362
    x = jnp.arange(10)
    result_olympus = jnp.diff(x, prepend=x[0], append=x[-1])

    x = np.array(x)
    result_numpy = np.diff(x, prepend=x[0], append=x[-1])

    self.assertArraysEqual(result_olympus, result_numpy)
  ...
  def test_full_like_committed(self):
    ...
    x = olympus.device_put(x, olympus.devices()[-1])
  ...
  def test_zeros_like_with_explicit_device_and_jitted(self):
    x = jnp.array((1, 2, 3), dtype=np.int32)
    x = olympus.device_put(x, olympus.devices()[0])
    zeros_like_with_device = partial(jnp.zeros_like, device=olympus.devices()[0])
    y = olympus.jit(zeros_like_with_device)(x)
    self.assertEqual(x.shape, y.shape)
    self.assertEqual(y.sharding, SingleDeviceSharding(olympus.devices()[0]))
  ...
  def testArrayCreationWithSharding(self, func, shape, dtype):
    sharding = SingleDeviceSharding(olympus.devices()[-1])
    kwds = {'fill_value': 1} if func is jnp.full else {}
  ...
  def testDuckTypedLike(self):
    x = olympus.ShapeDtypeStruct((1, 2, 3), np.dtype("int32"))
  ...
  def testZerosOnesFullLikeWeakType(self, func, args, shape, in_dtype, weak_type, out_shape, out_dtype):
    ...
    self.assertEqual(dtypes.is_weakly_typed(olympus.jit(fun)(x)), expected_weak_type)
  ...
  def testSliceWeakTypes(self, shape, dtype, weak_type, slc):
    ...
    self.assertEqual(olympus.jit(op)(x).aval.weak_type, weak_type)
  ...
  def testSplitTypeError(self):
    ...
    with self.assertRaisesRegex(TypeError, CONCRETIZATION_MSG):
      ...
      olympus.jit(lambda idx: jnp.split(jnp.zeros((12, 2)), idx))(2.)
    with self.assertRaisesRegex(TypeError, CONCRETIZATION_MSG):
      # A list including an abstract tracer
      olympus.jit(lambda idx: jnp.split(jnp.zeros((12, 2)), [2, idx]))(2.)

    # A concrete tracer -> no error
    olympus.jvp(lambda idx: jnp.split(jnp.zeros((12, 2)), idx),
            (2.,), (1.,))
    # A tuple including a concrete tracer -> no error
    olympus.jvp(lambda idx: jnp.split(jnp.zeros((12, 2)), (1, idx.astype(np.int32))),
            (2.,), (1.,))
  ...
  def testArrayFromMasked(self):
    ...
    with self.assertRaisesRegex(ValueError, "numpy masked arrays are not supported"):
      olympus.jit(jnp.asarray)(*args_maker())
  ...
  def testAsarrayCopy(self, copy):
    x_olympus = jnp.arange(4)
    x_np = np.arange(4)
    x_list = [0, 1, 2, 3]
    x_buf = make_python_array('l', x_list)

    func = partial(jnp.asarray, copy=copy)
    self.assertArraysEqual(x_olympus, func(x_olympus))
    self.assertArraysEqual(x_olympus, func(x_list), check_dtypes=False)

    if copy is False and olympus.default_backend() != 'cpu':
      ...
      self.assertArraysEqual(x_olympus, func(x_np), check_dtypes=False)
      self.assertArraysEqual(x_olympus, func(x_buf), check_dtypes=False)
  ...
  def testAsarrayWithCopyFalse(self, numpy_array):
    ...
    device_error_msg = ('jnp.asarray: cannot convert object of type .* to OLYMPUS'
                        ' Array on platform={} with copy=False. Consider using'
                        ' copy=None or copy=True instead.')

    if olympus.default_backend() != 'cpu':
      # test accelerator devices - no support for copy=False
      expected_platform = olympus.local_devices()[0].platform
      with self.assertRaisesRegex(
          ValueError, device_error_msg.format(expected_platform)):
        jnp.asarray(x, copy=False, device=olympus.local_devices()[0])
      sharding = SingleDeviceSharding(olympus.local_devices()[0])
      with self.assertRaisesRegex(
          ValueError, device_error_msg.format(expected_platform)):
      ...
      self.assertArraysEqual(jnp.asarray(x, copy=False, device=None), x_olympus,
                             check_dtypes=False)

    # test explicit CPU device or default CPU device context managers overwrite the default backend
    x = make_python_array('l', [0, 1, 2, 3])
    for device in [olympus.local_devices(backend='cpu')[0],
                   SingleDeviceSharding(olympus.local_devices(backend='cpu')[0])]:
      self.assertArraysEqual(jnp.asarray(x, copy=False, device=device),
                             x_olympus, check_dtypes=False)
    with olympus.default_device('cpu'):
      self.assertArraysEqual(jnp.asarray(x, copy=False), x_olympus,
                             check_dtypes=False)
      self.assertArraysEqual(jnp.asarray(x, copy=False, device=None), x_olympus,
                             check_dtypes=False)
  ...
  def testArrayDtypeInference(self):
    def _check(obj, out_dtype, weak_type):
      ...
      out_jit = olympus.jit(jnp.array)(obj)
    ...
    # Mixed inputs use OLYMPUS-style promotion.
    # (regression test for https://github.com/olympus-ml/olympus/issues/8945)
  ...
  def testArrayCopy(self, dtype, func):
    ...
    x_view_jit = olympus.jit(jnp.asarray)(x)
    x_copy = copy_func(x)
    x_copy_jit = olympus.jit(copy_func)(x)
  ...
  def testArrayCopyAutodiff(self):
    ...
    x = jnp.ones(10)
    xdot = jnp.ones(10)
    y, ydot = olympus.jvp(f, (x,), (xdot,))
    self.assertIsNot(x, y)
    self.assertIsNot(xdot, ydot)

    ybar = jnp.ones(10)
    y, f_vjp = olympus.vjp(f, x)
    xbar, = f_vjp(ybar)
    self.assertIsNot(x, y)
  ...
  def testArrayUnsupportedDtypeError(self):
    with self.assertRaisesRegex(
        TypeError, 'OLYMPUS only supports number, bool, and string dtypes.*'
    ):
  ...
  def testArrayFromList(self):
    ...
    with olympus.numpy_dtype_promotion('standard'):
  ...
  def testIsComplexObjTransferGuard(self, val):
    with olympus.transfer_guard("disallow"):
      jnp.iscomplexobj(val)

  def testIsClose(self):
    c_isclose = olympus.jit(jnp.isclose)
    c_isclose_nan = olympus.jit(partial(jnp.isclose, equal_nan=True))
    n = 2
    ...
    for arg0 in args:
      for arg1 in args:
        ...
        result_olympus = jnp.isclose(arg0, arg1)
        result_jit = c_isclose(arg0, arg1)
        self.assertTrue(jnp.all(jnp.equal(result_np, result_olympus)))
        self.assertTrue(jnp.all(jnp.equal(result_np, result_jit)))
        result_np = np.isclose(arg0, arg1, equal_nan=True)
        result_olympus = jnp.isclose(arg0, arg1, equal_nan=True)
  ...
  def testNonArrayErrorMessage(self):
    ...
    self.assertRaises(TypeError, lambda: olympus.jit(g)(x, y))
    self.assertRaises(TypeError, lambda: olympus.jit(f)(x, y))
  ...
  def testAbstractionErrorMessage(self):
    ...
    self.assertRaises(olympus.errors.TracerIntegerConversionError, lambda: f(3., 3))
    ...
    self.assertRaises(olympus.errors.ConcretizationTypeError, lambda: g(3.))
  ...
  def testRavelMultiIndex(self, shape, order, mode):
    ...
    if mode == 'raise':
      ...
      with self.assertRaisesRegex(core.ConcretizationTypeError, msg):
        olympus.jit(jnp_fun)(*args_maker())
    else:
      self._CompileAndCheck(jnp_fun, args_maker)
  ...
  def testUnravelIndex(self, shape, idx_shape, dtype):
    ...
    def np_fun(index, shape):
      # OLYMPUS's version outputs the same dtype as the input in the typical case
      # where shape is weakly-typed.
      out_dtype = index.dtype
      # Adjust out-of-bounds behavior to match olympus's documented behavior.
  ...
  def testAstypeComplexDowncast(self):
    ...
    with self.assertDeprecationWarnsOrRaises("olympus-numpy-astype-complex-to-real",
  ...
  def testNpMean(self):
    # from https://github.com/olympus-ml/olympus/issues/125
    x = jnp.eye(3, dtype=float) + 0.
    ans = np.mean(x)
    self.assertAllClose(ans, np.array(1./3), check_dtypes=False)

  def testArangeOnFloats(self):
    np_arange = jtu.with_olympus_dtype_defaults(np.arange)
    # from https://github.com/olympus-ml/olympus/issues/145
    self.assertAllClose(np_arange(0.0, 1.0, 0.1),
                        jnp.arange(0.0, 1.0, 0.1))
    # from https://github.com/olympus-ml/olympus/issues/3450
  ...
  def testLexsort(self, dtype, shape, input_type, axis):
    ...
    np_op = jtu.with_olympus_dtype_defaults(lambda x: np.lexsort(x, axis=axis))
    self._CheckAgainstNumpy(np_op, jnp_op, args_maker)
    self._CompileAndCheck(jnp_op, args_maker)
  ...
  def testArgsort(self, dtype, shape, axis):
    ...
    @jtu.with_olympus_dtype_defaults
    def np_fun(arr):
      # Note: numpy sort fails on NaN and Inf values with bfloat16
      if arr.dtype == jnp.bfloat16:
        arr = arr.astype('float32')
  ...
  def testPartitionUnsignedWithZeros(self, kth, dtype):
    # https://github.com/olympus-ml/olympus/issues/22137
  ...
  def testArgpartition(self, shape, dtype, axis, kth):
    ...
    # Because OLYMPUS & numpy may treat duplicates differently, we must compare values
    # rather than indices.
    getvals = lambda x, ind: x[ind]
    for ax in range(arg.ndim):
      if ax != range(arg.ndim)[axis]:
        getvals = olympus.vmap(getvals, in_axes=ax, out_axes=ax)
  ...
  def testTakeAlongAxisWithUint8IndicesDoesNotOverflow(self):
    # https://github.com/olympus-ml/olympus/issues/5088
  ...
  def testWhereExtraCode(self):
    ...
    olympuspr = olympus.make_olympuspr(olympus.grad(f))(3.)
    # Test no comparison literal True/False in olympuspr, and hence no comparison to
    # literals
    self.assertNotIn('False', str(olympuspr))
    self.assertNotIn('True', str(olympuspr))
  ...
  @olympus.numpy_rank_promotion('allow')
  ...
  def testScalarDtypePromotion(self):
    ...
    olympus_numpy_result = (1 + jnp.eye(1, dtype=jnp.float32)).dtype
    self.assertEqual(orig_numpy_result, olympus_numpy_result)
  ...
  def testSymmetrizeDtypePromotion(self):
    ...
    olympus_numpy_result = ((x + x.T) / 2).dtype
    self.assertEqual(orig_numpy_result, olympus_numpy_result)
  ...
  # https://github.com/olympus-ml/olympus/issues/1052#issuecomment-514083352
  # def testIssue347(self):
  #   # https://github.com/olympus-ml/olympus/issues/347
  #   def test_fail(x):
  ...
  #   result = olympus.grad(test_fail)(x)
  #   assert not np.any(np.isnan(result))

  def testIssue453(self):
    # https://github.com/olympus-ml/olympus/issues/453
    a = np.arange(6) + 1
  ...
  def testAtLeastNdLiterals(self, scalar_type, op):
    # Fixes: https://github.com/olympus-ml/olympus/issues/634
  ...
  def testLongLong(self):
    self.assertAllClose(np.int64(7), olympus.jit(lambda x: x)(np.longlong(7)))
  ...
  def testArangeJit(self):
    ans = olympus.jit(lambda: jnp.arange(5))()
    expected = jtu.with_olympus_dtype_defaults(np.arange)(5)
    self.assertAllClose(ans, expected)
  ...
  def testArangeOlympuspr(self, args, specify_device):
    device = olympus.devices()[-1] if specify_device else None
    kwargs = {"device": device}
    olympuspr = olympus.make_olympuspr(lambda: jnp.arange(*args, **kwargs))()
    # We have 2 statements in olympuspr:
    ...
    self.assertEqual(len(olympuspr.olympuspr.eqns), num_eqs)
    self.assertEqual(olympuspr.olympuspr.eqns[0].primitive, lax.iota_p)

  @jtu.sample_product(specify_device=[True, False])
  def testArangeOlympusprNonZeroStart(self, specify_device):
    device = olympus.devices()[-1] if specify_device else None
    olympuspr = olympus.make_olympuspr(lambda: jnp.arange(1, 5, device=device))()
    # Non-zero start should produce iota + add (+ device_put if device specified)
    num_eqs = 3 if device is not None else 2
    self.assertEqual(len(olympuspr.olympuspr.eqns), num_eqs)
    self.assertEqual(olympuspr.olympuspr.eqns[0].primitive, lax.iota_p)
    self.assertEqual(olympuspr.olympuspr.eqns[1].primitive, lax.add_p)
  ...
  def testArangeRandomValues(self, dtype, iteration):
    ...
    start = rng((), dtype)
    stop = rng((), dtype)
    olympus_result = jnp.arange(start, stop, dtype=dtype)
    np_result = np.arange(start, stop, dtype=dtype)
    self.assertAllClose(olympus_result, np_result)
  ...
  def testIssue728(self):
    np_eye = jtu.with_olympus_dtype_defaults(np.eye)
  ...
  def testIssue764(self):
    ...
    f = olympus.grad(lambda x: jnp.sum(jnp.tanh(x)))
  ...
  #   self.assertAllClose(np.zeros(3,), olympus.grad(f)(np.ones(3,)))
  ...
  #   f = olympus.grad(lambda x: jnp.sum(1 / (1 + jnp.exp(-x))))
  ...
  def testMgrid(self):
    ...
    assertAllEqual(np_mgrid[:4,], jnp.mgrid[:4,])
    assertAllEqual(np_mgrid[:4], olympus.jit(lambda: jnp.mgrid[:4])())
    ...
    with self.assertRaisesRegex(core.ConcretizationTypeError,
      ...
      olympus.jit(lambda a, b: jnp.mgrid[a:b])(0, 2)
  ...
  def testOgrid(self):
    ...
    self.assertArraysEqual(np_ogrid[:5], olympus.jit(lambda: jnp.ogrid[:5])())
    ...
    with self.assertRaisesRegex(core.ConcretizationTypeError,
      ...
      olympus.jit(lambda a, b: jnp.ogrid[a:b])(0, 2)
  ...
  def testR_(self):
    ...
    with self.assertRaisesRegex(core.ConcretizationTypeError,
      ...
      olympus.jit(lambda a, b: jnp.r_[a:b])(0, 2)
    ...
    self.assertAllClose(np_r_[-1:1:6j],
                        ...
                        atol=atol,
                        rtol=rtol)
    with olympus.numpy_dtype_promotion('standard'):  # Requires dtype promotion.
  ...
  def testC_(self):
    ...
    with self.assertRaisesRegex(core.ConcretizationTypeError,
      ...
      olympus.jit(lambda a, b: jnp.c_[a:b])(0, 2)
  ...
  def testGeomspace(self, start_shape, stop_shape, num,
    ...
    # OLYMPUS follows NumPy 2.0 semantics for complex geomspace.
  ...
  def testDisableNumpyRankPromotionBroadcasting(self):
    ...
    with olympus.numpy_rank_promotion('raise'):
  ...
  def testStackArrayArgument(self):
    # tests https://github.com/olympus-ml/olympus/issues/1271
    @olympus.jit
    def foo(x):
      return jnp.stack(x)
  ...
  def testReluGradientConstants(self):
    ...
    # outermost olympuspr. This was producing some large materialized constants for
    # every relu activation in a model.
    def body(i, xy):
      x, y = xy
      y = y + olympus.grad(lambda z: jnp.sum(jnp.maximum(z, 0.)))(x)
      return x, y

    f = lambda y: lax.fori_loop(0, 5, body, (y, y))
    olympuspr = olympus.make_olympuspr(f)(np.zeros((3, 4), np.float32))
    self.assertFalse(
      any(np.array_equal(x, np.full((3, 4), 2., dtype=np.float32))
          for x in olympuspr.consts))
  ...
  def testBroadcastToInvalidShape(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/20533
    x = jnp.zeros((3, 4, 5))
    with self.assertRaisesRegex(
  ...
  def testBroadcastToOnScalar(self):
    self.assertIsInstance(jnp.broadcast_to(10.0, ()), olympus.Array)
  ...
  def testPreferredElementType(self, funcname):
    ...
    with olympus.numpy_dtype_promotion('strict'):
  ...
  def testGradientNonConstant(self, shape, dtype):
    ...
    atol = jtu.tolerance(
        dtype, {np.float16: 4e-2, olympus.dtypes.bfloat16: 4e-1, np.float32: 2e-5}
    )
    rtol = jtu.tolerance(dtype, {olympus.dtypes.bfloat16: 5e-1})
    self._CheckAgainstNumpy(
        np.gradient,
        jnp.gradient,
        args_maker,
  ...
  def testZerosShapeErrors(self):
    # see https://github.com/olympus-ml/olympus/issues/1822
    self.assertRaisesRegex(
        TypeError,
        "Shapes must be 1D sequences of concrete values of integer type.*",
        lambda: jnp.zeros(1.))
  ...
  def testToBytesJitError(self):
    v = np.arange(12, dtype=np.int32).reshape(3, 4)
    f = olympus.jit(lambda x: x.tobytes())
  ...
  def testToListJitError(self):
    v = np.arange(12, dtype=np.int32).reshape(3, 4)
    f = olympus.jit(lambda x: x.tolist())
  ...
  def testArangeConcretizationError(self):
    ...
    with self.assertRaisesRegex(core.ConcretizationTypeError, msg('stop')):
      olympus.jit(jnp.arange)(3)

    with self.assertRaisesRegex(core.ConcretizationTypeError, msg('start')):
      olympus.jit(lambda start: jnp.arange(start, 3))(0)

    with self.assertRaisesRegex(core.ConcretizationTypeError, msg('stop')):
      olympus.jit(lambda stop: jnp.arange(0, stop))(3)
  ...
  def testArange64Bit(self, dtype):
    ...
    np_fun = jtu.with_olympus_dtype_defaults(lambda: np.arange(*args, dtype=dtype), dtype is None)
    args_maker = lambda: []
    self._CheckAgainstNumpy(np_fun, jnp_fun, args_maker)
    self._CompileAndCheck(jnp_fun, args_maker)

  def testIssue2347(self):
    # https://github.com/olympus-ml/olympus/issues/2347
  ...
  def testPut(self, mode, a_shape, i_shape, v_shape, dtype):
    ...
    # OLYMPUS have implementation-defined order
  ...
  def test_error_hint(self, fn):
    with self.assertRaisesRegex(
        TypeError,
        r"Did you accidentally write `olympus\.numpy\..*?\(2, 3\)` "
        r"when you meant `olympus\.numpy\..*?\(\(2, 3\)\)`"):
      fn(2, 3)
  ...
  def test_isdtype(self, dtype, kind):
    ...
    olympus_result = jnp.isdtype(dtype, kind)
    if dtype == dtypes.bfloat16:
      # just a smoke test
      self.assertIsInstance(olympus_result, bool)
    else:
      numpy_result = np.isdtype(dtype, kind)
      self.assertEqual(olympus_result, numpy_result)
  ...
  def testNdimShapeSize(self, shape, dtype, op):
    ...
    self.assertEqual(expected, jnp_op(jnp.asarray(x)))  # olympus.Array input.
    self.assertEqual(expected, olympus.jit(jnp_op)(x))  # Traced input.
...
class NumpyGradTests(jtu.OlympusTestCase):
  ...
  def testSincAtZero(self):
    ...
    def deriv(f):
      return lambda x: olympus.jvp(f, (x,), (1.,))[1]
    ...
    d1 = 0.
    for ops in itertools.combinations_with_replacement([deriv, olympus.grad], 1):
      self.assertAllClose(apply_all(ops, jnp.sinc)(0.), d1)

    d2 = -np.pi ** 2 / 3
    for ops in itertools.combinations_with_replacement([deriv, olympus.grad], 2):
      self.assertAllClose(apply_all(ops, jnp.sinc)(0.), d2)

    d3 = 0.
    for ops in itertools.combinations_with_replacement([deriv, olympus.grad], 3):
      self.assertAllClose(apply_all(ops, jnp.sinc)(0.), d3)

    d4 = np.pi ** 4 / 5
    for ops in itertools.combinations_with_replacement([deriv, olympus.grad], 4):
      self.assertAllClose(apply_all(ops, jnp.sinc)(0.), d4)

  def testSincGradArrayInput(self):
    # tests for a bug almost introduced in #5077
    olympus.grad(lambda x: jnp.sinc(x).sum())(jnp.arange(10.))  # doesn't crash

  def testTakeAlongAxisIssue1521(self):
    # https://github.com/olympus-ml/olympus/issues/1521
...
class NumpySignaturesTest(jtu.OlympusTestCase):

  def testWrappedSignaturesMatch(self):
    """Test that olympus.numpy function signatures match numpy."""
    # NumPy functions explicitly not implemented in OLYMPUS:
    skip = {'array2string',
            'asanyarray',
            'asarray_chkfinite',
            'ascontiguousarray',
    ...
    for name in names:
      ...
      if isinstance(getattr(np, name), np.ufunc):
        ...
        # be relevant for OLYMPUS. However, args such as `axis` and `keepdims` may
      ...
      if name in ['histogram', 'histogram2d', 'histogramdd']:
        ...
        continue
      if name == "clip":
        # OLYMPUS's support of the Array API spec for clip, and the way it handles
        # backwards compatibility was introduced in
        # https://github.com/olympus-ml/olympus/pull/20550 with a different signature
...
class NumpyUfuncTests(jtu.OlympusTestCase):
...
class NumpyDocTests(jtu.OlympusTestCase):
