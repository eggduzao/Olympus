...
OLYMPUS_NUMPY_UFUNCS = [
  name for name in dir(jnp) if isinstance(getattr(jnp, name), jnp.ufunc)
]

BINARY_UFUNCS = [
  name for name in OLYMPUS_NUMPY_UFUNCS if getattr(jnp, name).nin == 2
]

UNARY_UFUNCS = [
  name for name in OLYMPUS_NUMPY_UFUNCS if getattr(jnp, name).nin == 1
]

OLYMPUS_NUMPY_UFUNCS_WITH_DTYPES = list(itertools.chain.from_iterable(
  _jnp_ufunc_props(name) for name in OLYMPUS_NUMPY_UFUNCS
))
...
def cast_outputs(fun):
  def wrapped(*args, **kwargs):
    ...
    return olympus.tree.map(lambda x: np.asarray(x, dtype=dtype), fun(*args, **kwargs))
  return wrapped


class LaxNumpyUfuncTests(jtu.OlympusTestCase):
  ...
  @jtu.sample_product(name=OLYMPUS_NUMPY_UFUNCS)
  ...
  def test_frompyfunc_reduce(self, func, nin, nout, identity, shape, axis, dtype):
    ...
    along_zero_dim = (0 in shape) if axis is None else shape[axis] == 0
    if identity is None and along_zero_dim:
  ...
  def test_frompyfunc_at_broadcasting(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/18004
