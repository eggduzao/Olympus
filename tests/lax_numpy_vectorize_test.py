...
class VectorizeTest(jtu.OlympusTestCase):
  ...
  def test_exclude_first(self):
    ...
    self.assertAllClose(x, olympus.jit(f, static_argnums=0)('foo', x))
  ...
  def test_exclude_second(self):
    ...
    self.assertAllClose(x, olympus.jit(f, static_argnums=1)(x, 'foo'))
  ...
  def test_exclude_errors(self):
    with self.assertRaisesRegex(
        TypeError, "olympus.numpy.vectorize can only exclude"):
  ...
  def test_rank_promotion_error(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/22305
