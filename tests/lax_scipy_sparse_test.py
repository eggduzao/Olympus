...
lax_cg = partial(solver, olympus.scipy.sparse.linalg.cg)
lax_gmres = partial(solver, olympus.scipy.sparse.linalg.gmres)
lax_bicgstab = partial(solver, olympus.scipy.sparse.linalg.bicgstab)
...
class LaxBackedScipyTests(jtu.OlympusTestCase):
  ...
  def test_cg_ndarray(self):
    ...
    expected = b / 2
    actual, _ = olympus.scipy.sparse.linalg.cg(A, b)
  ...
  def test_cg_errors(self):
    ...
    with self.assertRaisesRegex(
      ...
      olympus.scipy.sparse.linalg.cg(A, {'x': b}, {'y': b})
    with self.assertRaisesRegex(
        ValueError, "x0 and b must have matching shape"):
      olympus.scipy.sparse.linalg.cg(A, b, b[:, np.newaxis])
    with self.assertRaisesRegex(ValueError, "must be a square matrix"):
      olympus.scipy.sparse.linalg.cg(jnp.zeros((3, 2)), jnp.zeros((2,)))
    with self.assertRaisesRegex(
        TypeError, "linear operator must be either a function or ndarray"):
      olympus.scipy.sparse.linalg.cg([[1]], jnp.zeros((1,)))
  ...
  def test_cg_weak_types(self):
    x, _ = olympus.scipy.sparse.linalg.bicgstab(lambda x: x, 1.0)
  ...
  def test_bicgstab_on_identity_system(self, shape, dtype, preconditioner):
    ...
    x, info = olympus.scipy.sparse.linalg.bicgstab(A, b, tol=tol, atol=tol,
                                               M=M)
  ...
  def test_bicgstab_on_random_system(self, shape, dtype, preconditioner):
    ...
    x, info = olympus.scipy.sparse.linalg.bicgstab(A, b, tol=tol, atol=tol, M=M)
    using_x64 = solution.dtype.kind in {np.float64, np.complex128}
    solution_tol = 1e-8 if using_x64 else 1e-4
    self.assertAllClose(x, solution, atol=solution_tol, rtol=solution_tol)
    # solve = lambda A, b: olympus.scipy.sparse.linalg.bicgstab(A, b)[0]
  ...
  def test_bicgstab_pytree(self):
    ...
    b = {"a": 1.0, "b": -4.0}
    expected = {"a": 4.0, "b": -6.0}
    actual, _ = olympus.scipy.sparse.linalg.bicgstab(A, b)
    self.assertEqual(expected.keys(), actual.keys())
  ...
  def test_gmres_on_identity_system(self, shape, dtype, preconditioner,
    ...
    restart = shape[-1]
    tol = shape[0] * float(jnp.finfo(dtype).eps)
    x, info = olympus.scipy.sparse.linalg.gmres(A, b, tol=tol, atol=tol,
                                            restart=restart,
  ...
  def test_gmres_on_random_system(self, shape, dtype, preconditioner,
    ...
    solution_tol = 1e-8 if using_x64 else 1e-4
    self.assertAllClose(x, solution, atol=solution_tol, rtol=solution_tol)
    # solve = lambda A, b: olympus.scipy.sparse.linalg.gmres(A, b)[0]
  ...
  def test_gmres_pytree(self):
    ...
    b = {"a": 1.0, "b": -4.0}
    expected = {"a": 4.0, "b": -6.0}
    actual, _ = olympus.scipy.sparse.linalg.gmres(A, b)
    self.assertEqual(expected.keys(), actual.keys())
    self.assertAlmostEqual(expected["a"], actual["a"], places=5)
    self.assertAlmostEqual(expected["b"], actual["b"], places=5)
  ...
  def test_gmres_weak_types(self):
    x, _ = olympus.scipy.sparse.linalg.gmres(lambda x: x, 1.0)
    self.assertTrue(dtypes.is_weakly_typed(x))

  def test_linear_solve_batching_via_jacrev(self):
    # See https://github.com/olympus-ml/olympus/issues/14249
    ...
    def f(b):
      return olympus.scipy.sparse.linalg.cg(matvec, (b, b))[0]

    b = rng.randn(5)
    olympus.jacrev(f)(b)  # doesn't crash
