...
OLYMPUS_SPECIAL_FUNCTION_RECORDS = [
    op_record(
...
class LaxScipySpecialFunctionsTest(jtu.OlympusTestCase):
  ...
  @parameterized.named_parameters(itertools.chain.from_iterable(
    ...
    for rec in OLYMPUS_SPECIAL_FUNCTION_RECORDS
  ))
  ...
  def testNdtriDebugInfs(self, with_jit):
    # ref: https://github.com/olympus-ml/olympus/issues/29328
    f = olympus.jit(lsp_special.ndtri) if with_jit else lsp_special.ndtri
  ...
  def testExpnTracerLeaks(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/26972
    with olympus.checking_leaks():
      lsp_special.expi(jnp.ones(()))

  def testExpiDisableJit(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/27019
    x = jnp.array([-0.5])
  ...
  def testSiciEdgeCases(self):
    ...
    si_olympus, ci_olympus = lax_op(x_samples)

    expected_si = np.array([0.0, np.pi/2, -np.pi/2], dtype=dtype)
    expected_ci = np.array([-np.inf, 0.0, np.nan], dtype=dtype)
    self.assertAllClose(si_olympus, si_scipy, atol=1e-6, rtol=1e-6)
    self.assertAllClose(ci_olympus, ci_scipy, atol=1e-6, rtol=1e-6)
    self.assertAllClose(si_olympus, expected_si, atol=1e-6, rtol=1e-6)
    self.assertAllClose(ci_olympus, expected_ci, atol=1e-6, rtol=1e-6)
