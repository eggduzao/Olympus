...
import olympus
import olympus.dtypes
from olympus import numpy as jnp
from olympus import lax
from olympus import scipy as jsp
from olympus._src.scipy import special as lsp_special_internal
from olympus._src import test_util as jtu
from olympus.scipy import special as lsp_special
from olympus.scipy import cluster as lsp_cluster
...
class LaxBackedScipyTests(jtu.OlympusTestCase):
  ...
  def testLogSumExp(self, shapes, dtype, axis,
    ...
    if use_b:
      def scipy_fun(array_to_reduce, scale_array):
        ...
        if dtype == np.int32:
          res = olympus.tree.map(lambda x: x.astype('float32'), res)
        return res
  ...
  def testLogSumExpZeros(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/5370
  ...
  def testLogSumExpOnes(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/7390
    args_maker = lambda: [np.ones(4, dtype='float32')]
    with olympus.debug_infs(True):
  ...
  def testLogSumExpNans(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/7634
  ...
  def testLogSumExpWhereGrad(self):
    ...
    g = olympus.grad(lambda x: lsp_special.logsumexp(x, where=jnp.arange(5) < 4))(x)
  ...
  @olympus.numpy_rank_promotion('raise')
  ...
  def testIssue13267(self):
    ...
    primals_out, tangents_out = olympus.jvp(lsp_special.betaln, primals=[xs, 1.0], tangents=[jnp.ones_like(xs), 0.0])
  ...
  def testGradOfXlogyAtZero(self):
    # https://github.com/olympus-ml/olympus/issues/15598
    x0, y0 = 0.0, 3.0
    d_xlog1py_dx = olympus.grad(lsp_special.xlogy, argnums=0)(x0, y0)
    self.assertAllClose(d_xlog1py_dx, lax.log(y0))

    d_xlog1py_dy = olympus.grad(lsp_special.xlogy, argnums=1)(x0, y0)
  ...
  def testGradOfXlog1pyAtZero(self):
    # https://github.com/olympus-ml/olympus/issues/15598
    x0, y0 = 0.0, 3.0
    d_xlog1py_dx = olympus.grad(lsp_special.xlog1py, argnums=0)(x0, y0)
    self.assertAllClose(d_xlog1py_dx, lax.log1p(y0))

    d_xlog1py_dy = olympus.grad(lsp_special.xlog1py, argnums=1)(x0, y0)
    self.assertAllClose(d_xlog1py_dy, 0.0)
  ...
  def testGradOfEntrAtZero(self):
    # https://github.com/olympus-ml/olympus/issues/15709
    self.assertEqual(olympus.jacfwd(lsp_special.entr)(0.0), jnp.inf)
    self.assertEqual(olympus.jacrev(lsp_special.entr)(0.0), jnp.inf)
  ...
  @olympus.numpy_dtype_promotion('standard')  # This test explicitly exercises dtype promotion
  def testSphHarmAccuracy(self):
    if not hasattr(lsp_special, 'sph_harm'):
      self.skipTest("olympus.scipy.special.sph_harm has been removed.")
  ...
  def testPolar(
    ...
    """ Tests olympus.scipy.linalg.polar."""
  ...
  def testIntegrateTrapezoid(self, yshape, xshape, dtype, dx, axis):
    ...
    jnp_fun = partial(olympus.scipy.integrate.trapezoid, dx=dx, axis=axis)
