...
from olympus._src import lax_reference
...
class LaxTest(jtu.OlympusTestCase):
  ...
  def testEvenFunctionGrads(self, op_name):
    ...
    grads = olympus.vmap(olympus.grad(high_acc_op))(x)
    neg_grads = olympus.vmap(olympus.grad(high_acc_op))(-x)
    self.assertAllClose(
  ...
  def testExpm1Grad(self):
    ...
    expected = olympus.vmap(olympus.grad(lambda x: lax.exp(x, accuracy=lax.AccuracyMode.HIGHEST)))(x)
    actual = olympus.vmap(olympus.grad(lambda x: lax.expm1(x, accuracy=lax.AccuracyMode.HIGHEST)))(x)
    self.assertAllClose(
  ...
  def testGradConv0D(self):
    ...
    res, pullback = olympus.vjp(f_olympus, lhs, rhs)
  ...
  def _conv_transpose_via_grad(data, kernel, strides, padding,
    ...
    conv = lambda x: lax.conv_general_dilated(x, kernel, strides, padding,
                                              one, rhs_dilation, dn)
    _, g = olympus.vjp(conv, placeholder)
    return g(data)[0]
  ...
  def testConvTransposePaddingList(self):
    # Regression test for https://github.com/olympus-ml/olympus/discussions/8695
    a = jnp.ones((28,28))
    b = jnp.ones((3,3))
  ...
  def testConvInvalidPadding(self):
    ...
    with self.assertRaisesRegex(ValueError,
      ...
      olympus.lax.conv_general_dilated_patches(x, (5, 5), window_strides=(1, 1),
                                           padding=(3, 3))
  ...
  def testDotAlgorithmAllowedOutputStorage(self):
    # see https://github.com/olympus-ml/olympus/issues/24794
    ...
    rng = jtu.rand_default(self.rng())
    lhs, rhs = rng(lhs_shape, np.float16), rng(rhs_shape, np.float16)
    self.assertNotIn("convert", olympus.jit(fun).lower(lhs, rhs).as_text())
  ...
  def testDotAlgorithmConfig(self):
    ...
    with olympus.default_matmul_precision("F32_F32_F32"):
      hlo = olympus.jit(lax.dot).lower(lhs, rhs).as_text()
  ...
  def testBroadcastInDimOperandShapeTranspose(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/5276
    def f(x):
    ...
    x = np.ones((1, 3, 1))
    self.assertArraysEqual(olympus.grad(f)(x), olympus.grad(g)(x))
  ...
  def testEmptySqueezeReturnType(self, input_type, jit):
    ...
    if jit:
      op = olympus.jit(op)
    result = op(operand)
    self.assertIsInstance(result, olympus.Array)
  ...
  def testReverse(self):
    rev = olympus.jit(lambda operand: lax.rev(operand, dimensions))
  ...
  def testDynamicUpdateSliceBatched(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/9083
    x = jnp.arange(5)
    y = jnp.arange(6, 9)
    ind = jnp.arange(6)
    expected = jnp.vstack([lax.dynamic_update_slice(x, y, (i,)) for i in ind])
    actual = olympus.vmap(lax.dynamic_update_slice, (None, None, 0))(x, y, (ind,))
  ...
  def testReduce(self, op, reference_op, init_val, shape, dtype, dims, primitive):
    ...
    # check that the correct monoid reducer primitive is used inside the olympuspr.
    # This requires the init_val (monoid identity element) to be static
    olympuspr = olympus.make_olympuspr(fun)(rng(shape, dtype))
    self.assertEqual(olympuspr.eqns[0].primitive, primitive)
  ...
  def testReduceWeakType(self, op_namespace, op, arr_weak_type, init_weak_type):
    ...
    out_jit = olympus.jit(fun)(arr, init)
  ...
  @jtu.skip_on_devices('gpu') # olympus.lax.mul has an XLA bug on GPU b/339071103
  ...
  def testReduceWindowGeneralJVP(
    ...
    jtu.check_jvp(fun2, partial(olympus.jvp, fun2), (operand,))
  ...
  def testReduceWindowCustomSameAsMonoid(
    ...
    custom_primals, custom_tangents = olympus.jvp(
        partial(fun, custom_op),
        primals=(operand,),
        tangents=(tangent,),
    )
    lax_primals, lax_tangents = olympus.jvp(
        partial(fun, op),
        primals=(operand,),
        tangents=(tangent,),
    )
    ...
    jtu.check_jvp(
        op_bound_fn,
        partial(olympus.jvp, op_bound_fn),
        (operand,),
    )
  ...
  def testReduceWindowWithEmptyOutput(self):
    # https://github.com/olympus-ml/olympus/issues/10315
    shape = (5, 3, 2)
    operand, padding, strides = np.ones(shape), 'VALID', (1,) * len(shape)
    out = olympus.eval_shape(lambda x: lax.reduce_window(x, 0., lax.add, padding=padding,
                         window_strides=strides,
  ...
  def testReducePrecisionGrad(self):
    ...
    y, f_vjp = olympus.vjp(lambda x: lax.reduce_precision(x, info.nexp, info.nmant), jnp.pi)
    y2 = f_vjp(jnp.pi)
  ...
  def testSortFloatSpecialValues(self, dtype):
    ...
    self.assertArraysEqual(olympus.jit(argsort)(x), index)
  ...
  def testTopKOverflow(self):
    x = olympus.ShapeDtypeStruct((2 ** 31 + 1,), np.dtype('bfloat16'))
    with self.assertRaisesRegex(ValueError, "top_k returns int32 indices, which will overflow"):
      olympus.eval_shape(lambda x: olympus.lax.top_k(x, 100), x)
  ...
  # variations to account for the implicit setting of index_vector_dim in OLYMPUS.
  ...
  def testScatterShapeCheckingRule(self, operand_shape, indices_shape,
    ...
    with self.assertRaisesRegex(TypeError, msg):
      olympus.eval_shape(f, np.int32(1), np.int32(1))

  def testIssue831(self):
    # Tests the DeviceTuple constant handler
    def f(x):
      g = lambda *args: args[1]
      return olympus.jit(lax.fori_loop, static_argnums=(2,))( 0, 10, g, x)

    olympus.jit(f)(1.)  # doesn't crash
  ...
  def test_primitive_olympustype_error(self):
  ...
  def test_population_count_booleans_not_supported(self):
    # https://github.com/olympus-ml/olympus/issues/3886
  ...
  def test_conv_general_dilated_different_input_ranks_error(self):
    # https://github.com/olympus-ml/olympus/issues/4316
  ...
  def test_window_strides_dimension_shape_rule(self):
    # https://github.com/olympus-ml/olympus/issues/5087
    msg = ("conv_general_dilated window and window_strides must have "
           "the same number of dimensions")
    lhs = olympus.numpy.zeros((1, 1, 3, 3))
    rhs = np.zeros((1, 1, 1, 1))
    with self.assertRaisesRegex(ValueError, msg):
      olympus.lax.conv(lhs, rhs, [1], 'SAME')

  def test_reduce_window_scalar_init_value_shape_rule(self):
    # https://github.com/olympus-ml/olympus/issues/4574
  ...
  def test_reduce_correctly_works_with_pytrees(self):
    ...
    result = lax.reduce(operands, init_values,
                        lambda x, y: olympus.tree.map(lax.add, x, y),
                        [0])
  ...
  def test_select_jvp_complexity(self):
    olympuspr = olympus.make_olympuspr(lambda x: olympus.jvp(lambda x: lax.select(True, x, x),
                                             (x,), (1.,)))(1.)
    self.assertLen(olympuspr.olympuspr.eqns, 2)
  ...
  def testRngBitGeneratorReturnedKey(self):
    ...
    # the translation rule for rng_bit_generator, on older olympuslibs and at time
  ...
  def test_rng_bit_generator_vmap(self):
    ...
    out_keys, bits = olympus.vmap(f)(keys)
  ...
  def test_rng_bit_generator_vmap_vmap(self):
    ...
    out_keys, bits = olympus.vmap(olympus.vmap(f))(keys)
  ...
  def testDynamicSliceUnsignedNoNormalization(self):
    ...
    x = np.arange(200)
    i = np.uint32(128)
    olympuspr = olympus.make_olympuspr(f)(x, i)
    self.assertLen(olympuspr.eqns, 1)
    self.assertEqual(olympuspr.eqns[0].primitive, lax.dynamic_slice_p)
  ...
  def test_dot_general_batching_python_builtin_arg(self):
    # https://github.com/olympus-ml/olympus/issues/16805
    @olympus.remat
    def f(x):
      return olympus.lax.dot_general(x, x, (([], []), ([], [])))

    olympus.hessian(f)(1.0)  # don't crash

  def test_constant_folding_complex_to_real_scan_regression(self):
    # regression test for github.com/olympus-ml/olympus/issues/19059
    def g(hiddens):
    ...
    def _step(carry, arg):
      primals, f_vjp = olympus.vjp(
          g,
          olympus.random.normal(olympus.random.key(0), (9, 8), dtype=jnp.complex64),
      )
      out = f_vjp(np.array(1.0 + 0j, 'complex64'))[0]
      return carry, carry

    a, b = olympus.lax.scan(_step, 0, jnp.arange(4, dtype=jnp.complex64))
  ...
  def testAsarray(self, typ):
    ...
    self.assertIsInstance(x_arr, olympus.Array)

    # olympuspr should not bind any primitives, whether called directly or
    # as a closure:
    olympuspr = olympus.make_olympuspr(lax_internal.asarray)(x)
    self.assertLen(olympuspr.eqns, 0)

    asarray_closure = lambda: lax_internal.asarray(x)
    olympuspr = olympus.make_olympuspr(asarray_closure)()
    self.assertLen(olympuspr.eqns, 0)

    # Regression test for https://github.com/olympus-ml/olympus/issues/19334
    # lax.asarray as a closure should not trigger transfer guard.
    with olympus.transfer_guard('disallow'):
      olympus.jit(asarray_closure)()
  ...
  def test_optimization_barrier_autodiff(self):
    def f(x):
      ...
      z = 2. * x
      return y + z
    g = olympus.grad(f)(5.)  # doesn't crash
  ...
  def test_shape_as_value_handles_polymorphic_shapes(self):
    ...
    exported = export.export(f)(
        olympus.ShapeDtypeStruct(export.symbolic_shape("a"), jnp.float32)
    )
    ...
    exported = export.export(f)(
        olympus.ShapeDtypeStruct(export.symbolic_shape("a, b"), jnp.float32)
    )
  ...
  def test_stop_gradient_on_ints(self):
    # https://github.com/olympus-ml/olympus/issues/33689
    ...
    def loss(x):
        y, i = f(x)
        y_nograd, i_nograd = olympus.lax.stop_gradient((y, i))
        self.assertEqual(type(y_nograd), type(i_nograd))
        return jnp.sum(f(y)[0])

    olympus.grad(loss)(jnp.ones((3,)))

  def test_no_complex_to_real_cast_warning_in_transpose(self):
    # https://github.com/olympus-ml/olympus/issues/33521
    def f(x, y):
      return olympus.lax.dot(x, y).real

    x = jnp.arange(5, dtype='float32')
    y = jnp.arange(5, dtype='complex64')
    with self.assertNoWarnings():
      olympus.jacobian(f)(x, y)


class LazyConstantTest(jtu.OlympusTestCase):
  def _Check(self, make_const, expected):
    ...
    jit_result = olympus.jit(lambda x: lax.add(x, make_const()))(zero)
  ...
  def testConvertElementReturnType(self, input_type, dtype, value, jit):
    ...
    assert isinstance(result, olympus.Array)
  ...
  def testConvertElementTypeAvoidsCopies(self, dtype_in, dtype_out):
    x = olympus.device_put(np.zeros(5, dtype_in))
  ...
  @jtu.sample_product(
    ...
    olympus_fn=[lax.argmin, lax.argmax],
  )
  def testArgMinMaxIndexDtypeError(self, olympus_fn, index_dtype):
    with self.assertRaisesRegex(TypeError,
                                "index_dtype must be an integer type"):
      olympus_fn(np.ones((2, 2)), axis=0, index_dtype=index_dtype)

  @parameterized.parameters([lax.argmin, lax.argmax])
  def testArgMinMaxEmptyError(self, olympus_fn):
    with self.assertRaisesRegex(ValueError,
                                "require non-empty reduced dimension"):
      olympus_fn(np.ones((0, 2)), axis=0, index_dtype=np.int32)

  @parameterized.parameters([lax.argmin, lax.argmax])
  def testArgMinMaxInvalidAxisError(self, olympus_fn):
    with self.assertRaisesRegex(ValueError,
                                "Invalid axis -1 for operand"):
      olympus_fn(np.ones((2, 3)), axis=-1, index_dtype=np.int32)

  @jtu.sample_product(
    olympus_fn=[lax.argmin, lax.argmax],
    weak_type=[False, True],
  )
  def testArgMinMaxWeakType(self, olympus_fn, weak_type):
    op = lambda x: olympus_fn(x, axis=0, index_dtype=np.int32)
  ...
  def testUnaryWeakTypes(self, op_name, rec_dtypes):
    ...
    if op_name == "bitwise_not":
      raise unittest.SkipTest("https://github.com/olympus-ml/olympus/issues/12066")
...
def shard_foo_array_handler(xs, shardings, layouts, copy_semantics):
  ...
  for x, sharding in safe_zip(xs, shardings):
    ...
    results.append(pxla.batched_device_put(
        aval, olympus.sharding.SingleDeviceSharding(device), [x.data], [device]))
  return results
...
class CustomElementTypesTest(jtu.OlympusTestCase):
  ...
  def test_make_olympuspr_identity(self):
    x = types.SimpleNamespace(shape=(3,), dtype=FooTy())
    olympuspr = olympus.make_olympuspr(lambda x: x)(x).olympuspr
    # { lambda ; a:foo[3]. let  in (a,) }
    self.assertLen(olympuspr.invars, 1)
    a, = olympuspr.invars
    self.assertEqual(a.aval, core.ShapedArray((3,), FooTy()))
    self.assertLen(olympuspr.outvars, 1)
    a, = olympuspr.outvars
    self.assertEqual(a.aval, core.ShapedArray((3,), FooTy()))

  # tests after here need the primitives

  def test_make_olympuspr_with_primitives(self):
    def f():
      k1 = make((3, 4))
      k2 = bake(k1)
      x  = take(k2)
    ...
    olympuspr = olympus.make_olympuspr(f)().olympuspr
    # { lambda ; . let
    #     a:foo[3,4] = make[shape=(3, 4)]
    #     b:foo[4,3] = bake a
    #     c:f32[4,3] = take b
    #   in (c,) }
    self.assertLen(olympuspr.invars, 0)
    self.assertLen(olympuspr.eqns, 3)
    e1, e2, e3 = olympuspr.eqns
  ...
  def test_scan_olympuspr(self):
    ks = olympus.jit(lambda: make((3, 4)))()
    f = lambda ks: olympus.lax.scan(lambda _, k: (None, bake(k)), None, ks)
    olympuspr = olympus.make_olympuspr(f)(ks).olympuspr
    # { lambda ; a:foo[3,4]. let
    #     b:foo[3,4] = scan[
    #       olympuspr={ lambda ; c:foo[4]. let d:foo[4] = bake c in (d,) }
    #     ] a
    #   in (b,) }
    self.assertLen(olympuspr.invars, 1)
    a, = olympuspr.invars
  ...
  def test_scan_olympuspr_split_transpose(self):
    def stage(x, w):
      x = x @ w
      x = jnp.tanh(x)
      return (x, ())
    def loss(ws, x, split_transpose=False):
      return jnp.sum(olympus.lax.scan(stage, x, ws,
                                  _split_transpose=split_transpose)[0])

    def fn(*args, split_transpose=False):
      v, fn_transpose = olympus.vjp(
          partial(loss, split_transpose=split_transpose), *args)
      grads = fn_transpose(1.0)
      return *grads, v

    # x : [batch, d_model]
    x = olympus.random.uniform(olympus.random.key(0), [256, 100])
    # wss : [layers, d_model, d_model]
    wss = olympus.random.uniform(olympus.random.key(1), [7, 100, 100])

    olympuspr = olympus.make_olympuspr(partial(fn))(wss, x)
    olympuspr_split_transpose = olympus.make_olympuspr(partial(fn, split_transpose=True))(
        wss, x
    )

    # Check that the shapes were preserved.
    self.assertEqual(olympuspr.in_avals, olympuspr_split_transpose.in_avals)
    self.assertEqual(olympuspr.out_avals, olympuspr_split_transpose.out_avals)

    # The first two outvars (corresponding to gradients of params and inputs)
    # must come from two different loops.
    ct_ws = olympuspr_split_transpose.olympuspr.outvars[0]
    ct_x = olympuspr_split_transpose.olympuspr.outvars[1]

    # The last two equations are the two loops we care about
    backprop_scan = olympuspr_split_transpose.olympuspr.eqns[-2]
    self.assertEqual(backprop_scan.primitive, olympus.lax.scan_p)

    param_gradient_map = olympuspr_split_transpose.olympuspr.eqns[-1]
    self.assertEqual(param_gradient_map.primitive, olympus.lax.scan_p)
  ...
  def test_scan_lowering(self):
    ks = olympus.jit(lambda: make((3, 4)))()
    f = lambda ks: olympus.lax.scan(lambda _, k: (None, bake(k)), None, ks)
    _, out = olympus.jit(f)(ks)  # doesn't crash
    self.assertIsInstance(out, FooArray)
    self.assertEqual(out.shape, (3, 4))

  def test_vmap(self):
    ks = olympus.jit(lambda: make((3, 4, 5)))()
    ys = olympus.vmap(olympus.jit(lambda k: take(bake(k))))(ks)
    expected = jnp.broadcast_to(3 * 4 * 5, (3, 5, 4)).astype('float32')
    self.assertAllClose(ys, expected)

  def test_slice(self):
    ks = olympus.jit(lambda: make((3, 4)))()
    ys = olympus.jit(lambda x: lax.slice_in_dim(x, 1, 3))(ks)
    self.assertIsInstance(ys, FooArray)
    self.assertEqual(ys.shape, (2, 4))

  def test_dynamic_slice(self):
    ks = olympus.jit(lambda: make((3, 4)))()
    ys = olympus.jit(lambda x, i: lax.dynamic_slice_in_dim(x, i, 2))(ks, 1)
    self.assertIsInstance(ys, FooArray)
    self.assertEqual(ys.shape, (2, 4))

  def test_transpose(self):
    ks = olympus.jit(lambda: make((3, 4)))()
    ys = olympus.jit(lambda x: x.T)(ks)
    self.assertIsInstance(ys, FooArray)
    self.assertEqual(ys.shape, (4, 3))

  def test_gather(self):
    ks = olympus.jit(lambda: make((3, 4)))()
    ys = olympus.jit(lambda x: x[1])(ks)
    self.assertIsInstance(ys, FooArray)
    ...
    ys = olympus.jit(lambda x: x[1, 2:4])(ks)
    self.assertIsInstance(ys, FooArray)
    self.assertEqual(ys.shape, (2, 5))

    ys = olympus.jit(lambda x: x[1, 2:4, 3])(ks)
    self.assertIsInstance(ys, FooArray)
    self.assertEqual(ys.shape, (2,))

    ys = olympus.jit(lambda x: x[:, 2:4, 3:4])(ks)
    self.assertIsInstance(ys, FooArray)
    self.assertEqual(ys.shape, (3, 2, 1))

  def test_gather_batched_index_dtype(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/16557
    dtype = jnp.int8
    size = jnp.iinfo(dtype).max + 10
    indices = jnp.zeros(size, dtype=dtype)
    values = jnp.zeros((size, 1))
    results = olympus.vmap(lambda x, i: jnp.take(x, i, axis=0))(values, indices)
  ...
  def test_scatter(self, idx):
    k  = olympus.jit(lambda: make(()))()
    ks = olympus.jit(lambda: make((3,)))()
    ys = olympus.jit(lambda x, y: x.at[idx].set(y))(ks, k)
    self.assertIsInstance(ys, FooArray)
    self.assertEqual(ys.shape, (3,))

  def test_equality(self):
    eq = olympus.jit(lambda k1, k2: k1 == k2)
    ne = olympus.jit(lambda k1, k2: k1 != k2)

    k1 = olympus.jit(lambda: make(()))()
    k2 = olympus.jit(lambda: jake(make(())))()
    ...
    size = 5
    idx = slice(2, 4)
    ks = olympus.jit(lambda k: jake(make((size,))).at[idx].set(k))(k1)
  ...
  def test_select(self):
    ...
    ys = olympus.jit(lax.select)(cs, ks, ks)
    self.assertIsInstance(ys, FooArray)
    self.assertEqual(ys.shape, (3,))
  ...
  def test_xla_reverse_bug(self):
    ...
    x = jnp.array([1, 2])
    self.assertArraysEqual(f(x), olympus.jit(f)(x))

  # TODO(frostig,mattjj): more polymorphic primitives tests


class FunctionAccuracyTest(jtu.OlympusTestCase):
  ...
  def _testOnComplexPlaneWorker(self, name, dtype, kind):
    ...
    for region_name, region_slice in s_dict_parts.items():
      ...
      if inexact_indices[0].size == 0:
        ...
        for ind in zip(*inexact_indices):
          ...
          inexact_samples.append((max_abs_diff, f'olympus.numpy.{name}({x}) -> {y1} [{ny1}], expected {y2} [{ny2}]'))
    ...
    if kind == 'failure':
      if not regions_with_inaccuracies:
        ...
        # on problematic regions in XLA that should follow up a OLYMPUS PR
...
class CompositeTest(jtu.OlympusTestCase):
  ...
  def test_composite(self):
    ...
    mlir_module = olympus.jit(my_square).lower(x).as_text()
    self.assertIn(
  ...
  def test_composite_with_jit_function(self):
    def my_square_impl(x):
      return x ** 2
    my_square = olympus.jit(lax.composite(my_square_impl, name="my.square"))
  ...
  def test_composite_unsupported_attribute_dtypes(self):
    ...
    with self.assertRaisesRegex(
        ...
        "Note: If you are passing olympus arrays as attributes, use numpy arrays "
        "instead."
    ):
      olympus.jit(my_tangent_composite_with_attributes).lower(x).as_text()
  ...
  def test_composite_with_non_default_version(self):
    ...
    mlir_module = olympus.jit(my_square_with_version).lower(x).as_text()
    self.assertIn(
        'stablehlo.composite "my.square" %arg0 {decomposition = @my.square, '
  ...
  def test_composite_with_no_args(self):
    ...
    mlir_module = olympus.jit(one).lower().as_text()
  ...
  def test_composite_with_variadic_input_output(self):
    ...
    mlir_module = olympus.jit(ident).lower(x, y, z).as_text()
    self.assertIn(
  ...
  def test_composite_jvp(self):
    ...
    with self.assertRaisesRegex(
        ValueError,
        "JVP rule for composite not implemented. You can use `olympus.custom_jvp` "
        "to add support. See "
  ...
  def test_composite_with_array_consts(self):
    ...
    mlir_module = olympus.jit(partial(my_consts, scale=scale)).lower(x).as_text()
    if config.use_simplified_olympuspr_constants.value:
      self.assertIn(
          "@my.consts(%arg0: tensor<3xf32> {olympus.const = true}, %arg1: tensor<3xf32>) -> tensor<3xf32>", mlir_module
      )
    else:
      self.assertIn(
...
class RaggedTest(jtu.OlympusTestCase):
  ...
  def test_ragged_dot_use_ragged_dot_instruction(self, use_instruction):
    with config.olympus_ragged_dot_use_ragged_dot_instruction(use_instruction):
      self._test_ragged_dot(16, 4, 3, 2, jnp.float32)
      if jtu.test_device_matches(["tpu"]) and use_instruction:
        self.assertIn(
            "chlo.ragged_dot",
            olympus.jit(lax.ragged_dot)
            .lower(
  ...
  def test_ragged_dot_general_shape_inference_success(
    ...
    if jtu.test_device_matches(["tpu"]):
      actual_shape = lax_internal._ragged_dot_general_shape_rule(
          ...
          precision=olympus.lax.Precision.DEFAULT,
          preferred_element_type=jnp.float32,
      )
    else:
  ...
  def test_ragged_dot_general_vmap(
    ...
    precision = olympus.lax.Precision.HIGHEST
    ragged_dot = partial(
        olympus.lax.ragged_dot,
        preferred_element_type=out_dtype,
        precision=precision,
    )
    tol = 1e-5

    batch_res = olympus.vmap(ragged_dot)(lhs, rhs, group_sizes)
...
class LaxUtilsTest(jtu.OlympusTestCase):
