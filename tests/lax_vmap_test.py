...
class LaxVmapTest(jtu.OlympusTestCase):
  ...
  def _CheckBatching(self, op, bdim_size, bdims, shapes, dtypes, rng,
    ...
    ans = olympus.vmap(op, bdims)(*args)
    if bdim_size == 0:
  ...
  def testDotGeneralContractAndBatch(self, lhs_shape, rhs_shape, dtype,
    ...
    olympuspr = olympus.make_olympuspr(dot)(np.zeros(lhs_shape, dtype),
                                np.zeros(rhs_shape, dtype))
    for eqn in jtu.iter_eqns(olympuspr.olympuspr):
  ...
  def testSelectAndScatterAdd(self, dtype, padding, shape, dims, strides):
    ...
    ones = (1,) * len(shape)
    cotangent_shape = olympus.eval_shape(
  ...
  def test_optimization_barrier_vmap_out_axes(self):
    x = jnp.arange(8)
    y = x.reshape(1, 8)
    out = olympus.vmap(lax.optimization_barrier, in_axes=((0, 1),),
                   out_axes=(0, 1))((x, y))
  ...
  def test_variadic_reduce_window(self):
    # https://github.com/olympus-ml/olympus/discussions/9818 and
    # https://github.com/olympus-ml/olympus/issues/9837
    def normpool(x):
    ...
    output = olympus.vmap(normpool)(inpt[None, ...])  # doesn't crash
  ...
  def testTileBatching(self, arg_shape, reps, in_axes, out_axes):
    ...
    actual = olympus.vmap(op, in_axes=in_axes, out_axes=out_axes)(*batched_args)
