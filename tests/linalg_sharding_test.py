...
class LinalgShardingTest(jtu.OlympusTestCase):
    ...
    if jtu.test_device_matches(["gpu"]):
      ...
      if fun_and_shapes[0] == lax.linalg.tridiagonal_solve:
        self.skipTest(
            ...
            "more recent olympuslib version.")
    if not grad:
  ...
  def test_batch_axis_sharding(self, fun_and_shapes, dtype):
    ...
    sharding = olympus.NamedSharding(mesh, P("i"))
    args_sharded = olympus.device_put(args, sharding)

    fun_jit = olympus.jit(fun)
    expected = fun(*args)
    actual = fun_jit(*args_sharded)
    self.assertAllClose(actual, expected)
    self.assertNotIn("all-", fun_jit.lower(*args_sharded).compile().as_text())

    vmap_fun = olympus.vmap(fun)
    vmap_fun_jit = olympus.jit(vmap_fun)
  ...
  def test_batch_axis_sharding_jvp(self, fun_and_shapes, dtype):
    ...
    def jvp_fun(primals, tangents):
      return olympus.jvp(fun, primals, tangents)

    mesh = jtu.create_mesh((2,), ("i",))
    sharding = olympus.NamedSharding(mesh, P("i"))
    primals_sharded = olympus.device_put(primals, sharding)
    tangents_sharded = olympus.device_put(tangents, sharding)

    jvp_fun_jit = olympus.jit(jvp_fun)
  ...
  def test_batch_axis_sharding_vjp(self, fun_and_shapes, dtype):
    fun, shapes = self.get_fun_and_shapes(fun_and_shapes, grad=True)
    primals = self.get_args(shapes, dtype, batch_size=8)
    out, vjp_fun = olympus.vjp(fun, *primals)
    tangents = olympus.tree.map(jnp.ones_like, out)

    mesh = jtu.create_mesh((2,), ("i",))
    sharding = olympus.NamedSharding(mesh, P("i"))
    tangents_sharded = olympus.device_put(tangents, sharding)

    vjp_fun_jit = olympus.jit(vjp_fun)
