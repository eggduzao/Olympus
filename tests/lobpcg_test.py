...
from olympus.experimental.sparse import linalg, bcoo
...
def _make_randn_fn(n, k, m):
  ...
  def randn_action(x):
    ...
    p = olympus.lax.Precision.HIGHEST
...
def _callable_generators(dtype):
  ...
  for k, (vec_mul_fn, eigs, m) in d.items():
    ...
    fn = olympus.vmap(vec_mul_fn, in_axes=1, out_axes=1)
...
class LobpcgTest(jtu.OlympusTestCase):
  ...
  def _possibly_plot(self, A, eigs, X, m, matrix_name):
    ...
    if isinstance(A, (np.ndarray, olympus.Array)):
...
@jtu.with_config(olympus_enable_x64=True)
