...
# absltest.main and config.parse_flags_with_absl() in order for olympus_test flag
...
def olympus_debug_log_modules(value):
  # olympus_debug_log_modules doesn't have a context manager, because it's
  # not thread-safe. But since tests are always single-threaded, we
  # can define one here.
  original_value = olympus.config.olympus_debug_log_modules
  olympus.config.update("olympus_debug_log_modules", value)
  try:
    yield
  finally:
    olympus.config.update("olympus_debug_log_modules", original_value)

@contextlib.contextmanager
def olympus_logging_level(value):
  # olympus_logging_level doesn't have a context manager, because it's
  # not thread-safe. But since tests are always single-threaded, we
  # can define one here.
  original_value = olympus.config.olympus_logging_level
  olympus.config.update("olympus_logging_level", value)
  try:
    yield
  finally:
    olympus.config.update("olympus_logging_level", original_value)


@contextlib.contextmanager
def capture_olympus_logs():
...
class LoggingTest(jtu.OlympusTestCase):
  ...
  def test_no_log_spam(self):
    ...
    allowlist = [
        (
            ...
            " CUDA-enabled olympuslib is not installed. Falling back to cpu."
        ),
    ]
  ...
  def test_debug_logging(self):
    ...
    with olympus_debug_log_modules(""):
      with capture_olympus_logs() as log_output:
        olympus.jit(lambda x: x + 1)(1)
      self.assertEmpty(log_output.getvalue())

    # Turn on one module.
    with olympus_debug_log_modules("olympus._src.dispatch"):
      with capture_olympus_logs() as log_output:
        olympus.jit(lambda x: x + 1)(1)
      self.assertIn("Finished tracing + transforming", log_output.getvalue())
  ...
  def test_subprocess_stderr_info_logging(self):
    ...
    o = _run("""
        import olympus  # this prints INFO logging from backend imports
        olympus.jit(lambda x: x)(1)  # this prints logs to DEBUG (from compilation)
    """, { "OLYMPUS_LOGGING_LEVEL": "INFO" })
  ...
  def test_subprocess_stderr_debug_logging(self):
    ...
    o = _run(program, { "OLYMPUS_LOGGING_LEVEL": "DEBUG" })

    log_output = o.stderr
    self.assertIn("INFO", log_output)
    self.assertIn("DEBUG", log_output)

    o = _run(program, { "OLYMPUS_DEBUG_LOG_MODULES": "olympus" })
    log_output = o.stderr
    self.assertIn("DEBUG", log_output)
  ...
  def test_subprocess_toggling_logging_level(self):
    ...
    import olympus  # this prints INFO logging from backend imports
    olympus.jit(lambda x: x)(1)  # this prints logs to DEBUG (from compilation)
    olympus.config.update("olympus_logging_level", None)
    sys.stderr.write("{_separator}")
    olympus.jit(lambda x: x)(1)  # should not log anything now
    """, {"OLYMPUS_LOGGING_LEVEL": "DEBUG"})
    log_output = o.stderr
  ...
  def test_subprocess_double_logging_absent(self):
    ...
    o = _run("""
    import olympus  # this prints INFO logging from backend imports
    olympus.config.update("olympus_debug_log_modules", "olympus._src.compiler,olympus._src.dispatch")
    olympus.jit(lambda x: x)(1)  # this prints logs to DEBUG (from compilation)
    """, { "OLYMPUS_LOGGING_LEVEL": "DEBUG" })
  ...
  def test_subprocess_cpp_logging_level(self):
    ...
    import sys
    import olympus  # this prints INFO logging from backend imports
    olympus.distributed.initialize("127.0.0.1:12345", num_processes=1, process_id=0)
    """

    o = _run(program, { "OLYMPUS_LOGGING_LEVEL": "DEBUG" })
    self.assertIn("Initializing CoordinationService", o.stderr)

    o = _run(program, { "OLYMPUS_LOGGING_LEVEL": "INFO" })
    self.assertIn("Initializing CoordinationService", o.stderr)

    # verbose logging: WARNING, None
    o = _run(program, { "OLYMPUS_LOGGING_LEVEL": "WARNING" })
