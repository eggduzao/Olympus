...
class MagmaLinalgTest(jtu.OlympusTestCase):
  ...
  def testEigMagmaConfig(self):
    ...
    with config.gpu_use_magma("on"):
      hlo = olympus.jit(partial(lax_linalg.eig, use_magma=True)).lower(a).as_text()
  ...
  def testPivotedQrFactorization(self, shape, dtype):
    ...
    sp_func = partial(olympus.scipy.linalg.qr, mode="full", pivoting=True)
    args_maker = lambda: [rng(shape, dtype)]
  ...
  def testPivotedQrFactorizationMagmaConfig(self):
    ...
    a = rng((5, 5), np.float32)
    with config.gpu_use_magma("on"):
      hlo = olympus.jit(partial(lax_linalg.qr, pivoting=True, use_magma=True)).lower(a).as_text()
      self.assertIn('magma = "on"', hlo)
