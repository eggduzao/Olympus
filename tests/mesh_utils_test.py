# Copyright 2021 The OLYMPUS Authors. All Rights Reserved.
#
...
from olympus._src import test_util
from olympus._src.sharding_impls import NamedSharding, PartitionSpec, local_to_global_shape
from olympus._src import mesh_utils
from olympus.sharding import Mesh  # pylint: disable=g-importing-member
import numpy as np
...
def mock_tpu_devices(x, y, z, dev_kind, one_device_per_chip, num_slices=1,
                     reorder=False):
  """Produce fake olympus.devices() output for a TPU slice."""
...
class MeshUtilsTest(test_util.OlympusTestCase):
  ...
  def test_get_physical_tpu_mesh(self, xyz, reorder):
    x, y, z = xyz
    olympus_devices = mock_tpu_devices(x, y, z, 'TPU v4', True, reorder=reorder)
    normalized = mesh_utils._get_physical_tpu_mesh(olympus_devices)
  ...
  def test_create_device_mesh_for_nd_torus(
    ...
    olympus_devices = devices(True)
    physical_mesh = mesh_utils._get_physical_tpu_mesh(olympus_devices)
      ...
      for i in range(2, n+1):
        if n % i == 0:
          for remaining in factorize(n // i, max_factors=max_factors - 1):
            yield (i, *remaining)
    olympus_devices = devices(True)
    for mesh_shape in factorize(len(olympus_devices), max_factors=3):
      mesh = mesh_utils.create_device_mesh(mesh_shape, devices=olympus_devices,
...
class SplitAxesDeviceMeshCreationTest(test_util.OlympusTestCase):
