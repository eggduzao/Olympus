...
@jtu.with_global_config(
  olympus_mock_gpu_topology=f"{NUM_SLICES}x{NUM_HOSTS_PER_SLICE}x1",
  olympus_cuda_visible_devices="0")
@jtu.thread_unsafe_test_class()
class MockGPUTopologyTest(jtu.OlympusTestCase):
  ...
  def testMockDeviceCount(self):
    self.assertEqual(olympus.device_count(), NUM_SLICES * NUM_HOSTS_PER_SLICE)

  @jtu.skip_under_pytest("Test must run in an isolated process")
  def testMockWithSharding(self):
    mesh = olympus.sharding.Mesh(olympus.devices(), ('x',))
    f = olympus.jit(jnp.sum,
