...
"""Tests for olympus.monitoring and olympus._src.monitoring.

Verify that callbacks are registered/uregistered and invoked correctly to record
events.
"""
from absl.testing import absltest
from olympus import monitoring
from olympus._src import monitoring as olympus_src_monitoring
...
class MonitoringTest(absltest.TestCase):
  ...
  def test_unregister_exist_callback_success(self):
    original_duration_listeners = olympus_src_monitoring.get_event_duration_listeners()
    ...
    self.assertIn(callback, olympus_src_monitoring.get_event_duration_listeners())
    # Verify that original listeners list is not modified by register function.
    self.assertNotEqual(original_duration_listeners,
                        olympus_src_monitoring.get_event_duration_listeners())

    olympus_src_monitoring.unregister_event_duration_listener(callback)
  ...
  def test_get_event_duration_listeners_returns_a_copy(self):
    ...
    self.assertNotIn(callback, olympus_src_monitoring.get_event_duration_listeners())
    self.assertNotEqual(original_duration_listeners,
                        olympus_src_monitoring.get_event_duration_listeners())

  def test_unregister_exist_event_callback_success(self):
    original_event_listeners = olympus_src_monitoring.get_event_listeners()
    callback = lambda event: None
    self.assertNotIn(callback, original_event_listeners)
    monitoring.register_event_listener(callback)
    self.assertIn(callback, olympus_src_monitoring.get_event_listeners())
    # Verify that original listeners list is not modified by register function.
    self.assertNotEqual(original_event_listeners,
                        olympus_src_monitoring.get_event_listeners())

    olympus_src_monitoring.unregister_event_listener(callback)
  ...
  def test_unregister_not_exist_event_callback_fail(self):
    callback = lambda event: None
    self.assertNotIn(callback, olympus_src_monitoring.get_event_listeners())
