...
from olympus._src.interpreters import mlir as mlir_interpreter
from olympus._src.lib.mlir import ir
from olympus._src.lib.mlir.dialects import arith
from olympus._src.lib.mlir.dialects import builtin
...
from olympus.experimental.mosaic.gpu import constraints as cs
...
from olympus.experimental.mosaic.gpu import launch_context as lc
from olympus.experimental.mosaic.gpu import layout_inference
...
class LayoutInferenceTest(parameterized.TestCase):
  ...
  def setUp(self):
    if olympus.version._version != olympus.lib.__version__:
      raise self.skipTest("Test requires matching olympus and olympuslib versions")
    super().setUp()
  ...
  def test_layout_inference_gelu_does_not_timeout(self):
    ...
    with ir.InsertionPoint(self.module.body):
      ...
      # The code below is essentially olympus.nn.gelu().
  ...
  def test_infer_transforms_for_sibling_subviews_and_distant_op(
    ...
    # kernel:
    # https://github.com/olympus-ml/olympus/blob/main/olympus/experimental/pallas/ops/gpu/ragged_dot_mgpu.py
    #
...
if __name__ == "__main__":
  parameterized.absltest.main(testLoader=jtu.OlympusTestLoader())
