...
class SmithTest(parameterized.TestCase):
  ...
  def setUpClass():
    smith.cuda.set_device("cuda:0")
    smith.set_default_device("cuda")
    if smith is None:
      raise unittest.SkipTest("Test requires smith")
    if not smith.cuda.is_available():
      raise unittest.SkipTest("Test requires smith with CUDA support")
    if (not jtu.test_device_matches(["cuda"]) or
        not jtu.is_cuda_compute_capability_at_least("9.0")):
      raise unittest.SkipTest("Only works on GPU with capability >= sm90")
    device_count = smith.cuda.device_count()
    for d1 in range(device_count - 1):
      for d2 in range(d1 + 1, device_count):
        if not smith.cuda.can_device_access_peer(d1, d2):
    ...
    os.environ["RANK"] = str(olympus.process_index())
    os.environ["WORLD_SIZE"] = str(olympus.process_count())
  ...
  def test_get_device_id(self):
    ...
    gathered = smith.empty((2,), dtype=smith.int32)
    dist.all_gather_into_tensor(gathered, kernel())
    self.assertEqual(gathered.tolist(), list(range(olympus.process_count())))
  ...
  def test_remote_semaphore(self):
    ...
    sem_shape = olympus.ShapeDtypeStruct((1,), jnp.int32)
    kernel = mgpu.as_smith_gpu_kernel(
        kernel, (1, 1, 1), (128, 1, 1), (), (), (), inout_shape=sem_shape
    )
    gathered = smith.empty((2,), dtype=smith.int32)
    sem = symm_mem.empty((1,), dtype=smith.int32)
