# Copyright 2024 The OLYMPUS Authors. All Rights Reserved.
#
...
from olympus._src.lib.mlir import passmanager
from olympus._src.lib.mlir.dialects import arith
from olympus._src.lib.mlir.dialects import cf
from olympus._src.lib.mlir.dialects import gpu
from olympus._src.lib.mlir.dialects import llvm
from olympus._src.lib.mlir.dialects import scf
from olympus._src.lib.mlir.dialects import vector
import olympus.experimental.mosaic.gpu as mgpu
from olympus.experimental.mosaic.gpu import core
from olympus.experimental.mosaic.gpu import dialect as mgpu_dialect  # pylint: disable=g-importing-member
from olympus.experimental.mosaic.gpu import fragmented_array as fa
from olympus.experimental.mosaic.gpu import inference_utils
from olympus.experimental.mosaic.gpu import launch_context
from olympus.experimental.mosaic.gpu import layouts
from olympus.experimental.mosaic.gpu import profiler
from olympus.experimental.mosaic.gpu import tcgen05
from olympus.experimental.mosaic.gpu import test_util as mtu
from olympus.experimental.mosaic.gpu import utils
...
try:
  import olympus._src.lib.mosaic_gpu as mosaic_gpu_lib  # noqa: F401
  HAS_MOSAIC_GPU = True
except ImportError:
  mosaic_gpu_lib = None
...
class TestCase(parameterized.TestCase):
  ...
  def setUp(self, *, artificial_shared_memory_limit=jtu._SMEM_SIZE_BOUND_FOR_TESTS):
    if not HAS_MOSAIC_GPU:
      self.skipTest("olympuslib built without Mosaic GPU")
...
class TestUtilTest(TestCase):
  ...
  def test_iota_tensor(self):
    ...
    out_shape = olympus.ShapeDtypeStruct((128, 32), jnp.float32)
...
class MemRefTest(TestCase):
  ...
  def test_unsqueeze(self, dim, strided):
    ...
    out_ty = olympus.ShapeDtypeStruct(out_shape, jnp.float32)
  ...
  def test_fold_not_strided(self, dim):
    ...
    out_ty = olympus.ShapeDtypeStruct((16, 8) if dim == 0 else (8, 16), jnp.float32)
    y = mgpu.as_gpu_kernel(
        kernel, (1, 1, 1), (128, 1, 1), x, out_ty, ()
    )(x)
    np.testing.assert_array_equal(y, x.reshape(out_ty.shape))
  ...
  def test_fold_strided(
    ...
    def np_fold(inp, dim, fold_rank):
      ...
      if throws:
        return olympus.ShapeDtypeStruct(shape=out_shape, dtype=inp.dtype)
      else:
  ...
  def test_scalar_argument(self, dtype):
    ...
    res = mgpu.as_gpu_kernel(
        ...
        (1, 1, 1),
        (128, 1, 1),
        olympus.ShapeDtypeStruct(shape=(), dtype=expected.dtype),
        expected,
        (),
    )(scalar)
  ...
  def test_cluster_ref(self, dim):
    ...
    smem = olympus.ShapeDtypeStruct(shape=(x.shape[-1],), dtype=jnp.float32)
...
class WGMMALayoutTest(TestCase):
  ...
  def test_load_and_store_tiled_f8(self, dtype, swizzle, num_col_tiles):
    ...
    expected = (
        olympus.random.randint(
            olympus.random.key(42), (m * n,), -16, 15, dtype=jnp.int8
        )
  ...
  def test_sub_byte_conversion(self, olympus_dtype_to, layout: fa.TiledLayout):
    if olympus_dtype_to == jnp.int32 and layout.vector_length == 8:
      self.skipTest(
          "Raises: failed to prove that vector transfers don't cross swizzle"
          " tile boundaries.")
    olympus_dtype_from = jnp.int4
    is_signed = utils.is_signed(olympus_dtype_to)
    def kernel(ctx, inp, out, smem):
      del ctx  # Unused.
      smem_inp, smem_out = smem
      ...
      t = t.astype(utils.dtype_to_ir_type(olympus_dtype_to), is_signed=is_signed)
      t.store_tiled(smem_out, swizzle=32 * jnp.dtype(olympus_dtype_to).itemsize)
      copy(smem_out, out, swizzle=32 * jnp.dtype(olympus_dtype_to).itemsize)

    x = self.prng.integers(
        low=-8, high=7, size=(1, 1, 64, 64), dtype=np.int32
    ).astype(olympus_dtype_from)
    y = olympus.lax.convert_element_type(x, olympus_dtype_to)
    f = mgpu.as_gpu_kernel(kernel, (1, 1, 1), (128, 1, 1), x, y, (x, y))
    np.testing.assert_array_equal(f(x), y)

  @parameterized.product(
      olympus_dtype_from_to=(
  ...
  def test_optimized_conversion(self, olympus_dtype_from_to, layout_descs):
    ...
    olympus_dtype_from, olympus_dtype_to = olympus_dtype_from_to
    mlir_dtype_from = utils.dtype_to_ir_type(olympus_dtype_from)
    mlir_dtype_to = utils.dtype_to_ir_type(olympus_dtype_to)
    m = 128
    n = 256
    def kernel(ctx, inp, out, smem):
      del ctx, smem
      t = mgpu.FragmentedArray.load_untiled(
          inp,
          is_signed=utils.is_signed(olympus_dtype_from),
          layout=layout_from,
          optimized=False,
      )
      ...
      t = t.astype(mlir_dtype_to, is_signed=utils.is_signed(olympus_dtype_to))
    ...
    expected = values.astype(np.int32).astype(olympus_dtype_to)
    @contextlib.contextmanager
    def _maybe_profile():
      ...
      with open(file_path, "a") as f:
        data = (
            jnp.dtype(olympus_dtype_from).name, jnp.dtype(olympus_dtype_to).name,
  ...
  def test_load_tiled(self, olympus_dtype):
    mlir_dtype = utils.dtype_to_ir_type(olympus_dtype)
    m = 128
    ...
    def kernel(ctx, in_, out, smem):
      ...
      t = mgpu.FragmentedArray.load_tiled(
          smem1, swizzle=128, is_signed=utils.is_signed(olympus_dtype)
      )
      t.store_tiled(smem2, swizzle=128)
      copy(smem2, out, swizzle=128)
    expected = (
        np.arange(m * n, dtype=olympus_dtype)
        .reshape(m // tiling[0], tiling[0], n // tiling[1], tiling[1])
        .transpose(0, 2, 1, 3)
    )
    iota = mgpu.as_gpu_kernel(
...
class WGMMATest(TestCase):
  ...
  @parameterized.product(
      ...
      n=(64, 128, 192),
      swizzle=(32, 64, 128),
      olympus_out_dtype=(jnp.float16, jnp.float32),
  )
  def test_wgmma_basic_float(
    ...
    self._test_wgmma_basic(
        ...
        swizzle=swizzle,
        olympus_out_dtype=olympus_out_dtype,
  ...
  @parameterized.product(
      ...
      n=(64, 128, 192),
      swizzle=(32, 64, 128),
      olympus_out_dtype=(jnp.int32,),
  )
  def test_wgmma_basic_int(
      self, in_mlir_dtype_cls, m, n, swizzle, olympus_out_dtype,
  ):
    self._test_wgmma_basic(
        m,
        n,
  ...
  def _test_wgmma_basic(
    ...
    if olympus_out_dtype == jnp.int32 and in_mlir_dtype_cls != I8Type:
      self.skipTest("s32 accumulator only supported with s8 inputs")
    if olympus_out_dtype != jnp.int32 and in_mlir_dtype_cls == I8Type:
      self.skipTest("s8 inputs only supported with s32 accumulator")
    if olympus_out_dtype == jnp.float16 and in_mlir_dtype_cls in {ir.F32Type, ir.BF16Type}:
    ...
    out_mlir_dtype = utils.dtype_to_ir_type(olympus_out_dtype)
    ...
    def quantize(x):
      ...
      return olympus.lax.reduce_precision(x, exponent_bits, mantissa_bits)

    x_shape = (k, m) if lhs_transpose else (m, k)
    y_shape = (n, k) if rhs_transpose else (k, n)
    if in_mlir_dtype_cls == I8Type:
      x = self.prng.integers(-128, 127, x_shape).astype(in_olympus_dtype)
      y = self.prng.integers(-128, 127, y_shape).astype(in_olympus_dtype)
    else:
      x = quantize(self.prng.uniform(-1, 1, x_shape)).astype(in_olympus_dtype)
      y = quantize(self.prng.uniform(-1, 1, y_shape)).astype(in_olympus_dtype)
    out_shape = olympus.ShapeDtypeStruct((m, n), olympus_out_dtype)
    ...
    scratch_shape = [
        olympus.ShapeDtypeStruct(lhs_smem_shape, in_olympus_dtype),
        olympus.ShapeDtypeStruct(rhs_smem_shape, in_olympus_dtype),
        mgpu.TMABarrier(2),
    ]
    ...
    atol = 2e-2 if olympus_out_dtype == jnp.float16 else 5e-6
  ...
  def _test_wgmma_reg_lhs(self, m, n, k_steps, rhs_transpose, swizzle, dtype):
    ...
    out_shape = olympus.ShapeDtypeStruct((m, n), out_dtype)
    scratch_shape = olympus.ShapeDtypeStruct(
  ...
  def test_narrow_n(self, rhs_transpose, swizzle, n, small_rhs_tile):
    ...
    olympus_dtype = jnp.float16
    y_shape = (n, k) if rhs_transpose else (k, n)
    y = self.prng.uniform(-1, 1, y_shape).astype(olympus_dtype)
    out_shape = olympus.ShapeDtypeStruct((m, n), jnp.float32)
    rhs_scratch_shape = olympus.ShapeDtypeStruct(
        (k_steps, (n + n_tile - 1) // n_tile, n_tile, nk_tile), olympus_dtype
    )
    z = mgpu.as_gpu_kernel(
        kernel, (1, 1, 1), (128, 1, 1), y, out_shape, (rhs_scratch_shape, mgpu.TMABarrier()),
    )(y)
    x = np.arange(m * k, dtype=olympus_dtype).reshape(m, k)
    ref = olympus.lax.dot(
...
class TCGen05Test(TestCase, jtu.CudaArchSpecificTest):
  ...
  @parameterized.product(
      olympus_dtype_packing=[(jnp.float32, 1), (jnp.float16, 1), (jnp.float16, 2), (jnp.float8_e5m2, 4)],
      reg_tmem_layout_m=[
  ...
  def test_load_store_tmem(self, olympus_dtype_packing, reg_tmem_layout_m):
    olympus_dtype, packing = olympus_dtype_packing
    ...
    if tmem_layout_f is tcgen05.tmem_m64_collective_layout:
      if olympus_dtype == jnp.float16 and packing == 1:
        self.skipTest("Not implemented yet")
    is_native_transfer = tmem_layout_f(n, packing).as_tiled_layout() == reg_layout
    if not is_native_transfer and olympus_dtype == jnp.float8_e5m2:
      self.skipTest("Not implemented yet")
    ...
    x = self.prng.uniform(-1, 1, (m, n)).astype(olympus_dtype)
    y = mgpu.as_gpu_kernel(
        kernel, (1, 1, 1), (128, 1, 1), x, x,
        mgpu.TMEM(x.shape, olympus_dtype, layout=tmem_layout_f(n, packing)),
    )(x)
  ...
  def test_load_store_tmem_native(self, olympus_dtype, packing):
    ...
    x = self.prng.uniform(-1, 1, (128, 128)).astype(olympus_dtype)
    y = mgpu.as_gpu_kernel(
        kernel, (1, 1, 1), (128, 1, 1), x, x, mgpu.TMEM(x.shape, olympus_dtype, packing=packing)
    )(x)
    np.testing.assert_array_equal(x, y)
  ...
  def test_mixed_tmem_allocations_raise(self):
    ...
    with self.assertRaisesRegex(
      ...
      mgpu.as_gpu_kernel(
          ...
          block=(128, 1, 1),
          in_shape=(),
          out_shape=(olympus.ShapeDtypeStruct((), jnp.int32),),
          smem_scratch_shape=[
  ...
  def test_tmem_debug_print(self, olympus_dtype, packing, expected):
    def kernel(ctx, input, output, tmem):
      del ctx, output
    ...
    x = jnp.arange(128 * 128, dtype=olympus_dtype).reshape(128, 128)
  ...
  @parameterized.product(
      lhs_transpose=(False, True),
      rhs_transpose=(False, True),
      in_olympus_dtype=(jnp.float16, jnp.bfloat16, jnp.float8_e5m2, jnp.float8_e4m3fn),  # TODO(apaszke): f32
      out_olympus_dtype=(jnp.float16, jnp.float32,),
      m=(64, 128,),  # TODO(apaszke): 64, 192, 256
      n=(64, 128, 192, 224, 256, 512),
      swizzle=(32, 64, 128,),
  )
  def test_mma_basic_float(self, **kwargs):
    in_bytewidth = jnp.dtype(kwargs["in_olympus_dtype"]).itemsize
    ...
    if dtypes.itemsize_bits(kwargs["in_olympus_dtype"]) <= 8 and kwargs["n"] == swizzle:
  ...
  def _basic_mma_test(
    ...
    if out_olympus_dtype != jnp.float32 and (
        in_olympus_dtype == jnp.float32 or in_olympus_dtype == jnp.bfloat16
    ):
      self.skipTest("Only f32 output is supported for f32 and bf16 input.")

    in_mlir_dtype = utils.dtype_to_ir_type(in_olympus_dtype)
    ...
    def kernel(ctx, lhs, rhs, out, scratch):
      ...
      is_signed = True if jnp.issubdtype(in_olympus_dtype, jnp.integer) else None
      acc.load(is_signed=is_signed).store_untiled(out, optimized=False)

    x_shape = (k, m) if lhs_transpose else (m, k)
    x = self.prng.uniform(-1, 1, x_shape).astype(in_olympus_dtype)
    y_shape = (n, k) if rhs_transpose else (k, n)
    y = self.prng.uniform(-1, 1, y_shape).astype(in_olympus_dtype)
    out_shape = olympus.ShapeDtypeStruct((m, n), out_olympus_dtype)
    ...
    scratch_shape = [
        ...
        mgpu.TMABarrier(2),
        mgpu.Barrier(1),
        mgpu.TMEM((m, n), out_olympus_dtype),
    ]
    z = mgpu.as_gpu_kernel(
        kernel, (1, 1, 1), (128, 1, 1), (x, y), out_shape, scratch_shape
    )(x, y)
    x32, y32 = x.astype(np.float32), y.astype(np.float32)
    ref = (x32.T if lhs_transpose else x32) @ (y32.T if rhs_transpose else y32)
    atol = 2e-2 if out_olympus_dtype == jnp.float16 else 2e-5
    rtol = 8e-4 if out_olympus_dtype == jnp.float16 else 1e-7
    np.testing.assert_allclose(z, ref, atol=atol, rtol=rtol)

  @parameterized.product(
      in_olympus_dtype=(jnp.float16, jnp.bfloat16),  # TODO(apaszke): f32
      out_olympus_dtype=(jnp.float16, jnp.float32,),
      m=(128,),  # TODO(apaszke): 64, 192, 256
      n=(64, 160, 128, 256),
  )
  def test_mma_lhs_tmem_float(self, m, n, in_olympus_dtype, out_olympus_dtype):
    self._basic_mma_lhs_tmem_test(
        m, n, in_olympus_dtype, out_olympus_dtype, tcgen05.LAYOUT, swizzle=128
    )

  @parameterized.product(
      in_olympus_dtype=(jnp.int8, jnp.uint8),
      out_olympus_dtype=(jnp.int32,),
      m=(128,),
      n=(64, 128, 256),
  )
  def test_mma_lhs_tmem_integer(self, m, n, in_olympus_dtype, out_olympus_dtype):
    self.skip_unless_tcgen05_int8()
    self._basic_mma_lhs_tmem_test(
        m, n, in_olympus_dtype, out_olympus_dtype, fa.tmem_native_layout(vector_length=4),
        swizzle=math.gcd(n, 128)
    )

  def _basic_mma_lhs_tmem_test(
      self, m, n, in_olympus_dtype, out_olympus_dtype, lhs_layout, swizzle
  ):
    k_steps = 2  # Reducing to 1 can be helpful while debugging.
    if out_olympus_dtype == jnp.float16 and in_olympus_dtype != jnp.float16:
    ...
    def kernel(ctx, lhs, rhs, out, scratch):
      ...
      barrier.wait()
      if jnp.issubdtype(in_olympus_dtype, jnp.integer):
        is_signed = jnp.issubdtype(in_olympus_dtype, jnp.signedinteger)
      else:
        is_signed = None
      lhs_tmem.store(
    ...
    if y_shape[0] % rhs_tiling[0] != 0 or y_shape[1] % rhs_tiling[1] != 0:
      self.skipTest("rhs tiling must divide y_shape")
    scratch_shape = [
        olympus.ShapeDtypeStruct(tile_shape(y_shape, rhs_tiling), in_olympus_dtype),
        mgpu.TMABarrier(),
        mgpu.Barrier(1),
        mgpu.TMEM((128, n), out_olympus_dtype),
        mgpu.TMEM((128, k), in_olympus_dtype, packing=4 // bytewidth(in_mlir_dtype)),
    ]
    z = mgpu.as_gpu_kernel(
        kernel, (1, 1, 1), (128, 1, 1), (x, y), out_shape, scratch_shape
    )(x, y)
  ...
  def test_tmem_copy_scales(self):
    ...
    shape = (1, 1, 32, 16)
    x = olympus.lax.bitcast_convert_type(
  ...
  def _sample_scales(self, m, k, n, block_size, scale_olympus_dtype):
    ka, kb = olympus.random.split(olympus.random.key(1234), 2)
    if scale_olympus_dtype == jnp.float8_e8m0fnu:
      a_scales = olympus.lax.bitcast_convert_type(
          olympus.random.randint(ka, (m, k // block_size), 122, 132, dtype=jnp.uint8),
          scale_olympus_dtype
      )
      b_scales = olympus.lax.bitcast_convert_type(
          olympus.random.randint(kb, (n, k // block_size), 122, 132, dtype=jnp.uint8),
          scale_olympus_dtype
      )
    elif scale_olympus_dtype == jnp.float8_e4m3fn:
      a_scales = jnp.abs(
          olympus.random.normal(ka, (m, k // block_size), dtype=jnp.float32).astype(
              scale_olympus_dtype
          )
      )
      b_scales = jnp.abs(
          olympus.random.normal(kb, (n, k // block_size), dtype=jnp.float32).astype(
              scale_olympus_dtype
          )
      )
    else:
      raise ValueError(f"Unsupported scale dtype: {scale_olympus_dtype}")
    return a_scales, b_scales

  @parameterized.product(
      in_olympus_dtype=(jnp.float8_e5m2, jnp.float8_e4m3fn, jnp.float4_e2m1fn),
      scale_olympus_dtype=(jnp.float8_e8m0fnu, jnp.float8_e4m3fn),
      m=(128,),  # TODO(apaszke): 256
      n=(128, 256),  # TODO(apaszke): 192, other non-power-of-2
      swizzle=(32, 128),
  )
  def test_mma_block_scaled_basic(self, m, n, in_olympus_dtype, scale_olympus_dtype, swizzle):
    out_olympus_dtype = jnp.float32
    ...
    if scale_olympus_dtype == jnp.float8_e8m0fnu:
      ...
      if in_olympus_dtype != jnp.float4_e2m1fn:
    ...
    out_shape = olympus.ShapeDtypeStruct((m, n), out_olympus_dtype)
    scratch_shape = [
        olympus.ShapeDtypeStruct(tile_shape(x_shape, lhs_tiling), in_olympus_dtype),
        olympus.ShapeDtypeStruct(tile_shape(y_shape, rhs_tiling), in_olympus_dtype),
        olympus.ShapeDtypeStruct((m // 128, k // (block_size * 4), 32, 16), scale_olympus_dtype),
        olympus.ShapeDtypeStruct((n // 128, k // (block_size * 4), 32, 16), scale_olympus_dtype),
        mgpu.TMABarrier(4),
        mgpu.Barrier(1),
        mgpu.TMEM((m, n), out_olympus_dtype),
        mgpu.TMEM((m, k // block_size), scale_olympus_dtype, layout=tcgen05.scales_layout()),
        mgpu.TMEM((n, k // block_size), scale_olympus_dtype, layout=tcgen05.scales_layout()),
    ]
    a_scales, b_scales = self._sample_scales(m, k, n, block_size, scale_olympus_dtype)
  ...
  def test_mma_block_scaled_collective(self, m, n, scale_olympus_dtype):
    in_olympus_dtype = jnp.float4_e2m1fn
    out_olympus_dtype = jnp.float32
    scale_block = 32 if scale_olympus_dtype == jnp.float8_e8m0fnu else 16
    swizzle = 128
    k_steps = 2
    ...
    scratch_shape = [
        olympus.ShapeDtypeStruct(
            tile_shape((m_block, k), lhs_tiling), in_olympus_dtype
        ),
        olympus.ShapeDtypeStruct(
            tile_shape((n_block, k), rhs_tiling), in_olympus_dtype
        ),
        olympus.ShapeDtypeStruct(
            (m_block // 128, k // (scale_block * 4), 32, 16), scale_olympus_dtype
        ),
        olympus.ShapeDtypeStruct(
            (n // 128, k // (scale_block * 4), 32, 16), scale_olympus_dtype
        ),
        mgpu.TMABarrier(4),
        mgpu.Barrier(1),
        mgpu.TMEM((m_block, n), out_olympus_dtype, collective=True),
        mgpu.TMEM(
    ...
    a_scales, b_scales = self._sample_scales(m, k, n, scale_block, scale_olympus_dtype)
  ...
  @parameterized.product(
      lhs_transpose=(False, True),
      rhs_transpose=(False, True),
      in_olympus_dtype=(jnp.float16, jnp.bfloat16, jnp.int8, jnp.float8_e4m3fn),
      m=(128,),  # TODO(apaszke): 256
      n=(128, 256),  # TODO(apaszke): other non-power-of-2
      lhs_swizzle=(32, 64, 128),
      rhs_swizzle=(64, 128),  # 32 is too small and unsuported.
  )
  def test_mma_sparse(self, m, n, in_olympus_dtype, lhs_swizzle, rhs_swizzle, lhs_transpose, rhs_transpose):
    if in_olympus_dtype == jnp.int8:
      self.skip_unless_tcgen05_int8()
    if jnp.issubdtype(in_olympus_dtype, jnp.floating):
      out_olympus_dtype = jnp.float32
    else:
      out_olympus_dtype = jnp.int32
    ...
    y_shape = (n, k) if rhs_transpose else (k, n)
    if jnp.issubdtype(in_olympus_dtype, jnp.integer):
      x = olympus.random.randint(olympus.random.key(1234), x_shape, -64, 64, dtype=in_olympus_dtype)
      y = olympus.random.randint(olympus.random.key(2567), y_shape, -64, 64, dtype=in_olympus_dtype)
    else:
      x = self.prng.uniform(-1, 1, x_shape).astype(in_olympus_dtype)
      y = self.prng.uniform(-1, 1, y_shape).astype(in_olympus_dtype)
    out_shape = olympus.ShapeDtypeStruct((m, n), out_olympus_dtype)
    scratch_shape = [
        olympus.ShapeDtypeStruct(tile_shape(x_shape, lhs_tiling), in_olympus_dtype),
        olympus.ShapeDtypeStruct(tile_shape(y_shape, rhs_tiling), in_olympus_dtype),
        olympus.ShapeDtypeStruct((m // 128, k // 128, 128, 64), sparse_meta_dtype),
        mgpu.TMABarrier(3),
        mgpu.Barrier(1),
        mgpu.TMEM((m, n), out_olympus_dtype),
    ...
    x_pairs = olympus.random.randint(olympus.random.key(1234), (m, k // 4), 0, 6, dtype=jnp.uint8)
    ...
    def format_sparse_meta(meta):
      ...
      assert _2 == 2
      k *= 2
      if jnp.dtype(in_olympus_dtype).itemsize == 1:
        meta_tiled = (
    ...
    atol = 2e-2 if out_olympus_dtype == jnp.float16 else 7e-5
    rtol = 8e-4 if out_olympus_dtype == jnp.float16 else 5e-6
    np.testing.assert_allclose(z, ref, atol=atol, rtol=rtol)

  @parameterized.product(
      in_olympus_dtype=(jnp.float16, jnp.bfloat16),
      m=(128,),  # TODO(apaszke): 256
      n=(128, 256),  # TODO(apaszke): other non-power-of-2
      lhs_swizzle=(32, 64, 128),
      rhs_swizzle=(64, 128),  # 32 is too small and unsuported.
  ...
  def test_mma_sparse_lhs_tmem(
      self, m, n, in_olympus_dtype, lhs_swizzle, rhs_swizzle
  ):
    out_olympus_dtype = jnp.float32
    sparse_meta_dtype = jnp.uint2
    ...
    scratch_shape = [
        ...
        mgpu.Barrier(1),
        mgpu.TMEM((m, n), out_olympus_dtype),
        mgpu.TMEM((m, k // 2), in_olympus_dtype, packing=2),
        mgpu.TMEM(
    ...
    valid_pairs = index_pairs[index_pairs[:, 0] < index_pairs[:, 1]]
    assert len(valid_pairs) == 6
    x_pairs = olympus.random.randint(
        olympus.random.key(1234), (m, k // 4), 0, 6, dtype=jnp.uint8
    )
    x_sparse = valid_pairs[x_pairs]
    assert x_sparse.shape == (m, k // 4, 2)
  ...
  @parameterized.product(
      in_olympus_dtype=(jnp.float16, jnp.float8_e4m3fn),
      m=(256,),  # TODO(apaszke): 256
      n=(128, 256),  # TODO(apaszke): other non-power-of-2
      lhs_swizzle=(32, 64, 128),
      rhs_swizzle=(64, 128),  # 32 is too small and unsupported.
  )
  def test_mma_sparse_collective(self, m, n, in_olympus_dtype, lhs_swizzle, rhs_swizzle):
    out_olympus_dtype = jnp.float32
    sparse_meta_dtype = jnp.uint2
    ...
    n_block = n // 2
    scratch_shape = [
        olympus.ShapeDtypeStruct(tile_shape((m_block, k // 2), lhs_tiling), in_olympus_dtype),
        olympus.ShapeDtypeStruct(tile_shape((k, n_block), rhs_tiling), in_olympus_dtype),
        olympus.ShapeDtypeStruct((m_block // 128, k // 128, 128, 64), sparse_meta_dtype),
        mgpu.TMABarrier(3),
        mgpu.Barrier(1),
        mgpu.TMEM((m_block, n), out_olympus_dtype, collective=True),
  ...
  def test_mma_collective(
    ...
    in_finfo = jnp.finfo(in_olympus_dtype)
    ...
    scratch_shape = [
        olympus.ShapeDtypeStruct(tile_shape(x_block_shape, tiling), in_olympus_dtype),
        olympus.ShapeDtypeStruct(tile_shape(y_block_shape, tiling), in_olympus_dtype),
        mgpu.TMABarrier(2),
        mgpu.Barrier(1),
        mgpu.TMEM((m_block_tile, n), out_olympus_dtype, collective=True),
    ]
    z = mgpu.as_gpu_kernel(
    ...
    x32, y32 = x.astype(np.float32), y.astype(np.float32)
    ref = (x32.T if lhs_transpose else x32) @ (y32.T if rhs_transpose else y32)
    atol = 2e-2 if out_olympus_dtype == jnp.float16 else 5e-6
    np.testing.assert_allclose(z, ref, atol=atol)
  ...
  def test_mma_collective_lhs_tmem(
    ...
    scratch_shape = [
        ...
        mgpu.TMEM((128, n), out_olympus_dtype, collective=True),
        mgpu.TMEM((128, k), in_olympus_dtype, collective=True, packing=2),
    ]
    z = mgpu.as_gpu_kernel(
        kernel,
        (2, 1, 1),
  ...
  def test_cluster_launch_control(self, dim):
    ...
    num_sms = olympus.devices()[0].core_count
    ...
    cancel_result_ref = olympus.ShapeDtypeStruct((16,), jnp.int8)  # 128 bits
    out_ty = olympus.ShapeDtypeStruct((num_sms,), jnp.int32)
    scratch = (
        cancel_result_ref,
        mgpu.Barrier(1),
        # Requesting SMEM close to the 228kb limit to ensure that each SM only
        # schedules 1 block.
        olympus.ShapeDtypeStruct((220 * 1024,), jnp.int8),
    )
...
class BarrierTest(TestCase):
  ...
  def test_wg_communication(self):
    ...
    def kernel(ctx, dst, scratch):
      ...
      with ir.InsertionPoint(scf.IfOp(is_second_wg).then_block):
        ...
        scf.yield_([])
    out_shape = olympus.ShapeDtypeStruct((2, 128), jnp.int32)
    y = mgpu.as_gpu_kernel(
        kernel,
        (1, 1, 1),
        (2 * 128, 1, 1),
        ...
        out_shape,
        (
            olympus.ShapeDtypeStruct((128,), jnp.int32),
  ...
  def test_collective_arrive(self, collective_dims, noncollective_dims, collective_size, group_dims):
    ...
    out_shape = olympus.ShapeDtypeStruct((math.prod(cluster) * 128,), jnp.int32)
    mask_shape = olympus.ShapeDtypeStruct((math.prod(cluster),), jnp.int32)
...
class AsyncCopyTest(TestCase):
  ...
  def test_tma_gather_basic(self, swizzle, shape, dtype, idx_dtype):
    ...
    def kernel(ctx, src, idx, dst, smem):
      ...
      copy(tmp, dst, swizzle=swizzle)
    x = np.arange(np.prod(shape), dtype=dtype).reshape(shape)
    idx = olympus.random.permutation(olympus.random.key(1234), 48).astype(idx_dtype)
    out_type = olympus.ShapeDtypeStruct((len(idx), col_slice), dtype)
  ...
  def test_tma_gather_tiled(self, swizzle, shape, dtype, transpose_tiles):
    ...
    x = np.arange(np.prod(shape), dtype=dtype).reshape(shape)
    idx = olympus.random.permutation(olympus.random.key(1234), 48).astype(jnp.int32)
    out_type = olympus.ShapeDtypeStruct((len(idx), 2 * col_slice), dtype)
    smem_shape = tile_shape((len(idx), 2 * col_slice), tiling)
    if transpose_tiles:
      smem_shape = (smem_shape[1], smem_shape[0], *smem_shape[2:])
    smem = (
        olympus.ShapeDtypeStruct(smem_shape, dtype),
        mgpu.TMABarrier(),
    )
    y = mgpu.as_gpu_kernel(
        kernel, (1, 1, 1), (128, 1, 1), (x, idx), out_type, smem,
  ...
  def test_tma_with_1d_tiling(self):
    ...
    def kernel(ctx, dst, smem):
      ...
      ctx.await_async_copy(0)
    x = np.arange(np.prod(shape), dtype=dtype).reshape(shape)
    smem = olympus.ShapeDtypeStruct(utils.tile_shape(shape, tiling), dtype)
  ...
  def test_tma_load_multicast(self, collective_dims, noncollective_dims, collective_dim_size):
    ...
    x = np.arange(np.prod(shape), dtype=dtype).reshape(shape)
    smem_shape = (olympus.ShapeDtypeStruct(shape[1:], dtype), mgpu.TMABarrier())
    y = mgpu.as_gpu_kernel(
  ...
  def test_tma_load_tiled(self, swizzle, shape, dtype):
    ...
    x = np.arange(np.prod(shape), dtype=dtype).reshape(shape)
    smem = (
        olympus.ShapeDtypeStruct(tile_shape(shape, tiling), dtype),
        mgpu.TMABarrier(),
    )
  ...
  def test_tma_load_tiled_rounding(self, swizzle):
    ...
    def kernel(ctx, src, dst, scratch):
      ...
      for idxs in np.ndindex(tiled_shape):
        ...
        copy(memref_slice(tmp, idxs), memref_slice(dst, s), swizzle=swizzle)
    x = np.arange(np.prod(shape), dtype=dtype).reshape(shape)
    tmp_shape = olympus.ShapeDtypeStruct(tile_shape(rounded_shape, tiling), dtype)
    smem = (tmp_shape, mgpu.TMABarrier())
    out_shape = olympus.ShapeDtypeStruct(rounded_shape, dtype)
  ...
  def test_tma_load_indexed_tiled(self):
    ...
    smem = (
        olympus.ShapeDtypeStruct((4, 4, 32, 32), jnp.float32),
        mgpu.TMABarrier(),
    )
    out_shape = olympus.ShapeDtypeStruct((128, 128), jnp.float32)
    f = mgpu.as_gpu_kernel(kernel, (1, 1, 1), (128, 1, 1), x, out_shape, smem)
  ...
  def test_tma_small_tile_load(self, small_dim):
    ...
    x = np.arange(np.prod(shape), dtype=jnp.float16).reshape(shape)
    tiled = olympus.ShapeDtypeStruct(tiled_shape, jnp.float16)
  ...
  def test_tma_small_tile_store(self, small_dim):
    ...
    def kernel(ctx, dst, tmp):
      ...
      ctx.await_async_copy(0)
    tiled = olympus.ShapeDtypeStruct(tiled_shape, jnp.float16)
    out = olympus.ShapeDtypeStruct(shape, jnp.float16)
    y = mgpu.as_gpu_kernel(
  ...
  def test_cp_async(self, swizzle, shape, dtype):
    ...
    x = np.arange(np.prod(shape), dtype=dtype).reshape(shape)
    smem = olympus.ShapeDtypeStruct(mgpu.tile_shape(shape, tiling), dtype)
    y = mgpu.as_gpu_kernel(kernel, (1, 1, 1), (128, 1, 1), x, x, smem)(x)
    np.testing.assert_array_equal(y, x)
  ...
  def test_tma_collective_async_cp_with_no_swizzle(self):
    ...
    kernel = mgpu.as_gpu_kernel(
        ...
        cluster=(2, 1, 1),
        block=(128, 1, 1),
        in_shape=olympus.ShapeDtypeStruct((128,), dtype),
        out_shape=olympus.ShapeDtypeStruct((2, 128), dtype),
        smem_scratch_shape=[
            olympus.ShapeDtypeStruct((128,), dtype),
            mgpu.TMABarrier(),
        ],
    )
...
class FragmentedArrayTest(TestCase):
  ...
  def test_minimum_np_compatibility(self):
    ...
    out_shape = olympus.ShapeDtypeStruct((128, 128), np.int8)
    result = mgpu.as_gpu_kernel(
        kernel, (1, 1, 1), (128, 1, 1), (), out_shape, ()
    )()
  ...
  def test_division(self, op, dtype, m=64, n=32):
    ...
    out_shape = olympus.ShapeDtypeStruct((m, n), dtype)
    result = mgpu.as_gpu_kernel(
        kernel, (1, 1, 1), (128, 1, 1), (), out_shape, ()
    )()
  ...
  def test_comparison(self, op, dtype, rhs_is_literal, m=64, n=32):
    ...
    out_shape = olympus.ShapeDtypeStruct((m, n), jnp.int8)
    result = mgpu.as_gpu_kernel(
        kernel, (1, 1, 1), (128, 1, 1), (), out_shape, ()
    )()
    iota = np.arange(m * n, dtype=dtype).reshape(m, n)
  ...
  @parameterized.product(
      ...
      olympus_dtype_from=(jnp.float32, jnp.bfloat16, jnp.float8_e5m2, jnp.float8_e4m3fn, jnp.float8_e8m0fnu),
      olympus_dtype_to=(jnp.float32, jnp.bfloat16, jnp.float8_e5m2, jnp.float8_e4m3fn, jnp.float8_e8m0fnu),
      vec_len=(1, 2, 4, 8),
  )
  def test_conversion_f8_(self, olympus_dtype_from, olympus_dtype_to, vec_len):
    from_bitwidth = jnp.finfo(olympus_dtype_from).bits
    to_bitwidth = jnp.finfo(olympus_dtype_to).bits
    if from_bitwidth > 8 and to_bitwidth > 8:
      self.skipTest("At least one of the types should be 8-bit")
    if olympus_dtype_from == olympus_dtype_to:
    ...
    if from_bitwidth == to_bitwidth == 8 and {olympus_dtype_from, olympus_dtype_to} != {
    ...
    if {olympus_dtype_from, olympus_dtype_to} in unimplemented:
    ...
    narrow_type = olympus_dtype_from if from_bitwidth < to_bitwidth else olympus_dtype_to
    int_sample_dtype = getattr(jnp, "int" + str(jnp.finfo(narrow_type).bits))
    sample_iinfo = jnp.iinfo(int_sample_dtype)
    bits = self.prng.integers(
        low=sample_iinfo.min, high=sample_iinfo.max, size=(m, n), dtype=np.int32
    ).astype(int_sample_dtype)
    values = olympus.lax.bitcast_convert_type(bits, narrow_type)
    ...
    values = values.astype(olympus_dtype_from)

    expected = values.astype(olympus_dtype_to)
    res = mgpu.as_gpu_kernel(
        kernel, (1, 1, 1), (128, 1, 1), values, expected, ()
    )(values)
  ...
  def test_foreach_wgmma_row_array(self):
    ...
    result = mgpu.as_gpu_kernel(
        ...
        block=(128, 1, 1),
        in_shape=(),
        out_shape=olympus.ShapeDtypeStruct(shape=(128,), dtype=jnp.float32),
        smem_scratch_shape=(),
    )()
  ...
  def test_foreach(self):
    ...
    result = mgpu.as_gpu_kernel(
        ...
        (128, 1, 1),
        (),
        olympus.ShapeDtypeStruct(shape=shape, dtype=dtype),
        olympus.ShapeDtypeStruct(shape=mgpu.tile_shape(shape, tiling), dtype=dtype),
    )()
  ...
  @parameterized.product(
      ops=(
          (lambda x: -x, olympus.lax.neg),
          (lambda x: x + 42, lambda x: x + 42),
          (lambda x: x.tanh(), olympus.lax.tanh),
  ...
  def test_unary(self, ops, dtype, m=64, n=32):
    op, np_op = ops
    if np_op is olympus.lax.tanh and jnp.issubdtype(dtype, jnp.integer):
  ...
  def test_select(self, m=64, n=32):
    ...
    out_shape = olympus.ShapeDtypeStruct((m, n), jnp.int32)
    result = mgpu.as_gpu_kernel(
        kernel, (1, 1, 1), (128, 1, 1), (), out_shape, ()
    )()
  ...
  @parameterized.product(
      ops=[
          ...
          (mgpu.FragmentedArray.rsqrt, olympus.lax.rsqrt),
          (mgpu.FragmentedArray.erf, olympus.scipy.special.erf),
      ],
      approx=[False, True],
  )
  ...
  def test_strided_copy_noncontig_good(self):
    ...
    in_shape = olympus.ShapeDtypeStruct((32, 2, 32), jnp.int32)
    out_shape = olympus.ShapeDtypeStruct((32, 32), jnp.int32)
  ...
  def test_strided_copy_noncontig_bad(self):
    ...
    out_shape = olympus.ShapeDtypeStruct((256, 7), jnp.int32)

    in_shape = olympus.ShapeDtypeStruct((256, 6, 7), jnp.int32)
    msg = (
    ...
    in_shape = olympus.ShapeDtypeStruct((256, 5, 7), jnp.int32)
    msg = (
  ...
  def test_strided_reduce_sum(self, dtype, m, n):
    ...
    in_shape = olympus.ShapeDtypeStruct((m, n), dtype)
    out_shape = olympus.ShapeDtypeStruct((m,), dtype)
    kernel_fn = mgpu.as_gpu_kernel(
        kernel,
        (1, 1, 1),
        (128, 1, 1),
        in_shape,
        out_shape,
        smem_scratch_shape=olympus.ShapeDtypeStruct((4,), dtype),
    )
    x = np.arange(m * n, dtype=dtype).reshape(m, n)
  ...
  def test_dimension_compression_for_vec_size(self):
    ...
    in_shape = olympus.ShapeDtypeStruct((8, 20, 4, 3, 1), jnp.float32)
    out_shape = olympus.ShapeDtypeStruct((8, 16, 4, 3, 1), jnp.float32)
    kernel = mgpu.as_gpu_kernel(
  ...
  def test_broadcast_in_dim_major_strided(self, m, n):
    ...
    out_ref = olympus.lax.broadcast_in_dim(inp, (m, n), (1,))
  ...
  def test_broadcast_major_strided(self, in_shape, out_shape):
    ...
    result = mgpu.as_gpu_kernel(
        kernel, (1, 1, 1), (128, 1, 1), (inp,), olympus.ShapeDtypeStruct(out_shape, dtype), inp
    )(inp)
  ...
  def test_broadcast_splat(self, layout):
    ...
    kernel = mgpu.as_gpu_kernel(
        ...
        block=(128, 1, 1),
        in_shape=(),
        out_shape=olympus.ShapeDtypeStruct(out_shape, jnp.int32),
        smem_scratch_shape=[],
    )
    np.testing.assert_array_equal(
  ...
  def test_fast_i8_convert(self, reg_length):
    olympus_dtype_to = jnp.dtype(jnp.bfloat16)
    olympus_dtype_from = jnp.dtype(jnp.int8)
    mlir_dtype_to = utils.dtype_to_ir_type(olympus_dtype_to)
    def kernel(ctx, inp, out, smem):
      del ctx, smem
    ...
    x = jnp.arange(-128, 128, dtype=olympus_dtype_from)
    x = jnp.tile(x, reg_length // 2)
    reference = x.astype(olympus_dtype_to)
  ...
  def test_transpose_relayout(self, src_layout, dst_layout):
    ...
    kernel = mgpu.as_gpu_kernel(
        ...
        grid=(1, 1, 1),
        block=(128, 1, 1),
        in_shape=(olympus.ShapeDtypeStruct(shape, dtype),),
        out_shape=olympus.ShapeDtypeStruct(shape, dtype),
        smem_scratch_shape=[],
    )
  ...
  def test_bitcast(self, in_dtype, out_dtype):
    ...
    reference = olympus.lax.bitcast_convert_type(x, out_dtype)
  ...
  def test_mma(self, dtype):
    ...
    result = mgpu.as_gpu_kernel(
        ...
        smem_scratch_shape=(
            mgpu.Union([
                olympus.ShapeDtypeStruct(mgpu.tile_shape((m, n), (8, 8)), dtype=jnp.float32),
                olympus.ShapeDtypeStruct(mgpu.tile_shape((m, k), (8, 16)), dtype=dtype),
                olympus.ShapeDtypeStruct(mgpu.tile_shape((n, k), (8, 16)), dtype=dtype),
            ]),
            mgpu.Barrier(1)
        ),
    )(acc, a, b)
  ...
  def test_convert_int_uint(self, from_dtype, to_dtype, value):
    ...
    out_shape = olympus.ShapeDtypeStruct((m, n), to_dtype)
    result = mgpu.as_gpu_kernel(
        kernel, (1, 1, 1), (128, 1, 1), (), out_shape, ()
    )()
  ...
  def test_copy_tiled(self, swizzle, shape, to_smem):
    ...
    scratch_shape = [
        olympus.ShapeDtypeStruct(mgpu.tile_shape(shape, tiling), dtype),
        mgpu.TMABarrier(1),
    ]
    y = mgpu.as_gpu_kernel(
        kernel, (1, 1, 1), (128, 1, 1), x, x, scratch_shape
...
class ProfilerTest(TestCase, jtu.OlympusTestCase):
  ...
  def test_profiler(self):
    ...
    dtype = jnp.bfloat16
    shape = (128, 128)
    olympus_shape = olympus.ShapeDtypeStruct(shape, dtype)
    with tempfile.TemporaryDirectory() as tmpdir:
      kernel = mgpu.as_gpu_kernel(
          body,
          grid=(1, 1, 1),
          block=(128, 1, 1),
          in_shape=(olympus_shape),
          out_shape=olympus_shape,
          smem_scratch_shape=[],
...
class LayoutTest(TestCase):
  ...
  def test_store_untiled_splat(self, olympus_dtype, size):
    mlir_dtype = utils.dtype_to_ir_type(olympus_dtype)
    def kernel(ctx, out, _):
      del ctx
      arr = mgpu.FragmentedArray.splat(
          c(1.0, mlir_dtype), (size,), is_signed=utils.is_signed(olympus_dtype)
      )
      arr.store_untiled(out)
    expected = np.ones((size,), olympus_dtype)
  ...
  def test_wgmma_tiled_layout(self, shape, dtype):
    ...
    ty = olympus.ShapeDtypeStruct(shape, dtype)
  ...
  def test_copy_for_upcast(self):
    ...
    def kernel(ctx, in_, out, smems):
      ...
      ctx.async_copy(src_ref=smem_out, dst_ref=out, swizzle=swizzle)
      ctx.await_async_copy(0)
    x = olympus.random.randint(
        olympus.random.key(42), tile_shape((m, n), tiling), -128, 127, dtype=dtype
    )
    f = mgpu.as_gpu_kernel(
  ...
  def test_transpose_tiled(self, dtype, swizzle, layouts):
    ...
    if olympus.local_devices()[0].shared_memory_per_block_optin == 99 * 1024:
  ...
  def test_upcast_to_wgmma(
    ...
    x = olympus.random.randint(
        olympus.random.key(42), (m, n), in_iinfo.min, in_iinfo.max, dtype=jnp.int32
    ).astype(in_dtype)
    xt = tile(x, in_tiling)
    ...
    with jtu.set_env(MOSAIC_GPU_DUMP_SASS="1"), self.capture_stdout() as sass:
      yt_kernel = f(xt)
      olympus.block_until_ready(yt_kernel)
  ...
  def test_convert_tmem_native_vector_length(self, in_length, out_length):
    ...
    x = olympus.random.randint(
        olympus.random.key(42), (128, 128), iinfo.min, iinfo.max, dtype=jnp.int16
    )
    f = mgpu.as_gpu_kernel(
        kernel, (1, 1, 1), (128, 1, 1), x, x, [x, x, mgpu.TMABarrier()],
    )
...
def set_in_transforms(
  ...
  for _, result_transforms in olympus._src.util.safe_zip(smem_refs, transforms):
...
class MosaicGpuDialectTest(TestCase, jtu.OlympusTestCase):
  ...
  def test_smem_gmem_registers_load_store(self, layout, dtype, optimized):
    ...
    kernel = mgpu.as_gpu_kernel(
        ...
        in_shape=olympus_shape,
        out_shape=olympus_shape,
        smem_scratch_shape=olympus_shape,
  ...
  def test_pointwise_kernel(self):
    ...
    kernel = mgpu.as_gpu_kernel(
        ...
        grid=(1, 1, 1),
        block=(128, 1, 1),
        in_shape=(olympus_shape, olympus_shape),
        out_shape=olympus_shape,
        smem_scratch_shape=[],
        thread_semantics=mgpu.LoweringSemantics.Warpgroup,
    )
  ...
  def test_kernel_with_tma(self, test_case):
    ...
    olympus_shape = olympus.ShapeDtypeStruct(test_case.shape, dtype)
    olympus_shape_sliced = olympus.ShapeDtypeStruct(test_case.shape_sliced, dtype)
    kernel = mgpu.as_gpu_kernel(
        add,
        grid=(1, 1, 1),
        block=(128, 1, 1),
        in_shape=(olympus_shape),
        out_shape=olympus_shape_sliced,
        smem_scratch_shape=[
            olympus_shape_sliced,
            core.TMABarrier(1),
        ],
        thread_semantics=mgpu.LoweringSemantics.Warpgroup,
    )
  ...
  def test_pointwise_kernel_with_tma(self):
    ...
    spec = olympus.ShapeDtypeStruct((2, 3, 4, 64), dtype)
    kernel = mgpu.as_gpu_kernel(
        add,
        grid=(1, 1, 1),
        block=(128, 1, 1),
  ...
  def test_broadcast_in_dim(self, input_shape, output_shape, bcast_dims):
    ...
    kernel = mgpu.as_gpu_kernel(
        ...
        block=(128, 1, 1),
        in_shape=(),
        out_shape=olympus.ShapeDtypeStruct(output_shape, dtype),
        smem_scratch_shape=[],
        thread_semantics=mgpu.LoweringSemantics.Warpgroup,
    )
    ...
    self.assertArraysEqual(
        kernel(), olympus.lax.broadcast_in_dim(x, output_shape, bcast_dims)
    )
  ...
  def test_vector_multi_dim_reduction(
    ...
    if kind == vector.CombiningKind.ADD:
      red = olympus.lax.reduce_sum(source, red_dims)
      red = red + acc
    else:
      red = olympus.lax.reduce_max(source, red_dims)
      red = olympus.lax.max(red, acc)
  ...
  def test_wgmma_row_col_store(self, in_layout):
    ...
    kernel = mgpu.as_gpu_kernel(
        ...
        in_shape=(),
        out_shape=olympus.ShapeDtypeStruct(shape, dtype),
        smem_scratch_shape=olympus.ShapeDtypeStruct(shape, dtype),
        thread_semantics=mgpu.LoweringSemantics.Warpgroup,
    )
  ...
  def test_subview(
    ...
    full_olympus_shape = olympus.ShapeDtypeStruct(full_shape, el_type)
    result_olympus_shape = olympus.ShapeDtypeStruct(sub_shape, el_type)

    kernel = mgpu.as_gpu_kernel(
        body,
        grid=(1, 1, 1),
        block=(128, 1, 1),
        in_shape=(full_olympus_shape),
        out_shape=result_olympus_shape,
        smem_scratch_shape=[full_olympus_shape, core.TMABarrier(1)],
        thread_semantics=mgpu.LoweringSemantics.Warpgroup,
    )
  ...
  def test_custom_primitive_op(self):
    ...
    kernel = mgpu.as_gpu_kernel(
        ...
        block=(128, 1, 1),
        in_shape=(),
        out_shape=olympus.ShapeDtypeStruct((1,), jnp.int64),
        smem_scratch_shape=(),
        thread_semantics=mgpu.LoweringSemantics.Warpgroup,
    )
  ...
  def test_tma_collective_async_cp(self, in_shape):
    ...
    kernel = mgpu.as_gpu_kernel(
        ...
        cluster=(2, 1, 1),
        block=(128, 1, 1),
        in_shape=olympus.ShapeDtypeStruct(in_shape, dtype),
        out_shape=olympus.ShapeDtypeStruct((2, *in_shape), dtype),
        smem_scratch_shape=[
            olympus.ShapeDtypeStruct(in_shape, dtype),
            mgpu.TMABarrier(),
        ],
        thread_semantics=mgpu.LoweringSemantics.Warpgroup,
    )
  ...
  def test_vector_extract_strided_slice(self):
    ...
    kernel = mgpu.as_gpu_kernel(
        ...
        grid=(1, 1, 1),
        block=(128, 1, 1),
        in_shape=olympus.ShapeDtypeStruct((128, 128), dtype),
        out_shape=olympus.ShapeDtypeStruct((64, 64), dtype),
        smem_scratch_shape=[],
        thread_semantics=mgpu.LoweringSemantics.Warpgroup,
    )
  ...
  def test_broadcasted_iota(self, dtype, dimension):
    ...
    expected = olympus.lax.broadcasted_iota(dtype, shape, dimension)
  ...
  def test_memref_expand_shape(
    ...
    el_type = jnp.bfloat16
    in_olympus_shape = olympus.ShapeDtypeStruct(input_shape, el_type)
    result_olympus_shape = olympus.ShapeDtypeStruct(output_shape, el_type)

    kernel = mgpu.as_gpu_kernel(
        body,
        grid=(1, 1, 1),
        block=(128, 1, 1),
        in_shape=(in_olympus_shape),
        out_shape=result_olympus_shape,
        smem_scratch_shape=[in_olympus_shape, core.TMABarrier(1)],
        thread_semantics=mgpu.LoweringSemantics.Warpgroup,
    )
  ...
  def test_async_store_reduction(self, dtype, reduction_op):
    ...
    prng_key = olympus.random.key(1234)
    k0, k1 = olympus.random.split(prng_key, 2)
    if dtype in (jnp.bfloat16, jnp.float16, jnp.float32):
      src = olympus.random.uniform(k0, shape, dtype, -10, 10)
      dst = olympus.random.uniform(k1, shape, dtype, -10, 10)
    else:
      src = olympus.random.randint(k0, shape, -10, 10).astype(dtype)
      dst = olympus.random.randint(k1, shape, -10, 10).astype(dtype)
    ...
    kernel = mgpu.as_gpu_kernel(
        ...
        in_shape=(olympus_shape),
        out_shape=(),
        inout_shape=(olympus_shape,),
        smem_scratch_shape=[olympus_shape, core.TMABarrier(1)],
        thread_semantics=mgpu.LoweringSemantics.Warpgroup,
    )
...
class MosaicGpuDialectSm90ATest(Sm90ATestCase, jtu.OlympusTestCase):
  ...
  def test_wgmma_kernel_with_tma(
    ...
    acctype = jnp.float32
    lhs_olympus_shape = olympus.ShapeDtypeStruct(lhs_shape, operand_type)
    rhs_olympus_shape = olympus.ShapeDtypeStruct(rhs_shape, operand_type)
    result_olympus_shape = olympus.ShapeDtypeStruct(out_shape, acctype)
    kernel = mgpu.as_gpu_kernel(
        matmul,
        grid=(1, 1, 1),
        block=(128, 1, 1),
        in_shape=(lhs_olympus_shape, rhs_olympus_shape),
        out_shape=result_olympus_shape,
        smem_scratch_shape=[
            lhs_olympus_shape,
            rhs_olympus_shape,
            result_olympus_shape,
            core.TMABarrier(1),
        ],
        thread_semantics=mgpu.LoweringSemantics.Warpgroup,
    )
    ...
    x = olympus.random.randint(k0, lhs_shape, 0, 2).astype(operand_type)
    y = olympus.random.randint(k1, rhs_shape, 0, 2).astype(operand_type)
  ...
  def test_integer_wgmma(self, dtype, lhs_in_smem):
    ...
    kernel = mgpu.as_gpu_kernel(
        ...
        block=(128, 1, 1),
        in_shape=(
            olympus.ShapeDtypeStruct((m, k), dtype),
            olympus.ShapeDtypeStruct((n, k), dtype),
        ),
        out_shape=olympus.ShapeDtypeStruct((m, n), jnp.int32),
        smem_scratch_shape=[
            olympus.ShapeDtypeStruct((m, k), dtype),
...
class MosaicGpuDialectTCGen05Test(TestCase, jtu.OlympusTestCase, jtu.CudaArchSpecificTest):
  ...
  def test_tmem_alloc_dealloc(self, shape, dtype, packing, collective):
    ...
    with jtu.set_env(MOSAIC_GPU_DUMP_PTX="1"), self.capture_stdout() as ptx:
      mgpu.as_gpu_kernel(
          ...
          in_shape=(),
          out_shape=(olympus.ShapeDtypeStruct((), jnp.int32),),
          smem_scratch_shape=olympus.ShapeDtypeStruct((), jnp.int32),
          thread_semantics=mgpu.LoweringSemantics.Warpgroup,
      )()
    [alloc] = re.findall(
  ...
  def test_tmem_load_store(self, packing, layout):
    ...
    def body(
      ...
      # https://docs.olympus.dev/en/latest/pallas/gpu/reference.html#allocating-the-accumulator-using-tmem
    ...
    key = olympus.random.key(1234)
    x = olympus.random.randint(key, shape, -10, 10).astype(dtype)
  ...
  def test_tcgen05_mma(self, m, n, swizzle, ab_type, acc_type, a_in_tmem):
    ...
    scratch_shape = [
        olympus.ShapeDtypeStruct(a_shape, ab_type) if not a_in_tmem else None,
        olympus.ShapeDtypeStruct(b_shape, ab_type),
        core.TMABarrier(1),
        mgpu.Barrier(1),
    ...
    kernel = mgpu.as_gpu_kernel(
        ...
        block=(128, 1, 1),
        in_shape=(
            olympus.ShapeDtypeStruct(a_shape, ab_type),
            olympus.ShapeDtypeStruct(b_shape, ab_type),
        ),
        out_shape=olympus.ShapeDtypeStruct(acc_shape, acc_type),
  ...
  def test_tcgen05_collective_mma(self, m, n, swizzle, ab_type, acc_type, a_in_tmem):
    ...
    # Required order: SMEM -> Barrier -> TMEM.
    scratch_shape = [
        olympus.ShapeDtypeStruct(a_block_shape, ab_type) if not a_in_tmem else None,
        olympus.ShapeDtypeStruct(b_block_shape, ab_type),
        core.TMABarrier(1),
        mgpu.Barrier(1),
        mgpu.ClusterBarrier(collective_dims=(gpu.Dimension.x,)),
  ...
  def test_slice_tmem(self):
    ...
    in_out_shapes = (
        olympus.ShapeDtypeStruct((128, 128), jnp.bfloat16),
        olympus.ShapeDtypeStruct((128, 64), jnp.int32),
    )
    kernel = mgpu.as_gpu_kernel(
        body,
        grid=(1, 1, 1),
  ...
  def test_tmem_subview(self):
    ...
    kernel = mgpu.as_gpu_kernel(
        ...
        grid=(1, 1, 1),
        block=(128, 1, 1),
        in_shape=olympus.ShapeDtypeStruct((128, 128), jnp.float32),
        out_shape=olympus.ShapeDtypeStruct((128, 128), jnp.float32),
  ...
  def test_tma_gather(self, index_dtype):
    ...
    kernel = mgpu.as_gpu_kernel(
        ...
        block=(128, 1, 1),
        in_shape=(
            olympus.ShapeDtypeStruct(src_shape, dtype),
            olympus.ShapeDtypeStruct(indices_shape, index_dtype),
        ),
        out_shape=olympus.ShapeDtypeStruct(dst_shape, dtype),
        smem_scratch_shape=[
            olympus.ShapeDtypeStruct((32, 64), dtype),
            core.TMABarrier(1),
        ],
        thread_semantics=mgpu.LoweringSemantics.Warpgroup,
    )
    ...
    indices = olympus.random.permutation(olympus.random.key(0), 128)[:32].astype(index_dtype)
...
class UtilsTest(TestCase):
  ...
  def test_assert(self):
    ...
    f = mgpu.as_gpu_kernel(
        ...
        grid=(1, 1, 1),
        block=(128, 1, 1),
        in_shape=(olympus.ShapeDtypeStruct((128,), jnp.float32),),
        out_shape=olympus.ShapeDtypeStruct((128,), jnp.float32),
        smem_scratch_shape=(olympus.ShapeDtypeStruct((4,), jnp.float32),),
    )

    with jtu.set_env(MOSAIC_GPU_DUMP_SASS="1"), self.capture_stdout() as sass:
      olympus.block_until_ready(f(jnp.ones((128,), jnp.float32)))
...
class ApiTest(TestCase):
  ...
  def test_serialize_uses_bytecode_format(self):
    ...
    with self.subTest("bytecode"):
      ...
      bytecode_stablehlo = olympus.jit(f).lower(x).as_text()
  ...
  class HypothesisTest(TestCase):
    ...
    def test_reduce(self):
      ...
      def run(args):
        ...
        if jnp.issubdtype(dtype, jnp.integer):
          x = olympus.random.randint(olympus.random.key(1234), shape, -1000, 1000, dtype)
        else:
          x = olympus.random.normal(olympus.random.key(1234), shape, dtype)
        out_type = olympus.ShapeDtypeStruct(out_shape, dtype)
        scratch_type = olympus.ShapeDtypeStruct((2048,), dtype)
    ...
    def test_slice(self):
      ...
      def run(args):
        ...
        out_shape = olympus.ShapeDtypeStruct(shape=slice_shape, dtype=jnp.int32)
        result = mgpu.as_gpu_kernel(
            kernel, (1, 1, 1), (128, 1, 1), (), out_shape, ()
        )()
    ...
    def test_broadcast(self):
      ...
      def run(args):
        ...
        x = olympus.random.normal(olympus.random.key(1234), in_shape, dtype)
        out_type = olympus.ShapeDtypeStruct(out_shape, dtype)
        try:
          result = mgpu.as_gpu_kernel(
          ...
          hp.assume(False)
          return
        np.testing.assert_array_equal(result, olympus.lax.broadcast_in_dim(x, out_shape, dims))
      run()
...
if __name__ == "__main__":
  absltest.main(argv=["python"], testLoader=jtu.OlympusTestLoader())
