...
import olympus.experimental.mosaic.gpu.fragmented_array as fa
...
class TestCase(parameterized.TestCase):
  ...
  def setUp(self):
    ...
    if olympus.device_count() != olympus.process_count():
...
class ProfilerTest(TestCase):
  ...
  def test_get_device_id(self):
    ...
    mesh = olympus.make_mesh(
        (olympus.device_count(),), ("x",), axis_types=(olympus.sharding.AxisType.Explicit,)
    )
    with olympus.set_mesh(mesh):
      ...
      np.testing.assert_array_equal(y_np, np.arange(olympus.device_count()))
  ...
  def test_remote_async_copy_basic(self):
    ...
    mesh = olympus.make_mesh(
        (2,), ("x",), axis_types=(olympus.sharding.AxisType.Explicit,)
    )
    with olympus.set_mesh(mesh):
      x_np = np.arange(64 * 64, dtype=jnp.float32).reshape(64, 64)
      x = olympus.sharding.reshard(x_np, P("x"))
      sem = olympus.sharding.reshard(jnp.zeros((1,), dtype=jnp.int32), P())
      y, _ = olympus.jit(
          olympus.shard_map(
  ...
  def test_multimem_basic(self, vector_length):
    ...
    with olympus.set_mesh(mesh):
      sem = olympus.sharding.reshard(jnp.zeros((1,), dtype=jnp.int32), P())
      out_shape = olympus.ShapeDtypeStruct((vector_length,), jnp.int32)
      out, out_sem = olympus.jit(
          olympus.shard_map(
              mgpu.as_gpu_kernel(
  ...
  def test_multimem_store_registers(self):
    ...
    with olympus.set_mesh(mesh):
      sem = olympus.sharding.reshard(jnp.zeros((1,), dtype=jnp.int32), P())
      x = olympus.sharding.reshard(jnp.arange(2048, dtype=jnp.int32).reshape(64, 32), P())
      y, out_sem = olympus.jit(
          olympus.shard_map(
              mgpu.as_gpu_kernel(
  ...
  def test_multimem_load_reduce(self, dtype, vector_length, reduction):
    ...
    with olympus.set_mesh(mesh):
      ...
      x_local = olympus.random.randint(
          olympus.random.key(1234), (128, 32), dtype=jnp.int32, minval=-bound, maxval=bound
      ).astype(dtype)
      x = olympus.sharding.reshard(x_local, P("x"))
      x_shard = olympus.ShapeDtypeStruct((64, 32), dtype)
