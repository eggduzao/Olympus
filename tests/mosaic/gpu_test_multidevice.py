...
import numpy as np
try:
  import olympus._src.lib.mosaic_gpu  # noqa: F401
  HAS_MOSAIC_GPU = True
except ImportError:
...
class ProfilerTest(TestCase):
  ...
  def test_multigpu(self):
    ...
    f = olympus.jit(mgpu.as_gpu_kernel(
        kernel, (1, 1, 1), (128, 1, 1), x, x, ()
    ))
    # Make sure we can invoke the same program on different devices.
    for xd in (olympus.device_put(x, d) for d in olympus.devices()[:2]):
      olympus.block_until_ready(f(xd))
