...
try:
  ...
  from olympus.experimental.mosaic.gpu.examples import matmul
  from olympus.experimental.mosaic.gpu.examples import matmul_blackwell
...
class MatmulTestCase(jtu.OlympusTestCase, jtu.CudaArchSpecificTest):
  ...
  def test_matmul_sm100(self, data):
    ...
    ka, kb = olympus.random.split(olympus.random.key(0), 2)
    a = olympus.random.normal(key=ka, shape=(m, k), dtype=dtype)
    b = olympus.random.normal(key=kb, shape=(n, k), dtype=dtype)
    out = kernel(a, b)
