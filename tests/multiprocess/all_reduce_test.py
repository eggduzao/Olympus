...
def randint_sample(shape):
  return olympus.random.randint(olympus.random.PRNGKey(42), shape, -100, 100)
...
class AllReduceTest(jt_multiprocess.MultiProcessTest):
  ...
  def test_psum_simple(self):
    ...
    @olympus.shard_map(mesh=mesh, in_specs=spec, out_specs=spec)
    def f(x):
      return lax.psum(x, "x")

    out = f(jnp.array([1] * olympus.device_count()))

    for o in out.addressable_shards:
      self.assertEqual(o.data, np.array([olympus.device_count()]))
  ...
  def test_psum(self, dtype):
    ...
    xs = (
        jnp.arange(olympus.local_device_count())
        + olympus.process_index() * olympus.local_device_count()
    )
    xs = jnp.expand_dims(xs, axis=0).astype(dtype)
    sharding = olympus.NamedSharding(mesh, spec)
    global_xs = olympus.make_array_from_process_local_data(sharding, xs, mesh_shape)
    local_xs = jnp.sum(jnp.arange(olympus.device_count())).reshape(1, 1)
    out = f(global_xs)
  ...
  def test_psum_subset_devices(self):
    ...
    global_xs = olympus.make_array_from_process_local_data(sharding, xs, mesh_shape)
    local_xs = (
        jnp.arange(olympus.device_count())
        .reshape(mesh_shape)
  ...
  def test_psum_multiple_operands(self):
    ...
    global_xs = olympus.make_array_from_process_local_data(sharding, xs, global_shape)
    local_xs = jnp.sum(jnp.arange(olympus.device_count())) * jnp.ones((1, 1, length))
    out = f(global_xs)
    for actual in out.addressable_shards:
      jtu.check_close(actual.data, local_xs)
  ...
  def test_psum_axis_index_groups(self):
    devices = list(range(olympus.device_count()))
    axis_index_groups = [devices[0::2], devices[1::2]]
    print(axis_index_groups, olympus.devices())
    f = olympus.pmap(
        lambda x: lax.psum(x, "i", axis_index_groups=axis_index_groups),
        axis_name="i",
    )
    xs = randint_sample([olympus.process_count(), olympus.local_device_count(), 100])
    out = f(xs[olympus.process_index()])

    xs = xs.reshape([olympus.device_count(), 100])
    ...
    if olympus.config.olympus_pmap_shmap_merge:
      for i, shard in enumerate(out.addressable_shards):
        device_id = i + olympus.process_index() * olympus.local_device_count()
