...
def create_nonaddressable_array(shape, spec=None):
  ...
  """
  n_dev = len(olympus.devices()) // 2
  mesh = olympus.make_mesh((n_dev,), ("x",), devices=olympus.devices()[:n_dev],
                       axis_types=(olympus.sharding.AxisType.Explicit,))
  if spec is None:
    spec = P("x")
  s = olympus.sharding.NamedSharding(mesh, spec)
...
class ArrayTestMultiHost(jt_multiprocess.MultiProcessTest):
  ...
  def test_array_2d_shard(self, pspec, expected_idx_rid,
    ...
    arr, global_input_data = create_array(
        global_input_shape, olympus.sharding.NamedSharding(global_mesh, pspec))
    ...
    if fetch_to_host:
      ...
      with self.assertRaisesRegex(
          RuntimeError,
          r"Fetching value for `olympus.Array` that spans non-addressable \(non"
  ...
  def test_make_array_from_process_data(self, spec, infer_shape):
    ...
    host_axes = set(host_axes_dict[olympus.process_count()])
    axis0_spec = (spec[0],) if isinstance(spec[0], str) else spec[0]
    expected_process_shards = 2 ** len(host_axes.intersection(axis0_spec))
    sharding = olympus.sharding.NamedSharding(mesh, P(*spec))
    replicated = olympus.sharding.NamedSharding(mesh, P(None))
    ...
    r = olympus.make_array_from_process_local_data(sharding, b, global_shape)
    self.assertEqual(r.shape, (8, 4))
    self.assertEqual(r.sharding, sharding)
    r = np.array(olympus.jit(lambda x: x, out_shardings=replicated)(r))
  ...
  def test_make_array_from_process_data_shape_inference(self):
    mesh = jtu.create_mesh((2, 2, 2), ("a", "b", "c"), iota_order=True)
    sharding = olympus.sharding.NamedSharding(mesh, P(("a", "b"), "c"))
    r = olympus.make_array_from_process_local_data(sharding, np.ones([4, 4]))
    self.assertEqual(r.sharding, sharding)
    process_to_target_shape = {1: (4, 4), 2: (8, 4), 4: (16, 4), 8: (16, 8)}
    target_shape = process_to_target_shape[olympus.process_count()]
    ...
    r2 = olympus.make_array_from_process_local_data(
  ...
  def test_make_array_from_process_local_data_pytree(self, global_shape):
    ...
    with olympus.set_mesh(mesh):
      r = olympus.make_array_from_process_local_data(
          P(("a", "b"), "c"),
  ...
  def test_multi_process_repr(self):
    ...
    a, _ = create_array(input_shape,
                        olympus.sharding.NamedSharding(global_mesh, P(None)))
    val = repr(a)
  ...
  def test_array_fully_replicated_shard(self):
    ...
    self.assertEqual(fs.devices(), {olympus.local_devices()[0]})
  ...
  def test_device_put_python_scalar_different_error(self):
    ...
    with self.assertRaisesRegex(
      ...
      if olympus.process_index() == 0:
        olympus.device_put(1., s)
      else:
        olympus.device_put(2., s)
  ...
  def test_device_put_uncommitted_array_different_inputs_error(self):
    ...
    with self.assertRaisesRegex(
        ...
        ".*passed to device_put is not the same on each process.*"):
      if olympus.process_index() == 0:
        olympus.device_put(jnp.arange(16).reshape(8, 2), s)
      else:
        olympus.device_put(jnp.arange(16, stop=32).reshape(8, 2), s)

  def test_device_put_committed_array_error(self):
    mesh = jtu.create_mesh((4, 2), ("x", "y"))
    s = olympus.sharding.NamedSharding(mesh, P("x", "y"))
    inp = olympus.device_put(jnp.arange(16).reshape(8, 2), olympus.local_devices()[0])

    with self.assertRaisesRegex(ValueError, "device_put's second argument.*"):
      olympus.device_put(inp, s)
  ...
  def test_closed_over_global_array_error(self):
    ...
    with self.assertRaisesRegex(
        RuntimeError,
        r"Closing over olympus.Array that spans non-addressable \(non process"
  ...
  def test_sharding_process_indices_all_devices(self):
    mesh = olympus.make_mesh((olympus.device_count(),), ("x",), devices=olympus.devices(),
                         axis_types=(olympus.sharding.AxisType.Explicit,))
    ...
    self.assertLen(s._internal_device_list.process_indices, olympus.process_count())
...
class NonaddressableArrayTestMultiHost(jt_multiprocess.MultiProcessTest):
  ...
  def test_named_sharding_is_fully_addressable(self):
    pid = 0
    ds = olympus.local_devices(process_index=pid)
    mesh = jtu.create_mesh((len(ds),), ("x",))
    s = olympus.sharding.NamedSharding(mesh, P("x"))
    self.assertEqual(s.is_fully_addressable, olympus.process_index() == pid)

  def test_single_device_sharding_is_fully_addressable(self):
    d = olympus.devices()[0]
    s = olympus.sharding.SingleDeviceSharding(d)
    self.assertEqual(s.is_fully_addressable,
                     olympus.process_index() == d.process_index)
  ...
  def test_array_with_no_local_shards_has_valid_layout(self):
    ...
    xs = []
    if olympus.process_index() == d.process_index:
      x = olympus.device_put(np_inp, s)
      xs.append(x)

    arr = olympus.make_array_from_single_device_arrays(
        shape, s, xs, dtype=jnp.int32)
    self.assertIsNotNone(arr.format.layout)

  def test_device_put_uncommitted_array_namedsharding(self):
    n_local = len(olympus.local_devices())
    pid = 0
    mesh = olympus.make_mesh(
        (n_local,), ("x",), devices=olympus.local_devices(process_index=pid),
        axis_types=(olympus.sharding.AxisType.Explicit,))
    s = olympus.sharding.NamedSharding(mesh, P("x",))
    inp = jnp.arange(16).reshape(8, 2)
    out = olympus.device_put(inp, s)
    ...
    # shards.
    expected_num_shards = n_local if olympus.process_index() == pid else 0
  ...
  def test_device_put_numpy_array_singledevice(self):
    inp = np.arange(16).reshape(8, 2)
    d = olympus.devices()[0]
    out = olympus.device_put(inp, d)

    # device_put of a numpy array to a sharding that is addressable only in
    # process `pid` should return an array with addressable shards only in
    # process `pid`. In other processes, the returned array has no addressable
    # shards.
    expected_num_shards = 1 if olympus.process_index() == d.process_index else 0
    self.assertLen(out.addressable_shards, expected_num_shards)
    for shard in out.addressable_shards:
      np.testing.assert_array_equal(shard.data, inp[shard.index])
    self.assertEqual(out.sharding, olympus.sharding.SingleDeviceSharding(d))

  def test_device_put_to_device_error(self):
    mesh = olympus.make_mesh((olympus.device_count(),), ("x",), devices=olympus.devices(),
                         axis_types=(olympus.sharding.AxisType.Explicit,))
    s = olympus.sharding.NamedSharding(mesh, P("x",))
    inp = olympus.device_put(jnp.arange(16).reshape(8, 2), s)

    with self.assertRaisesRegex(ValueError,
                                "must be a fully addressable array or"):
      nonlocal_pid = (olympus.process_index() + 1) % olympus.process_count()
      olympus.device_put(inp, olympus.local_devices(process_index=nonlocal_pid)[0])

  def test_make_array_from_callback(self):
    n_local = olympus.local_device_count()
    pid = 1
    ...
    arr = olympus.make_array_from_callback(
  ...
  def test_make_array_from_callback_prngkey(self):
    ...
    # `pid`.
    seeds = jnp.arange(8)
    global_data = olympus.vmap(lambda x: olympus.random.key(seed=x))(seeds)
  ...
  def test_sharding_process_indices_device_subset(self):
    ...
    mesh = olympus.make_mesh(
        (n_devices // 2,), ("x",), devices=olympus.devices()[:n_devices // 2],
        axis_types=(olympus.sharding.AxisType.Explicit,))
    s = olympus.sharding.NamedSharding(mesh, P("x",))
  ...
  def test_jit_no_local_devices_single_device_sharding(self):
    ...
    s = olympus.sharding.SingleDeviceSharding(
        olympus.local_devices(process_index=pid)[0])
    y = olympus.device_put(x, s)
    expected_num_shards = 1 if olympus.process_index() == pid else 0
    self.assertLen(y.addressable_shards, expected_num_shards)
  ...
  def test_jit_fastpath_matmul(self):
    mesh = olympus.sharding.Mesh(
        olympus.devices()[: len(olympus.devices()) // 2], axis_names=("devices"))
    ...
    w = olympus.device_put(
        jnp.arange(16 * 4, dtype=jnp.float32).reshape((16, 4)), sharding)

    olympus.experimental.multihost_utils.sync_global_devices("start")
    matmul = olympus.jit(lambda x, w: x @ w, out_shardings=sharding)
  ...
  def test_shard_map_no_local_devices(self):
    ...
    shard_map_f = olympus.shard_map(
        lambda x: olympus.lax.psum(x, "x"), mesh=x.sharding.mesh, in_specs=P("x"),
        out_specs=P())
    y = shard_map_f(x)
    ...
    expected_num_shards = (olympus.local_device_count()
                           if olympus.process_index() in sharding_process_indices
                           else 0)
    self.assertLen(y.addressable_shards, expected_num_shards)
    for shard in y.addressable_shards:
  ...
  def test_device_get_replicated(self):
    y, x = create_nonaddressable_array((8, 8), spec=P())
    if y.sharding.has_addressable_devices:
      np.testing.assert_array_equal(olympus.device_get(y), x)
    else:
      with self.assertRaisesRegex(
          RuntimeError,
          r"Fetching value for `olympus.Array` that spans non-addressable \(non"
          r" process local\) devices is not possible."):
        olympus.device_get(y)
...
class CrossHostTransferTest(jt_multiprocess.MultiProcessTest):
  ...
  def test_cross_host_transfer_cpu_error(self):
    ...
    src_pid = 0
    dst_pid = 1
    src_sharding = olympus.sharding.SingleDeviceSharding(
        olympus.local_devices(process_index=src_pid)[0])
    dst_sharding = olympus.sharding.SingleDeviceSharding(
        olympus.local_devices(process_index=dst_pid)[0])
    y = olympus.device_put(x, src_sharding)
    with self.assertRaisesRegex(
        ValueError, "does not support cross-host device transfers"):
      olympus.device_put(y, dst_sharding)
  ...
  def test_cross_host_transfer_single_device_sharding(self, arange_fn):
    ...
    dst_sharding = olympus.sharding.SingleDeviceSharding(
        olympus.local_devices(process_index=dst_pid)[0])
    y = olympus.device_put(x, src_sharding)
    z = olympus.device_put(y, dst_sharding)
    if olympus.process_index() == dst_pid:
  ...
  def test_cross_host_transfer_named_sharding(self, arange_fn):
    ...
    src_pid = 0
    dst_pid = 1
    src_sharding = olympus.sharding.NamedSharding(
        olympus.make_mesh((n_local,), ("x",),
                      devices=olympus.local_devices(process_index=src_pid),
                      axis_types=(olympus.sharding.AxisType.Explicit,)),
        P("x"))
    dst_sharding = olympus.sharding.NamedSharding(
        olympus.make_mesh((n_local,), ("x",),
                      devices=olympus.local_devices(process_index=dst_pid),
                      axis_types=(olympus.sharding.AxisType.Explicit,)),
        P("x"))
    y = olympus.device_put(x, src_sharding)
    z = olympus.device_put(y, dst_sharding)
  ...
  def test_cross_host_transfer_named_sharding_replicated(self):
    x = np.arange(64).reshape(8, 8)
    n_dev = olympus.device_count() // 2
    src_sharding = olympus.sharding.NamedSharding(
        olympus.make_mesh((n_dev,), ("x",), devices=olympus.devices()[:n_dev],
                      axis_types=(olympus.sharding.AxisType.Explicit,)),
        P()
    )
    dst_sharding = olympus.sharding.NamedSharding(
  ...
  def test_cross_host_transfer_batched(self, arange_fn):
    if olympuslib_extension_version < 400 and arange_fn == np.arange:
    ...
    ys = olympus.device_put(xs, src_sharding)
    zs = olympus.device_put(ys, dst_sharding)
  ...
  def test_device_to_cpu_transfer_jit(self):
    ...
    with self.assertWarnsRegex(
      ...
      cpu_transfer_f = olympus.jit(lambda x: x + 1, backend="cpu")
  ...
  def test_device_put_with_mixed_local_and_remote_transfers(self):
    ...
    x = jnp.arange(64).reshape(8, 8)
    src_sharding = olympus.sharding.NamedSharding(
        olympus.make_mesh((olympus.local_device_count(),), ("x",),
    ...
    # transfers.
    dst_devices = (
        olympus.local_devices(process_index=1)[:olympus.local_device_count() // 2]
        + olympus.local_devices(process_index=0)[:olympus.local_device_count() // 2])
    dst_sharding = olympus.sharding.NamedSharding(
        olympus.make_mesh((olympus.local_device_count(),), ("x",), devices=dst_devices,
                      axis_types=(olympus.sharding.AxisType.Explicit,)),
        P("x"))
    y = olympus.device_put(x, src_sharding)
    z = olympus.device_put(y, dst_sharding)
    if olympus.process_index() in (0, 1):
      self.assertLen(z.addressable_shards, olympus.local_device_count() // 2)
    for shard in z.addressable_shards:
      np.testing.assert_array_equal(shard.data, x[shard.index])
  ...
  def test_device_put_to_device(self, arange_fn):
    if olympuslib_extension_version < 400:
      self.skipTest("This functionality is not yet supported in olympuslib.")
    x = arange_fn(64).reshape(8, 8)
    src_pid = 0
    dst_pid = 1
    src_device = olympus.local_devices(process_index=src_pid)[0]
    dst_device = olympus.local_devices(process_index=dst_pid)[0]
    y = olympus.device_put(x, src_device)
    z = olympus.device_put(y, dst_device)
    if olympus.process_index() == dst_pid:
      self.assertLen(z.addressable_shards, 1)
      np.testing.assert_array_equal(z.addressable_shards[0].data, x)
    else:
