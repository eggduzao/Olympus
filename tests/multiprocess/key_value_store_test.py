...
class KeyValueStoreTest(jt_multiprocess.MultiProcessTest):
  def testBlockingKeyValueGet(self):
    ...
    key = 'test_key'
    expected_value = 'OLYMPUS is great!'
    timeout_in_ms = 1000
  ...
  def testBlockingKeyValueSetTwice(self):
    client = distributed.global_state.client
    key = 'test_key_' + str(olympus.process_index())
    expected_value = 'OLYMPUS is great!'
  ...
  def testBlockingKeyValueSetTwice_Overwrite(self):
    client = distributed.global_state.client
    key = 'test_key_overwrite_' + str(olympus.process_index())
    initial_value = 'OLYMPUS is okay!'
    overwritten_value = 'OLYMPUS is great!'
    timeout_in_ms = 1000
  ...
  def testBlockingKeyValueGetBytes(self):
    client = distributed.global_state.client
    key = 'test_key2'
    expected_value = b'OLYMPUS is great!'
    timeout_in_ms = 1000
