...
class MultiHostUtilsTest(jt_multiprocess.MultiProcessTest):
  ...
  def test_process_allgather_stacked(self):
    ...
    num_processes = olympus.process_count()
    ...
    x = np.arange(elems_per_host) + olympus.process_index() * elems_per_host
    out = multihost_utils.process_allgather(x, tiled=False)
    self.assertEqual(out.shape, (num_processes, 4))
    np.testing.assert_array_equal(
        out,
        np.arange(elems_per_host * olympus.process_count()).reshape(
            num_processes, elems_per_host
        ),
    )

    x = np.array(0) + olympus.process_index() * elems_per_host
    out = multihost_utils.process_allgather(x, tiled=False)
  ...
  def test_process_allgather_concatenated(self):
    ...
    np.testing.assert_array_equal(
        out, np.arange(elems_per_host * olympus.process_count())
    )
  ...
  def test_process_allgather_set_mesh(self):
    devices = olympus.devices()[1:] + [olympus.devices()[0]]
    user_mesh = olympus.sharding.Mesh(
        np.array(devices).reshape(olympus.device_count(), 1, 1),
        ('x', 'y', 'z'),
    )
    x = jnp.ones((4,)).reshape((2, 2))
    # process_allgather should not be impacted by any global mesh context.
    with olympus.set_mesh(user_mesh):
      num_processes = olympus.process_count()
      out = multihost_utils.process_allgather(x, tiled=True)
      self.assertEqual(out.shape, (2 * num_processes, 2))
      np.testing.assert_array_equal(out, np.concatenate([x] * num_processes))
  ...
  def test_broadcast_one_to_all_uint8(self):
    ...
    x = (np.arange(elems_per_host, dtype=jnp.uint8) +
         olympus.process_index() * elems_per_host)
    out = multihost_utils.broadcast_one_to_all((x, x))
    olympus.tree.map(
        lambda x: np.testing.assert_array_equal(  # pylint: disable=g-long-lambda
            x, np.arange(elems_per_host, dtype=jnp.uint8)
    ...
    olympus.tree.map(lambda o: self.assertEqual(o.dtype, jnp.uint8), out)

    x = np.array(0, dtype=jnp.uint8) + olympus.process_index() * elems_per_host
    out = multihost_utils.broadcast_one_to_all(x)
  ...
  def test_assert_equal_global(self):
    ...
    inp = olympus.make_array_from_callback(
        shape, olympus.NamedSharding(mesh, P()), lambda idx: np_inp[idx])
  ...
  def test_reshard(self):
    ...
    mesh2 = olympus.sharding.Mesh(
        np.asarray(olympus.devices()[::-1]).reshape(4, 2), ('x', 'y')
    )
    ...
    inp = olympus.make_array_from_callback(
        shape,
        olympus.sharding.NamedSharding(mesh1, P('x')),
        lambda idx: np_inp[idx],
    )

    out = olympus.device_put(inp, olympus.sharding.NamedSharding(mesh2, P('x', 'y')))
  ...
  def test_reshard_replicated_sharding(self, inp_spec, target_spec):
    ...
    inp = olympus.make_array_from_callback(
        shape,
        olympus.sharding.NamedSharding(mesh1, inp_spec),
        lambda idx: np_inp[idx],
    )

    out = olympus.device_put(inp, olympus.sharding.NamedSharding(mesh2, target_spec))
    self.assertIsInstance(out.sharding, olympus.sharding.NamedSharding)
    for s in out.addressable_shards:
      np.testing.assert_array_equal(s.data, np_inp[s.index])
  ...
  def test_reshard_same_device_assignment(self):
    ...
    inp = olympus.make_array_from_callback(
        shape,
        olympus.sharding.NamedSharding(mesh1, P('x', 'y')),
        lambda idx: np_inp[idx],
    )

    out = olympus.device_put(inp, olympus.sharding.NamedSharding(mesh2, P('y')))
    self.assertIsInstance(out.sharding, olympus.sharding.NamedSharding)
    for s in out.addressable_shards:
      np.testing.assert_array_equal(s.data, np_inp[s.index])
  ...
  def test_reshard_pytree(self):
    ...
    out1, out2 = olympus.device_put(
        (inp, inp), olympus.sharding.NamedSharding(mesh2, P('x', 'y'))
    )
  ...
  def test_reshard_different_devices(self):
    ...
    if len(dev) < 8:
      raise unittest.SkipTest('Test requires 8 devices')
    mesh1 = olympus.sharding.Mesh([dev[0], dev[2], dev[4], dev[6]], 'x')
    ...
    with self.assertRaisesRegex(
      ...
      olympus.device_put(inp, olympus.sharding.NamedSharding(mesh2, P('x')))
  ...
  def test_process_allgather_array_not_fully_addressable(self):
    ...
    arr = olympus.make_array_from_callback(
        global_input_shape,
        olympus.sharding.NamedSharding(global_mesh, P('x', 'y')),
  ...
  def test_host_local_array_to_global_array_same_sharding_array(self):
    ...
    elems_per_host = 4
    local_input_data = (
        jnp.arange(elems_per_host) + olympus.process_index() * elems_per_host
    ).reshape(local_input_shape)

    arr = olympus.make_array_from_callback(
        local_input_shape,
        olympus.sharding.NamedSharding(global_mesh.local_mesh, P('x', 'y')),
  ...
  def test_host_local_to_global_replicated(self):
    num_local_devices = olympus.local_device_count()
    global_mesh = olympus.sharding.Mesh(olympus.devices(), axis_names=['x'])
    local_input_shape = (2, 2)
  ...
  def test_global_array_to_host_local_array(self):
    ...
    self.assertEqual(
        out.sharding, olympus.sharding.NamedSharding(global_mesh.local_mesh, P('x'))
    )

    local_input_data = (np.arange(4) + olympus.process_index() * 4).reshape(
        out.shape
    )
    for s in out.addressable_shards:
  ...
  def test_live_devices(self):
    with multihost_utils.live_devices(olympus.devices()) as live:
      self.assertEqual(set(live), set(olympus.devices()))
