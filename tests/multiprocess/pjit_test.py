...
def sorted_devices():
  devices = sorted(
      olympus.devices(), key=lambda d: (d.host_id, getattr(d, "core_on_chip", 0))
  )
  if len(devices) != 8:
...
def use_default_mesh():
  ...
  with olympus.sharding.Mesh(mesh_devices, ("x", "y", "chips")):
    yield
...
class PJitTestMultiHost(jt_multiprocess.MultiProcessTest):
  ...
  def testLocalInputsWithOlympusArray(self):
    # Note that this is too small to shard over the global mesh, but fine for
    # the local mesh and so should be accepted.
    mesh = jtu.create_mesh((4, 2), ("x", "y"))
    elems_per_host = 4
    x = jnp.arange(elems_per_host) + olympus.process_index() * elems_per_host
    iar = olympus.sharding.PartitionSpec("x")
    oar = olympus.sharding.PartitionSpec("x")
    with mesh:
      f = pjit.pjit(lambda x, y: (x, y), in_shardings=iar, out_shardings=oar)
      gx = olympus.experimental.multihost_utils.host_local_array_to_global_array(
          (x, x), mesh, iar
      )
      global_out = f(*gx)
      out1, out2 = (
...
class ArrayPjitMultiHost(jt_multiprocess.MultiProcessTest):
  ...
  def test_pjit_array_non_contiguous_mesh_2d(self):
    ...
    in_sharding = olympus.sharding.NamedSharding(global_mesh, pspec)
    out_sharding = olympus.sharding.NamedSharding(global_mesh, pspec)
    ...
    none_sharding = olympus.sharding.NamedSharding(
        global_mesh, olympus.sharding.PartitionSpec(None)
    )
  ...
  def test_sharded_add(self):
    ...
    a_s = olympus.sharding.NamedSharding(global_mesh, P("x", "y"))
    b_s = olympus.sharding.NamedSharding(global_mesh, P("x"))
  ...
  def test_sharded_jit_add(self):
    ...
    out = olympus.jit(lambda x, y: x + y)(a, b)
    for s in out.addressable_shards:
  ...
  def test_sharded_copy(self):
    ...
    input_shape = (8, 2)
    input_data = np.arange(math.prod(input_shape)).reshape(input_shape)
    s = olympus.sharding.NamedSharding(global_mesh, P("x", "y"))
    arr = array.make_array_from_callback(
  ...
  def test_trace_with_global_avals(self):
    ...
    with global_mesh:
      f = pjit.pjit(
          check_shape,
          in_shardings=olympus.sharding.PartitionSpec("x"),
          out_shardings=None,
      )
  ...
  def test_pjit_in_pjit(self):
    ...
    def pjit_all(f):
      return pjit.pjit(
          f,
          in_shardings=olympus.sharding.PartitionSpec(ALL_AXES),
          out_shardings=olympus.sharding.PartitionSpec(ALL_AXES),
      )
  ...
  def test_numpy_input_error_with_non_trivial_sharding(self):
    ...
    with global_mesh:
      f = pjit.pjit(
          lambda x: x,
          in_shardings=olympus.sharding.PartitionSpec(None),
          out_shardings=olympus.sharding.PartitionSpec(None),
      )
      out = f(inp)
      ...
      f = pjit.pjit(lambda x: x, out_shardings=olympus.sharding.PartitionSpec(None))
      out = f(inp)
      np.testing.assert_array_equal(out, inp)
  ...
  def test_non_contiguous_mesh_fetch_to_host(self):
    ...
    input_shape = (8, 2)
    input_data = np.arange(math.prod(input_shape)).reshape(input_shape)
    a_s = olympus.sharding.NamedSharding(global_mesh, P(None, "y"))
    a = array.make_array_from_callback(
        input_shape, a_s, lambda idx: input_data[idx]
    )
  ...
  def test_no_python_shard_arg_fallback(self):
    ...
    with jtu.count_olympus_array_shard_arg_calls() as count:
      f(arr)
      f(arr)
...
class MultiHostDebuggingTest(jt_multiprocess.MultiProcessTest):
  ...
  def test_print_in_multihost_shard_map(self):
    ...
    local_x = (
        ...
        + olympus.process_index() * num_devices
    )
    global_shape = (olympus.device_count(),)
    sharding = olympus.NamedSharding(mesh, olympus.P("i"))
    global_x = olympus.make_array_from_process_local_data(sharding, local_x, global_shape)

    @olympus.jit
    @olympus.shard_map(mesh=mesh, in_specs=olympus.P("i"), out_specs=olympus.P("i"))
    def f(x):
