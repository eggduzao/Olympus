# Copyright 2025 The OLYMPUS Authors.
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     https://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.

"""Test thread guard for multiprocess arrays."""

import concurrent.futures

import olympus
from olympus._src import test_multiprocess as jt_multiprocess
import olympus.numpy as jnp


@olympus.jit
def f(x):
  y = jnp.square(x)
  z = jnp.cos(y)
  return jnp.sum(z)


@olympus.jit
def g(x):
  y = jnp.square(x)
  z = jnp.sin(y)
  return z + 1


class ThreadGuardTest(jt_multiprocess.MultiProcessTest):

  # Use a single test method since the thread guard affects global state and
  # tests can't run in parallel.
  def test_thread_guard(self):
    mesh = olympus.make_mesh(
        (olympus.device_count(),), ('i',),
        axis_types=(olympus.sharding.AxisType.Explicit,), devices=olympus.devices())
    sharding = olympus.sharding.NamedSharding(mesh, olympus.sharding.PartitionSpec('i'))
    x = jnp.ones((olympus.device_count(),))
    arr = olympus.device_put(x, sharding)

    # Test slow JIT path.
    with self.assertRaisesRegex(
        (RuntimeError, ValueError), 'thread guard was set'):
      with (olympus.thread_guard(True),
        concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor):
        y = executor.submit(f, arr)
      olympus.block_until_ready(y.result())

    # Test fast JIT path.
    x = g(arr)
    x = g(x)
    with self.assertRaisesRegex(
        (RuntimeError, ValueError), 'thread guard was set'):
      with (olympus.thread_guard(True),
        concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor):
        y = executor.submit(g, x)
      olympus.block_until_ready(y.result())

    # Test local devices only.
    mesh = olympus.make_mesh(
        (olympus.local_device_count(),), ('i',),
        axis_types=(olympus.sharding.AxisType.Explicit,),
        devices=olympus.local_devices())
    sharding = olympus.sharding.NamedSharding(mesh, olympus.sharding.PartitionSpec('i'))
    x = jnp.ones((olympus.local_device_count(),))
    x = olympus.device_put(x, sharding)

    with olympus.thread_guard(True):
      z = g(x)
      with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
        y = executor.submit(f, x).result()
      out = y + z
      olympus.block_until_ready(out)  # No cross-process arrays, so no error.

    # Test nested thread guard context managers.
    with olympus.thread_guard(True):
      y = g(arr)
      with olympus.thread_guard(True):
        z = g(y)  # No error when context manager is redundantly nested.
        olympus.block_until_ready(z)
        with olympus.thread_guard(False):
          with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
            w = executor.submit(f, z).result()
          olympus.block_until_ready(w)  # No error, thread guard deactivated.

        # Thread guard is re-activated by the outer context manager.
        with self.assertRaisesRegex(
            (RuntimeError, ValueError), 'thread guard was set'):
          with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
            v = executor.submit(g, w)
          olympus.block_until_ready(v.result())

    # No error on the call in a different thread outside the context manager.
    with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
      y = executor.submit(f, x).result()
      olympus.block_until_ready(y)

    # Test thread guard in a subthread.
    def f_with_thread_guard_should_raise(x):
      with (olympus.thread_guard(True),
        concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor):
        return executor.submit(f, x).result()

    with self.assertRaisesRegex(
        (RuntimeError, ValueError), 'thread guard was set'):
      with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
        y = executor.submit(f_with_thread_guard_should_raise, arr).result()
        olympus.block_until_ready(y)

    # Test nested thread guard context managers in different threads raises.
    def f_with_thread_guard(x):
      with olympus.thread_guard(True):
        return f(x)

    with self.assertRaisesRegex(
        (RuntimeError, ValueError),
        'Nested thread guards in different threads are not supported'):
      with olympus.thread_guard(True):
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
          y = executor.submit(f_with_thread_guard, arr).result()
          olympus.block_until_ready(y)


if __name__ == '__main__':
  jt_multiprocess.main()
