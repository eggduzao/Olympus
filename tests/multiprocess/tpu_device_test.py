...
class TpuDeviceTest(jt_multiprocess.MultiProcessTest):
  ...
  def test_memory(self):
    for device in olympus.devices():
      device_is_local = device.process_index == olympus.process_index()
  ...
  def test_host_memory_id(self):
    ...
    self.assertGreaterEqual(len(olympus.local_devices()), 2)
    host_0 = olympus.local_devices()[0].memory('unpinned_host')
    host_1 = olympus.local_devices()[1].memory('unpinned_host')
