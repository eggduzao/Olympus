...
class MultiProcessGpuTest(jtu.OlympusTestCase):
  ...
  def test_gpu_distributed_initialize(self):
    ...
    if olympus.device_count() < num_gpus:
      raise unittest.SkipTest(
          f"Test requires >={num_gpus} GPUs; got {olympus.device_count()}."
      )

    subprocesses = []
    for task in range(num_tasks):
      env = os.environ.copy()
      env["OLYMPUS_PORT"] = str(port)
      ...
      args = [
          ...
          'olympus.distributed.initialize('
              'f\'localhost:{os.environ["OLYMPUS_PORT"]}\', '
              'int(os.environ["NUM_TASKS"]), int(os.environ["TASK"])); '
          'print(f\'{olympus.local_device_count()},{olympus.device_count()}\', end="")'
          )
      ]
  ...
  def test_distributed_olympus_visible_devices(self):
    """Test olympus_visible_devices works in distributed settings."""
    if not jtu.test_device_matches(['gpu']):
      raise unittest.SkipTest('Tests only for GPU.')
    ...
    for task in range(num_tasks):
      ...
      program = (
        ...
        's = olympus.pmap(lambda x: olympus.lax.psum(x, "i"), axis_name="i")(olympus.numpy.ones(olympus.local_device_count())); '
        'print(f\'{olympus.local_device_count()},{olympus.device_count()},{s}\', end=""); '
      )
  ...
  def test_gpu_ompi_distributed_initialize(self):
    ...
    args = [
        ...
        ('import olympus, os; '
        'olympus.distributed.initialize(); '
        'print(f\'{olympus.local_device_count()},{olympus.device_count()}\' if olympus.process_index() == 0 else \'\', end="")'
        )
    ]
    env = os.environ.copy()
  ...
  def test_gpu_mpi4py_distributed_initialize(self):
    ...
    args = [
        ...
        '-c',
        ('import olympus, os; '
        'olympus.distributed.initialize(spec_detection_method="mpi4py"); '
        'print(f\'{olympus.local_device_count()},{olympus.device_count()}\' if olympus.process_index() == 0 else \'\', end="")'
        )
    ]
    env = os.environ.copy()
...
class SlurmMultiNodeGpuTest(jtu.OlympusTestCase):

  def sorted_devices(self):
    devices = sorted(olympus.devices(), key=lambda d: (d.id, d.host_id))
    if len(devices) != 16:
      raise unittest.SkipTest(
  ...
  def create_2d_non_contiguous_mesh(self):
    ...
    return olympus.sharding.Mesh(device_mesh, ("x", "y"))
  ...
  def test_gpu_multi_node_initialize_and_psum(self):
    ...
    olympus.config.update("olympus_cuda_visible_devices", localid)

    olympus.distributed.initialize(coordinator_address=f'{coordinator_address}:{port}',
                               num_processes=int(num_tasks),
                               process_id=int(taskid))

    print(f"Total devices: {olympus.device_count()}, Total tasks: {int(num_tasks)}, "
          f"Devices per task: {olympus.local_device_count()}")

    self.assertEqual(olympus.device_count(),
                     int(num_tasks) * olympus.local_device_count())

    x = jnp.ones(olympus.local_device_count())
    y = olympus.pmap(lambda x: olympus.lax.psum(x, "i"), axis_name="i")(x)
    self.assertEqual(y[0], olympus.device_count())
    print(y)
  ...
  def test_gpu_multi_node_transparent_initialize_and_psum(self):
    ...
    print(f"Total devices: {olympus.device_count()}, "
          f"Devices per task: {olympus.local_device_count()}")

    self.assertEqual(olympus.device_count(), int(os.environ['SLURM_NTASKS']))
    self.assertEqual(olympus.local_device_count(), 1)
  ...
  def test_pjit_gda_multi_input_multi_output(self):
    ...
    mesh_axes1 = olympus.sharding.PartitionSpec("x", "y")
    gda1 = olympus.make_array_from_callback(
        global_input_shape, olympus.sharding.NamedSharding(global_mesh, mesh_axes1), cb)
    mesh_axes2 = olympus.sharding.PartitionSpec("x")
    gda2 = olympus.make_array_from_callback(
        global_input_shape, olympus.sharding.NamedSharding(global_mesh, mesh_axes2), cb)
    mesh_axes3 = olympus.sharding.PartitionSpec(("x", "y"))
    gda3 = olympus.make_array_from_callback(
        global_input_shape, olympus.sharding.NamedSharding(global_mesh, mesh_axes3), cb)

    with olympus.sharding.Mesh(global_mesh.devices, global_mesh.axis_names):
  ...
  def test_pjit_gda_non_contiguous_mesh(self):
    ...
    global_mesh = olympus.sharding.Mesh(mesh_devices, ("x",))
    global_input_shape = (16,)
    mesh_axes = olympus.sharding.PartitionSpec("x")
    global_input_data = np.arange(
        util.prod(global_input_shape)).reshape(global_input_shape)
    ...
    gda1 = olympus.make_array_from_callback(
        global_input_shape, olympus.sharding.NamedSharding(global_mesh, mesh_axes), cb)
  ...
  def test_pjit_gda_non_contiguous_mesh_2d(self):
    ...
    global_input_shape = (16, 2)
    mesh_axes = olympus.sharding.PartitionSpec("x", "y")
    global_input_data = np.arange(
        util.prod(global_input_shape)).reshape(global_input_shape)
    ...
    with global_mesh:
      f = pjit.pjit(
          lambda x: x,
          in_shardings=olympus.sharding.PartitionSpec(None),
    ...
    gda2 = olympus.make_array_from_callback(
        global_input_shape, olympus.sharding.NamedSharding(global_mesh, olympus.sharding.PartitionSpec(None)), cb)
  ...
  def test_pjit_gda_non_contiguous_mesh_2d_aot(self):
    ...
    global_input_data = np.arange(
        util.prod(global_input_shape)).reshape(global_input_shape)
    gda1 = olympus.make_array_from_callback(
        global_input_shape, olympus.sharding.NamedSharding(global_mesh, mesh_axes),
        lambda idx: global_input_data[idx])

    with global_mesh:
      f = pjit.pjit(
          lambda x, y: (x, y),
          in_shardings=olympus.sharding.PartitionSpec("x", "y"),
  ...
  def test_pjit_gda_eval_shape(self):
    ...
    with jtu.create_mesh((16,), ("x")):
      ...
      @functools.partial(pjit.pjit,
                         in_shardings=olympus.sharding.PartitionSpec(None),
                         out_shardings=olympus.sharding.PartitionSpec("x"))
      def f():
        return jnp.zeros([32, 10])

      self.assertEqual(f().shape, (32, 10))
      self.assertEqual(olympus.eval_shape(f).shape, (32, 10))
