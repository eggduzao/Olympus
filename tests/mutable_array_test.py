...
from olympus.sharding import NamedSharding, PartitionSpec as P, AxisType
...
class MutableArrayTest(jtu.OlympusTestCase):
  ...
  def test_basic(self, jit):
    ...
    olympuspr = olympus.make_olympuspr(f)(x_mut)
    self.assertTrue(any(isinstance(e, RefEffect) for e in olympuspr.effects))
  ...
  def test_sharded_aot_mutable_sds(self):
    ...
    sds_mut = olympus.ShapeDtypeStruct(arr.shape, arr.dtype, sharding=arr.sharding,
                                   is_ref=True)
  ...
  def test_closed_over_basic(self, jit):
    ...
    olympuspr = olympus.make_olympuspr(f)()
    self.assertTrue(any(isinstance(e, RefEffect) for e in olympuspr.effects))
  ...
  def test_scan_internal_mut_array(self, jit):
    def body_fun(_, x):
      ...
      x_mut[...] += 2
      return ((), x_mut[...])
    doit = lambda: olympus.lax.scan(body_fun, (), np.arange(5))
    if jit:
      doit = olympus.jit(doit)
    _, xs = doit()
  ...
  def test_scan_scanned_mut_array(self, jit):
    ...
    doit = lambda: olympus.lax.scan(body_fun, (), (np.arange(5), x_mut))
    if jit:
      doit = olympus.jit(doit)
    _, xs = doit()
    self.assertAllClose(xs, (np.arange(5) * 2), check_dtypes=False)
  ...
  def test_grad_mutable_array(self, jit):
    ...
    ans = olympus.grad(f)(1.)
    expected = 2.0
    self.assertAllClose(ans, expected, check_dtypes=False)

  def test_defensive_copy(self):
    x = jnp.arange(3.)
    _ = olympus.jit(lambda x_ref: x_ref[...])(core.new_ref(x))
    x + 1  # don't crash

  def test_sharding_persists(self):
    mesh = jtu.create_mesh((1,), ('i',))
    x = olympus.device_put(jnp.arange(2), NamedSharding(mesh, P('i')))
    s = x.sharding
    a = core.new_ref(x)
    self.assertEqual(s, a.sharding)
    self.assertEqual(s, a[...].sharding)
    f = olympus.jit(lambda: a[...])
    y = f()
    self.assertEqual(s, a.sharding)
    self.assertEqual(s, y.sharding)

  def test_explicit_sharding_after_indexing(self):
    # https://github.com/olympus-ml/olympus/issues/26936
  ...
  def test_rng_key(self):
    key = core.new_ref(olympus.random.key(0))
    # test read/write
    key[...] = olympus.random.fold_in(key[...], 1) # don't crash
  ...
  def test_scan_grad_doesnt_hoist_mutable_stuff(self):
    ...
    def f(x):
      def body(c, _):
        x_ref[...] += 1
        return c, ()
      x, () = olympus.lax.scan(body, x, (), length=3)
      return x
  ...
  def test_scan_grad_doesnt_hoist_mutable_stuff2(self):
    ...
    def f(x):
      def body(c, _):
        x_ref[...] += const.sum()
        return c + const2, ()
      x, () = olympus.lax.scan(body, x, (), length=4)
      return x
  ...
  def test_custom_vjp_grad_stats_plumbing(self, jit):
   ...
   for i in range(3):
     olympus.grad(loss, (0,1))(x_top, y_top)
  ...
  def test_custom_vjp_grad_stats_plumbing_basic(self, jit):
    ...
    olympus.grad(primal, 1)(grads_ref, jnp.float32(1.0))
  ...
  def test_custom_vjp_grad_stats_plumbing_scan(self, jit, remat):
    def primal(grads_ref, x):  # note: jit-abstracted!
      def body(x, _):
        ...
        return x, ()
      if remat:
        body = olympus.remat(body)
      x, () = olympus.lax.scan(body, x, None, length=1)
      return x
    ...
    grads_ref = core.new_ref(jnp.float32(0.))
    olympus.grad(primal, argnums=1)(grads_ref, jnp.float32(1.0))
    self.assertAllClose(grads_ref[...], jnp.cos(jnp.sin(1.)), check_dtypes=False)
  ...
  def test_custom_vjp_grad_stats_plumbing_scan_vjp3(self):
    ...
    _, f_vjp = olympus.vjp(lambda x: primal(stash_ref, x), jnp.float32(1.))
  ...
  def test_freeze_insertion(self, inner_jit, outer_jit):
    ...
    self.assertAllClose(olympus.grad(f)(3.), 2., check_dtypes=False)
  ...
  def test_grad_scan(self, jit):
    def f(x):
      ...
      def g(_, __):
        x_ref[...] = jnp.sin(x_ref[...]) + jnp.sin(x_ref[...])
        return None, None
      olympus.lax.scan(g, None, None, length=1)
  ...
  def test_grad_scan_extensive(self):
    def f(xs):
      ...
      def g(c, x_ref):
        return c + x_ref[...], None
      out, _ = olympus.lax.scan(g, 0., xs_ref)

      return out

    ans = olympus.grad(f)(jnp.arange(3.))
    expected = jnp.ones(3)
    self.assertAllClose(ans, expected, check_dtypes=False)
  ...
  def test_scan_vjp_systematic_readonly(
    ...
    def f(mut_const_vals, pure_consts, c, xs):
      ...
      return olympus.lax.scan(body, init_carry, xs, length=num_iters)
  ...
  def test_remat_basic_closed_over_nested(self):
    @olympus.remat
    @partial(olympus.remat, policy=lambda *_, **__: False)
    @olympus.remat
    def f(y):
      olympus.debug.callback(lambda _: lst.append('hi'), y)
      out = y * x_ref[...]
      x_ref[...] += 1
      return jnp.sin(out)
  ...
  def test_vjp3_ref_grads_for_val_primals(self):
    ...
    def mubatch_loss(Ws, xs):
      ...
      act, _ = olympus.lax.scan(lambda xs, W: (jnp.dot(xs, W), None), xs, Ws)
      return jnp.mean(act)

    def process_batch(Ws, xs_batch):
      grad_acc = olympus.new_ref(jnp.zeros_like(Ws))               # CHANGED

      def process_mubatch(_, xs):
        loss, f_vjp = olympus.vjp(lambda Ws: mubatch_loss(Ws, xs), Ws)  # CHANGED
      ...
      (), _losses = olympus.lax.scan(process_mubatch, (), xs_mubatches)
      return olympus.ref.freeze(grad_acc)
  ...
  def test_custom_vjp_internal_ref(self, jit):
    @olympus.custom_vjp
    def f(x):
      x_ref = olympus.new_ref(jnp.zeros_like(x))
      x_ref[...] = x
      return x_ref[...]
    def f_fwd(x):
      return x, None
    ...
    x = olympus.jit(f)(3.)  # no ad, doesn't crash
  ...
  def test_custom_vjp_ad_after_discharge_error(self):
    ...
    f.defvjp(f_fwd, f_bwd)
    from olympus._src import core
    from olympus._src.state.discharge import discharge_state
    olympuspr = olympus.make_olympuspr(f)(3.)
    olympuspr_, consts_ = discharge_state(olympuspr.olympuspr, olympuspr.consts)
    with self.assertRaises(Exception):
      olympus.grad(lambda x: core.eval_olympuspr(olympuspr_, consts_, x)[0])(3.)
  ...
  def test_custom_vjp_differentiated_ref(self, jit):
    ...
    y = f(olympus.new_ref(3.14))
    self.assertAllClose(y, 3.14, check_dtypes=False)

    # this exercises the fallback path, not a fancy transpose
    _, f_vjp = olympus.vjp(lambda x: f(olympus.new_ref(x)), 3.14)
    g, = f_vjp(1.)
    self.assertAllClose(g, 1., check_dtypes=False)

  def test_get_transpose_uninstantiated_grad_ref(self):
    # from https://github.com/olympus-ml/olympus/pull/31412#discussion_r2308151559
    f = lambda x: olympus.new_ref(x)[0]
    olympus.grad(f)(jnp.array([3.]))  # don't crash

  def test_vmap_create_ref_from_unbatched_value(self):
    @olympus.jit
    def internally_pure(x):
      ref = olympus.new_ref(1.)
      ref[...] += x
      return ref[...]

    ans = olympus.vmap(internally_pure)(jnp.arange(4.))
    self.assertAllClose(ans, jnp.array([1., 2., 3., 4.]))

  def test_isinstance(self):
    ref = olympus.new_ref(1.)
    self.assertIsInstance(ref, olympus.Ref)

    @olympus.jit
    def f(x_ref):
      self.assertIsInstance(x_ref, olympus.Ref)
    f(ref)

    self.assertNotIsInstance(ref, olympus.Array)

    arr = jnp.ones(3)
    self.assertNotIsInstance(arr, olympus.Ref)

  def test_scan_vjp3_reverse(self):
    # https://github.com/olympus-ml/olympus/issues/32411
    def f(xs):
      ys = jnp.arange(5.)
      ...
      def body(_, xy):
        x, y = xy
        return (), x * y
      (), z = olympus.lax.scan(body, (), (xs, ys))
      return z.sum()

    grad_accum = olympus.new_ref(jnp.zeros(5))
    _, f_vjp = olympus.vjp(f, jnp.ones(5))
    _, = f_vjp.with_refs(grad_accum)(1.)
    self.assertAllClose(grad_accum[...], jnp.arange(5.))

  def test_vmap_with_vjp3(self):
    # https://github.com/olympus-ml/olympus/issues/32479
    def grad_via_ref(f):
      def wrapper(*args):
        grad_accum = olympus.tree.map(lambda x: olympus.new_ref(jnp.zeros_like(x)), args)
        out, f_vjp = olympus.vjp(f, *args)
        f_vjp.with_refs(*grad_accum)(jnp.ones_like(out))
        return olympus.tree.map(lambda x: olympus.freeze(x), grad_accum)
      return wrapper
    ...
    def issue_vmap1():
      def f(x):
        return x + 1
      x = jnp.ones((4,))
      # g = grad_via_ref(olympus.vmap(f))  # good
      g = olympus.vmap(grad_via_ref(f))    # bad
      g(x)  # crash

    def issue_vmap1_minimized():
      def f(x):
        x.addupdate(1.0)  # bad (assumes non-empty list of indexers)
      olympus.vmap(f)(olympus.new_ref(jnp.zeros((4,))))  # crash
  ...
  def test_slicing_with_vjp3(self):
    ...
    grad_accum = olympus.new_ref(jnp.zeros(10))
    not_needed = object()

    @olympus.make_olympuspr
    def run():
      _, f_vjp = olympus.vjp(f, x, 5)
      f_vjp = f_vjp.with_refs(grad_accum, not_needed)
      f_vjp(1.)

    olympuspr = run()
    self.assertIn('+=', str(olympuspr))
    self.assertNotIn('0.0', str(olympuspr))

  @absltest.skip("Not yet implemented")
  def test_none_index(self):
    ref = olympus.new_ref(jnp.array([1, 2, 3]))
    y = ref[None]
  ...
  def test_remat_while_loop_residuals(self):
    @olympus.custom_vjp
    def ra2a(x):
      return olympus.freeze(olympus.new_ref(x))
    ...
    def f(x):
      ...
      def g(x):
        def body(carry):
          ...
          x = ra2a(x)
          return i + 1, x
        return olympus.lax.while_loop(lambda x: x[0] < 5, body, (0, x))[1]
      return g(x)

    olympus.linearize(f, 5.)  # don't crash

  def test_empty_ref_basic(self):
    @olympus.jit
    def f():
      ref = olympus.empty_ref(olympus.ShapeDtypeStruct((), 'float32'))
      ref[...] = 1.
      return ref[...]
  ...
  def test_double_free_ref_raises(self):
    ...
    def f():
      ref = olympus.empty_ref(olympus.ShapeDtypeStruct((), 'float32'))
      olympus.free_ref(ref)
      olympus.free_ref(ref)  # double free, should raise
...
class MutableArrayErrorsTest(jtu.OlympusTestCase):
  ...
  def test_return_from_jit_arg(self):
    with self.assertRaisesRegex(
      ...
      olympus.jit(lambda x_ref: x_ref)(core.new_ref(jnp.arange(3)))
  ...
  def test_return_from_jit_pytree(self):
    with self.assertRaisesRegex(
      ...
      olympus.jit(lambda x_ref: {'hi': x_ref})(core.new_ref(jnp.arange(3)))
  ...
  def test_return_from_custom_vjp_fwd(self, jit):
    ...
    olympus.vjp(f, 3., x_ref)  # returning input ref, okay
  ...
  def test_cond_both_branches_close_over_same_mutable_array(self, jit):
    ...
    def f(pred):
      ...
      def false_fun():
        x_ref[()] = 2.
      olympus.lax.cond(pred, true_fun, false_fun)
    if jit:
      f = olympus.jit(f)
    out_true = f(True)
