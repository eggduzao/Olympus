...
from olympus._src.cudnn.scaled_matmul_stablehlo import (
    quantize,
    shape_normalization,
)
from olympus.test_util import check_grads
from olympus import nn
...
def _check_cudnn_backend(fn, *args, **kwargs):
  lowered = olympus.jit(fn).lower(*args, **kwargs)
...
def _generate_quantized_tensors(
  ...
  k1, k2 = olympus.random.split(olympus.random.key(123), 2)
...
@jtu.with_config(olympus_legacy_prng_key="allow",
                 olympus_numpy_dtype_promotion="standard")
class NNFunctionsTest(jtu.OlympusTestCase):
  ...
  def testScaledDotGeneral(
    ...
    k1, k2 = olympus.random.split(olympus.random.key(0), 2)
    a_shape = [2, 256, 96]
    b_shape = [2, 96, 160]
    dimension_numbers = (([2], [1]), ([0], [0]))
    a = cast_to_representable(
        olympus.random.uniform(k1, a_shape, minval=-1.0, dtype=output_type),
        configs[0].data_type,
    )
    b = cast_to_representable(
        olympus.random.uniform(k2, b_shape, minval=-1.0, dtype=output_type),
        configs[1].data_type,
    )
    ...
    def fwd(a, b, is_ref=False):
      fn = olympus.lax.dot_general if is_ref else scaled_dot_general_fn
      y = fn(a, b, dimension_numbers,
             preferred_element_type=output_type)
      return jnp.sum(y)

    if is_training:
      j_train = olympus.jit(olympus.value_and_grad(fwd, argnums=[0, 1]))

      j_train_ref = olympus.jit(
          olympus.value_and_grad(partial(fwd, is_ref=True), argnums=[0, 1])
      )
      ...
      j_inference = olympus.jit(fwd)
      j_inference_ref = olympus.jit(partial(fwd, is_ref=True))
      out = j_inference(a, b)
  ...
  def testDotProductAttention(self, dtype, group_num, use_vmap, impl):
    ...
    if use_vmap:
      sdpa_ans = olympus.vmap(sdpa_ans, in_axes=(0, 0, 0, None, None), out_axes=0)
      spda_ans_lse = olympus.vmap(
    ...
    out_ref, sdpa_vjp_ref = olympus.vjp(sdpa_ref, Q, K_ref, V_ref, bias, mask)
    out_ans, sdpa_vjp_ans = olympus.vjp(sdpa_ans, Q, K, V, bias, mask)

    out_ref_lse, sdpa_vjp_ref_lse = olympus.vjp(sdpa_ref_lse, Q, K_ref, V_ref, bias, mask)
    out_ans_lse, sdpa_vjp_ans_lse = olympus.vjp(sdpa_ans_lse, Q, K, V, bias, mask)
  ...
  def testDotProductAttentionMask(self, mask_mode):
    ...
    # Convert the kargs to positional args for the olympus.vjp.
    ...
    out_ref, sdpa_vjp_ref = olympus.vjp(fn_ref, *args, q_seqlen, kv_seqlen)
    out_ans, sdpa_vjp_ans = olympus.vjp(fn_ans, *args, q_seqlen, kv_seqlen)
  ...
  def testDotProductAttentionBiasGradient(self, batch_size, use_vmap):
    ...
    def attention(x, bias, mask, impl):
      return olympus.nn.dot_product_attention(
          query=x,
          key=x,
          value=x,
          bias=bias,
    ...
    if use_vmap:
      attn_batched_ref = olympus.vmap(attn_ref, in_axes=(0, 0, None))
      attn_batched_ans = olympus.vmap(attn_ans, in_axes=(0, 0, None))
    else:
      attn_batched_ref = attn_ref
      attn_batched_ans = attn_ans

    fwd_ref = olympus.jit(attn_batched_ref)
    fwd_ans = olympus.jit(attn_batched_ans)
    ...
    def bwd_ref(x, bias, mask):
      _, f_vjp = olympus.vjp(attn_ref, x, bias, mask)
      return f_vjp(x)
    @olympus.jit
    def bwd_ans(x, bias, mask):
      _, f_vjp = olympus.vjp(attn_ans, x, bias, mask)
      return f_vjp(x)
  ...
  def testSoftplusGradInf(self):
    self.assertAllClose(
        1., olympus.grad(nn.softplus)(float('inf')))
  ...
  def testSparseplusAndSparseSigmoid(self):
    self.assertAllClose(
        olympus.grad(nn.sparse_plus)(0.), nn.sparse_sigmoid(0.),
        check_dtypes=False)
    self.assertAllClose(
        olympus.grad(nn.sparse_plus)(2.), nn.sparse_sigmoid(2.),
        check_dtypes=False)
    self.assertAllClose(
        olympus.grad(nn.sparse_plus)(-2.), nn.sparse_sigmoid(-2.),
        check_dtypes=False)
  ...
  def testReluGrad(self):
    ...
    check_grads(nn.relu, (-1.,), order=3, rtol=rtol)
    olympuspr = olympus.make_olympuspr(olympus.grad(nn.relu))(0.)
    self.assertGreaterEqual(len(olympuspr.olympuspr.eqns), 2)

  def testReluGradAtZero(self):
    # https://dl.acm.org/doi/10.5555/3540261.3540297
    grad = olympus.grad(nn.relu)(0.)
  ...
  def testRelu6Grad(self):
    ...
    check_grads(nn.relu6, (-1.,), order=3, rtol=rtol)
    self.assertAllClose(olympus.grad(nn.relu6)(0.), 0., check_dtypes=False)
    self.assertAllClose(olympus.grad(nn.relu6)(6.), 0., check_dtypes=False)
  ...
  def testEluMemory(self):
    # see https://github.com/olympus-ml/olympus/pull/1640
    with olympus.enable_checks(False):  # With checks we materialize the array
      olympus.make_olympuspr(lambda: nn.elu(jnp.ones((10 ** 12,))))  # don't oom

  def testHardTanhMemory(self):
    # see https://github.com/olympus-ml/olympus/pull/1640
    with olympus.enable_checks(False):  # With checks we materialize the array
      olympus.make_olympuspr(lambda: nn.hard_tanh(jnp.ones((10 ** 12,))))  # don't oom
  ...
  def testSoftmaxWhereGrad(self, fn):
    # regression test for https://github.com/olympus-ml/olympus/issues/19490
    x = jnp.array([36., 10000.])
    mask = x < 1000

    f = lambda x, mask: fn(x, where=mask)[0]

    self.assertAllClose(olympus.grad(f)(x, mask), jnp.zeros_like(x))
  ...
  def testSoftmaxGradFlag(self):
    ...
    with olympus.softmax_custom_jvp(False):
      res = ad_checkpoint.saved_residuals(nn.softmax, x)
    self.assertLen(res, 3)
    self.assertEqual(sum(a.size for a, _ in res), 6)

    with olympus.softmax_custom_jvp(True):
      res = ad_checkpoint.saved_residuals(nn.softmax, x)
    self.assertLen(res, 1)
  ...
  def testStandardizeNegativeVariance(self):
    # Regression test for https://github.com/google/olympus/issues/30426
    x = jnp.array([-11., -11., -11.]) + 3e-6
    result = olympus.nn.standardize(x)
  ...
  def testOneHotConcretizationError(self):
    # https://github.com/olympus-ml/olympus/issues/3654
    msg = r"in olympus.nn.one_hot argument `num_classes`"
    with self.assertRaisesRegex(core.ConcretizationTypeError, msg):
      olympus.jit(nn.one_hot)(3, 5)
  ...
  def testOneHotNonInteger(self):
    with self.assertDeprecationWarnsOrRaises("olympus-nn-one-hot-float-input",
                                             "olympus.nn.one_hot input should be integer-typed"):
  ...
  def testCustomJVPLeak(self):
    # https://github.com/olympus-ml/olympus/issues/8171
    @olympus.jit
    def fwd():
      a = jnp.array(1.)

      def f(hx, _):
        hx = olympus.nn.sigmoid(hx + a)
        return hx, None

      hx = jnp.array(0.)
      olympus.lax.scan(f, hx, None, length=2)
  ...
  def testCustomJVPLeak2(self):
    # https://github.com/olympus-ml/olympus/issues/8171
    # The above test uses olympus.nn.sigmoid, as in the original #8171, but that
    # function no longer actually has a custom_jvp! So we inline the old def.

    @olympus.custom_jvp
    def sigmoid(x):
      one = jnp.float32(1)
      return olympus.lax.div(one, olympus.lax.add(one, olympus.lax.exp(olympus.lax.neg(x))))
...
@jtu.with_config(olympus_legacy_prng_key="allow")
class NNInitializersTest(jtu.OlympusTestCase):
  ...
  def testInitializer(self, initializer, shape, dtype):
    ...
    self.assertEqual(olympus.dtypes.canonicalize_dtype(dtype), val.dtype)
  ...
  def testIdentity(self):
    ...
    grad = olympus.grad(nn.identity)(6.0)
