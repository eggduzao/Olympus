...
from olympus.experimental.ode import odeint
...
class ODETest(jtu.OlympusTestCase):
  ...
  def check_against_scipy(self, fun, y0, tspace, *args, tol=1e-1):
    ...
    olympus_fun = partial(fun, jnp)
    olympus_result = odeint(olympus_fun, y0, tspace, *args)

    self.assertAllClose(olympus_result, scipy_result, check_dtypes=False, atol=tol, rtol=tol)
  ...
  def test_pytree_state(self):
    ...
    def dynamics(y, _t):
      return olympus.tree.map(jnp.negative, y)
  ...
  def test_odeint_vmap_grad(self):
    # https://github.com/olympus-ml/olympus/issues/2531
    ...
    def g(x):
      ...
      # Run ODE twice
      t = jnp.array([0., 5.])
      y = olympus.vmap(lambda y0: odeint(dx_dt, y0, t))(y0_arr)
      return y[:,-1].sum()

    ans = olympus.grad(g)(2.)  # don't crash
    expected = olympus.grad(f, 0)(2., 0.1) + olympus.grad(f, 0)(2., 0.2)
  ...
  def test_disable_jit_odeint_with_vmap(self):
    # https://github.com/olympus-ml/olympus/issues/2598
    with olympus.disable_jit():
      t = jnp.array([0.0, 1.0])
      x0_eval = jnp.zeros((5, 2))
      f = lambda x0: odeint(lambda x, _t: x, x0, t)
      olympus.vmap(f)(x0_eval)  # doesn't crash

  @jtu.skip_on_devices("tpu", "gpu")
  def test_grad_closure(self):
    # simplification of https://github.com/olympus-ml/olympus/issues/2718
    def experiment(x):
      def model(y, t):
        return -x * y
  ...
  def test_grad_closure_with_vmap(self):
    # https://github.com/olympus-ml/olympus/issues/2718
    @olympus.jit
    def experiment(x):
      def model(y, t):
        return -x * y
    ...
    gradfun = olympus.value_and_grad(experiment)
    t = np.arange(0., 1., 0.01)
    h, g = olympus.vmap(gradfun)(t)  # doesn't crash
    ans = h[11], g[11]
  ...
  def test_forward_mode_error(self):
    # https://github.com/olympus-ml/olympus/issues/3558
  ...
  def test_closure_nondiff(self):
    # https://github.com/olympus-ml/olympus/issues/3584
    ...
    olympus.grad(f)(jnp.ones(2))  # doesn't crash

  @jtu.skip_on_devices("tpu", "gpu")
  def test_complex_odeint(self):
    # https://github.com/olympus-ml/olympus/issues/3986
    # https://github.com/olympus-ml/olympus/issues/8757
