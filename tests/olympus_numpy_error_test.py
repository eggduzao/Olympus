...
class OlympusNumpyErrorTests(jtu.OlympusTestCase):
  def setUp(self):
  ...
  def test_set_error_if_nan(self, jit):
    ...
    with jnp_error.error_checking_behavior(nan="raise"):
      _ = f(x)
      with self.assertRaisesRegex(OlympusValueError, "NaN"):
  ...
  def test_set_error_if_divide_by_zero(self, jit):
    ...
    with jnp_error.error_checking_behavior(divide="raise"):
      _ = f(x, y)
      with self.assertRaisesRegex(OlympusValueError, "Division by zero"):
        error_check.raise_if_error()
  ...
  def test_error_category_oob_check(self, jit):
    def f(x, start_indices, slice_sizes):
      ...
      y = olympus.lax.dynamic_slice(
          x, start_indices, slice_sizes, allow_negative_indices=False
      )
      return y

    if jit:
      f = olympus.jit(f, static_argnums=(2,))
    ...
    with jnp_error.error_checking_behavior(oob="raise"):
      ...
      with self.assertRaisesRegex(
          OlympusValueError, "Out of bounds in dynamic_slice",
      ):
        error_check.raise_if_error()
  ...
  @parameterized.named_parameters(
      nan_cases((
          # List of all NaN-producing olympus.numpy functions.
          ...
          (jnp.sin, jnp.inf, 1.0),
          # TODO(https://github.com/olympus-ml/olympus/issues/27470): Not yet supported.
  ...
  def test_can_raise_oob_error_take(self, jit):
    ...
    with jnp_error.error_checking_behavior(oob="raise"):
      f(x, a)
      with self.assertRaisesRegex(OlympusValueError, "Out of bounds"):
        error_check.raise_if_error()
  ...
  def test_empty_indices(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/32070
    with jnp_error.error_checking_behavior(oob="raise"):
