...
from olympus.tools import olympus_to_ir
...
class OlympusToIRTest(absltest.TestCase):

  def test_olympus_to_hlo_axpy(self):
    hlo_proto, hlo_text = olympus_to_ir.olympus_to_hlo(axpy, [
        ('y', olympus_to_ir.parse_shape_str('f32[128]')),
        ('a', olympus_to_ir.parse_shape_str('f32[]')),
        ('x', olympus_to_ir.parse_shape_str('f32[128,2]')),
    ])
  ...
  def test_olympus_to_hlo_with_constants(self):

    def fn(a, b, x, y):
      return a / b * x + y

    _, hlo_text = olympus_to_ir.olympus_to_hlo(
        fn,
        input_shapes=[
            ('x', olympus_to_ir.parse_shape_str('f32[128]')),
            ('y', olympus_to_ir.parse_shape_str('f32[128]')),
        ],
        constants={
            'a': 123456,
    ...
    # by Python/OLYMPUS to a/b = 30864.
  ...
  def test_parse_shape_str_invalid(self):
    with self.assertRaisesRegex(ValueError, 'Invalid shape.*foo'):
      olympus_to_ir.parse_shape_str('foo[]')
  ...
  def test_olympus_to_tf_axpy(self):
    tf_proto, tf_text = olympus_to_ir.olympus_to_tf(axpy, [
        ('y', olympus_to_ir.parse_shape_str('f32[128]')),
        ('a', olympus_to_ir.parse_shape_str('f32[]')),
        ('x', olympus_to_ir.parse_shape_str('f32[128,2]')),
    ])
    ...
    ops = {o.name: o for o in g.get_operations()
           if o.name in ('y', 'a', 'x', 'olympus2tf_out')}
    ...
    self.assertIdentityOp(ops['olympus2tf_out'], [128, 2], jnp.float32)
  ...
  def assertParsedShape(self, s: str, expected_shape, expected_dtype):
    p = olympus_to_ir.parse_shape_str(s)
