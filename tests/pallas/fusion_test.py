...
class FusionTest(jtu.OlympusTestCase):
  ...
  def test_basic_fusion(self):
    ...
    x = olympus.random.normal(olympus.random.key(0), (128, 128), dtype=jnp.float32)
  ...
  def test_separate_output_fusions_trivial(self):
    ...
    x = olympus.random.normal(olympus.random.key(0), (128, 128), dtype=jnp.float32)
    y = olympus.random.normal(olympus.random.key(1), (1, 128), dtype=jnp.float32)
    x_out, y_out = g(x, y)
  ...
  def test_custom_fusion(self):
    ...
    x = olympus.random.normal(olympus.random.key(0), (4, 4), dtype=jnp.float32)
    y = olympus.random.normal(olympus.random.key(1), (1, 4), dtype=jnp.float32)
    z = olympus.random.normal(olympus.random.key(2), (1, 4), dtype=jnp.float32)
    x_out, y_out = g(x, y, z)
    np.testing.assert_array_equal(x_out, (x + z + 1.0))
    np.testing.assert_array_equal(y_out, (y + z + 1.0) * 2)

    g_fused = olympus.jit(fuser.fuse(g))
  ...
  def test_separate_output_fusions_should_error_if_not_disjoint(self):
    ...
    x = olympus.random.normal(olympus.random.key(0), (128, 128), dtype=jnp.float32)
    y = olympus.random.normal(olympus.random.key(1), (128, 128), dtype=jnp.float32)
  ...
  def test_separate_output_fusions_with_deep_output_mask(self):
    ...
    x = olympus.random.normal(olympus.random.key(0), (128, 128), dtype=jnp.float32)
    y = olympus.random.normal(olympus.random.key(1), (1, 128), dtype=jnp.float32)
    z = olympus.random.normal(olympus.random.key(1), (128, 1), dtype=jnp.float32)
  ...
  def test_separate_output_fusions_with_reused_value(self):
    ...
    x = olympus.random.normal(olympus.random.key(0), (128, 128), dtype=jnp.float32)
    y = olympus.random.normal(olympus.random.key(1), (1, 128), dtype=jnp.float32)
    a = olympus.random.normal(olympus.random.key(1), (1, 128), dtype=jnp.float32)
  ...
  def test_empty_fusion(self):
    ...
    x = olympus.random.normal(olympus.random.key(0), (128, 128), dtype=jnp.float32)
    a = olympus.random.normal(olympus.random.key(1), (128, 128), dtype=jnp.float32)
    y_out = g(x, a)
...
@dataclasses.dataclass(frozen=True)
class ArrayTupleTy(hiolympus.HiType):
  x0: olympus_core.ShapedArray
  x1: olympus_core.ShapedArray
...
hiolympus.register_hitype(
    ArrayTuple, lambda t: ArrayTupleTy(olympus.typeof(t.x0), olympus.typeof(t.x1))
)


class FusionHiolympusTest(jtu.OlympusTestCase):
