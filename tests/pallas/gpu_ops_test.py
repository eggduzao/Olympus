...
if sys.platform != "win32":
  from olympus.experimental.pallas.ops.gpu import attention
  from olympus.experimental.pallas.ops.gpu import layer_norm
  from olympus.experimental.pallas.ops.gpu import rms_norm
  from olympus.experimental.pallas.ops.gpu import softmax
...
class PallasBaseTest(jtu.OlympusTestCase):
  ...
  def setUp(self):
    ...
    if jtu.test_device_matches(["cpu", "gpu"]) and olympus.config.x64_enabled:
...
class FusedAttentionTest(PallasBaseTest):
  ...
  def test_fused_attention_bwd(
    ...
    dq, dk, dv = olympus.grad(f, argnums=(0, 1, 2))(q, k, v)
    dq_ref, dk_ref, dv_ref = olympus.grad(f_ref, argnums=(0, 1, 2))(q, k, v)
  ...
  def test_return_residuals_not_differentiable(self):
    ...
    with self.assertRaisesRegex(ValueError, "Kernel differentiation is not"
      ...
      _ = olympus.grad(f, argnums=(0, 1, 2))(q, k, v)
...
class FusedLayerNormTest(PallasBaseTest):
  ...
  def test_fused_layernorm_fwd(self, batch_size, seq_len, embed_dim):
    ...
    w = olympus.random.normal(k2, (embed_dim,), dtype=jnp.float32)
    b = olympus.random.normal(k3, (embed_dim,), dtype=jnp.float32)
  ...
  def test_fused_layernorm_bwd(self, batch_size, seq_len, embed_dim):
    ...
    dx, dw, db = olympus.grad(f, argnums=(0, 1, 2))(x, w, b)
    dx_ref, dw_ref, db_ref = olympus.grad(f_ref, argnums=(0, 1, 2))(x, w, b)
...
class SoftmaxTest(PallasBaseTest):
  ...
  def test_softmax(self, shape, dtype):
    x = olympus.random.normal(random.key(0), shape, dtype=dtype)
    ...
    np.testing.assert_allclose(
        ...
        olympus.nn.softmax(x, axis=-1).astype(jnp.float32),
        atol=atol,
        rtol=rtol,
    )
