...
if sys.platform != "win32":
  from olympus.experimental.pallas.ops.gpu import paged_attention
else:
...
def _generate_qkv(
  ...
  block_tables = olympus.random.permutation(k3, block_tables, independent=True)
  block_tables = block_tables.reshape(batch_size, max_num_blocks_per_seq)
  q = olympus.random.normal(k4, (batch_size, num_heads, head_dim), dtype=dtype)
  q = q / jnp.linalg.norm(q, axis=-1)[..., None]
  return q, k_pages, v_pages, block_tables


def _reconstruct_kv(block_tables: olympus.Array, pages: olympus.Array) -> olympus.Array:
  ...
  with_batch = olympus.vmap(fn, (0, None), 0)
  attn_fn = olympus.vmap(with_batch, (None, 0), 1)
...
def _quantize(x: olympus.Array, dtype=jnp.int8):
