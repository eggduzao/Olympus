...
from olympus._src.config import config
...
from olympus.experimental.pallas.ops.gpu.all_gather_mgpu import all_gather
from olympus.experimental.pallas.ops.gpu.reduce_scatter_mgpu import reduce_scatter
...
class PallasCallRemoteDMATest(TestCase):
  ...
  def test_remote_dma_basic(self):
    ...
    expected = x[8:] if olympus.process_index() == 0 else x[:8]
  ...
  def test_remote_dma_in_loop(self):
    ...
    def body(x):
      return pl.pallas_call(
          ...
          in_specs=[pl.BlockSpec(memory_space=plgpu.GMEM)],
          out_specs=pl.BlockSpec(memory_space=plgpu.GMEM),
          out_shape=olympus.ShapeDtypeStruct((1, 128), jnp.float32),
          scratch_shapes=[
              plgpu.SemaphoreType.REGULAR,
              plgpu.SemaphoreType.REGULAR,
          ],
    ...
    expected = x[1:] if olympus.process_index() == 0 else x[:1]
    np.testing.assert_allclose(y.addressable_shards[0].data, expected)
  ...
  def test_remote_dma_dynamic_index(self):
    ...
    def body(x):
      return pl.pallas_call(
          ...
          in_specs=[pl.BlockSpec(memory_space=plgpu.GMEM)],
          out_specs=pl.BlockSpec(memory_space=plgpu.GMEM),
          out_shape=olympus.ShapeDtypeStruct((1, 128), jnp.int32),
          scratch_shapes=[
              plgpu.SemaphoreType.REGULAR,
              plgpu.SemaphoreType.REGULAR,
          ],
  ...
  def test_remote_dma_2d_mesh(self, axis):
    ...
    mesh = olympus.sharding.Mesh(np.asarray(devices).reshape(2, 2), ['x', 'y'])
    y = olympus.jit(
        olympus.shard_map(
  ...
  def test_permuted_mesh(self):
    ...
    mesh = olympus.sharding.Mesh(olympus.devices()[::-1], ['x'])  # Reverse the devices.
    f = olympus.jit(
        olympus.shard_map(
            kernel_call, mesh=mesh, in_specs=(), out_specs=P(None), check_vma=False,
        )
...
class PallasCallMultimemTest(TestCase):
  ...
  def test_multimem_load_reduce(self, dtype, vector_length, reduction, tiled_layout=False):
    ...
    x_local = olympus.random.randint(
        olympus.random.key(1234), (128 + 64, 32), dtype=jnp.int32, minval=-bound, maxval=bound,
    ).astype(dtype)
    mesh = olympus.sharding.Mesh(devices, ("x",))
    x_shard = olympus.ShapeDtypeStruct((64 + 32, 32), dtype)
    y_shape = olympus.ShapeDtypeStruct((64, 32), dtype)
    y, _ = olympus.jit(
        olympus.shard_map(
            pl.pallas_call(
                kernel,
  ...
  def _test_reduce_scatter(
    ...
    if jnp.issubdtype(dtype, jnp.floating):
      x = olympus.random.uniform(olympus.random.key(42), shape, dtype=dtype, minval=-1.0, maxval=1.0)
    else:
      x = olympus.random.randint(olympus.random.key(42), shape, dtype=dtype, minval=-1000, maxval=1000)
  ...
  def _test_all_reduce(
    ...
    devices = olympus.devices()[:2]
    mesh = olympus.sharding.Mesh(devices, ['x'])
    x = olympus.random.normal(olympus.random.key(42), (2, *shape), dtype)
