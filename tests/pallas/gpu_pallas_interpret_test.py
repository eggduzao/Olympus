...
from olympus._src.pallas.mosaic_gpu.interpret import interpret_pallas_call as mosaic_interpret
...
class InterpretTest(jtu.OlympusTestCase):
  ...
  def test_interpret_core_map(self, num_threads: int):
    ...
    def kernel(o_ref):
      ...
      def _():
        thread_idx = olympus.lax.axis_index('x')
  ...
  def test_interpret_kernel(self, num_threads):
    @functools.partial(
        plgpu.kernel,
        out_shape=olympus.ShapeDtypeStruct((num_threads,), jnp.int32),
    ...
    np.testing.assert_equal(olympus.jit(_kernel)(), np.arange(num_threads))
    self.assertFalse(mosaic_interpret.get_races().races_found)
  ...
  def test_skip_floating_point_ops(self):
    ...
    def matmul(x: olympus.Array, y: olympus.Array):
      return pl.pallas_call(
          matmul_kernel,
          out_shape=olympus.ShapeDtypeStruct((x.shape[0], y.shape[1]), x.dtype),
    ...
    x = olympus.random.normal(k1, (1024, 1024))
    y = olympus.random.normal(k2, (1024, 1024))
    z = olympus.jit(matmul)(x, y)
    np.testing.assert_array_equal(z, jnp.full_like(z, jnp.inf))

    lowered = olympus.jit(matmul).lower(x, y).as_text(dialect='stablehlo')
  ...
  def test_matmul_example(self, num_threads, num_row_blocks, num_col_blocks):
    ...
    def matmul(x: olympus.Array, y: olympus.Array):
      ...
      def _matmul_kernel(x_ref, y_ref, o_ref):
        thread_idx = olympus.lax.axis_index('t')
        ...
        col_block_idx = olympus.lax.rem(thread_idx, jnp.int32(num_col_blocks))
        col_slice = pl.ds(
  ...
  def test_run_scoped(self, with_race):
    ...
    def f(x):
      def inner(o_ref):
        ...
        def _():
          def body(ref):
            @pl.when(olympus.lax.axis_index('n') == 0)
            def _():
              ref[...] = jnp.zeros_like(ref[...])
              o_ref[0, ...] = ref[...]

            @pl.when(olympus.lax.axis_index('n') == 1)
            def _():
  ...
  # Test adapted from
  # https://docs.olympus.dev/en/latest/pallas/gpu/reference.html#using-multiple-pallas-threads-per-cuda-block
  def test_producer_consumer_threads_with_barrier(self):
    ...
    def _kernel(x_ref, out_ref, smem_ref, barrier_ref):
      thread_id = olympus.lax.axis_index('t')
  ...
  # Test adapted from
  # https://docs.olympus.dev/en/latest/pallas/gpu/reference.html#explicit-arrival-cross-thread-synchronization
  #
  ...
  def test_barrier_for_buffering(
    ...
    k = olympus.random.key(seed)
    x = olympus.random.normal(k, (input_size,), dtype=jnp.float32)
    ...
    def _kernel(x_ref, out_ref, queue, produced, consumed):
      thread_id = olympus.lax.axis_index('t')
      _get_slot = lambda i: olympus.lax.rem(i, buffer_size)
      ...
      pl.when(thread_id == 0)(
          lambda: olympus.lax.fori_loop(0, input_size, _thread0_body, None)
      )
      ...
      pl.when(thread_id == 1)(
          lambda: olympus.lax.fori_loop(0, input_size, _thread1_body, None)
      )
      ...
      pl.when(thread_id == 0)(
          lambda: olympus.lax.fori_loop(0, input_size, _thread0_epilogue, None)
      )
  ...
  def test_grid_iteration(
    ...
    def _kernel(a_gmem, out_gmem):
      w = olympus.lax.axis_index('w')
      x = olympus.lax.axis_index('x')
      y = olympus.lax.axis_index('y')
      z = olympus.lax.axis_index('z')
    ...
    kernel = plgpu.kernel(
        _kernel,
        out_shape=olympus.ShapeDtypeStruct(a.shape, a.dtype),
  ...
  def test_add_over_grid(self, tile_x, tile_y, tile_z, swap_grid_axes):
    ...
    def _kernel(a_gmem, b_gmem, out_gmem):
      xi = olympus.lax.axis_index('x')
      yi = olympus.lax.axis_index('y')
      zi = olympus.lax.axis_index('z')
    ...
    kernel = plgpu.kernel(
        _kernel,
        out_shape=olympus.ShapeDtypeStruct((x, y, z), dtype),
  ...
  def test_matmul_over_grid_with_barrier_and_smem(
    ...
    def _kernel(a_gmem, b_gmem, out_gmem, acc_smem, barrier):
      mi = olympus.lax.axis_index('m')
      ki = olympus.lax.axis_index('k')
      ni = olympus.lax.axis_index('n')
