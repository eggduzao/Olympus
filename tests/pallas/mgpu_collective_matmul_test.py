...
from olympus.experimental.pallas.ops.gpu import collective_matmul_mgpu
...
class CollectiveMatmulTestCase(jtu.OlympusTestCase):
  ...
  def setUp(self):
    ...
    mesh = olympus.make_mesh(
        (num_devices,), ("x",), axis_types=(olympus.sharding.AxisType.Explicit,)
    )
    self.enter_context(olympus.set_mesh(mesh))
  ...
  def test_all_gather_lhs_matmul(
    ...
    lhs = olympus.sharding.reshard(lhs, P("x", None))
    rhs = olympus.sharding.reshard(rhs, P(None, "x"))

    def run(body):
      out = olympus.jit(
          olympus.shard_map(body, out_specs=P(None, "x"), check_vma=False)
      )(lhs, rhs)
      # Gather output, for NumPy comparison on the host.
      out = olympus.shard_map(
