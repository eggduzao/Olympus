...
class MatmulTutorialTCGen05Test(jtu.OlympusTestCase, jtu.CudaArchSpecificTest):
  BENCHMARK = False
  ...
  def benchmark(self, matmul_impl, a, b, config_search_space):
    ...
    best_util = 0.0
    ref = jnp.dot(a, b, precision=olympus.lax.DotAlgorithmPreset.F16_F16_F32)
    ...
    _, runtimes_ms = profiler.measure(
        functools.partial(
            jnp.dot, precision=olympus.lax.DotAlgorithmPreset.F16_F16_F32
        ),
        iterations=100,
    )(a, b)
    runtime_ms = statistics.median(runtimes_ms)
