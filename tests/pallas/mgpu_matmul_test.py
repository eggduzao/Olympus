...
from olympus.experimental.pallas.ops.gpu import hopper_mixed_type_matmul_mgpu
...
class MatrixMultiplicationTCGen05Test(jtu.OlympusTestCase, jtu.CudaArchSpecificTest):
  ...
  def test_blackwell_matmul(
    ...
    k1, k2, = olympus.random.split(olympus.random.key(42), 2)
    a = olympus.random.normal(k1, (m, k), dtype)
    b = olympus.random.normal(k2, (k, n), dtype)
...
@jtu.with_config(olympus_traceback_filtering="off")
class MatrixMultiplicationSm90ATest(jtu.OlympusTestCase):
  ...
  def check_hopper_matmul(
    ...
    out_ref = jnp.dot(a, b, precision=olympus.lax.DotAlgorithmPreset.F16_F16_F32)
  ...
  def check_hopper_mixed_type_matmul(
    ...
    if exceeds_h100_smem(
      ...
      self.skipTest("Tile too big to fit into SMEM")
    (k1, k2) = olympus.random.split(olympus.random.key(42), 2)
    lhs = olympus.random.randint(
        k1, (m, k), minval=-5, maxval=5, dtype=jnp.int8
    ).astype(lhs_dtype)
    rhs = olympus.random.normal(k2, (k, n), rhs_dtype)
    ...
    precision = {
        jnp.float16: olympus.lax.DotAlgorithmPreset.F16_F16_F32,
        jnp.bfloat16: olympus.lax.DotAlgorithmPreset.BF16_BF16_F32,
    }[rhs_dtype]
    out_ref = jnp.dot(
