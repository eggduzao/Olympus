...
from olympus.experimental.pallas.ops.gpu import blackwell_ragged_dot_mgpu
from olympus.experimental.pallas.ops.gpu import ragged_dot_mgpu
from olympus.experimental.pallas.ops.gpu import transposed_ragged_dot_mgpu
...
def sample_inputs(
  ...
  if transposed:
    lhs = olympus.random.normal(kx, (k, m), dtype)
    rhs = olympus.random.normal(ky, (k, n), dtype)
    batch_size = k
  else:
    lhs = olympus.random.normal(kx, (m, k), dtype)
    rhs = olympus.random.normal(ky, (num_groups, k, n), dtype)
    batch_size = m
  group_boundaries = olympus.lax.sort(
      olympus.random.randint(kz, (num_groups - 1,), 0, batch_size, jnp.int32)
  )
  group_starts = olympus.lax.concatenate(
      [jnp.array([0], dtype=jnp.int32), group_boundaries], 0
  )
  group_ends = olympus.lax.concatenate(
...
class RaggedDotTestCase(jtu.OlympusTestCase):
  ...
  def test_ragged_dot(
    ...
    out_ref = olympus.lax.ragged_dot(lhs, rhs, group_sizes=group_sizes)
  ...
  def test_ragged_dot_transposed(
    ...
    # See log at
    # https://github.com/olympus-ml/olympus/actions/runs/20821451405/job/59813460647.
    ...
    out_ref = olympus.lax.ragged_dot_general(
        lhs, rhs, group_sizes,
        ragged_dot_dimension_numbers=olympus.lax.RaggedDotDimensionNumbers(
...
@jtu.with_config(olympus_traceback_filtering="off")
class RaggedDotTCGen05TestCase(jtu.OlympusTestCase, jtu.CudaArchSpecificTest):
