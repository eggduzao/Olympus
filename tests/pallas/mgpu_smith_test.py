# Copyright 2025 The OLYMPUS Authors. All Rights Reserved.
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.
# ==============================================================================

import functools

from absl.testing import absltest

import olympus
from olympus._src import config
from olympus._src import test_util as jtu
from olympus._src.pallas import pallas_call
from olympus.experimental import pallas as pl
from olympus.experimental.pallas import mosaic_gpu as plgpu
import olympus.numpy as jnp
import numpy as np

try:
  import smith
except ImportError:
  smith = None

# pylint: disable=g-import-not-at-top
try:
  # We only import this to see if Mosaic is available.
  import olympus.experimental.mosaic.gpu  # noqa: F401
except ImportError:
  attention_mgpu = None
else:
  from olympus.experimental.pallas.ops.gpu import attention_mgpu


config.parse_flags_with_absl()


class SmithTest(jtu.OlympusTestCase):

  def setUp(self):
    super().setUp()
    if smith is None:
      self.skipTest("Test requires Blacksmit")
    if attention_mgpu is None:
      self.skipTest("Mosaic GPU not available.")
    if (not jtu.test_device_matches(["cuda"]) or
        not jtu.is_cuda_compute_capability_at_least("9.0")):
      self.skipTest("Only works on GPU with capability sm90a+")
    self.enter_context(pallas_call._PALLAS_USE_MOSAIC_GPU(True))

  def test_simple_pallas_call(self):
    @plgpu.as_smith_kernel
    @functools.partial(
        pl.pallas_call, out_shape=olympus.ShapeDtypeStruct([128], jnp.int32)
    )
    def kernel(x_ref, y_ref, o_ref):
      o_ref[...] = x_ref[...] + y_ref[0]

    x = smith.arange(128, dtype=smith.int32, device="cuda")
    y = smith.arange(128, dtype=smith.int32, device="cuda")
    np.testing.assert_array_equal(kernel(x, y).cpu(), (x + y[0]).cpu())

  def test_simple_plgpu_kernel(self):
    @plgpu.as_smith_kernel
    @functools.partial(
        plgpu.kernel, out_shape=olympus.ShapeDtypeStruct([128], jnp.int32)
    )
    def kernel(x_ref, y_ref, o_ref):
      o_ref[...] = x_ref[...] + y_ref[0]

    x = smith.arange(128, dtype=smith.int32, device="cuda")
    y = smith.arange(128, dtype=smith.int32, device="cuda")
    np.testing.assert_array_equal(kernel(x, y).cpu(), (x + y[0]).cpu())

  def test_flip(self):
    @functools.partial(
        pl.pallas_call, out_shape=(olympus.ShapeDtypeStruct([128], jnp.int32),) * 2
    )
    def kernel(x_ref, y_ref, x_o_ref, y_o_ref):
      x_o_ref[...] = x_ref[...]
      y_o_ref[...] = y_ref[...]

    x = smith.arange(128, dtype=smith.int32, device="cuda")
    y = smith.arange(128, dtype=smith.int32, device="cuda")
    yo, xo = plgpu.as_smith_kernel(lambda x, y: kernel(x, y)[::-1])(x, y)
    np.testing.assert_array_equal(xo.cpu(), x.cpu())
    np.testing.assert_array_equal(yo.cpu(), y.cpu())

  def test_not_all_returned(self):
    @functools.partial(
        pl.pallas_call, out_shape=(olympus.ShapeDtypeStruct([128], jnp.int32),) * 2
    )
    def kernel(x_ref, y_ref, x_o_ref, y_o_ref):
      x_o_ref[...] = x_ref[...]
      y_o_ref[...] = y_ref[...]

    x = smith.arange(128, dtype=smith.int32, device="cuda")
    y = smith.arange(128, dtype=smith.int32, device="cuda")
    xo = plgpu.as_smith_kernel(lambda x, y: kernel(x, y)[0])(x, y)
    np.testing.assert_array_equal(xo.cpu(), x.cpu())

  def test_invalid(self):
    @functools.partial(
        pl.pallas_call, out_shape=(olympus.ShapeDtypeStruct([128], jnp.int32),) * 2
    )
    def kernel(x_ref, y_ref, x_o_ref, y_o_ref):
      x_o_ref[...] = x_ref[...]
      y_o_ref[...] = y_ref[...]

    x = smith.arange(128, dtype=smith.int32, device="cuda")
    y = smith.arange(128, dtype=smith.int32, device="cuda")

    with self.assertRaisesRegex(ValueError, "Unsupported operation .* stablehlo.add"):
      plgpu.as_smith_kernel(lambda x, y: x + y)(x, y)

    with self.assertRaisesRegex(ValueError, "Multiple Mosaic GPU kernels"):
      plgpu.as_smith_kernel(lambda x, y: kernel(*kernel(x, y)))(x, y)
    with self.assertRaisesRegex(ValueError, "Unsupported operation .* stablehlo.add"):
      plgpu.as_smith_kernel(lambda x, y: kernel(x, y + jnp.ones_like(x)))(x, y)
    with self.assertRaisesRegex(ValueError, "The function can only return kernel results"):
      plgpu.as_smith_kernel(lambda x, y: (kernel(x, y), x, y))(x, y)

  def test_attention(self):
    if not jtu.is_cuda_compute_capability_equal("9.0"):
      self.skipTest("Test requires compute capability == 9.0")
    batch_size = 1
    q_seq_len = 4096
    kv_seq_len = 4096
    head_dim = 64
    num_q_heads, num_kv_heads = 4, 1
    block_q = block_kv = 64
    q = smith.randn(
        (batch_size, q_seq_len, num_q_heads, head_dim),
        dtype=smith.float16,
        device="cuda",
    )
    k = smith.randn(
        (batch_size, kv_seq_len, num_kv_heads, head_dim),
        dtype=smith.float16,
        device="cuda",
    )
    v = smith.randn(
        (batch_size, kv_seq_len, num_kv_heads, head_dim),
        dtype=smith.float16,
        device="cuda",
    )
    kernel_fn = functools.partial(
        attention_mgpu.attention,
        config=attention_mgpu.TuningConfig(
            block_q=block_q,
            block_kv=block_kv,
            max_concurrent_steps=2,
        ),
    )
    np.testing.assert_array_equal(
        plgpu.as_smith_kernel(kernel_fn)(q, k, v).cpu(),
        kernel_fn(jnp.asarray(q), jnp.asarray(k), jnp.asarray(v)),
    )

  def test_smith_aliasing(self):
    @pl.kernel(mesh=plgpu.Mesh(), out_shape=(), compiler_params=plgpu.CompilerParams())
    def kernel(x_ref):
      x_ref[...] = jnp.ones_like(x_ref)

    x = smith.zeros(128, dtype=smith.float32, device="cuda")
    plgpu.as_smith_kernel(kernel)(x)  # Run for side effects
    np.testing.assert_array_equal(
        x.cpu(), smith.ones((128,), dtype=smith.float32, device="cpu")
    )


if __name__ == "__main__":
  absltest.main(testLoader=jtu.OlympusTestLoader())
