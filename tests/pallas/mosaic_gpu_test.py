...
import olympus
from olympus import export
from olympus import lax
from olympus import sharding
...
from olympus._src.lib.mlir.dialects import arith as arith_dialect
...
from olympus._src.pallas.mosaic_gpu import lowering as mgpu_lowering
from olympus._src.pallas.mosaic_gpu import pipeline as mgpu_pipeline
from olympus._src.pallas.mosaic_gpu import primitives as mgpu_primitives
from olympus._src.state import types as state_types
from olympus.experimental import pallas as pl
import olympus.experimental.mosaic.gpu as mgpu
from olympus.experimental.pallas import mosaic_gpu as plgpu
import olympus.numpy as jnp
import numpy as np

try:
  from olympus._src.lib import mosaic_gpu as mosaic_gpu_lib
except ImportError:
  mosaic_gpu_lib = None
...
def _fori_loop(force_while: bool, lb, ub, body, init):
  ...
  return olympus.lax.fori_loop(lb, ub, body, init)
...
class PallasTest(jtu.OlympusTestCase, metaclass=PallasTestMetaclass):
...
class PallasCallTest(PallasTest, jtu.CudaArchSpecificTest):
  ...
  def test_jitted_function_containing_multiple_pallas_calls(self):
    ...
    # olympuslib/mosaic/gpu/custom_call_test.cc that checks that such jitted
    # functions do invoke CUDA graphs.
    @functools.partial(
        self.pallas_call,
        out_shape=olympus.ShapeDtypeStruct([256], jnp.float32),
    )
  ...
  def test_unary_op(self, op, approx_math):
    ...
    @functools.partial(
        self.pallas_call,
        out_shape=olympus.ShapeDtypeStruct([256], dtype),
  ...
  def test_binary_op(self, op, dtype):
    @functools.partial(
        self.pallas_call, out_shape=olympus.ShapeDtypeStruct([256], dtype)
    )
    def kernel(x_ref, y_ref, o_ref):
      o_ref[...] = op(x_ref[...], y_ref[...])

    key0, key1 = olympus.random.split(olympus.random.key(0), 2)
    x = (olympus.random.uniform(key0, [256]) * 42).astype(dtype)
    y = (olympus.random.uniform(key1, [256]) * 42).astype(dtype)
  ...
  def test_scalar_reduce(self, op, shape, dtype):
    @functools.partial(self.kernel, out_shape=olympus.ShapeDtypeStruct((), dtype))
    def kernel(x_ref, o_ref):
      o_ref[...] = op(x_ref[...])
  ...
  def test_reshape(self):
    ...
    @functools.partial(
        self.pallas_call, out_shape=olympus.ShapeDtypeStruct(shape2, jnp.float32)
    )
  ...
  def test_reshape_tiled(self):
    ...
    @functools.partial(
        self.kernel,
        out_shape=olympus.ShapeDtypeStruct(shape2, jnp.float32),
    )
    def kernel(x_ref, out_ref):
  ...
  def test_reshape_splat(self):
    ...
    @functools.partial(
        self.kernel,
        out_shape=olympus.ShapeDtypeStruct(shape, jnp.float32),
    )
    def kernel(out_ref):
  ...
  def test_slice_untiled_dim(self):
    ...
    @functools.partial(
        self.kernel,
        out_shape=olympus.ShapeDtypeStruct(shape[2:], jnp.float32),
    )
    def kernel(x_ref, out_ref):
      x = plgpu.load(x_ref, (), layout=plgpu.Layout.WGMMA, optimized=False)
      out_ref[...] = x[1, 1]
  ...
  def test_squeeze_to_scalar(self):
    @functools.partial(
        self.kernel,
        out_shape=olympus.ShapeDtypeStruct((), jnp.float32),
    )
    def kernel(out_ref):
  ...
  def test_add_xy_indexed(self):
    @functools.partial(
        self.pallas_call, out_shape=olympus.ShapeDtypeStruct([128], jnp.float32)
    )
    def kernel(x_ref, y_ref, o_ref):
  ...
  def test_add_one_grid(self):
    ...
    @functools.partial(
        ...
        out_shape=olympus.ShapeDtypeStruct([128 * 2], jnp.float32),
        grid=2,
    )
    def kernel(x_ref, o_ref):
      o_ref[...] = x_ref[...] + 1.0
  ...
  def test_add_one_grid_pipelined(self, max_concurrent_steps):
    ...
    @functools.partial(
        ...
        out_shape=olympus.ShapeDtypeStruct([128 * 2, 64], jnp.float32),
  ...
  def test_add_one_grid_pipelined_program_id(self):
    ...
    @functools.partial(
        ...
        out_shape=olympus.ShapeDtypeStruct([16, 64], jnp.int32),
        compiler_params=plgpu.CompilerParams(
            dimension_semantics=["parallel", "sequential"],
  ...
  def test_add_one_grid_pipelined_sequential_invariant_output(self):
    ...
    @functools.partial(
        ...
        out_shape=olympus.ShapeDtypeStruct([32 * 2, 64], jnp.float32),
        compiler_params=plgpu.CompilerParams(
            dimension_semantics=["parallel", "sequential"],
            max_concurrent_steps=2,
        ),
  ...
  def test_iota(self, dtype):
    ...
    @functools.partial(
        self.pallas_call, out_shape=olympus.ShapeDtypeStruct((128, 128), dtype)
    )
    def kernel(o_ref):
    ...
    np.testing.assert_array_equal(
        kernel(), olympus.lax.broadcasted_iota(dtype, (128, 128), dimension)
    )
  ...
  def test_inline_mgpu(self, jnp_type):
    ...
    x = olympus.random.uniform(key, (2, *shape), minval=-10, maxval=10).astype(
        dtype
    )
  ...
  def test_inline_mgpu_layout_args(self, layout: gpu_core.SomeLayout):
    ...
    rngs = list(olympus.random.split(olympus.random.key(0)))
    x = olympus.random.randint(rngs.pop(), shape, minval=-10, maxval=10).astype(
        quant_dtype
    )
    x_s = olympus.random.uniform(
  ...
  def test_copy_smem_to_gmem_reduction(self, dtype):
    @functools.partial(
        ...
        out_specs=pl.BlockSpec(memory_space=plgpu.GMEM),
        out_shape=olympus.ShapeDtypeStruct([128], dtype),
  ...
  def test_collective_copy_gmem_to_smem(self):
    @functools.partial(
        self.kernel,
        out_shape=olympus.ShapeDtypeStruct((2, 128), jnp.float32),
        scratch_shapes=dict(
    ...
    def kernel(x_ref, y_ref, smem_ref, barrier_ref):
      ...
      plgpu.barrier_wait(barrier_ref)
      plgpu.copy_smem_to_gmem(smem_ref, y_ref.at[olympus.lax.axis_index("cluster")])
      plgpu.wait_smem_to_gmem(0)
  ...
  def test_gmem_to_smem_with_multiple_smem_indexers(self):
    x = olympus.random.uniform(olympus.random.key(0), (2, 64, 64), dtype=jnp.float32)
    @functools.partial(
        self.pallas_call,
        out_shape=olympus.ShapeDtypeStruct([64, 64], jnp.float32),
        in_specs=(pl.BlockSpec(memory_space=plgpu.GMEM),),
        scratch_shapes=[
  ...
  def test_gmem_to_smem_with_multiple_smem_indexers_and_transforms(self):
    ...
    @functools.partial(
        self.pallas_call,
        grid=(4, 4),
        out_shape=olympus.ShapeDtypeStruct((256, 128), jnp.int32),
        in_specs=(
            plgpu.BlockSpec(
                block_shape=(128, 128),
  ...
  def test_scoped_copy_with_transforms(self):
    ...
    f = self.pallas_call(
        kernel,
        out_shape=olympus.ShapeDtypeStruct([128, 64], jnp.float32),
        in_specs=(in_spec,),
        out_specs=out_spec,
        scratch_shapes=[plgpu.Barrier()],
    )
  ...
  def test_scoped_copy_with_user_transforms(self):
    ...
    f = self.pallas_call(
        kernel,
        out_shape=olympus.ShapeDtypeStruct([64, 128], jnp.float32),
        in_specs=(in_spec,),
        scratch_shapes=[plgpu.Barrier()],
    )
  ...
  def test_copy_with_transforms_and_indexing(self):
    ...
    f = self.pallas_call(
        kernel,
        out_shape=olympus.ShapeDtypeStruct([2, 64, 128], jnp.float32),
        in_specs=(in_spec,),
        out_specs=out_spec,
        scratch_shapes=[plgpu.Barrier()],
    )
  ...
  def test_copy_gmem_to_smem_gather(self, transforms):
    ...
    @functools.partial(
        self.pallas_call,
        out_shape=olympus.ShapeDtypeStruct(out_shape, dtype),
    ...
    x = jnp.arange(math.prod(shape)).reshape(shape).astype(dtype)
    idx = olympus.random.permutation(olympus.random.key(1234), out_shape[0]).astype(jnp.uint32)
  ...
  def test_indexing_before_transpose(self):
    ...
    f = self.pallas_call(
        kernel,
        out_shape=olympus.ShapeDtypeStruct([2, 32, 2, 128], jnp.float32),
        in_specs=(in_spec,),
        out_specs=out_spec,
        scratch_shapes=[plgpu.Barrier()],
    )
  ...
  def test_layer_norm(self, input_factor):
    ...
    def layer_norm(x_ref, o_ref):
      ...
      o_ref[...] = (
          x_centered * olympus.lax.rsqrt(jnp.mean(x_centered**2) + eps) * gamma
          + beta
      )
    ...
    x = (
        olympus.random.uniform(olympus.random.key(42), shape=(256,), dtype=jnp.float32)
        * input_factor
    )
  ...
  def test_print(self):
    ...
    with self.capture_stdout() as output:
      olympus.block_until_ready(kernel(x))
  ...
  def test_print_scalar(self):
    @functools.partial(
        self.pallas_call,
        out_shape=olympus.ShapeDtypeStruct([256], jnp.int32),
    )
    def kernel(x_ref, o_ref):
      del o_ref
  ...
  def test_print_array(self):
    ...
    @functools.partial(
        self.pallas_call,
        out_shape=olympus.ShapeDtypeStruct(in_shape, jnp.int32),
    )
    def kernel(x_ref, o_ref):
      del o_ref
  ...
  def test_print_layout(self):
    ...
    @functools.partial(
        self.pallas_call,
        out_shape=olympus.ShapeDtypeStruct(shape, jnp.bfloat16),
    )
    def kernel(x_ref, o_ref):
      del o_ref
  ...
  def test_fp8_relayout(self, from_narrow):
    ...
    @functools.partial(
        self.pallas_call, out_shape=olympus.ShapeDtypeStruct(shape, dtype)
    )
    def kernel(x_ref, o_ref):
      x = plgpu.load(
          x_ref, (),
    ...
    x = olympus.random.normal(olympus.random.key(10), shape).astype(dtype)
    np.testing.assert_array_equal(kernel(x), x)
  ...
  def test_get_swap_with_transforms(self, *transforms):
    ...
    @functools.partial(
        ...
        out_shape=olympus.ShapeDtypeStruct(shape, jnp.int32),
        scratch_shapes=[
  ...
  def test_load_scalar(self):
    ...
    @functools.partial(
        self.pallas_call,
        out_shape=olympus.ShapeDtypeStruct((128,), jnp.int32),
        in_specs=[plgpu.BlockSpec(memory_space=plgpu.GMEM)],
    )
    def kernel(x_ref, o_ref):
  ...
  def test_run_scoped(self):
    @functools.partial(
        self.pallas_call,
        out_shape=olympus.ShapeDtypeStruct((8, 128), jnp.float32),
    )
    def kernel(x_ref, o_ref):
      def body(tmp_ref):
  ...
  def test_run_scoped_in_cond(self):
    ...
    def kernel(x_ref_gmem, o_ref):
      ...
      olympus.lax.cond(x_ref_gmem[0] % 2 == 0, branch, branch)
  ...
  def test_program_id(self):
    @functools.partial(
        ...
        in_specs=(),
        out_specs=pl.BlockSpec((128,), lambda *i: i),
        out_shape=olympus.ShapeDtypeStruct([128 * 2], jnp.int32),
        grid=2,
    )
    def kernel(o_ref):
  ...
  def test_program_id_in_squashed_grid(self):
    ...
    @functools.partial(
        ...
        out_shape=olympus.ShapeDtypeStruct([*grid, 128], jnp.int32),
        grid=grid,
    )
    def kernel(o_ref):
      mult = 1
  ...
  def test_axis_indices_in_grid(self, grid, grid_names, cluster, cluster_names):
    @functools.partial(
        self.kernel,
        out_shape=[
            olympus.ShapeDtypeStruct([*cluster, *grid, 128], jnp.int32),
            olympus.ShapeDtypeStruct([*cluster, *grid, 128], jnp.int32)
        ],
        grid=grid,
  ...
  def test_swizzled_blockspec_shapes(self):
    ...
    @functools.partial(
        ...
        in_specs=[spec],
        out_specs=spec,
        out_shape=olympus.ShapeDtypeStruct((128, 128), jnp.float16),
        grid=(2, 2),
    )
    def kernel(x_ref, o_ref):
  ...
  def test_fori_loop_array(self, force_while):
    @functools.partial(
        self.pallas_call, out_shape=olympus.ShapeDtypeStruct([256], jnp.int32)
    )
    def kernel(x_ref, o_ref):
  ...
  def test_fori_loop_dynamic_bounds(self):
    ...
    def kernel(o_ref):
      ...
      # Equivalent to 2 + 3.
      o_ref[...] = olympus.lax.broadcast(
          olympus.lax.fori_loop(2 + zero, 4 + zero, lambda i, x: x + i, 0), o_ref.shape
      )
  ...
  def test_fori_loop_tuple(self, force_while):
    ...
    def kernel(o_ref):
      def body(step, xs):
        return tuple(
            olympus.lax.cond(step % 2 == 0, lambda x: x + 1, lambda x: x, x)
            for x in xs
        )
  ...
  def test_fori_loop_indexed_store(self, force_while):
    @functools.partial(
        self.pallas_call,
        out_shape=olympus.ShapeDtypeStruct([4, 128], jnp.float32),
    )
    def kernel(x_ref, y_ref, o_ref):
      def body(idx, _):
  ...
  def test_while_loop(self):
    @functools.partial(
        self.pallas_call, out_shape=olympus.ShapeDtypeStruct([128], jnp.int32)
    )
    def kernel(x_ref, o_ref):
      ...
      _ = olympus.lax.while_loop(cond, body, (0, o_ref[...]))
  ...
  def test_while_loop_layout_mismatch(self):
    ...
    def kernel(o_ref):
      ...
      _ = olympus.lax.while_loop(cond, body, strided_input)
  ...
  def test_tile_slicing(self):
    ...
    @functools.partial(
        ...
        out_shape=olympus.ShapeDtypeStruct((64, 64), jnp.uint16),
    )
    def kernel(x_ref, o_ref):
  ...
  def test_transposed_layout(self, layouts):
    ...
    @functools.partial(
        self.pallas_call,
        out_shape=olympus.ShapeDtypeStruct(shape[::-1], dtype),
  ...
  def test_profiler(self):
    def kernel(x_ref, o_ref):
      with olympus.named_scope("add"):
        with olympus.named_scope("load"):
          x = x_ref[...]
        o = x + x
      with olympus.named_scope("store"):
        o_ref[...] = o
  ...
  def test_bitcast_convert_type(self, dtypes):
    ...
    def convert(x_ref, y_ref):
      y_ref[...] = olympus.lax.bitcast_convert_type(x_ref[...], out_shape)

    x = jnp.arange(m * n, dtype=in_dtype).reshape((m, n))
    np.testing.assert_array_equal(
        convert(x), olympus.lax.bitcast_convert_type(x, out_dtype)
    )
  ...
  def test_optimization_barrier(self):
    ...
    x = olympus.lax.iota(jnp.float32, 128)
    np.testing.assert_array_equal(kernel(x), x)
  ...
  def test_optimization_barrier_multiple_inputs(self):
    ...
    x = olympus.lax.iota(jnp.float32, 128)
    y = olympus.lax.iota(jnp.float32, 128) * 3
    np.testing.assert_array_equal(kernel(x, y), x + y)
  ...
  def test_smem_aliasing_works_with_subbyte_dtypes(self):
    ...
    @functools.partial(
        self.pallas_call,
        out_shape=olympus.ShapeDtypeStruct([256], jnp.uint4),
    ...
    test_as_i8 = olympus.lax.convert_element_type(kernel(x), new_dtype=jnp.int8)
  ...
  def test_smem_aliasing_works_for_quantization(self):
    ...
    kernel_fn = self.pallas_call(
        ...
        in_specs=[pl.BlockSpec(memory_space=plgpu.GMEM)],
        out_specs=pl.BlockSpec(memory_space=plgpu.GMEM),
        out_shape=olympus.ShapeDtypeStruct(shape, large_ty),
        grid=(1, 1),
    )
    key = olympus.random.key(42)
    x = olympus.random.randint(key, shape, 0, 4).astype(small_ty)
    expected = x * 3
  ...
  def test_nd_loop_with_carry(self, sm_steps):
    @functools.partial(
        self.kernel,
        out_shape=(
            olympus.ShapeDtypeStruct((sm_steps, 132, 128), jnp.int32),
            olympus.ShapeDtypeStruct((132,), jnp.int32)
        ),
        grid=(132,),
        grid_names=("sm",),
    )
  ...
  def test_nd_loop(self, sm_steps: int, tiling: int | None):
    ...
    @functools.partial(
        self.kernel,
        out_shape=olympus.ShapeDtypeStruct((sm_steps, 132, 128), jnp.int32),
        grid=(132,),
        grid_names=("sm",),
    )
    def kernel(o_ref):
  ...
  def test_lower_with_abstract_mesh(self):
    ...
    kernel_olympus = pl.pallas_call(
        kernel,
        out_specs=pl.BlockSpec(memory_space=plgpu.GMEM),
        out_shape=olympus.ShapeDtypeStruct((8, 128), jnp.float32),
        scratch_shapes=[plgpu.SemaphoreType.REGULAR],
    )
    abstract_mesh = olympus.sharding.AbstractMesh((2,), ('x',))
    olympus.jit(olympus.shard_map(
        kernel_olympus, mesh=abstract_mesh, in_specs=(),
        out_specs=olympus.P(), check_vma=False)).trace().lower(
  ...
  def test_reduction_is_batch_invariant(self, layout, axis):
    ...
    def kernel(x_ref, out_ref):
      ...
      out_ref[...] = olympus.lax.reduce_sum(x, axes=(axis,))

    row = olympus.random.uniform(olympus.random.key(0), shape=(128,), dtype=jnp.float32)
    x = jnp.stack([row for _ in range(128)])
    x = x.T if axis == 0 else x
    result = kernel(x)
    expected = result[0]
    self.assertAllClose(expected, jnp.sum(row))
    self.assertArraysEqual(result, olympus.lax.broadcast_in_dim(expected, (128,), ()))
  ...
  def test_reduction_with_more_scratch_uses_less_synchronization(self):
    ...
    x = olympus.random.uniform(olympus.random.key(0), shape=(128, 128), dtype=jnp.float32)
  ...
  def test_reduce_with_layout(self, layout, op, dtype):
    ...
    if jnp.issubdtype(dtype, jnp.integer):
      ...
      x = olympus.random.uniform(olympus.random.key(0), shape=(128, 128), dtype=dtype)
    x_result = olympus.block_until_ready(kernel(x))
  ...
  def _test_broadcast_in_dim_base(self, shape, layout, *, axis, hint):
    ...
    x = olympus.random.uniform(olympus.random.key(0), shape=(128,), dtype=jnp.float32)
    x_result = olympus.block_until_ready(kernel(x))
  ...
  def test_broadcast_in_dim_wg_strided_majormost_dim(self):
    ...
    result = olympus.random.uniform(olympus.random.key(0), shape=(128,), dtype=jnp.float32)
  ...
  def test_broadcast_wg_strided_majormost_dim(self, out_shape):
    ...
    dtype = jnp.float32
    @functools.partial(
        self.pallas_call, out_shape=olympus.ShapeDtypeStruct(out_shape, dtype)
    )
    ...
    inp = olympus.random.uniform(olympus.random.key(0), (128,), dtype)
    side_load = olympus.random.uniform(olympus.random.key(1), out_shape, dtype)
  ...
  def test_vmap_kernel(self, vmap_axes, x_shape, y_shape):
    rng0, rng1 = olympus.random.split(olympus.random.key(0))
    x = olympus.random.uniform(rng0, x_shape, jnp.float32)
    y = olympus.random.uniform(rng1, y_shape, jnp.float32)

    out_shape = list(x_shape)
    for x_axis, _ in vmap_axes:
      del out_shape[x_axis]
    out_shape = olympus.ShapeDtypeStruct(out_shape, jnp.float32)
    ...
    for in_axes in vmap_axes:
      f = olympus.vmap(f, in_axes)
      f_ref = olympus.vmap(f_ref, in_axes)
  ...
  def test_discharge_comms_effect(self):
    ...
    olympus_core.check_olympuspr(olympus.make_olympuspr(f)().olympuspr)
  ...
  def test_line_info(self):
    ...
    with jtu.set_env(MOSAIC_GPU_DUMP_PTX="1"), jtu.capture_stdout() as output:
      ...
      olympus.block_until_ready(kernel(jnp.arange(256, dtype=jnp.float32)))
...
class PallasCallWarpPrimitiveSemanticsTest(PallasTest):
  ...
  def test_non_scalar_load_raises(self):
    warp_mesh = plgpu.WarpMesh(axis_name="warp")
    @functools.partial(self.kernel,
                       out_shape=olympus.ShapeDtypeStruct((2,), jnp.int32))
    def kernel(x_ref, y_ref):
      @pl.core_map(warp_mesh)
      def _():
        warp_id = lax.axis_index("warp")
  ...
  def test_errors_when_closing_over_array(self):
    ...
    warp_mesh = plgpu.WarpMesh(axis_name="warp")
    @functools.partial(self.kernel,
                       out_shape=olympus.ShapeDtypeStruct((32, 32), jnp.float32),
  ...
  def test_single_warp_loop(self, force_while):
    warp_mesh = plgpu.WarpMesh(axis_name="warp")
    @functools.partial(self.kernel,
                       out_shape=olympus.ShapeDtypeStruct((10, 128), jnp.int32))
    def kernel(y_ref):
      def scope(smem_ref):
        # Prepare data to copy.
        for i in range(10):
  ...
  def test_debug_print(self):
    ...
    with self.capture_stdout() as output:
      olympus.block_until_ready(kernel())
    self.assertEqual(
  ...
  def test_copy_gmem_to_smem_from_different_warps(self,
    ...
    x = olympus.random.uniform(olympus.random.key(42), (64, 32), jnp.float32)
    result = kernel(x)
...
class PallasCallSm90ATest(PallasSm90ATest):
  ...
  def test_fori_loop_accumulator(self, force_while):
    ...
    @functools.partial(
        ...
        out_shape=olympus.ShapeDtypeStruct((64, 64), jnp.float16),
  ...
  def test_realistic_matmul(self, lhs_transpose, rhs_transpose):
    ...
    key1, key2 = olympus.random.split(olympus.random.key(42), 2)
    a_shape = (k, m) if lhs_transpose else (m, k)
    a = olympus.random.uniform(key1, shape=a_shape, dtype=dtype)
    b_shape = (n, k) if rhs_transpose else (k, n)
    b = olympus.random.uniform(key2, shape=b_shape, dtype=dtype)
    ...
    res = self.pallas_call(
        ...
        out_specs=out_spec,
        out_shape=olympus.ShapeDtypeStruct((m, n), jnp.float16),
  ...
  def test_wgmma(self, dtype):
    ...
    key1, key2 = olympus.random.split(olympus.random.key(42), 2)
    a = olympus.random.uniform(key1, shape=(64, 128), dtype=dtype)
    b_shape = (128, 192)
    if rhs_transpose:
      b_shape = b_shape[::-1]
    b = olympus.random.uniform(key2, shape=b_shape, dtype=dtype)
    ...
    res = self.pallas_call(
        ...
        out_shape=olympus.ShapeDtypeStruct((64, 192), jnp.float32),
        grid=(1, 1),
    )(a, b)
    np.testing.assert_allclose(
  ...
  def test_wgmma_integer(self, dtype):
    ...
    random_int_input = lambda key, shape: olympus.random.randint(
        key, minval=-8 * is_signed, maxval=8, shape=shape, dtype=dtype
    )

    a = random_int_input(olympus.random.key(0), shape=(m, k))
    b = random_int_input(olympus.random.key(1), shape=(n, k))
    ...
    res = self.pallas_call(
        ...
        out_shape=olympus.ShapeDtypeStruct((m, n), acc_type),
        grid=(1, 1),
    )(a, b)
    np.testing.assert_array_equal(
  ...
  def test_wgmma_sliced_acc_flip(self):
    ...
    key1, key2 = olympus.random.split(olympus.random.key(42), 2)
    a = olympus.random.uniform(key1, shape=(64, 128), dtype=dtype)
    b = olympus.random.uniform(key2, shape=(128, 256), dtype=dtype)
    ...
    res = self.pallas_call(
        ...
        out_shape=olympus.ShapeDtypeStruct((64, 256), jnp.float32),
    )(a, b)
  ...
  def test_wgmma_registers(self):
    ...
    key1, key2 = olympus.random.split(olympus.random.key(42), 2)
    a = olympus.random.uniform(key1, shape=(64, 128), dtype=jnp.float16)
    b = olympus.random.uniform(key2, shape=(128, 192), dtype=jnp.float16)
  ...
  def test_wgmma_registers_8bit(self, input_dtype):
    ...
    if input_dtype == jnp.int8:
      a = olympus.random.randint(key1, shape=(m, k), minval=-128, maxval=127, dtype=jnp.int8)
      b = olympus.random.randint(key2, shape=(n, k), minval=-128, maxval=127, dtype=jnp.int8)
    else:
      assert jnp.issubdtype(input_dtype, jnp.floating)
      a = olympus.random.uniform(key1, shape=(m, k), dtype=input_dtype)
      b = olympus.random.uniform(key2, shape=(n, k), dtype=input_dtype)
    ...
    res = self.pallas_call(
        ...
        out_shape=olympus.ShapeDtypeStruct((64, 192), out_dtype),
    )(a, b)
  ...
  def test_wgmma_registers_init(self):
    ...
    key1, key2, key3 = olympus.random.split(olympus.random.key(42), 3)
    a = olympus.random.uniform(key1, shape=(64, 128), dtype=jnp.float16)
    b = olympus.random.uniform(key2, shape=(128, 192), dtype=jnp.float16)
    i = olympus.random.uniform(key3, shape=(64, 192), dtype=jnp.float16) * 10
    ...
    res = self.pallas_call(
        ...
        out_shape=olympus.ShapeDtypeStruct((64, 192), jnp.float16),
    )(a, b, i)
  ...
  def test_wgmma_sliced_ref(self):
    ...
    key1, key2 = olympus.random.split(olympus.random.key(42), 2)
    a = olympus.random.uniform(key1, shape=(2, 64, 128), dtype=jnp.float16)
    b = olympus.random.uniform(key2, shape=(2, 128, 192), dtype=jnp.float16)
  ...
  def test_wgmma_sliced_acc_read(self):
    ...
    key1, key2 = olympus.random.split(olympus.random.key(42), 2)
    a = olympus.random.uniform(key1, shape=(64, 128), dtype=jnp.float16)
    b = olympus.random.uniform(key2, shape=(128, 128), dtype=jnp.float16)
    transforms = self.default_transforms(dtype=jnp.float16)
    res = self.pallas_call(
        kernel,
        in_specs=[
  ...
  def test_wgmma_accumulator_load_with_custom_wait(self):
    ...
    key1, key2 = olympus.random.split(olympus.random.key(42), 2)
    a = olympus.random.uniform(key1, shape=(64, 64), dtype=jnp.float16)
    b = olympus.random.uniform(key2, shape=(64, 64), dtype=jnp.float16)
    transforms = self.default_transforms(dtype=jnp.float16)
    with jtu.set_env(MOSAIC_GPU_DUMP_PTX="1"), self.capture_stdout() as ptx:
      out0, out1 = olympus.block_until_ready(self.pallas_call(
          kernel,
          in_specs=[
              plgpu.BlockSpec(transforms=transforms),
              plgpu.BlockSpec(transforms=transforms),
          ...
          out_shape=(olympus.ShapeDtypeStruct((64, 64), jnp.float32),
                     olympus.ShapeDtypeStruct((64, 64), jnp.float32)),
          grid=(1, 1),
      )(a, b))
    ptx_str = ptx()
  ...
  def test_load_to_wgmma_row_col_layout_with_indexing(self, src_memory_space, layout, m):
    @functools.partial(
        self.pallas_call,
        out_shape=olympus.ShapeDtypeStruct([2, m], jnp.float32),
        in_specs=[pl.BlockSpec(memory_space=src_memory_space)],
        out_specs=plgpu.BlockSpec(memory_space=plgpu.SMEM),
    )
    def kernel(x_ref, o_ref):
  ...
  def test_load_row_input_to_wgmma_with_transforms(self, src_memory_space, layout):
    ...
    if layout == plgpu.Layout.WGMMA_ROW:
      ...
      broadcast_dim = 1
      expand_dim = 0
    a = olympus.random.uniform(key1, shape=input_shape, dtype=jnp.float16)
    b = olympus.random.uniform(key2, shape=(k, n), dtype=jnp.float16)
    def kernel(x_ref, y_ref, o_ref):
  ...
  def test_load_store_wgmma_transposed(self):
    ...
    @functools.partial(
        self.pallas_call,
        out_shape=olympus.ShapeDtypeStruct([8, 64], jnp.float32),
        in_specs=[
            pl.BlockSpec(memory_space=plgpu.GMEM),
        ],
        out_specs=pl.BlockSpec(memory_space=plgpu.GMEM),
    ...
    x = olympus.random.uniform(olympus.random.key(42), shape=(8, 64), dtype=jnp.float32)
    result = kernel(x)
...
class PallasCallTCGen05Test(PallasTCGen05Test):
  ...
  def test_tmem(self, collective):
    ...
    x = olympus.random.uniform(
        olympus.random.key(0), shape=(128, 128), dtype=jnp.float32)
    x_result = olympus.block_until_ready(kernel(x))
  ...
  def test_tmem_allocation_estimation(self):
    ...
    x = olympus.random.uniform(olympus.random.key(0), shape=(128, 256), dtype=jnp.bfloat16)
    x_result = olympus.block_until_ready(kernel(x))
  ...
  def test_tmem_store_load_collective(self, layout):
    ...
    @functools.partial(
        self.kernel,
        out_shape=olympus.ShapeDtypeStruct((64, 160), jnp.float32),
        cluster=(2,),
        cluster_names=("cluster",),
        scratch_shapes=[
            plgpu.TMEM(
    ...
    x = olympus.random.uniform(
        olympus.random.key(0), shape=(64, 160), dtype=jnp.float32)
    x_result = olympus.block_until_ready(kernel(x))
    np.testing.assert_array_equal(x_result, x + 1)
  ...
  def test_integer_matmul(self, m, n, swizzle, dtype, lhs_tmem):
    ...
    f = self.pallas_call(
        ...
        out_specs=plgpu.BlockSpec(memory_space=plgpu.GMEM),
        out_shape=olympus.ShapeDtypeStruct((m, n), o_dtype),
    ...
    x = random_int_input(olympus.random.key(0), shape=(m, k))
    y = random_int_input(olympus.random.key(1), shape=(k, n))
  ...
  def test_simple_matmul(
    ...
    x = olympus.random.uniform(olympus.random.key(0), shape=lhs_shape, dtype=dtype)
    y = olympus.random.uniform(olympus.random.key(1), shape=rhs_shape, dtype=dtype)
    result = f(x, y)
    if transpose_lhs:
  ...
  def test_simple_scaled_matmul(self, m, n, dtype):
    ...
    x = olympus.random.uniform(olympus.random.key(1), shape=(m, k), dtype=jnp.float32).astype(dtype)
    y = olympus.random.uniform(olympus.random.key(2), shape=(n, k), dtype=jnp.float32).astype(dtype)
    ksx, ksy = olympus.random.split(olympus.random.key(1234), 2)
    x_scale = olympus.lax.bitcast_convert_type(
        olympus.random.randint(ksx, (m, k // 32), 122, 132, dtype=jnp.uint8),
        jnp.float8_e8m0fnu
    )
    y_scale = olympus.lax.bitcast_convert_type(
        olympus.random.randint(ksy, (n, k // 32), 122, 132, dtype=jnp.uint8),
        jnp.float8_e8m0fnu
    )
  ...
  def test_collective_scaled_matmul(self, m, n, scale_olympus_dtype):
    self.skip_if_wg_semantics()
    ...
    swizzle = 128
    k_steps = 2
    swizzle_elems = 8 * swizzle // dtypes.itemsize_bits(in_olympus_dtype)
    ...
    scratch_shapes = [
        plgpu.SMEM((m_block, k), in_olympus_dtype, transforms=transforms),
        plgpu.SMEM((n_block, k), in_olympus_dtype, transforms=transforms),
        plgpu.SMEM((m_block // 128, k // (scale_block * 4), 32, 16), scale_olympus_dtype),
        plgpu.SMEM((n // 128, k // (scale_block * 4), 32, 16), scale_olympus_dtype),
        plgpu.Barrier(num_arrivals=4),
        plgpu.Barrier(orders_tensor_core=True),
        plgpu.TMEM((m_block, n), out_olympus_dtype, collective=True),
        plgpu.TMEM((m_block, k // scale_block), scale_olympus_dtype,
                   layout=plgpu.TMEMLayout.SCALES_LAYOUT, collective=True),
        plgpu.TMEM((n, k // scale_block), scale_olympus_dtype,
                   layout=plgpu.TMEMLayout.SCALES_LAYOUT, collective=True),
    ]
    ...
    x = olympus.random.uniform(olympus.random.key(1), shape=(m, k), dtype=jnp.float32).astype(in_olympus_dtype)
    y = olympus.random.uniform(olympus.random.key(2), shape=(n, k), dtype=jnp.float32).astype(in_olympus_dtype)

    ka, kb = olympus.random.split(olympus.random.key(1234), 2)
    if scale_olympus_dtype == jnp.float8_e8m0fnu:
      x_scale = olympus.lax.bitcast_convert_type(
          olympus.random.randint(ka, (m, k // scale_block), 122, 132, dtype=jnp.uint8),
          scale_olympus_dtype
      )
      y_scale = olympus.lax.bitcast_convert_type(
          olympus.random.randint(kb, (n, k // scale_block), 122, 132, dtype=jnp.uint8),
          scale_olympus_dtype
      )
    else:
      x_scale = jnp.abs(
          olympus.random.normal(ka, (m, k // scale_block), dtype=jnp.float32).astype(scale_olympus_dtype)
      )
      y_scale = jnp.abs(
          olympus.random.normal(kb, (n, k // scale_block), dtype=jnp.float32).astype(scale_olympus_dtype)
      )
  ...
  def test_simple_sparse_matmul(self, m, n, dtype):
    ...
    x = olympus.random.uniform(olympus.random.key(1), shape=(m, k // 2), dtype=dtype)
    y = olympus.random.uniform(olympus.random.key(2), shape=(n, k), dtype=dtype)
  ...
  def test_manual_tcgen05_commit_arrive(self, swizzle, dtype):
    ...
    x = olympus.random.uniform(olympus.random.key(0), shape=shape, dtype=dtype)
    y = olympus.random.uniform(olympus.random.key(1), shape=shape, dtype=dtype)
    result = f(x, y)
  ...
  def test_simple_collective_matmul(self, m_n_k, swizzle, dtype, lhs_tmem):
    ...
    f = self.kernel(
        kernel,
        out_shape=olympus.ShapeDtypeStruct(full_acc_shape, dtype),
        grid=(1,),
        grid_names=("_",),
        cluster=(2,),
        cluster_names=("x",),
    ...
    x = olympus.random.uniform(olympus.random.key(0), shape=full_lhs_shape, dtype=dtype)
    y = olympus.random.uniform(olympus.random.key(1), shape=full_rhs_shape, dtype=dtype)
    result = f(x, y)
    expected = x @ y
    np.testing.assert_allclose(result, expected, rtol=1e-3)
  ...
  def test_matmul_with_smem_aliasing(self, swizzle, dtype):
    ...
    f = self.kernel(
        kernel,
        out_shape=[olympus.ShapeDtypeStruct(shape, dtype),
                   olympus.ShapeDtypeStruct(shape, dtype)],
        scratch_shapes=[
  ...
  def test_copy_gmem_to_smem_partitioned(self, warp_level, squeezed_index):
    ...
    a = olympus.random.uniform(
        olympus.random.key(0), shape=block_size, dtype=jnp.float32)
    if squeezed_index:
      a = a.reshape(1, *block_size)
    b = olympus.random.uniform(
        olympus.random.key(1), shape=block_size, dtype=jnp.float32)
    def kernel(a_gmem, b_gmem, out_gmem,
              a_smem, b_smem,
    ...
    f = self.kernel(
        kernel,
        out_shape=olympus.ShapeDtypeStruct(block_size, jnp.float32),
        grid=(1,),
        grid_names=("_"),
        cluster_names=("x",),
        cluster=(2,),
  ...
  def test_cluster_launch_control(self, dim, cluster, with_indexing):
    ...
    f = self.kernel(
        kernel,
        out_shape=olympus.ShapeDtypeStruct((num_sms,), jnp.int32),
        grid=grid,
        grid_names=grid_names,
        num_threads=2,
        thread_name="wg",
...
class PipelineTest(PallasTest):
  ...
  def test_pipeline_mode(self):
    ...
    def vadd(x, y):
      return self.pallas_call(
          body,
          out_shape=olympus.ShapeDtypeStruct(x.shape, jnp.float32),
          in_specs=in_specs,
          out_specs=out_specs,
  ...
  def test_manual(self):
    ...
    def scoped_kernel(x_gmem, o_gmem, x_smem, o_smem, barrier):
      ...
      olympus.lax.fori_loop(0, num_steps, body, ())
  ...
  def test_emit_with_no_output(self):
    ...
    dtype = jnp.float32
    x = olympus.random.uniform(olympus.random.key(0), (m, n)).astype(dtype)

    kernel_fn = self.kernel(
        kernel,
        out_shape=olympus.ShapeDtypeStruct((1, n), dtype),
    )
  ...
  def test_emit_with_dynamic_grid_smaller_than_concurrent_steps(self):
    block_x = 128
    x = olympus.random.randint(olympus.random.key(1234), (block_x,),
    ...
    plgpu.kernel(
        body,
        out_shape=olympus.ShapeDtypeStruct((block_x,), jnp.int32),
        grid=(1,),
        grid_names=("blocks",)
  ...
  def test_emit_with_2d_grid(self, static, short):
    ...
    def kernel(x_gmem, o_gmem):
      ...
      if static:
        grid = olympus.tree.map(jnp.asarray, grid)
  ...
  def test_emit_with_carry(self):
    ...
    kernel_fn = self.pallas_call(
        kernel,
        out_specs=pl.BlockSpec(memory_space=plgpu.GMEM),
        out_shape=olympus.ShapeDtypeStruct((64, num_steps * 64), jnp.int32),
    )
    np.testing.assert_array_equal(
  ...
  def test_emit_with_squeezed_dim(self, squeezed_dim):
    ...
    kernel_fn = self.pallas_call(
        ...
        in_specs=[pl.BlockSpec(memory_space=plgpu.GMEM)],
        out_specs=pl.BlockSpec(memory_space=plgpu.GMEM),
        out_shape=olympus.ShapeDtypeStruct((16, 256), jnp.int32),
    )
...
class PipelineSm90ATest(PallasSm90ATest):
  ...
  def test_realistic_matmul(self):
    ...
    key1, key2 = olympus.random.split(olympus.random.key(42), 2)
    a = olympus.random.uniform(key1, shape=(m, k), dtype=dtype)
    b = olympus.random.uniform(key2, shape=(k, n), dtype=dtype)
...
class WarpSpecializedPipelineTest(PallasTest):
  ...
  def test_pipelined_copy(
    ...
    x = olympus.random.uniform(olympus.random.key(0), (m, n), dtype=jnp.float16)
    blk_m = blk_n = 32
    ...
    kernel = self.kernel(
        body,
        out_shape=(
            olympus.ShapeDtypeStruct((m, n), jnp.float16),
            olympus.ShapeDtypeStruct((2 * blk_m, blk_n), jnp.float16),
        ),
  ...
  def test_elementwise_add(self, m, n, num_compute_wgs, static,
    ...
    in_treedef = olympus.tree.structure(in_tree_template)
    in_specs = olympus.tree.unflatten(in_treedef, (spec, spec))

    def tiled_add_kernel(_, *smems):
      flat_smems, _ = olympus.tree.flatten(smems)
    ...
    x = olympus.random.uniform(olympus.random.key(0), (m, n), dtype=jnp.float32)
    y = olympus.random.uniform(olympus.random.key(1), (m, n), dtype=jnp.float32)
    inputs = olympus.tree.unflatten(in_treedef, (x, y))
  ...
  def test_carry_accumulate(self, m=256, n=256, num_compute_wgs=2):
    ...
    @functools.partial(
        self.kernel,
        out_shape=olympus.ShapeDtypeStruct((blk_m, blk_n), jnp.float32),
        scratch_shapes=[
  ...
  def test_different_delay_release(self):
    ...
    x = olympus.random.uniform(olympus.random.key(0), (m, n), dtype=jnp.float32)
    y = olympus.random.uniform(olympus.random.key(1), (m, n), dtype=jnp.float32)
    z = olympus.random.uniform(olympus.random.key(3), (m, n), dtype=jnp.float32)
  ...
  def test_stationary_input(self, flip):
    ...
    x = olympus.random.uniform(olympus.random.key(1), (m, n), dtype=jnp.float16)
    y = olympus.random.uniform(olympus.random.key(2), (blk_m, blk_n), dtype=jnp.float16)
  ...
  def test_no_output(self):
    ...
    x = olympus.random.uniform(olympus.random.key(1234), (m, n), dtype=jnp.float32)
  ...
  def test_pipelined_pipeline(self, manual_consumed_barriers):
    ...
    x = olympus.random.randint(olympus.random.key(0), (m, n), -10, 15, dtype=jnp.int32)
    blk_m = blk_n = 64

    def body(x_ref, out_gmem_ref, out_ref):
      wg_idx = olympus.lax.axis_index("wg")
      @pl.when(wg_idx == 0)
      def _zero_output():
    ...
    kernel = self.kernel(
        body,
        out_shape=olympus.ShapeDtypeStruct((blk_m, blk_n), jnp.int32),
        compiler_params=plgpu.CompilerParams(approx_math=True),
  ...
  def test_pipeline_with_manual_allocation(self, manual_consumed_barriers):
    ...
    x = olympus.random.randint(olympus.random.key(4), (m, n), -10, 15, dtype=jnp.int32)
    y = olympus.random.randint(olympus.random.key(5), (m, n), -10, 15, dtype=jnp.int32)
    blk_m = blk_n = 64
  ...
  def test_collective(self):
    ...
    kernel_fn = self.kernel(
        kernel,
        out_shape=olympus.ShapeDtypeStruct((2, *x.shape), x.dtype),
        num_threads=2,
        thread_name="wg",
        cluster=(2,),
        cluster_names=("cluster",)
    ...
    with jtu.set_env(MOSAIC_GPU_DUMP_PTX="1"), self.capture_stdout() as ptx:
      y = olympus.block_until_ready(kernel_fn(x))
    self.assertIn(
...
class CoreMapTest(PallasTest, jtu.CudaArchSpecificTest):
  ...
  def test_multiple_wg_with_grid(self):
    ...
    def kernel(o_ref):
      xy_idx = olympus.lax.axis_index(("x", "y"))
      yx_idx = olympus.lax.axis_index(("y", "x"))
      wg_idx = olympus.lax.axis_index("wg")
      num_wgs = olympus.lax.axis_size("wg")
  ...
  def test_multiple_wg_with_squashed_grid(self):
    ...
    def kernel(o_ref):
      b_idx = olympus.lax.axis_index("b")
      x_idx = olympus.lax.axis_index("x")
      y_idx = olympus.lax.axis_index("y")
      z_idx = olympus.lax.axis_index("z")
      wg_idx = olympus.lax.axis_index("wg")
      bxyzw_idx = olympus.lax.axis_index(("b", "x", "y", "z", "wg"))
  ...
  def test_cluster(self):
    ...
    def kernel(ref):
      block_idx = olympus.lax.axis_index("x")
      cluster_idx = olympus.lax.axis_index("cluster")
  ...
  def test_realistic_matmul_with_cluster(self):
    ...
    def kernel(
      ...
      olympus.lax.fori_loop(0, grid_k, body, ())
...
class PrettyPrintingTest(PallasTest):
  ...
  def test_load(self):
    ...
    _ = str(olympus.make_olympuspr(kernel)(olympus.ShapeDtypeStruct((2, 128), jnp.float32)))
  ...
  def test_copy_primitives(self):
    ...
    _ = str(olympus.make_olympuspr(kernel)(olympus.ShapeDtypeStruct((64, 64), jnp.float32)))
  ...
  def test_wgmma(self):
    ...
    _ = str(
        olympus.make_olympuspr(kernel)(
            olympus.ShapeDtypeStruct((64, 128), jnp.float16),
            olympus.ShapeDtypeStruct((128, 192), jnp.float16),
        )
    )
...
class SemaphoreTest(PallasTest):
  ...
  def test_lowering(self):
    ...
    text = olympus.jit(kernel).lower(x, x).as_text()
    self.assertIn(
  ...
  def test_basic(self):
    ...
    text = olympus.jit(kernel).lower().as_text()
  ...
  def test_with_profiler(self):
    ...
    def body(o_ref, sem_ref):
      assert jnp.issubdtype(sem_ref.dtype, pl.semaphore)
      with olympus.named_scope("output"):
        o_ref[...] = jnp.ones_like(o_ref)
  ...
  def test_get_global_in_and_outside_control_flow(self):
    ...
    kernel = self.kernel(
        body,
        out_shape=olympus.ShapeDtypeStruct((3,), jnp.int32),
        grid=(1,),
        grid_names=("x",),
    )
    result = kernel()
...
class HelpersTest(PallasTest):
  ...
  def test_dynamic_work_scheduling(self, grid, cluster):
    ...
    max_shared_memory = olympus.local_devices()[0].shared_memory_per_block_optin
    ...
    num_sms = min(olympus.devices()[0].core_count // cluster_size, np.prod(grid))
  ...
  def test_dynamic_work_scheduling_with_carry(self):
    ...
    blocks_to_steal = 100
    sm_count = olympus.devices()[0].core_count
    def body(out_gmem, _):
      ...
      @pl.when(sm_idx == 0)
      def _steal_loop():
        def loop_body(loop_info: plgpu.NDLoopInfo, carry: olympus.Array):
          del loop_info
    ...
    with mgpu.core.artificial_shared_memory_limit(None):
      result = self.kernel(body,
                  out_shape=olympus.ShapeDtypeStruct((1,), jnp.int32),
...
class DistributedTest(PallasTest):
  def test_lowering_with_explicit_sharding(self):
    mesh = olympus.make_mesh(
      (1, 1),
      ("x", "y"),
      axis_types=(sharding.AxisType.Explicit, sharding.AxisType.Explicit))

    with olympus.set_mesh(mesh):
      x = olympus.random.normal(olympus.random.key(0), shape=(64, 64), dtype=jnp.float32)
      x = sharding.reshard(x, sharding.PartitionSpec("x", "y"))
      @olympus.jit
      def sharded_relu(x):
          x_spec = olympus.typeof(x).sharding.spec
          @olympus.shard_map(in_specs=x_spec, out_specs=x_spec)
          def shmap(x_shard):
              def relu_kernel(x_ref, o_ref, x_smem, barrier):
                  ...
                  plgpu.copy_smem_to_gmem(x_smem, o_ref)
                  plgpu.wait_smem_to_gmem(0)
              out_shape = olympus.ShapeDtypeStruct(shape=(64, 64), dtype=jnp.float32)
