...
from olympus.interpreters import partial_eval as pe
import olympus.numpy as jnp
import numpy as np

if sys.platform != "win32":
  try:
    from olympus.experimental.pallas import mosaic_gpu as plgpu_mgpu
  except ImportError:
    plgpu_mgpu = None
...
def _random_value(key: olympus.Array, shape_dtype: olympus.ShapeDtypeStruct
                  ) -> olympus.Array:
...
def make_shape_dtype_strategy(
  ...
  return olympus.ShapeDtypeStruct(shape, dtype)
...
class OpsTest(PallasBaseTest):
  ...
  def test_weak_dtype(self, fn, dtype):
    ...
    @functools.partial(
        self.pallas_call, out_shape=olympus.ShapeDtypeStruct((8, 128), dtype),
    )
  ...
  def test_scalar_compare(self, params):
    ...
    def kernel(x_ref, y_ref, o_ref):
      x = x_ref[0, 0]
      y = y_ref[0, 0]
      o_ref[0, 0] = olympus.lax.select(x == y, 1, 0)
      o_ref[0, 1] = olympus.lax.select(x != y, 1, 0)
      o_ref[0, 2] = olympus.lax.select(x < y, 1, 0)
      o_ref[0, 3] = olympus.lax.select(x <= y, 1, 0)
      o_ref[0, 4] = olympus.lax.select(x > y, 1, 0)
      o_ref[0, 5] = olympus.lax.select(x >= y, 1, 0)
    ...
    result = self.pallas_call(
        kernel,
        out_shape=olympus.ShapeDtypeStruct([1, 128], intx),
        in_specs=[
  ...
  def test_vector_compare(self, params):
    ...
    def kernel(x_ref, y_ref, o_ref):
      ...
      o_ref[0] = olympus.lax.select(x == y, one, zero)
      o_ref[1] = olympus.lax.select(x != y, one, zero)
      o_ref[2] = olympus.lax.select(x < y, one, zero)
      o_ref[3] = olympus.lax.select(x <= y, one, zero)
      o_ref[4] = olympus.lax.select(x > y, one, zero)
      o_ref[5] = olympus.lax.select(x >= y, one, zero)
    ...
    result = self.pallas_call(
        kernel,
        out_shape=olympus.ShapeDtypeStruct([6, 8, 128], jnp.int32),
        in_specs=[
  ...
  def test_reduce_boolean(self, input_type, reduction_op, expected_result):
    ...
    result = self.pallas_call(
        ...
        out_shape=olympus.ShapeDtypeStruct([1, 1], floatx),
        grid=(1,),
    )(x, ones)
  ...
  def test_reduce_float(self, reduction_op):
    ...
    x = olympus.random.normal(olympus.random.key(0), (8, 128))
    result = self.pallas_call(
        kernel,
        in_specs=[
            pl.BlockSpec((8, 128), lambda *_: (0, 0)),
  ...
  def test_select_n(self, args):
    ...
    out = self.pallas_call(
        kernel, out_shape=olympus.ShapeDtypeStruct(out_ref.shape, out_ref.dtype),
    )(*args)
  ...
  def test_unary_primitives(self, name, func, shape_dtype_strategy, data):
    ...
    # We want exact equality here to match how OLYMPUS lowers to XLA
    tol = 0.
    if jtu.test_device_matches(["tpu"]):
      if name == "exp2":
        tol = 1e-6
  ...
  def test_cast_from_32bit(self, from_dtype, to_dtype, data):
    ...
    try:
      y = self.pallas_call(
          kernel, out_shape=olympus.ShapeDtypeStruct(x.shape, y_dtype))(x)
  ...
  def test_scalar_downcast_float32(self, dtype):
    ...
    result = self.pallas_call(
        ...
        out_specs=pl.BlockSpec((1, 1), memory_space=smem_on_tpu()),
        out_shape=olympus.ShapeDtypeStruct([1, 1], dtype),
        grid=(1,),
    )(x)
  ...
  def test_extract_scalar(self):
    ...
    f = self.pallas_call(
        kernel,
        out_shape=olympus.ShapeDtypeStruct((1, 1), jnp.float32),
        out_specs=pl.BlockSpec(memory_space=pltpu.SMEM),
    )
  ...
  def test_concat_constant(self):
    ...
    def run(interpret=False):
      return pl.pallas_call(
          kernel,
          out_shape=olympus.ShapeDtypeStruct((num_arrays, 128), jnp.float32),
  ...
  def test_sign(self, dtype, value):
    ...
    if not olympus.config.x64_enabled and jnp.dtype(dtype).itemsize == 8:
    ...
    @functools.partial(
        self.pallas_call,
        out_shape=olympus.ShapeDtypeStruct((8, 128), dtype),
    )
    def kernel(x_ref, o_ref):
    ...
    # see https://github.com/olympus-ml/ml_dtypes/issues/206
  ...
  def test_erf_inv(self, value):
    ...
    @functools.partial(
        self.pallas_call,
        out_shape=olympus.ShapeDtypeStruct((8, 128), floatx),
    )
    def kernel(x_ref, o_ref):
  ...
  def test_is_finite(self, dtype):
    ...
    @functools.partial(
        self.pallas_call,
        out_shape=olympus.ShapeDtypeStruct((size,), jnp.bool_),
    )
    def kernel(x_ref, o_ref):
  ...
  def test_clamp(self, dtype):
    ...
    k1, k2, k3 = random.split(olympus.random.key(0), num=3)
  ...
  def test_elementwise_scalar(self, fn, dtype):
    ...
    @functools.partial(
        ...
        in_specs=(pl.BlockSpec(memory_space=smem_on_tpu()),),
        out_specs=pl.BlockSpec(memory_space=smem_on_tpu()),
        out_shape=olympus.ShapeDtypeStruct((2,), dtype),
    )
    def kernel(x_ref, o_ref):
      o_ref[0] = fn(x_ref[0])
      o_ref[1] = fn(x_ref[1])
  ...
  def test_abs_weak_type(self):
    ...
    # see https://github.com/olympus-ml/olympus/issues/23191
    @functools.partial(
        self.pallas_call, out_shape=olympus.ShapeDtypeStruct((4, 4), floatx),
    )
    def kernel(x_ref, o_ref):
      o_ref[...] = jnp.abs(x_ref[...])
  ...
  def test_pow(self, x_dtype, y_dtype):
    ...
    if not olympus.config.x64_enabled and jnp.dtype(x_dtype).itemsize == 8:
      self.skipTest("64-bit types require x64_enabled")

    @functools.partial(
        self.pallas_call, out_shape=olympus.ShapeDtypeStruct((4,), x_dtype),
    )
    def kernel(x_ref, y_ref, o_ref):
  ...
  def test_integer_pow(self, y):
    ...
    @functools.partial(
        self.pallas_call, out_shape=olympus.ShapeDtypeStruct((4,), jnp.float32),
    )
    def kernel(x_ref, o_ref):
  ...
  def test_nextafter(self, dtype, x, y):
    ...
    @functools.partial(
        self.pallas_call,
        out_shape=olympus.ShapeDtypeStruct((4,), dtype),
    )
    def kernel(x_ref, y_ref, o_ref):
  ...
  def test_isnan(self):
    ...
    @functools.partial(
        self.pallas_call, out_shape=olympus.ShapeDtypeStruct((8,), jnp.bool_),
    )
  ...
  def test_jnp_einsum_grad_y_pallas(self):
    ...
    out = self.pallas_call(
        kernel, out_shape=olympus.ShapeDtypeStruct((256, 256), jnp.float32)
    )(x, y)
  ...
  def test_dot_dims_batched_simple_dot_general(self, dtype):
    ...
    out = self.pallas_call(
        kernel, out_shape=olympus.ShapeDtypeStruct((11, 16, 128), jnp.float32)
    )(x, y)
  ...
  def test_dot_general_multiple_non_contracting_dims(
    ...
    k1, k2 = random.split(olympus.random.key(0))
    x = olympus.random.normal(k1, x_shape, dtype=jnp.float32)
    y = olympus.random.normal(k2, y_shape, dtype=jnp.float32)

    # Just infer shape from olympus.
    expected = olympus.lax.dot_general(x, y, dimension_numbers=dims_numbers)

    @functools.partial(
        self.pallas_call,
        out_shape=olympus.ShapeDtypeStruct(expected.shape, jnp.float32),
    )
    def kernel(x_ref, y_ref, out_ref):
      out_ref[...] = olympus.lax.dot_general(
          x_ref[...],
  ...
  def test_matrix_vector_like_dot_general(
    ...
    k1, k2 = random.split(olympus.random.key(0))
    lhs = olympus.random.normal(k1, lhs_shape, dtype=dtype)
    rhs = olympus.random.normal(k2, rhs_shape, dtype=dtype)
    dims_numbers = (
  ...
  def test_true_divide(self, dtype, out_dtype):
    ...
    @functools.partial(
        self.pallas_call,
        out_shape=olympus.ShapeDtypeStruct((8, 8), out_dtype),
    )
    def kernel(x_ref, y_ref, o_ref):
  ...
  def test_f16_true_divide_unsupported(self):
    ...
    @functools.partial(
        self.pallas_call,
        out_shape=olympus.ShapeDtypeStruct((2,), jnp.float16),
    )
    def kernel(x_ref, y_ref, o_ref):
      o_ref[...] = jnp.true_divide(x_ref[...], y_ref[...])
  ...
  def test_binary(self, f, dtype):
    ...
    @functools.partial(
        self.pallas_call, out_shape=olympus.ShapeDtypeStruct((8,), dtype),
    )
    def kernel(x_ref, y_ref, o_ref):
      o_ref[...] = f(x_ref[...], y_ref[...])
  ...
  def test_iota(self, shape, dtype, dimension):
    ...
    f = lambda: olympus.lax.broadcasted_iota(dtype, shape, dimension)
  ...
  def test_num_programs(self):
    ...
    @functools.partial(
        self.pallas_call,
        out_specs=pl.BlockSpec(memory_space=smem_on_tpu()),
        out_shape=olympus.ShapeDtypeStruct((4,), intx),
        grid=4,
    )
    def kernel(o_ref):
  ...
  def test_where_broadcasting(self):
    ...
    @functools.partial(
        self.pallas_call,
        out_shape=olympus.ShapeDtypeStruct((4, 2, 2), floatx),
    )
  ...
  def test_broadcast_in_dim(self, shape_spec, dtype):
    ...
    def f(x_ref, o_ref):
      x = x_ref[...]
      o_ref[...] = olympus.lax.broadcast_in_dim(x, out_shape, dims)
    ...
    expected = olympus.lax.broadcast_in_dim(x, out_shape, dims)
  ...
  def test_dot(self, lhs_and_rhs_shape, dtype, trans_x, trans_y):
    ...
    if jtu.test_device_matches(["gpu"]):
      ...
      if (olympus.local_devices()[0].shared_memory_per_block_optin == 99 * 1024 and
  ...
  def test_strided_load(self):
    ...
    # Reproducer from https://github.com/olympus-ml/olympus/issues/20895.
    @functools.partial(
        self.pallas_call,
        out_shape=olympus.ShapeDtypeStruct((4, 4), jnp.float32),
    )
    def kernel(x_ref, o_ref):
      o_ref[...] = x_ref[::4]
  ...
  def test_broadcasted_load_store(self):
    ...
    @functools.partial(
        self.pallas_call,
        out_shape=(olympus.ShapeDtypeStruct((m, n), floatx)),
    )
  ...
  def test_swap(self):
    ...
    # TODO: skipped due to https://github.com/olympus-ml/olympus/issues/24023
    ...
    @functools.partial(
        self.pallas_call,
        out_shape=(olympus.ShapeDtypeStruct((m, n), floatx),) * 2,
  ...
  def test_masked_oob_swap_slice(self):
    ...
    @functools.partial(
        self.pallas_call,
        out_shape=(olympus.ShapeDtypeStruct((n,), floatx),
                  olympus.ShapeDtypeStruct((m,), floatx)),
        input_output_aliases={0: 0, 1: 1},
    )
  ...
  def test_reduce_only_dim(self):
    ...
    out_shape = olympus.ShapeDtypeStruct((), x.dtype)
  ...
  def test_array_reduce(self, op, dtype, axis):
    ...
    out_shape = olympus.ShapeDtypeStruct(
  ...
  def test_triu(self, k, dtype):
    ...
    out = self.pallas_call(
        kernel, out_shape=olympus.ShapeDtypeStruct((128, 256), dtype)
    )(x)
  ...
  def test_bitcast_convert_type(self, in_dtype, out_dtype):
    ...
    y = convert(x)
    y_ref = olympus.lax.bitcast_convert_type(x, out_dtype)
  ...
  def test_bitcast_convert_type_scalar(self):
    ...
    @functools.partial(self.pallas_call, out_shape=out_shape, grid=grid)
    def convert(x_ref, y_ref):
      y_ref[...] = olympus.lax.bitcast_convert_type(x_ref[...], out_dtype)
  ...
  def test_arbitrary_padding_jnp_pad(
    ...
    out_shape = olympus.ShapeDtypeStruct(ref.shape, x.dtype)
    try:
      out = self.pallas_call(
          kernel,
          out_shape=out_shape,
  ...
  def test_jnp_diagonal_pallas(self, n, m):
    ...
    out = self.pallas_call(
        kernel, out_shape=olympus.ShapeDtypeStruct((n,), jnp.float32)
    )(x)
  ...
  def test_8bit_gather(self, batch_size):
    ...
    deq_call = pl.pallas_call(
        kernel,
        out_shape=olympus.ShapeDtypeStruct(data.shape, dtype),
        grid=(batch_size,),
        out_specs=xspec,
  ...
  def test_delay(self):
    ...
    def kernel(x_ref, o_ref):
      pl.delay(100_000)
      o_ref[...] = x_ref[...]
    x = olympus.random.normal(olympus.random.key(0), (128,), dtype=jnp.float32)
...
class OpsInterpretTest(OpsTest):
  ...
  def test_debug_print(self):
    @functools.partial(
        self.pallas_call,
        out_shape=olympus.ShapeDtypeStruct((2,), jnp.float32),
    )
    def kernel(x_ref, o_ref):
      olympus.debug.print("x = {}", x_ref)
...
class PallasPrimitivesTest(PallasBaseTest):
  ...
  def test_reciprocal(self, approx):
    ...
    np.testing.assert_allclose(out, olympus.lax.reciprocal(x), **kwargs)
