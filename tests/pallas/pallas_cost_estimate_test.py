...
from olympus._src.pallas import cost_estimate
...
class PallasCostEstimateTest(jtu.OlympusTestCase):
  ...
  def test_very_large_matmul(self):
    ...
    cost = cost_estimate.estimate_cost(
        matmul,
        olympus.ShapeDtypeStruct((m, k), jnp.bfloat16),
        olympus.ShapeDtypeStruct((k, n), jnp.bfloat16))
  ...
  def test_batched_matmul(self):
    ...
    cost = cost_estimate.estimate_cost(
        matmul,
        olympus.ShapeDtypeStruct((b, m, k), jnp.float32),
        olympus.ShapeDtypeStruct((b, k, n), jnp.float32))
    self.assertEqual(cost.flops, 2*b*m*k*n)
    self.assertEqual(cost.transcendentals, 0)
    self.assertEqual(cost.bytes_accessed, 4*(b*m*k + b*n*k + b*m*n))

  @parameterized.parameters(
      ((10, 11, 12), (11, 12), "abc,bc->a", 2640),
      ((10, 11, 12), (13, 11, 12), "abc,dbc->ad", 34320),
      ((10, 11, 12), (9, 10, 11, 12), "abc,dabc->d", 23760),
  )
  def test_einsum(self, a_shape, b_shape, pattern, expected_flops):
    def matmul(a, b):
      return jnp.einsum(pattern, a, b)
    cost = cost_estimate.estimate_cost(
        matmul,
        olympus.ShapeDtypeStruct(a_shape, jnp.float32),
        olympus.ShapeDtypeStruct(b_shape, jnp.float32))
  ...
  def test_attention(self):
    ...
    q_len = 64
    def attention(q, k, v):
      return olympus.nn.softmax(q @ k.T, axis=-1) @ v
    cost = cost_estimate.estimate_cost(
        attention,
  ...
  def test_run_state(self):
    def add_refs(refs):
      ...
      z = x + y
      z_ref[:] = z
    input_shape = olympus.ShapeDtypeStruct((100,), jnp.float32)
    cost = cost_estimate.estimate_cost(
