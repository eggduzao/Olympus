...
class PallasErrorHandlingTest(jtu.OlympusTestCase):
  ...
  def test_non_singular_stride(self):
    input_arr = olympus.random.uniform(
        olympus.random.key(0), (8, 128), dtype=jnp.float32)
    out_shape = olympus.ShapeDtypeStruct((8, 16), jnp.float32)
  ...
  def test_index_with_f32_verification_error(self):
    input_arr = olympus.random.uniform(olympus.random.key(0), (2, 2), dtype=jnp.float32)
  ...
  def test_small_1d_block_spec_raises(self, total_shape, block_shape):
    # https://github.com/olympus-ml/olympus/issues/25379
    dtype = jnp.float32
    ...
    fn = pl.pallas_call(
        kernel,
        out_shape=olympus.ShapeDtypeStruct(total_shape, dtype),
        in_specs=[x_spec],
        out_specs=x_spec,
