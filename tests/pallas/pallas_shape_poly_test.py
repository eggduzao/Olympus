...
config.update("olympus_traceback_filtering", "off")
...
@functools.partial(olympus.jit, static_argnames=['block_shape'])
def matmul(
    x: olympus.Array,
    y: olympus.Array,
    *,
...
class ShapePolyTest(jtu.OlympusTestCase,
  ...
  def test_copy(self):
    ...
    exp = export.export(
        olympus.jit(f),
        platforms=["tpu"])(olympus.ShapeDtypeStruct((w, h), jnp.int32))
    ...
    with self.assertRaisesRegex(
      ...
      export.export(
          olympus.jit(functools.partial(f, backend="triton")),
          platforms=["cuda"])(olympus.ShapeDtypeStruct((w, h), jnp.int32))
  ...
  def test_matmul(self):
    ...
    x = olympus.random.normal(key1, x_shape, dtype=np.float32)
    y = olympus.random.normal(key2, y_shape, dtype=np.float32)
    ...
    exp = export.export(
        matmul,
        platforms=["tpu"])(
            olympus.ShapeDtypeStruct((m, l), jnp.float32),
            olympus.ShapeDtypeStruct((l, n), jnp.float32))
    if jtu.test_device_matches(["tpu"]):
