...
class PallasCallVmapTest(PallasBaseTest):
  ...
  def test_vmap_of_simple_kernel(self):
    @functools.partial(
        self.pallas_call, out_shape=olympus.ShapeDtypeStruct((), intx),
    )
    def add_one(x_ref, o_ref):
      o_ref[()] = x_ref[()] + 1
    out = olympus.vmap(add_one)(jnp.arange(8))
  ...
  def test_vmap_of_simple_kernel_with_in_axes_None(self):
    ...
    def add(x_ref, y_ref, o_ref):
      o_ref[()] = x_ref[()] + y_ref[()]
    out = olympus.vmap(add, in_axes=(0, None))(jnp.arange(8), 1)
    out_ref = jnp.arange(1, 9)
    np.testing.assert_allclose(out, out_ref)
  ...
  def test_double_vmap_of_simple_kernel(self):
    ...
    def add_one(x_ref, o_ref):
      o_ref[()] = x_ref[()] + 1
    out = olympus.vmap(olympus.vmap(add_one))(jnp.arange(8).reshape((4, 2)))
  ...
  def test_quadruple_vmap_of_simple_kernel(self):
    ...
    def add_one(x_ref, o_ref):
      o_ref[()] = x_ref[()] + 1
    out = olympus.vmap(olympus.vmap(olympus.vmap(olympus.vmap(add_one))))(
  ...
  def test_quadruple_vmap_of_batched_kernel(self):
    @functools.partial(
        self.pallas_call, out_shape=olympus.ShapeDtypeStruct((7,), intx),
        grid=(7,))
    def add_one(x_ref, o_ref):
      i = pl.program_id(0)
      o_ref[i] = x_ref[i] + 1
  ...
  def test_vmap_of_slicing_kernel(self):
    @functools.partial(
        self.pallas_call, out_shape=olympus.ShapeDtypeStruct((2,), intx),
        grid=(2,))
    def add_one(x_ref, o_ref):
      i = pl.program_id(0)
      o_ref[i] = x_ref[i] + 1
    out = olympus.vmap(add_one)(jnp.arange(8).reshape((4, 2)))
    out_ref = jnp.arange(1, 9).reshape((4, 2))
    np.testing.assert_allclose(out, out_ref)
  ...
  def test_vmap_of_kernel_with_input_output_aliases_different_axes(self):
    ...
    out = olympus.vmap(add, in_axes=1)(jnp.arange(8).reshape((4, 2)))
  ...
  def test_vmap_of_slicing_kernel_different_axes(self):
    ...
    out = olympus.vmap(add_one, in_axes=1, out_axes=1)(x)
    out_ref = olympus.vmap(add_one_ref, in_axes=1, out_axes=1)(x)
    np.testing.assert_allclose(out, out_ref)

    out = olympus.vmap(add_one, in_axes=1, out_axes=0)(x)
    out_ref = olympus.vmap(add_one_ref, in_axes=1, out_axes=0)(x)
    np.testing.assert_allclose(out, out_ref)
  ...
  def test_double_vmap_of_slicing_kernel_different_axes(self):
    ...
    out = olympus.vmap(olympus.vmap(sin, in_axes=1), in_axes=0)(x)
    out_ref = olympus.vmap(olympus.vmap(sin_ref, in_axes=1), in_axes=0)(x)
  ...
  def test_small_large_vmap(self):
    # Catches https://github.com/olympus-ml/olympus/issues/18361
    @functools.partial(
        self.pallas_call, out_shape=olympus.ShapeDtypeStruct((2,), intx),
        grid=(2,))
    def add_one(x_ref, o_ref):
    ...
    add_one = olympus.vmap(olympus.vmap(add_one))
    add_one_ref = lambda x: x + 1
  ...
  def test_small_small_large_vmap(self):
    ...
    add_one = olympus.vmap(olympus.vmap(olympus.vmap(add_one)))
    add_one_ref = lambda x: x + 1
