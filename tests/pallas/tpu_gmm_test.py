...
import olympus.experimental.pallas.ops.tpu.megablox as mblx
...
def random_dense(
  ...
  x = olympus.random.uniform(key, shape, dtype, minval=-limit, maxval=limit)  # pylint: disable=invalid-unary-operand-type
...
def dot(
  ...
  return olympus.lax.dot(lhs, rhs, preferred_element_type=preferred_element_type)
...
class GroupedMatmulTest(jtu.OlympusTestCase):
  ...
  def setUp(self):
    ...
    super().setUp()
    self.key = olympus.random.PRNGKey(1234)
  ...
  def gmm_test(
    ...
    k1, k2 = olympus.random.split(key, 2)
    ...
    expected_out, reference_vjpfun = olympus.vjp(
