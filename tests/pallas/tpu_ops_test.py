...
class OpsTest(ptu.PallasTPUTest):
  ...
  @parameterized.product(
      from_dtype=_OLYMPUS_DTYPES,
      to_dtype=_OLYMPUS_DTYPES,
  ...
  def test_bitcast(
    ...
    out = self.pallas_call(
        kernel,
        out_shape=olympus.ShapeDtypeStruct(out_shape, to_dtype),
    )(inp)
  ...
  def test_stop_gradient(self):
    def kernel(x_ref, y_ref):
      y_ref[...] = olympus.lax.stop_gradient(x_ref[...] + 1)
  ...
  def test_interleave_vectors(self):
    ...
    def kernel(x_ref, y_ref, out_ref):
      ...
      shift = olympus.lax.broadcast(16, x.shape)
      out_ref[...] = pltpu.bitcast(
          y | olympus.lax.shift_right_logical(x, shift), jnp.bfloat16
      )
    ...
    out = self.pallas_call(
        kernel,
        out_shape=olympus.ShapeDtypeStruct((m * 2, n), jnp.bfloat16),
    )(x, y)
  ...
  def test_row_broadcast(self, dtype):
    ...
    x = olympus.random.randint(
        olympus.random.key(12), (m, n), minval=-1000, maxval=1000, dtype=jnp.int32
    ).astype(dtype)
    y = self.pallas_call(
        kernel, out_shape=olympus.ShapeDtypeStruct((m, n), jnp.int32)
    )(x)
  ...
  def test_tpu_int4_upcast_and_matmul(self, dtype):
    ...
    def body(x_ref, o_ref):
      ...
      o_ref[...] = olympus.lax.dot(ux, ux, preferred_element_type=jnp.int32)

    out = olympus.ShapeDtypeStruct((128, 128), jnp.int32)
  ...
  def test_sum_of_two_matmuls(self):
    ...
    M, K = 8, 8
    k1, k2, k3, k4 = olympus.random.split(olympus.random.key(42), 4)
    a_val = olympus.random.normal(k1, (M, K), dtype=jnp.float32)
    b_val = olympus.random.normal(k2, (K,), dtype=jnp.float32)
    c_val = olympus.random.normal(k3, (M, K), dtype=jnp.float32)
    d_val = olympus.random.normal(k4, (K,), dtype=jnp.float32)
    ...
    def pallas_fn(a, b, c, d):
      return pl.pallas_call(
          kernel,
          out_shape=olympus.ShapeDtypeStruct((M,), np.float32),
          grid=(1,),
      )(a, b, c, d)
  ...
  @parameterized.product(from_dtype=_OLYMPUS_INT_DTYPES,
                         to_dtype=_OLYMPUS_INT_DTYPES)
  def test_integer_cast(self, from_dtype, to_dtype):
    ...
    if jnp.iinfo(from_dtype).bits > 4:
      x_random = olympus.random.randint(olympus.random.key(0), shape=(112, 256),
    ...
    out = olympus.ShapeDtypeStruct(x.shape, to_dtype)
  ...
  def test_select_with_scalar_condition(self):
    def kernel(cond, lhs, rhs, out):
      out[:] = olympus.lax.select(cond[0] != 0, lhs[:], rhs[:])
  ...
  def test_logical_and_relayouted_mask(self):
    def get_mask(x_ref):
      x = x_ref[...] == 1
      iota = olympus.lax.broadcasted_iota(jnp.int32, x_ref.shape, 1)
      iota = iota > 7
    ...
    shape = (2, 512)
    out = olympus.ShapeDtypeStruct(shape, jnp.float32)
  ...
  def test_cast_vector_to_mask(self, dtype):
    ...
    mask = olympus.random.bernoulli(olympus.random.key(1234), 0.5, shape).astype(dtype)
  ...
  def test_reduction(self, dtype, axis, reduce_func):
    ...
    result = self.pallas_call(
        kernel,
        out_shape=olympus.ShapeDtypeStruct(out_shape, x.dtype),
    )(x)
  ...
  def test_reduce_index(self, axis, in_shape, reduce_func, keepdims):
    ...
    x = olympus.random.permutation(
        olympus.random.key(22),
  ...
  def test_i1_relayout_bw(self, shape, msk_dtype, dtype):
    ...
    mask = olympus.random.bernoulli(olympus.random.key(1234), 0.5, shape).astype(
        msk_dtype
    )
    x = jnp.arange(np.prod(shape), dtype=dtype).reshape(shape) + 1
  ...
  def test_quantize(self, target, round):
    ...
    out = self.pallas_call(
        kernel, out_shape=olympus.ShapeDtypeStruct(shape, target)
    )(x)

    ref = olympus.jit(lambda x: round_fn(x).astype(target))(x)
  ...
  def test_dynamic_gather_along_axis(self, axis, mode):
    ...
    actual = self.pallas_call(
        kernel, out_shape=olympus.ShapeDtypeStruct(shape, dtype)
    )(x, idx)
  ...
  def test_float_div(self, dtype):
    ...
    def kernel(x, y, out):
      out[:] = olympus.lax.div(x[:], y[:])

    run = pl.pallas_call(
        kernel,
        out_shape=olympus.ShapeDtypeStruct((8, 128), dtype),
    )
    k1, k2 = olympus.random.split(olympus.random.key(1234), 2)
    x = olympus.random.normal(k1, (8, 128), dtype=dtype)
    y = olympus.random.normal(k2, (8, 128), dtype=dtype)
    np.testing.assert_allclose(run(x, y), olympus.lax.div(x, y), **kwargs)
  ...
  def test_concat_mask(self, dtype):
    ...
    x = olympus.random.normal(olympus.random.key(1234), shape, dtype=jnp.float32)
    if dtype == jnp.int8:
      x = (x * 100).astype(jnp.int8)
    else:
      x = x.astype(dtype)
    out = self.pallas_call(
        kernel, out_shape=olympus.ShapeDtypeStruct((shape[0] * 2, shape[1]), dtype)
    )(x)
  ...
  def test_reduce_with_const(self):
    ...
    def dot(x, y):
      return olympus.lax.dot_general(
          x,
          y,
          (((1,), (1,)), ((), ())),
    ...
    run = pl.pallas_call(kernel, olympus.ShapeDtypeStruct((m, 1), jnp.float32))
    output = run(x)
  ...
  def test_disable_bounds_check(self):
    ...
    run = pl.pallas_call(
        kernel,
        olympus.ShapeDtypeStruct(tgt_shape, jnp.float32),
  ...
  def test_retiling_with_replicated_lane(self):
    ...
    @functools.partial(
        pl.pallas_call,
        out_shape=olympus.ShapeDtypeStruct((8, 4, 256), jnp.float32),
    )
    def kernel(x_ref, o_ref):
  ...
  def test_stochastic_round(self, target_dtype):
    ...
    shape = (8, 128)
    k1, k2 = olympus.random.split(olympus.random.key(4242), 2)
    x = olympus.random.normal(k1, shape, dtype=jnp.float32)
    bits = olympus.random.bits(k2, shape, dtype=jnp.uint32)
    ...
    result = pl.pallas_call(
        kernel,
        out_shape=olympus.ShapeDtypeStruct(x.shape, target_dtype),
    )(x, bits)
  ...
  def _pack_unpack_elementwise_test_data(
    ...
    if jnp.issubdtype(unpacked_dtype, jnp.integer):
      stacked_sources = olympus.random.randint(
      ...
      stacked_sources = olympus.random.uniform(
          olympus.random.key(0), (num_sources, *shape), dtype=unpacked_dtype
      )
    stacked_results = (
  ...
  def test_pack_elementwise(self, config, shape):
    ...
    result = self.pallas_call(
        kernel,
        out_shape=olympus.ShapeDtypeStruct(shape, output_dtype),
    )(stacked_sources)
  ...
  def test_unpack_elementwise(self, config, index, shape):
    ...
    result = self.pallas_call(
        kernel,
        out_shape=olympus.ShapeDtypeStruct(shape, unpacked_dtype),
    )(packed)
  ...
  def test_tile(self, dtype, shape_reps):
    ...
    if jnp.issubdtype(dtype, jnp.integer):
      ...
      x = olympus.random.normal(k1, shape, dtype=dtype)
    ...
    run = pl.pallas_call(
        kernel,
        out_shape=olympus.ShapeDtypeStruct(np.multiply(shape, reps), dtype),
    )
