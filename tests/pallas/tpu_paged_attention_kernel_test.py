...
from olympus.experimental.pallas.ops.tpu import paged_attention
from olympus.experimental.pallas.ops.tpu.paged_attention import quantization_utils
from olympus.experimental.pallas.ops.tpu.paged_attention import util
...
def _generate_qkv_simplest(
    dtype: jnp.dtype,
) -> tuple[olympus.Array, olympus.Array, olympus.Array, olympus.Array, olympus.Array]:
...
class OlympusGroupedQueryAttentionReferenceTest(jtu.OlympusTestCase):
  ...
  def test_grouped_query_attention(self, dtype: jnp.dtype, case: int):
    ...
    olympus.debug.print("seq_lens: {seq_lens}", seq_lens=seq_lens)
    olympus.debug.print("queries: {queries}", queries=queries)
    olympus.debug.print("k_pages: {k_pages}", k_pages=k_pages)
    olympus.debug.print("v_pages: {v_pages}", v_pages=v_pages)
    olympus.debug.print("expected: {expected}", expected=expected)

    # calculate grouped query attention
    attention = util.grouped_query_attention_reference(
        queries, k_pages, v_pages, seq_lens
    )
    olympus.debug.print("attention: {attention}", attention=attention)
...
def _generate_random_qkv(
  ...
  k1, k2, k3, k4 = olympus.random.split(prng_key, 4)
  k_pages = olympus.random.normal(
      k1, (num_kv_heads, total_pages, page_size, head_dim), dtype=dtype
  )
  v_pages = olympus.random.normal(
  ...
  page_indices = olympus.random.permutation(k3, page_indices, independent=True)
  page_indices = page_indices.reshape(batch_size, pages_per_sequence)
  q = olympus.random.normal(k4, (batch_size, num_q_heads, head_dim), dtype=dtype)
...
def _reconstruct_kv(page_indices, pages):
  ...
  gathered = olympus.vmap(per_sequence_page_gather, in_axes=(None, 0))(
      pages, page_indices
  )
...
def _megacore_enabled():
  return olympus.devices()[0].device_kind == "TPU v4" or jtu.is_device_tpu(
      version=5, variant="p"
  )


@jtu.with_config(olympus_numpy_dtype_promotion="standard")
class PagedAttentionKernelTest(jtu.OlympusTestCase):
