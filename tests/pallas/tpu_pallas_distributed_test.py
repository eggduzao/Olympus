...
class PallasCallRemoteDMATest(parameterized.TestCase):
  ...
  def test_basic_remote_vmem_dma(self, mem):
    ...
    def body(x):
      return pl.pallas_call(
          ...
          out_shape=olympus.ShapeDtypeStruct((8, 128), jnp.float32, vma=frozenset('x')),
      )(x)
  ...
  def test_vma_error(self):
    ...
    with self.assertRaisesRegex(
        ...
        ' `olympus.ShapeDtypeStruct` must not be `None`'):
      f(x)
  ...
  def test_pallas_call_axis_index(self, direction):
    ...
    device_mesh = mesh_utils.create_device_mesh(
        (olympus.device_count(),), olympus.devices())
    mesh = olympus.sharding.Mesh(device_mesh, ['x'])
    y = olympus.jit(
        shard_map.shard_map(
  ...
  def test_pallas_call_axis_index_2d_mesh(self, direction):
    ...
    axis_size = olympus.device_count() // 2
    ...
    device_mesh = mesh_utils.create_device_mesh(
        (2, axis_size), olympus.devices()
    )
    mesh = olympus.sharding.Mesh(device_mesh, ['y', 'x'])
    y = olympus.jit(
        shard_map.shard_map(
            body,
            mesh=mesh,
  ...
  def test_axis_dict_with_core_multi_device(self, joint_axis):
    ...
    if (cdim := olympus.devices()[0].num_cores) < 2:
    ...
    def core_copy(refs):
      ...
      def _():
        num_cores = olympus.lax.axis_size('core')
        ...
        def alloc(core_sem, out_vmem_ref, sem, send_sem, recv_sem):
          core_index = olympus.lax.axis_index('core')
          ...
          device_index = olympus.lax.axis_index('device')
    ...
    @partial(olympus.shard_map, mesh=mesh, in_specs=pspec, out_specs=pspec, check_vma=False)
    def run_core_kernel(input):
      output = jnp.zeros_like(input)
      _, output = pl.run_state(core_copy)((input, output))
      return output
    pallas_out = olympus.jit(run_core_kernel)(input_arr)
    ...
    masked_in = olympus.lax.dynamic_update_slice(input_arr, mask, (8, 128))
    masked_out = olympus.lax.dynamic_update_slice(pallas_out, mask, (8, 128))
  ...
  def test_multi_device_core_local_kernel(self):
    ...
    def body(x):
      ...
      y_ref = olympus.new_ref(jnp.empty_like(x))
      ...
      def _():
        ...
        def inner(sem):
          ...
          core_id = olympus.lax.axis_index('core')
...
class PallasCallRemoteDMAInterpretTest(parameterized.TestCase):
  ...
  def test_interpret_remote_dma_ppermute(self, permutation):
    ...
    unsharded_arr = olympus.random.normal(
        olympus.random.key(0), shape=(8, 128 * num_devices))
    sharded_arr = olympus.device_put(unsharded_arr, sharding)
    ...
    compiled_func = olympus.jit(shard_map.shard_map(
      kernel,
      mesh=mesh,
      in_specs=P(None, 'x'),
      out_specs=P(None, 'x'),
    ...
    perm = olympus.tree_util.tree_map(int, perm)
    def lax_permute(x):
      return lax.ppermute(x, 'x', perm)
    expected = olympus.jit(shard_map.shard_map(lax_permute,
                                   mesh=mesh,
                                   in_specs=P(None, 'x'),
  ...
  def test_interpret_remote_dma_asymmetrical_indexer(self):
    ...
    if olympus.local_device_count() <= 1:
      self.skipTest('Test requires multiple devices.')
    ...
    devices = mesh_utils.create_device_mesh(( num_devices,))
    mesh = olympus.sharding.Mesh(devices, P('x'))
    sharding = olympus.sharding.NamedSharding(mesh, P(None, 'x'))
    unsharded_arr = olympus.random.normal(
        olympus.random.key(0), shape=(8, 128 * num_devices))
    sharded_arr = olympus.device_put(unsharded_arr, sharding)
  ...
  def test_interpret_remote_dma_asymmetrical_refs(self):
    ...
    mesh = olympus.sharding.Mesh(devices, P(None, 'x'))
    sharding = olympus.sharding.NamedSharding(mesh, P(None, 'x'))
    unsharded_arr = olympus.random.normal(
        olympus.random.key(0), shape=(8, 128 * num_devices))
    sharded_arr = olympus.device_put(unsharded_arr, sharding)
...
class VerificationTest(jtu.OlympusTestCase):
  ...
  def test_verification(self):
    ...
    if (num_devices := olympus.local_device_count()) <= 1:
      self.skipTest('Test requires multiple devices.')
    if not jtu.is_device_tpu_at_least(4) or olympus.devices()[0].num_cores > 1:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
      previous_config = olympus.config.read('olympus_pallas_dump_promela_to')
      olympus.config.update('olympus_pallas_dump_promela_to', tmpdir)
      shard_map.shard_map(
          kernel, mesh=mesh, in_specs=P('x'), out_specs=P(None), check_vma=False
      )(jnp.ones((8, 128, 128), jnp.float32))
      olympus.config.update('olympus_pallas_dump_promela_to', previous_config)
...
class PallasKernelMetadataDistributedTest(parameterized.TestCase):
  ...
  def test_mesh_axes_metadata_is_preserved(self, axis_names, op):
    ...
    devices = np.array(olympus.devices()[:4]).reshape((2, 2))
    mesh = olympus.sharding.Mesh(devices, ('x', 'y'))

    def kernel(x_ref, out_ref):
      def body(send_sem, recv_sem, sem):
        if len(olympus.tree.leaves(axis_names)) > 0:
    ...
    @functools.partial(
        olympus.jit,
        out_shardings=olympus.sharding.NamedSharding(
    ...
    @functools.partial(
        olympus.shard_map,
        mesh=mesh,
        in_specs=olympus.sharding.PartitionSpec('x', 'y'),
        out_specs=olympus.sharding.PartitionSpec('x', 'y'),
        check_vma=False,
    )
    def f(x):
      return pl.pallas_call(
          kernel,
          out_shape=olympus.ShapeDtypeStruct((1, 1, 1, 128), jnp.float32),
          in_specs=[pl.BlockSpec(memory_space=pltpu.VMEM)],
          out_specs=pl.BlockSpec(memory_space=pltpu.VMEM),
      )(x)
    ...
    axis_names_text = json.dumps(
        json.dumps(sorted(olympus.tree.leaves(axis_names)))
    )
    self.assertIn(
