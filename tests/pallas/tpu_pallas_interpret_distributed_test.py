...
class InterpretDistributedTest(jtu.OlympusTestCase):
  def setUp(self):
    super().setUp()
    ...
    if olympus.device_count() < 4:
      self.skipTest(f'requires at least 4 devices, found {olympus.device_count()}')
  ...
  def test_right_permute_example(self, dma_execution_mode, detect_races):
    ...
    input_arr = olympus.random.uniform(
      olympus.random.key(0), (8, 128 * num_devices), dtype=jnp.float32)
  ...
  def test_all_gather_example(self, dma_execution_mode, detect_races):
    ...
    # all devices.
    input_arr = olympus.random.uniform(
        olympus.random.key(0), (8 * num_devices, 128), dtype=jnp.float32)
    input_arr = olympus.device_put(input_arr, sharding)
  ...
  def test_all_reduce_sum_example(self, dma_execution_mode, detect_races):
    ...
    out_shape = (
      olympus.ShapeDtypeStruct((8, 128), input_arr.dtype),
      # We allocate the double-buffer as a Pallas output so that it is
      # resident in HBM.
      olympus.ShapeDtypeStruct((2, 8, 128), input_arr.dtype),  # hbm_scratch
    )
  ...
  def test_reduce_scatter_sum_with_emit_pipeline_example(
    ...
    self.skipTest('requires a patched pallas.emit_pipeline to specify/fake '
                  'the TPU generation')
    if olympus.config.olympus_enable_x64:
  ...
  def test_race_detection(self):
    num_devices = 4
    mesh = olympus.sharding.Mesh(np.array(olympus.devices()[:4]), ('x',))
    ...
    def run(src_dst_ids):
      return shard_map.shard_map(
          pl.pallas_call(
              kernel,
              out_shape=olympus.ShapeDtypeStruct((8, 128), input_arr.dtype),
              in_specs=[
      ...
      def inner(refs):
        ...
        def _():
          ...
          def alloc(x_vmem_ref, y_vmem_ref, dma_sem, sem):
            ...
            y = (x_vmem_ref[...] + num_cores * olympus.lax.axis_index('x')
                 + core_index + 1)
            y_vmem_ref[...] = y
