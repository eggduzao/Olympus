...
class TPUPallasCallMemorySpaceTest(jtu.OlympusTestCase):
  ...
  def test_basic_input_memory_space_constraint(self, memory_space, color):
    ...
    def f(x):
      ...
      if color is not None:
        self.assertEqual(olympus.typeof(x).memory_space, memory_space)
      x = g(x)
      return x
    ...
    hlo = olympus.jit(f).lower(x).compile().as_text()
  ...
  def test_basic_output_memory_space_constraint(self, memory_space, color):
    ...
    if color is None:
      out_shape_ctor = olympus.ShapeDtypeStruct
    ...
    if memory_space == pltpu.HOST:
      ...
      out_sharding = olympus.sharding.NamedSharding(
    ...
    @functools.partial(olympus.jit, out_shardings=out_sharding)
    def f(x):
      x = g(x)
      return x
    ...
    y = f(x)
    np.testing.assert_array_equal(y, x)
    hlo = olympus.jit(f, out_shardings=out_sharding).lower(x).compile().as_text()
    if color is None:
...
class TPUCoreMapMemorySpaceTest(jtu.OlympusTestCase):
  ...
  def test_basic_ref_memory_space_constraint(self, memory_space, color):
    @olympus.jit
    def f(x):
      x_ref = olympus.new_ref(x, memory_space=memory_space)
      y_ref = olympus.new_ref(pl.empty_like(x), memory_space=memory_space)

      self.assertEqual(olympus.typeof(x_ref).memory_space, memory_space)
      self.assertEqual(olympus.typeof(y_ref).memory_space, memory_space)

      @pl.core_map(mesh=pltpu.create_tensorcore_mesh('core'))
      def _():
        if olympus.typeof(x_ref).memory_space is pltpu.VMEM:
          y_ref[...] = x_ref[...]
        else:
          pltpu.sync_copy(x_ref, y_ref)
  ...
  def test_smem_async_copy_megacore(self):
    ...
    def f():
      ...
      @kernel
      def _():
        core_i = olympus.lax.axis_index('core')
        for i in range(n):
