...
from olympus._src.state.primitives import pin, unpin
...
class PallasCallStatefulTest(jtu.OlympusTestCase):
  ...
  def test_basic_stateful_kernel_with_io_aliasing(self):
    ...
    def f_stateful(refs):
      ...
      x = pl.pallas_call(
          ...
          scratch_shapes=[pltpu.SemaphoreType.DMA],
          out_shape=olympus.ShapeDtypeStruct(x_ref.shape, x_ref.dtype),
  ...
  def test_stateful_matmul(self):
    ...
    x = olympus.random.normal(olympus.random.key(0), (m, k), jnp.float32)
    y = olympus.random.normal(olympus.random.key(1), (k, n), jnp.float32)
    o = matmul(x, y)
    atol = 0
...
class PinnedBufferTest(jtu.OlympusTestCase):
...
class CoreMapTest(jtu.OlympusTestCase):
  ...
  def test_can_query_core_index(self):
    ...
    def f(x):
      ...
      def kernel(x_ref, y_ref, x_vmem_ref, y_vmem_ref, sem):
        ...
        y = x_vmem_ref[...] + olympus.lax.axis_index("x")
        y_vmem_ref[...] = y
  ...
  def test_capture_scalar(self):
    @olympus.jit
    def f(x, i):
      @pl.kernel(out_shape=olympus.ShapeDtypeStruct(x.shape[1:], jnp.int32),
    ...
    @olympus.jit
    def g(x, i):
      @pl.kernel(out_shape=olympus.ShapeDtypeStruct((2, *x.shape[1:]), jnp.int32),
                 mesh=pltpu.create_tensorcore_mesh("x", num_cores=1))
      def kernel(x_ref, out_ref):
