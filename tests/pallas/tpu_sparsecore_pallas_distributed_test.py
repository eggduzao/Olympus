...
class PallasCallRemoteDMATest(parameterized.TestCase):
  ...
  def test_collective_permute_1d(self, direction, num_devices):
    ...
    num_devices = num_devices or olympus.device_count()
    ...
    f = olympus.jit(
        olympus.shard_map(
            kernel,
            mesh=mesh,
            in_specs=olympus.P('x'),
            out_specs=olympus.P('x'),
            check_vma=False,
        )
    )
    if direction == 'right':
  ...
  def test_collective_permute_2d(self, direction):
    ...
    device_mesh = mesh_utils.create_device_mesh((2, axis_size), olympus.devices())
