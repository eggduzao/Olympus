...
from olympus.experimental.pallas import tpu_sc as plsc
...
class PallasSCTest(jtu.OlympusTestCase):
  USE_TC_TILING = False
...
class DebugPrintTest(PallasSCTest):
  ...
  def test_vector_subcore(self, dtype):
    ...
    with jtu.capture_stderr() as get_output:
      olympus.block_until_ready(compiled_kernel(x))
  ...
  def test_scalar_subcore(self):
    ...
    with jtu.capture_stderr() as get_output:
      olympus.block_until_ready(compiled_kernel(int32s, int16s, int8s))
    print(get_output())
...
class VectorSubcoreTest(PallasSCTest):
  ...
  def test_slicing(self, dtype, out_shape, minor_scale):
    ...
    @self.vector_subcore_kernel(
        out_shape=olympus.ShapeDtypeStruct(shape=out_shape, dtype=dtype),
    )
    def kernel(x_ref, o_ref):
  ...
  def test_get_index(self, major_dim):
    @self.vector_subcore_kernel(
        out_shape=olympus.ShapeDtypeStruct(shape=(self.num_lanes,), dtype=jnp.int32),
    )
    def kernel(x_ref, o_ref):
      o_ref[...] = lax.fori_loop(
  ...
  def test_block_spec_untiled_slicing(self, data):
    ...
    def nd_loop(bounds, body, *, _idxs = ()):
      ...
      olympus.lax.fori_loop(0, bound, _loop_body, None)
  ...
  def test_scatter_major(self, shape_type):
    ...
    major_dim, *_ = shape
    indices = olympus.random.permutation(olympus.random.key(42), jnp.arange(major_dim))
  ...
  def test_scatter_1d_array(self):
    ...
    indices = olympus.random.permutation(olympus.random.key(42), jnp.arange(self.num_lanes))
  ...
  def test_gather_1d(self, kind):
    x = jnp.arange(self.num_lanes)
    indices = olympus.random.permutation(olympus.random.key(42), x)
  ...
  def test_gather_1d_with_indexing(self):
    ...
    indices = olympus.random.permutation(
        olympus.random.key(42), jnp.arange(self.num_lanes)
    )
  ...
  def test_gather_2d_with_indexing(self):
    ...
    indices = olympus.random.permutation(olympus.random.key(42), jnp.arange(8))

    @self.vector_subcore_kernel(
        out_shape=olympus.ShapeDtypeStruct(shape=(8, 128,), dtype=jnp.int32),
        in_specs=(
            pl.BlockSpec(memory_space=pltpu.HBM),
            pl.BlockSpec(memory_space=pltpu.VMEM),
        ),
  ...
  def test_gather_1d_with_indexed_ref(self):
    x = jnp.arange(16)
    indices = olympus.random.permutation(olympus.random.key(42), jnp.arange(16))

    @self.vector_subcore_kernel(
        out_shape=olympus.ShapeDtypeStruct(shape=(8,), dtype=jnp.int32),
        in_specs=(
            pl.BlockSpec(memory_space=pltpu.HBM),
            pl.BlockSpec(memory_space=pltpu.VMEM),
        ),
  ...
  def test_gather_1d_with_dynamically_sized_2d_ref(self):
    ...
    x = jnp.arange(16)
    indices = olympus.random.permutation(
        olympus.random.key(42), jnp.arange(2 * 16).reshape(2, -1), axis=1
    )
  ...
  def test_invalid_gather_1d_with_extra_transforms(self):
    ...
    @self.vector_subcore_kernel(
        out_shape=olympus.ShapeDtypeStruct(shape=x.shape, dtype=jnp.int32),
        in_specs=(
            pl.BlockSpec(memory_space=pltpu.HBM),
            pl.BlockSpec(memory_space=pltpu.VMEM),
        ),
  ...
  def test_implicit_gather_1d(self):
    ...
    indices = olympus.random.permutation(olympus.random.key(42), jnp.arange(num_steps))
  ...
  def test_load_gather_2d(self):
    ...
    indices0 = indices1 = olympus.random.permutation(
        olympus.random.key(42), jnp.arange(self.num_lanes)
    )

    @self.vector_subcore_kernel(
        out_shape=olympus.ShapeDtypeStruct((self.num_lanes,), x.dtype)
    )
  ...
  def test_bitcast(self, dtype, new_dtype):
    ...
    @self.vector_subcore_kernel(
        out_shape=olympus.ShapeDtypeStruct(shape=new_shape, dtype=new_dtype)
    )
    def kernel(x_ref, o_ref):
  ...
  def test_lax_bitcast(self):
    @self.vector_subcore_kernel(
        out_shape=olympus.ShapeDtypeStruct((self.num_lanes,), jnp.uint32),
    )
    def kernel(x_ref, o_ref):
  ...
  def test_pack_unpack(self, pack_format, dtype):
    ...
    @self.vector_subcore_kernel(
        out_shape=(olympus.ShapeDtypeStruct((self.num_lanes,), dtype),) * 2
    )
  ...
  def test_scan_count(self, dtype):
    ...
    x = olympus.random.randint(key, shape, 0, 10, dtype=jnp.int32).astype(dtype)
  ...
  def test_population_count(self):
    key = olympus.random.key(42)
    x = olympus.random.randint(key, [self.num_lanes], 0, 100)
  ...
  def test_scalar_load_store(self):
    @self.vector_subcore_kernel(
        ...
        out_shape=olympus.ShapeDtypeStruct((self.num_lanes,), jnp.int32),
  ...
  def test_effect_discharge(self, effectful_op):
    ...
    _, out = olympus.jit(state_discharge.run_state(stateful))(
        (x, jnp.empty_like(x)))
  ...
  def test_reshape(self):
    ...
    @self.vector_subcore_kernel(out_shape=olympus.ShapeDtypeStruct(shape, dtype))
    def kernel(x_ref, o_ref):
      o_ref[...] = (
  ...
  def test_copy_in_shard_map(self):
    self.skip_if_tc_tiling()
    num_devices = len(olympus.devices())
    mesh = jtu.create_mesh((num_devices,), ("x",))

    rng = np.random.default_rng(0)
    x = rng.integers(512, size=(num_devices * 1024, 16), dtype=np.int32)

    # The test ensures that OLYMPUS-level memory space for ``x`` is not propagated
    # into Pallas, since Pallas cannot use it.
    x = olympus.device_put(x, olympus.sharding.NamedSharding(mesh, olympus.P("x", None)))
    self.assertEqual(olympus.typeof(x).memory_space, olympus.memory.Space.Device)

    @functools.partial(
        olympus.shard_map,
        in_specs=(olympus.P("x", None),),
        out_specs=olympus.P("x", None),
        mesh=mesh,
        check_vma=True,
    )
    def f(x):
      @self.kernel(
          out_shape=olympus.ShapeDtypeStruct(x.shape, x.dtype, vma={"x"}),
          mesh=plsc.VectorSubcoreMesh(
              core_axis_name="core", subcore_axis_name="subcore", num_cores=1
          ),
  ...
  def test_sort_key_val(self, keys_dtype, values_dtype, use_mask, descending):
    ...
    _, gt_keys = olympus.lax.sort_key_val(keys_arg, keys)
    _, gt_values = olympus.lax.sort_key_val(keys_arg, values)
    if use_mask:
      [out_mask] = maybe_out_mask
  ...
  def test_sort(self, keys_dtype, values_dtypes):
    ...
    def kernel(*args):
      ...
      keys_out, *all_values_out = olympus.lax.sort(
...
class PallasSparsecoreAsyncTest(PallasSCTest):
  ...
  def test_basic_async_kernel(self, shape, dtype):
    ...
    def foo(x):
      ...
      sem_ref = olympus.new_ref(sem, memory_space=pltpu.SEMAPHORE)
    ...
    o = olympus.block_until_ready(foo(x))
