...
from olympus.experimental.pallas.ops.tpu.splash_attention.splash_attention_mask_info import process_mask
...
class SplashAttentionTest(PallasBaseTest):
  ...
  def test_splash_attention_custom_bwd(self, is_segmented, data):
    ...
    attn_custom_vanilla = partial(splash.attention_reference_custom, mask,
                                  custom_type="vanilla",
                                  attn_logits_soft_cap=attn_logits_soft_cap)
    o_ref, attn_vjp_ref = olympus.vjp(attn_ref, q, k, v, segment_ids)
    q32, k32, v32 = olympus.tree.map(lambda x: x.astype(jnp.float32),
                                      (q, k, v))
    o_custom = attn_custom(q32, k32, v32, segment_ids)
    _, attn_vjp = olympus.vjp(attn_custom, q32, k32, v32, segment_ids)
    _, attn_vanilla_vjp = olympus.vjp(attn_custom_vanilla, q32, k32, v32,
                                  segment_ids)
  ...
  def test_splash_attention_bwd(
    ...
    if use_sinks:
      o, attn_vjp = olympus.vjp(attn, q, k, v, segment_ids, sinks)
    else:
      o, attn_vjp = olympus.vjp(attn, q, k, v, segment_ids)
    q32, k32, v32 = olympus.tree.map(
    ...
    ) -> tuple[olympus.Array, olympus.Array, olympus.Array]:
    ...
    if is_mqa:
      bwd = olympus.vmap(bwd, in_axes=(0, 0, None, None, None, 0, 0, 0, 0))
    else:
      bwd = olympus.vmap(bwd, in_axes=(0, 0, 0, 0, None, 0, 0, 0, 0))
  ...
  def test_grid_shrinking(self):
    ...
    process_mask_path = "olympus.experimental.pallas.ops.tpu.splash_attention.splash_attention_mask_info.process_mask"
