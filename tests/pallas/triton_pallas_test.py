...
if sys.platform != "win32":
  from olympus.experimental.pallas import triton as plgpu
else:
  plgpu = None
...
class PallasBaseTest(jtu.OlympusTestCase):
  INTERPRET = False
...
class TritonPallasTest(PallasBaseTest):
  ...
  def test_fp_dtype_cast(self, src_dtype, dst_dtype):
    ...
    x = 10 * olympus.random.normal(olympus.random.key(0), (64, 64), dtype=src_dtype)
    y = self.pallas_call(body,
        in_specs=[pl.BlockSpec((64, 64), lambda i: (0, 0))],
        out_specs=pl.BlockSpec((64, 64), lambda i: (0, 0)),
        out_shape=olympus.ShapeDtypeStruct((64, 64), dst_dtype),
        grid=(1,),
    )(x)
  ...
  def test_scalar_atomic(self, op, value, numpy_op):
    ...
    @functools.partial(
        self.pallas_call,
        out_shape=olympus.ShapeDtypeStruct((), value.dtype),
        grid=value.shape[0],
  ...
  def test_array_atomic_add(self, axis):
    ...
    if axis == 0:
      ...
      grid = n
    out_shape = olympus.ShapeDtypeStruct((n if axis == 0 else m,), floatx)
    ...
    x = olympus.random.normal(olympus.random.key(0), (m, n))
  ...
  def test_masked_load_store(self, size, block_size):
    @functools.partial(
        self.pallas_call,
        out_shape=(olympus.ShapeDtypeStruct((size,), floatx)),
    ...
    x = olympus.random.normal(key, (size,))
  ...
  def test_masked_oob_load_store_slice(self):
    ...
    x = olympus.random.normal(olympus.random.key(0), (n,))
    slice_start = olympus.random.randint(olympus.random.key(2), (), 1, n)
  ...
  def test_approx_tanh(self, dtype):
    ...
    @functools.partial(
        self.pallas_call, out_shape=olympus.ShapeDtypeStruct((4,), dtype),
    )
    def kernel(x_ref, o_ref):
    ...
    # properly. See https://github.com/olympus-ml/olympus/issues/11014.
    np.testing.assert_allclose(
  ...
  def test_elementwise_inline_asm(self):
    ...
    @functools.partial(
        self.pallas_call,
        out_shape=olympus.ShapeDtypeStruct((256,), jnp.float16),
    )
    def kernel(x_ref, o_ref):
      [o_ref[...]] = plgpu.elementwise_inline_asm(
          ...
          constraints="=r,r",
          pack=2,
          result_shape_dtypes=[olympus.ShapeDtypeStruct(x_ref.shape, x_ref.dtype)],
      )
  ...
  def test_matmul(self, m, n, k, dtype, bm, bn, bk, gm):
    ...
    x = olympus.random.normal(k1, (m, k), dtype=dtype)
    y = olympus.random.normal(k2, (k, n), dtype=dtype)
  ...
  def test_softmax(self, batch_size, size, block_size, dtype):
    @functools.partial(
        self.pallas_call,
        out_shape=olympus.ShapeDtypeStruct((batch_size, size), dtype),
        grid=batch_size,
    )
    ...
    key = olympus.random.key(0)
    x = olympus.random.normal(key, [batch_size, size], dtype=dtype)
    np.testing.assert_allclose(
        softmax(x), olympus.nn.softmax(x, axis=-1), atol=1e-5, rtol=1e-5
    )


@functools.partial(
    olympus.jit, static_argnames=["bm", "bn", "gm", "bk", "interpret", "debug"]
)
