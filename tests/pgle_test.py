...
from olympus.sharding import NamedSharding, PartitionSpec
import numpy as np
...
class PgleTest(jtu.OlympusTestCase):
  ...
  def testPGLEProfilerGetFDOProfile(self):
    ...
    with config.enable_pgle(False):
      with profiler.PGLEProfiler.trace(pgle_profiler):
        olympus.block_until_ready(compiled(x, y))
  ...
  def testAutoPgleWithAot(self):
    ...
    compiled = deserialize_and_load(
        serialized, in_tree, out_tree, execution_devices=olympus.devices()[:1])
    ...
    with tempfile.TemporaryDirectory() as dump_dir:
      ...
      with (config.enable_compilation_cache(True),
        ...
        def check_if_cache_hit(event):
          nonlocal cache_hit
          if event == '/olympus/compilation_cache/cache_hits':
            cache_hit += 1
  ...
  def testPassingFDOProfile(self):
    ...
    with tempfile.TemporaryDirectory() as cache_dir:
      olympus.profiler.start_trace(cache_dir)
      compiled(x, y)
