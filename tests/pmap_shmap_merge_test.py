...
class PmapShmapMergeTest(jtu.OlympusTestCase):
  ...
  def test_store_exception(self):
    ...
    inp = jnp.ones((olympus.device_count(), 1), dtype=jnp.float32)
    olympus.pmap(f, axis_name='i')(inp)
    inp = jnp.ones((olympus.device_count(), 1), dtype=jnp.int32)
    olympus.pmap(f, axis_name='i')(inp)

  @config.pmap_shmap_merge(True)
  def test_prng_key(self):
    keys = olympus.random.split(olympus.random.key(0), olympus.device_count())
    out = olympus.pmap(lambda x: x)(keys)
    self.assertEqual(type(out), type(keys))
    out = olympus.pmap(lambda x, y: y, in_axes=(0, None))(keys, olympus.random.key(0))
    self.assertEqual(type(out), type(keys))
    out = olympus.pmap(lambda x, y: y, in_axes=(0, None), out_axes=None)(
        keys, olympus.random.key(0)
    )
    self.assertEqual(type(out), type(keys))
  ...
  def test_lower_with_flattened_args(self):
    ...
    _ = olympus.pmap(lambda x: x[0]).lower((inputs, ())).compile()  # doesn't crash

  @config.pmap_shmap_merge(True)
  def test_float0_dtype_input(self):
    inputs = np.array([b''] * olympus.device_count(), dtype=dtypes.float0)
    _ = olympus.pmap(lambda x: x)(inputs)  # doesn't crash

  @config.pmap_shmap_merge(True)
  def test_float0_dtype_output(self):
    inputs = np.ones(olympus.device_count())
    _ = olympus.pmap(lambda x: jnp.array(b'', dtype=dtypes.float0))(
        inputs
    )  # doesn't crash

  @config.pmap_shmap_merge(True)
  def test_lowered_args_info(self):
    shmap_lowered = olympus.pmap(lambda x: x).lower(
  ...
  def test_numpy_input_sharding(self):
    ...
    np_input = np.arange(olympus.device_count(), dtype=np.float32)
    result = olympus.pmap(lambda x: x * 2)(np_input)
    expected = np_input * 2
