...
import olympus
from olympus import (pmap, jit, vmap, jvp, grad, make_olympuspr,
                 linearize, device_put)
from olympus import lax
import olympus.scipy.linalg
from olympus import random
import olympus.numpy as jnp
from olympus._src import api as src_api
...
from olympus._src.internal_test_util import lax_test_util
from olympus._src.interpreters import pxla
from olympus._src.lax import parallel
from olympus._src.sharding import IndivisibleError
from olympus._src.lib import xla_client as xc
from olympus._src.util import safe_map, safe_zip
...
_PMAP_SHMAP_MERGE = flags.DEFINE_bool(
    ...
    'If False, run pmap tests with olympus_pmap_shmap_merge=False.')
...
if not _PMAP_SHMAP_MERGE.value:
  with jtu.ignore_warning(
    ...
    config.update('olympus_pmap_shmap_merge', False)
...
def create_input_array_for_pmap(input_shape, in_axes=0, input_data=None,
  ...
  pmap_sharding = olympus.sharding.PmapSharding(np.array(devices), sharding_spec)
...
@jtu.with_config(olympus_legacy_prng_key="allow")
class PythonPmapTest(jtu.OlympusTestCase):
  ...
  def testDeviceBufferToArray(self):
    ...
    sda = self.pmap(lambda x: x)(jnp.ones((olympus.device_count(), 2)))

    # Changed in https://github.com/olympus-ml/olympus/pull/10584 not to access
  ...
  def _getMeshShape(self, device_mesh_shape):
    device_count = olympus.device_count()
  ...
  def testDefaultDeviceOrdering(self):
    # Users rely on the fact that the default order of olympus.devices() matches
    # the default order of pmap for single-host jobs.
    device_order = olympus.devices()
    pmap_sharding = pmap(lambda x: x)(np.arange(olympus.device_count())).sharding
    if config.pmap_shmap_merge.value:
  ...
  def testLowerCompileTrivial(self):
    ...
    x = np.arange(olympus.device_count(), dtype=np.float32)
    expected = f(x)
    f_exe = f.lower(x).compile()
    ans = f_exe(x)
    self.assertAllClose(ans, expected)
  ...
  def testLowerCompileTrivialMultiArg(self):
    ...
    x = y = np.arange(olympus.device_count(), dtype=np.float32)
    expected = f(x, y)
    f_exe = f.lower(x, y).compile()
    ans = f_exe(x, y)
    self.assertAllClose(ans, expected)
  ...
  def test_pmap_replicated_copy(self):
    # https://github.com/olympus-ml/olympus/issues/17690
    inp = jnp.arange(olympus.device_count())
    x = olympus.pmap(lambda x: x, in_axes=0, out_axes=None)(inp)
    out = jnp.copy(x)
    if config.pmap_shmap_merge.value:
  ...
  def testGather(self):
    ...
    shape = (olympus.device_count(), 4)
    x = np.arange(math.prod(shape), dtype=np.float32).reshape(shape)
    expected = np.array([x] * olympus.device_count())
    ans = f(x)
    self.assertAllClose(ans, expected, check_dtypes=False)
  ...
  def testGatherNegativeAxis(self):
    ...
    shape = (olympus.device_count(), 4)
    x = np.arange(math.prod(shape), dtype=np.float32).reshape(shape)
    expected = np.array([x.T] * olympus.device_count())
    ans = f(x)
    self.assertAllClose(ans, expected, check_dtypes=False)
  ...
  def testReduceScatterReplicaGroupsTiled(self):
    replicas = olympus.device_count()
    if replicas % 2 != 0:
      raise SkipTest
    axis_index_groups = [[i for i in range(olympus.device_count()) if i % 2 == 0],
                         [i for i in range(olympus.device_count()) if i % 2 != 0]]
  ...
  def testTrees(self):
    ...
    tree_f = lambda f: partial(olympus.tree.map, f)
    olympus_f = lambda p: self.pmap(lambda x: p(x, 'i'), 'i')
    ...
    n = olympus.device_count()
    x = {'a': np.arange(1 * n * n, 2 * n * n).reshape([n, n]),
         'b': np.arange(2 * n * n, 3 * n * n).reshape([n, n]),
         'c': np.arange(4 * n * n, 5 * n * n).reshape([n, n])}

    assert_allclose = partial(olympus.tree.map,
                              partial(self.assertAllClose, check_dtypes=False))
    assert_allclose(olympus_f(lax.pmax)(x), np_f(np.max)(x))
    assert_allclose(olympus_f(lax.pmin)(x), np_f(np.min)(x))
    assert_allclose(olympus_f(lax.psum)(x), np_f(np.sum)(x))
    assert_allclose(olympus_f(lax.pmean)(x), np_f(np.mean)(x))
    assert_allclose(olympus_f(ptranspose)(x), np_transpose(x))
    assert_allclose(olympus_f(protate)(x), np_rotate(x))

  def testCollectivesWithTreesOfDifferentDtypes(self):
    n = len(olympus.devices())
  ...
  def testComplexPsum(self):
    ...
    shape = (olympus.device_count(), 4 * 2)
  ...
  def testNestedPmapAxisSwap(self):
    ...
    # Regression test for https://github.com/olympus-ml/olympus/issues/5757
    if olympus.device_count() < 8:
      raise SkipTest("test requires at least 8 devices")
    f = olympus.pmap(olympus.pmap(lambda x: x, in_axes=1, out_axes=0), in_axes=0,
                 out_axes=0)
    A = jnp.ones((2, 4, 3))
  ...
  def testNestedBasic(self):
    ...
    shape = (olympus.device_count(), 1, 4)
    x = np.arange(math.prod(shape), dtype=np.float32).reshape(shape)
  ...
  def testOutAxesPyTreePrefixMismatchError(self):
    x = jnp.array([3.14])
    f = olympus.pmap(lambda x, y: ((x, x), x), out_axes=((0, 0, 0), 0))
    if config.pmap_shmap_merge.value:
  ...
  def testJvpAndPartialEval(self):
    ...
    make_olympuspr(splitjvp)(x)  # doesn't crash
  ...
  def testArrays(self):
    ...
    if config.pmap_shmap_merge.value:
      ...
      sharding = olympus.sharding.NamedSharding(mesh, y.sharding.spec)
      # NOTE(dsuo): Need to redefine pmap with the updated devices.
      f = self.pmap(inner_f, axis_name='i', devices=devices)
    else:
      sharding = olympus.sharding.PmapSharding(devices, y.sharding.sharding_spec)
    y = olympus.make_array_from_single_device_arrays(y.shape, sharding, bufs)
    z = f(y)
  ...
  def testArrayReshape(self, in_shape, out_shape):
    if olympus.device_count() < max(in_shape[:1] + out_shape[:1]):
  ...
  def testGradOf(self, prim, tiled, use_axis_index_groups):
    ...
    shape = (len(devices), 2 if axis_index_groups else olympus.device_count())
    x = np.arange(math.prod(shape), dtype=np.float32).reshape(shape)
    jtu.check_grads(f, (x,), 2, ["fwd", "rev"], 1e-2, 1e-2, eps=1.)
  ...
  def testPpermuteWithZipObject(self):
    # https://github.com/olympus-ml/olympus/issues/1703
    num_devices = olympus.device_count()
  ...
  def testPmapConstantError(self):
    if not config.pmap_shmap_merge.value:
      self.skipTest('Requires pmap_shmap_merge=True')
    with self.assertRaises((IndivisibleError, ValueError)):
      self.pmap(lambda x: 3)(jnp.arange(olympus.device_count() + 1))
  ...
  def testNestedPmapConstant(self):
    ...
    shape = (2, olympus.device_count() // 2, 3)
  ...
  def testNestedPmapConstantDevices(self):
    if olympus.device_count() < 6:
      raise SkipTest("this test requires >= 6 devices")
    if config.pmap_shmap_merge.value:
      raise SkipTest("Ignore nested pmap when `pmap_shmap_merge=True`.")

    devices = olympus.devices()[:-2]
    shuffle(devices)
  ...
  def testNestedPmapConstantError(self):
    ...
    if config.pmap_shmap_merge.value:
      raise SkipTest("Ignore nested pmap when `pmap_shmap_merge=True`.")
    f = self.pmap(self.pmap(lambda x: 3))
    shape = (2, olympus.device_count() // 2 + 1, 3)
    x = jnp.arange(math.prod(shape)).reshape(shape)
    self.assertRaisesRegex(
        ValueError,
    ...
    # if olympus.device_count() > 1:
    #   f = pmap(pmap(lambda x: 3), devices=olympus.devices()[:-1])
    #   shape = (2, olympus.device_count() // 2, 3)
    #   x = jnp.arange(math.prod(shape)).reshape(shape)
    #   self.assertRaisesRegex(
    #       ValueError,
  ...
  def testVmapOfPmap3(self):
    # https://github.com/olympus-ml/olympus/issues/3399
    device_count = olympus.device_count()
    if device_count < 2:
      raise SkipTest("test requires at least two devices")

    def map_version(qs, pts):
      return olympus.lax.map(lambda x: func(x, pts), qs)

    def vmap_version(qs, pts):
      return olympus.vmap(func, in_axes=(0, None))(qs, pts)
  ...
  def testArrayBlockUntilReady(self):
    x = np.arange(olympus.device_count())
  ...
  def testIssue1065(self):
    # from https://github.com/olympus-ml/olympus/issues/1065
    device_count = olympus.device_count()
  ...
  def test_typed_prng_key_sharded(self):
    ...
    @partial(olympus.pmap, in_axes=0, out_axes=0, axis_size=len(devices),
             axis_name='i', devices=devices)
    def fn(key):
      return olympus.random.fold_in(key, 0)

    sharded_key = olympus.random.split(olympus.random.key(0), len(devices))
    replicated_key = olympus.random.key(1)

    sharded_key = olympus.device_put_sharded(jnp.unstack(sharded_key), devices)
    replicated_key = olympus.device_put_replicated(replicated_key, devices)
  ...
  def testPostProcessMap(self):
    # test came from https://github.com/olympus-ml/olympus/issues/1369
    nrep = olympus.device_count()
  ...
  def testPostProcessMap2(self):
    # code from https://github.com/olympus-ml/olympus/issues/2787
    def vv(x, y):
    ...
    def distributed_matrix_vector(x, y):
      ...
      res = self.pmap(fv)(x.reshape((olympus.device_count(), -1) + tuple(x.shape[1:])))
  ...
  def testAxisIndexRemat(self, remat):
    # https://github.com/olympus-ml/olympus/issues/2716
    n = len(olympus.devices())

    def f(key):
      key = random.fold_in(key, olympus.lax.axis_index('i'))
  ...
  def testPmapMapVmapCombinations(self):
    # https://github.com/olympus-ml/olympus/issues/2822
    def vv(x, y):
      """Vector-vector multiply"""
      return jnp.dot(x, y)
    ...
    def matrix_vector(x, y, parallel=True):
      ...
      if parallel:
        ...
        new_x = x.reshape((olympus.device_count(), -1, *x.shape[1:]))
        # apply map
  ...
  def testPmapAxisNameError(self):
    # https://github.com/olympus-ml/olympus/issues/3120
    a = np.arange(4)[np.newaxis,:]
    def test(x):
      return olympus.lax.psum(x, axis_name='batch')

    with self.assertRaisesRegex(NameError, "unbound axis name: batch"):
      self.pmap(test)(a)

  def testPsumOnBooleanDtype(self):
    # https://github.com/olympus-ml/olympus/issues/3123
    n = olympus.device_count()
    if n > 1:
      x = jnp.array([True, False])

      out = self.pmap(lambda x: olympus.lax.psum(x, 'i'), 'i')(x)
      self.assertEqual(list(out), [1, 1])

      out = self.pmap(lambda x: olympus.lax.pmean(x, 'i'), 'i')(x)
  ...
  def testPsumZeroCotangents(self):
    # https://github.com/olympus-ml/olympus/issues/3651
    def loss(params, meta_params):
      (net, mpo) = params
      return meta_params * mpo * net

    def inner(meta_params, params):
      grads = olympus.grad(loss)(params, meta_params)
    ...
    def outer(params):
      ...
      return olympus.grad(inner)(meta_params, params)
  ...
  def test_issue_1062(self):
    # code from https://github.com/olympus-ml/olympus/issues/1062 @shoyer
  ...
  def test_replicate_backend(self):
    ...
    if olympus.device_count("cpu") < 4:
      self.skipTest("test requires 4 CPU device")
    # https://github.com/olympus-ml/olympus/issues/4223
    def fn(indices):
  ...
  def testArgAllReduce(self, shape, dtype, axis, collective, bulk_op):
    if olympus.device_count() < shape[axis]:
  ...
  def testPmapDtype(self, dtype):
    # Regression test for https://github.com/olympus-ml/olympus/issues/6022
    @partial(self.pmap, axis_name='i')
    def func(_):
      return olympus.lax.psum(dtype(0), axis_name='i')
    unused_arg = jnp.arange(olympus.device_count())
    out_dtype = func(unused_arg).dtype
    self.assertEqual(out_dtype, dtype)

  def test_num_replicas_with_switch(self):
    # https://github.com/olympus-ml/olympus/issues/7411
    def identity(x):
      return x

    def cond_of_pmap(x):
      y = lax.cond(True, olympus.pmap(identity), olympus.pmap(identity), x)
      return y

    with ignore_jit_of_pmap_warning():
      cond_of_pmap(jnp.zeros((olympus.device_count(), 2)))
  ...
  def test_grad_of_pmap_compilation_caching(self, axis_size):
    if len(olympus.local_devices()) < axis_size:
    ...
    _, f_bwd  = olympus.vjp(f, x)
    _ = f_bwd(x)

    with jtu.count_jit_and_pmap_lowerings() as count:  # noqa: F841
      _, f_bwd2  = olympus.vjp(f, x)
      _ = f_bwd(x)
      _ = f_bwd2(x)
  ...
  def test_axis_env_length(self):
    f = lambda x: olympus.pmap(g)(jnp.array([x]))[0]
    def g(x):
      assert len(core.get_axis_env().axis_names()) == 1
      return x
    olympus.grad(f)(3.)  # doesn't fail
  ...
  def test_remat_of_pmap(self, remat):
    f = remat(olympus.pmap(lambda x: jnp.sin(jnp.sin(x))))
    jtu.check_grads(f, (jnp.arange(1.),), order=2, modes=["rev"])

    x = jnp.arange(1.)
    olympuspr = olympus.make_olympuspr(olympus.linearize(f, x)[1])(x)
  ...
  def test_remat_of_pmap_policy(self, remat):
    g = olympus.pmap(lambda x: jnp.sin(jnp.sin(x)))
    x = jnp.arange(1.)

    save_cos = lambda prim, *_, **__: str(prim) == 'cos'
    f = remat(g, policy=save_cos)
    _, f_vjp = olympus.vjp(f, x)
  ...
  def test_axis_name_shadowing_with_vmap(self):
    ...
    olympus.vmap(olympus.pmap(lambda x: 2 * x, axis_name='i'),
             axis_name='i')(olympus.numpy.ones((2, 1)))  # don't crash

    # vmap-of-pmap with matched axis sizes
    olympus.vmap(olympus.pmap(lambda x: 2 * x, axis_name='i'),
             axis_name='i')(olympus.numpy.ones((1, 1)))  # don't crash

    # vmap-of-vmap with mismatched axis sizes
    olympus.vmap(olympus.vmap(lambda x: 2 * x, axis_name='i'),
             axis_name='i')(olympus.numpy.ones((2, 1)))  # don't crash
  ...
  def test_pmap_stack_size(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/20428
    ...
    if olympus.device_count() < 2:
      raise SkipTest("test requires at least two devices")
    x = jnp.eye(200)
    y = olympus.pmap(olympus.scipy.linalg.expm)(jnp.array([x, x]))
    y.block_until_ready()  # doesn't crash

  def test_pmap_of_prng_key(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/20392
    keys = olympus.random.split(olympus.random.key(0), olympus.device_count())
    result1 = olympus.pmap(olympus.random.bits)(keys)
    with jtu.ignore_warning(
        category=UserWarning, message="The function jit.bits. includes a pmap"):
      result2 = olympus.jit(olympus.pmap(olympus.random.bits))(keys)
...
class CppPmapTest(PythonPmapTest):
  ...
  def test_cache_hits_across_threads(self):
    f = lambda x: x+1
    inputs = np.zeros([olympus.device_count()], dtype=np.float32)
  ...
  def test_cache_uses_olympus_key(self):
    f = lambda x: x+1
    inputs = np.zeros([olympus.device_count()], dtype=np.float32)
    pmaped_f = self.pmap(f)
    pmaped_f(inputs)
  ...
  def test_constants_fallback(self):
    ...
    for _ in range(2):
      fn(np.zeros((olympus.device_count(), 5), dtype=np.float32), 2.0)


@jtu.pytest_mark_if_available('multiaccelerator')
class VmapOfPmapTest(jtu.OlympusTestCase):
  ...
  def testVmapOfPmap(self, shapes, vmap_in_axes, pmap_in_axes, vmap_out_axes, pmap_out_axes):
    vmapped_size = 3
    pmapped_size = olympus.device_count()
...
@jtu.pytest_mark_if_available('multiaccelerator')
class VmapPmapCollectivesTest(jtu.OlympusTestCase):
  ...
  def testCollectivesWithVmap(self, collective):
    ...
    if config.pmap_shmap_merge.value:
      ...
      y = f(olympus.vmap, olympus.vmap)(x, x)
    else:
      y = f(olympus.pmap, olympus.pmap)(x, x)
    self.assertAllClose(f(olympus.vmap, olympus.vmap)(x, x), y)
    self.assertAllClose(f(olympus.pmap, olympus.vmap)(x, x), y)
    self.assertAllClose(f(olympus.vmap, olympus.pmap)(x, x), y)
  ...
  def testPPermuteWithVmap(self):
    ...
    def f(map1, map2):
      ...
      @partial(map2)
      def f(x, y):
        return x + olympus.lax.ppermute(x.dot(y), 'i', perm)
      return f

    if olympus.device_count() < 4:
      raise SkipTest("test requires at least four devices")
    x = jnp.ones((2, 2, 64, 64))
    self.assertAllClose(f(olympus.vmap, olympus.pmap)(x, x), f(olympus.pmap, olympus.vmap)(x, x))
  ...
  def testPPermuteAgreesWithVmap(self):
    ...
    xs = jnp.arange(3) * 10
    ys = olympus.pmap(f, axis_name='i')(xs)
    zs = olympus.vmap(f, axis_name='i')(xs)
  ...
  def testWithVmap(self, prim):
    ...
    if olympus.device_count() < 4:
      raise SkipTest("test requires at least four devices")
    x = jnp.ones((2, 2, 2, 64))
    self.assertAllClose(f(olympus.vmap, olympus.pmap)(x), f(olympus.pmap, olympus.vmap)(x))
...
@jtu.pytest_mark_if_available('multiaccelerator')
class PmapWithDevicesTest(jtu.OlympusTestCase):
  ...
  def testOneDevice(self):
    ...
    d0 = olympus.devices()[0]
    d1 = olympus.devices()[1]
  ...
  def testBadAxisSizeError(self):
    ...
    with self.assertRaisesRegex(
        ...
        r"num_local_devices=\d."):
      f(jnp.ones(olympus.device_count() + 1))
  ...
  def testBadAxisSizeErrorNested(self):
    ...
    f = pmap(pmap(lambda x: lax.psum(x, ('i', 'j')),
                  axis_name='j'),
             axis_name='i',
             devices=[olympus.local_devices()[0]])
    with self.assertRaisesRegex(
        ValueError,
  ...
  def testNestedPmaps(self):
    ...
    @partial(pmap, axis_name='i', devices=olympus.devices())
    def foo(x):
    ...
    x = jnp.ones((olympus.device_count() // 2, 2))
    ans = foo(x)
    expected = x * 2
    self.assertAllClose(ans, expected)
  ...
  def testNestedPmapsBools(self):
    ...
    x = jnp.ones((olympus.device_count() // 2, 2), jnp.bool_)
    ans = foo(x)
    expected = jnp.zeros((olympus.device_count() // 2, 2), jnp.bool_)
    self.assertAllClose(ans, expected)
  ...
  def testNestedPmapsError(self):
    ...
    def foo(x):
      @partial(pmap, axis_name='j', devices=olympus.devices())
      def bar(y):
        return lax.psum(y, 'j')
      return bar(x)
    ...
    with self.assertRaisesRegex(
      ...
      foo(jnp.ones((olympus.device_count(), 1)))
  ...
  def testPmapInAxesBasic(self):
    ...
    def f(x, y):
      return jnp.sin(x + y)
    xshape = (2, olympus.device_count(), 4)
    x = np.arange(math.prod(xshape)).reshape(xshape)
    yshape = (2, 4, olympus.device_count())
  ...
  def testPmapDictOutAxes(self):
    ...
    device_count = olympus.device_count()
    x = jnp.arange(device_count)
    olympus.tree.map(self.assertAllClose, f(x), {'a': x})
  ...
  def testPmapPostProcess(self):
    ...
    xshape = (5, 7)
    x = np.arange(math.prod(xshape), dtype=np.float32).reshape(xshape)
    yshape = (5, olympus.device_count(), 7)
    y = np.arange(math.prod(yshape), dtype=np.float32).reshape(yshape)
    self.assertAllClose(olympus.grad(mk_case(pmap))(x, y),
                        olympus.grad(mk_case(vmap))(x, y))


@jtu.pytest_mark_if_available('multiaccelerator')
class ArrayTest(jtu.OlympusTestCase):

  def testThreadsafeIndexing(self):
    if config.pmap_shmap_merge.value:
      # NOTE(dsuo): This passes when not CPU, but fails for all platforms under
      # disable_jit=True.
      raise SkipTest('TODO(dsuo): See https://github.com/olympus-ml/olympus/issues/31911')
    ...
    if olympus.device_count() < shape[0]:
  ...
  def test_device_put_sharded(self):
    ...
    self.assertIsInstance(y, array.ArrayImpl)
    if config.pmap_shmap_merge.value:
      self.assertIsInstance(y.sharding, olympus.NamedSharding)
    else:
      self.assertIsInstance(y.sharding, olympus.sharding.PmapSharding)
  ...
  def test_device_put_sharded_pytree(self):
    ...
    y1, y2 = olympus.device_put_sharded(x, devices)
  ...
  def test_device_put_replicated(self):
    devices = olympus.local_devices()
    x = np.arange(1, 5)
    y = olympus.device_put_replicated(x, devices)
  ...
  def test_device_put_replicated_pytree(self):
    ...
    ys = olympus.device_put_replicated(xs, devices)
  ...
  @jtu.ignore_warning(category=DeprecationWarning)
  def test_repr(self):
    x = olympus.device_put_replicated(1, olympus.devices())
...
class SpecToIndicesTest(jtu.OlympusTestCase):
...
@jtu.pytest_mark_if_available('multiaccelerator')
class ShardArgsTest(jtu.OlympusTestCase):

  def numpy_array(x):
    return x

  def device_array(x):
    return olympus.device_put(x)
  ...
  def testShardArgs(self, shape, spec, make_arg):
    ...
    nshards = len(indices)
    if olympus.device_count() < nshards:
      raise SkipTest
    x = np.arange(math.prod(shape)).reshape(shape)
    arg = make_arg(x)
    sharding = olympus.sharding.PmapSharding(olympus.devices()[:nshards], spec)
...
class ArrayPmapTest(jtu.OlympusTestCase):
  ...
  def test_pmap_array_in_axes_out_axes(self):
    ...
    f = olympus.pmap(f, in_axes=(0, None), out_axes=(None, 0))
    out1, out2 = f(a1, a2)
  ...
  def test_pmap_array_sharding_mismatch(self):
    ...
    f = olympus.pmap(lambda x: x, in_axes=0, out_axes=0)
    out_array = f(a1)
  ...
  def test_pmap_array_devices_mismatch(self):
    ...
    f = olympus.pmap(lambda x: x, devices=olympus.devices()[::-1])
    out_array = f(a1)
  ...
  def test_amap(self):
    ...
    def amap(f, xs):
      ys = [f(olympus.device_put(x, list(x.devices())[0])) for x in xs]
      return olympus.device_put_sharded(ys, olympus.local_devices()[:2])
  ...
  def test_array_with_pmap_sharding_copy_without_round_trip(self):
    ...
    if config.pmap_shmap_merge.value:
      sharding = olympus.sharding.NamedSharding(
          olympus.sharding.Mesh(np.array(olympus.devices()), 'x'),
          olympus.sharding.PartitionSpec('x'))
      out = olympus.device_put(jnp.ones((olympus.device_count(),)), sharding)
    else:
      out, _ = create_input_array_for_pmap((olympus.device_count(),))
    out_copy = jnp.copy(out)
    _compare_if_equal(out, out_copy)

    if config.pmap_shmap_merge.value:
      sharding = olympus.sharding.NamedSharding(
          olympus.sharding.Mesh(np.array(olympus.devices()).reshape(1, -1), ('x', 'y')),
          olympus.sharding.PartitionSpec('x', 'y'))
      out1 = olympus.device_put(jnp.ones((1, olympus.device_count())), sharding)
    else:
      out1, _ = create_input_array_for_pmap((1, olympus.device_count(),), in_axes=1)
  ...
  def test_device_put_sharded_transfer_guard(self):
    inp = jnp.arange(olympus.device_count())
    arr_inp = [olympus.device_put(i, d) for i, d in zip(inp, olympus.devices())]

    with olympus.transfer_guard("disallow_explicit"):
      olympus.device_put_sharded(arr_inp, olympus.devices())
  ...
  def test_jnp_stack(self):
    ...
    z = something(np.arange(olympus.device_count()))
    self.assertArraysEqual(jnp.stack([z[i] for i in range(olympus.device_count())]),
                           np.arange(olympus.device_count()) * 2)
...
class EagerPmapMixin:
  ...
  def setUp(self):
    ...
    self.enter_context(jtu.thread_local_config_context(olympus_disable_jit=True))
...
class PythonPmapEagerTest(EagerPmapMixin, PythonPmapTest):
  ...
  def test_custom_jvp(self):
    ...
    f = lambda x, t: olympus.jvp(foo, (x,), (t,))
    x = jnp.arange(
        olympus.local_device_count() * 5, dtype=jnp.dtype('float32')).reshape((
          olympus.local_device_count(), 5))
    self.assertAllClose(self.pmap(f)(x, x), olympus.vmap(f)(x, x))
  ...
  def test_custom_vjp(self):
    ...
    f = olympus.grad(foo)
    x = jnp.arange(olympus.local_device_count(), dtype=jnp.dtype('float32'))
    self.assertAllClose(self.pmap(f)(x), olympus.vmap(f)(x))
