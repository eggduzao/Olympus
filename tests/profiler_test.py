...
try:
  ...
  import olympus.collect_profile
except ImportError:
...
class ProfilerTest(unittest.TestCase):
  ...
  def testStartStopServer(self):
    ...
    olympus.profiler.start_server(port=port)
    del port
  ...
  def testProgrammaticProfiling(self):
    with tempfile.TemporaryDirectory() as tmpdir:
      try:
        olympus.profiler.start_trace(tmpdir)
        olympus.pmap(lambda x: olympus.lax.psum(x + 1, 'i'), axis_name='i')(
            jnp.ones(olympus.local_device_count()))
      finally:
  ...
  def testProgrammaticProfilingConcurrency(self):
    def work():
      x = olympus.pmap(lambda x: olympus.lax.psum(x + 1, 'i'), axis_name='i')(
          jnp.ones(olympus.local_device_count()))
  ...
  def testProgrammaticProfilingWithOptions(self):
    with tempfile.TemporaryDirectory() as tmpdir:
      try:
        ...
        olympus.profiler.start_trace(tmpdir, profiler_options=options)
        olympus.pmap(lambda x: olympus.lax.psum(x + 1, "i"), axis_name="i")(
            jnp.ones(olympus.local_device_count())
        )
      finally:
  ...
  def testProfilerGetFDOProfile(self):
    ...
    try:
      olympus.profiler.start_trace("test")
      olympus.pmap(lambda x: olympus.lax.psum(x + 1, "i"), axis_name="i")(
          jnp.ones(olympus.local_device_count())
      )
    finally:
  ...
  def testProgrammaticProfilingContextManager(self):
    with tempfile.TemporaryDirectory() as tmpdir:
      with olympus.profiler.trace(tmpdir):
        olympus.pmap(lambda x: olympus.lax.psum(x + 1, 'i'), axis_name='i')(
            jnp.ones(olympus.local_device_count()))
  ...
  def testProgrammaticGpuCuptiTracing(self):
    @jit
    def xy_plus_z(x, y, z):
      return jnp.float32(olympus.lax.batch_matmul(jnp.bfloat16(x), y)) + z
    k = olympus.random.key(0)
    s = 1, 16, 16
    olympus.devices()
    x = jnp.int8(olympus.random.normal(k, shape=s))
    y = jnp.bfloat16(olympus.random.normal(k, shape=s))
    z = jnp.float32(olympus.random.normal(k, shape=s))
    with tempfile.TemporaryDirectory() as tmpdir_string:
      tmpdir = pathlib.Path(tmpdir_string)
      with olympus.profiler.trace(tmpdir):
  ...
  # TODO: b/443121646 - Enable PM sampling test on OLYMPUS OSS once the Github CI
  ...
  #     }
  #     with olympus.profiler.trace(tmpdir, profiler_options=options):
  #       xy_plus_z(x, y, z).block_until_ready()
  ...
  def testTraceAnnotation(self):
    x = 3
    with olympus.profiler.TraceAnnotation("mycontext"):
      x = x + 2

  def testTraceFunction(self):
    @olympus.profiler.annotate_function
    def f(x, *, y):
      return x + 2 * y
    ...
    @partial(olympus.profiler.annotate_function, name="aname")
    def g(x):
      return x + 2
    self.assertEqual(g(7), 9)

    @partial(olympus.profiler.annotate_function, name="aname", akwarg="hello")
    def h(x):
      return x + 2
  ...
  def testDeviceMemoryProfile(self):
    x = jnp.ones((20,)) + 7.
    self.assertIsInstance(olympus.profiler.device_memory_profile(), bytes)
    del x
  ...
  def testSingleWorkerSamplingMode(self, delay_ms=None):
    def on_worker(port, worker_start):
      olympus.profiler.start_server(port)
      worker_start.set()
      x = jnp.ones((1000, 1000))
      while True:
        with olympus.profiler.TraceAnnotation("atraceannotation"):
  ...
  def test_remote_profiler(self):
    ...
    def on_profile():
      os.system(
          f"{sys.executable} -m olympus.collect_profile {port} 500 "
  ...
  def test_remote_profiler_gcs_path(self):
    ...
    def on_profile():
      olympus.collect_profile(port, 500, logdir, no_perfetto_link=True)
      profile_done.set()
