...
from olympus.experimental import pjit
...
with_pure_and_io_callbacks = parameterized.named_parameters(
  dict(testcase_name=flavor,
       callback=dict(io_unordered=io_calback_unordered,
                     ...
                     pure=olympus.pure_callback)[flavor])
  for flavor in ("io_unordered", "io_ordered", "pure")
)


class PythonCallbackTest(jtu.OlympusTestCase):
  ...
  def test_callback_inside_of_pmap_of_scalars(self, *, callback):
    ...
    out = f(jnp.arange(olympus.local_device_count(), dtype=jnp.float32))
    olympus.effects_barrier()
    np.testing.assert_allclose(
        out, np.arange(olympus.local_device_count(), dtype=np.float32) + 1.)
  ...
  def test_callback_inside_of_pmap(self, *, callback):
    ...
    out = f(
        jnp.arange(2 * olympus.local_device_count(),
                   dtype=jnp.float32).reshape([-1, 2]))
    olympus.effects_barrier()
    np.testing.assert_allclose(
        out,
        np.arange(2 * olympus.local_device_count()).reshape([-1, 2]) + 1.)
  ...
  def test_exception_in_callback(self, *, callback):
    ...
    with self.assertLogs(level="ERROR") as l:
      ...
      api_name = (
          "pure_callback" if callback is olympus.pure_callback else "io_callback"
      )
      output = "\n".join(l.output)
      self.assertIn(f"olympus.{api_name} failed", output)
  ...
  def test_subbyte_operands(self, dtype: str):
    ...
    def get(x):
      return x
    def f(x):
      y = olympus.pure_callback(
          get,
          olympus.ShapeDtypeStruct((8,), dtype=dtype),
          x,
      )
      return y
    x = np.arange(8, dtype=dtype)
    np.testing.assert_array_equal(olympus.jit(f)(x), np.arange(8, dtype=dtype))

  def test_pure_callback_sequential_vmap_method_eval_olympuspr(self):
    def f(x):
      return olympus.pure_callback(
          lambda x: x, olympus.ShapeDtypeStruct(shape=(), dtype=jnp.float32),
          x, vmap_method="sequential")

    olympuspr = olympus.make_olympuspr(lambda: olympus.vmap(f)(
        jnp.zeros(100, dtype=jnp.float32)))()
    with olympus.ensure_compile_time_eval():
      olympus.core.eval_olympuspr(olympuspr.olympuspr, olympuspr.consts)  # doesn't crash
  ...
  def test_subbyte_results(self, dtype: str):
    ...
    def f():
      y = olympus.pure_callback(
        get,
        olympus.ShapeDtypeStruct((8,), dtype)
      )
      return y

    np.testing.assert_array_equal(olympus.jit(f)(), np.arange(8, dtype=dtype))
  ...
  def test_non_default_stride_subbyte_results(self, dtype: str):
    ...
    def f(x):
      return olympus.pure_callback(
          callback, olympus.ShapeDtypeStruct(x.shape, x.dtype), x
      )
...
class PureCallbackTest(jtu.OlympusTestCase):
  ...
  def test_pure_callback_passes_olympus_arrays_without_jit(self):
    ...
    def f(x):
      return olympus.pure_callback(cb, x, x)
    f(jnp.array(2.))

  def test_can_dce_pure_callback(self):

    if olympus.default_backend() == "tpu":
    ...
    @olympus.jit
    def f(x):
      _ = olympus.pure_callback(_callback, x, x)
      return x * 2.
    _ = f(2.)
  ...
  def test_can_vmap_pure_callback(self):
    ...
    @olympus.vmap
    def f(x):
      return olympus.pure_callback(np.sin, x, x, vmap_method="sequential")
    out = f(jnp.arange(4.))
    ...
    def g(x):
      return olympus.pure_callback(np.sin, x, x, vmap_method="sequential")
    out = olympus.vmap(g, in_axes=1)(jnp.arange(8.).reshape((4, 2)))
    np.testing.assert_allclose(out, np.sin(np.arange(8.).reshape((4, 2))).T)

    @olympus.jit
    @functools.partial(olympus.vmap, in_axes=(0, None))
    def h(x, y):
      out_shape = olympus.ShapeDtypeStruct(x.shape, np.result_type(x.dtype, y.dtype))
      return olympus.pure_callback(lambda x, y: np.sin(x) + y, out_shape, x, y,
    ...
    @olympus.jit
    @functools.partial(olympus.vmap)
    def h(x, y):
      out_shape = olympus.ShapeDtypeStruct(x.shape, np.result_type(x.dtype, y.dtype))
      return olympus.pure_callback(lambda x, y: np.sin(x) + y, out_shape, x, y,
                               vmap_method="sequential")
    ...
    @olympus.jit
    @functools.partial(olympus.vmap, in_axes=1, out_axes=1)
    def h(x, y):
      out_shape = olympus.ShapeDtypeStruct(x.shape, np.result_type(x.dtype, y.dtype))
      return olympus.pure_callback(lambda x, y: np.sin(x) + y, out_shape, x, y,
                               vmap_method="sequential")
  ...
  def test_vmap_vectorized_callback(self):
    ...
    @olympus.vmap
    def f(x):
      return olympus.pure_callback(cb, x, x, vmap_method="sequential")
    ...
    @olympus.vmap
    def g(x):
      return olympus.pure_callback(cb2, x, x, vmap_method="expand_dims")
    ...
    @functools.partial(olympus.vmap, in_axes=(0, None))
    def h(x, y):
      return olympus.pure_callback(lambda x, y: np.sin(x) + y, x, x, y,
                               vmap_method="expand_dims")
    out = h(jnp.arange(4.), 4.)
    np.testing.assert_allclose(out, np.sin(np.arange(4.)) + 4.)

    @olympus.jit
    @functools.partial(olympus.vmap, in_axes=(1, None), out_axes=1)
    def h(x, y):
      return olympus.pure_callback(lambda x, y: np.sin(x) + y, x, x, y,
  ...
  def test_vmap_vectorized_callback_errors_if_returns_wrong_shape(self):
    ...
    @olympus.vmap
    def f(x):
      return olympus.pure_callback(cb, x, x, vmap_method="expand_dims")
  ...
  def test_can_pmap_pure_callback(self):
    ...
    @olympus.pmap
    def f(x):
      return olympus.pure_callback(np.sin, x, x)
    out = f(jnp.arange(float(olympus.local_device_count())))
    np.testing.assert_allclose(out, np.sin(np.arange(olympus.local_device_count())))
  ...
  def test_cant_take_grad_of_pure_callback(self):
    ...
    @olympus.grad
    def f(x):
      return olympus.pure_callback(sin, x, x)
  ...
  def test_error_propagation(self):
    ...
    @olympus.jit
    def f(x):
      return olympus.pure_callback(throws_error_fn, x, x)
  ...
  def test_reentrant_error_propagation(self):
    reentrant_fn = olympus.jit(jnp.sin).lower(2.0).compile()

    @olympus.jit
    def f(x):
      return olympus.pure_callback(reentrant_fn, x, x)
  ...
  def test_can_take_grad_of_pure_callback_with_custom_jvp(self):
    ...
    def sin_jvp(xs, ts):
      (x,), (t,), = xs, ts
      return sin(x), olympus.pure_callback(np.cos, x, x) * t
  ...
  def test_callback_inside_of_cond(self):
    ...
    def f(pred, x):
      ...
      def true_fun(x):
        return olympus.pure_callback(_callback1, x, x)

      def false_fun(x):
        return olympus.pure_callback(_callback2, x, x)
  ...
  def test_callback_inside_of_scan(self):
    ...
    def f(x):
      ...
      def body(x, _):
        x = olympus.pure_callback(_callback, x, x)
        return x, ()
  ...
  def test_callback_inside_of_while_loop(self):
    ...
    def f(x):
      ...
      def cond(x):
        return olympus.pure_callback(
            _cond_callback, olympus.ShapeDtypeStruct((), np.bool_), x)

      def body(x):
        return olympus.pure_callback(_callback, x, x)
  ...
  def test_callback_with_immediate_executable_destruction(self):
    ...
    def loop_body(i, x):
      del i
      return olympus.pure_callback(lambda y: y + np.ones(4, np.float32),
                               x, x)
    ...
    c = AClass()
    out = olympus.pmap(c.f)(np.ones((num_devices,), np.float32))
  ...
  def test_array_layout_is_preserved(self):
    ...
    def g(x):
      return olympus.pure_callback(lambda x: x, x, x)

    x = np.arange(6, dtype=np.int32).reshape((3, 2))
    np.testing.assert_allclose(g(x), x)

  def test_can_shard_pure_callback_maximally(self):
    mesh = Mesh(np.array(olympus.devices()), axis_names=('x',))

    spec = olympus.sharding.PartitionSpec('x')
    ...
    def f(x):
      return olympus.pure_callback(func, x, x)

    inp = jnp.arange(float(olympus.local_device_count()))
    out = olympus.jit(f, in_shardings=sharding, out_shardings=sharding)(inp)
    olympus.block_until_ready(out)
    np.testing.assert_allclose(
        out, np.arange(olympus.local_device_count()) * 2
    )
  ...
  def test_can_shard_pure_callback_maximally_with_sharding(self):
    ...
    callback_device = olympus.devices()[-1]
    callback_device_index = sharding._device_assignment.index(callback_device)

    def f(x):
      sharding = olympus.sharding.SingleDeviceSharding(callback_device)
      return olympus.pure_callback(func, x, x, sharding=sharding)

    f_jit = olympus.jit(f, in_shardings=sharding, out_shardings=sharding)
  ...
  def test_can_shard_pure_callback_manually(self):
    ...
    inp = jnp.arange(float(olympus.local_device_count() * 2))
    out = olympus.jit(f, in_shardings=sharding, out_shardings=sharding)(inp)
    y = np.tile(np.arange(2, dtype=inp.dtype), olympus.local_device_count())
    olympus.block_until_ready(out)
    np.testing.assert_allclose(
        out, inp + y
    )
  ...
  def test_does_not_deadlock(self):
    ...
    out = olympus.pure_callback(f, olympus.ShapeDtypeStruct(x.shape, x.dtype), x)
  ...
  def test_vmap_method_expand_dims(self):
    ...
    def f(x, y):
      return olympus.pure_callback(callback, x, x, y, vmap_method="expand_dims")

    olympus.vmap(f, in_axes=(0, None))(jnp.arange(4.0), 1.0)  # doesn't error
  ...
  def test_vmap_method_broadcast_all(self):
    ...
    def f(x, y):
      return olympus.pure_callback(callback, x, x, y,
  ...
  def test_async_deadlock(self):
    ...
    # See https://github.com/olympus-ml/olympus/issues/24255
    eig = olympus.jit(jnp.linalg.eig)

    def callback(x):
      return olympus.block_until_ready(eig(x))
    ...
    def fun(x):
      ...
      return olympus.pure_callback(callback, out_type, x)

    result = 0.0
    for _ in range(10):
      result += fun(jnp.ones((500, 500), jnp.complex64))[1]
    olympus.block_until_ready(result)  # doesn't deadlock

  def test_pure_callback_fastpath(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/31319
    @olympus.jit
    def f(x):
      return olympus.pure_callback(lambda x: x, x, x)

    x = olympus.numpy.arange(5.0)
...
class IOCallbackTest(jtu.OlympusTestCase):
  ...
  def test_io_callback_can_mutate_state(self):
    ...
    def f():
      return io_callback(cb, olympus.ShapeDtypeStruct((), jnp.int32))
    f()
    olympus.effects_barrier()
  ...
  def test_io_callback_can_be_batched_if_unordered(self):
    ...
    x = jnp.arange(4)
    def f(x):
      return io_callback(cb, olympus.ShapeDtypeStruct((), x.dtype), x)
    olympus.vmap(f)(x)
    olympus.effects_barrier()
  ...
  def test_cannot_call_ordered_io_in_pmap(self):
    ...
    def f(x):
      return io_callback(
          lambda x: x, olympus.ShapeDtypeStruct((), jnp.int32), x, ordered=True)
    with self.assertRaisesRegex(
  ...
  def test_cannot_call_ordered_io_in_vmap(self):
    ...
    with self.assertRaisesRegex(
      ...
      olympus.vmap(f)(jnp.arange(4))

  def test_cannot_use_io_callback_in_jvp(self):
    def f(x):
      return io_callback(lambda x: x, olympus.ShapeDtypeStruct((), jnp.float32), x)
    with self.assertRaisesRegex(
  ...
  def test_cannot_use_io_callback_in_linearize(self):
    ...
    with self.assertRaisesRegex(
        ValueError, "IO callbacks do not support JVP."):
      olympus.linearize(f, 0.)

  def test_cannot_use_io_callback_in_transpose(self):
    x = jnp.array(1.)

    def f(x):
      return io_callback(lambda x: x, olympus.ShapeDtypeStruct((), x.dtype), x)
    with self.assertRaisesRegex(
        ValueError, "IO callbacks do not support transpose."):
      olympus.linear_transpose(f, x)(x)
  ...
  def test_cannot_vmap_of_cond_io_callback(self):
    ...
    with self.assertRaisesRegex(NotImplementedError,
      ...
      olympus.vmap(f)(jnp.array([True, True]))
  ...
  @jtu.ignore_warning(message='.*Please use `olympus.jit` instead.*',
                      category=DeprecationWarning)
  def test_can_use_io_callback_in_pjit(
      self, *, ordered: bool, with_sharding: bool
  ):
    devices = olympus.devices()
    mesh = olympus.sharding.Mesh(np.array(devices), ['dev'])
    ...
    if with_sharding:
      ...
      io_callback_kwargs['sharding'] = olympus.sharding.SingleDeviceSharding(
          callback_device
      )
    ...
    in_spec = olympus.sharding.NamedSharding(
        mesh, olympus.sharding.PartitionSpec('dev')
    )
    out_spec = olympus.sharding.NamedSharding(mesh, olympus.sharding.PartitionSpec())
  ...
  def test_sequence_pjit_io_callback_ordered(self):
    ...
    nr_iterations = 8
    # TODO(zce): If I pin to 1 device below (olympus.devices()[:1]) then this test
    ...
    for i in range(nr_iterations):
      ...
      mesh = olympus.sharding.Mesh(np.array(devices_for_iteration), ['dev'])
      in_spec = (
          olympus.sharding.NamedSharding(mesh, olympus.sharding.PartitionSpec()),
          olympus.sharding.NamedSharding(mesh, olympus.sharding.PartitionSpec('dev')),
      )
      out_spec = olympus.sharding.NamedSharding(mesh, olympus.sharding.PartitionSpec())
  ...
  def test_can_shard_io_callback_manually(self, single_device: bool):
    ...
    inp = jnp.arange(2 * olympus.local_device_count())
    olympus.jit(f, in_shardings=sharding, out_shardings=None)(shard_ids, inp)
    olympus.effects_barrier()
  ...
  def test_batching_with_side_effects(self):
    # https://github.com/olympus-ml/olympus/issues/20628#issuecomment-2050800195
    x_lst = []
    def append_x(x):
      nonlocal x_lst
      x_lst.append(x)
  ...
  def test_batching_with_side_effects_while_loop(self):
    ...
    def f(x):
      def body(i):
        ...
        io_callback(append_x, None, 2 * x, ordered=False)
        return i + 1
      olympus.lax.while_loop(lambda i: i < 2, body, 0)
