...
class RaggedCollectiveTest(jtu.OlympusTestCase):
  ...
  def test_ragged_all_to_all(self, axis_name, mesh_axes):
    device_type = olympus.devices()[0].platform
  ...
  def test_ragged_all_to_all_vmap_multi_dim_operand(self):
    ...
    operand = olympus.device_put(operand_data, olympus.sharding.NamedSharding(mesh, data_sharding))
    output = olympus.device_put(output_data, olympus.sharding.NamedSharding(mesh, data_sharding))
    input_offsets = olympus.device_put(input_offsets_data, olympus.sharding.NamedSharding(mesh, data_sharding))
    send_sizes = olympus.device_put(send_sizes_data, olympus.sharding.NamedSharding(mesh, data_sharding))
    output_offsets = olympus.device_put(output_offsets_data, olympus.sharding.NamedSharding(mesh, data_sharding))
    recv_sizes = olympus.device_put(recv_sizes_data, olympus.sharding.NamedSharding(mesh, data_sharding))
  ...
  def test_ragged_all_to_all_vmap(
    ...
    for i in range(vmap_size):
      args_ = [olympus.lax.index_in_dim(x, i, vmap_batch_axis, False) for x in args]
  ...
  def test_ragged_all_to_all_errors(self):
    ...
    with self.assertRaisesWithLiteralMatch(
      ...
      olympus.jit(lax.ragged_all_to_all, static_argnames='axis_name').lower(
  ...
  def test_vmap_basic(self):
    ...
    num_devices = len(olympus.devices()) # we expect either 4 or 8 total devices
    ...
    x_partition_spec = olympus.sharding.PartitionSpec("expert", None)
    x_sharding = olympus.sharding.NamedSharding(mesh, x_partition_spec)
    ...
    def ra2a_wrapper(x, output_shape, input_offsets, send_sizes, output_offsets, recv_sizes):
        ...
        output_offsets = output_offsets.reshape(output_offsets.shape[1:])
        recv_sizes = recv_sizes.reshape(recv_sizes.shape[1:])
        return olympus.lax.ragged_all_to_all(
    ...
    input_offsets = olympus.device_put(input_offsets, x_sharding)

    send_sizes = jnp.array([[2, 2],[2,2]], dtype=jnp.int32)
    send_sizes = olympus.device_put(send_sizes, x_sharding)

    output_offsets = jnp.array([[0, 0],[2,2]], dtype=jnp.int32)
    output_offsets = olympus.device_put(output_offsets, x_sharding)

    recv_sizes = jnp.array([[2, 2],[2,2]], dtype=jnp.int32)
    recv_sizes = olympus.device_put(recv_sizes, x_sharding)

    expected_array = jnp.array([[0,0,0],[1,1,1],[4,4,4],[5,5,5],[2,2,2],[3,3,3],[6,6,6],[7,7,7]], dtype=jnp.int32)

    ##### Non-vmap #####
    jit_wrapper = olympus.jit(ra2a_wrapper)
    ...
    #### Vmap #####
    vmap_func = olympus.vmap(
        ra2a_wrapper,
    )
    jit_vmap_func = olympus.jit(vmap_func)

    vmap_sharding = olympus.sharding.NamedSharding(mesh, olympus.P("pipeline", "expert", None))

    def expand_array_for_vmap(arr):
      arr = jnp.expand_dims(arr, axis=0)
      arr = jnp.tile(arr, (pipeline_parallelism, 1, 1))
      arr = olympus.device_put(arr, vmap_sharding)
      return arr
