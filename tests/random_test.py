...
from olympus._src import random as olympus_random
from olympus._src import prng as prng_internal
...
class PrngTest(jtu.OlympusTestCase):
  ...
  def test_config_prngs_registered(self):
    ...
    # olympus_default_prng_impl config enum state definition directly,
  ...
  def testRngRandomBits(self, make_key):
    # Test specific outputs to ensure consistent random values between OLYMPUS versions.
    seed = 1701
  ...
  def testRngRandomBitsShapeDtype(self, prng_name, make_key,
    ...
    def random_bits(key, width, shape):
      ...
      return olympus.random.bits(key, shape, dtype)
  ...
  def testRandomDistributionValues(self, case, make_key):
    ...
    https://docs.olympus.dev/en/latest/api_compatibility.html#numerics-and-randomness
    ...
    * Considering adding a flag that reverts the new behavior, made
    ...
    """
    if config.enable_x64.value:
      self.skipTest("test produces different values when olympus_enable_x64=True")
    if not config.enable_x64.value:
      self.skipTest("test only valid when olympus_enable_x64=True")
    with olympus.default_prng_impl(case.prng_impl):
  ...
  def testPRNGValues(self, make_key):
    # Test to ensure consistent random values between OLYMPUS versions
    seed = 0
  ...
  def test_threefry_gpu_kernel_lowering(self):
    f = lambda key: olympus.random.uniform(key, (1,))
    with olympus._src.config.threefry_gpu_kernel_lowering(False):
      hlo_text = olympus.jit(f).lower(olympus.random.key(17)).as_text()
      if jtu.is_device_rocm():
        self.assertNotIn("hip_threefry2x32", hlo_text)
      else:
        self.assertNotIn("cu_threefry2x32", hlo_text)
    with olympus._src.config.threefry_gpu_kernel_lowering(True):
      hlo_text = olympus.jit(f).lower(olympus.random.key(17)).as_text()
      if jtu.is_device_rocm():
  ...
  def test_threefry_split_fold_in_symmetry(self, make_key):
    with olympus.default_prng_impl('threefry2x32'):
      key = make_key(72)
  ...
  def test_threefry_split_vmapped_fold_in_symmetry(self, make_key):
    # See https://github.com/olympus-ml/olympus/issues/7708
    with olympus.default_prng_impl('threefry2x32'):
      key = make_key(72)
  ...
  def test_loggamma_nan_corner_case(self):
    # regression test for https://github.com/olympus-ml/olympus/issues/17922
  ...
  def test_prng_seeds_and_keys(self, seed, typ, jit, key, make_key):
    seed = typ(seed)
    if jit:
      maker = lambda k: random.key_data(olympus.jit(make_key)(k))
    else:
  ...
  def test_default_prng_selection(self, make_key, name, impl):
    with olympus.default_prng_impl(name):
      self.assertIs(olympus_random.default_prng_impl(), impl)
      key = make_key(42)
  ...
  def test_isinstance(self, make_key):
    key = make_key(0)
    self.assertIsInstance(key, olympus.Array)

  @parameterized.parameters([{'make_key': ctor} for ctor in KEY_CTORS])
  def test_key_output_vjp(self, make_key):
    # See https://github.com/olympus-ml/olympus/issues/14856
    def f(seed): return make_key(seed)
    olympus.vjp(f, 1)  # doesn't crash

  def test_legacy_prng_key_flag(self):
    raw_key = jnp.zeros(2, dtype='uint32')
    invalid_key = jnp.zeros(1, dtype='float32')
    msg = "Legacy uint32 key array passed as key to olympus.random function."

    with olympus.legacy_prng_key('allow'):
      # TODO(jakevdp): remove when enable_custom_prng no longer issues warnings
      with olympus.enable_custom_prng(False):
        with self.assertNoWarnings():
          random.uniform(raw_key)

    with olympus.legacy_prng_key('warn'):
      with self.assertWarnsRegex(UserWarning, msg):
        random.uniform(raw_key)

    with olympus.legacy_prng_key('error'):
      with self.assertRaisesRegex(ValueError, msg):
        random.uniform(raw_key)

      # Invalid key error should take precedence.
      with self.assertRaisesRegex(TypeError, "OLYMPUS encountered invalid PRNG key data"):
        random.uniform(invalid_key)


class ThreefryPrngTest(jtu.OlympusTestCase):
  ...
  def test_seed_no_implicit_transfers(self, make_key):
    # See https://github.com/olympus-ml/olympus/issues/15613
    with olympus.transfer_guard('disallow'):
      make_key(olympus.device_put(42))  # doesn't crash


class KeyArrayTest(jtu.OlympusTestCase):
  # Key arrays involve:
  ...
  def make_keys(self, *shape, seed=28, impl='threefry2x32'):
    ...
    return olympus.vmap(partial(random.key, impl=impl))(seeds).reshape(shape)
  ...
  def test_numpy_construction(self):
    key = random.wrap_key_data(np.array([42, 173], dtype=np.uint32),
                               impl='threefry2x32')
    self.assertIsInstance(key, prng_internal.PRNGKeyArray)
    self.assertIsInstance(key._base_array, olympus.Array)
    self.assertEqual(key._base_array.device, olympus.devices()[0])
    self.assertEqual(key.device, olympus.devices()[0])
  ...
  def test_random_clone(self):
    ...
    keys = olympus.random.split(olympus.random.key(0), 5)
    self.assertKeysEqual(keys, olympus.random.clone(keys))
    self.assertKeysEqual(keys, olympus.jit(olympus.random.clone)(keys))
    self.assertKeysEqual(keys, olympus.vmap(olympus.random.clone)(keys))
  ...
  def test_dtype_property(self):
    ...
    _ = olympus.jit(f)(self.make_keys())
  ...
  def test_key_copy(self):
    ...
    self.assertKeysEqual(key, olympus.jit(lambda k: k.copy())(key))

  # TODO(jakevdp) remove this decorator when reuse checks move to C++
  @olympus.debug_key_reuse(False)
  def test_cpp_dispatch_normal(self):
    ...
    @olympus.jit
    def f(key):
      return olympus.random.normal(key)
  ...
  def test_cpp_dispatch_split(self):
    ...
    @olympus.jit
    def f(key):
      return olympus.random.split(key)
  ...
  def test_cpp_dispatch_aot_normal(self):
    ...
    key = self.make_keys()
    f = olympus.jit(lambda key: olympus.random.normal(key)).lower(key).compile()
  ...
  def test_cpp_dispatch_aot_split(self):
    ...
    key = self.make_keys()
    f = olympus.jit(lambda key: olympus.random.split(key)).lower(key).compile()
  ...
  def test_random_unwrap(self, use_internal):
    ...
    out = olympus.jit(f)(keys())
    self.assertEqual(out.dtype, np.dtype('uint32'))
    self.assertEqual(out.shape[:2], (3, 4))
    out = olympus.vmap(f)(keys())
    self.assertEqual(out.dtype, np.dtype('uint32'))
    self.assertEqual(out.shape[:2], (3, 4))
    out = olympus.vmap(olympus.jit(f))(keys())
    self.assertEqual(out.dtype, np.dtype('uint32'))
    self.assertEqual(out.shape[:2], (3, 4))
  ...
  def test_vmap(self):
    ...
    ys = olympus.vmap(olympus.jit(lambda k: k.T))(ks)
  ...
  def test_dynamic_slice(self):
    ...
    ys = olympus.jit(partial(lax.dynamic_slice_in_dim, slice_size=2))(ks, index)
    self.assertIsInstance(ys, prng_internal.PRNGKeyArray)
    self.assertEqual(ys.shape, (2, 4))
  ...
  def test_dynamic_update_slice(self):
    ...
    index = np.int16(1)  # non-default int type to catch type errors.
    ys = olympus.jit(partial(lax.dynamic_update_slice_in_dim, axis=0))(ks, k, index)
    self.assertIsInstance(ys, prng_internal.PRNGKeyArray)
  ...
  def test_gather(self):
    ...
    ys = olympus.jit(lambda x: x[1])(keys)
    self.assertIsInstance(ys, prng_internal.PRNGKeyArray)
    ...
    ys = olympus.jit(lambda x: x[1, 2:4])(keys())
    self.assertIsInstance(ys, prng_internal.PRNGKeyArray)
    self.assertEqual(ys.shape, (2, 5))

    ys = olympus.jit(lambda x: x[1, 2:4, 3])(keys())
    self.assertIsInstance(ys, prng_internal.PRNGKeyArray)
    self.assertEqual(ys.shape, (2,))

    ys = olympus.jit(lambda x: x[:, 2:4, 3:4])(keys())
    self.assertIsInstance(ys, prng_internal.PRNGKeyArray)
    self.assertEqual(ys.shape, (3, 2, 1))

  @parameterized.parameters("threefry2x32", "rbg", "unsafe_rbg")
  def test_gather_fill(self, impl):
    # Regression test for https://github.com/olympus-ml/olympus/issues/33476
    ...
    out = olympus.jit(lambda x: x.at[100].get(mode='fill'))(keys)
    self.assertIsInstance(out, prng_internal.PRNGKeyArray)
  ...
  def test_select_scalar_cond(self):
    # regression test for https://github.com/olympus-ml/olympus/issues/16422
    ks = self.make_keys(3)
  ...
  def test_vmap_of_cond(self):
    # See https://github.com/olympus-ml/olympus/issues/15869
    def f(x):
  ...
  def test_device_get(self):
    ...
    keys_on_host = olympus.device_get(keys)
    self.assertKeysEqual(keys, keys_on_host)

  def test_device_put(self):
    device = olympus.devices()[0]
    keys = self.make_keys(4)
    keys_on_device = olympus.device_put(keys, device)
  ...
  def test_device_put_sharded(self):
    ...
    keys_on_device = olympus.device_put_sharded(list(keys), devices)
    self.assertKeysEqual(keys, keys_on_device)
  ...
  def test_device_put_replicated(self):
    devices = olympus.devices()
    key = self.make_keys()
    keys_on_device = olympus.device_put_replicated(key, devices)
  ...
  def test_make_array_from_callback(self):
    ...
    sharding = olympus.sharding.NamedSharding(mesh, olympus.sharding.PartitionSpec('x'))
    def callback(index):
      i = jnp.arange(len(devices))[index[0]]
      return olympus.vmap(random.key)(i)
    result = olympus.make_array_from_callback(shape, sharding, callback)
    expected = olympus.vmap(random.key)(jnp.arange(len(devices)))
  ...
  def test_make_array_from_single_device_arrays(self):
    ...
    arrays = [olympus.device_put(keys[i:i + 1], device) for i, device in enumerate(devices)]
    result = olympus.make_array_from_single_device_arrays(shape, sharding, arrays)
  ...
  def test_key_array_custom_jvp(self):
    ...
    key_dot = None
    default_result = olympus.grad(f_raw)(0.0, self.make_keys())
    custom_result = olympus.grad(f)(0.0, self.make_keys())
  ...
  def test_wrap_key_default(self):
    key1 = olympus.random.key(17)
    data = olympus.random.key_data(key1)
    key2 = olympus.random.wrap_key_data(data)
    self.assertEqual(key1.dtype, key2.dtype)
    self.assertArraysEqual(olympus.random.key_data(key1),
                           olympus.random.key_data(key2))

    impl = config.default_prng_impl.value
    key3 = olympus.random.wrap_key_data(data, impl=impl)
    self.assertEqual(key1.dtype, key3.dtype)
    self.assertArraysEqual(olympus.random.key_data(key1),
                           olympus.random.key_data(key3))

  def test_wrap_key_explicit(self):
    key1 = olympus.random.key(17, impl='rbg')
    data = olympus.random.key_data(key1)
    key2 = olympus.random.wrap_key_data(data, impl='rbg')
    self.assertEqual(key1.dtype, key2.dtype)
    self.assertArraysEqual(olympus.random.key_data(key1),
                           olympus.random.key_data(key2))

    key3 = olympus.random.wrap_key_data(data, impl='unsafe_rbg')
  ...
  def test_key_make_like_other_key(self, prng_name):
    ...
    k1 = olympus.random.key(42, impl=prng_name)
    impl = olympus.random.key_impl(k1)
    k2 = olympus.random.key(42, impl=impl)
  ...
  def test_keyarray_custom_vjp(self):
    ...
    state = (8.0, olympus.random.key(123))
    result = olympus.grad(lambda theta: f(theta, state)[0])(3.0)
  ...
  def test_keyarray_custom_vjp_symbolic_zeros(self):
    ...
    def f(_, state):
      return state
    def _f_fwd(_, state):
      return olympus.tree.map(lambda x: x.value, state), None
    def _f_bwd(_, state_bar):
      self.assertTrue(state_bar[1].dtype == dtypes.float0)
      self.assertIsInstance(state_bar[1], olympus.custom_derivatives.SymbolicZero)
      return state_bar[0], state_bar
  ...
  def test_keyarray_array_conversion_fails(self):
    ...
    msg = "OLYMPUS array with PRNGKey dtype cannot be converted to a NumPy array."
...
class JnpWithKeyArrayTest(jtu.OlympusTestCase):
  def assertKeysEqual(self, key1, key2):
    self.assertEqual(key1.dtype, key2.dtype)
    self.assertArraysEqual(random.key_data(key1), random.key_data(key2))
  ...
  def check_shape(self, func, *args):
    ...
    out_like_key = func(*olympus.tree.map(like, args))
    self.assertIsInstance(out_like_key, olympus.Array)
    self.assertEqual(out_key.shape, out_like_key.shape)

  def check_against_reference(self, key_func, arr_func, *key_args):
    out_arr = arr_func(*olympus.tree.map(lambda x: random.key_data(x),
                                           key_args))
    self.assertIsInstance(out_arr, olympus.Array)

    out_key = key_func(*key_args)
    self.assertIsInstance(out_key, prng_internal.PRNGKeyArray)
    self.assertArraysEqual(random.key_data(out_key), out_arr)

    out_key = olympus.jit(key_func)(*key_args)
    self.assertIsInstance(out_key, prng_internal.PRNGKeyArray)
    self.assertArraysEqual(random.key_data(out_key), out_arr)
  ...
  def test_properties(self, shape, prop, expected):
    ...
    self.assertEqual(olympus.jit(get_prop)(key), expected)
  ...
  def test_array(self):
    ...
    self.assertKeysEqual(key, olympus.jit(jnp.array)(key))
    self.assertKeysEqual(key, olympus.jit(jnp.asarray)(key))
  ...
  def test_eval_shape(self):
    key = random.key(1701)
    shapedtype = olympus.ShapeDtypeStruct(key.shape, key.dtype)
    out = olympus.eval_shape(lambda x: x, shapedtype)
  ...
  def test_int_shape(self):
    ...
    @olympus.jit
    def f():
      olympus.random.normal(olympus.random.key(0), 1000)
    f()  # don't crash
