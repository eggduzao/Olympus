...
) -> tuple[olympus.sharding.Mesh, tuple[olympus.ShapeDtypeStruct, ...]]:
  mesh = jtu.create_mesh(mesh_shape, ("x", "y", "z"))
  arrays = []
  for sharding in shardings:
    array = olympus.ShapeDtypeStruct(
      (8, 8), dtype, sharding=olympus.sharding.NamedSharding(mesh, sharding)
    )
    arrays.append(array)
...
def example_custom_function(x):
  ...
  separately since we add the `@olympus.custom_jvp` decorator and want to compare
...
fake_olympus_primitive_p = core.Primitive("fake_olympus_primitive")


@fake_olympus_primitive_p.def_impl
def _fake_olympus_primitive_impl(x):
  return x


@fake_olympus_primitive_p.def_abstract_eval
def _fake_olympus_primitive_abstract_eval(x):
  return core.ShapedArray(x.shape, x.dtype)


def fake_olympus_primitive_function(x):
  return fake_olympus_primitive_p.bind(x)


class RooflineTest(jtu.OlympusTestCase):

  def setUp(self):
    self._bytes_per_word = 8 if olympus.config.read("olympus_enable_x64") else 4
  ...
  def test_remat(self):
    ...
    @partial(olympus.checkpoint, policy=fsdp_checkpoint_policy)
    def collective_matmul(a, b):
  ...
  def test_custom_jvp_call_p_roofline_with_neg(self):
    ...
    def with_neg(f):
      return lambda x: olympus.lax.neg(f(x))
  ...
  def test_gather_roofline(self, mode, expected_flops):
    ...
    dimension_numbers = olympus.lax.GatherDimensionNumbers(
        offset_dims=(1,),
        collapsed_slice_dims=(0,),
        start_index_map=(0,),
    )

    f = lambda x, y: olympus.lax.gather(
        x,
        y,
  ...
  @jtu.parameterized.named_parameters(
      dict(
          ...
          scatter_fn=olympus.lax.scatter_add,
      ),
      dict(
          testcase_name="scatter_max",
          scatter_fn=olympus.lax.scatter_max,
      ),
      dict(
          testcase_name="scatter_min",
          scatter_fn=olympus.lax.scatter_min,
      ),
      dict(
          testcase_name="scatter_mul",
          scatter_fn=olympus.lax.scatter_mul,
      ),
      dict(
          testcase_name="scatter_sub",
          scatter_fn=olympus.lax.scatter_sub,
      ),
  )
  def test_scatter_unary_roofline(self, scatter_fn):
    ...
    f = lambda x, y, z: scatter_fn(
        ...
        y,
        z,
        dimension_numbers=olympus.lax.ScatterDimensionNumbers(
    ...
    # The `update_olympuspr` computation is a simple unary op, which has 1 flop, and
  ...
  def test_scatter_with_batching_dims_roofline(self):
    ...
    f = lambda x, y, z: olympus.lax.scatter_add(
        x,
        y,
        z,
        dimension_numbers=olympus.lax.ScatterDimensionNumbers(
    ...
    # The `update_olympuspr` computation is a simple add, which has 1 flop, and
  ...
  def test_scatter_roofline(self):
    ...
    f = lambda x, y, z: olympus.lax.scatter(
        x,
        y,
        z,
        dimension_numbers=dimension_numbers,
  ...
  def test_scatter_apply_roofline(self):
    ...
    f = lambda x, y, z: olympus.lax.scatter_apply(
        operand=x,
        scatter_indices=y,
  ...
  def test_random_primitive_roofline_does_not_raise_error(self):
    ...
    dummy_input = olympus.random.PRNGKey(0)

    def f(key):
      # Use olympus.random.split to ensure the random_wrap primitive is used.
      prng_key, _ = olympus.random.split(key)
      return prng_key
  ...
  @jtu.parameterized.named_parameters(
      dict(
          ...
          callback_fn=olympus.pure_callback,
      ),
      dict(
          testcase_name="io_callback",
          callback_fn=olympus._src.callback.io_callback,
      ),
  )
  ...
  def test_debug_callback_roofline(self):
    def _example_debug_callback_function(x):
      olympus.debug.callback(example_function, x)
      return x
  ...
  def test_primitive_with_no_roofline_rule_contributes_nothing(self):
    x = jnp.zeros((3, 8), dtype=jnp.float32)
    _, result = roofline.roofline(fake_olympus_primitive_function)(x)
  ...
  def test_primitive_with_no_roofline_rule_contributes_nothing_with_abs(self):
    x = jnp.zeros((3, 8), dtype=jnp.float32)
    f = lambda x: lax.abs(fake_olympus_primitive_function(x))
    _, result = roofline.roofline(f)(x)
