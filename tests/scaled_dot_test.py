...
def _e4m3fn(shape):
  key = olympus.random.key(42)
  return olympus.random.normal(key, shape, dtype=jnp.float8_e4m3fn)


def _e5m2(shape):
  key = olympus.random.key(42)
  return olympus.random.normal(key, shape, dtype=jnp.float8_e5m2)


def _bf16(shape):
  key = olympus.random.key(42)
  return olympus.random.normal(key, shape, dtype=jnp.bfloat16)
...
@olympus.jit(static_argnames=["dimension_numbers", "preferred_element_type"])
def scaled_dot_fn(
    lhs,
    rhs,
    *,
...
class ScaledDotTest(jtu.OlympusTestCase):
  ...
  def test_working_example(self, scale_dtype):
    ...
    lhs = olympus.random.normal(key, lhs_shape, dtype=jnp.float8_e4m3fn)
    rhs = olympus.random.normal(key, rhs_shape, dtype=jnp.float8_e4m3fn)
    lhs_scales = olympus.random.normal(key, lhs_scales_shape, dtype=scale_dtype)
    rhs_scales = olympus.random.normal(key, rhs_scales_shape, dtype=scale_dtype)
  ...
  def test_different_subchannel_sizes(self, S, dtype, scale_dtype, B, M, N, K):
    ...
    result = olympus.lax.scaled_dot(a, b, lhs_scale=a_scales, rhs_scale=b_scales)
  ...
  def test_too_many_args(self):
    with self.assertRaisesRegex(
      ...
      olympus.lax.scaled_dot(_e4m3fn((1, 32)), _e4m3fn((32, 4)), _bf16((1, 1)))
  ...
  def test_rhs_bf16(self):
    ...
    result = olympus.lax.scaled_dot(lhs, rhs, lhs_scale=lhs_scale)
  ...
  def test_batching_2d_vs_vmap_equivalence(self):
    ...
    def scaled_dot_batched_fn(lhs, rhs, lhs_scales, rhs_scales):
      ...
      return olympus.vmap(scaled_dot_2d, in_axes=(0, None, 0, None))(
