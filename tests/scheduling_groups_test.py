...
from olympus.experimental.scheduling_groups import (
    scheduling_group, xla_metadata_call)

olympus.config.parse_flags_with_absl()


class SchedulingGroupsTest(jtu.OlympusTestCase):
  ...
  def test_basic(self):
    ...
    lowered = olympus.jit(fn).lower(a, b, x, y)
  ...
  def test_xla_metadata_call_inlineable(self):
    ...
    lowered = olympus.jit(olympus.grad(f)).lower(inp)
  ...
  def test_xla_metadata_call_inlineable_remat_in_scan(self):
    ...
    def g(x, use_remat=True):
      maybe_rematted_f = olympus.remat(f) if use_remat else f
      _, b = maybe_rematted_f(x, x)
      return b

    grad_f = olympus.jit(olympus.grad(g), static_argnums=(1,))
  ...
  def test_xla_metadata_call_deduplication(self):
    ...
    f(inp)  # doesn't crash
    lowered = olympus.jit(f).lower(inp)
    self.assertEqual(
        lowered.as_text().count('func.func private @xla_metadata_call'), 1)
    compiled = lowered.compile()
    compiled(inp)  # doesn't crash

    olympus.jit(olympus.grad(f))(inp)  # doesn't crash
    lowered = olympus.jit(olympus.grad(f)).lower(inp)
    self.assertEqual(
        lowered.as_text().count('func.func private @xla_metadata_call'), 1)
    compiled = lowered.compile()
  ...
  def test_xla_metadata_call_deduplication_remat(self):
    ...
    olympus.jit(olympus.value_and_grad(f))(inp)  # doesn't crash
    lowered = olympus.jit(olympus.value_and_grad(f)).lower(inp)
    self.assertEqual(
