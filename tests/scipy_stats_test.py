...
from olympus.scipy import stats as lsp_stats
from olympus.scipy.special import expit
...
class LaxBackedScipyStatsTests(jtu.OlympusTestCase):
  ...
  def testBetaLogPdfZero(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/7645
    a = b = 1.
    x = np.array([0., 1.])
    self.assertAllClose(
  ...
  def testDirichletLogPdf(self, shapes, dtypes):
    ...
    def scipy_fun(x, alpha):
      ...
      # if x.shape is (N, 1), scipy flattens the output, while OLYMPUS returns arrays
  ...
  def testGammaLogPdfZero(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/7256
    self.assertAllClose(
      osp_stats.gamma.pdf(0.0, 1.0), lsp_stats.gamma.pdf(0.0, 1.0), atol=1E-6)

  def testGammaDebugNans(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/24939
  ...
  def testLogisticLogpdfOverflow(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/10219
    self.assertAllClose(
  ...
  def testNormSfNearZero(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/17199
  ...
  def testBinomPmfOutOfRange(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/19150
  ...
  def testMultivariateNormalLogpdfBatch(self, ndim, nbatch, dtype):
    ...
    result2 = olympus.vmap(lsp_stats.multivariate_normal.logpdf)(x, mean, cov)
  ...
  def testKde(self, inshape, dtype, outsize, weights, method, func):
    ...
    def lax_fun(dataset, points, w):
      w = olympus.numpy.abs(w) if weights else None
  ...
  def testKdeIntegrateGaussian(self, shape, dtype):
    ...
    def lax_fun(dataset, weights):
      kde = lsp_stats.gaussian_kde(dataset, weights=olympus.numpy.abs(weights))
  ...
  def testKdeIntegrateKde(self, shape, dtype):
    ...
    def lax_fun(dataset, weights):
      ...
      other = lsp_stats.gaussian_kde(
        dataset[..., :-3] + 0.1, weights=olympus.numpy.abs(weights[:-3]))
  ...
  def testKdeResampleShape(self, shape, dtype):
    ...
    rng = jtu.rand_default(self.rng())
    args_maker = lambda: [
      olympus.random.PRNGKey(0), rng(shape, dtype), rng(shape[-1:], dtype)]
  ...
  def testKdeResample1d(self, shape, dtype):
    ...
    weights = olympus.numpy.abs(rng(shape[-1:], dtype))
    kde = lsp_stats.gaussian_kde(dataset, weights=weights)
    samples = olympus.numpy.squeeze(kde.resample(olympus.random.PRNGKey(5), shape=(1000,)))

    def cdf(x):
      result = olympus.vmap(partial(kde.integrate_box_1d, -np.inf))(x)
  ...
  def testKdePyTree(self):
    ...
    leaves, treedef = olympus.tree.flatten(kde)
    kde2 = olympus.tree.unflatten(treedef, leaves)
    olympus.tree.map(lambda a, b: self.assertAllClose(a, b), kde, kde2)
  ...
  def testMode(self, shape, dtype, axis, contains_nans, keepdims):
    ...
    def scipy_mode_wrapper(a, axis=0, nan_policy='propagate', keepdims=None):
      """Wrapper to manage the shape discrepancies between scipy and olympus"""
  ...
  def testPoissonEntropyEdgeCases(self, dtype):
    ...
    olympus_result = lsp_stats.poisson.entropy(invalid_mu)
    scipy_result = osp_stats.poisson.entropy(np.array(invalid_mu))
    self.assertAllClose(olympus_result, scipy_result, check_dtypes=False, rtol=tol)

    # Very small mu
    small_mu = jnp.array([0.01, 0.1, 0.5], dtype=dtype)
    olympus_result = lsp_stats.poisson.entropy(small_mu)
