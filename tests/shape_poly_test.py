...
from olympus._src.export import shape_poly_decision
...
from olympus._src.state import primitives as ref_primitives
...
from olympus._src.internal_test_util import test_harnesses
from olympus._src.internal_test_util.test_harnesses import Harness, CustomArg, RandArg, StaticArg
...
def _start_profile(tst: jtu.OlympusTestCase):
  tst.prof = None
  if os.getenv("OLYMPUS_PROFILE_TEST", False):
    tst.prof = cProfile.Profile()
    tst.prof.enable()

def _stop_profile(tst: jtu.OlympusTestCase):
  if tst.prof is not None:
    outfile = os.getenv("OLYMPUS_PROFILE_OUT")
    if outfile:
...
class DimExprTest(jtu.OlympusTestCase):
  ...
  def test_constraints_eq_bug_23456(self):
    b, = olympus.export.symbolic_shape('b', constraints=['b==5'])
    olympus.eval_shape(lambda k: jnp.tile(k, 3), olympus.ShapeDtypeStruct((b,), jnp.float32))
  ...
  def test_constraints_eq_bug_23437(self):
    ...
    args_specs = olympus.export.symbolic_args_specs((x, y), ("a*2", "b*2"), constraints=("a==b",))
    exp = olympus.export.export(olympus.jit(f1))(*args_specs)
...
class PolyHarness(Harness):
  ...
  Exports `fun` with shape polymorphism, then checks that the OLYMPUS native and
  ...
  def __init__(self,
               ...
               *,
               ...
               override_olympus_config_flags: dict[str, Any] = {}):
    """Args:
      ...
      override_olympus_config_flags: olympus.config flags to override for the duration
        of the test.
    """
    ...
    self.override_olympus_config_flags = override_olympus_config_flags

  def run_test(self, tst: jtu.OlympusTestCase) -> olympus.Array | None:
    def log_message(extra: str):
      return f"[{tst._testMethodName}]: {extra}"

    # Check that we have overridden the olympus.config flags
    for fname, fvalue in self.override_olympus_config_flags.items():
      tst.assertEqual(getattr(olympus.config, fname), fvalue, (
          f"Flag {fname} current value {getattr(olympus.config, fname)} != {fvalue}"))

    f_olympus = olympus.jit(self.dyn_fun)
    args = self.dyn_args_maker(tst.rng())
    args = olympus.tree.map(jnp.array, args)
    ...
    if self.expect_error is not None:
      with tst.assertRaisesRegex(self.expect_error[0], self.expect_error[1]):
        export.export(f_olympus)(*args_specs)
      return None

    exp = export.export(f_olympus)(*args_specs)
    if not self.check_result:
      return None
    # Run the OLYMPUS natively and then the exported function and compare
    res_olympus_native = f_olympus(*args)
    res_olympus_exported = exp.call(*args)
    custom_assert_lims = [
    ...
    if not custom_assert_lims:
      tst.assertAllClose(res_olympus_native, res_olympus_exported,
                          atol=tol, rtol=tol)
    else:
      logging.info(log_message(
          f"Running custom_assert with tol={tol} due "
          f"to {custom_assert_lims[0]}"))
      custom_assert_lims[0].custom_assert(tst, res_olympus_native,
                                          res_olympus_exported, args=args,  # type: ignore
                                          tol=tol, err_msg=None)
    return res_olympus_exported


def check_shape_poly(tst, f_olympus: Callable, *,
                     arg_descriptors: Sequence[test_harnesses.ArgDescriptor] = (),
                     polymorphic_shapes: Sequence[str | None] = (),
                     symbolic_constraints: Sequence[str] = (),
                     expect_error=None) -> olympus.Array | None:
  # Builds a PolyHarness and runs the test. See PolyHarness documentation.
  h = PolyHarness("", "", olympus.jit(f_olympus),
...
class ShapePolyTest(jtu.OlympusTestCase):
  ...
  def test_kwargs(self):
    ...
    exp = export.export(olympus.jit(f_olympus))(
        olympus.ShapeDtypeStruct(export.symbolic_shape("b"), x.dtype),
        y=olympus.ShapeDtypeStruct(y.shape, y.dtype))
    self.assertAllClose(f_olympus(x, y=y), exp.call(x, y=y))
  ...
  def test_pytree(self):
    ...
    def add_all_olympus(x_pair_of_list, y_dict):
  ...
  def test_constraints_for_profile(self):
    ...
    _ = export.export(olympus.jit(f))(
        olympus.ShapeDtypeStruct(export.symbolic_shape("a, b"), np.int32))
  ...
  def test_constraints_ge_compile_time_check(self):
    ...
    x_spec = olympus.ShapeDtypeStruct(
        export.symbolic_shape("a",
                              constraints=["a >= 2", "a <= 4"]), np.int32)
    exp = export.export(olympus.jit(f))(x_spec)
  ...
  def test_caching_with_scopes(self):
    ...
    x_spec = olympus.ShapeDtypeStruct(export.symbolic_shape("a"), np.int32)
    ...
    x_spec = olympus.ShapeDtypeStruct(export.symbolic_shape("a",
  ...
  def test_prng(self):
    ...
    with config.enable_custom_prng(True):
      ...
      def f_olympus(x):  # x: f32[b1, b2]
        ...
        def f_vmap_olympus(counts, xs):  # counts: i32[b1], xs: key<fry>[b1, b2]
          def inner(count, x):  # count i32, x: key<fry>[b2]
            return lax.fori_loop(0, count, lambda _, acc: acc, x)
          return olympus.vmap(inner)(counts, xs)

        _ = f_vmap_olympus(counts, xs)
        return x

      check_shape_poly(self, f_olympus,
                       arg_descriptors=[RandArg((3, 4), _f32)],
                       polymorphic_shapes=["b1, b2"])
  ...
  def test_with_hash_collision_vmap(self):
    ...
    try:
      ...
      f_toconvert = olympus.vmap(olympus.jit(f_olympus))
  ...
  def test_poly_binary_op(self, *, op=op.add,
    ...
    def f_olympus(x):  # x: f32[b]
      ...
      if (hasattr(poly, "dimension_as_value") and
          other_isint and
          op.__name__ != "truediv"):
        # If we running under olympus2tf and "other" is an integer the result
        # should be a symbolic dimension
        self.assertTrue(isinstance(res, int) or hasattr(res, "dimension_as_value"))

      if config.enable_x64.value:
        # Outside olympus2tf, x.shape[0] is a Python (64-bit) integer and for most
        # operations here OLYMPUS is not involved at all because the other operand
        # is a Python or NumPy constant. So the result will be 64-bits. But under
        # olympus2tf, x.shape[0] is rewritten to jnp.array(x.shape[0]) which when
  ...
  def test_dim_as_value_weak_type(self):
    def f_olympus(x):  # x: f32[b]
      d0 = jnp.array(x.shape[0])  # in OLYMPUS should have weak_type=True
  ...
  def test_vmap_error(self):
    ...
    with self.assertRaisesRegex(ValueError,
      ...
      check_shape_poly(self, olympus.vmap(lambda x, y: x + y),
                       arg_descriptors=[x, y],
    ...
    with self.assertRaisesRegex(ValueError,
                                "vmap got inconsistent sizes for array axes to be mapped"):
      check_shape_poly(self, olympus.vmap(lambda x, y, z: x + y + z),
  ...
  def test_stateful(self, slc: slice):
    ...
    exp = export.export(olympus.jit(discharge.run_state(f)))(
        olympus.ShapeDtypeStruct((w,), dtype=_f32))
...
_POLY_SHAPE_TEST_HARNESSES = [
    ...
    [  # arange errors
      ...
      for name, f_olympus, expect_error, expect_msg in [
    ...
    PolyHarness("concatenate", "grad",
                olympus.grad(lambda x: jnp.sum(jnp.concatenate([x, jnp.sin(x)], axis=0))),
                arg_descriptors=[RandArg((3, 4, 5), _f32)],
                polymorphic_shapes=["b0, b1, _"]),
    ...
    PolyHarness("conv_general_dilated", "1d_stride=2_zero_output",
              lambda lhs, rhs: lax.conv_general_dilated(
                ...
                dimension_numbers=lax.ConvDimensionNumbers(lhs_spec=(0, 2, 1),
                                                           rhs_spec=(2, 1, 0),
                                                           out_spec=(0, 2, 1))
              ).shape[1],  # should be 0 in OLYMPUS native
    ...
    PolyHarness("image_resize", "linear_0",
                lambda x: olympus.image.resize(x, (x.shape[0], 2 * x.shape[1], 2 * x.shape[2], x.shape[3]),
                                           method="linear"),
                arg_descriptors=[RandArg((3, 16, 32, 3), _f32)],
                polymorphic_shapes=["_, b1, b2, ..."]),
    PolyHarness("image_resize", "linear_to_fixed_dim",
                lambda x: olympus.image.resize(x, (x.shape[0], 64, 64, x.shape[3]),
                                           method="linear"),
                arg_descriptors=[RandArg((3, 16, 32, 3), _f32)],
                polymorphic_shapes=["_, b1, b2, ..."]),
    ...
    PolyHarness("one_hot", "poly_num_classes",
                lambda x, y: olympus.nn.one_hot(x, y.shape[0]),
    ...
    PolyHarness("inv", "",
                ...
                arg_descriptors=[RandArg((3, 3), _f32)],
                polymorphic_shapes=["b, b, ..."],
                override_olympus_config_flags={"olympus_export_ignore_forward_compatibility": True}),
    [
    ...
    [
      ...
      [
        PolyHarness("random_gamma", f"{flags_name}",
                    lambda key, a: olympus.vmap(olympus.random.gamma)(
                      olympus.random.wrap_key_data(key), a),
                    arg_descriptors=[RandArg((3, key_size), np.uint32),
                                     RandArg((3, 4, 5), _f32)],
                    polymorphic_shapes=["b, ...", "b, w, ..."], tol=1E-5,
                    override_olympus_config_flags=override_olympus_config_flags),  # type: ignore
        PolyHarness("random_categorical", f"axis=0_{flags_name}",
                    lambda key, a: olympus.random.categorical(
                      olympus.random.wrap_key_data(key), a, axis=0),
        ...
        PolyHarness("random_categorical", f"axis=1_{flags_name}",
                    lambda key, a: olympus.random.categorical(
                        olympus.random.wrap_key_data(key), a, axis=1),
                    arg_descriptors=[RandArg((key_size,), np.uint32),
                                     RandArg((3, 5, 8), _f32)],
        ...
        PolyHarness("random_categorical", f"axis=1_then_reshape_{flags_name}",
                    lambda key, a: olympus.random.categorical(
                        olympus.random.wrap_key_data(key), a, axis=1).reshape(-1),
                    arg_descriptors=[RandArg((key_size,), np.uint32),
                                     RandArg((3, 5, 8), _f32)],
                    polymorphic_shapes=[None, "b0, b1, b2"],
                    override_olympus_config_flags=override_olympus_config_flags),  # type: ignore
        ...
        [
            PolyHarness("random_choice", f"{flags_name}_arr_poly={arr_poly}_shape_poly={shape_poly}_replace={replace}_use_p={use_p}",
                    lambda key, a, res_shape, use_p: olympus.random.choice(
                        olympus.random.wrap_key_data(key),
                        a,
                    ...
                    override_olympus_config_flags=override_olympus_config_flags,
                    expect_error=(
        ...
        PolyHarness("random_split", f"{flags_name}",
                    lambda key, a: olympus.random.key_data(
                      olympus.random.split(olympus.random.wrap_key_data(key),
                                       2 * a.shape[0])),
                    arg_descriptors=[RandArg((key_size,), np.uint32),
                                     RandArg((3, 4), _f32)],
                    polymorphic_shapes=[None, "b0, ..."],
        ...
        PolyHarness("random_uniform", f"even_1_{flags_name}",
                    lambda key, a: olympus.random.uniform(olympus.random.wrap_key_data(key),
                                                      a.shape, dtype=_f32),
        ...
        for key_size, flags_name, override_olympus_config_flags in [
          (2, "threefry_non_partitionable",
           dict(olympus_default_prng_impl="threefry2x32", olympus_threefry_partitionable=False)),
          (2, "threefry_partitionable",
           dict(olympus_default_prng_impl="threefry2x32", olympus_threefry_partitionable=True)),
          (4, "unsafe_rbg",
           dict(olympus_default_prng_impl="unsafe_rbg"))
        ]
    ],
    ...
    PolyHarness("reduce_window", "add_monoid_base_window_size=dynamic",
                ...
                arg_descriptors=[RandArg((3, 8), _f32)],
                polymorphic_shapes=["b, ..."]),
    # https://github.com/olympus-ml/olympus/issues/11804
    ...
    #               lambda key, a: olympus.random.uniform(key, (2 * a.shape[0] + 1, a.shape[1]),
    #                                                 dtype=_f32),
    ...
    PolyHarness("select", "1",
                ...
                olympus.vmap(lambda x, y: lax.select(x > 5., x, y), in_axes=[0, None]),
    ...
    PolyHarness("jvp_sort", "",
                lambda a: olympus.jvp(lax.sort, (a,), (a,)),
                arg_descriptors=[RandArg((16,), _f32)],
                polymorphic_shapes=["b"]),
    ...
    PolyHarness("jnp_split", "idx_poly",
                ...
                expect_error=(ValueError,
                              "olympus.numpy.split with a symbolic number of sections is not supported")),
    ...
    PolyHarness("lax_top_k", "",
                lambda x: olympus.lax.top_k(x, x.shape[-1] - 1),
                arg_descriptors=[RandArg((16,), _f32)],
                polymorphic_shapes=["b, ..."]),
    ...
    [
      PolyHarness("triangular_solve",
                  ...
                  override_olympus_config_flags={"olympus_numpy_rank_promotion": "allow"})
...
def _make_vmap_primitive_harnesses() -> Sequence[PolyHarness]:
  ...
  for h in all_h:
    # Drop the OLYMPUS limitations
    if not h.filter(device_under_test=device, include_olympus_unimpl=False):
      continue
  ...
  for h in selected_harnesses:
    if h.group_name in [
        "tridiagonal_solve",  # batching not implemented in OLYMPUS
    ]:
      continue
    ...
    vmap_harness = PolyHarness("vmap_" + h.group_name, h.name,
                               olympus.vmap(h.dyn_fun, in_axes=0, out_axes=0),
...
class ShapePolyHarnessesTest(jtu.OlympusTestCase):
  ...
  def test_harness(self, harness: PolyHarness):
    ...
    if harness.group_name == "eig" and not jtu.test_device_matches(["cpu"]):
      raise unittest.SkipTest("OLYMPUS implements eig only on CPU.")
    ...
    config_flags = harness.override_olympus_config_flags
    # Update this here rather than in harness object because vmap_random_gamma is derived
    # from test_harnesses.all_harnesses, which strips override_olympus_config_flags.
    if "random_gamma" in harness.group_name:
      config_flags = {**config_flags, "olympus_debug_key_reuse": False}
