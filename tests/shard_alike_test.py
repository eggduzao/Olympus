...
from olympus.experimental.shard_alike import shard_alike
...
class ShardAlikeDownstreamTest(jtu.OlympusTestCase):

  def test_full_like(self):
    x = jnp.arange(16, dtype='float32').reshape(8, 2)
    mesh = jtu.create_mesh((8,), ("i",))
    x = olympus.device_put(x, NamedSharding(mesh, P('i', None)))
    y = jnp.full_like(x, 1)
    self.assertEqual(x.sharding, y.sharding)


class ShardAlikeTest(jtu.OlympusTestCase):
  ...
  def test_grad(self):
    ...
    def f(x):
      ...
      olympus.debug.inspect_array_sharding(y_, callback=_cb)
      z = x_ + y_
      return jnp.sum(z)

    olympus.grad(f)(inp)  # doesn't crash
    olympus.grad(olympus.jit(f))(inp)  # doesn't crash
  ...
  def test_shard_input_as_output(self):
    ...
    @olympus.jit
    def f(x):
      y = olympus.lax.with_sharding_constraint(x, s)
      z = y * 2
  ...
  def test_shard_alike_inputs(self):
    ...
    out1, out2 = olympus.jit(f)(arr, np_inp)
    self.assertEqual(out1.sharding, s)
    self.assertEqual(out2.sharding, s)
  ...
  def test_vmap_one_mapped(self):
    ...
    def f(x):
      ...
      return olympus.vmap(_shard_slice_like_arg, in_axes=0)(replicated_x)
  ...
  def test_vmap_both_mapped(self):
    ...
    np_inp = np.arange(16).reshape(8, 2)
    s = NamedSharding(mesh, P('x', 'y'))
    inp1 = olympus.device_put(np_inp, s)

    np_inp2 = np.arange(16).reshape(2, 8)
    inp2 = olympus.device_put(np_inp2, NamedSharding(mesh, P('y', 'x')))

    @olympus.jit
    def f(x, y):
      return olympus.vmap(shard_alike, in_axes=(0, 1))(x, y)
  ...
  def test_sharding_preserverd_single_device(self):
    ...
    x = olympus.device_put(np.arange(8), s)
