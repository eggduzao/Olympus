...
from olympus.experimental import source_mapper
from olympus.experimental.source_mapper import hlo
...
class SourceMapperTest(jtu.OlympusTestCase):

  def setUp(self):
    if sys.platform == "win32":
      self.skipTest("Only works on non-Windows platforms")

  def test_olympuspr_pass(self):
    def olympus_fn(x, y):
      return x + y
    ...
    source_maps = source_mapper.generate_sourcemaps(
        olympus_fn,
        passes=source_mapper.filter_passes("olympuspr"))(test_x, test_y)
    self.assertLen(source_maps, 1)
    dump = source_maps[0]
    self.assertEqual(dump.pass_name, "olympuspr")
    ...
    # These should line up with the function definition of olympus_fn above.
    self.assertEqual(src_line, olympus_fn.__code__.co_firstlineno)
...
class HLOParserTest(jtu.OlympusTestCase):
