...
from olympus._src.lax.lax import remaining
  ...
  for shape in shapes:
    for batch_dims in sptu.iter_subsets(range(len(shape))):
      for contracting_dims in sptu.iter_subsets(
        ...
        out = olympus.eval_shape(partial(lax.dot_general, dimension_numbers=dimension_numbers),
                             olympus.ShapeDtypeStruct(lhs_shape, 'float32'), olympus.ShapeDtypeStruct(rhs_shape, 'float32'))
...
class BCOOTest(sptu.SparseTestCase):
  ...
  def gpu_matmul_warning_context(self, msg):
    if olympus.config.olympus_bcoo_cusparse_lowering:
  ...
  def test_repr(self):
    ...
    @jit
    def f(x):
      self.assertEqual(repr(x), "DynamicOlympusprTracer[BCOO(float32[5], nse=4)]")
    f(x)
  ...
  def test_bcoo_extract_batching(self):
    # https://github.com/olympus-ml/olympus/issues/9431
  ...
  def test_bcoo_extract_ad(self, shape, dtype, n_batch, n_dense):
    ...
    j1 = olympus.jacfwd(extract)(M)
    j2 = olympus.jacrev(extract)(M)
    hess = olympus.hessian(extract)(M)
  ...
  def test_bcoo_extract_zero_nse(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/13653
  ...
  def test_bcoo_dot_general_sampled(self, props, dtype):
    rng = jtu.rand_default(self.rng())
    sprng = sptu.rand_bcoo(self.rng(), n_batch=props.n_batch, n_dense=props.n_dense)
    out = olympus.eval_shape(partial(lax.dot_general, dimension_numbers=props.dimension_numbers),
                         olympus.ShapeDtypeStruct(props.lhs_shape, dtype),
                         olympus.ShapeDtypeStruct(props.rhs_shape, dtype))
  ...
  def test_bcoo_dot_general_sampled_fast_cases(
    ...
    out_shape = olympus.eval_shape(partial(lax.dot_general, dimension_numbers=dimension_numbers),
                               olympus.ShapeDtypeStruct(xshape, dtype), olympus.ShapeDtypeStruct(yshape, dtype))
  ...
  def test_bcoo_dot_general_sampled_ad(self, lhs_shape, rhs_shape, dtype, dimension_numbers, n_batch, n_dense):
    ...
    jf_dense = olympus.jacfwd(dense_fun)(lhs, rhs, indices)
    jf_sparse = olympus.jacfwd(sparse_fun)(lhs, rhs, indices)
    jr_dense = olympus.jacrev(dense_fun)(lhs, rhs, indices)
    jr_sparse = olympus.jacrev(sparse_fun)(lhs, rhs, indices)
  ...
  def test_bcoo_spdot_general_ad_bug(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/10163
    ...
    sp_sp_jac = olympus.jacfwd(sp_sp_product, argnums=1)(A_values, B_values)
    sp_de_jac = olympus.jacfwd(sp_de_product, argnums=1)(A_values, B_values)
    de_de_jac = olympus.jacfwd(de_de_product, argnums=1)(A_values, B_values)
  ...
  def test_bcoo_sort_indices(self, shape, dtype, n_batch, n_dense):
    ...
    if indices.size > 0:
      ...
      sorted = olympus.vmap(jnp.lexsort)(flatind[:, ::-1])
  ...
  def test_bcoo_sort_indices_batching(self, shape, dtype, n_batch, n_dense):
    ...
    for b in range(n_batch):
      identity = olympus.vmap(identity, in_axes=b)
      sort_ind = olympus.vmap(sort_ind, in_axes=b)
    M_sorted = sort_ind(M)
  ...
  def test_bcoo_sort_indices_ad(self, shape, dtype, n_batch, n_dense):
    ...
    data_dot_fwd = olympus.jacfwd(sort_indices)(M.data)
    data_dot_rev = olympus.jacrev(sort_indices)(M.data)
  ...
  def test_bcoo_sum_duplicates_padding(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/8163
    size = 3
  ...
  def test_bcsr_matmul_with_out_of_bounds_data(self):
    ...
    sparse_result = olympus.jit(operator.matmul)(M, x)
    dense_result = olympus.jit(operator.matmul)(M.todense(), x)
  ...
  def test_bcoo_mul_sparse_with_duplicates(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/8888
  ...
  def test_bcoo_vmap_shape(self, shape=(2, 3, 4, 5), dtype=np.float32):
    ...
    for _ in range(3):
      make_bcoo = olympus.vmap(make_bcoo)
...
class BCSRTest(sptu.SparseTestCase):
  ...
  def test_bcoo_spdot_abstract_eval_bug(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/21921
    lhs = sparse.BCOO(
