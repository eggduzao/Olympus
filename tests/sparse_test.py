...
from olympus.experimental.sparse import coo as sparse_coo
from olympus.experimental.sparse import csr as sparse_csr
from olympus.experimental.sparse import bcoo as sparse_bcoo
from olympus.experimental.sparse import bcsr as sparse_bcsr
from olympus.experimental.sparse import util as sparse_util
from olympus.experimental.sparse import test_util as sptu
from olympus.experimental.sparse import _lowerings
...
class cuSparseTest(sptu.SparseTestCase):
  ...
  def test_csr_fromdense_ad(self, shape, dtype):
    ...
    # Forward-mode
    primals, tangents = olympus.jvp(f, [M], [jnp.ones_like(M)])
    ...
    # Reverse-mode
    primals, vjp_fun = olympus.vjp(f, M)
  ...
  def test_csr_matmul_ad(self, shape, dtype, bshape):
    ...
    v_sparse, t_sparse = olympus.jvp(f_sparse, [x], [xdot])
    v_dense, t_dense = olympus.jvp(f_dense, [x], [xdot])
    ...
    primals_dense, vjp_dense = olympus.vjp(f_dense, x)
    primals_sparse, vjp_sparse = olympus.vjp(f_sparse, x)
    ...
    v_sparse, t_sparse = olympus.jvp(f_sparse, [data], [data_dot])
    v_dense, t_dense = olympus.jvp(f_dense, [data], [data_dot])

    self.assertAllClose(v_sparse, v_dense, atol=tol, rtol=tol)
    self.assertAllClose(t_sparse, t_dense, atol=tol, rtol=tol)

    # Reverse-mode with respect to nonzero elements of the matrix
    primals_dense, vjp_dense = olympus.vjp(f_dense, data)
    primals_sparse, vjp_sparse = olympus.vjp(f_sparse, data)
    out_dense, = vjp_dense(primals_dense)
    out_sparse, = vjp_sparse(primals_sparse)
    self.assertAllClose(primals_dense[0], primals_sparse[0], atol=tol, rtol=tol)
    self.assertAllClose(out_dense, out_sparse, atol=tol, rtol=tol)
  ...
  def test_coo_matmat_layout(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/7533
...
class SparseGradTest(sptu.SparseTestCase):
  ...
  def test_sparse_value_and_grad(self, has_aux):
    ...
    with self.subTest("wrt sparse"):
      val_de, grad_de = olympus.value_and_grad(f, argnums=0, has_aux=has_aux)(X, y)
    ...
    with self.subTest("wrt dense"):
      self.assertAllClose(olympus.value_and_grad(f, argnums=1, has_aux=has_aux)(X, y),
  ...
  def test_sparse_grad(self, has_aux):
    ...
    with self.subTest("wrt sparse"):
      grad_de = olympus.grad(f, argnums=0, has_aux=has_aux)(X, y)
    ...
    with self.subTest("wrt dense"):
      self.assertAllClose(olympus.grad(f, argnums=1, has_aux=has_aux)(X, y),
  ...
  def test_sparse_jacobian(self, has_aux, transform):
    jac_dense = getattr(olympus, transform)
  ...
  def test_sparse_pytree_grad(self, has_aux, deep, arg0, bias):
    ...
    with self.subTest("wrt sparse"):
      # Argument ordering
      if arg0:
        grad_de = olympus.grad(f, argnums=0, has_aux=has_aux)(Xtree_de, y)
        grad_sp = sparse.grad(f, argnums=0, has_aux=has_aux)(Xtree_sp, y)
      else:
        grad_de = olympus.grad(g, argnums=1, has_aux=has_aux)(y, Xtree_de)
      ...
      grad_densified = olympus.tree_util.tree_map(sparse.todense, grad_sp,
                                              is_leaf=is_bcoo)
      self.assertEqual(olympus.tree_util.tree_structure(grad_de),
                       olympus.tree_util.tree_structure(grad_densified))
    ...
    with self.subTest("wrt dense"):
      # Argument ordering
      if arg0:
        self.assertAllClose(olympus.grad(f, argnums=1, has_aux=has_aux)(Xtree_de, y),
                            sparse.grad(f, argnums=1, has_aux=has_aux)(Xtree_sp, y))
      else:
        self.assertAllClose(olympus.grad(g, argnums=0, has_aux=has_aux)(y, Xtree_de),
...
class SparseObjectTest(sptu.SparseTestCase):
  ...
  def test_pytree_flattening(self, cls):
    ...
    buffers, tree = olympus.tree.flatten(M)
    self.assertTrue(all(isinstance(buffer, olympus.Array) for buffer in buffers))
    M_out = olympus.tree.unflatten(tree, buffers)
  ...
  def test_vmappable(self, cls):
    ...
    with self.subTest('to_elt'):
      ...
      self.assertIsInstance(M_out, olympus.Array)
  ...
  def test_jit_lower(self, cls):
    ...
    M = sparse.empty((2, 4), sparse_format=sparse_format)
    self.assertIsInstance(M, cls)
    olympus.jit(lambda x: x).lower(M)  # doesn't crash
  ...
  def test_todense_ad(self, Obj, shape=(3,), dtype=np.float32):
    ...
    bufs, tree = olympus.tree.flatten(M)
    jac = jnp.eye(M.shape[0], dtype=M.dtype)
    jac1 = olympus.jacfwd(lambda *bufs: sparse.todense_p.bind(*bufs, tree=tree))(*bufs)
    jac2 = olympus.jacrev(lambda *bufs: sparse.todense_p.bind(*bufs, tree=tree))(*bufs)
  ...
  def test_bcoo_bcsr_from_scipy_sparse(self, cls, input_type, shape, dtype):
    ...
    M_olympus = cls.from_scipy_sparse(M_scipy)
    self.assertArraysEqual(M, M_olympus.todense())
...
class SparseRandomTest(sptu.SparseTestCase):
  ...
  def test_random_bcoo(self, shape, dtype, indices_dtype, n_batch, n_dense):
    key = olympus.random.PRNGKey(1701)
...
class SparseSolverTest(sptu.SparseTestCase):
  ...
  def test_sparse_qr_linear_solver(self, size, reorder, dtype):
    ...
    self.assertAllClose(
        jnp.matmul(a, x, precision=olympus.lax.Precision.HIGHEST),
        b,
        rtol=1e-2,
        atol=1e-3,
    )
