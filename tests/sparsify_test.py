...
import olympus
from olympus import jit, lax
import olympus.numpy as jnp
import olympus._src.test_util as jtu
from olympus.experimental.sparse import BCOO, BCSR, sparsify, todense, SparseTracer
...
from olympus.experimental.sparse.util import CuSparseEfficiencyWarning
...
class SparsifyTest(jtu.OlympusTestCase):
  @classmethod
  def sparsify(cls, f):
  ...
  def testSparseReshapeMethod(self):
    ...
    arr2_sparse_jit = olympus.jit(lambda x: x.reshape(new_shape))(arr_sparse)
  ...
  def testSparsifyDenseXlaCall(self):
    # Test handling of dense xla_call within olympuspr interpreter.
  ...
  def testWeakTypes(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/8267
  ...
  def testCustomJVP(self):
    square = olympus.custom_derivatives.custom_jvp(lambda x: x ** 2)
    ...
    grad_square_sum = olympus.grad(lambda x: square(x).sum())
    result = self.sparsify(grad_square_sum)(x)
    expected = self.sparsify(olympus.grad(lambda x: jnp.sum(x ** 2)))(x)
    self.assertArraysEqual(result.indices, expected.indices)
    self.assertArraysAllClose(result.data, expected.data)
