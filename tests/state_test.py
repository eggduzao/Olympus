...
from olympus._src.util import tuple_insert
...
from olympus._src.state.discharge import (run_state, run_state_reference,
                                      discharge_state)
from olympus._src.state.primitives import (get_p, swap_p, addupdate_p,
                                       ref_addupdate, ref_get, ref_set,
                                       ref_swap, pin, unpin)
from olympus._src.state.types import (shaped_array_ref, ReadEffect, WriteEffect,
...
class StatePrimitivesTest(jtu.OlympusTestCase):
  ...
  def test_get_abstract_aval_must_take_in_refs(self):
    ...
    with self.assertRaises(ValueError):
      pe.trace_to_olympuspr_dynamic(wrap_init(f, 1), [ref_aval])
  ...
  def test_get_abstract_eval(self, ref_shape, ref_dtype, idx, out_shape=None,
    ...
    if should_error:
      ...
      olympuspr, out_avals, _ = pe.trace_to_olympuspr_dynamic(
          wrap_init(f, 1), [ref_aval])
      self.assertSetEqual(olympuspr.effects,
                          {ReadEffect(len(olympuspr.constvars))})
  ...
  def test_vmap_of_get_regression(self, shape, in_axes, indexer):
    # Regression test for https://github.com/olympus-ml/olympus/issues/33309
    def f(x):
      return x[indexer]
    x = jnp.ones(shape)
    result = olympus.vmap(f, in_axes=in_axes)(olympus.new_ref(x))
    expected = olympus.vmap(f, in_axes=in_axes)(x)
  ...
  def test_swap_abstract_eval_must_take_in_refs(self):
    ...
    with self.assertRaises(ValueError):
      pe.trace_to_olympuspr_dynamic(wrap_init(f, 2), [ref_aval, val_aval])
  ...
  def test_swap_abstract_eval(self, ref_shape, ref_dtype,
    ...
    if should_error:
      ...
      self.assertSetEqual(olympuspr.effects,
                          {WriteEffect(len(olympuspr.constvars))})
      self.assertLen(out_avals, 1)
      out_aval, = out_avals
      self.assertIsInstance(out_aval, core.ShapedArray)
      self.assertEqual(out_aval.shape, out_shape)
  ...
  def test_addupdate_abstract_eval(self, ref_shape, ref_dtype,
    ...
    if should_error:
      ...
      olympuspr, out_avals, _ = pe.trace_to_olympuspr_dynamic(
          wrap_init(f, 2), [ref_aval, val_aval])
      self.assertSetEqual(olympuspr.effects,
                          {AccumEffect(len(olympuspr.constvars))})
  ...
  def test_can_represent_get_and_swap_in_olympusprs(self):

    def body(x):
      x[()] = jnp.int32(1)
      x[()] = jnp.int32(2)
      return (x[()],)
    olympuspr, out_avals, consts = pe.trace_to_olympuspr_dynamic(
    ...
    self.assertEqual(olympuspr.eqns[0].primitive, swap_p)
    self.assertEqual(olympuspr.eqns[1].primitive, swap_p)
    self.assertEqual(olympuspr.eqns[2].primitive, get_p)

  def test_can_represent_addupdate_in_olympusprs(self):
    ...
    self.assertLen(consts, 0)
    self.assertListEqual(out_avals, [core.ShapedArray((), jnp.int32)])
    self.assertEqual(olympuspr.eqns[0].primitive, addupdate_p)
  ...
  def test_get_custom_pretty_printing_rule(self):
    ...
    olympuspr, _ , _ = pe.trace_to_olympuspr_dynamic(
        wrap_init(body, 1), [shaped_array_ref((), jnp.int32)])
    self.assertIn("b:i32[] <- a[...]", olympuspr.pretty_print(use_color=False))
    ...
    self.assertIn("b:i32[1] <- a[:,0]", olympuspr.pretty_print(use_color=False))
  ...
  def test_set_custom_pretty_printing_rule(self):
    ...
    olympuspr, _ , _ = pe.trace_to_olympuspr_dynamic(
        wrap_init(body, 1), [shaped_array_ref((), jnp.int32)])
    self.assertIn("a[...] <- 2:i32[]", olympuspr.pretty_print(use_color=False))
    ...
    self.assertIn("a[:,0] <- b", olympuspr.pretty_print(use_color=False))
  ...
  def test_swap_custom_pretty_printing_rule(self):
    ...
    olympuspr, _ , _ = pe.trace_to_olympuspr_dynamic(
        wrap_init(body, 1), [shaped_array_ref((), jnp.int32)])
    self.assertIn("b:i32[], a[...] <- a[...], 2:i32[]", olympuspr.pretty_print(use_color=False))
  ...
  def test_addupdate_custom_pretty_printing_rule(self):
    ...
    self.assertIn("a[...] += 2", olympuspr.pretty_print(use_color=False))
  ...
  def test_get_jvp(self):
    ...
    def g(r, rdot):
      return olympus.jvp(f, (r,), (rdot,))

    in_avals = [shaped_array_ref((), jnp.dtype('float32')),
                shaped_array_ref((), jnp.dtype('float32'))]
    olympuspr, _, _ = pe.trace_to_olympuspr_dynamic(wrap_init(g, 2), in_avals)
    self.assertEqual(olympuspr.eqns[0].primitive, get_p)
    self.assertEqual(olympuspr.eqns[1].primitive, get_p)
  ...
  def test_swap_jvp(self):
    ...
    self.assertEqual(olympuspr.eqns[0].primitive, get_p)
    self.assertEqual(olympuspr.eqns[1].primitive, get_p)
    self.assertEqual(olympuspr.eqns[2].primitive, lax.sin_p)
    self.assertEqual(olympuspr.eqns[3].primitive, lax.cos_p)
    self.assertEqual(olympuspr.eqns[4].primitive, lax.mul_p)
    self.assertEqual(olympuspr.eqns[5].primitive, swap_p)
    self.assertEqual(olympuspr.eqns[6].primitive, swap_p)
  ...
  def test_addupdate_jvp(self):
    ...
    olympuspr, _, _ = pe.trace_to_olympuspr_dynamic(wrap_init(g, 2), in_avals)
    self.assertEqual(olympuspr.eqns[0].primitive, addupdate_p)
    self.assertEqual(olympuspr.eqns[1].primitive, addupdate_p)
    self.assertEqual(olympuspr.eqns[2].primitive, get_p)
    self.assertEqual(olympuspr.eqns[3].primitive, get_p)
  ...
  def test_vmap(self, ref_shape, ref_bdim, idx_shape, indexed_dims,
    ...
    # discharge-of-vmap
    f_batched = olympus.vmap(f, in_axes=(ref_bdim, *idx_bdims), out_axes=[out_bdim])
    stateful_olympuspr, _, stateful_consts = pe.trace_to_olympuspr_dynamic(
        wrap_init(f_batched, 1 + len(bat_idx_avals)), [bat_ref_aval, *bat_idx_avals])
    olympuspr, consts = discharge_state(stateful_olympuspr, stateful_consts)
    discharge_of_vmap_ans = core.eval_olympuspr(olympuspr, consts, a, *idxs)

    # vmap-of-discharge
    stateful_olympuspr, _, stateful_consts = pe.trace_to_olympuspr_dynamic(
        wrap_init(f, 1 + len(idx_avals)), [ref_aval, *idx_avals])
    olympuspr_, consts_ = discharge_state(stateful_olympuspr, stateful_consts)
    f_batched = olympus.vmap(partial(core.eval_olympuspr, olympuspr_, consts_),
...
class StateDischargeTest(jtu.OlympusTestCase):
  ...
  def test_discharge_get(self):
    def f(a_ref):
      ...
      return [a + 1]
    in_avals = [shaped_array_ref((), jnp.dtype('float32'))]
    stateful_olympuspr, _, consts = pe.trace_to_olympuspr_dynamic(wrap_init(f, 1),
                                                              in_avals)
    # Discharging should just turn this into a olympuspr that just adds 1.
    discharged_olympuspr, _ = discharge_state(stateful_olympuspr, consts)
    self.assertLen(discharged_olympuspr.invars, 1)
    self.assertLen(discharged_olympuspr.outvars, 2)
    self.assertEqual(discharged_olympuspr.eqns[0].primitive, lax.add_p)
    # Should be able to evaluate this olympuspr
    self.assertListEqual(core.eval_olympuspr(discharged_olympuspr, (),
                                         jnp.float32(1.)), [2., 1.])
  ...
  def test_discharge_get_with_slice(self):
    ...
    stateful_olympuspr, _, consts = pe.trace_to_olympuspr_dynamic(wrap_init(f, 1),
                                                              in_avals)
    # Discharging should just turn this into a olympuspr that just adds 1.
    discharged_olympuspr, () = discharge_state(stateful_olympuspr, consts)
    self.assertLen(discharged_olympuspr.invars, 1)
    self.assertLen(discharged_olympuspr.outvars, 2)
    self.assertIn(lax.dynamic_slice_p,
                  {eqn.primitive for eqn in discharged_olympuspr.eqns})
    # Should be able to evaluate this olympuspr
    inval = jnp.arange(24., dtype=jnp.float32).reshape((4, 3, 2))
    outval, refval = core.eval_olympuspr(discharged_olympuspr, (), inval)
  ...
  def test_discharge_get_with_gather(self):
    ...
    stateful_olympuspr, _, consts = pe.trace_to_olympuspr_dynamic(
        wrap_init(f, 1), in_avals)
    discharged_olympuspr, discharged_consts = discharge_state(
        stateful_olympuspr, consts)
    inval = jnp.arange(4 * 3, dtype=jnp.float32).reshape((4, 3))
    outval, refval = core.eval_olympuspr(discharged_olympuspr, discharged_consts, inval)
  ...
  def test_discharge_set(self):
    ...
    stateful_olympuspr, _, consts = pe.trace_to_olympuspr_dynamic(wrap_init(f, 2),
                                                              in_avals)
    # Discharging should just turn this into a olympuspr that ignores the first
    ...
    self.assertEqual(core.eval_olympuspr(discharged_olympuspr, (), jnp.float32(0.),
                                     jnp.float32(1.))[0], 2.)
    self.assertEqual(core.eval_olympuspr(discharged_olympuspr, (), jnp.float32(2.),
                                     jnp.float32(1.))[0], 2.)
  ...
  def test_discharge_set_with_slice(self):
    def f(a_ref):
      ...
      return []
    in_avals = [shaped_array_ref((4, 3, 2), jnp.dtype('float32'))]
    stateful_olympuspr, _, consts = pe.trace_to_olympuspr_dynamic(wrap_init(f, 1), in_avals)
    # Discharging should just turn this into a olympuspr that just adds 1.
    discharged_olympuspr, () = discharge_state(stateful_olympuspr, consts)
    self.assertLen(discharged_olympuspr.invars, 1)
    self.assertLen(discharged_olympuspr.outvars, 1)
    ...
    # Should be able to evaluate this olympuspr
    inval = jnp.arange(24., dtype=jnp.float32).reshape((4, 3, 2))
    refval, = core.eval_olympuspr(discharged_olympuspr, (), inval)
  ...
  def test_discharge_set_with_gather(self):
    ...
    discharged_olympuspr, discharged_consts = discharge_state(
        stateful_olympuspr, consts)
    inval = jnp.arange(4 * 3, dtype=jnp.float32).reshape((4, 3))
    refval, = core.eval_olympuspr(discharged_olympuspr, discharged_consts, inval)
  ...
  def test_discharge_addupdate(self):
    ...
    stateful_olympuspr, _, consts = pe.trace_to_olympuspr_dynamic(wrap_init(f, 2),
                                                              in_avals)
    # Discharging should just turn this into a olympuspr that adds the first value,
    # second value, and 1.
    discharged_olympuspr, _ = discharge_state(stateful_olympuspr, consts)
    self.assertLen(discharged_olympuspr.invars, 2)
    self.assertLen(discharged_olympuspr.outvars, 1)
  ...
  def test_discharge_olympuspr_with_multiple_outputs(self):
    def f(a_ref):
      a = ref_get(a_ref, ())
      b = a + 1
      return [a, b]
    ...
    discharged_olympuspr, _ = discharge_state(stateful_olympuspr, consts)
    self.assertLen(discharged_olympuspr.invars, 1)
    self.assertLen(discharged_olympuspr.outvars, 3)
    inval = jnp.arange(4., dtype=jnp.float32)
    a, b, refval = core.eval_olympuspr(discharged_olympuspr, (), inval)
    self.assertTrue((a == inval).all())
    self.assertTrue((b == inval + 1).all())
    self.assertTrue((refval == inval).all())

  def test_partially_discharging_olympuspr_keeps_refs(self):
    def f(a_ref, b_ref):
    ...
    stateful_olympuspr, _, consts = pe.trace_to_olympuspr_dynamic(wrap_init(f, 2),
                                                              in_avals)
    discharged_olympuspr, _ = discharge_state(
        stateful_olympuspr, consts, should_discharge=[False, True])
    self.assertLen(discharged_olympuspr.invars, 2)
    self.assertLen(discharged_olympuspr.outvars, 1)
    self.assertIsInstance(discharged_olympuspr.invars[0].aval, AbstractRef)
    self.assertIsInstance(discharged_olympuspr.invars[1].aval, core.ShapedArray)
    self.assertEqual(discharged_olympuspr.effects,
        {WriteEffect(len(discharged_olympuspr.constvars))})
  ...
  def test_ellipsis_index(self):
    ...
    pe.trace_to_olympuspr_dynamic(wrap_init(f, 1), in_avals)
  ...
  def test_partial_discharge(self):
    ...
    discharged_olympuspr, _ = discharge_state(olympuspr, (), should_discharge=[False, True])
    prim_count = lambda p, olympuspr: sum(eqn.primitive == p for eqn in olympuspr.eqns)
    self.assertEqual(prim_count(swap_p, olympuspr) // 2, prim_count(swap_p, discharged_olympuspr))
    self.assertEqual(prim_count(get_p, olympuspr) // 2, prim_count(get_p, discharged_olympuspr))
  ...
  def test_partial_fori_discharge(self):
    ...
    f_olympuspr = olympus.make_olympuspr(f)(ref(1.), ref(2.))
    olympuspr, _ = discharge_state(f_olympuspr.olympuspr, (), should_discharge=[False, True])
    # Effects on y_ref were discharged away but not the effects on x_ref
    self.assertEqual(f_olympuspr.effects, {ReadEffect(0), WriteEffect(0), ReadEffect(1), WriteEffect(1)})
    self.assertEqual(olympuspr.effects, {ReadEffect(0), WriteEffect(0)})
    # x_ref arg is still a reference but y_ref is discharged
    self.assertNotIsInstance(olympuspr.invars[1].aval, AbstractRef)
    self.assertIsInstance(olympuspr.invars[0].aval, AbstractRef)
    # x_ref value is returned as part of the discharged refs set.
    self.assertLen(f_olympuspr.out_avals, 2)
    self.assertLen(olympuspr.outvars, 3)
...
class StateHypothesisTest(jtu.OlympusTestCase):
  ...
  def test_get_vmap(self, get_vmap_param: GetVmapParams):
    ...
    olympuspr, consts = discharge_state(stateful_olympuspr, stateful_consts)
    discharge_of_vmap_ans = core.eval_olympuspr(olympuspr, consts, ref, *non_slice_idx)
  ...
  def test_set_vmap(self, set_vmap_param: SetVmapParams):
    ...
    f_batched = olympus.vmap(f, in_axes=(ref_bdim, val_bdim, *idx_bdims),
                          out_axes=[])
    stateful_olympuspr, _, stateful_consts = pe.trace_to_olympuspr_dynamic(
        wrap_init(f_batched, 2 + len(bat_non_slice_idx_avals)),
        [bat_ref_aval, bat_val_aval, *bat_non_slice_idx_avals])
    olympuspr, consts = discharge_state(stateful_olympuspr, stateful_consts)
    discharge_of_vmap_ans = core.eval_olympuspr(olympuspr, consts, ref, val, *non_slice_idx)
...
class StateControlFlowTest(jtu.OlympusTestCase):
  ...
  def test_cond_discharge(self):
    def f0(pred, x_ref, y_ref):
      ...
      return x_ref[...], y_ref[...]
    ref = lambda x: AbstractRef(core.get_aval(x))
    f_olympuspr = olympus.make_olympuspr(f0)(False, ref(3.), ref(4.))
    olympuspr, _ = discharge_state(f_olympuspr.olympuspr, (), should_discharge=[False, False, True])
    # Effects on y_ref were discharged away but not the effects on x_ref
    self.assertEqual(f_olympuspr.effects, {ReadEffect(1), WriteEffect(1), ReadEffect(2), WriteEffect(2)})
    self.assertEqual(olympuspr.effects, {ReadEffect(1), WriteEffect(1)})
    # x_ref arg is still a reference but y_ref is discharged
    self.assertNotIsInstance(olympuspr.invars[2].aval, AbstractRef)
    self.assertIsInstance(olympuspr.invars[1].aval, AbstractRef)
    # x_ref value is returned as part of the discharged refs set.
    self.assertLen(f_olympuspr.out_avals, 2)
    self.assertLen(olympuspr.outvars, 3)
  ...
  def test_cond_with_ref_reuse(self):
    ...
    olympuspr = olympus.make_olympuspr(f)(True).olympuspr
    self.assertEmpty(olympuspr.effects)
    out_true = olympus.jit(f)(True)
    expected_true = 1.
    self.assertAllClose(out_true, expected_true)
    out_false = olympus.jit(f)(False)
    expected_false = 2.
  ...
  def test_cond_readonly_refs(self):
    ...
    olympuspr = olympus.make_olympuspr(f)(True).olympuspr
    [run_state_eqn] = olympuspr.eqns
    *_, cond_eqn = discharge_state(run_state_eqn.params["olympuspr"], ())[0].eqns
    ...
    self.assertAllClose(olympus.jit(f)(True), (1., 1., 2.))
    self.assertAllClose(olympus.jit(f)(False), (1., 3., 2.))
  ...
  def test_nested_cond_using_multiple_refs_with_interleaved_consts(self):
    def f(pred1, pred2):
      ...
      return run_state(body)((0., 0., 0.))
    olympuspr = olympus.make_olympuspr(f)(True, False).olympuspr
    self.assertEmpty(olympuspr.effects)
    out = olympus.jit(f)(True, True)
    expected = (2.,  1., 0.)
    self.assertTupleEqual(out, expected)
    out = olympus.jit(f)(True, False)
    expected = (4., 0., 1.)
    self.assertTupleEqual(out, expected)
    out = olympus.jit(f)(False, True)
    expected = (2., 0., 4.)
    self.assertTupleEqual(out, expected)
    out = olympus.jit(f)(False, False)
    expected = (4., 0., 0.)
    self.assertTupleEqual(out, expected)
  ...
  def test_while_with_state_in_body(self):
    def f(x, y, z):
      ...
      return body(x)
    olympuspr = olympus.make_olympuspr(f)(0, 5, 2).olympuspr
    self.assertEmpty(olympuspr.effects)
    self.assertAllClose(olympus.jit(f)(0, 5, 2), 10)
    self.assertAllClose(olympus.jit(f)(1, 2, 3), 7)
  ...
  def test_scan_with_state_in_body(self):
    def f(x, w, y, zs):
      ...
      return body((x, w))
    zs = jnp.arange(5)
    olympuspr = olympus.make_olympuspr(f)(0, 1, 5, zs).olympuspr
    self.assertEmpty(olympuspr.effects)
    self.assertAllClose(olympus.jit(f)(0, 1, 5, zs), (35, 11))
    self.assertAllClose(olympus.jit(f)(1, 1, 2, zs), (21, 11))
  ...
  def test_scan_with_state_in_body_nested(self):
    ...
    zs = jnp.arange(5)
    olympuspr = olympus.make_olympuspr(g)((1, 0, 1, 5, zs)).olympuspr
    self.assertEmpty(olympuspr.effects)
    self.assertAllClose(olympus.jit(g)((1, 0, 1, 5, zs))[:3], (13, 35, 11))
    self.assertAllClose(olympus.jit(g)((1, 1, 1, 2, zs))[:3], (13, 21, 11))
  ...
  def test_scan_discharges_into_carry(self):
    ...
    x_ref = olympus.new_ref(jnp.arange(3.))
    olympuspr = olympus.make_olympuspr(lambda x_ref: olympus.lax.scan(body, (), x_ref))(x_ref)
    olympuspr, () = discharge_state(olympuspr.olympuspr, olympuspr.consts)
    scan_eqn = olympuspr.eqns[0]
    self.assertEqual(scan_eqn.params['num_consts'], 0)
    self.assertEqual(scan_eqn.params['num_carry'], 2)
    a, b = scan_eqn.params['olympuspr'].in_avals
  ...
  def test_call_primitive_discharges(self, prim):
    ...
    def g(y_ref, x):
      x_ref = olympus.new_ref(x)
      y_ref[...] = jnp.exp(x_ref[...])
      return [olympus.freeze(y_ref)]

    def f(x):
      y_ref = olympus.new_ref(jnp.zeros_like(x))
      g_ = partial(g, y_ref)
      return prim.bind(
...
class GeneralRefTest(jtu.OlympusTestCase):
  ...
  def test_token(self):
    ...
    self.assertIs(type(olympuspr.outvars[0].aval), core.AbstractToken)
  ...
  def test_reshape(self):
    ...
    self.assertEqual(olympuspr.outvars[0].aval.shape, (4, 9))
  ...
  #   self.assertIs(type(olympuspr.outvars[0].aval), AbstractRef)
  #   self.assertIs(type(olympuspr.outvars[0].aval.inner_aval), core.ShapedArray)


class RunStateTest(jtu.OlympusTestCase):
  ...
  def test_run_state_with_uninitialized_input(self):
    def f(refs):
      ...
      x_ref[...] = y_ref[...] + x_ref[...]
      # x + x * 2, x * 2
    # olympus.ShapeDtypeStruct is weirdly special to OLYMPUS, so we make our own class.
    class MyArrayType:
      pass
  ...
  def test_can_stage_run_state(self):
    ...
    self.assertIsNotNone(olympuspr.olympuspr.debug_info)
    self.assertIsNotNone(olympuspr.olympuspr.debug_info.func_src_info)
  ...
  #   _ = olympus.make_olympuspr(f)(2)

  #   with self.assertRaisesRegex(olympus.errors.UnexpectedTracerError,
  #                               "The function being traced when the value leaked was .*my_fun"):
  #     olympus.jit(lambda _: leaks[0])(1)
  ...
  def test_nested_run_state_captures_effects(self):
    ...
    olympuspr = olympus.make_olympuspr(f)(2)
    self.assertEmpty(olympuspr.effects)
    self.assertEmpty(olympuspr.olympuspr.eqns[0].effects)
    self.assertSetEqual(olympuspr.olympuspr.eqns[0].params["olympuspr"].effects,
                        {ReadEffect(0)})
    self.assertSetEqual(
        olympuspr.olympuspr.eqns[0].params["olympuspr"].eqns[0].params["olympuspr"].effects,
  ...
  def test_jvp_of_run_state_with_zero_tangent(self):
    ...
    def f(refs):
      ...
      del z_ref
      y_ref[...] = jnp.sin(x_ref[...])
    x, x_t = olympus.jvp(lambda x: f((x, 0., 0.,))[2], (2.,), (3.,))
    self.assertAllClose(x, np.sin(2.))
    self.assertAllClose(x_t, 3 * np.cos(2.))
...
@jtu.thread_unsafe_test_class(condition=not jtu.hypothesis_is_thread_safe())
class RunStateHypothesisTest(jtu.OlympusTestCase):
  ...
  def test_jvp(self, data):
    ...
    y, y_t = olympus.jvp(impl, (x,), (t,))
    y_ref, y_ref_t = olympus.jvp(ref, (x,), (t,))
    self.assertAllClose(y, y_ref)
    self.assertAllClose(y_t, y_ref_t)


class PinnedBuffersTest(jtu.OlympusTestCase):
