...
from olympus.experimental import random as exp_random
...
class StatefulRNGTest(jtu.OlympusTestCase):
  ...
  def test_stateful_rng_counter_increment(self, seed=7865943):
    ...
    _ = olympus.jit(rng.key)()  # implicit update
  ...
  def test_stateful_rng_invalid_instantiation(self):
    valid_key = olympus.random.key(0)
    valid_counter = olympus.new_ref(0)
    invalid_key = olympus.numpy.array([0, 1], dtype='uint32')
    invalid_counter = 0
  ...
  def testShapedKeys(self, seed=7589432):
    ...
    self.assertTrue(olympus.dtypes.issubdtype(keys1.dtype, olympus.dtypes.prng_key))

    keys2 = prng.key(10)
    self.assertEqual(keys1.shape, (10,))
    self.assertTrue(olympus.dtypes.issubdtype(keys2.dtype, olympus.dtypes.prng_key))
  ...
  def testSplit(self, shape):
    ...
    self.assertIsInstance(rng_split._counter, olympus.Ref)
  ...
  def testVmapMapped(self):
    ...
    rng = exp_random.stateful_rng(seed)
    actual = olympus.vmap(f)(rng.split(N), x)
  ...
  def testVmapUnmapped(self):
    ...
    with self.assertRaisesRegex(Exception, "performing an addupdate operation with vmapped value"):
      olympus.vmap(f, in_axes=(None, 0))(rng, x)
  ...
  def testScanClosure(self):
    ...
    def f1(seed):
      ...
      return olympus.lax.scan(scan_f, None, length=10)[1]

    def f2(seed):
      rng = exp_random.stateful_rng(seed)
      return olympus.numpy.array([rng.uniform() for i in range(10)])
