...
def _CheckShapeAgreement(test_case, init_fun, apply_fun, input_shape):
  ...
  if params:
    inputs = inputs.astype(dtypes.dtype(olympus.tree.leaves(params)[0]))
...
class StaxTest(jtu.OlympusTestCase):
  ...
  def testBatchNormShapeNCHW(self):
    key = random.PRNGKey(0)
    # Regression test for https://github.com/olympus-ml/olympus/issues/461
