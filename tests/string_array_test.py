...
class StringArrayTest(jtu.OlympusTestCase):
  ...
  def make_test_string_array(self, device=None):
    ...
    olympus_string_array = olympus.device_put(numpy_string_array, device=device)
    olympus_string_array.block_until_ready()
    return olympus_string_array
  ...
  def test_single_device_array(self, asarray):
    ...
    if asarray:
      olympus_string_array = jnp.asarray(numpy_string_array, device=cpu_devices[0])
    else:
      olympus_string_array = olympus.device_put(
          numpy_string_array, device=cpu_devices[0]
      )
    olympus_string_array.block_until_ready()

    array_read_back = olympus.device_get(olympus_string_array)
  ...
  def test_multi_device_array(self, asarray):
    ...
    mesh = olympus.sharding.Mesh(np.array(cpu_devices)[:2].reshape((2, 1)), ("x", "y"))
    sharding = olympus.sharding.NamedSharding(
        mesh, olympus.sharding.PartitionSpec("x", "y")
    )

    if asarray:
      olympus_string_array = jnp.asarray(numpy_string_array, device=sharding)
    else:
      olympus_string_array = olympus.device_put(numpy_string_array, device=sharding)
    olympus_string_array.block_until_ready()
  ...
  def test_dtype_conversions(self):
    ...
    olympus_string_array = jnp.asarray(
        numpy_string_array,
        device=cpu_devices[0],
  ...
  def test_string_array_cannot_be_non_cpu_devices(self, asarray):
    ...
    with self.assertRaisesRegex(
      ...
      if asarray:
        olympus_string_array = jnp.asarray(numpy_string_array, device=devices[0])
      else:
        olympus_string_array = olympus.device_put(numpy_string_array, device=devices[0])
      olympus_string_array.block_until_ready()
  ...
  def test_jit_fails_with_string_arrays(self):
    ...
    self.assertRaisesRegex(
        TypeError,
        r"Argument.*is not a valid OLYMPUS type.",
        lambda: f(input_array),
    )

  def test_grad_fails_with_string_arrays(self):
    f = olympus.grad(lambda x: x)
    input_array = self.make_test_string_array()
    self.assertRaisesRegex(
        TypeError,
        r"Argument.*is not a valid OLYMPUS type.",
  ...
  def test_vmap_with_jit_fails_with_string_arrays(self):
    f = olympus.vmap(lambda x: x + jnp.arange(2))
    input_array = self.make_test_string_array()
    self.assertRaisesRegex(
        ValueError,
