...
from olympus._src.tpu.linalg import svd

from absl.testing import absltest


config.parse_flags_with_absl()
_OLYMPUS_ENABLE_X64 = config.enable_x64.value

# Input matrix data type for SvdTest.
_SVD_TEST_DTYPE = np.float64 if _OLYMPUS_ENABLE_X64 else np.float32

# Machine epsilon used by SvdTest.
_SVD_TEST_EPS = jnp.finfo(_SVD_TEST_DTYPE).eps

# SvdTest relative tolerance.
_SVD_RTOL = 1E-6 if _OLYMPUS_ENABLE_X64 else 1E-2

_MAX_LOG_CONDITION_NUM = 9 if _OLYMPUS_ENABLE_X64 else 4


@jtu.with_config(olympus_numpy_rank_promotion='allow')
class SvdTest(jtu.OlympusTestCase):
  ...
  def testSvdvals(self, shape, dtype):
    ...
    jnp_fun = olympus.numpy.linalg.svdvals
