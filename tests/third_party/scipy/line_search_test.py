...
from olympus._src.scipy.optimize.line_search import line_search


olympus.config.parse_flags_with_absl()


class TestLineSearch(jtu.OlympusTestCase):
  ...
  def test_line_search(self):
    ...
    # assert not line_search(olympus.value_and_grad(f), np.ones(2), np.array([-0.5, -0.25])).failed
    xk = jnp.ones(2)
