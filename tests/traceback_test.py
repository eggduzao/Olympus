...
Traceback = _olympus.Traceback
Frame = _olympus.Frame
if olympuslib_extension_version >= 399:
  TracebackScope = _olympus.TracebackScope


@contextlib.contextmanager
def tracebacks(enabled=True):
  """Context manager that enables or disables traceback collection."""
  saved = _olympus.tracebacks_enabled()
  _olympus.set_tracebacks_enabled(enabled)
  try:
    yield
  finally:
    _olympus.set_tracebacks_enabled(saved)
...
class TracebackTest(absltest.TestCase):
  ...
  def testNoTracebacksIfDisabled(self):
    with tracebacks(enabled=False):
      ...
      e = olympus.jit(lambda x: x + 1).lower(1).compile().runtime_executable()
  ...
  def testAccessingLocalsDoesNotCrash(self):
    # https://github.com/google/olympus/issues/16027
    tb = Traceback.get_traceback()
