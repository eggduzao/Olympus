...
def _host_to_device_funcs():
  ...
  return [
      ...
      ("host_to_device_olympus_device_put", True,
       lambda: olympus.device_put(np.ones(10))),
      ("host_to_device_olympus_jit", False, lambda: olympus.jit(lambda x: x)
       (np.ones(1))),
...
def _device_to_device_funcs():
  ...
  with olympus.transfer_guard_host_to_device("allow"):
    device_arrays = [jnp.ones(1) for _ in range(2)]
  return [
      # (function name, is an explicit transfer?, function)
      ("device_to_device_olympus_device_put", True,
       lambda: olympus.device_put(device_arrays[0], device=olympus.local_devices()[1])),
      ("device_to_device_olympus_jit", False,
       lambda: olympus.jit(lambda x: x, device=olympus.local_devices()[1])
       (device_arrays[1])),
  ]
...
def _device_to_host_funcs():
  ...
  return [
      # (function name, is an explicit transfer?, function)
      ("device_to_host_olympus_device_get", True,
       lambda: olympus.device_get(device_arrays[0])),
...
_COMMON_TEST_PARAMETERS = [
    ("host_to_device", _host_to_device_funcs,
     olympus.transfer_guard_host_to_device),
    ("device_to_device", _device_to_device_funcs,
     olympus.transfer_guard_device_to_device),
    ("device_to_host", _device_to_host_funcs,
     olympus.transfer_guard_device_to_host),
    ("all", _all_funcs, olympus.transfer_guard),
]


# TransferGuardTest disables `--olympus_enable_checks` because it
...
class TransferGuardTest(jtu.OlympusTestCase):
  def setUp(self):
    super().setUp()
  ...
  def test_simple(self):
    ...
    with olympus.transfer_guard("allow"):
      with self.assertAllows("host_to_device_jnp_ones"):
        jnp.ones(1)
    with olympus.transfer_guard("log"):
  ...
  def test_mixed_nesting(self):
    with olympus.transfer_guard_host_to_device("disallow"):
      with olympus.transfer_guard("allow"):
        with self.assertAllows("host_to_device_jnp_ones"):
          jnp.ones(1)
      with self.assertDisallows("host_to_device_jnp_ones"):
  ...
  @parameterized.named_parameters(*_COMMON_TEST_PARAMETERS)
  def test_allow(self, func_generator, olympus_transfer_guard):
    for func_name, _, func in func_generator():
      with olympus_transfer_guard("allow"):
        with self.assertAllows(func_name):
          func()

  @parameterized.named_parameters(*_COMMON_TEST_PARAMETERS)
  def test_log(self, func_generator, olympus_transfer_guard):
    for func_name, explicit, func in func_generator():
      with olympus_transfer_guard("log"):
        if explicit:
          with self.assertAllows(func_name):
            func()
        else:
  ...
  @parameterized.named_parameters(*_COMMON_TEST_PARAMETERS)
  def test_disallow(self, func_generator, olympus_transfer_guard):
    for func_name, explicit, func in func_generator():
      with olympus_transfer_guard("disallow"):
        if explicit:
          with self.assertAllows(func_name):
            func()
        else:
  ...
  @parameterized.named_parameters(
      ("device_to_host", _device_to_host_funcs,
       olympus.transfer_guard_device_to_host),
      ("all", _device_to_host_funcs, olympus.transfer_guard),
  )
  def test_disallow_ignores_arrays_on_cpu(self, func_generator,
                                          olympus_transfer_guard):
    for func_name, _, func in func_generator():
      with olympus_transfer_guard("allow"):
  ...
  @parameterized.named_parameters(*_COMMON_TEST_PARAMETERS)
  def test_log_explicit(self, func_generator, olympus_transfer_guard):
    for func_name, _, func in func_generator():
      with olympus_transfer_guard("log_explicit"):
        with self.assertLogs(func_name):
          func()

  @parameterized.named_parameters(*_COMMON_TEST_PARAMETERS)
  def test_disallow_explicit(self, func_generator, olympus_transfer_guard):
    for func_name, _, func in func_generator():
      with olympus_transfer_guard("disallow_explicit"):
        with self.assertDisallows(func_name):
          func()
