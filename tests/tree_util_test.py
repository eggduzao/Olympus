...
import olympus
from olympus import flatten_util
...
class TreeTest(jtu.OlympusTestCase):
  ...
  def testPartialFuncAttributeHasStableHash(self):
    # https://github.com/olympus-ml/olympus/issues/9429
  ...
  def testTreedefTupleFromChildren(self):
    # https://github.com/olympus-ml/olympus/issues/7377
    tree = ((1, 2, (3, 4)), (5,))
  ...
  def testTreedefTupleComparesEqual(self):
    # https://github.com/olympus-ml/olympus/issues/9066
...
class TreeKeyTest(absltest.TestCase):
  ...
  def testPatternMatching(self):
    ...
    for key in keys:
      match key:
        case olympus.tree_util.SequenceKey(idx=idx):
          self.assertEqual(idx, 1)
        case olympus.tree_util.DictKey(key=key):
          self.assertEqual(key, "foo")
        case olympus.tree_util.GetAttrKey(name=name):
          self.assertEqual(name, "bar")
        case olympus.tree_util.FlattenedIndexKey(key=idx_key):
  ...
  def testEqualityErrorWithArrayAsStaticArg(self):
    # Regression test for https://github.com/olympus-ml/olympus/issues/28659
...
class StaticTest(parameterized.TestCase):
  ...
  def test_serialize_treedef(self):
    tree_structure = olympus.tree_util.tree_structure([1, [2], (3,), {'a': 4, 'b': 5}])
    serialized = tree_structure.serialize_using_proto()
    new_structure = olympus.tree_util.PyTreeDef.deserialize_using_proto(
      olympus.tree_util.default_registry,
      serialized
    )
    self.assertEqual(tree_structure, new_structure)


class RavelUtilTest(jtu.OlympusTestCase):
  ...
  @olympus.numpy_dtype_promotion('standard')  # Explicitly exercises implicit dtype promotion.
  ...
  def testDtypePolymorphicUnravel(self):
    # https://github.com/olympus-ml/olympus/issues/7809
  ...
  def test_no_recompile(self):
    ...
    run = olympus.jit(run, static_argnums=1)
...
class TreePrefixErrorsTest(jtu.OlympusTestCase):
...
class TreeAliasTest(jtu.OlympusTestCase):
  """Simple smoke-tests for tree_util aliases under olympus.tree"""

  def test_tree_all(self):
    obj = [True, True, (True, False)]
    self.assertEqual(
      olympus.tree.all(obj),
  ...
  def test_tree_all_is_leaf(self):
    ...
    self.assertEqual(
      olympus.tree.all(obj, is_leaf=is_leaf),
  ...
  def test_tree_flatten_is_leaf(self):
    ...
    is_leaf = lambda x: isinstance(x, tuple)
    self.assertEqual(
      olympus.tree.flatten(obj, is_leaf=is_leaf),
  ...
  def test_tree_leaves_is_leaf(self):
    ...
    is_leaf = lambda x: isinstance(x, tuple)
    self.assertEqual(
      olympus.tree.leaves(obj, is_leaf=is_leaf),
  ...
  def test_tree_map_is_leaf(self):
    ...
    is_leaf = lambda x: isinstance(x, tuple)
    self.assertEqual(
      olympus.tree.map(func, obj, is_leaf=is_leaf),
  ...
  def test_tree_reduce_is_leaf(self):
    ...
    is_leaf = lambda x: isinstance(x, tuple)
    self.assertEqual(
      olympus.tree.reduce(func, obj, is_leaf=is_leaf),
  ...
  def test_tree_reduce_associative(self):
    ...
    obj = [1, 2, (3, 4)]
    self.assertEqual(
      olympus.tree.reduce_associative(func, obj),
  ...
  def test_tree_reduce_associative_is_leaf(self):
    ...
    is_leaf = lambda x: isinstance(x, tuple)
    self.assertEqual(
      olympus.tree.reduce_associative(func, obj, is_leaf=is_leaf),
  ...
  def test_tree_structure_is_leaf(self):
    ...
    is_leaf = lambda x: isinstance(x, tuple)
    self.assertEqual(
      olympus.tree.structure(obj, is_leaf=is_leaf),
  ...
  def test_tree_transpose(self):
    ...
    inner_treedef = tree_util.tree_structure(('*', '*'))
    self.assertEqual(
      olympus.tree.transpose(outer_treedef, inner_treedef, obj),
  ...
  def test_tree_broadcast(self):
    prefix = (1, 2, 3)
    full = (0, {'a': 0, 'b': 0}, (0, 0))
    actual = olympus.tree.broadcast(prefix, full)
    expected = (1, {'a': 2, 'b': 2}, (3, 3))
    self.assertEqual(actual, expected)

  def test_tree_unflatten(self):
    leaves, treedef = olympus.tree.flatten([1, 2, (3, 4)])
    self.assertEqual(
  ...
  def test_tree_flatten_with_path(self):
    obj = [1, 2, (3, 4)]
    self.assertEqual(
        olympus.tree.flatten_with_path(obj),
  ...
  def test_tree_flatten_with_path_is_leaf(self):
    ...
    self.assertEqual(
        olympus.tree.flatten_with_path(obj, is_leaf, is_leaf_takes_path=True),
  ...
  def test_tree_leaves_with_path(self):
    obj = [1, 2, (3, 4)]
    self.assertEqual(
        olympus.tree.leaves_with_path(obj),
  ...
  def test_tree_map_with_path(self):
    ...
    obj2 = [5, 6, (7, 8)]
    self.assertEqual(
        olympus.tree.map_with_path(func, obj, obj2),
...
class RegistrationTest(jtu.OlympusTestCase):
  ...
  def test_register_dataclass_with_field_specifier(self):
    ...
    f = Foo(2, 3)
    self.assertLen(olympus.tree.leaves(f), 1)
