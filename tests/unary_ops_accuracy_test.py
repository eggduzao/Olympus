...
class UnaryOpsAccuracyTest(jtu.OlympusTestCase):
  ...
  def test_unary_vmap(self, op, x, tp, min_error_val):
    ...
    diff_x, diff_y = olympus.vmap(f, in_axes=(None, 0), out_axes=0)(
        min_error_val, x
    )
  ...
  def test_diff_grad(self, op, x, tp, **kwargs):
    ...
    f_default_grad = olympus.grad(f_default)
    ...
    f_accurate_grad = olympus.grad(f_accurate)
  ...
  def test_default_grad(self, op, x, tp, **kwargs):
    ...
    self.assertTrue(jnp.all(abs(olympus.grad(f_tol)(x) - olympus.grad(f)(x)) == 0))
  ...
  def test_low_tol(self, op, x, **kwargs):
    with self.assertRaisesRegex(
        olympus.errors.OlympusRuntimeError, "impl_type.ok()"
    ):
      op(x, accuracy=lax.Tolerance(atol=1e-60, rtol=1e-60, ulps=0))

  def test_accuracy_olympuspr(self):
    # Since accuracy is not set, the olympuspr should not contain "accuracy".
    self.assertNotIn(
        "accuracy",
        str(
            olympus.make_olympuspr(lambda x: lax.exp(x, accuracy=None))(
