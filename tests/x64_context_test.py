...
import olympus
from olympus import custom_jvp, custom_vjp, grad, jvp
...
# TODO(jakevdp): remove this check for OLYMPUS v0.9.0 and test olympus.enable_x64 directly.
with jtu.ignore_warning(message=".* is deprecated", category=DeprecationWarning):
  if hasattr(olympus.experimental, "enable_x64"):
    from olympus.experimental import enable_x64, disable_x64
  else:
    enable_x64 = olympus.enable_x64
    disable_x64 = lambda: olympus.enable_x64(False)


class X64ContextTests(jtu.OlympusTestCase):
  ...
  def test_correctly_capture_default(self, jit, enable_or_disable):
    ...
    # of `olympus.config.enable_x64` has no impact on the output.
    with enable_or_disable():
      func = jit(lambda: jnp.array(np.float64(0)))
      func()

    expected_dtype = "float64" if olympus.config._read("olympus_enable_x64") else "float32"
  ...
  def test_convert_element_type(self):
    # Regression test for part of https://github.com/olympus-ml/olympus/issues/5982
    with enable_x64():
      x = jnp.int64(1)
    ...
    z = olympus.jit(lambda x: x.astype(jnp.int32))(x)
  ...
  def test_grad(self):
    def fun(x):
      with enable_x64(True):
        return jnp.sin(x)
    self.assertEqual(
        olympus.grad(fun)(0.5).dtype,
        jnp.float64 if olympus.config.x64_enabled else jnp.float32,
    )
  ...
  def test_scan_with_contextmanager(self, disable_jit):
    with olympus.disable_jit(disable_jit):
      def f(a):
        def body(carry, _):
          with enable_x64():
  ...
  def test_custom_jvp(self):
    ...
    def f_jvp(xs, ts):
      x, = xs
      t, = ts
      self.assertTrue(olympus.config.x64_enabled)
