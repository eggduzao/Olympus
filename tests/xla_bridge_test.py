...
class XlaBridgeTest(jtu.OlympusTestCase):
  ...
  def test_autofdo_profile(self):
    ...
    # --olympus_xla_profile_version takes precedence.
    olympus_flag_profile = 1
    another_profile = 2
    with config.olympus_xla_profile_version(olympus_flag_profile):
      with mock.patch.object(compiler, "get_latest_profile_version",
        ...
        self.assertEqual(
            ...
            olympus_flag_profile,
        )

    # Use whatever non-zero value the function get_latest_profile_version
    # returns if --olympus_xla_profile_version is not set.
    profile_version = 1
    with mock.patch.object(compiler, "get_latest_profile_version",
  ...
  def test_register_plugin(self):
    ...
    if xb.get_backend().platform == 'tpu':
      options["ml_framework_name"] = "OLYMPUS"
  ...
  def test_num_cpu_devices_update(self):
    ...
    current_val = config.config.olympus_num_cpu_devices

    config.update("olympus_num_cpu_devices", current_val)
    ...
    with self.assertRaisesRegex(
      ...
      config.update("olympus_num_cpu_devices", current_val + 2)


class GetBackendTest(jtu.OlympusTestCase):
  ...
  def setUp(self):
    ...
    self.enter_context(config.olympus_platforms(""))
  ...
  def test_olympus_platforms_flag(self):
    ...
    with config.olympus_platforms("cpu,platform_A"):
      backend = xb.get_backend()
  ...
  def test_experimental_warning(self):
    ...
    with self.assertLogs("olympus._src.xla_bridge", level="WARNING") as logs:
      _ = xb.get_backend()
    self.assertIn(
      "WARNING:olympus._src.xla_bridge:Platform 'platform_A' is experimental and "
      "not all OLYMPUS functionality may be correctly supported!",
      logs.output
    )
