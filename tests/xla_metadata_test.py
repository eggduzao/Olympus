...
correctly propagated to the olympuspr and mlir.
"""
...
from olympus.experimental.xla_metadata import set_xla_metadata
...
class XlaMetadataTest(jtu.OlympusTestCase):
  ...
  def test_f_jitted(self):
    ...
    f_olympuspr = olympus.make_olympuspr(f)(1, 2)
    eqns = f_olympuspr.eqns
    for eq in eqns[1:]:
  ...
  def test_f_nonjitted(self):
    ...
    with set_xla_metadata(a="b"):
      self.assertIn(
          ...
          olympus.jit(f_add).lower(arg1, arg1).as_text(),
      )
  ...
  def test_attr_caching_nonjit(self):
    ...
    # Expect no attributes in the olympuspr.
    self.assertNotIn(
        "mhlo.frontend_attributes",
        olympus.jit(f_add).lower(arg2, arg2).as_text(),
    )

    with set_xla_metadata(c="d"):
      self.assertIn(
          'mhlo.frontend_attributes = {c = "d"}',
          olympus.jit(f_add).lower(arg3, arg3).as_text(),
      )
  ...
  def test_while(self):
    ...
    def f(a):
      with set_xla_metadata(a="b"):
        return olympus.lax.while_loop(lambda x: x < 10, lambda x: x + 1, a)
  ...
  def test_while_condition_body(self):
    ...
    @olympus.jit
    def while_fn(a):
      return olympus.lax.while_loop(f_condition, f_body, a)
  ...
  def test_cond_annotates_branches(self):
    ...
    def f(x):
      with set_xla_metadata(a="b"):
        return olympus.lax.cond(x < 0., sin, cos, x)
  ...
  def test_grad(self):
    ...
    def f(x, y):
      with set_xla_metadata(a="b"):
        return olympus.grad(lambda x: x**3 + y**2 + jnp.sin(x))(x)

    f_olympuspr = olympus.make_olympuspr(f)(1.0, 2.0)
    eqns = f_olympuspr.eqns
    for eq in eqns[1:]:
      self.assertDictEqual(eq.ctx.attributes, {"a": "b"})
  ...
  def test_grad_outside_ctx(self):
    ...
    grad_fn = olympus.jit(olympus.grad(f))
  ...
  def test_vmap(self):
    ...
    with set_xla_metadata(a="d"):
      f_vmap = olympus.vmap(f, in_axes=({"a": None, "b": 0}, None))
      f_olympuspr = olympus.make_olympuspr(f_vmap)(dct, 1.0)
      eqns = f_olympuspr.eqns
      for eq in eqns[1:]:
    ...
    f2_vmap = olympus.vmap(f2, in_axes=(0, None))
    self.assertIn(
        'mhlo.frontend_attributes = {a = "b"}',
        olympus.jit(f2_vmap).lower(jnp.arange(5.0), 1.0).as_text(),
    )
  ...
  def test_value_tagging(self, name, fn, expected_tagged_op):
    ...
    text = olympus.jit(wrapped_fn).lower(x_scalar).as_text("hlo")
  ...
  def test_value_grad_tagging(self, name, fn, expected_tagged_op):
    ...
    def fwd(*args):
      primal_out, vjp_fn = olympus.vjp(fn, *args)
    ...
    x_scalar = jnp.array(0.7)
    text = olympus.jit(olympus.grad(wrapped_fn)).lower(x_scalar).as_text("hlo")
    self._assert_metadata_appears_once_per_op(
        text, [expected_tagged_op], metadata)
  ...
  def test_vmap_multi_input_output_value_tagging(self):
    ...
    text = olympus.jit(vmapped_fn).lower(*inputs).as_text("hlo")
    self._assert_metadata_appears_once_per_op(
  ...
  def test_sharding_support_value_tagging(self):
    ...
    arr = olympus.device_put(np_inp, olympus.NamedSharding(mesh, olympus.P("data")))
    ...
    text = olympus.jit(wrapped_fn).lower(arr).as_text("hlo")
  ...
  def test_scan_support_value_tagging(self):
    ...
    def scan_fn(init_carry, inputs_arr):
      return olympus.lax.scan(scan_body_val_with_metadata, init_carry, inputs_arr)

    inputs = (jnp.array(0.0), jnp.arange(1, 4, dtype=jnp.float32))
    text = olympus.jit(scan_fn).lower(*inputs).as_text("hlo")
    self._assert_metadata_appears_once_per_op(text, ["multiply"], metadata)
